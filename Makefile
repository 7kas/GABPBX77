...
export GABPBXVERSION
export GABPBXVERSIONNUM
...
# Trying to run gabpbx from the DESTDIR is completely unsupported
# behavior.
...
# GABpbx.conf is located in ASTETCDIR or by using the -C flag
# when starting GABpbx
ASTCONFPATH=$(ASTETCDIR)/gabpbx.conf
...
# If the file .gabpbx.makeopts is present in your home directory, you can
# include all of your favorite menuselect options so that every time you download
# a new version of GABpbx, you don't have to run menuselect to set them.
# The file /etc/gabpbx.makeopts will also be included but can be overridden
# by the file in your home directory.

ifeq ($(wildcard menuselect.makeopts),)
	USER_MAKEOPTS=$(wildcard ~/.gabpbx.makeopts)
	GLOBAL_MAKEOPTS=$(wildcard /etc/gabpbx.makeopts)
else
	USER_MAKEOPTS=
	GLOBAL_MAKEOPTS=
endif
...
ifeq ($(GREP),)
  ...
  GABPBXVERSION:=$(shell GREP=$(GREP) AWK=$(AWK) GIT=$(GIT) build_tools/make_version .)
endif
ifneq ($(AWK),)
  ifneq ($(wildcard .version),)
    GABPBXVERSIONNUM:=$(shell $(SED) -e 's/^certified\///' -e 's/-cert/./' .version | $(AWK) -F. '{printf "%01d%02d%02d", $$1, $$2, $$3}')
  endif
endif

ifneq ($(wildcard .svn),)
  GABPBXVERSIONNUM:=999999
endif
...
all: _all
	@echo " +--------- GABpbx Build Complete ---------+"
	@echo " + GABpbx has successfully been built, and +"
...
$(SUBDIRS): makeopts .lastclean main/version.c include/gabpbx/build.h defaults.h

ifeq ($(findstring $(OSARCH), mingw32 cygwin ),)
main: third-party
else
    # Windows: we need to build main (i.e. the gabpbx dll) first,
...
main/version.c: FORCE include/gabpbx/buildopts.h menuselect.makeopts .lastclean
	@build_tools/make_version_c > $@.tmp
	@cmp -s $@.tmp $@ || mv $@.tmp $@
	@rm -f $@.tmp

include/gabpbx/buildopts.h: menuselect.makeopts .lastclean
...
# been created.
include/gabpbx/build.h: .lastclean
...
distclean: $(SUBDIRS_DIST_CLEAN) _clean
	...
	rm -f include/gabpbx/autoconfig.h
	rm -f include/gabpbx/buildopts.h
	rm -rf doc/api
	rm -f doc/Doxyfile
...
ifneq ($(DISABLE_XMLDOC),yes)
	...
	if [ -d doc/tex/gabpbx ] ; then \
		$(INSTALL) -d "$(DESTDIR)$(ASTDATADIR)/static-http/docs" ; \
		for n in doc/tex/gabpbx/* ; do \
...
# core-en_US.xml is the normal documentation created with gabpbx builds.
...
NEWHEADERS=$(notdir $(wildcard include/gabpbx/*.h))
...
ifneq ($(HAVE_SBIN_LAUNCHD),1)
	./build_tools/install_subst contrib/scripts/safe_gabpbx "$(DESTDIR)$(ASTSBINDIR)/safe_gabpbx";
endif
...
ifneq ($(DISABLE_XMLDOC),yes)
	...
	$(INSTALL) -m 644 doc/gabpbx.8 "$(DESTDIR)$(ASTMANDIR)/man8"
	$(INSTALL) -m 644 doc/astdb*.8 "$(DESTDIR)$(ASTMANDIR)/man8"
	$(INSTALL) -m 644 contrib/scripts/astgenkey.8 "$(DESTDIR)$(ASTMANDIR)/man8"
	$(INSTALL) -m 644 contrib/scripts/autosupport.8 "$(DESTDIR)$(ASTMANDIR)/man8"
	$(INSTALL) -m 644 contrib/scripts/safe_gabpbx.8 "$(DESTDIR)$(ASTMANDIR)/man8"
	if [ -f contrib/firmware/iax/iaxy.bin ] ; then \
		$(INSTALL) -m 644 contrib/firmware/iax/iaxy.bin "$(DESTDIR)$(ASTDATADIR)/firmware/iax/iaxy.bin"; \
	fi
ifeq ($(HAVE_DAHDI),1)
	$(INSTALL) -d $(DESTDIR)/$(DAHDI_UDEV_HOOK_DIR)
	$(INSTALL) -m 644 contrib/scripts/dahdi_span_config_hook $(DESTDIR)$(DAHDI_UDEV_HOOK_DIR)/40-gabpbx
endif
...
oldmodcheck:
	@if [ -n "$(BADMODS)" ]; then \
		...
		echo "" ;\
		echo " Your GABpbx modules directory, located at" ;\
		echo " $(DESTDIR)$(ASTMODDIR)" ;\
		echo " contains modules that were not installed by this " ;\
		echo " version of GABpbx. Please ensure that these" ;\
		echo " modules are compatible with this version before" ;\
		echo " attempting to run GABpbx." ;\
		echo "" ;\
		for f in $(BADMODS); do \
			echo "    $$f" ;\
		done ;\
...
ifeq ($(DESTDIR),)  # DESTDIR means binary archive creation; ldconfig should be run on postinst
	@if [ $$(id -u) -eq 0 ] ; then \
		...
		echo "  * Run gabpbx with 'LD_LIBRARY_PATH=$(ASTLIBDIR) gabpbx' " ;\
		echo "" ;\
		echo " WARNING WARNING WARNING" ;\
	fi
endif
...
check-old-libdir:
	...
	oldfiles=`find "$(_oldlibdir)" -name libgabpbx* -print -quit -o \( -path *gabpbx/modules/* -a -name *.so \) -print -quit 2>/dev/null` ;\
	if [ "x$$oldfiles" != "x" ] ; then \
		echo " WARNING WARNING WARNING" ;\
		echo "" ;\
		echo " Installation is to: " ;\
		echo "   $(DESTDIR)$(ASTLIBDIR)" ;\
		echo " but there are gabpbx shared libraries in: " ;\
		echo "   $(_oldlibdir)" ;\
		echo " or" ;\
		echo "   $(_oldlibdir)/gabpbx/modules" ;\
		echo "" ;\
		echo " It is unlikely that gabpbx will start." ;\
		echo "" ;\
...
versioncheck:
ifeq ($(GABPBXVERSION),UNKNOWN__git_check_fail)
	@echo "GABpbx Version is unknown due to a git error. If you are running make"
...
install: badshell versioncheck bininstall datafiles
	@if [ -x /usr/sbin/gabpbx-post-install ]; then \
		/usr/sbin/gabpbx-post-install "$(DESTDIR)" . ; \
	fi
	@echo " +---- GABpbx Installation Complete -------+"
	@echo " +                                           +"
	@echo " +    YOU MUST READ THE SECURITY DOCUMENT    +"
	@echo " +                                           +"
	@echo " + GABpbx has successfully been installed. +"
	...
	@echo " +-----------------  or ---------------------+"
	@echo " +                                           +"
	@echo " + You can go ahead and install the gabpbx +"
...
# Will create all components of "$(DESTDIR)$(ASTHEADERDIR)/doxygen" including "$(DESTDIR)$(includedir)"
	...
	$(INSTALL) -m 644 include/gabpbx.h "$(DESTDIR)$(includedir)"
	$(INSTALL) -m 644 include/gabpbx/*.h "$(DESTDIR)$(ASTHEADERDIR)"
	$(INSTALL) -m 644 include/gabpbx/doxygen/*.h "$(DESTDIR)$(ASTHEADERDIR)/doxygen"
...
webvmail:
	...
	$(INSTALL) -d "$(DESTDIR)$(HTTP_DOCSDIR)/_gabpbx"
	for x in images/*.gif; do \
		$(INSTALL) -m 644 $$x "$(DESTDIR)$(HTTP_DOCSDIR)/_gabpbx/"; \
	done
	@echo " +--------- GABpbx Web Voicemail ----------+"
	@echo " +                                           +"
	@echo " + GABpbx Web Voicemail is installed in    +"
...
ifeq ($(GABPBXVERSION),UNKNOWN__and_probably_unsupported)
	@echo "GABpbx Version is unknown, not configuring Doxygen PROJECT_NUMBER."
else ifeq ($(GABPBXVERSION),UNKNOWN__git_check_fail)
	@echo "GABpbx Version is unknown due to a git error. If you are running make"
	@echo "as a different user than the project owner, this can be resolved by"
	@echo "running the following command as the user currently executing make: "$$USER
	@echo "git config --global --add safe.directory "$(mkfile_dir:/=)
	...
	@echo "PROJECT_NUMBER = $(GABPBXVERSION)" >> doc/Doxyfile
endif
...
install-logrotate:
	...
	sed 's#__LOGDIR__#$(ASTLOGDIR)#g' < contrib/scripts/gabpbx.logrotate | sed 's#__SBINDIR__#$(ASTSBINDIR)#g' > contrib/scripts/gabpbx.logrotate.tmp
	$(INSTALL) -m 0644 contrib/scripts/gabpbx.logrotate.tmp "$(DESTDIR)$(ASTETCDIR)/../logrotate.d/gabpbx"
	rm -f contrib/scripts/gabpbx.logrotate.tmp

config:
	@if [ -f /etc/redhat-release -o -f /etc/fedora-release ]; then \
		./build_tools/install_subst contrib/init.d/rc.redhat.gabpbx  "$(DESTDIR)/etc/rc.d/init.d/gabpbx"; \
		if [ ! -f "$(DESTDIR)/etc/sysconfig/gabpbx" ] ; then \
			$(INSTALL) -m 644 contrib/init.d/etc_default_gabpbx "$(DESTDIR)/etc/sysconfig/gabpbx" ; \
		fi ; \
		if [ -z "$(DESTDIR)" ] ; then \
			/sbin/chkconfig --add gabpbx ; \
		fi ; \
	elif [ -f /etc/debian_version ] ; then \
		./build_tools/install_subst contrib/init.d/rc.debian.gabpbx  "$(DESTDIR)/etc/init.d/gabpbx"; \
		if [ ! -f "$(DESTDIR)/etc/default/gabpbx" ] ; then \
			$(INSTALL) -m 644 contrib/init.d/etc_default_gabpbx "$(DESTDIR)/etc/default/gabpbx" ; \
		fi ; \
		if [ -z "$(DESTDIR)" ] ; then \
			/usr/sbin/update-rc.d gabpbx defaults 50 91 ; \
		fi ; \
	elif [ -f /etc/gentoo-release ] ; then \
		./build_tools/install_subst contrib/init.d/rc.gentoo.gabpbx  "$(DESTDIR)/etc/init.d/gabpbx"; \
		if [ -z "$(DESTDIR)" ] ; then \
			/sbin/rc-update add gabpbx default ; \
		fi ; \
	elif [ -f /etc/mandrake-release -o -f /etc/mandriva-release ] ; then \
		./build_tools/install_subst contrib/init.d/rc.mandriva.gabpbx  "$(DESTDIR)/etc/rc.d/init.d/gabpbx"; \
		if [ ! -f /etc/sysconfig/gabpbx ] ; then \
			$(INSTALL) -m 644 contrib/init.d/etc_default_gabpbx "$(DESTDIR)/etc/sysconfig/gabpbx" ; \
		fi ; \
		if [ -z "$(DESTDIR)" ] ; then \
			/sbin/chkconfig --add gabpbx ; \
		...
		./build_tools/install_subst contrib/init.d/rc.suse.gabpbx  "$(DESTDIR)/etc/init.d/gabpbx"; \
		if [ ! -f /etc/sysconfig/gabpbx ] ; then \
			$(INSTALL) -m 644 contrib/init.d/etc_default_gabpbx "$(DESTDIR)/etc/sysconfig/gabpbx" ; \
		fi ; \
		if [ -z "$(DESTDIR)" ] ; then \
		...
		./build_tools/install_subst contrib/init.d/rc.archlinux.gabpbx  "$(DESTDIR)/etc/init.d/gabpbx"; \
	elif [ -f /etc/slackware-version ]; then \
		./build_tools/install_subst contrib/init.d/rc.slackware.gabpbx  "$(DESTDIR)/etc/rc.d/rc.gabpbx"; \
	elif [ -f /etc/os-release ] && [ "slackware" = "$(shell . /etc/os-release 2>/dev/null && echo $$ID)" ] ; then \
		./build_tools/install_subst contrib/init.d/rc.slackware.gabpbx  "$(DESTDIR)/etc/rc.d/rc.gabpbx"; \
	elif [ -d "$(DESTDIR)/Library/LaunchDaemons" ]; then \
		if [ ! -f "$(DESTDIR)/Library/LaunchDaemons/org.gabpbx.gabpbx.plist" ]; then \
			./build_tools/install_subst contrib/init.d/org.gabpbx.gabpbx.plist "$(DESTDIR)/Library/LaunchDaemons/org.gabpbx.gabpbx.plist"; \
		fi; \
		if [ ! -f "$(DESTDIR)/Library/LaunchDaemons/org.gabpbx.muted.plist" ]; then \
			./build_tools/install_subst contrib/init.d/org.gabpbx.muted.plist "$(DESTDIR)/Library/LaunchDaemons/org.gabpbx.muted.plist"; \
		fi; \
	else \
...
uninstall-headers:
	...
	rm -f "$(DESTDIR)$(includedir)/gabpbx.h"
...
_uninstall: $(SUBDIRS_UNINSTALL) main-binuninstall
	...
	rm -f "$(DESTDIR)$(ASTMANDIR)/man8/gabpbx.8"
	rm -f "$(DESTDIR)$(ASTMANDIR)/man8/astgenkey.8"
	rm -f "$(DESTDIR)$(ASTMANDIR)/man8/autosupport.8"
	rm -f "$(DESTDIR)$(ASTMANDIR)/man8/safe_gabpbx.8"
ifeq ($(HAVE_DAHDI),1)
	rm -f $(DESTDIR)$(DAHDI_UDEV_HOOK_DIR)/40-gabpbx
endif
...
uninstall: _uninstall
	@echo " +--------- GABpbx Uninstall Complete -----+"
	@echo " + GABpbx binaries, sounds, man pages,     +"
	@echo " + headers, modules, and firmware builds,    +"
	@echo " + have all been uninstalled.                +"
	@echo " +                                           +"
	@echo " + To remove ALL traces of GABpbx,         +"
...
cmenuselect: menuselect/cmenuselect menuselect-tree menuselect.makeopts
	-@menuselect/cmenuselect menuselect.makeopts && (echo "menuselect changes saved!"; rm -f channels/h323/Makefile.ast main/gabpbx) || echo "menuselect changes NOT saved!"

gmenuselect: menuselect/gmenuselect menuselect-tree menuselect.makeopts
	-@menuselect/gmenuselect menuselect.makeopts && (echo "menuselect changes saved!"; rm -f channels/h323/Makefile.ast main/gabpbx) || echo "menuselect changes NOT saved!"

nmenuselect: menuselect/nmenuselect menuselect-tree menuselect.makeopts
	-@menuselect/nmenuselect menuselect.makeopts && (echo "menuselect changes saved!"; rm -f channels/h323/Makefile.ast main/gabpbx) || echo "menuselect changes NOT saved!"
...
menuselect-tree: $(foreach dir,$(filter-out main,$(MOD_SUBDIRS)),$(wildcard $(dir)/*.c) $(wildcard $(dir)/*.cc) $(wildcard $(dir)/*.xml)) build_tools/cflags.xml build_tools/cflags-devmode.xml sounds/sounds.xml utils/utils.xml agi/agi.xml configure makeopts
	...
	@echo >> $@
	@echo "<menu name=\"GABpbx Module and Build Option Selection\">" >> $@
