...
static int mbl_sendsms_exec(struct ast_channel *ast, const char *data)
{
	...
	if (ast_strlen_zero(args.device)) {
		ast_log_chan(NULL, LOG_ERROR,"NULL device for message -- SMS will not be sent.\n");
		return -1;
	}

	if (ast_strlen_zero(args.dest)) {
		ast_log_chan(NULL, LOG_ERROR,"NULL destination for message -- SMS will not be sent.\n");
		return -1;
	}

	if (ast_strlen_zero(args.message)) {
		ast_log_chan(NULL, LOG_ERROR,"NULL Message to be sent -- SMS will not be sent.\n");
		return -1;
	}
	...
	if (!pvt) {
		ast_log_chan(NULL, LOG_ERROR,"Bluetooth device %s wasn't found in the list -- SMS will not be sent.\n", args.device);
		goto e_return;
	}

	ast_mutex_lock(&pvt->lock);
	if (!pvt->connected) {
		ast_log_chan(NULL, LOG_ERROR,"Bluetooth device %s wasn't connected -- SMS will not be sent.\n", args.device);
		goto e_unlock_pvt;
	}

	if (!pvt->has_sms) {
		ast_log_chan(NULL, LOG_ERROR,"Bluetooth device %s doesn't handle SMS -- SMS will not be sent.\n", args.device);
		goto e_unlock_pvt;
	}
	...
	if (hfp_send_cmgs(pvt->hfp, args.dest)
		...
		ast_log_chan(NULL, LOG_ERROR, "[%s] problem sending SMS message\n", pvt->id);
		goto e_free_message;
	}
		...
		const struct ast_assigned_ids *assignedids, const struct ast_channel *requestor, const char *data, int *cause)
{
	...
	if (ast_format_cap_iscompatible_format(cap, DEVICE_FRAME_FORMAT) == AST_FORMAT_CMP_NOT_EQUAL) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Asked to get a channel of unsupported format '%s'\n", ast_format_cap_get_names(cap, &codec_buf));
	...
	if (!pvt || !pvt->connected || pvt->owner) {
		ast_log_chan(NULL, LOG_WARNING, "Request to call on device %s which is not connected / already in use.\n", dest_dev);
		*cause = AST_CAUSE_REQUESTED_CHAN_UNAVAIL;
		return NULL;
	}

	if ((pvt->type == MBL_TYPE_PHONE) && !dest_num) {
		ast_log_chan(NULL, LOG_WARNING, "Can't determine destination number.\n");
		*cause = AST_CAUSE_INCOMPATIBLE_DESTINATION;
		return NULL;
	}
...
static int mbl_call(struct ast_channel *ast, const char *dest, int timeout)
{
	...
	if (pvt->type == MBL_TYPE_PHONE) {
		dest_num = strchr(dest_dev, '/');
		if (!dest_num) {
			ast_log_chan(NULL, LOG_WARNING, "Cant determine destination number.\n");
			return -1;
		}
		*dest_num++ = 0x00;
	}
	...
	if ((ast_channel_state(ast) != AST_STATE_DOWN) && (ast_channel_state(ast) != AST_STATE_RESERVED)) {
		ast_log_chan(NULL, LOG_WARNING, "mbl_call called on %s, neither down nor reserved\n", ast_channel_name(ast));
		return -1;
	}
	...
	if (pvt->type == MBL_TYPE_PHONE) {
		if (hfp_send_atd(pvt->hfp, dest_num)) {
			ast_mutex_unlock(&pvt->lock);
			ast_log_chan(NULL, LOG_ERROR, "error sending ATD command on %s\n", pvt->id);
			return -1;
		}
		pvt->hangupcause = 0;
		pvt->needchup = 1;
		...
		if (hsp_send_ring(pvt->rfcomm_socket)) {
			ast_log_chan(NULL, LOG_ERROR, "[%s] error ringing device\n", pvt->id);
			ast_mutex_unlock(&pvt->lock);
			return -1;
		}
...
e_return:
	ast_log_chan(NULL, LOG_ERROR, "error parsing SMS prompt on rfcomm socket\n");
	return res;
}
...
static int rfcomm_read_cmgr(int rsock, char **buf, size_t count, size_t *in_count)
{
	...
	if ((res = rfcomm_read_until_ok(rsock, buf, count, in_count)) != 1) {
		ast_log_chan(NULL, LOG_ERROR, "error reading +CMGR message on rfcomm socket\n");
	}
...
e_return:
	ast_log_chan(NULL, LOG_ERROR, "error parsing AT result on rfcomm socket\n");
	return res;
}
...
static int sco_accept(int *id, int fd, short events, void *data)
{
	...
	if ((sock = accept(fd, (struct sockaddr *)&addr, &addrlen)) == -1) {
		ast_log_chan(NULL, LOG_ERROR, "error accepting audio connection on adapter %s\n", adapter->id);
		return 0;
	}
	...
	AST_RWLIST_UNLOCK(&devices);
	if (!pvt) {
		ast_log_chan(NULL, LOG_WARNING, "could not find device for incoming audio connection\n");
		close(sock);
		return 1;
	}
...
static int sco_bind(struct adapter_pvt *adapter)
{
	...
	if ((adapter->sco_socket = socket(PF_BLUETOOTH, SOCK_SEQPACKET, BTPROTO_SCO)) < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to create sco listener socket for adapter %s.\n", adapter->id);
		goto e_return;
	}
	...
	if (bind(adapter->sco_socket, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to bind sco listener socket. (%d)\n", errno);
		goto e_close_socket;
	}
	if (setsockopt(adapter->sco_socket, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt)) == -1) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to setsockopt sco listener socket.\n");
		goto e_close_socket;
	}
	if (listen(adapter->sco_socket, 5) < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to listen sco listener socket.\n");
		goto e_close_socket;
	}
...
static sdp_session_t *sdp_register(void)
{
	...
	const char *service_name = "GABpbx PABX";
	const char *service_dsc = "GABpbx PABX";
	const char *service_prov = "GABpbx";
	...
	if (!(session = sdp_connect(BDADDR_ANY, BDADDR_LOCAL, SDP_RETRY_IF_BUSY)))
		ast_log_chan(NULL, LOG_WARNING, "Failed to connect sdp and create session.\n");
	else {
		if (sdp_record_register(session, record, 0) < 0) {
			ast_log_chan(NULL, LOG_WARNING, "Failed to sdp_record_register error: %d\n", errno);
			return NULL;
		}
	}
	...
	if ((entry = msg_queue_head(pvt)) && entry->expected == AT_OK) {
		...
		case AT_CIND:
			...
			if (pvt->hfp->cind_state[pvt->hfp->cind_map.call]) {
				ast_verb_chan(NULL, 3, "Bluetooth Device %s has a call in progress - delaying connection.\n", pvt->id);
				goto e_return;
			}
		...
		case AT_ECAM:
			...
			ast_verb_chan(NULL, 3, "Bluetooth Device %s initialized and ready.\n", pvt->id);
...
static int handle_response_error(struct mbl_pvt *pvt, char *buf)
{
	...
	if ((entry = msg_queue_head(pvt))
		...
		case AT_VTS:
			...
			break;
		case AT_CUSD:
			ast_verb_chan(NULL, 0, "[%s] error sending CUSD command\n", pvt->id);
			break;
		case AT_UNKNOWN:
		default:
	...
	case HFP_CIND_CALL:
		...
		case HFP_CIND_CALL_NONE:
			...
			if (pvt->owner) {
				...
				if (mbl_queue_hangup(pvt)) {
					ast_log_chan(NULL, LOG_ERROR, "[%s] error queueing hangup, disconnecting...\n", pvt->id);
					return -1;
				}
			}
			pvt->needchup = 0;
		...
		case HFP_CIND_CALL_ACTIVE:
			if (pvt->outgoing) {
				...
				ast_verb_chan(NULL, 3, "[%s] user answered bluetooth device from handset, disconnecting\n", pvt->id);
				mbl_queue_hangup(pvt);
				return -1;
			}
			break;
	...
	case HFP_CIND_CALLSETUP:
		...
		case HFP_CIND_CALLSETUP_OUTGOING:
			if (pvt->outgoing) {
				...
				ast_verb_chan(NULL, 3, "[%s] user dialed from handset, disconnecting\n", pvt->id);
				return -1;
			}
			break;
...
static int handle_response_clip(struct mbl_pvt *pvt, char *buf)
{
	...
	if ((msg = msg_queue_head(pvt)) && msg->expected == AT_CLIP) {
		...
		if (!(chan = mbl_new(AST_STATE_RING, pvt, &cidinfo, NULL, NULL))) {
			ast_log_chan(NULL, LOG_ERROR, "[%s] unable to allocate channel for incoming call\n", pvt->id);
			hfp_send_chup(pvt->hfp);
			msg_queue_push(pvt, AT_OK, AT_CHUP);
			return -1;
		}
		...
		if (ast_pbx_start(chan)) {
			ast_log_chan(NULL, LOG_ERROR, "[%s] unable to start pbx on incoming call\n", pvt->id);
			mbl_ast_hangup(pvt);
			return -1;
		}
	}
...
static int handle_response_cmgr(struct mbl_pvt *pvt, char *buf)
{
	...
	if ((msg = msg_queue_head(pvt)) && msg->expected == AT_CMGR) {
		...
		if (ast_pbx_start(chan)) {
			ast_log_chan(NULL, LOG_ERROR, "[%s] unable to start pbx on incoming sms\n", pvt->id);
			mbl_ast_hangup(pvt);
		}
	} else {
...
static int handle_response_cusd(struct mbl_pvt *pvt, char *buf)
{
	...
	if (!(cusd = hfp_parse_cusd(pvt->hfp, buf))) {
		ast_verb_chan(NULL, 0, "[%s] error parsing CUSD: %s\n", pvt->id, buf);
		return 0;
	}

	ast_verb_chan(NULL, 0, "[%s] CUSD response: %s\n", pvt->id, cusd);
...
static int handle_response_no_dialtone(struct mbl_pvt *pvt, char *buf)
{
	ast_verb_chan(NULL, 1, "[%s] mobile reports NO DIALTONE\n", pvt->id);
	pvt->needchup = 1;
	mbl_queue_control(pvt, AST_CONTROL_CONGESTION);
	return 0;
}
...
static int handle_response_no_carrier(struct mbl_pvt *pvt, char *buf)
{
	ast_verb_chan(NULL, 1, "[%s] mobile reports NO CARRIER\n", pvt->id);
	pvt->needchup = 1;
	mbl_queue_control(pvt, AST_CONTROL_CONGESTION);
	return 0;
}
...
e_cleanup:
	...
	if (!hfp->initialized)
		ast_verb_chan(NULL, 3, "Error initializing Bluetooth device %s.\n", pvt->id);
	...
	ast_verb_chan(NULL, 3, "Bluetooth Device %s has disconnected.\n", pvt->id);
...
static void *do_monitor_headset(void *data)
{
	...
	ast_verb_chan(NULL, 3, "Bluetooth Device %s initialised and ready.\n", pvt->id);
			...
			if (pvt->outgoing) {
				...
				if (pvt->answered) {
					...
					if ((pvt->sco_socket = sco_connect(pvt->adapter->addr, pvt->addr)) == -1) {
						ast_log_chan(NULL, LOG_ERROR, "[%s] unable to create audio connection\n", pvt->id);
						mbl_queue_hangup(pvt);
						ast_mutex_unlock(&pvt->lock);
						goto e_cleanup;
					}
					...
					if (!adapter->inuse && !pvt->connected && !strcmp(adapter->id, pvt->adapter->id)) {
						if ((pvt->rfcomm_socket = rfcomm_connect(adapter->addr, pvt->addr, pvt->rfcomm_port)) > -1) {
							if (start_monitor(pvt)) {
								...
								ast_verb_chan(NULL, 3, "Bluetooth Device %s has connected, initializing...\n", pvt->id);
							}
						}
					}
					ast_mutex_unlock(&pvt->lock);
...
static void *do_sco_listen(void *data)
{
	...
	while (!check_unloading()) {
		...
		if (ast_io_wait(adapter->accept_io, 0) == -1) {
			/* handle errors */
			ast_log_chan(NULL, LOG_ERROR, "ast_io_wait() failed for adapter %s\n", adapter->id);
			break;
		}

		/* handle audio data */
		if (ast_io_wait(adapter->io, 1) == -1) {
			ast_log_chan(NULL, LOG_ERROR, "ast_io_wait() failed for audio on adapter %s\n", adapter->id);
			break;
		}
	}
...
static struct adapter_pvt *mbl_load_adapter(struct ast_config *cfg, const char *cat)
{
	...
	if (ast_strlen_zero(id) || ast_strlen_zero(address)) {
		ast_log_chan(NULL, LOG_ERROR, "Skipping adapter. Missing id or address settings.\n");
		goto e_return;
	}
	...
	if (!(adapter = ast_calloc(1, sizeof(*adapter)))) {
		ast_log_chan(NULL, LOG_ERROR, "Skipping adapter %s. Error allocating memory.\n", id);
		goto e_return;
	}
	...
	if (adapter->dev_id < 0 || adapter->hci_socket < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Skipping adapter %s. Unable to communicate with adapter.\n", adapter->id);
		goto e_free_adapter;
	}
	...
	vs = htobs(vs);
	if (vs != 0x0060) {
		ast_log_chan(NULL, LOG_ERROR, "Skipping adapter %s. Voice setting must be 0x0060 - see 'man hciconfig' for details.\n", adapter->id);
			...
			if (ast_true(v->value)) {
				...
				if (hci_strtolm("master", &dr.dev_opt)) {
					if (ioctl(adapter->hci_socket, HCISETLINKMODE, (unsigned long) &dr) < 0) {
						ast_log_chan(NULL, LOG_WARNING, "Unable to set adapter %s link mode to MASTER. Ignoring 'forcemaster' option.\n", adapter->id);
					}
				}
			}
	...
	if (!(adapter->accept_io = io_context_create())) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to create I/O context for audio connection listener\n");
		goto e_hci_close_dev;
	}

	if (!(adapter->io = io_context_create())) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to create I/O context for audio connections\n");
		goto e_destroy_accept_io;
	}

	/* bind the sco listener socket */
	if (sco_bind(adapter) < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Skipping adapter %s. Error binding audio connection listener socket.\n", adapter->id);
		goto e_destroy_io;
	}
	...
	if (!(adapter->sco_id = ast_io_add(adapter->accept_io, adapter->sco_socket, sco_accept, AST_IO_IN, adapter))) {
		ast_log_chan(NULL, LOG_ERROR, "Skipping adapter %s. Error adding listener socket to I/O context.\n", adapter->id);
		goto e_close_sco;
	}
	...
	if (ast_pthread_create_background(&adapter->sco_listener_thread, NULL, do_sco_listen, adapter)) {
		ast_log_chan(NULL, LOG_ERROR, "Skipping adapter %s. Error creating audio connection listener thread.\n", adapter->id);
		goto e_remove_sco;
	}
...
static struct mbl_pvt *mbl_load_device(struct ast_config *cfg, const char *cat)
{
	...
	if(ast_strlen_zero(adapter_str)) {
		ast_log_chan(NULL, LOG_ERROR, "Skipping device %s. No adapter specified.\n", cat);
		goto e_return;
	}
	...
	AST_RWLIST_UNLOCK(&adapters);
	if (!adapter) {
		ast_log_chan(NULL, LOG_ERROR, "Skipping device %s. Unknown adapter '%s' specified.\n", cat, adapter_str);
		goto e_return;
	}
	...
	if (ast_strlen_zero(port) || ast_strlen_zero(address)) {
		ast_log_chan(NULL, LOG_ERROR, "Skipping device %s. Missing required port or address setting.\n", cat);
		goto e_return;
	}

	/* create and initialize our pvt structure */
	if (!(pvt = ast_calloc(1, sizeof(*pvt)))) {
		ast_log_chan(NULL, LOG_ERROR, "Skipping device %s. Error allocating memory.\n", cat);
		goto e_return;
	}
	...
	if (!(pvt->bt_out_smoother = ast_smoother_new(DEVICE_FRAME_SIZE))) {
		ast_log_chan(NULL, LOG_ERROR, "Skipping device %s. Error setting up frame bt_out_smoother.\n", cat);
		goto e_free_pvt;
	}

	/* setup the bt_in_smoother */
	if (!(pvt->bt_in_smoother = ast_smoother_new(CHANNEL_FRAME_SIZE))) {
		ast_log_chan(NULL, LOG_ERROR, "Skipping device %s. Error setting up frame bt_in_smoother.\n", cat);
		goto e_free_bt_out_smoother;
	}

	/* setup the dsp */
	if (!(pvt->dsp = ast_dsp_new())) {
		ast_log_chan(NULL, LOG_ERROR, "Skipping device %s. Error setting up dsp for dtmf detection.\n", cat);
		goto e_free_bt_in_smoother;
	}

	/* setup the scheduler */
	if (!(pvt->sched = ast_sched_context_create())) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to create scheduler context for headset device\n");
		goto e_free_dsp;
	}
	...
	for (v = ast_variable_browse(cfg, "general"); v; v = v->next) {
		if (!strcasecmp(v->name, "interval")) {
			if (!sscanf(v->value, "%d", &discovery_interval)) {
				ast_log_chan(NULL, LOG_NOTICE, "error parsing 'interval' in general section, using default value\n");
			}
		}
	}
...
static int load_module(void)
{
	...
	if (dev_id < 0 || s < 0) {
		ast_log_chan(NULL, LOG_ERROR, "No Bluetooth devices found. Not loading module.\n");
		ao2_ref(mbl_tech.capabilities, -1);
		mbl_tech.capabilities = NULL;
		hci_close_dev(s);
		return AST_MODULE_LOAD_DECLINE;
	...
	if (mbl_load_config()) {
		ast_log_chan(NULL, LOG_ERROR, "Errors reading config file %s. Not loading module.\n", MBL_CONFIG);
		ao2_ref(mbl_tech.capabilities, -1);
		mbl_tech.capabilities = NULL;
		return AST_MODULE_LOAD_DECLINE;
	}
	...
	if (ast_pthread_create_background(&discovery_thread, NULL, do_discovery, NULL) < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to create discovery thread.\n");
		goto e_cleanup;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "Bluetooth Mobile Device Channel Driver",
