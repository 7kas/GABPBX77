...
#define DEFAULT_H323ID "GABpbx PBX"
...
struct ooh323_peer{
	...
	int         port;
	char        *h323id;    /* H323-ID alias, which gabpbx will register with gk to reach this peer*/
	char        *email;     /* Email alias, which gabpbx will register with gk to reach this peer*/
	char        *url;       /* url alias, which gabpbx will register with gk to reach this peer*/
	char        *e164;      /* e164 alias, which gabpbx will register with gk to reach this peer*/
	int         rtptimeout;
	struct OOH323Regex	    *rtpmask;
	char	    rtpmaskstr[120];
	int	    rtdrcount,rtdrinterval;
											 ...
											 const struct ast_assigned_ids *assignedids, const struct ast_channel *requestor)
{
	...
	if (gH323Debug) {
		ast_verb_chan(NULL, 0, "---   ooh323_new - %s\n", host);
	}
...
static struct ooh323_pvt *ooh323_alloc(int callref, char *callToken)
{
	...
	if (gH323Debug) {
		ast_verb_chan(NULL, 0, "---   ooh323_alloc\n");
	}

	if (!(pvt = ast_calloc(1, sizeof(*pvt)))) {
		ast_log_chan(NULL, LOG_ERROR, "Couldn't allocate private ooh323 structure\n");
		return NULL;
	}
...

{
	...
	if (gH323Debug) {
		ast_verb_chan(NULL, 0, "---   ooh323_request - data %s format %s\n", data, ast_format_cap_get_names(cap, &codec_buf));
	}

	if (!(ast_format_cap_has_type(cap, AST_MEDIA_TYPE_AUDIO))) {
		ast_log_chan(NULL, LOG_NOTICE, "Asked to get a channel of unsupported format '%s'\n", ast_format_cap_get_names(cap, &codec_buf));
		return NULL;
	}
	...
	if (!p) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to build pvt data for '%s'\n", data);
		return NULL;
	}
	...
	if (dest) {
		...
		ooh323_destroy(p);
		ast_mutex_unlock(&iflock);
		ast_log_chan(NULL, LOG_ERROR, "Destination format is not supported\n");
	...
	if (peer) {
		...
		if (gRasGkMode ==  RasNoGatekeeper) {
			/* no gk and no peer */
			ast_log_chan(NULL, LOG_ERROR, "Call to undefined peer %s", dest);
			ast_mutex_lock(&iflock);
			ast_mutex_unlock(&p->lock);
			ooh323_destroy(p);
			ast_mutex_unlock(&iflock);
			...
			ast_log_chan(NULL, LOG_ERROR, "Gatekeeper client is configured but not registered\n");
	...
	if (!chan) {
		...
		ast_mutex_unlock(&iflock);
      		...
      		if(!p->callToken) {
       			...
       			ooh323_destroy(p);
       			ast_mutex_unlock(&iflock);
       			ast_log_chan(NULL, LOG_ERROR, "Failed to allocate memory for callToken\n");
       			return NULL;
      		}
	...
	restart_monitor();
	if (gH323Debug)
		ast_verb_chan(NULL, 0, "+++   ooh323_request\n");
...
static struct ooh323_pvt* find_call(ooCallData *call)
{
	...
	if (gH323Debug)
		ast_verb_chan(NULL, 0, "---   find_call\n");
...
struct ooh323_user *find_user(const char * name, const char* ip)
{
	...
	if (gH323Debug)
      ast_verb_chan(NULL, 0, "---   find_user: %s, %s\n",name,ip);
	...
	if (gH323Debug)
		ast_verb_chan(NULL, 0, "+++   find_user\n");
...
struct ooh323_peer *find_friend(const char *name, int port)
{
	...
	if (gH323Debug)
		ast_verb_chan(NULL, 0, "---   find_friend \"%s\"\n", name);


	ast_mutex_lock(&peerl.lock);
	for (peer = peerl.peers; peer; peer = peer->next) {
		if (gH323Debug) {
			ast_verb_chan(NULL, 0, "		comparing with \"%s\"\n", peer->ip);
		}
	...
	if (gH323Debug) {
		if (peer) {
			ast_verb_chan(NULL, 0, "		found matching friend\n");
		}
		ast_verb_chan(NULL, 0, "+++   find_friend \"%s\"\n", name);
	}
...
struct ooh323_peer *find_peer(const char * name, int port)
{
	...
	if (gH323Debug)
		ast_verb_chan(NULL, 0, "---   find_peer \"%s\"\n", name);
	...
	if (gH323Debug) {
		if (peer) {
			ast_verb_chan(NULL, 0, "		found matching peer\n");
		}
		ast_verb_chan(NULL, 0, "+++   find_peer \"%s\"\n", name);
	}
...
static int ooh323_digit_begin(struct ast_channel *chan, char digit)
{
	...
	if (gH323Debug)
		ast_verb_chan(NULL, 0, "---   ooh323_digit_begin\n");

	if (!p) {
		ast_log_chan(NULL, LOG_ERROR, "No private structure for call\n");
		return -1;
	}
	ast_mutex_lock(&p->lock);
	...
	if (p->rtp && ((p->dtmfmode & H323_DTMF_RFC2833) || (p->dtmfmode & H323_DTMF_CISCO))) {
		...
		res = -1; // tell GABpbx to generate inband indications
	}
	ast_mutex_unlock(&p->lock);

	if (gH323Debug) {
		ast_verb_chan(NULL, 0, "+++   ooh323_digit_begin, res = %d\n", res);
	}
	return res;
}
...
static int ooh323_digit_end(struct ast_channel *chan, char digit, unsigned int duration)
{
	...
	if (gH323Debug)
		ast_verb_chan(NULL, 0, "---   ooh323_digit_end\n");
	...
	if (p->rtp && ((p->dtmfmode & H323_DTMF_RFC2833) || (p->dtmfmode & H323_DTMF_CISCO)) ) {
		...
		res = -1; // tell GABpbx to stop inband indications
	}

	ast_mutex_unlock(&p->lock);

	if (gH323Debug) {
		ast_verb_chan(NULL, 0, "+++   ooh323_digit_end, res = %d\n", res);
	}
	return res;
}
...
static int ooh323_call(struct ast_channel *ast, const char *dest, int timeout)
{
	...
	if (gH323Debug)
		ast_verb_chan(NULL, 0, "---   ooh323_call- %s\n", dest);


   	if ((ast_channel_state(ast) != AST_STATE_DOWN) && (ast_channel_state(ast) != AST_STATE_RESERVED)) {
		ast_log_chan(NULL, LOG_WARNING, "ooh323_call called on %s, neither down nor "
	...
	ast_mutex_unlock(&p->lock);
	if (res != OO_OK) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to make call\n");
      		return -1; /* ToDO: cleanup */
	}
	if (gH323Debug)
		ast_verb_chan(NULL, 0, "+++   ooh323_call\n");
...
static int ooh323_hangup(struct ast_channel *ast)
{
	...
	if (gH323Debug)
		ast_verb_chan(NULL, 0, "---   ooh323_hangup\n");
	...
	if (p) {
		...
		if (gH323Debug)
			ast_verb_chan(NULL, 0, "    hanging %s with cause: %d\n", p->username, q931cause);
		ast_channel_tech_pvt_set(ast, NULL);
		if (!ast_test_flag(p, H323_ALREADYGONE)) {
         		ooHangCall(p->callToken,
				ooh323_convert_hangupcause_gabpbxToH323(q931cause), q931cause);
...
static int ooh323_answer(struct ast_channel *ast)
{
	...
	if (gH323Debug)
		ast_verb_chan(NULL, 0, "--- ooh323_answer\n");
...
/* sending progress for first */
		...
		ast_mutex_unlock(&p->lock);
			...
			ast_log_chan(NULL, LOG_WARNING, "Can't send %u type frames with OOH323 write\n",
									 f->frametype);
			ast_mutex_unlock(&p->lock);
			return 0;
		}
...
static int ooh323_indicate(struct ast_channel *ast, int condition, const void *data, size_t datalen)
{
	...
	if (!callToken) {
		if (gH323Debug)
			ast_verb_chan(NULL, 0, "	ooh323_indicate - No callToken\n");
		return -1;
	}
	...
	if (gH323Debug) {
		ast_verb_chan(NULL, 0, "----- ooh323_indicate %d on call %s\n", condition, callToken);
	}
	...
	case AST_CONTROL_CONNECTED_LINE:
      ...
      case AST_CONTROL_T38_PARAMETERS:
		...
		if (datalen != sizeof(struct ast_control_t38_parameters)) {
			ast_log_chan(NULL, LOG_ERROR, "Invalid datalen for AST_CONTROL_T38. "
					   "Expected %d, got %d\n",
	...
	case -1:
		break;
	default:
		ast_log_chan(NULL, LOG_WARNING, "Don't know how to indicate condition %d on %s\n",
									condition, callToken);
	}
	...
	if (gH323Debug) {
		ast_verb_chan(NULL, 0, "++++  ooh323_indicate %d on %s is %d\n", condition, callToken, res);
	}
...
static int ooh323_queryoption(struct ast_channel *ast, int option, void *data, int *datalen)
{
	...
	if (gH323Debug)
		ast_verb_chan(NULL, 0, "----- ooh323_queryoption %d on channel %s\n", option, ast_channel_name(ast));
	...
	switch (option) {
		...
		case AST_OPTION_T38_STATE:
			...
			if (*datalen != sizeof(enum ast_t38_state)) {
				ast_log_chan(NULL, LOG_ERROR, "Invalid datalen for AST_OPTION_T38_STATE option."
...
static int ooh323_fixup(struct ast_channel *oldchan, struct ast_channel *newchan)
{
	...
	if (gH323Debug)
		ast_verb_chan(NULL, 0, "--- ooh323c ooh323_fixup\n");

	ast_mutex_lock(&p->lock);
	if (p->owner != oldchan) {
		ast_log_chan(NULL, LOG_WARNING, "Old channel wasn't %p but was %p\n", oldchan, p->owner);
		ast_mutex_unlock(&p->lock);
		return -1;
	}
...
void ooh323_set_write_format(ooCallData *call, struct ast_format *fmt, int txframes)
{
	...
	if (gH323Debug)
		ast_verb_chan(NULL, 0, "---   ooh323_update_writeformat %s/%d\n",
				ast_format_get_name(fmt), txframes);

	p = find_call(call);
	if (!p) {
		ast_log_chan(NULL, LOG_ERROR, "No matching call found for %s\n", call->callToken);
		return;
	}
	...
	if (p->owner) {
		...
		caps = ast_format_cap_alloc(AST_FORMAT_CAP_FLAG_DEFAULT);
		if (!caps) {
			ast_log_chan(NULL, LOG_ERROR, "Could not allocate capabilities structure\n");
			return;
		}
		...
		if (!p->owner) {
			ast_mutex_unlock(&p->lock);
			ast_log_chan(NULL, LOG_ERROR, "Channel has no owner\n");
			ao2_ref(caps, -1);
			return;
		}
		if (gH323Debug) {
			struct ast_str *codec_buf = ast_str_alloca(AST_FORMAT_CAP_NAMES_LEN);
			ast_verb_chan(NULL, 0, "Writeformat before update %s/%s\n",
		...
		ast_log_chan(NULL, LOG_ERROR, "No owner found\n");


	ast_mutex_unlock(&p->lock);

	if (gH323Debug)
		ast_verb_chan(NULL, 0, "+++   ooh323_update_writeformat\n");
}
...
void ooh323_set_read_format(ooCallData *call, struct ast_format *fmt)
{
	...
	if (gH323Debug)
		ast_verb_chan(NULL, 0, "---   ooh323_update_readformat %s\n",
	...
	if (p->owner) {
		...
		if (gH323Debug) {
			ast_verb_chan(NULL, 0, "Readformat before update %s\n",
	...
	if (gH323Debug)
		ast_verb_chan(NULL, 0, "+++   ooh323_update_readformat\n");
}
...
int onAlerting(ooCallData *call)
{
	...
	if (gH323Debug)
		ast_verb_chan(NULL, 0, "--- onAlerting %s\n", call->callToken);

   	p = find_call(call);

   	if(!p) {
		ast_log_chan(NULL, LOG_ERROR, "No matching call found\n");
		return -1;
	}
	ast_mutex_lock(&p->lock);
	if (!p->owner) {
...
int onProgress(ooCallData *call)
{
	...
	if (gH323Debug)
		ast_verb_chan(NULL, 0, "--- onProgress %s\n", call->callToken);
...
/**
  * Callback for sending digits from H.323 up to gabpbx
  *
  */
int ooh323_onReceivedDigit(OOH323CallData *call, const char *digit)
{
	...
	p = find_call(call);
	if (!p) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to find a matching call.\n");
		return -1;
	}
	if (!p->owner) {
		ast_log_chan(NULL, LOG_ERROR, "Channel has no owner\n");
...
int ooh323_onReceivedSetup(ooCallData *call, Q931Message *pmsg)
{
	...
	if (gH323Debug)
		ast_verb_chan(NULL, 0, "---   ooh323_onReceivedSetup %s\n", call->callToken);


	if (!(p = ooh323_alloc(call->callReference, call->callToken))) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to create a new call.\n");
		return -1;
	}
	ast_mutex_lock(&p->lock);
	...
	if (ast_strlen_zero(p->exten)) {
	 ...
	 if (!OO_TESTFLAG(p->flags,H323_DISABLEGK)) {
	  ...
	  ast_mutex_unlock(&p->lock);
	  ast_log_chan(NULL, LOG_ERROR, "Unacceptable ip %s\n", call->remoteIP);
	  if (!user) {
	   ooHangCall(call->callToken, ooh323_convert_hangupcause_gabpbxToH323(AST_CAUSE_CALL_REJECTED), AST_CAUSE_CALL_REJECTED);
	   call->callEndReason = OO_REASON_REMOTE_REJECTED;
	  }
	  else {
	   ooHangCall(call->callToken, ooh323_convert_hangupcause_gabpbxToH323(AST_CAUSE_NORMAL_CIRCUIT_CONGESTION), AST_CAUSE_NORMAL_CIRCUIT_CONGESTION);
	   call->callEndReason = OO_REASON_REMOTE_REJECTED;
	  }
	  ast_set_flag(p, H323_NEEDDESTROY);
	  return -1;
...
/* Incoming call */
  	...
  	if(!c) {
   	ast_mutex_unlock(&p->lock);
   	ast_log_chan(NULL, LOG_ERROR, "Could not create ast_channel\n");
         return -1;
  	}
	...
	if (!configure_local_rtp(p, call)) {
		ast_mutex_unlock(&p->lock);
		ast_log_chan(NULL, LOG_ERROR, "Couldn't create rtp structure\n");
		return -1;
	}
	...
	if (gH323Debug)
		ast_verb_chan(NULL, 0, "+++   ooh323_onReceivedSetup - Determined context %s, "
						"extension %s\n", p->context, p->exten);
...
int onOutgoingCall(ooCallData *call)
{
	...
	if (gH323Debug)
		ast_verb_chan(NULL, 0, "---   onOutgoingCall %lx: %s\n", (long unsigned int) call, call->callToken);
	...
	if (!strcmp(call->callType, "outgoing")) {
		...
		if (!ast_strlen_zero(p->caller_dialedDigits)) {
			if (gH323Debug) {
				ast_verb_chan(NULL, 0, "Setting dialed digits %s\n", p->caller_dialedDigits);
			}
			ooCallAddAliasDialedDigits(call, p->caller_dialedDigits);
		} else if (!ast_strlen_zero(p->callerid_num)) {
			if (ooIsDialedDigit(p->callerid_num)) {
				if (gH323Debug) {
					ast_verb_chan(NULL, 0, "setting callid number %s\n", p->callerid_num);
				}
	...
	if (gH323Debug)
		ast_verb_chan(NULL, 0, "+++   onOutgoingCall %s\n", call->callToken);
	return OO_OK;
}
...
int onNewCallCreated(ooCallData *call)
{
	...
	if (gH323Debug)
		ast_verb_chan(NULL, 0, "---   onNewCallCreated %lx: %s\n", (long unsigned int) call, call->callToken);

   	ast_mutex_lock(&call->Lock);
   	if (ooh323c_start_call_thread(call)) {
    		ast_log_chan(NULL, LOG_ERROR,"Failed to create call thread.\n");
	...
	if (!strcmp(call->callType, "outgoing")) {
		...
		if (gH323Debug) {
			...
			ast_verb_chan(NULL, 0, " Outgoing call %s(%s) - Codec prefs - %s\n",
   	...
   	ast_mutex_unlock(&call->Lock);
	if (gH323Debug)
		ast_verb_chan(NULL, 0, "+++   onNewCallCreated %s\n", call->callToken);
	return OO_OK;
}
...
int onCallEstablished(ooCallData *call)
{
	...
	if (gH323Debug)
		ast_verb_chan(NULL, 0, "---   onCallEstablished %s\n", call->callToken);
...
int onCallCleared(ooCallData *call)
{
	...
	if (gH323Debug)
		ast_verb_chan(NULL, 0, "---   onCallCleared %s \n", call->callToken);
   ...
   if ((p = find_call(call))) {
	...
	if (gH323Debug)
		ast_verb_chan(NULL, 0, "+++   onCallCleared\n");
...
/* static void ooh323_delete_user(struct ooh323_user *user)
{
	...
	if (gH323Debug)
		ast_verb_chan(NULL, 0, "---   ooh323_delete_user\n");
...
void ooh323_delete_peer(struct ooh323_peer *peer)
{
	...
	if (gH323Debug)
		ast_verb_chan(NULL, 0, "---   ooh323_delete_peer\n");
...
static struct ooh323_user *build_user(const char *name, struct ast_variable *v)
{
	...
	if (gH323Debug)
		ast_verb_chan(NULL, 0, "---   build_user\n");
				...
				if (ast_true(v->value)) {
					...
					while ((word = strsep(&next, ","))) {
						if (!strcasecmp(word, "cng")) {
							...
							ast_log_chan(NULL, LOG_WARNING, "Unknown faxdetect mode '%s' on line %d.\n", word, v->lineno);
						}
					}
...
static struct ooh323_peer *build_peer(const char *name, struct ast_variable *v, int friend_type)
{
	...
	if (gH323Debug)
		ast_verb_chan(NULL, 0, "---   build_peer\n");
	...
	if (peer) {
	    ...
	    if (!(peer->h323id = ast_strdup(v->value))) {
					ast_log_chan(NULL, LOG_ERROR, "Could not allocate memory for h323id of "
											 "peer %s\n", name);
				...
				if (valid) {
					if (!(peer->e164 = ast_strdup(v->value))) {
						ast_log_chan(NULL, LOG_ERROR, "Could not allocate memory for e164 of "
											 "peer %s\n", name);
						ooh323_delete_peer(peer);
						return NULL;
					}
				} else {
					ast_log_chan(NULL, LOG_ERROR, "Invalid e164: %s for peer %s\n", v->value, name);
				}
			} else  if (!strcasecmp(v->name, "email")) {
				if (!(peer->email = ast_strdup(v->value))) {
					ast_log_chan(NULL, LOG_ERROR, "Could not allocate memory for email of "
											 "peer %s\n", name);
					ooh323_delete_peer(peer);
					return NULL;
				}
...
static int ooh323_do_reload(void)
{
	...
	if (gH323Debug) {
		ast_verb_chan(NULL, 0, "---   ooh323_do_reload\n");
	}
	...
	/* Set aliases if any */
	if (gH323Debug) {
		ast_verb_chan(NULL, 0, "updating local aliases\n");
	}
...
char *handle_cli_ooh323_reload(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	if (gH323Debug)
		ast_verb_chan(NULL, 0, "---   ooh323_reload\n");

	ast_mutex_lock(&h323_reload_lock);
	if (h323_reloading) {
		ast_verb_chan(NULL, 0, "Previous OOH323 reload not yet done\n");
   } else {
		h323_reloading = 1;
	}
...
int reload_config(int reload)
{
	...
	if (gH323Debug)
		ast_verb_chan(NULL, 0, "---   reload_config\n");

	cfg = ast_config_load((char*)config, config_flags);

	/* We *must* have a config file otherwise stop immediately */
	if (!cfg) {
		ast_log_chan(NULL, LOG_NOTICE, "Unable to load config %s, OOH323 disabled\n", config);
		return 1;
	...
	if (reload) {
		...
		delete_peers();
		if (gH323Debug) {
			ast_verb_chan(NULL, 0, "  reload_config - Freeing up alias list\n");
		}
		cur = gAliasList;
		while (cur) {
			prev = cur;
		...
		if (!strcasecmp(v->name, "port")) {
			...
			if (endlimit) {
				*endlimit = '\0';
				...
				ast_log_chan(NULL, LOG_ERROR, "h225portrange: Invalid format, separate port range with \",\"\n");
			}
			...
			if (!pNewAlias) {
				ast_log_chan(NULL, LOG_ERROR, "Failed to allocate memory for h323id alias\n");
				ast_config_destroy(cfg);
				return 1;
			}
			...
			if (valid) {
         			pNewAlias = ast_calloc(1, sizeof(struct ooAliases));
				if (!pNewAlias) {
					ast_log_chan(NULL, LOG_ERROR, "Failed to allocate memory for e164 alias\n");
					ast_config_destroy(cfg);
					return 1;
				}
				...
				ast_log_chan(NULL, LOG_ERROR, "Invalid e164: %s\n", v->value);
			}
		} else if (!strcasecmp(v->name, "email")) {
         		pNewAlias = ast_calloc(1, sizeof(struct ooAliases));
			if (!pNewAlias) {
				ast_log_chan(NULL, LOG_ERROR, "Failed to allocate memory for email alias\n");
				ast_config_destroy(cfg);
				return 1;
			}
         ...
         ast_copy_string(version, v->value, sizeof(version));
			...
			ast_verb_chan(NULL, 3, "  == Setting RAS IP to %s\n", gRASIP);
			...
			if (v->value[0] == '/') {
				...
				snprintf(gLogFile, sizeof(gLogFile), "%s/%s", ast_config_AST_LOG_DIR, v->value);
         		...
         		ast_verb_chan(NULL, 3, "  == Setting default context to %s\n", gContext);
		} else if (!strcasecmp(v->name, "nat")) {
			...
			if (sscanf(v->value, "%30i", &format) == 1)
				...
				ast_log_chan(NULL, LOG_WARNING, "Invalid tos value at line %d, should be "
         ...
         ast_copy_string(gAccountcode, v->value, sizeof(gAccountcode));
            ...
            ast_log_chan(NULL, LOG_WARNING, "Unknown dtmf mode '%s', using rfc2833\n",
                                                                    v->value);
				gDTMFMode = H323_DTMF_RFC2833;
			}
		} else if (!strcasecmp(v->name, "relaxdtmf")) {
			...
			if (utype) {
				...
				if (!strcmp(utype, "user") || 0 == friend_type) {
					...
					if (user) {
						...
						ast_log_chan(NULL, LOG_WARNING, "Failed to build user %s\n", cat);
					}
				}
				if (!strcasecmp(utype, "peer") || 0 == friend_type) {
					...
					if (peer) {
						...
						ast_log_chan(NULL, LOG_WARNING, "Failed to build peer %s\n", cat);
					}
				}
			}
		}
	...
	if (ast_strlen_zero(gIP)) {
		...
		if (!strcmp(gIP, "127.0.0.1") || !strcmp(gIP, "::1")) {
			ast_log_chan(NULL, LOG_NOTICE, "Failed to determine local ip address. Please "
...
static int function_ooh323_read(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	if (strcmp(ast_channel_tech(chan)->type, "OOH323")) {
		ast_log_chan(NULL, LOG_ERROR, "This function is only supported on OOH323 channels, Channel is %s\n", ast_channel_tech(chan)->type);
		ast_channel_unlock(chan);
		return -1;
	}
		...
		if (ast_true(value)) {
			...
			while ((word = strsep(&next, ","))) {
				if (!strcasecmp(word, "cng")) {
					...
					ast_log_chan(NULL, LOG_WARNING, "Unknown faxdetect mode '%s'.\n", word);
					res = -1;
				}
			}
...
static int load_module(void)
{
	...
	if (!(sched = ast_sched_context_create())) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to create schedule context\n");
	}
	if (!(io = io_context_create())) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to create I/O context\n");
	}
	...
	if (!reload_config(0)) {
		...
		if (OO_OK != ooH323EpInitialize(OO_CALLMODE_AUDIOCALL, gLogFile, gInitError, sizeof(gInitError))) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to initialize OOH323 endpoint: %s"
				"OOH323 Disabled\n", gInitError);
			ao2_ref(gCap, -1);
			gCap = NULL;
		...
		if(ooH323EpSetTCPPortRange(ooconfig.mTCPPortStart, ooconfig.mTCPPortEnd) == OO_FAILED) {
			ast_log_chan(NULL, LOG_ERROR, "h225portrange: Failed to set range\n");
		}
		...
		if (ooh323c_set_capability(gCap, gDTMFMode, gDTMFCodec) < 0) {
			ast_log_chan(NULL, LOG_ERROR, "Capabilities failure for OOH323. OOH323 Disabled.\n");
			ao2_ref(gCap, -1);
			gCap = NULL;
			ao2_ref(ooh323_tech.capabilities, -1);
			ooh323_tech.capabilities = NULL;
		...
		if (ooCreateH323Listener() != OO_OK) {
         		ast_log_chan(NULL, LOG_ERROR, "OOH323 Listener Creation failure. "
                            "OOH323 DISABLED\n");
		...
		if (ooh323c_start_stack_thread() < 0) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to start OOH323 stack thread. "
                            "OOH323 DISABLED\n");
			ooH323EpDestroy();
			ao2_ref(gCap, -1);
			gCap = NULL;
		...
		ast_log_chan(NULL, LOG_ERROR, "Can't load ooh323 config file, OOH323 Disabled\n");
		return AST_MODULE_LOAD_DECLINE;
	}
...
static void *do_monitor(void *data)
{
	...
	for (;;) {
		...
		ast_mutex_unlock(&h323_reload_lock);
		if (reloading) {
			ast_verb_chan(NULL, 1, "Reloading H.323\n");
			ooh323_do_reload();
		}
		if (gH323ep.gkClient && gH323ep.gkClient->state == GkClientStopped) {
			ooGkClientDestroy();
			ast_verb_chan(NULL, 0, "Restart stopped gatekeeper client\n");
			ooGkClientInit(gRasGkMode, (gRasGkMode == RasUseSpecificGatekeeper) ?
									gGatekeeper : 0, gRASIP, 0);
			ooGkClientStart(gH323ep.gkClient);
		}
		...
		while (h323) {
			...
			if (h323->rtp && h323->owner && h323->rtptimeout &&
				...
				if (!ast_channel_trylock(h323->owner)) {
					...
					ast_log_chan(NULL, LOG_NOTICE, "Disconnecting call '%s' for lack of RTP activity in %ld seconds\n", ast_channel_name(h323->owner), (long) (t - h323->lastrtprx));
...
int ooh323_destroy(struct ooh323_pvt *p)
{
	...
	if (gH323Debug) {
		ast_verb_chan(NULL, 0, "---   ooh323_destroy \n");

		if (p)
			ast_verb_chan(NULL, 0, " Destroying %s\n", p->username);
	}
...
static int unload_module(void)
{
	...
	if (gH323Debug) {
		ast_verb_chan(NULL, 0, "--- ooh323  unload_module \n");
	}
	...
	if (gH323Debug) {
		ast_verb_chan(NULL, 0, "  unload_module - hanging up all interfaces\n");
	}
	if (!ast_mutex_lock(&iflock)) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Unable to lock the interface list\n");
		return -1;
	}


	if (gH323Debug) {
		ast_verb_chan(NULL, 0, "  unload_module - stopping monitor thread\n");
	}
	if (monitor_thread != AST_PTHREADT_NULL) {
		if (!ast_mutex_lock(&monlock)) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Unable to lock the monitor\n");
			return -1;
		}
	}
	...
	if (gH323Debug) {
		ast_verb_chan(NULL, 0, "   unload_module - stopping stack thread\n");
	}
	ooh323c_stop_stack_thread();


	if (gH323Debug) {
		ast_verb_chan(NULL, 0, "   unload_module - freeing up memory used by interfaces\n");
	}
	if (!ast_mutex_lock(&iflock)) {
	...
	if (gH323Debug) {
		ast_verb_chan(NULL, 0, "  unload_module - deleting users\n");
	}
	delete_users();


	if (gH323Debug) {
		ast_verb_chan(NULL, 0, "  unload_module - deleting peers\n");
	}
	delete_peers();


	if (gH323Debug) {
		ast_verb_chan(NULL, 0, "  unload_module - Freeing up alias list\n");
	}
	cur = gAliasList;
	while (cur) {
	  prev = cur;
	...
	if (gH323Debug) {
		ast_verb_chan(NULL, 0, "	unload_module- destroying OOH323 endpoint \n");
	}
	ooH323EpDestroy();
...
static void ooh323_get_codec(struct ast_channel *chan, struct ast_format_cap *result)
{
	struct ooh323_pvt *p = (struct ooh323_pvt *) ast_channel_tech_pvt(chan);
	if (gH323Debug) {
		ast_verb_chan(NULL, 0, "+++ ooh323  get_codec, %s\n", ast_channel_name(chan));
	}
...
static enum ast_rtp_glue_result ooh323_get_rtp_peer(struct ast_channel *chan, struct ast_rtp_instance **rtp)
{
	...
	if (gH323Debug) {
		ast_verb_chan(NULL, 0, "+++ ooh323  get_rtp_peer \n");
	}
	...
	*rtp = p->rtp ? ao2_ref(p->rtp, +1), p->rtp : NULL;
	...
	if (gH323Debug) {
		ast_verb_chan(NULL, 0, "ooh323_get_rtp_peer  %s -> %s:%d, %u\n", ast_channel_name(chan), ast_sockaddr_stringify_addr(&tmp),
						ast_sockaddr_port(&tmp), res);
	}
	if (gH323Debug) {
		ast_verb_chan(NULL, 0, "--- ooh323  get_rtp_peer, res = %d\n", (int) res);
	}
...
int ooh323_convertGABpbxCapToH323Cap(struct ast_format *format)
{
	if (ast_format_cmp(format, ast_format_ulaw) == AST_FORMAT_CMP_EQUAL) {
		...
		ast_log_chan(NULL, LOG_NOTICE, "Don't know how to deal with mode %s\n", ast_format_get_name(format));
		return -1;
	}
}
	 ...
	 struct ast_rtp_instance *vrtp, struct ast_rtp_instance *trtp, const struct ast_format_cap *cap, int nat_active)
{
	...
	if (gH323Debug) {
		ast_verb_chan(NULL, 0, "---   ooh323_set_peer - %s\n", ast_channel_name(chan));
	}

	if (ooh323_convertGABpbxCapToH323Cap(ast_channel_writeformat(chan)) < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Unknown format.\n");
		return -1;
	}
	...
	if (!callToken) {
		if (gH323Debug) {
			ast_verb_chan(NULL, 0, "	set_rtp_peer - No callToken\n");
		}
		ast_mutex_unlock(&p->lock);
		return -1;
	}
	...
	if (changed) {
		if (!ast_sockaddr_isnull(&p->redirip)) {
			...
			if (gH323Debug) {
				ast_verb_chan(NULL, 0, "ooh323_set_rtp_peer  return back to local\n");
			}
...
int configure_local_rtp(struct ooh323_pvt *p, ooCallData *call)
{
	...
	if (gH323Debug)
		ast_verb_chan(NULL, 0, "---   configure_local_rtp\n");

	memset(&mediaInfo, 0, sizeof(mediaInfo));
	if (ast_parse_arg(call->localIP, PARSE_ADDR, &tmp)) {
		ast_sockaddr_copy(&tmp, &bindaddr);
	}
	if (!(p->rtp = ast_rtp_instance_new("gabpbx", sched, &tmp, NULL))) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to create RTP session: %s\n",
			strerror(errno));
		return 0;
	}
	...
	if (!(p->udptl = ast_udptl_new_with_bindaddr(sched, io, 0, &tmp))) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to create UDPTL session: %s\n",
			strerror(errno));
		return 0;
	}
	...
	if (p->rtdrcount) {
		if (gH323Debug)
			ast_verb_chan(NULL, 0, "Setup RTDR info: %d, %d\n", p->rtdrinterval, p->rtdrcount);
	...
	for (x = 0; x < ast_format_cap_count(p->cap); x++) {
		...
		mediaInfo.cap = ooh323_convertGABpbxCapToH323Cap(format);
...
void setup_rtp_remote(ooCallData *call, const char *remoteIp, int remotePort)
{
	...
	if (gH323Debug) {
		ast_verb_chan(NULL, 0, "---   setup_rtp_remote %s:%d\n", remoteIp, remotePort);
	}
	if (!remoteIp || !remoteIp[0] || !remotePort) {
		if (gH323Debug) {
			ast_verb_chan(NULL, 0, "+++ setup_rtp_remote no data\n");
		}
		return;
	}
	...
	if (!p || !p->rtp) {
		ast_log_chan(NULL, LOG_ERROR, "Something is wrong: rtp\n");
		return;
	}
	...
	if (gH323Debug) {
		ast_verb_chan(NULL, 0, "+++   setup_rtp_remote\n");
	}
...
void setup_rtp_connection(ooCallData *call, const char *remoteIp, int remotePort)
{
	...
	if (gH323Debug)
		ast_verb_chan(NULL, 0, "---   setup_rtp_connection %s:%d\n", remoteIp, remotePort);
	...
	if(gH323Debug)
		ast_verb_chan(NULL, 0, "+++   setup_rtp_connection\n");
...
void close_rtp_connection(ooCallData *call)
{
   ...
   if(gH323Debug)
		ast_verb_chan(NULL, 0, "---   close_rtp_connection\n");

	p = find_call(call);
	if (!p) {
      ast_log_chan(NULL, LOG_ERROR, "Couldn't find matching call to close rtp "
                         "connection\n");
		return;
	}
	ast_mutex_lock(&p->lock);
								  ...
								  int remotePort)
{
	...
	if (gH323Debug)
		ast_verb_chan(NULL, 0, "---   setup_udptl_connection\n");
...
void close_udptl_connection(ooCallData *call)
{
	struct ooh323_pvt *p = NULL;
   	...
   	if(gH323Debug)
		ast_verb_chan(NULL, 0, "---   close_udptl_connection\n");

	p = find_call(call);
	if (!p) {
      		ast_log_chan(NULL, LOG_ERROR, "Couldn't find matching call to close udptl "
                         "connection\n");
		return;
	}
	ast_mutex_lock(&p->lock);
...
struct ast_frame *ooh323_rtp_read(struct ast_channel *ast, struct ooh323_pvt *p)
{
	...
	if (dfr && (dfr->frametype == AST_FRAME_DTMF) && ((dfr->subclass.integer == 'f') || (dfr->subclass.integer == 'e'))) {
		...
		if (!p->faxmode && !p->chmodepend && (dfr->subclass.integer == 'e') && (p->t38support != T38_DISABLED)) {
			if (gH323Debug)
				ast_verb_chan(NULL, 0, "request to change %s to t.38 because fax ced\n", p->callToken);
			p->chmodepend = 1;
			p->faxdetected = 1;
			ooRequestChangeMode(p->callToken, 1);
			...
			if ((strcmp(ast_channel_exten(p->owner), "fax")) &&
				...
				ast_verb_chan(NULL, 2, "Redirecting '%s' to fax extension due to CNG detection\n", ast_channel_name(p->owner));
				pbx_builtin_setvar_helper(p->owner, "FAXEXTEN", ast_channel_exten(p->owner));
				if (ast_async_goto(p->owner, target_context, "fax", 1)) {
					ast_log_chan(NULL, LOG_NOTICE, "Failed to async goto '%s' into fax of '%s'\n", ast_channel_name(p->owner),target_context);
				}
				p->faxdetected = 1;
				ast_frfree(dfr);
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "Objective Systems H323 Channel",
