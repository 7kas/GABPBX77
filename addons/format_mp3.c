 ...
 *
 * Derived from other gabpbx sound formats by
...
static int mp3_squeue(struct ast_filestream *s)
{
	...
	if (p->sbuflen < MP3_SCACHE) {
		if (ferror(s->f)) {
			ast_log_chan(NULL, LOG_WARNING, "Error while reading MP3 file: %s\n", strerror(errno));
			return -1;
		}
	}
...
{
	ast_log_chan(NULL, LOG_ERROR,"I Can't write MP3 only read them.\n");
	return -1;
...
AST_MODULE_INFO_STANDARD_EXTENDED(GABPBX_GPL_KEY, "MP3 format [Any rate but 8000hz mono is optimal]");
