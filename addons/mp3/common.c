...
int decode_header(struct frame *fr,unsigned long newhead)
{
    ...
    if( ((newhead>>10)&0x3) == 0x3) {
      ast_log_chan(NULL, LOG_WARNING,"Stream error\n");
      return (0);
    }
    if(fr->mpeg25) {
    ...
    if(!fr->bitrate_index)
    {
      ast_log_chan(NULL, LOG_WARNING,"Free format not supported.\n");
      return (0);
    }
...
#else
        ast_log_chan(NULL, LOG_WARNING,"Layer 1 not supported!\n");
#endif
        break;
      case 2:
#if 0
...
#else
        ast_log_chan(NULL, LOG_WARNING,"Layer 2 not supported!\n");
#endif
        break;
      case 3:
#if 0
        ...
        break; 
      default:
        ast_log_chan(NULL, LOG_WARNING,"Sorry, unknown layer type.\n"); 
        return (0);
    }
    return 1;
}
...
void print_header(struct frame *fr)
{
	...
	ast_log_chan(NULL, LOG_WARNING,"MPEG %s, Layer: %s, Freq: %ld, mode: %s, modext: %d, BPF : %d\n", 
		fr->mpeg25 ? "2.5" : (fr->lsf ? "2.0" : "1.0"),
		layers[fr->lay],freqs[fr->sampling_frequency],
		modes[fr->mode],fr->mode_ext,fr->framesize+4);
	ast_log_chan(NULL, LOG_WARNING,"Channels: %d, copyright: %s, original: %s, CRC: %s, emphasis: %d.\n",
		fr->stereo,fr->copyright?"Yes":"No",
		fr->original?"Yes":"No",fr->error_protection?"Yes":"No",
		fr->emphasis);
	ast_log_chan(NULL, LOG_WARNING,"Bitrate: %d Kbits/s, Extension value: %d\n",
...
void print_header_compact(struct frame *fr)
{
	...
	ast_log_chan(NULL, LOG_WARNING,"MPEG %s layer %s, %d kbit/s, %ld Hz %s\n",
		fr->mpeg25 ? "2.5" : (fr->lsf ? "2.0" : "1.0"),
		layers[fr->lay],
