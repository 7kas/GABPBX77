...
{
	if(param.verbose > 1)
		ast_log_chan(NULL, LOG_WARNING,"Init rate converter: %ld->%ld\n",m,n);

	if(n >= 96000 || m >= 96000 || m == 0 || n == 0) {
		ast_log_chan(NULL, LOG_WARNING,"NtoM converter: illegal rates\n");
		return (1);
	}
	...
	if(ntom_step > 8*NTOM_MUL) {
		ast_log_chan(NULL, LOG_WARNING,"max. 1:8 conversion allowed!\n");
		return (1);
	}
