...
static struct buf *addbuf(struct mpstr *mp,char *buf,int size)
{
	...
	nbuf = ast_malloc( sizeof(struct buf) );
	if(!nbuf) {
		ast_log_chan(NULL, LOG_WARNING,"Out of memory!\n");
		return NULL;
	}
	nbuf->pnt = ast_malloc(size);
	if(!nbuf->pnt) {
		ast_free(nbuf);
		return NULL;
	}
...
static int read_buf_byte(int *error, struct mpstr *mp)
{
	...
	while(pos >= mp->tail->size) {
		...
		if(!mp->tail) {
			...
			ast_log_chan(NULL, LOG_WARNING,"Fatal Buffer error!\n");
			*error = 1;
			return (0);		
		}
	}
		...
		int osize,int *done)
{
	...
	if(osize < 4608) {
		ast_log_chan(NULL, LOG_WARNING,"To less out space\n");
		return MP3_ERR;
	}
	...
	if(mp->framesize == 0) {
		...
		if(!head_check(mp->header) ) {
			...
			ast_log_chan(NULL, LOG_WARNING,"Junk at the beginning of frame %08lx\n",mp->header);
			...
			if(i == 65536) {
				ast_log_chan(NULL, LOG_WARNING,"Giving up searching valid MPEG header\n");
				return MP3_ERR;
			}
		}
...
int set_pointer(struct mpstr *mp, long backstep)
{
  ...
  if(mp->fsizeold < 0 && backstep > 0) {
    ast_log_chan(NULL, LOG_WARNING,"Can't step back %ld!\n",backstep);
    return MP3_ERR;
  }
