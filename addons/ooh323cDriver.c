...
/* make new thread */
 if (cur == NULL) {
	if (!(cur = ast_calloc(1, sizeof(struct callthread)))) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to allocate thread structure for call %s\n",
							call->callToken);
		return -1;
	}
	...
	if ((socketpair(PF_LOCAL, SOCK_STREAM, 0, cur->thePipe)) == -1) {
		ast_log_chan(NULL, LOG_ERROR, "Can't create thread pipe for call %s\n", call->callToken);
		ast_free(cur);
		return -1;
	}
	cur->inUse = TRUE;
	...
	if(ast_pthread_create_detached_background(&call->callThread, NULL, ooh323c_call_thread, cur) < 0)
 	{
  		ast_log_chan(NULL, LOG_ERROR, "Unable to start ooh323c call thread for call %s\n",
					call->callToken);
...
int ooh323c_start_stack_thread()
{
   ...
   {
      ast_log_chan(NULL, LOG_ERROR, "Unable to start ooh323c thread.\n");
      return -1;
   }
   if(ast_pthread_create_background(&ooh323cmd_thread, NULL, ooh323c_cmd_thread, NULL) < 0)
   {
      ast_log_chan(NULL, LOG_ERROR, "Unable to start ooh323cmd thread.\n");
      return -1;
   }
   return 0;
}
   ...
   (struct ast_format_cap *cap, int dtmf, int dtmfcodec)
{
   int ret = 0, x;
   if (gH323Debug) {
     ast_verb_chan(NULL, 0, "\tAdding capabilities to H323 endpoint\n");
   }
   ...
   for(x=0; x<ast_format_cap_count(cap); x++)
   {
    struct ast_format *format = ast_format_cap_get_format(cap, x);
      ...
      {
         if (gH323Debug) {
            ast_verb_chan(NULL, 0, "\tAdding g711 ulaw capability to H323 endpoint\n");
	 }
      ...
      {
         if (gH323Debug) {
            ast_verb_chan(NULL, 0, "\tAdding g711 alaw capability to H323 endpoint\n");
	 }
      ...
      {
         if (gH323Debug) {
	    ast_verb_chan(NULL, 0, "\tAdding g729A capability to H323 endpoint\n");
	 }
         ...
         if (gH323Debug) {
            ast_verb_chan(NULL, 0, "\tAdding g729 capability to H323 endpoint\n");
	 }
         ret |= ooH323EpAddG729Capability(OO_G729, 2, 24,
                                     ...
                                     &ooh323c_stop_transmit_channel);
         if (gH323Debug) {
            ast_verb_chan(NULL, 0, "\tAdding g729b capability to H323 endpoint\n");
	 }
      ...
      {
         if (gH323Debug) {
            ast_verb_chan(NULL, 0, "\tAdding g7231 capability to H323 endpoint\n");
	 }
      ...
      {
         if (gH323Debug) {
            ast_verb_chan(NULL, 0, "\tAdding g726 capability to H323 endpoint\n");
	 }
      ...
      {
         if (gH323Debug) {
            ast_verb_chan(NULL, 0, "\tAdding g726aal2 capability to H323 endpoint\n");
	 }
      ...
      {
         if (gH323Debug) {
            ast_verb_chan(NULL, 0, "\tAdding h263 capability to H323 endpoint\n");
	 }
      ...
      {
         if (gH323Debug) {
            ast_verb_chan(NULL, 0, "\tAdding gsm capability to H323 endpoint\n");
	 }
      ...
      {
         if (gH323Debug) {
            ast_verb_chan(NULL, 0, "\tAdding speex capability to H323 endpoint\n");
	 }
		 ...
		 int t38support, int g729onlyA)
{
   ...
   if (gH323Debug) {
     ast_verb_chan(NULL, 0, "\tAdding capabilities to call(%s, %s)\n", call->callType,
                                                            call->callToken);
   }
   ...
   for(x=0; x<ast_format_cap_count(cap); x++)
   {
    struct ast_format *format = ast_format_cap_get_format(cap, x);
      ...
      {
         if (gH323Debug) {
            ast_verb_chan(NULL, 0, "\tAdding g711 ulaw capability to call(%s, %s)\n",
	 ...
	 txframes = ast_format_cap_get_format_framing(cap, format);
      ...
      {
         if (gH323Debug) {
            ast_verb_chan(NULL, 0, "\tAdding g711 alaw capability to call(%s, %s)\n",
                                            call->callType, call->callToken);
	 }
         txframes = ast_format_cap_get_format_framing(cap, format);
      ...
      {
         if (gH323Debug) {
            ast_verb_chan(NULL, 0, "\tAdding g726 capability to call (%s, %s)\n",
                                           call->callType, call->callToken);
	 }
	 txframes = ast_format_cap_get_format_framing(cap, format);
      ...
      {
         if (gH323Debug) {
            ast_verb_chan(NULL, 0, "\tAdding g726aal2 capability to call (%s, %s)\n",
                                           call->callType, call->callToken);
	 }
	 txframes = ast_format_cap_get_format_framing(cap, format);
      ...
      if(ast_format_cmp(format, ast_format_g729) == AST_FORMAT_CMP_EQUAL)
      {
         ...
         if (gH323Debug) {
            ast_verb_chan(NULL, 0, "\tAdding g729A capability to call(%s, %s)\n",
                                            call->callType, call->callToken);
	 }
	 ...
	 if (g729onlyA)
		continue;
         if (gH323Debug) {
            ast_verb_chan(NULL, 0, "\tAdding g729 capability to call(%s, %s)\n",
                                            call->callType, call->callToken);
	 }
         ret|= ooCallAddG729Capability(call, OO_G729, txframes, txframes,
                                     ...
                                     &ooh323c_stop_transmit_channel);
         if (gH323Debug) {
            ast_verb_chan(NULL, 0, "\tAdding g729B capability to call(%s, %s)\n",
                                            call->callType, call->callToken);
	 }
      ...
      {
         if (gH323Debug) {
            ast_verb_chan(NULL, 0, "\tAdding g7231 capability to call (%s, %s)\n",
                                           call->callType, call->callToken);
	 }
      ...
      {
         if (gH323Debug) {
            ast_verb_chan(NULL, 0, "\tAdding h263 capability to call (%s, %s)\n",
                                           call->callType, call->callToken);
	 }
      ...
      {
         if (gH323Debug) {
            ast_verb_chan(NULL, 0, "\tAdding gsm capability to call(%s, %s)\n",
                                             call->callType, call->callToken);
	 }
      ...
      {
         if (gH323Debug) {
            ast_verb_chan(NULL, 0, "\tAdding Speex capability to call(%s, %s)\n",
                                             call->callType, call->callToken);
	 }
...
int ooh323c_start_receive_channel(ooCallData *call, ooLogicalChannel *pChannel)
{
   ...
   tmpfmt = convertH323CapToGABpbxCap(pChannel->chanCap->cap);
   if(tmpfmt) {
      /* ooh323_set_read_format(call, fmt); */
   }else{
     ast_log_chan(NULL, LOG_ERROR, "Invalid capability type for receive channel %s\n",
                                                          call->callToken);
     return -1;
   }
   return 1;
...
struct ast_format *convertH323CapToGABpbxCap(int cap)
{
   switch(cap)
   {
      case OO_G711ULAW64K:
