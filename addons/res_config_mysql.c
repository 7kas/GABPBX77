...
static struct tables *find_table(const char *database, const char *tablename)
{
	...
	if (mysql_real_query(&dbh->handle, ast_str_buffer(sql), ast_str_strlen(sql))) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to query database '%s', table '%s' columns: %s\n", database, tablename, mysql_error(&dbh->handle));
		release_database(dbh);
		AST_LIST_UNLOCK(&mysql_tables);
		return NULL;
	}
	...
	if ((result = mysql_store_result(&dbh->handle))) {
		while ((row = mysql_fetch_row(result))) {
			...
			if (!(column = ast_calloc(1, sizeof(*column) + strlen(fname) + strlen(ftype) + strlen(fdflt) + 3))) {
				ast_log_chan(NULL, LOG_ERROR, "Unable to allocate column element %s for %s\n", fname, tablename);
				destroy_table(table);
				release_database(dbh);
				AST_LIST_UNLOCK(&mysql_tables);
				return NULL;
...
static struct ast_variable *realtime_mysql(const char *database, const char *table, const struct ast_variable *rt_fields)
{
	...
	if (!(dbh = find_database(database, 0))) {
		ast_log_chan(NULL, LOG_WARNING, "MySQL RealTime: Invalid database specified: %s (check res_mysql.conf)\n", database);
		return NULL;
	}

	if (!table) {
		ast_log_chan(NULL, LOG_WARNING, "MySQL RealTime: No table specified.\n");
		release_database(dbh);
		return NULL;
	}
	...
	if (!field) {
		ast_log_chan(NULL, LOG_WARNING, "MySQL RealTime: Realtime retrieval requires at least 1 parameter and 1 value to search on.\n");
		release_database(dbh);
		return NULL;
	}
	...
	/* Execution. */
	if (mysql_real_query(&dbh->handle, ast_str_buffer(sql), ast_str_strlen(sql))) {
		ast_log_chan(NULL, LOG_WARNING, "MySQL RealTime: Failed to query database: %s\n", mysql_error(&dbh->handle));
		release_database(dbh);
		return NULL;
	}
...
static int update_mysql(const char *database, const char *tablename, const char *keyfield, const char *lookup, const struct ast_variable *rt_fields)
{
	...
	if (!(column = find_column(table, keyfield))) {
		ast_log_chan(NULL, LOG_ERROR, "MySQL RealTime: Updating on column '%s', but that column does not exist within the table '%s' (db '%s')!\n", keyfield, tablename, database);
		release_table(table);
		release_database(dbh);
		return -1;
	}
	...
	/* Get the first parameter and first value in our list of passed paramater/value pairs */
	if (!field) {
		ast_log_chan(NULL, LOG_WARNING, "MySQL RealTime: Realtime update requires at least 1 parameter and 1 value to update.\n");
		release_table(table);
		release_database(dbh);
		return -1;
	}
	...
	if (!(column = find_column(table, field->name))) {
		ast_log_chan(NULL, LOG_ERROR, "MySQL RealTime: Updating column '%s', but that column does not exist within the table '%s' (first pair MUST exist)!\n", field->name, tablename);
		release_table(table);
		release_database(dbh);
		return -1;
	}
	...
	while ((field = field->next)) {
		...
		if (!(column = find_column(table, field->name))) {
			ast_log_chan(NULL, LOG_WARNING, "Attempted to update column '%s' in table '%s', but column does not exist!\n", field->name, tablename);
			continue;
		}
	...
	/* Execution. */
	if (mysql_real_query(&dbh->handle, ast_str_buffer(sql), ast_str_strlen(sql))) {
		ast_log_chan(NULL, LOG_WARNING, "MySQL RealTime: Failed to update database: %s\n", mysql_error(&dbh->handle));
		release_table(table);
		release_database(dbh);
		return -1;
	}
...
static int update2_mysql(const char *database, const char *tablename, const struct ast_variable *lookup_fields, const struct ast_variable *update_fields)
{
	...
	if (!(dbh = find_database(database, 1))) {
		ast_log_chan(NULL, LOG_ERROR, "Invalid database specified: %s\n", database);
		return -1;
	}
	...
	for (field = lookup_fields; field; field = field->next) {
		if (!(column = find_column(table, field->name))) {
			ast_log_chan(NULL, LOG_ERROR, "Updating on column '%s', but that column does not exist within the table '%s'!\n", field->name, tablename);
			release_table(table);
			release_database(dbh);
			return -1;
		}
...
static int store_mysql(const char *database, const char *table, const struct ast_variable *rt_fields)
{
	...
	/* Get the first parameter and first value in our list of passed paramater/value pairs */
	if (!field) {
		ast_log_chan(NULL, LOG_WARNING, "MySQL RealTime: Realtime storage requires at least 1 parameter and 1 value to search on.\n");
		release_database(dbh);
		return -1;
	}
	/* Must connect to the server before anything else, as the escape function requires the mysql handle. */
	...
	/* Execution. */
	if (mysql_real_query(&dbh->handle, ast_str_buffer(sql), ast_str_strlen(sql))) {
		ast_log_chan(NULL, LOG_WARNING, "MySQL RealTime: Failed to insert into database: %s\n", mysql_error(&dbh->handle));
		release_database(dbh);
		return -1;
	}
...
static int destroy_mysql(const char *database, const char *table, const char *keyfield, const char *lookup, const struct ast_variable *rt_fields)
{
	...
	if (ast_strlen_zero(keyfield) || ast_strlen_zero(lookup))  {
		ast_log_chan(NULL, LOG_WARNING, "MySQL RealTime: Realtime destroying requires at least 1 parameter and 1 value to search on.\n");
		release_database(dbh);
		return -1;
	}
	...
	/* Execution. */
	if (mysql_real_query(&dbh->handle, ast_str_buffer(sql), ast_str_strlen(sql))) {
		ast_log_chan(NULL, LOG_WARNING, "MySQL RealTime: Failed to delete from database: %s\n", mysql_error(&dbh->handle));
		release_database(dbh);
		return -1;
	}
...
static struct ast_config *config_mysql(const char *database, const char *table, const char *file, struct ast_config *cfg, struct ast_flags config_flags, const char *unused, const char *who_asked)
{
	...
	if (!file || !strcmp(file, RES_CONFIG_MYSQL_CONF)) {
		ast_log_chan(NULL, LOG_WARNING, "MySQL RealTime: Cannot configure myself.\n");
		return NULL;
	}
	...
	if (mysql_real_query(&dbh->handle, ast_str_buffer(sql), ast_str_strlen(sql))) {
		ast_log_chan(NULL, LOG_WARNING, "MySQL RealTime: Failed to query database. Check debug for more info.\n");
	...
	if ((result = mysql_store_result(&dbh->handle))) {
		...
		ast_log_chan(NULL, LOG_WARNING, "MySQL RealTime: Could not find config '%s' in database.\n", file);
	}
			...
			if (strcmp(column->name, elm) == 0) {
				...
				if (strncmp(column->type, "char", 4) == 0 || strncmp(column->type, "varchar", 7) == 0) {
					if ((size > column->len) && column->len != -1) {
						ast_log_chan(NULL, LOG_WARNING, "Realtime table %s@%s: Column '%s' should be at least %d long, but is only %d long.\n", database, tablename, column->name, size, column->len);
						res = -1;
					}
				} else if (strcasestr(column->type, "unsigned")) {
					if (!ast_rq_is_int(type)) {
						ast_log_chan(NULL, LOG_WARNING, "Realtime table %s@%s: column '%s' cannot be type '%s' (need %s)\n",
						...
						if (type != RQ_UINTEGER1) {
							ast_log_chan(NULL, LOG_WARNING, "Realtime table %s@%s: column '%s' may not be large enough for "            \
					...
					if (!ast_rq_is_int(type) && type != RQ_FLOAT) {
						ast_log_chan(NULL, LOG_WARNING, "Realtime table %s@%s: Column %s cannot be a %s\n", tablename, database, column->name, column->type);
						res = -1;
					}
...
static int load_module(void)
{
	...
	ast_verb_chan(NULL, 2, "MySQL RealTime driver loaded.\n");
...
static int unload_module(void)
{
	...
	ast_verb_chan(NULL, 2, "MySQL RealTime unloaded.\n");
...
{
	parse_config(1);
	ast_verb_chan(NULL, 2, "MySQL RealTime reloaded.\n");
	return 0;
}
...
static int parse_config(int reload)
{
	...
	if (config == CONFIG_STATUS_FILEMISSING) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Not %sloading " RES_CONFIG_MYSQL_CONF "\n", reload ? "re" : "");
	}
	...
	for (catg = ast_category_browse(config, NULL); catg; catg = ast_category_browse(config, catg)) {
		...
		if (!cur) {
			if (!(cur = ast_calloc(1, sizeof(*cur) + strlen(catg) + 1))) {
				ast_log_chan(NULL, LOG_WARNING, "Could not allocate space for MySQL database '%s'\n", catg);
				continue;
			}
...
static int load_mysql_config(struct ast_config *config, const char *category, struct mysql_conn *conn)
{
	...
	if (!(s = ast_variable_retrieve(config, category, "dbuser"))) {
		ast_log_chan(NULL, LOG_WARNING, "MySQL RealTime: No database user found, using 'gabpbx' as default.\n");
		s = "gabpbx";
	}
	ast_copy_string(conn->user, s, sizeof(conn->user));

	if (!(s = ast_variable_retrieve(config, category, "dbpass"))) {
		ast_log_chan(NULL, LOG_WARNING, "MySQL RealTime: No database password found, using 'gabpbx' as default.\n");
		s = "gabpbx";
	}
	ast_copy_string(conn->pass, s, sizeof(conn->pass));

	if (!(s = ast_variable_retrieve(config, category, "dbhost"))) {
		ast_log_chan(NULL, LOG_WARNING, "MySQL RealTime: No database host found, using localhost via socket.\n");
		s = "";
	}
	ast_copy_string(conn->host, s, sizeof(conn->host));

	if (!(s = ast_variable_retrieve(config, category, "dbname"))) {
		ast_log_chan(NULL, LOG_WARNING, "MySQL RealTime: No database name found, using 'gabpbx' as default.\n");
		s = "gabpbx";
	}
	ast_copy_string(conn->name, s, sizeof(conn->name));

	if (!(s = ast_variable_retrieve(config, category, "dbport"))) {
		ast_log_chan(NULL, LOG_WARNING, "MySQL RealTime: No database port found, using 3306 as default.\n");
		conn->port = 3306;
	} else
		conn->port = atoi(s);
	...
	if (!(s = ast_variable_retrieve(config, category, "dbsock"))) {
		if (ast_strlen_zero(conn->host)) {
			...
			for (i = 0; i < 3; i++) {
				if (!stat(paths[i], &st)) {
					ast_log_chan(NULL, LOG_WARNING, "MySQL RealTime: No database socket found, using '%s' as default.\n", paths[i]);
					ast_copy_string(conn->sock, paths[i], sizeof(conn->sock));
				}
			}
			if (i == 3) {
				ast_log_chan(NULL, LOG_WARNING, "MySQL RealTime: No database socket found (and unable to detect a suitable path).\n");
				return 0;
			}
		}
	} else
	...
	if (!(s = ast_variable_retrieve(config, category, "requirements"))) {
		ast_log_chan(NULL, LOG_WARNING, "MySQL realtime: no requirements setting found, using 'warn' as default.\n");
		...
		ast_log_chan(NULL, LOG_WARNING, "MySQL realtime: unrecognized requirements setting '%s', using 'warn'\n", s);
		conn->requirements = RQ_WARN;
	}
...
reconnect_tryagain:
	if ((!conn->connected) && (!ast_strlen_zero(conn->host) || !ast_strlen_zero(conn->sock)) && !ast_strlen_zero(conn->user) && !ast_strlen_zero(conn->name)) {
		if (!mysql_init(&conn->handle)) {
			ast_log_chan(NULL, LOG_WARNING, "MySQL RealTime: Insufficient memory to allocate MySQL resource.\n");
			conn->connected = 0;
			return 0;
		}
		...
		if (mysql_real_connect(&conn->handle, conn->host, conn->user, conn->pass, conn->name, conn->port, conn->sock, 0)) {
			...
			ast_log_chan(NULL, LOG_ERROR, "MySQL RealTime: Failed to connect database server %s on %s (err %d). Check debug for more info.\n", conn->name, !ast_strlen_zero(conn->host) ? conn->host : conn->sock, mysql_errno(&conn->handle));
		...
		if (mysql_ping(&conn->handle) != 0 && (usleep(1) + 2 > 0) && mysql_ping(&conn->handle) != 0) {
			conn->connected = 0;
			conn->connect_time = 0;
			ast_log_chan(NULL, LOG_ERROR, "MySQL RealTime: Ping failed (%d).  Trying an explicit reconnect.\n", mysql_errno(&conn->handle));
		...
		if (mysql_select_db(&conn->handle, conn->name) != 0) {
			ast_log_chan(NULL, LOG_WARNING, "MySQL RealTime: Unable to select database: %s. Still Connected (%u) - %s.\n", conn->name, mysql_errno(&conn->handle), mysql_error(&conn->handle));
			return 0;
		}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "MySQL RealTime Configuration Driver",
