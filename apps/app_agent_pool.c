...
static int bridge_agent_hold_deferred_create(void)
{
	...
	if (!holding) {
		...
		if (!holding) {
			ast_log_chan(NULL, LOG_ERROR, "Could not create agent holding bridge.\n");
			return -1;
		}
	}
	return 0;
...
static void agent_logout(struct agent_pvt *agent)
{
	...
	ast_channel_unlock(logged);
	ast_verb_chan(NULL, 2, "Agent '%s' logged out.  Logged in for %ld seconds.\n",
...
static void agent_after_bridge_cb_failed(enum ast_bridge_after_cb_reason reason, void *data)
{
	...
	ast_log_chan(NULL, LOG_WARNING, "Agent %s: Forced logout.  Lost control of %s because: %s\n",
		agent->username, ast_channel_name(chan),
...
static int caller_safety_timeout(struct ast_bridge_channel *bridge_channel, void *hook_pvt)
{
	...
	if (agent->state == AGENT_STATE_CALL_PRESENT) {
		ast_log_chan(NULL, LOG_WARNING, "Agent '%s' process did not respond.  Safety timeout.\n",
			agent->username);
...
static int caller_joined_bridge(struct ast_bridge_channel *bridge_channel, void *hook_pvt)
{
	...
	logged = agent_bridge_channel_get_lock(agent);
	if (!logged) {
		ast_verb_chan(NULL, 3, "Agent '%s' not logged in.\n", agent->username);
	...
	ao2_ref(logged, -1);
	if (res) {
		ast_verb_chan(NULL, 3, "Agent '%s': Failed to alert the agent.\n", agent->username);
		pbx_builtin_setvar_helper(bridge_channel->chan, "AGENT_STATUS", "ERROR");
...
static int agent_request_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(args.agent_id)) {
		ast_log_chan(NULL, LOG_WARNING, "AgentRequest requires an AgentId\n");
		return -1;
	}
	...
	if (!agent) {
		ast_verb_chan(NULL, 3, "Agent '%s' does not exist.\n", args.agent_id);
	...
	default:
		...
		ast_bridge_destroy(caller_bridge, 0);
		ast_bridge_features_cleanup(&caller_features);
		ast_verb_chan(NULL, 3, "Agent '%s' is busy.\n", agent->username);
	...
	if (ast_bridge_join(caller_bridge, chan, NULL, &caller_features, NULL,
		AST_BRIDGE_JOIN_PASS_REFERENCE)) {
		caller_abort_agent(agent);
		ast_verb_chan(NULL, 3, "Agent '%s': Caller %s failed to join the bridge.\n",
			agent->username, ast_channel_name(chan));
...
static int agent_login_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(args.agent_id)) {
		ast_log_chan(NULL, LOG_WARNING, "AgentLogin requires an AgentId\n");
		return -1;
	}
	...
	if (agent->logged) {
		agent_unlock(agent);
		ast_verb_chan(NULL, 3, "Agent '%s' already logged in.\n", agent->username);
	...
	ast_verb_chan(NULL, 2, "Agent '%s' logged in (format %s/%s)\n", agent->username,
...
static int agent_function_read(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	if (ast_strlen_zero(args.agentid)) {
		ast_log_chan(NULL, LOG_WARNING, "The AGENT function requires an argument - agentid!\n");
		return -1;
	}
	if (!args.item) {
		args.item = "status";
	...
	if (!agent) {
		ast_log_chan(NULL, LOG_WARNING, "Agent '%s' not found!\n", args.agentid);
		return -1;
	}
...
static int load_module(void)
{
	...
	if (res) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to register application. Not loading module.\n");
		unload_module();
		return AST_MODULE_LOAD_DECLINE;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "Call center agent pool applications",
