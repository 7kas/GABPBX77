...
static void isAnsweringMachine(struct ast_channel *chan, const char *data)
{
	...
	ast_verb_chan(NULL, 3, "AMD: %s %s %s (Fmt: %s)\n", ast_channel_name(chan),
	...
	ast_verb_chan(NULL, 3, "AMD: initialSilence [%d] greeting [%d] afterGreetingSilence [%d] "
	...
	if (ast_set_read_format(chan, ast_format_slin) < 0 ) {
		ast_log_chan(NULL, LOG_WARNING, "AMD: Channel [%s]. Unable to set to linear mode, giving up\n", ast_channel_name(chan));
	...
	if (!(silenceDetector = ast_dsp_new())) {
		ast_log_chan(NULL, LOG_WARNING, "AMD: Channel [%s]. Unable to create silence detector :(\n", ast_channel_name(chan));
		pbx_builtin_setvar_helper(chan , "AMDSTATUS", "");
		pbx_builtin_setvar_helper(chan , "AMDCAUSE", "");
		return;
	}
	...
	while ((res = ast_waitfor(chan, 2 * maxWaitTimeForFrame)) > -1) {
		...
		if (!(f = ast_read(chan))) {
			ast_verb_chan(NULL, 3, "AMD: Channel [%s]. HANGUP\n", ast_channel_name(chan));
		...
		if ( (ast_tvdiff_ms(ast_tvnow(), amd_tvstart)) > totalAnalysisTime ) {
			...
			if ( audioFrameCount == 0 ) {
				ast_verb_chan(NULL, 3, "AMD: Channel [%s]. No audio data received in [%d] seconds.\n", ast_channel_name(chan), totalAnalysisTime);
				sprintf(amdCause , "NOAUDIODATA-%d", iTotalTime);
				break;
			}
			ast_verb_chan(NULL, 3, "AMD: Channel [%s]. Timeout...\n", ast_channel_name(chan));
		...
		if (f->frametype == AST_FRAME_VOICE || f->frametype == AST_FRAME_CNG) {
			...
			if (iTotalTime >= totalAnalysisTime) {
				ast_verb_chan(NULL, 3, "AMD: Channel [%s]. Too long...\n", ast_channel_name(chan));
				ast_frfree(f);
				strcpy(amdStatus , "NOTSURE");
				sprintf(amdCause , "TOOLONG-%d", iTotalTime);
				break;
			...
			if (dspsilence > 0) {
				...
				if (inInitialSilence == 1  && silenceDuration >= initialSilence) {
					ast_verb_chan(NULL, 3, "AMD: Channel [%s]. ANSWERING MACHINE: silenceDuration:%d initialSilence:%d\n",
				...
				if (silenceDuration >= afterGreetingSilence  &&  inGreeting == 1) {
					ast_verb_chan(NULL, 3, "AMD: Channel [%s]. HUMAN: silenceDuration:%d afterGreetingSilence:%d\n",
				...
				if (consecutiveVoiceDuration >= minimumWordLength && currentState == STATE_IN_SILENCE) {
					iWordsCount++;
					ast_verb_chan(NULL, 3, "AMD: Channel [%s]. Word detected. iWordsCount:%d\n", ast_channel_name(chan), iWordsCount);
					currentState = STATE_IN_WORD;
				}
				if (consecutiveVoiceDuration >= maximumWordLength){
					ast_verb_chan(NULL, 3, "AMD: Channel [%s]. Maximum Word Length detected. [%d]\n", ast_channel_name(chan), consecutiveVoiceDuration);
					ast_frfree(f);
					strcpy(amdStatus , "MACHINE");
				...
				if (iWordsCount > maximumNumberOfWords) {
					ast_verb_chan(NULL, 3, "AMD: Channel [%s]. ANSWERING MACHINE: iWordsCount:%d\n", ast_channel_name(chan), iWordsCount);
					ast_frfree(f);
					strcpy(amdStatus , "MACHINE");
				...
				if (inGreeting == 1 && voiceDuration >= greeting) {
					ast_verb_chan(NULL, 3, "AMD: Channel [%s]. ANSWERING MACHINE: voiceDuration:%d greeting:%d\n", ast_channel_name(chan), voiceDuration, greeting);
					ast_frfree(f);
					strcpy(amdStatus , "MACHINE");
				...
				if (voiceDuration >= minimumWordLength ) {
					if (silenceDuration > 0)
						ast_verb_chan(NULL, 3, "AMD: Channel [%s]. Detected Talk, previous silence duration: %d\n", ast_channel_name(chan), silenceDuration);
					silenceDuration = 0;
				}
				if (consecutiveVoiceDuration >= minimumWordLength && inGreeting == 0) {
					...
					if (silenceDuration > 0)
						ast_verb_chan(NULL, 3, "AMD: Channel [%s]. Before Greeting Time:  silenceDuration: %d voiceDuration: %d\n", ast_channel_name(chan), silenceDuration, voiceDuration);
					inInitialSilence = 0;
					inGreeting = 1;
				}
	...
	if (readFormat && ast_set_read_format(chan, readFormat))
		ast_log_chan(NULL, LOG_WARNING, "AMD: Unable to restore read format on '%s'\n", ast_channel_name(chan));
...
static int load_config(int reload)
{
	...
	if (!(cfg = ast_config_load("amd.conf", config_flags))) {
		ast_log_chan(NULL, LOG_ERROR, "Configuration file amd.conf missing.\n");
		return -1;
	} else if (cfg == CONFIG_STATUS_FILEUNCHANGED) {
		return 0;
	} else if (cfg == CONFIG_STATUS_FILEINVALID) {
		ast_log_chan(NULL, LOG_ERROR, "Config file amd.conf is in an invalid format.  Aborting.\n");
		return -1;
	}
		...
		if (!strcasecmp(cat, "general") ) {
			...
			while (var) {
				if (!strcasecmp(var->name, "initial_silence")) {
					...
					ast_log_chan(NULL, LOG_WARNING, "%s: Cat:%s. Unknown keyword %s at line %d of amd.conf\n",
	...
	ast_verb_chan(NULL, 5, "AMD defaults: initialSilence [%d] greeting [%d] afterGreetingSilence [%d] "
		"totalAnalysisTime [%d] minimumWordLength [%d] betweenWordsSilence [%d] maximumNumberOfWords [%d] silenceThreshold [%d] maximumWordLength [%d]\n",
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_DEFAULT, "Answering Machine Detection Application",
