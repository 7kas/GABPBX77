...
/*** DOCUMENTATION
	<application name="AudioSocket" language="en_US">
		...
		<description>
			...
			<para>Protocol is specified at https://docs.gabpbx.org/Configuration/Channel-Drivers/AudioSocket/</para>
...
static int audiosocket_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(args.idStr)) {
		ast_log_chan(NULL, LOG_ERROR, "UUID is required\n");
		return -1;
	}
	...
	if (ast_set_write_format(chan, ast_format_slin)) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to set write format to SLINEAR for channel %s\n", chanName);
	...
	if (ast_set_read_format(chan, ast_format_slin)) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to set read format to SLINEAR for channel %s\n", chanName);
	...
	if (res) {
		...
		if (ast_set_read_format(chan, readFormat)) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to restore read format for channel %s\n", chanName);
		}
		ao2_ref(writeFormat, -1);
		ao2_ref(readFormat, -1);
		close(s);
...
static int audiosocket_run(struct ast_channel *chan, const char *id, int svc)
{
	...
	if (!chan || ast_channel_state(chan) != AST_STATE_UP) {
		ast_log_chan(NULL, LOG_ERROR, "Channel is %s\n", chan ? "not answered" : "missing");
		return -1;
	}

	if (ast_audiosocket_init(svc, id)) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to intialize AudioSocket\n");
		return -1;
	}
	...
	while (1) {
		...
		if (targetChan) {
			...
			if (f->frametype == AST_FRAME_VOICE) {
				...
				if (ast_audiosocket_send_frame(svc, f)) {
					ast_log_chan(NULL, LOG_ERROR, "Failed to forward channel frame from %s to AudioSocket\n",
						chanName);
					ast_frfree(f);
					return -1;
				}
		...
		if (outfd >= 0) {
			...
			if (!f) {
				ast_log_chan(NULL, LOG_ERROR, "Failed to receive frame from AudioSocket message for"
					"channel %s\n", chanName);
				return -1;
			}
			if (ast_write(chan, f)) {
				ast_log_chan(NULL, LOG_WARNING, "Failed to forward frame to channel %s\n", chanName);
				ast_frfree(f);
				return -1;
			}
			ast_frfree(f);
