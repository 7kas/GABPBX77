...
static int auth_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "Authenticate requires an argument(password)\n");
		return -1;
	}
	...
	for (retries = 0; retries < 3; retries++) {
		...
		if (arglist.password[0] != '/') {
			...
			if (!(f = fopen(arglist.password, "r"))) {
				ast_log_chan(NULL, LOG_WARNING, "Unable to open file '%s' for authentication: %s\n", arglist.password, strerror(errno));
				continue;
			}
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Authentication Application");
