	...
	<application name="BlindTransfer" language="en_US">
		...
		<syntax>
			...
			<parameter name="context">
				...
				By default, GABpbx will use the caller channel context.</para>
			</parameter>
		</syntax>
		<description>
...
static int blind_transfer_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero((char *)data)) {
		ast_log_chan(NULL, LOG_WARNING, "%s requires an argument (exten)\n", app);
...
AST_MODULE_INFO_STANDARD_EXTENDED(GABPBX_GPL_KEY, "Blind transfer channel to the given destination");
