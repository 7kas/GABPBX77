...
{
	ast_verb_chan(NULL, 3, "Channel %s timed out.\n", ast_channel_name(bridge_channel->chan));
...
static int apply_option_timeout(struct ast_bridge_features *features, char *duration_arg)
{
	...
	if (ast_strlen_zero(duration_arg)) {
		ast_log_chan(NULL, LOG_ERROR, "Timeout option 'S': No value provided.\n");
		return -1;
	}
	if (sscanf(duration_arg, "%u", &duration) != 1 || duration == 0) {
		ast_log_chan(NULL, LOG_ERROR, "Timeout option 'S': Invalid value provided '%s'.\n",
			duration_arg);
		return -1;
	}
	...
	if (ast_bridge_interval_hook(features, 0, duration, bridgewait_timeout_callback,
		...
		ast_log_chan(NULL, LOG_ERROR, "Timeout option 'S': Could not create timer.\n");
		return -1;
	}
...
static int apply_option_entertainment(struct ast_channel *chan, const char *entertainment_arg)
{
	...
	default:
		ast_log_chan(NULL, LOG_ERROR, "Invalid argument for BridgeWait entertainment '%s'\n", entertainment_arg);
		return -1;
	}
}
...
static int bridgewait_exec(struct ast_channel *chan, const char *data)
{
	...
	if (!ast_strlen_zero(args.role)) {
		...
		if (role == ROLE_INVALID) {
			ast_log_chan(NULL, LOG_ERROR, "Requested waiting bridge role '%s' is invalid.\n", args.role);
			return -1;
		}
	}
	...
	if (ast_bridge_features_init(&chan_features)) {
		...
		ast_log_chan(NULL, LOG_ERROR, "'%s' failed to enter the waiting bridge - could not set up channel features\n",
	...
	if (!bridge_wrapper) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to find or create waiting bridge '%s' for '%s'.\n", bridge_name, ast_channel_name(chan));
		ast_bridge_features_cleanup(&chan_features);
		return -1;
	}

	ast_verb_chan(NULL, 3, "%s is entering waiting bridge %s:%s\n", ast_channel_name(chan), bridge_name, bridge_wrapper->bridge->uniqueid);
	...
	if (res) {
		 ...
		 */
		ast_log_chan(NULL, LOG_WARNING, "Failed to join waiting bridge '%s' for '%s'.\n", bridge_name, ast_channel_name(chan));
	}
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Place the channel into a holding bridge application");
