...
#include "gabpbx/cli.h" /* use ESS macro */
	...
	struct ast_audiohook *bridge_whisper_audiohook, const char *spyer_name, const char *name, struct ast_flags *flags)
{
	...
	if (!bridged) {
		...
		if (ast_test_flag(flags, OPTION_ANSWER_WARN) && ast_channel_is_bridged(spyee_chan)) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Barge failed: channel is bridged, but not to a 2-party bridge. Use the 'r' option.\n");
		}
		return -1;
	}
...
static int do_broadcast(struct ast_channel *chan, struct ast_flags *flags, const char *channels)
{
	...
	while ((next = strsep(&chansdup, ","))) {
		...
		if (!strcmp(next, ast_channel_name(chan))) {
			ast_log_chan(NULL, LOG_WARNING, "Refusing to broadcast to ourself: %s\n", next);
			continue;
		}
		ochan = ast_channel_get_by_name(next);
		if (!ochan) {
			ast_log_chan(NULL, LOG_WARNING, "No such channel: %s\n", next);
			continue;
		}
		/* Append to end of list. */
		if (!(mac = ast_calloc(1, sizeof(*mac)))) {
			ast_log_chan(NULL, LOG_WARNING, "Multi autochan allocation failure\n");
			continue;
		}
		...
		if (ast_test_flag(flags, OPTION_WHISPER)) {
			...
			if (start_whispering(mac->autochan, next, &mac->whisper_audiohook, flags)) {
				ast_log_chan(NULL, LOG_WARNING, "Unable to attach whisper audiohook to %s\n", next);
				multi_autochan_free(mac);
				continue;
			}
		}
		if (ast_test_flag(flags, OPTION_SPY)) {
			...
			if (start_spying(mac->autochan, next, &mac->spy_audiohook, flags)) {
				ast_log_chan(NULL, LOG_WARNING, "Unable to attach spy audiohook to %s\n", next);
				multi_autochan_free(mac);
				continue;
			}
		}
	...
	ast_verb_chan(NULL, 4, "Broadcasting to %d channel%s on %s\n", numchans, ESS(numchans), ast_channel_name(chan));
	...
	while (numchans && ast_waitfor(chan, -1) > 0) {
		...
		AST_RWLIST_TRAVERSE_SAFE_BEGIN(&chanlist, mac, entry) {
			...
			if (fres) {
				ast_log_chan(NULL, LOG_WARNING, "Failed to write to audiohook for %s\n", mac->name);
				fres = 0;
			}
		}
		AST_RWLIST_TRAVERSE_SAFE_END;
...
static int broadcast_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "Broadcast requires at least one channel\n");
		return -1;
	}
	...
	if (ast_strlen_zero(args.channels)) {
		ast_log_chan(NULL, LOG_WARNING, "Must specify at least one channel for broadcast\n");
		return -1;
	}
	if (args.options) {
	...
	if (!ast_test_flag(&flags, OPTION_BARGE) && !ast_test_flag(&flags, OPTION_SPY) && !ast_test_flag(&flags, OPTION_WHISPER)) {
		ast_log_chan(NULL, LOG_WARNING, "At least one of the b, s, or w option must be specified (provided options have no effect)\n");
		return -1;
	}

	write_format = ao2_bump(ast_channel_writeformat(chan));
	if (ast_set_write_format(chan, ast_format_slin) < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to set write format to slin.\n");
		goto cleanup;
	}
	...
	if (ast_set_write_format(chan, write_format)) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to restore write format for channel %s\n", ast_channel_name(chan));
	}
...
AST_MODULE_INFO_STANDARD_EXTENDED(GABPBX_GPL_KEY, "Channel Audio Broadcasting");
