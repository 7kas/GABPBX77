	...
	while ((tech = strsep(&rest, "&"))) {
		...
		if (!number) {
			if (!ast_strlen_zero(tech)) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid ChanIsAvail technology/resource argument: '%s'\n",
					tech);
			}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_DEFAULT, "Check channel availability",
