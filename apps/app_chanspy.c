...
/*** DOCUMENTATION
	<application name="ChanSpy" language="en_US">
		...
		<description>
			<para>This application is used to listen to the audio from an GABpbx channel. This includes the audio
	...
	<application name="ExtenSpy" language="en_US">
		...
		<description>
			<para>This application is used to listen to the audio from an GABpbx channel. This includes
...
static int start_spying(struct ast_autochan *autochan, const char *spychan_name, struct ast_audiohook *audiohook, struct ast_flags *flags)
{
	...
	ast_verb_chan(NULL, 3, "Attaching spy channel %s to %s\n",
									...
									int start)
{
	...
	if (!spyer) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Attempt to publish ChanSpy message for NULL spyer channel\n");
		return;
	}
	blob = ast_json_null();
	if (!blob || !type) {
	...
	const char *spyer_name, const char *name, struct ast_flags *flags)
{
	...
	if (start_whispering(internal_bridge_autochan, spyer_name, bridge_whisper_audiohook, flags)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to attach barge audiohook on spyee '%s'. Barge mode disabled.\n", name);
		retval = -1;
	}
	...
	char *exitcontext)
{
	...
	ast_verb_chan(NULL, 2, "Spying on channel %s\n", name);
	...
	if (ast_test_flag(flags, OPTION_WHISPER | OPTION_BARGE | OPTION_DTMF_SWITCH_MODES)) {
		...
		*/
		...
		if (start_whispering(spyee_autochan, spyer_name, &csth.whisper_audiohook, flags)) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to attach whisper audiohook to spyee %s. Whisper mode disabled!\n", name);
		}
	}
	...
	*/
	while (ast_waitfor(chan, -1) > -1 && csth.spy_audiohook.status == AST_AUDIOHOOK_STATUS_RUNNING) {
		...
		if (res == user_options->cycle) {
			...
			if (*volfactor > 4)
				*volfactor = -4;
			ast_verb_chan(NULL, 3, "Setting spy volume on %s to %d\n", ast_channel_name(chan), *volfactor);
	...
	ast_verb_chan(NULL, 2, "Done Spying on channel %s\n", name);
...
static int chanspy_exec(struct ast_channel *chan, const char *data)
{
	...
	if (args.options) {
		...
		if (ast_test_flag(&flags, OPTION_DTMF_EXIT) && opts[OPT_ARG_EXIT]) {
			...
			if (strchr("0123456789*#", tmp) && tmp != '\0') {
				...
				ast_log_chan(NULL, LOG_NOTICE, "Argument for option 'x' must be a valid DTMF digit.\n");
			}
		}
		...
		if (ast_test_flag(&flags, OPTION_DTMF_CYCLE) && opts[OPT_ARG_CYCLE]) {
			...
			if (strchr("0123456789*#", tmp) && tmp != '\0') {
				...
				ast_log_chan(NULL, LOG_NOTICE, "Argument for option 'c' must be a valid DTMF digit.\n");
			}
		}
		...
		if (ast_test_flag(&flags, OPTION_VOLUME) && opts[OPT_ARG_VOLUME]) {
			...
			if ((sscanf(opts[OPT_ARG_VOLUME], "%30d", &vol) != 1) || (vol > 4) || (vol < -4))
				ast_log_chan(NULL, LOG_NOTICE, "Volume factor must be a number between -4 and 4\n");
			else
				volfactor = vol;
		}
	...
	if (ast_set_write_format(chan, ast_format_slin) < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Could Not Set Write Format.\n");
		return -1;
	}
	...
	if (recbase) {
		...
		if ((fd = open(filename, O_CREAT | O_WRONLY | O_TRUNC, AST_FILE_MODE)) <= 0) {
			ast_log_chan(NULL, LOG_WARNING, "Cannot open '%s' for recording\n", filename);
			fd = 0;
		}
	}
	...
	if (ast_test_flag(&flags, OPTION_EXITONHANGUP)) {
		ast_verb_chan(NULL, 3, "Stopped spying due to the spied-on channel hanging up.\n");
	}
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Listen to the audio of an active channel");
