...
static int conf_start_record(struct confbridge_conference *conference)
{
	...
	if (!mixmonapp) {
		ast_log_chan(NULL, LOG_WARNING, "Cannot record ConfBridge, MixMonitor app is not installed\n");
		return -1;
	}
		     ...
		     const char *filename)
{
	...
	if (digit < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to playback file '%s' to channel\n", filename);
		return -1;
	}
...
void confbridge_handle_atxfer(struct ast_attended_transfer_message *msg)
{
	...
	if (msg->to_transferee.channel_snapshot
		...
		ast_log_chan(NULL, LOG_ERROR, "Could not determine proper channels\n");
		return;
	}
	...
	if (ast_strlen_zero(old_snapshot->dialplan->data)) {
		ast_log_chan(NULL, LOG_ERROR, "Channel '%s' didn't have app data set\n", old_snapshot->base->name);
		return;
	}
	...
	if (!conference) {
		ast_log_chan(NULL, LOG_ERROR, "Conference bridge '%s' not found\n", confbr_name);
		return;
	}
	ao2_lock(conference);
	...
	if (!found_user) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to find user profile for channel '%s' in bridge '%s'\n",
...
static struct confbridge_conference *join_conference_bridge(const char *conference_name, struct confbridge_user *user)
{
	...
	if (!conference) {
		...
		if (!(conference = ao2_alloc(sizeof(*conference), destroy_conference_bridge))) {
			ao2_unlock(conference_bridges);
			ast_log_chan(NULL, LOG_ERROR, "Conference '%s' could not be created.\n", conference_name);
			return NULL;
		}
		...
		if (!conference->bridge) {
			ao2_ref(conference, -1);
			ao2_unlock(conference_bridges);
			ast_log_chan(NULL, LOG_ERROR, "Conference '%s' mixing bridge could not be created.\n", conference_name);
			return NULL;
		}
		...
		if (alloc_playback_chan(conference)) {
			...
			ao2_ref(conference, -1);
			ao2_unlock(conference_bridges);
			ast_log_chan(NULL, LOG_ERROR, "Could not allocate announcer channel for conference '%s'\n", conference_name);
			return NULL;
		}
		...
		if (push_announcer(conference)) {
			...
			ao2_ref(conference, -1);
			ao2_unlock(conference_bridges);
			ast_log_chan(NULL, LOG_ERROR, "Could not add announcer channel for conference '%s' bridge\n", conference_name);
			return NULL;
		}
...
static int play_sound_helper(struct confbridge_conference *conference, const char *filename, int say_number)
{
	...
	if (ast_taskprocessor_push(conference->playback_queue, playback_task, &ptd)) {
		if (!ast_strlen_zero(filename)) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to play file '%s' to conference %s\n",
				filename, conference->name);
		} else {
			ast_log_chan(NULL, LOG_WARNING, "Unable to say number '%d' to conference %s\n",
...
static int async_delete_name_rec_task(void *data)
{
	...
	ast_log_chan(NULL, LOG_DEBUG, "Conference '%s' removed user name file '%s'\n",
	...
	const char *filename)
{
	...
	if (ast_taskprocessor_push(conference->playback_queue, async_delete_name_rec_task, atd)) {
		ast_log_chan(NULL, LOG_WARNING, "Conference '%s' was unable to remove user name file '%s'\n",
...
static int confbridge_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(args.conf_name)) {
		pbx_builtin_setvar_helper(chan, "CONFBRIDGE_RESULT", "FAILED");
		ast_log_chan(NULL, LOG_WARNING, "%s requires an argument (conference name[,options])\n", app);
		res = -1;
		goto confbridge_cleanup;
	}
	...
	if (strlen(args.conf_name) >= MAX_CONF_NAME) {
		pbx_builtin_setvar_helper(chan, "CONFBRIDGE_RESULT", "FAILED");
		ast_log_chan(NULL, LOG_WARNING, "%s does not accept conference names longer than %d\n", app, MAX_CONF_NAME - 1);
		res = -1;
		goto confbridge_cleanup;
	}
	...
	if (!conf_find_bridge_profile(chan, b_profile_name, &user.b_profile)) {
		pbx_builtin_setvar_helper(chan, "CONFBRIDGE_RESULT", "FAILED");
		ast_log_chan(NULL, LOG_WARNING, "Conference bridge profile %s does not exist\n", b_profile_name ?
	...
	if (!conf_find_user_profile(chan, u_profile_name, &user.u_profile)) {
		pbx_builtin_setvar_helper(chan, "CONFBRIDGE_RESULT", "FAILED");
		ast_log_chan(NULL, LOG_WARNING, "Conference user profile %s does not exist\n", u_profile_name ?
	...
	if (conf_set_menu_to_user(chan, &user, menu_profile_name)) {
		pbx_builtin_setvar_helper(chan, "CONFBRIDGE_RESULT", "FAILED");
		ast_log_chan(NULL, LOG_WARNING, "Conference menu profile %s does not exist\n", menu_profile_name ?
	...
	if (res) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Couldn't add bridge join hook for channel '%s'\n", ast_channel_name(chan));
		goto confbridge_cleanup;
	}
	...
	if (res) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Couldn't add bridge leave hook for channel '%s'\n", ast_channel_name(chan));
		goto confbridge_cleanup;
	}
	...
	struct confbridge_user *user)
{
	...
	if (!isadmin) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Only admin users can use the kick_last menu action. Channel %s of conf %s is not an admin.\n",
...
static int func_confbridge_channels(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	if (ast_strlen_zero(args.confno) || ast_strlen_zero(args.type)) {
		ast_log_chan(NULL, LOG_WARNING, "Usage: %s(category,confno)", cmd);
		return -1;
	}
	...
	if (!strcasecmp(args.type, "parties")) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Invalid keyword '%s' passed to %s.\n", args.type, cmd);
	}
	ao2_unlock(conference);
	ao2_ref(conference, -1);
	return 0;
...
static int func_confbridge_info(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	if (!strcasecmp(args.type, "parties")) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Invalid keyword '%s' passed to CONFBRIDGE_INFO.\n", args.type);
	}
...
static int confkick_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "No conference bridge specified.\n");
	...
	if (!conference) {
		ast_log_chan(NULL, LOG_WARNING, "No conference bridge named '%s' found!\n", args.confbridge);
		pbx_builtin_setvar_helper(chan, "CONFKICKSTATUS", "FAILURE");
		return 0;
	}
	...
	if (not_found) {
		if (ast_strlen_zero(args.channel) || !strcasecmp("all", args.channel) || !strcasecmp("participants", args.channel)) {
			ast_log_chan(NULL, LOG_WARNING, "No participants found in conference bridge '%s'!\n", args.confbridge);
		} else {
			ast_log_chan(NULL, LOG_WARNING, "No participant named '%s' found in conference bridge '%s'!\n", args.channel, args.confbridge);
		}
		pbx_builtin_setvar_helper(chan, "CONFKICKSTATUS", "FAILURE");
		return 0;
	}
...
static int register_channel_tech(struct ast_channel_tech *tech)
{
	...
	if (ast_channel_register(tech)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to register channel technology %s(%s).\n",
...
static int load_module(void)
{
	...
	if (conf_load_config()) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to load config. Not loading module.\n");
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "Conference Bridge Application",
