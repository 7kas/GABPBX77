...
/*** DOCUMENTATION
	<application name="DBdeltree" language="en_US">
		...
		<synopsis>
			Delete a family or keytree from the gabpbx database.
		</synopsis>
		<syntax argsep="/">
...
static int deltree_exec(struct ast_channel *chan, const char *data)
{
	...
	if (keytree) {
		ast_verb_chan(NULL, 3, "DBdeltree: family=%s, keytree=%s\n", family, keytree);
	} else {
		ast_verb_chan(NULL, 3, "DBdeltree: family=%s\n", family);
	}

	if (ast_db_deltree(family, keytree) < 0) {
		ast_verb_chan(NULL, 3, "DBdeltree: Error deleting key from database.\n");
	}
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Database Access Functions");
