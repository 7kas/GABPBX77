	...
	struct ast_party_id *forced_clid, struct ast_party_id *stored_clid)
{
	...
	ast_verb_chan(in, 3, "Now forwarding %s to '%s/%s' (thanks to %s)\n", ast_channel_name(in), tech, stuff, ast_channel_name(c));
	/* If we have been told to ignore forwards, just set this channel to null and continue processing extensions normally */
	if (ast_test_flag64(peerflags, OPT_IGNORE_FORWARDING)) {
		ast_verb_chan(in, 3, "Forwarding %s to '%s/%s' prevented.\n", ast_channel_name(in), tech, stuff);
		...
		if (c) {
			...
			ast_verb_chan(in, 3, "Not accepting call completion offers from call-forward recipient %s\n",
				ast_channel_name(o->chan));
		} else
			ast_log_chan(in, LOG_NOTICE,
	...
	if (!c) {
		...
		if (ast_call(c, stuff, 0)) {
			ast_log_chan(in, LOG_NOTICE, "Forwarding failed to dial '%s/%s'\n",
				tech, stuff);
	...
	while ((*to_answer = ast_remaining_ms(start, orig_answer_to)) && (*to_progress = ast_remaining_ms(start, orig_progress_to)) && !peer) {
		...
		if (pos == 1) { /* only the input channel is available */
			if (numlines == (num.busy + num.congestion + num.nochan)) {
				ast_verb_chan(in, 2, "Everyone is busy/congested at this time (%d:%d/%d/%d)\n", numlines, num.busy, num.congestion, num.nochan);
				if (num.busy)
				...
				ast_verb_chan(in, 3, "No one is available to answer at this time (%d:%d/%d/%d)\n", numlines, num.busy, num.congestion, num.nochan);
			}
			*to_answer = 0;
			if (is_cc_recall) {
		...
		AST_LIST_TRAVERSE(out_chans, o, node) {
			...
			if (ast_test_flag64(o, DIAL_STILLGOING) && ast_channel_state(c) == AST_STATE_UP) {
				if (!peer) {
					ast_verb_chan(in, 3, "%s answered %s\n", ast_channel_name(c), ast_channel_name(in));
					if (o->orig_chan_name
			...
			case AST_FRAME_CONTROL:
				...
				case AST_CONTROL_CONGESTION:
					ast_verb_chan(in, 3, "%s is circuit-busy\n", ast_channel_name(c));
					ast_channel_hangupcause_set(in, ast_channel_hangupcause(c));
				...
				case AST_CONTROL_RINGING:
					...
					*to_progress = -1;
					...
					if (ignore_cc || cc_frame_received || num_ringing == numlines) {
						ast_verb_chan(in, 3, "%s is ringing\n", ast_channel_name(c));
				...
				case AST_CONTROL_PROGRESS:
					ast_verb_chan(in, 3, "%s is making progress passing it to %s\n", ast_channel_name(c), ast_channel_name(in));
					/* Setup early media if appropriate */
					if (single && !caller_entertained
						&& CAN_EARLY_BRIDGE(peerflags, in, c)) {
						ast_channel_early_bridge(in, c);
					...
					*to_progress = -1;
					...
					if (!sent_progress) {
						...
						if (res) {
							ast_log_chan(in, LOG_WARNING, "Called channel %s hung up post-progress before all digits could be sent\n", ast_channel_name(c));
							goto wait_over;
						}
					}
				...
				case AST_CONTROL_WINK:
					ast_verb_chan(in, 3, "%s winked, passing it to %s\n", ast_channel_name(c), ast_channel_name(in));
					if (!sent_wink) {
						sent_wink = 1;
						...
						if (res) {
							ast_log_chan(in, LOG_WARNING, "Called channel %s hung up post-wink before all digits could be sent\n", ast_channel_name(c));
							goto wait_over;
						}
					}
				...
				case AST_CONTROL_SRCCHANGE:
					...
					ast_verb_chan(in, 3, "%s requested media update control %d, passing it to %s\n",
				...
				case AST_CONTROL_CONNECTED_LINE:
					if (ast_test_flag64(o, OPT_IGNORE_CONNECTEDLINE)) {
						ast_verb_chan(in, 3, "Connected line update to %s prevented.\n", ast_channel_name(in));
						break;
					}
					if (!single) {
						struct ast_party_connected_line connected;

						ast_verb_chan(in, 3, "%s connected line has changed. Saving it until answer for %s\n",
							ast_channel_name(c), ast_channel_name(in));
				...
				case AST_CONTROL_REDIRECTING:
					...
					if (ast_test_flag64(o, OPT_IGNORE_CONNECTEDLINE)) {
						ast_verb_chan(in, 3, "Redirecting update to %s prevented.\n", ast_channel_name(in));
						break;
					}
					ast_verb_chan(in, 3, "%s redirecting info has changed, passing it to %s\n",
						ast_channel_name(c), ast_channel_name(in));
				...
				case AST_CONTROL_PROCEEDING:
					ast_verb_chan(in, 3, "%s is proceeding passing it to %s\n", ast_channel_name(c), ast_channel_name(in));
					if (single && !caller_entertained
						&& CAN_EARLY_BRIDGE(peerflags, in, c)) {
						ast_channel_early_bridge(in, c);
					}
				...
				case AST_CONTROL_HOLD:
					...
					ast_verb_chan(in, 3, "Call on %s placed on hold\n", ast_channel_name(c));
					ast_indicate_data(in, AST_CONTROL_HOLD, f->data.ptr, f->datalen);
					break;
				case AST_CONTROL_UNHOLD:
					/* XXX this should be saved like AST_CONTROL_CONNECTED_LINE for !single || caller_entertained */
					ast_verb_chan(in, 3, "Call on %s left from hold\n", ast_channel_name(c));
				...
				case -1:
					if (single && !caller_entertained) {
						ast_verb_chan(in, 3, "%s stopped sounds\n", ast_channel_name(c));
						ast_indicate(in, -1);
						pa->sentringing = 0;
					}
					break;
			...
			case AST_FRAME_TEXT:
				if (single && ast_write(in, f)) {
					ast_log_chan(in, LOG_WARNING, "Unable to write frametype: %u\n",
						f->frametype);
				}
				break;
			case AST_FRAME_HTML:
				if (single && !ast_test_flag64(outgoing, DIAL_NOFORWARDHTML)
					...
					ast_log_chan(in, LOG_WARNING, "Unable to send URL\n");
				}
				break;
			default:
				break;
			...
			if (f->frametype == AST_FRAME_DTMF) {
				if (ast_test_flag64(peerflags, OPT_DTMF_EXIT)) {
					...
					if (onedigit_goto(in, context, (char) f->subclass.integer, 1)) {
						ast_verb_chan(in, 3, "User hit %c to disconnect call.\n", f->subclass.integer);
						*to_answer = 0;
				...
				if (ast_test_flag64(peerflags, OPT_CALLER_HANGUP) &&
					...
					ast_verb_chan(in, 3, "User requested call disconnect.\n");
					*to_answer = 0;
					strcpy(pa->status, "CANCEL");
					pa->canceled = 1;
					publish_dial_end_event(in, out_chans, NULL, pa->status);
			...
			AST_LIST_TRAVERSE(out_chans, o, node) {
				...
				case AST_FRAME_DTMF_END:
					if (ast_write(o->chan, f)) {
						ast_log_chan(in, LOG_WARNING, "Unable to forward frametype: %u\n",
							f->frametype);
					}
					break;
				case AST_FRAME_CONTROL:
					...
					case AST_CONTROL_UNHOLD:
						...
						break;
					case AST_CONTROL_FLASH:
						ast_verb_chan(in, 3, "Hook flash on %s\n", ast_channel_name(o->chan));
...
wait_over:
	if (!*to_answer || ast_check_hangup(in)) {
		ast_verb_chan(in, 3, "Nobody picked up in %d ms\n", orig_answer_to);
		publish_dial_end_event(in, out_chans, NULL, "NOANSWER");
	} else if (!*to_progress) {
		ast_verb_chan(in, 3, "No early media received in %d ms\n", orig_progress_to);
	...
	struct ast_flags64 *opts, char **opt_args, struct privacy_args *pa)
{
	...
	if (ast_test_flag64(opts, OPT_PRIVACY) && (res2 >= '1' && res2 <= '5')) {
		...
		int i = res2 - '1';
		ast_verb_chan(chan, 3, "--Set privacy database entry %s/%s to %s\n",
	...
	default: /* bad input or -1 if failure to start autoservice */
		...
		ast_verb_chan(chan, 3, "privacy: no valid response from the callee. Sending the caller to voicemail, the callee isn't responding\n");
	...
	if (res2 == '1') { /* the only case where we actually connect */
		...
		if (strncmp(pa->privcid, "NOCALLERID", 10) == 0 || ast_test_flag64(opts, OPT_SCREEN_NOINTRO)) {
			...
			if (ast_fileexists(pa->privintro, NULL, NULL) > 0)
				ast_log_chan(chan, LOG_NOTICE, "privacy: ast_filedelete didn't do its job on %s\n", pa->privintro);
			else
				ast_verb_chan(chan, 3, "Successfully deleted %s intro file\n", pa->privintro);
		}
	...
	struct ast_flags64 *opts, char *opt_args[], struct ast_channel *chan)
{
	...
	if (ast_channel_caller(chan)->id.number.valid
		...
		if (ast_test_flag64(opts, OPT_PRIVACY) ) {
			ast_verb_chan(chan, 3, "Privacy DB is '%s', clid is '%s'\n", opt_args[OPT_ARG_PRIVACY], l);
			pa->privdb_val = ast_privacy_check(opt_args[OPT_ARG_PRIVACY], l);
		} else {
			ast_verb_chan(chan, 3, "Privacy Screening, clid is '%s'\n", l);
		...
		ast_verb_chan(chan, 3, "Privacy-- callerid is empty\n");
	...
	if (strncmp(pa->privcid, "NOCALLERID", 10) != 0 && ast_test_flag64(opts, OPT_SCREEN_NOCALLERID)) {
		...
		ast_verb_chan(NULL, 3, "CallerID set (%s); N option set; Screening should be off\n", pa->privcid);
		pa->privdb_val = AST_PRIVACY_ALLOW;
	} else if (ast_test_flag64(opts, OPT_SCREEN_NOCALLERID) && strncmp(pa->privcid, "NOCALLERID", 10) == 0) {
		ast_verb_chan(NULL, 3, "CallerID blank; N option set; Screening should happen; dbval is %d\n", pa->privdb_val);
	}

	if (pa->privdb_val == AST_PRIVACY_DENY) {
		ast_verb_chan(NULL, 3, "Privacy DB reports PRIVACY_DENY for this callerid. Dial reports unavailable\n");
		ast_copy_string(pa->status, "NOANSWER", sizeof(pa->status));
		return 0;
		...
		if ((res = ast_mkdir(pa->privintro, 0755))) {
			ast_log_chan(NULL, LOG_WARNING, "privacy: can't create directory priv-callerintros: %s\n", strerror(res));
			return -1;
		}
		...
		if (ast_fileexists(pa->privintro, NULL, NULL ) > 0 && strncmp(pa->privcid, "NOCALLERID", 10) != 0) {
			...
			*/
			...
			if (res == -1) {
				...
				ast_filedelete(pa->privintro, NULL);
				if (ast_fileexists(pa->privintro, NULL, NULL) > 0)
					ast_log_chan(NULL, LOG_NOTICE, "privacy: ast_filedelete didn't do its job on %s\n", pa->privintro);
				else
					ast_verb_chan(NULL, 3, "Successfully deleted %s intro file\n", pa->privintro);
				return -1;
			}
...
static int dial_handle_playtones(struct ast_channel *chan, const char *data)
{
	...
	if (res) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to start playtone \'%s\'\n", str);
	}
...
static int dial_exec_full(struct ast_channel *chan, const char *data, struct ast_flags64 *peerflags, int *continue_exec)
{
	...
	if (max_forwards <= 0) {
		ast_log_chan(chan, LOG_WARNING, "Cannot place outbound call from channel '%s'. Max forwards exceeded\n",
	...
	if (ast_check_hangup_locked(chan)) {
		 ...
		 */
		ast_verb_chan(chan, 3, "Caller hung up before dial.\n");
	...
	if (ast_test_flag64(&opts, OPT_SCREEN_NOINTRO) && !ast_strlen_zero(opt_args[OPT_ARG_SCREEN_NOINTRO])) {
		...
		if (delprivintro < 0 || delprivintro > 1) {
			ast_log_chan(chan, LOG_WARNING, "Unknown argument %d specified to n option, ignoring\n", delprivintro);
			delprivintro = 0;
		}
	}
	...
	if (ast_test_flag64(&opts, OPT_OPERMODE)) {
		...
		ast_verb_chan(chan, 3, "Setting operator services mode to %d.\n", opermode);
	}
	...
	if (ast_test_flag64(&opts, OPT_DURATION_STOP) && !ast_strlen_zero(opt_args[OPT_ARG_DURATION_STOP])) {
		...
		if (!calldurationlimit.tv_sec) {
			ast_log_chan(chan, LOG_WARNING, "Dial does not accept S(%s)\n", opt_args[OPT_ARG_DURATION_STOP]);
			pbx_builtin_setvar_helper(chan, "DIALSTATUS", pa.status);
			goto done;
		}
		ast_verb_chan(chan, 3, "Setting call duration limit to %.3lf seconds.\n", calldurationlimit.tv_sec + calldurationlimit.tv_usec / 1000000.0);
	}
	...
	while ((cur = strsep(&rest, "&"))) {
		...
		if (ast_strlen_zero(number)) {
			ast_log_chan(chan, LOG_WARNING, "Dial argument takes format (technology/resource)\n");
			goto out;
		}
		...
		if (!tc) {
			...
			ast_log_chan(chan, LOG_NOTICE, "Unable to create channel of type '%s' (cause %d - %s)\n",
	...
	if (!num_dialed) {
		ast_verb_chan(chan, 3, "No devices or endpoints to dial (technology/resource)\n");
		if (continue_exec) {
	...
	AST_LIST_TRAVERSE_SAFE_BEGIN(&out_chans, tmp, node) {
		...
		if (res) {
			...
			ast_verb_chan(chan, 3, "Couldn't call %s\n", tmp->interface);
	...
	if (ast_strlen_zero(args.timeout)) {
		...
		if (!ast_strlen_zero(anstimeout)) {
			...
			if (to_answer > 0) {
				...
				ast_log_chan(chan, LOG_WARNING, "Invalid answer timeout specified: '%s'. Setting timeout to infinite\n", args.timeout);
				to_answer = -1;
			}
		} else {
			to_answer = -1;
		...
		if (!ast_strlen_zero(args.timeout)) {
			...
			if (to_progress > 0) {
				...
				ast_log_chan(chan, LOG_WARNING, "Invalid progress timeout specified: '%s'. Setting timeout to infinite\n", args.timeout);
				to_progress = -1;
			}
		} else {
			to_progress = -1;
	...
	if (!peer) {
		...
		if (ast_test_flag64(&opts, OPT_HANGUPCAUSE)
			...
			if (cause <= 0) {
				if (!strcasecmp(opt_args[OPT_ARG_HANGUPCAUSE], "NONE")) {
					...
					|| cause < 0) {
					ast_log_chan(chan, LOG_WARNING, "Invalid cause given to Dial(...Q(<cause>)): \"%s\"\n",
		...
		if (!ast_test_flag64(&opts, OPT_ANNOUNCE) || ast_strlen_zero(opt_args[OPT_ARG_ANNOUNCE])) {
			...
			if (!ast_strlen_zero(calledfile)) {
				...
				if (res) {
					res = 0;
					ast_log_chan(chan, LOG_ERROR, "error streaming file '%s' to callee\n", calledfile);
				} else {
					calledstream = 1;
				}
			}
			if (!ast_strlen_zero(callerfile)) {
				...
				if (res) {
					res = 0;
					ast_log_chan(chan, LOG_ERROR, "error streaming file '%s' to caller\n", callerfile);
				} else {
					callerstream = 1;
				}
			}
		...
		if (ast_test_flag64(&opts, OPT_CALLEE_GOSUB) && !ast_strlen_zero(opt_args[OPT_ARG_CALLEE_GOSUB])) {
			...
			if (gosub_args) {
				...
				ast_log_chan(chan, LOG_ERROR, "Could not Allocate string for Gosub arguments -- Gosub Call Aborted!\n");
			}
		...
		if (!res) {
			...
			if (!ast_strlen_zero(dtmfcalled)) {
				ast_verb_chan(chan, 3, "Sending DTMF '%s' to the called party.\n", dtmfcalled);
				res = ast_dtmf_stream(peer, chan, dtmfcalled, 250, 0);
			}
			if (!ast_strlen_zero(dtmfcalling)) {
				ast_verb_chan(chan, 3, "Sending DTMF '%s' to the calling party.\n", dtmfcalling);
		...
		if (res) { /* some error */
			...
			if (res < 0) {
				ast_log_chan(chan, LOG_WARNING, "Had to drop call because I couldn't make %s compatible with %s\n", ast_channel_name(chan), ast_channel_name(peer));
				ast_autoservice_chan_hangup_peer(chan, peer);
				res = -1;
				goto done;
			}
...
static int retrydial_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(chan, LOG_WARNING, "RetryDial requires an argument!\n");
		return -1;
	}
	...
	if (!args.dialdata) {
		ast_log_chan(chan, LOG_ERROR, "%s requires a 4th argument (dialdata)\n", rapp);
		goto done;
	}
			...
			if (ast_test_flag64(&peerflags, OPT_DTMF_EXIT)) {
				if (!ast_strlen_zero(args.announce)) {
					if (ast_fileexists(args.announce, NULL, ast_channel_language(chan)) > 0) {
						...
						ast_log_chan(chan, LOG_WARNING, "Announce file \"%s\" specified in Retrydial does not exist\n", args.announce);
				}
				if (!res && sleepms) {
					if (!ast_test_flag(ast_channel_flags(chan), AST_FLAG_MOH))
						ast_moh_start(chan, NULL, NULL);
