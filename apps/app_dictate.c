...
/*! \file
 *
 * \brief Virtual Dictation Machine Application For GABpbx
 *
...
static int dictate_exec(struct ast_channel *chan, const char *data)
{
	...
	if ((res = ast_set_read_format(chan, ast_format_slin)) < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to set to linear mode.\n");
		ao2_cleanup(oldr);
		return -1;
	}
...
AST_MODULE_INFO_STANDARD_EXTENDED(GABPBX_GPL_KEY, "Virtual Dictation Machine");
