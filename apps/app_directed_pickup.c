...
static int pickup_by_exten(struct ast_channel *chan, const char *exten, const char *context)
{
	...
	while ((target = ast_channel_iterator_next(iter))) {
		...
		if ((chan != target) && ast_can_pickup(target)) {
			ast_log_chan(NULL, LOG_NOTICE, "%s pickup by %s\n", ast_channel_name(target), ast_channel_name(chan));
			break;
		}
		ast_channel_unlock(target);
		target = ast_channel_unref(target);
...
static int pickup_exec(struct ast_channel *chan, const char *data)
{
	...
	for (;;) {
		...
		ast_log_chan(NULL, LOG_NOTICE, "No target channel found for %s@%s.\n", exten, context);
	}
...
static int pickupchan_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(args.channel)) {
		ast_log_chan(NULL, LOG_WARNING, "PickupChan requires an argument (channel)!\n");
		/* Pickup failed.  Keep going in the dialplan. */
		return 0;
	}
	...
	for (;;) {
		...
		ast_log_chan(NULL, LOG_NOTICE, "No target channel found for %s.\n", pickup);
	}
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Directed Call Pickup Application");
