...
static int goto_exten(struct ast_channel *chan, const char *dialcontext, char *ext)
{
	if (!ast_goto_if_exists(chan, S_OR(dialcontext, ast_channel_context(chan)), ext, 1)) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Can't find extension '%s' in current context.  "
...
static struct ast_config *realtime_directory(char *context, const char *filename)
{
	...
	if (!cfg) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Loading config failed.\n");
		return NULL;
	...
	while ((category = ast_category_browse(rtdata, category))) {
		...
		if ((var = ast_variable_new(mailbox, ast_str_buffer(tmp), ""))) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Out of memory adding mailbox '%s'\n", mailbox);
		}
	}
	ast_config_destroy(rtdata);
...
static int do_directory(struct ast_channel *chan, struct ast_config *vmcfg, struct ast_config *ucfg, char *context, char *dialcontext, char digit, int digits, struct ast_flags *flags, char *opts[])
{
	...
	if (DEBUG_ATLEAST(2)) {
		ast_log_chan(NULL, LOG_DEBUG, "Listing matching entries:\n");
...
static int directory_exec(struct ast_channel *chan, const char *data)
{
	...
	if (!cfg) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to read the configuration data!\n");
		return -1;
	}
	...
	if (ast_test_flag(&flags, OPT_ADSI)) {
		if (!ast_adsi_available(chan)) {
			ast_log_chan(NULL, LOG_WARNING, "ADSI not available on %s\n", ast_channel_name(chan));
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Extension Directory");
