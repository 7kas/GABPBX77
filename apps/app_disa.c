...
static int disa_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "DISA requires an argument (passcode/passcode file)\n");
		return -1;
	}
			...
			if (!(k&1)) { /* if in password state */
				if (j == '#') { /* end of password */
					...
					if (sscanf(args.passcode,"%30d",&j) < 1) { /* nope, it must be a filename */
						...
						if (!fp) {
							ast_log_chan(NULL, LOG_WARNING,"DISA password file %s not found on chan %s\n",args.passcode,ast_channel_name(chan));
							ast_channel_clear_flag(chan, AST_FLAG_END_DTMF_ONLY);
							return -1;
						}
						pwline[0] = 0;
					...
					/* compare the two */
					if (strcmp(exten,args.passcode)) {
						ast_log_chan(NULL, LOG_WARNING,"DISA on chan %s got bad password %s\n",ast_channel_name(chan),exten);
						goto reorder;
	...
	if (k == 3) {
		...
		if (!recheck
			...
			if (ast_pbx_exec_application(chan, "ResetCDR", special_noanswer ? "" : "e")) {
				ast_log_chan(NULL, AST_LOG_NOTICE, "ResetCDR application not found; CDR will not be reset\n");
			}
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "DISA (Direct Inward System Access) Application");
