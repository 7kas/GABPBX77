...
static int remove_dtmf_store(struct ast_channel *chan)
{
	...
	if (!datastore) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Cannot remove StoreDTMF from %s: StoreDTMF not currently enabled\n",
	...
	if (ast_framehook_detach(chan, data->framehook_id)) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Failed to remove StoreDTMF framehook from channel %s\n",
		        ast_channel_name(chan));
		return -1;
	}

	if (ast_channel_datastore_remove(chan, datastore)) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Failed to remove StoreDTMF datastore from channel %s\n",
		        ast_channel_name(chan));
		return -1;
	}
...
static int dtmfstore_exec(struct ast_channel *chan, const char *appdata)
{
	...
	if (ast_strlen_zero(appdata)) {
		ast_log_chan(NULL, AST_LOG_WARNING, "StoreDTMF requires an argument\n");
		return -1;
	}
	...
	datastore = ast_channel_datastore_find(chan, &dtmf_store_datastore, NULL);
	if (datastore) {
		ast_log_chan(NULL, AST_LOG_WARNING, "StoreDTMF already set on '%s'\n",
		        ast_channel_name(chan));
		return 0;
	}
	...
	if (!strcasecmp(args.direction, "tx")) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Direction must be either RX or TX\n");
		return -1;
	}

	if (!ast_strlen_zero(args.maxdigits)) {
		if (ast_str_to_int(args.maxdigits,&(data->maxdigits))) {
			ast_log_chan(NULL, LOG_ERROR, "Invalid integer: %s\n", args.maxdigits);
			return -1;
		}
		if (data->maxdigits < 0) {
			ast_log_chan(NULL, LOG_ERROR, "Invalid natural number: %d\n", data->maxdigits);
			return -1;
		} else if (data->maxdigits == 0) {
			ast_log_chan(NULL, LOG_WARNING, "No maximum digit count set\n");
		}
	}

	data->framehook_id = ast_framehook_attach(chan, &digit_framehook_interface);
	if (data->framehook_id < 0) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Failed to attach StoreDTMF framehook to '%s'\n",
		        ast_channel_name(chan));
		ast_datastore_free(datastore);
		ast_free(data);
		return -1;
...
AST_MODULE_INFO_STANDARD_EXTENDED(GABPBX_GPL_KEY, "Technology independent async DTMF storage");
