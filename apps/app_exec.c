...
static int execif_exec(struct ast_channel *chan, const char *data)
{
	...
	if ((firstcomma != NULL && firstquestion != NULL && firstcomma < firstquestion) || (firstquestion == NULL)) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Deprecated syntax found.  Please upgrade to using ExecIf(<expr>?%s(%s))\n", depr.appname, depr.appargs);
		...
		if (ast_strlen_zero(expr.remainder)) {
			ast_log_chan(NULL, LOG_ERROR, "Usage: ExecIf(<expr>?<appiftrue>(<args>)[:<appiffalse>(<args)])\n");
			return -1;
		}
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Executes dialplan applications");
