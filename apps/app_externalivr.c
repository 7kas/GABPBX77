...
#define ast_chan_log(level, channel, format, ...) ast_log_chan(NULL, level, "%s: " format, ast_channel_name(channel) , ## __VA_ARGS__)
...
static int app_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_ERROR, "ExternalIVR requires a command to execute\n");
		goto exit;
	}
	...
	ast_verb_chan(NULL, 4, "ExternalIVR received application and arguments: %s\n", eivr_args.application);
	ast_verb_chan(NULL, 4, "ExternalIVR received options: %s\n", eivr_args.options);
	...
	if ((s = strchr(eivr_args.application, '('))) {
		...
		if ((e = strrchr(s, ')'))) {
			*e = '\0';
			...
			ast_log_chan(NULL, LOG_ERROR, "Parse error, missing closing parenthesis\n");
			goto exit;
		}
	}
	...
	ast_verb_chan(NULL, 4, "Parsing options from: [%s]\n", eivr_args.options);
	ast_app_parse_options(app_opts, &flags, opts, eivr_args.options);
	if (ast_test_flag(&flags, noanswer)) {
		ast_verb_chan(NULL, 4, "noanswer is set\n");
	}
	if (ast_test_flag(&flags, ignore_hangup)) {
		ast_verb_chan(NULL, 4, "ignore_hangup is set\n");
	}
	if (ast_test_flag(&flags, run_dead)) {
		ast_verb_chan(NULL, 4, "run_dead is set\n");
	}

	if (!(ast_test_flag(&flags, noanswer))) {
		ast_verb_chan(NULL, 3, "Answering channel and starting generator\n");
	...
	const struct ast_str *args, const struct ast_flags flags)
{
	...
	while (1) {
		...
		if (!hangup_info_sent && !(ast_test_flag(&flags, run_dead)) && ast_check_hangup(chan)) {
			if (ast_test_flag(&flags, ignore_hangup)) {
				ast_verb_chan(NULL, 3, "Got check_hangup, but ignore_hangup set so sending 'I' command\n");
				send_eivr_event(eivr_events, 'I', "HANGUP", chan);
				hangup_info_sent = 1;
			} else {
				ast_verb_chan(NULL, 3, "Got check_hangup\n");
		...
		if (ast_channel_state(chan) == AST_STATE_UP && !(ast_check_hangup(chan)) && rchan) {
			...
			f = ast_read(chan);
			if (!f) {
				ast_verb_chan(NULL, 3, "Returned no frame\n");
				send_eivr_event(eivr_events, 'H', NULL, chan);
				break;
			}
			if (f->frametype == AST_FRAME_DTMF) {
				...
				ast_verb_chan(NULL, 3, "Got AST_CONTROL_HANGUP\n");
				send_eivr_event(eivr_events, 'H', NULL, chan);
				if (f->data.uint32) {
			...
			ast_strip(input);
			ast_verb_chan(NULL, 4, "got command '%s'\n", input);
			...
			if (input[0] == EIVR_CMD_PARM) {
				...
				ast_verb_chan(NULL, 4, "Sending DTMF: %s\n", &input[2]);
				ast_eivr_senddtmf(chan, &input[2]);
			} else if (input[0] == EIVR_CMD_ANS) {
				ast_verb_chan(NULL, 3, "Answering channel if needed and starting generator\n");
				if (ast_channel_state(chan) != AST_STATE_UP) {
					if (ast_test_flag(&flags, run_dead)) {
				...
				char response[2048];
				ast_verb_chan(NULL, 4, "Retriving Variables from channel: %s\n", &input[2]);
				...
				ast_verb_chan(NULL, 4, "Setting Variables in channel: %s\n", &input[2]);
...
AST_MODULE_INFO_STANDARD_EXTENDED(GABPBX_GPL_KEY, "External IVR Interface Application");
