...
static int send_waveform_to_fd(char *waveform, int length, int fd)
{
	...
	res = ast_safe_fork(0);
	if (res < 0)
		ast_log_chan(NULL, LOG_WARNING, "Fork failed\n");
	if (res) {
		return res;
	}
	dup2(fd, 0);
...
static int send_waveform_to_channel(struct ast_channel *chan, char *waveform, int length, char *intkeys)
{
	...
	if (pipe(fds)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to create pipe\n");
		return -1;
	}
	...
	if (res < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to set write format to signed linear\n");
	...
	if (res >= 0) {
		...
		for (;;) {
			...
			f = ast_read(chan);
			if (!f) {
				ast_log_chan(NULL, LOG_WARNING, "Null frame == hangup() detected\n");
				res = -1;
				break;
			}
			...
			if (f->frametype == AST_FRAME_VOICE) {
				...
				if (needed > sizeof(myf.frdata)) {
					ast_log_chan(NULL, LOG_WARNING, "Only able to deliver %d of %d requested samples\n",
...
static int festival_exec(struct ast_channel *chan, const char *vdata)
{
	...
	if (ast_strlen_zero(vdata)) {
		ast_log_chan(NULL, LOG_WARNING, "festival requires an argument (text)\n");
		return -1;
	}

	cfg = ast_config_load(FESTIVAL_CONFIG, config_flags);
	if (!cfg) {
		ast_log_chan(NULL, LOG_WARNING, "No such configuration file %s\n", FESTIVAL_CONFIG);
		return -1;
	} else if (cfg == CONFIG_STATUS_FILEINVALID) {
		ast_log_chan(NULL, LOG_ERROR, "Config file " FESTIVAL_CONFIG " is in an invalid format.  Aborting.\n");
		return -1;
	}
	...
	if (!(festivalcommand = ast_variable_retrieve(cfg, "general", "festivalcommand"))) {
		const char *startcmd = "(tts_textgabpbx \"";
	...
	if (fd < 0) {
		ast_log_chan(NULL, LOG_WARNING, "festival_client: can't get socket\n");
	...
	if ((serv_addr.sin_addr.s_addr = inet_addr(host)) == -1) {
		...
		if (ast_sockaddr_resolve_first_af(&addr, host, PARSE_PORT_FORBID, AF_INET)) {
			ast_log_chan(NULL, LOG_WARNING, "festival_client: ast_sockaddr_resolve_first_af() failed\n");
			ast_config_destroy(cfg);
			close(fd);
			return -1;
		}
	...
	if (connect(fd, (struct sockaddr *)&serv_addr, sizeof(serv_addr)) != 0) {
		ast_log_chan(NULL, LOG_WARNING, "festival_client: connect to server failed\n");
		ast_config_destroy(cfg);
		close(fd);
		return -1;
	}
		...
		if (fdesc == -1) {
			...
			if (strlen(args.text) == strln) {
				...
				if (strcmp(bigstring, args.text) == 0) {
					...
					ast_log_chan(NULL, LOG_WARNING, "Strings do not match\n");
				}
			} else {
				ast_log_chan(NULL, LOG_WARNING, "Size mismatch\n");
			}
		}
	}
	...
	do {
		...
		for (n = 0; n < 3; ) {
			 ...
			 */
			if (read_data == -1) {
				ast_log_chan(NULL, LOG_WARNING, "Unable to read from cache/festival fd\n");
				close(fd);
				ast_config_destroy(cfg);
				return -1;
			}
		...
		if (strcmp(ack, "WV\n") == 0) {         /* receive a waveform */
			...
			if ((waveform = socket_receive_file_to_buff(fd, &filesize))) {
				waveform[filesize] = '\0';
				ast_log_chan(NULL, LOG_WARNING, "Festival returned LP : %s\n", waveform);
				ast_free(waveform);
			}
		} else if (strcmp(ack, "ER\n") == 0) {    /* server got an error */
			ast_log_chan(NULL, LOG_WARNING, "Festival returned ER\n");
			res = -1;
			break;
		}
	} while (strcmp(ack, "OK\n") != 0);
...
AST_MODULE_INFO_STANDARD_EXTENDED(GABPBX_GPL_KEY, "Simple Festival Interface");
