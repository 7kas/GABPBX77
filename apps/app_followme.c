		...
		<syntax>
			...
			<parameter name="options">
				<optionlist>
					...
					<option name="I">
						<para>GABpbx will ignore any connected line update requests
...
static void profile_set_param(struct call_followme *f, const char *param, const char *val, int linenum, int failunknown)
{
	...
	if (!strcasecmp(param, "musicclass") || !strcasecmp(param, "musiconhold") || !strcasecmp(param, "music"))
		...
		if (linenum >= 0)
			ast_log_chan(NULL, LOG_WARNING, "Unknown keyword in profile '%s': %s at line %d of followme.conf\n", f->name, param, linenum);
		else
			ast_log_chan(NULL, LOG_WARNING, "Unknown keyword in profile '%s': %s\n", f->name, param);
	}
}
...
static int reload_followme(int reload)
{
	...
	if (!(cfg = ast_config_load("followme.conf", config_flags))) {
		ast_log_chan(NULL, LOG_WARNING, "No follow me config file (followme.conf), so no follow me\n");
		return 0;
	} else if (cfg == CONFIG_STATUS_FILEUNCHANGED) {
		return 0;
	} else if (cfg == CONFIG_STATUS_FILEINVALID) {
		ast_log_chan(NULL, LOG_ERROR, "Config file followme.conf is in an invalid format.  Aborting.\n");
		return 0;
	}
	...
	for (;;) {
		...
		AST_LIST_TRAVERSE(findme_user_list, tmpuser, entry) {
			...
			if (tmpuser->digts && (tmpuser->digts > featuredigittimeout)) {
				ast_verb_chan(NULL, 3, "<%s> We've been waiting for digits longer than we should have.\n",
				...
				if (tpargs->enable_callee_prompt) {
					if (!ast_strlen_zero(tpargs->namerecloc)) {
						...
						if (!ast_streamfile(tmpuser->ochan, callfromname, ast_channel_language(tmpuser->ochan))) {
							...
							ast_log_chan(NULL, LOG_WARNING, "Unable to playback %s.\n", callfromname);
							clear_caller(tmpuser);
							continue;
						}
					} else {
			...
			if (ast_channel_stream(tmpuser->ochan)) {
				...
				if (tmpto > 0 && tmpto < to)
					...
					case 1:
						ast_verb_chan(NULL, 3, "<%s> Playback of the call-from file appears to be done.\n",
							ast_channel_name(tmpuser->ochan));
						if (!ast_streamfile(tmpuser->ochan, tpargs->namerecloc, ast_channel_language(tmpuser->ochan))) {
							tmpuser->state = 2;
						} else {
							ast_log_chan(NULL, LOG_NOTICE, "<%s> Unable to playback %s. Maybe the caller didn't record their name?\n",
						...
						break;
					case 2:
						ast_verb_chan(NULL, 3, "<%s> Playback of name file appears to be done.\n",
							ast_channel_name(tmpuser->ochan));
						memset(tmpuser->yn, 0, sizeof(tmpuser->yn));
						tmpuser->ynidx = 0;
						if (!ast_streamfile(tmpuser->ochan, pressbuttonname, ast_channel_language(tmpuser->ochan))) {
						...
						break;
					case 3:
						ast_verb_chan(NULL, 3, "<%s> Playback of the next step file appears to be done.\n",
							ast_channel_name(tmpuser->ochan));
						tmpuser->digts = 0;
						break;
					default:
		...
		if (!livechannels) {
			ast_verb_chan(NULL, 3, "No live channels left for this step.\n");
			return NULL;
		}
		...
		wtd = to;
		if (totalwait <= 0) {
			ast_verb_chan(NULL, 3, "We've hit our timeout for this step. Dropping unanswered calls and starting the next step.\n");
			...
			if (f) {
				if (f->frametype == AST_FRAME_CONTROL) {
					...
					case AST_CONTROL_HANGUP:
						ast_verb_chan(NULL, 3, "%s received a hangup frame.\n", ast_channel_name(winner));
						if (f->data.uint32) {
							ast_channel_hangupcause_set(winner, f->data.uint32);
						}
						if (!tmpuser) {
							ast_verb_chan(NULL, 3, "The calling channel hungup. Need to drop everyone.\n");
					...
					case AST_CONTROL_ANSWER:
						...
						if (tpargs->enable_callee_prompt) {
							ast_verb_chan(NULL, 3, "Starting playback of %s\n", callfromname);
							if (!ast_strlen_zero(tpargs->namerecloc)) {
					...
					case AST_CONTROL_PROGRESS:
						ast_verb_chan(NULL, 3, "%s is making progress\n", ast_channel_name(winner));
						ast_channel_publish_dial(caller, winner, NULL, "PROGRESS");
						break;
					case AST_CONTROL_VIDUPDATE:
						ast_verb_chan(NULL, 3, "%s requested a video update\n", ast_channel_name(winner));
						break;
					case AST_CONTROL_SRCUPDATE:
						ast_verb_chan(NULL, 3, "%s requested a source update\n", ast_channel_name(winner));
						break;
					case AST_CONTROL_PROCEEDING:
						ast_verb_chan(NULL, 3, "%s is proceeding\n", ast_channel_name(winner));
						ast_channel_publish_dial(caller, winner, NULL, "PROCEEDING");
						break;
					case AST_CONTROL_HOLD:
						ast_verb_chan(NULL, 3, "%s placed call on hold\n", ast_channel_name(winner));
						if (!tmpuser) {
					...
					case AST_CONTROL_UNHOLD:
						ast_verb_chan(NULL, 3, "%s removed call from hold\n", ast_channel_name(winner));
						if (!tmpuser) {
					...
					case AST_CONTROL_CONNECTED_LINE:
						if (ast_test_flag(&tpargs->followmeflags, FOLLOWMEFLAG_IGNORE_CONNECTEDLINE)) {
							ast_verb_chan(NULL, 3, "Connected line update from %s prevented.\n",
								ast_channel_name(winner));
							break;
						}
						if (!tmpuser) {
					...
					case AST_CONTROL_PVT_CAUSE_CODE:
						...
						break;
					case -1:
						ast_verb_chan(NULL, 3, "%s stopped sounds\n", ast_channel_name(winner));
						break;
					default:
...
static struct ast_channel *findmeexec(struct fm_args *tpargs, struct ast_channel *caller)
{
	...
	for (idx = 1; !ast_check_hangup(caller); ++idx) {
		...
		if (!nm) {
			ast_verb_chan(NULL, 3, "No more steps left.\n");
			break;
		}
		...
		for (number = num; number; number = rest) {
			...
			if (!ast_exists_extension(caller, tpargs->context, number, 1, S_COR(ast_channel_caller(caller)->id.number.valid, ast_channel_caller(caller)->id.number.str, NULL))) {
				ast_log_chan(NULL, LOG_ERROR, "Extension '%s@%s' doesn't exist\n", number, tpargs->context);
				continue;
			}
			...
			if (!outbound) {
				ast_log_chan(NULL, LOG_WARNING, "Unable to allocate a channel for Local/%s cause: %s\n",
		...
		AST_LIST_TRAVERSE_SAFE_BEGIN(&new_user_list, tmpuser, entry) {
			ast_verb_chan(NULL, 3, "calling Local/%s\n", tmpuser->dialarg);
			if (ast_call(tmpuser->ochan, tmpuser->dialarg, 0)) {
				ast_verb_chan(NULL, 3, "couldn't reach at this number.\n");
...
static int app_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "%s requires an argument (followmeid)\n", app);
		return -1;
	}
	...
	if (max_forwards <= 0) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to execute FollowMe on channel %s. Max forwards exceeded\n",
	...
	if (!f) {
		ast_log_chan(NULL, LOG_WARNING, "Profile requested, %s, not found in the configuration.\n", args.followmeid);
		ast_free(targs);
		return 0;
	}
...
outrun:
	...
	if (!ast_strlen_zero(targs->namerecloc)) {
		...
		ret = unlink(fn);
		if (ret != 0) {
			ast_log_chan(NULL, LOG_NOTICE, "Failed to delete recorded name file %s: %d (%s)\n",
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_DEFAULT, "Find-Me/Follow-Me Application",
