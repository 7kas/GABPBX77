...
static void forkcdr_callback(void *data, struct stasis_subscription *sub, struct stasis_message *message)
{
	...
	if (ast_cdr_fork(payload->channel_name, payload->flags)) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Failed to fork CDR for channel %s\n",
...
static int forkcdr_exec(struct ast_channel *chan, const char *data)
{
	...
	if (!forkcdr_message_type()) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Failed to manipulate CDR for channel %s: no message type\n",
	...
	if (!message) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Failed to fork CDR for channel %s: unable to create message\n",
			ast_channel_name(chan));
		return -1;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_DEFAULT, "Fork The CDR into 2 separate entities",
