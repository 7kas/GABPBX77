...
static int cpeid_exec(struct ast_channel *chan, const char *idata)
{
	...
	if (res > 0) {
		...
		if (res > 0) {
			gotcpeid = 1;
			ast_verb_chan(NULL, 3, "Got CPEID of '%02hhx:%02hhx:%02hhx:%02hhx' on '%s'\n",
		...
		if (res > -1) {
			...
			if (res > -1) {
				ast_verb_chan(NULL, 3, "CPE has %d lines, %d columns, and %d buttons on '%s'\n", height, width, buttons, ast_channel_name(chan));
				gotgeometry = 1;
			}
		}
		if (res > -1) {
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_DEFAULT, "Get ADSI CPE ID",
