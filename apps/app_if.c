 ...
 *
 * Copyright 2022, Naveen Albert <gabpbx@phreaknet.org>
 *
...
static int if_helper(struct ast_channel *chan, const char *data, int end)
{
	...
	if ((end <= 1 && !pbx_checkcondition(ast_strdupa(data))) || (end > 1)) {
		...
		if ((goto_str = pbx_builtin_getvar_helper(chan, end_varname))) {
			...
			ast_verb_chan(NULL, 3, "Taking conditional false branch, jumping to priority %d\n", pri);
			...
			ast_verb_chan(NULL, 3, "Taking absolute false branch, jumping to priority %d\n", pri);
			ast_channel_priority_set(chan, pri);
		} else {
			pri = endifpri;
			if (pri > 0) {
				ast_verb_chan(NULL, 3, "Exiting conditional, jumping to priority %d\n", pri);
				ast_channel_priority_set(chan, pri);
			} else if (end == 4) { /* Condition added because of end > 0 instead of end == 4 */
				ast_log_chan(NULL, LOG_WARNING, "Couldn't find matching EndIf? (If at %s@%s priority %d)\n", ast_channel_context(chan), ast_channel_exten(chan), ast_channel_priority(chan));
			}
		}
		ast_channel_unlock(chan);
		return res;
...
AST_MODULE_INFO_STANDARD_EXTENDED(GABPBX_GPL_KEY, "If Branch and Conditional Execution");
