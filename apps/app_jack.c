 ...
 *
 * This is an application to connect an GABpbx channel to an input
...
"                running.\n" \
"    c(<name>) - By default, GABpbx will use the channel name for the jack client\n" \
		...
		<syntax>
			<parameter name="options" required="false">
				<optionlist>
					...
					<option name="c">
						<argument name="name" required="true">
							<para>By default, GABpbx will use the channel name for the jack client name.</para>
...
static void log_jack_status(const char *prefix, jack_status_t status)
{
	...
	ast_log_chan(NULL, LOG_NOTICE, "%s: %s\n", prefix, ast_str_buffer(str));
}
	...
	*resample_factor = to_srate / from_srate;
	...
	if (!(*resampler = resample_open(RESAMPLE_QUALITY,
		*resample_factor, *resample_factor))) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to open %s resampler\n",
			input ? "input" : "output");
		return -1;
	}
	...
	struct jack_data *jack_data)
{
	...
	if (jack_data->input_resampler) {
		...
		while (total_in_buf_used < nframes) {
			...
			if (total_out_buf_used == ARRAY_LEN(f_buf)) {
				ast_log_chan(NULL, LOG_ERROR, "Output buffer filled ... need to increase its size, "
	...
	if (res != write_len) {
		ast_log_chan(NULL, LOG_WARNING, "Tried to write %d bytes to the ringbuffer, but only wrote %d\n",
...
static int init_jack_data(struct ast_channel *chan, struct jack_data *jack_data)
{
	...
	*/
	...
	if (!jack_data->input_port) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to create input port for jack port\n");
		return -1;
	}
	...
	if (!jack_data->output_port) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to create output port for jack port\n");
		return -1;
	}

	if (jack_set_process_callback(jack_data->client, jack_process, jack_data)) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to register process callback with jack client\n");
		return -1;
	}
	...
	if (jack_activate(jack_data->client)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to activate jack client\n");
		return -1;
	}
	...
	while (!ast_strlen_zero(jack_data->connect_input_port)) {
		...
		if (!ports) {
			ast_log_chan(NULL, LOG_ERROR, "No input port matching '%s' was found\n",
		...
		if (jack_connect(jack_data->client, jack_port_name(jack_data->output_port), ports[0])) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to connect '%s' to '%s'\n", ports[0],
	...
	while (!ast_strlen_zero(jack_data->connect_output_port)) {
		...
		if (!ports) {
			ast_log_chan(NULL, LOG_ERROR, "No output port matching '%s' was found\n",
 ...
 *
 * \param[in]  chan The GABpbx channel to write the frames to if no output frame
 *             is provided.
	...
	struct ast_frame *out_frame)
{
	...
	for (;;) {
		...
		if (res < read_len) {
			ast_log_chan(NULL, LOG_ERROR, "Error reading from ringbuffer, even though it said there was enough data\n");
			break;
		}
...
static int handle_options(struct jack_data *jack_data, const char *__options_str)
{
	...
	if (ast_test_flag(&options, OPT_SERVER_NAME)) {
		if (!ast_strlen_zero(option_args[OPT_ARG_SERVER_NAME]))
			...
			ast_log_chan(NULL, LOG_ERROR, "A server name must be provided with the s() option\n");
			return -1;
		}
	}
	...
	if (ast_test_flag(&options, OPT_CLIENT_NAME)) {
		if (!ast_strlen_zero(option_args[OPT_ARG_CLIENT_NAME]))
			...
			ast_log_chan(NULL, LOG_ERROR, "A client name must be provided with the c() option\n");
			return -1;
		}
	}
	...
	if (ast_test_flag(&options, OPT_INPUT_PORT)) {
		if (!ast_strlen_zero(option_args[OPT_ARG_INPUT_PORT]))
			...
			ast_log_chan(NULL, LOG_ERROR, "A name must be provided with the i() option\n");
			return -1;
		}
	}
	...
	if (ast_test_flag(&options, OPT_OUTPUT_PORT)) {
		if (!ast_strlen_zero(option_args[OPT_ARG_OUTPUT_PORT]))
			...
			ast_log_chan(NULL, LOG_ERROR, "A name must be provided with the o() option\n");
			return -1;
		}
	}
	...
	struct ast_frame *frame, enum ast_audiohook_direction direction)
{
	...
	if (!(datastore = ast_channel_datastore_find(chan, &jack_hook_ds_info, NULL))) {
		ast_log_chan(NULL, LOG_ERROR, "JACK_HOOK datastore not found for '%s'\n", ast_channel_name(chan));
		ast_channel_unlock(chan);
		return -1;
	}
	...
	if (ast_format_cmp(frame->subclass.format, jack_data->audiohook_format) == AST_FORMAT_CMP_NOT_EQUAL) {
		ast_log_chan(NULL, LOG_WARNING, "Expected frame in %s for the audiohook, but got format %s\n",
...
static int enable_jack_hook(struct ast_channel *chan, char *data)
{
	...
	if ((datastore = ast_channel_datastore_find(chan, &jack_hook_ds_info, NULL))) {
		ast_log_chan(NULL, LOG_ERROR, "JACK_HOOK already enabled for '%s'\n", ast_channel_name(chan));
		goto return_error;
	}

	if (ast_strlen_zero(args.mode) || strcasecmp(args.mode, "manipulate")) {
		ast_log_chan(NULL, LOG_ERROR, "'%s' is not a supported mode.  Only manipulate is supported.\n",
			S_OR(args.mode, "<none>"));
		goto return_error;
	}
...
static int disable_jack_hook(struct ast_channel *chan)
{
	...
	if (!(datastore = ast_channel_datastore_find(chan, &jack_hook_ds_info, NULL))) {
		ast_channel_unlock(chan);
		ast_log_chan(NULL, LOG_WARNING, "No JACK_HOOK found to disable\n");
		return -1;
	}
	...
	const char *value)
{
	...
	if (!strcasecmp(value, "on"))
		...
		ast_log_chan(NULL, LOG_ERROR, "'%s' is not a valid value for JACK_HOOK()\n", value);
		res = -1;
	}
...
AST_MODULE_INFO_STANDARD_EXTENDED(GABPBX_GPL_KEY, "JACK Interface");
