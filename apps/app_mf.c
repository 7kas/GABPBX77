...
#define BUFFER_SIZE 256
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "ReceiveMF requires an argument (variable)\n");
		return -1;
	}
	...
	if (ast_strlen_zero(arglist.variable)) {
		ast_log_chan(NULL, LOG_WARNING, "Invalid! Usage: ReceiveMF(variable[,timeout][,option])\n");
		return -1;
	}
	if (ast_test_flag(&flags, OPT_MAXDIGITS) && !ast_strlen_zero(optargs[OPT_ARG_MAXDIGITS])) {
		maxdigits = atoi(optargs[OPT_ARG_MAXDIGITS]);
		if (maxdigits <= 0) {
			ast_log_chan(NULL, LOG_WARNING, "Invalid maximum number of digits, ignoring: '%s'\n", optargs[OPT_ARG_MAXDIGITS]);
			maxdigits = 0;
		}
	}
	...
	if (!ast_strlen_zero(tmp)) {
		ast_verb_chan(NULL, 3, "MF digits received: '%s'\n", tmp);
	} else if (!res) { /* if channel hung up, don't print anything out */
		ast_verb_chan(NULL, 3, "No MF digits received.\n");
	}
	return res;
}
...
static int sendmf_exec(struct ast_channel *chan, const char *vdata)
{
	...
	if (ast_strlen_zero(vdata)) {
		ast_log_chan(NULL, LOG_WARNING, "SendMF requires an argument\n");
		return 0;
	}
	...
	if (ast_strlen_zero(args.digits)) {
		ast_log_chan(NULL, LOG_WARNING, "The digits argument is required (0-9,*#ABC,wf)\n");
		return 0;
	}
...
AST_MODULE_INFO_STANDARD_EXTENDED(GABPBX_GPL_KEY, "MF Sender and Receiver Applications");
