...
/*! \file
 *
 * \brief MiniVoiceMail - A Minimal Voicemail System for GABpbx
 *
 ...
 *
 * \page App_minivm	GABpbx Mini-voicemail - A minimal voicemail system
 *
 ...
 *	\note Don't use language codes like the rest of GABpbx, two letter countrycode. Use
 ...
 */
/*! \page Minivm_directories GABpbx Mini-Voicemail Directory structure
 *
 *	The directory structure for storing voicemail
 *		- AST_SPOOL_DIR - usually /var/spool/gabpbx (configurable in gabpbx.conf)
 ...
 *		\b /var/spool/gabpbx/voicemail/localdomain.xx/anita
 *
...
/*! \page App_minivm_todo GABpbx Mini-Voicemail - todo
...
#include "gabpbx/paths.h"	/* use various paths */
...
<function name="MINIVMCOUNTER" language="en_US">
	...
	<description>
		<para>The operation is atomic and the counter is locked while changing the value. The counters are stored as text files in the minivm account directories. It might be better to use realtime functions if you are using a database to operate your GABpbx.</para>
	</description>
	<see-also>
...
#define GABPBX_USERNAME "gabpbx"	/*!< Default username for sending mail is gabpbx\@localhost */
...
static int message_template_build(const char *name, struct ast_variable *var)
{
	...
	if (!template) {
		ast_log_chan(NULL, LOG_ERROR, "Out of memory, can't allocate message template object %s.\n", name);
		return -1;
	}
	...
	while (var) {
		...
		if (!strcasecmp(var->name, "fromaddress")) {
			...
			if (!template->body) {
				ast_log_chan(NULL, LOG_ERROR, "Error reading message body definition file %s\n", var->value);
				error++;
			}
			...
			if (!template->body) {
				ast_log_chan(NULL, LOG_ERROR, "Error parsing message body definition:\n          %s\n", var->value);
				error++;
			}
		} else {
			ast_log_chan(NULL, LOG_ERROR, "Unknown message template configuration option \"%s=%s\"\n", var->name, var->value);
			error++;
		}
		var = var->next;
	}
	if (error)
		ast_log_chan(NULL, LOG_ERROR, "-- %d errors found parsing message template definition %s\n", error, name);
...
static void prep_email_sub_vars(struct ast_channel *channel, const struct minivm_account *vmu, const char *cidnum, const char *cidname, const char *dur, const char *date, const char *counter)
{
	...
	if (!channel) {
		ast_log_chan(NULL, LOG_ERROR, "No allocated channel, giving up...\n");
		return;
	}
...
static struct minivm_account *find_account(const char *domain, const char *username, int createtemp)
{
	...
	if (ast_strlen_zero(domain) || ast_strlen_zero(username)) {
		ast_log_chan(NULL, LOG_NOTICE, "No username or domain? \n");
		return NULL;
	}
...
static int sendmail(struct minivm_template *template, struct minivm_account *vmu, char *cidnum, char *cidname, const char *filename, char *format, int duration, int attach_user_voicemail, enum mvm_messagetype type, const char *counter)
{
	...
	if (ast_strlen_zero(email)) {
		ast_log_chan(NULL, LOG_WARNING, "No address to send message to.\n");
		return -1;
	}
	...
	if (type == MVM_MESSAGE_EMAIL && (vmu->volgain < -.001 || vmu->volgain > .001) ) {
		...
		if (!mkdtemp(sox_gain_tmpdir)) {
			ast_log_chan(NULL, LOG_WARNING, "Failed to create temporary directory for volgain: %d\n", errno);
			return -1;
		}
	...
	if (!p) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to open temporary file '%s'\n", tmp);
		goto out;
	}
	...
	fprintf(p, "Message-ID: <GABpbx-%u-%s-%d-%s>\n", (unsigned int)ast_random(), vmu->username, (int)getpid(), who);
	...
	if (DEBUG_ATLEAST(3))
		fprintf(p, "X-GABpbx-debug: template %s user account %s@%s\n", template->name, vmu->username, vmu->domain);
	...
	if (res < sizeof(mail_cmd_buffer)) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Could not send message, command line too long\n");
		res = -1;
		goto out;
	}
...
static int create_dirpath(char *dest, int len, char *domain, char *username, char *folder)
{
	...
	if ((res = ast_mkdir(dest, 0777))) {
		ast_log_chan(NULL, LOG_WARNING, "ast_mkdir '%s' failed: %s\n", dest, strerror(res));
		return -1;
	}
			      ...
			      signed char record_gain)
{
	...
	if (duration == NULL) {
		ast_log_chan(NULL, LOG_WARNING, "Error play_record_review called without duration pointer\n");
		return -1;
	}
...
static int notify_new_message(struct ast_channel *chan, const char *templatename, struct minivm_account *vmu, const char *filename, long duration, const char *format, char *cidnum, char *cidname)
{
	...
	if (!ast_strlen_zero(vmu->attachfmt)) {
		if (strstr(format, vmu->attachfmt)) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Attachment format '%s' is not one of the recorded formats '%s'.  Falling back to default format for '%s@%s'.\n", vmu->attachfmt, format, vmu->username, vmu->domain);
		}
	}
	...
	if (!ast_strlen_zero(etemplate->locale)) {
		...
		if (new_locale == NULL) {
			ast_log_chan(NULL, LOG_WARNING, "-_-_- Changing to new locale did not work. Locale: %s\n", etemplate->locale);
		}
	}
...
static int leave_voicemail(struct ast_channel *chan, char *username, struct leave_vm_options *options)
{
	...
	if (!(vmu = find_account(domain, username, TRUE))) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Can't allocate temporary account for '%s@%s'\n", username, domain);
	...
	if (ast_strlen_zero(fmt)) {
		ast_log_chan(NULL, LOG_WARNING, "No format for saving voicemail? Default %s\n", default_vmformat);
		pbx_builtin_setvar_helper(chan, "MVM_RECORD_STATUS", "FAILED");
		return res;
	}
	...
	if (txtdes < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to create message file %s: %s\n", tmptxtfile, strerror(errno));
...
/*!\internal
 * \brief Send MWI using interal GABpbx event subsystem */
static int minivm_mwi_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(data))  {
		ast_log_chan(NULL, LOG_ERROR, "Minivm needs at least an account argument \n");
		return -1;
	}
	tmpptr = ast_strdupa((char *)data);
	argc = ast_app_separate_args(tmpptr, ',', argv, ARRAY_LEN(argv));
	if (argc < 4) {
		ast_log_chan(NULL, LOG_ERROR, "%d arguments passed to MiniVM_MWI, need 4.\n", argc);
		return -1;
	}
	...
	if (ast_strlen_zero(domain) || ast_strlen_zero(mailbox)) {
		ast_log_chan(NULL, LOG_ERROR, "Need mailbox@context as argument. Sorry. Argument 0 %s\n", argv[0]);
		return -1;
	}
...
static int minivm_notify_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(domain) || ast_strlen_zero(username)) {
		ast_log_chan(NULL, LOG_ERROR, "Need username@domain as argument. Sorry. Argument 0 %s\n", argv[0]);
		return -1;
	}

	if(!(vmu = find_account(domain, username, TRUE))) {
		/* We could not find user, let's exit */
		ast_log_chan(NULL, LOG_WARNING, "Could not allocate temporary memory for '%s@%s'\n", username, domain);
...
static int minivm_record_exec(struct ast_channel *chan, const char *data)
{
	...
	if (argc == 2) {
		...
		if (ast_test_flag(&flags, OPT_RECORDGAIN)) {
			...
			if (sscanf(opts[OPT_ARG_RECORDGAIN], "%30d", &gain) != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid value '%s' provided for record gain option\n", opts[OPT_ARG_RECORDGAIN]);
				return -1;
			} else
	...
	if (res == ERROR_LOCK_PATH) {
		ast_log_chan(NULL, LOG_ERROR, "Could not leave voicemail. The path is already locked.\n");
		pbx_builtin_setvar_helper(chan, "MVM_RECORD_STATUS", "FAILED");
		res = 0;
	}
	pbx_builtin_setvar_helper(chan, "MVM_RECORD_STATUS", "SUCCESS");
...
static int minivm_greet_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(domain) || ast_strlen_zero(username)) {
		ast_log_chan(NULL, LOG_ERROR, "Need username@domain as argument. Sorry. Argument:  %s\n", argv[0]);
		return -1;
	}
	ast_debug(1, "Trying to find configuration for user %s in domain %s\n", username, domain);

	if (!(vmu = find_account(domain, username, TRUE))) {
		ast_log_chan(NULL, LOG_ERROR, "Could not allocate memory. \n");
		return -1;
	}
...
static int minivm_delete_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(filename)) {
		ast_log_chan(NULL, LOG_ERROR, "No filename given in application arguments or channel variable MVM_FILENAME\n");
		return res;
	}
...
static int minivm_accmess_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(data))  {
		ast_log_chan(NULL, LOG_ERROR, "MinivmAccmess needs at least two arguments: account and option\n");
		error = TRUE;
	} else {
		tmpptr = ast_strdupa((char *)data);
		argc = ast_app_separate_args(tmpptr, ',', argv, ARRAY_LEN(argv));
	...
	if (!error && strlen(argv[1]) > 1) {
		ast_log_chan(NULL, LOG_ERROR, "MinivmAccmess can only handle one option at a time. Bad option string: %s\n", argv[1]);
		error = TRUE;
	}

	if (!error && ast_app_parse_options(minivm_accmess_options, &flags, opts, argv[1])) {
		ast_log_chan(NULL, LOG_ERROR, "Can't parse option %s\n", argv[1]);
		error = TRUE;
	}
...
static int create_vmaccount(char *name, struct ast_variable *var, int realtime)
{
	...
	if (ast_strlen_zero(domain)) {
		ast_log_chan(NULL, LOG_ERROR, "No domain given for mini-voicemail account %s. Not configured.\n", name);
		return 0;
	}
	...
	while (var) {
		...
		if (!strcasecmp(var->name, "serveremail")) {
			...
			ast_log_chan(NULL, LOG_ERROR, "Unknown configuration option for minivm account %s : %s\n", name, var->name);
		}
		var = var->next;
	}
...
static int timezone_add(const char *zonename, const char *config)
{
	...
	if (!msg_format) {
		ast_log_chan(NULL, LOG_WARNING, "Invalid timezone definition : %s\n", zonename);
		ast_free(newzone);
		return 0;
	}
...
static char *message_template_parse_filebody(const char *filename) {
	...
	if (!(fi = fopen(filenamebuf, "r"))) {
		ast_log_chan(NULL, LOG_ERROR, "Can't read message template from file: %s\n", filenamebuf);
		return NULL;
	}
	writepos = buf;
...
static char *message_template_parse_emailbody(const char *configuration)
{
	...
	while ((tmpwrite = strchr(tmpread,'\\'))) {
	       ...
	       case 't':
		      ...
		      break;
	       default:
		      ast_log_chan(NULL, LOG_NOTICE, "Substitution routine does not support this character: %c\n", tmpwrite[1]);
	       }
	...
	while (var) {
		...
		if (!strcmp(var->name, "mailcmd")) {
			...
			if (sscanf(var->value, "%30d", &x) == 1) {
				...
				error ++;
				ast_log_chan(NULL, LOG_WARNING, "Invalid max message time length\n");
			}
			...
			if (sscanf(var->value, "%30d", &x) == 1) {
				...
				if (global_maxsilence <= global_vmminmessage)
					ast_log_chan(NULL, LOG_WARNING, "maxsilence should be less than minmessage or you may get empty messages\n");
			} else {
				error ++;
				ast_log_chan(NULL, LOG_WARNING, "Invalid min message time length\n");
			}
...
static int load_config(int reload)
{
	...
	if (!cfg) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to load configuration file. Module activated with default settings.\n");
	...
	if (error)
		ast_log_chan(NULL, LOG_ERROR, "--- A total of %d errors found in mini-voicemail configuration\n", error);
	...
	if(!ast_strlen_zero(global_logfile)) {
		...
		if(!minivmlogfile)
			ast_log_chan(NULL, LOG_ERROR, "Failed to open minivm log file %s : %s\n", global_logfile, strerror(errno));
		if (minivmlogfile)
...
static int minivm_account_func_read(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	if (ast_strlen_zero(username) || ast_strlen_zero(domain)) {
		ast_log_chan(NULL, LOG_ERROR, "This function needs a username and a domain: username@domain\n");
		return 0;
	}
...
static int access_counter_file(char *directory, char *countername, int value, int operand)
{
	...
	if (!counterfile) {
		ast_log_chan(NULL, LOG_ERROR, "Could not open counter file for writing : %s - %s\n", filename, strerror(errno));
		ast_unlock_path(directory);
...
static int minivm_counter_func_read(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	*buf = '\0';
	...
	if (ast_strlen_zero(username) && ast_strlen_zero(domain)) {
		ast_log_chan(NULL, LOG_ERROR, "No account given\n");
		return -1;
	}

	if (ast_strlen_zero(countername)) {
		ast_log_chan(NULL, LOG_ERROR, "This function needs two arguments: Account:countername\n");
		return -1;
	}
	...
	if (!ast_strlen_zero(username) && !find_account(domain, username, FALSE)) {
		ast_log_chan(NULL, LOG_ERROR, "Minivm account does not exist: %s@%s\n", username, domain);
		return 0;
	}
...
static int minivm_counter_func_write(struct ast_channel *chan, const char *cmd, char *data, const char *value)
{
	...
	if (ast_strlen_zero(operand) || ast_strlen_zero(countername)) {
		ast_log_chan(NULL, LOG_ERROR, "Writing to this function requires three arguments: Account:countername:operand\n");
		return -1;
	}
	...
	if (*operand == 'i') /* Increment */
		...
		ast_log_chan(NULL, LOG_ERROR, "Unknown operator: %s\n", operand);
		return -1;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_DEFAULT, "Mini VoiceMail (A minimal Voicemail e-mail System)",
