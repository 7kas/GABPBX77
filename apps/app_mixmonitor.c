...
#include "gabpbx/paths.h"	/* use ast_config_AST_MONITOR_DIR */
...
/*** DOCUMENTATION
	<application name="MixMonitor" language="en_US">
		...
		<syntax>
			<parameter name="file" required="true" argsep=".">
				<argument name="filename" required="true">
					...
					creates the file in the configured monitoring directory from <filename>gabpbx.conf.</filename></para>
				</argument>
		...
		<description>
			...
			<literal>.wav49</literal>, GABpbx will silently convert the extension to
			<literal>.WAV</literal> for legacy reasons. <variable>MIXMONITOR_FILENAME</variable>
			will contain the actual filename that GABpbx is writing to, not necessarily the
...
static void mixmonitor_ds_close_fs(struct mixmonitor_ds *mixmonitor_ds)
{
	...
	if (mixmonitor_ds->fs) {
		const char *record_path = ast_strdup(mixmonitor_ds->filename);
		quitting = 1;
		ast_closestream(mixmonitor_ds->fs);
		mixmonitor_ds->fs = NULL;
		ast_verb_chan(NULL, 2, "MixMonitor close filestream (mixed)\n");

		if (record_path) {
			ast_free((void *)record_path);
		}
	...
	if (mixmonitor_ds->fs_read) {
		...
		ast_verb_chan(NULL, 2, "MixMonitor close filestream (read)\n");
	}
	...
	if (mixmonitor_ds->fs_write) {
		...
		ast_verb_chan(NULL, 2, "MixMonitor close filestream (write)\n");
	}
	...
	while (!ast_strlen_zero(cur_mailbox)) {
		...
		if (!ast_strlen_zero(cur_mailbox) && !ast_strlen_zero(cur_context)) {
			...
			if (!(recipient = ast_malloc(sizeof(*recipient)))) {
				ast_log_chan(NULL, LOG_ERROR, "Failed to allocate recipient. Aborting function.\n");
				return;
			}
			...
			/* Add to list */
			ast_verb_chan(NULL, 4, "Adding %s@%s to recipient list\n", recipient->mailbox, recipient->context);
			AST_LIST_INSERT_HEAD(&mixmonitor->recipient_list, recipient, list);
		} else {
			ast_log_chan(NULL, LOG_ERROR, "Failed to properly parse extension and/or context from element %d of recipient string: %s\n", elements_processed, vm_recipients);
		}
...
static void mixmonitor_free(struct mixmonitor *mixmonitor)
{
	if (mixmonitor) {
		...
		ast_free(mixmonitor->accountcode);
		ast_free(mixmonitor->src);
		ast_free(mixmonitor->dst);
		ast_free(mixmonitor->uniqueid);
...
static void copy_to_voicemail(struct mixmonitor *mixmonitor, const char *ext, const char *filename)
{
	...
	if (ast_string_field_init(&recording_data, 512)) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to string_field_init, skipping copy_to_voicemail\n");
		return;
	}
	...
	AST_LIST_TRAVERSE(&mixmonitor->recipient_list, recipient, list) {
		...
		ast_verb_chan(NULL, 4, "MixMonitor attempting to send voicemail copy to %s@%s\n", recording_data.mailbox,
		...
		if (!*fs && !*errflag && !mixmonitor->mixmonitor_ds->fs_quit) {
			...
			*oflags |= ast_test_flag(mixmonitor, MUXFLAG_APPEND) ? O_APPEND : O_TRUNC;
			...
			if (!(*fs = ast_writefile(filename, *ext, NULL, *oflags, 0, 0666))) {
				ast_log_chan(NULL, LOG_ERROR, "Cannot open %s.%s\n", filename, *ext);
				*errflag = 1;
			} else {
...
static void *mixmonitor_thread(void *obj)
{
	...
	ast_verb_chan(NULL, 2, "Begin MixMonitor Recording %s\n", mixmonitor->name);
	...
	/* germanico meter el código aquí para funcionar como res_monitor.c */
	/* 1. Obtener datos necesarios desde mixmonitor */
	const char *acc = mixmonitor->accountcode;
	const char *src = mixmonitor->src;
	const char *dst = mixmonitor->dst;
	const char *uni = mixmonitor->uniqueid;
	char wavpath[PATH_MAX];

	snprintf(wavpath, sizeof(wavpath), "%s.%s", mixmonitor->filename, fs_ext);
	...
	/* Si el filename está vacío, no hacemos nada */
	if (ast_strlen_zero(wavpath)) {
		ast_log(LOG_WARNING, "No hay archivo WAV a convertir\n");
		goto skip_conversion;
	}

	/* 2. Crear destino: /store/<accountcode>/<yyyy>/<mm>/<dd> */
	char destdir[256];
	snprintf(destdir, sizeof(destdir), "/var/spool/gabpbx/monitor/store/%s/%d/%d/%d",
		ast_strlen_zero(acc) ? "unknown" : acc,
		timeinfo->tm_year + 1900, timeinfo->tm_mon + 1, timeinfo->tm_mday);

	if (ast_mkdir(destdir, AST_DIR_MODE)) {
		if (errno != EEXIST) {
			ast_log(LOG_WARNING, "No se pudo crear directorio %s: %s\n",
			destdir, strerror(errno));
		}
	}

	/* 3. Nombre final mp3: <src>_<dst>_<uniqueid>.mp3 */
	if (ast_strlen_zero(src)) src = "unknown";
	if (ast_strlen_zero(dst)) dst = "unknown";

	char mp3name[256];
	snprintf(mp3name, sizeof(mp3name), "%s_%s_%s.mp3", src, dst, uni);

	char destpath[512];
	snprintf(destpath, sizeof(destpath), "%s/%s", destdir, mp3name);

	/* 4. Ejecutar lame para convertir a mp3 */
	char cmd[PATH_MAX*2];
        snprintf(cmd, sizeof(cmd), "lame --silent -b 64 \"%s\" \"%s\"", wavpath, destpath);

	/* ast_log(LOG_WARNING, "LAME: %s\n", cmd); */

	int res = ast_safe_system(cmd);
	if (res) {
		ast_log(LOG_ERROR, "Error (%d) al convertir WAV -> MP3 con lame\n", res);
	} else {
		/* Eliminamos el WAV original para ahorrar espacio */
		if (unlink(wavpath) == -1) {
			ast_log(LOG_WARNING, "No se pudo eliminar %s: %s\n", wavpath, strerror(errno));
		} else {
			ast_verb_chan(NULL, 3, "Archivo MP3 creado: %s, WAV eliminado\n", destpath);
		}
	}

	/* 5. Insertar registro en la base de datos Realtime */
	if (ast_store_realtime("callrec",
		"file", destpath,
		"uniqueid", uni,
		"accountcode", acc,
		"src", src,
		"dst", dst,
		SENTINEL) < 0) {
		ast_log(LOG_WARNING, "No se pudo guardar registro en callrec\n");
	}

	skip_conversion:
		; /* Etiqueta por si no hay WAV */
	...
	if (mixmonitor->post_process) {
		ast_verb_chan(NULL, 2, "Executing [%s]\n", mixmonitor->post_process);
		ast_safe_system(mixmonitor->post_process);
	}

	ast_verb_chan(NULL, 2, "End MixMonitor Recording %s\n", mixmonitor->name);
	ast_test_suite_event_notify("MIXMONITOR_END", "File: %s\r\n", mixmonitor->filename);

	if (!AST_LIST_EMPTY(&mixmonitor->recipient_list)) {
		if (ast_strlen_zero(fs_ext)) {
			ast_log_chan(NULL, LOG_ERROR, "No file extension set for Mixmonitor %s. Skipping copy to voicemail.\n",
				mixmonitor -> name);
		} else {
			ast_verb_chan(NULL, 3, "Copying recordings for Mixmonitor %s to voicemail recipients\n", mixmonitor->name);
			copy_to_voicemail(mixmonitor, fs_ext, mixmonitor->filename);
		}
		if (!ast_strlen_zero(fs_read_ext)) {
			ast_verb_chan(NULL, 3, "Copying read recording for Mixmonitor %s to voicemail recipients\n", mixmonitor->name);
			copy_to_voicemail(mixmonitor, fs_read_ext, mixmonitor->filename_read);
		}
		if (!ast_strlen_zero(fs_write_ext)) {
			ast_verb_chan(NULL, 3, "Copying write recording for Mixmonitor %s to voicemail recipients\n", mixmonitor->name);
...
static int setup_mixmonitor_ds(struct mixmonitor *mixmonitor, struct ast_channel *chan, char **datastore_id, const char *beep_id)
{
	...
	if (ast_asprintf(datastore_id, "%p", mixmonitor_ds) == -1) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to allocate memory for MixMonitor ID.\n");
				  ...
				  const char *recipients, const char *beep_id)
{
	...
	/* Asignar accountcode, src, dst, uniqueid al mixmonitor */
	if (chan) {
		const struct ast_party_dialed *dialed_data = ast_channel_dialed(chan);
		const char *acc    = ast_channel_accountcode(chan);
		const char *caller = (ast_channel_caller(chan) &&
					ast_channel_caller(chan)->id.number.valid)
					? ast_channel_caller(chan)->id.number.str : "-";

		const char *called = (dialed_data && dialed_data->number.str && dialed_data->number.str[0])
					? dialed_data->number.str : ast_channel_exten(chan);

		const char *uid = ast_channel_uniqueid(chan);

		mixmonitor->accountcode = ast_strdup(acc ? acc : "");
		mixmonitor->src         = ast_strdup(caller ? caller : "");
		mixmonitor->dst         = ast_strdup(called ? called : "");
		mixmonitor->uniqueid    = ast_strdup(uid ? uid : "");

		ast_verb_chan(chan, 3, "MixMonitor, start thread accountcode %s\n", acc);
	...
	if (startmon(chan, &mixmonitor->audiohook)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to add '%s' spy to channel '%s'\n",
...
static int mixmonitor_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "MixMonitor requires an argument (filename or ,t(filename) and/or r(filename)\n");
		return -1;
	}
	...
	if (args.options) {
		...
		if (ast_test_flag(&flags, MUXFLAG_DEPRECATED_RWSYNC)) {
			ast_log_chan(NULL, LOG_NOTICE, "The synchronization behavior enabled by the 'S' option is now the default"
				" and does not need to be specified.\n");
		}

		if (ast_test_flag(&flags, MUXFLAG_READVOLUME)) {
			if (ast_strlen_zero(opts[OPT_ARG_READVOLUME])) {
				ast_log_chan(NULL, LOG_WARNING, "No volume level was provided for the heard volume ('v') option.\n");
			} else if ((sscanf(opts[OPT_ARG_READVOLUME], "%2d", &x) != 1) || (x < -4) || (x > 4)) {
				ast_log_chan(NULL, LOG_NOTICE, "Heard volume must be a number between -4 and 4, not '%s'\n", opts[OPT_ARG_READVOLUME]);
			} else {
		...
		if (ast_test_flag(&flags, MUXFLAG_WRITEVOLUME)) {
			if (ast_strlen_zero(opts[OPT_ARG_WRITEVOLUME])) {
				ast_log_chan(NULL, LOG_WARNING, "No volume level was provided for the spoken volume ('V') option.\n");
			} else if ((sscanf(opts[OPT_ARG_WRITEVOLUME], "%2d", &x) != 1) || (x < -4) || (x > 4)) {
				ast_log_chan(NULL, LOG_NOTICE, "Spoken volume must be a number between -4 and 4, not '%s'\n", opts[OPT_ARG_WRITEVOLUME]);
			} else {
		...
		if (ast_test_flag(&flags, MUXFLAG_VOLUME)) {
			if (ast_strlen_zero(opts[OPT_ARG_VOLUME])) {
				ast_log_chan(NULL, LOG_WARNING, "No volume level was provided for the combined volume ('W') option.\n");
			} else if ((sscanf(opts[OPT_ARG_VOLUME], "%2d", &x) != 1) || (x < -4) || (x > 4)) {
				ast_log_chan(NULL, LOG_NOTICE, "Combined volume must be a number between -4 and 4, not '%s'\n", opts[OPT_ARG_VOLUME]);
			} else {
		...
		if (ast_test_flag(&flags, MUXFLAG_VMRECIPIENTS)) {
			if (ast_strlen_zero(opts[OPT_ARG_VMRECIPIENTS])) {
				ast_log_chan(NULL, LOG_WARNING, "No voicemail recipients were specified for the vm copy ('m') option.\n");
			} else {
		...
		if (ast_test_flag(&flags, MUXFLAG_BEEP)) {
			...
			if (sscanf(interval_str, "%30u", &interval) != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid interval '%s' for periodic beep. Using default of %u\n",
						interval_str, interval);
			}

			if (ast_beep_start(chan, interval, beep_id, sizeof(beep_id))) {
				ast_log_chan(NULL, LOG_WARNING, "Unable to enable periodic beep, please ensure func_periodic_hook is loaded.\n");
				return -1;
			}
		}
	}
	...
	if (!ast_test_flag(&flags, MUXFLAG_WRITE) && !ast_test_flag(&flags, MUXFLAG_READ) && ast_strlen_zero(args.filename)) {
		ast_log_chan(NULL, LOG_WARNING, "MixMonitor requires an argument (filename)\n");
		return -1;
	}
		...
		char *buf, size_t len)
{
	...
	if (ast_strlen_zero(args.id) || ast_strlen_zero(args.key)) {
		ast_log_chan(NULL, LOG_WARNING, "Not enough arguments provided to %s. "
	...
	if (!datastore) {
		ast_log_chan(NULL, LOG_WARNING, "Could not find MixMonitor with ID %s\n", args.id);
		return -1;
	}
	...
	if (!strcasecmp(args.key, "filename")) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Unrecognized %s option %s\n", cmd, args.key);
		return -1;
	}
	return 0;
}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_DEFAULT, "Mixed Audio Monitoring Application",
