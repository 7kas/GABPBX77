...
/*** DOCUMENTATION
	<application name="MP3Player" language="en_US">
		...
		<description>
			...
			exten => 1234,1,MP3Player(/var/lib/gabpbx/playlist.m3u)
			</example>
...
static int timed_read(int fd, void *data, int datalen, int timeout, int pid)
{
	...
	for (i = 0; i < timeout; i++) {
		...
		if (res > 0) {
			...
			ast_log_chan(NULL, LOG_NOTICE, "error polling mpg123: %s\n", strerror(errno));
			return -1;
		}
	}

	if (i == timeout) {
		ast_log_chan(NULL, LOG_NOTICE, "Poll timed out.\n");
		return -1;
	}
...
static int mp3_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "MP3 Playback requires an argument (filename)\n");
		return -1;
	}
				...
				if (res > 0) {
					...
					if (!startedmp3) { /* we couldn't do anything, which means this stream doesn't work */
						if (!strncasecmp(data, "https://", 8)) {
							ast_log_chan(NULL, LOG_WARNING, "%s() does not support HTTPS streams. Use HTTP instead.\n", app);
						}
						ast_log_chan(NULL, LOG_WARNING, "MP3 stream '%s' is broken or nonexistent\n", data);
					}
					res = 0;
					break;
				}
...
AST_MODULE_INFO_STANDARD_EXTENDED(GABPBX_GPL_KEY, "Silly MP3 Application");
