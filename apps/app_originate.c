...
static int originate_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_ERROR, "Originate() requires arguments\n");
		goto return_cleanup;
	}
	...
	if (args.argc < 3) {
		ast_log_chan(NULL, LOG_ERROR, "Incorrect number of arguments\n");
		goto return_cleanup;
	}

	if (!ast_strlen_zero(args.timeout)) {
		if(sscanf(args.timeout, "%u", &timeout) != 1) {
			ast_log_chan(NULL, LOG_NOTICE, "Invalid timeout: '%s'. Setting timeout to 30 seconds\n", args.timeout);
			timeout = 30;
		}
	}
	...
	if (ast_strlen_zero(chandata) || ast_strlen_zero(chantech)) {
		ast_log_chan(NULL, LOG_ERROR, "Channel Tech/Data invalid: '%s'\n", args.tech_data);
		goto return_cleanup;
	}

	if (!ast_strlen_zero(args.options) &&
		ast_app_parse_options64(originate_exec_options, &opts, opt_args, args.options)) {
		ast_log_chan(NULL, LOG_ERROR, "Invalid options: '%s'\n", args.options);
		goto return_cleanup;
	}
	...
	if (strcasecmp(args.type, "exten") && strcasecmp(args.type, "app")) {
		ast_log_chan(NULL, LOG_ERROR, "Incorrect type, it should be 'exten' or 'app': %s\n",
				args.type);
		goto return_cleanup;
	}
	...
	if (ast_test_flag64(&opts, OPT_VARIABLES)
		...
		while ((vartext = ast_strsep(&text, '^', 0))) {
			...
			if (!(varname = ast_strsep(&vartext, '=', 0))) {
				ast_log_chan(NULL, LOG_ERROR, "Variable syntax error: %s\n", vartext);
				goto return_cleanup;
			}
			...
			if (!var) {
				ast_log_chan(NULL, LOG_ERROR, "Failed to allocate variable: %s\n", varname);
				goto return_cleanup;
			}
	...
	if (!strcasecmp(args.type, "exten")) {
		...
		if (args.argc == 5) {
			...
			if (sscanf(args.arg3, "%30d", &priority) != 1) {
				ast_log_chan(NULL, LOG_ERROR, "Invalid priority: '%s'\n", args.arg3);
				goto return_cleanup;
			}
		} else if (args.argc == 3) {
			/* Exten not specified */
...
return_cleanup:
	if (res) {
		...
		case AST_CONTROL_RINGING:
			...
			break;
		default:
			ast_log_chan(NULL, LOG_WARNING, "Unknown originate status result of '%d'\n",
					outgoing_status);
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Originate call");
