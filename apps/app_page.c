...
static int page_exec(struct ast_channel *chan, const char *data)
{
	...
	if (!(app = pbx_findapp("ConfBridge"))) {
		ast_log_chan(NULL, LOG_WARNING, "There is no ConfBridge application available!\n");
		return -1;
	};
	...
	if (!(dial_list = ast_calloc(num_dials, sizeof(struct ast_dial *)))) {
		ast_log_chan(NULL, LOG_ERROR, "Can't allocate %ld bytes for dial list\n", (long)(sizeof(struct ast_dial *) * num_dials));
		return -1;
	}
	...
	while ((tech = strsep(&args.devices, "&"))) {
		...
		if (!(resource = strchr(tech, '/'))) {
			ast_log_chan(NULL, LOG_WARNING, "Incomplete destination: '%s' supplied.\n", tech);
			continue;
		}
		...
		if (ast_test_flag(&options.flags, PAGE_SKIP)) {
			...
			if (state == AST_DEVICE_UNKNOWN) {
				ast_verb_chan(NULL, 3, "Destination '%s' has device state '%s'. Paging anyway.\n",
					tech, ast_devstate2str(state));
			} else if (state != AST_DEVICE_NOT_INUSE) {
				ast_verb_chan(NULL, 3, "Destination '%s' has device state '%s'.\n",
					tech, ast_devstate2str(state));
				continue;
			}
		}
		...
		*resource++ = '\0';
		...
		if (!(dial = ast_dial_create())) {
			ast_log_chan(NULL, LOG_WARNING, "Failed to create dialing structure.\n");
			continue;
		}

		/* Append technology and resource */
		if (ast_dial_append(dial, tech, resource, NULL) == -1) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to add %s/%s to outbound dial\n", tech, resource);
	...
	if (!ast_test_flag(&options.flags, PAGE_QUIET)) {
		if (!ast_fileexists(PAGE_BEEP, NULL, NULL)) {
			ast_log_chan(NULL, LOG_WARNING, "Missing required sound file: '" PAGE_BEEP "'\n");
		} else {
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_DEFAULT, "Page Multiple Phones",
