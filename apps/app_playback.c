 ...
 * some CLI commands.
 */
#include "gabpbx/say.h"	/*!< provides config-file based 'say' functions */
...
static int do_say(say_args_t *a, const char *s, const char *options, int depth)
{
	...
	if (depth++ > 10) {
		ast_log_chan(NULL, LOG_WARNING, "recursion too deep, exiting\n");
		return -1;
	} else if (!say_cfg) {
		ast_log_chan(NULL, LOG_WARNING, "no say.conf, cannot spell '%s'\n", s);
		return -1;
	}
	...
	if (!n) {
		ast_log_chan(NULL, LOG_ERROR, "Memory allocation error in do_say\n");
		ast_free(rule_head);
		return -1;
	}
...
static int say_init_mode(const char *mode) {
	if (!strcmp(mode, say_new)) {
		if (say_cfg == NULL) {
			ast_log_chan(NULL, LOG_ERROR, "There is no say.conf file to use new mode\n");
			return -1;
		}
		...
		*/
		...
		ast_log_chan(NULL, LOG_WARNING, "unrecognized mode %s\n", mode);
		return -1;
	}
...
static char *__say_cli_init(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	case CLI_INIT:
		...
		e->usage =
			...
			"       say load old\n"
			"           Set old say method, coded in gabpbx core\n";
		return NULL;
	case CLI_GENERATE:
		return NULL;
	}
...
static int playback_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(chan, LOG_WARNING, "Playback requires an argument (filename)\n");
		return -1;
	}
		...
		while (!res && (front = ast_strsep(&back, '&', AST_STRSEP_STRIP | AST_STRSEP_TRIM))) {
			...
			if (!res) {
				...
				if (!ast_check_hangup(chan)) {
					ast_log_chan(chan, LOG_WARNING, "Playback failed on %s for %s\n", ast_channel_name(chan), (char *)data);
				}
				res = 0;
				mres = 1;
			}
...
static int reload(void)
{
	...
	if ((newcfg = ast_config_load("say.conf", config_flags)) == CONFIG_STATUS_FILEUNCHANGED) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Config file say.conf is in an invalid format.  Aborting.\n");
		return 0;
	}

	if (say_cfg) {
		ast_config_destroy(say_cfg);
		ast_log_chan(NULL, LOG_NOTICE, "Reloading say.conf\n");
	}
	say_cfg = newcfg;
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_DEFAULT, "Sound File Playback Application",
