...
static int handle_playtones(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(str)) {
		ast_log_chan(NULL, LOG_NOTICE,"Nothing to play\n");
		return -1;
	}
	...
	if (res) {
		ast_log_chan(NULL, LOG_NOTICE, "Unable to start playtones\n");
	}
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Playtones Application");
