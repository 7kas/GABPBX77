...
static int privacy_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_channel_caller(chan)->id.number.valid
		...
		ast_verb_chan(NULL, 3, "CallerID number present: Skipping\n");
	} else {
		...
		if (!ast_strlen_zero(args.maxretries)) {
			if (sscanf(args.maxretries, "%30d", &x) == 1 && x > 0) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Invalid max retries argument: '%s'\n", args.maxretries);
			}
		}
		if (!ast_strlen_zero(args.minlength)) {
			if (sscanf(args.minlength, "%30d", &x) == 1 && x > 0) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Invalid min length argument: '%s'\n", args.minlength);
			}
		}
		...
		if ((retries < maxretries) && res >= 0) {
			...
			ast_verb_chan(NULL, 3, "Changed Caller*ID number to '%s'\n", phone);
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Require phone number to be entered, if no CallerID sent");
