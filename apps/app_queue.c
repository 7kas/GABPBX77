 ...
 *             When gabpbx is restarted, each agent will be automatically
 ...
 * Added Priority jumping code for adding and removing queue members by Jonathan Stanton <gabpbx@doilooklikeicare.com>
 *
...
#include "gabpbx/core_local.h"
#include "gabpbx/mixmonitor.h"
#include "gabpbx/bridge_basic.h"
...
/*** DOCUMENTATION
	<application name="Queue" language="en_US">
		...
		<syntax>
			...
			<parameter name="announceoverride" argsep="&amp;">
				...
				will be searched for in the GABpbx sounds directory. If the
				filename is able to be parsed as a URL, GABpbx will
	...
	<manager name="QueueAdd" language="en_US">
		...
		<syntax>
			...
			<parameter name="Penalty">
				<para>A penalty (number) to apply to this member. GABpbx will distribute calls to members with higher penalties only after attempting to distribute calls to those with lower penalty.</para>
			</parameter>
...
struct callattempt {
	...
	char interface[256];			/*!< An GABpbx dial string (not a channel name) */
	int metric;
...
static struct ast_manager_event_blob *queue_multi_channel_to_ami(const char *type, struct stasis_message *message)
{
	...
	if (caller) {
		...
		if (!caller_event_string) {
			ast_log_chan(NULL, LOG_NOTICE, "No caller event string, bailing\n");
			return NULL;
		}
	}
	...
	if (agent) {
		...
		if (!agent_event_string) {
			ast_log_chan(NULL, LOG_NOTICE, "No agent event string, bailing\n");
			return NULL;
		}
	}
...
static struct member *create_queue_member(const char *interface, const char *membername, int penalty, int paused, const char *state_interface, int ringinuse, int wrapuptime)
{
	...
	if ((cur = ao2_alloc(sizeof(*cur), destroy_queue_member_cb))) {
		...
		if (!strchr(cur->interface, '/')) {
			ast_log_chan(NULL, LOG_WARNING, "No location at interface '%s'\n", interface);
		}
...
static int insert_penaltychange(const char *list_name, const char *content, const int linenum)
{
	...
	if (!(maxstr = strchr(contentdup, ','))) {
		ast_log_chan(NULL, LOG_WARNING, "Improperly formatted penaltychange rule at line %d. Ignoring.\n", linenum);
		ast_free(rule);
		return -1;
	}
	...
	*maxstr++ = '\0';
	...
	if ((penaltychangetime = atoi(timestr)) < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Improper time parameter specified for penaltychange rule at line %d. Ignoring.\n", linenum);
		ast_free(rule);
		return -1;
	}
	...
	if (!inserted) {
		ast_log_chan(NULL, LOG_WARNING, "Unknown rule list name %s; ignoring.\n", list_name);
		ast_free(rule);
		return -1;
	}
	return 0;
...
static int load_realtime_rules(void)
{
	...
	if (!ast_check_realtime("queue_rules")) {
		ast_log_chan(NULL, LOG_WARNING, "Missing \"queue_rules\" in extconfig.conf\n");
		return 0;
	}
	if (!(cfg = ast_load_realtime_multientry("queue_rules", "rule_name LIKE", "%", SENTINEL))) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to load queue rules from realtime\n");
		return 0;
	}
	while ((rulecat = ast_category_browse(cfg, rulecat))) {
		...
		if (!(timestr) || sscanf(timestr, "%30d", &penaltychangetime) != 1) {
			ast_log_chan(NULL, LOG_NOTICE, "Failed to parse time (%s) for one of the %s rules,	skipping it\n",
	...
	while ((option = strsep(&value_copy, ","))) {
		if (!strcasecmp(option, "paused")) {
			...
			*empty = 0;
			...
			ast_log_chan(NULL, LOG_WARNING, "Unknown option %s for '%s'\n", option, joinempty ? "joinempty" : "leavewhenempty");
		}
	}
}
...
static void queue_set_param(struct call_queue *q, const char *param, const char *val, int linenum, int failunknown)
{
	if (!strcasecmp(param, "musicclass") ||
		...
		if (!(q->roundingseconds == 0 || q->roundingseconds == 5 || q->roundingseconds == 10
			...
			if (linenum >= 0) {
				ast_log_chan(NULL, LOG_WARNING, "'%s' isn't a valid value for %s "
		...
		if (strategy < 0) {
			ast_log_chan(NULL, LOG_WARNING, "'%s' isn't a valid strategy for queue '%s', using ringall instead\n",
				val, q->name);
		...
		if (strategy == QUEUE_STRATEGY_LINEAR) {
			ast_log_chan(NULL, LOG_WARNING, "Changing to the linear strategy currently requires gabpbx to be restarted.\n");
			return;
		}
		q->strategy = strategy;
		...
		if (linenum >= 0) {
			ast_log_chan(NULL, LOG_WARNING, "Unknown keyword in queue '%s': %s at line %d of queues.conf\n",
				q->name, param, linenum);
		} else {
			ast_log_chan(NULL, LOG_WARNING, "Unknown keyword in queue '%s': %s\n", q->name, param);
		}
	}
}
...
static void rt_handle_member_record(struct call_queue *q, char *category, struct ast_config *member_config)
{
	...
	if (ast_strlen_zero(rt_uniqueid)) {
		ast_log_chan(NULL, LOG_WARNING, "Realtime field 'uniqueid' is empty for member %s\n",
			S_OR(membername, "NULL"));
		return;
	}

	if (ast_strlen_zero(interface)) {
		ast_log_chan(NULL, LOG_WARNING, "Realtime field 'interface' is empty for member %s\n",
			S_OR(membername, "NULL"));
		return;
	}
	...
	if ((config_val = ast_variable_retrieve(member_config, category, realtime_ringinuse_field))) {
		if (ast_true(config_val)) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Invalid value of '%s' field for %s in queue '%s'\n", realtime_ringinuse_field, interface, q->name);
		}
	}
...
static struct call_queue *find_queue_by_name_rt(const char *queuename, struct ast_variable *queue_vars, struct ast_config *member_config)
{
	...
	if ((q = ao2_t_find(queues, &tmpq, OBJ_POINTER, "Check if static queue exists"))) {
		...
		if (!q->realtime) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Static queue '%s' already exists. Not loading from realtime\n", q->name);
			ao2_unlock(q);
			return q;
		}
		...
		*reason = QUEUE_FULL;
		...
		while (cur) {
			...
			if (!inserted && (qe->prio >= cur->prio) && position && (position <= pos + 1)) {
				...
				if (position < pos) {
					ast_log_chan(NULL, LOG_NOTICE, "Asked to be inserted at position %d but forced into position %d due to higher priority callers\n", position, pos);
				}
			}
			cur->pos = ++pos;
			prev = cur;
...
static int say_position(struct queue_ent *qe, int ringing)
{
	...
	ast_verb_chan(NULL, 3, "Hold time for %s is %ld minute(s) %ld seconds\n", qe->parent->name, avgholdmins, avgholdsecs);
...
posout:
	if (qe->parent->announceposition) {
		ast_verb_chan(NULL, 3, "Told %s in %s their queue position (which was %d)\n",
...
static int ring_entry(struct queue_ent *qe, struct callattempt *tmp, int *busies)
{
	...
	if ((res = ast_call(tmp->chan, location, 0))) {
		...
		ast_verb_chan(NULL, 3, "Couldn't call %s\n", tmp->interface);
		do_hang(tmp);
		++*busies;
		return 0;
	}
...
static int say_periodic_announcement(struct queue_ent *qe, int ringing)
{
	...
	ast_verb_chan(NULL, 3, "Playing periodic announcement\n");
...
static void rna(int rnatime, struct queue_ent *qe, struct ast_channel *peer, char *interface, char *membername, int autopause)
{
	...
	ast_verb_chan(NULL, 3, "Nobody picked up in %d ms\n", rnatime);
	...
	if (qe->parent->autopause != QUEUE_AUTOPAUSE_OFF && autopause) {
		...
		if (qe->parent->autopause == QUEUE_AUTOPAUSE_ON) {
			if (!set_member_paused(qe->parent->name, interface, "Auto-Pause", 1)) {
				ast_verb_chan(NULL, 3, "Auto-Pausing Queue Member %s in queue %s since they failed to answer.\n",
					interface, qe->parent->name);
			} else {
				ast_verb_chan(NULL, 3, "Failed to pause Queue Member %s in queue %s!\n", interface, qe->parent->name);
			}
		} else {
			...
			* the function will stop in all queues */
			if (!set_member_paused("", interface, "Auto-Pause", 1)) {
				ast_verb_chan(NULL, 3, "Auto-Pausing Queue Member %s in all queues since they failed to answer on queue %s.\n",
						interface, qe->parent->name);
			} else {
				ast_verb_chan(NULL, 3, "Failed to pause Queue Member %s in all queues!\n", interface);
			}
		}
	}
	return;
	...
	while ((*to = ast_remaining_ms(start_time_tv, orig)) && !peer) {
		...
		for (o = start; o; o = o->call_next) {
			...
			if (o->stillgoing && (o->chan) &&  (ast_channel_state(o->chan) == AST_STATE_UP)) {
				if (!peer) {
					ast_verb_chan(NULL, 3, "%s answered %s\n", ochan_name, inchan_name);
					if (o->orig_chan_name
				...
				if (!ast_strlen_zero(ast_channel_call_forward(o->chan)) && !forwardsallowed) {
					ast_verb_chan(NULL, 3, "Forwarding %s to '%s' prevented.\n", inchan_name, ast_channel_call_forward(o->chan));
					...
					ast_verb_chan(NULL, 3, "Now forwarding %s to '%s/%s' (thanks to %s)\n", inchan_name, tech, stuff, ochan_name);
					/* Setup parameters */
					...
					if (!o->chan) {
						...
						if (ast_call(o->chan, stuff, 0)) {
							ast_log_chan(NULL, LOG_NOTICE, "Forwarding failed to dial '%s/%s'\n",
								tech, stuff);
							failed = 1;
						}
					}
				...
				if (f) {
					if (f->frametype == AST_FRAME_CONTROL) {
						...
						case AST_CONTROL_BUSY:
							ast_verb_chan(NULL, 3, "%s is busy\n", ochan_name);
						...
						case AST_CONTROL_CONGESTION:
							ast_verb_chan(NULL, 3, "%s is circuit-busy\n", ochan_name);
						...
						case AST_CONTROL_RINGING:
							ast_verb_chan(NULL, 3, "%s is ringing\n", ochan_name);
						...
						case AST_CONTROL_CONNECTED_LINE:
							if (o->block_connected_update) {
								ast_verb_chan(NULL, 3, "Connected line update to %s prevented.\n", inchan_name);
								break;
							}
							if (qe->parent->strategy == QUEUE_STRATEGY_RINGALL) {
								struct ast_party_connected_line connected;

								ast_verb_chan(NULL, 3, "%s connected line has changed. Saving it until answer for %s\n", ochan_name, inchan_name);
						...
						case AST_CONTROL_REDIRECTING:
							...
							if (o->block_connected_update) {
								ast_verb_chan(NULL, 3, "Redirecting update to %s prevented\n",
									inchan_name);
								break;
							}
							ast_verb_chan(NULL, 3, "%s redirecting info has changed, passing it to %s\n",
								ochan_name, inchan_name);
		...
		if (winner == in) {
			...
			if (!f || ((f->frametype == AST_FRAME_CONTROL) && (f->subclass.integer == AST_CONTROL_HANGUP))) {
				...
				*to = -1;
				...
				ast_verb_chan(NULL, 3, "User hit %c to disconnect call.\n", f->subclass.integer);
				*to = 0;
				ast_frfree(f);
				canceled_by_caller = 1;
			} else if ((f->frametype == AST_FRAME_DTMF) && valid_exit(qe, f->subclass.integer)) {
				ast_verb_chan(NULL, 3, "User pressed digit: %c\n", f->subclass.integer);
				*to = 0;
...
static int calc_metric(struct call_queue *q, struct member *mem, int pos, struct queue_ent *qe, struct callattempt *tmp)
{
	...
	case QUEUE_STRATEGY_LEASTRECENT:
		...
		break;
	default:
		ast_log_chan(NULL, LOG_WARNING, "Can't calculate metric for unknown strategy %d\n", q->strategy);
		break;
	}
	return 0;
}
		...
		struct ast_attended_transfer_message *atxfer_msg)
{
	...
	if (!transfer_str) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to log attended transfer to queue log\n");
		return;
	}
		...
		struct stasis_message *msg)
{
	...
	if (!optimization->source_chan_uniqueid) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to track local channel optimization for channel %s. Expect further errors\n", local_one->base->name);
		return;
	}
		...
		struct stasis_message *msg)
{
	...
	if (!optimization->in_progress) {
		ast_log_chan(NULL, LOG_WARNING, "Told of a local optimization end when we had no previous begin\n");
		return;
	}

	if (id != optimization->id) {
		ast_log_chan(NULL, LOG_WARNING, "Local optimization end event ID does not match begin (%u != %u)\n",
				id, optimization->id);
		return;
	}
...
static void setup_mixmonitor(struct queue_ent *qe, const char *filename)
{
	...
	if (ast_start_mixmonitor(qe->chan, file_with_ext, mixmonargs)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to start mixmonitor. Is the MixMonitor app loaded?\n");
	}
}
	...
	*/
	...
	if (!peer) {
		...
		if (announce || qe->parent->reportholdtime || qe->parent->memberdelay) {
			...
			if (!res2) {
				if (qe->parent->memberdelay) {
					ast_log_chan(NULL, LOG_NOTICE, "Delaying member connect for %d seconds\n", qe->parent->memberdelay);
				...
				if (!res2 && announce) {
					...
					while ((front = ast_strsep(&announcefiles, '&', AST_STRSEP_STRIP | AST_STRSEP_TRIM))) {
						if (play_file(peer, front) < 0) {
							ast_log_chan(NULL, LOG_ERROR, "play_file failed for '%s' on %s\n", front, ast_channel_name(peer));
						}
					}
				}
			...
			if (ast_check_hangup(peer)) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Agent on %s hungup on the customer.\n", ast_channel_name(peer));
				...
				ast_log_chan(NULL, LOG_NOTICE, "Caller was about to talk to agent on %s but the caller hungup.\n", ast_channel_name(peer));
		...
		if (res < 0) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Had to drop call because I couldn't make %s compatible with %s\n", ast_channel_name(qe->chan), ast_channel_name(peer));
			record_abandoned(qe);
			ast_channel_publish_dial(qe->chan, peer, member->interface, ast_hangup_cause_to_dial_status(ast_channel_hangupcause(peer)));
			ast_autoservice_chan_hangup_peer(qe->chan, peer);
			pending_members_remove(member);
		...
		if (!ast_strlen_zero(qe->parent->sound_callerannounce)) {
			if (play_file(qe->chan, qe->parent->sound_callerannounce)) {
				ast_log_chan(NULL, LOG_WARNING, "Announcement file '%s' is unavailable, continuing anyway...\n", qe->parent->sound_callerannounce);
			}
		}
		...
		if (!ast_strlen_zero(gosubexec)) {
			...
			if (gosub_args) {
				...
				ast_log_chan(NULL, LOG_ERROR, "Could not Allocate string for Gosub arguments -- Gosub Call Aborted!\n");
			}
		}
		...
		if (!ast_strlen_zero(agi)) {
			...
			if (application) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Asked to execute an AGI on this channel, but could not find application (agi)!\n");
			}
		}
		qe->handled++;
...
static void dump_queue_members(struct call_queue *pm_queue)
{
	...
	if (ast_str_strlen(value) && !cur_member) {
		if (ast_db_put(pm_family, pm_queue->name, ast_str_buffer(value))) {
			ast_log_chan(NULL, LOG_WARNING, "failed to create persistent dynamic entry!\n");
		}
	} else {
	...
	if (mem->realtime && !ast_strlen_zero(mem->rt_uniqueid)) {
		if (realtime_reason_paused) {
			if (ast_update_realtime("queue_members", "uniqueid", mem->rt_uniqueid, "reason_paused", S_OR(reason, ""), "paused", paused ? "1" : "0", SENTINEL) < 0) {
				ast_log_chan(NULL, LOG_WARNING, "Failed update of realtime queue member %s:%s %spause and reason '%s'\n",
			...
			if (ast_update_realtime("queue_members", "uniqueid", mem->rt_uniqueid, "paused", paused ? "1" : "0", SENTINEL) < 0) {
				ast_log_chan(NULL, LOG_WARNING, "Failed %spause update of realtime queue member %s:%s\n",
...
static int set_member_value_help_members(struct call_queue *q, const char *interface, int property, int value)
{
	...
	default:
		ast_log_chan(NULL, LOG_ERROR, "Attempted to set invalid property\n");
		return 0;
	}
}
...
static int set_member_value(const char *queuename, const char *interface, int property, int value)
{
	...
	case MEMBER_PENALTY:
		if (value < 0 && !negative_penalty_invalid) {
			ast_log_chan(NULL, LOG_ERROR, "Invalid penalty (%d)\n", value);
			...
			if (queue_config) {
				...
				while ((category = ast_category_browse(queue_config, category))) {
					...
					if (ast_strlen_zero(name)) {
						ast_log_chan(NULL, LOG_WARNING, "Ignoring realtime queue with a NULL or empty 'name.'\n");
						continue;
					}
	...
	if (foundinterface) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Invalid queuename\n");
	} else {
		ast_log_chan(NULL, LOG_ERROR, "Invalid interface\n");
	}
...
static void reload_queue_members(void)
{
	...
	for (entry = db_tree; entry; entry = entry->next) {
		...
		if (!cur_queue) {
			 ...
			 * database */
			ast_log_chan(NULL, LOG_WARNING, "Error loading persistent queue: '%s': it does not exist\n", queue_name);
		...
		while ((member = strsep(&cur_ptr, ",|"))) {
			...
			if (!penalty_tok) {
				ast_log_chan(NULL, LOG_WARNING, "Error parsing persistent member string for '%s' (penalty)\n", queue_name);
				break;
			}
			penalty = strtol(penalty_tok, NULL, 10);
			if (errno == ERANGE) {
				ast_log_chan(NULL, LOG_WARNING, "Error converting penalty: %s: Out of range.\n", penalty_tok);
				break;
			}

			if (!paused_tok) {
				ast_log_chan(NULL, LOG_WARNING, "Error parsing persistent member string for '%s' (paused)\n", queue_name);
				break;
			}
			paused = strtol(paused_tok, NULL, 10);
			if ((errno == ERANGE) || paused < 0 || paused > 1) {
				ast_log_chan(NULL, LOG_WARNING, "Error converting paused: %s: Expected 0 or 1.\n", paused_tok);
				break;
			}
			...
			if (!ast_strlen_zero(wrapuptime_tok)) {
				...
				if (errno == ERANGE) {
					ast_log_chan(NULL, LOG_WARNING, "Error converting wrapuptime: %s: Out of range.\n", wrapuptime_tok);
					break;
				}
			}
			...
			if (add_to_queue(queue_name, interface, membername, penalty, paused, 0, state_interface, reason_paused, wrapuptime) == RES_OUTOFMEMORY) {
				ast_log_chan(NULL, LOG_ERROR, "Out of Memory when reloading persistent queue member\n");
				break;
			}
		}
		queue_t_unref(cur_queue, "Expire reload reference");
	...
	if (db_tree) {
		ast_log_chan(NULL, LOG_NOTICE, "Queue members successfully reloaded from database.\n");
...
static int pqm_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "PauseQueueMember requires an argument ([queuename],interface[,options][,reason])\n");
		return -1;
	}
	...
	if (ast_strlen_zero(args.interface)) {
		ast_log_chan(NULL, LOG_WARNING, "Missing interface argument to PauseQueueMember ([queuename],interface[,options[,reason]])\n");
		return -1;
	}

	if (set_member_paused(args.queuename, args.interface, args.reason, 1)) {
		ast_log_chan(NULL, LOG_WARNING, "Attempt to pause interface %s, not found\n", args.interface);
...
static int upqm_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "UnpauseQueueMember requires an argument ([queuename],interface[,options[,reason]])\n");
		return -1;
	}
	...
	if (ast_strlen_zero(args.interface)) {
		ast_log_chan(NULL, LOG_WARNING, "Missing interface argument to UnpauseQueueMember ([queuename],interface[,options[,reason]])\n");
		return -1;
	}

	if (set_member_paused(args.queuename, args.interface, args.reason, 0)) {
		ast_log_chan(NULL, LOG_WARNING, "Attempt to unpause interface %s, not found\n", args.interface);
...
static int rqm_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "RemoveQueueMember requires an argument (queuename[,interface])\n");
		return -1;
	}
	...
	case RES_OKAY:
		...
		ast_log_chan(NULL, LOG_NOTICE, "Removed interface '%s' from queue '%s'\n", args.interface, args.queuename);
	...
	case RES_NOSUCHQUEUE:
		ast_log_chan(NULL, LOG_WARNING, "Unable to remove interface from queue '%s': No such queue\n", args.queuename);
		pbx_builtin_setvar_helper(chan, "RQMSTATUS", "NOSUCHQUEUE");
		res = 0;
		break;
	case RES_NOT_DYNAMIC:
		ast_log_chan(NULL, LOG_WARNING, "Unable to remove interface from queue '%s': '%s' is not a dynamic member\n", args.queuename, args.interface);
...
static int aqm_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "AddQueueMember requires an argument (queuename[,interface[,penalty[,options[,membername[,stateinterface][,wrapuptime]]]]])\n");
		return -1;
	}
	...
	if (!ast_strlen_zero(args.penalty)) {
		if ((sscanf(args.penalty, "%30d", &penalty) != 1) || penalty < 0) {
			ast_log_chan(NULL, LOG_WARNING, "Penalty '%s' is invalid, must be an integer >= 0\n", args.penalty);
			penalty = 0;
		}
	}
	...
	case RES_OKAY:
		...
		ast_log_chan(NULL, LOG_NOTICE, "Added interface '%s' to queue '%s'\n", args.interface, args.queuename);
		pbx_builtin_setvar_helper(chan, "AQMSTATUS", "ADDED");
		res = 0;
		break;
	case RES_EXISTS:
		ast_log_chan(NULL, LOG_WARNING, "Unable to add interface '%s' to queue '%s': Already there\n", args.interface, args.queuename);
		pbx_builtin_setvar_helper(chan, "AQMSTATUS", "MEMBERALREADY");
		res = 0;
		break;
	case RES_NOSUCHQUEUE:
		ast_log_chan(NULL, LOG_WARNING, "Unable to add interface to queue '%s': No such queue\n", args.queuename);
		pbx_builtin_setvar_helper(chan, "AQMSTATUS", "NOSUCHQUEUE");
		res = 0;
		break;
	case RES_OUTOFMEMORY:
		ast_log_chan(NULL, LOG_ERROR, "Out of memory adding interface %s to queue %s\n", args.interface, args.queuename);
		break;
	}
...
static int ql_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "QueueLog requires arguments (queuename,uniqueid,membername,event[,additionalinfo]\n");
		return -1;
	}
...
static void copy_rules(struct queue_ent *qe, const char *rulename)
{
	...
	if (rl_iter) {
		AST_LIST_TRAVERSE(&rl_iter->rules, pr_iter, list) {
			...
			if (!new_pr) {
				ast_log_chan(NULL, LOG_ERROR, "Memory allocation error when copying penalty rules! Aborting!\n");
				break;
			}
...
static int queue_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "Queue requires an argument: queuename[,options[,URL[,announceoverride[,timeout[,agi[,gosub[,rule[,position]]]]]]]]\n");
		return -1;
	}
	...
	if (max_forwards <= 0) {
		ast_log_chan(NULL, LOG_WARNING, "Channel '%s' cannot enter queue. Max forwards exceeded\n", ast_channel_name(chan));
		return -1;
	}
	...
	if (user_priority) {
		if (sscanf(user_priority, "%30d", &prio) == 1) {
			...
			ast_log_chan(NULL, LOG_WARNING, "${QUEUE_PRIO}: Invalid value (%s), channel %s.\n",
	...
	if ((max_penalty_str = pbx_builtin_getvar_helper(chan, "QUEUE_MAX_PENALTY"))) {
		if (sscanf(max_penalty_str, "%30d", &max_penalty) == 1) {
			...
			ast_log_chan(NULL, LOG_WARNING, "${QUEUE_MAX_PENALTY}: Invalid value (%s), channel %s.\n",
	...
	if ((min_penalty_str = pbx_builtin_getvar_helper(chan, "QUEUE_MIN_PENALTY"))) {
		if (sscanf(min_penalty_str, "%30d", &min_penalty) == 1) {
			...
			ast_log_chan(NULL, LOG_WARNING, "${QUEUE_MIN_PENALTY}: Invalid value (%s), channel %s.\n",
	...
	if ((raise_penalty_str = pbx_builtin_getvar_helper(chan, "QUEUE_RAISE_PENALTY"))) {
		if (sscanf(raise_penalty_str, "%30d", &raise_penalty) == 1) {
			...
			ast_log_chan(NULL, LOG_WARNING, "${QUEUE_RAISE_PENALTY}: Invalid value (%s), channel %s.\n",
	...
	if (args.position) {
		...
		if (position < 0) {
			ast_log_chan(NULL, LOG_WARNING, "Invalid position '%s' given for call to queue '%s'. Assuming no preference for position\n", args.position, args.queuename);
			position = 0;
		}
	}
	...
	if (join_queue(args.queuename, &qe, &reason, position)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to join queue '%s'\n", args.queuename);
...
check_turns:
	...
	for (;;) {
		...
		if (noption && tries >= ao2_container_count(qe.parent->members)) {
			ast_verb_chan(NULL, 3, "Exiting on time-out cycle\n");
			ast_queue_log(args.queuename, ast_channel_uniqueid(chan), "NONE", "EXITWITHTIMEOUT",
				"%d|%d|%ld", qe.pos, qe.opos, (long) (time(NULL) - qe.start));
			record_abandoned(&qe);
			reason = QUEUE_TIMEOUT;
...
static int queue_function_var(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_ERROR, "%s requires an argument: queuename\n", cmd);
		return -1;
	}
	...
	if ((q = find_load_queue_rt_friendly(data))) {
		...
		ast_log_chan(NULL, LOG_WARNING, "queue %s was not found\n", data);
	}
...
static struct member *get_interface_helper(struct call_queue *q, const char *interface)
{
	...
	if (ast_strlen_zero(interface)) {
		ast_log_chan(NULL, LOG_ERROR, "QUEUE_MEMBER: Missing required interface argument.\n");
		return NULL;
	}

	m = interface_exists(q, interface);
	if (!m) {
		ast_log_chan(NULL, LOG_ERROR, "Queue member interface '%s' not in queue '%s'.\n",
			interface, q->name);
	}
	return m;
}
...
static int queue_function_mem_read(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	if ((q = find_load_queue_rt_friendly(args.queuename))) {
		...
		if (!strcasecmp(args.option, "logged")) {
			...
			ast_log_chan(NULL, LOG_ERROR, "%s: Invalid option '%s' provided.\n", cmd, args.option);
		}
		ao2_unlock(q);
...
static int queue_function_mem_write(struct ast_channel *chan, const char *cmd, char *data, const char *value)
{
	...
	if (!strcasecmp(args.option, "penalty")) {
		if (set_member_value(args.queuename, args.interface, MEMBER_PENALTY, memvalue)) {
			ast_log_chan(NULL, LOG_ERROR, "Invalid interface, queue, or penalty\n");
			return -1;
		}
	} else if (!strcasecmp(args.option, "paused")) {
		memvalue = (memvalue <= 0) ? 0 : 1;
		if (set_member_paused(args.queuename, args.interface, NULL, memvalue)) {
			ast_log_chan(NULL, LOG_ERROR, "Invalid interface or queue\n");
			return -1;
		}
	} else if (!strcasecmp(args.option, "ignorebusy") /* ignorebusy is legacy */
		|| !strcasecmp(args.option, "ringinuse")) {
...
static int queue_function_qac_dep(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	if (depflag) {
		depflag = 0;
		ast_log_chan(NULL, LOG_NOTICE, "The function QUEUE_MEMBER_COUNT has been deprecated in favor of the QUEUE_MEMBER function and will not be in further releases.\n");
	}
...
static int queue_function_queuegetchannel(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_ERROR, "Missing argument. QUEUE_GET_CHANNEL(<queuename>,<position>)\n");
		return -1;
	}
	...
	if (ast_strlen_zero(args.queuename)) {
		ast_log_chan(NULL, LOG_ERROR, "The <queuename> parameter is required.\n");
		return -1;
	}
	...
	if (ast_strlen_zero(args.position)) {
		...
		if (sscanf(args.position, "%30d", &position) != 1) {
			ast_log_chan(NULL, LOG_ERROR, "<position> parameter must be an integer.\n");
			return -1;
		}
		if (position < 1) {
			ast_log_chan(NULL, LOG_ERROR, "<position> parameter must be an integer greater than zero.\n");
			return -1;
		}
	}
...
static int queue_function_queuewaitingcount(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_ERROR, "QUEUE_WAITING_COUNT requires an argument: queuename\n");
		return -1;
	}
...
static int queue_function_queuememberlist(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_ERROR, "QUEUE_MEMBER_LIST requires an argument: queuename\n");
		return -1;
	}
	...
	if ((q = find_load_queue_rt_friendly(data))) {
		...
		while ((m = ao2_iterator_next(&mem_iter))) {
			...
			if (buflen >= len - 2) {
				ao2_ref(m, -1);
				ast_log_chan(NULL, LOG_WARNING, "Truncating list\n");
				break;
			}
			ao2_ref(m, -1);
		}
...
static int queue_function_memberpenalty_read(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_ERROR, "Missing argument. QUEUE_MEMBER_PENALTY(<queuename>,<interface>)\n");
		return -1;
	}
...
static int queue_function_memberpenalty_write(struct ast_channel *chan, const char *cmd, char *data, const char *value)
{
	...
	if (ast_strlen_zero(args.interface)) {
		ast_log_chan(NULL, LOG_ERROR, "<interface> parameter can't be null\n");
		return -1;
	}
...
static int reload_queue_rules(int reload)
{
	...
	if (!(cfg = ast_config_load("queuerules.conf", config_flags))) {
		ast_log_chan(NULL, LOG_NOTICE, "No queuerules.conf file found, queues will not follow penalty rules\n");
		return AST_MODULE_LOAD_SUCCESS;
	} else if (cfg == CONFIG_STATUS_FILEUNCHANGED) {
		ast_log_chan(NULL, LOG_NOTICE, "queuerules.conf has not changed since it was last loaded. Not taking any action.\n");
		return AST_MODULE_LOAD_SUCCESS;
	} else if (cfg == CONFIG_STATUS_FILEINVALID) {
		ast_log_chan(NULL, LOG_ERROR, "Config file queuerules.conf is in an invalid format.  Aborting.\n");
		return AST_MODULE_LOAD_SUCCESS;
	}
		...
		if (!(new_rl = ast_calloc(1, sizeof(*new_rl)))) {
			...
			for (rulevar = ast_variable_browse(cfg, rulecat); rulevar; rulevar = rulevar->next)
				if(!strcasecmp(rulevar->name, "penaltychange"))
					...
					ast_log_chan(NULL, LOG_WARNING, "Don't know how to handle rule type '%s' on line %d\n", rulevar->name, rulevar->lineno);
		}
	}
...
static void reload_single_member(const char *memberdata, struct call_queue *q)
{
	...
	if (ast_strlen_zero(memberdata)) {
		ast_log_chan(NULL, LOG_WARNING, "Empty queue member definition. Moving on!\n");
		return;
	}
	...
	if (!ast_strlen_zero(args.ringinuse)) {
		...
		if (ast_true(tmp)) {
			...
			ast_log_chan(NULL, LOG_ERROR, "Member %s has an invalid ringinuse value. Using %s ringinuse value.\n",
				membername, q->name);
	...
	if (!ast_strlen_zero(args.paused)) {
		...
		if (ast_true(tmp)) {
			...
			ast_log_chan(NULL, LOG_ERROR, "Member %s has an invalid paused value.\n", membername);
			paused = 0;
		}
	} else {
		paused = 0;
...
static void reload_single_queue(struct ast_config *cfg, struct ast_flags *mask, const char *queuename)
{
	...
	if (q->found) {
		ast_log_chan(NULL, LOG_WARNING, "Queue '%s' already defined! Skipping!\n", queuename);
		if (!new) {
...
static int reload_queues(int reload, struct ast_flags *mask, const char *queuename)
{
	...
	if (!(cfg = ast_config_load("queues.conf", config_flags))) {
		ast_log_chan(NULL, LOG_NOTICE, "No call queueing config file (queues.conf), so no call queues\n");
		return -1;
	} else if (cfg == CONFIG_STATUS_FILEUNCHANGED) {
		return 0;
	} else if (cfg == CONFIG_STATUS_FILEINVALID) {
		ast_log_chan(NULL, LOG_ERROR, "Config file queues.conf is in an invalid format.  Aborting.\n");
		return -1;
	}
...
static char *handle_queue_change_priority_caller(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	if (a->argc < 8) {
		...
		ast_log_chan(NULL, LOG_ERROR, "<priority> parameter must be an integer.\n");
		return CLI_SHOWUSAGE;
	} else if (a->argc == 9) {
...
static int qupd_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "QueueUpdate requires arguments (queuename,uniqueid,agent,status,talktime,params[totaltime,callednumber])\n");
		return -1;
	}
	...
	if (ast_strlen_zero(args.queuename) || ast_strlen_zero(args.uniqueid) || ast_strlen_zero(args.agent) || ast_strlen_zero(args.status)) {
		ast_log_chan(NULL, LOG_WARNING, "Missing argument to QueueUpdate (queuename,uniqueid,agent,status,talktime,params[totaltime|callednumber])\n");
		return -1;
	}
	...
	if (!q) {
		ast_log_chan(NULL, LOG_WARNING, "QueueUpdate could not find requested queue '%s'\n", args.queuename);
		return 0;
	}
...
static int load_module(void)
{
	...
	if (!member_config) {
		...
		if ((config_val = ast_variable_retrieve(member_config, NULL, "ringinuse"))) {
			ast_log_chan(NULL, LOG_NOTICE, "ringinuse field entries found in queue_members table. Using 'ringinuse'\n");
			realtime_ringinuse_field = "ringinuse";
		} else if ((config_val = ast_variable_retrieve(member_config, NULL, "ignorebusy"))) {
			ast_log_chan(NULL, LOG_NOTICE, "ignorebusy field found in queue_members table with no ringinuse field. Using 'ignorebusy'\n");
			realtime_ringinuse_field = "ignorebusy";
		} else {
			ast_log_chan(NULL, LOG_NOTICE, "No entries were found for ringinuse/ignorebusy in queue_members table. Using 'ringinuse'\n");
			realtime_ringinuse_field = "ringinuse";
		}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "True Call Queueing",
