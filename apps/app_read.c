	...
	<application name="Read" language="en_US">
		...
		<syntax>
			...
			<parameter name="filenames" argsep="&amp;">
				...
				slash), it will be searched for in the GABpbx sounds
				directory. If the filename is able to be parsed as a URL,
				GABpbx will download the file and then begin playback on
...
static int read_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(chan, LOG_WARNING, "Read requires an argument (variable)\n");
		return 0;
	}
	...
	if (!ast_strlen_zero(arglist.maxdigits)) {
		...
		if ((maxdigits < 1) || (maxdigits > 255)) {
			...
			ast_verb_chan(chan, 3, "Accepting a maximum of %d digits.\n", maxdigits);
	}
	if (ast_strlen_zero(arglist.variable)) {
		ast_log_chan(chan, LOG_WARNING, "Invalid! Usage: Read(variable[,filename][,maxdigits][,option][,attempts][,timeout])\n\n");
		return 0;
	}
		...
		while (tries && !res) {
			...
			if (res > -1) {
				...
				if (!ast_strlen_zero(tmp)) {
					ast_verb_chan(chan, 3, "User entered '%s'\n", tmp);
					tries = 0;
				} else {
					tries--;
					if (tries)
						ast_verb_chan(chan, 3, "User entered nothing, %d chance%s left\n", tries, (tries != 1) ? "s" : "");
					else
						ast_verb_chan(chan, 3, "User entered nothing.\n");
				}
				res = 0;
			} else {
				pbx_builtin_setvar_helper(chan, arglist.variable, tmp);
				ast_verb_chan(chan, 3, "User disconnected\n");
			}
		}
	}
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Read Variable Application");
