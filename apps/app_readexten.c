...
static int readexten_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "ReadExten requires at least one argument\n");
	...
	if (ast_strlen_zero(arglist.variable)) {
		ast_log_chan(NULL, LOG_WARNING, "Usage: ReadExten(variable[,filename[,context[,options[,timeout]]]])\n");
		pbx_builtin_setvar_helper(chan, "READEXTENSTATUS", "ERROR");
		return 0;
	}
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Read and evaluate extension validity");
