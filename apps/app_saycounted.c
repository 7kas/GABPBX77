...
/*** DOCUMENTATION
	<application name="SayCountedNoun" language="en_US">
		...
		<description>
			...
			to implement in the GABpbx dialplan language.</para>
...
static int saycountednoun_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "SayCountedNoun requires two arguments (<number>,<noun>)\n");
		return -1;
	}
	...
	if (args.argc != 2) {
		ast_log_chan(NULL, LOG_WARNING, "SayCountedNoun requires two arguments\n");
		return -1;
	}

	if (sscanf(args.number, "%d", &number) != 1) {
		ast_log_chan(NULL, LOG_WARNING, "First argument must be a number between 0 and 2,147,483,647.\n");
		return -1;
	}
...
static int saycountedadj_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "SayCountedAdj requires two or three arguments (<number>,<adjective>[,<gender>])\n");
		return -1;
	}
	...
	if (args.argc < 2) {
		ast_log_chan(NULL, LOG_WARNING, "SayCountedAdj requires at least two arguments\n");
		return -1;
	}
	...
	if (!ast_strlen_zero(args.gender)) {
		if (strchr("cCfFmMnN", args.gender[0])) {
			ast_log_chan(NULL, LOG_WARNING, "SayCountedAdj gender option must be one of 'f', 'm', 'c', or 'n'.\n");
			return -1;
		}
	}
...
AST_MODULE_INFO_STANDARD_EXTENDED(GABPBX_GPL_KEY, "Decline words according to channel language");
