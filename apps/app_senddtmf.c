...
static int senddtmf_exec(struct ast_channel *chan, const char *vdata)
{
	...
	if (ast_strlen_zero(vdata)) {
		ast_log_chan(NULL, LOG_WARNING, "SendDTMF requires an argument\n");
		return 0;
	}
	...
	if (ast_strlen_zero(args.digits)) {
		ast_log_chan(NULL, LOG_WARNING, "The digits argument is required (0-9,*#,a-d,A-D,wfF)\n");
		return 0;
	}
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Send DTMF digits Application");
