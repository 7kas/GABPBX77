...
#define BUFFER_SIZE 256
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "ReceiveSF requires an argument (variable)\n");
		return -1;
	}
	...
	if (!ast_strlen_zero(arglist.digits) && (ast_str_to_int(arglist.digits, &digits) || digits <= 0)) {
		ast_log_chan(NULL, LOG_WARNING, "Invalid number of digits: %s\n", arglist.digits);
		return -1;
	}

	if (!ast_strlen_zero(arglist.freq) && (ast_str_to_int(arglist.freq, &freq) || freq <= 0)) {
		ast_log_chan(NULL, LOG_WARNING, "Invalid freq: %s\n", arglist.freq);
		return -1;
	}

	if (ast_strlen_zero(arglist.variable)) {
		ast_log_chan(NULL, LOG_WARNING, "Invalid! Usage: ReceiveSF(variable[,timeout][,option])\n");
		return -1;
	}
	...
	if (!ast_strlen_zero(tmp)) {
		ast_verb_chan(NULL, 3, "SF digits received: '%s'\n", tmp);
	} else if (!res) { /* if channel hung up, don't print anything out */
		ast_verb_chan(NULL, 3, "No SF digits received.\n");
	}
	return res;
}
...
static int sendsf_exec(struct ast_channel *chan, const char *vdata)
{
	...
	if (ast_strlen_zero(vdata)) {
		ast_log_chan(NULL, LOG_WARNING, "SendSF requires an argument\n");
		return 0;
	}
	...
	if (ast_strlen_zero(args.digits)) {
		ast_log_chan(NULL, LOG_WARNING, "The digits argument is required (0-9,wf)\n");
		return 0;
	}
...
AST_MODULE_INFO_STANDARD_EXTENDED(GABPBX_GPL_KEY, "SF Sender and Receiver Applications");
