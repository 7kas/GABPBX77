	...
	if (!s) {
		if (addnew) { /* signal doesn't exist, so create it */
			...
			if (s) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Failed to create signal item for '%s'\n", sname);
			}
		} else {
...
static int wait_for_signal_or_hangup(struct ast_channel *chan, char *signame, int timeout)
{
	...
	if (s->watchers) { /* folks are still waiting for this, pass it on... */
		...
		if (ast_alertpipe_write(s->sig_alert_pipe)) {
			ast_log_chan(NULL, LOG_WARNING, "%s: write() failed: %s\n", __FUNCTION__, strerror(errno));
		}
		errno = save_errno;
...
static int send_signal(char *signame, char *payload)
{
	...
	if (payload && *payload) {
		...
		if (!s->payload) {
			ast_log_chan(NULL, LOG_WARNING, "Failed to allocate signal payload '%s'\n", payload);
		} else {
...
static int waitsignal_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "Signal() requires arguments\n");
		return -1;
	}
	...
	if (ast_strlen_zero(args.signame)) {
		ast_log_chan(NULL, LOG_WARNING, "Missing signal name\n");
		return -1;
	}
	if (strlen(args.signame) >= AST_MAX_CONTEXT) {
		ast_log_chan(NULL, LOG_WARNING, "Signal name '%s' is too long\n", args.signame);
		return -1;
	}
	if (!ast_strlen_zero(args.sigtimeout)) {
		if (sscanf(args.sigtimeout, "%30lg", &timeout) != 1 || timeout < 0) {
			ast_log_chan(NULL, LOG_WARNING, "Invalid timeout provided: %s. Defaulting to no timeout.\n", args.sigtimeout);
		} else {
...
static int unload_module(void)
{
	...
	if (res) {
		ast_log_chan(NULL, LOG_WARNING, "One or more signals is currently in use. Unload failed.\n");
		return res;
	}
...
AST_MODULE_INFO_STANDARD_EXTENDED(GABPBX_GPL_KEY, "Channel Signaling Applications");
