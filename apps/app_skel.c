 ...
 * https://docs.gabpbx.org/Development/Policies-and-Procedures/Coding-Guidelines/
 */
...
/*** DOCUMENTATION
	<application name="SkelGuessNumber" language="en_US">
		...
		from. It shows you the basic structure to create your own GABpbx applications.</para>
		</description>
	</application>
...
static int app_exec(struct ast_channel *chan, const char *data)
{
	...
	if (!cfg) {
		ast_log_chan(NULL, LOG_ERROR, "Couldn't access configuratino data!\n");
		return -1;
	}

	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "%s requires an argument (level[,options])\n", app);
		return -1;
	}
	...
	if (ast_strlen_zero(args.level)) {
		ast_log_chan(NULL, LOG_ERROR, "%s requires a level argument\n", app);
		return -1;
	}

	if (!(level = ao2_find(cfg->levels, args.level, OBJ_KEY))) {
		ast_log_chan(NULL, LOG_ERROR, "Unknown level: %s\n", args.level);
		return -1;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_DEFAULT, "Skeleton (sample) Application",
