...
static void sla_queue_event_conf(enum sla_event_type type, struct ast_channel *chan, const char *confname)
{
	...
	if (ast_strlen_zero(trunk_name)) {
		ast_log_chan(NULL, LOG_ERROR, "Invalid conference name for SLA - '%s'!\n", confname);
		return;
	}
...
static int remove_framehook(struct ast_channel *chan)
{
	...
	if (!datastore) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Cannot remove framehook from %s: HOLD_INTERCEPT not currently enabled\n", ast_channel_name(chan));
		return -1;
	}
	data = datastore->data;
	...
	if (ast_framehook_detach(chan, data->framehook_id)) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Failed to remove framehook from channel %s\n", ast_channel_name(chan));
		return -1;
	}
	if (ast_channel_datastore_remove(chan, datastore)) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Failed to remove datastore from channel %s\n", ast_channel_name(chan));
		return -1;
	}
...
static int attach_framehook(struct ast_channel *chan, const char *confname)
{
	...
	datastore = ast_channel_datastore_find(chan, &sla_framehook_datastore, NULL);
	if (datastore) {
		ast_log_chan(NULL, AST_LOG_WARNING, "SLA framehook already set on '%s'\n", ast_channel_name(chan));
		return 0;
	}
	...
	if (!data->confname || data->framehook_id < 0) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Failed to attach SLA framehook to '%s'\n", ast_channel_name(chan));
		ast_datastore_free(datastore);
		ast_free(data);
		return -1;
	}
...
static int conf_run(struct ast_channel *chan, const char *confname, struct ast_flags *confflags, char *optargs[])
{
	...
	if (res) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to set up conference, aborting\n");
		return -1;
	}
...
static int conf_kick_all(struct ast_channel *chan, const char *confname)
{
	...
	if (chan) {
		...
		if (!chan) {
			ast_log_chan(NULL, LOG_WARNING, "Failed to allocate dummy channel\n");
			return -1;
		} else {
...
static int sla_station_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "Invalid Arguments to SLAStation!\n");
	...
	if (!station) {
		ast_log_chan(NULL, LOG_WARNING, "Station '%s' not found!\n", station_name);
		pbx_builtin_setvar_helper(chan, "SLASTATION_STATUS", "FAILURE");
		return 0;
	}
	...
	if (!trunk_ref) {
		if (ast_strlen_zero(trunk_name)) {
			ast_log_chan(NULL, LOG_NOTICE, "No trunks available for call.\n");
		} else {
			ast_log_chan(NULL, LOG_NOTICE, "Can't join existing call on trunk '%s' due to access controls.\n", trunk_name);
		}
...
static int sla_trunk_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_ERROR, "The SLATrunk application requires an argument, the trunk name\n");
		return -1;
	}
	...
	if (args.argc == 2) {
		if (ast_app_parse_options(sla_trunk_opts, &opt_flags, opts, args.options)) {
			ast_log_chan(NULL, LOG_ERROR, "Error parsing options for SLATrunk\n");
			return -1;
		}
	}
	...
	if (!trunk) {
		ast_log_chan(NULL, LOG_ERROR, "SLA Trunk '%s' not found!\n", args.trunk_name);
		pbx_builtin_setvar_helper(chan, "SLATRUNK_STATUS", "FAILURE");
		return 0;
	}

	if (trunk->chan) {
		ast_log_chan(NULL, LOG_ERROR, "Call came in on %s, but the trunk is already in use!\n", args.trunk_name);
		pbx_builtin_setvar_helper(chan, "SLATRUNK_STATUS", "FAILURE");
		return 0;
	}
...
static enum ast_device_state sla_state(const char *data)
{
	...
	if (res == AST_DEVICE_INVALID) {
		ast_log_chan(NULL, LOG_ERROR, "Could not determine state for trunk %s on station %s!\n", trunk_name, station_name);
	}
...
static int sla_build_trunk(struct ast_config *cfg, const char *cat)
{
	...
	if (!(dev = ast_variable_retrieve(cfg, cat, "device"))) {
		ast_log_chan(NULL, LOG_ERROR, "SLA Trunk '%s' defined with no device!\n", cat);
		return -1;
	}

	if (sla_check_device(dev)) {
		ast_log_chan(NULL, LOG_ERROR, "SLA Trunk '%s' defined with invalid device '%s'!\n", cat, dev);
		return -1;
	}
	...
	for (var = ast_variable_browse(cfg, cat); var; var = var->next) {
		if (!strcasecmp(var->name, "autocontext")) {
			...
			if (sscanf(var->value, "%30u", &trunk->ring_timeout) != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid ringtimeout '%s' specified for trunk '%s'\n", var->value, trunk->name);
			...
			if (!strcasecmp(var->value, "private")) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Invalid value '%s' for hold on trunk %s\n", var->value, trunk->name);
			}
		} else if (strcasecmp(var->name, "type") && strcasecmp(var->name, "device")) {
			ast_log_chan(NULL, LOG_ERROR, "Invalid option '%s' specified at line %d of %s!\n", var->name, var->lineno, SLA_CONFIG_FILE);
		}
	}
	...
	if (!ast_strlen_zero(trunk->autocontext)) {
		if (!ast_context_find_or_create(NULL, NULL, trunk->autocontext, sla_registrar)) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to automatically find or create context '%s' for SLA!\n", trunk->autocontext);
			return -1;
		}
		...
		if (ast_add_extension(trunk->autocontext, 0 /* don't replace */, "s", 1,
			...
			ast_log_chan(NULL, LOG_ERROR, "Failed to automatically create extension for trunk '%s'!\n", trunk->name);
			return -1;
		}
	}
...
static void sla_add_trunk_to_station(struct sla_station *station, struct ast_variable *var)
{
	...
	if (!trunk) {
		ast_log_chan(NULL, LOG_ERROR, "Trunk '%s' not found!\n", var->value);
		return;
	}
	...
	while ((cur = strsep(&options, ","))) {
		...
		if (!strcasecmp(name, "ringtimeout")) {
			if (sscanf(value, "%30u", &trunk_ref->ring_timeout) != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid ringtimeout value '%s' for trunk '%s' on station '%s'\n", value, trunk->name, station->name);
				trunk_ref->ring_timeout = 0;
			}
		} else if (!strcasecmp(name, "ringdelay")) {
			if (sscanf(value, "%30u", &trunk_ref->ring_delay) != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid ringdelay value '%s' for trunk '%s' on station '%s'\n", value, trunk->name, station->name);
				trunk_ref->ring_delay = 0;
			}
		} else {
			ast_log_chan(NULL, LOG_WARNING, "Invalid option '%s' for trunk '%s' on station '%s'\n", name, trunk->name, station->name);
		}
	}
...
static int sla_build_station(struct ast_config *cfg, const char *cat)
{
	...
	if (!(dev = ast_variable_retrieve(cfg, cat, "device"))) {
		ast_log_chan(NULL, LOG_ERROR, "SLA Station '%s' defined with no device!\n", cat);
		return -1;
	}
	...
	for (var = ast_variable_browse(cfg, cat); var; var = var->next) {
		if (!strcasecmp(var->name, "trunk")) {
			...
			if (sscanf(var->value, "%30u", &station->ring_timeout) != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid ringtimeout '%s' specified for station '%s'\n", var->value, station->name);
				station->ring_timeout = 0;
			}
		} else if (!strcasecmp(var->name, "ringdelay")) {
			if (sscanf(var->value, "%30u", &station->ring_delay) != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid ringdelay '%s' specified for station '%s'\n", var->value, station->name);
			...
			if (!strcasecmp(var->value, "private")) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Invalid value '%s' for hold on station %s\n", var->value, station->name);
			}
		} else if (strcasecmp(var->name, "type") && strcasecmp(var->name, "device")) {
			ast_log_chan(NULL, LOG_ERROR, "Invalid option '%s' specified at line %d of %s!\n", var->name, var->lineno, SLA_CONFIG_FILE);
		}
	...
	if (!ast_strlen_zero(station->autocontext)) {
		...
		AST_LIST_TRAVERSE(&station->trunks, trunk_ref, entry) {
			...
			if (ast_add_extension(station->autocontext, 0 /* don't replace */, exten, PRIORITY_HINT,
				...
				ast_log_chan(NULL, LOG_ERROR, "Failed to automatically create hint for trunk '%s'!\n", station->name);
				return -1;
			}
		}
	}
...
static int sla_load_config(int reload)
{
	...
	if (!(cfg = ast_config_load(SLA_CONFIG_FILE, config_flags))) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Config file " SLA_CONFIG_FILE " is in an invalid format.  Aborting.\n");
		return 0;
	}
	...
	while ((cat = ast_category_browse(cfg, cat)) && !res) {
		...
		if (!(type = ast_variable_retrieve(cfg, cat, "type"))) {
			ast_log_chan(NULL, LOG_WARNING, "Invalid entry in %s defined with no type!\n", SLA_CONFIG_FILE);
			continue;
		}
		if (!strcasecmp(type, "trunk")) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Entry in %s defined with invalid type '%s'!\n", SLA_CONFIG_FILE, type);
		}
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "Shared Line Appearances",
