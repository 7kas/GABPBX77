...
#include "gabpbx/paths.h"  /* use ast_config_AST_SPOOL_DIR and LOG_DIR */
	...
	<application name="SMS" language="en_US">
		...
		<syntax>
			<parameter name="name" required="true">
				<para>The name of the queue used in <filename>/var/spool/gabpbx/sms</filename></para>
			</parameter>
...
static void sms_readfile(sms_t * h, char *fn)
{
	...
	ast_log_chan(NULL, LOG_NOTICE, "Sending %s\n", fn);
			...
			if (*p == '=') {
				*p++ = 0;
				if (!strcmp(line, "ud")) {  /* parse message (UTF-8) */
					...
					h->udl = o;
					if (*p) {
						ast_log_chan(NULL, LOG_WARNING, "UD too long in %s\n", fn);
					}
				} else {
					while (isspace (*p)) {
						p++;
					...
					if (!strcmp(line, "oa") && strlen(p) < sizeof(h->oa)) {
						...
						if (sscanf(p, "%4d-%2d-%2dT%2d:%2d:%2d", &Y, &m, &d, &H, &M, &S) == 6) {
							...
							if (h->scts.tv_sec == 0) {
								ast_log_chan(NULL, LOG_WARNING, "Bad date/timein %s: %s", fn, p);
							}
						}
					} else {
						ast_log_chan(NULL, LOG_WARNING, "Cannot parse in %s: %s=%si\n", fn, line, p);
					}
				}
				...
				*p++ = 0;
				if (*p == '#') {
					...
					if (!strcmp(line, "ud")) {        /* user data */
						...
						h->udl = o;
						if (*p)
							ast_log_chan(NULL, LOG_WARNING, "UD too long / invalid UCS-2 hex in %s\n", fn);
					} else
						ast_log_chan(NULL, LOG_WARNING, "Only ud can use ## format, %s\n", fn);
					...
					h->udl = o;
					if (*p) {
						ast_log_chan(NULL, LOG_WARNING, "UD too long / invalid UCS-1 hex in %s\n", fn);
					}
					...
					h->udhl = o;
					if (*p) {
						ast_log_chan(NULL, LOG_WARNING, "UDH too long / invalid hex in %s\n", fn);
					}
				} else {
					ast_log_chan(NULL, LOG_WARNING, "Only ud and udh can use # format, %s\n", fn);
				}
			} else {
				ast_log_chan(NULL, LOG_WARNING, "Cannot parse in %s: %s\n", fn, line);
			}
		}
		fclose(s);
		if (!dcsset && packsms7(0, h->udhl, h->udh, h->udl, h->ud) < 0) {
			if (packsms8(0, h->udhl, h->udh, h->udl, h->ud) < 0) {
				if (packsms16(0, h->udhl, h->udh, h->udl, h->ud) < 0) {
					ast_log_chan(NULL, LOG_WARNING, "Invalid UTF-8 message even for UCS-2 (%s)\n", fn);
				} else {
					h->dcs = 0x08;          /* default to 16 bit */
					ast_log_chan(NULL, LOG_WARNING, "Sending in 16 bit format(%s)\n", fn);
				}
			} else {
				h->dcs = 0xF5;              /* default to 8 bit */
				ast_log_chan(NULL, LOG_WARNING, "Sending in 8 bit format(%s)\n", fn);
			}
		}
		if (is7bit(h->dcs) && packsms7(0, h->udhl, h->udh, h->udl, h->ud) < 0) {
			ast_log_chan(NULL, LOG_WARNING, "Invalid 7 bit GSM data %s\n", fn);
		}
		if (is8bit(h->dcs) && packsms8(0, h->udhl, h->udh, h->udl, h->ud) < 0) {
			ast_log_chan(NULL, LOG_WARNING, "Invalid 8 bit data %s\n", fn);
		}
		if (is16bit(h->dcs) && packsms16(0, h->udhl, h->udh, h->udl, h->ud) < 0) {
			ast_log_chan(NULL, LOG_WARNING, "Invalid 16 bit data %s\n", fn);
		}
	}
}
...
static void sms_writefile(sms_t * h)
{
	...
	if (rename(fn, fn2)) {
		...
		ast_log_chan(NULL, LOG_NOTICE, "Received to %s\n", fn2);
	}
}
...
static unsigned char sms_handleincoming (sms_t * h)
{
	...
	if (h->smsc) {                          /* SMSC */
		if ((h->imsg[2] & 3) == 1) {        /* SMS-SUBMIT */
			...
			if (p != h->imsg[1] + 2) {
				ast_log_chan(NULL, LOG_WARNING, "Mismatch receive unpacking %d/%d\n", p, h->imsg[1] + 2);
				return 0xFF;		  /* duh! */
			}
		} else {
			ast_log_chan(NULL, LOG_WARNING, "Unknown message type %02hhX\n", h->imsg[2]);
			return 0xFF;
		}
	} else {                                /* client */
...
static int sms_handleincoming_proto2(sms_t *h)
{
	...
	sz = h->imsg[1] + 2;
	/* ast_verb_chan(NULL, 3, "SMS-P2 Frame: %s\n", sms_hexdump(h->imsg, sz, debug_buf)); */
	...
	for (f = 4; f < sz; ) {
		...
		switch (msg) {
		case 0x13:                          /* Body */
			ast_verb_chan(NULL, 3, "SMS-P2 Body#%02X=[%.*s]\n", (unsigned)msg, msgsz, &h->imsg[f]);
		...
		case 0x14:                          /* Date SCTS */
			...
			ast_verb_chan(NULL, 3, "SMS-P2 Date#%02X=%02d/%02d %02d:%02d\n", (unsigned)msg, tm.tm_mday, tm.tm_mon + 1, tm.tm_hour, tm.tm_min);
			break;
		case 0x15:                          /* Calling line (from SMSC) */
			if (msgsz >= 20) {
				msgsz = 20 - 1;
			}
			ast_verb_chan(NULL, 3, "SMS-P2 Origin#%02X=[%.*s]\n", (unsigned)msg, msgsz, &h->imsg[f]);
...
#define DLL2_ACK(h) ((h->framenumber & 1) ? DLL2_SMS_ACK1: DLL2_SMS_ACK1)
	...
	case DLL2_SMS_ACK1:
		...
		if ( (h->omsg[0] & DLL_SMS_MASK) == DLL2_SMS_REL) {
			...
			ast_log_chan(NULL, LOG_NOTICE, "SMS_SUBMIT or SMS_DELIVERY\n");
			sms_nextoutgoing (h);
		}
		break;
...
#define MAXSAMPLES (800)
	...
	if (samples > MAXSAMPLES) {
		ast_log_chan(NULL, LOG_WARNING, "Only doing %d samples (%d requested)\n",
			 MAXSAMPLES, samples);
		samples = MAXSAMPLES;
	}
	...
	for ( ; samples-- ; data++) {
		...
		if (h->imag <= 500) {               /* below [arbitrary] threahold: lost carrier */
			if (h->idle++ == 80000) {       /* nothing happening */
				ast_log_chan(NULL, LOG_NOTICE, "No data, hanging up\n");
				h->hangup = 1;
				h->err = 1;
			}
			if (h->ierr) {                  /* error */
				ast_log_chan(NULL, LOG_NOTICE, "Error %d, hanging up\n", h->ierr);
				/* Protocol 1 */
				h->err = 1;
				h->omsg[0] = 0x92;          /* error */
				h->omsg[1] = 1;
		...
		if (bit && h->ibitc == 200) {       /* sync, restart message */
			...
			if (h->framenumber < 0 && h->ibytec >= 160 && !memcmp(h->imsg, "UUUUUUUUUUUUUUUUUUUU", 20)) {
				h->framenumber = 1;
				ast_verb_chan(NULL, 3, "SMS protocol 2 detected\n");
				h->protocol = 2;
				...
				if (h->ibitn++ == 9) {      /* end of byte */
					if (!bit) {             /* bad stop bit */
						if (h->sent_rel) {
							...
							ast_log_chan(NULL, LOG_NOTICE, "Bad stop bit\n");
						...
						if (h->ibytep < sizeof(h->imsg)) {
							...
							ast_log_chan(NULL, LOG_NOTICE, "msg too large\n");
						...
						if (h->ibytep > 1 && h->ibytep == 3 + h->imsg[1] && !h->ierr) {
							if (!h->ibytec) {
								...
								ast_log_chan(NULL, LOG_NOTICE, "bad checksum\n");
...
static int sms_exec(struct ast_channel *chan, const char *data)
{
	...
	if (!data) {
		ast_log_chan(NULL, LOG_ERROR, "Requires queue name at least\n");
		return -1;
	}
	...
	ast_verb_chan(NULL, 1, "sms argc %u queue <%s> opts <%s> addr <%s> body <%s>\n",
	...
	if (ast_strlen_zero(sms_args.queue)) {
		ast_log_chan(NULL, LOG_ERROR, "Requires queue name\n");
		goto done;
	}
	if (strlen(sms_args.queue) >= sizeof(h.queue)) {
		ast_log_chan(NULL, LOG_ERROR, "Queue name too long\n");
		goto done;
	}
	...
	ast_verb_chan(NULL, 1, "initial delay %dms\n", h.opause_0);
	...
	if (sms_args.argc > 2) {
		...
		if (ast_strlen_zero(sms_args.addr) || strlen(sms_args.addr) >= sizeof(h.oa)) {
			ast_log_chan(NULL, LOG_ERROR, "Address too long %s\n", sms_args.addr);
			goto done;
		}
		if (h.smsc) {
		...
		if (ast_strlen_zero(sms_args.body)) {
			ast_log_chan(NULL, LOG_ERROR, "Missing body for %s\n", sms_args.addr);
			goto done;
		}
		...
		if (is7bit(h.dcs) && packsms7(0, h.udhl, h.udh, h.udl, h.ud) < 0) {
			ast_log_chan(NULL, LOG_WARNING, "Invalid 7 bit GSM data\n");
			goto done;
		}
		if (is8bit(h.dcs) && packsms8(0, h.udhl, h.udh, h.udl, h.ud) < 0) {
			ast_log_chan(NULL, LOG_WARNING, "Invalid 8 bit data\n");
			goto done;
		}
		if (is16bit(h.dcs) && packsms16(0, h.udhl, h.udh, h.udl, h.ud) < 0) {
			ast_log_chan(NULL, LOG_WARNING, "Invalid 16 bit data\n");
			goto done;
		}
		h.rx = 0;                           /* sent message */
		h.mr = -1;
	...
	if (res < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to set to linear mode, giving up\n");
		goto done;
	}

	if ( (res = ast_activate_generator(chan, &smsgen, &h)) < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to activate generator on '%s'\n", ast_channel_name(chan));
		goto done;
	}
	...
	for (;;) {
		...
		if (i < 0) {
			ast_log_chan(NULL, LOG_NOTICE, "waitfor failed\n");
			break;
		}
		if (h.hangup) {
			ast_log_chan(NULL, LOG_NOTICE, "channel hangup\n");
			break;
		}
		f = ast_read(chan);
		if (!f) {
			ast_log_chan(NULL, LOG_NOTICE, "ast_read failed\n");
			break;
		}
...
AST_MODULE_INFO_STANDARD_EXTENDED(GABPBX_GPL_KEY, "SMS/PSTN handler");
