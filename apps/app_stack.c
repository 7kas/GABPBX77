...
static int pop_exec(struct ast_channel *chan, const char *data)
{
	...
	if (!(stack_store = ast_channel_datastore_find(chan, &stack_info, NULL))) {
		ast_log_chan(NULL, LOG_WARNING, "%s called with no gosub stack allocated.\n", app_pop);
...
static int return_exec(struct ast_channel *chan, const char *data)
{
	...
	ast_channel_lock(chan);
	if (!(stack_store = ast_channel_datastore_find(chan, &stack_info, NULL))) {
		ast_log_chan(NULL, LOG_ERROR, "Return without Gosub: stack is unallocated\n");
		ast_channel_unlock(chan);
		return -1;
	}
	...
	if (!oldframe) {
		ast_log_chan(NULL, LOG_ERROR, "Return without Gosub: stack is empty\n");
		ast_channel_unlock(chan);
		return -1;
	}
	if (oldframe->is_special) {
	...
	if (parse) {
		...
		if (endparen) {
			*endparen = '\0';
			...
			ast_log_chan(NULL, LOG_WARNING, "Ouch.  No closing paren: '%s'?\n", args);
		}
	}
...
static int gosub_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_ERROR, "%s requires an argument: %s([[context,]exten,]priority[(arg1[,...][,argN])])\n", app_gosub, app_gosub);
		return -1;
	}
	...
	if (ast_parseable_goto(chan, label)) {
		ast_log_chan(NULL, LOG_ERROR, "%s address is invalid: '%s'\n", app_gosub, data);
		goto error_exit;
	}
	...
	if (!ast_exists_extension(chan, dest_context, dest_exten, dest_priority, caller_id)) {
		ast_log_chan(NULL, LOG_ERROR, "%s attempted to reach non-existent destination '%s,%s,%d' from '%s,%s,%d'",
	...
	if (!(stack_store = ast_channel_datastore_find(chan, &stack_info, NULL))) {
		...
		if (!stack_store) {
			ast_log_chan(NULL, LOG_ERROR, "Unable to allocate new datastore.  %s failed.\n",
				app_gosub);
		...
		if (!oldlist) {
			ast_log_chan(NULL, LOG_ERROR, "Unable to allocate datastore list head.  %s failed.\n",
				app_gosub);
...
static int gosubif_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "GosubIf requires an argument: GosubIf(cond?label1(args):label2(args)\n");
		return 0;
	}
...
static int local_write(struct ast_channel *chan, const char *cmd, char *var, const char *value)
{
	...
	ast_channel_lock(chan);
	if (!(stack_store = ast_channel_datastore_find(chan, &stack_info, NULL))) {
		ast_log_chan(NULL, LOG_ERROR, "Tried to set LOCAL(%s), but we aren't within a Gosub routine\n", var);
		ast_channel_unlock(chan);
		return -1;
	}
...
static int peek_read(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	if (!chan) {
		ast_log_chan(NULL, LOG_ERROR, "LOCAL_PEEK must be called on an active channel\n");
		return -1;
	}
	...
	if (ast_strlen_zero(args.n) || ast_strlen_zero(args.name)) {
		ast_log_chan(NULL, LOG_ERROR, "LOCAL_PEEK requires parameters n and varname\n");
		return -1;
	}
...
static int stackpeek_read(struct ast_channel *chan, const char *cmd, char *data, struct ast_str **str, ssize_t len)
{
	...
	if (!chan) {
		ast_log_chan(NULL, LOG_ERROR, "STACK_PEEK must be called on an active channel\n");
		return -1;
	}
	...
	if (ast_strlen_zero(args.n) || ast_strlen_zero(args.which)) {
		ast_log_chan(NULL, LOG_ERROR, "STACK_PEEK requires parameters n and which\n");
		return -1;
	}

	n = atoi(args.n);
	if (n <= 0) {
		ast_log_chan(NULL, LOG_ERROR, "STACK_PEEK must be called with a positive peek value\n");
		return -1;
	}
	...
	if (!(stack_store = ast_channel_datastore_find(chan, &stack_info, NULL))) {
		if (!ast_true(args.suppress)) {
			ast_log_chan(NULL, LOG_ERROR, "STACK_PEEK called on a channel without a gosub stack\n");
		}
		ast_channel_unlock(chan);
		return -1;
	}
	...
	if (!frame) {
		/* Too deep */
		if (!ast_true(args.suppress)) {
			ast_log_chan(NULL, LOG_ERROR, "Stack peek of '%s' is more stack frames than I have\n", args.n);
		}
		AST_LIST_UNLOCK(oldlist);
		ast_channel_unlock(chan);
		return -1;
	...
	case 'p': /* priority */
		...
		break;
	default:
		ast_log_chan(NULL, LOG_ERROR, "Unknown argument '%s' to STACK_PEEK\n", args.which);
		break;
	}
...
static void balance_stack(struct ast_channel *chan)
{
	...
	if (!stack_store) {
		ast_log_chan(NULL, LOG_WARNING, "No %s stack allocated.\n", app_gosub);
		return;
	}
...
static int gosub_run(struct ast_channel *chan, const char *sub_args, int ignore_hangup)
{
	...
	ast_verb_chan(NULL, 3, "%s Internal %s(%s) start\n",
	...
	if (!res) {
		...
		if (!stack_store) {
			/* Should never happen! */
			ast_log_chan(NULL, LOG_ERROR, "No %s stack!\n", app_gosub);
			res = -1;
		} else {
			struct gosub_stack_list *oldlist;
	...
	if (!res) {
		...
		do {
			...
			if (ast_check_hangup(chan)) {
				if (ast_channel_softhangup_internal_flag(chan) & AST_SOFTHANGUP_ASYNCGOTO) {
					ast_log_chan(NULL, LOG_ERROR, "%s An async goto just messed up our execution location.\n",
						ast_channel_name(chan));
					break;
				}
				if (!ignore_hangup) {
		...
		if (found && res) {
			...
			ast_verb_chan(NULL, 2, "Spawn extension (%s, %s, %d) exited non-zero on '%s'\n",
				ast_channel_context(chan), ast_channel_exten(chan),
		...
		if (ast_channel_priority(chan) == saved_priority
			...
			ast_verb_chan(NULL, 3, "%s Internal %s(%s) complete GOSUB_RETVAL=%s\n",
				ast_channel_name(chan), app_gosub, sub_args,
				S_OR(pbx_builtin_getvar_helper(chan, "GOSUB_RETVAL"), ""));
		} else {
			ast_log_chan(NULL, LOG_WARNING, "%s Abnormal '%s(%s)' exit.  Popping routine return locations.\n",
				ast_channel_name(chan), app_gosub, sub_args);
			balance_stack(chan);
...
static int handle_gosub(struct ast_channel *chan, AGI *agi, int argc, const char * const *argv)
{
	...
	if (sscanf(argv[3], "%30d", &priority) != 1 || priority < 1) {
		...
		if (priority < 0) {
			ast_log_chan(NULL, LOG_ERROR, "Priority '%s' not found in '%s@%s'\n", argv[3], argv[2], argv[1]);
	...
	if (!res) {
		...
		if (ast_channel_priority(chan) == old_priority
			...
			ast_verb_chan(NULL, 3, "%s AGI %s(%s) complete GOSUB_RETVAL=%s\n",
				ast_channel_name(chan), app_gosub, gosub_args,
				S_OR(pbx_builtin_getvar_helper(chan, "GOSUB_RETVAL"), ""));
			abnormal_exit = 0;
		} else {
			ast_log_chan(NULL, LOG_NOTICE, "%s Abnormal AGI %s(%s) exit.  Popping routine return locations.\n",
				ast_channel_name(chan), app_gosub, gosub_args);
			balance_stack(chan);
			pbx_builtin_setvar_helper(chan, "GOSUB_RETVAL", "");
			abnormal_exit = 1;
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_DEFAULT | AST_MODFLAG_LOAD_ORDER, "Dialplan subroutines (Gosub, Return, etc)",
