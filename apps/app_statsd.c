...
static int value_in_range(const char *value) {
	...
	if (numerical_value < pow(-2, 63) || numerical_value > pow(2, 63)) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Value %lf out of range!\n", numerical_value);
		return 1;
	}
...
static int validate_metric(const char *metric)
{
	...
	if (ast_strlen_zero(metric)) {
		ast_log_chan(NULL, AST_LOG_ERROR, "Missing metric type argument.\n");
		return 1;
	}
	...
	ast_log_chan(NULL, AST_LOG_ERROR, "Invalid metric type %s.\n", metric);
...
static int validate_numeric(const char *numeric_value) {
	...
	while (*num) {
		if (!isdigit(*num++)) {
			...
			ast_log_chan(NULL, AST_LOG_ERROR, "%s is not a number!\n", numeric_value);
			return 1;
		}
	}
...
static int determine_actual_value(const char *raw_value) {
	...
	if ((raw_value[0] == '+') || (raw_value[0] == '-')) {
		...
		if (ast_strlen_zero(actual_value)) {
			ast_log_chan(NULL, AST_LOG_ERROR, "Value argument %s only contains a sign"
				" operator.\n", raw_value);
			return 1;
		}
	} else {
...
static int validate_name(const char *name)
{
	if (ast_strlen_zero(name) || (strstr(name, "|") != NULL)) {
		ast_log_chan(NULL, AST_LOG_ERROR, "Statistic name %s is missing or contains a pipe (|)"
			" character.\n", name);
		return 1;
	}
...
static int validate_metric_type_gauge(const char *statistic_name, const char *value) {
	...
	if (ast_strlen_zero(value)) {
		ast_log_chan(NULL, AST_LOG_ERROR, "Missing value argument.\n");
		return 1;
	}
...
static int validate_metric_type_set(const char *statistic_name, const char *value) {
	...
	if (strstr(value, "|") != NULL) {
		ast_log_chan(NULL, AST_LOG_ERROR, "Pipe (|) character is not allowed for value %s"
			" in a set metric.\n", value);
		return 1;
	}
...
static int statsd_exec(struct ast_channel *chan, const char *data)
{
	...
	if (!data) {
		ast_log_chan(NULL, AST_LOG_ERROR, "No parameters were provided. Correct format is "
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_DEFAULT, "StatsD Dialplan Application",
