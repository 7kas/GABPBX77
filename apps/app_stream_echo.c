...
static int stream_echo_write_error(struct ast_channel *chan, struct ast_frame *frame, int pos)
{
	...
	ast_log_chan(NULL, LOG_ERROR, "%s - unable to write frame type '%s' to stream type '%s' at "
	...
	while (ast_waitfor(chan, -1) > -1) {
		...
		if (request_change) {
			...
			if (ast_channel_request_stream_topology_change(chan, topology, NULL)) {
				ast_log_chan(NULL, LOG_WARNING, "Request stream topology change not supported "
...
static int stream_echo_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_str_to_uint(args.num, &num)) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to parse the first parameter '%s' into a"
	...
	if (!topology) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to create '%u' streams of type '%s' to"
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Stream Echo Application");
