...
#include "gabpbx/channel.h"	/* autoservice */
...
static int system_exec_helper(struct ast_channel *chan, const char *data, int failmode)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "System requires an argument(command)\n");
	...
	if (strchr("\"'", cbuf[0]) && cbuf[ast_str_strlen(buf) - 1] == cbuf[0]) {
		...
		cbuf++;
		ast_log_chan(NULL, LOG_NOTICE, "It is not necessary to quote the argument to the System application.\n");
	}

	res = ast_safe_system(cbuf);

	if ((res < 0) && (errno != ECHILD)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to execute '%s'\n", (char *)data);
		pbx_builtin_setvar_helper(chan, chanvar, "FAILURE");
		res = failmode;
	} else if (res == 127) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to execute '%s'\n", (char *)data);
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Generic System() application");
