...
static int background_detect_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "BackgroundDetect requires an argument (filename)\n");
		return -1;
	}
	...
	do {
		...
		if ((ast_set_read_format(chan, ast_format_slin))) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to set read format to linear!\n");
			res = -1;
			break;
		}
		...
		if (ast_streamfile(chan, tmp, ast_channel_language(chan))) {
			ast_log_chan(NULL, LOG_WARNING, "ast_streamfile failed on %s for %s\n", ast_channel_name(chan), (char *)data);
			break;
		}
		...
		while (ast_channel_stream(chan)) {
			...
			if (res < 0) {
				ast_log_chan(NULL, LOG_WARNING, "Waitfor failed on %s\n", ast_channel_name(chan));
				break;
			} else if (res > 0) {
				fr = ast_read(chan);
				if (continue_analysis && analysistime >= 0) {
					...
					if (ast_tvdiff_ms(ast_tvnow(), detection_start) >= analysistime) {
						continue_analysis = 0;
						ast_verb_chan(NULL, 3, "BackgroundDetect: Talk analysis time complete on %s.\n", ast_channel_name(chan));
					}
				}
	...
	if (res > -1) {
		if (origrformat && ast_set_read_format(chan, origrformat)) {
			ast_log_chan(NULL, LOG_WARNING, "Failed to restore read format for %s to %s\n",
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Playback with Talk Detection");
