...
/*** DOCUMENTATION
	<application name="TestServer" language="en_US">
		...
		<description>
			...
			<filename>/var/log/gabpbx/testreports/&lt;testid&gt;-server.txt</filename></para>
		</description>
		<see-also>
	...
	<application name="TestClient" language="en_US">
		...
		<description>
			...
			<filename>/var/log/gabpbx/testreports/&lt;testid&gt;-client.txt</filename></para>
		</description>
		<see-also>
...
static int measurenoise(struct ast_channel *chan, int ms, char *who)
{
	...
	if (ast_set_read_format(chan, ast_format_slin)) {
		ast_log_chan(NULL, LOG_NOTICE, "Unable to set to linear mode!\n");
		ao2_cleanup(rformat);
		return -1;
	}
	start = ast_tvnow();
	...
	if (rformat) {
		if (ast_set_read_format(chan, rformat)) {
			ast_log_chan(NULL, LOG_NOTICE, "Unable to restore original format!\n");
			ao2_ref(rformat, -1);
			return -1;
		}
		ao2_ref(rformat, -1);
	...
	if (res < 0)
		return res;
	if (!samples) {
		ast_log_chan(NULL, LOG_NOTICE, "No samples were received from the other side!\n");
		return -1;
	}
...
static int testclient_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(testid)) {
		ast_log_chan(NULL, LOG_WARNING, "TestClient requires an argument - the test id\n");
		return -1;
	}
	...
	if ((res >=0) && (!ast_strlen_zero(testid))) {
		...
		ast_log_chan(NULL, LOG_NOTICE, "Did not read a test ID on '%s'\n", ast_channel_name(chan));
		res = -1;
	}
	return res;
}
...
static int testserver_exec(struct ast_channel *chan, const char *data)
{
	...
	if ((res >=0) && (!ast_strlen_zero(testid))) {
		...
		if ((f = fopen(fn, "w+"))) {
			...
			ast_log_chan(NULL, LOG_NOTICE, "-- TEST COMPLETE--\n");
			fprintf(f, "-- END TEST--\n");
			fclose(f);
			res = -1;
		} else
...
AST_MODULE_INFO_STANDARD_EXTENDED(GABPBX_GPL_KEY, "Interface Test Application");
