...
static int verbose_exec(struct ast_channel *chan, const char *data)
{
	...
	if (sscanf(args.level, "%30u", &vsize) != 1) {
		vsize = 0;
		ast_log_chan(NULL, LOG_WARNING, "'%s' is not a verboser number\n", args.level);
	} else if (4 < vsize) {
		vsize = 4;
	}

	ast_verb_chan(NULL, vsize, "%s\n", args.msg);
...
static int log_exec(struct ast_channel *chan, const char *data)
{
	...
	if (lnum > -1) {
		...
		ast_log_chan(NULL, lnum, extension, ast_channel_priority(chan), context, "%s\n", args.msg);
	} else {
		ast_log_chan(NULL, LOG_ERROR, "Unknown log level: '%s'\n", args.level);
		return 0;
	}
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Send verbose output");
