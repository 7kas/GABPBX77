 ...
 * \note For information about voicemail IMAP storage, https://docs.gabpbx.org/Configuration/Applications/Voicemail/IMAP-Voicemail-Storage/
...
#include "gabpbx/paths.h"	/* use ast_config_AST_SPOOL_DIR */
#include <sys/time.h>
#include <sys/stat.h>
#include <sys/mman.h>
#include <time.h>
...
/*** DOCUMENTATION
	...
	<manager name="VoicemailRefresh" language="en_US">
		...
		<synopsis>
			Tell GABpbx to poll mailboxes for a change
		</synopsis>
		<syntax>
			<xi:include xpointer="xpointer(/docs/manager[@name='Login']/syntax/parameter[@name='ActionID'])" />
		...
		<description>
			<para>Normally, MWI indicators are only sent when GABpbx itself
...
#define VM_ODBC_AUDIO_ON_DISK  (1 << 21) /*!< When using ODBC, leave the message and greeting recordings on disk instead of moving them into the table */

#define ERROR_LOCK_PATH  -100
#define ERROR_MAX_MSGS   -101
#define OPERATOR_EXIT     300

#define MSGFILE_LEN (7) /*!< Length of the message file name: msgXXXX */
...
#ifdef ODBC_STORAGE
static char odbc_database[80] = "gabpbx";
...
#define DELETE(a,b,c,d) (odbc_delete_message(a,b))
...
static char serveremail[80] = GABPBX_USERNAME;
...
static int inprocess_count(const char *context, const char *mailbox, int delta)
{
	...
	if (delta < 0) {
		ast_log_chan(NULL, LOG_WARNING, "BUG: ref count decrement on non-existing object???\n");
	}
...
/*
 * These macros are currently only used by the ODBC code but
 * should be used anyplace we're allocating a PATH_MAX
 * sized buffer for something which will be much smaller
 * in practice.
 */
#ifdef ODBC_STORAGE
/*!
 * \internal
 * \brief Get the length needed for a message path base.
 *
 * \param dir The absolute path to a directory
 *
 * \note The directory provided should include all components
 * down to the folder level.  For example:
 *  /var/spool/gabpbx/voicemail/default/1234/INBOX
 *
 * \returns Number of bytes needed for the message path base.
 */
static size_t get_msg_path_len(const char *dir)
{
	if (ast_strlen_zero(dir)) {
		return 0;
	}
	/* dir + '/' + MSGFILE_LEN + '\0' */
	return strlen(dir) + 1 + MSGFILE_LEN + 1;
}

/*
 * The longest sound file extension we currently
 * have is "siren14" which is 7 characters long
 * but we'll make this 12 to be extra safe.
 */
#define MAX_SOUND_EXTEN_LEN 12
/*!
 * \internal
 * \brief Get the length needed for a message sound or metadata file path.
 *
 * \param dir The absolute path to a directory
 *
 * \note The directory provided should include all components
 ...
 *  /var/spool/gabpbx/voicemail/default/1234/INBOX
 *
 * \returns Number of bytes needed for the message sound or metadata file path.
 */
static size_t get_msg_path_ext_len(const char *dir)
{
	if (ast_strlen_zero(dir)) {
		return 0;
	}
	/* dir + '/' + MSGFILE_LEN + extlen(including '.') + '\0' */
	return strlen(dir) + 1 + MSGFILE_LEN + MAX_SOUND_EXTEN_LEN + 1;
}

/*!
 * \internal
 * \brief Allocate a buffer on the stack containing a message file path base.
 *
 * \param dir The absolute path to a directory
 * \param msgnum The message number or -1 for a prompt file
 *
 * \note The returned buffer will automatically be freed when it
 * goes out of scope.
 *
 * If passed (/var/spool/gabpbx/voicemail/default/1234/INBOX, 0)
 * the returned path would be:
 * /var/spool/gabpbx/voicemail/default/1234/INBOX/msg0000
 *
 * If passed (/var/spool/gabpbx/voicemail/default/1234/unavail, -1)
 * the returned path would be:
 * /var/spool/gabpbx/voicemail/default/1234/unavail
 * (no change)
 *
 * \returns A pointer to the buffer containing the path.
 */
#define MAKE_FILE_PTRA(dir, msgnum) \
({ \
    size_t __len = get_msg_path_len(dir); \
    char *__var; \
    if (msgnum < 0) { \
        __var = ast_strdupa(dir); \
    } else { \
        __var = ast_alloca(__len); \
        snprintf(__var, __len, "%s/msg%04d", dir, msgnum); \
    } \
    __var; \
})
...
/*!
 * \internal
 * \brief Allocate a buffer on the stack for a message sound or metadata file path.
 *
 * \param dir The absolute path to a directory
 * \param msgnum The message number or -1 for a prompt file
 *
 ...
 * goes out of scope.
 *
 * If passed (/var/spool/gabpbx/voicemail/default/1234/INBOX, 0, "txt")
 * the returned path would be:
 * /var/spool/gabpbx/voicemail/default/1234/INBOX/msg0000.txt
 *
 * If passed (/var/spool/gabpbx/voicemail/default/1234/unavail, -1, "WAV")
 * the returned path would be:
 * /var/spool/gabpbx/voicemail/default/1234/unavail.WAV
 *
 * The buffer returned has enough space that you can safely re-use it
 * for other sound file extensions.  For instance:
 *
 * 	char *fn = MAKE_FILE_PTRA(dir, msgnum);
 *	char *full_fn = MAKE_FILE_EXT_PTRA(dir, msgnum, "txt");
 *	char *fmt = "g722";
 *  ...do domething with full_fn then re-use it...
 *  sprintf(full_fn, "%s.%s", fn, fmt); * Safe since the same dir and msgnum were used *
 *
 * \returns A pointer to the allocated buffer
 */
#define MAKE_FILE_EXT_PTRA(dir, msgnum, ext) \
({ \
    size_t __len = get_msg_path_ext_len(dir); \
    char *__var = ast_alloca(__len); \
    if (msgnum < 0) { \
        snprintf(__var, __len, "%s.%s", dir, ext); \
    } else { \
        snprintf(__var, __len, "%s/msg%04d.%s", dir, msgnum, ext); \
    } \
	__var; \
})
#endif
...
static void apply_option(struct ast_vm_user *vmu, const char *var, const char *value)
{
	...
	if (!strcasecmp(var, "attach")) {
		...
		if (sscanf(value, "%30d", &x) == 1) {
			...
			ast_log_chan(NULL, AST_LOG_WARNING, "Invalid min duration for say duration\n");
		}
		...
		if (sscanf(value, "%30d", &x) == 1 && x >= 0) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Invalid min message length of %s. Using global value %d\n", value, vmminsecs);
			vmu->minsecs = vmminsecs;
		}
	} else if (!strcasecmp(var, "maxmessage") || !strcasecmp(var, "maxsecs")) {
		vmu->maxsecs = atoi(value);
		if (vmu->maxsecs <= 0) {
			ast_log_chan(NULL, AST_LOG_WARNING, "Invalid max message length of %s. Using global value %d\n", value, vmmaxsecs);
			vmu->maxsecs = vmmaxsecs;
		} else {
			vmu->maxsecs = atoi(value);
		}
		if (!strcasecmp(var, "maxmessage"))
			ast_log_chan(NULL, AST_LOG_WARNING, "Option 'maxmessage' has been deprecated in favor of 'maxsecs'.  Please make that change in your voicemail config.\n");
	} else if (!strcasecmp(var, "maxmsg")) {
		vmu->maxmsg = atoi(value);
		/* Accept maxmsg=0 (Greetings only voicemail) */
		if (vmu->maxmsg < 0) {
			ast_log_chan(NULL, AST_LOG_WARNING, "Invalid number of messages per folder maxmsg=%s. Using default value %d\n", value, MAXMSG);
			vmu->maxmsg = MAXMSG;
		} else if (vmu->maxmsg > MAXMSGLIMIT) {
			ast_log_chan(NULL, AST_LOG_WARNING, "Maximum number of messages per folder is %d. Cannot accept value maxmsg=%s\n", MAXMSGLIMIT, value);
		...
		if (vmu->maxdeletedmsg < 0) {
			ast_log_chan(NULL, AST_LOG_WARNING, "Invalid number of deleted messages saved per mailbox backupdeleted=%s. Using default value %d\n", value, MAXMSG);
			vmu->maxdeletedmsg = MAXMSG;
		} else if (vmu->maxdeletedmsg > MAXMSGLIMIT) {
			ast_log_chan(NULL, AST_LOG_WARNING, "Maximum number of deleted messages saved per mailbox is %d. Cannot accept value backupdeleted=%s\n", MAXMSGLIMIT, value);
	...
	if (pipe(fds)) {
		...
		if (pid < 0) {
			...
			if (read(fds[0], buf, len) < 0) {
				ast_log_chan(NULL, LOG_WARNING, "read() failed: %s\n", strerror(errno));
			}
			close(fds[0]);
		} else {
			/*  child */
	...
	if (!ast_strlen_zero(ext_pass_check_cmd)) {
		...
		if (vm_check_password_shell(cmd, buf, sizeof(buf))) {
			...
			if (!strncasecmp(buf, "VALID", 5)) {
				...
				ast_log_chan(NULL, AST_LOG_WARNING, "Unable to execute password validation script: '%s'.\n", buf);
				return 0;
			} else {
				ast_log_chan(NULL, AST_LOG_NOTICE, "Password doesn't match policies for user %s %s\n", vmu->mailbox, password);
				return 1;
			}
		}
	}
		...
		if (!strcasecmp(var->name, "vmsecret")) {
			...
			if (ast_strlen_zero(retval->password)) {
				if (!ast_strlen_zero(var->value) && var->value[0] == '*') {
					ast_log_chan(NULL, LOG_WARNING, "Invalid password detected for mailbox %s.  The password"
...
static int is_valid_dtmf(const char *key)
{
	...
	for (i = 0; i < strlen(key); ++i) {
		if (!strchr(VALID_DTMF, *local_key)) {
			ast_log_chan(NULL, AST_LOG_WARNING, "Invalid DTMF key \"%c\" used in voicemail configuration file\n", *local_key);
			return 0;
		}
		local_key++;
	}
...
static void vm_change_password(struct ast_vm_user *vmu, const char *newpassword)
{
	...
	case OPT_PWLOC_SPOOLDIR:
		...
		if (write_password_to_file(secretfn, newpassword) == 0) {
			...
			ast_verb_chan(NULL, 4, "Writing voicemail password to file %s succeeded\n", secretfn);
			reset_user_pw(vmu->context, vmu->mailbox, newpassword);
			ast_copy_string(vmu->password, newpassword, sizeof(vmu->password));
			break;
		} else {
			ast_log_chan(NULL, LOG_WARNING, "Writing voicemail password to file %s failed, falling back to config file\n", secretfn);
		}
		/* Fall-through */
			...
			while ((category = ast_category_browse(cfg, category))) {
				if (!strcasecmp(category, vmu->context)) {
					...
					if (!(tmp = ast_variable_retrieve(cfg, category, vmu->mailbox))) {
						ast_log_chan(NULL, AST_LOG_WARNING, "We could not find the mailbox.\n");
						break;
					}
					value = strstr(tmp, ",");
					if (!value) {
					...
					if (!(cat = ast_category_get(cfg, category, NULL))) {
						ast_log_chan(NULL, AST_LOG_WARNING, "Failed to get category structure.\n");
						ast_free(new);
						break;
					}
...
static int create_dirpath(char *dest, int len, const char *context, const char *ext, const char *folder)
{
	...
	if ((res = ast_mkdir(dest, mode))) {
		ast_log_chan(NULL, AST_LOG_WARNING, "ast_mkdir '%s' failed: %s\n", dest, strerror(res));
		return -1;
	}
	return 0;
}
...
static void vm_imap_delete(char *file, int msgnum, struct ast_vm_user *vmu)
{
	...
	if (!(vms = get_vm_state_by_mailbox(vmu->mailbox, vmu->context, 1)) && !(vms = get_vm_state_by_mailbox(vmu->mailbox, vmu->context, 0))) {
		ast_log_chan(NULL, LOG_WARNING, "Couldn't find a vm_state for mailbox %s. Unable to set \\DELETED flag for message %d\n", vmu->mailbox, msgnum);
		return;
	}
	...
	if (messageNum == 0) {
		ast_log_chan(NULL, LOG_WARNING, "msgnum %d, mailbox message %lu is zero.\n", msgnum, messageNum);
		return;
	}
...
static int imap_retrieve_greeting(const char *dir, const int msgnum, struct ast_vm_user *vmu)
{
	...
	if (!(vms_p = get_vm_state_by_mailbox(vmu->mailbox, vmu->context, 1)) &&
		...
		*/
		if (!(vms_p = create_vm_state_from_user(vmu))) {
			ast_log_chan(NULL, LOG_NOTICE, "Unable to create vm_state object!\n");
			return -1;
		}
	}
	...
	*vms_p->introfn = '\0';
	...
	if (init_mailstream(vms_p, GREETINGS_FOLDER) || !vms_p->mailstream) {
		ast_log_chan(NULL, AST_LOG_ERROR, "IMAP mailstream is NULL or can't init_mailstream\n");
	...
	for (i = 0; i < vms_p->mailstream->nmsgs; i++) {
		...
		if (body->nested.part && body->nested.part->next && body->nested.part->next->body.parameter->value) {
			...
			ast_log_chan(NULL, AST_LOG_ERROR, "There is no file attached to this IMAP message.\n");
			ret = -1;
			break;
		}
	}
...
static int imap_retrieve_file(const char *dir, const int msgnum, const char *mailbox, const char *context)
{
	...
	if (!(vmu = find_user(NULL, context, mailbox))) {
		ast_log_chan(NULL, LOG_WARNING, "Couldn't find user with mailbox %s@%s\n", mailbox, context);
		return -1;
	}
	...
	if (!(vms = get_vm_state_by_mailbox(vmu->mailbox, vmu->context, 1)) && !(vms = get_vm_state_by_mailbox(vmu->mailbox, vmu->context, 0))) {
		 ...
		 */
		ast_log_chan(NULL, LOG_ERROR, "Couldn't find a vm_state for mailbox %s!!! Oh no!\n", vmu->mailbox);
		res = -1;
		goto exit;
	}
	...
	if (!vms->mailstream) {
		ast_log_chan(NULL, AST_LOG_ERROR, "IMAP mailstream for %s is NULL\n", vmu->mailbox);
		res = -1;
		goto exit;
	}
	...
	if (vms->msgArray[msgnum] == 0) {
		ast_log_chan(NULL, LOG_WARNING, "Trying to access unknown message\n");
		res = -1;
		goto exit;
	}
	...
	if (ast_strlen_zero(header_content)) {
		ast_log_chan(NULL, LOG_ERROR, "Could not fetch header for message number %ld\n", vms->msgArray[msgnum]);
		res = -1;
		goto exit;
	}
	...
	if (body->nested.part && body->nested.part->next && body->nested.part->next->body.parameter->value) {
		...
		ast_log_chan(NULL, LOG_ERROR, "There is no file attached to this IMAP message.\n");
		res = -1;
		goto exit;
	}
	...
	if (!attachedfilefmt) {
		ast_log_chan(NULL, LOG_ERROR, "File format could not be obtained from IMAP message attachment\n");
		res = -1;
		goto exit;
	}
	...
	if (!(text_file_ptr = fopen(text_file, "w"))) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to open/create file %s: %s\n", text_file, strerror(errno));
		goto exit;
	}

	fprintf(text_file_ptr, "%s\n", "[message]");

	if (get_header_by_tag(header_content, "X-GABpbx-VM-Caller-ID-Name:", buf, sizeof(buf))) {
		fprintf(text_file_ptr, "callerid=\"%s\" ", S_OR(buf, ""));
	}
	if (get_header_by_tag(header_content, "X-GABpbx-VM-Caller-ID-Num:", buf, sizeof(buf))) {
		fprintf(text_file_ptr, "<%s>\n", S_OR(buf, ""));
	}
	if (get_header_by_tag(header_content, "X-GABpbx-VM-Context:", buf, sizeof(buf))) {
		fprintf(text_file_ptr, "context=%s\n", S_OR(buf, ""));
	}
	if (get_header_by_tag(header_content, "X-GABpbx-VM-Orig-time:", buf, sizeof(buf))) {
		fprintf(text_file_ptr, "origtime=%s\n", S_OR(buf, ""));
	}
	if (get_header_by_tag(header_content, "X-GABpbx-VM-Duration:", buf, sizeof(buf))) {
		fprintf(text_file_ptr, "duration=%s\n", S_OR(buf, ""));
	}
	if (get_header_by_tag(header_content, "X-GABpbx-VM-Category:", buf, sizeof(buf))) {
		fprintf(text_file_ptr, "category=%s\n", S_OR(buf, ""));
	}
	if (get_header_by_tag(header_content, "X-GABpbx-VM-Flag:", buf, sizeof(buf))) {
		fprintf(text_file_ptr, "flag=%s\n", S_OR(buf, ""));
	}
	if (get_header_by_tag(header_content, "X-GABpbx-VM-Message-ID:", buf, sizeof(buf))) {
...
static int __messagecount(const char *context, const char *mailbox, const char *folder)
{
	...
	if (!vmu) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Couldn't find mailbox %s in context %s\n", mailbox, context);
		free_user(vmu);
		return -1;
	} else {
		/* No IMAP account available */
		if (vmu->imapuser[0] == '\0') {
			ast_log_chan(NULL, AST_LOG_WARNING, "IMAP user not set for mailbox %s\n", vmu->mailbox);
			free_user(vmu);
			return -1;
		}
	}
	...
	if (!vms_p->mailstream) {
		ast_log_chan(NULL, AST_LOG_ERROR, "Houston we have a problem - IMAP mailstream is NULL\n");
		free_user(vmu);
		return -1;
	}
	if (ret == 0) {
		...
		hdr = mail_newsearchheader ("X-GABpbx-VM-Extension", (char *)(!ast_strlen_zero(vmu->imapvmshareid) ? vmu->imapvmshareid : mailbox));
		hdr->next = mail_newsearchheader("X-GABpbx-VM-Context", (char *) S_OR(context, "default"));
		pgm->header = hdr;
		if (fold != OLD_FOLDER) {
			pgm->unseen = 1;
			pgm->seen = 0;
...
static int imap_check_limits(struct ast_channel *chan, struct vm_state *vms, struct ast_vm_user *vmu, int msgnum)
{
	...
	if (msgnum >= vmu->maxmsg - inprocess_count(vmu->mailbox, vmu->context, +1)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to leave message since we will exceed the maximum number of messages allowed (%u >= %u)\n", msgnum, vmu->maxmsg);
		if (chan) {
			ast_play_and_wait(chan, "vm-mailboxfull");
...
static int imap_store_file(const char *dir, const char *mailboxuser, const char *mailboxcontext, int msgnum, struct ast_channel *chan, struct ast_vm_user *vmu, char *fmt, int duration, struct vm_state *vms, const char *flag, const char *msg_id)
{
	...
	if (!(p = ast_file_mkftemp(tmp, VOICEMAIL_FILE_MODE & ~my_umask))) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Unable to store '%s' (can't create temporary file)\n", fn);
		if (tempcopy) {
			ast_free(vmu->email);
			vmu->email = NULL;
		}
	...
	if (msgnum < 0 && imapgreetings) {
		if ((ret = init_mailstream(vms, GREETINGS_FOLDER))) {
			ast_log_chan(NULL, AST_LOG_WARNING, "Unable to open mailstream.\n");
			return -1;
		}
	...
	if (!(buf = ast_malloc(len + 1))) {
		ast_log_chan(NULL, AST_LOG_ERROR, "Can't allocate %ld bytes to read message\n", len + 1);
		fclose(p);
		if (tempcopy)
			*(vmu->email) = '\0';
		return -1;
	...
	if (fread(buf, 1, len, p) != len) {
		if (ferror(p)) {
			ast_log_chan(NULL, LOG_ERROR, "Error while reading mail file: %s\n", tmp);
			return -1;
		}
	}
	((char *) buf)[len] = '\0';
	...
	if (ret == 0) {
		...
		if(!mail_append_full(vms->mailstream, mailbox, imap_flags, NIL, &str))
			ast_log_chan(NULL, LOG_ERROR, "Error while sending the message to %s\n", mailbox);
		ast_mutex_unlock(&vms->lock);
		fclose(p);
		unlink(tmp);
		ast_free(buf);
	} else {
		ast_log_chan(NULL, LOG_ERROR, "Could not initialize mailstream for %s\n", mailbox);
		fclose(p);
		unlink(tmp);
		ast_free(buf);
		return -1;
...
static int inboxcount2(const char *mailbox_context, int *urgentmsgs, int *newmsgs, int *oldmsgs)
{
	...
	if (newmsgs) {
		...
		if (!vmu) {
			ast_log_chan(NULL, AST_LOG_ERROR, "Couldn't find mailbox %s in context %s\n", mailboxnc, context);
			return -1;
		}
...
static int copy_message(struct ast_channel *chan, struct ast_vm_user *vmu, int imbox, int msgnum, long duration, struct ast_vm_user *recip, char *fmt, char *dir, char *flag, const char *dest_folder)
{
	...
	if (msgnum >= recip->maxmsg) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to copy mail, mailbox %s is full\n", recip->mailbox);
		return -1;
	}
	if (!(sendvms = get_vm_state_by_imapuser(vmu->imapuser, 0))) {
		ast_log_chan(NULL, LOG_ERROR, "Couldn't get vm_state for originator's mailbox!!\n");
		return -1;
	}
	if (!get_vm_state_by_imapuser(recip->imapuser, 0)) {
		ast_log_chan(NULL, LOG_ERROR, "Couldn't get vm_state for destination mailbox!\n");
		return -1;
	}
	...
	ast_mutex_unlock(&sendvms->lock);
	ast_log_chan(NULL, LOG_WARNING, "Unable to copy message from mailbox %s to mailbox %s\n", vmu->mailbox, recip->mailbox);
	return -1;
}
...
static int init_mailstream(struct vm_state *vms, int box)
{
	...
	if (!vms) {
		ast_log_chan(NULL, LOG_ERROR, "vm_state is NULL!\n");
		return -1;
	}
...
#include "linkage.c"
		...
		ast_mutex_unlock(&vms->lock);
		if (stream == NIL) {
			ast_log_chan(NULL, LOG_ERROR, "Can't connect to imap server %s\n", tmp);
			return -1;
		}
...
static int open_mailbox(struct vm_state *vms, struct ast_vm_user *vmu, int box)
{
	...
	if (init_mailstream(vms, box) || !vms->mailstream) {
		ast_log_chan(NULL, AST_LOG_ERROR, "Could not initialize mailstream\n");
		return -1;
	}
	...
	/* Check IMAP folder for GABpbx messages only... */
	hdr = mail_newsearchheader("X-GABpbx-VM-Extension", (!ast_strlen_zero(vmu->imapvmshareid) ? vmu->imapvmshareid : vmu->mailbox));
	hdr->next = mail_newsearchheader("X-GABpbx-VM-Context", vmu->context);
	pgm->header = hdr;
	pgm->deleted = 0;
	pgm->undeleted = 1;
	...
	if (box == 0 && !vms->dh_arraysize) {
		ast_log_chan(NULL, LOG_WARNING, "The code expects the old messages to be checked first, fix the code.\n");
	}
...
static void write_file(char *filename, char *buffer, unsigned long len)
{
	...
	if (fwrite(buffer, len, 1, output) != 1) {
		if (ferror(output)) {
			ast_log_chan(NULL, LOG_ERROR, "Short write while writing e-mail body: %s.\n", strerror(errno));
		}
	}
	fclose (output);
}
...
void mm_log(char *string, long errflg)
{
	switch ((short) errflg) {
		...
		case PARSE:
		case WARN:
			ast_log_chan(NULL, AST_LOG_WARNING, "IMAP Warning: %s\n", string);
			break;
		case ERROR:
			ast_log_chan(NULL, AST_LOG_ERROR, "IMAP Error: %s\n", string);
			break;
	}
}
...
{
	ast_log_chan(NULL, AST_LOG_NOTICE, "%s\n", string);
}
...
{
	ast_log_chan(NULL, AST_LOG_ERROR, "IMAP access FATAL error: %s\n", string);
}
...
static void mm_parsequota(MAILSTREAM *stream, unsigned char *msg, QUOTALIST *pquota)
{
	...
	if (!(user = get_user_by_mailbox(mailbox, buf, sizeof(buf))) || (!(vms = get_vm_state_by_imapuser(user, 2)) && !(vms = get_vm_state_by_imapuser(user, 0)))) {
		ast_log_chan(NULL, AST_LOG_ERROR, "No state found.\n");
		return;
	}
...
static void vmstate_delete(struct vm_state *vms)
{
	...
	if (vc) {
		...
		ast_log_chan(NULL, AST_LOG_ERROR, "No vmstate found for user:%s, mailbox %s\n", vms->imapuser, vms->username);
	}
}
...
static void set_update(MAILSTREAM * stream)
{
	...
	if (!(user = get_user_by_mailbox(mailbox, buf, sizeof(buf))) || !(vms = get_vm_state_by_imapuser(user, 0))) {
		if (user && DEBUG_ATLEAST(3))
			ast_log_chan(NULL, AST_LOG_WARNING, "User %s mailbox not found for update.\n", user);
		return;
	}
...
static int save_body(BODY *body, struct vm_state *vms, char *section, char *format, int is_intro)
{
	...
	if (len > MAX_MAIL_BODY_CONTENT_SIZE) {
		ast_log_chan(NULL, AST_LOG_ERROR,
...
static void check_quota(struct vm_state *vms, char *mailbox) {
	...
	if (vms && vms->mailstream != NULL) {
		...
		ast_log_chan(NULL, AST_LOG_WARNING, "Mailstream not available for mailbox: %s\n", mailbox);
	}
	ast_mutex_unlock(&vms->lock);
}
...
/*!
 * \internal
 * \brief Create a buffer containing the SQL statement with the table name inserted.
 *
 * \param __sql_fmt The SQL statement with a single '%s' where the table name should be inserted.
 *
 * \note The buffer is allocated on the stack and should not be freed.
 *
 * \return A pointer to a buffer containing the SQL statement with the table name inserted.
 */
#define MAKE_SQL_PTRA(__sql_fmt) \
({ \
	/* The NULL terminator is included in odbc_table_len. */ \
	char *__sql = ast_alloca(strlen(__sql_fmt) + odbc_table_len); \
	sprintf(__sql, __sql_fmt, odbc_table); /* Safe */ \
	__sql; \
})

/*!
 * \internal
 * \brief Create a buffer containing the SQL statement with the table name inserted twice.
 *
 * \param __sql_fmt The SQL statement with two '%s' where the table name should be inserted.
 *
 * \note The buffer is allocated on the stack and should not be freed.
 *
 * \return A pointer to a buffer containing the SQL statement with the table name inserted.
 */
#define MAKE_SQL_PTRA2(__sql_fmt) \
({ \
	/* The NULL terminator is included in odbc_table_len. */ \
	char *__sql = ast_alloca(strlen(__sql_fmt) + (odbc_table_len * 2)); \
	sprintf(__sql, __sql_fmt, odbc_table, odbc_table); /* Safe */ \
	__sql; \
})
...
static SQLHSTMT generic_prepare(struct odbc_obj *obj, void *data)
{
	...
	if (!SQL_SUCCEEDED(res)) {
		ast_log_chan(NULL, AST_LOG_WARNING, "SQL Alloc Handle failed!\n");
		return NULL;
	}
	res = ast_odbc_prepare(obj, stmt, gps->sql);
	if (!SQL_SUCCEEDED(res)) {
		ast_log_chan(NULL, AST_LOG_WARNING, "SQL Prepare failed![%s]\n", gps->sql);
...
{
	SQLHSTMT stmt;
	char *sql = MAKE_SQL_PTRA("UPDATE %s SET msg_id=? WHERE dir=? AND msgnum=?");
	struct odbc_obj *obj;
	char msg_num_str[20];
	char *argv[] = { msg_id, dir, msg_num_str };
	struct generic_prepare_struct gps = { .sql = sql, .argc = 3, .argv = argv };
	SCOPE_ENTER(3, "dir: %s msg_num: %d msg_id: %s\n", dir, msg_num, msg_id);

	obj = ast_odbc_request_obj(odbc_database, 0);
	if (!obj) {
		SCOPE_EXIT_LOG_RTN(LOG_WARNING, "Unable to update message ID for message %d in %s\n", msg_num, dir);
	}

	snprintf(msg_num_str, sizeof(msg_num_str), "%d", msg_num);
	stmt = ast_odbc_prepare_and_execute(obj, generic_prepare, &gps);
	if (!stmt) {
		ast_log_chan(NULL, LOG_WARNING, "SQL Execute error!\n[%s]\n\n", sql);
	} else {
		SQLFreeHandle(SQL_HANDLE_STMT, stmt);
	}
...
#define AUDIO_ON_DISK_MAGIC "AUDMAGIC"
#define AUDIO_ON_DISK_MAGIC_LEN 8

static void odbc_update_set_audmagic(char *dir, int msg_num)
{
	SQLHSTMT stmt;
	char *sql = MAKE_SQL_PTRA("UPDATE %s SET recording=? WHERE dir=? AND msgnum=?");
	struct odbc_obj *obj;
	SQLLEN datalen = AUDIO_ON_DISK_MAGIC_LEN;
	SQLLEN indlen = datalen;
	int res;
	char msg_num_str[20];
	SCOPE_ENTER(3, "dir: %s msg_num: %d\n", dir, msg_num);

	obj = ast_odbc_request_obj(odbc_database, 0);
	if (!obj) {
		SCOPE_EXIT_LOG_RTN(LOG_WARNING, "Unable to request obj for message %d in %s\n", msg_num, dir);
	}

	res = SQLAllocHandle(SQL_HANDLE_STMT, obj->con, &stmt);
	if (!SQL_SUCCEEDED(res)) {
		ast_odbc_release_obj(obj);
		SCOPE_EXIT_LOG_RTN(LOG_WARNING, "Unable to allocate stmt for message %d in %s\n", msg_num, dir);
	}

	snprintf(msg_num_str, sizeof(msg_num_str), "%d", msg_num);

	SQLBindParameter(stmt, 1, SQL_PARAM_INPUT, SQL_C_BINARY, SQL_LONGVARBINARY,
		datalen, 0, (void *) AUDIO_ON_DISK_MAGIC,
		datalen, &indlen);

	SQLBindParameter(stmt, 2, SQL_PARAM_INPUT, SQL_C_CHAR, SQL_CHAR,
		strlen(dir), 0, (void *) dir, 0, NULL);

	SQLBindParameter(stmt, 3, SQL_PARAM_INPUT, SQL_C_CHAR, SQL_CHAR,
		strlen(msg_num_str), 0, (void *) msg_num_str, 0, NULL);

	res = ast_odbc_execute_sql(obj, stmt, sql);
	if (!SQL_SUCCEEDED(res)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to execute stmt for message %d in %s\n", msg_num, dir);
...
static int odbc_retrieve_message(char *dir, int msgnum)
{
	...
	SQLHSTMT stmt;
	char *sql = MAKE_SQL_PTRA("SELECT * FROM %s WHERE dir=? AND msgnum=?");
	char fmt[80] = "";
	char *c;
	char coltitle[256];
	SQLSMALLINT collen;
	...
	char *full_fn = MAKE_FILE_EXT_PTRA(dir, msgnum, "txt");
	char msgnums[80];
	char *mailboxuser = NULL;
	char *mailboxcontext = NULL;
	char msg_id[MSG_ID_LEN] = "";
	char *argv[] = { dir, msgnums };
	struct generic_prepare_struct gps = { .sql = sql, .argc = 2, .argv = argv };
	struct odbc_obj *obj;
	int storage_conversion_to_disk = 0;
	int storage_conversion_to_odbc = 0;
	SCOPE_ENTER(3, "dir: %s msgnum: %d  msgtype: %s\n", dir, msgnum, msgnum < 0 ? "Greeting" : "Message");

	obj = ast_odbc_request_obj(odbc_database, 0);
	if (!obj) {
		SCOPE_EXIT_LOG_RTN_VALUE(-1, AST_LOG_WARNING, "Failed to obtain database object for '%s'!\n", odbc_database);
	}
	...
	ast_trace(-1, "Opening '%s' for writing\n", full_fn);
	if (!(f = fopen(full_fn, "w+"))) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Failed to open/create '%s'\n", full_fn);
		goto bail;
	}

	sprintf(full_fn, "%s.%s", fn, fmt); /* Safe.  We're just replacing the file exten. */

	stmt = ast_odbc_prepare_and_execute(obj, generic_prepare, &gps);
	if (!stmt) {
		ast_log_chan(NULL, AST_LOG_WARNING, "SQL Execute error!\n[%s]\n\n", sql);
		goto bail;
	}
	...
	if (!SQL_SUCCEEDED(res)) {
		if (res != SQL_NO_DATA) {
			ast_log_chan(NULL, AST_LOG_WARNING, "SQL Fetch error!\n[%s]\n\n", sql);
		}
	...
	if (!SQL_SUCCEEDED(res)) {
		ast_log_chan(NULL, AST_LOG_WARNING, "SQL Column Count error!\n[%s]\n\n", sql);
		goto bail_with_handle;
	}
	...
	for (x = 0; x < colcount; x++) {
		...
		if (!SQL_SUCCEEDED(res)) {
			ast_log_chan(NULL, AST_LOG_WARNING, "SQL Describe Column error!\n[%s]\n\n", sql);
			goto bail_with_handle;
		}
		...
		if (!strcasecmp(coltitle, "recording")) {
			...
			fdlen = colsize2;
			ast_trace(-1, "Audio size: %ld\n", colsize2);
			if (colsize2 == AUDIO_ON_DISK_MAGIC_LEN) {
				res = SQLGetData(stmt, x + 1, SQL_BINARY, rowdata, AUDIO_ON_DISK_MAGIC_LEN, NULL);
				if (memcmp(rowdata, AUDIO_ON_DISK_MAGIC, AUDIO_ON_DISK_MAGIC_LEN) != 0) {
					ast_log_chan(NULL, AST_LOG_WARNING, "Invalid audio magic number '0x%02X%02X%02X%02X%02X%02X%02X%02X' for '%s'\n",
						rowdata[0], rowdata[1], rowdata[2], rowdata[3], rowdata[4], rowdata[5], rowdata[6],
						rowdata[7], full_fn);
					goto bail_with_handle;
				}
				ast_trace(-1, "Audio is stored on disk.  No need to write '%s'\n", full_fn);
				if (!ast_test_flag((&globalflags), VM_ODBC_AUDIO_ON_DISK)) {
					storage_conversion_to_odbc = 1;
				}
			...
			if (fd < 0) {
				ast_log_chan(NULL, AST_LOG_WARNING, "Failed to open '%s' for writing: %s\n", full_fn, strerror(errno));
				goto bail_with_handle;
			}
			if (ast_test_flag((&globalflags), VM_ODBC_AUDIO_ON_DISK)) {
				storage_conversion_to_disk = 1;
			}
			...
			for (offset = 0; offset < colsize2; offset += CHUNKSIZE) {
				if ((fdm = mmap(NULL, CHUNKSIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd, offset)) == MAP_FAILED) {
					ast_log_chan(NULL, AST_LOG_WARNING, "Could not mmap the output file: %s (%d)\n", strerror(errno), errno);
					goto bail_with_handle;
				}
				res = SQLGetData(stmt, x + 1, SQL_BINARY, fdm, CHUNKSIZE, NULL);
				munmap(fdm, CHUNKSIZE);
				if (!SQL_SUCCEEDED(res)) {
					ast_log_chan(NULL, AST_LOG_WARNING, "SQL Get Data error!\n[%s]\n\n", sql);
					unlink(full_fn);
					goto bail_with_handle;
				}
			}
			if (truncate(full_fn, fdlen) < 0) {
				ast_log_chan(NULL, LOG_WARNING, "Unable to truncate '%s': %s\n", full_fn, strerror(errno));
			}
			ast_trace(-1, "Wrote %d bytes to '%s'\n", (int)fdlen, full_fn);
		} else {
			res = SQLGetData(stmt, x + 1, SQL_CHAR, rowdata, sizeof(rowdata), NULL);
			if (res == SQL_NULL_DATA && !strcasecmp(coltitle, "msg_id")) {
				/*
				 * Generate msg_id if there wasn't one, but don't store it until we're
				 * done with this connection.
				 */
				generate_msg_id(msg_id);
				ast_trace(-1, "msg_id was NULL.  Generating new one: %s\n", msg_id);
				snprintf(rowdata, sizeof(rowdata), "%s", msg_id);
			} else if (!strcasecmp(coltitle, "mailboxuser")) {
				mailboxuser = ast_strdupa(rowdata);
			} else if (!strcasecmp(coltitle, "mailboxcontext")) {
				mailboxcontext = ast_strdupa(rowdata);
				...
				ast_log_chan(NULL, AST_LOG_WARNING, "SQL Get Data error! coltitle=%s\n[%s]\n\n", coltitle, sql);
				goto bail_with_handle;
			}
...
bail:
	...
	if (SQL_SUCCEEDED(res)) {
		if (storage_conversion_to_disk) {
			/*
			 * If we're currently storing audio on disk but there was pre-existing
			 * audio in the database, we need to update the database to set the
			 * 'recording' column to AUDIO_ON_DISK_MAGIC.
			 */
			SCOPE_CALL(-1, odbc_update_set_audmagic, dir, msgnum);
		}
		if (storage_conversion_to_odbc) {
			/*
			 * If we're currently storing audio in the database but there was
			 * pre-existing audio on disk, we need to add the audio back
 ...
 */
static int odbc_last_message_index(char *dir)
{
	int x = -1;
	int res;
	SQLHSTMT stmt;
	char *sql = MAKE_SQL_PTRA("SELECT msgnum FROM %s WHERE dir=? order by msgnum desc");
	char rowdata[20];
	char *argv[] = { dir };
	...
	if (!SQL_SUCCEEDED(res)) {
		if (res == SQL_NO_DATA) {
			ast_trace(-1, "Directory '%s' has no messages and therefore no index was retrieved.\n", dir);
		} else {
			ast_log_chan(NULL, AST_LOG_WARNING, "SQL Fetch error!\n[%s]\n\n", sql);
		}
		goto bail_with_handle;
	...
	if (sscanf(rowdata, "%30d", &x) != 1) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Failed to read message index!\n");
	}
...
bail:
	...
	SCOPE_EXIT_RTN_VALUE(x, "Done.  Last message index: %d\n", x);
}
 ...
 */
static int odbc_message_exists(char *dir, int msgnum)
{
	int x = 0;
	int res;
	SQLHSTMT stmt;
	char *sql = MAKE_SQL_PTRA("SELECT COUNT(*) FROM %s WHERE dir=? AND msgnum=?");
	char rowdata[20];
	char msgnums[20];
	char *argv[] = { dir, msgnums };
	struct generic_prepare_struct gps = { .sql = sql, .argc = 2, .argv = argv };
	...
	obj = ast_odbc_request_obj(odbc_database, 0);
	if (!obj) {
		SCOPE_EXIT_LOG_RTN_VALUE(0, AST_LOG_WARNING, "Failed to obtain database object for '%s'!\n", odbc_database);
	}
	...
	if (sscanf(rowdata, "%30d", &x) != 1) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Failed to read message count!\n");
	}
...
bail:
	ast_odbc_release_obj(obj);
	SCOPE_EXIT_RTN_VALUE(x, "Done.  Msg %s\n", x ? "exists" : "does not exist");
}
...
static int odbc_count_messages(struct ast_vm_user *vmu, char *dir)
{
	...
	int res;
	SQLHSTMT stmt;
	char *sql = MAKE_SQL_PTRA("SELECT COUNT(*) FROM %s WHERE dir=?");
	char rowdata[20];
	char *argv[] = { dir };
	struct generic_prepare_struct gps = { .sql = sql, .argc = 1, .argv = argv };
	struct odbc_obj *obj;
...
bail:
	ast_odbc_release_obj(obj);
	SCOPE_EXIT_RTN_VALUE(x, "Done.  Count %d\n", x);
}
 ...
 */
#define DELETE_SQL_FMT "DELETE FROM %s WHERE dir=? AND msgnum=?"
static void odbc_delete_message(const char *sdir, int smsg)
{
	SQLHSTMT stmt;
	char *sql = MAKE_SQL_PTRA(DELETE_SQL_FMT);
	char msgnums[20];
	...
	obj = ast_odbc_request_obj(odbc_database, 0);
	if (!obj) {
		SCOPE_EXIT_LOG_RTN(AST_LOG_WARNING, "Failed to obtain database object for '%s'!\n", odbc_database);
	}
	...
	if (ast_test_flag((&globalflags), VM_ODBC_AUDIO_ON_DISK)) {
		char *src_fn = MAKE_FILE_PTRA(sdir, smsg);
		ast_trace(-1, "Audio stored on disk. Deleting '%s'\n", src_fn);
		ast_filedelete(src_fn, NULL);
	}
...
static void odbc_copy_message(char *sdir, int smsg, char *ddir, int dmsg, char *dmailboxuser, char *dmailboxcontext)
{
	...
	SCOPE_CALL(-1, odbc_delete_message, ddir, dmsg);
	...
	if (ast_test_flag((&globalflags), VM_ODBC_AUDIO_ON_DISK)) {
		char *src_fn = MAKE_FILE_PTRA(sdir, smsg);
		char *dst_fn = MAKE_FILE_PTRA(ddir, dmsg);

		ast_trace(-1, "Audio stored on disk.  Copying '%s' to '%s'\n", src_fn, dst_fn);
		ast_filecopy(src_fn, dst_fn, NULL);
	}
...
static SQLHSTMT odbc_insert_data_cb(struct odbc_obj *obj, void *vdata)
{
	...
	res = SQLAllocHandle(SQL_HANDLE_STMT, obj->con, &stmt);
	if (!SQL_SUCCEEDED(res)) {
		SCOPE_EXIT_LOG_RTN_VALUE(NULL, AST_LOG_WARNING, "SQL Alloc Handle failed!\n");
	}

	/* Delete any existing row with the same dir and msgnum */
	delete_sql = MAKE_SQL_PTRA(DELETE_SQL_FMT);
	...
	res = ast_odbc_execute_sql(obj, stmt, delete_sql);
	if (!SQL_SUCCEEDED(res)) {
		ast_trace(-1, "There wasn't an existing row.  Good.\n");
	} else {
		ast_trace(-1, "There WAS an existing row. This is OK if we're replacing a message.\n");
	}
	SQLFreeHandle(SQL_HANDLE_STMT, stmt);
	stmt = NULL;
	...
	if (!ast_strlen_zero(data->category)) {
		...
		insert_sql = MAKE_SQL_PTRA(STORE_SQL_FMT);
	}
...
static int odbc_store_message(const char *dir, const char *mailboxuser, const char *mailboxcontext, int msgnum)
{
	...
	do {
		...
		ast_trace(-1, "Formats: %s Using format: '%s'\n", vmfmts, fmt);
		snprintf(msgnums, sizeof(msgnums), "%d", msgnum);

		ast_trace(-1, "Base path: '%s'\n", fn);
		ast_trace(-1, "Opening '%s'\n", full_fn);
		cfg = ast_config_load(full_fn, config_flags);
		if (!valid_config(cfg)) {
			if (msgnum < 0) {
				ast_trace(-1, "No information file found for '%s'.  This is a greeting so this is OK.\n", full_fn);
			} else {
				ast_log_chan(NULL, AST_LOG_WARNING, "Failed to open '%s'\n", full_fn);
				res = -1;
				break;
			}
		}

		sprintf(full_fn, "%s.%s", fn, fmt); /* Safe */
		if (ast_test_flag((&globalflags), VM_ODBC_AUDIO_ON_DISK)) {
			ast_trace(-1, "Audio stored on disk.  No need to open '%s'\n", full_fn);
		} else {
			ast_trace(-1, "Opening '%s'\n", full_fn);
			fd = open(full_fn, O_RDWR);
			if (fd < 0) {
				ast_log_chan(NULL, AST_LOG_WARNING, "Open of sound file '%s' failed: %s\n", full_fn, strerror(errno));
				res = -1;
				break;
			}
		}
		...
		if (valid_config(cfg)) {
			ast_trace(-1, "Using information file '%s'\n", fn);
		...
		if (fd < 0) {
			...
			idata.datalen = idata.indlen = AUDIO_ON_DISK_MAGIC_LEN;
		} else {
			ast_trace(-1, "Reading sound file '%s'\n", full_fn);
			fdlen = lseek(fd, 0, SEEK_END);
			if (fdlen < 0 || lseek(fd, 0, SEEK_SET) < 0) {
				ast_log_chan(NULL, AST_LOG_WARNING, "Failed to process sound file '%s': %s\n", full_fn, strerror(errno));
				res = -1;
				break;
			}
			fdm = mmap(NULL, fdlen, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
			if (fdm == MAP_FAILED) {
				ast_log_chan(NULL, AST_LOG_WARNING, "Memory map failed for sound file '%s'!\n", full_fn);
				res = -1;
				break;
			}
			idata.data = fdm;
		...
		if ((stmt = ast_odbc_direct_execute(obj, odbc_insert_data_cb, &idata))) {
			SQLFreeHandle(SQL_HANDLE_STMT, stmt);
		} else {
			res = -1;
		}
...
{
	SQLHSTMT stmt;
	char *sql = MAKE_SQL_PTRA("UPDATE %s SET dir=?, msgnum=? WHERE mailboxuser=? AND mailboxcontext=? AND dir=? AND msgnum=?");
	char msgnums[20];
	char msgnumd[20];
	struct odbc_obj *obj;
	char *argv[] = { ddir, msgnumd, mailboxuser, mailboxcontext, sdir, msgnums };
	struct generic_prepare_struct gps = { .sql = sql, .argc = 6, .argv = argv };
	SCOPE_ENTER(3, "sdir: %s smsg: %d user: %s context: %s ddir: %s dmsg: %d\n", sdir, smsg,
		mailboxuser, mailboxcontext, ddir, dmsg);
	...
	if (ast_test_flag((&globalflags), VM_ODBC_AUDIO_ON_DISK)) {
		...
		ast_trace(-1, "Recordings stored on disk.  Renaming '%s' to '%s'\n", src_fn, dst_fn);
		ast_filerename(src_fn, dst_fn, NULL);
	}
 ...
 * \return zero on success, -1 on error.
 */
static int odbc_remove_files(char *dir, int msgnum)
{
	char *fn = MAKE_FILE_PTRA(dir, msgnum);
	char *full_fn = MAKE_FILE_EXT_PTRA(dir, msgnum, "txt");
	SCOPE_ENTER(3, "dir: %s msgnum: %d\n", dir, msgnum);
	...
	if (ast_test_flag((&globalflags), VM_ODBC_AUDIO_ON_DISK)) {
		...
		ast_trace(-1, "Audio stored in ODBC.  Removing '%s' sound files\n", fn);
		ast_filedelete(fn, NULL);
	}

	/* Always remove the information file */
	ast_trace(-1, "Removing '%s' information file\n", full_fn);
	unlink(full_fn);
...
static int copy(char *infile, char *outfile)
{
	...
	if ((ifd = open(infile, O_RDONLY)) < 0) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Unable to open %s in read-only mode: %s\n", infile, strerror(errno));
		return -1;
	}

	if ((ofd = open(outfile, O_WRONLY | O_TRUNC | O_CREAT, VOICEMAIL_FILE_MODE)) < 0) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Unable to open %s in write-only mode: %s\n", outfile, strerror(errno));
		close(ifd);
		return -1;
	}
	...
	for (;;) {
		...
		if (len < 0) {
			ast_log_chan(NULL, AST_LOG_WARNING, "Read failed on %s: %s\n", infile, strerror(errno));
			break;
		}

		wrlen = write(ofd, buf, len);
		if (errno == ENOMEM || errno == ENOSPC || wrlen != len) {
			ast_log_chan(NULL, AST_LOG_WARNING, "Write failed on %s (%d of %d): %s\n", outfile, wrlen, len, strerror(errno));
			break;
		}
	}
...
static int vm_delete(char *file)
{
	...
	ast_trace(-1, "unlinking '%s'\n", txt);
	unlink(txt);
	ast_trace(-1, "deleting sound files '%s'\n", file);
		...
		const char *msg_id)
{
	...
	if (!ast_strlen_zero(fromstring) || !ast_strlen_zero(vmu->fromstring)) {
		...
		if ((ast = ast_dummy_channel_alloc())) {
			...
			ast_log_chan(NULL, AST_LOG_WARNING, "Cannot allocate the channel for variables substitution\n");
		}
	} else {
		fprintf(p, "From: GABpbx PBX <%s>" ENDL, who);
	}
	...
	fprintf(p, "Message-ID: <GABpbx-%d-%u-%s-%d@%s>" ENDL, msgnum + 1,
		(unsigned int) ast_random(), mailbox, (int) getpid(), host);
	if (imap) {
		/* additional information needed for IMAP searching */
		fprintf(p, "X-GABpbx-VM-Message-Num: %d" ENDL, msgnum + 1);
		/* fprintf(p, "X-GABpbx-VM-Orig-Mailbox: %s" ENDL, ext); */
		fprintf(p, "X-GABpbx-VM-Server-Name: %s" ENDL, fromstring);
		fprintf(p, "X-GABpbx-VM-Context: %s" ENDL, context);
#ifdef IMAP_STORAGE
		fprintf(p, "X-GABpbx-VM-Extension: %s" ENDL, (!ast_strlen_zero(vmu->imapvmshareid) ? vmu->imapvmshareid : mailbox));
#else
		fprintf(p, "X-GABpbx-VM-Extension: %s" ENDL, mailbox);
#endif
		/* flag added for Urgent */
		fprintf(p, "X-GABpbx-VM-Flag: %s" ENDL, S_OR(flag, ""));
		fprintf(p, "X-GABpbx-VM-Priority: %d" ENDL, chan ? ast_channel_priority(chan) : 0);
		fprintf(p, "X-GABpbx-VM-Caller-ID-Num: %s" ENDL, enc_cidnum);
		fprintf(p, "X-GABpbx-VM-Caller-ID-Name: %s" ENDL, enc_cidname);
		fprintf(p, "X-GABpbx-VM-Duration: %d" ENDL, duration);
		if (!ast_strlen_zero(category)) {
			fprintf(p, "X-GABpbx-VM-Category: %s" ENDL, category);
		} else {
			fprintf(p, "X-GABpbx-VM-Category: " ENDL);
		}
		fprintf(p, "X-GABpbx-VM-Message-Type: %s" ENDL, msgnum > -1 ? "Message" : greeting_attachment);
		fprintf(p, "X-GABpbx-VM-Orig-date: %s" ENDL, date);
		fprintf(p, "X-GABpbx-VM-Orig-time: %ld" ENDL, (long) time(NULL));
		fprintf(p, "X-GABpbx-VM-Message-ID: %s" ENDL, msg_id);
	}
	if (!ast_strlen_zero(cidnum)) {
		fprintf(p, "X-GABpbx-CallerID: %s" ENDL, enc_cidnum);
	}
	if (!ast_strlen_zero(cidname)) {
		fprintf(p, "X-GABpbx-CallerIDName: %s" ENDL, enc_cidname);
	}
		...
		if (strcmp(vmu->mailbox, mailbox)) {
			...
			if ((msg_cfg = ast_config_load(fromfile, config_flags)) && valid_config(msg_cfg)) {
				...
				fprintf(p, "Dear %s:" ENDL ENDL "\tJust wanted to let you know you were just forwarded"
					...
					" chance.  Thanks!" ENDL ENDL "\t\t\t\t--GABpbx" ENDL ENDL, vmu->fullname, dur,
...
plain_message:
			fprintf(p, "Dear %s:" ENDL ENDL "\tJust wanted to let you know you were just left a "
				...
				"want to check it when you get a chance.  Thanks!" ENDL ENDL "\t\t\t\t--GABpbx"
...
static int add_email_attachment(FILE *p, struct ast_vm_user *vmu, char *format, char *attach, char *greeting_attachment, char *mailbox, char *bound, char *filename, int last, int msgnum)
{
	...
	if (!ast_file_is_readable(fname)) {
		...
		ast_log_chan(NULL, AST_LOG_WARNING, "Failed to open file: %s: %s - trying first/alternate format %s\n", fname, strerror(errno), altformat);
		snprintf(altfname, sizeof(altfname), "%s.%s", attach, altformat);
		if (!ast_file_is_readable(altfname)) {
			ast_log_chan(NULL, AST_LOG_WARNING, "Failed to open file: %s: %s - alternate format %s failure\n", altfname, strerror(errno), altformat);
		} else {
			altused = 1;
		}
	}
	...
	while (vmu->volgain < -.001 || vmu->volgain > .001 || altused) {
		...
		if (res >= sizeof(sox_gain_tmpdir)) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to create temporary directory path %s: Out of buffer space\n", tmpdir);
			break;
		}
		...
		if (mkdtemp(sox_gain_tmpdir)) {
			...
			if (res >= sizeof(fname)) {
				ast_log_chan(NULL, LOG_ERROR, "Failed to create filename buffer for %s/output.%s: Too long\n", sox_gain_tmpdir, format);
				break;
			}
			...
			if (res >= sizeof(sox_gain_cmd)) {
				ast_log_chan(NULL, LOG_ERROR, "Failed to generate sox command, out of buffer space\n");
				break;
			}
			...
			if (!soxstatus) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Sox failed to re-encode %s: %s (have you installed support for all sox file formats?)\n",
						fname,
						soxstatus == 1 ? "Problem with command line options" : "An error occurred during file processing");
				ast_log_chan(NULL, LOG_WARNING, "Voicemail attachment will have no volume gain.\n");
			}
		}
	...
	if (!file_to_delete) {
		...
		if (res >= sizeof(fname)) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to create filename buffer for %s.%s: Too long\n", attach, format);
			return -1;
		}
	}
		...
		const char *msg_id)
{
	...
	if (vmu && ast_strlen_zero(vmu->email)) {
		ast_log_chan(NULL, AST_LOG_WARNING, "E-mail address missing for mailbox [%s].  E-mail will not be sent.\n", vmu->mailbox);
		return(0);
	}
	...
	if ((p = ast_file_mkftemp(tmp, VOICEMAIL_FILE_MODE & ~my_umask)) == NULL) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Unable to launch '%s' (can't create temporary file)\n", mailcmd);
		return -1;
	} else {
...
static int invent_message(struct ast_channel *chan, char *context, char *ext, int busy, char *ecodes)
{
	...
	if ((res = create_dirpath(dest, sizeof(dest), context, ext, ""))) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Failed to make directory(%s)\n", fn);
		return -1;
	}
...
#ifdef ODBC_STORAGE
#define COUNT_MSGS_SQL_FMT "SELECT COUNT(*) FROM %s WHERE dir = '%s%s/%s/%s'"
...
{
	int res;
	char sql[sizeof(COUNT_MSGS_SQL_FMT) + odbc_table_len + strlen(VM_SPOOL_DIR)
			 + strlen(context) + strlen(mailbox) + strlen(folder)];
	char rowdata[20];
	SQLHSTMT stmt = NULL;
	struct generic_prepare_struct gps = { .sql = sql, .argc = 0 };
	SCOPE_ENTER(3, "context: %s  mb: %s  folder: %s", context, mailbox, folder);

	if (!messages) {
		SCOPE_EXIT_RTN_VALUE(0, "No messages pointer\n");
	}

	snprintf(sql, sizeof(sql), COUNT_MSGS_SQL_FMT, odbc_table, VM_SPOOL_DIR, context, mailbox, folder);
	if (!(stmt = ast_odbc_prepare_and_execute(odbc, generic_prepare, &gps))) {
		SCOPE_EXIT_LOG_RTN_VALUE(1, LOG_WARNING, "SQL Execute error!\n[%s]\n\n", sql);
	}
	res = SQLFetch(stmt);
	if (!SQL_SUCCEEDED(res)) {
		SQLFreeHandle(SQL_HANDLE_STMT, stmt);
		SCOPE_EXIT_LOG_RTN_VALUE(1, LOG_WARNING, "SQL Fetch error!\n[%s]\n\n", sql);
	}
	res = SQLGetData(stmt, 1, SQL_CHAR, rowdata, sizeof(rowdata), NULL);
	if (!SQL_SUCCEEDED(res)) {
		SQLFreeHandle(SQL_HANDLE_STMT, stmt);
		SCOPE_EXIT_LOG_RTN_VALUE(1, LOG_WARNING, "SQL Get Data error!\n[%s]\n\n", sql);
	}
...
static int inboxcount2(const char *mailbox, int *urgentmsgs, int *newmsgs, int *oldmsgs)
{
	...
	/* If no mailbox, return immediately */
	if (ast_strlen_zero(mailbox)) {
		SCOPE_EXIT_RTN_VALUE(0, "No mailbox\n");
	}
	...
	if (strchr(mailbox, ' ') || strchr(mailbox, ',')) {
		...
		while ((next = strsep(&remaining, " ,"))) {
			if (inboxcount2(next, urgentmsgs ? &u : NULL, &n, &o)) {
				SCOPE_EXIT_RTN_VALUE(-1, "Failed to obtain message count for mailbox %s\n", next);
			}
			if (urgentmsgs) {
				*urgentmsgs += u;
			}
	...
	if (count_messages_in_folder(obj, context, tmp, "INBOX", newmsgs)
	   ...
	   || count_messages_in_folder(obj, context, tmp, "Urgent", urgentmsgs)) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Failed to obtain message count for mailbox %s@%s\n",
				tmp, context);
	}
 ...
 * \return The number of messages in this mailbox folder (zero or more).
 */
#define MSGCOUNT_SQL_FMT_INBOX "SELECT COUNT(*) FROM %s WHERE dir = '%s%s/%s/INBOX' OR dir = '%s%s/%s/Urgent'"
#define MSGCOUNT_SQL_FMT "SELECT COUNT(*) FROM %s WHERE dir = '%s%s/%s/%s'"
static int messagecount(const char *mailbox_id, const char *folder)
{
	...
	SQLHSTMT stmt = NULL;
	char rowdata[20];
	struct generic_prepare_struct gps = { .argc = 0 };
	SCOPE_ENTER(3, "mb: %s  folder: %s", mailbox_id, folder);
 ...
 * \return zero on success, -1 on error.
 */
static int copy_message(struct ast_channel *chan, struct ast_vm_user *vmu, int imbox, int msgnum,
	long duration, struct ast_vm_user *recip, char *fmt, char *dir, const char *flag,
	const char *dest_folder)
{
	...
	int recipmsgnum;
	int res = 0;
	SCOPE_ENTER(3, "mb: %s  imb: %d  msgnum: %d recip: %s dir: %s dest_folder: %s",
		vmu->mailbox, imbox, msgnum, recip->mailbox, dir, dest_folder);

	ast_log_chan(NULL, AST_LOG_NOTICE, "Copying message from %s@%s to %s@%s\n", vmu->mailbox, vmu->context, recip->mailbox, recip->context);
	...
	ast_trace(-1, "fromdir: %s\n", fromdir);

	make_file(frompath, sizeof(frompath), fromdir, msgnum);
	ast_trace(-1, "frompath: %s\n", frompath);

	make_dir(todir, sizeof(todir), recip->context, recip->mailbox, userfolder);
	ast_trace(-1, "todir: %s\n", todir);

	if (vm_lock_path(todir)) {
		SCOPE_EXIT_RTN_VALUE(ERROR_LOCK_PATH);
	}

	recipmsgnum = LAST_MSG_INDEX(todir) + 1;
	ast_trace(-1, "recip msgnum: %d\n", recipmsgnum);
	...
	if (recipmsgnum < recip->maxmsg - (imbox ? 0 : inprocess_count(vmu->mailbox, vmu->context, 0))) {
		...
		exists = SCOPE_CALL_WITH_INT_RESULT(-1, EXISTS, fromdir, msgnum, frompath, chan ? ast_channel_language(chan) : "");
		if (exists) {
			SCOPE_CALL(-1, COPY, fromdir, msgnum, todir, recipmsgnum, recip->mailbox, recip->context, frompath, topath);
		} else {
			SCOPE_CALL(-1, copy_plain_file,frompath, topath);
			SCOPE_CALL(-1, STORE, todir, recip->mailbox, recip->context, recipmsgnum, chan, recip, fmt, duration, NULL, NULL, NULL);
			SCOPE_CALL(-1, vm_delete, topath);

		}
	} else {
		ast_log_chan(NULL, AST_LOG_ERROR, "Recipient mailbox %s@%s is full\n", recip->mailbox, recip->context);
		res = -1;
	}
...
static void run_externnotify(const char *context, const char *extension, const char *flag)
{
	...
	if (smdi_iface) {
		...
		if ((mwi_msg = ast_smdi_mwi_message_wait_station(smdi_iface, SMDI_MWI_WAIT_TIMEOUT, extension))) {
			ast_log_chan(NULL, AST_LOG_ERROR, "Error executing SMDI MWI change for %s\n", extension);
			if (!strncmp(mwi_msg->cause, "INV", 3))
				ast_log_chan(NULL, AST_LOG_ERROR, "Invalid MWI extension: %s\n", mwi_msg->fwd_st);
			else if (!strncmp(mwi_msg->cause, "BLK", 3))
				ast_log_chan(NULL, AST_LOG_WARNING, "MWI light was already on or off for %s\n", mwi_msg->fwd_st);
			ast_log_chan(NULL, AST_LOG_WARNING, "The switch reported '%s'\n", mwi_msg->cause);
			ao2_ref(mwi_msg, -1);
		} else {
	...
	if (!ast_strlen_zero(externnotify)) {
		if (inboxcount2(ext_context, &urgentvoicemails, &newvoicemails, &oldvoicemails)) {
			ast_log_chan(NULL, AST_LOG_ERROR, "Problem in calculating number of voicemail messages available for extension %s\n", extension);
		} else {
...
static int msg_create_from_file(struct ast_vm_recording_data *recdata)
{
	...
	if ((recording_fs = ast_readfile(recdata->recording_file, recdata->recording_ext, NULL, 0, 0, VOICEMAIL_DIR_MODE))) {
		if (!ast_seekstream(recording_fs, 0, SEEK_END)) {
			...
			if (sample_rate) {
				...
				ast_log_chan(NULL, LOG_ERROR,"Unable to determine sample rate of recording %s\n", recdata->recording_file);
			}
		}
	...
	if (duration < recipient->minsecs) {
		ast_log_chan(NULL, LOG_NOTICE, "Copying recording to voicemail %s@%s skipped because duration was shorter than "
	...
	if ((res = create_dirpath(tmpdir, sizeof(tmpdir), recipient->context, recdata->mailbox, "tmp"))) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to make directory.\n");
	}
	...
	if (txtdes < 0) {
		...
		ast_log_chan(NULL, AST_LOG_ERROR, "Unable to create message file: %s\n", strerror(errno));
		free_user(recipient);
		return -1;
	}
	...
	if (txt) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Error opening text file for output\n");
		if (ast_check_realtime("voicemail_data")) {
	...
	if (res < 0) {
		ast_log_chan(NULL, LOG_NOTICE, "Can not leave voicemail, unable to count messages\n");
		free_user(recipient);
		unlink(tmptxtfile);
		return -1;
	}
	 ...
	 */
		if (!(vms = create_vm_state_from_user(recipient))) {
			ast_log_chan(NULL, LOG_ERROR, "Couldn't allocate necessary space\n");
			free_user(recipient);
			unlink(tmptxtfile);
			return -1;
		}
	...
	if ((res = imap_check_limits(NULL, vms, recipient, msgnum))) {
		ast_log_chan(NULL, LOG_NOTICE, "Didn't copy to voicemail. Mailbox for %s@%s is full.\n", recipient->mailbox, recipient->context);
	...
	if (COUNT(recipient, dir) > recipient->maxmsg - inprocess_count(recipient->mailbox, recipient->context, +1)) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Didn't copy to voicemail. Mailbox for %s@%s is full.\n", recipient->mailbox, recipient->context);
		inprocess_count(recipient->mailbox, recipient->context, -1);
		free_user(recipient);
		unlink(tmptxtfile);
		return -1;
	...
	if (vm_lock_path(dir)) {
		ast_log_chan(NULL, LOG_ERROR, "Couldn't lock directory %s.  Voicemail will be lost.\n", dir);
		/* Delete files */
	...
	if (ast_filecopy(recdata->recording_file, tmpaudiofile, recdata->recording_ext)) {
		ast_log_chan(NULL, LOG_ERROR, "Audio file failed to copy to tmp dir. Probably low disk space.\n");
	...
	if (ast_filerename(tmpaudiofile, destination, recdata->recording_ext)) {
		ast_log_chan(NULL, LOG_ERROR, "Audio file failed to move to destination directory. Permissions/Overlap?\n");
		inprocess_count(recipient->mailbox, recipient->context, -1);
		ast_unlock_path(dir);
		free_user(recipient);
		unlink(tmptxtfile);
	...
	if (chmod(desttxtfile, VOICEMAIL_FILE_MODE) < 0) {
		ast_log_chan(NULL, AST_LOG_ERROR, "Couldn't set permissions on voicemail text file %s: %s", desttxtfile, strerror(errno));
	}
	...
	if (ast_fileexists(destination, NULL, NULL) > 0) {
		...
		if (chan) {
			...
			if (send_email) { /* We tried and failed. */
				ast_log_chan(NULL, LOG_WARNING, "Failed to allocate dummy channel, email will not be sent\n");
			}
...
static int leave_voicemail(struct ast_channel *chan, char *ext, struct leave_vm_options *options)
{
	...
	if (!tmp) {
		SCOPE_EXIT_RTN_VALUE(-1, "Failed to allocate memory for tmp\n");
	}
	...
	if (!(vmu = find_user(&svm, context, ext))) {
		pbx_builtin_setvar_helper(chan, "VMSTATUS", "FAILED");
		ast_free(tmp);
		SCOPE_EXIT_LOG_RTN_VALUE(res, LOG_WARNING,
			"%s: Exten: %s: No entry in voicemail config file for '%s'\n", ast_channel_name(chan), ext, ext);
	}
	...
	*/
	...
	SCOPE_CALL(-1, DISPOSE, tempfile, -1);
	...
	if (!ast_strlen_zero(prefile)) {
		...
		if (res < 0) {
			...
			pbx_builtin_setvar_helper(chan, "VMSTATUS", "FAILED");
			ast_free(tmp);
			SCOPE_EXIT_RTN_VALUE(-1, "Hang up during prefile playback\n");
		}
	}
	if (res == '#') {
	...
	transfer:
		...
		free_user(vmu);
		ast_free(tmp);
		SCOPE_EXIT_RTN_VALUE(OPERATOR_EXIT, "User escaped with '0'\n");
	}
	...
	if (res < 0) {
		...
		pbx_builtin_setvar_helper(chan, "VMSTATUS", "FAILED");
		ast_free(tmp);
		SCOPE_EXIT_RTN_VALUE(-1, "Other error '%d'\n", res);
	}
	...
	if (!ast_strlen_zero(fmt)) {
		...
		res = inboxcount(ext_context, &newmsgs, &oldmsgs);
		if (res < 0) {
			ast_log_chan(NULL, AST_LOG_NOTICE, "Can not leave voicemail, unable to count messages\n");
			free_user(vmu);
			ast_free(tmp);
			return -1;
		}
		 ...
		 */
			if (!(vms = create_vm_state_from_user(vmu))) {
				ast_log_chan(NULL, AST_LOG_ERROR, "Couldn't allocate necessary space\n");
				free_user(vmu);
				ast_free(tmp);
				return -1;
			}
...
#else
		if (COUNT(vmu, dir) >= vmu->maxmsg - inprocess_count(vmu->mailbox, vmu->context, +1)) {
			res = ast_streamfile(chan, "vm-mailboxfull", ast_channel_language(chan));
			if (!res)
				res = ast_waitstream(chan, "");
			ast_log_chan(NULL, AST_LOG_WARNING, "No more messages possible\n");
			pbx_builtin_setvar_helper(chan, "VMSTATUS", "FAILED");
			inprocess_count(vmu->mailbox, vmu->context, -1);
			goto leave_vm_out;
		}
		...
		if (txt) {
			...
			ast_log_chan(NULL, AST_LOG_WARNING, "Error opening text file for output\n");
			inprocess_count(vmu->mailbox, vmu->context, -1);
			if (ast_check_realtime("voicemail_data")) {
				ast_destroy_realtime("voicemail_data", "filename", tmptxtfile, SENTINEL);
			}
		...
		res = SCOPE_CALL_WITH_INT_RESULT(-1, play_record_review, chan, NULL, tmptxtfile, vmu->maxsecs, fmt, 1, vmu, &duration, &sound_duration, NULL, options->record_gain, vms, flag, msg_id, 0);
		...
		if (txt) {
			...
			if (sound_duration < vmu->minsecs) {
				fclose(txt);
				ast_verb_chan(NULL, 3, "Recording was %d seconds long but needs to be at least %d - abandoning\n", sound_duration, vmu->minsecs);
				ast_filedelete(tmptxtfile, NULL);
				unlink(tmptxtfile);
				if (ast_check_realtime("voicemail_data")) {
					ast_destroy_realtime("voicemail_data", "filename", tmptxtfile, SENTINEL);
				...
				fclose(txt);
				if (vm_lock_path(dir)) {
					ast_log_chan(NULL, AST_LOG_ERROR, "Couldn't lock directory %s.  Voicemail will be lost.\n", dir);
					/* Delete files */
					ast_filedelete(tmptxtfile, NULL);
					unlink(tmptxtfile);
					inprocess_count(vmu->mailbox, vmu->context, -1);
					...
					ast_trace(-1, "Renaming recordings '%s' -> fn '%s'\n", tmptxtfile, fn);
					/* ast_filerename renames the recordings but not the txt file */
					ast_filerename(tmptxtfile, fn, NULL);

					ast_trace(-1, "Renaming txt file '%s' -> fn '%s'\n", tmptxtfile, txtfile);
					...
					if (chmod(txtfile, VOICEMAIL_FILE_MODE) < 0)
						ast_log_chan(NULL, AST_LOG_ERROR, "Couldn't set permissions on voicemail text file %s: %s", txtfile, strerror(errno));
					...
					if (ast_fileexists(fn, NULL, NULL)) {
						SCOPE_CALL(-1, DISPOSE, dir, msgnum);
					}
				}
			}
		} else {
		...
		ast_log_chan(NULL, AST_LOG_WARNING, "No format for saving voicemail?\n");
leave_vm_out:
	free_user(vmu);
...
static int save_to_folder(struct ast_vm_user *vmu, struct vm_state *vms, int msg, int box, int *newmsg, int move)
{
	...
	if (vms->mailstream && !mail_status(vms->mailstream, mailbox, SA_UIDNEXT)) {
			...
			ast_log_chan(NULL, AST_LOG_NOTICE, "Folder %s created!\n", mbox(vmu, box));
		}
	}
	...
	int x, i;
	SCOPE_ENTER(3, "dir: %s msg: %d  box: %d  dbox: %s  move? %d  \n", dir, msg, box, dbox, move);

	create_dirpath(ddir, sizeof(ddir), context, username, dbox);
	ast_trace(-1, "ddir: %s\n", ddir);

	if (vm_lock_path(ddir)) {
		SCOPE_EXIT_RTN_VALUE(ERROR_LOCK_PATH, "Failed to lock path %s\n", ddir);
	}

	x = LAST_MSG_INDEX(ddir) + 1;

	if (box == 10 && x >= vmu->maxdeletedmsg) { /* "Deleted" folder*/
		ast_trace(-1, "Deleting message %d\n", msg);
		x--;
		for (i = 1; i <= x; i++) {
			...
			if (EXISTS(ddir, i, sfn, NULL)) {
				SCOPE_CALL(-1, RENAME, ddir, i, vmu->mailbox, vmu->context, ddir, i - 1, sfn, dfn);
			} else
				break;
		}
	} else {
		if (x >= vmu->maxmsg) {
			...
			SCOPE_EXIT_RTN_VALUE(ERROR_MAX_MSGS, "Max messages reached\n");
		}
	}
	make_file(sfn, sizeof(sfn), dir, msg);
	make_file(dfn, sizeof(dfn), ddir, x);
	if (strcmp(sfn, dfn)) {
		ast_trace(-1, "Copying message '%s' to '%s'\n", sfn, dfn);
		SCOPE_CALL(-1, COPY, dir, msg, ddir, x, username, context, sfn, dfn);
	}
	ast_unlock_path(ddir);
...
static void adsi_begin(struct ast_channel *chan, int *useadsi)
{
	...
	if (!x) {
		if (adsi_load_vmail(chan, useadsi)) {
			ast_log_chan(NULL, AST_LOG_WARNING, "Unable to upload voicemail scripts\n");
			return;
		}
	} else
		*useadsi = 1;
	...
	for (x = start; x < 5; x++) {	/* For all folders */
		...
		if (x == 0) {
			if (ast_fileexists(fn, NULL, NULL)) {
				...
				ast_verb_chan(NULL, 4, "Failed to find file %s; falling back to INBOX\n", fn);
...
static int notify_new_message(struct ast_channel *chan, struct ast_vm_user *vmu, struct vm_state *vms, int msgnum, long duration, char *fmt, char *cidnum, char *cidname, const char *flag)
{
	...
	if (!ast_strlen_zero(vmu->attachfmt)) {
		if (strstr(fmt, vmu->attachfmt))
			...
			ast_log_chan(NULL, AST_LOG_WARNING, "Attachment format '%s' is not one of the recorded formats '%s'.  Falling back to default format for '%s@%s'.\n", vmu->attachfmt, fmt, vmu->mailbox, vmu->context);
	}
	...
	while (!res && !valid_extensions) {
		...
		if (use_directory) {
			...
			if (directory_app) {
				...
				ast_log_chan(NULL, AST_LOG_WARNING, "Could not find the Directory application, disabling directory_forward\n");
		...
		while (s) {
			...
			if ((is_new_message == 1 || strcmp(s, sender->mailbox)) && (receiver = find_user(NULL, context, s))) {
				...
				if (inboxcount(mailbox_context, &newmsgs, &oldmsgs)) {
					ast_log_chan(NULL, LOG_ERROR, "Problem in calculating number of voicemail messages available for extension %s\n", mailbox_context);
				...
				if (dstvms->quota_limit && dstvms->quota_usage >= dstvms->quota_limit) {
					ast_log_chan(NULL, LOG_NOTICE, "Mailbox '%s' is exceeded quota %u >= %u\n", mailbox_context, dstvms->quota_usage, dstvms->quota_limit);
				...
				if ((newmsgs + oldmsgs) >= capacity) {
					ast_log_chan(NULL, LOG_NOTICE, "Mailbox '%s' is full with capacity of %d, prompting for another extension.\n", mailbox_context, capacity);
					res = ast_play_and_wait(chan, "vm-mailboxfull");
					valid_extensions = 0;
					while ((vmtmp = AST_LIST_REMOVE_HEAD(&extensions, list))) {
						inprocess_count(vmtmp->mailbox, vmtmp->context, -1);
				...
				ast_log_chan(NULL, LOG_NOTICE, "'%s' is not a valid mailbox\n", mailbox_context);
		...
		if (!cmd) {
			AST_LIST_TRAVERSE_SAFE_BEGIN(&extensions, vmtmp, list) {
				...
				if (dstvms) {
					...
					ast_log_chan(NULL, AST_LOG_ERROR, "Could not find state information for mailbox %s\n", vmtmp->mailbox);
				}
...
static int wait_file2(struct ast_channel *chan, struct vm_state *vms, char *file)
{
	...
	if ((res = ast_stream_and_wait(chan, file, AST_DIGIT_ANY)) < 0)
		ast_log_chan(NULL, AST_LOG_WARNING, "Unable to play message %s\n", file);
	return res;
}
...
static int play_message_category(struct ast_channel *chan, const char *category)
{
	...
	if (res) {
		ast_log_chan(NULL, AST_LOG_WARNING, "No sound file for category '%s' was found.\n", category);
		res = 0;
	}
...
static int play_message_datetime(struct ast_channel *chan, struct ast_vm_user *vmu, const char *origtime, const char *filename)
{
	...
	if (ast_get_time_t(origtime, &t, 0, NULL)) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Couldn't find origtime in %s\n", filename);
		return 0;
	}
		...
		if (i != MAX_NUM_CID_CONTEXTS){ /* internal context? */
			if (!res) {
				...
				if (!ast_strlen_zero(prefile)) {
					...
					if (ast_fileexists(prefile, NULL, NULL) > 0) {
						ast_verb_chan(NULL, 3, "Playing envelope info: CID number '%s' matches mailbox number, playing recorded name\n", callerid);
						if (!callback)
							res = wait_file2(chan, vms, "vm-from");
						res = ast_stream_and_wait(chan, prefile, "");
					} else {
						ast_verb_chan(NULL, 3, "Playing envelope info: message from '%s'\n", callerid);
			...
			if (!callback) {
				...
				if (!saycidnumber && ast_fileexists(prefile, NULL, NULL) > 0) {
					ast_verb_chan(NULL, 3, "Playing recorded name for CID number '%s' - '%s'\n", callerid,prefile);
					wait_file2(chan, vms, "vm-from");
					res = ast_stream_and_wait(chan, prefile, "");
					ast_verb_chan(NULL, 3, "Played recorded name result '%d'\n", res);
				} else {
...
static int play_message(struct ast_channel *chan, struct ast_vm_user *vmu, struct vm_state *vms)
{
	...
	msg_cfg = ast_config_load(filename, config_flags);
	if (!valid_config(msg_cfg)) {
		ast_log_chan(NULL, LOG_WARNING, "No message attribute file?!! (%s)\n", filename);
		return 0;
	}
	...
	if (!valid_config(msg_cfg)) {
		SCOPE_EXIT_LOG_RTN_VALUE(0, AST_LOG_WARNING, "No message attribute file?!! (%s)\n", filename);
	}

	if (!(origtime = ast_variable_retrieve(msg_cfg, "message", "origtime"))) {
		SCOPE_CALL(-1, DISPOSE, vms->curdir, vms->curmsg);
		ast_config_destroy(msg_cfg);
		SCOPE_EXIT_LOG_RTN_VALUE(0, AST_LOG_WARNING, "No origtime?!\n");
	}
	...
	if (!res) {
		...
		if ((res = wait_file(chan, vms, vms->fn)) < 0) {
			ast_log_chan(NULL, AST_LOG_WARNING, "Playback of message %s failed\n", vms->fn);
			res = 0;
		}
		ast_test_suite_event_notify("USERPRESS", "Message: User pressed %c\r\nDTMF: %c",
			isprint(res) ? res : '?', isprint(res) ? res : '?');
...
static int imap_delete_old_greeting (char *dir, struct vm_state *vms)
{
	...
	if (file) {
		*file++ = '\0';
		...
		ast_log_chan(NULL, AST_LOG_ERROR, "Failed to procure file name from directory passed. You should never see this.\n");
		return -1;
	}
...
static int open_mailbox(struct vm_state *vms, struct ast_vm_user *vmu, int box)
{
	...
	SCOPE_ENTER(3, "user: %s dir: %s msg: %d box %d\n",
		vms->username, vms->curdir, vms->curmsg, box);
	...
	if (count_msg < 0) {
		SCOPE_EXIT_RTN_VALUE(count_msg, "msgs: %d\n", count_msg);
	} else {
		vms->lastmsg = count_msg - 1;
	}

	if (vm_allocate_dh(vms, vmu, count_msg)) {
		SCOPE_EXIT_RTN_VALUE(-1, "failed to allocate dh\n");
	}
	...
	*/
	...
	if (vm_lock_path(vms->curdir)) {
		SCOPE_EXIT_LOG_RTN_VALUE(ERROR_LOCK_PATH, AST_LOG_ERROR, "Could not open mailbox %s:  mailbox is locked\n", vms->curdir);
	}

	/* for local storage, checks directory for messages up to MAXMSGLIMIT */
	last_msg = LAST_MSG_INDEX(vms->curdir);
	ast_unlock_path(vms->curdir);

	if (last_msg < -1) {
		SCOPE_EXIT_RTN_VALUE(last_msg, "last msg: %d\n", last_msg);
	} else if (vms->lastmsg != last_msg) {
		ast_log_chan(NULL, LOG_NOTICE, "Resequencing Mailbox: %s, expected %d but found %d message(s) in box with max threshold of %d.\n", vms->curdir, last_msg + 1, vms->lastmsg + 1, vmu->maxmsg);
...
static int close_mailbox(struct vm_state *vms, struct ast_vm_user *vmu)
{
	...
	char fn2[PATH_MAX];
#endif
	SCOPE_ENTER(3, "user: %s dir: %s msg: %d\n",
		vms->username, vms->curdir, vms->curmsg);

	if (vms->lastmsg <= -1) {
		ast_trace(-1, "No messages in mailbox\n");
		goto done;
	}
	...
	if (vm_lock_path(vms->curdir)) {
		SCOPE_EXIT_RTN_VALUE(ERROR_LOCK_PATH, "Could not open mailbox %s:  mailbox is locked\n", vms->curdir);
	}

	/* update count as message may have arrived while we've got mailbox open */
	last_msg_idx = LAST_MSG_INDEX(vms->curdir);
	if (last_msg_idx != vms->lastmsg) {
		ast_log_chan(NULL, AST_LOG_NOTICE, "%d messages received after mailbox opened.\n", last_msg_idx - vms->lastmsg);
	}
	...
	for (x = 0; x < last_msg_idx + 1; x++) {
		if (!vms->deleted[x] && ((strcasecmp(vms->curbox, "INBOX") && strcasecmp(vms->curbox, "Urgent")) || !vms->heard[x] || (vms->heard[x] && !ast_test_flag(vmu, VM_MOVEHEARD)))) {
			...
			if (strcmp(vms->fn, fn2)) {
				SCOPE_CALL(-1, RENAME, vms->curdir, x, vmu->mailbox, vmu->context, vms->curdir, vms->curmsg, vms->fn, fn2);
			}
			...
			res = SCOPE_CALL_WITH_INT_RESULT(-1, save_to_folder, vmu, vms, x, 1, NULL, 0);
			if (res == ERROR_LOCK_PATH || res == ERROR_MAX_MSGS) {
				/* If save failed do not delete the message */
				ast_log_chan(NULL, AST_LOG_WARNING, "Save failed.  Not moving message: %s.\n", res == ERROR_LOCK_PATH ? "unable to lock path" : "destination folder full");
				vms->deleted[x] = 0;
				vms->heard[x] = 0;
				--x;
			}
			...
			res = SCOPE_CALL_WITH_INT_RESULT(-1, save_to_folder, vmu, vms, x, 10, NULL, 0);
			if (res == ERROR_LOCK_PATH) {
				ast_trace(-1, "Unable to lock path.  Not moving message to deleted folder.\n");
				/* If save failed do not delete the message */
				vms->deleted[x] = 0;
				vms->heard[x] = 0;
				--x;
			...
			make_file(vms->fn, sizeof(vms->fn), vms->curdir, x);
			res = SCOPE_CALL_WITH_INT_RESULT(-1, EXISTS, vms->curdir, x, vms->fn, NULL);
			if (res) {
				SCOPE_CALL(-1, DELETE, vms->curdir, x, vms->fn, vmu);
			}
		}
	}
...
static int vm_intro(struct ast_channel *chan, struct ast_vm_user *vmu, struct vm_state *vms)
{
	...
	if (0) {
		...
		if (deprecation_warning++ % 10 == 0) {
			ast_log_chan(NULL, LOG_WARNING, "cz is not a standard language code.  Please switch to using cs instead.\n");
		}
		return vm_intro_cs(chan, vms);
	} else if (!strncasecmp(ast_channel_language(chan), "de", 2)) {  /* GERMAN syntax */
...
static int vm_newuser_setup(struct ast_channel *chan, struct ast_vm_user *vmu, struct vm_state *vms, char *fmtc, signed char record_gain)
{
	...
	for (;;) {
		...
		if (cmd != 0) {
			ast_log_chan(NULL, AST_LOG_NOTICE, "Invalid password for user %s (%s)\n", vms->username, newpassword);
			...
			if (!strcmp(newpassword, newpassword2))
				break;
			ast_log_chan(NULL, AST_LOG_NOTICE, "Password mismatch for user %s (%s != %s)\n", vms->username, newpassword, newpassword2);
...
static int vm_options(struct ast_channel *chan, struct ast_vm_user *vmu, struct vm_state *vms, char *fmtc, signed char record_gain)
{
	...
	unsigned char buf[256];
	int bytes = 0;
	SCOPE_ENTER(3, "%s: %s entering mailbox options", ast_channel_name(chan), vms->username);
			...
			int skipuser, int max_logins, int silent)
{
	...
	if (!silent && !skipuser && ast_streamfile(chan, vm_login, ast_channel_language(chan))) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Couldn't stream login file\n");
		return -1;
	}
	...
	while (!valid && (logretries < max_logins)) {
		...
		if (!skipuser && ast_readstring(chan, mailbox, mailbox_size - 1, 2000, 10000, "#") < 0) {
			ast_log_chan(NULL, AST_LOG_WARNING, "Couldn't read username\n");
			return -1;
		}
		if (ast_strlen_zero(mailbox)) {
			if (ast_channel_caller(chan)->id.number.valid && ast_channel_caller(chan)->id.number.str) {
				...
				ast_verb_chan(NULL, 3, "Username not entered\n");
				return -1;
			}
		} else if (mailbox[0] == '*') {
			/* user entered '*' */
			ast_verb_chan(NULL, 4, "Mailbox begins with '*', attempting jump to extension 'a'\n");
			if (ast_exists_extension(chan, ast_channel_context(chan), "a", 1,
				S_COR(ast_channel_caller(chan)->id.number.valid, ast_channel_caller(chan)->id.number.str, NULL))) {
				return -1;
			}
			ast_verb_chan(NULL, 4, "Jump to extension 'a' failed; setting mailbox to NULL\n");
			mailbox[0] = '\0';
		}
		...
		if (vmu && (vmu->password[0] == '\0' || (vmu->password[0] == '-' && vmu->password[1] == '\0'))) {
			...
			if (ast_streamfile(chan, vm_password, ast_channel_language(chan))) {
				if (!ast_check_hangup(chan)) {
					ast_log_chan(NULL, AST_LOG_WARNING, "Unable to stream password file\n");
				}
				free_user(vmu);
				return -1;
			}
			if (ast_readstring(chan, password, sizeof(password) - 1, 2000, 10000, "#") < 0) {
				ast_log_chan(NULL, AST_LOG_NOTICE, "Unable to read password\n");
				free_user(vmu);
				return -1;
			} else if (password[0] == '*') {
				/* user entered '*' */
				ast_verb_chan(NULL, 4, "Password begins with '*', attempting jump to extension 'a'\n");
				if (ast_exists_extension(chan, ast_channel_context(chan), "a", 1,
					S_COR(ast_channel_caller(chan)->id.number.valid, ast_channel_caller(chan)->id.number.str, NULL))) {
					mailbox[0] = '*';
					free_user(vmu);
				...
				ast_verb_chan(NULL, 4, "Jump to extension 'a' failed; setting mailbox and user to NULL\n");
				mailbox[0] = '\0';
		...
		if (vmu && !strcmp(passptr, password))
			...
			ast_verb_chan(NULL, 3, "Incorrect password '%s' for user '%s' (context = %s)\n", password, mailbox, context ? context : "default");
			if (!ast_strlen_zero(prefix))
				mailbox[0] = '\0';
		}
		logretries++;
		if (!valid) {
			if (skipuser || logretries >= max_logins) {
				if (ast_streamfile(chan, "vm-incorrect", ast_channel_language(chan))) {
					ast_log_chan(NULL, AST_LOG_WARNING, "Unable to stream incorrect message\n");
					free_user(vmu);
					return -1;
				}
				...
				if (ast_streamfile(chan, "vm-incorrect-mailbox", ast_channel_language(chan))) {
					ast_log_chan(NULL, AST_LOG_WARNING, "Unable to stream incorrect mailbox message\n");
					free_user(vmu);
					return -1;
				}
			}
...
static int play_message_by_id(struct ast_channel *chan, const char *mailbox, const char *context, const char *msg_id)
{
	...
	for (i = 0; i < ARRAY_LEN(mailbox_folders) && !played; i++) {
		...
		if ((res = open_mailbox(&vms, vmu, i)) < 0) {
			ast_log_chan(NULL, LOG_WARNING, "Could not open mailbox %s\n", mailbox);
			res = -1;
			goto play_msg_cleanup;
		}
		open = 1;
...
static int vm_execmain(struct ast_channel *chan, const char *data)
{
	...
	if (!ast_strlen_zero(data)) {
		...
		if (args.argc == 2) {
			if (ast_app_parse_options(vm_app_options, &flags, opts, args.argv1)) {
				SCOPE_EXIT_LOG_RTN_VALUE(-1, AST_LOG_WARNING, "Invalid option string '%s'\n", args.argv1);
			}
			if (ast_test_flag(&flags, OPT_RECORDGAIN)) {
				int gain;
				if (!ast_strlen_zero(opts[OPT_ARG_RECORDGAIN])) {
					if (sscanf(opts[OPT_ARG_RECORDGAIN], "%30d", &gain) != 1) {
						SCOPE_EXIT_LOG_RTN_VALUE(-1, AST_LOG_WARNING, "Invalid value '%s' provided for record gain option\n", opts[OPT_ARG_RECORDGAIN]);
					} else {
						record_gain = (signed char) gain;
					}
				} else {
					ast_log_chan(NULL, AST_LOG_WARNING, "Invalid Gain level set with option g\n");
				}
			}
			if (ast_test_flag(&flags, OPT_AUTOPLAY) ) {
				...
				if (!ast_strlen_zero(opts[OPT_ARG_PLAYFOLDER])) {
					...
					ast_log_chan(NULL, AST_LOG_WARNING, "Invalid folder set with option a\n");
				}
				if (play_folder > 9 || play_folder < 0) {
					ast_log_chan(NULL, AST_LOG_WARNING,
		...
		if (!ast_strlen_zero(vms.username)) {
			if ((vmu = find_user(&vmus, context ,vms.username))) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Mailbox '%s%s%s' doesn't exist\n", vms.username, context ? "@": "", context ? context : "");
				valid = 0;
			}
		} else {
			valid = 0;
	...
	if (vms.username[0] == '*') {
		ast_trace(-1, "user pressed * in context '%s'\n", ast_channel_context(chan));
	...
	ast_trace(-1, "Before open_mailbox\n");
	res = SCOPE_CALL_WITH_INT_RESULT(-1, open_mailbox, &vms, vmu, OLD_FOLDER); /* Count all messages, even Urgent */
	if (res < 0) {
		ast_trace(-1, "open mailbox: %d\n", res);
		goto out;
	}
	vms.oldmessages = vms.lastmsg + 1;
	ast_trace(-1, "Number of old messages: %d\n", vms.oldmessages);
	/* check INBOX */
	...
	ast_trace(-1, "Number of new messages: %d\n", vms.newmessages);
	/* Start in Urgent */
	in_urgent = 1;
	res = SCOPE_CALL_WITH_INT_RESULT(-1, open_mailbox, &vms, vmu, 11); /*11 is the Urgent folder */
	if (res < 0) {
		ast_trace(-1, "open mailbox: %d\n", res);
		goto out;
	}
	vms.urgentmessages = vms.lastmsg + 1;
	ast_trace(-1, "Number of urgent messages: %d\n", vms.urgentmessages);
	...
	if (play_auto) {
		...
		if (vms.urgentmessages) {
			in_urgent = 1;
			res = SCOPE_CALL_WITH_INT_RESULT(-1, open_mailbox, &vms, vmu, 11);
		} else {
			in_urgent = 0;
			res = SCOPE_CALL_WITH_INT_RESULT(-1, open_mailbox, &vms, vmu, play_folder);
		}
		if (res < 0) {
			ast_trace(-1, "open mailbox: %d\n", res);
			goto out;
	...
	if (!strcasecmp(vmu->mailbox, vmu->password) &&
		...
		if (ast_play_and_wait(chan, vm_newuser) == -1)
			ast_log_chan(NULL, AST_LOG_WARNING, "Couldn't stream new user file\n");
		...
		if ((vms.newmessages + vms.oldmessages) >= vmu->maxmsg) {
			ast_log_chan(NULL, AST_LOG_WARNING, "No more messages possible.  User has %d messages and limit is %d.\n", (vms.newmessages + vms.oldmessages), vmu->maxmsg);
			cmd = ast_play_and_wait(chan, "vm-mailboxfull");
		}
#endif
	...
	while ((cmd > -1) && (cmd != 't') && (cmd != '#')) {
		...
		case '2': /* Change folders */
			...
			if (cmd == '#') {
				...
				res = SCOPE_CALL_WITH_INT_RESULT(-1, open_mailbox, &vms, vmu, cmd);
				if (res < 0) {
					ast_trace(-1, "open mailbox: %d\n", res);
					goto out;
				}
		...
		case '3': /* Advanced options */
			...
			while ((cmd > -1) && (cmd != 't') && (cmd != '#')) {
				...
				case '2': /* Callback */
					if (!vms.starting)
						ast_verb_chan(NULL, 3, "Callback Requested\n");
			...
			if (vms.curmsg > 0) {
				...
				*/
				if (in_urgent == 0 && vms.urgentmessages > 0) {
					...
					res = SCOPE_CALL_WITH_INT_RESULT(-1, open_mailbox, &vms, vmu, 11);  /* Open Urgent folder */
					if (res < 0) {
						ast_trace(-1, "open mailbox: %d\n", res);
						goto out;
					}
...
out:
	...
	if (vmu) {
		SCOPE_CALL(-1, close_mailbox, &vms, vmu);
	}
	if (valid) {
...
static int vm_exec(struct ast_channel *chan, const char *data)
{
	...
	if (!ast_strlen_zero(data)) {
		...
		if (args.argc == 2) {
			if (ast_app_parse_options(vm_app_options, &flags, opts, args.argv1)) {
				SCOPE_EXIT_RTN_VALUE(-1, "parse options failed for '%s'\n", args.argv1);
			}
		...
		if (res < 0) {
			SCOPE_EXIT_RTN_VALUE(res, "getdata failed.  RC: %d", res);
		}
	...
	res = SCOPE_CALL_WITH_INT_RESULT(-1, leave_voicemail, chan, args.argv0, &leave_options);
	if (res == 't') {
		ast_play_and_wait(chan, "vm-goodbye");
		res = 0;
	}
	...
	if (res == ERROR_LOCK_PATH) {
		ast_log_chan(NULL, AST_LOG_ERROR, "Could not leave voicemail. The path is already locked.\n");
		pbx_builtin_setvar_helper(chan, "VMSTATUS", "FAILED");
		res = 0;
	}

	SCOPE_EXIT_RTN_VALUE(res, "Done.  RC: %d", res);
}
...
static int add_message_id(struct ast_config *msg_cfg, char *dir, int msg, char *filename, char *id, size_t id_size, struct ast_vm_user *vmu, int folder)
{
	...
	if (!cat) {
		ast_log_chan(NULL, LOG_ERROR, "Voicemail data file %s/%d.txt has no [message] category?\n", dir, msg);
		ast_variables_destroy(var);
		return -1;
	}
	...
	if (ast_config_text_file_save(filename, msg_cfg, "app_voicemail")) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to update %s to have a message ID\n", filename);
		return -1;
	}
...
static struct ast_vm_user *find_or_create(const char *context, const char *box)
{
	...
	if (!ast_strlen_zero(box) && box[0] == '*') {
		ast_log_chan(NULL, LOG_WARNING, "Mailbox %s in context %s begins with '*' character.  The '*' character,"
	...
	AST_LIST_TRAVERSE(&users, vmu, list) {
		if (ast_test_flag((&globalflags), VM_SEARCH) && !strcasecmp(box, vmu->mailbox)) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Ignoring duplicated mailbox %s\n", box);
			return NULL;
		}
		if (!strcasecmp(context, vmu->context) && !strcasecmp(box, vmu->mailbox)) {
			ast_log_chan(NULL, LOG_WARNING, "Ignoring duplicated mailbox %s in context %s\n", box, context);
			return NULL;
		}
	}
...
static int acf_vm_info(struct ast_channel *chan, const char *cmd, char *args, char *buf, size_t len)
{
	...
	if (ast_strlen_zero(args)) {
		ast_log_chan(NULL, LOG_ERROR, "VM_INFO requires an argument (<mailbox>[@<context>],attribute[,folder])\n");
		return -1;
	}
	...
	if (vmu) {
		if (!strncasecmp(arg.attribute, "password", 8)) {
			...
			if (res < 0) {
				ast_log_chan(NULL, LOG_ERROR, "Unable to retrieve message count for mailbox %s\n", arg.mailbox_context);
				free_user(vmu);
				return -1;
			}
			snprintf(buf, len, "%d", res);
		} else {
			ast_log_chan(NULL, LOG_ERROR, "Unknown attribute '%s' for VM_INFO\n", arg.attribute);
			free_user(vmu);
			return -1;
		}
		free_user(vmu);
...
static void start_poll_thread(void)
{
	...
	if ((errcode = ast_pthread_create(&poll_thread, NULL, mb_poll_thread, NULL))) {
		ast_log_chan(NULL, LOG_ERROR, "Could not create thread: %s\n", strerror(errcode));
	}
}
		...
		)
{
	...
	if((s == NULL) || (vmu == NULL) || (event_name == NULL) || (actionid == NULL)) {
		ast_log_chan(NULL, LOG_ERROR, "Wrong input parameter.");
		return 0;
	}
	...
	if (ret == -1) {
		ast_log_chan(NULL, LOG_ERROR, "Could not create mailbox string. err[%s]\n", strerror(errno));
		return 0;
	}
	...
	ast_free(mailbox);
	if (ret == -1) {
		ast_log_chan(NULL, LOG_ERROR, "Could not get mailbox count. user[%s], context[%s]\n",
		...
		const char* actionid)
{
	...
	if (!mailbox_snapshot) {
		ast_log_chan(NULL, LOG_ERROR, "Could not append voicemail box info for box %s@%s.",
			vmu->mailbox, vmu->context);
		return 0;
	}
...
static int manager_status_voicemail_user(struct mansession *s, const struct message *m)
{
	...
	free_user(vmu);
	if(ret == 0) {
		ast_log_chan(NULL, LOG_ERROR, "Could not append voicemail user info.");
	}
...
static const char *substitute_escapes(const char *value)
{
	...
	for (current = (char *) value; *current; current++) {
		if (*current == '\\') {
			current++;
			if (!*current) {
				ast_log_chan(NULL, AST_LOG_NOTICE, "Incomplete escape at end of value.\n");
				break;
			}
			switch (*current) {
			case '\\':
			...
			case 't':
				...
				break;
			default:
				ast_log_chan(NULL, AST_LOG_NOTICE, "Substitution routine does not support this character: \\%c\n", *current);
				break;
			}
		} else {
...
static int load_config_force(int reload, int force)
{
	...
	if ((cfg = ast_config_load(VOICEMAIL_CONFIG, config_flags)) == CONFIG_STATUS_FILEUNCHANGED) {
		if ((ucfg = ast_config_load("users.conf", config_flags)) == CONFIG_STATUS_FILEUNCHANGED) {
			...
			ast_log_chan(NULL, LOG_ERROR, "Config file users.conf is in an invalid format.  Avoiding.\n");
			ucfg = NULL;
		}
		ast_clear_flag(&config_flags, CONFIG_FLAG_FILEUNCHANGED);
		if ((cfg = ast_config_load(VOICEMAIL_CONFIG, config_flags)) == CONFIG_STATUS_FILEINVALID) {
			ast_config_destroy(ucfg);
			ast_log_chan(NULL, LOG_ERROR, "Config file " VOICEMAIL_CONFIG " is in an invalid format.  Aborting.\n");
			return 0;
		}
	...
	while (var) {
		if (var->value) {
			...
			if (msg_format) {
				...
				ast_log_chan(NULL, AST_LOG_WARNING, "Invalid timezone definition at line %d\n", var->lineno);
				ast_free(z);
			}
		}
		var = var->next;
...
static int actual_load_config(int reload, struct ast_config *cfg, struct ast_config *ucfg)
{
	...
	if (cfg) {
		...
		odbc_table_len = strlen(odbc_table);

		ast_set2_flag((&globalflags), 0, VM_ODBC_AUDIO_ON_DISK);
		if (!(val = ast_variable_retrieve(cfg, "general", "odbc_audio_on_disk")))
			val = "no";
		...
		if (!(val = ast_variable_retrieve(cfg, "general", "maxmsg"))) {
			...
			maxmsg = atoi(val);
			if (maxmsg < 0) {
				ast_log_chan(NULL, AST_LOG_WARNING, "Invalid number of messages per folder '%s'. Using default value %i\n", val, MAXMSG);
				maxmsg = MAXMSG;
			} else if (maxmsg > MAXMSGLIMIT) {
				ast_log_chan(NULL, AST_LOG_WARNING, "Maximum number of messages per folder is %i. Cannot accept value '%s'\n", MAXMSGLIMIT, val);
				maxmsg = MAXMSGLIMIT;
			}
		}
		...
		if (!(val = ast_variable_retrieve(cfg, "general", "backupdeleted"))) {
			...
			if (maxdeletedmsg < 0) {
				ast_log_chan(NULL, AST_LOG_WARNING, "Invalid number of deleted messages saved per mailbox '%s'. Using default value %i\n", val, MAXMSG);
				maxdeletedmsg = MAXMSG;
			} else if (maxdeletedmsg > MAXMSGLIMIT) {
				ast_log_chan(NULL, AST_LOG_WARNING, "Maximum number of deleted messages saved per mailbox is %i. Cannot accept value '%s'\n", MAXMSGLIMIT, val);
		...
		if ((val = ast_variable_retrieve(cfg, "general", "smdienable")) && ast_true(val)) {
			...
			if (!smdi_iface) {
				ast_log_chan(NULL, AST_LOG_ERROR, "No valid SMDI interface specfied, disabling SMDI voicemail notification\n");
			}
		}
		...
		if ((val = ast_variable_retrieve(cfg, "general", "maxsecs"))) {
			if (sscanf(val, "%30d", &x) == 1) {
				...
				ast_log_chan(NULL, AST_LOG_WARNING, "Invalid max message time length\n");
			}
			...
			if (maxmessage_deprecate == 0) {
				...
				ast_log_chan(NULL, AST_LOG_WARNING, "Setting 'maxmessage' has been deprecated in favor of 'maxsecs'.\n");
			}
			if (sscanf(val, "%30d", &x) == 1) {
				vmmaxsecs = x;
			} else {
		...
		if ((val = ast_variable_retrieve(cfg, "general", "minsecs"))) {
			if (sscanf(val, "%30d", &x) == 1) {
				...
				if (maxsilence / 1000 >= vmminsecs) {
					ast_log_chan(NULL, AST_LOG_WARNING, "maxsilence should be less than minsecs or you may get empty messages\n");
				}
			} else {
				ast_log_chan(NULL, AST_LOG_WARNING, "Invalid min message time length\n");
			}
		} else if ((val = ast_variable_retrieve(cfg, "general", "minmessage"))) {
			static int maxmessage_deprecate = 0;
			if (maxmessage_deprecate == 0) {
				maxmessage_deprecate = 1;
				ast_log_chan(NULL, AST_LOG_WARNING, "Setting 'minmessage' has been deprecated in favor of 'minsecs'.\n");
			}
			if (sscanf(val, "%30d", &x) == 1) {
				vmminsecs = x;
				if (maxsilence / 1000 >= vmminsecs) {
					ast_log_chan(NULL, AST_LOG_WARNING, "maxsilence should be less than minmessage or you may get empty messages\n");
				}
			} else {
				ast_log_chan(NULL, AST_LOG_WARNING, "Invalid min message time length\n");
			}
		...
		if (!val) {
			...
			if (!val) {
				ast_log_chan(NULL, LOG_ERROR, "Error processing format string, defaulting to format 'wav'\n");
				val = "wav";
			}
		}
		...
		if ((val = ast_variable_retrieve(cfg, "general", "maxgreet"))) {
			if (sscanf(val, "%30d", &x) == 1) {
				...
				ast_log_chan(NULL, AST_LOG_WARNING, "Invalid max message greeting length\n");
			}
		}
		...
		if ((val = ast_variable_retrieve(cfg, "general", "skipms"))) {
			if (sscanf(val, "%30d", &x) == 1) {
				...
				ast_log_chan(NULL, AST_LOG_WARNING, "Invalid skipms value\n");
			}
		}
		...
		if ((val = ast_variable_retrieve(cfg, "general", "maxlogins"))) {
			if (sscanf(val, "%30d", &x) == 1) {
				...
				ast_log_chan(NULL, AST_LOG_WARNING, "Invalid max failed login attempts\n");
			}
		}
		...
		if ((val = ast_variable_retrieve(cfg, "general", "minpassword"))) {
			if (sscanf(val, "%30d", &x) == 1) {
				...
				ast_log_chan(NULL, AST_LOG_WARNING, "Invalid minimum password length.  Default to %d\n", minpassword);
			}
		}
		...
		if ((val = ast_variable_retrieve(cfg, "general", "pollfreq"))) {
			if (sscanf(val, "%30u", &poll_freq) != 1) {
				poll_freq = DEFAULT_POLL_FREQ;
				ast_log_chan(NULL, AST_LOG_ERROR, "'%s' is not a valid value for the pollfreq option!\n", val);
			}
		}
		...
		if ((val = ast_variable_retrieve(cfg, "general", "tps_queue_high"))) {
			if (sscanf(val, "%30ld", &tps_queue_high) != 1 || tps_queue_high <= 0) {
				ast_log_chan(NULL, AST_LOG_WARNING, "Invalid the taskprocessor high water alert trigger level '%s'\n", val);
				tps_queue_high = AST_TASKPROCESSOR_HIGH_WATER_LEVEL;
			}
		}
		tps_queue_low = -1;
		if ((val = ast_variable_retrieve(cfg, "general", "tps_queue_low"))) {
			if (sscanf(val, "%30ld", &tps_queue_low) != 1 ||
				...
				ast_log_chan(NULL, AST_LOG_WARNING, "Invalid the taskprocessor low water clear alert level '%s'\n", val);
				tps_queue_low = -1;
			}
		}
		if (ast_taskprocessor_alert_set_levels(mwi_subscription_tps, tps_queue_low, tps_queue_high)) {
			ast_log_chan(NULL, AST_LOG_WARNING, "Failed to set alert levels for voicemail taskprocessor.\n");
		}
		...
		AST_LIST_UNLOCK(&users);
		ast_log_chan(NULL, AST_LOG_WARNING, "Failed to load configuration file.\n");
		return 0;
	}
}
...
static void read_password_from_file(const char *secretfn, char *password, int passwordlen) {
	...
	ast_log_chan(NULL, LOG_NOTICE, "Failed reading voicemail password from %s, using secret from config file\n", secretfn);
}
...
static int write_password_to_file(const char *secretfn, const char *password) {
	...
	if (!(conf = ast_config_new())) {
		ast_log_chan(NULL, LOG_ERROR, "Error creating new config structure\n");
		return res;
	}
	if (!(cat = ast_category_new("general", "", 1))) {
		ast_log_chan(NULL, LOG_ERROR, "Error creating new category structure\n");
		ast_config_destroy(conf);
		return res;
	}
	if (!(var = ast_variable_new("password", password, ""))) {
		ast_log_chan(NULL, LOG_ERROR, "Error creating new variable structure\n");
		ast_config_destroy(conf);
	...
	if (!ast_config_text_file_save(secretfn, conf, "app_voicemail")) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Error writing voicemail password to %s\n", secretfn);
	}
...
static int vmsayname_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(data)
		...
		ast_log_chan(NULL, LOG_WARNING, "VMSayName requires argument mailbox@context\n");
		return -1;
	}
	...
	if (!(res = vmsayname_exec(test_channel1, dir))) {
		...
		if (ast_fileexists(dir, NULL, NULL)) {
			...
			if ((res = create_dirpath(dir, sizeof(dir), TEST_CONTEXT, TEST_EXTENSION, ""))) {
				ast_log_chan(NULL, AST_LOG_WARNING, "Failed to make test directory\n");
				goto exit_vmsayname_test;
			}
			...
			if ((res = symlink(dir, dir2))) {
				ast_log_chan(NULL, LOG_WARNING, "Symlink reported %s\n", strerror(errno));
				goto exit_vmsayname_test;
			}
...
AST_TEST_DEFINE(test_voicemail_load_config)
{
	...
	case TEST_INIT:
		...
		info->description =
			...
			"This is to test regressions of GABPBX-18838 where it was noticed that "
 ...
 * has another issue not fatal to GABpbx itself, return AST_MODULE_LOAD_DECLINE.
 *
...
static int load_module(void)
{
	...
	if (!alias_mailbox_mappings) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to create alias_mailbox_mappings container\n");
	...
	if (res) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to register alias_mailbox_mappings container\n");
		ao2_cleanup(inprocess_container);
		ao2_cleanup(alias_mailbox_mappings);
		return AST_MODULE_LOAD_DECLINE;
	}
	...
	if (!mailbox_alias_mappings) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to create mailbox_alias_mappings container\n");
		ao2_cleanup(inprocess_container);
		ao2_container_unregister("voicemail_alias_mailbox_mappings");
		ao2_cleanup(alias_mailbox_mappings);
		return AST_MODULE_LOAD_DECLINE;
	...
	if (res) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to register mailbox_alias_mappings container\n");
		ao2_cleanup(inprocess_container);
		ao2_container_unregister("voicemail_alias_mailbox_mappings");
		ao2_cleanup(alias_mailbox_mappings);
		ao2_cleanup(mailbox_alias_mappings);
	...
	if (!(mwi_subscription_tps = ast_taskprocessor_get("app_voicemail", 0))) {
		ast_log_chan(NULL, AST_LOG_WARNING, "failed to reference mwi subscription taskprocessor.  MWI will not work\n");
	}
	...
	if (res) {
		ast_log_chan(NULL, LOG_ERROR, "Failure registering applications, functions or tests\n");
		unload_module();
		return AST_MODULE_LOAD_DECLINE;
	}
	...
	if (res) {
		ast_log_chan(NULL, LOG_ERROR, "Failure registering as a voicemail provider\n");
		unload_module();
		return AST_MODULE_LOAD_DECLINE;
	}
	...
	if (res) {
		ast_log_chan(NULL, LOG_ERROR, "Failure registering as a greeter provider\n");
		unload_module();
		return AST_MODULE_LOAD_DECLINE;
	}
...
static int dialout(struct ast_channel *chan, struct ast_vm_user *vmu, char *num, char *outgoing_context)
{
	...
	if (!num) {
		ast_verb_chan(NULL, 3, "Destination number will be entered manually\n");
		while (retries < 3 && cmd != 't') {
			...
			if (!cmd) {
				...
				if (cmd < 0)
					return 0;
				if (cmd == '*') {
					ast_verb_chan(NULL, 3, "User hit '*' to cancel outgoing call\n");
					return 0;
				}
		...
		ast_verb_chan(NULL, 3, "Destination number is CID number '%s'\n", num);
	...
	if (!ast_strlen_zero(destination)) {
		if (destination[strlen(destination) -1 ] == '*')
			return 0;
		ast_verb_chan(NULL, 3, "Placing outgoing call to extension '%s' in context '%s' from context '%s'\n", destination, outgoing_context, ast_channel_context(chan));
...
static int advanced_options(struct ast_channel *chan, struct ast_vm_user *vmu, struct vm_state *vms, int msg, int option, signed char record_gain)
{
	...
	DISPOSE(vms->curdir, vms->curmsg);
	if (!valid_config(msg_cfg)) {
		ast_log_chan(NULL, AST_LOG_WARNING, "No message attribute file?!! (%s)\n", filename);
		return 0;
	}
		...
		while ((res > -1) && (res != 't')) {
			...
			case '2':
				...
				if (!ast_strlen_zero(vmu->dialout)) {
					...
					ast_verb_chan(NULL, 3, "Caller can not specify callback number - no dialout context available\n");
			...
			default:
				if (num) {
					ast_verb_chan(NULL, 3, "Confirm CID number '%s' is number to use for callback\n", num);
	...
	case 1:	/* Reply */
		...
		ast_callerid_parse(cid, &name, &num);
		if (!num) {
			ast_verb_chan(NULL, 3, "No CID number available, no reply sent\n");
			if (!res)
				res = ast_play_and_wait(chan, "vm-nonumber");
			ast_config_destroy(msg_cfg);
			return res;
			...
			if (vmu3) {
				...
				ast_verb_chan(NULL, 3, "Leaving voicemail for '%s' in context '%s'\n", num, vmu->context);
				...
				ast_verb_chan(NULL, 3, "No mailbox number '%s' in context '%s', no reply sent\n", num, vmu->context);
			...
			signed char record_gain, struct vm_state *vms, char *flag, const char *msg_id, int forwardintro)
{
	...
	char *canceldtmf = "";
	int canceleddtmf = 0;
	SCOPE_ENTER(3, "%s: rf: %s fmt: %s type: %s vmu: %s\n",
		ast_channel_name(chan), recordfile, fmt, outsidecaller ? "msg" : "greeting",
			vmu->mailbox);
	/* Note that urgent and private are for flagging messages as such in the future */

	/* barf if no pointer passed to store duration in */
	if (duration == NULL) {
		SCOPE_EXIT_LOG_RTN_VALUE(-1, LOG_WARNING, "%s\n", "Error play_record_review called without duration pointer\n");
	}
	...
	while ((cmd >= 0) && (cmd != 't')) {
		...
		case '1':
			if (!msg_exists) {
				...
				ast_verb_chan(NULL, 3, "Saving message as is\n");
				if (!outsidecaller) {
					ast_trace(-1, "Renaming greeting '%s' to '%s'\n", tempfile, recordfile);
				...
				if (!outsidecaller) {
					...
					ast_trace(-1, "Saving greeting '%s'\n", recordfile);
					SCOPE_CALL(-1, STORE, recordfile, vmu->mailbox, vmu->context, -1, chan, vmu, fmt, *duration, vms, flag, msg_id);
					SCOPE_CALL(-1, DISPOSE, recordfile, -1);
				}
				cmd = 't';
				SCOPE_EXIT_RTN_VALUE(res, "Message saved to %s\n", recordfile);
			}
		case '2':
			/* Review */
			ast_verb_chan(NULL, 3, "Reviewing the message\n");
		...
		case '3':
			...
			/* Record */
			if (recorded == 1)
				ast_verb_chan(NULL, 3, "Re-recording the message\n");
			else
				ast_verb_chan(NULL, 3, "Recording the message\n");
			...
			if (cmd == -1) {
				...
				SCOPE_EXIT_RTN_VALUE(cmd, "User hung up before message could be rerecorded.  Deleted '%s'\n", tempfile);
			}
			recorded = 1;
			...
			if (cmd == -1) {
				...
				SCOPE_EXIT_RTN_VALUE(cmd, "User hung up after recording. %s %s\n",
		...
		case '4':
			if (outsidecaller && ast_test_flag(vmu, VM_MARK_URGENT)) {  /* only mark vm messages */
				...
				if ((flag && ast_strlen_zero(flag)) || (!ast_strlen_zero(flag) && strcmp(flag, "Urgent"))) {
					ast_verb_chan(NULL, 3, "marking message as Urgent\n");
					res = ast_play_and_wait(chan, "vm-marked-urgent");
					strcpy(flag, "Urgent");
				} else if (flag) {
					ast_verb_chan(NULL, 3, "UNmarking message as Urgent\n");
...
/*  XXX Commented out for the moment because of the dangers of deleting
    a message while recording (can put the message numbers out of sync) */
		...
		case '0':
			...
			if (msg_exists || recorded) {
				...
				if (cmd == '1') {
					...
					ast_trace(-1, "Deleting '%s'\n", tempfile);
					ast_play_and_wait(chan, "vm-deleted");
					SCOPE_CALL(-1, DELETE, tempfile, -1, tempfile, vmu);
					SCOPE_CALL(-1, DISPOSE, tempfile, -1);
					cmd = '0';
				}
			}
		...
		default:
			...
			if (outsidecaller && !ast_test_flag(vmu, VM_REVIEW) && !forwardintro) {
				SCOPE_EXIT_RTN_VALUE(cmd, "Done. Outside caller, review not set, no forwardintro\n");
			}
			if (msg_exists) {
	...
	if (!outsidecaller && (cmd == -1 || cmd == 't')) {
		...
		ast_trace(-1, "Deleting '%s' on hangup or timeout\n", tempfile);
		ast_filedelete(tempfile, NULL);
	}
	...
	for (vms->curmsg = 0; vms->curmsg <= vms->lastmsg; vms->curmsg++) {
		...
		if ((value = ast_variable_retrieve(msg_cfg, "message", "msg_id"))) {
			...
			if (!(add_message_id(msg_cfg, vms->curdir, vms->curmsg,
				...
				ast_log_chan(NULL, LOG_WARNING, "Unable to create a message ID for message %s/%d\n", vms->curdir, vms->curmsg);
			}
		}
	...
	int combine_INBOX_and_OLD)
{
	...
	if (ast_strlen_zero(mailbox)) {
		ast_log_chan(NULL, LOG_WARNING, "Cannot create a mailbox snapshot since no mailbox was specified\n");
		return NULL;
	}
	...
	if (!(vmu = find_user(&vmus, context, mailbox))) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Failed to create mailbox snapshot for unknown voicemail user %s@%s\n", mailbox, context);
		return NULL;
	}

	if (!(mailbox_snapshot = ast_calloc(1, sizeof(*mailbox_snapshot)))) {
		ast_log_chan(NULL, AST_LOG_ERROR, "Failed to allocate memory for mailbox snapshot\n");
		free_user(vmu);
		return NULL;
	}
	...
	for (i = 0; i < mailbox_snapshot->folders; i++) {
		...
		if (vms.lastmsg != -1) {
			if ((vm_msg_snapshot_create(vmu, &vms, mailbox_snapshot, msg_folder_index, i, descending, sort_val))) {
				ast_log_chan(NULL, LOG_WARNING, "Failed to create msg snapshots for %s@%s\n", mailbox, context);
				goto snapshot_cleanup;
			}
		}
	...
	int delete_old)
{
	...
	if (ast_strlen_zero(from_mailbox) || ast_strlen_zero(to_mailbox)) {
		ast_log_chan(NULL, LOG_WARNING, "Cannot forward message because either the from or to mailbox was not specified\n");
		return -1;
	}

	if (!num_msgs) {
		ast_log_chan(NULL, LOG_WARNING, "Invalid number of messages specified to forward: %zu\n", num_msgs);
		return -1;
	}

	if (ast_strlen_zero(from_folder) || ast_strlen_zero(to_folder)) {
		ast_log_chan(NULL, LOG_WARNING, "Cannot forward message because the from_folder or to_folder was not specified\n");
		return -1;
	}
	...
	if (!(vmu = find_user(&vmus, from_context, from_mailbox))) {
		ast_log_chan(NULL, LOG_WARNING, "Can't find voicemail user to forward from (%s@%s)\n", from_mailbox, from_context);
		return -1;
	}

	if (!(to_vmu = find_user(&to_vmus, to_context, to_mailbox))) {
		ast_log_chan(NULL, LOG_WARNING, "Can't find voicemail user to forward to (%s@%s)\n", to_mailbox, to_context);
		free_user(vmu);
		return -1;
	}
	...
	if ((from_vms.lastmsg + 1) < num_msgs) {
		ast_log_chan(NULL, LOG_WARNING, "Folder %s has less than %zu messages\n", from_folder, num_msgs);
		res = -1;
		goto vm_forward_cleanup;
	}
	...
	const char *newfolder)
{
	...
	if (ast_strlen_zero(mailbox)) {
		ast_log_chan(NULL, LOG_WARNING, "Cannot move message because no mailbox was specified\n");
		return -1;
	}

	if (!num_msgs) {
		ast_log_chan(NULL, LOG_WARNING, "Invalid number of messages specified to move: %zu\n", num_msgs);
		return -1;
	}

	if (ast_strlen_zero(oldfolder) || ast_strlen_zero(newfolder)) {
		ast_log_chan(NULL, LOG_WARNING, "Cannot move message because either oldfolder or newfolder was not specified\n");
		return -1;
	}
	...
	const char *msgs[])
{
	...
	if (ast_strlen_zero(mailbox)) {
		ast_log_chan(NULL, LOG_WARNING, "Cannot remove message because no mailbox was specified\n");
		return -1;
	}

	if (!num_msgs) {
		ast_log_chan(NULL, LOG_WARNING, "Invalid number of messages specified to remove: %zu\n", num_msgs);
		return -1;
	}

	if (ast_strlen_zero(folder)) {
		ast_log_chan(NULL, LOG_WARNING, "Cannot remove message because no folder was specified\n");
		return -1;
	}
	...
	if (folder_index == -1) {
		ast_log_chan(NULL, LOG_WARNING, "Could not remove msgs from unknown folder %s\n", folder);
		return -1;
	}

	if (!(vmu = find_user(&vmus, context, mailbox))) {
		ast_log_chan(NULL, LOG_WARNING, "Can't find voicemail user to remove msg from (%s@%s)\n", mailbox, context);
		return -1;
	}
	...
	if ((res = close_mailbox(&vms, vmu) == ERROR_LOCK_PATH)) {
		res = -1;
		ast_log_chan(NULL, AST_LOG_ERROR, "Failed to close mailbox folder %s while removing msgs\n", folder);
		goto vm_remove_cleanup;
	}
	open = 0;
	...
	ast_vm_msg_play_cb cb)
{
	...
	if (ast_strlen_zero(mailbox)) {
		ast_log_chan(NULL, LOG_WARNING, "Cannot play message because no mailbox was specified\n");
		return -1;
	}

	if (ast_strlen_zero(folder)) {
		ast_log_chan(NULL, LOG_WARNING, "Cannot play message because no folder was specified\n");
		return -1;
	}

	if (ast_strlen_zero(msg_id)) {
		ast_log_chan(NULL, LOG_WARNING, "Cannot play message because no message number was specified\n");
		return -1;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_DEFAULT, tdesc,
