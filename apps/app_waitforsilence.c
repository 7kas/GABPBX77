...
static int do_waiting(struct ast_channel *chan, int timereqd, time_t waitstart, int timeout, const struct wait_type *wait_for)
{
	...
	if ((res = ast_set_read_format(chan, ast_format_slin)) < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to set channel to linear mode, giving up\n");
		return -1;
	}
	...
	for (;;) {
		...
		if (dsptime >= timereqd) {
			ast_verb_chan(NULL, 3, "Exiting with %dms of %s >= %dms required\n", dsptime, wait_for->name, timereqd);
	...
	if (rfmt && ast_set_read_format(chan, rfmt)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to restore format %s to channel '%s'\n", ast_format_get_name(rfmt), ast_channel_name(chan));
	}
...
static int waitfor_exec(struct ast_channel *chan, const char *data, const struct wait_type *wait_for)
{
	...
	if (!ast_strlen_zero(args.timereqd)) {
		if (sscanf(args.timereqd, "%30d", &timereqd) != 1 || timereqd < 0) {
			ast_log_chan(NULL, LOG_ERROR, "Argument '%srequired' must be an integer greater than or equal to zero.\n",
					wait_for->name);
			return -1;
		}
	}
	...
	if (!ast_strlen_zero(args.iterations)) {
		if (sscanf(args.iterations, "%30d", &iterations) != 1 || iterations < 1) {
			ast_log_chan(NULL, LOG_ERROR, "Argument 'iterations' must be an integer greater than 0.\n");
			return -1;
		}
	}
	...
	if (!ast_strlen_zero(args.timeout)) {
		if (sscanf(args.timeout, "%30d", &timeout) != 1 || timeout < 0) {
			ast_log_chan(NULL, LOG_ERROR, "Argument 'timeout' must be an integer greater than or equal to zero.\n");
			return -1;
		}
	}
	...
	ast_verb_chan(NULL, 3, "Waiting %d time(s) for %dms of %s with %ds timeout\n",
...
AST_MODULE_INFO_STANDARD_EXTENDED(GABPBX_GPL_KEY, "Wait For Silence/Noise");
