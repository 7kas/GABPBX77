	...
	*/
	...
	if ((!end && !pbx_checkcondition(condition)) || (end == 2)) {
		...
		if ((goto_str = pbx_builtin_getvar_helper(chan, end_varname))) {
			...
			if (pri > 0) {
				ast_verb_chan(NULL, 3, "Jumping to priority %d\n", pri);
				ast_channel_priority_set(chan, pri);
			} else {
				ast_log_chan(NULL, LOG_WARNING, "Couldn't find matching EndWhile? (While at %s@%s priority %d)\n", ast_channel_context(chan), ast_channel_exten(chan), ast_channel_priority(chan));
			}
		}
		ast_channel_unlock(chan);
		return res;
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "While Loops and Conditional Execution");
