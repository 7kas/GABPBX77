		...
		<configFile name="confbridge.conf">
			...
			<configObject name="user_profile">
				...
				<configOption name="dsp_drop_silence">
					...
					<synopsis>Drop what GABpbx detects as silence from audio sent to the bridge</synopsis>
					<description><para>
					This option drops what GABpbx detects as silence from
			...
			<configObject name="bridge_profile">
				...
				<configOption name="internal_sample_rate">
					...
					<description><para>
						...
						sample rate is set that GABpbx does not support, the
						closest sample rate GABpbx does support to the one requested
						will be used.
					</para></description>
				</configOption>
				...
				<configOption name="record_conference">
					...
					<description><para>
						...
						located in the configured monitoring directory in <filename>gabpbx.conf</filename>.
					</para></description>
				</configOption>
				<configOption name="record_file" default="confbridge-${name of conference bridge}-${start time}.wav">
					...
					<description><para>
						...
						By default, the record_file is stored in GABpbx's spool/monitor directory
...
static int set_sound(const char *sound_name, const char *sound_file, struct bridge_profile *b_profile)
{
	...
	if (!strcasecmp(sound_name, "sound_only_person")) {
		...
		if (deprecation_warning) {
			ast_log_chan(NULL, LOG_WARNING, "sound_place_into_conference is deprecated"
...
int func_confbridge_helper(struct ast_channel *chan, const char *cmd, char *data, const char *value)
{
	...
	ast_log_chan(NULL, LOG_WARNING, "%s(%s,%s) cannot be set to '%s'. Invalid type, option, or value.\n",
...
static int verify_default_profiles(void)
{
	...
	if (!bridge_profile) {
		...
		ast_log_chan(NULL, AST_LOG_NOTICE, "Adding %s profile to app_confbridge\n", DEFAULT_BRIDGE_PROFILE);
	...
	if (!menu_profile) {
		...
		ast_log_chan(NULL, AST_LOG_NOTICE, "Adding %s menu to app_confbridge\n", DEFAULT_MENU_PROFILE);
...
/* XXX GABPBX-21271 need a user supplied bridge merge_priority to merge ConfBridges (default = 1, range 1-INT_MAX) */
	/* Bridge options */
