	...
	struct ast_channel *chan)
{
	...
	struct ast_msg_data_attribute attrs[] = {
		...
		{ .type = AST_MSG_DATA_ATTR_CONTENT_TYPE, .value = "application/x-gabpbx-confbridge-event+json"},
	...
	if (!json) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to convert json_object for %s message to string\n", msg_name);
		return;
	}
	attrs[2].value = json;
	...
	if (!data_msg) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to create %s message for channel '%s'\n", msg_name,
	...
	if (rc != 0) {
		...
		if (strcmp(confbridge_event_type_to_string(confbridge_leave_type()), msg_name) != 0) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to queue %s message to '%s'\n%s\n", msg_name,
	...
	struct ast_channel *chan, struct stasis_message *msg)
{
	...
	if (chan) {
		if (!conf_find_user_profile(chan, NULL, &u_profile)) {
			ast_log_chan(NULL, LOG_ERROR, "Unable to retrieve user profile for channel '%s'\n",
				ast_channel_name(chan));
			return;
		}
	...
	AST_LIST_TRAVERSE(&conference->active_list, user, list) {
		...
		if (source_send_events && stasis_message_type(msg) == confbridge_join_type()) {
			...
			if (!target_snapshot) {
				ast_log_chan(NULL, LOG_ERROR, "Unable to get a channel snapshot for '%s'\n",
			...
			if (!json_channels) {
				...
				if (!json_channels) {
					ast_log_chan(NULL, LOG_ERROR, "Unable to allocate json array\n");
		...
		if (!json_object) {
			ast_log_chan(NULL, LOG_ERROR, "Unable to convert %s message to json\n", msg_name);
			continue;
		}
	...
	if (source_send_events && json_channels
		...
		if (!json_object) {
			ast_log_chan(NULL, LOG_ERROR, "Unable to convert ConfbridgeWelcome message to json\n");
			return;
		}
