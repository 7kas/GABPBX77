...
{
	ast_verb_chan(NULL, 4, "AutoMixMonitor used to stop recording call.\n");

	if (ast_stop_mixmonitor(peer_chan, NULL)) {
		ast_verb_chan(NULL, 4, "Failed to stop AutoMixMonitor for %s.\n", ast_channel_name(bridge_channel->chan));
...
static void start_automixmonitor(struct ast_bridge_channel *bridge_channel, struct ast_channel *peer_chan, struct ast_features_general_config *features_cfg, const char *start_message)
{
	...
	ast_verb_chan(NULL, 4, "AutoMixMonitor used to record call. Filename: %s\n", touch_filename);
	...
	if (!ast_strlen_zero(touch_monitor_beep)) {
		...
		if (interval < 5) {
			interval = 5;
			ast_log_chan(NULL, LOG_WARNING, "Interval '%s' too small for periodic beep. Using minimum of %u\n",
					touch_monitor_beep, interval);
		}
	...
	if (ast_start_mixmonitor(peer_chan, touch_filename, mix_options)) {
		ast_verb_chan(NULL, 4, "AutoMixMonitor feature was tried by '%s' but MixMonitor failed to start.\n",
			ast_channel_name(bridge_channel->chan));
...
static int feature_automixmonitor(struct ast_bridge_channel *bridge_channel, void *hook_pvt)
{
	...
	if (!peer_chan) {
		ast_verb_chan(NULL, 4, "Cannot start AutoMixMonitor for %s - cannot determine peer in bridge.\n",
			ast_channel_name(bridge_channel->chan));
		if (features_cfg && !ast_strlen_zero(features_cfg->recordingfailsound)) {
			ast_bridge_channel_queue_playfile(bridge_channel, NULL, features_cfg->recordingfailsound, NULL);
		}
	...
	case AUTO_MONITOR_START:
		...
		ast_verb_chan(NULL, 4, "AutoMixMonitor already recording call.\n");
		break;
	case AUTO_MONITOR_STOP:
		...
		ast_verb_chan(NULL, 4, "AutoMixMonitor already stopped on call.\n");
		break;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_DEFAULT, "Built in bridging features",
