...
struct native_rtp_bridge_channel_data {
	...
	/*!
	 * \brief Glue callbacks to bring remote channel streams back to GABpbx.
...
static void native_rtp_bridge_start(struct ast_bridge *bridge, struct ast_channel *target)
{
	...
	case AST_RTP_GLUE_RESULT_LOCAL:
		...
		ast_verb_chan(NULL, 4, "Locally RTP bridged '%s' and '%s' in stack\n",
	...
	case AST_RTP_GLUE_RESULT_REMOTE:
		...
		if (!target) {
			...
			ast_verb_chan(NULL, 4, "Remotely bridged '%s' and '%s' - media will flow directly between them\n",
				ast_channel_name(bc0->chan), ast_channel_name(bc1->chan));
		} else {
			/*
...
static void native_rtp_bridge_stop(struct ast_bridge *bridge, struct ast_channel *target)
{
	...
	case AST_RTP_GLUE_RESULT_REMOTE:
		if (target) {
			 ...
			 * receive media from GABpbx instead of what it was previously connected to.
			 */
			ast_debug(2, "Bridge '%s'.  Bringing back '%s' to us\n",
				bridge->uniqueid, ast_channel_name(target));
			if (bc0->chan == target) {
	...
	ast_debug(2, "Discontinued RTP bridging of '%s' and '%s' - media will flow through GABpbx core\n",
		ast_channel_name(bc0->chan), ast_channel_name(bc1->chan));
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Native RTP bridging module");
