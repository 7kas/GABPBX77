		...
		const char *channel_name)
{
	...
	if (data->pos_ids[pos_id] != 1) {
		ast_log_chan(NULL, LOG_ERROR, "Channel %s: Channel pair has no active member! (pos id = %d)\n",
				channel_name, pos_id);
		return NULL;
	}
	...
	if (do_convolve(&chan_pair->chan_left, in_samples, in_sample_size, data->hrtf_length)) {
		ast_log_chan(NULL, LOG_ERROR, "Channel %s: Binaural processing failed.", channel_name);
		return NULL;
	}
...
#else
	ast_log_chan(NULL, LOG_ERROR, "Requesting data for the binaural conference feature without "
			"it beeing active.\n");
#endif
		...
		unsigned int hrtf_len, unsigned int chan_pos, unsigned int default_sample_size)
{
	...
	return success;
#else
	ast_log_chan(NULL, LOG_ERROR, "Requesting data for the binaural conference feature "
