...
static int load_config(void)
{
	...
	if (!cfg || cfg == CONFIG_STATUS_FILEINVALID) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to load " CONFIG ".  No adaptive ODBC CDRs.\n");
		return -1;
	}
	...
	for (catg = ast_category_browse(cfg, NULL); catg; catg = ast_category_browse(cfg, catg)) {
		...
		if (ast_strlen_zero(tmp = ast_variable_retrieve(cfg, catg, "connection"))) {
			ast_log_chan(NULL, LOG_WARNING, "No connection parameter found in '%s'.  Skipping.\n", catg);
			continue;
		}
		...
		if (!obj) {
			ast_log_chan(NULL, LOG_WARNING, "No such connection '%s' in the '%s' section of " CONFIG ".  Check res_odbc.conf.\n", connection, catg);
			continue;
		}

		if (ast_strlen_zero(tmp = ast_variable_retrieve(cfg, catg, "table"))) {
			ast_log_chan(NULL, LOG_NOTICE, "No table name found.  Assuming 'cdr'.\n");
			tmp = "cdr";
		}
		...
		if (strlen(tmp) > 1) {
			ast_log_chan(NULL, LOG_ERROR, "The quoted_identifiers setting only accepts a single character,"
		...
		if ((res != SQL_SUCCESS) && (res != SQL_SUCCESS_WITH_INFO)) {
			ast_log_chan(NULL, LOG_WARNING, "SQL Alloc Handle failed on connection '%s'!\n", connection);
		...
		if ((res != SQL_SUCCESS) && (res != SQL_SUCCESS_WITH_INFO)) {
			ast_log_chan(NULL, LOG_ERROR, "Unable to query database columns on connection '%s'.  Skipping.\n", connection);
		...
		if (!tableptr) {
			ast_log_chan(NULL, LOG_ERROR, "Out of memory creating entry for table '%s' on connection '%s'%s%s%s\n", table, connection,
		...
		ast_verb_chan(NULL, 3, "Found adaptive CDR table %s@%s.\n", tableptr->table, tableptr->connection);
		...
		for (var = ast_variable_browse(cfg, catg); var; var = var->next) {
			if (strncmp(var->name, "filter", 6) == 0) {
				...
				ast_verb_chan(NULL, 3, "Found filter %s'%s' for CDR variable %s in %s@%s\n", negate ? "!" : "", var->value, cdrvar, tableptr->table, tableptr->connection);

				entry = ast_calloc(sizeof(char), sizeof(*entry) + strlen(cdrvar) + 1 + strlen(var->value) + 1);
				if (!entry) {
					ast_log_chan(NULL, LOG_ERROR, "Out of memory creating filter entry for CDR variable '%s' in table '%s' on connection '%s'\n", cdrvar, table, connection);
					res = -1;
					break;
				}
		...
		while ((res = SQLFetch(stmt)) != SQL_NO_DATA && res != SQL_ERROR) {
			...
			for (var = ast_variable_browse(cfg, catg); var; var = var->next) {
				if (strncmp(var->name, "alias", 5) == 0 && strcasecmp(var->value, columnname) == 0) {
					...
					ast_verb_chan(NULL, 3, "Found alias %s for column %s in %s@%s\n", cdrvar, columnname, tableptr->table, tableptr->connection);
					break;
			...
			if (!entry) {
				ast_log_chan(NULL, LOG_ERROR, "Out of memory creating entry for column '%s' in table '%s' on connection '%s'\n", columnname, table, connection);
				res = -1;
				SQLFreeHandle(SQL_HANDLE_STMT, stmt);
				break;
			}
			...
			ast_verb_chan(NULL, 4, "Found %s column with type %hd with len %ld, octetlen %ld, and numlen (%hd,%hd)\n", entry->name, entry->type, (long) entry->size, (long) entry->octetlen, entry->decimals, entry->radix);
...
static SQLHSTMT generic_prepare(struct odbc_obj *obj, void *data)
{
	...
	res = SQLAllocHandle (SQL_HANDLE_STMT, obj->con, &stmt);
	if ((res != SQL_SUCCESS) && (res != SQL_SUCCESS_WITH_INFO)) {
		ast_log_chan(NULL, LOG_WARNING, "SQL Alloc Handle failed!\n");
		return NULL;
	}

	res = ast_odbc_prepare(obj, stmt, data);
	if ((res != SQL_SUCCESS) && (res != SQL_SUCCESS_WITH_INFO)) {
		ast_log_chan(NULL, LOG_WARNING, "SQL Prepare failed![%s]\n", (char *) data);
...
static int odbc_log(struct ast_cdr *cdr)
{
	...
	if (AST_RWLIST_RDLOCK(&odbc_tables)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to lock table list.  Insert CDR(s) failed.\n");
		ast_free(sql);
		ast_free(sql2);
		return -1;
	}
	...
	AST_LIST_TRAVERSE(&odbc_tables, tableptr, list) {
		...
		if (!(obj = ast_odbc_request_obj(tableptr->connection, 0))) {
			ast_log_chan(NULL, LOG_WARNING, "cdr_adaptive_odbc: Unable to retrieve database handle for '%s:%s'.  CDR failed: %s\n", tableptr->connection, tableptr->table, ast_str_buffer(sql));
			continue;
		}
		...
		AST_LIST_TRAVERSE(&(tableptr->columns), entry, list) {
			...
			if (colptr) {
				...
				if ((entry->filtervalue && !entry->negatefiltervalue && strcasecmp(colptr, entry->filtervalue) != 0) ||
					...
					ast_verb_chan(NULL, 4, "CDR column '%s' with value '%s' does not match filter of"
				...
				case SQL_TYPE_DATE:
					if (ast_strlen_zero(colptr)) {
						...
						if (sscanf(colptr, "%4d-%2d-%2d", &year, &month, &day) != 3 || year <= 0 ||
							...
							ast_log_chan(NULL, LOG_WARNING, "CDR variable %s is not a valid date ('%s').\n", entry->name, colptr);
							continue;
						}
				...
				case SQL_TYPE_TIME:
					if (ast_strlen_zero(colptr)) {
						...
						if ((count != 2 && count != 3) || hour < 0 || hour > 23 || minute < 0 || minute > 59 || second < 0 || second > 59) {
							ast_log_chan(NULL, LOG_WARNING, "CDR variable %s is not a valid time ('%s').\n", entry->name, colptr);
							continue;
						}
				...
				case SQL_DATETIME:
					if (ast_strlen_zero(colptr)) {
						...
						if ((count != 3 && count != 5 && count != 6) || year <= 0 ||
							...
							ast_log_chan(NULL, LOG_WARNING, "CDR variable %s is not a valid timestamp ('%s').\n", entry->name, colptr);
							continue;
						}
				...
				case SQL_INTEGER:
					if (ast_strlen_zero(colptr)) {
						...
						if (sscanf(colptr, "%30d", &integer) != 1) {
							ast_log_chan(NULL, LOG_WARNING, "CDR variable %s is not an integer.\n", entry->name);
							continue;
						}
				...
				case SQL_DECIMAL:
					if (ast_strlen_zero(colptr)) {
						...
						if (sscanf(colptr, "%30lf", &number) != 1) {
							ast_log_chan(NULL, LOG_WARNING, "CDR variable %s is not an numeric type.\n", entry->name);
							continue;
						}
				...
				case SQL_DOUBLE:
					...
					break;
				default:
					ast_log_chan(NULL, LOG_WARNING, "Column type %d (field '%s:%s:%s') is unsupported at this time.\n", entry->type, tableptr->connection, tableptr->table, entry->name);
					continue;
				}
				if (quoted) {
				...
				&& ((!entry->negatefiltervalue && entry->filtervalue[0] != '\0')
					|| (entry->negatefiltervalue && entry->filtervalue[0] == '\0'))) {
				ast_verb_chan(NULL, 4, "CDR column '%s' was not set and does not match filter of"
					" %s'%s'.  Cancelling this CDR.\n",
		...
		if (rows == 0) {
			ast_log_chan(NULL, LOG_WARNING, "cdr_adaptive_odbc: Insert failed on '%s:%s'.  CDR failed: %s\n", tableptr->connection, tableptr->table, ast_str_buffer(sql));
		}
early_release:
		ast_odbc_release_obj(obj);
	}
...
static int unload_module(void)
{
	...
	if (AST_RWLIST_WRLOCK(&odbc_tables)) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Unable to lock column list.  Unload failed.\n");
		return -1;
	}
...
{
	if (AST_RWLIST_WRLOCK(&odbc_tables)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to lock column list.  Load failed.\n");
		return 0;
	}
...
{
	if (AST_RWLIST_WRLOCK(&odbc_tables)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to lock column list.  Reload failed.\n");
		return -1;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "Adaptive ODBC CDR backend",
