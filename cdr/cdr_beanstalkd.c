...
/*!
 * \file
 * \brief GABpbx Beanstalkd CDR records.
 *
...
#define DEFAULT_BEANSTALK_TUBE "gabpbx-cdr"
...
static int load_config(int reload) {
	...
	if (cfg == CONFIG_STATUS_FILEINVALID) {
		ast_log_chan(NULL, LOG_ERROR, "Config file '%s' could not be parsed\n", CONF_FILE);
		return -1;
	}

	if (!cfg) {
		/* Standard configuration */
		ast_log_chan(NULL, LOG_WARNING, "Failed to load configuration file. Module not activated.\n");
		if (enablecdr) {
	...
	if (!newenablecdr) {
		...
		ast_log_chan(NULL, LOG_NOTICE, "Added beanstalkd server %s at port %d with tube %s", bs_host, bs_port, bs_tube);
	}
...
static int load_module(void) {
	if (ast_cdr_register(name, "GABpbx CDR Beanstalkd Backend", beanstalk_put)) {
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "GABpbx Beanstalkd CDR Backend",
