...
static int writefile(char *s, char *file_path)
{
	...
	if (!(f = fopen(file_path, "a"))) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to open file %s : %s\n", file_path, strerror(errno));
		return -1;
	}
	fputs(s, f);
...
static int csv_log(struct ast_cdr *cdr)
{
	...
	if (build_csv_record(buf, sizeof(buf), cdr)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to create CSV record in %d bytes.  CDR not recorded!\n", (int)sizeof(buf));
		return 0;
	}

	ast_mutex_lock(&f_lock);
	if (writefile(buf, file_csv_master))
		ast_log_chan(NULL, LOG_WARNING, "Unable to write CSV record to master '%s' : %s\n", file_csv_master, strerror(errno));

	if (accountlogs && !ast_strlen_zero(cdr->accountcode)) {
		if (writefile_account(buf, cdr->accountcode))
			ast_log_chan(NULL, LOG_WARNING, "Unable to write CSV record to account file '%s' : %s\n", cdr->accountcode, strerror(errno));
	}
	ast_mutex_unlock(&f_lock);
	return 0;
}
...
static int load_module(void)
{
	...
	if ((res = ast_cdr_register(name, ast_module_info->description, csv_log))) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to register CSV CDR handling\n");
	} else {
		loaded = 1;
	}
	return res;
...
static int reload(void)
{
	if (load_config(1)) {
		...
		loaded = 0;
		ast_log_chan(NULL, LOG_WARNING, "No [csv] section in cdr.conf.  Unregistering backend.\n");
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "Comma Separated Values CDR Backend",
