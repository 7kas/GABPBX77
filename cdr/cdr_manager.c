...
/*!
 * \file
 * \brief GABpbx Call Manager CDR records.
 *
 * See also
 * \arg \ref AstCDR
 * \arg \ref AstAMI
			...
			while (v) {
				if (customfields && !ast_strlen_zero(v->name) && !ast_strlen_zero(v->value)) {
					if ((ast_str_strlen(customfields) + strlen(v->value) + strlen(v->name) + 14) < ast_str_size(customfields)) {
						...
						ast_log_chan(NULL, LOG_NOTICE, "Added mapping %s: ${CDR(%s)}\n", v->value, v->name);
					} else {
						ast_log_chan(NULL, LOG_WARNING, "No more buffer space to add other custom fields\n");
						break;
					}
...
{
	if (ast_cdr_register(name, "GABpbx Manager Interface CDR Backend", manager_log)) {
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "GABpbx Manager Interface CDR Backend",
