...
static SQLHSTMT execute_cb(struct odbc_obj *obj, void *data)
{
	...
	if ((ODBC_res != SQL_SUCCESS) && (ODBC_res != SQL_SUCCESS_WITH_INFO)) {
		ast_log_chan(NULL, LOG_WARNING, "cdr_odbc: Failure in AllocStatement %d\n", ODBC_res);
	...
	if ((ODBC_res != SQL_SUCCESS) && (ODBC_res != SQL_SUCCESS_WITH_INFO)) {
		ast_log_chan(NULL, LOG_WARNING, "cdr_odbc: Error in ExecDirect: %d, query is: %s\n", ODBC_res, sqlcmd);
		SQLFreeHandle(SQL_HANDLE_STMT, stmt);
		return NULL;
	}
...
static int odbc_log(struct ast_cdr *cdr)
{
	...
	if (!obj) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to retrieve database handle.  CDR failed.\n");
		return -1;
	}
	...
	if (stmt) {
		...
		if (rows == 0)
			ast_log_chan(NULL, LOG_WARNING, "CDR successfully ran, but inserted 0 rows?\n");
	} else
		ast_log_chan(NULL, LOG_ERROR, "CDR direct execute failed\n");
...
static int odbc_load_module(int reload)
{
	...
	do {
		...
		if (!cfg || cfg == CONFIG_STATUS_FILEINVALID) {
			ast_log_chan(NULL, LOG_WARNING, "cdr_odbc: Unable to load config for ODBC CDR's: %s\n", config_file);
		...
		if ((tmp = ast_variable_retrieve(cfg, "global", "dsn")) == NULL) {
			ast_log_chan(NULL, LOG_WARNING, "cdr_odbc: dsn not specified.  Assuming gabpbxdb\n");
			tmp = "gabpbxdb";
		}
		if (dsn)
			ast_free(dsn);
		...
		if ((tmp = ast_variable_retrieve(cfg, "global", "table")) == NULL) {
			ast_log_chan(NULL, LOG_WARNING, "cdr_odbc: table not specified.  Assuming cdr\n");
			tmp = "cdr";
		}
		if (table)
			ast_free(table);
		...
		if (!ast_test_flag(&config, CONFIG_REGISTERED)) {
			...
			if (res) {
				ast_log_chan(NULL, LOG_ERROR, "cdr_odbc: Unable to register ODBC CDR handling\n");
			} else {
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "ODBC CDR Backend",
