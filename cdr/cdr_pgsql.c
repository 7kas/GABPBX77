			...
			do { \
				...
				if (ast_str_strlen(var_sql) + size + 1 > ast_str_size(var_sql)) { \
					if (ast_str_make_space(&var_sql, ((ast_str_size(var_sql) + size + 3) / 512 + 1) * 512) != 0) { \
						ast_log_chan(NULL, LOG_ERROR, "Unable to allocate sufficient memory. Insert CDR '%s:%s' failed.\n", pghostname, table); \
						ast_free(sql); \
						ast_free(sql2); \
...
static void pgsql_reconnect(void)
{
	...
	if (ast_str_strlen(conn_info) == 0) {
		ast_log_chan(NULL, LOG_ERROR, "Connection string is blank.\n");
		return;
	}
...
static int pgsql_log(struct ast_cdr *cdr)
{
	...
	if ((!connected) && pghostname && pgdbuser && pgpassword && pgdbname) {
		...
		if (PQstatus(conn) != CONNECTION_BAD) {
			...
			if (PQsetClientEncoding(conn, encoding)) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Failed to set encoding to '%s'.  Encoding set to default '%s'\n", encoding, pg_encoding_to_char(PQclientEncoding(conn)));
#else
				ast_log_chan(NULL, LOG_WARNING, "Failed to set encoding to '%s'.  Encoding set to default.\n", encoding);
#endif
			}
		} else {
			pgerror = PQerrorMessage(conn);
			ast_log_chan(NULL, LOG_ERROR, "Unable to connect to database server %s.  Calls will not be logged!\n", pghostname);
	...
	if (connected) {
		...
		if (PQstatus(conn) == CONNECTION_OK) {
			...
			ast_log_chan(NULL, LOG_ERROR, "Connection was lost... attempting to reconnect.\n");
			PQreset(conn);
			if (PQstatus(conn) == CONNECTION_OK) {
				ast_log_chan(NULL, LOG_ERROR, "Connection reestablished.\n");
				connected = 1;
				connect_time = time(NULL);
				records = 0;
			} else {
		...
		if (PQresultStatus(result) != PGRES_COMMAND_OK) {
			...
			ast_log_chan(NULL, LOG_ERROR, "Failed to insert call detail record into database!\n");
			ast_log_chan(NULL, LOG_ERROR, "Reason: %s\n", pgerror);
			ast_log_chan(NULL, LOG_ERROR, "Connection may have been lost... attempting to reconnect.\n");
			PQreset(conn);
			if (PQstatus(conn) == CONNECTION_OK) {
				ast_log_chan(NULL, LOG_ERROR, "Connection reestablished.\n");
				connected = 1;
...
static int config_module(int reload)
{
	...
	if ((cfg = ast_config_load(config, config_flags)) == NULL || cfg == CONFIG_STATUS_FILEINVALID) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to load config for PostgreSQL CDR's: %s\n", config);
		return -1;
	...
	if (!ast_variable_browse(cfg, "global")) {
		...
		ast_mutex_unlock(&pgsql_lock);
		ast_log_chan(NULL, LOG_NOTICE, "cdr_pgsql configuration contains no global section, skipping module %s.\n",
			reload ? "reload" : "load");
		return -1;
	}
	...
	if (!(tmp = ast_variable_retrieve(cfg, "global", "table"))) {
		ast_log_chan(NULL, LOG_WARNING, "CDR table not specified.  Assuming cdr\n");
		tmp = "cdr";
	}
	...
	if (!(tmp = ast_variable_retrieve(cfg, "global", "encoding"))) {
		ast_log_chan(NULL, LOG_WARNING, "Encoding not specified.  Assuming LATIN9\n");
		tmp = "LATIN9";
	}
	...
	if (DEBUG_ATLEAST(1)) {
		if (ast_strlen_zero(pghostname)) {
			ast_log_chan(NULL, LOG_DEBUG, "using default unix socket\n");
		} else {
			ast_log_chan(NULL, LOG_DEBUG, "got hostname of %s\n", pghostname);
		}
		ast_log_chan(NULL, LOG_DEBUG, "got port of %s\n", pgdbport);
		ast_log_chan(NULL, LOG_DEBUG, "got user of %s\n", pgdbuser);
		ast_log_chan(NULL, LOG_DEBUG, "got dbname of %s\n", pgdbname);
		ast_log_chan(NULL, LOG_DEBUG, "got password of %s\n", pgpassword);
		ast_log_chan(NULL, LOG_DEBUG, "got application name of %s\n", pgappname);
		ast_log_chan(NULL, LOG_DEBUG, "got sql table name of %s\n", table);
		ast_log_chan(NULL, LOG_DEBUG, "got encoding of %s\n", encoding);
		ast_log_chan(NULL, LOG_DEBUG, "got timezone of %s\n", tz);
	}
	...
	if (PQstatus(conn) != CONNECTION_BAD) {
		...
		if (rows == 0) {
			ast_log_chan(NULL, LOG_ERROR, "cdr_pgsql: Failed to query database columns. No columns found, does the table exist?\n");
			PQclear(result);
			unload_module();
			ast_mutex_unlock(&pgsql_lock);
			return AST_MODULE_LOAD_DECLINE;
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "PostgreSQL CDR Backend",
