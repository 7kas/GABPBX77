...
static int load_config(int reload)
{
	...
	if (!ast_strlen_zero(tmp = ast_variable_retrieve(cfg, "master", "table"))) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Table name not specified.  Assuming cdr.\n");
		strcpy(table, "cdr");
	}
	...
	ast_verb_chan(NULL, 4, "cdr_sqlite3_custom: Logging CDR records to table '%s' in 'master.db'\n", table);
...
static int load_module(void)
{
	...
	if (res) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to register custom SQLite3 CDR handling\n");
		free_config(0);
		return AST_MODULE_LOAD_DECLINE;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "SQLite3 Custom CDR Module",
