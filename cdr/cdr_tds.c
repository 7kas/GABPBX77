...
retry:
	...
	if (erc == FAIL) {
		if (attempt++ < 3) {
			...
			ast_log_chan(NULL, LOG_ERROR, "Failed to build INSERT statement, no CDR was logged.\n");
			goto done;
		}
	}
	...
	if (dbsqlexec(settings->dbproc) == FAIL) {
		if (attempt++ < 3) {
			...
			ast_log_chan(NULL, LOG_ERROR, "Failed to execute INSERT statement, no CDR was logged.\n");
			goto done;
		}
	}
...
static int mssql_connect(void)
{
	...
	if (execute_and_consume(settings->dbproc, "SELECT userfield FROM [%s] WHERE 1 = 0", settings->table)) {
		ast_log_chan(NULL, LOG_NOTICE, "Unable to find 'userfield' column in table '%s'\n", settings->table);
...
static int tds_load_module(int reload)
{
	...
	if (!cfg || cfg == CONFIG_STATUS_FILEINVALID) {
		ast_log_chan(NULL, LOG_NOTICE, "Unable to load TDS config for CDRs: %s\n", config);
		return 0;
	...
	if (ptr) {
		...
		if (ptr) {
			...
			ast_log_chan(NULL, LOG_ERROR, "Failed to connect: Database server connection not specified.\n");
			goto failed;
		}
	}
	...
	if (ptr) {
		...
		ast_log_chan(NULL, LOG_NOTICE, "Table name not specified, using 'cdr' by default.\n");
	...
	if (ptr && ast_true(ptr)) {
		...
		ast_log_chan(NULL, LOG_NOTICE, "High Resolution Time not found, using integers for billsec and duration fields by default.\n");
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "FreeTDS CDR Backend",
