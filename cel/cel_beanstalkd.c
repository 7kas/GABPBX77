...
/*! \file
 *
 * \brief GABpbx Channel Event Beanstalkd backend
 *
...
#define DEFAULT_BEANSTALK_TUBE "gabpbx-cel"
...
static void cel_bs_put(struct ast_event *event)
{
	...
	if (bs_use(bs_socket, bs_tube) != BS_STATUS_OK) {
		ast_log_chan(NULL, LOG_ERROR, "Connection to Beanstalk tube %s @ %s:%d had failed", bs_tube, bs_host, bs_port);
	...
	if (bs_id > 0) {
		ast_log_chan(NULL, LOG_DEBUG, "Successfully created job %d with %s\n", bs_id, cel_buffer);
	} else {
		ast_log_chan(NULL, LOG_ERROR, "CDR job creation failed for %s\n", cel_buffer);
	}
...
static int load_config(int reload)
{
	...
	if (cfg == CONFIG_STATUS_FILEINVALID) {
		ast_log_chan(NULL, LOG_WARNING, "Configuration file '%s' is invalid. CEL Beanstalkd Module not activated.\n",
			CONF_FILE);
		return -1;
	} else if (!cfg) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to load configuration file. CEL Beanstalkd Module not activated.\n");
		if (enablecel) {
	...
	while ((cat = ast_category_browse(cfg, cat))) {
		...
		for (v = ast_variable_browse(cfg, cat); v; v = v->next) {
			if (!strcasecmp(v->name, "enabled")) {
				...
				ast_log_chan(NULL, LOG_NOTICE, "Unknown option '%s' specified "
	...
	if (enablecel && !newenablecel) {
		...
		if (ast_cel_backend_register(CEL_BACKEND_NAME, cel_bs_put)) {
			ast_log_chan(NULL, LOG_ERROR, "Unable to register Beanstalkd CEL handling\n");
		}
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "Beanstalkd CEL Backend",
