...
static int load_config(void)
{
	...
	if (!cfg || cfg == CONFIG_STATUS_FILEINVALID) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to load " CONFIG ". Not logging CEL to custom CSVs.\n");
		return -1;
	}

	if (!(var = ast_variable_browse(cfg, "mappings"))) {
		ast_log_chan(NULL, LOG_NOTICE, "No mappings found in " CONFIG ". Not logging CEL to custom CSVs.\n");
	}
	...
	while (var) {
		if (!ast_strlen_zero(var->name) && !ast_strlen_zero(var->value)) {
			...
			ast_verb_chan(NULL, 3, "Added CEL CSV mapping for '%s'.\n", sink->filename);
			mappings += 1;
	...
	ast_verb_chan(NULL, 1, "Added CEL CSV mapping for %d files.\n", mappings);
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "Customizable Comma Separated Values CEL Backend",
