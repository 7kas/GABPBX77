...
/*! \file
 *
 * \brief GABpbx Channel Event records.
 *
 * See also
 * \arg \ref AstCDR
 * \arg \ref AstAMI
...
static int load_config(int reload)
{
	...
	if (cfg == CONFIG_STATUS_FILEINVALID) {
		ast_log_chan(NULL, LOG_WARNING, "Configuration file '%s' is invalid. CEL manager Module not activated.\n",
			CONF_FILE);
		enablecel = 0;
		return -1;
	} else if (!cfg) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to load configuration file. CEL manager Module not activated.\n");
		enablecel = 0;
		return -1;
	}
	...
	if (enablecel && !newenablecel) {
		...
		if (ast_cel_backend_register(MANAGER_BACKEND_NAME, manager_log)) {
			ast_log_chan(NULL, LOG_ERROR, "Unable to register GABpbx Call Manager CEL handling\n");
		}
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "GABpbx Manager Interface CEL Backend",
