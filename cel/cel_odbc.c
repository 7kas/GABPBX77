...
static int load_config(void)
{
	...
	if (!cfg || cfg == CONFIG_STATUS_FILEINVALID) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to load " CONFIG ".  No ODBC CEL records!\n");
		return -1;
	}
	...
	for (catg = ast_category_browse(cfg, NULL); catg; catg = ast_category_browse(cfg, catg)) {
		...
		if (ast_strlen_zero(tmp = ast_variable_retrieve(cfg, catg, "table"))) {
			ast_log_chan(NULL, LOG_NOTICE, "No table name found.  Assuming 'cel'.\n");
			tmp = "cel";
		}
		...
		if (!tableptr) {
			ast_log_chan(NULL, LOG_ERROR, "Out of memory creating entry for table '%s' on connection '%s'\n", table, connection);
			ast_odbc_release_obj(obj);
			res = -1;
			break;
		}
		...
		ast_verb_chan(NULL, 3, "Found CEL table %s@%s.\n", tableptr->table, tableptr->connection);
		...
		for (var = ast_variable_browse(cfg, catg); var; var = var->next) {
			if (strncmp(var->name, "filter", 6) == 0) {
				...
				ast_verb_chan(NULL, 3, "Found filter %s for cel variable %s in %s@%s\n", var->value, celvar, tableptr->table, tableptr->connection);

				entry = ast_calloc(sizeof(char), sizeof(*entry) + strlen(celvar) + 1 + strlen(var->value) + 1);
				if (!entry) {
					ast_log_chan(NULL, LOG_ERROR, "Out of memory creating filter entry for CEL variable '%s' in table '%s' on connection '%s'\n", celvar, table, connection);
					res = -1;
					break;
				}
		...
		while ((res = SQLFetch(stmt)) != SQL_NO_DATA && res != SQL_ERROR) {
			...
			ast_verb_chan(NULL, 10, "Found %s column with type %hd with len %ld, octetlen %ld, and numlen (%hd,%hd)\n", entry->name, entry->type, (long) entry->size, (long) entry->octetlen, entry->decimals, entry->radix);
...
static void odbc_log(struct ast_event *event)
{
	...
	if (AST_RWLIST_RDLOCK(&odbc_tables)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to lock table list.  Insert CEL(s) failed.\n");
		ast_free(sql);
		ast_free(sql2);
		return;
	}
	...
	AST_LIST_TRAVERSE(&odbc_tables, tableptr, list) {
		...
		if (!(obj = ast_odbc_request_obj(tableptr->connection, 0))) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to retrieve database handle for '%s:%s'.  CEL failed: %s\n", tableptr->connection, tableptr->table, ast_str_buffer(sql));
			continue;
		}
		...
		AST_LIST_TRAVERSE(&(tableptr->columns), entry, list) {
			...
			if (colptr && !unknown) {
				...
				if (entry->filtervalue && strcasecmp(colptr, entry->filtervalue) != 0) {
					ast_verb_chan(NULL, 4, "CEL column '%s' with value '%s' does not match filter of"
					...
					if (ast_strlen_zero(colptr)) {
						...
						if (strcasecmp(entry->name, "eventdate") == 0) {
							...
							if (sscanf(colptr, "%4d-%2d-%2d", &year, &month, &day) != 3 || year <= 0 ||
								...
								ast_log_chan(NULL, LOG_WARNING, "CEL variable %s is not a valid date ('%s').\n", entry->name, colptr);
								continue;
							}
					...
					if (ast_strlen_zero(colptr)) {
						...
						if (strcasecmp(entry->name, "eventdate") == 0) {
							...
							if ((count != 2 && count != 3) || hour < 0 || hour > 23 || minute < 0 || minute > 59 || second < 0 || second > (tableptr->allowleapsec ? 60 : 59)) {
								ast_log_chan(NULL, LOG_WARNING, "CEL variable %s is not a valid time ('%s').\n", entry->name, colptr);
								continue;
							}
						}
						...
						if (datefield) {
							...
							if (strcasecmp(entry->name, "eventdate") == 0) {
								...
								if ((count != 3 && count != 5 && count != 6) || year <= 0 ||
									...
									ast_log_chan(NULL, LOG_WARNING, "CEL variable %s is not a valid timestamp ('%s').\n", entry->name, colptr);
									continue;
								}
				...
				case SQL_INTEGER:
					{
						...
						if (sscanf(colptr, "%30d", &integer) != 1) {
							ast_log_chan(NULL, LOG_WARNING, "CEL variable %s is not an integer.\n", entry->name);
							continue;
						}
				...
				case SQL_BIGINT:
					{
						...
						if ((ret = sscanf(colptr, "%30lld", &integer)) != 1) {
							ast_log_chan(NULL, LOG_WARNING, "CEL variable %s is not an integer. (%d - '%s')\n", entry->name, ret, colptr);
							continue;
						}
				...
				case SQL_DECIMAL:
					{
						...
						if (sscanf(colptr, "%30lf", &number) != 1) {
							ast_log_chan(NULL, LOG_WARNING, "CEL variable %s is not an numeric type.\n", entry->name);
							continue;
						}
		...
		if (rows == 0) {
			ast_log_chan(NULL, LOG_WARNING, "Insert failed on '%s:%s'.  CEL failed: %s\n", tableptr->connection, tableptr->table, ast_str_buffer(sql));
		}
early_release:
		ast_odbc_release_obj(obj);
	}
...
static int load_module(void)
{
	...
	if (ast_cel_backend_register(ODBC_BACKEND_NAME, odbc_log)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to subscribe to CEL events\n");
		free_config();
		return AST_MODULE_LOAD_DECLINE;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "ODBC CEL backend",
