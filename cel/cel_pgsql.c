	...
	do { \
		...
		if (ast_str_strlen(var_sql) + size + 1 > ast_str_size(var_sql)) { \
			if (ast_str_make_space(&var_sql, ((ast_str_size(var_sql) + size + 3) / 512 + 1) * 512) != 0) { \
				ast_log_chan(NULL, LOG_ERROR, "Unable to allocate sufficient memory.  Insert CEL '%s:%s' failed.\n", pghostname, table); \
				ast_free(sql); \
				ast_free(sql2); \
...
static void pgsql_reconnect(void)
{
	...
	if (!conn_info) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to allocate memory for connection string.\n");
		return;
	}
...
static void pgsql_log(struct ast_event *event)
{
	...
	if ((!connected) && pghostname && pgdbuser && pgpassword && pgdbname) {
		...
		if (PQstatus(conn) != CONNECTION_BAD) {
			...
			ast_log_chan(NULL, LOG_ERROR, "cel_pgsql: Unable to connect to database server %s.  Calls will not be logged!\n", pghostname);
			ast_log_chan(NULL, LOG_ERROR, "cel_pgsql: Reason: %s\n", pgerror);
			PQfinish(conn);
			conn = NULL;
		}
	}
...
#define SEP (first ? "" : ",")
		...
		if (PQstatus(conn) == CONNECTION_OK) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Connection was lost... attempting to reconnect.\n");
			PQreset(conn);
			if (PQstatus(conn) == CONNECTION_OK) {
				ast_log_chan(NULL, LOG_NOTICE, "Connection reestablished.\n");
				connected = 1;
			} else {
				pgerror = PQerrorMessage(conn);
				ast_log_chan(NULL, LOG_ERROR, "Unable to reconnect to database server %s. Calls will not be logged!\n", pghostname);
				ast_log_chan(NULL, LOG_ERROR, "Reason: %s\n", pgerror);
				PQfinish(conn);
				conn = NULL;
				connected = 0;
				goto ast_log_cleanup;
		...
		if (PQresultStatus(result) != PGRES_COMMAND_OK) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Failed to insert call detail record into database!\n");
			ast_log_chan(NULL, LOG_WARNING, "Reason: %s\n", pgerror);
			ast_log_chan(NULL, LOG_WARNING, "Connection may have been lost... attempting to reconnect.\n");
			PQreset(conn);
			if (PQstatus(conn) == CONNECTION_OK) {
				ast_log_chan(NULL, LOG_NOTICE, "Connection reestablished.\n");
				connected = 1;
				...
				if (PQresultStatus(result) != PGRES_COMMAND_OK) {
					pgerror = PQresultErrorMessage(result);
					ast_log_chan(NULL, LOG_ERROR, "HARD ERROR!  Attempted reconnection failed.  DROPPING CALL RECORD!\n");
					ast_log_chan(NULL, LOG_ERROR, "Reason: %s\n", pgerror);
				}
			}
		}
...
static int process_my_load_module(struct ast_config *cfg)
{
	...
	if (!(var = ast_variable_browse(cfg, "global"))) {
		ast_log_chan(NULL, LOG_WARNING,"CEL pgsql config file missing global section.\n");
		return AST_MODULE_LOAD_DECLINE;
	}
	if (!(tmp = ast_variable_retrieve(cfg,"global","hostname"))) {
		ast_log_chan(NULL, LOG_WARNING,"PostgreSQL server hostname not specified.  Assuming unix socket connection\n");
		tmp = "";	/* connect via UNIX-socket by default */
	}
	if (pghostname)
		ast_free(pghostname);
	if (!(pghostname = ast_strdup(tmp))) {
		ast_log_chan(NULL, LOG_WARNING,"PostgreSQL Ran out of memory copying host info\n");
		return AST_MODULE_LOAD_DECLINE;
	}
	if (!(tmp = ast_variable_retrieve(cfg, "global", "dbname"))) {
		ast_log_chan(NULL, LOG_WARNING,"PostgreSQL database not specified.  Assuming gabpbx\n");
		tmp = "gabpbxceldb";
	}
	if (pgdbname)
		ast_free(pgdbname);
	if (!(pgdbname = ast_strdup(tmp))) {
		ast_log_chan(NULL, LOG_WARNING,"PostgreSQL Ran out of memory copying dbname info\n");
		return AST_MODULE_LOAD_DECLINE;
	}
	if (!(tmp = ast_variable_retrieve(cfg, "global", "user"))) {
		ast_log_chan(NULL, LOG_WARNING,"PostgreSQL database user not specified.  Assuming gabpbx\n");
		tmp = "gabpbx";
	}
	if (pgdbuser)
		ast_free(pgdbuser);
	if (!(pgdbuser = ast_strdup(tmp))) {
		ast_log_chan(NULL, LOG_WARNING,"PostgreSQL Ran out of memory copying user info\n");
		return AST_MODULE_LOAD_DECLINE;
	}
	if (!(tmp = ast_variable_retrieve(cfg, "global", "password"))) {
		ast_log_chan(NULL, LOG_WARNING, "PostgreSQL database password not specified.  Assuming blank\n");
		tmp = "";
	}
	if (pgpassword)
		ast_free(pgpassword);
	if (!(pgpassword = ast_strdup(tmp))) {
		ast_log_chan(NULL, LOG_WARNING,"PostgreSQL Ran out of memory copying password info\n");
		return AST_MODULE_LOAD_DECLINE;
	}
	...
	if (!(pgappname = ast_strdup(tmp))) {
		ast_log_chan(NULL, LOG_WARNING,"PostgreSQL Ran out of memory copying appname info\n");
		return AST_MODULE_LOAD_DECLINE;
	}

	if (!(tmp = ast_variable_retrieve(cfg,"global","port"))) {
		ast_log_chan(NULL, LOG_WARNING,"PostgreSQL database port not specified.  Using default 5432.\n");
		tmp = "5432";
	}
	if (pgdbport)
		ast_free(pgdbport);
	if (!(pgdbport = ast_strdup(tmp))) {
		ast_log_chan(NULL, LOG_WARNING,"PostgreSQL Ran out of memory copying port info\n");
		return AST_MODULE_LOAD_DECLINE;
	}
	if (!(tmp = ast_variable_retrieve(cfg, "global", "table"))) {
		ast_log_chan(NULL, LOG_WARNING,"CEL table not specified.  Assuming cel\n");
		tmp = "cel";
	}
	if (table)
		ast_free(table);
	...
	if (!(schema = ast_strdup(tmp))) {
		ast_log_chan(NULL, LOG_WARNING,"PostgreSQL Ran out of memory copying schema info\n");
		return AST_MODULE_LOAD_DECLINE;
	}
	if (DEBUG_ATLEAST(3)) {
		if (ast_strlen_zero(pghostname)) {
			ast_log_chan(NULL, LOG_DEBUG, "cel_pgsql: using default unix socket\n");
		} else {
			ast_log_chan(NULL, LOG_DEBUG, "cel_pgsql: got hostname of %s\n", pghostname);
		}
		ast_log_chan(NULL, LOG_DEBUG, "cel_pgsql: got port of %s\n", pgdbport);
		ast_log_chan(NULL, LOG_DEBUG, "cel_pgsql: got user of %s\n", pgdbuser);
		ast_log_chan(NULL, LOG_DEBUG, "cel_pgsql: got dbname of %s\n", pgdbname);
		ast_log_chan(NULL, LOG_DEBUG, "cel_pgsql: got password of %s\n", pgpassword);
		ast_log_chan(NULL, LOG_DEBUG, "cel_pgsql: got sql table name of %s\n", table);
		ast_log_chan(NULL, LOG_DEBUG, "cel_pgsql: got show_user_defined of %s\n",
	...
	if (PQstatus(conn) != CONNECTION_BAD) {
		...
		if (PQresultStatus(result) != PGRES_TUPLES_OK) {
			pgerror = PQresultErrorMessage(result);
			ast_log_chan(NULL, LOG_ERROR, "Failed to query database columns: %s\n", pgerror);
			PQclear(result);
			unload_module();
			return AST_MODULE_LOAD_DECLINE;
		}
...
static int my_load_module(int reload)
{
	...
	if ((cfg = ast_config_load(config, config_flags)) == NULL || cfg == CONFIG_STATUS_FILEINVALID) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to load config for PostgreSQL CEL's: %s\n", config);
		return AST_MODULE_LOAD_DECLINE;
	...
	if (ast_cel_backend_register(PGSQL_BACKEND_NAME, pgsql_log)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to subscribe to CEL events for pgsql\n");
		return AST_MODULE_LOAD_DECLINE;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "PostgreSQL CEL Backend",
