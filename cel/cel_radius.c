...
static void radius_log(struct ast_event *event)
{
	...
	if (result != OK_RC) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to record Radius CEL record!\n");
	}
...
static int load_module(void)
{
	...
	if (!(rh = rc_read_config(radiuscfg))) {
		ast_log_chan(NULL, LOG_NOTICE, "Cannot load radiusclient-ng configuration file %s.\n", radiuscfg);
		return AST_MODULE_LOAD_DECLINE;
	}

	/* read radiusclient-ng dictionaries */
	if (rc_read_dictionary(rh, rc_conf_str(rh, "dictionary"))) {
		ast_log_chan(NULL, LOG_NOTICE, "Cannot load radiusclient-ng dictionary file.\n");
		rc_destroy(rh);
		rh = NULL;
		return AST_MODULE_LOAD_DECLINE;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "RADIUS CEL Backend",
