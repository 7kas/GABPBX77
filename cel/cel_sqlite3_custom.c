...
static int load_column_config(const char *tmp)
{
	...
	if (ast_strlen_zero(tmp)) {
		ast_log_chan(NULL, LOG_WARNING, "Column names not specified. Module not loaded.\n");
		return -1;
	}
	if (!(column_string = ast_str_create(1024))) {
		ast_log_chan(NULL, LOG_ERROR, "Out of memory creating temporary buffer for column list for table '%s.'\n", table);
		return -1;
	}
	...
	while ((col = strsep(&cols, ","))) {
		...
		if (!escaped) {
			ast_log_chan(NULL, LOG_ERROR, "Out of memory creating entry for column '%s' in table '%s.'\n", col, table);
			ast_free(column_string);
			ast_free(save);
			return -1;
		}
	...
	if (!(columns = ast_strdup(ast_str_buffer(column_string)))) {
		ast_log_chan(NULL, LOG_ERROR, "Out of memory copying columns string for table '%s.'\n", table);
		ast_free(column_string);
		ast_free(save);
		return -1;
	}
...
static int load_values_config(const char *tmp)
{
	...
	if (ast_strlen_zero(tmp)) {
		ast_log_chan(NULL, LOG_WARNING, "Values not specified. Module not loaded.\n");
		return -1;
	}
	if (!(save = vals = ast_strdup(tmp))) {
		ast_log_chan(NULL, LOG_ERROR, "Out of memory creating temporary buffer for value '%s'\n", tmp);
		return -1;
	}
	while ((val = strsep(&vals, ","))) {
		...
		if (!value) {
			ast_log_chan(NULL, LOG_ERROR, "Out of memory creating entry for value '%s'\n", val);
			ast_free(save);
			return -1;
		}
...
static int load_config(int reload)
{
	...
	if ((cfg = ast_config_load(config_file, config_flags)) == CONFIG_STATUS_FILEMISSING || cfg == CONFIG_STATUS_FILEINVALID) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to %sload configuration file. %s\n",
	...
	if (!ast_strlen_zero(tmp = ast_variable_retrieve(cfg, "master", "table"))) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Table name not specified.  Assuming cel.\n");
		strcpy(table, "cel");
	}
	...
	if ((tmp = ast_variable_retrieve(cfg, "master", "busy_timeout")) != NULL) {
		if (ast_parse_arg(tmp, PARSE_INT32|PARSE_DEFAULT, &busy_timeout, 1000) != 0) {
			ast_log_chan(NULL, LOG_WARNING, "Invalid busy_timeout value '%s' specified. Using 1000 instead.\n", tmp);
		}
	} else {
		busy_timeout = 1000;
	}
	...
	ast_verb_chan(NULL, 3, "Logging CEL records to table '%s' in 'master.db'\n", table);
...
static void write_cel(struct ast_event *event)
{
	...
	{ /* Make it obvious that only sql should be used outside of this block */
		...
		if (!dummy) {
			ast_log_chan(NULL, LOG_ERROR, "Unable to fabricate channel from CEL event.\n");
	...
	if (sqlite3_exec(db, sql, NULL, NULL, &error) != SQLITE_OK) {
		ast_log_chan(NULL, LOG_ERROR, "%s. SQL: %s.\n", error, sql);
		sqlite3_free(error);
	}
...
static int load_module(void)
{
	...
	if (res != SQLITE_OK) {
		ast_log_chan(NULL, LOG_ERROR, "Could not open database %s.\n", filename);
		free_config();
		return AST_MODULE_LOAD_DECLINE;
	}
	...
	if (res != SQLITE_OK) {
		...
		sqlite3_free(sql);
		if (res != SQLITE_OK) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to create table '%s': %s.\n", table, error);
			sqlite3_free(error);
			free_config();
			return AST_MODULE_LOAD_DECLINE;
		}
	...
	if (ast_cel_backend_register(SQLITE_BACKEND_NAME, write_cel)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to register custom SQLite3 CEL handling\n");
		free_config();
		return AST_MODULE_LOAD_DECLINE;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "SQLite3 Custom CEL Module",
