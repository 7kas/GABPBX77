...
retry:
	...
	if (!settings->connected) {
		ast_log_chan(NULL, LOG_NOTICE, "Attempting to reconnect to %s (Attempt %d)\n", settings->connection, attempt);
		if (mssql_connect()) {
			/* Connect failed */
			if (attempt++ < 3) {
				goto retry;
	...
	if (erc == FAIL) {
		if (attempt++ < 3) {
			ast_log_chan(NULL, LOG_NOTICE, "Failed to build INSERT statement, retrying...\n");
			mssql_disconnect();
			goto retry;
		} else {
			ast_log_chan(NULL, LOG_ERROR, "Failed to build INSERT statement, no CEL was logged.\n");
			goto done;
		}
	}
	...
	if (dbsqlexec(settings->dbproc) == FAIL) {
		if (attempt++ < 3) {
			ast_log_chan(NULL, LOG_NOTICE, "Failed to execute INSERT statement, retrying...\n");
			mssql_disconnect();
			goto retry;
		} else {
			ast_log_chan(NULL, LOG_ERROR, "Failed to execute INSERT statement, no CEL was logged.\n");
			goto done;
		}
	}
...
static int mssql_connect(void)
{
	...
	if ((login = dblogin()) == NULL) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to allocate login structure for db-lib\n");
		return -1;
	}
	...
	if ((settings->dbproc = dbopen(login, (char *) settings->connection)) == NULL) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to connect to %s\n", settings->connection);
		dbloginfree(login);
		return -1;
	}
	...
	if (dbuse(settings->dbproc, (char *) settings->database) == FAIL) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to select database %s\n", settings->database);
		goto failed;
	}

	if (execute_and_consume(settings->dbproc, "SELECT 1 FROM [%s]", settings->table)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to find table '%s'\n", settings->table);
		goto failed;
	}
...
static int tds_message_handler(DBPROCESS *dbproc, DBINT msgno, int msgstate, int severity, char *msgtext, char *srvname, char *procname, int line)
{
	...
	ast_log_chan(NULL, LOG_NOTICE, "%s\n", msgtext);
...
static int tds_load_module(int reload)
{
	...
	if (!cfg || cfg == CONFIG_STATUS_FILEINVALID) {
		ast_log_chan(NULL, LOG_NOTICE, "Unable to load TDS config for CELs: %s\n", config);
		return 0;
	...
	if (!ast_variable_browse(cfg, "global")) {
		...
		ast_log_chan(NULL, LOG_NOTICE, "cel_tds has no global category, nothing to configure.\n");
		return 0;
	}
	...
	if (ptr) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Failed to connect: Database connection name not specified.\n");
		goto failed;
	}
	...
	if (ptr) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Failed to connect: Database dbname not specified.\n");
		goto failed;
	}
	...
	if (ptr) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Failed to connect: Database dbuser not specified.\n");
		goto failed;
	}
	...
	if (ptr) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Failed to connect: Database password not specified.\n");
		goto failed;
	}
	...
	if (ptr) {
		...
		ast_log_chan(NULL, LOG_NOTICE, "Table name not specified, using 'cel' by default.\n");
...
{
	if (dbinit() == FAIL) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to initialize FreeTDS db-lib\n");
	...
	if (!tds_load_module(0)) {
		...
		settings = NULL;
		dbexit();
		ast_log_chan(NULL, LOG_WARNING,"cel_tds module had config problems; declining load\n");
		return AST_MODULE_LOAD_DECLINE;
	}

	/* Register MSSQL CEL handler */
	if (ast_cel_backend_register(TDS_BACKEND_NAME, tds_log)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to register MSSQL CEL handling\n");
		ast_string_field_free_memory(settings);
		ast_free(settings);
		settings = NULL;
		dbexit();
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "FreeTDS CEL Backend",
