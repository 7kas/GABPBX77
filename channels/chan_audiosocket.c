	...
	const struct ast_channel *requestor, const char *data, int *cause)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_ERROR, "Destination is required for the 'AudioSocket' channel\n");
		goto failure;
	}
	...
	if (ast_sockaddr_resolve_first_af
		...
		ast_log_chan(NULL, LOG_ERROR, "Destination '%s' could not be parsed\n", args.destination);
		goto failure;
	}

	if (ast_strlen_zero(args.idStr)) {
		ast_log_chan(NULL, LOG_ERROR, "UUID is required for the 'AudioSocket' channel\n");
		goto failure;
	}
	...
	if (!ast_strlen_zero(args.options)
		...
		ast_log_chan(NULL, LOG_ERROR, "'AudioSocket' channel options '%s' parse error\n",
			args.options);
		goto failure;
	}
	...
	if (ast_test_flag(&opts, OPT_AUDIOSOCKET_CODEC)
		...
		if (!fmt) {
			ast_log_chan(NULL, LOG_ERROR, "Codec '%s' not found for AudioSocket connection to '%s'\n",
		...
		if (!fmt) {
			ast_log_chan(NULL, LOG_ERROR, "No codec available for AudioSocket connection to '%s'\n",
				args.destination);
			goto failure;
		}
	}
	...
	if (!instance) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to allocate AudioSocket channel pvt\n");
		goto failure;
	}
...
static int load_module(void)
{
	...
	if (ast_channel_register(&audiosocket_channel_tech)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to register channel class AudioSocket");
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "AudioSocket Channel",
