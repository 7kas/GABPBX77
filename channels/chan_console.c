 ...
 *
 * 320 samples (20 ms) is the most common frame size in GABpbx.  So, the code
 * in this module reads 320 sample frames from the portaudio stream and queues
 * them up on the GABpbx channel.  Frames of any size can be written to a
 ...
 * be read from the GABpbx channel.
 */
static void *stream_monitor(void *data)
{
	...
	for (;;) {
		...
		if (res == paNoError) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Console ReadStream failed: %s\n", Pa_GetErrorText(res));
		}
	}
...
static int open_stream(struct console_pvt *pvt)
{
	...
	if (!strcasecmp(pvt->input_device, "default") &&
		...
		if (input_params.device == paNoDevice)
			ast_log_chan(NULL, LOG_ERROR, "No input device found for console device '%s'\n", pvt->name);
		if (output_params.device == paNoDevice)
			ast_log_chan(NULL, LOG_ERROR, "No output device found for console device '%s'\n", pvt->name);
...
static int start_stream(struct console_pvt *pvt)
{
	...
	res = open_stream(pvt);
	if (res != paNoError) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to open stream - (%d) %s\n",
	...
	if (res != paNoError) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to start stream - (%d) %s\n",
			res, Pa_GetErrorText(res));
		ret_val = -1;
		goto return_unlock;
	}
	...
	if (ast_pthread_create_background(&pvt->thread, NULL, stream_monitor, pvt)) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to start stream monitor thread\n");
		ret_val = -1;
	}
...
static struct ast_channel *console_request(const char *type, struct ast_format_cap *cap, const struct ast_assigned_ids *assignedids, const struct ast_channel *requestor, const char *data, int *cause)
{
	...
	if (!(pvt = find_pvt(data))) {
		ast_log_chan(NULL, LOG_ERROR, "Console device '%s' not found\n", data);
		return NULL;
	}

	if (!(ast_format_cap_iscompatible(cap, console_tech.capabilities))) {
		struct ast_str *cap_buf = ast_str_alloca(AST_FORMAT_CAP_NAMES_LEN);
		ast_log_chan(NULL, LOG_NOTICE, "Channel requested with unsupported format(s): '%s'\n",
			ast_format_cap_get_names(cap, &cap_buf));
		goto return_unref;
	}

	if (pvt->owner) {
		ast_log_chan(NULL, LOG_NOTICE, "Console channel already active!\n");
	...
	if (!chan)
		ast_log_chan(NULL, LOG_WARNING, "Unable to create new Console channel!\n");
...
static int console_digit_begin(struct ast_channel *c, char digit)
{
	ast_verb_chan(NULL, 1, V_BEGIN "Console Received Beginning of Digit %c" V_END, digit);
...
static int console_digit_end(struct ast_channel *c, char digit, unsigned int duration)
{
	ast_verb_chan(NULL, 1, V_BEGIN "Console Received End of Digit %c (duration %u)" V_END,
		digit, duration);
...
static int console_text(struct ast_channel *c, const char *text)
{
	ast_verb_chan(NULL, 1, V_BEGIN "Console Received Text '%s'" V_END, text);
...
static int console_hangup(struct ast_channel *c)
{
	...
	ast_verb_chan(NULL, 1, V_BEGIN "Hangup on Console" V_END);
...
static int console_answer(struct ast_channel *c)
{
	...
	ast_verb_chan(NULL, 1, V_BEGIN "Call from Console has been Answered" V_END);
...
static int console_call(struct ast_channel *c, const char *dest, int timeout)
{
	...
	ast_verb_chan(NULL, 1, V_BEGIN "Call to device '%s' on console from '%s' <%s>" V_END,
		dest,
	...
	if (pvt->autoanswer) {
		pvt->hookstate = 1;
		console_pvt_unlock(pvt);
		ast_verb_chan(NULL, 1, V_BEGIN "Auto-answered" V_END);
		ctrl = AST_CONTROL_ANSWER;
	} else {
		console_pvt_unlock(pvt);
		ast_verb_chan(NULL, 1, V_BEGIN "Type 'console answer' to answer, or use the 'autoanswer' option "
...
static int console_indicate(struct ast_channel *chan, int cond, const void *data, size_t datalen)
{
	...
	case AST_CONTROL_HOLD:
		ast_verb_chan(NULL, 1, V_BEGIN "Console Has Been Placed on Hold" V_END);
		ast_moh_start(chan, data, pvt->mohinterpret);
		break;
	case AST_CONTROL_UNHOLD:
		ast_verb_chan(NULL, 1, V_BEGIN "Console Has Been Retrieved from Hold" V_END);
		ast_moh_stop(chan);
		break;
	default:
		ast_log_chan(NULL, LOG_WARNING, "Don't know how to display condition %d on %s\n",
...
static char *cli_console_mute(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	ast_verb_chan(NULL, 1, V_BEGIN "The Console is now %s" V_END,
...
{
	if (pvt == &globals) {
		ast_log_chan(NULL, LOG_ERROR, "active is only valid as a per-device setting\n");
		return;
	}
...
static char *cli_console_active(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	case CLI_INIT:
		...
		e->usage =
			...
			"       Set or show the active console device for the GABpbx CLI.\n";
		return NULL;
	case CLI_GENERATE:
		if (a->pos == e->args) {
			struct ao2_iterator i;
...
static int load_config(int reload)
{
	...
	if (!(cfg = ast_config_load(config_file, config_flags))) {
		ast_log_chan(NULL, LOG_NOTICE, "Unable to open configuration file %s!\n", config_file);
		return -1;
	} else if (cfg == CONFIG_STATUS_FILEINVALID) {
		ast_log_chan(NULL, LOG_NOTICE, "Config file %s has an invalid format\n", config_file);
		return -1;
	}
...
static int load_module(void)
{
	...
	res = Pa_Initialize();
	if (res != paNoError) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to initialize audio system - (%d) %s\n",
			res, Pa_GetErrorText(res));
		goto return_error_pa_init;
	}

	if (ast_channel_register(&console_tech)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to register channel type 'Console'\n");
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "Console Channel Driver",
