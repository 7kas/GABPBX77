		...
		<managerEventInstance class="EVENT_FLAG_SYSTEM">
			...
			<syntax>
				<parameter name="DAHDIChannel">
					...
					<note><para>This is not an GABpbx channel identifier.</para></note>
				</parameter>
			</syntax>
...
static int analogsub_to_dahdisub(enum analog_sub analogsub)
{
	...
	case ANALOG_SUB_THREEWAY:
		...
		break;
	default:
		ast_log_chan(NULL, LOG_ERROR, "Unidentified sub!\n");
		index = SUB_REAL;
	}
...
static void pri_queue_for_destruction(struct sig_pri_span *pri)
{
	...
	if (!entry) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Failed allocating memory for a doomed_pri.\n");
		AST_LIST_UNLOCK(&doomed_pris);
		return;
	}
	entry->pri = pri;
...
static int dahdi_dial_str(struct dahdi_pvt *pvt, int operation, const char *dial_str)
{
	...
	if (res) {
		ast_log_chan(NULL, LOG_WARNING, "Channel %d: Couldn't dial '%s': %s\n",
...
static int my_start_cid_detect(void *pvt, int cid_signalling)
{
	...
	if (!p->cs) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to alloc callerid\n");
		return -1;
	}
	bump_gains(p);
...
static int my_get_callerid(void *pvt, char *namebuf, char *numbuf, enum analog_event *ev, size_t timeout)
{
	...
	if (poller.revents & POLLIN) {
		...
		if (res < 0) {
			ast_log_chan(NULL, LOG_WARNING, "read returned error: %s\n", strerror(errno));
			return -1;
		}
		...
		if (res < 0) {
			 ...
			 */
			ast_log_chan(NULL, LOG_WARNING, "Failed to decode CallerID\n");
			return -1;
		}
...
static int my_distinctive_ring(struct ast_channel *chan, void *pvt, int idx, int *ringdata)
{
	...
	if ((checkaftercid && distinctiveringaftercid) || !checkaftercid) {
		...
		if (checkaftercid && distinctiveringaftercid) {
			ast_verb_chan(NULL, 3, "Detecting post-CID distinctive ring\n");
		}
		...
		for (;;) {
			...
			if (res) {
				ast_log_chan(NULL, LOG_WARNING, "I/O MUX failed: %s\n", strerror(errno));
				ast_hangup(chan);
				return 1;
			}
	...
	ast_verb_chan(NULL, 3, "Detected ring pattern: %d,%d,%d\n", ringdata[0], ringdata[1], ringdata[2]);
	matched_context = p->defcontext;
	for (counter = 0; counter < 3; counter++) {
		int range = p->drings.ringnum[counter].range;

		distMatches = 0;
		ast_verb_chan(NULL, 3, "Checking %d,%d,%d with +/- %d range\n",
		...
		for (counter1 = 0; counter1 < 3; counter1++) {
			...
			if (ring == -1) {
				ast_verb_chan(NULL, 3, "Pattern ignore (-1) detected, so matching pattern %d regardless.\n",
					ringdata[counter1]);
				distMatches++;
			} else if (ring - range <= ringdata[counter1] && ringdata[counter1] <= ring + range) {
				ast_verb_chan(NULL, 3, "Ring pattern %d is in range: %d to %d\n",
		...
		if (distMatches == 3) {
			...
			ast_verb_chan(NULL, 3, "Matched Distinctive Ring context %s\n", matched_context);
			break;
		}
	}
...
static int my_callwait(void *pvt)
{
	...
	if (p->cidspill) {
		ast_log_chan(NULL, LOG_WARNING, "Spill already exists?!?\n");
		ast_free(p->cidspill);
	}
...
static int my_send_callerid(void *pvt, int cwcid, struct ast_party_caller *caller)
{
	...
	if (p->cidspill) {
		ast_log_chan(NULL, LOG_WARNING, "cidspill already exists??\n");
		ast_free(p->cidspill);
	}
	...
	if ((p->cidspill = ast_malloc(MAX_CALLERID_SIZE))) {
		...
		if (cwcid == 0) {
			...
			ast_verb_chan(NULL, 3, "CPE supports Call Waiting Caller*ID.  Sending '%s/%s'\n",
...
static int my_dsp_set_digitmode(void *pvt, enum analog_dsp_digitmode mode)
{
	...
	if (p->channel == CHAN_PSEUDO)
		ast_log_chan(NULL, LOG_ERROR, "You have assumed incorrectly sir!\n");
	...
	if (mode == ANALOG_DIGITMODE_DTMF) {
		...
		if (!p->dsp) {
			p->dsp = ast_dsp_new();
			if (!p->dsp) {
				ast_log_chan(NULL, LOG_ERROR, "Unable to allocate DSP\n");
				return -1;
			}
		}
...
static int my_wink(void *pvt, enum analog_sub sub)
{
	...
	if (index != SUB_REAL) {
		ast_log_chan(NULL, LOG_ERROR, "We used a sub other than SUB_REAL (incorrect assumption sir)\n");
	}
			...
			if ((p->callprogress & CALLPROGRESS_FAX) && !p->faxhandled) {
				...
				if (p->usefaxbuffers && !p->bufferoverrideinuse) {
					...
					if ((res = ioctl(p->subs[idx].dfd, DAHDI_SET_BUFINFO, &bi)) < 0) {
						ast_log_chan(NULL, LOG_WARNING, "Channel '%s' unable to set buffer policy, reason: %s\n", ast_channel_name(ast), strerror(errno));
					} else {
				...
				if (strcmp(ast_channel_exten(ast), "fax")) {
					...
					if (ast_exists_extension(ast, target_context, "fax", 1,
						...
						ast_verb_chan(NULL, 3, "Redirecting %s to fax extension\n", ast_channel_name(ast));
						/* Save the DID/DNIS when we transfer the fax call to a "fax" extension */
						pbx_builtin_setvar_helper(ast, "FAXEXTEN", ast_channel_exten(ast));
						if (ast_async_goto(ast, target_context, "fax", 1))
							ast_log_chan(NULL, LOG_WARNING, "Failed to async goto '%s' into fax of '%s'\n", ast_channel_name(ast), target_context);
					} else {
						ast_log_chan(NULL, LOG_NOTICE, "Fax detected, but no fax extension\n");
					}
...
static void my_set_cadence(void *pvt, int *cid_rings, struct ast_channel *ast)
{
	...
	if ((p->distinctivering > 0) && (p->distinctivering <= num_cadence)) {
		if (ioctl(p->subs[SUB_REAL].dfd, DAHDI_SETCADENCE, &cadences[p->distinctivering - 1]))
			ast_log_chan(NULL, LOG_WARNING, "Unable to set distinctive ring cadence %d on '%s': %s\n", p->distinctivering, ast_channel_name(ast), strerror(errno));
		*cid_rings = cidrings[p->distinctivering - 1];
	} else {
		if (p->distinctivering > 0) {
			ast_log_chan(NULL, LOG_WARNING, "Cadence %d is not defined, falling back to default ring cadence\n", p->distinctivering);
		}
		if (ioctl(p->subs[SUB_REAL].dfd, DAHDI_SETCADENCE, NULL))
			ast_log_chan(NULL, LOG_WARNING, "Unable to reset default ring on '%s': %s\n", ast_channel_name(ast), strerror(errno));
...
static void my_all_subchannels_hungup(void *pvt)
{
	...
	if (res < 0)
		ast_log_chan(NULL, LOG_WARNING, "Unable to set law on channel %d to default: %s\n", p->channel, strerror(errno));
	...
	const struct ast_channel *requestor)
{
	...
	default:
		...
		if (ioctl(p->subs[SUB_REAL].dfd, DAHDI_AUDIOMODE, &audio) == -1) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to set audio mode on channel %d to %d: %s\n",
...
static void my_pri_ss7_open_media(void *p)
{
	...
	if (res < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to enable audio mode on channel %d (%s)\n",
	...
	if (res < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to set law on channel %d\n", pvt->channel);
	}
	...
	if (res < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to set gains on channel %d\n", pvt->channel);
	}
...
static int my_is_off_hook(void *pvt)
{
	...
	if (res) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to check hook state on channel %d: %s\n", p->channel, strerror(errno));
	}
...
static struct dahdi_pvt *fxo_pvt(struct ast_channel *chan)
{
	...
	if (!(dahdip.sigtype & __DAHDI_SIG_FXO)) {
		ast_log_chan(NULL, LOG_WARNING, "%s is not FXO signalled\n", ast_channel_name(chan));
		return NULL;
	}

	pvt = ast_channel_tech_pvt(chan);
	if (!dahdi_analog_lib_handles(pvt->sig, 0, 0)) {
		ast_log_chan(NULL, LOG_WARNING, "Channel signalling is not analog");
		return NULL;
	}
...
static int polarity_write(struct ast_channel *chan, const char *cmd, char *data, const char *value)
{
	...
	if (polarity != POLARITY_IDLE && polarity != POLARITY_REV) {
		ast_log_chan(NULL, LOG_WARNING, "Invalid polarity: '%s'\n", value);
		return -1;
	}
...
static int my_dial_digits(void *pvt, enum analog_sub sub, struct analog_dialoperation *dop)
{
	...
	if (dop->op != ANALOG_DIAL_OP_REPLACE) {
		ast_log_chan(NULL, LOG_ERROR, "Fix the dial_digits callback!\n");
		return -1;
	}

	if (sub != ANALOG_SUB_REAL) {
		ast_log_chan(NULL, LOG_ERROR, "Trying to dial_digits '%s' on channel %d subchannel %u\n",
...
static void my_handle_dchan_exception(struct sig_pri_span *pri, int index)
{
	...
	case DAHDI_EVENT_NOALARM:
		...
		/* Fall through */
	default:
		ast_log_chan(NULL, LOG_NOTICE, "Got DAHDI event: %s (%d) on D-channel of span %d\n",
 ...
 * \since 1.8
 *
 * \param pri GABpbx D channel control structure.
 *
...
static void my_handle_link_exception(struct sig_ss7_linkset *linkset, int which)
{
	...
	if (ioctl(linkset->fds[which], DAHDI_GETEVENT, &event)) {
		ast_log_chan(NULL, LOG_ERROR, "SS7: Error in exception retrieval on span %d/%d!\n",
			linkset->span, which);
		return;
	}
	switch (event) {
	case DAHDI_EVENT_NONE:
		break;
	case DAHDI_EVENT_ALARM:
		ast_log_chan(NULL, LOG_ERROR, "SS7 got event: %s(%d) on span %d/%d\n",
	...
	case DAHDI_EVENT_NOALARM:
		...
		break;
	default:
		ast_log_chan(NULL, LOG_NOTICE, "SS7 got event: %s(%d) on span %d/%d\n",
			event2str(event), event, linkset->span, which);
		break;
	}
}
...
static void my_ss7_set_loopback(void *pvt, int enable)
{
	...
	if (ioctl(p->subs[SUB_REAL].dfd, DAHDI_LOOPBACK, &enable)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to set loopback on channel %d: %s\n", p->channel,
			strerror(errno));
	}
}
#endif	/* defined(HAVE_SS7) */
...
/*!
 * \internal
 * \brief Create a new gabpbx channel structure for SS7.
 * \since 1.8
 *
 ...
 *
 * 1) It generates an internal GABpbx event notifying any other module that
...
static void my_handle_notify_message(struct ast_channel *chan, void *pvt, int cid_flags, int neon_mwievent)
{
	...
	if (neon_mwievent == ANALOG_EVENT_NEONMWI_ACTIVE || cid_flags & CID_MSGWAITING) {
		ast_log_chan(NULL, LOG_NOTICE, "MWI: Channel %d message waiting, mailbox %s\n", p->channel, p->mailbox);
		notify_message(p->mailbox, 1);
	} else if (neon_mwievent == ANALOG_EVENT_NEONMWI_INACTIVE || cid_flags & CID_NOMSGWAITING) {
		ast_log_chan(NULL, LOG_NOTICE, "MWI: Channel %d no message waiting, mailbox %s\n", p->channel, p->mailbox);
...
static void publish_channel_alarm_clear(int channel)
{
	...
	ast_log_chan(NULL, LOG_NOTICE, "Alarm cleared on channel DAHDI/%d\n", channel);
...
static void publish_span_alarm_clear(int span)
{
	...
	ast_log_chan(NULL, LOG_NOTICE, "Alarm cleared on span %d\n", span);
...
static openr2_calling_party_category_t dahdi_r2_get_channel_category(struct ast_channel *c)
{
	...
	if ((cat = openr2_proto_get_category(catstr)) == OR2_CALLING_PARTY_CATEGORY_UNKNOWN) {
		ast_log_chan(NULL, LOG_WARNING, "Invalid category specified '%s' for chan %s, using default %s\n",
...
static void dahdi_r2_on_call_init(openr2_chan_t *r2chan)
{
	...
	if (p->mfcr2call) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Collision of calls on chan %d detected!.\n", openr2_chan_get_number(r2chan));
		return;
	}
	p->mfcr2call = 1;
...
static void dahdi_r2_on_os_error(openr2_chan_t *r2chan, int errorcode)
{
	...
	ast_log_chan(NULL, LOG_ERROR, "OS error on chan %d: %s\n", openr2_chan_get_number(r2chan), strerror(errorcode));
	ast_mutex_lock(&p->lock);
	/* Disconnected? */
	if (errorcode == ENODEV) {
...
{
	struct dahdi_pvt *p = openr2_chan_get_client_data(r2chan);
	ast_log_chan(NULL, LOG_ERROR, "MFC/R2 protocol error on chan %d: %s\n", openr2_chan_get_number(r2chan), openr2_proto_get_error(reason));
	if (p->owner) {
...
static void dahdi_r2_disconnect_call(struct dahdi_pvt *p, openr2_call_disconnect_cause_t cause)
{
	if (openr2_chan_disconnect_call(p->r2chan, cause)) {
		ast_log_chan(NULL, LOG_NOTICE, "Bad! failed to disconnect call on channel %d with reason %s, hope for the best!\n",
...
static void dahdi_r2_on_call_offered(openr2_chan_t *r2chan, const char *ani, const char *dnis, openr2_calling_party_category_t category)
{
	...
	if (!p->mfcr2_allow_collect_calls && category == OR2_CALLING_PARTY_CATEGORY_COLLECT_CALL) {
		ast_log_chan(NULL, LOG_NOTICE, "Rejecting MFC/R2 collect call\n");
	...
	if (!ast_exists_extension(NULL, p->context, p->exten, 1, p->cid_num)) {
		ast_log_chan(NULL, LOG_NOTICE, "MFC/R2 call on channel %d requested non-existent extension '%s' in context '%s'. Rejecting call.\n",
	...
	if (!p->mfcr2_accept_on_offer) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Unable to create PBX channel in DAHDI channel %d\n", p->channel);
...
static void dahdi_r2_write_log(openr2_log_level_t level, char *logmessage)
{
	...
	case OR2_LOG_WARNING:
		ast_log_chan(NULL, LOG_WARNING, "%s", logmessage);
		break;
	case OR2_LOG_ERROR:
		ast_log_chan(NULL, LOG_ERROR, "%s", logmessage);
		break;
	...
	case OR2_LOG_EX_DEBUG:
		...
		break;
	default:
		ast_log_chan(NULL, LOG_WARNING, "We should handle logging level %d here.\n", level);
		ast_debug(1, "%s", logmessage);
		break;
	}
}
...
static void dahdi_r2_on_line_blocked(openr2_chan_t *r2chan)
{
	...
	p->remotelyblocked = 1;
	ast_mutex_unlock(&p->lock);
	ast_log_chan(NULL, LOG_NOTICE, "Far end blocked on chan %d\n", openr2_chan_get_number(r2chan));
}
...
static void dahdi_r2_on_line_idle(openr2_chan_t *r2chan)
{
	...
	p->remotelyblocked = 0;
	ast_mutex_unlock(&p->lock);
	ast_log_chan(NULL, LOG_NOTICE, "Far end unblocked on chan %d\n", openr2_chan_get_number(r2chan));
}
...
static int dahdi_open(char *fn)
{
	...
	if (isnum) {
		chan = atoi(fn);
		if (chan < 1) {
			ast_log_chan(NULL, LOG_WARNING, "Invalid channel number '%s'\n", fn);
			return -1;
		}
	...
	if (fd < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to open '%s': %s\n", fn, strerror(errno));
		return -1;
	}
	if (chan) {
		if (ioctl(fd, DAHDI_SPECIFY, &chan)) {
			...
			close(fd);
			errno = x;
			ast_log_chan(NULL, LOG_WARNING, "Unable to specify channel %d: %s\n", chan, strerror(errno));
			return -1;
		}
	}
	bs = READ_SIZE;
	if (ioctl(fd, DAHDI_SET_BLOCKSIZE, &bs) == -1) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to set blocksize '%d': %s\n", bs,  strerror(errno));
		x = errno;
		close(fd);
		errno = x;
		return -1;
...
static int alloc_sub(struct dahdi_pvt *p, int x)
{
	...
	int res;
	if (p->subs[x].dfd >= 0) {
		ast_log_chan(NULL, LOG_WARNING, "%s subchannel of %d already in use\n", subnames[x], p->channel);
		return -1;
	}

	p->subs[x].dfd = dahdi_open("/dev/dahdi/pseudo");
	if (p->subs[x].dfd <= -1) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to open pseudo channel: %s\n", strerror(errno));
		return -1;
	}
	...
	if (!res) {
		...
		if (res < 0) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to set buffer policy on channel %d: %s\n", x, strerror(errno));
		}
	} else
		ast_log_chan(NULL, LOG_WARNING, "Unable to check buffer policy on channel %d: %s\n", x, strerror(errno));

	if (ioctl(p->subs[x].dfd, DAHDI_CHANNO, &p->subs[x].chan) == 1) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to get channel number for pseudo channel on FD %d: %s\n", p->subs[x].dfd, strerror(errno));
		dahdi_close_sub(p, x);
		p->subs[x].dfd = -1;
		return -1;
	}
...
{
	if (!x) {
		ast_log_chan(NULL, LOG_WARNING, "Trying to unalloc the real channel %d?!?\n", p->channel);
		return -1;
	}
...
static int dahdi_digit_begin(struct ast_channel *chan, char digit)
{
	...
	if (pvt->pulse || ioctl(pvt->subs[SUB_REAL].dfd, DAHDI_SENDTONE, &dtmf)) {
		...
		if (res) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to flush the DAHDI write buffer to send DTMF on channel %d: %s\n",
				pvt->channel, strerror(errno));
		}
...
static int conf_add(struct dahdi_pvt *p, struct dahdi_subchannel *c, int idx, int slavechannel)
{
	...
	if (ioctl(c->dfd, DAHDI_SETCONF, &zi)) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to add %d to conference %d/%d: %s\n", c->dfd, zi.confmode, zi.confno, strerror(errno));
		return -1;
	}
	if (slavechannel < 1) {
		p->confno = zi.confno;
...
static int conf_del(struct dahdi_pvt *p, struct dahdi_subchannel *c, int idx)
{
	...
	memset(&zi, 0, sizeof(zi));
	if (ioctl(c->dfd, DAHDI_SETCONF, &zi)) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to drop %d from conference %d/%d: %s\n", c->dfd, c->curconf.confmode, c->curconf.confno, strerror(errno));
		return -1;
	}
...
static int reset_conf(struct dahdi_pvt *p)
{
	...
	if (p->subs[SUB_REAL].dfd > -1) {
		...
		if (ioctl(p->subs[SUB_REAL].dfd, DAHDI_SETCONF, &zi))
			ast_log_chan(NULL, LOG_WARNING, "Failed to reset conferencing on channel %d: %s\n", p->channel, strerror(errno));
	}
	return 0;
}
...
void dahdi_ec_enable(struct dahdi_pvt *p)
{
	...
	if (p->echocancel.head.tap_length) {
		...
		if (res) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to enable echo cancellation on channel %d (%s)\n", p->channel, strerror(errno));
		} else {
			p->echocanon = 1;
...
static void dahdi_train_ec(struct dahdi_pvt *p)
{
	...
	if (p && p->echocanon && p->echotraining) {
		...
		if (res)
			ast_log_chan(NULL, LOG_WARNING, "Unable to request echo training on channel %d: %s\n", p->channel, strerror(errno));
		else
...
void dahdi_ec_disable(struct dahdi_pvt *p)
{
	...
	if (p->echocanon) {
		...
		if (res)
			ast_log_chan(NULL, LOG_WARNING, "Unable to disable echo cancellation on channel %d: %s\n", p->channel, strerror(errno));
		else
...
static int bump_gains(struct dahdi_pvt *p)
{
	...
	if (res) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to bump gain: %s\n", strerror(errno));
		return -1;
	}
...
static int restore_gains(struct dahdi_pvt *p)
{
	...
	if (res) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to restore gains: %s\n", strerror(errno));
		return -1;
	}
...
static inline int dahdi_set_hook(int fd, int hs)
{
	...
	if (res < 0) {
		if (errno == EINPROGRESS)
			return 0;
		ast_log_chan(NULL, LOG_WARNING, "DAHDI hook failed returned %d (trying %d): %s\n", res, hs, strerror(errno));
...
static inline int dahdi_confmute(struct dahdi_pvt *p, int muted)
{
	...
	case SIG_SS7:
#endif	/* defined(HAVE_SS7) */
		{
			...
			if (res)
				ast_log_chan(NULL, LOG_WARNING, "Unable to set audio mode on %d: %s\n",
					p->channel, strerror(errno));
		}
		break;
	default:
	...
	if (res < 0)
		ast_log_chan(NULL, LOG_WARNING, "DAHDI confmute(%d) failed on channel %d: %s\n", muted, p->channel, strerror(errno));
	return res;
}
...
static int save_conference(struct dahdi_pvt *p)
{
	...
	if (p->saveconf.confmode) {
		ast_log_chan(NULL, LOG_WARNING, "Can't save conference -- already in use\n");
		return -1;
	}
	p->saveconf.chan = 0;
	res = ioctl(p->subs[SUB_REAL].dfd, DAHDI_GETCONF, &p->saveconf);
	if (res) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to get conference info: %s\n", strerror(errno));
	...
	if (res) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to set conference info: %s\n", strerror(errno));
		return -1;
	}
...
static int restore_conference(struct dahdi_pvt *p)
{
	...
	if (p->saveconf.confmode) {
		...
		p->saveconf.confmode = 0;
		if (res) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to restore conference info: %s\n", strerror(errno));
			return -1;
		}
	...
	while (p->cidpos < p->cidlen) {
		...
		if (res < 0) {
			if (errno == EAGAIN)
				...
				ast_log_chan(NULL, LOG_WARNING, "write failed: %s\n", strerror(errno));
				return -1;
			}
		}
		if (!res)
...
static int dahdi_call(struct ast_channel *ast, const char *rdest, int timeout)
{
	...
	if ((ast_channel_state(ast) != AST_STATE_DOWN) && (ast_channel_state(ast) != AST_STATE_RESERVED)) {
		ast_log_chan(NULL, LOG_WARNING, "dahdi_call called on %s, neither down nor reserved\n", ast_channel_name(ast));
		ast_mutex_unlock(&p->lock);
		return -1;
	}
	...
	if (res)
		ast_log_chan(NULL, LOG_WARNING, "Unable to flush input on channel %d: %s\n", p->channel, strerror(errno));
	p->outgoing = 1;
	...
	if (p->mfcr2) {
		...
		if (strlen(c) < p->stripmsd) {
			ast_log_chan(NULL, LOG_WARNING, "Number '%s' is shorter than stripmsd (%d)\n", c, p->stripmsd);
			ast_mutex_unlock(&p->lock);
			return -1;
		}
		p->dialing = 1;
		...
		if (-1 == callres) {
			ast_mutex_unlock(&p->lock);
			ast_log_chan(NULL, LOG_ERROR, "unable to make new MFC/R2 call!\n");
			return -1;
		}
		p->mfcr2_call_accepted = 0;
...
static void destroy_all_channels(void)
{
	...
	while (iflist) {
		...
		destroy_dahdi_pvt(p);
		ast_verb_chan(NULL, 3, "Unregistered channel %d\n", chan);
	}
	ifcount = 0;
...
static int dahdi_send_callrerouting_facility_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(args.destination)) {
		ast_log_chan(NULL, LOG_WARNING, "callrerouting facility requires at least destination number argument\n");
		return -1;
	}

	if (ast_strlen_zero(args.original)) {
		ast_log_chan(NULL, LOG_WARNING, "Callrerouting Facility without original called number argument\n");
		args.original = NULL;
	}

	if (ast_strlen_zero(args.reason)) {
		ast_log_chan(NULL, LOG_NOTICE, "Callrerouting Facility without diversion reason argument, defaulting to unknown\n");
		args.reason = NULL;
	}
...
static int dahdi_accept_r2_call_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(args.charge)) {
		ast_log_chan(NULL, LOG_WARNING, "DAHDIAcceptR2Call requires 'yes' or 'no' for the charge parameter\n");
		return -1;
	}
	...
	if (openr2_chan_accept_call(p->r2chan, accept_mode)) {
		ast_mutex_unlock(&p->lock);
		ast_log_chan(NULL, LOG_WARNING, "Failed to accept MFC/R2 call!\n");
		return -1;
	}
	ast_mutex_unlock(&p->lock);
...
static int revert_fax_buffers(struct dahdi_pvt *p, struct ast_channel *ast)
{
	if (p->bufferoverrideinuse) {
		...
		if ((bpres = ioctl(p->subs[SUB_REAL].dfd, DAHDI_SET_BUFINFO, &bi)) < 0) {
			ast_log_chan(NULL, LOG_WARNING, "Channel '%s' unable to revert buffer policy: %s\n", ast_channel_name(ast), strerror(errno));
		}
...
static int dahdi_hangup(struct ast_channel *ast)
{
	...
	if (!ast_channel_tech_pvt(ast)) {
		ast_log_chan(NULL, LOG_WARNING, "Asked to hangup channel not connected\n");
		return 0;
	}
	...
	if (idx > -1) {
		...
		if (idx == SUB_REAL) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Index found but not any type of call?\n");
		}
	}
	...
	if (!p->subs[SUB_REAL].owner && !p->subs[SUB_CALLWAIT].owner && !p->subs[SUB_THREEWAY].owner) {
		...
		if (res < 0) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to hangup line %s\n", ast_channel_name(ast));
		}
		switch (p->sig) {
		case SIG_FXOGS:
		case SIG_FXOLS:
...
static int dahdi_answer(struct ast_channel *ast)
{
	...
	case 0:
		...
		return 0;
	default:
		ast_log_chan(NULL, LOG_WARNING, "Don't know how to answer signalling %d (channel %d)\n", p->sig, p->channel);
		res = -1;
		break;
	}
	ast_mutex_unlock(&p->lock);
...
static int dahdi_setoption(struct ast_channel *chan, int option, void *data, int datalen)
{
	...
	case AST_OPTION_TXGAIN:
		...
		if (idx < 0) {
			ast_log_chan(NULL, LOG_WARNING, "No index in TXGAIN?\n");
			return -1;
		}
	...
	case AST_OPTION_RXGAIN:
		...
		idx = dahdi_get_index(chan, p, 0);
		if (idx < 0) {
			ast_log_chan(NULL, LOG_WARNING, "No index in RXGAIN?\n");
			return -1;
		}
	...
	case AST_OPTION_TDD:
		...
		if (!p->didtdd) { /* if haven't done it yet */
			...
			idx = dahdi_get_index(chan, p, 0);
			if (idx < 0) {
				ast_log_chan(NULL, LOG_WARNING, "No index in TDD?\n");
				return -1;
			}
			fd = p->subs[idx].dfd;
			while (len) {
	...
	case AST_OPTION_OPRMODE:  /* Operator services mode */
		...
		if (strcasecmp(ast_channel_tech(chan)->type, ast_channel_tech(oprmode->peer)->type)) {
			ast_log_chan(NULL, LOG_NOTICE, "Operator mode not supported on %s to %s calls.\n",
...
static int dahdi_func_read(struct ast_channel *chan, const char *function, char *data, char *buf, size_t len)
{
	...
	if (!strcasecmp(data, "rxgain")) {
		...
		if (dahdi_analog_lib_handles(p->sig, 0, 0) && analog_p) {
			...
			ast_log_chan(NULL, LOG_WARNING, "%s only supported on analog channels\n", data);
			*buf = '\0';
			res = -1;
		}
		ast_mutex_unlock(&p->lock);
...
static int parse_buffers_policy(const char *parse, int *num_buffers, int *policy)
{
	...
	if ((res = sscanf(parse, "%30d,%20s", num_buffers, policy_str)) != 2) {
		ast_log_chan(NULL, LOG_WARNING, "Parsing buffer string '%s' failed.\n", parse);
		return 1;
	}
	if (*num_buffers < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Invalid buffer count given '%d'.\n", *num_buffers);
		return -1;
	}
	if (!strcasecmp(policy_str, "full")) {
		...
		*policy = DAHDI_POLICY_HALF_FULL;
		...
		ast_log_chan(NULL, LOG_WARNING, "Invalid policy name given '%s'.\n", policy_str);
		return -1;
	}
...
static int dahdi_func_write(struct ast_channel *chan, const char *function, char *data, const char *value)
{
	...
	if (!strcasecmp(data, "buffers")) {
		...
		if (!(parse_buffers_policy(value, &num_bufs, &policy))) {
			...
			if ((bpres = ioctl(p->subs[SUB_REAL].dfd, DAHDI_SET_BUFINFO, &bi)) < 0) {
				ast_log_chan(NULL, LOG_WARNING, "Channel '%d' unable to override buffer policy: %s\n", p->channel, strerror(errno));
			} else {
				p->bufferoverrideinuse = 1;
			}
		} else {
		...
		if (!strcasecmp(value, "on")) {
			...
			if (ioctl(p->subs[SUB_REAL].dfd, DAHDI_ECHOCANCEL_FAX_MODE, &blah)) {
				ast_log_chan(NULL, LOG_WARNING, "Unable to place echocan into fax mode on channel %d: %s\n", p->channel, strerror(errno));
			}
			ast_mutex_unlock(&p->lock);
			...
			if (ioctl(p->subs[SUB_REAL].dfd, DAHDI_ECHOCANCEL_FAX_MODE, &blah)) {
				ast_log_chan(NULL, LOG_WARNING, "Unable to place echocan into voice mode on channel %d: %s\n", p->channel, strerror(errno));
			}
			ast_mutex_unlock(&p->lock);
#endif
		} else {
			ast_log_chan(NULL, LOG_WARNING, "Unsupported value '%s' provided for '%s' item.\n", value, data);
			res = -1;
		}
	} else if (!strcasecmp(data, "dialmode")) {
		struct analog_pvt *analog_p;
		...
		if (!strcasecmp(value, "pulse")) {
			...
			ast_log_chan(NULL, LOG_WARNING, "'%s' is an invalid setting for %s\n", value, data);
			res = -1;
		}
		ast_mutex_unlock(&p->lock);
	} else if (!strcasecmp(data, "waitfordialtone")) {
		if (ast_strlen_zero(value)) {
			ast_log_chan(NULL, LOG_WARNING, "waitfordialtone requires a duration in ms\n");
			return -1;
		}

		ast_mutex_lock(&p->lock);
		if (!CANPROGRESSDETECT(p)) {
			ast_log_chan(NULL, LOG_WARNING, "%s only supported on analog trunks\n", data);
			ast_mutex_unlock(&p->lock);
			return -1;
		}
...
void dahdi_master_slave_link(struct dahdi_pvt *slave, struct dahdi_pvt *master)
{
	int x;
	if (!slave || !master) {
		ast_log_chan(NULL, LOG_WARNING, "Tried to link to/from NULL??\n");
		return;
	}
	for (x = 0; x < MAX_SLAVES; x++) {
		if (!master->slaves[x]) {
	...
	if (x >= MAX_SLAVES) {
		ast_log_chan(NULL, LOG_WARNING, "Replacing slave %d with new slave, %d\n", master->slaves[MAX_SLAVES - 1]->channel, slave->channel);
		master->slaves[MAX_SLAVES - 1] = slave;
	}
	if (slave->master)
		ast_log_chan(NULL, LOG_WARNING, "Replacing master %d with new master, %d\n", slave->master->channel, master->channel);
	slave->master = master;
	...
	do {
		...
		if (res) {
			...
			case EINPROGRESS:
				...
				break;
			default:
				ast_log_chan(NULL, LOG_WARNING, "Couldn't ring the phone: %s\n", strerror(errno));
				res = 0;
			}
		}
	} while (res);
...
static int attempt_transfer(struct dahdi_pvt *p)
{
	...
	ast_verb_chan(NULL, 3, "TRANSFERRING %s to %s\n",
...
static int check_for_conference(struct dahdi_pvt *p)
{
	...
	if (ioctl(p->subs[SUB_REAL].dfd, DAHDI_GETCONF, &ci)) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to get conference info on channel %d: %s\n", p->channel, strerror(errno));
		return 0;
	}
	...
	if ((p->subs[SUB_REAL].curconf.confno != ci.confno) || (p->subs[SUB_REAL].curconf.confmode != ci.confmode)) {
		ast_verb_chan(NULL, 3, "Avoiding 3-way call when in an external conference\n");
		return 1;
	}
	return 0;
}
...
static int get_alarms(struct dahdi_pvt *p)
{
	...
	if ((res = ioctl(p->subs[SUB_REAL].dfd, DAHDI_SPANSTAT, &zi)) >= 0) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Unable to determine alarm on channel %d: %s\n", p->channel, strerror(errno));
		return 0;
	}
	...
	ast_log_chan(NULL, LOG_WARNING, "Unable to determine alarm on channel %d\n", p->channel);
...
static void handle_alarms(struct dahdi_pvt *p, int alms)
{
	...
	if (report_alarms & REPORT_CHANNEL_ALARMS) {
		ast_log_chan(NULL, LOG_WARNING, "Detected alarm on channel %d: %s\n", p->channel, alarm_str);
		publish_channel_alarm(p->channel, alarm_str);
	}

	if (report_alarms & REPORT_SPAN_ALARMS && p->manages_span_alarms) {
		ast_log_chan(NULL, LOG_WARNING, "Detected alarm on span %d: %s\n", p->span, alarm_str);
...
static struct ast_frame *dahdi_handle_event(struct ast_channel *ast)
{
	...
	switch (res) {
	case DAHDI_EVENT_EC_DISABLED:
		ast_verb_chan(NULL, 3, "Channel %d echo canceler disabled.\n", p->channel);
		p->echocanon = 0;
		break;
#ifdef HAVE_DAHDI_ECHOCANCEL_FAX_MODE
	case DAHDI_EVENT_TX_CED_DETECTED:
		ast_verb_chan(NULL, 3, "Channel %d detected a CED tone towards the network.\n", p->channel);
		break;
	case DAHDI_EVENT_RX_CED_DETECTED:
		ast_verb_chan(NULL, 3, "Channel %d detected a CED tone from the network.\n", p->channel);
		break;
	case DAHDI_EVENT_EC_NLP_DISABLED:
		ast_verb_chan(NULL, 3, "Channel %d echo canceler disabled its NLP.\n", p->channel);
		break;
	case DAHDI_EVENT_EC_NLP_ENABLED:
		ast_verb_chan(NULL, 3, "Channel %d echo canceler enabled its NLP.\n", p->channel);
		break;
#endif
	case DAHDI_EVENT_BITSCHANGED:
#ifdef HAVE_OPENR2
		if (p->sig != SIG_MFCR2) {
			ast_log_chan(NULL, LOG_WARNING, "Received bits changed on %s signalling?\n", sig2str(p->sig));
		} else {
	...
	case DAHDI_EVENT_ONHOOK:
		...
		if (p->oprmode < 0)
		{
			if (p->oprmode != -1) { /* Operator flash recall */
				ast_verb_chan(NULL, 4, "Operator mode enabled on channel %d, holding line for channel %d\n", p->channel, p->oprpeer->channel);
				break;
			}
			...
			if ((p->sig == SIG_FXOLS) || (p->sig == SIG_FXOKS) || (p->sig == SIG_FXOGS))
			{
				...
				ast_verb_chan(NULL, 4, "Operator recall, channel %d ringing back channel %d\n", p->oprpeer->channel, p->channel);
			}
			break;
		}
		switch (p->sig) {
		...
		case SIG_FXOKS:
			...
			if (idx == SUB_REAL) {
				...
				if (p->subs[SUB_CALLWAIT].owner) {
					...
					swap_subs(p, SUB_CALLWAIT, SUB_REAL);
					ast_verb_chan(NULL, 3, "Channel %d still has (callwait) call, ringing phone\n", p->channel);
					unalloc_sub(p, SUB_CALLWAIT);
#if 0
					p->subs[idx].needanswer = 0;
					p->subs[idx].needringing = 0;
					...
					while (p->subs[SUB_THREEWAY].owner && ast_channel_trylock(p->subs[SUB_THREEWAY].owner)) {
						...
						DLA_LOCK(&p->lock);
						if (p->owner != ast) {
							ast_log_chan(NULL, LOG_WARNING, "This isn't good...\n");
							return NULL;
						}
					}
					if (!p->subs[SUB_THREEWAY].owner) {
						ast_log_chan(NULL, LOG_NOTICE, "Whoa, threeway disappeared kinda randomly.\n");
						return NULL;
					}
				...
				ast_log_chan(NULL, LOG_WARNING, "Got a hangup and my index is %d?\n", idx);
			}
			/* Fall through */
		default:
			dahdi_ec_disable(p);
	...
	case DAHDI_EVENT_RINGOFFHOOK:
		...
		if (((mysig == SIG_E911) || (mysig == SIG_FGC_CAMA) || (mysig == SIG_FGC_CAMAMF)) && (ast_channel_state(ast) == AST_STATE_DIALING_OFFHOOK)) {
			...
			if (*c) {
				...
				if (numchars >= sizeof(p->dop.dialstr)) {
					ast_log_chan(NULL, LOG_WARNING, "Dial string '%s' truncated\n", c);
				}
			} else {
		...
		case SIG_FXOKS:
			...
			case AST_STATE_RESERVED:
				...
				break;
			default:
				ast_log_chan(NULL, LOG_WARNING, "FXO phone off hook in weird state %u??\n", ast_channel_state(ast));
			}
			break;
		case SIG_FXSLS:
		case SIG_FXSGS:
		...
		case SIG_SF_FEATB:
			...
			if ((ast_channel_state(ast) == AST_STATE_DOWN) || (ast_channel_state(ast) == AST_STATE_RING)) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Ring/Off-hook in strange state %u on channel %d\n", ast_channel_state(ast), p->channel);
			break;
		default:
			ast_log_chan(NULL, LOG_WARNING, "Don't know how to handle ring/off hook for signalling %d\n", p->sig);
		}
		break;
	case DAHDI_EVENT_RINGBEGIN:
		switch (p->sig) {
	...
	case DAHDI_EVENT_WINKFLASH:
		...
		if (p->oprmode > 1)
		{
			...
			if (ioctl(p->oprpeer->subs[SUB_REAL].dfd, DAHDI_GET_PARAMS, &par) != -1)
			{
				if (!par.rxisoffhook)
				{
					...
					ast_verb_chan(NULL, 4, "Operator flash recall, channel %d ringing back channel %d\n", p->oprpeer->channel, p->channel);
				}
			}
			break;
		}
		...
		case SIG_FXOKS:
			...
			if (idx != SUB_REAL) {
				ast_log_chan(NULL, LOG_WARNING, "Got flash hook with index %d on channel %d?!?\n", idx, p->channel);
				goto winkflashdone;
			}
			...
			if (p->subs[SUB_CALLWAIT].owner) {
				...
				if (!p->threewaycalling) {
					...
					if (alloc_sub(p, SUB_THREEWAY)) {
						ast_log_chan(NULL, LOG_WARNING, "Unable to allocate three-way subchannel\n");
						goto winkflashdone;
					}
					...
					if (res)
						ast_log_chan(NULL, LOG_WARNING, "Unable to start dial recall tone on channel %d\n", p->channel);
					p->owner = chan;
					if (!chan) {
						ast_log_chan(NULL, LOG_WARNING, "Cannot allocate new structure on channel %d\n", p->channel);
					} else if (ast_pthread_create_detached(&threadid, NULL, analog_ss_thread, chan)) {
						ast_log_chan(NULL, LOG_WARNING, "Unable to start simple switch on channel %d\n", p->channel);
						res = tone_zone_play_tone(p->subs[SUB_REAL].dfd, DAHDI_TONE_CONGESTION);
						dahdi_ec_enable(p);
						ast_hangup(chan);
					} else {
						ast_verb_chan(NULL, 3, "Started three way call on channel %d\n", p->channel);
				...
				if (p->subs[SUB_THREEWAY].inthreeway) {
					...
					ast_verb_chan(NULL, 3, "Dropping three-way call on %s\n", ast_channel_name(p->subs[SUB_THREEWAY].owner));
					ast_channel_softhangup_internal_flag_add(p->subs[SUB_THREEWAY].owner, AST_SOFTHANGUP_DEV);
					p->subs[SUB_REAL].inthreeway = 0;
					p->subs[SUB_THREEWAY].inthreeway = 0;
				} else {
					...
					if (((ast_channel_pbx(ast)) || (ast_channel_state(ast) == AST_STATE_UP)) &&
						...
						ast_verb_chan(NULL, 3, "Building conference call with %s and %s\n",
							ast_channel_name(p->subs[SUB_THREEWAY].owner),
						...
						ast_verb_chan(NULL, 3, "Dumping incomplete call on %s\n", ast_channel_name(p->subs[SUB_THREEWAY].owner));
						swap_subs(p, SUB_THREEWAY, SUB_REAL);
						ast_channel_softhangup_internal_flag_add(p->subs[SUB_THREEWAY].owner, AST_SOFTHANGUP_DEV);
						p->owner = p->subs[SUB_REAL].owner;
...
winkflashdone:
		...
		case SIG_FEATDMF_TA:
			...
			case 1:
				...
				break;
			case 2:
				ast_log_chan(NULL, LOG_WARNING, "Received unexpected wink on channel of type SIG_FEATDMF_TA\n");
				return NULL;
			}
			p->whichwink++;
			/* Fall through */
...
static struct ast_frame *__dahdi_exception(struct ast_channel *ast)
{
	...
	if ((!p->owner) && (!(p->radio || (p->oprmode < 0)))) {
		...
		case DAHDI_EVENT_ONHOOK:
			dahdi_ec_disable(p);
			if (p->owner) {
				ast_verb_chan(NULL, 3, "Channel %s still has call, ringing phone\n", ast_channel_name(p->owner));
				dahdi_ring_phone(p);
				p->callwaitingrepeat = 0;
				p->cidcwexpire = 0;
				p->cid_suppress_expire = 0;
			} else
				ast_log_chan(NULL, LOG_WARNING, "Absorbed on hook, but nobody is left!?!?\n");
			dahdi_conf_update(p);
			break;
		case DAHDI_EVENT_RINGOFFHOOK:
			dahdi_ec_enable(p);
		...
		case DAHDI_EVENT_WINKFLASH:
			p->flashtime = ast_tvnow();
			if (p->owner) {
				ast_verb_chan(NULL, 3, "Channel %d flashed to other channel %s\n", p->channel, ast_channel_name(p->owner));
				if (ast_channel_state(p->owner) != AST_STATE_UP) {
					/* Answer if necessary */
			...
			break;
		default:
			ast_log_chan(NULL, LOG_WARNING, "Don't know how to absorb event %s\n", event2str(res));
		}
		f = &p->subs[idx].f;
		return f;
	}
	...
	if (ast != p->owner) {
		if (p->owner) {
			ast_log_chan(NULL, LOG_WARNING, "We're %s, not %s\n", ast_channel_name(ast), ast_channel_name(p->owner));
		}
		f = &p->subs[idx].f;
		return f;
	}
...
static struct ast_frame *dahdi_read(struct ast_channel *ast)
{
	 ...
	 * analog ports can have more than one GABpbx channel using
	...
	while (ast_mutex_trylock(&p->lock)) {
		 ...
		 * private structure.  While the GABpbx channel was unlocked
	...
	if (idx < 0)	{
		ast_log_chan(NULL, LOG_WARNING, "We don't exist?\n");
		ast_mutex_unlock(&p->lock);
		return NULL;
	}
	...
	if (ast_format_cmp(ast_channel_rawreadformat(ast), ast_format_slin) == AST_FORMAT_CMP_EQUAL) {
		if (!p->subs[idx].linear) {
			...
			if (res)
				ast_log_chan(NULL, LOG_WARNING, "Unable to set channel %d (index %d) to linear mode.\n", p->channel, idx);
		}
	} else {
		if (p->subs[idx].linear) {
			p->subs[idx].linear = 0;
			res = dahdi_setlinear(p->subs[idx].dfd, p->subs[idx].linear);
			if (res)
				ast_log_chan(NULL, LOG_WARNING, "Unable to set channel %d (index %d) to companded mode.\n", p->channel, idx);
		}
	}
	...
	if (res < 0) {
		...
		if (res == -1) {
			if (errno == EAGAIN) {
				...
				ast_log_chan(NULL, LOG_WARNING, "dahdi_rec: %s\n", strerror(errno));
		}
		ast_mutex_unlock(&p->lock);
		return f;
	}
	...
	if (idx == SUB_REAL) {
		...
		if (p->cidcwexpire) {
			if (!--p->cidcwexpire) {
				/* Expired CID/CW */
				ast_verb_chan(NULL, 3, "CPE does not support Call Waiting Caller*ID.\n");
				restore_conference(p);
			}
		}
		...
		if (f) {
			...
			if ((f->frametype == AST_FRAME_CONTROL) && (f->subclass.integer == AST_CONTROL_BUSY)) {
				...
				if (ast_tvdiff_ms(ast_tvnow(), p->waitingfordt) >= p->waitfordialtoneduration) {
					p->waitingfordt.tv_sec = 0;
					ast_log_chan(NULL, LOG_NOTICE, "Never saw dialtone on channel %d\n", p->channel);
					ast_frfree(f);
					f = NULL;
...
static int dahdi_write(struct ast_channel *ast, struct ast_frame *frame)
{
	...
	if (frame->frametype != AST_FRAME_VOICE) {
		if (frame->frametype != AST_FRAME_IMAGE) {
			ast_log_chan(NULL, LOG_WARNING, "Don't know what to do with frame type '%u'\n",
				frame->frametype);
		}
		return 0;
	}
	...
	if (idx < 0) {
		ast_mutex_unlock(&p->lock);
		ast_log_chan(NULL, LOG_WARNING, "%s doesn't really exist?\n", ast_channel_name(ast));
		return -1;
	}
	...
	if (ast_format_cmp(frame->subclass.format, ast_format_slin) == AST_FORMAT_CMP_EQUAL) {
		if (!p->subs[idx].linear) {
			...
			res = dahdi_setlinear(p->subs[idx].dfd, p->subs[idx].linear);
			if (res)
				ast_log_chan(NULL, LOG_WARNING, "Unable to set linear mode on channel %d\n", p->channel);
		}
		...
		if (p->subs[idx].linear) {
			...
			res = dahdi_setlinear(p->subs[idx].dfd, p->subs[idx].linear);
			if (res)
				ast_log_chan(NULL, LOG_WARNING, "Unable to set companded mode on channel %d\n", p->channel);
		}
		res = my_dahdi_write(p, (unsigned char *)frame->data.ptr, frame->datalen, idx, 0);
	} else {
		ast_mutex_unlock(&p->lock);
		ast_log_chan(NULL, LOG_WARNING, "Cannot handle frames in %s format\n",
		...
		case AST_CONTROL_FLASH:
			...
			if (ISTRUNK(p) && (p->sig != SIG_PRI)) {
				...
				if ((ioctl(p->subs[SUB_REAL].dfd,DAHDI_HOOK,&func) == -1) && (errno != EINPROGRESS)) {
					ast_log_chan(NULL, LOG_WARNING, "Unable to flash external trunk on channel %s: %s\n",
						ast_channel_name(chan), strerror(errno));
				} else
					res = 0;
			} else
...
static struct ast_channel *dahdi_new(struct dahdi_pvt *i, int state, int startpbx, int idx, int law, const struct ast_assigned_ids *assignedids, const struct ast_channel *requestor, ast_callid callid)
{
	...
	if (i->subs[idx].owner) {
		ast_log_chan(NULL, LOG_WARNING, "Channel %d already has a %s call\n", i->channel,subnames[idx]);
		return NULL;
	}
	...
	if (startpbx) {
		...
		if (ast_pbx_start(tmp)) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to start PBX on %s\n", ast_channel_name(tmp));
			ast_hangup(tmp);
			return NULL;
		}
	}
...
static int dahdi_dnd(struct dahdi_pvt *dahdichan, int flag)
{
	...
	/* Do not disturb */
	dahdichan->dnd = flag;
	ast_verb_chan(NULL, 3, "%s DND on channel %d\n",
...
static void *analog_ss_thread(void *data)
{
	...
	*/
	if (!p) {
		ast_log_chan(NULL, LOG_WARNING, "Channel became a zombie before simple switch could be started (%s)\n", ast_channel_name(chan));
		ast_hangup(chan);
		goto quit;
	}
	ast_verb_chan(NULL, 3, "Starting simple switch on '%s'\n", ast_channel_name(chan));
	idx = dahdi_get_index(chan, p, 1);
	if (idx < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Huh?\n");
		ast_hangup(chan);
		goto quit;
	}
	...
	if (!pickup_cfg) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to retrieve pickup configuration options. Unable to detect call pickup extension\n");
		pickupexten = "";
	} else {
	...
	case SIG_FGC_CAMA:
		...
		if (res == -1) {
			ast_log_chan(NULL, LOG_WARNING, "getdtmf on channel %d: %s\n", p->channel, strerror(errno));
			ast_hangup(chan);
			goto quit;
		} else if (res < 0) {
		...
		if (p->sig == SIG_FEATD || p->sig == SIG_EMWINK) {
			...
			if (exten[0] == '*') {
				...
				ast_log_chan(NULL, LOG_WARNING, "Got a non-Feature Group D input on channel %d.  Assuming E&M Wink instead\n", p->channel);
		}
		...
		if ((p->sig == SIG_E911) || (p->sig == SIG_FGC_CAMAMF)) {
			if (exten[0] == '*') {
				...
				ast_log_chan(NULL, LOG_WARNING, "Got a non-E911/FGC CAMA input on channel %d.  Assuming E&M Wink instead\n", p->channel);
		}
		if (p->sig == SIG_FEATB) {
			if (exten[0] == '*') {
				char *stringp=NULL;
				...
				ast_log_chan(NULL, LOG_WARNING, "Got a non-Feature Group B input on channel %d.  Assuming E&M Wink instead\n", p->channel);
		}
		if ((p->sig == SIG_FEATDMF) || (p->sig == SIG_FEATDMF_TA)) {
			dahdi_wink(p, idx);
		...
		if (ast_exists_extension(chan, ast_channel_context(chan), exten, 1,
			...
			if (res) {
				ast_log_chan(NULL, LOG_WARNING, "PBX exited non-zero\n");
				res = tone_zone_play_tone(p->subs[idx].dfd, DAHDI_TONE_CONGESTION);
			}
			goto quit;
		} else {
			ast_verb_chan(NULL, 2, "Unknown extension '%s' in context '%s' requested\n", exten, ast_channel_context(chan));
			sleep(2);
			res = tone_zone_play_tone(p->subs[idx].dfd, DAHDI_TONE_INFO);
			if (res < 0)
				ast_log_chan(NULL, LOG_WARNING, "Unable to start special tone on %d\n", p->channel);
			else
				sleep(1);
			...
			if (ast_exists_extension(chan, ast_channel_context(chan), exten, 1, p->cid_num) && !is_exten_parking) {
				if (!res || !ast_matchmore_extension(chan, ast_channel_context(chan), exten, 1, p->cid_num)) {
					if (getforward) {
						...
						ast_verb_chan(NULL, 3, "Setting call forward to '%s' on channel %d\n", p->call_forward, p->channel);
				...
				ast_verb_chan(NULL, 3, "Disabling call waiting on %s\n", ast_channel_name(chan));
				/* Disable call waiting if enabled */
				p->callwaiting = 0;
				res = tone_zone_play_tone(p->subs[idx].dfd, DAHDI_TONE_DIALRECALL);
				if (res) {
					ast_log_chan(NULL, LOG_WARNING, "Unable to do dial recall on channel %s: %s\n",
						ast_channel_name(chan), strerror(errno));
				}
				len = 0;
				...
				if (idx == SUB_REAL) {
					...
					ast_log_chan(NULL, LOG_WARNING, "Huh?  Got *8# on call not on real\n");
					ast_hangup(chan);
					goto quit;
				}

			} else if (!p->hidecallerid && !strcmp(exten, "*67")) {
				ast_verb_chan(NULL, 3, "Disabling Caller*ID on %s\n", ast_channel_name(chan));
				...
				ast_verb_chan(NULL, 3, "Cancelling call forwarding on channel %d\n", p->channel);
				res = tone_zone_play_tone(p->subs[idx].dfd, DAHDI_TONE_DIALRECALL);
				...
				if (bridge_channel) {
					if (!ast_parking_blind_transfer_park(bridge_channel, ast_channel_context(chan), exten, NULL, NULL)) {
						...
						ast_verb_chan(NULL, 3, "%s: Parked call\n", ast_channel_name(chan));
						ast_hangup(chan);
				...
				ast_verb_chan(NULL, 3, "Enabling Caller*ID on %s\n", ast_channel_name(chan));
	...
	case SIG_FXSKS:
		...
		if (p->use_smdi && p->smdi_iface) {
			...
			if (smdi_msg != NULL) {
				...
				ast_log_chan(NULL, LOG_WARNING, "SMDI enabled but no SMDI message present\n");
			}
		}
			...
			if (p->cid_signalling == CID_SIG_DTMF) {
				...
				for (;;) {
					...
					if (res <= 0) {
						 ...
						 */
						ast_log_chan(NULL, LOG_WARNING, "DTMFCID timed out waiting for ring. "
			...
			/* If set to use V23 Signalling, launch our FSK gubbins and listen for it */
				...
				if (cs) {
					...
					for (;;) {
						...
						if (i & DAHDI_IOMUX_SIGEVENT) {
							res = dahdi_get_event(p->subs[idx].dfd);
							ast_log_chan(NULL, LOG_NOTICE, "Got event %d (%s)...\n", res, event2str(res));
							if (res == DAHDI_EVENT_NOALARM) {
								p->inalarm = 0;
							}
					...
					if (res == 1) {
						...
						ast_log_chan(NULL, LOG_NOTICE, "CallerID number: %s, name: %s, flags=%d\n", number, name, flags);
					}
					...
					for (;;) {
						...
						res = ast_waitfor(chan, ms);
						if (res <= 0) {
							ast_log_chan(NULL, LOG_WARNING, "CID timed out waiting for ring. "
								"Exiting simple switch\n");
							ast_hangup(chan);
							goto quit;
						}
						if (!(f = ast_read(chan))) {
							ast_log_chan(NULL, LOG_WARNING, "Hangup received waiting for ring. Exiting simple switch\n");
							ast_hangup(chan);
							goto quit;
						}
						ast_frfree(f);
					...
					if (p->usedistinctiveringdetection) {
						...
						for (counter = 0; counter < 3; counter++) {
							...
							distMatches = 0;
							for (counter1 = 0; counter1 < 3; counter1++) {
								ast_verb_chan(NULL, 3, "Ring pattern check range: %d\n", p->drings.ringnum[counter].range);
								if (p->drings.ringnum[counter].ring[counter1] == -1) {
									...
									ast_verb_chan(NULL, 3, "Ring pattern matched in range: %d to %d\n",
							...
							if (distMatches == 3) {
								...
								ast_verb_chan(NULL, 3, "Distinctive Ring matched context %s\n",p->context);
								break;
							}
						}
					}
					...
					ast_log_chan(NULL, LOG_WARNING, "Unable to get caller ID space\n");
			} else {
				ast_log_chan(NULL, LOG_WARNING, "Channel %s in prering "
				...
				if (cs) {
					...
					if (p->usedistinctiveringdetection) {
						...
						for (counter = 0; counter < 3; counter++) {
							...
							channel */
							/* this only shows up if you have n of the dring patterns filled in */
							ast_verb_chan(NULL, 3, "Checking %d,%d,%d\n",
									p->drings.ringnum[counter].ring[0],
									p->drings.ringnum[counter].ring[1],
									p->drings.ringnum[counter].ring[2]);
							distMatches = 0;
...
#endif
					if (res < 0) {
						ast_log_chan(NULL, LOG_WARNING, "CallerID returned with error on channel '%s'\n", ast_channel_name(chan));
					}
				} else
					ast_log_chan(NULL, LOG_WARNING, "Unable to get caller ID space\n");
			}
		...
		goto quit;
	default:
		ast_log_chan(NULL, LOG_WARNING, "Don't know how to handle simple switch with signalling %s on channel %d\n", sig2str(p->sig), p->channel);
		res = tone_zone_play_tone(p->subs[idx].dfd, DAHDI_TONE_CONGESTION);
		if (res < 0)
				ast_log_chan(NULL, LOG_WARNING, "Unable to play congestion tone on channel %d\n", p->channel);
	}
	res = tone_zone_play_tone(p->subs[idx].dfd, DAHDI_TONE_CONGESTION);
	if (res < 0)
			ast_log_chan(NULL, LOG_WARNING, "Unable to play congestion tone on channel %d\n", p->channel);
...
static void *mwi_thread(void *data)
{
	...
	for (;;) {
		...
		if (i & DAHDI_IOMUX_SIGEVENT) {
			...
			default:
				callid_created = ast_callid_threadstorage_auto(&callid);
				ast_log_chan(NULL, LOG_NOTICE, "Got event %d (%s)...  Passing along to analog_ss_thread\n", res, event2str(res));
				callerid_free(cs);
				...
				if ((chan = dahdi_new(mtd->pvt, AST_STATE_RING, 0, SUB_REAL, 0, NULL, NULL, callid))) {
					...
					if (result) {
						ast_log_chan(NULL, LOG_WARNING, "Unable to start simple switch thread on channel %d\n", mtd->pvt->channel);
					...
					ast_log_chan(NULL, LOG_WARNING, "Could not create channel to handle call\n");
				}
	...
	if (spill_result == 1) {
		...
		if (flags & CID_MSGWAITING) {
			ast_log_chan(NULL, LOG_NOTICE, "mwi: Have Messages on channel %d\n", mtd->pvt->channel);
			notify_message(mtd->pvt->mailbox, 1);
		} else if (flags & CID_NOMSGWAITING) {
			ast_log_chan(NULL, LOG_NOTICE, "mwi: No Messages on channel %d\n", mtd->pvt->channel);
			notify_message(mtd->pvt->mailbox, 0);
		} else {
			ast_log_chan(NULL, LOG_NOTICE, "mwi: Status unknown on channel %d\n", mtd->pvt->channel);
		}
	}
...
static int mwi_send_init(struct dahdi_pvt * pvt)
{
	...
	if (pvt->cidspill) {
		ast_log_chan(NULL, LOG_WARNING, "cidspill already exists when trying to send FSK MWI\n");
	...
	*/
	if (MWI_SEND_DONE != pvt->mwisend_data.mwisend_current && !pvt->cidspill) {
		...
		case MWI_SEND_SA:
			...
			if (res) {
				ast_log_chan(NULL, LOG_WARNING, "Unable to set RP-AS ring cadence: %s\n", strerror(errno));
				goto quit;
			}
		...
		case MWI_SEND_SPILL:
			...
			if (0 < num_read) {
				...
				if (res > 0) {
					...
					ast_log_chan(NULL, LOG_WARNING, "MWI FSK Send Write failed: %s\n", strerror(errno));
					goto quit;
				}
			}
			break;
		...
		case DAHDI_EVENT_RINGEROFF:
			if (pvt->mwisend_data.mwisend_current == MWI_SEND_SA_WAIT) {
				...
				if (dahdi_set_hook(pvt->subs[SUB_REAL].dfd, DAHDI_RINGOFF) ) {
					ast_log_chan(NULL, LOG_WARNING, "Unable to finish RP-AS: %s mwi send aborted\n", strerror(errno));
					ast_free(pvt->cidspill);
					pvt->cidspill = NULL;
					pvt->mwisend_data.mwisend_current = MWI_SEND_DONE;
					pvt->mwisendactive = 0;
...
static struct dahdi_pvt *handle_init_event(struct dahdi_pvt *i, int event)
{
	...
	case DAHDI_EVENT_RINGOFFHOOK:
		...
		case SIG_FXOKS:
			...
			if (i->immediate) {
				...
				if (!chan) {
					ast_log_chan(NULL, LOG_WARNING, "Unable to start PBX on channel %d\n", i->channel);
				...
				if (chan) {
					...
					if (res < 0)
						ast_log_chan(NULL, LOG_WARNING, "Unable to play dialtone on channel %d, do you have defaultzone and loadzone defined?\n", i->channel);
					...
					ast_log_chan(NULL, LOG_WARNING, "Unable to create channel\n");
			}
		...
		case SIG_SF:
			...
			break;
		default:
			ast_log_chan(NULL, LOG_WARNING, "Don't know how to handle ring/answer with signalling %s on channel %d\n", sig2str(i->sig), i->channel);
			res = tone_zone_play_tone(i->subs[SUB_REAL].dfd, DAHDI_TONE_CONGESTION);
			if (res < 0)
				ast_log_chan(NULL, LOG_WARNING, "Unable to play congestion tone on channel %d\n", i->channel);
			return NULL;
	...
	case DAHDI_EVENT_ONHOOK:
		...
		case SIG_PRI_LIB_HANDLE_CASES:
			...
			break;
		default:
			ast_log_chan(NULL, LOG_WARNING, "Don't know how to handle on hook with signalling %s on channel %d\n", sig2str(i->sig), i->channel);
			res = tone_zone_play_tone(i->subs[SUB_REAL].dfd, -1);
			return NULL;
		}
		break;
	case DAHDI_EVENT_POLARITY:
		...
		case SIG_FXSGS:
			...
			if (i->cid_start == CID_START_POLARITY || i->cid_start == CID_START_POLARITY_IN) {
				i->polarity = POLARITY_REV;
				ast_verb_chan(NULL, 2, "Starting post polarity "
			...
			break;
		default:
			ast_log_chan(NULL, LOG_WARNING, "handle_init_event detected "
	...
	case DAHDI_EVENT_NEONMWI_ACTIVE:
		if (i->mwimonitor_neon) {
			...
			ast_log_chan(NULL, LOG_NOTICE, "NEON MWI set for channel %d, mailbox %s \n", i->channel, i->mailbox);
		}
		break;
	case DAHDI_EVENT_NEONMWI_INACTIVE:
		if (i->mwimonitor_neon) {
			notify_message(i->mailbox, 0);
			ast_log_chan(NULL, LOG_NOTICE, "NEON MWI cleared for channel %d, mailbox %s\n", i->channel, i->mailbox);
		}
		break;
	}
	return NULL;
...
static void *do_monitor(void *data)
{
	...
	if (pthread_setcanceltype(PTHREAD_CANCEL_ASYNCHRONOUS, NULL)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to set cancel type to asynchronous\n");
		return NULL;
	}
	...
	for (;;) {
		...
		for (i = iflist; i; i = i->next) {
			...
			if (pfds && (i->subs[SUB_REAL].dfd > -1) && i->sig && (!i->radio) && !(i->sig & SIG_MFCR2)) {
				if (dahdi_analog_lib_handles(i->sig, i->radio, i->oprmode)) {
					...
					if (!p) {
						ast_log_chan(NULL, LOG_ERROR, "No sig_pvt?\n");
		...
		if (res < 0) {
			if ((errno != EAGAIN) && (errno != EINTR))
				ast_log_chan(NULL, LOG_WARNING, "poll return %d: %s\n", res, strerror(errno));
			continue;
		}
						...
						if (analog_p
							...
							if (analog_p->msgstate != res) {
								...
								if (mwi_send_init(last)) {
									ast_log_chan(NULL, LOG_WARNING, "Unable to initiate mwi send sequence on channel %d\n", last->channel);
								}
			...
			if ((i->subs[SUB_REAL].dfd > -1) && i->sig) {
				...
				if (pollres & POLLIN) {
					if (i->owner || i->subs[SUB_REAL].owner) {
						...
						if (!i->pri)
#endif
							ast_log_chan(NULL, LOG_WARNING, "Whoa....  I'm owned but found (%d) in read...\n", i->subs[SUB_REAL].dfd);
						continue;
					}
					if (!i->mwimonitor_fsk && !i->mwisendactive  && i->cid_start != CID_START_DTMF_NOALERT) {
						ast_log_chan(NULL, LOG_WARNING, "Whoa....  I'm not looking for MWI or sending MWI but am reading (%d)...\n", i->subs[SUB_REAL].dfd);
						continue;
					}
					...
					if (res > 0) {
							...
							if (calc_energy((unsigned char *) buf, res, AST_LAW(i)) > mwilevel) {
								...
								if ((mtd = ast_calloc(1, sizeof(*mtd)))) {
									...
									if (ast_pthread_create_background(&threadid, &attr, mwi_thread, mtd)) {
										ast_log_chan(NULL, LOG_WARNING, "Unable to start mwi thread on channel %d\n", i->channel);
						...
						ast_log_chan(NULL, LOG_WARNING, "Read failed with %d: %s\n", res, strerror(errno));
					}
				}
				if (pollres & POLLPRI) {
					if (i->owner || i->subs[SUB_REAL].owner) {
						...
						if (!i->pri)
#endif
							ast_log_chan(NULL, LOG_WARNING, "Whoa....  I'm owned but found (%d)...\n", i->subs[SUB_REAL].dfd);
						continue;
					}
					res = dahdi_get_event(i->subs[SUB_REAL].dfd);
					...
					if (i->doreoriginate && res == DAHDI_EVENT_HOOKCOMPLETE) {
						...
						if (!my_is_off_hook(i)) {
							...
							ast_verb_chan(NULL, 3, "Automatic reorigination on channel %d\n", i->channel);
...
static int restart_monitor(void)
{
	...
	if (monitor_thread == pthread_self()) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Cannot kill myself\n");
		return -1;
	}
	if (monitor_thread != AST_PTHREADT_NULL) {
		...
		if (ast_pthread_create_background(&monitor_thread, NULL, do_monitor, NULL) < 0) {
			ast_mutex_unlock(&monlock);
			ast_log_chan(NULL, LOG_ERROR, "Unable to start monitor thread.\n");
			return -1;
		}
	}
	ast_mutex_unlock(&monlock);
...
static int pri_resolve_span(int *span, int channel, int offset, struct dahdi_spaninfo *si)
{
	...
	if (trunkgroup) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Channel %d on span %d configured to use nonexistent trunk group %d\n", channel, *span, trunkgroup);
		*span = -1;
	} else {
		if (pris[*span].pri.trunkgroup) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to use span %d implicitly since it is trunk group %d (please use spanmap)\n", *span, pris[*span].pri.trunkgroup);
			*span = -1;
		} else if (pris[*span].mastertrunkgroup) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to use span %d implicitly since it is already part of trunk group %d\n", *span, pris[*span].mastertrunkgroup);
			*span = -1;
		} else {
			if (si->totalchans == 31) {
				/* E1 */
				...
				ast_log_chan(NULL, LOG_WARNING, "Unable to use span %d, since the D-channel cannot be located (unexpected span size of %d channels)\n", *span, si->totalchans);
				*span = -1;
				return 0;
			}
...
static int pri_create_trunkgroup(int trunkgroup, int *channels)
{
	...
	for (x = 0; x < NUM_SPANS; x++) {
		if (pris[x].pri.trunkgroup == trunkgroup) {
			ast_log_chan(NULL, LOG_WARNING, "Trunk group %d already exists on span %d, Primary d-channel %d\n", trunkgroup, x + 1, pris[x].dchannels[0]);
			return -1;
		}
	}
	for (y = 0; y < SIG_PRI_NUM_DCHANS; y++) {
		...
		if (fd < 0) {
			ast_log_chan(NULL, LOG_WARNING, "Failed to open channel: %s\n", strerror(errno));
			return -1;
		}
		x = channels[y];
		if (ioctl(fd, DAHDI_SPECIFY, &x)) {
			ast_log_chan(NULL, LOG_WARNING, "Failed to specify channel %d: %s\n", channels[y], strerror(errno));
			close(fd);
			return -1;
		}
		if (ioctl(fd, DAHDI_GET_PARAMS, &p)) {
			ast_log_chan(NULL, LOG_WARNING, "Failed to get channel parameters for channel %d: %s\n", channels[y], strerror(errno));
			close(fd);
			return -1;
		}
		if (ioctl(fd, DAHDI_SPANSTAT, &si)) {
			ast_log_chan(NULL, LOG_WARNING, "Failed go get span information on channel %d (span %d): %s\n", channels[y], p.spanno, strerror(errno));
			close(fd);
			return -1;
		}
		span = p.spanno - 1;
		if (pris[span].pri.trunkgroup) {
			ast_log_chan(NULL, LOG_WARNING, "Span %d is already provisioned for trunk group %d\n", span + 1, pris[span].pri.trunkgroup);
			close(fd);
			return -1;
		}
		if (pris[span].pri.pvts[0]) {
			ast_log_chan(NULL, LOG_WARNING, "Span %d is already provisioned with channels (implicit PRI maybe?)\n", span + 1);
			close(fd);
			return -1;
		}
		if (!y) {
...
static int pri_create_spanmap(int span, int trunkgroup, int logicalspan)
{
	if (pris[span].mastertrunkgroup) {
		ast_log_chan(NULL, LOG_WARNING, "Span %d is already part of trunk group %d, cannot add to trunk group %d\n", span + 1, pris[span].mastertrunkgroup, trunkgroup);
		return -1;
	}
...
static struct r2link_entry *dahdi_r2_get_link(const struct dahdi_chan_conf *conf)
{
	...
	if (!cur) {
		...
		if (!cur) {
			ast_log_chan(NULL, LOG_ERROR, "Cannot allocate R2 link!\n");
			return NULL;
		}
...
static int dahdi_r2_set_context(struct dahdi_mfcr2 *r2_link, const struct dahdi_chan_conf *conf)
{
	...
	if (ast_strlen_zero(conf->mfcr2.logdir)) {
		if (openr2_context_set_log_directory(r2_link->protocol_context, tmplogdir)) {
			ast_log_chan(NULL, LOG_ERROR, "Failed setting default MFC/R2 log directory %s\n", tmplogdir);
		}
	} else {
		snres = snprintf(logdir, sizeof(logdir), "%s/%s/%s", ast_config_AST_LOG_DIR, "mfcr2", conf->mfcr2.logdir);
		if (snres >= sizeof(logdir)) {
			ast_log_chan(NULL, LOG_ERROR, "MFC/R2 logging directory truncated, using %s\n", tmplogdir);
			if (openr2_context_set_log_directory(r2_link->protocol_context, tmplogdir)) {
				ast_log_chan(NULL, LOG_ERROR, "Failed setting default MFC/R2 log directory %s\n", tmplogdir);
			}
		} else {
			if (openr2_context_set_log_directory(r2_link->protocol_context, logdir)) {
				ast_log_chan(NULL, LOG_ERROR, "Failed setting MFC/R2 log directory %s\n", logdir);
			}
		}
	}
	if (!ast_strlen_zero(conf->mfcr2.r2proto_file)) {
		if (openr2_context_configure_from_advanced_file(r2_link->protocol_context, conf->mfcr2.r2proto_file)) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to configure r2context from advanced configuration file %s\n", conf->mfcr2.r2proto_file);
		}
	}
...
static struct dahdi_pvt *mkintf(int channel, const struct dahdi_chan_conf *conf, int reloading)
{
	...
	if (tmp) {
		...
		if (!here) {
			...
			if ((channel != CHAN_PSEUDO)) {
				...
				if (tmp->subs[SUB_REAL].dfd < 0) {
					ast_log_chan(NULL, LOG_ERROR, "Unable to open channel %d: %s\nhere = %d, tmp->channel = %d, channel = %d\n", channel, strerror(errno), here, tmp->channel, channel);
				...
				if (res < 0) {
					ast_log_chan(NULL, LOG_ERROR, "Unable to get parameters: %s\n", strerror(errno));
					destroy_dahdi_pvt(tmp);
					return NULL;
				}
				if (conf->is_sig_auto)
				...
				if (p.sigtype != (chan_sig & 0x3ffff)) {
					ast_log_chan(NULL, LOG_ERROR, "Signalling requested on channel %d is %s but line is in %s signalling\n", channel, sig2str(chan_sig), sig2str(p.sigtype));
					destroy_dahdi_pvt(tmp);
					return NULL;
				}
			...
			if (chan_sig == SIG_SS7) {
				...
				if (ioctl(tmp->subs[SUB_REAL].dfd, DAHDI_AUDIOMODE, &clear)) {
					ast_log_chan(NULL, LOG_ERROR, "Unable to set clear mode on clear channel %d of span %d: %s\n", channel, p.spanno, strerror(errno));
					destroy_dahdi_pvt(tmp);
					return NULL;
				}
				...
				if (!ss7) {
					ast_log_chan(NULL, LOG_ERROR, "Unable to find linkset %d\n", cur_linkset);
					destroy_dahdi_pvt(tmp);
					return NULL;
				}
				ss7->ss7.span = cur_linkset;
				if (cur_cicbeginswith < 0) {
					ast_log_chan(NULL, LOG_ERROR, "Need to set cicbeginswith for the channels!\n");
					destroy_dahdi_pvt(tmp);
					return NULL;
				}
			...
			if (chan_sig == SIG_MFCR2) {
				...
				r2_link = &r2_le->mfcr2;
				if (!r2_link) {
					ast_log_chan(NULL, LOG_WARNING, "Cannot get another R2 DAHDI context!\n");
					destroy_dahdi_pvt(tmp);
					return NULL;
				}
				if (!r2_link->protocol_context && dahdi_r2_set_context(r2_link, conf)) {
					ast_log_chan(NULL, LOG_ERROR, "Cannot create OpenR2 protocol context.\n");
					destroy_dahdi_pvt(tmp);
					return NULL;
				}
				if (r2_link->numchans == ARRAY_LEN(r2_link->pvts)) {
					ast_log_chan(NULL, LOG_ERROR, "Cannot add more channels to this link!\n");
					destroy_dahdi_pvt(tmp);
					return NULL;
				}
				...
				if (!tmp->r2chan) {
					...
					ast_log_chan(NULL, LOG_ERROR, "Cannot create OpenR2 channel: %s\n", openr2_context_error_string(err));
					destroy_dahdi_pvt(tmp);
					return NULL;
				}
				r2_link->live_chans++;
			...
			if (dahdi_sig_pri_lib_handles(chan_sig)) {
				...
				if (span >= NUM_SPANS) {
					ast_log_chan(NULL, LOG_ERROR, "Channel %d does not lie on a span I know of (%d)\n", channel, span);
					destroy_dahdi_pvt(tmp);
					return NULL;
				} else {
					si.spanno = 0;
					if (ioctl(tmp->subs[SUB_REAL].dfd,DAHDI_SPANSTAT,&si) == -1) {
						ast_log_chan(NULL, LOG_ERROR, "Unable to get span status: %s\n", strerror(errno));
						destroy_dahdi_pvt(tmp);
						return NULL;
					}
					...
					if (span < 0) {
						ast_log_chan(NULL, LOG_WARNING, "Channel %d: Unable to find locate channel/trunk group!\n", channel);
						destroy_dahdi_pvt(tmp);
						return NULL;
					}
					...
					if (!matchesdchan) {
						...
						ast_log_chan(NULL, LOG_ERROR, "Channel %d is reserved for D-channel.\n", p.chanpos);
						destroy_dahdi_pvt(tmp);
						return NULL;
					}
				}
			...
			ast_log_chan(NULL, LOG_WARNING, "Attempt to configure channel %d with signaling %s ignored because it is already configured to be %s.\n", tmp->channel, dahdi_sig2str(chan_sig), dahdi_sig2str(tmp->sig));
			chan_sig = tmp->sig;
		...
		if (tmp->subs[SUB_REAL].dfd >= 0)
		{
			...
			if (res < 0) {
				ast_log_chan(NULL, LOG_ERROR, "Unable to set parameters: %s\n", strerror(errno));
				destroy_dahdi_pvt(tmp);
				return NULL;
			}
		}
		...
		if (tmp->echocancel.head.tap_length) {
			...
			if (conf->chan.echocanbridged)
				ast_log_chan(NULL, LOG_NOTICE, "echocancelwhenbridged requires echocancel to be enabled; ignoring\n");
		...
		if (tmp->usedistinctiveringdetection) {
			if (!tmp->use_callerid) {
				ast_log_chan(NULL, LOG_NOTICE, "Distinctive Ring detect requires 'usecallerid' be on\n");
				tmp->use_callerid = 1;
			}
		}
		...
		if (tmp->cid_signalling == CID_SIG_SMDI) {
			if (!tmp->use_smdi) {
				ast_log_chan(NULL, LOG_WARNING, "SMDI callerid requires SMDI to be enabled, enabling...\n");
				tmp->use_smdi = 1;
			}
		}
		if (tmp->use_smdi) {
			...
			if (!(tmp->smdi_iface)) {
				ast_log_chan(NULL, LOG_ERROR, "Invalid SMDI port specified, disabling SMDI support\n");
				tmp->use_smdi = 0;
			}
		}
...
static int dahdi_new_pri_nobch_channel(struct sig_pri_span *pri)
{
	...
	if (pri->numchans == pvt_idx) {
		if (ARRAY_LEN(pri->pvts) <= pvt_idx) {
			ast_log_chan(NULL, LOG_ERROR, "Unable to add a no-B-channel interface!\n");
			return -1;
		}
	...
	if (pvt->subs[SUB_REAL].dfd < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to open no B channel interface pseudo channel: %s\n",
			strerror(errno));
		destroy_dahdi_pvt(pvt);
		return -1;
	}
...
static struct dahdi_pvt *duplicate_pseudo(struct dahdi_pvt *src)
{
	...
	*p = *src;
	...
	if (p->subs[SUB_REAL].dfd < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to dup channel: %s\n", strerror(errno));
		destroy_dahdi_pvt(p);
		return NULL;
	}
	...
	if (!res) {
		...
		if (res < 0) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to set buffer policy on dup channel: %s\n", strerror(errno));
		}
	} else
		ast_log_chan(NULL, LOG_WARNING, "Unable to check buffer policy on dup channel: %s\n", strerror(errno));
	p->destroy = 1;
...
static struct dahdi_pvt *determine_starting_point(const char *data, struct dahdi_starting_point *param)
{
	...
	if (data) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Channel requested with no data\n");
		return NULL;
	}
	AST_NONSTANDARD_APP_ARGS(args, dest, '/');
	if (!args.argc || ast_strlen_zero(args.group)) {
		ast_log_chan(NULL, LOG_WARNING, "No channel/group specified\n");
		return NULL;
	}
	...
	if (strchr(args.group, '!') != NULL) {
		...
		*(prev - 1) = '\0';
		...
		if (res < 1) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to determine ISDN span for data %s\n", data);
			return NULL;
		}
		param->span = x;
	...
	if (toupper(args.group[0]) == 'G' || toupper(args.group[0])=='R') {
		...
		if (res < 1) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to determine group for data %s\n", data);
			return NULL;
		}
		...
		if (toupper(args.group[0]) == 'G') {
			...
			if (ARRAY_LEN(round_robin) <= x) {
				ast_log_chan(NULL, LOG_WARNING, "Round robin index %d out of range for data %s\n",
					x, data);
				return NULL;
			}
			if (args.group[0] == 'R') {
		...
		if (!strcasecmp(s, "pseudo")) {
			...
			res = sscanf(s, "%30d%1c%30d", &x, &param->opt, &param->cadence);
			if (res < 1) {
				ast_log_chan(NULL, LOG_WARNING, "Unable to determine channel for data %s\n", data);
				return NULL;
			} else {
				param->channelmatch = x;
			}
		...
		if (subdir) {
			...
			if (stat(path, &stbuf) < 0) {
				ast_log_chan(NULL, LOG_WARNING, "stat(%s) failed: %s\n",
						path, strerror(errno));
				return NULL;
			}
			if (!S_ISCHR(stbuf.st_mode)) {
				ast_log_chan(NULL, LOG_ERROR, "%s: Not a character device file\n",
						path);
				return NULL;
			}
	...
	if (param->opt == 'r' && res < 3) {
		ast_log_chan(NULL, LOG_WARNING, "Distinctive ring missing identifier in '%s'\n", data);
		param->opt = '\0';
	}
	...
	while (p && !tmp) {
		...
		if (is_group_or_channel_match(p, start.span, start.groupmatch, &groupmatched, start.channelmatch, &channelmatched)
			...
			case 'd':
				...
				break;
			default:
				ast_log_chan(NULL, LOG_WARNING, "Unknown option '%c' in '%s'\n", start.opt, data);
				break;
			}
 ...
 * \since 1.8
 *
 * \param inbound Incoming gabpbx channel.
...
static void *mfcr2_monitor(void *data)
{
	...
	while (1) {
		...
		if ((res < 0) && (errno != EINTR)) {
			ast_log_chan(NULL, LOG_ERROR, "going out, poll failed: %s\n", strerror(errno));
			break;
		}
	...
	ast_log_chan(NULL, LOG_NOTICE, "Quitting MFC/R2 monitor thread\n");
	return 0;
}
#endif /* HAVE_OPENR2 */
...
static int prepare_pri(struct dahdi_pri *pri)
{
	...
	for (i = 0; i < SIG_PRI_NUM_DCHANS; i++) {
		...
		if ((pri->pri.fds[i] < 0)) {
			ast_log_chan(NULL, LOG_ERROR, "Unable to open D-channel (fd=%d) (%s)\n",
		...
		if (res) {
			...
			ast_log_chan(NULL, LOG_ERROR, "Unable to SPECIFY channel %d (%s)\n", x, strerror(errno));
			return -1;
		}
		memset(&p, 0, sizeof(p));
		...
		if (res) {
			dahdi_close_pri_fd(pri, i);
			ast_log_chan(NULL, LOG_ERROR, "Unable to get parameters for D-channel %d (%s)\n", x, strerror(errno));
			return -1;
		}
		if ((p.sigtype != DAHDI_SIG_HDLCFCS) && (p.sigtype != DAHDI_SIG_HARDHDLC)) {
			dahdi_close_pri_fd(pri, i);
			ast_log_chan(NULL, LOG_ERROR, "D-channel %d is not in HDLC/FCS mode.\n", x);
			return -1;
		}
		memset(&si, 0, sizeof(si));
		...
		if (res) {
			dahdi_close_pri_fd(pri, i);
			ast_log_chan(NULL, LOG_ERROR, "Unable to get span state for D-channel %d (%s)\n", x, strerror(errno));
		}
		if (!si.alarms) {
		...
		if (ioctl(pri->pri.fds[i], DAHDI_SET_BUFINFO, &bi)) {
			ast_log_chan(NULL, LOG_ERROR, "Unable to set appropriate buffering on channel %d: %s\n", x, strerror(errno));
			dahdi_close_pri_fd(pri, i);
			return -1;
		}
...
static char *handle_pri_service_generic(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a, int changestatus)
{
	...
	for (tmp = iflist; tmp; tmp = tmp->next) {
		if (tmp->pri && tmp->channel == channel) {
			...
			/* case 4:  -- shutdown */
			default:
				ast_log_chan(NULL, LOG_WARNING, "Unsupported changestatus: '%d'\n", changestatus);
				break;
			}
...
static char *handle_pri_show_channels(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	case CLI_INIT:
		...
		e->usage =
			...
			"       of DAHDI B channels to GABpbx channel names and which calls\n"
...
static void pri_destroy_span(struct sig_pri_span *pri)
{
	...
	if (res != 0) {
		ast_log_chan(NULL, LOG_NOTICE, "pthread_join failed: %d\n", res);
	}
...
static int dahdi_restart(void)
{
	...
	ast_verb_chan(NULL, 1, "Destroying channels and reloading DAHDI configuration.\n");
	dahdi_softhangup_all();
	ast_verb_chan(NULL, 4, "Initial softhangup of all DAHDI channels complete.\n");
#ifdef HAVE_OPENR2
	...
	dahdi_softhangup_all();
	ast_verb_chan(NULL, 4, "Final softhangup of all DAHDI channels complete.\n");
	...
	if (setup_dahdi(2) != 0) {
		ast_log_chan(NULL, LOG_WARNING, "Reload channels from dahdi config failed!\n");
		ast_mutex_unlock(&ss_thread_lock);
		return 1;
	}
	ast_mutex_unlock(&ss_thread_lock);
		...
		if (tmp->channel == channel) {
			...
			if (tmp->subs[SUB_REAL].dfd > -1) {
				...
				if (ioctl(tmp->subs[SUB_REAL].dfd, DAHDI_GET_PARAMS, &ps) < 0) {
					ast_log_chan(NULL, LOG_WARNING, "Failed to get parameters on channel %d: %s\n", tmp->channel, strerror(errno));
				} else {
...
static int dahdi_fake_event(struct dahdi_pvt *p, int mode)
{
	if (p) {
		...
		case HANGUP:
			...
			break;
		default:
			ast_log_chan(NULL, LOG_WARNING, "I don't know how to handle transfer event with this: %d on channel %s\n",mode, ast_channel_name(p->owner));
		}
	}
	return 0;
}
...
static int linkset_addsigchan(int sigchan)
{
	...
	if (sigchan < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Invalid sigchan!\n");
		return -1;
	}
	if (cur_ss7type < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Unspecified or invalid ss7type\n");
		return -1;
	}
	if (cur_pointcode < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Unspecified pointcode!\n");
		return -1;
	}
	if (cur_adjpointcode < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Unspecified adjpointcode!\n");
		return -1;
	}
	if (cur_defaultdpc < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Unspecified defaultdpc!\n");
		return -1;
	}
	if (cur_networkindicator < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Invalid networkindicator!\n");
		return -1;
	}
	link = ss7_resolve_linkset(cur_linkset);
	if (!link) {
		ast_log_chan(NULL, LOG_ERROR, "Invalid linkset number.  Must be between 1 and %d\n", NUM_SPANS + 1);
		return -1;
	}
	if (link->ss7.numsigchans >= SIG_SS7_NUM_DCHANS) {
		ast_log_chan(NULL, LOG_ERROR, "Too many sigchans on linkset %d\n", cur_linkset);
		return -1;
	}
	...
	if (link->ss7.fds[curfd] < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to open SS7 sigchan %d (%s)\n", sigchan,
			strerror(errno));
		return -1;
	}
	if (ioctl(link->ss7.fds[curfd], DAHDI_SPECIFY, &sigchan) == -1) {
		dahdi_close_ss7_fd(link, curfd);
		ast_log_chan(NULL, LOG_ERROR, "Unable to specify SS7 sigchan %d (%s)\n", sigchan,
			strerror(errno));
		return -1;
	}
	...
	if (res) {
		dahdi_close_ss7_fd(link, curfd);
		ast_log_chan(NULL, LOG_ERROR, "Unable to get parameters for sigchan %d (%s)\n", sigchan,
			strerror(errno));
		return -1;
	}
	if (params.sigtype != DAHDI_SIG_HDLCFCS
		...
		dahdi_close_ss7_fd(link, curfd);
		ast_log_chan(NULL, LOG_ERROR, "sigchan %d is not in HDLC/FCS mode.\n", sigchan);
		return -1;
	}
	...
	if (res) {
		dahdi_close_ss7_fd(link, curfd);
		ast_log_chan(NULL, LOG_ERROR, "Unable to get span state for sigchan %d (%s)\n", sigchan,
			strerror(errno));
	}
...
static int build_channels(struct dahdi_chan_conf *conf, const char *value, int reload, int lineno)
{
	...
	if ((reload == 0) && (conf->chan.sig < 0) && !conf->is_sig_auto) {
		ast_log_chan(NULL, LOG_ERROR, "Signalling must be specified before any channels are.\n");
		return -1;
	}
	...
	while ((chan = strsep(&c, ","))) {
		if (sscanf(chan, "%30d-%30d", &start, &finish) == 2) {
			...
			ast_log_chan(NULL, LOG_ERROR, "Syntax error parsing '%s' at '%s'\n", value, chan);
			return -1;
		}
		if (finish < start) {
			ast_log_chan(NULL, LOG_WARNING, "Silliness: %d < %d\n", start, finish);
			x = finish;
			finish = start;
			start = x;
		}
		...
		for (x = start; x <= finish; x++) {
			...
			if (tmp) {
				ast_verb_chan(NULL, 3, "%s channel %d, %s signalling\n", reload ? "Reconfigured" : "Registered", x, sig2str(tmp->sig));
			} else {
				ast_log_chan(NULL, LOG_ERROR, "Unable to %s channel '%s'\n",
...
static void process_echocancel(struct dahdi_chan_conf *confp, const char *data, unsigned int line)
{
	...
	for (x = 1; x < param_count; x++) {
		...
		if (ast_app_separate_args(params[x], '=', (char **) &param, 2) < 1) {
			ast_log_chan(NULL, LOG_WARNING, "Invalid echocancel parameter supplied at line %u: '%s'\n", line, params[x]);
			continue;
		}
		...
		if (param.value) {
			if (sscanf(param.value, "%30d", &confp->chan.echocancel.params[confp->chan.echocancel.head.param_count].value) != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid echocancel parameter value supplied at line %u: '%s'\n", line, param.value);
				continue;
			}
		}
...
static void parse_busy_pattern(struct ast_variable *v, struct ast_dsp_busy_pattern *busy_cadence)
{
	...
	for (; ;) {
		...
		if (!sscanf(v->value, "%30d", &norval) && count_pattern == 0) {
			ast_log_chan(NULL, LOG_ERROR, "busypattern= expects either busypattern=tonelength,quietlength or busypattern=t1length, q1length, t2length, q2length at line %d.\n", v->lineno);
			break;
		}
...
static int process_dahdi(struct dahdi_chan_conf *confp, const char *cat, struct ast_variable *v, int reload, int options)
{
	...
	for (; v; v = v->next) {
		...
		if (!strcasecmp(v->name, "channel") || !strcasecmp(v->name, "channels")) {
			if (options & PROC_DAHDI_OPT_NOCHAN) {
				ast_log_chan(NULL, LOG_WARNING, "Channel '%s' ignored.\n", v->value);
				continue;
			}
			if (build_channels(confp, v->value, reload, v->lineno)) {
				if (confp->ignore_failed_channels) {
					ast_log_chan(NULL, LOG_WARNING, "Channel '%s' failure ignored: ignore_failed_channels.\n", v->value);
					continue;
				} else {
					return -1;
				}
			...
			if (parse_buffers_policy(v->value, &confp->chan.buf_no, &confp->chan.buf_policy)) {
				ast_log_chan(NULL, LOG_WARNING, "Using default buffer policy.\n");
			...
			if (sscanf(v->value, "%30d", &y) == 1) {
				if ((y < 10) || (y > 4000)) {
					ast_log_chan(NULL, LOG_WARNING, "Echo training time must be within the range of 10 to 4000 ms at line %d.\n", v->lineno);
				} else {
			...
			ast_log_chan(NULL, LOG_NOTICE, "Configuration option \"%s\" has been deprecated. Please use dialplan instead\n", v->name);
			...
			if (!((confp->chan.sig == SIG_FXOKS) || (confp->chan.sig == SIG_FXOGS) || (confp->chan.sig == SIG_FXOLS))) {
				ast_log_chan(NULL, LOG_WARNING, "Only FXO signalled channels may belong to a call group\n");
			}
			...
			if (!((confp->chan.sig == SIG_FXOKS) || (confp->chan.sig == SIG_FXOGS) || (confp->chan.sig == SIG_FXOLS))) {
				ast_log_chan(NULL, LOG_WARNING, "Only FXO signalled channels may belong to a pickup group\n");
			}
			if (!strcasecmp(v->value, "none"))
			...
			if (!((confp->chan.sig == SIG_FXOKS) || (confp->chan.sig == SIG_FXOGS) || (confp->chan.sig == SIG_FXOLS))) {
				ast_log_chan(NULL, LOG_WARNING, "Only FXO signalled channels may belong to a named call group\n");
			}
			confp->chan.named_callgroups = ast_get_namedgroups(v->value);
		} else if (!strcasecmp(v->name, "namedpickupgroup")) {
			if (!((confp->chan.sig == SIG_FXOKS) || (confp->chan.sig == SIG_FXOGS) || (confp->chan.sig == SIG_FXOLS))) {
				ast_log_chan(NULL, LOG_WARNING, "Only FXO signalled channels may belong to a named pickup group\n");
			}
			...
			if (strcasecmp(v->value, "disabled")) {
				if (sscanf(v->value, "%30f", &confp->chan.hwrxgain) == 1) {
					...
					ast_log_chan(NULL, LOG_WARNING, "Invalid hwrxgain: %s at line %d.\n", v->value, v->lineno);
				}
			}
			...
			if (strcasecmp(v->value, "disabled")) {
				if (sscanf(v->value, "%30f", &confp->chan.hwtxgain) == 1) {
					...
					ast_log_chan(NULL, LOG_WARNING, "Invalid hwtxgain: %s at line %d.\n", v->value, v->lineno);
				}
			}
		} else if (!strcasecmp(v->name, "cid_rxgain")) {
			if (sscanf(v->value, "%30f", &confp->chan.cid_rxgain) != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid cid_rxgain: %s at line %d.\n", v->value, v->lineno);
			}
		} else if (!strcasecmp(v->name, "rxgain")) {
			if (sscanf(v->value, "%30f", &confp->chan.rxgain) != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid rxgain: %s at line %d.\n", v->value, v->lineno);
			}
		} else if (!strcasecmp(v->name, "txgain")) {
			if (sscanf(v->value, "%30f", &confp->chan.txgain) != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid txgain: %s at line %d.\n", v->value, v->lineno);
			}
		} else if (!strcasecmp(v->name, "txdrc")) {
			if (sscanf(v->value, "%f", &confp->chan.txdrc) != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid txdrc: %s\n", v->value);
			}
		} else if (!strcasecmp(v->name, "rxdrc")) {
			if (sscanf(v->value, "%f", &confp->chan.rxdrc) != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid rxdrc: %s\n", v->value);
			}
		} else if (!strcasecmp(v->name, "tonezone")) {
			if (sscanf(v->value, "%30d", &confp->chan.tonezone) != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid tonezone: %s at line %d.\n", v->value, v->lineno);
			}
			...
			if (y < 0)
				ast_log_chan(NULL, LOG_WARNING, "Invalid AMA flags: %s at line %d.\n", v->value, v->lineno);
			else
			...
			if ( (strcasestr(v->value, "neon")) || (strcasestr(v->value, "hvac")) ) {	/* 90V DC pulses */
			 ...
			 if (!strcasecmp(v->name, "signalling") || !strcasecmp(v->name, "signaling")) {
				...
				if (!strcasecmp(v->value, "em")) {
					...
					confp->pri.pri.nodetype = PRI_NETWORK;
#else
					ast_log_chan(NULL, LOG_WARNING, "How cool would it be if someone implemented this mode!  For now, sucks for you. (line %d)\n", v->lineno);
#endif	/* !defined(HAVE_PRI_CALL_HOLD) */
#endif
#if defined(HAVE_SS7)
					...
					ast_log_chan(NULL, LOG_ERROR, "Unknown signalling method '%s' at line %d.\n", v->value, v->lineno);
				}
				...
				if (!strcasecmp(v->value, "national")) {
					...
					ast_log_chan(NULL, LOG_WARNING, "Unknown PRI dialplan '%s' at line %d.\n", v->value, v->lineno);
				}
				...
				if (!strcasecmp(v->value, "national")) {
					...
					ast_log_chan(NULL, LOG_WARNING, "Unknown PRI localdialplan '%s' at line %d.\n", v->value, v->lineno);
				}
				...
				if (!strcasecmp(v->value, "national")) {
					...
					ast_log_chan(NULL, LOG_WARNING, "Unknown PRI cpndialplan '%s' at line %d.\n", v->value, v->lineno);
				}
				...
				if (!strcasecmp(v->value, "national"))
					...
					ast_log_chan(NULL, LOG_ERROR, "Unknown switchtype '%s' at line %d.\n", v->value, v->lineno);
					return -1;
				}
				...
				if (!strcasecmp(v->value, "sdn"))
					...
					ast_log_chan(NULL, LOG_WARNING, "Unknown network-specific facility '%s' at line %d.\n", v->value, v->lineno);
					confp->pri.pri.nsf = PRI_NSF_NONE;
				}
				...
				if (!strcasecmp(v->value, "outofband"))
					...
					ast_log_chan(NULL, LOG_WARNING, "'%s' is not a valid pri indication value, should be 'inband' or 'outofband' at line %d.\n",
						v->value, v->lineno);
				...
				if (!strcasecmp(v->value, "never"))
					...
					ast_log_chan(NULL, LOG_WARNING, "'%s' is not a valid reset interval, should be >= 60 seconds or 'never' at line %d.\n",
						v->value, v->lineno);
				...
				if (!strcasecmp(v->value, "itu")) {
					...
					ast_log_chan(NULL, LOG_WARNING, "'%s' is an unknown ss7 switch type at line %d.!\n", v->value, v->lineno);
				}
				...
				if (!strcasecmp(v->value, "national")) {
					...
					ast_log_chan(NULL, LOG_WARNING, "Unknown SS7 called_nai '%s' at line %d.\n", v->value, v->lineno);
				}
				...
				if (!strcasecmp(v->value, "national")) {
					...
					ast_log_chan(NULL, LOG_WARNING, "Unknown SS7 calling_nai '%s' at line %d.\n", v->value, v->lineno);
				}
				...
				if (!link->ss7.ss7) {
					ast_log_chan(NULL, LOG_ERROR, "Please specify isup timers after sigchan!\n");
				} else if (!ss7_set_isup_timer(link->ss7.ss7, strstr(v->name, ".") + 1, atoi(v->value))) {
					ast_log_chan(NULL, LOG_ERROR, "Invalid isup timer %s\n", v->name);
				}
				...
				if (!link->ss7.ss7) {
					ast_log_chan(NULL, LOG_ERROR, "Please specify mtp3 timers after sigchan!\n");
				} else if (!ss7_set_mtp3_timer(link->ss7.ss7, strstr(v->name, ".") + 1, atoi(v->value))) {
					ast_log_chan(NULL, LOG_ERROR, "Invalid mtp3 timer %s\n", v->name);
				}
				...
				if (!link->ss7.ss7) {
					ast_log_chan(NULL, LOG_ERROR, "Please specify inr_if_no_calling after sigchan!\n");
				} else if (ast_true(v->value)) {
				...
				if (!link->ss7.ss7) {
					ast_log_chan(NULL, LOG_ERROR, "Please specify non_isdn_access after sigchan!\n");
				} else if (ast_true(v->value)) {
				...
				if (sls_shift < 0 || sls_shift > 7) {
					ast_log_chan(NULL, LOG_ERROR, "Invalid sls_shift value.  Must be between 0 and 7\n");
					return -1;
				}
				...
				if (!link->ss7.ss7) {
					ast_log_chan(NULL, LOG_ERROR, "Please specify sls_shift after sigchan!\n");
				} else {
				...
				if (cause_location < 0 || cause_location > 15) {
					ast_log_chan(NULL, LOG_ERROR, "Invalid cause_location value.  Must be between 0 and 15\n");
					return -1;
				}
				link = ss7_resolve_linkset(cur_linkset);
				if (!link) {
				...
				if (!link->ss7.ss7) {
					ast_log_chan(NULL, LOG_ERROR, "Please specify cause_location after sigchan!\n");
				} else {
				...
				ast_log_chan(NULL, LOG_WARNING, "MFC/R2 Protocol file '%s' will be used, you only should use this if you *REALLY KNOW WHAT YOU ARE DOING*.\n", confp->mfcr2.r2proto_file);
				...
				if (OR2_VAR_UNKNOWN == confp->mfcr2.variant) {
					ast_log_chan(NULL, LOG_WARNING, "Unknown MFC/R2 variant '%s' at line %d, defaulting to ITU.\n", v->value, v->lineno);
				...
				if (!confp->mfcr2.mfback_timeout) {
					ast_log_chan(NULL, LOG_WARNING, "MF timeout of 0? hum, I will protect you from your ignorance. Setting default.\n");
					confp->mfcr2.mfback_timeout = -1;
				} else if (confp->mfcr2.mfback_timeout > 0 && confp->mfcr2.mfback_timeout < 500) {
					ast_log_chan(NULL, LOG_WARNING, "MF timeout less than 500ms is not recommended, you have been warned!\n");
				}
				...
				if (confp->mfcr2.metering_pulse_timeout > 500) {
					ast_log_chan(NULL, LOG_WARNING, "Metering pulse timeout greater than 500ms is not recommended, you have been warned!\n");
				}
#if defined(OR2_LIB_INTERFACE) && OR2_LIB_INTERFACE > 2
				...
				if (OR2_CALLING_PARTY_CATEGORY_UNKNOWN == confp->mfcr2.category) {
					...
					ast_log_chan(NULL, LOG_WARNING, "Invalid MFC/R2 caller category '%s' at line %d. Using national subscriber as default.\n",
							v->value, v->lineno);
				}
				...
				while (logval) {
					...
					if (-1 == (tmplevel = openr2_log_get_level(clevel))) {
						ast_log_chan(NULL, LOG_WARNING, "Ignoring invalid logging level: '%s' at line %d.\n", clevel, v->lineno);
						continue;
					}
				...
				if (element_count % 2 == 1) {
					ast_log_chan(NULL, LOG_ERROR, "Must be a silence duration for each ring duration: %s at line %d.\n", original_args, v->lineno);
					cadence_is_ok = 0;
				}
				...
				for (i = 0; i < element_count; i++) {
					if (c[i] == 0) {
						ast_log_chan(NULL, LOG_ERROR, "Ring or silence duration cannot be zero: %s at line %d.\n", original_args, v->lineno);
						cadence_is_ok = 0;
						break;
					} else if (c[i] < 0) {
						if (i % 2 == 1) {
							...
							if (cid_location == -1) {
								...
								ast_log_chan(NULL, LOG_ERROR, "CID location specified twice: %s at line %d.\n", original_args, v->lineno);
								cadence_is_ok = 0;
								break;
							}
						} else {
							if (firstcadencepos == 0) {
								firstcadencepos = i; /* only recorded to avoid duplicate specification */
								 ...
								 ast_log_chan(NULL, LOG_ERROR, "First cadence position specified twice: %s at line %d.\n", original_args, v->lineno);
								cadence_is_ok = 0;
								break;
							}
						}
				...
				if (cadence_is_ok) {
					...
					if (element_count < 2) {
						ast_log_chan(NULL, LOG_ERROR, "Minimum cadence is ring,pause: %s at line %d.\n", original_args, v->lineno);
					} else {
						if (cid_location == -1) {
						...
						if ((num_cadence+1) >= NUM_CADENCE_MAX)
							ast_log_chan(NULL, LOG_ERROR, "Already %d cadences; can't add another: %s at line %d.\n", NUM_CADENCE_MAX, original_args, v->lineno);
						else {
							cadences[num_cadence] = new_cadence;
							cidrings[num_cadence++] = cid_location;
							ast_verb_chan(NULL, 3, "cadence 'r%d' added: %s\n",num_cadence,original_args);
						}
					}
				}
				...
				if (ctlfd == -1) {
					ast_log_chan(NULL, LOG_ERROR, "Unable to open /dev/dahdi/ctl to set toneduration at line %d.\n", v->lineno);
					return -1;
				}
				...
				if (toneduration > -1) {
					...
					if (res < 0) {
						ast_log_chan(NULL, LOG_ERROR, "Invalid tone duration: %d ms at line %d: %s\n", toneduration, v->lineno, strerror(errno));
						close(ctlfd);
						return -1;
					}
				}
			...
			ast_log_chan(NULL, LOG_NOTICE, "Ignoring any changes to '%s' (on reload) at line %d.\n", v->name, v->lineno);
	}
	...
	if (!has_pseudo && reload != 1 && !(options & PROC_DAHDI_OPT_NOCHAN)) {
		...
		*/
		...
		if (tmp) {
			ast_verb_chan(NULL, 3, "Automatically generated pseudo channel\n");
			has_pseudo = 1;
		} else {
			ast_log_chan(NULL, LOG_WARNING, "Unable to register pseudo channel!\n");
		}
		ast_cc_config_params_destroy(conf.chan.cc_params);
	}
...
static int setup_dahdi_int(int reload, struct dahdi_chan_conf *default_conf, struct dahdi_chan_conf *base_conf, struct dahdi_chan_conf *conf)
{
	...
	if (!cfg) {
		...
		if (had_cfg_before) {
			ast_log_chan(NULL, LOG_ERROR, "Unable to load config %s\n", config);
		...
		if (ucfg == CONFIG_STATUS_FILEINVALID) {
			ast_log_chan(NULL, LOG_ERROR, "File users.conf cannot be parsed.  Aborting.\n");
			ast_config_destroy(cfg);
			return 0;
		}
		...
		if (!cfg) {
			if (had_cfg_before) {
				...
				ast_log_chan(NULL, LOG_ERROR, "Bad. Unable to load config %s\n", config);
			...
			ast_log_chan(NULL, LOG_ERROR, "File %s cannot be parsed.  Aborting.\n", config);
			ast_config_destroy(ucfg);
			return 0;
		}
	} else if (cfg == CONFIG_STATUS_FILEINVALID) {
				...
				if (trunkgroup > 0) {
					if ((c = strchr(v->value, ','))) {
						...
						while (c && (i < SIG_PRI_NUM_DCHANS)) {
							...
							if (dchannels[i] < 0) {
								ast_log_chan(NULL, LOG_WARNING, "D-channel for trunk group %d must be a positive number at line %d of chan_dahdi.conf\n", trunkgroup, v->lineno);
							} else
								i++;
							c = strchr(c + 1, ',');
						}
						if (i) {
							if (pri_create_trunkgroup(trunkgroup, dchannels)) {
								ast_log_chan(NULL, LOG_WARNING, "Unable to create trunk group %d with Primary D-channel %d at line %d of chan_dahdi.conf\n", trunkgroup, dchannels[0], v->lineno);
						} else
								ast_verb_chan(NULL, 2, "Created trunk group %d with Primary D-channel %d and %d backup%s\n", trunkgroup, dchannels[0], i - 1, (i == 1) ? "" : "s");
						} else
							ast_log_chan(NULL, LOG_WARNING, "Trunk group %d lacks any valid D-channels at line %d of chan_dahdi.conf\n", trunkgroup, v->lineno);
					} else
						ast_log_chan(NULL, LOG_WARNING, "Trunk group %d lacks a primary D-channel at line %d of chan_dahdi.conf\n", trunkgroup, v->lineno);
				} else
					ast_log_chan(NULL, LOG_WARNING, "Trunk group identifier must be a positive integer at line %d of chan_dahdi.conf\n", v->lineno);
						...
						if (trunkgroup > 0) {
							...
							if (logicalspan >= 0) {
								if (pri_create_spanmap(spanno - 1, trunkgroup, logicalspan)) {
									ast_log_chan(NULL, LOG_WARNING, "Failed to map span %d to trunk group %d (logical span %d)\n", spanno, trunkgroup, logicalspan);
							} else
									ast_verb_chan(NULL, 2, "Mapped span %d to trunk group %d (logical span %d)\n", spanno, trunkgroup, logicalspan);
							} else
								ast_log_chan(NULL, LOG_WARNING, "Logical span must be a positive number, or '0' (for unspecified) at line %d of chan_dahdi.conf\n", v->lineno);
						} else
							ast_log_chan(NULL, LOG_WARNING, "Trunk group must be a positive number at line %d of chan_dahdi.conf\n", v->lineno);
					} else
						ast_log_chan(NULL, LOG_WARNING, "Missing trunk group for span map at line %d of chan_dahdi.conf\n", v->lineno);
				} else
					ast_log_chan(NULL, LOG_WARNING, "Span number must be a positive integer at line %d of chan_dahdi.conf\n", v->lineno);
			} else {
				ast_log_chan(NULL, LOG_NOTICE, "Ignoring unknown keyword '%s' in trunkgroups\n", v->name);
			}
			v = v->next;
		}
	}
		...
		for (x = 0; x < NUM_SPANS; x++) {
			if (pris[x].pri.pvts[0] &&
				...
				if (sig_pri_start_pri(&pris[x].pri)) {
					ast_log_chan(NULL, LOG_ERROR, "Unable to start D-channel on span %d\n", x + 1);
					return -1;
				} else
					ast_verb_chan(NULL, 2, "Starting D-Channel on span %d\n", x + 1);
			}
		}
	}
#endif
		...
		for (x = 0; x < NUM_SPANS; x++) {
			if (linksets[x].ss7.ss7) {
				if (ast_pthread_create(&linksets[x].ss7.master, NULL, ss7_linkset, &linksets[x].ss7)) {
					ast_log_chan(NULL, LOG_ERROR, "Unable to start SS7 linkset on span %d\n", x + 1);
					return -1;
				} else
					ast_verb_chan(NULL, 2, "Starting SS7 linkset on span %d\n", x + 1);
			}
		}
	}
#endif	/* defined(HAVE_SS7) */
		...
		AST_LIST_TRAVERSE(&r2links, cur, list) {
			...
			if (r2->r2master == AST_PTHREADT_NULL) {
				if (ast_pthread_create(&r2->r2master, NULL, mfcr2_monitor, r2)) {
					ast_log_chan(NULL, LOG_ERROR, "Unable to start R2 monitor on channel group %d\n", x + 1);
					return -1;
				} else {
					ast_verb_chan(NULL, 2, "Starting R2 monitor on channel group %d\n", x + 1);
				}
				x++;
			}
		}
...
static int load_module(void)
{
	...
	if (ast_channel_register(&dahdi_tech)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to register channel class 'DAHDI'\n");
		__unload_module();
		return AST_MODULE_LOAD_DECLINE;
	}
#ifdef HAVE_PRI
...
#define	ASCII_BYTES_PER_CHAR 80
	...
	if (idx < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Huh?  I don't exist?\n");
		return -1;
	}
	if ((!p->tdd) && (!p->mate)) {
#if defined(HAVE_PRI)
	...
	if (p->mate) {
		...
		if (len < 1) {
			ast_log_chan(NULL, LOG_ERROR, "TDD generate (len %d) failed!!\n", (int)strlen(text));
			ast_free(mybuf);
			return -1;
		}
	}
...
static int reload(void)
{
	...
	res = setup_dahdi(1);
	if (res) {
		ast_log_chan(NULL, LOG_WARNING, "Reload of chan_dahdi.so is unsuccessful!\n");
		return -1;
	}
	return 0;
}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, tdesc,
