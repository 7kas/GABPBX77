...
/*! \file
 *
 * \brief Implementation of Inter-GABpbx eXchange Version 2
...
static const char tdesc[] = "Inter GABpbx eXchange Driver (Ver 2)";
...
#define IAX_DEBUGDIGEST(msg, key) do { \
		...
		\
		ast_log_chan(NULL, LOG_NOTICE, msg " IAX_COMMAND_RTKEY to rotate key to '%s'\n", digest); \
	} while(0)
 ...
 * However, to maintain old behavior for GABpbx 1.4, these are set to
...
{
	ast_log_chan(NULL, LOG_WARNING, "%s", data);
}
...
static void __attribute__((format(printf, 1, 2))) jb_error_output(const char *fmt, ...)
{
	...
	ast_log_chan(NULL, LOG_ERROR, "%s", buf);
}
...
static void __attribute__((format(printf, 1, 2))) jb_warning_output(const char *fmt, ...)
{
	...
	ast_log_chan(NULL, LOG_WARNING, "%s", buf);
}
	...
	struct stasis_message *message)
{
	...
	ast_verb_chan(NULL, 1, "IAX, got a network change message, renewing all IAX registrations.\n");
	...
	struct stasis_message *message)
{
	...
	ast_log_chan(NULL, LOG_NOTICE, "Reloading chan_iax2 in response to ACL change event.\n");
	reload_config(1);
}
...
static void *iax2_dup_variable_datastore(void *old)
{
	...
	if (!newlist) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to duplicate iax2 variables\n");
		return NULL;
	}
	...
	AST_LIST_TRAVERSE(oldlist, oldvar, entries) {
		...
		if (newvar)
			...
			ast_log_chan(NULL, LOG_ERROR, "Unable to duplicate iax2 variable '%s'\n", ast_var_name(oldvar));
	}
...
static unsigned char compress_subclass(iax2_format subclass)
{
	...
	for (x = 0; x < IAX_MAX_SHIFT; x++) {
		if (subclass & (1LL << x)) {
			if (power > -1) {
				ast_log_chan(NULL, LOG_WARNING, "Can't compress subclass %lld\n", (long long) subclass);
				return 0;
			} else
				power = x;
		}
...
static int make_trunk(unsigned short callno, int locked)
{
	...
	if (iaxs[callno]->oseqno) {
		ast_log_chan(NULL, LOG_WARNING, "Can't make trunk once a call has started!\n");
		return -1;
	}
	if (callno >= TRUNK_CALL_START) {
		ast_log_chan(NULL, LOG_WARNING, "Call %d is already a trunk\n", callno);
		return -1;
	}
	...
	if (get_unused_callno(
			...
			&entry)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to trunk call: Insufficient space\n");
		return -1;
	}
...
static void store_by_transfercallno(struct chan_iax2_pvt *pvt)
{
	if (!pvt->transfercallno) {
		ast_log_chan(NULL, LOG_ERROR, "This should not be called without a transfer call number.\n");
		return;
	}
...
{
	if (!pvt->peercallno) {
		ast_log_chan(NULL, LOG_ERROR, "This should not be called without a peer call number.\n");
		return;
	}
...
static int peercnt_add(struct ast_sockaddr *addr)
{
	...
	if (peercnt->limit > peercnt->cur) {
		...
		ast_log_chan(NULL, LOG_ERROR, "maxcallnumber limit of %d for %s has been reached!\n", peercnt->limit, ast_sockaddr_stringify_addr(addr));
		res = -1;
	}
...
static void build_callno_limits(struct ast_variable *v)
{
	...
	for (; v; v = v->next) {
		...
		if (error) {
			ast_log_chan(NULL, LOG_ERROR, "Call number limit for %s could not be added, Invalid address range\n.", v->name);
			continue;
		} else if ((sscanf(v->value, "%d", &limit) != 1) || (limit < 0)) {
			ast_log_chan(NULL, LOG_ERROR, "Call number limit for %s could not be added. Invalid limit %s\n.", v->name, v->value);
			ast_free_ha(ha);
			continue;
		}
...
static int add_calltoken_ignore(const char *addr)
{
	...
	if (ast_strlen_zero(addr)) {
		ast_log_chan(NULL, LOG_WARNING, "invalid calltokenoptional (null)\n");
		return -1;
	}
	...
	/* check for valid config information */
	if (error) {
		ast_log_chan(NULL, LOG_WARNING, "Error %d creating calltokenoptional entry %s\n", error, addr);
		return -1;
	}
...
static int get_unused_callno(enum callno_type type, int validated, callno_entry *entry)
{
	...
	*entry = 0;
	...
	if (!pool->available) {
		ast_log_chan(NULL, LOG_WARNING, "Out of call numbers\n");
...
static int __find_callno(unsigned short callno, unsigned short dcallno, struct ast_sockaddr *addr, int new, int sockfd, int return_locked, int check_dcallno)
{
	...
	if (!res && (new >= NEW_ALLOW)) {
		...
		if (get_unused_callno(CALLNO_TYPE_NORMAL, validated, &entry)) {
			...
			ast_log_chan(NULL, LOG_WARNING, "No more space\n");
			return 0;
		}
		x = CALLNO_ENTRY_GET_CALLNO(entry);
		ast_mutex_lock(&iaxsl[x]);
		...
		if (iaxs[x]) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Out of resources\n");
			ast_mutex_unlock(&iaxsl[x]);
...
/*!
 * \brief Queue a frame to a call's owning gabpbx channel
 *
...
static int handle_error(void)
{
	...
	if (res < 0)
		ast_log_chan(NULL, LOG_WARNING, "Error detected, but unable to read error: %s\n", strerror(errno));
	else {
		if (m.msg_controllen) {
			sin = (struct sockaddr_in *)SO_EE_OFFENDER(&e);
			if (sin)
				ast_log_chan(NULL, LOG_WARNING, "Receive error from %s\n", ast_inet_ntoa(sin->sin_addr));
			else
				ast_log_chan(NULL, LOG_WARNING, "No address detected??\n");
		} else {
			ast_log_chan(NULL, LOG_WARNING, "Local error: %s\n", strerror(e.ee_errno));
		}
	}
#endif
	return 0;
		...
		if (f->retries < 0) {
			...
			if (f->transfer) {
				...
				if (iaxs[callno]->owner) {
					ast_log_chan(NULL, LOG_WARNING, "Max retries exceeded to host %s on %s (type = %u, subclass = %d, ts=%u, seqno=%d)\n",
...
static void __get_from_jb(const void *p)
{
	...
	if (ms >= (next = jb_next(pvt->jb))) {
		...
		if (!voicefmt) {
			...
			ast_log_chan(NULL, LOG_ERROR, "No voice, peer, or chosen format available on %s, backlogging frame\n", ast_channel_name(pvt->owner));
...
static int iax2_fixup(struct ast_channel *oldchannel, struct ast_channel *newchan)
{
	...
	if (iaxs[callno])
		...
		ast_log_chan(NULL, LOG_WARNING, "Uh, this isn't a good sign...\n");
	ast_mutex_unlock(&iaxsl[callno]);
	return 0;
}
	...
	for (tmp = var; tmp; tmp = tmp->next) {
		...
		if (!strcasecmp(tmp->name, "type")) {
			...
			if (ast_parse_arg(tmp->value, PARSE_ADDR | PARSE_PORT_FORBID, NULL)) {
				ast_log_chan(NULL, LOG_WARNING, "Failed to parse sockaddr '%s' for ipaddr of realtime peer '%s'\n", tmp->value, tmp->name);
			} else {
...
static int create_addr(const char *peername, struct ast_channel *c, struct ast_sockaddr *addr, struct create_addr_info *cai)
{
	...
	if (!(peer = find_peer(peername, 1))) {
		...
		if (ast_get_ip_or_srv(&peer_addr, peername, srvlookup ? "_iax._udp" : NULL)) {
			ast_log_chan(NULL, LOG_WARNING, "No such host: %s\n", peername);
			return -1;
		}
	...
	if (ast_strlen_zero(peer->dbsecret)) {
		...
		if (!key || ast_db_get(family, key, cai->secret, sizeof(cai->secret))) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to retrieve database password for family/key '%s'!\n", peer->dbsecret);
			goto return_unref;
		}
	}
...
static void __auto_congest(const void *nothing)
{
	...
	if (iaxs[callno]) {
		...
		ast_log_chan(NULL, LOG_NOTICE, "Auto-congesting call due to slow response\n");
	}
	ast_mutex_unlock(&iaxsl[callno]);
}
...
#define CALLTOKEN_IE_FORMAT   "%u?%s"     /* time + ? + (40 char hash) */
	...
	/* ----- Case 2 ----- */
		...
		if (strcmp(hash, rec_hash)) {
			ast_log_chan(NULL, LOG_WARNING, "Address %s failed CallToken hash inspection\n", ast_sockaddr_stringify(addr));
			goto reject; /* received hash does not match ours, reject */
		} else if ((t < rec_time) || ((t - rec_time) >= max_calltoken_delay)) {
			ast_log_chan(NULL, LOG_WARNING, "Too much delay in IAX2 calltoken timestamp from address %s\n", ast_sockaddr_stringify(addr));
	...
	/* ----- Case 3 ----- */
		...
		if (calltoken_required(addr, ies->username, subclass)) {
			ast_log_chan(NULL, LOG_ERROR, "Call rejected, CallToken Support required. If unexpected, resolve by placing address %s in the calltokenoptional list or setting user %s requirecalltoken=no\n", ast_sockaddr_stringify(addr), S_OR(ies->username, "guest"));
			goto reject;
		}
...
static int iax2_call(struct ast_channel *c, const char *dest, int timeout)
{
	...
	if ((ast_channel_state(c) != AST_STATE_DOWN) && (ast_channel_state(c) != AST_STATE_RESERVED)) {
		ast_log_chan(NULL, LOG_WARNING, "Channel is already in use (%s)?\n", ast_channel_name(c));
		return -1;
	}
	...
	if (ast_strlen_zero(pds.peer)) {
		ast_log_chan(NULL, LOG_WARNING, "No peer provided in the IAX2 dial string '%s'\n", dest);
		return -1;
	}
	if (!pds.exten) {
	...
	if (create_addr(pds.peer, c, &addr, &cai)) {
		ast_log_chan(NULL, LOG_WARNING, "No address associated with '%s'\n", pds.peer);
		return -1;
	}
	...
	if (ast_test_flag64(&cai, IAX_FORCE_ENCRYPT) ||
		...
		if (!cai.encmethods) {
			ast_log_chan(NULL, LOG_WARNING, "Encryption forced for call, but not enabled\n");
		...
		if (((cai.authmethods & IAX_AUTH_RSA) || (cai.authmethods & IAX_AUTH_MD5) || (cai.authmethods & IAX_AUTH_PLAINTEXT)) &&
			ast_strlen_zero(cai.secret) && ast_strlen_zero(pds.password)) {
		        ast_log_chan(NULL, LOG_WARNING, "Call terminated. Encryption forced but no secret provided\n");
		        return -1;
		}
	}
	...
	if (!ast_strlen_zero(osp_token_ptr)) {
		if ((osp_token_length = strlen(osp_token_ptr)) <= IAX_MAX_OSPTOKEN_SIZE) {
			...
			ast_log_chan(NULL, LOG_WARNING, "OSP token is too long\n");
	} else if (iaxdebug)
	...
	if (callno && iaxs[callno]) {
		...
		if (!iaxs[callno]->error && !alreadygone) {
			if (send_command_final(iaxs[callno], AST_FRAME_IAX, IAX_COMMAND_HANGUP, 0, ied.buf, ied.pos, -1)) {
				ast_log_chan(NULL, LOG_WARNING, "No final packet could be sent for callno %d\n", callno);
			}
			if (!iaxs[callno]) {
				ast_mutex_unlock(&iaxsl[callno]);
				return 0;
		...
		if (iaxs[callno] && alreadygone) {
			...
			if (ast_sched_add(sched, 10000, scheduled_destroy, CALLNO_TO_PTR(callno)) < 0) {
				ast_log_chan(NULL, LOG_ERROR, "Unable to schedule iax2 callno %d destruction?!!  Destroying immediately.\n", callno);
				iax2_destroy(callno);
			}
		}
...
static enum ast_bridge_result iax2_bridge(struct ast_channel *c0, struct ast_channel *c1, int flags, struct ast_frame **fo, struct ast_channel **rc, int timeoutms)
{
	...
	for (/* ever */;;) {
		...
		if ((ast_channel_tech(c0) != &iax2_tech) || (ast_channel_tech(c1) != &iax2_tech)) {
			ast_verb_chan(NULL, 3, "Can't masquerade, we're different...\n");
		...
		if (!(ast_format_cap_identical(ast_channel_nativeformats(c0), ast_channel_nativeformats(c1)))) {
			...
			ast_verb_chan(NULL, 3, "Operating with different codecs [%s] [%s] , can't native bridge...\n",
		...
		if (!transferstarted && !ast_test_flag64(iaxs[callno0], IAX_NOTRANSFER) && !ast_test_flag64(iaxs[callno1], IAX_NOTRANSFER)) {
			...
			if (iax2_start_transfer(callno0, callno1, (flags & (AST_BRIDGE_DTMF_CHANNEL_0 | AST_BRIDGE_DTMF_CHANNEL_1)) ||
				...
				ast_log_chan(NULL, LOG_WARNING, "Unable to start the transfer\n");
			transferstarted = 1;
		}
	...
	const struct ast_channel *requestor, unsigned int cachable)
{
	...
	if (!(i = iaxs[callno])) {
		ast_log_chan(NULL, LOG_WARNING, "No IAX2 pvt found for callno '%d' !\n", callno);
		return NULL;
	}

	if (!capability) {
		ast_log_chan(NULL, LOG_WARNING, "No formats specified for call to: IAX2/%s-%d\n",
			i->host, i->callno);
		return NULL;
	}
	...
	if (iax2_codec_pref_best_bitfield2cap(capability, prefs, native)
		...
		ast_log_chan(NULL, LOG_WARNING, "No requested formats available for call to: IAX2/%s-%d\n",
			i->host, i->callno);
		ao2_ref(native, -1);
		return NULL;
	}
		...
		if (variablestore && varlist) {
			...
			for (var = i->iaxvars; var; var = var->next) {
				...
				if (!newvar) {
					...
					ast_log_chan(NULL, LOG_ERROR, "Memory allocation error while processing IAX2 variables\n");
				} else {
	...
	if (tpeer) {
		...
		if (tpeer->trunkdatalen + f->datalen + 4 >= tpeer->trunkdataalloc) {
			...
			if (tpeer->trunkdataalloc < trunkmaxsize) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Maximum trunk data space exceeded to %s\n", ast_sockaddr_stringify(&tpeer->addr));
				ast_mutex_unlock(&tpeer->lock);
				return -1;
			}
		}
...
static void memcpy_decrypt(unsigned char *dst, const unsigned char *src, int len, ast_aes_decrypt_key *dcx)
{
	...
	int x;
	if (len % 16)
		ast_log_chan(NULL, LOG_WARNING, "len should be multiple of 16, not %d!\n", len);
	for (x=0;x<len;x++)
		dst[x] = src[x] ^ 0xff;
#else
...
static int iax2_send(struct chan_iax2_pvt *pvt, struct ast_frame *f, unsigned int ts, int seqno, int now, int transfer, int final)
{
	...
	if (!pvt) {
		ast_log_chan(NULL, LOG_WARNING, "No private structure for packet?\n");
		return -1;
	}
	...
	if (!fr) {
		ast_log_chan(NULL, LOG_WARNING, "Out of memory\n");
		return -1;
	}
	...
	if (!sendmini) {
		...
		if (ast_test_flag64(pvt, IAX_ENCRYPTED)) {
			if (ast_test_flag64(pvt, IAX_KEYPOPULATED)) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Supposed to send packet encrypted, but no key?\n");
		}
...
static int check_access(int callno, struct ast_sockaddr *addr, struct iax_ies *ies)
{
	...
	if (version > IAX_PROTO_VERSION) {
		ast_log_chan(NULL, LOG_WARNING, "Peer '%s' has too new a protocol version (%d) for me\n",
...
static int authenticate_verify(struct chan_iax2_pvt *p, struct iax_ies *ies)
{
	...
	if (ast_test_flag64(p, IAX_FORCE_ENCRYPT) && !p->encmethods) {
		ast_log_chan(NULL, LOG_WARNING, "Call Terminated, incoming call is unencrypted while force encrypt is enabled.\n");
		return res;
	}
	...
	if ((p->authmethods & IAX_AUTH_RSA) && !ast_strlen_zero(rsasecret) && !ast_strlen_zero(p->inkeys)) {
		...
		if (!(tmpkey = ast_strdup(p->inkeys))) {
			ast_log_chan(NULL, LOG_ERROR, "Unable to create a temporary string for parsing stored 'inkeys'\n");
			return res;
		}
		stringp = tmpkey;
		keyn = strsep(&stringp, ":");
		while(keyn) {
			...
			if (key && !ast_check_signature(key, p->challenge, rsasecret)) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Requested inkey '%s' for RSA authentication does not exist\n", keyn);
			}
			keyn = strsep(&stringp, ":");
		}
		ast_free(tmpkey);
		if (res && authdebug) {
			ast_log_chan(NULL, LOG_WARNING, "No RSA public keys on file matched incoming call\n");
		}
		...
		while((tmppw = strsep(&stringp, ";"))) {
			...
			if (!strcasecmp(requeststr, md5secret)) {
				...
				ast_log_chan(NULL, LOG_WARNING, "MD5 secret mismatch\n");
			}
		}
		...
		if (!strcmp(secret, p->secret)) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Plaintext secret mismatch\n");
		}
	}
	return res;
}
...
static int register_verify(int callno, struct ast_sockaddr *addr, struct iax_ies *ies)
{
	...
	if (ast_strlen_zero(peer)) {
		ast_log_chan(NULL, LOG_NOTICE, "Empty registration from %s\n", ast_sockaddr_stringify_addr(addr));
		return -1;
	}
	...
	if (!p || !iaxs[callno]) {
		...
		if (authdebug && !p)
			ast_log_chan(NULL, LOG_NOTICE, "No registration for peer '%s' (from %s)\n", peer, ast_sockaddr_stringify_addr(addr));
		goto return_unref;
	}

	if (!ast_test_flag64(p, IAX_DYNAMIC)) {
		if (authdebug)
			ast_log_chan(NULL, LOG_NOTICE, "Peer '%s' is not dynamic (from %s)\n", peer, ast_sockaddr_stringify_addr(addr));
		goto return_unref;
	}

	if (!ast_apply_acl(p->acl, addr, "IAX2 Peer ACL: ")) {
		if (authdebug)
			ast_log_chan(NULL, LOG_NOTICE, "Host %s denied access to register peer '%s'\n", ast_sockaddr_stringify_addr(addr), p->name);
		goto return_unref;
	}
	...
	if (!ast_strlen_zero(rsasecret) && (p->authmethods & IAX_AUTH_RSA) && !ast_strlen_zero(iaxs[callno]->challenge)) {
		if (!ast_strlen_zero(p->inkeys)) {
			...
			while(keyn) {
				...
				if (key && !ast_check_signature(key, iaxs[callno]->challenge, rsasecret)) {
					...
					ast_log_chan(NULL, LOG_WARNING, "requested inkey '%s' does not exist\n", keyn);
				keyn = strsep(&stringp, ":");
			}
			ast_free(tmpkey);
			if (!keyn) {
				if (authdebug)
					ast_log_chan(NULL, LOG_NOTICE, "Host %s failed RSA authentication with inkeys '%s'\n", peer, p->inkeys);
				goto return_unref;
			}
		} else {
			if (authdebug)
				ast_log_chan(NULL, LOG_NOTICE, "Host '%s' trying to do RSA authentication, but we have no inkeys\n", peer);
			goto return_unref;
		}
		...
		if (tmppw) {
			...
			if (authdebug)
				ast_log_chan(NULL, LOG_NOTICE, "Host %s failed MD5 authentication for '%s' (%s != %s)\n", ast_sockaddr_stringify_addr(addr), p->name, requeststr, md5secret);
			goto return_unref;
		}
		...
		if (strcmp(secret, p->secret)) {
			if (authdebug)
				ast_log_chan(NULL, LOG_NOTICE, "Host %s did not provide proper plaintext password for '%s'\n", ast_sockaddr_stringify_addr(addr), p->name);
			goto return_unref;
		} else
			ast_set_flag(&iaxs[callno]->state, IAX_STATE_AUTHENTICATED);
...
static int authenticate(const char *challenge, const char *secret, const char *keyn, int authmethods, struct iax_ie_data *ied, struct ast_sockaddr *addr, struct chan_iax2_pvt *pvt)
{
	...
	if (!ast_strlen_zero(keyn)) {
		if (!(authmethods & IAX_AUTH_RSA)) {
			if (ast_strlen_zero(secret)) {
				ast_log_chan(NULL, LOG_WARNING, "Asked to authenticate to %s with an RSA key, but they don't allow RSA authentication\n", ast_sockaddr_stringify_addr(addr));
			}
		} else if (ast_strlen_zero(challenge)) {
			ast_log_chan(NULL, LOG_WARNING, "No challenge provided for RSA authentication to %s\n", ast_sockaddr_stringify_addr(addr));
		} else {
			char sig[256];
			struct ast_key *key;
			key = ast_key_get(keyn, AST_KEY_PRIVATE);
			if (!key) {
				ast_log_chan(NULL, LOG_WARNING, "Unable to find private key '%s'\n", keyn);
			} else {
				if (ast_sign(key, (char*)challenge, sig)) {
					ast_log_chan(NULL, LOG_WARNING, "Unable to sign challenge with key\n");
					res = -1;
				} else {
	...
	if (res && !ast_strlen_zero(secret)) {
		if ((authmethods & IAX_AUTH_MD5) && !ast_strlen_zero(challenge)) {
			...
			ast_log_chan(NULL, LOG_WARNING, "No way to send secret to peer '%s' (their methods: %d)\n", ast_sockaddr_stringify_addr(addr), authmethods);
	}
	return res;
}
...
static int authenticate_reply(struct chan_iax2_pvt *p, struct ast_sockaddr *addr, struct iax_ies *ies, const char *override, const char *okey)
{
	...
	if (!(ies->authmethods & (IAX_AUTH_MD5 | IAX_AUTH_PLAINTEXT)) && (ies->authmethods & IAX_AUTH_RSA) && ast_strlen_zero(okey)) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Call terminated. RSA authentication requires an outkey\n");
		return -1;
	}
	...
	if (ies->encmethods) {
		if (ast_strlen_zero(p->secret) &&
			...
			ast_log_chan(NULL, LOG_WARNING, "Call terminated. Encryption requested by peer but no secret available locally\n");
			return -1;
		}
		...
		ast_log_chan(NULL, LOG_NOTICE, "Call initiated without encryption while forceencryption=yes option is set\n");
...
static int try_transfer(struct chan_iax2_pvt *pvt, struct iax_ies *ies)
{
	...
	if (!newcall || ast_sockaddr_isnull(&new)) {
		ast_log_chan(NULL, LOG_WARNING, "Invalid transfer request\n");
		return -1;
	}
...
static int iax2_ack_registry(struct iax_ies *ies, struct ast_sockaddr *addr, int callno)
{
	...
	reg = iaxs[callno]->reg;
	if (!reg) {
		ast_log_chan(NULL, LOG_WARNING, "Registry acknowledge on unknown registry '%s'\n", peer);
		return -1;
	}
	ast_sockaddr_copy(&oldus, &reg->us);
	oldmsgs = reg->messages;
	if (ast_sockaddr_cmp(&reg->addr, addr)) {
		ast_log_chan(NULL, LOG_WARNING, "Received unsolicited registry ack from '%s'\n", ast_sockaddr_stringify(addr));
		return -1;
	}
	...
	*/
	...
	if (ast_sockaddr_cmp(&oldus, &reg->us) || (reg->messages != oldmsgs)) {
		...
		ast_verb_chan(NULL, 3, "Registered IAX2 to '%s', who sees us as %s%s\n", ast_sockaddr_stringify(addr), ourip, msgstatus);
...
static int iax2_register(const char *value, int lineno)
{
	...
	if (!hostname) {
		ast_log_chan(NULL, LOG_WARNING, "Format for registration is user[:secret]@host[:port] at line %d\n", lineno);
		return -1;
	}
	...
	if (porta && !atoi(porta)) {
		ast_log_chan(NULL, LOG_WARNING, "%s is not a valid port number at line %d\n", porta, lineno);
		return -1;
	}
...
static void reg_source_db(struct iax2_peer *p)
{
	...
	expiry = strrchr(data, ':');
	if (!expiry) {
		ast_log_chan(NULL, LOG_NOTICE, "IAX/Registry astdb entry missing expiry: '%s'\n", data);
		return;
	}
	*expiry++ = '\0';

	if (!ast_sockaddr_parse(&p->addr, data, PARSE_PORT_REQUIRE)) {
		ast_log_chan(NULL, LOG_NOTICE, "IAX/Registry astdb host:port invalid - '%s'\n", data);
		return;
	}

	p->expiry = atoi(expiry);

	ast_verb_chan(NULL, 3, "Seeding '%s' at %s for %d\n", p->name,
...
static int update_registry(struct ast_sockaddr *addr, int callno, char *devtype, int fd, unsigned short refresh)
{
	...
	if (!(p = find_peer(peer_name, 1))) {
		ast_mutex_lock(&iaxsl[callno]);
		ast_log_chan(NULL, LOG_WARNING, "No such peer '%s'\n", peer_name);
		return -1;
	}
	ast_mutex_lock(&iaxsl[callno]);
	if (!iaxs[callno])
	...
	if (refresh > max_reg_expire) {
		ast_log_chan(NULL, LOG_NOTICE, "Restricting registration for peer '%s' to %d seconds (requested %d)\n",
	...
	if (ast_sockaddr_cmp(&p->addr, addr)) {
		...
		if (!ast_test_flag64(p, IAX_TEMPONLY) && !ast_sockaddr_isnull(addr)) {
			...
			ast_verb_chan(NULL, 3, "Registered IAX2 '%s' (%s) at %s\n",
						p->name,
			...
			ast_verb_chan(NULL, 3, "Unregistered IAX2 '%s' (%s)\n",
						p->name,
					    ast_test_flag(&iaxs[callno]->state, IAX_STATE_AUTHENTICATED) ? "AUTHENTICATED" : "UNAUTHENTICATED");
...
static int registry_rerequest(struct iax_ies *ies, int callno, struct ast_sockaddr *addr)
{
	...
	if (reg) {
		...
		if (ast_sockaddr_cmp(&reg->addr, addr)) {
			ast_log_chan(NULL, LOG_WARNING, "Received unsolicited registry authenticate request from '%s'\n", ast_sockaddr_stringify(addr));
			return -1;
		}
		if (ast_strlen_zero(reg->secret)) {
			ast_log_chan(NULL, LOG_NOTICE, "No secret associated with peer '%s'\n", reg->username);
		...
		ast_log_chan(NULL, LOG_NOTICE, "Can't reregister without a reg\n");
	return -1;
}
...
static int timing_read(int *id, int fd, short events, void *cbdata)
{
	...
	if (timer) {
		if (ast_timer_ack(timer, 1) < 0) {
			ast_log_chan(NULL, LOG_ERROR, "Timer failed acknowledge\n");
			return 0;
		}
	}
...
static void spawn_dp_lookup(int callno, const char *context, const char *callednum, const char *callerid)
{
	...
	if (ast_pthread_create_detached(&newthread, NULL, dp_lookup_thread, dpr)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to start lookup thread!\n");
	}
}
...
static int socket_read(int *id, int fd, short events, void *cbdata)
{
	...
	if (thread->buf_len < 0) {
		if (errno != ECONNREFUSED && errno != EAGAIN)
			ast_log_chan(NULL, LOG_WARNING, "Error: %s\n", strerror(errno));
		handle_error();
	...
	struct iax_frame *fr)
{
	...
	if (packet_len < sizeof(*meta)) {
		ast_log_chan(NULL, LOG_WARNING, "Rejecting packet from '%s' that is flagged as a meta frame but is too short\n",
				ast_sockaddr_stringify(addr));
		return 1;
	}
	...
	if (packet_len < (sizeof(*meta) + sizeof(*mth))) {
		ast_log_chan(NULL, LOG_WARNING, "midget meta trunk packet received (%d of %d min)\n", packet_len,
	...
	if (!tpeer) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to accept trunked packet from '%s': No matching peer\n",
				ast_sockaddr_stringify(addr));
		return 1;
	}
	tpeer->trunkact = ast_tvnow();
	...
	while (packet_len >= sizeof(*mte)) {
		...
		if (metatype == IAX_META_TRUNK_MINI) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Unknown meta trunk cmd from '%s': dropping\n", ast_sockaddr_stringify(addr));
			break;
		}
		...
		if (!iaxs[fr->callno]) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Received trunked frame before first full voice frame\n");
			iax2_vnak(fr->callno);
			...
			if (f.datalen >= 0) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Datalen < 0?\n");
			}
		}
		ast_mutex_unlock(&iaxsl[fr->callno]);
		ptr += len;
...
static int acf_iaxvar_write(struct ast_channel *chan, const char *cmd, char *data, const char *value)
{
	...
	if (!variablestore) {
		variablestore = ast_datastore_alloc(&iax2_variable_datastore_info, NULL);
		if (!variablestore) {
			ast_log_chan(NULL, LOG_ERROR, "Memory allocation error\n");
			return -1;
		}
		varlist = ast_calloc(1, sizeof(*varlist));
		if (!varlist) {
			ast_datastore_free(variablestore);
			ast_log_chan(NULL, LOG_ERROR, "Unable to assign new variable '%s'\n", data);
			return -1;
		}
...
static int socket_process_helper(struct iax2_thread *thread)
{
	...
	if (res < sizeof(*mh)) {
		ast_log_chan(NULL, LOG_WARNING, "midget packet received (%d of %d min)\n", res, (int) sizeof(*mh));
		return 1;
	}
	if ((vh->zeros == 0) && (ntohs(vh->callno) & 0x8000)) {
		if (res < sizeof(*vh)) {
			ast_log_chan(NULL, LOG_WARNING, "Rejecting packet from '%s' that is flagged as a video frame but is too short\n",
					ast_sockaddr_stringify(&addr));
			return 1;
		}
	...
	if (ntohs(mh->callno) & IAX_FLAG_FULL) {
		if (res < sizeof(*fh)) {
			ast_log_chan(NULL, LOG_WARNING, "Rejecting packet from '%s' that is flagged as a full frame but is too short\n",
				ast_sockaddr_stringify(&addr));
			return 1;
		}
		...
		if ((dcallno != 1) && (fr->callno = find_callno(ntohs(mh->callno) & ~IAX_FLAG_FULL, dcallno, &addr, NEW_PREVENT, fd, 1))) {
			...
			if (iaxs[fr->callno] && ast_test_flag64(iaxs[fr->callno], IAX_ENCRYPTED)) {
				if (decrypt_frame(fr->callno, fh, &f, &res)) {
					ast_log_chan(NULL, LOG_NOTICE, "Packet Decrypt Failed!\n");
					ast_mutex_unlock(&iaxsl[fr->callno]);
					return 1;
				}
				decrypted = 1;
		...
		if (f.datalen) {
			if (f.frametype == AST_FRAME_IAX) {
				if (iax_parse_ies(&ies, thread->buf + sizeof(struct ast_iax2_full_hdr), f.datalen)) {
					ast_log_chan(NULL, LOG_WARNING, "Undecodable frame received from '%s'\n", ast_sockaddr_stringify(&addr));
	...
	if (!fr->callno) {
		 ...
		 *    http://lists.digium.com/pipermail/gabpbx-dev/2008-May/033217.html
		 */
		...
		if (!(fr->callno = find_callno(ntohs(mh->callno) & ~IAX_FLAG_FULL, dcallno, &addr, new, fd, check_dcallno))) {
			...
			if (f.frametype == AST_FRAME_IAX && f.subclass.integer == IAX_COMMAND_NEW) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Silently dropping frame without existent call number: %d\n", ntohs(mh->callno) & ~IAX_FLAG_FULL);
			}
			ast_variables_destroy(ies.vars);
			return 1;
		}
	...
	if (ast_test_flag64(iaxs[fr->callno], IAX_ENCRYPTED) && !decrypted) {
		if (decrypt_frame(fr->callno, fh, &f, &res)) {
			ast_log_chan(NULL, LOG_WARNING, "Packet Decrypt Failed!\n");
			ast_variables_destroy(ies.vars);
			ast_mutex_unlock(&iaxsl[fr->callno]);
			return 1;
		}
		...
		if (f.frametype == AST_FRAME_IAX) {
			...
			case IAX_COMMAND_NEW:
				...
				if (check_access(fr->callno, &addr, &ies)) {
					...
					if (authdebug) {
						ast_log_chan(NULL, LOG_NOTICE, "Rejected connect attempt from %s, who was trying to reach '%s@%s'\n",
				...
				if (ast_strlen_zero(iaxs[fr->callno]->secret) && ast_test_flag64(iaxs[fr->callno], IAX_FORCE_ENCRYPT)) {
					auth_fail(fr->callno, IAX_COMMAND_REJECT);
					ast_log_chan(NULL, LOG_WARNING, "Rejected connect attempt.  No secret present while force encrypt enabled.\n");
					break;
				}
				...
				if (ast_strlen_zero(iaxs[fr->callno]->secret) && ast_strlen_zero(iaxs[fr->callno]->inkeys)) {
					if (strcmp(iaxs[fr->callno]->exten, "TBD") && !exists) {
						...
						if (authdebug) {
							ast_log_chan(NULL, LOG_NOTICE, "Rejected connect attempt from %s, request '%s@%s' does not exist\n",
									ast_sockaddr_stringify(&addr), iaxs[fr->callno]->exten, iaxs[fr->callno]->context);
						}
					} else {
						...
						if (!format) {
							...
							if (!format) {
								...
								if (authdebug) {
									...
									if (ast_test_flag64(iaxs[fr->callno], IAX_CODEC_NOCAP)) {
										ast_log_chan(NULL, LOG_NOTICE, "Rejected connect attempt from %s, requested '%s' incompatible with our capability '%s'.\n",
											ast_sockaddr_stringify(&addr),
										...
										ast_log_chan(NULL, LOG_NOTICE, "Rejected connect attempt from %s, requested/capability '%s'/'%s' incompatible with our capability '%s'.\n",
											ast_sockaddr_stringify(&addr),
											iax2_getformatname_multiple(iaxs[fr->callno]->peerformat, &peer_form_buf),
						...
						if (format) {
							...
							if (strcmp(iaxs[fr->callno]->exten, "TBD")) {
								...
								ast_verb_chan(NULL, 3, "Accepting UNAUTHENTICATED call from %s:\n"
												"%srequested format = %s,\n"
												"%srequested prefs = %s,\n"
												"%sactual format = %s,\n"
												"%shost prefs = %s,\n"
								...
								ast_verb_chan(NULL, 3, "Accepted unauthenticated TBD call from %s\n", ast_sockaddr_stringify(&addr));
							}
						}
					}
					break;
			...
			case IAX_COMMAND_REJECT:
				...
				if (!ast_test_flag64(iaxs[fr->callno], IAX_PROVISION)) {
					if (iaxs[fr->callno]->owner && authdebug)
						ast_log_chan(NULL, LOG_WARNING, "Call rejected by %s: %s\n",
							ast_sockaddr_stringify(&addr),
			...
			case IAX_COMMAND_TRANSFER:
			{
				...
				if (iaxs[fr->callno]->owner) {
					...
					if (ast_bridge_transfer_blind(1, owner, ies.called_number,
						...
						ast_log_chan(NULL, LOG_WARNING, "Blind transfer of '%s' to '%s@%s' failed\n",
			...
			case IAX_COMMAND_ACCEPT:
				...
				ast_verb_chan(NULL, 3, "Call accepted by %s (format %s)\n", ast_sockaddr_stringify(&addr),
						iax2_getformatname(iaxs[fr->callno]->peerformat));
				if (!(iaxs[fr->callno]->peerformat & iaxs[fr->callno]->capability)) {
					memset(&ied0, 0, sizeof(ied0));
					iax_ie_append_str(&ied0, IAX_IE_CAUSE, "Unable to negotiate codec");
					...
					if (authdebug) {
						...
						ast_log_chan(NULL, LOG_NOTICE, "Rejected call to %s, format %s incompatible with our capability %s.\n",
							ast_sockaddr_stringify(&addr),
				...
				if (iaxs[fr->callno]->peerpoke) {
					...
					if ((peer->lastms < 0)  || (peer->historicms > peer->maxms)) {
						if (iaxs[fr->callno]->pingtime <= peer->maxms) {
							ast_log_chan(NULL, LOG_NOTICE, "Peer '%s' is now REACHABLE! Time: %u\n", peer->name, iaxs[fr->callno]->pingtime);
						...
						if (iaxs[fr->callno]->pingtime > peer->maxms) {
							ast_log_chan(NULL, LOG_NOTICE, "Peer '%s' is now TOO LAGGED (%u ms)!\n", peer->name, iaxs[fr->callno]->pingtime);
							ast_endpoint_set_state(peer->endpoint, AST_ENDPOINT_ONLINE);
							blob = ast_json_pack("{s: s, s: I}",
								"peer_status", "Lagged",
								"time", (ast_json_int_t)iaxs[fr->callno]->pingtime);
			...
			case IAX_COMMAND_AUTHREQ:
				if (ast_test_flag(&iaxs[fr->callno]->state, IAX_STATE_STARTED | IAX_STATE_TBD)) {
					ast_log_chan(NULL, LOG_WARNING, "Call on %s is already up, can't start on it\n", iaxs[fr->callno]->owner ? ast_channel_name(iaxs[fr->callno]->owner) : "<Unknown>");
					break;
				}
			...
			case IAX_COMMAND_AUTHREP:
				...
				if (authenticate_verify(iaxs[fr->callno], &ies)) {
					if (authdebug)
						ast_log_chan(NULL, LOG_WARNING, "Host %s failed to authenticate as %s\n", ast_sockaddr_stringify(&addr),
				...
				if (strcmp(iaxs[fr->callno]->exten, "TBD") && !exists) {
					if (authdebug)
						ast_log_chan(NULL, LOG_WARNING, "Rejected connect attempt from %s, request '%s@%s' does not exist\n",
								ast_sockaddr_stringify(&addr),
					...
					if (!format) {
						...
						if (!format) {
							if (authdebug) {
								if (ast_test_flag64(iaxs[fr->callno], IAX_CODEC_NOCAP)) {
									ast_log_chan(NULL, LOG_WARNING, "Rejected connect attempt from %s, requested '%s' incompatible with our capability '%s'.\n",
											ast_sockaddr_stringify(&addr),
										iax2_getformatname_multiple(iaxs[fr->callno]->peerformat, &peer_form_buf),
										iax2_getformatname_multiple(iaxs[fr->callno]->capability, &cap_buf));
								} else {
									ast_log_chan(NULL, LOG_WARNING, "Rejected connect attempt from %s, requested/capability '%s'/'%s' incompatible with our capability '%s'.\n",
										ast_sockaddr_stringify(&addr),
										iax2_getformatname_multiple(iaxs[fr->callno]->peerformat, &peer_form_buf),
										iax2_getformatname_multiple(iaxs[fr->callno]->peercapability, &peer_buf),
										iax2_getformatname_multiple(iaxs[fr->callno]->capability, &cap_buf));
					...
					if (format) {
						...
						if (strcmp(iaxs[fr->callno]->exten, "TBD")) {
							...
							ast_set_flag(&iaxs[fr->callno]->state, IAX_STATE_STARTED);
							ast_verb_chan(NULL, 3, "Accepting AUTHENTICATED call from %s:\n"
							...
							ast_set_flag(&iaxs[fr->callno]->state, IAX_STATE_TBD);
							/* If this is a TBD call, we're ready but now what...  */
							ast_verb_chan(NULL, 3, "Accepted AUTHENTICATED TBD call from %s\n", ast_sockaddr_stringify(&addr));
...
immediatedial:
				if (ast_test_flag(&iaxs[fr->callno]->state, IAX_STATE_TBD)) {
					...
					if (!ast_exists_extension(NULL, iaxs[fr->callno]->context, iaxs[fr->callno]->exten, 1, iaxs[fr->callno]->cid_num)) {
						if (authdebug)
							ast_log_chan(NULL, LOG_WARNING, "Rejected dial attempt from %s, request '%s@%s' does not exist\n",
									ast_sockaddr_stringify(&addr),
									iaxs[fr->callno]->exten,
									iaxs[fr->callno]->context);
						memset(&ied0, 0, sizeof(ied0));
						...
						struct ast_str *cap_buf = ast_str_alloca(AST_FORMAT_CAP_NAMES_LEN);
						ast_set_flag(&iaxs[fr->callno]->state, IAX_STATE_STARTED);
						ast_verb_chan(NULL, 3, "Accepting DIAL from %s, formats = %s\n",
								ast_sockaddr_stringify(&addr),
			...
			case IAX_COMMAND_REGREL:
				...
				if ((ast_strlen_zero(iaxs[fr->callno]->secret) && ast_strlen_zero(iaxs[fr->callno]->inkeys)) ||
					...
					if (update_registry(&addr, fr->callno, ies.devicetype, fd, ies.refresh)) {
						ast_log_chan(NULL, LOG_WARNING, "Registry error\n");
					}
					if (!iaxs[fr->callno]) {
						break;
					}
			...
			case IAX_COMMAND_REGACK:
				if (iax2_ack_registry(&ies, &addr, fr->callno)) {
					ast_log_chan(NULL, LOG_WARNING, "Registration failure\n");
				}
				/* Send ack immediately, before we destroy */
				send_command_immediate(iaxs[fr->callno], AST_FRAME_IAX, IAX_COMMAND_ACK, fr->ts, NULL, 0,fr->iseqno);
				iax2_destroy(fr->callno);
			...
			case IAX_COMMAND_REGREJ:
				if (iaxs[fr->callno]->reg) {
					if (authdebug) {
						ast_log_chan(NULL, LOG_NOTICE, "Registration of '%s' rejected: '%s' from: '%s'\n",
			...
			case IAX_COMMAND_TXREJ:
				...
				ast_verb_chan(NULL, 3, "Channel '%s' unable to transfer\n", iaxs[fr->callno]->owner ? ast_channel_name(iaxs[fr->callno]->owner) : "<Unknown>");
			...
			case IAX_COMMAND_TXREADY:
				...
				ast_verb_chan(NULL, 3, "Channel '%s' ready to transfer\n", iaxs[fr->callno]->owner ? ast_channel_name(iaxs[fr->callno]->owner) : "<Unknown>");
				...
				if (iaxs[fr->callno]->transferring == TRANSFER_MREADY) {
					ast_verb_chan(NULL, 3, "Attempting media bridge of %s and %s\n", iaxs[fr->callno]->owner ? ast_channel_name(iaxs[fr->callno]->owner) : "<Unknown>",
					...
					ast_verb_chan(NULL, 3, "Releasing %s and %s\n", iaxs[fr->callno]->owner ? ast_channel_name(iaxs[fr->callno]->owner) : "<Unknown>",
							iaxs[iaxs[fr->callno]->bridgecallno]->owner ? ast_channel_name(iaxs[iaxs[fr->callno]->bridgecallno]->owner) : "<Unknown>");
			...
			case IAX_COMMAND_UNSUPPORT:
				ast_log_chan(NULL, LOG_NOTICE, "Peer did not understand our iax command '%d'\n", ies.iax_unknown);
				break;
			case IAX_COMMAND_FWDOWNL:
				/* Firmware download */
		...
		if (iaxs[fr->callno]->videoformat > 0) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Received mini frame before first full video frame\n");
			iax2_vnak(fr->callno);
			ast_variables_destroy(ies.vars);
			ast_mutex_unlock(&iaxsl[fr->callno]);
			return 1;
...
static int iax2_do_register(struct iax2_registry *reg)
{
	...
	if (!reg->callno) {
		...
		if (reg->callno < 1) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to create call for registration\n");
			return -1;
		} else
...
static int iax2_prov_app(struct ast_channel *chan, const char *data)
{
	...
	if (ast_channel_tech(chan) != &iax2_tech) {
		ast_log_chan(NULL, LOG_NOTICE, "Can't provision a non-IAX device!\n");
		return -1;
	}
	if (!callno || !iaxs[callno] || ast_sockaddr_isnull(&iaxs[callno]->addr)) {
		ast_log_chan(NULL, LOG_NOTICE, "Can't provision something with no IP?\n");
		return -1;
	}
	res = iax2_provision(&iaxs[callno]->addr, iaxs[callno]->sockfd, NULL, sdata, force);
	ast_verb_chan(NULL, 3, "Provisioned IAXY at '%s' with '%s'= %d\n",
...
static void __iax2_poke_noanswer(const void *data)
{
	...
	if (peer->lastms > -1) {
		...
		ast_log_chan(NULL, LOG_NOTICE, "Peer '%s' is now UNREACHABLE! Time: %d\n", peer->name, peer->lastms);
		ast_endpoint_set_state(peer->endpoint, AST_ENDPOINT_OFFLINE);
		blob = ast_json_pack("{s: s, s: i}",
...
static int iax2_poke_peer(struct iax2_peer *peer, int heldcall)
{
	...
	if ((callno = peer->callno) > 0) {
		ast_log_chan(NULL, LOG_NOTICE, "Still have a callno...\n");
		ast_mutex_lock(&iaxsl[callno]);
		iax2_destroy(callno);
		ast_mutex_unlock(&iaxsl[callno]);
	}
	...
	if (callno < 1) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to allocate call for poking peer '%s'\n", peer->name);
		return -1;
	}
...
static struct ast_channel *iax2_request(const char *type, struct ast_format_cap *cap, const struct ast_assigned_ids *assignedids, const struct ast_channel *requestor, const char *data, int *cause)
{
	...
	callno = find_callno_locked(0, 0, &addr, NEW_FORCE, cai.sockfd, 0);
	if (callno < 1) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to create call\n");
	...
	if (c) {
		...
		if (!ast_format_cap_count(joint)) {
			...
			if (res < 0) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Unable to create translator path for %s to %s on %s\n",
...
static void *network_thread(void *ignore)
{
	...
	for (;;) {
		...
		if (res < 0 && errno != -EINTR) {
			ast_log_chan(NULL, LOG_ERROR, "IAX2 network thread unexpected exit: %s\n", strerror(errno));
			break;
		}
	}
...
static int start_network_thread(void)
{
	...
	for (x = 0; x < iaxthreadcount; x++) {
		...
		if (thread) {
			...
			if (ast_pthread_create_background(&thread->threadid, NULL, iax2_process_thread, thread)) {
				ast_log_chan(NULL, LOG_WARNING, "Failed to create new thread!\n");
				ast_mutex_destroy(&thread->lock);
				ast_cond_destroy(&thread->cond);
				ast_mutex_unlock(&thread->init_lock);
				ast_mutex_destroy(&thread->init_lock);
	...
	if (ast_pthread_create_background(&netthreadid, NULL, network_thread, NULL)) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to create new thread!\n");
		return -1;
	}
	ast_verb_chan(NULL, 2, "%d helper threads started\n", threadcount);
	return 0;
}
...
static int check_srcaddr(struct ast_sockaddr *addr)
{
	...
	if (sd < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Socket: %s\n", strerror(errno));
		return -1;
	}
		...
		while(v) {
			if (!strcasecmp(v->name, "secret")) {
				...
				if (ast_test_flag64(peer, IAX_TRUNK) && !timer) {
					ast_log_chan(NULL, LOG_WARNING, "Unable to support trunking on peer '%s' without a timing interface\n", peer->name);
					ast_clear_flag64(peer, IAX_TRUNK);
				}
			} else if (!strcasecmp(v->name, "auth")) {
				peer->authmethods = get_auth_methods(v->value);
				if (peer->authmethods & IAX_AUTH_PLAINTEXT) {
					ast_log_chan(NULL, LOG_WARNING, "Auth method for peer '%s' is set to deprecated 'plaintext' at line %d of iax.conf\n", peer->name, v->lineno);
				}
				...
				if (!strcasecmp(v->value, "no")) {
					...
					ast_log_chan(NULL, LOG_WARNING, "Qualification of peer '%s' should be 'yes', 'no', or a number of milliseconds at line %d of iax.conf\n", peer->name, v->lineno);
					peer->maxms = 0;
				}
				...
				if (sscanf(v->value, "%30d", &peer->pokefreqok) != 1) {
					ast_log_chan(NULL, LOG_WARNING, "Qualification testing frequency of peer '%s' when OK should a number of milliseconds at line %d of iax.conf\n", peer->name, v->lineno);
				}
			} else if (!strcasecmp(v->name, "qualifyfreqnotok")) {
				if (sscanf(v->value, "%30d", &peer->pokefreqnotok) != 1) {
					ast_log_chan(NULL, LOG_WARNING, "Qualification testing frequency of peer '%s' when NOT OK should be a number of milliseconds at line %d of iax.conf\n", peer->name, v->lineno);
				}
				...
				if (sscanf(v->value, "%10hu", &peer->maxcallno) != 1) {
					ast_log_chan(NULL, LOG_WARNING, "maxcallnumbers must be set to a valid number. %s is not valid at line %d.\n", v->value, v->lineno);
				} else {
				...
				if (ast_false(v->value)) {
					...
					ast_log_chan(NULL, LOG_WARNING, "requirecalltoken must be set to a valid value. at line %d\n", v->lineno);
				}
			} /* else if (strcasecmp(v->name,"type")) */
			/*	ast_log_chan(NULL, LOG_WARNING, "Ignoring %s\n", v->name); */
			v = v->next;
			if (!v) {
				v = alt;
				alt = NULL;
		...
		while(v) {
			if (!strcasecmp(v->name, "context")) {
				...
				if (ast_test_flag64(user, IAX_TRUNK) && !timer) {
					ast_log_chan(NULL, LOG_WARNING, "Unable to support trunking on user '%s' without a timing interface\n", user->name);
					ast_clear_flag64(user, IAX_TRUNK);
				}
			} else if (!strcasecmp(v->name, "auth")) {
				user->authmethods = get_auth_methods(v->value);
				if (user->authmethods & IAX_AUTH_PLAINTEXT) {
					ast_log_chan(NULL, LOG_WARNING, "Auth method for user '%s' is set to deprecated 'plaintext' at line %d of iax.conf\n", user->name, v->lineno);
				}
			} else if (!strcasecmp(v->name, "encryption")) {
...
static int set_config(const char *config_file, int reload, int forced)
{
	...
	if (!cfg) {
		...
		if ((cfg = ast_config_load(config_file, config_flags)) == CONFIG_STATUS_FILEINVALID) {
			ast_log_chan(NULL, LOG_ERROR, "Config file %s is in an invalid format.  Aborting.\n", config_file);
			ast_config_destroy(ucfg);
			return 0;
		}
		if (!cfg) {
			/* should have been able to load the config here */
			ast_log_chan(NULL, LOG_ERROR, "Unable to load config %s again\n", config_file);
			return -1;
		}
		...
		if ((ucfg = ast_config_load("users.conf", config_flags)) == CONFIG_STATUS_FILEINVALID) {
			ast_log_chan(NULL, LOG_ERROR, "Config file users.conf is in an invalid format.  Aborting.\n");
	...
	if (tosval) {
		if (ast_str2tos(tosval, &qos.tos))
			ast_log_chan(NULL, LOG_WARNING, "Invalid tos value, refer to QoS documentation\n");
	}
	/* Seed initial cos value */
	tosval = ast_variable_retrieve(cfg, "general", "cos");
	if (tosval) {
		if (ast_str2cos(tosval, &qos.cos))
			ast_log_chan(NULL, LOG_WARNING, "Invalid cos value, refer to QoS documentation\n");
	}
	while(v) {
		if (!strcasecmp(v->name, "bindport")) {
			if (reload) {
				ast_log_chan(NULL, LOG_NOTICE, "Ignoring bindport on reload\n");
			}
			...
			if (reload) {
				if (atoi(v->value) != iaxthreadcount)
					ast_log_chan(NULL, LOG_NOTICE, "Ignoring any changes to iaxthreadcount during reload\n");
			} else {
				iaxthreadcount = atoi(v->value);
				if (iaxthreadcount < 1) {
					ast_log_chan(NULL, LOG_NOTICE, "iaxthreadcount must be at least 1.\n");
					iaxthreadcount = 1;
				} else if (iaxthreadcount > 256) {
					ast_log_chan(NULL, LOG_NOTICE, "limiting iaxthreadcount to 256\n");
					iaxthreadcount = 256;
				}
			}
			...
			if (reload) {
				...
				if (iaxmaxthreadcount < 0) {
					ast_log_chan(NULL, LOG_NOTICE, "iaxmaxthreadcount must be at least 0.\n");
					iaxmaxthreadcount = 0;
				} else if (iaxmaxthreadcount > 256) {
					ast_log_chan(NULL, LOG_NOTICE, "Limiting iaxmaxthreadcount to 256\n");
...
#else
			if (ast_true(v->value))
				ast_log_chan(NULL, LOG_WARNING, "Disabling RTP checksums is not supported on this operating system!\n");
#endif
		}
			...
			if (reload) {
				ast_log_chan(NULL, LOG_NOTICE, "Ignoring bindaddr on reload\n");
			} else {
				...
				if (!ast_parse_arg(v->value, PARSE_ADDR, NULL)) {
					...
					if (!(ns = ast_netsock_bindaddr(netsock, io, &bindaddr, qos.tos, qos.cos, socket_read, NULL))) {
						ast_log_chan(NULL, LOG_WARNING, "Unable to apply binding to '%s' at line %d\n", v->value, v->lineno);
					} else {
						ast_verb_chan(NULL, 2, "Binding IAX2 to address %s\n", ast_sockaddr_stringify(&bindaddr));
					...
					ast_log_chan(NULL, LOG_WARNING, "Invalid address '%s' specified, at line %d\n", v->value, v->lineno);
				}
			}
		} else if (!strcasecmp(v->name, "auth")) {
			iax2_authmethods = get_auth_methods(v->value);
			if (iax2_authmethods & IAX_AUTH_PLAINTEXT) {
				ast_log_chan(NULL, LOG_WARNING, "Default auth method is set to deprecated 'plaintext' at line %d of iax.conf\n", v->lineno);
			}
			...
			if (trunkfreq < 10) {
				ast_log_chan(NULL, LOG_NOTICE, "trunkfreq must be between 10ms and 1000ms, using 10ms instead.\n");
				trunkfreq = 10;
			} else if (trunkfreq > 1000) {
				ast_log_chan(NULL, LOG_NOTICE, "trunkfreq must be between 10ms and 1000ms, using 1000ms instead.\n");
				trunkfreq = 1000;
			}
			if (timer) {
			...
			if (mtuv  == 0 )
				...
				ast_log_chan(NULL, LOG_NOTICE, "trunkmtu value out of bounds (%d) at line %d\n",
					mtuv, v->lineno);
			...
			if (sscanf(v->value, "%30d", &x) == 1) {
				if (x >= 0)
					...
					ast_log_chan(NULL, LOG_NOTICE, "Nice try, but autokill has to be >0 or 'yes' or 'no' at line %d\n", v->lineno);
			} else if (ast_true(v->value)) {
			...
			if (!strcasecmp(v->value, "low")) {
				...
				ast_log_chan(NULL, LOG_WARNING, "bandwidth must be either low, medium, or high\n");
			}
		} else if (!strcasecmp(v->name, "allow")) {
			...
			if (ast_str2tos(v->value, &qos.tos))
				ast_log_chan(NULL, LOG_WARNING, "Invalid tos value at line %d, refer to QoS documentation\n", v->lineno);
		} else if (!strcasecmp(v->name, "cos")) {
			if (ast_str2cos(v->value, &qos.cos))
				ast_log_chan(NULL, LOG_WARNING, "Invalid cos value at line %d, refer to QoS documentation\n", v->lineno);
		} else if (!strcasecmp(v->name, "parkinglot")) {
			...
			if (sscanf(v->value, "%10hu", &global_maxcallno_nonval) != 1) {
				ast_log_chan(NULL, LOG_WARNING, "maxcallnumbers_nonvalidated must be set to a valid number.  %s is not valid at line %d.\n", v->value, v->lineno);
			}
		} else if (!strcasecmp(v->name, "calltokenoptional")) {
			if (add_calltoken_ignore(v->value)) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid calltokenoptional address range - '%s' line %d\n", v->value, v->lineno);
				return -1;
			}
			...
			if( temp <= 0 ){
				ast_log_chan(NULL, LOG_WARNING, "Invalid calltokenexpiration value %s. Should be integer greater than 0.\n", v->value);
			} else {
			...
			if (ast_true(v->value)) {
				...
				ast_log_chan(NULL, LOG_WARNING, "subscribe_network_change_event value %s is not valid at line %d.\n", v->value, v->lineno);
			}
			...
			if (ast_true(v->value)) {
				...
				ast_log_chan(NULL, LOG_WARNING, "shrinkcallerid value %s is not valid at line %d.\n", v->value, v->lineno);
			}
		}/*else if (strcasecmp(v->name,"type")) */
		/*	ast_log_chan(NULL, LOG_WARNING, "Ignoring %s\n", v->name); */
		v = v->next;
	...
	if (defaultsockfd < 0) {
		...
		if (!(ns = ast_netsock_bindaddr(netsock, io, &bindaddr, qos.tos, qos.cos, socket_read, NULL))) {
			ast_log_chan(NULL, LOG_ERROR, "Unable to create network socket: %s\n", strerror(errno));
		} else {
			ast_verb_chan(NULL, 2, "Binding IAX2 to default address %s\n", ast_sockaddr_stringify(&bindaddr));
			defaultsockfd = ast_netsock_sockfd(ns);
			ast_netsock_unref(ns);
		}
	}
	if (reload) {
		...
		if (!outsock) {
			ast_log_chan(NULL, LOG_ERROR, "Could not allocate outsock list.\n");
			return -1;
		}
	...
	if (min_reg_expire > max_reg_expire) {
		ast_log_chan(NULL, LOG_WARNING, "Minimum registration interval of %d is more than maximum of %d, resetting minimum to %d\n",
		...
		if (strcasecmp(cat, "general")) {
			...
			if (!strcasecmp(cat, "callnumberlimits")) {
				...
				if (!strcasecmp(utype, "peer") || !strcasecmp(utype, "friend")) {
					...
					ast_log_chan(NULL, LOG_WARNING, "Unknown type '%s' for '%s' in %s\n", utype, cat, config_file);
				}
			} else
				ast_log_chan(NULL, LOG_WARNING, "Section '%s' lacks type\n", cat);
		}
...
static struct iax2_dpcache *find_cache(struct ast_channel *chan, const char *data, const char *context, const char *exten, int priority)
{
	...
	AST_LIST_TRAVERSE_SAFE_BEGIN(&dpcache, dp, cache_list) {
		if (ast_tvcmp(now, dp->expiry) > 0) {
			...
			if ((dp->flags & CACHE_FLAG_PENDING) || dp->callno)
				ast_log_chan(NULL, LOG_WARNING, "DP still has peer field or pending or callno (flags = %d, peer = blah, callno = %d)\n", dp->flags, dp->callno);
			else
				ast_free(dp);
			continue;
		}
	...
	if (!dp) {
		...
		if ((callno = cache_get_callno_locked(data)) < 0) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to generate call for '%s'\n", data);
			return NULL;
		}
	...
	if (dp->flags & CACHE_FLAG_PENDING) {
		...
		if (x >= ARRAY_LEN(dp->waiters)) {
			ast_log_chan(NULL, LOG_WARNING, "No more waiter positions available\n");
			return NULL;
		}
		if (pipe(com)) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to create pipe for comm\n");
			return NULL;
		}
		dp->waiters[x] = com[1];
		/* Okay, now we wait */
		...
		if (res < 0) {
			ast_log_chan(NULL, LOG_WARNING, "poll returned < 0: %s\n", strerror(errno));
			return NULL;
		} else if (!pfd.revents) {
			ast_log_chan(NULL, LOG_WARNING, "Timeout waiting for %s exten %s\n", data, exten);
		}
...
static int iax2_exists(struct ast_channel *chan, const char *context, const char *exten, int priority, const char *callerid, const char *data)
{
	...
	struct iax2_dpcache *dp = NULL;
#if 0
	ast_log_chan(NULL, LOG_NOTICE, "iax2_exists: con: %s, exten: %s, pri: %d, cid: %s, data: %s\n", context, exten, priority, callerid ? callerid : "<unknown>", data);
#endif
	...
	if ((dp = find_cache(chan, data, context, exten, priority))) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Unable to make DP cache\n");
	}
	AST_LIST_UNLOCK(&dpcache);
...
static int iax2_canmatch(struct ast_channel *chan, const char *context, const char *exten, int priority, const char *callerid, const char *data)
{
	...
	struct iax2_dpcache *dp = NULL;
#if 0
	ast_log_chan(NULL, LOG_NOTICE, "iax2_canmatch: con: %s, exten: %s, pri: %d, cid: %s, data: %s\n", context, exten, priority, callerid ? callerid : "<unknown>", data);
#endif
	if ((priority != 1) && (priority != 2))
		return 0;
...
static int iax2_matchmore(struct ast_channel *chan, const char *context, const char *exten, int priority, const char *callerid, const char *data)
{
	...
	struct iax2_dpcache *dp = NULL;
#if 0
	ast_log_chan(NULL, LOG_NOTICE, "iax2_matchmore: con: %s, exten: %s, pri: %d, cid: %s, data: %s\n", context, exten, priority, callerid ? callerid : "<unknown>", data);
#endif
	if ((priority != 1) && (priority != 2))
		return 0;
...
static int iax2_exec(struct ast_channel *chan, const char *context, const char *exten, int priority, const char *callerid, const char *data)
{
...
#if 0
	ast_log_chan(NULL, LOG_NOTICE, "iax2_exec: con: %s, exten: %s, pri: %d, cid: %s, data: %s, newstack: %d\n", context, exten, priority, callerid ? callerid : "<unknown>", data, newstack);
#endif
	if (priority == 2) {
	...
	if ((dp = find_cache(chan, data, context, exten, priority))) {
		if (dp->flags & CACHE_FLAG_EXISTS) {
			...
			ast_verb_chan(NULL, 3, "Executing Dial('%s')\n", req);
		} else {
			AST_LIST_UNLOCK(&dpcache);
			ast_log_chan(NULL, LOG_WARNING, "Can't execute nonexistent extension '%s[@%s]' in data '%s'\n", exten, context, data);
			return -1;
		}
	}
	AST_LIST_UNLOCK(&dpcache);
	...
	if ((dial = pbx_findapp("Dial")))
		...
		ast_log_chan(NULL, LOG_WARNING, "No dial application registered\n");
...
static int acf_channel_read(struct ast_channel *chan, const char *funcname, char *args, char *buf, size_t buflen)
{
	...
	if (!chan || ast_channel_tech(chan) != &iax2_tech) {
		ast_log_chan(NULL, LOG_ERROR, "This function requires a valid IAX2 channel\n");
		return -1;
	}
...
static int load_module(void)
{
	...
	if (!(sched = ast_sched_context_create())) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to create scheduler thread\n");
		ao2_ref(iax2_tech.capabilities, -1);
		iax2_tech.capabilities = NULL;
		return AST_MODULE_LOAD_DECLINE;
	}
	...
	if (!(io = io_context_create())) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to create I/O context\n");
		ast_sched_context_destroy(sched);
		ao2_ref(iax2_tech.capabilities, -1);
		iax2_tech.capabilities = NULL;
		sched = NULL;
	...
	if (!(netsock = ast_netsock_list_alloc())) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to create netsock list\n");
	...
	ast_manager_register_xml("IAXregistry", EVENT_FLAG_SYSTEM | EVENT_FLAG_REPORTING, manager_iax2_show_registry);
 	...
 	if (ast_channel_register(&iax2_tech)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to register channel class %s\n", "IAX2");
		__unload_module();
		return AST_MODULE_LOAD_DECLINE;
	}

	if (ast_register_switch(&iax2_switch)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to register IAX switch\n");
	}

	if (start_network_thread()) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to start network thread\n");
		__unload_module();
		return AST_MODULE_LOAD_DECLINE;
	} else {
		ast_verb_chan(NULL, 2, "IAX Ready and Listening\n");
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "Inter GABpbx eXchange (Ver 2)",
