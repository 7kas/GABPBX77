...
/*! \brief GABpbx core interaction functions */
...
static void jingle_enable_video(struct jingle_session *session)
{
	...
	if (!(session->vrtp = ast_rtp_instance_new("gabpbx", sched, &tmp, NULL))) {
		return;
	}
...
static struct jingle_session *jingle_alloc(struct jingle_endpoint *endpoint, const char *from, const char *sid)
{
	...
	if (!(session->rtp = ast_rtp_instance_new("gabpbx", sched, &tmp, NULL))) {
		ao2_ref(session, -1);
		return NULL;
	}
...
/*! \brief Function called to create a new Jingle GABpbx channel */
...
static void jingle_send_response(struct ast_xmpp_client *connection, ikspak *pak)
{
	...
	if (!(response = iks_new("iq"))) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to allocate an IKS response stanza\n");
		return;
	}
...
static void jingle_send_error_response(struct ast_xmpp_client *connection, ikspak *pak, const char *type, const char *reasonstr, const char *reasonstr2)
{
	...
	if (!(response = iks_new("iq")) ||
		...
		ast_log_chan(NULL, LOG_ERROR, "Unable to allocate IKS error response stanzas\n");
		goto end;
	}
...
static int jingle_add_ice_udp_candidates_to_transport(struct ast_rtp_instance *rtp, iks *transport, iks **candidates, unsigned int maximum)
{
	...
	if (!(ice = ast_rtp_instance_get_ice(rtp)) || !(local_candidates = ice->get_local_candidates(rtp))) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to add ICE-UDP candidates as ICE support not available or no candidates available\n");
		return -1;
	}
	...
	while ((candidate = ao2_iterator_next(&it)) && (i < maximum)) {
		...
		if (!(local_candidate = iks_new("candidate"))) {
			res = -1;
			ast_log_chan(NULL, LOG_ERROR, "Unable to allocate IKS candidate stanza for ICE-UDP transport\n");
			break;
		}
...
static int jingle_add_google_candidates_to_transport(struct ast_rtp_instance *rtp, iks *transport, iks **candidates, unsigned int video, enum jingle_transport transport_type, unsigned int maximum)
{
	...
	if (!(ice = ast_rtp_instance_get_ice(rtp)) || !(local_candidates = ice->get_local_candidates(rtp))) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to add Google ICE candidates as ICE support not available or no candidates available\n");
		return -1;
	}
	...
	while ((candidate = ao2_iterator_next(&it)) && (i < maximum)) {
		...
		if (!(local_candidate = iks_new("candidate"))) {
			res = -1;
			ast_log_chan(NULL, LOG_ERROR, "Unable to allocate IKS candidate stanza for Google ICE transport\n");
			break;
		}
...
static void jingle_send_session_terminate(struct jingle_session *session, const char *reasontext)
{
	...
	if (!(iq = iks_new("iq")) || !(jingle = iks_new(session->transport == JINGLE_TRANSPORT_GOOGLE_V1 ? "session" : "jingle")) ||
		...
		ast_log_chan(NULL, LOG_ERROR, "Failed to allocate stanzas for session-terminate message on session '%s'\n", session->sid);
		goto end;
	}
...
static void jingle_send_session_info(struct jingle_session *session, const char *info)
{
	...
	if (!(iq = iks_new("iq")) || !(jingle = iks_new("jingle")) || !(text = iks_new(info))) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to allocate stanzas for session-info message on session '%s'\n", session->sid);
		goto end;
	}
...
static void jingle_send_transport_info(struct jingle_session *session, const char *from)
{
	...
	if (!(iq = iks_new("iq")) ||
		...
		ast_log_chan(NULL, LOG_ERROR, "Failed to allocate stanzas for transport-info message, hanging up session '%s'\n", session->sid);
		return;
	}
...
static void jingle_send_session_action(struct jingle_session *session, const char *action)
{
	...
	if (session->rtp && (audio = iks_new("content")) && (audio_description = iks_new("description")) &&
		...
		ast_log_chan(NULL, LOG_ERROR, "Failed to allocate audio content stanzas for session '%s', hanging up\n", session->sid);
		res = -1;
	}
	...
	if ((session->transport != JINGLE_TRANSPORT_GOOGLE_V1) && !res && session->vrtp) {
		if ((video = iks_new("content")) && (video_description = iks_new("description")) &&
			...
			ast_log_chan(NULL, LOG_ERROR, "Failed to allocate video content stanzas for session '%s', hanging up\n", session->sid);
			res = -1;
		}
	}
...
static int jingle_write(struct ast_channel *ast, struct ast_frame *frame)
{
	...
	case AST_FRAME_VIDEO:
		...
		break;
	default:
		ast_log_chan(NULL, LOG_WARNING, "Can't send %u type frames with Jingle write\n",
			frame->frametype);
		return 0;
	}
...
static int jingle_indicate(struct ast_channel *ast, int condition, const void *data, size_t datalen)
{
	...
	case -1:
		...
		break;
	default:
		ast_log_chan(NULL, LOG_NOTICE, "Don't know how to indicate condition '%d'\n", condition);
		res = -1;
	}
...
static struct ast_channel *jingle_request(const char *type, struct ast_format_cap *cap, const struct ast_assigned_ids *assignedids, const struct ast_channel *requestor, const char *data, int *cause)
{
	...
	if (!ast_format_cap_has_type(cap, AST_MEDIA_TYPE_AUDIO)) {
		ast_log_chan(NULL, LOG_ERROR, "Motif channel driver requires an audio format when dialing a destination\n");
		*cause = AST_CAUSE_BEARERCAPABILITY_NOTAVAIL;
		return NULL;
	}

	if (ast_strlen_zero(data) || !(dialed = ast_strdupa(data))) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to create channel with empty destination.\n");
	...
	if (ast_strlen_zero(args.name) || ast_strlen_zero(args.target)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to determine endpoint name and target.\n");
		*cause = AST_CAUSE_CHANNEL_UNACCEPTABLE;
		return NULL;
	}

	if (!(endpoint = jingle_endpoint_find(cfg->endpoints, args.name))) {
		ast_log_chan(NULL, LOG_ERROR, "Endpoint '%s' does not exist.\n", args.name);
		*cause = AST_CAUSE_CHANNEL_UNACCEPTABLE;
		return NULL;
	}
	...
	if (!endpoint->connection) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to create Jingle session on endpoint '%s' as no valid connection exists\n", args.name);
	...
	if (ast_strlen_zero(target)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to create Jingle session on endpoint '%s' as no capable resource for target '%s' was found\n", args.name, args.target);
		*cause = AST_CAUSE_SWITCH_CONGESTION;
		return NULL;
	}

	if (!(session = jingle_alloc(endpoint, target, NULL))) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to create Jingle session on endpoint '%s'\n", args.name);
		*cause = AST_CAUSE_SWITCH_CONGESTION;
		return NULL;
	}
	...
	if (!(chan = jingle_new(endpoint, session, AST_STATE_DOWN, target, assignedids, requestor, NULL))) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to create Jingle channel on endpoint '%s'\n", args.name);
		*cause = AST_CAUSE_SWITCH_CONGESTION;
		ao2_ref(session, -1);
		return NULL;
	}
...
static int jingle_interpret_description(struct jingle_session *session, iks *description, const char *name, struct ast_rtp_instance **rtp)
{
	...
	if (session->transport == JINGLE_TRANSPORT_GOOGLE_V1) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Received a content description on session '%s' without a name\n", session->sid);
		return -1;
	}
	...
	if (!strcasecmp(media, "audio")) {
		...
		*rtp = session->vrtp;
		...
		if (!session->vrtp) {
			jingle_queue_hangup_with_cause(session, AST_CAUSE_BEARERCAPABILITY_NOTAVAIL);
			ast_log_chan(NULL, LOG_ERROR, "Received a video content description on session '%s' but could not enable video\n", session->sid);
			return -1;
		}
		...
		/* Unknown media type */
		jingle_queue_hangup_with_cause(session, AST_CAUSE_BEARERCAPABILITY_NOTAVAIL);
		ast_log_chan(NULL, LOG_ERROR, "Unsupported media type '%s' received in content description on session '%s'\n", media, session->sid);
		return -1;
	}

	if (ast_rtp_codecs_payloads_initialize(&codecs)) {
		jingle_queue_hangup_with_cause(session, AST_CAUSE_BEARERCAPABILITY_NOTAVAIL);
		ast_log_chan(NULL, LOG_ERROR, "Could not initialize codecs for negotiation on session '%s'\n", session->sid);
		return -1;
	}
...
static int jingle_interpret_ice_udp_transport(struct jingle_session *session, iks *transport, struct ast_rtp_instance *rtp)
{
	...
	if (!ice) {
		jingle_queue_hangup_with_cause(session, AST_CAUSE_SWITCH_CONGESTION);
		ast_log_chan(NULL, LOG_ERROR, "Received ICE-UDP transport information on session '%s' but ICE support not available\n", session->sid);
		return -1;
	}
	...
	for (candidate = iks_child(transport); candidate; candidate = iks_next(candidate)) {
		...
		if (ast_strlen_zero(component) || ast_strlen_zero(foundation) || ast_strlen_zero(generation) || ast_strlen_zero(id) ||
			...
			jingle_queue_hangup_with_cause(session, AST_CAUSE_PROTOCOL_ERROR);
			ast_log_chan(NULL, LOG_ERROR, "Incomplete ICE-UDP candidate received on session '%s'\n", session->sid);
			return -1;
		}
		...
		if ((sscanf(component, "%30u", &local_candidate.id) != 1) ||
			...
			jingle_queue_hangup_with_cause(session, AST_CAUSE_PROTOCOL_ERROR);
			ast_log_chan(NULL, LOG_ERROR, "Invalid ICE-UDP candidate information received on session '%s'\n", session->sid);
			return -1;
		}
...
static int jingle_interpret_google_transport(struct jingle_session *session, iks *transport, struct ast_rtp_instance *rtp)
{
	...
	if (!ice) {
		jingle_queue_hangup_with_cause(session, AST_CAUSE_SWITCH_CONGESTION);
		ast_log_chan(NULL, LOG_ERROR, "Received Google transport information on session '%s' but ICE support not available\n", session->sid);
		return -1;
	}
	...
	for (candidate = iks_child(transport); candidate; candidate = iks_next(candidate)) {
		...
		if (ast_strlen_zero(address) || ast_strlen_zero(port) || ast_strlen_zero(username) ||
			...
			jingle_queue_hangup_with_cause(session, AST_CAUSE_PROTOCOL_ERROR);
			ast_log_chan(NULL, LOG_ERROR, "Incomplete Google candidate received on session '%s'\n", session->sid);
			return -1;
		}
		...
		if (sscanf(port, "%30d", &real_port) != 1) {
			jingle_queue_hangup_with_cause(session, AST_CAUSE_PROTOCOL_ERROR);
			ast_log_chan(NULL, LOG_ERROR, "Invalid Google candidate port '%s' received on session '%s'\n", port, session->sid);
			return -1;
		}
	...
	for (content = iks_child(iks_child(pak->x)); content; content = iks_next(content)) {
		...
		if (session->transport != JINGLE_TRANSPORT_GOOGLE_V1) {
			...
			if (ast_strlen_zero(name) && !(name = iks_find_attrib(content, "jin:name"))) {
				jingle_queue_hangup_with_cause(session, AST_CAUSE_BEARERCAPABILITY_NOTAVAIL);
				ast_log_chan(NULL, LOG_ERROR, "Received content without a name on session '%s'\n", session->sid);
				return -1;
			}
		...
		if (!rtp) {
			ast_log_chan(NULL, LOG_ERROR, "Received a content stanza but have no RTP instance for it on session '%s'\n", session->sid);
			jingle_queue_hangup_with_cause(session, AST_CAUSE_SWITCH_CONGESTION);
			return -1;
		}
		...
		if ((transport = iks_find_with_attrib(content, "transport", "xmlns", JINGLE_ICE_UDP_NS))) {
			...
			jingle_queue_hangup_with_cause(session, AST_CAUSE_FACILITY_NOT_IMPLEMENTED);
			ast_log_chan(NULL, LOG_ERROR, "Unsupported transport type received on session '%s'\n", session->sid);
			return -1;
		}
	}
...
static void jingle_action_session_initiate(struct jingle_endpoint *endpoint, struct jingle_session *session, ikspak *pak)
{
	...
	/* Create a new GABpbx channel using the above local session */
	...
	switch (res) {
	case AST_PBX_FAILED:
		ast_log_chan(NULL, LOG_WARNING, "Failed to start PBX :(\n");
		jingle_send_error_response(endpoint->connection, pak, "cancel", "service-unavailable xmlns='urn:ietf:params:xml:ns:xmpp-stanzas'", NULL);
		session->gone = 1;
		ast_hangup(chan);
		break;
	case AST_PBX_CALL_LIMIT:
		ast_log_chan(NULL, LOG_WARNING, "Failed to start PBX (call limit reached) \n");
...
static int jingle_action_hook(void *data, ikspak *pak)
{
	...
	if (!handled) {
		ast_log_chan(NULL, LOG_NOTICE, "Received action '%s' for session '%s' that has no handler\n", action, sid);
	}
...
static int custom_connection_handler(const struct aco_option *opt, struct ast_variable *var, void *obj)
{
	...
	if (!(endpoint->connection = ast_xmpp_client_find(var->value))) {
		ast_log_chan(NULL, LOG_ERROR, "Connection '%s' configured on endpoint '%s' could not be found\n", var->value, endpoint->name);
		return -1;
	}
	...
	if (!(endpoint->rule = iks_filter_add_rule(endpoint->connection->filter, jingle_action_hook, endpoint,
						   ...
						   IKS_RULE_DONE))) {
		ast_log_chan(NULL, LOG_ERROR, "Action hook could not be added to connection '%s' on endpoint '%s'\n", var->value, endpoint->name);
		return -1;
	}
...
static int custom_transport_handler(const struct aco_option *opt, struct ast_variable *var, void *obj)
{
	...
	if (!strcasecmp(var->value, "ice-udp")) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Unknown transport type '%s' on endpoint '%s', defaulting to 'ice-udp'\n", var->value, endpoint->name);
		endpoint->transport = JINGLE_TRANSPORT_ICE_UDP;
	}
...
static int load_module(void)
{
	...
	if (aco_info_init(&cfg_info)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to initialize configuration for chan_motif.\n");
		goto end;
	}
	...
	if (aco_process_config(&cfg_info, 0)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to read config file motif.conf. Module loaded but not running.\n");
	...
	if (!(sched = ast_sched_context_create())) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to create scheduler context.\n");
		goto end;
	}

	if (ast_sched_start_thread(sched)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to create scheduler context thread.\n");
		goto end;
	}
...
/*! \brief Unload the jingle channel from GABpbx */
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "Motif Jingle Channel Driver",
