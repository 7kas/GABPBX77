		...
		int nat_active)
{
	...
	if (ast_sip_push_task(session->serializer, send_direct_media_request, cdata)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to send direct media request for channel %s\n", ast_channel_name(chan));
		ao2_ref(cdata, -1);
	}
...
/*! \brief Function called to create a new PJSIP GABpbx channel */
static struct ast_channel *chan_pjsip_new(struct ast_sip_session *session, int state, const char *exten, const char *title, const struct ast_assigned_ids *assignedids, const struct ast_channel *requestor, const char *cid_name)
{
	...
	if (!ast_strlen_zero(session->endpoint->zone)) {
		...
		if (!zone) {
			ast_log_chan(NULL, LOG_ERROR, "Unknown country code '%s' for tonezone. Check indications.conf for available country codes.\n", session->endpoint->zone);
		}
...
static int answer(void *data)
{
	...
	if (session->inv_session->state == PJSIP_INV_STATE_DISCONNECTED) {
		ast_log_chan(NULL, LOG_ERROR, "Session already DISCONNECTED [reason=%d (%s)]\n",
	...
	if (session->inv_session->invite_tsx) {
		...
		ast_log_chan(NULL, LOG_ERROR,"Cannot answer '%s' because there is no associated SIP transaction\n",
	...
	if (status != PJ_SUCCESS) {
		...
		ast_log_chan(NULL, LOG_WARNING,"Cannot answer '%s': %s\n",
...
static int chan_pjsip_answer(struct ast_channel *ast)
{
	...
	if (res) {
		if (res == -1) {
			ast_log_chan(NULL, LOG_ERROR,"Cannot answer '%s': Unable to push answer task to the threadpool.\n",
				ast_channel_name(session->channel));
		}
		ao2_ref(session, -1);
	...
	struct ast_frame *f)
{
	...
	if (exists) {
		...
		if (ast_async_goto(ast, target_context, "fax", 1)) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to async goto '%s' into fax extension in '%s'\n",
				ast_channel_name(ast), target_context);
		}
	} else {
		ast_log_chan(NULL, LOG_NOTICE, "FAX CNG detected on '%s' but no fax extension in '%s'\n",
			ast_channel_name(ast), target_context);
	}
...
static int chan_pjsip_write_stream(struct ast_channel *ast, int stream_num, struct ast_frame *frame)
{
	...
	case AST_FRAME_RTCP:
		...
		break;
	default:
		ast_log_chan(NULL, LOG_WARNING, "Can't send %u type frames with PJSIP\n", frame->frametype);
		break;
	}
...
static int transmit_info_with_vidupdate(void *data)
{
	...
	if (ast_sip_create_request("INFO", session->inv_session->dlg, session->endpoint, NULL, NULL, &tdata)) {
		ast_log_chan(NULL, LOG_ERROR, "Could not create text video update INFO request\n");
		return -1;
	}
	if (ast_sip_add_body(tdata, &body)) {
		ast_log_chan(NULL, LOG_ERROR, "Could not add body to text video update INFO request\n");
		return -1;
	}
...
static int chan_pjsip_indicate(struct ast_channel *ast, int condition, const void *data, size_t datalen)
{
	...
	case AST_CONTROL_FLASH:
		...
		if (ast_sip_push_task(channel->session->serializer, transmit_info_dtmf, dtmf_data)) {
			ast_log_chan(NULL, LOG_WARNING, "Error sending FLASH via INFO on channel %s\n", ast_channel_name(ast));
	...
	case AST_CONTROL_HOLD:
		...
		if (!channel->session->moh_passthrough) {
			...
			if (ast_sip_push_task(channel->session->serializer, remote_send_hold, ao2_bump(channel->session))) {
				ast_log_chan(NULL, LOG_WARNING, "Could not queue task to remotely put session '%s' on hold with endpoint '%s'\n",
	...
	case AST_CONTROL_UNHOLD:
		...
		if (!channel->session->moh_passthrough) {
			...
			if (ast_sip_push_task(channel->session->serializer, remote_send_unhold, ao2_bump(channel->session))) {
				ast_log_chan(NULL, LOG_WARNING, "Could not queue task to remotely take session '%s' off hold with endpoint '%s'\n",
					ast_sorcery_object_get_id(channel->session), ast_sorcery_object_get_id(channel->session->endpoint));
				ao2_ref(channel->session, -1);
			}
		}
	...
	if (response_code) {
		...
		if (ast_sip_push_task(channel->session->serializer, indicate, ind_data)) {
			ast_log_chan(NULL, LOG_ERROR, "%s: Cannot send response code %d to endpoint %s. Could not queue task properly\n",
...
static void transfer_redirect(struct ast_sip_session *session, const char *target)
{
	...
	if (pjsip_inv_end_session(session->inv_session, 302, NULL, &packet) != PJ_SUCCESS
		|| !packet) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to redirect PJSIP session for channel %s\n",
			ast_channel_name(session->channel));
	...
	if (!(contact->uri = pjsip_parse_uri(packet->pool, tmp.ptr, tmp.slen, PJSIP_PARSE_URI_AS_NAMEADDR))) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to parse destination URI '%s' for channel %s\n",
...
/*! \brief Function called by core for GABpbx initiated transfer */
static int chan_pjsip_transfer(struct ast_channel *chan, const char *target)
{
	...
	if (ast_sip_push_task(channel->session->serializer, transfer, trnf_data)) {
		ast_log_chan(NULL, LOG_WARNING, "Error requesting transfer\n");
...
static int transmit_info_dtmf(void *data)
{
	...
	if (!(body_text = ast_str_create(32))) {
		ast_log_chan(NULL, LOG_ERROR, "Could not allocate buffer for INFO DTMF.\n");
		return -1;
	}
	...
	if (ast_sip_create_request("INFO", session->inv_session->dlg, session->endpoint, NULL, NULL, &tdata)) {
		ast_log_chan(NULL, LOG_ERROR, "Could not create DTMF INFO request\n");
		return -1;
	}
	if (ast_sip_add_body(tdata, &body)) {
		ast_log_chan(NULL, LOG_ERROR, "Could not add body to DTMF INFO request\n");
...
static int chan_pjsip_digit_end(struct ast_channel *ast, char digit, unsigned int duration)
{
	...
	if (!channel || !channel->session) {
		/* This happens when the channel is hungup while a DTMF digit is playing. See GABPBX-28086 */
	...
	case AST_SIP_DTMF_INFO:
	{
		...
		if (ast_sip_push_task(channel->session->serializer, transmit_info_dtmf, dtmf_data)) {
			ast_log_chan(NULL, LOG_WARNING, "Error sending DTMF via INFO.\n");
...
static int chan_pjsip_call(struct ast_channel *ast, const char *dest, int timeout)
{
	...
	if (ast_sip_push_task(channel->session->serializer, call, channel)) {
		ast_log_chan(NULL, LOG_WARNING, "Error attempting to place outbound call to '%s'\n", dest);
		ao2_cleanup(channel);
		SCOPE_EXIT_RTN_VALUE(-1, "Couldn't push task\n");
	}
...
/*! \brief Internal function which translates from GABpbx cause codes to SIP response codes */
...
static int chan_pjsip_hangup(struct ast_channel *ast)
{
	...
	if (ast_sip_push_task(channel->session->serializer, hangup, h_data)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to push hangup task to the threadpool. Expect bad things\n");
		goto failure;
	}
...
static int request(void *obj)
{
	...
	if (ast_strlen_zero(tmp)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to create PJSIP channel with empty destination\n");
	...
	if (ast_sip_get_disable_multi_domain()) {
		...
		if (ast_strlen_zero(endpoint_name)) {
			if (request_user) {
				ast_log_chan(NULL, LOG_ERROR, "Unable to create PJSIP channel with empty endpoint name: %s@<endpoint-name>\n",
					request_user);
			} else {
				ast_log_chan(NULL, LOG_ERROR, "Unable to create PJSIP channel with empty endpoint name\n");
			}
			req_data->cause = AST_CAUSE_CHANNEL_UNACCEPTABLE;
		...
		endpoint = ast_sorcery_retrieve_by_id(ast_sip_get_sorcery(), "endpoint",
			endpoint_name);
		if (!endpoint) {
			ast_log_chan(NULL, LOG_ERROR, "Unable to create PJSIP channel - endpoint '%s' was not found\n", endpoint_name);
	...
	ao2_ref(endpoint, -1);
	if (!session) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to create outgoing session to endpoint '%s'\n", endpoint_name);
		req_data->cause = AST_CAUSE_NO_ROUTE_DESTINATION;
...
static int load_module(void)
{
	...
	if (ast_custom_function_register(&chan_pjsip_dial_contacts_function)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to register PJSIP_DIAL_CONTACTS dialplan function\n");
		goto end;
	}

	if (ast_custom_function_register(&chan_pjsip_parse_uri_function)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to register PJSIP_PARSE_URI dialplan function\n");
		goto end;
	}

	if (ast_custom_function_register(&chan_pjsip_parse_uri_from_function)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to register PJSIP_PARSE_URI_FROM dialplan function\n");
		goto end;
	}

	if (ast_custom_function_register(&media_offer_function)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to register PJSIP_MEDIA_OFFER dialplan function\n");
		goto end;
	}

	if (ast_custom_function_register(&dtmf_mode_function)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to register PJSIP_DTMF_MODE dialplan function\n");
		goto end;
	}

	if (ast_custom_function_register(&moh_passthrough_function)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to register PJSIP_MOH_PASSTHROUGH dialplan function\n");
		goto end;
	}

	if (ast_custom_function_register(&session_refresh_function)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to register PJSIP_SEND_SESSION_REFRESH dialplan function\n");
		goto end;
	}

	if (ast_custom_function_register(&transfer_handling_function)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to register PJSIP_TRANSFER_HANDLING dialplan function\n");
		goto end;
	}

	if (ast_register_application_xml(app_pjsip_hangup, pjsip_app_hangup)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to register PJSIPHangup dialplan application\n");
		goto end;
	}
	...
	if (!(pjsip_uids_onhold = ao2_container_alloc_hash(AO2_ALLOC_OPT_LOCK_RWLOCK,
		...
		ast_log_chan(NULL, LOG_ERROR, "Unable to create held channels container\n");
		goto end;
	}
	...
	if (pjsip_channel_cli_register()) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to register PJSIP Channel CLI\n");
		goto end;
	}
...
/*! \brief Unload the PJSIP channel from GABpbx */
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "PJSIP Channel Driver",
