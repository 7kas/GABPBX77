 ...
 * \file
 *
 * \brief chan_unistim channel driver for GABpbx
...
#include "gabpbx/paths.h"	/* ast_config_AST_LOG_DIR used in (too ?) many places */
...
struct unistim_subchannel {
	...
	struct ast_channel *owner;	/*! GABpbx channel used by the subchannel */
...
static struct unistim_device {
	...
	char language[MAX_LANGUAGE];    /*!< Language for gabpbx sounds */
...
static const char *ustmtext(const char *str, struct unistimsession *pte)
{
	...
	if (!lang->trans) {
		...
		if (!lang->trans) {
			ast_log_chan(NULL, LOG_ERROR, "Unable to allocate container for translation!\n");
			return str;
		}
		...
		f = fopen(tmp, "r");
		if (!f) {
			ast_log_chan(NULL, LOG_WARNING, "There is no translation file for '%s'\n", lang->lang_short);
			return str;
		}
		while (fgets(tmp, sizeof(tmp), f)) {
			if (!(p = strchr(tmp, '\n'))) {
				ast_log_chan(NULL, LOG_ERROR, "Too long line found in language file - truncated!\n");
				continue;
			}
			*p = '\0';
			if (!(p = strchr(tmp, '"'))) {
...
static void display_last_error(const char *sz_msg)
{
	...
	ast_log_chan(NULL, LOG_WARNING, "%s : (%d) %s\n", sz_msg, errno, strerror(errno));
}
...
static void send_client(int size, const unsigned char *data, struct unistimsession *pte)
{
	...
	if (buf_pos >= MAX_BUF_NUMBER) {
		ast_log_chan(NULL, LOG_WARNING, "Error : send queue overflow\n");
...
/*#ifdef DUMP_PACKET */
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "Sending data with seq #0x%04x Using slot #%d :\n", (unsigned)pte->seq_server, buf_pos);
	}
/*#endif */
...
static void send_ping(struct unistimsession *pte)
{
	BUFFSEND;
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "Sending ping\n");
	}
...
static int get_to_address(int fd, struct sockaddr_in *toAddr)
{
	...
	if (err == -1) {
		ast_log_chan(NULL, LOG_WARNING, "recvmsg returned an error: %s\n", strerror(errno));
		return err;
	}
		...
		cmsg = CMSG_NXTHDR(&mh, cmsg))
	{
		 if (cmsg->cmsg_level == IPPROTO_IP && cmsg->cmsg_type == IP_PKTINFO) {
			...
			addr = pkt->ipi_addr;
			if (unistimdebug) {
				ast_verb_chan(NULL, 0, "message received on address %s\n", ast_inet_ntoa(addr));
			}
		}
	}
...
static struct unistimsession *create_client(const struct sockaddr_in *addr_from)
{
	...
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "Creating a new entry for the phone from %s received via server ip %s\n",
...
static void send_end_call(struct unistimsession *pte)
{
	BUFFSEND;
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "Sending end call\n");
	}
...
static void check_send_queue(struct unistimsession *pte)
{
	...
	if (pte->last_buf_available == 1) {
		if (unistimdebug) {
			ast_verb_chan(NULL, 0, "Our single packet was ACKed.\n");
		}
		pte->last_buf_available--;
		set_ping_timer(pte);
		return;
	...
	/* Check if this ACK catch up our latest packet */
		...
		if (unistimdebug) {
			ast_verb_chan(NULL, 0, "Our send queue is completely ACKed.\n");
		}
	...
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "We still have packets in our send queue\n");
	}
	return;
}
...
static void send_start_timer(struct unistimsession *pte)
{
	BUFFSEND;
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "Sending start timer\n");
	}
...
static void send_stop_timer(struct unistimsession *pte)
{
	BUFFSEND;
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "Sending stop timer\n");
	}
...
static void send_icon(unsigned char pos, unsigned char status, struct unistimsession *pte)
{
	BUFFSEND;
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "Sending icon pos %d with status 0x%02hhx\n", pos, status);
	}
...
static void send_expansion_icon(unsigned char pos, unsigned char status, struct unistimsession *pte)
{
	BUFFSEND;
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "Sending expansion icon pos %d with status 0x%02hhx\n", pos, status);
	}
...
static void send_expansion_text(unsigned char pos, struct unistimsession *pte, const char *text)
{
	...
	BUFFSEND;
	if (!text) {
		ast_log_chan(NULL, LOG_ERROR, "[expansion] Asked to display NULL text (pos %d)\n", pos);
		return;
	}
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "[expansion] Sending text at pos %d\n", pos);
	}
...
static void send_tone(struct unistimsession *pte, uint16_t tone1, uint16_t tone2)
{
	...
	if (!tone1) {
		if (unistimdebug) {
			ast_verb_chan(NULL, 0, "Sending Stream Based Tone Off\n");
		}
	...
	/* Since most of the world use a continuous tone, it's useless
	   if (unistimdebug)
	   ast_verb_chan(NULL, 0, "Sending Stream Based Tone Cadence Download\n");
	...
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "Sending Stream Based Tone Frequency Component List Download %d %d\n", tone1, tone2);
	}
	tone1 *= 8;
	if (!tone2) {
	...
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "Sending Stream Based Tone On\n");
	}
			 ...
			 const char *text)
{
	...
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "Sending favorite pos %d with status 0x%02hhx\n", pos, status);
	}
...
static void refresh_all_favorite(struct unistimsession *pte)
{
	...
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "Refreshing all favorite\n");
	}
	for (i = 0; i < FAVNUM; i++) {
...
static int get_avail_softkey(struct unistimsession *pte, const char* name)
{
	...
	for (i = 0; i < FAVNUM; i++) {
		...
		if (is_key_line(pte->device, i)) {
			...
			if (unistimdebug) {
				ast_verb_chan(NULL, 0, "Found softkey %d for device %s\n", i, name);
			}
			return i;
		}
	}
...
static int register_extension(const struct unistimsession *pte)
{
	...
	line = AST_LIST_FIRST(&pte->device->lines);
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "Trying to register extension '%s' into context '%s' to %s\n",
...
{
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "Trying to unregister extension '%s' context '%s'\n",
					pte->device->extension_number, pte->device->context);
	}
...
static void close_client(struct unistimsession *s)
{
	...
	if (cur) {				      /* Session found ? */
		if (cur->device) {	      /* This session was registered ? */
			...
			if (unistimdebug) {
				ast_verb_chan(NULL, 0, "close_client session %p device %p\n", s, s->device);
			}
			...
			AST_LIST_TRAVERSE_SAFE_BEGIN(&s->device->subs, sub, list) {
				...
				if (sub->owner) {       /* Call in progress ? */
					if (unistimdebug) {
						ast_verb_chan(NULL, 0, "Aborting call\n");
					}
			...
			if (unistimdebug) {
				ast_verb_chan(NULL, 0, "Freeing an unregistered client\n");
			}
		}
		if (prev) {
			prev->next = cur->next;
		...
		ast_log_chan(NULL, LOG_WARNING, "Trying to delete non-existent session %p?\n", s);
	}
	ast_mutex_unlock(&sessionlock);
	return;
}
...
static int send_retransmit(struct unistimsession *pte)
{
	...
	if (++pte->nb_retransmit >= NB_MAX_RETRANSMIT) {
		if (unistimdebug) {
			ast_verb_chan(NULL, 0, "Too many retransmit - freeing client\n");
		}
		ast_mutex_unlock(&pte->lock);
		close_client(pte);
		return 1;
	...
	for (i = pte->last_buf_available - (pte->seq_server - pte->last_seq_ack);
		...
		if (unistimdebug) {
			...
			seq = ntohs(sbuf[1]);
			ast_verb_chan(NULL, 0, "Retransmit slot #%d (seq=#0x%04x), last ack was #0x%04x\n", i,
		 ...
		 const char *text)
{
	...
	BUFFSEND;
	if (!text) {
		ast_log_chan(NULL, LOG_ERROR, "Asked to display NULL text (pos %d, inverse flag %d)\n", pos, inverse);
		return;
	}
	...
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "Sending text at pos %d, inverse flag %d\n", pos, inverse);
	}
...
static void send_text_status(struct unistimsession *pte, const char *text)
{
	...
	int i;
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "Sending status text\n");
	}
	if (pte->device) {
...
static void send_led_update(struct unistimsession *pte, unsigned char led)
{
	BUFFSEND;
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "Sending led_update (%x)\n", (unsigned)led);
	}
...
*/
	BUFFSEND;
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "Sending mute packet (%x)\n", (unsigned)mute);
	}
				 ...
				 unsigned char mute)
{
	...
	int mute_icon = -1;
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "Sending select output packet output=%x volume=%x mute=%x\n",
	...
	if (output == OUTPUT_HANDSET) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Invalid output (%d)\n", output);
	}
	if (mute_icon != -1) {
...
static void send_ring(struct unistimsession *pte, signed char volume, signed char style)
{
	BUFFSEND;
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "Sending ring packet\n");
	}
...
static void send_no_ring(struct unistimsession *pte)
{
	BUFFSEND;
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "Sending no ring packet\n");
	}
...
static void send_texttitle(struct unistimsession *pte, const char *text)
{
	...
	int i;
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "Sending title text\n");
	}
...
static void send_month_labels(struct unistimsession *pte, int month)
{
	...
	int i = 0;
	if (unistimdebug) {
	        ast_verb_chan(NULL, 0, "Sending Month Labels\n");
	}
	...
	while (i < 2) {
		...
		ast_log_chan(NULL, LOG_WARNING,"%s\n", month_name);
...
static void send_date_time(struct unistimsession *pte)
{
	...
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "Sending Time & Date\n");
	}
...
static void send_date_time2(struct unistimsession *pte)
{
	...
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "Sending Time & Date #2\n");
	}
...
static void send_date_time3(struct unistimsession *pte)
{
	...
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "Sending Time & Date #3\n");
	}
...
static void send_blink_cursor(struct unistimsession *pte)
{
	BUFFSEND;
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "Sending set blink\n");
	}
...
static void send_cursor_pos(struct unistimsession *pte, unsigned char pos)
{
	BUFFSEND;
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "Sending set cursor position\n");
	}
...
static void send_charset_update(struct unistimsession *pte, int charset)
{
	...
	BUFFSEND;
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "Sending set default charset\n");
	}
...
static void rcv_resume_connection_with_server(struct unistimsession *pte)
{
	BUFFSEND;
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "ResumeConnectionWithServer received\n");
		ast_verb_chan(NULL, 0, "Sending packet_send_query_mac_address\n");
	}
...
static struct unistim_subchannel *unistim_alloc_sub(struct unistim_device *d, int x)
{
	...
	if (unistimdebug) {
		ast_verb_chan(NULL, 3, "Allocating UNISTIM subchannel #%d on %s ptr=%p\n", x, d->name, sub);
	}
...
static void rcv_mac_addr(struct unistimsession *pte, const unsigned char *buf)
{
	...
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "MAC Address received: %s\n", addrmac);
	}
	...
	if (!res) {
		...
		case AUTOPROVISIONING_NO:
			ast_log_chan(NULL, LOG_WARNING, "No entry found for this phone : %s\n", addrmac);
		...
		case AUTOPROVISIONING_YES:
			{
				...
				if (unistimdebug) {
					ast_verb_chan(NULL, 0, "New phone, autoprovisioning on\n");
				}
				...
				ast_mutex_unlock(&devicelock);
				if (!d) {
					ast_log_chan(NULL, LOG_WARNING, "No entry [template] found in unistim.conf\n");
					pte->state = STATE_AUTHDENY;
				}
			}
			break;
		case AUTOPROVISIONING_TN:
			...
			break;
		default:
			ast_log_chan(NULL, LOG_WARNING, "Internal error : unknown autoprovisioning value = %u\n",
					autoprovisioning);
		}
	}
	if (pte->state != STATE_AUTHDENY) {
		...
		ast_verb_chan(NULL, 3, "Device '%s' successfully registered\n", pte->device->name);

		AST_LIST_LOCK(&pte->device->subs);
		AST_LIST_TRAVERSE_SAFE_BEGIN(&pte->device->subs, sub, list) {
			if (sub) {
				ast_log_chan(NULL, LOG_ERROR, "Subchannel lost sice reboot. Hanged channel may appear!\n");
				AST_LIST_REMOVE_CURRENT(list);
				ast_free(sub);
			}
		}
		...
		case EXTENSION_TN:
			...
			break;
		default:
			ast_log_chan(NULL, LOG_WARNING, "Internal error, extension value unknown : %u\n",
	...
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "Sending query_basic_manager_04\n");
	}
	...
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "Sending query_basic_manager_10\n");
	}
...
static int write_history(struct unistimsession *pte, char way, char ismissed)
{
	...
	if (strchr(pte->device->name, '/') || (pte->device->name[0] == '.')) {
		ast_log_chan(NULL, LOG_WARNING, "Account code '%s' insecure for writing file\n",
				pte->device->name);
		return -1;
	}
	...
	if (ast_mkdir(tmp, 0770)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to create directory for history\n");
                return -1;
	}
	...
	if (count > MAX_ENTRY_LOG) {
		ast_log_chan(NULL, LOG_WARNING, "Invalid count in history header of %s (%d max %d)\n", tmp,
				count, MAX_ENTRY_LOG);
		fclose(f);
		return -1;
	}
...
static int attempt_transfer(struct unistim_subchannel *p1, struct unistim_subchannel *p2)
{
	...
	if (!p1->owner || !p2->owner) {
		ast_log_chan(NULL, LOG_WARNING, "Transfer attempted without dual ownership?\n");
		return -1;
	}
	...
	case AST_BRIDGE_TRANSFER_INVALID:
		ast_log_chan(NULL, LOG_WARNING, "Transfer failed. Invalid bridge setup\n");
		break;
	case AST_BRIDGE_TRANSFER_NOT_PERMITTED:
		ast_log_chan(NULL, LOG_WARNING, "Transfer not permitted\n");
		break;
	case AST_BRIDGE_TRANSFER_FAIL:
		ast_log_chan(NULL, LOG_WARNING, "Transfer encountered internal error\n");
		break;
...
static void sub_start_silence(struct unistimsession *pte, struct unistim_subchannel *sub)
{
	...
	if (!pte->device->silence_generator) {
		...
		if (pte->device->silence_generator == NULL) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to start a silence generator.\n");
		} else if (unistimdebug) {
			ast_verb_chan(NULL, 0, "Starting silence generator\n");
		}
	}
...
static void sub_stop_silence(struct unistimsession *pte, struct unistim_subchannel *sub)
{
	...
	if (pte->device->silence_generator) {
		if (unistimdebug) {
			ast_verb_chan(NULL, 0, "Stopping silence generator\n");
		}
		if (sub->owner) {
			ast_channel_stop_silence_generator(sub->owner, pte->device->silence_generator);
		} else {
			ast_log_chan(NULL, LOG_WARNING, "Trying to stop silence generator on a null channel!\n");
		}
...
static void close_call(struct unistimsession *pte)
{
	...
	send_stop_timer(pte);
	if (!sub) {
		ast_log_chan(NULL, LOG_WARNING, "Close call without sub\n");
		return;
	}
	...
	if (sub->owner) {
		...
		if (sub_transf) {
			...
			if (attempt_transfer(sub, sub_transf) < 0) {
				ast_verb_chan(NULL, 0, "attempt_transfer failed.\n");
			}
		} else {
		...
		if (sub_transf) {
			if (sub_transf->owner) {
				...
				ast_log_chan(NULL, LOG_WARNING, "threeway sub without owner\n");
			}
		} else {
			ast_verb_chan(NULL, 0, "USTM(%s@%s-%d) channel already destroyed\n", sub->parent->name,
...
static void *unistim_ss(void *data)
{
	...
	ast_verb_chan(NULL, 3, "Starting switch on '%s@%s-%d' to %s\n", l->name, l->parent->name, sub->softkey, s->device->phone_number);
...
static void send_start_rtp(struct unistim_subchannel *sub)
{
	...
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "RTP started : Our IP/port is : %s:%hd with codec %s\n",
			 ast_inet_ntoa(us.sin_addr),
			 htons(us.sin_port), ast_format_get_name(ast_channel_readformat(sub->owner)));
		ast_verb_chan(NULL, 0, "Starting phone RTP stack. Our public IP is %s\n",
	...
	if ((ast_format_cmp(ast_channel_readformat(sub->owner), ast_format_ulaw) == AST_FORMAT_CMP_EQUAL) ||
		...
		if (unistimdebug) {
			ast_verb_chan(NULL, 0, "Sending packet_send_rtp_packet_size for codec %d\n", codec);
		}
	...
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "Sending Jitter Buffer Parameters Configuration\n");
	}
	...
	if (pte->device->rtp_method != 0) {
		...
		if (unistimdebug) {
			ast_verb_chan(NULL, 0, "Sending OpenAudioStreamTX using method #%d\n", pte->device->rtp_method);
		}
		...
		if (unistimdebug) {
			ast_verb_chan(NULL, 0, "Sending OpenAudioStreamRX\n");
		}
		if (pte->device->rtp_method == 3) {
		...
		if (unistimdebug) {
			ast_verb_chan(NULL, 0, "Sending packet_send_call default method\n");
		}
		...
		if (ast_format_cmp(ast_channel_readformat(sub->owner), ast_format_ulaw) == AST_FORMAT_CMP_EQUAL) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Unsupported codec %s!\n",
...
static void start_rtp(struct unistim_subchannel *sub)
{
	...
	/* Sanity checks */
	if (!sub) {
		ast_log_chan(NULL, LOG_WARNING, "start_rtp with a null subchannel !\n");
		return;
	}
	if (!sub->parent) {
		ast_log_chan(NULL, LOG_WARNING, "start_rtp with a null line!\n");
		return;
	}
	if (!sub->parent->parent) {
		ast_log_chan(NULL, LOG_WARNING, "start_rtp with a null device!\n");
		return;
	}
	if (!sub->parent->parent->session) {
		ast_log_chan(NULL, LOG_WARNING, "start_rtp with a null session!\n");
		return;
	}
	if (!sub->owner) {
		ast_log_chan(NULL, LOG_WARNING, "start_rtp with a null gabpbx channel!\n");
		return;
	}
	...
	/* Allocate the RTP */
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "Starting RTP. Bind on %s\n", ast_inet_ntoa(sout.sin_addr));
	}
	ast_sockaddr_from_sin(&sout_tmp, &sout);
	sub->rtp = ast_rtp_instance_new("gabpbx", sched, &sout_tmp, NULL);
	if (!sub->rtp) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to create RTP session: %s binaddr=%s\n",
...
static void send_dial_tone(struct unistimsession *pte)
{
	...
	if ((ts = ast_get_indication_tone(pte->device->tz, "dial"))) {
		...
		if (unistimdebug) {
			ast_verb_chan(NULL, 0, "Country code found (%s), freq1=%u freq2=%u\n",
...
static void swap_subs(struct unistim_subchannel *a, struct unistim_subchannel *b)
{
	...
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "Swapping %p and %p\n", a, b);
	}
	if ((!a->owner) || (!b->owner)) {
		ast_log_chan(NULL, LOG_WARNING,
...
static void transfer_call_step1(struct unistimsession *pte)
{
	...
	if (!sub || !sub->owner) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to find subchannel for music on hold\n");
		return;
	}
	/* Start music on hold if appropriate */
	if (sub->moh) {
		ast_log_chan(NULL, LOG_WARNING, "Transfer with peer already listening music on hold\n");
	} else {
...
static void transfer_cancel_step2(struct unistimsession *pte)
{
	...
	if (sub_trans) {
		if (unistimdebug) {
			ast_verb_chan(NULL, 0, "Transfer canceled, hangup our threeway channel\n");
		}
		if (sub->owner) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Canceling a threeway channel without owner\n");
		}
		return;
	}
}
...
static void handle_call_outgoing(struct unistimsession *s)
{
	...
	if (sub) {
		...
		if (sub_trans) {
			ast_log_chan(NULL, LOG_WARNING, "Can't transfer while active subchannel exists!\n");
			return;
		}
		if (!sub->owner) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to find subchannel with music on hold\n");
			return;
		}
		...
		if (!c) {
			ast_log_chan(NULL, LOG_WARNING, "Cannot allocate new structure on channel %p\n", sub->parent);
			return;
		}
		...
		if (ast_pthread_create(&sub->ss_thread, NULL, unistim_ss, c)) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to start simple switch on channel %p\n", c);
			sub->ss_thread = AST_PTHREADT_NULL;
			ast_hangup(c);
			return;
		}
		if (unistimdebug) {
			ast_verb_chan(NULL, 0, "Started three way call on channel %p (%s) subchan %u\n",
	...
	if (softkey == -1) {
		ast_log_chan(NULL, LOG_WARNING, "Have no avail softkey for calling\n");
		return;
	}
	...
	if (!(sub = unistim_alloc_sub(s->device, SUB_REAL))) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to allocate subchannel!\n");
		return;
	}
	...
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "Using softkey %d, line %p\n", sub->softkey, sub->parent);
	}
	...
	if (!sub->owner) {		      /* A call is already in progress ? */
		...
		if (c && !strcmp(s->device->phone_number, pickupexten)) {
			if (unistimdebug) {
				ast_verb_chan(NULL, 0, "Try to pickup in unistim_new\n");
			}
			send_text(TEXT_LINE0, TEXT_NORMAL, s, "");
			send_text_status(s, ustmtext("       Transf        Hangup", s));
			send_start_timer(s);
			if (ast_pickup_call(c)) {
				ast_log_chan(NULL, LOG_NOTICE, "Nothing to pick up\n");
			...
			/* start switch */
			if (ast_pthread_create(&sub->ss_thread, NULL, unistim_ss, c)) {
				ast_log_chan(NULL, LOG_WARNING, "Unable to create switch thread\n");
				sub->ss_thread = AST_PTHREADT_NULL;
				ast_queue_hangup_with_cause(c, AST_CAUSE_SWITCH_CONGESTION);
			}
		} else
			ast_log_chan(NULL, LOG_WARNING, "Unable to create channel for %s@%s\n",
...
static void handle_call_incoming(struct unistimsession *s)
{
	...
	if (!sub) {
		ast_log_chan(NULL, LOG_WARNING, "No ringing lines on: %s\n", s->device->name);
		return;
	}
	...
	if (sub->softkey < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Can not assign softkey for incoming call on: %s\n", s->device->name);
		return;
	}
	send_favorite_short(sub->softkey, FAV_ICON_OFFHOOK_BLACK, s);
	...
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "Handle Call Incoming for %s@%s\n", sub->parent->name,
					s->device->name);
	}
	start_rtp(sub);
	if (!sub->rtp) {
...
static int send_dtmf_tone(struct unistimsession *pte, char digit)
{
	...
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "Phone Play Digit %c\n", digit);
	}
...
static int unistim_do_senddigit(struct unistimsession *pte, char digit)
{
	...
	if (!sub || !sub->owner || sub->alreadygone) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to find subchannel in dtmf senddigit\n");
		return -1;
	}
	...
	if (pte->device->dtmfduration > 0) {
		if (unistimdebug) {
			ast_verb_chan(NULL, 0, "Send Digit %c (%i ms)\n", digit, pte->device->dtmfduration);
		}
...
static void handle_key_fav(struct unistimsession *pte, char keycode)
{
	...
	if (!pte->device->ssub[keynum]) { /* Key have no assigned call */
		...
		if (is_key_line(pte->device, keynum)) {
			if (unistimdebug) {
				ast_verb_chan(NULL, 0, "Handle line w/o sub - dialpage\n");
			}
			...
			if (unistimdebug) {
				ast_verb_chan(NULL, 0, "Handle favorite w/o sub - dialing\n");
			}
			if ((pte->device->output == OUTPUT_HANDSET) &&
				(pte->device->receiver_state == STATE_ONHOOK)) {
				send_select_output(pte, OUTPUT_SPEAKER, pte->device->volume, MUTE_OFF);
...
static void key_favorite(struct unistimsession *pte, char keycode)
{
	...
	if (!is_key_favorite(pte->device, fav)) {
		ast_log_chan(NULL, LOG_WARNING, "It's not a favorite key\n");
		return;
	}
...
static void init_phone_step2(struct unistimsession *pte)
{
	BUFFSEND;
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "Sending S4\n");
	}
	...
	send_date_time3(pte);
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "Sending S7\n");
	}
	...
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "Sending Contrast\n");
	}
	...
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "Sending S9\n");
	}
	...
	send_ping(pte);
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "Sending init language\n");
	}
	if (pte->device) {
	...
	refresh_all_favorite(pte);
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "Sending arrow\n");
	}
...
static void process_request(int size, unsigned char *buf, struct unistimsession *pte)
{
	...
	if (memcmp(buf + SIZE_HEADER, packet_recv_firm_version, sizeof(packet_recv_firm_version)) == 0) {
		buf[size] = 0;
		if (unistimdebug) {
			ast_verb_chan(NULL, 0, "Got the firmware version : '%s'\n", buf + 13);
		}
	...
	if (memcmp(buf + SIZE_HEADER, packet_recv_it_type, sizeof(packet_recv_it_type)) == 0) {
		char type = buf[13];
		if (unistimdebug) {
			ast_verb_chan(NULL, 0, "Got the equipment type: '%d'\n", type);
		}
		switch (type) {
		case 0x03: /* i2002 */
			if (pte->device) {
	...
	if (memcmp(buf + SIZE_HEADER, packet_recv_r2, sizeof(packet_recv_r2)) == 0) {
		if (unistimdebug) {
			ast_verb_chan(NULL, 0, "R2 received\n");
		}
		return;
	}
	...
	if (pte->state < STATE_MAINPAGE) {
		if (unistimdebug) {
			ast_verb_chan(NULL, 0, "Request not authorized in this state\n");
		}
		return;
	}
	if (!memcmp(buf + SIZE_HEADER, packet_recv_expansion_pressed_key, sizeof(packet_recv_expansion_pressed_key))) {
		...
		if (unistimdebug) {
			ast_verb_chan(NULL, 0, "Expansion key pressed: keycode = 0x%02hhx - current state: %s\n", (unsigned char)keycode,
	...
	if (!memcmp(buf + SIZE_HEADER, packet_recv_pressed_key, sizeof(packet_recv_pressed_key))) {
		...
		if (unistimdebug) {
			ast_verb_chan(NULL, 0, "Key pressed: keycode = 0x%02hhx - current state: %s\n", (unsigned char)keycode,
						ptestate_tostr(pte->state));
		}
		if (keycode == KEY_MUTE) {
		...
		case STATE_INIT:
			if (unistimdebug) {
				ast_verb_chan(NULL, 0, "No keys allowed in the init state\n");
			}
			break;
		case STATE_AUTHDENY:
			if (unistimdebug) {
				ast_verb_chan(NULL, 0, "No keys allowed in authdeny state\n");
			}
			break;
		case STATE_MAINPAGE:
		...
		case STATE_HISTORY:
			...
			break;
		default:
			ast_log_chan(NULL, LOG_WARNING, "Key : Unknown state\n");
		}
		return;
	}
	if (memcmp(buf + SIZE_HEADER, packet_recv_pick_up, sizeof(packet_recv_pick_up)) == 0) {
		if (unistimdebug) {
			ast_verb_chan(NULL, 0, "Handset off hook, current state: %s\n", ptestate_tostr(pte->state));
		}
	...
	if (memcmp(buf + SIZE_HEADER, packet_recv_hangup, sizeof(packet_recv_hangup)) == 0) {
		if (unistimdebug) {
			ast_verb_chan(NULL, 0, "Handset on hook, current state: %s\n", ptestate_tostr(pte->state));
		}
		if (!pte->device) {
			return;
		}
	...
	struct sockaddr_in *addr_from)
{
	...
	if (size < 10) {
		if (size == 0) {
			ast_log_chan(NULL, LOG_WARNING, "%s Read error\n", tmpbuf);
		} else {
			ast_log_chan(NULL, LOG_NOTICE, "%s Packet too short - ignoring\n", tmpbuf);
		}
		return;
	}
	if (sbuf[0] == 0xffff) {	/* Starting with 0xffff ? *//* Yes, discovery packet ? */
		if (size != sizeof(packet_rcv_discovery)) {
			ast_log_chan(NULL, LOG_NOTICE, "%s Invalid size of a discovery packet\n", tmpbuf);
		} else {
			if (memcmp(buf, packet_rcv_discovery, sizeof(packet_rcv_discovery)) == 0) {
				if (unistimdebug) {
					ast_verb_chan(NULL, 0, "Discovery packet received - Sending Discovery ACK\n");
				}
				if (pte) {	      /* A session was already active for this IP ? */
					if (pte->state == STATE_INIT) { /* Yes, but it's a dupe */
						if (unistimdebug) {
							ast_verb_chan(NULL, 1, "Duplicated Discovery packet\n");
						}
			...
			ast_log_chan(NULL, LOG_NOTICE, "%s Invalid discovery packet\n", tmpbuf);
		}
		return;
	}
	if (!pte) {
		if (unistimdebug) {
			ast_verb_chan(NULL, 0, "%s Not a discovery packet from an unknown source : ignoring\n", tmpbuf);
		}
		return;
	}

	if (sbuf[0] != 0) {		     /* Starting with something else than 0x0000 ? */
		ast_log_chan(NULL, LOG_NOTICE, "Unknown packet received - ignoring\n");
		return;
	}
	if (buf[5] != 2) {
		ast_log_chan(NULL, LOG_NOTICE, "%s Wrong direction : got 0x%02hhx expected 0x02\n", tmpbuf, buf[5]);
		return;
	}
	seq = ntohs(sbuf[1]);
	if (buf[4] == 1) {
		ast_mutex_lock(&pte->lock);
		if (unistimdebug) {
			ast_verb_chan(NULL, 0, "ACK received for packet #0x%04x\n", (unsigned)seq);
		}
		pte->nb_retransmit = 0;
		...
		if (pte->last_seq_ack > seq) {
			if (pte->last_seq_ack == 0xffff) {
				ast_verb_chan(NULL, 0, "ACK at 0xffff, restarting counter.\n");
				pte->last_seq_ack = 0;
			} else {
		...
		if (unistimdebug) {
			ast_verb_chan(NULL, 0, "%s ACK gap : Received ACK #0x%04x, previous was #0x%04x\n",
						tmpbuf, (unsigned)seq, (unsigned)pte->last_seq_ack);
		}
	...
	if (buf[4] == 2) {
		if (unistimdebug) {
			ast_verb_chan(NULL, 0, "Request received\n");
		}
	...
	if (buf[4] == 0) {
		ast_log_chan(NULL, LOG_NOTICE, "%s Retransmit request for packet #0x%04x\n", tmpbuf, (unsigned)seq);
		if (pte->last_seq_ack > seq) {
			ast_log_chan(NULL, LOG_NOTICE,
	...
	ast_log_chan(NULL, LOG_NOTICE, "%s Unknown request : got 0x%02hhx expected 0x00,0x01 or 0x02\n", tmpbuf, buf[4]);
	return;
}
...
static struct unistimsession *channel_to_session(struct ast_channel *ast)
{
	struct unistim_subchannel *sub;
	if (!ast) {
		ast_log_chan(NULL, LOG_WARNING, "Unistim callback function called with a null channel\n");
		return NULL;
	}
	if (!ast_channel_tech_pvt(ast)) {
		ast_log_chan(NULL, LOG_WARNING, "Unistim callback function called without a tech_pvt\n");
		return NULL;
	}
	sub = ast_channel_tech_pvt(ast);

	if (!sub->parent) {
		ast_log_chan(NULL, LOG_WARNING, "Unistim callback function called without a line\n");
		return NULL;
	}
	if (!sub->parent->parent) {
		ast_log_chan(NULL, LOG_WARNING, "Unistim callback function called without a device\n");
		return NULL;
	}
	ast_mutex_lock(&sub->parent->parent->lock);
	if (!sub->parent->parent->session) {
		ast_log_chan(NULL, LOG_WARNING, "Unistim callback function called without a session\n");
...
static int unistim_call(struct ast_channel *ast, const char *dest, int timeout)
{
	...
	if (!session) {
		ast_log_chan(NULL, LOG_ERROR, "Device not registered, cannot call %s\n", dest);
		return -1;
	}
	sub = ast_channel_tech_pvt(ast);
	sub_real = get_sub(session->device, SUB_REAL);
	if ((ast_channel_state(ast) != AST_STATE_DOWN) && (ast_channel_state(ast) != AST_STATE_RESERVED)) {
		ast_log_chan(NULL, LOG_WARNING, "unistim_call called on %s, neither down nor reserved\n",
				ast_channel_name(ast));
		return -1;
	}

	if (unistimdebug) {
		ast_verb_chan(NULL, 3, "unistim_call(%s)\n", ast_channel_name(ast));
	}
	...
	if (ast_strlen_zero(ast_channel_call_forward(ast))) { /* Send ring only if no call forward, otherwise short ring will appear */
		...
		for (i = 0; i < FAVNUM; i++) {
			...
			if (is_key_line(session->device, i) && !strcmp(sub->parent->name, session->device->sline[i]->name)) {
				if (unistimdebug) {
					ast_verb_chan(NULL, 0, "Found softkey %d for line %s\n", i, sub->parent->name);
				}
...
static int unistim_hangup_clean(struct ast_channel *ast, struct unistim_subchannel *sub) {
	...
	if (sub->rtp) {
		if (unistimdebug) {
			ast_verb_chan(NULL, 0, "Destroying RTP session\n");
		}
...
static int unistim_hangup(struct ast_channel *ast)
{
	...
	if (sub_trans && (sub_trans->owner) && (sub->subtype == SUB_REAL)) { /* 3rd party busy or congested and transfer_cancel_step2 does not called */
		if (unistimdebug) {
			ast_verb_chan(NULL, 0, "Threeway call disconnected, switching to real call\n");
		}
		ast_queue_unhold(sub_trans->owner);
		sub_trans->moh = 0;
		sub_trans->subtype = SUB_REAL;
	...
	if (sub_real && (sub_real->owner) && (sub->subtype == SUB_THREEWAY) && (s->state == STATE_CALL)) { /* 3way call cancelled by softkey pressed */
		if (unistimdebug) {
			ast_verb_chan(NULL, 0, "Real call disconnected, stay in call\n");
		}
		send_text_status(s, ustmtext("       Transf        Hangup", s));
...
static int unistim_answer(struct ast_channel *ast)
{
	...
	s = channel_to_session(ast);
	if (!s) {
		ast_log_chan(NULL, LOG_WARNING, "unistim_answer on a disconnected device ?\n");
		return -1;
	}
	sub = ast_channel_tech_pvt(ast);
	l = sub->parent;
	...
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "unistim_answer(%s) on %s@%s-%d\n", ast_channel_name(ast), l->name,
...
static int unistimsock_read(int *id, int fd, short events, void *ignore)
{
	...
	if (dw_num_bytes_rcvd == -1) {
		if (errno == EAGAIN) {
			ast_log_chan(NULL, LOG_NOTICE, "UNISTIM: Received packet with bad UDP checksum\n");
		} else if (errno != ECONNREFUSED) {
			ast_log_chan(NULL, LOG_WARNING, "Recv error %d (%s)\n", errno, strerror(errno));
		}
		return 1;
	}
	...
	for (dw_num_bytes_rcvdd = 0; dw_num_bytes_rcvdd < dw_num_bytes_rcvd;
		 dw_num_bytes_rcvdd++)
		ast_verb_chan(NULL, 0, "%02hhx ", buff[dw_num_bytes_rcvdd]);
	ast_verb_chan(NULL, 0, "\n******************************************\n");
#endif

	if (!found) {
		if (unistimdebug) {
			ast_verb_chan(NULL, 0, "Received a packet from an unknown source\n");
		}
	...
	const struct unistim_subchannel *sub)
{
	...
	if (!ast) {
		ast_log_chan(NULL, LOG_WARNING, "Channel NULL while reading\n");
		return &ast_null_frame;
	}

	if (!sub->rtp) {
		ast_log_chan(NULL, LOG_WARNING, "RTP handle NULL while reading on subchannel %u\n",
				sub->subtype);
		return &ast_null_frame;
	}
...
static int unistim_write(struct ast_channel *ast, struct ast_frame *frame)
{
	...
	if (frame->frametype != AST_FRAME_VOICE) {
		if (frame->frametype == AST_FRAME_IMAGE) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Can't send %u type frames with unistim_write\n",
					frame->frametype);
			return 0;
		}
	} else {
...
static int unistim_fixup(struct ast_channel *oldchan, struct ast_channel *newchan)
{
	...
	if (p->owner != oldchan) {
		ast_log_chan(NULL, LOG_WARNING, "old channel wasn't %s (%p) but was %s (%p)\n",
	...
	const char *indication)
{
	...
	if ((ts = ast_get_indication_tone(tz, indication))) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Unable to get indication tone for %s\n", indication);
	}
}
...
static int unistim_indicate(struct ast_channel *ast, int ind, const void *data, size_t datalen)
{
	...
	if (unistimdebug) {
		ast_verb_chan(NULL, 3, "Asked to indicate '%s' (%d) condition on channel %s\n",
	...
	case -1:
		...
		break;
        case AST_CONTROL_CONNECTED_LINE:
		ast_log_chan(NULL, LOG_NOTICE, "Connected party is now %s <%s>\n",
	...
	case AST_CONTROL_SRCCHANGE:
		...
		break;
	default:
		ast_log_chan(NULL, LOG_WARNING, "Don't know how to indicate condition %d\n", ind);
		/* fallthrough */
...
static struct unistim_subchannel *find_subchannel_by_name(const char *dest)
{
	...
	at = strchr(line, '@');
	if (!at) {
		ast_log_chan(NULL, LOG_NOTICE, "Device '%s' has no @ (at) sign!\n", dest);
		return NULL;
	}
	*at = '\0';
	at++;
	...
	while (d) {
		if (!strcasecmp(d->name, device)) {
			if (unistimdebug) {
				ast_verb_chan(NULL, 0, "Found device: %s\n", d->name);
			}
			/* Found the device */
			AST_LIST_LOCK(&d->lines);
			AST_LIST_TRAVERSE(&d->lines, l, list) {
				...
				if (!strcasecmp(l->name, line)) {
					if (unistimdebug) {
						ast_verb_chan(NULL, 0, "Found line: %s\n", l->name);
					}
					sub = get_sub(d, SUB_REAL);
					if (!sub) {
					...
					if (sub->owner) {
						/* Allocate additional channel if gabpbx channel already here */
						sub = unistim_alloc_sub(d, SUB_REAL);
						sub->holding = 1;
					}
					sub->ringvolume = -1;
					...
					if (at) {       /* Other options ? */
						...
						if (*at == 'r') {       /* distinctive ring */
							...
							if ((*at < '0') || (*at > '7')) { /* ring style */
								ast_log_chan(NULL, LOG_WARNING, "Invalid ring selection (%s)", at);
							} else {
								...
								if (unistimdebug) {
									ast_verb_chan(NULL, 0, "Distinctive ring: style #%d volume %d\n",
...
static int unistim_senddigit_end(struct ast_channel *ast, char digit, unsigned int duration)
{
	...
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "Send Digit off %c (duration %d)\n", digit, duration);
	}
	send_tone(pte, 0, 0);
	return 0;
}
...
static int unistim_sendtext(struct ast_channel *ast, const char *text)
{
	...
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "unistim_sendtext called\n");
	}
	if (!text) {
		ast_log_chan(NULL, LOG_WARNING, "unistim_sendtext called with a null text\n");
		return -1;
	}
	...
	if (text[0] == '@') {
		...
		while (text[i]) {
			...
			case 1:
				if (cur != '@') {
					ast_log_chan(NULL, LOG_WARNING, "sendtext failed : invalid position\n");
					return 1;
				}
				tok = 2;
				continue;
		...
		if (tok != 6) {
			ast_log_chan(NULL, LOG_WARNING, "sendtext failed : incomplet command\n");
			return 1;
		}
		if (!pte->device) {
			ast_log_chan(NULL, LOG_WARNING, "sendtext failed : no device ?\n");
			return 1;
		}
...
static struct ast_channel *unistim_new(struct unistim_subchannel *sub, int state, const struct ast_assigned_ids *assignedids, const struct ast_channel *requestor)
{
	...
	if (!sub) {
		ast_log_chan(NULL, LOG_WARNING, "subchannel null in unistim_new\n");
		return NULL;
	}
	if (!sub->parent) {
		ast_log_chan(NULL, LOG_WARNING, "no line for subchannel %p\n", sub);
		return NULL;
	}
	...
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "unistim_new sub=%u (%p) chan=%p line=%s\n", sub->subtype, sub, tmp, l->name);
	}
	if (!tmp) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to allocate channel structure\n");
		ao2_ref(caps, -1);
		return NULL;
	}
	...
	if (unistimdebug) {
		...
		ast_verb_chan(NULL, 0, "Best codec = %s from nativeformats %s (line cap=%s global=%s)\n",
			ast_format_get_name(tmpfmt),
	...
	if ((sub->rtp) && (sub->subtype == 0)) {
		if (unistimdebug) {
			ast_verb_chan(NULL, 0, "New unistim channel with a previous rtp handle ?\n");
		}
...
/*      tmp->type = type; */
	...
	if (state != AST_STATE_DOWN) {
		if (unistimdebug) {
			ast_verb_chan(NULL, 0, "Starting pbx in unistim_new\n");
		}
...
static void *do_monitor(void *data)
{
	...
	for (;;) {
		...
		if (reloading) {
			ast_verb_chan(NULL, 1, "Reloading unistim.conf...\n");
			reload_config();
		}
		pthread_testcancel();
	}
...
static int restart_monitor(void)
{
	...
	if (ast_mutex_lock(&monlock)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to lock monitor\n");
		return -1;
	}
										   ...
										   int *cause)
{
	...
	if (ast_strlen_zero(tmp)) {
		ast_log_chan(NULL, LOG_NOTICE, "Unistim channels require a device\n");
		return NULL;
	}
	sub = find_subchannel_by_name(tmp);
	if (!sub) {
		ast_log_chan(NULL, LOG_NOTICE, "No available lines on: %s\n", dest);
	...
	if (sub_ring || sub_trans) {
		if (unistimdebug) {
			ast_verb_chan(NULL, 0, "Can't create channel, request already in progress: Busy!\n");
		}
		unistim_unalloc_sub(d, sub);
	...
	if (d->session->state == STATE_DIALPAGE) {
		if (unistimdebug) {
			ast_verb_chan(NULL, 0, "Can't create channel, user on dialpage: Busy!\n");
		}
		unistim_unalloc_sub(d, sub);
		*cause = AST_CAUSE_BUSY;
		return NULL;
		...
		if (unistimdebug) {
			ast_verb_chan(NULL, 0, "Can't create channel for line %s, all lines busy\n", sub->parent->name);
		}
		unistim_unalloc_sub(d, sub);
		*cause = AST_CAUSE_BUSY;
		return NULL;
	...
	if (!tmpc) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to make channel for '%s'\n", tmp);
	}
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "unistim_request owner = %p\n", sub->owner);
	}
	restart_monitor();
	/* and finish */
	return tmpc;
 ...
 * Runs in the gabpbx main thread, so don't do anything useful
...
static int parse_bookmark(const char *text, struct unistim_device *d)
{
	...
	if ((len > 2) && (line[1] == '@')) {
		...
		if (d->softkeyicon[p] != 0) {
			ast_log_chan(NULL, LOG_WARNING, "Invalid position %d for bookmark : already used:\n", p);
			return 0;
		}
		...
		if (p == FAVNUM) {
			ast_log_chan(NULL, LOG_WARNING, "No more free bookmark position\n");
			return 0;
		}
	}
	at = strchr(line, '@');
	if (!at) {
		ast_log_chan(NULL, LOG_NOTICE, "Bookmark entry '%s' has no @ (at) sign!\n", text);
		return 0;
	}
	*at = '\0';
	at++;
	...
	if (ast_strlen_zero(number)) {
		ast_log_chan(NULL, LOG_NOTICE, "Bookmark entry '%s' has no number\n", text);
		return 0;
	}
	if (ast_strlen_zero(line)) {
		ast_log_chan(NULL, LOG_NOTICE, "Bookmark entry '%s' has no description\n", text);
		return 0;
	}
	...
	if (!at) {
		...
		*at = '\0';
		...
		if (ast_strlen_zero(icon)) {
			ast_log_chan(NULL, LOG_NOTICE, "Bookmark entry '%s' has no icon value\n", text);
			return 0;
		}
	...
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "New bookmark at pos %d label='%s' number='%s' icon=%#x\n",
	...
	while (d) {
		for (i = 0; i < 6; i++) {
			if (d->softkeyicon[i] == 1) {   /* Something for us */
				...
				if (d->sp[i] == NULL) {
					ast_log_chan(NULL, LOG_NOTICE, "Bookmark entry with device %s not found\n",
							d->softkeydevice[i]);
				}
			}
		}
...
static void delete_device(struct unistim_device *d)
{
	...
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "Removing device '%s'\n", d->name);
	}
	AST_LIST_LOCK(&d->subs);
...
static struct unistim_device *build_device(const char *cat, const struct ast_variable *v)
{
	...
	while (d) {
		if (!strcmp(d->name, cat)) {
			...
			if (unistimsock < 0) {
				/* It's a dupe */
				ast_log_chan(NULL, LOG_WARNING, "Duplicate entry found (%s), ignoring.\n", cat);
				ast_mutex_unlock(&devicelock);
				return NULL;
			}
	...
	while (v) {
		if (!strcasecmp(v->name, "rtp_port")) {
			...
			if (acl_error) {
				ast_log_chan(NULL, LOG_ERROR, "Invalid ACL '%s' specified for device '%s' on line %d. Deleting device\n",
			...
			if ((d->contrast < 0) || (d->contrast > 15)) {
				ast_log_chan(NULL, LOG_WARNING, "contrast must be between 0 and 15\n");
				d->contrast = 8;
			}
			...
			if (!strcasecmp(v->value, "none")) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Unknown extension option.\n");
			}
			...
			if (create_line) {
				...
				if (!ast_strlen_zero(l->mailbox)) {
					if (unistimdebug) {
						ast_verb_chan(NULL, 3, "Setting mailbox '%s' on %s@%s\n", l->mailbox, d->name, l->name);
					}
				}
			...
			ast_log_chan(NULL, LOG_WARNING, "Don't know keyword '%s' at line %d\n", v->name,
					v->lineno);
		v = v->next;
	}
	ast_free(lt);
	if (linecnt == 0) {
		ast_log_chan(NULL, LOG_ERROR, "An Unistim device must have at least one line!\n");
		ast_free(d);
		return NULL;
	}
	...
	if ((d->tz == NULL) && !ast_strlen_zero(d->country)) {
		ast_log_chan(NULL, LOG_WARNING, "Country '%s' was not found in indications.conf\n",
				d->country);
	}
	...
	if ((autoprovisioning == AUTOPROVISIONING_TN) &&
		...
		if (strcmp(d->name, "template")) {
			ast_log_chan(NULL, LOG_ERROR, "You must specify the mac address with device=\n");
			if (d->tz) {
				d->tz = ast_tone_zone_unref(d->tz);
			}
			ast_free(d);
	...
	if (ast_strlen_zero(d->titledefault)) {
		...
		if ((ast_localtime(&cur_time, &tm, 0)) == 0 || ast_strlen_zero(tm.tm_zone)) {
			ast_log_chan(NULL, LOG_WARNING, "Error in ast_localtime()\n");
	...
	if (create) {
		...
		devices = d;
		ast_mutex_unlock(&devicelock);
		ast_verb_chan(NULL, 3, "Added device '%s'\n", d->name);
	} else {
		ast_verb_chan(NULL, 3, "Device '%s' reloaded\n", d->name);
	}
	return d;
}
...
static int reload_config(void)
{
		...
		if (!strcasecmp(v->name, "keepalive")) {
			...
			unistim_port = atoi(v->value);
                        ...
                        if (ast_str2tos(v->value, &qos.tos_audio)) {
                            ast_log_chan(NULL, LOG_WARNING, "Invalid tos_audio value at line %d, refer to QoS documentation\n", v->lineno);
			}
                        ...
                        if (ast_str2cos(v->value, &qos.cos_audio)) {
                            ast_log_chan(NULL, LOG_WARNING, "Invalid cos_audio value at line %d, refer to QoS documentation\n", v->lineno);
			}
	...
	if ((unistim_keepalive < 10) ||
		...
		ast_log_chan(NULL, LOG_ERROR, "keepalive is invalid in %s\n", config);
	...
	if ((unistim_port < 1) || (unistim_port > 65535)) {
		ast_log_chan(NULL, LOG_ERROR, "port is not set or invalid in %s\n", config);
		ast_config_destroy(cfg);
		return -1;
	}
	...
	if (unistimsock < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to create UNISTIM socket: %s\n", strerror(errno));
		return -1;
	}
#ifdef HAVE_PKTINFO
	{
	...
	if (bind(unistimsock, (struct sockaddr *) &bindaddr, sizeof(bindaddr)) < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to bind to %s:%d: %s\n",
				ast_inet_ntoa(bindaddr.sin_addr), htons(bindaddr.sin_port),
				strerror(errno));
		close(unistimsock);
		unistimsock = -1;
	} else {
		ast_verb_chan(NULL, 2, "UNISTIM Listening on %s:%d\n", ast_inet_ntoa(bindaddr.sin_addr), htons(bindaddr.sin_port));
...
static int unistim_set_rtp_peer(struct ast_channel *chan, struct ast_rtp_instance *rtp, struct ast_rtp_instance *vrtp, struct ast_rtp_instance *trtp, const struct ast_format_cap *codecs, int nat_active)
{
	...
	if (!sub) {
		ast_log_chan(NULL, LOG_ERROR, "No Private Structure, this is bad\n");
		return -1;
	}
	{
...
int load_module(void)
{
	...
	if (!io) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to allocate IO context\n");
		goto io_failed;
	}

	sched = ast_sched_context_create();
	if (!sched) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to allocate scheduler context\n");
		goto sched_failed;
	}
	...
	if (ast_channel_register(&unistim_tech)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to register channel type '%s'\n", channel_type);
		goto chanreg_failed;
	}
...
/*! reload: Part of GABpbx module interface ---*/
int reload(void)
{
	if (unistimdebug) {
		ast_verb_chan(NULL, 0, "reload unistim\n");
	}
	ast_mutex_lock(&unistim_reload_lock);
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_DEFAULT, "UNISTIM Protocol (USTM)",
