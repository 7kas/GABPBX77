...
#include "gabpbx.h"	/* ast_strdupa */
#include "gabpbx/utils.h"	/* ast_strdupa */
	...
	SDL_Surface *font, SDL_Rect *font_rects)
{
	...
	if (b->text == NULL) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to allocate board history memory.\n");
		ast_free(b);
		return NULL;
	}
	...
	if (b->blank == NULL) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to allocate board virtual screen: %s\n",
				SDL_GetError());
		ast_free(b->text);
		ast_free(b);
		return NULL;
	...
	if (0) ast_log_chan(NULL, LOG_WARNING, "Message board %dx%d@%d,%d successfully initialized\n",
