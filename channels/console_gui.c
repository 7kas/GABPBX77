...
#include "gabpbx/utils.h"	/* ast_calloc and ast_realloc */
#include <math.h>		/* sqrt */
...
{
	ast_log_chan(NULL, LOG_WARNING, "keypad_toggle(%i) called\n", index);
...
static void keypad_pick_up(struct video_desc *env)
{
	...
	ast_log_chan(NULL, LOG_WARNING, "keypad_pick_up called\n");
	...
	if (env->owner) { /* someone is calling us, just answer */
		...
		ast_log_chan(NULL, LOG_WARNING, "doing <%s>\n", buf);
...
static int switch_video_out(struct video_desc *env, int index, Uint8 button)
{
	...
	if (index >= env->out.device_num) {
		ast_log_chan(NULL, LOG_WARNING, "no devices\n");
		return 1;
	}
	...
	if (index == *p) {
		ast_log_chan(NULL, LOG_WARNING, "device %s already selected\n", env->out.devices[index].name);
		return 0;
	}
	ast_log_chan(NULL, LOG_WARNING, "switching to %s...\n", env->out.devices[index].name);
	/* already open */
	if (env->out.devices[index].grabber) {
		...
		*p = index;
		ast_log_chan(NULL, LOG_WARNING, "done\n");
	...
	ast_log_chan(NULL, LOG_WARNING, "device is down\n");
	return 1;
}
...
static void handle_mousedown(struct video_desc *env, SDL_MouseButtonEvent button)
{
...
#if 0
	ast_log_chan(NULL, LOG_WARNING, "event %d %d have %d/%d regions at %p\n",
	...
	if (index < 128) {	/* surely clicked on the keypad, don't care which key */
		...
		/* turn on or off the devices selectively with other mouse buttons */
			...
			if (!ret)
				ast_log_chan(NULL, LOG_WARNING, "unable to turn on device %s\n",
					env->out.devices[index].name);
			else if (ret == 1)
				ast_log_chan(NULL, LOG_WARNING, "device %s changed state to on\n",
					env->out.devices[index].name);
			else if (ret == 2)
				ast_log_chan(NULL, LOG_WARNING, "device %s changed state to off\n",
					env->out.devices[index].name);
			return;
		}
	}
	...
	*/
	...
	case KEY_OUT_OF_KEYPAD:
		ast_log_chan(NULL, LOG_WARNING, "nothing clicked, coordinates: %d, %d\n", button.x, button.y);
		break;
	...
	default:
		ast_log_chan(NULL, LOG_WARNING, "function not yet defined %i\n", index);
	}
}
...
#define MY_EV (SDL_MOUSEBUTTONDOWN|SDL_KEYDOWN)
	while ( (n = SDL_PeepEvents(ev, N_EVENTS, SDL_GETEVENT, SDL_ALLEVENTS)) > 0) {
		for (i = 0; i < n; i++) {
#if 0
			ast_log_chan(NULL, LOG_WARNING, "------ event %d at %d %d\n",
			...
			case SDL_ACTIVEEVENT:
				...
				if (ev[i].active.gain == 0 && ev[i].active.state & SDL_APPACTIVE) {
					ast_log_chan(NULL, LOG_WARNING, "/* somebody has killed us ? */\n");
			...
			case SDL_MOUSEBUTTONUP:
				if (drag->drag_window == DRAG_LOCAL && env->out.device_num) {
					...
					/* ast_log_chan(NULL, LOG_WARNING, "moving: %d, %d\n", dx, dy); */
					pip_move(env, dx, dy);
...
static struct gui_info *gui_init(const char *keypad_file, const char *font)
{
	...
	if (!ast_strlen_zero(font)) {
		...
		if (!gui->font) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to load font %s, no output available\n", font);
			goto error;
		}
		ast_log_chan(NULL, LOG_WARNING, "Loaded font %s\n", font);
...
#define FONT_W 9
	...
	if (gui->outfd < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Unable output fd\n");
		goto error;
	}
	return gui;
...
static void keypad_setup(struct gui_info *gui, const char *kp_file)
{
	...
	fd = fopen(kp_file, "r");
	if (fd == NULL) {
		ast_log_chan(NULL, LOG_WARNING, "fail to open %s\n", kp_file);
		return;
	}
	/*
...
{
	ast_log_chan(NULL, LOG_WARNING, "%s error_code %d\n", __FUNCTION__, e->error_code);
	return 0;
}
#endif /* HAVE_X11 */
...
static void sdl_setup(struct video_desc *env)
{
	...
	if (!ast_strlen_zero(e)) {
		...
		if (!success) {
			ast_log_chan(NULL, LOG_WARNING, "%s error in window\n", __FUNCTION__);
			return;
		}
	}
#endif
	...
	if (gui == NULL && SDL_Init(SDL_INIT_VIDEO)) {
 		ast_log_chan(NULL, LOG_WARNING, "Could not initialize SDL - %s\n",
                        SDL_GetError());
	 ...
	 */
	if (!info || !info->vfmt) {
 		ast_log_chan(NULL, LOG_WARNING, "Bad SDL_GetVideoInfo - %s\n",
                        SDL_GetError());
		return;
	}
	...
	if (!gui->screen) {
		ast_log_chan(NULL, LOG_ERROR, "SDL: could not set video mode - exiting\n");
		goto no_sdl;
	}
	...
	SDL_WM_SetCaption("GABpbx console Video Output", NULL);
...
#if 0
	ast_log_chan(NULL, LOG_WARNING, "result %d [%d] for match %d,%d in type %d p0 %d,%d p1 %d,%d h %d\n",
...
static int keypad_cfg_read(struct gui_info *gui, const char *val)
{
	...
	case 7:
		if (e.c == KEY_FONT) {	/* font - x0 y0 w h rows cols */
			ast_log_chan(NULL, LOG_WARNING, "font not supported yet\n");
			break;
		}
		/* token circle|rect x0 y0 x1 y1 h */
		if (e.x1 < e.x0 || e.h <= 0) {
			ast_log_chan(NULL, LOG_WARNING, "error in coordinates\n");
			e.type = 0;
			break;
		}
	...
	// ast_log_chan(NULL, LOG_WARNING, "reading [%s] returns %d %d\n", val, i, ret);
	if (ret == 0)
		return 0;
	if (gui->kp_size == 0) {
		gui->kp = ast_calloc(10, sizeof(e));
		if (gui->kp == NULL) {
			ast_log_chan(NULL, LOG_WARNING, "cannot allocate kp\n");
			return 0;
		}
		gui->kp_size = 10;
	}
	if (gui->kp_size == gui->kp_used) { /* must allocate */
		...
		if (a == NULL) {
			ast_log_chan(NULL, LOG_WARNING, "cannot reallocate kp\n");
			return 0;
		}
		gui->kp = a;
		gui->kp_size += 10;
	...
	// ast_log_chan(NULL, LOG_WARNING, "now %d regions\n", gui->kp_used);
	return 1;
}
#endif	/* HAVE_SDL */
