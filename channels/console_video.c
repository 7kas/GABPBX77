...
{
	ast_log_chan(NULL, LOG_NOTICE, "voice only, console video support not present\n");
}
...
used_mem(const char *msg)
{
	...
	ast_log_chan(NULL, LOG_WARNING, "used mem (vsize, rss) %s ", msg);
	system(in);
	return 0;
}
#endif
...
static struct video_codec_desc *map_config_video_format(char *name)
{
	...
	if (supported_codecs[i] == NULL) {
		ast_log_chan(NULL, LOG_WARNING, "Cannot find codec for '%s'\n", name);
		i = 0;
		strcpy(name, supported_codecs[i]->name);
	}
	ast_log_chan(NULL, LOG_WARNING, "Using codec '%s'\n", name);
...
static int video_out_init(struct video_desc *env)
{
	...
	if (!v->codec) {
		ast_log_chan(NULL, LOG_WARNING, "Cannot find the encoder for format %d\n",
			codec);
	...
	if (!enc_in->data) {
		ast_log_chan(NULL, LOG_WARNING, "Cannot allocate encoder input buffer\n");
		return video_out_uninit(env);
	}
	/* construct an AVFrame that points into buf_in */
	v->enc_in_frame = avcodec_alloc_frame();
	if (!v->enc_in_frame) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to allocate the encoding video frame\n");
		return video_out_uninit(env);
	}
	 ...
	 */
	{
		...
		if (avcodec_open(enc_ctx, v->codec) < 0) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to initialize the encoder %d\n", codec);
			av_free(enc_ctx);
			v->enc_ctx = NULL;
			return video_out_uninit(env);
		}
	...
	struct fbuf_t *out, AVPicture *p_out)
{
...
#else /* XXX replacement */
	{
		...
		if (convert_ctx == NULL) {
			ast_log_chan(NULL, LOG_ERROR, "FFMPEG::convert_cmodel : swscale context initialization failed\n");
			return;
		}
		if (0)
			ast_log_chan(NULL, LOG_WARNING, "in %d %dx%d out %d %dx%d\n",
...
/*
 * This function is called (by gabpbx) for each video packet
...
int console_write_video(struct ast_channel *chan, struct ast_frame *f)
{
	...
	if (v == NULL) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Cannot initialize input decoder\n");
		return 0;
	}
	...
	if ((random() % 10000) <= 100*DROP_PACKETS) {
		ast_log_chan(NULL, LOG_NOTICE, "Packet lost [%d]\n", f->seqno);
		return 0;
	}
#endif
	if (v->discard) {
		...
		if (f->subclass & 0x01) {
			...
			v->discard = 0;
			ast_log_chan(NULL, LOG_WARNING, "out of discard mode, frame %d\n", f->seqno);
		}
		return 0;
	}
	...
	if (v->next_seq != f->seqno) {
		ast_log_chan(NULL, LOG_WARNING, "discarding frame out of order, %d %d\n",
			v->next_seq, f->seqno);
		v->discard = 1;
		return 0;
	}
	...
	if (f->data.ptr == NULL || f->datalen < 2) {
		ast_log_chan(NULL, LOG_WARNING, "empty video frame, discard\n");
		return 0;
	}
	if (v->d_callbacks->dec_decap(v->dec_in_cur, f->data.ptr, f->datalen)) {
		ast_log_chan(NULL, LOG_WARNING, "error in dec_decap, enter discard\n");
		v->discard = 1;
	}
...
static struct ast_frame *get_video_frames(struct video_desc *env, struct ast_frame **tail)
{
	...
	*tail = NULL;
	...
	if (v->enc_out.data == NULL) {
		...
		if (a++ < 2)
			ast_log_chan(NULL, LOG_WARNING, "fail, no encoder output buffer\n");
		return NULL;
	}
	v->enc->enc_run(v);
...
static void *video_thread(void *arg)
{
	...
	if (grabber_open(&env->out)) {
		ast_log_chan(NULL, LOG_WARNING, "cannot open local video source\n");
	}
		...
		if (ast_channel_alertable(chan)) {
			for (p = f; p; p = AST_LIST_NEXT(p, frame_list)) {
				if (ast_channel_alert(chan)) {
					ast_log_chan(NULL, LOG_WARNING, "Unable to write to alert pipe on %s, frametype/subclass %d/%d: %s!\n",
...
void console_video_start(struct video_desc *env, struct ast_channel *owner)
{
	ast_log_chan(NULL, LOG_WARNING, "env %p chan %p\n", env, owner);
	...
	ast_log_chan(NULL, LOG_WARNING, "start video out %s %dx%d\n",
	...
	if (env->out.fps == 0) {
		env->out.fps = 15;
		ast_log_chan(NULL, LOG_WARNING, "fps unset, forcing to %d\n", env->out.fps);
	}
	if (env->out.bitrate == 0) {
		env->out.bitrate = 65000;
		ast_log_chan(NULL, LOG_WARNING, "bitrate unset, forcing to %d\n", env->out.bitrate);
	}
...
static int video_geom(struct fbuf_t *b, const char *s)
{
	...
	if (fp->s) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Invalid video_size %s, using 352x288\n", s);
		b->w = 352;
		b->h = 288;
	}
	return 0;
	...
	const char *var, const char *val)
{
	...
	if (penv == NULL) {
		ast_log_chan(NULL, LOG_WARNING, "bad argument penv=NULL\n");
		return 1;	/* error */
	}
	...
	if (env == NULL) {
		...
		if (env == NULL) {
			ast_log_chan(NULL, LOG_WARNING, "fail to allocate video_desc\n");
			return 1;	/* error */
