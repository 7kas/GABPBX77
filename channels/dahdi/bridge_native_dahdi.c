...
struct native_pvt_bridge {
	 ...
	 * span.  One leg comes into GABpbx, the other leg goes out of
	 * GABpbx, and GABpbx is natively bridging the two legs.
	 */
...
static int native_start(struct ast_bridge *bridge)
{
	...
	if (npc0->index == SUB_REAL && npc1->index == SUB_REAL) {
		if (p0->owner && p1->owner) {
			...
			if (!p0->subs[SUB_CALLWAIT].inthreeway && !p1->subs[SUB_REAL].inthreeway) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Huh?  Both calls are callwaits or 3-ways?  That's clever...?\n");
				ast_log_chan(NULL, LOG_WARNING, "p0: chan %d/%d/CW%d/3W%d, p1: chan %d/%d/CW%d/3W%d\n",
					p0->channel,
	...
	if (master && slave) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Can't link %d/%s with %d/%s\n",
...
static int native_bridge_write(struct ast_bridge *bridge, struct ast_bridge_channel *bridge_channel, struct ast_frame *frame)
{
	...
	case AST_FRAME_VIDEO:
		 ...
		 * also passes the frames up to GABpbx anyway.  Discard the
		 * media frames.
		 */
		return 0;
	default:
