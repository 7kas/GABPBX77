...
static int try_firmware(char *s)
{
	...
	if (stat(s, &stbuf) < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to stat '%s': %s\n", s, strerror(errno));
		return -1;
	}
	...
	ifd = open(s, O_RDONLY);
	if (ifd < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Cannot open '%s': %s\n", s, strerror(errno));
		return -1;
	}
	fd = open(s2, O_RDWR | O_CREAT | O_EXCL, AST_FILE_MODE);
	if (fd < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Cannot open '%s' for writing: %s\n", s2, strerror(errno));
		close(ifd);
		return -1;
	}
	...
	while(len) {
		...
		if (res != chunk) {
			ast_log_chan(NULL, LOG_WARNING, "Only read %d of %d bytes of data :(: %s\n", res, chunk, strerror(errno));
			close(ifd);
			close(fd);
			return -1;
		}
		...
		if (res != chunk) {
			ast_log_chan(NULL, LOG_WARNING, "Only write %d of %d bytes of data :(: %s\n", res, chunk, strerror(errno));
			close(ifd);
			close(fd);
			return -1;
		}
	...
	if ((res = read(fd, &fwh2, sizeof(fwh2))) != sizeof(fwh2)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to read firmware header in '%s'\n", s);
		close(fd);
		return -1;
	}
	if (ntohl(fwh2.magic) != IAX_FIRMWARE_MAGIC) {
		ast_log_chan(NULL, LOG_WARNING, "'%s' is not a valid firmware file\n", s);
		close(fd);
		return -1;
	}
	if (ntohl(fwh2.datalen) != (stbuf.st_size - sizeof(fwh2))) {
		ast_log_chan(NULL, LOG_WARNING, "Invalid data length in firmware '%s'\n", s);
		close(fd);
		return -1;
	}
	if (fwh2.devname[sizeof(fwh2.devname) - 1] || ast_strlen_zero((char *)fwh2.devname)) {
		ast_log_chan(NULL, LOG_WARNING, "No or invalid device type specified for '%s'\n", s);
		close(fd);
		return -1;
	}
	fwh = (struct ast_iax2_firmware_header*)mmap(NULL, stbuf.st_size, PROT_READ, MAP_PRIVATE, fd, 0);
	if (fwh == MAP_FAILED) {
		ast_log_chan(NULL, LOG_WARNING, "mmap failed: %s\n", strerror(errno));
		close(fd);
		return -1;
	}
	MD5Init(&md5);
	...
	if (memcmp(sum, fwh->chksum, sizeof(sum))) {
		ast_log_chan(NULL, LOG_WARNING, "Firmware file '%s' fails checksum\n", s);
	...
	if (fwd) {
		while((de = readdir(fwd))) {
			if (de->d_name[0] != '.') {
				...
				if (!try_firmware(fn)) {
					ast_verb_chan(NULL, 2, "Loaded firmware '%s'\n", de->d_name);
				}
			}
		}
		closedir(fwd);
	} else {
		ast_log_chan(NULL, LOG_WARNING, "Error opening firmware directory '%s': %s\n", fn, strerror(errno));
	}
