...
int iax_provision_version(unsigned int *version, const char *template, int force)
{
	...
	if (ast_db_get("iax/provisioning/cache", template, tmp, sizeof(tmp))) {
		ast_log_chan(NULL, LOG_ERROR, "ast_db_get failed to retrieve iax/provisioning/cache/%s\n", template);
	}
...
static int iax_template_parse(struct iax_template *cur, struct ast_config *cfg, const char *s, const char *def)
{
	...
	if (def) {
		...
		if (t && strlen(t)) {
			...
			if (!src)
				ast_log_chan(NULL, LOG_WARNING, "Unable to find base template '%s' for creating '%s'.  Trying '%s'\n", t, s, def);
			else
				def = t;
		}
		if (!src) {
			...
			if (!src)
				ast_log_chan(NULL, LOG_WARNING, "Unable to locate default base template '%s' for creating '%s', omitting.\n", def, s);
		}
		if (!src)
			return -1;
		ast_mutex_lock(&provlock);
	...
	while(v) {
		if (!strcasecmp(v->name, "port") || !strcasecmp(v->name, "serverport")) {
			if ((sscanf(v->value, "%5d", &x) == 1) && (x > 0) && (x < 65535)) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Ignoring invalid %s '%s' for '%s' at line %d\n", v->name, v->value, s, v->lineno);
			...
			if ((tmpfmt = ast_format_cache_get(v->value))) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Ignoring invalid codec '%s' for '%s' at line %d\n", v->value, s, v->lineno);
			...
			ast_log_chan(NULL, LOG_WARNING, "Unknown keyword '%s' in definition of '%s' at line %d\n", v->name, s, v->lineno);
		}
		v = v->next;
	}
	if (!foundportno)
	...
	if (cfg != NULL && cfg != CONFIG_STATUS_FILEUNCHANGED && cfg != CONFIG_STATUS_FILEINVALID) {
		...
		while(cat) {
			if (strcasecmp(cat, "general")) {
				...
				found++;
				ast_verb_chan(NULL, 3, "Loaded provisioning template '%s'\n", cat);
			}
		...
		ast_log_chan(NULL, LOG_NOTICE, "No IAX provisioning configuration found, IAX provisioning disabled.\n");
