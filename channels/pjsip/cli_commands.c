...
int pjsip_channel_cli_register(void)
{
	...
	if (!channel_formatter) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to allocate memory for channel_formatter\n");
		return -1;
	}
	...
	if (!channelstats_formatter) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Unable to allocate memory for channelstats_formatter\n");
		return -1;
	}
