...
static int channel_read_rtp(struct ast_channel *chan, const char *type, const char *field, char *buf, size_t buflen)
{
	...
	if (!channel) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Channel %s has no pvt!\n", ast_channel_name(chan));
		return -1;
	}

	session = channel->session;
	if (!session) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Channel %s has no session!\n", ast_channel_name(chan));
		return -1;
	}

	if (ast_strlen_zero(type)) {
		ast_log_chan(NULL, AST_LOG_WARNING, "You must supply a type field for 'rtp' information\n");
		return -1;
	}
	...
	if (ast_strlen_zero(field) || !strcmp(field, "audio")) {
		...
		ast_log_chan(NULL, AST_LOG_WARNING, "Unknown media type field '%s' for 'rtp' information\n", field);
		return -1;
	}

	if (!media || !media->rtp) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Channel %s has no %s media/RTP session\n",
	...
	if (!strcmp(type, "src")) {
		...
		ast_log_chan(NULL, AST_LOG_WARNING, "Unknown type field '%s' specified for 'rtp' information\n", type);
		return -1;
	}
...
static int channel_read_rtcp(struct ast_channel *chan, const char *type, const char *field, char *buf, size_t buflen)
{
	...
	if (ast_strlen_zero(type)) {
		ast_log_chan(NULL, AST_LOG_WARNING, "You must supply a type field for 'rtcp' information\n");
		return -1;
	}
	...
	if (ast_strlen_zero(field) || !strcmp(field, "audio")) {
		...
		ast_log_chan(NULL, AST_LOG_WARNING, "Unknown media type field '%s' for 'rtcp' information\n", field);
		return -1;
	}
	...
	if (!strncasecmp(type, "all", 3)) {
		...
		if (!ast_rtp_instance_get_quality(media->rtp, stat_field, buf, buflen)) {
			ast_log_chan(NULL, AST_LOG_WARNING, "Unable to retrieve 'rtcp' statistics for %s\n", ast_channel_name(chan));
			return -1;
		}
	} else {
		...
		ast_log_chan(NULL, AST_LOG_WARNING, "Unrecognized argument '%s' for 'rtcp' information\n", type);
		return -1;
	}
	...
	pjsip_uri_context_e context, const void *uri, char *buf, size_t size)
{
	...
	if (res < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Channel %s: Unescaped %s too long for %d byte buffer\n",
...
static int channel_read_pjsip(struct ast_channel *chan, const char *type, const char *field, char *buf, size_t buflen)
{
	...
	if (ast_strlen_zero(type)) {
		ast_log_chan(NULL, LOG_WARNING, "You must supply a type field for 'pjsip' information\n");
		return -1;
...
#else
		ast_log_chan(NULL, LOG_WARNING, "GABpbx has been built against a version of pjproject which does not have the required functionality to support the 'secure' argument. Please upgrade to version 2.3 or later.\n");
		return -1;
#endif
		...
		if (!datastore) {
			ast_log_chan(NULL, AST_LOG_WARNING, "No transport information for channel %s\n", ast_channel_name(chan));
			return -1;
		}
		...
		ast_log_chan(NULL, AST_LOG_WARNING, "Unrecognized argument '%s' for 'pjsip' information\n", type);
		return -1;
	}
...
static int read_pjsip(void *data)
{
	...
	if (!strcmp(func_args->param, "rtp")) {
		...
		if (!func_args->session->endpoint) {
			ast_log_chan(NULL, AST_LOG_WARNING, "Channel %s has no endpoint!\n", func_args->session->channel ?
...
int pjsip_acf_channel_read(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	if (ast_strlen_zero(parse)) {
		ast_log_chan(NULL, LOG_ERROR, "Cannot call %s without arguments\n", cmd);
		return -1;
	}
	...
	if (strcmp(ast_channel_tech(chan)->type, "PJSIP")) {
		ast_log_chan(NULL, LOG_WARNING, "Cannot call %s on a non-PJSIP channel\n", cmd);
	...
	if (!channel) {
		ast_log_chan(NULL, LOG_WARNING, "Channel %s has no pvt!\n", ast_channel_name(chan));
		ast_channel_unlock(chan);
		return -1;
	}

	if (!channel->session) {
		ast_log_chan(NULL, LOG_WARNING, "Channel %s has no session\n", ast_channel_name(chan));
		ast_channel_unlock(chan);
		return -1;
	}
	...
	if (ast_sip_push_task_wait_serializer(func_args.session->serializer, read_pjsip, &func_args)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to read properties of channel %s: failed to push task\n", ast_channel_name(chan));
...
int pjsip_acf_dial_contacts_read(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	if (ast_strlen_zero(args.endpoint_name)) {
		ast_log_chan(NULL, LOG_WARNING, "An endpoint name must be specified when using the '%s' dialplan function\n", cmd);
		return -1;
	} else if (!(endpoint = ast_sorcery_retrieve_by_id(ast_sip_get_sorcery(), "endpoint", args.endpoint_name))) {
		ast_log_chan(NULL, LOG_WARNING, "Specified endpoint '%s' was not found\n", args.endpoint_name);
		return -1;
	}
	...
	if (ast_strlen_zero(aor_name)) {
		ast_log_chan(NULL, LOG_WARNING, "No AOR has been provided and no AORs are configured on endpoint '%s'\n", args.endpoint_name);
		return -1;
	} else if (!(dial = ast_str_create(len))) {
		ast_log_chan(NULL, LOG_WARNING, "Could not get enough buffer space for dialing contacts\n");
		return -1;
	} else if (!(rest = ast_strdupa(aor_name))) {
		ast_log_chan(NULL, LOG_WARNING, "Could not duplicate provided AORs\n");
		return -1;
	}
...
static int parse_uri_cb(void *data)
{
	...
	if (!pool) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to allocate ParseUri endpoint pool.\n");
		args->ret = -1;
		return 0;
	}
	...
	if (!sip_uri) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to get an URI object for '%s'\n", args->uri);
		pjsip_endpt_release_pool(ast_sip_get_pjsip_endpoint(), pool);
		args->ret = -1;
		return 0;
	}
	...
	if (!strcmp(args->type, "user")) {
		...
		ast_log_chan(NULL, AST_LOG_WARNING, "Unknown type part '%s' specified\n", args->type);
		pjsip_endpt_release_pool(ast_sip_get_pjsip_endpoint(), pool);
		args->ret = -1;
		return 0;
	}
...
int pjsip_acf_parse_uri_read(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t buflen)
{
	...
	if (reading_uri_from_var) {
		...
		if (ast_strlen_zero(args.uri_str)) {
			ast_log_chan(NULL, LOG_WARNING, "The name of a variable containing a URI must be specified when using the '%s' dialplan function\n", cmd);
			return -1;
		}
	...
	if (ast_strlen_zero(args.uri_str)) {
		if (reading_uri_from_var) {
			ast_log_chan(NULL, LOG_WARNING, "The variable provided to the '%s' dialplan function must contain a URI\n", cmd);
		} else {
			ast_log_chan(NULL, LOG_WARNING, "A URI must be specified when using the '%s' dialplan function\n", cmd);
		}
		return -1;
	}

	if (ast_strlen_zero(args.type)) {
		ast_log_chan(NULL, LOG_WARNING, "A type part of the URI must be specified when using the '%s' dialplan function\n", cmd);
		return -1;
	}
	...
	if (ast_sip_push_task_wait_serializer(NULL, parse_uri_cb, &func_args)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to parse URI: failed to push task\n");
		return -1;
	}
...
int pjsip_acf_dtmf_mode_read(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	if (ast_sip_dtmf_to_str(channel->session->dtmf, buf, len) < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Unknown DTMF mode %d on PJSIP channel %s\n", channel->session->dtmf, ast_channel_name(chan));
		ast_channel_unlock(chan);
		return -1;
	}
...
int pjsip_acf_moh_passthrough_read(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	if (len < 3) {
		ast_log_chan(NULL, LOG_WARNING, "%s: buffer too small\n", cmd);
		return -1;
	}
...
int pjsip_acf_dtmf_mode_write(struct ast_channel *chan, const char *cmd, char *data, const char *value)
{
	...
	if (dtmf == -1) {
		ast_log_chan(NULL, LOG_WARNING, "Cannot set DTMF mode to '%s' on channel '%s' as value is invalid.\n", value,
			ast_channel_name(chan));
		ast_channel_unlock(chan);
		return -1;
	}
...
int pjsip_acf_session_refresh_write(struct ast_channel *chan, const char *cmd, char *data, const char *value)
{
	...
	if (ast_channel_state(chan) != AST_STATE_UP) {
		ast_log_chan(NULL, LOG_WARNING, "'%s' not allowed on unanswered channel '%s'.\n", cmd, ast_channel_name(chan));
		return -1;
	}
...
static int response_code_validator(const char *channel_name,
	...
	if (rc != 0) {
		...
		if (response_code < 0) {
			ast_log_chan(NULL, LOG_WARNING, "%s: Unrecognized response code parameter '%s'."
	...
	if (response_code < 400 || response_code > 699) {
		ast_log_chan(NULL, LOG_WARNING, "%s: Response code %d is out of range 400 -> 699."
			" Defaulting to 603 DECLINE\n",
...
static void pjsip_app_hangup_handler(struct ast_channel *chan, int response_code)
{
	...
	ast_channel_lock(chan);
	if (strcmp(ast_channel_tech(chan)->type, "PJSIP")) {
		ast_log_chan(NULL, LOG_WARNING, "%s: Not a PJSIP channel\n", tag);
		ast_channel_unlock(chan);
		return;
	}
	...
	if (hdata.session->inv_session->role != PJSIP_ROLE_UAS || (
		...
		ast_log_chan(NULL, LOG_WARNING, "%s: Not an incoming channel or invalid state '%s'\n",
	...
	if (ast_sip_push_task_wait_serializer(channel->session->serializer,
		...
		ast_log_chan(NULL, LOG_WARNING, "%s: failed to push hangup task to serializer\n", tag);
	}
...
int pjsip_app_hangup(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "%s: Missing response code parameter\n", tag);
		return -1;
	}
...
int pjsip_transfer_handling_write(struct ast_channel *chan, const char *cmd, char *data, const char *value)
{
	...
	ast_channel_lock(chan);
	if (strcmp(ast_channel_tech(chan)->type, "PJSIP")) {
		ast_log_chan(NULL, LOG_WARNING, "Cannot call %s on a non-PJSIP channel %s\n", cmd, ast_channel_name(chan));
		ast_channel_unlock(chan);
		return -1;
	}
	...
	if (ast_strlen_zero(value) || !strcmp(value, "core")) {
		...
		ast_log_chan(NULL, AST_LOG_WARNING, "Cannot set unknown transfer handling '%s' on channel '%s', transfer handling will remain unchanged.",
