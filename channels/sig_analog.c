	...
	*callwait = (p->owner != NULL);
	...
	if (p->owner) {
		if (analog_alloc_sub(p, ANALOG_SUB_CALLWAIT)) {
			ast_log_chan(NULL, LOG_ERROR, "Unable to alloc subchannel\n");
			return NULL;
		}
	}
...
int analog_call(struct analog_pvt *p, struct ast_channel *ast, const char *rdest, int timeout)
{
	...
	if ((ast_channel_state(ast) != AST_STATE_DOWN) && (ast_channel_state(ast) != AST_STATE_RESERVED)) {
		ast_log_chan(NULL, LOG_WARNING, "analog_call called on %s, neither down nor reserved\n", ast_channel_name(ast));
		return -1;
	}
	...
	case ANALOG_SIG_FXOKS:
		if (p->owner == ast) {
			...
			if (c && (strlen(c) > sizeof(p->dop.dialstr) - 3 /* "Tw\0" */)) {
				ast_log_chan(NULL, LOG_WARNING, "Number '%s' is longer than %d bytes\n", c, (int)sizeof(p->dop.dialstr) - 2);
				c = NULL;
			}
			if (c) {
			...
			if (analog_ring(p)) {
				ast_log_chan(NULL, LOG_WARNING, "Unable to ring phone: %s\n", strerror(errno));
				return -1;
			}
			analog_set_dialing(p, 1);
		} else {
			...
			if (analog_play_tone(p, ANALOG_SUB_CALLWAIT, ANALOG_TONE_RINGTONE)) {
				ast_log_chan(NULL, LOG_WARNING, "Unable to generate call-wait ring-back on channel %s\n", ast_channel_name(ast));
			}
...
#endif
			if (!ozz || !cic) {
				ast_log_chan(NULL, LOG_WARNING, "Unable to dial channel of type feature group D MF tandem access without CIC or OZZ set\n");
				return -1;
			}
...
int analog_hangup(struct analog_pvt *p, struct ast_channel *ast)
{
	...
	if (idx > -1) {
		...
		default:
			 ...
			 */
			ast_log_chan(NULL, LOG_ERROR, "Index found but not any type of call?\n");
			break;
		}
	}
	...
	ast_verb_chan(NULL, 3, "Hanging up on '%s'\n", ast_channel_name(ast));
	...
	case ANALOG_SIG_FGC_CAMA:
		...
		if ((p->sig == ANALOG_SIG_E911) || (p->sig == ANALOG_SIG_FGC_CAMAMF)) {
			if (exten[0] == '*') {
				...
				ast_log_chan(NULL, LOG_WARNING, "A KP was expected to start signaling for Feature Group C CAMA-MF, but we got something else. Received: %s on channel %d\n", exten, p->channel);
			}
		}
		if (p->sig == ANALOG_SIG_FEATB) {
			if (exten[0] == '*') {
				...
				ast_log_chan(NULL, LOG_WARNING, "A KP was expected to start signaling for Feature Group B, but we got something else. Received: %s on channel %d\n", exten, p->channel);
			}
		}
		if ((p->sig == ANALOG_SIG_FEATDMF) || (p->sig == ANALOG_SIG_FEATDMF_TA)) {
			analog_wink(p, idx);
		...
		if (ast_exists_extension(chan, ast_channel_context(chan), exten, 1,
			...
			ast_verb_chan(NULL, 3, "Unknown extension '%s' in context '%s' requested\n", exten, ast_channel_context(chan));
			sleep(2);
		...
		while (len < AST_MAX_EXTENSION-1) {
			...
			if (p->lastnumredial && !strcmp(exten, "##") && !ast_exists_extension(chan, ast_channel_context(chan), "##", 1, p->cid_num)) {
				...
				if (!ast_strlen_zero(p->lastexten)) {
					ast_verb_chan(NULL, 4, "Redialing last number dialed on channel %d\n", p->channel);
					ast_copy_string(exten, p->lastexten, sizeof(exten));
				} else {
					ast_verb_chan(NULL, 3, "Last Number Redial not possible on channel %d (no saved number)\n", p->channel);
					res = analog_play_tone(p, idx, ANALOG_TONE_CONGESTION);
					analog_wait_event(p);
					ast_hangup(chan);
					goto quit;
			 ...
			 * to prevent people from dialing *67 if that won't actually do anything. */
				...
				ast_verb_chan(NULL, 3, "Blocking Caller*ID on %s\n", ast_channel_name(chan));
				...
				ast_verb_chan(NULL, 3, "Blacklisting number %s\n", p->lastcid_num);
				...
				ast_verb_chan(NULL, 3, "Allowing Caller*ID on %s\n", ast_channel_name(chan));
...
static void analog_publish_channel_alarm_clear(int channel)
{
	...
	ast_log_chan(NULL, LOG_NOTICE, "Alarm cleared on channel %d\n", channel);
...
static struct ast_frame *__analog_handle_event(struct analog_pvt *p, struct ast_channel *ast)
{
	...
	if (idx != ANALOG_SUB_REAL) {
		ast_log_chan(NULL, LOG_ERROR, "We got an event on a non real sub.  Fix it!\n");
	}
	...
	switch (res) {
	case ANALOG_EVENT_EC_DISABLED:
		ast_verb_chan(NULL, 3, "Channel %d echo canceller disabled due to CED detection\n", p->channel);
		analog_set_echocanceller(p, 0);
		break;
#ifdef HAVE_DAHDI_ECHOCANCEL_FAX_MODE
	case ANALOG_EVENT_TX_CED_DETECTED:
	...
	case ANALOG_EVENT_RX_CED_DETECTED:
		...
		break;
	case ANALOG_EVENT_EC_NLP_DISABLED:
		ast_verb_chan(NULL, 3, "Channel %d echo canceller disabled its NLP.\n", p->channel);
		break;
	case ANALOG_EVENT_EC_NLP_ENABLED:
		ast_verb_chan(NULL, 3, "Channel %d echo canceller enabled its NLP.\n", p->channel);
		break;
#endif
	case ANALOG_EVENT_PULSE_START:
	...
	case ANALOG_EVENT_ONHOOK:
		if (p->calledsubscriberheld && (p->sig == ANALOG_SIG_FXOLS || p->sig == ANALOG_SIG_FXOGS || p->sig == ANALOG_SIG_FXOKS) && idx == ANALOG_SUB_REAL) {
			...
			if (p->subs[ANALOG_SUB_CALLWAIT].owner || p->subs[ANALOG_SUB_THREEWAY].owner) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Got ONHOOK but channel %d is off hook?\n", p->channel); /* Shouldn't happen */
			} else {
				ast_verb_chan(NULL, 3, "Holding incoming call %s for channel %d\n", ast_channel_name(ast), p->channel);
		...
		case ANALOG_SIG_FXOKS:
			...
			if (idx == ANALOG_SUB_REAL) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Got a hangup and my index is %u?\n", idx);
			}
			/* Fall through */
		default:
			analog_set_echocanceller(p, 0);
	...
	case ANALOG_EVENT_WINKFLASH:
		...
		case ANALOG_SIG_FXOKS:
			...
			if (idx != ANALOG_SUB_REAL) {
				ast_log_chan(NULL, LOG_WARNING, "Got flash hook with index %u on channel %d?!?\n", idx, p->channel);
				goto winkflashdone;
			}
			...
			if (p->subs[ANALOG_SUB_CALLWAIT].owner) {
				...
				if (!p->subs[ANALOG_SUB_CALLWAIT].owner) {
					 ...
					 */
					ast_log_chan(NULL, LOG_NOTICE, "Whoa, the call-waiting call disappeared.\n");
					goto winkflashdone;
				}
				...
				if (!p->subs[ANALOG_SUB_THREEWAY].owner) {
					 ...
					 * Let's just ignore this flash-hook.
					 */
					ast_log_chan(NULL, LOG_NOTICE, "Whoa, the 3-way call disappeared.\n");
					goto winkflashdone;
				}
...
winkflashdone:
		...
		case ANALOG_SIG_FEATDMF_TA:
			...
			case 1:
				...
				break;
			case 2:
				ast_log_chan(NULL, LOG_WARNING, "Received unexpected wink on channel of type ANALOG_SIG_FEATDMF_TA\n");
				return NULL;
			}
			p->whichwink++;
			/* Fall through */
	...
	if (!p->owner) {
		...
		if ((res != ANALOG_EVENT_RINGEROFF) && (res != ANALOG_EVENT_RINGERON) &&
			...
			if (p->owner && ast != p->owner) {
				 ...
				 */
				ast_log_chan(NULL, LOG_WARNING, "Event %s on %s is not restored owner %s\n",
		...
		case ANALOG_EVENT_ONHOOK:
			...
			if (p->owner) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Absorbed %s, but nobody is left!?!?\n",
...
void *analog_handle_init_event(struct analog_pvt *i, int event)
{
		...
		case ANALOG_SIG_FXSKS:
			if (i->immediate) {
				if (i->use_callerid || i->usedistinctiveringdetection) {
					ast_log_chan(NULL, LOG_WARNING, "Can't start PBX immediately, must wait for Caller ID / distinctive ring\n");
				} else {
	...
	case ANALOG_EVENT_POLARITY:
		...
		case ANALOG_SIG_FXSGS:
			...
			if (i->cid_start == ANALOG_CID_START_POLARITY || i->cid_start == ANALOG_CID_START_POLARITY_IN) {
				i->polarity = POLARITY_REV;
				ast_verb_chan(NULL, 2, "Starting post polarity CID detection on channel %d\n",
					i->channel);
				chan = analog_new_ast_channel(i, AST_STATE_PRERING, 0, ANALOG_SUB_REAL, NULL);
				i->ss_astchan = chan;
				if (!chan) {
	...
	case ANALOG_EVENT_DTMFCID:
		...
		case ANALOG_SIG_FXSGS:
			...
			if (i->cid_start == ANALOG_CID_START_DTMF_NOALERT) {
				ast_verb_chan(NULL, 2, "Starting DTMF CID detection on channel %d\n",
					i->channel);
				chan = analog_new_ast_channel(i, AST_STATE_PRERING, 0, ANALOG_SUB_REAL, NULL);
				i->ss_astchan = chan;
				if (!chan) {
	...
	case ANALOG_EVENT_REMOVED: /* destroy channel, will actually do so in do_monitor */
		ast_log_chan(NULL, LOG_NOTICE, "Got ANALOG_EVENT_REMOVED. Destroying channel %d\n",
			i->channel);
		return i->chan_pvt;
	case ANALOG_EVENT_NEONMWI_ACTIVE:
