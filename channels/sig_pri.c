...
struct sig_pri_cc_agent_prv {
	/*! GABpbx span D channel control structure. */
...
struct sig_pri_cc_monitor_instance {
	/*! \brief GABpbx span D channel control structure. */
	struct sig_pri_span *pri;
...
static const char *sig_pri_get_orig_dialstring(struct sig_pri_chan *p)
{
	...
	ast_log_chan(NULL, LOG_ERROR, "get_orig_dialstring callback not defined\n");
	return "";
}
...
static void sig_pri_make_cc_dialstring(struct sig_pri_chan *p, char *buf, size_t buf_size)
{
	if (sig_pri_callbacks.make_cc_dialstring) {
		...
		ast_log_chan(NULL, LOG_ERROR, "make_cc_dialstring callback not defined\n");
		buf[0] = '\0';
	}
}
#endif	/* defined(HAVE_PRI_CCSS) */
...
/*!
 * \internal
 * \brief Convert PRI redirecting reason to gabpbx version.
 * \since 1.8
 *
 * \param pri_reason PRI redirecting reason.
 *
 * \return Equivalent gabpbx redirecting reason value.
 */
...
/*!
 * \internal
 * \brief Convert gabpbx redirecting reason to PRI version.
 * \since 1.8
 *
 * \param ast_reason GABpbx redirecting reason.
 *
...
/*!
 * \internal
 * \brief Convert PRI number presentation to gabpbx version.
 * \since 1.8
 *
 * \param pri_presentation PRI number presentation.
 *
 * \return Equivalent gabpbx number presentation value.
 */
...
/*!
 * \internal
 * \brief Convert gabpbx number presentation to PRI version.
 * \since 1.8
 *
 * \param ast_presentation GABpbx number presentation.
 *
...
/*!
 * \internal
 * \brief Convert PRI name char_set to gabpbx version.
 * \since 1.8
 *
 * \param pri_char_set PRI name char_set.
 *
 * \return Equivalent gabpbx name char_set value.
 */
...
/*!
 * \internal
 * \brief Convert gabpbx name char_set to PRI version.
 * \since 1.8
 *
 * \param ast_char_set GABpbx name char_set.
 *
...
/*!
 * \internal
 * \brief Fill in the gabpbx party subaddress from the given PRI party subaddress.
 * \since 1.8
 *
 * \param ast_subaddress GABpbx party subaddress structure.
...
/*!
 * \internal
 * \brief Fill in the PRI party subaddress from the given gabpbx party subaddress.
 * \since 1.8
 *
 * \param pri_subaddress PRI party subaddress structure.
 * \param ast_subaddress GABpbx party subaddress structure.
...
/*!
 * \internal
 * \brief Fill in the PRI party name from the given gabpbx party name.
 * \since 1.8
 *
 * \param pri_name PRI party name structure.
 * \param ast_name GABpbx party name structure.
 *
...
/*!
 * \internal
 * \brief Fill in the PRI party number from the given gabpbx party number.
 * \since 1.8
 *
 * \param pri_number PRI party number structure.
 * \param ast_number GABpbx party number structure.
 *
...
/*!
 * \internal
 * \brief Fill in the PRI party id from the given gabpbx party id.
 * \since 1.8
 *
 * \param pri_id PRI party id structure.
 * \param ast_id GABpbx party id structure.
 *
...
static int pri_active_dchan_index(struct sig_pri_span *pri)
{
	...
	ast_log_chan(NULL, LOG_WARNING, "No active dchan found!\n");
	return -1;
}
		...
		if (newslot < 0) {
			...
			if (!pri->no_d_channels) {
				...
				if (old && oldslot != newslot) {
					...
					ast_log_chan(NULL, LOG_WARNING, "Span %d: No D-channels up!\n", pri->span);
				}
			}
		} else {
			pri->no_d_channels = 0;
		...
		if (newslot < 0) {
			...
			if (!pri->no_d_channels) {
				...
				if (pri->sig != SIG_BRI_PTMP) {
					ast_log_chan(NULL, LOG_WARNING, "Span %d: D-channel is down!\n", pri->span);
				}
			}
		} else {
			pri->no_d_channels = 0;
...
static int pri_fixup_principle(struct sig_pri_span *pri, int principle, q931_call *call)
{
	...
	for (x = 0; x < pri->numchans; x++) {
		...
		ast_verb_chan(NULL, 3, "Moving call (%s) from channel %d to %d.\n",
	...
	ast_verb_chan(NULL, 3, "Call specified, but not found.\n");
	return -1;
}
...
static int pri_find_fixup_principle(struct sig_pri_span *pri, int channel, q931_call *call)
{
	...
	if (chanpos < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Span %d: PRI requested channel %d/%d is unconfigured.\n",
	...
	if (chanpos < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Span %d: PRI requested channel %d/%d is not available.\n",
			pri->span, PRI_SPAN(channel), PRI_CHANNEL(channel));
		/*
...
static void *do_idle_thread(void *v_pvt)
{
	...
	ast_verb_chan(NULL, 3, "Initiating idle call on channel %s\n", ast_channel_name(chan));
	snprintf(ex, sizeof(ex), "%d/%s", pvt->channel, pvt->pri->idledial);
	if (ast_call(chan, ex, 0)) {
		ast_log_chan(NULL, LOG_WARNING, "Idle dial failed on '%s' to '%s'\n", ast_channel_name(chan), ex);
		ast_hangup(chan);
		return NULL;
	}
	start = ast_tvnow();
	while ((ms = ast_remaining_ms(start, timeout_ms))) {
		...
		if (f->frametype == AST_FRAME_CONTROL) {
			...
			case AST_CONTROL_ANSWER:
				...
				ast_verb_chan(NULL, 4, "Idle channel '%s' answered, sending to %s@%s\n", ast_channel_name(chan), ast_channel_exten(chan), ast_channel_context(chan));
				ast_pbx_run(chan);
				/* It's already hungup, return immediately */
				return NULL;
			case AST_CONTROL_BUSY:
				ast_verb_chan(NULL, 4, "Idle channel '%s' busy, waiting...\n", ast_channel_name(chan));
				break;
			case AST_CONTROL_CONGESTION:
				ast_verb_chan(NULL, 4, "Idle channel '%s' congested, waiting...\n", ast_channel_name(chan));
				break;
			};
		}
		ast_frfree(f);
...
static void *pri_ss_thread(void *data)
{
	...
	if (ast_strlen_zero(exten)) {
		ast_verb_chan(NULL, 3, "Going to extension s|1 because of empty extension received on overlap call\n");
		exten[0] = 's';
		exten[1] = '\0';
	} else {
	...
	if (ast_exists_extension(chan, ast_channel_context(chan), exten, 1, p->cid_num)) {
		...
		sig_pri_dsp_reset_and_flush_digits(p);
#if defined(JIRA_GABPBX_15594)
		/*
		 * Conditionaled out this code to effectively revert the JIRA
		 * GABPBX-15594 change.  It breaks overlap dialing through
		 * GABpbx.  There is not enough information available at this
...
#endif	/* defined(JIRA_GABPBX_15594) */
...
/*!
 * \internal
 * \brief Convert libpri party name into gabpbx party name.
 * \since 1.8
 *
 * \param ast_name GABpbx party name structure to fill.  Must already be set initialized.
...
/*!
 * \internal
 * \brief Convert libpri party number into gabpbx party number.
 * \since 1.8
 *
 * \param ast_number GABpbx party number structure to fill.  Must already be set initialized.
...
/*!
 * \internal
 * \brief Convert libpri party id into gabpbx party id.
 * \since 1.8
 *
 * \param ast_id GABpbx party id structure to fill.  Must already be set initialized.
...
/*!
 * \internal
 * \brief Convert libpri redirecting information into gabpbx redirecting information.
 * \since 1.8
 *
 * \param ast_redirecting GABpbx redirecting structure to fill.
 * \param pri_redirecting libpri redirecting structure containing source information.
 * \param ast_guide GABpbx redirecting structure to use as an initialization guide.
 * \param pri PRI span control structure.
 *
 ...
 * \param owner GABpbx channel associated with the call.
 * NULL if GABpbx no longer has the ast_channel struct.
 *
 * \note Assumes the pri->lock is already obtained.
 ...
 * \param device_name Name of device (GABpbx channel name less sequence number).
 *
 * \note
 * Since monitor_instances are refcounted, and this function returns
 * a reference to the instance, it is imperative that you decrement
	...
	for (index = 0; index < subcmds->counter_subcmd; ++index) {
		...
		case PRI_SUBCMD_CC_REQ_RSP:
			...
			case 0:/* success */
				...
				break;
			case 1:/* timeout */
				ast_verb_chan(NULL, 2, "core_id:%d %s CC request timeout\n", monitor->core_id,
					sig_pri_cc_type_name);
				ast_cc_monitor_failed(monitor->core_id, monitor->name,
					"%s CC request timeout", sig_pri_cc_type_name);
				break;
			case 2:/* error */
				ast_verb_chan(NULL, 2, "core_id:%d %s CC request error: %s\n", monitor->core_id,
					sig_pri_cc_type_name,
				...
				break;
			case 3:/* reject */
				ast_verb_chan(NULL, 2, "core_id:%d %s CC request reject: %s\n", monitor->core_id,
					sig_pri_cc_type_name,
				...
				break;
			default:
				ast_verb_chan(NULL, 2, "core_id:%d %s CC request unknown status %d\n",
	...
	for (index = 0; index < subcmds->counter_subcmd; ++index) {
		...
		case PRI_SUBCMD_REROUTING:
			...
			if (owner) {
				...
				ast_verb_chan(NULL, 3, "Span %d: %s is CallRerouting/CallDeflection to '%s'.\n",
					pri->span, ast_channel_name(owner), subcmd->u.rerouting.deflection.to.number.str);
...
static void sig_pri_moh_fsm_event(struct ast_channel *chan, struct sig_pri_chan *pvt, enum sig_pri_moh_event event)
{
	...
	if (orig_state < SIG_PRI_MOH_STATE_IDLE || SIG_PRI_MOH_STATE_NUM <= orig_state
		...
		ast_log_chan(NULL, LOG_ERROR, "MOH state not implemented: %s(%u)\n",
...
static int sig_pri_handle_hold(struct sig_pri_span *pri, pri_event *ev)
{
	...
	if (chanpos_old < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Span %d: Received HOLD for unknown call.\n", pri->span);
		return -1;
	}
...
static void sig_pri_handle_hold_rej(struct sig_pri_span *pri, pri_event *ev)
{
	...
	if (chanpos < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Span %d: Could not find principle for HOLD_REJECT\n",
			pri->span);
...
static void sig_pri_handle_retrieve(struct sig_pri_span *pri, pri_event *ev)
{
	...
	if (pri_find_principle_by_call(pri, ev->retrieve.call) < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Span %d: Received RETRIEVE for unknown call.\n", pri->span);
		pri_retrieve_rej(pri->pri, ev->retrieve.call,
			PRI_CAUSE_RESOURCE_UNAVAIL_UNSPECIFIED);
		return;
	}
...
static void sig_pri_handle_retrieve_rej(struct sig_pri_span *pri, pri_event *ev)
{
	...
	if (chanpos < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Span %d: Could not find principle for RETRIEVE_REJECT\n",
			pri->span);
...
static void sig_pri_handle_setup(struct sig_pri_span *pri, pri_event *e)
{
	...
	if (pri->append_msn_to_user_tag) {
		...
		if (len >= sizeof(pri->pvts[chanpos]->user_tag)) {
			ast_log_chan(NULL, LOG_WARNING, "user_tag '%s' truncated\n", pri->pvts[chanpos]->user_tag);
		}
	} else {
	...
	if (pri->pvts[chanpos]->immediate) {
		...
		ast_verb_chan(NULL, 3, "Going to extension s|1 because of immediate=yes\n");
	...
	if (e->ring.complete && (ast_strlen_zero(e->ring.callednum))) {
		ast_verb_chan(NULL, 3, "Going to extension s|1 because of Complete received\n");
		pri->pvts[chanpos]->exten[0] = 's';
		pri->pvts[chanpos]->exten[1] = '\0';
	}
	...
	if (c) {
		...
		if (could_match_more) {
			...
			if (!ast_pthread_create_detached(&threadid, NULL, pri_ss_thread,
				pri->pvts[chanpos])) {
				ast_verb_chan(NULL, 3, "Accepting overlap call from '%s' to '%s' on channel %d/%d, span %d\n",
			...
			if (!ast_pbx_start(c)) {
				ast_verb_chan(NULL, 3, "Accepting call from '%s' to '%s' on channel %d/%d, span %d\n",
	...
	ast_log_chan(NULL, LOG_WARNING, "Unable to start PBX on channel %d/%d, span %d\n",
		...
		if (cc) {
			*cc = '\0';
			...
			if (!ast_exists_extension(NULL, pri->idlecontext, pri->idleext, 1, NULL))
				ast_log_chan(NULL, LOG_WARNING, "Extension '%s @ %s' does not exist\n", pri->idleext, pri->idlecontext);
			else
#endif
				doidling = 1;
		} else
			ast_log_chan(NULL, LOG_WARNING, "Idle dial string '%s' lacks '@context'\n", pri->idleext);
	}
	for (;;) {
		ast_callid callid = 0;
				...
				if (ast_tvdiff_ms(ast_tvnow(), lastidle) > 1000) {
					...
					if (idle) {
						...
						if (ast_pthread_create_background(&p, NULL, do_idle_thread, pri->pvts[nextidle])) {
							ast_log_chan(NULL, LOG_WARNING, "Unable to start new thread for idle channel '%s'\n", ast_channel_name(idle));
							ast_mutex_unlock(&pri->lock);
							ast_hangup(idle);
							ast_mutex_lock(&pri->lock);
						}
						...
						ast_log_chan(NULL, LOG_WARNING, "Unable to request channel 'DAHDI/%s' for idle call\n", idlen);
					}
					gettimeofday(&lastidle, NULL);
				}
		...
		if (!res) {
			...
			for (which = 0; which < SIG_PRI_NUM_DCHANS; which++) {
				...
				if ((errno != 0) && (errno != EINTR)) {
					ast_log_chan(NULL, LOG_NOTICE, "pri_check_event returned error %d (%s)\n",
			...
			ast_log_chan(NULL, LOG_WARNING, "pri_event returned error %d (%s)\n", errno, strerror(errno));
		...
		if (e) {
			...
			if (e->e != PRI_EVENT_DCHAN_DOWN) {
				if (!(pri->dchanavail[which] & DCHAN_UP)) {
					ast_verb_chan(NULL, 2, "%s D-Channel on span %d up\n", pri_order(which), pri->span);
				}
				pri->dchanavail[which] |= DCHAN_UP;
			} else {
				if (pri->dchanavail[which] & DCHAN_UP) {
					ast_verb_chan(NULL, 2, "%s D-Channel on span %d down\n", pri_order(which), pri->span);
				}
				pri->dchanavail[which] &= ~DCHAN_UP;
			}
			...
			case PRI_EVENT_RESTART:
				if (e->restart.channel > -1 && PRI_CHANNEL(e->restart.channel) != 0xFF) {
					...
					if (chanpos < 0)
						...
						sig_pri_lock_private(pri->pvts[chanpos]);
						if (!skipit) {
							ast_verb_chan(NULL, 3, "Span %d: Channel %d/%d restarted\n", pri->span,
					...
					ast_verb_chan(NULL, 3, "Restart requested on entire span %d\n", pri->span);
					for (x = 0; x < pri->numchans; x++)
						if (pri->pvts[x]) {
			...
			case PRI_EVENT_SERVICE:
				...
				if (chanpos < 0) {
					ast_log_chan(NULL, LOG_WARNING, "Received service change status %d on unconfigured channel %d/%d span %d\n",
					...
					case 2: /* out-of-service */
						...
						*why |= SRVST_FAREND;
						...
						break;
					default:
						ast_log_chan(NULL, LOG_ERROR, "Huh?  changestatus is: %d\n", e->service.changestatus);
						break;
					}
					ast_log_chan(NULL, LOG_NOTICE, "Channel %d/%d span %d (logical: %d) received a change of service message, status '%d'\n",
			...
			case PRI_EVENT_SERVICE_ACK:
				...
				if (chanpos < 0) {
					ast_log_chan(NULL, LOG_WARNING, "Received service acknowledge change status '%d' on unconfigured channel %d/%d span %d\n",
						e->service_ack.changestatus, PRI_SPAN(e->service_ack.channel), PRI_CHANNEL(e->service_ack.channel), pri->span);
				} else {
			...
			case PRI_EVENT_PROGRESS:
				...
				if (e->proceeding.cause > -1) {
					...
					ast_verb_chan(NULL, 3, "PROGRESS with cause code %d received\n", e->proceeding.cause);

					/* Work around broken, out of spec USER_BUSY cause in a progress message */
					if (e->proceeding.cause == AST_CAUSE_USER_BUSY) {
						if (pri->pvts[chanpos]->owner) {
							ast_verb_chan(NULL, 3, "PROGRESS with 'user busy' received, signaling AST_CONTROL_BUSY instead of AST_CONTROL_PROGRESS\n");
			...
			case PRI_EVENT_FACILITY:
				...
				if (chanpos < 0) {
					ast_log_chan(NULL, LOG_WARNING, "Span %d: Received facility for unknown call.\n",
						pri->span);
					break;
				}
				sig_pri_lock_private(pri->pvts[chanpos]);
			...
			case PRI_EVENT_HANGUP:
				...
				if (!pri->pvts[chanpos]->alreadyhungup) {
					...
					ast_verb_chan(NULL, 3, "Span %d: Channel %d/%d got hangup, cause %d\n",
						pri->span, pri->pvts[chanpos]->logicalspan,
				...
				if (e->hangup.aoc_units > -1)
					ast_verb_chan(NULL, 3, "Channel %d/%d, span %d received AOC-E charging %d unit%s\n",
			...
			case PRI_EVENT_HANGUP_REQ:
				...
				if (pri->pvts[chanpos]->owner) {
					...
					ast_verb_chan(NULL, 3, "Span %d: Channel %d/%d got hangup request, cause %d\n",
						pri->span, pri->pvts[chanpos]->logicalspan,
						pri->pvts[chanpos]->prioffset, e->hangup.cause);
				} else {
					/*
			...
			case PRI_EVENT_HANGUP_ACK:
				...
				pri->pvts[chanpos]->call = NULL;
				if (pri->pvts[chanpos]->owner) {
					ast_verb_chan(NULL, 3, "Span %d: Channel %d/%d got hangup ACK\n", pri->span,
						pri->pvts[chanpos]->logicalspan, pri->pvts[chanpos]->prioffset);
				}
#ifdef SUPPORT_USERUSER
				if (!ast_strlen_zero(e->hangup.useruserinfo)) {
			...
			case PRI_EVENT_CONFIG_ERR:
				ast_log_chan(NULL, LOG_WARNING, "PRI Error on span %d: %s\n", pri->span, e->err.err);
				break;
			...
			case PRI_EVENT_NOTIFY:
				...
				if (chanpos < 0) {
					ast_log_chan(NULL, LOG_WARNING, "Span %d: Received NOTIFY for unknown call.\n",
						pri->span);
					break;
				}
#else
				...
				if (chanpos < 0) {
					ast_log_chan(NULL, LOG_WARNING, "Received NOTIFY on unconfigured channel %d/%d span %d\n",
...
int sig_pri_call(struct sig_pri_chan *p, struct ast_channel *ast, const char *rdest, int timeout, int layer1)
{
	...
	if (!p->pri) {
		ast_log_chan(NULL, LOG_ERROR, "Could not find pri on channel %d\n", p->channel);
		return -1;
	}

	if ((ast_channel_state(ast) != AST_STATE_DOWN) && (ast_channel_state(ast) != AST_STATE_RESERVED)) {
		ast_log_chan(NULL, LOG_WARNING, "sig_pri_call called on %s, neither down nor reserved\n", ast_channel_name(ast));
		return -1;
	}
	...
	if (!(p->call = pri_new_call(p->pri->pri))) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to create call on channel %d\n", p->channel);
		pri_rel(p->pri);
		return -1;
	}
	if (!(sr = pri_sr_new())) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to allocate setup request on channel %d\n",
			p->channel);
		pri_destroycall(p->pri->pri, p->call);
		p->call = NULL;
		pri_rel(p->pri);
	...
	while (c[p->stripmsd] > '9' && c[p->stripmsd] != '*' && c[p->stripmsd] != '#') {
		...
		default:
			if (isalpha(c[p->stripmsd])) {
				ast_log_chan(NULL, LOG_WARNING, "Unrecognized pridialplan %s modifier: %c\n",
	...
	if (ast_cc_is_recall(ast, &core_id, sig_pri_cc_type_name)) {
		...
		if (monitor) {
			...
			if (pri_cc_call(p->pri->pri, instance->cc_id, p->call, sr)) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Unable to setup CC recall call to device %s\n",
					device_name);
				ao2_ref(monitor, -1);
				pri_destroycall(p->pri->pri, p->call);
				p->call = NULL;
	...
	if (core_id == -1 && pri_setup(p->pri->pri, p->call, sr)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to setup call to %s (using %s)\n",
...
int sig_pri_start_pri(struct sig_pri_span *pri)
{
	...
	for (i = 0; i < ARRAY_LEN(pri->mbox); ++i) {
		...
		if (!pri->mbox[i].sub) {
			ast_log_chan(NULL, LOG_ERROR, "%s span %d could not subscribe to MWI events for %s(%s).\n",
				sig_pri_cc_type_name, pri->span, pri->mbox[i].vm_box, mbox_id);
		}
#if defined(HAVE_PRI_MWI_V2)
		if (ast_strlen_zero(pri->mbox[i].vm_number)) {
			ast_log_chan(NULL, LOG_WARNING, "%s span %d MWI voicemail number for %s(%s) is empty.\n",
				sig_pri_cc_type_name, pri->span, pri->mbox[i].vm_box, mbox_id);
		}
#endif	/* defined(HAVE_PRI_MWI_V2) */
	}
	...
	for (i = 0; i < SIG_PRI_NUM_DCHANS; i++) {
		...
		if (!pri->dchans[i]) {
			if (pri->fds[i] > 0)
				close(pri->fds[i]);
			pri->fds[i] = -1;
			ast_log_chan(NULL, LOG_ERROR, "Unable to create PRI structure\n");
			return -1;
		}
	...
	if (ast_pthread_create_background(&pri->master, NULL, pri_dchannel, pri)) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Unable to spawn D-channel: %s\n", strerror(errno));
		return -1;
	}
 ...
 * known case for this is if GABpbx requests CC over an ISDN
 * PTMP link as the TE side. If other phones are in the same
 * recall group as the GABpbx server, and one of those phones
 * picks up the recall notice, then GABpbx will receive a
 * "stop ringing" notification from the NT side of the PTMP
 * link. This indication needs to be passed to the phone
 * on the other side of the GABpbx server which originally
 ...
 * a phone places a call over ISDN PTMP to GABpbx, who then
 ...
 * and sched usage in GABpbx prevents such usage.
 *
 * \retval 0 on success
 * \retval -1 on failure.
 */
