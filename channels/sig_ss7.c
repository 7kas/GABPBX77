...
static int ss7_find_cic_gripe(struct sig_ss7_linkset *linkset, int cic, unsigned int dpc, const char *msg_name)
{
	...
	if (chanpos < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Linkset %d: SS7 %s requested on unconfigured CIC/DPC %d/%d.\n",
...
void sig_ss7_cb_call_null(struct ss7 *ss7, struct isup_call *call, int lock)
{
	...
	for (i = 0; i < winner->numchans; i++) {
		if (winner->pvts[i] && (winner->pvts[i]->ss7call == call)) {
			...
			ast_log_chan(NULL, LOG_WARNING, "libss7 asked set ss7 call to NULL on CIC %d DPC %d\n", winner->pvts[i]->cic, winner->pvts[i]->dpc);
		}
	}
}
...
static void ss7_handle_cqm(struct sig_ss7_linkset *linkset, ss7_event *e)
{
	...
	if (p) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Could not find any equipped circuits within CQM CICs\n");
	}
...
static void ss7_reset_linkset(struct sig_ss7_linkset *linkset)
{
	...
	for (i = 0; i < linkset->numchans; i++) {
		if (linkset->pvts[i+1]
			...
			ast_verb_chan(NULL, 1, "Resetting CICs %d to %d\n", startcic, endcic);

			sig_ss7_lock_private(p);
			if (!ss7_find_alloc_call(p)) {
				ast_log_chan(NULL, LOG_ERROR, "Unable to allocate new ss7call\n");
...
static void ss7_start_call(struct sig_ss7_chan *p, struct sig_ss7_linkset *linkset)
{
	...
	if (!c) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to start PBX on CIC %d\n", p->cic);
	...
	if (p->redirect_counter) {
		...
		if (p->redirect_info_counter) {
			...
			if (p->redirect_info_counter != p->redirect_counter) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Redirect counters differ: %u while info says %u - using %u\n",
	...
	if (ast_pbx_start(c)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to start PBX on %s (CIC %d)\n", ast_channel_name(c), p->cic);
		ast_hangup(c);
	} else {
		ast_verb_chan(NULL, 3, "Accepting call to '%s' on CIC %d\n", p->exten, p->cic);
	}
...
{
	ast_verb_chan(NULL, 3, "SS7 exten: %s complete: %d\n", p->exten, p->called_complete);
...
#define SS7_MAX_POLL	60000	/* Maximum poll time in ms. */
	...
	for (;;) {
		...
		if ((res < 0) && (errno != EINTR)) {
			ast_log_chan(NULL, LOG_ERROR, "poll(%s)\n", strerror(errno));
		} else if (!res) {
		...
		while ((e = ss7_check_event(ss7))) {
			...
			case SS7_EVENT_UP:
				if (linkset->state != LINKSET_STATE_UP) {
					ast_verb_chan(NULL, 1, "--- SS7 Up ---\n");
					ss7_reset_linkset(linkset);
				}
				linkset->state = LINKSET_STATE_UP;
				break;
			case SS7_EVENT_DOWN:
				ast_verb_chan(NULL, 1, "--- SS7 Down ---\n");
				...
				break;
			case MTP2_LINK_UP:
				ast_verb_chan(NULL, 1, "MTP2 link up (SLC %d)\n", e->gen.data);
				break;
			case MTP2_LINK_DOWN:
				ast_log_chan(NULL, LOG_WARNING, "MTP2 link down (SLC %d)\n", e->gen.data);
				break;
			case ISUP_EVENT_CPG:
				...
				break;
			case ISUP_EVENT_RSC:
				ast_verb_chan(NULL, 1, "Resetting CIC %d\n", e->rsc.cic);
			...
			case ISUP_EVENT_GRS:
				if (!sig_ss7_find_cic_range(linkset, e->grs.startcic, e->grs.endcic,
					e->grs.opc)) {
					ast_log_chan(NULL, LOG_WARNING, "GRS on unconfigured range CIC %d - %d PC %d\n",
			...
			case ISUP_EVENT_GRA:
				if (!sig_ss7_find_cic_range(linkset, e->gra.startcic,
					...
					ast_log_chan(NULL, LOG_WARNING, "GRA on unconfigured range CIC %d - %d PC %d\n",
							e->gra.startcic, e->gra.endcic, e->gra.opc);
					isup_free_call(ss7, e->gra.call);
					break;
				}
				ast_verb_chan(NULL, 1, "Got reset acknowledgement from CIC %d to %d DPC: %d\n",
					e->gra.startcic, e->gra.endcic, e->gra.opc);
			...
			case ISUP_EVENT_SAM:
				...
				sig_ss7_lock_owner(linkset, chanpos);
				if (p->owner) {
					ast_log_chan(NULL, LOG_WARNING, "SAM on CIC %d PC %d already have call\n", e->sam.cic, e->sam.opc);
					ast_channel_unlock(p->owner);
					sig_ss7_unlock_private(p);
					break;
				}
			...
			case ISUP_EVENT_IAM:
				...
				if (p->remotelyblocked) {
					ast_log_chan(NULL, LOG_NOTICE, "Got IAM on remotely blocked CIC %d DPC %d remove blocking\n", e->iam.cic, e->iam.opc);
					sig_ss7_set_remotelyblocked(p, 0, SS7_BLOCKED_MAINTENANCE | SS7_BLOCKED_HARDWARE);
					sig_ss7_set_inservice(p, 1);
				}
				...
				if (!sig_ss7_is_chan_available(p)) {
					...
					if (p->locallyblocked) {
						...
						ast_log_chan(NULL, LOG_WARNING, "Got IAM on locally blocked CIC %d DPC %d, ignore\n", e->iam.cic, e->iam.opc);
					}
					sig_ss7_unlock_private(p);
					break;
				}
				...
				if (e->iam.cot_performed_on_previous_cic) {
					...
					if (chanpos < 0) {
						...
						ast_verb_chan(NULL, 1, "COT request on previous nonexistent CIC %d in IAM PC %d\n", (e->iam.cic - 1), e->iam.opc);
						break;
					}
					ast_verb_chan(NULL, 1, "COT request on previous CIC %d in IAM PC %d\n", (e->iam.cic - 1), e->iam.opc);
					p = linkset->pvts[chanpos];
					sig_ss7_lock_private(p);
					if (sig_ss7_is_chan_available(p)) {
			...
			case ISUP_EVENT_COT:
				if (e->cot.cot_performed_on_previous_cic) {
					...
					if (-1 < chanpos) {
						...
						sig_ss7_loopback(p, 0);
						sig_ss7_unlock_private(p);;
						ast_verb_chan(NULL, 1, "Loop turned off on CIC: %d PC: %d\n",  (e->cot.cic - 1), e->cot.opc);
					}
				}
			...
			case ISUP_EVENT_REL:
				...
				if (p->owner) {
					...
					ast_verb_chan(NULL, 1, "REL on CIC %d DPC %d without owner!\n", p->cic, p->dpc);
...
int sig_ss7_add_sigchan(struct sig_ss7_linkset *linkset, int which, int ss7type, int transport, int inalarm, int networkindicator, int pointcode, int adjpointcode, int cur_slc)
{
	if (!linkset->ss7) {
		...
		if (!linkset->ss7) {
			ast_log_chan(NULL, LOG_ERROR, "Can't create new SS7!\n");
			return -1;
		}
	}
	...
	if (ss7_add_link(linkset->ss7, transport, linkset->fds[which], cur_slc, adjpointcode)) {
		ast_log_chan(NULL, LOG_WARNING, "Could not add SS7 link!\n");
	}
...
int sig_ss7_available(struct sig_ss7_chan *p)
{
	...
	if (available) {
		...
		if (!p->ss7call) {
			ast_log_chan(NULL, LOG_ERROR, "Unable to allocate new SS7 call!\n");
			available = 0;
		} else {
			p->call_level = SIG_SS7_CALL_LEVEL_ALLOCATED;
		}
 ...
 *
 * \param p Signaling private structure pointer.
 * \param ast GABpbx channel structure pointer.
 ...
 *
 * \param p Signaling private structure pointer.
 * \param chan GABpbx channel structure pointer.
 ...
 * \param requestor GABpbx channel requesting a channel to dial (Can be NULL)
