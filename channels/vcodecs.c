...
#include "gabpbx/utils.h"	/* ast_calloc() */
...
static uint32_t getbits(struct bitbuf *b, int n)
{
	...
	uint32_t retval = 0;
	if (n> 31) {
		ast_log_chan(NULL, LOG_WARNING, "too many bits %d, max 32\n", n);
		return 0;
	}
	if (n + b->ofs > b->bitsize) {
		ast_log_chan(NULL, LOG_WARNING, "bitbuf overflow %d of %d\n", n + b->ofs, b->bitsize);
		n = b->bitsize - b->ofs;
	}
...
static void check_h261(struct fbuf_t *b)
{
	...
	if (x != 0x10) {
		ast_log_chan(NULL, LOG_WARNING, "bad PSC 0x%x\n", x);
		return;
	}
	x = getbits(&a, 5);	/* temporal reference */
	y = getbits(&a, 6);	/* ptype */
	if (0)
	ast_log_chan(NULL, LOG_WARNING, "size %d TR %d PTY spl %d doc %d freeze %d %sCIF hi %d\n",
		b->used,
		x,
		(y & 0x20) ? 1 : 0,
		(y & 0x10) ? 1 : 0,
	...
	while ( (x = getbits(&a, 1)) == 1)
		ast_log_chan(NULL, LOG_WARNING, "PSPARE 0x%x\n", getbits(&a, 8));
	// ast_log_chan(NULL, LOG_WARNING, "PSPARE 0 - start GOB LAYER\n");
	while ( (x = bitbuf_left(&a)) > 0) {
		// ast_log_chan(NULL, LOG_WARNING, "GBSC %d bits left\n", x);
		x = getbits(&a, 16); /* GBSC 0000 0000 0000 0001 */
		if (x != 0x1) {
			ast_log_chan(NULL, LOG_WARNING, "bad GBSC 0x%x\n", x);
			break;
		}
		x = getbits(&a, 4);	/* group number */
		y = getbits(&a, 5);	/* gquant */
		if (x == 0) {
			ast_log_chan(NULL, LOG_WARNING, "  bad GN %d\n", x);
			break;
		}
		while ( (x = getbits(&a, 1)) == 1)
			ast_log_chan(NULL, LOG_WARNING, "GSPARE 0x%x\n", getbits(&a, 8));
...
void dump_buf(struct fbuf_t *b)
{
	...
	ast_log_chan(NULL, LOG_WARNING, "buf size %d of %d\n", b->used, b->size);
	for (i = 0; i < b->used; i++) {
		x = i & 0xf;
		if ( x == 0) {	/* new line */
			if (i != 0)
	               ...
	               int format, int head, struct ast_frame *prev)
{
	...
	if (f == NULL || data == NULL) {
		ast_log_chan(NULL, LOG_WARNING, "--- frame error f %p data %p len %d format %d\n",
				f, data, len, format);
		if (f)
			ast_free(f);
		if (data)
	...
	int sbit, int ebit)
{
	...
	if (b->data == NULL) {
		ast_log_chan(NULL, LOG_WARNING, "alloc failure for %d, discard\n",
			b->size);
		return 1;
	}
	if (b->used == 0 && b->ebit != 0) {
		ast_log_chan(NULL, LOG_WARNING, "ebit not reset at start\n");
		b->ebit = 0;
	}
	dst = b->data + b->used;
	...
	if (i == 0) {	/* easy case, just append */
		...
		ast_log_chan(NULL, LOG_WARNING, "must handle shift %d %d at %d\n",
			b->ebit, sbit, b->used);
		return 1;
	}
	memcpy(dst, src, len);
	...
	struct ast_frame **tail)
{
	...
	for (;len > 0; len -= l, d += l) {
		...
		if (l < 1 || l > mtu) {
			ast_log_chan(NULL, LOG_WARNING, "--- frame error l %d\n", l);
			break;
		}
...
static int h263p_decap(struct fbuf_t *b, uint8_t *data, int len)
{
	...
	if (len < 2) {
		ast_log_chan(NULL, LOG_WARNING, "invalid framesize %d\n", len);
		return 1;
	}
...
static int ffmpeg_encode(struct video_out_desc *v)
{
	...
	if (i > 0) {
		ast_log_chan(NULL, LOG_WARNING, "have %d more bytes\n", i);
		b->used += i;
	}
	return 0;
}
...
static int ffmpeg_decode(struct video_dec_desc *v, struct fbuf_t *b)
{
	...
	while (srclen) {
		...
		if (full_frame == 1)	/* full frame */
			break;
		if (ret < 0) {
			ast_log_chan(NULL, LOG_NOTICE, "Error decoding\n");
			break;
		}
	}
...
#define H263_MIN_LEN	6
	...
	for (i = H263_MIN_LEN, start = 0; start < len; start = i, i += 3) {
		//ast_log_chan(NULL, LOG_WARNING, "search at %d of %d/%d\n", i, start, len);
		for (; i < len ; i++) {
			uint8_t x, rpos, lpos;
			...
			if (rpos == 0x80) {	/* lucky case */
				...
				ast_log_chan(NULL, LOG_WARNING, "unaligned GBSC 0x%x %d\n",
					rpos, rpos_i);
			}
			break;
		}
...
static int h263_decap(struct fbuf_t *b, uint8_t *data, int len)
{
	...
	if ( (data[0] & 0x80) == 0) {
		...
		ast_log_chan(NULL, LOG_WARNING, "unsupported mode 0x%x\n",
			data[0]);
		return 1;
	}
...
#define HAVE_NAL(x) (x[-4] == 0 && x[-3] == 0 && x[-2] == 0 && x[-1] == 1)
   ...
   for (;start < end - 4; start = d) {
	...
	if (d >= end) {	/* NAL not found */
		...
		ast_log_chan(NULL, LOG_WARNING, "skip invalid nal type %d at %d of %d\n",
...
static int h264_decap(struct fbuf_t *b, uint8_t *data, int len)
{
	...
	if (len < 2) {
		ast_log_chan(NULL, LOG_WARNING, "--- invalid len %d\n", len);
		return 1;
	}
	/* first of all, check if the packet has F == 0 */
	if (data[0] & 0x80) {
		ast_log_chan(NULL, LOG_WARNING, "--- forbidden packet; nal: %02hhx\n",
			data[0]);
		return 1;
	}
	...
	case 0:
	case 31:
		ast_log_chan(NULL, LOG_WARNING, "--- invalid type: %d\n", type);
		return 1;
	case 24:
	case 25:
	case 26:
	case 27:
	case 29:
		ast_log_chan(NULL, LOG_WARNING, "--- encapsulation not supported : %d\n", type);
		return 1;
	case 28:	/* FU-A Unit */
	...
	if (retval)
		ast_log_chan(NULL, LOG_WARNING, "result %d\n", retval);
	return retval;
}
...
/*
 * Table of translation between gabpbx and ffmpeg formats.
 ...
 */
struct _cm {    /* map ffmpeg codec types to gabpbx formats */
...
/*! \brief map an gabpbx format into an ffmpeg one */
...
static struct video_codec_desc *map_video_codec(int fmt)
{
	...
	for (i = 0; supported_codecs[i]; i++)
		if (fmt == supported_codecs[i]->format) {
			ast_log_chan(NULL, LOG_WARNING, "using %s for format 0x%x\n",
...
static struct video_dec_desc *dec_init(uint32_t the_ast_format)
{
	...
	if (v->d_callbacks == NULL) {
		ast_log_chan(NULL, LOG_WARNING, "cannot find video codec, drop input 0x%x\n", the_ast_format);
		return dec_uninit(v);
	}
	...
	if (!v->codec) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to find the decoder for format %d\n", codec);
		return dec_uninit(v);
	}
	/*
	...
	if (!v->dec_ctx) {
		ast_log_chan(NULL, LOG_WARNING, "Cannot allocate the decoder context\n");
		return dec_uninit(v);
	}
	/* XXX call dec_init() ? */
	if (avcodec_open(v->dec_ctx, v->codec) < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Cannot open the decoder context\n");
		av_free(v->dec_ctx);
		v->dec_ctx = NULL;
		return dec_uninit(v);
	}
	...
	if (!v->parser) {
		ast_log_chan(NULL, LOG_WARNING, "Cannot initialize the decoder parser\n");
		return dec_uninit(v);
	}

	v->d_frame = avcodec_alloc_frame();
	if (!v->d_frame) {
		ast_log_chan(NULL, LOG_WARNING, "Cannot allocate decoding video frame\n");
		return dec_uninit(v);
	}
