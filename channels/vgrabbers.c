...
static void *grab_x11_open(const char *name, struct fbuf_t *geom, int fps)
{
	...
	if (v->dpy == NULL) {
		ast_log_chan(NULL, LOG_WARNING, "error opening display\n");
		goto error;
	}
	...
	if (v->image == NULL) {
		ast_log_chan(NULL, LOG_WARNING, "error creating Ximage\n");
		goto error;
	}
	...
	ast_log_chan(NULL, LOG_NOTICE, "image: data %p %d bpp fmt %d, mask 0x%lx 0x%lx 0x%lx\n",
		im->data,
		im->bits_per_pixel,
		b->pix_fmt,
...
static void *grab_v4l1_open(const char *dev, struct fbuf_t *geom, int fps)
{
	...
	if (fd < 0) {
		ast_log_chan(NULL, LOG_WARNING, "error opening camera %s\n", dev);
		return NULL;
	}

	v = ast_calloc(1, sizeof(*v));
	if (v == NULL) {
		ast_log_chan(NULL, LOG_WARNING, "no memory for camera %s\n", dev);
		close(fd);
		return NULL;	/* no memory */
	}
	v->fd = fd;
	...
	if (ioctl(fd, VIDIOCSWIN, &vw) == -1) {
		ast_log_chan(NULL, LOG_WARNING, "error setting format for %s [%s]\n",
			dev, strerror(errno));
		goto error;
	}
	if (ioctl(fd, VIDIOCGPICT, &vp) == -1) {
		ast_log_chan(NULL, LOG_WARNING, "error reading picture info\n");
		goto error;
	}
	...
	if (ioctl(v->fd, VIDIOCSPICT, &vp) == -1) {
		ast_log_chan(NULL, LOG_WARNING, "error setting palette, using %d\n",
			b->pix_fmt);
	} else
		b->pix_fmt = vp.palette;
	...
	ast_log_chan(NULL, LOG_WARNING, "videodev %s opened, size %dx%d %d\n",
		dev, b->w, b->h, b->size);
	b->data = ast_calloc(1, b->size);
	if (!b->data) {
		ast_log_chan(NULL, LOG_WARNING, "error allocating buffer %d bytes\n",
			b->size);
		goto error;
	}
	ast_log_chan(NULL, LOG_WARNING, "success opening camera\n");
	return v;
...
static struct fbuf_t *grab_v4l1_read(void *desc)
{
	...
	for (;;) {
		...
		// ast_log_chan(NULL, LOG_WARNING, "read %d of %d bytes from webcam\n", r, l);
		if (r < 0)	/* read error */
			break;
		if (r == 0)	/* no data */
			break;
