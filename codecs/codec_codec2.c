...
#include "gabpbx/frame.h"             /* for ast_frame                  */
#include "gabpbx/linkedlists.h"       /* for AST_LIST_NEXT, etc         */
#include "gabpbx/logger.h"            /* for ast_log, etc               */
#include "gabpbx/module.h"
#include "gabpbx/rtp_engine.h"        /* ast_rtp_engine_(un)load_format */
...
static int codec2_new(struct ast_trans_pvt *pvt)
{
	...
	if (!tmp->state) {
		ast_log_chan(NULL, LOG_ERROR, "Error creating Codec 2 conversion\n");
		return -1;
	}
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Codec 2 Coder/Decoder");
