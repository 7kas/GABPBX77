...
/*!
 * \internal
 * \brief Convert DAHDI format bitfield to old GABpbx format bitfield.
 * \since 13.0.0
 *
 ...
 *
 * \return Old GABpbx bitfield equivalent.
 */
...
static int lintoulaw(struct ast_trans_pvt *pvt, struct ast_frame *f)
{
	...
	if (dahdip->samples_in_buffer + i > sizeof(dahdip->ulaw_buffer)) {
		ast_log_chan(NULL, LOG_ERROR, "Out of buffer space!\n");
		return -i;
	}
...
static void dahdi_write_frame(struct codec_dahdi_pvt *dahdip, const uint8_t *buffer, const ssize_t count)
{
	...
	if (-1 == res) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to write to transcoder: %s\n", strerror(errno));
	}
	if (count != res) {
		ast_log_chan(NULL, LOG_ERROR, "Requested write of %zd bytes, but only wrote %d bytes.\n", count, res);
	}
}
...
static struct ast_frame *dahdi_encoder_frameout(struct ast_trans_pvt *pvt)
{
	...
	if (-1 == res) {
		if (EWOULDBLOCK == errno) {
			...
			ast_log_chan(NULL, LOG_ERROR, "Failed to read from transcoder: %s\n", strerror(errno));
			return NULL;
		}
	} else {
		pvt->f.datalen = res;
...
retry:
	if (ioctl(fd, DAHDI_TC_ALLOCATE, &dahdip->fmts)) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Unable to attach to transcoder: %s\n", strerror(errno));
		close(fd);
...
static int find_transcoders(void)
{
	...
	if ((fd = open("/dev/dahdi/transcode", O_RDWR)) < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to open /dev/dahdi/transcode: %s\n", strerror(errno));
		return 0;
	}

	for (info.tcnum = 0; !ioctl(fd, DAHDI_TC_GETINFO, &info); info.tcnum++) {
		ast_verb_chan(NULL, 2, "Found transcoder '%s'.\n", info.name);
	...
	if (!info.tcnum) {
		ast_verb_chan(NULL, 2, "No hardware transcoders found.\n");
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_DEFAULT, "Generic DAHDI Transcoder Codec Translator",
