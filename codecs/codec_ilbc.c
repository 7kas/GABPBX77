...
#include "gabpbx/codec.h"             /* for AST_MEDIA_TYPE_AUDIO */
...
static int ilbctolin_framein(struct ast_trans_pvt *pvt, struct ast_frame *f)
{
	...
	if (datalen % octets_per_frame) {
		ast_log_chan(NULL, LOG_WARNING, "Huh?  An ilbc frame that isn't a multiple of %u bytes long from %s (%d)?\n", octets_per_frame, f->src, datalen);
		return -1;
	}
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "iLBC Coder/Decoder");
