...
#if defined(GABPBX_REGISTER_FILE)
GABPBX_REGISTER_FILE()
#elif defined(GABPBX_FILE_VERSION)
GABPBX_FILE_VERSION(__FILE__, "$Revision: $")
#endif

#include "gabpbx/astobj2.h"           /* for ao2_ref */
#include "gabpbx/cli.h"               /* for ast_cli_entry, ast_cli, etc */
#include "gabpbx/codec.h"             /* for ast_codec_get */
#include "gabpbx/format.h"            /* for ast_format_get_attribute_data */
#include "gabpbx/frame.h"             /* for ast_frame, etc */
#include "gabpbx/linkedlists.h"       /* for AST_LIST_NEXT, etc */
#include "gabpbx/lock.h"              /* for ast_atomic_fetchadd_int */
#include "gabpbx/logger.h"            /* for ast_log, LOG_ERROR, etc */
#include "gabpbx/module.h"
#include "gabpbx/translate.h"         /* for ast_trans_pvt, etc */
#include "gabpbx/utils.h"             /* for ARRAY_LEN */

#include <opus/opus.h>

#include "gabpbx/opus.h"              /* for CODEC_OPUS_DEFAULT_* */
...
static int opustolin_framein(struct ast_trans_pvt *pvt, struct ast_frame *f)
{
	 ...
	 * issue on <https://github.com/traud/gabpbx-opus/issues>. I am just
	...
	if (f->datalen == 0) { /* decode_fec && !opvt->previous_lost */
		 ...
		 * but wait for the next frame. Till GABpbx 13.7, this creates the
	...
	if (!decode_fec) { /* 0 < f->datalen && opvt->previous_lost */
		 ...
		 * <https://issues.gabpbx.org/jira/browse/GABPBX-25483>. If this is
		 * an issue for your use-case, please, file and issue report on
		 * <https://github.com/traud/gabpbx-opus/issues>.
		 */
		decode_fec = 0;
		opus_decoder_ctl(opvt->opus, OPUS_GET_LAST_PACKET_DURATION(&frame_size));
		dst = pvt->outbuf.i16 + (pvt->samples * opvt->channels);
