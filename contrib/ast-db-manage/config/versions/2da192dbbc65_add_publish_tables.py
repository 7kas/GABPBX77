...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column('event_gabpbx-devicestate', sa.String(40)),
        sa.Column('event_gabpbx-mwi', sa.String(40)),
    )

    op.create_index('ps_inbound_publications_id', 'ps_inbound_publications', ['id'])

    op.create_table(
        'ps_gabpbx_publications',
        sa.Column('id', sa.String(40), nullable=False, unique=True),
    ...
    op.create_index('ps_gabpbx_publications_id', 'ps_gabpbx_publications', ['id'])

def downgrade():
    op.drop_table('ps_outbound_publishes')
    op.drop_table('ps_inbound_publications')
    op.drop_table('ps_gabpbx_publications')
