#!/usr/bin/perl -w
#
#  Simple GABpbx Manager Proxy, Version 1.01
#  2004-09-26
...
#############################
# Configuration for logging in to your gabpbx server
# Check you GABpbx config file "manager.conf" for details
...
# Port For this proxy
my $listen_port = 1234;
my $manager_pid = "/var/run/gabpbx_managerproxy.pid";
...
$p =
  ...
  or die "\nCould not connect to GABpbx Manager Port at $manager_host\n";
...
sub manager_reconnect()
{
    ...
    do
    {
        ...
        {
            die("!! Could not reconnect to GABpbx Manager port");
        }
            ...
            if ($_ == $m)
            {
                ...
                if (defined($R) && $R == 0)
                {
                    ...
                    if (!defined($T))
                    {
                        ...
                        # If we lost the socket for the GABpbx Mgr, then reconnect
                        if ($_ == $p)
                        {
                            log_debug(
                                     "** GABpbx Manager connection lost!!!!!",
                                     16);
                            manager_reconnect();
                        } else {
                ...
                else  # Socket is active and we are ready to read something from it
                {
                    ...
                    # If block is from gabpbx, send to clients
                    if ($_ == $p) {
                       # block is from gabpbx, send to clients
                       print "gabpbx: $_\n$blocks{$_}" if $debug;
                       my $cnt = 0;
                       ...
                       # Blocks are from clients, send to gabpbx
                       syswrite($p, $blocks{$_});
                       print "client: $_\n$blocks{$_}\n" if $debug;
                    }
                    delete $blocks{$_};
