/*
 * GABpbx EAGI -> TCP/IP proxy
 * 	by Danijel Korzinek (devil_slayer _at_ hotmail.com)
 *
 * This simple C application allows you to control gabpbx thru one TCP/IP
 ...
 * 	any AGI command there (http://www.voip-info.org/wiki-GABpbx+AGI),
 * 	e.g.:
 ...
 * 1) Main thread - reads commands from the socket and sends them to gabpbx
 * 2) stdin_thread - reads gabpbx output and sends it to the command socket
 * 3) signal_thread - reads the sound from gabpbx and sends it to the signal socket
 */
