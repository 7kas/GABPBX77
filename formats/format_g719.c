...
static int g719trunc(struct ast_filestream *fs)
{
	...
	if ((fd = fileno(fs->f)) < 0) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Unable to determine file descriptor for g719 filestream %p: %s\n", fs, strerror(errno));
		return -1;
	}
	if ((cur = ftello(fs->f)) < 0) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Unable to determine current position in g719 filestream %p: %s\n", fs, strerror(errno));
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "ITU G.719",
