...
/*!\file
 *
 * \brief Headerless G.726 (16/24/32/40kbps) data format for GABpbx.
 *
...
static int g726_write(struct ast_filestream *s, struct ast_frame *f)
{
	...
	if (f->datalen % frame_size[fs->rate]) {
		ast_log_chan(NULL, LOG_WARNING, "Invalid data length %d, should be multiple of %d\n",
...
static int g726_seek(struct ast_filestream *fs, off_t sample_offset, int whence)
{
	...
	if ((cur = ftello(fs->f)) < 0) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Unable to determine current position in g726 filestream %p: %s\n", fs, strerror(errno));
		return -1;
	}

	if (fseeko(fs->f, 0, SEEK_END) < 0) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Unable to seek to end of g726 filestream %p: %s\n", fs, strerror(errno));
		return -1;
	}

	if ((max = ftello(fs->f)) < 0) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Unable to determine max position in g726 filestream %p: %s\n", fs, strerror(errno));
		return -1;
	}
...
static int unload_module(void)
{
	...
	for (i = 0; f_def[i].desc_size ; i++) {
		if (ast_format_def_unregister(f_def[i].name))
			ast_log_chan(NULL, LOG_WARNING, "Failed to unregister format %s.\n", f_def[i].name);
	}
	return(0);
}
...
static int load_module(void)
{
	...
	for (i = 0; f_def[i].desc_size ; i++) {
		...
		if (ast_format_def_register(&f_def[i])) {	/* errors are fatal */
			ast_log_chan(NULL, LOG_WARNING, "Failed to register format %s.\n", f_def[i].name);
			unload_module();
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "Raw G.726 (16/24/32/40kbps) data",
