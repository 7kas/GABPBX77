	...
	if (!(f->datalen % 65)) {
		...
		while(len < f->datalen) {
			...
			if ((res = fwrite(gsm, 1, 2*GSM_FRAME_SIZE, fs->f)) != 2*GSM_FRAME_SIZE) {
				ast_log_chan(NULL, LOG_WARNING, "Bad write (%d/66): %s\n", res, strerror(errno));
				return -1;
			}
			len += 65;
		}
		...
		if (f->datalen % GSM_FRAME_SIZE) {
			ast_log_chan(NULL, LOG_WARNING, "Invalid data length, %d, should be multiple of 33\n", f->datalen);
			return -1;
		}
		if ((res = fwrite(f->data.ptr, 1, f->datalen, fs->f)) != f->datalen) {
				ast_log_chan(NULL, LOG_WARNING, "Bad write (%d/33): %s\n", res, strerror(errno));
				return -1;
		}
	}
	return 0;
...
static int gsm_seek(struct ast_filestream *fs, off_t sample_offset, int whence)
{
	...
	if ((cur = ftello(fs->f)) < 0) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Unable to determine current position in g719 filestream %p: %s\n", fs, strerror(errno));
		return -1;
	}

	if (fseeko(fs->f, 0, SEEK_END) < 0) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Unable to seek to end of g719 filestream %p: %s\n", fs, strerror(errno));
		return -1;
	}

	if ((max = ftello(fs->f)) < 0) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Unable to determine max position in g719 filestream %p: %s\n", fs, strerror(errno));
		return -1;
	}
...
static int gsm_trunc(struct ast_filestream *fs)
{
	...
	if ((fd = fileno(fs->f)) < 0) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Unable to determine file descriptor for gsm filestream %p: %s\n", fs, strerror(errno));
		return -1;
	}
	if ((cur = ftello(fs->f)) < 0) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Unable to determine current position in gsm filestream %p: %s\n", fs, strerror(errno));
		return -1;
	}
...
static off_t gsm_tell(struct ast_filestream *fs)
{
	...
	if (offset < 0) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Unable to determine offset for gsm filestream %p: %s\n", fs, strerror(errno));
		return 0;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "Raw GSM data",
