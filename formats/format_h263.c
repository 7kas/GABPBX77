...
static int h263_open(struct ast_filestream *s)
{
	...
	if (fread(&ts, 1, sizeof(ts), s->f) != sizeof(ts)) {
		ast_log_chan(NULL, LOG_WARNING, "Empty file!\n");
		return -1;
	}
	return 0;
}
...
static struct ast_frame *h263_read(struct ast_filestream *s, int *whennext)
{
	...
	len &= 0x7fff;
	if (len > BUF_SIZE) {
		ast_log_chan(NULL, LOG_WARNING, "Length %d is too long\n", len);
		return NULL;
	}
...
static int h263_write(struct ast_filestream *fs, struct ast_frame *f)
{
	...
	if ((res = fwrite(&ts, 1, sizeof(ts), fs->f)) != sizeof(ts)) {
			ast_log_chan(NULL, LOG_WARNING, "Bad write (%d/4): %s\n", res, strerror(errno));
			return -1;
	}
	len = htons(f->datalen | mark);
	if ((res = fwrite(&len, 1, sizeof(len), fs->f)) != sizeof(len)) {
			ast_log_chan(NULL, LOG_WARNING, "Bad write (%d/2): %s\n", res, strerror(errno));
			return -1;
	}
...
static int h263_trunc(struct ast_filestream *fs)
{
	...
	if ((fd = fileno(fs->f)) < 0) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Unable to determine file descriptor for h263 filestream %p: %s\n", fs, strerror(errno));
		return -1;
	}
	if ((cur = ftello(fs->f)) < 0) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Unable to determine current position in h263 filestream %p: %s\n", fs, strerror(errno));
		return -1;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "Raw H.263 data",
