...
#include "gabpbx/format_cache.h"  /* for ast_format_slin48 */
#include "gabpbx/frame.h"         /* for ast_frame, AST_FRIENDLY_OFFSET */
#include "gabpbx/logger.h"        /* for ast_log, LOG_ERROR, AST_LOG_ERROR */
#include "gabpbx/mod_format.h"    /* for ast_filestream, ast_format_def */
#include "gabpbx/module.h"
#if defined(HAVE_OPUSENC)
#include "gabpbx/config.h"        /* for ast_variable, ast_config_destroy */
#include "gabpbx/format.h"        /* for ast_format_get_... */
#include "gabpbx/utils.h"         /* for ast_flags */
#endif
...
static int ogg_opus_rewrite(struct ast_filestream *fs, const char *comment)
{
	...
	ope_comments_add(desc->comments, "ENCODER", "GABpbx PBX");
	if (comment)
		ope_comments_add(desc->comments, "COMMENT", comment);

	rate = ast_format_get_sample_rate(fs->fmt->format);
#if defined(GABPBX_VERSION_NUM) && (GABPBX_VERSION_NUM < 150000)
	channels = 1;
#else
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "OGG/Opus audio",
