...
static int ogg_speex_open(struct ast_filestream *fs)
{
	...
	if (result != 1) {
		if(bytes < BLOCK_SIZE) {
			ast_log_chan(NULL, LOG_ERROR, "Run out of data...\n");
		} else {
			ast_log_chan(NULL, LOG_ERROR, "Input does not appear to be an Ogg bitstream.\n");
		}
		ogg_sync_clear(&s->oy);
		return -1;
	}
	...
	if (ogg_stream_pagein(&s->os, &s->og) < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Error reading first page of Ogg bitstream data.\n");
		goto error;
	}

	if (read_packet(fs) < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Error reading initial header packet.\n");
		goto error;
	}

	hdr = speex_packet_to_header((char*)s->op.packet, s->op.bytes);
	if (memcmp(hdr->speex_string, "Speex   ", 8)) {
		ast_log_chan(NULL, LOG_ERROR, "OGG container does not contain Speex audio!\n");
		goto error;
	}
	if (hdr->frames_per_packet != 1) {
		ast_log_chan(NULL, LOG_ERROR, "Only one frame-per-packet OGG/Speex files are currently supported!\n");
		goto error;
	}
	if (hdr->nb_channels != 1) {
		ast_log_chan(NULL, LOG_ERROR, "Only monophonic OGG/Speex files are currently supported!\n");
		goto error;
	}
	if (hdr->rate != expected_rate) {
		ast_log_chan(NULL, LOG_ERROR, "Unexpected sampling rate (%d != %d)!\n",
	...
	if (read_packet(fs) < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Error reading comment packet.\n");
		goto error;
	}
	for (i = 0; i < hdr->extra_headers; i++) {
		if (read_packet(fs) < 0) {
			ast_log_chan(NULL, LOG_ERROR, "Error reading extra header packet %d.\n", i+1);
			goto error;
		}
	}
...
{
	ast_log_chan(NULL, LOG_WARNING, "Truncation is not supported on OGG/Speex streams!\n");
	return -1;
}

static int ogg_speex_write(struct ast_filestream *s, struct ast_frame *f)
{
	ast_log_chan(NULL, LOG_WARNING, "Writing is not supported on OGG/Speex streams!\n");
	return -1;
}
...
{
	ast_log_chan(NULL, LOG_WARNING, "Seeking is not supported on OGG/Speex streams!\n");
	return -1;
}

static off_t ogg_speex_tell(struct ast_filestream *s)
{
	ast_log_chan(NULL, LOG_WARNING, "Telling is not supported on OGG/Speex streams!\n");
	return -1;
}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "OGG/Speex audio",
