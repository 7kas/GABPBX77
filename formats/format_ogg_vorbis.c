...
static int ogg_vorbis_open(struct ast_filestream *s)
{
	...
	if (result != 0) {
		ast_log_chan(NULL, LOG_ERROR, "Error opening Ogg/Vorbis file stream.\n");
		return -1;
	}

	/* check stream(s) type */
	if (desc->ov_f.vi->channels != 1) {
		ast_log_chan(NULL, LOG_ERROR, "Only monophonic OGG/Vorbis files are currently supported!\n");
		ov_clear(&desc->ov_f);
		return -1;
	}

	if (desc->ov_f.vi->rate != DEFAULT_SAMPLE_RATE) {
		ast_log_chan(NULL, LOG_ERROR, "Only 8000Hz OGG/Vorbis files are currently supported!\n");
		ov_clear(&desc->ov_f);
		return -1;
	}
						 ...
						 const char *comment)
{
	...
	if (vorbis_encode_init_vbr(&tmp->vi, 1, DEFAULT_SAMPLE_RATE, 0.4)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to initialize Vorbis encoder!\n");
		vorbis_info_clear(&tmp->vi);
		return -1;
	}

	vorbis_comment_init(&tmp->vc);
	vorbis_comment_add_tag(&tmp->vc, "ENCODER", "GABpbx PBX");
	if (comment)
...
static int ogg_vorbis_write(struct ast_filestream *fs, struct ast_frame *f)
{
	...
	if (!s->writing) {
		ast_log_chan(NULL, LOG_ERROR, "This stream is not set up for writing!\n");
		return -1;
	}
	if (!f->datalen)
		return -1;
					 ...
					 int *whennext)
{
	...
	if (desc->writing) {
		ast_log_chan(NULL, LOG_WARNING, "Reading is not supported on OGG/Vorbis on write files.\n");
		return NULL;
	}
...
{
	ast_log_chan(NULL, LOG_WARNING, "Truncation is not supported on OGG/Vorbis streams!\n");
	return -1;
}
...
static int ogg_vorbis_seek(struct ast_filestream *fs, off_t sample_offset, int whence)
{
	...
	if (desc->writing) {
		ast_log_chan(NULL, LOG_WARNING, "Seeking is not supported on OGG/Vorbis streams in writing mode!\n");
		return -1;
	}
	...
	case SEEK_END:
		...
		break;
	default:
		ast_log_chan(NULL, LOG_WARNING, "Unknown *whence* to seek on OGG/Vorbis streams!\n");
		break;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "OGG/Vorbis audio",
