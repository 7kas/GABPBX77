...
static unsigned long get_time(void)
{
	...
	cur = times( &buf );
	if( cur < 0 ) {
		ast_log_chan(NULL,  LOG_WARNING, "Cannot get current time\n" );
		return 0;
	}
...
static int pcm_seek(struct ast_filestream *fs, off_t sample_offset, int whence)
{
	...
	if ((cur = ftello(fs->f)) < 0) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Unable to determine current position in pcm filestream %p: %s\n", fs, strerror(errno));
		return -1;
	}

	if (fseeko(fs->f, 0, SEEK_END) < 0) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Unable to seek to end of pcm filestream %p: %s\n", fs, strerror(errno));
		return -1;
	}

	if ((max = ftello(fs->f)) < 0) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Unable to determine max position in pcm filestream %p: %s\n", fs, strerror(errno));
		return -1;
	}
	...
	case SEEK_FORCECUR:
		...
		break;
	default:
		ast_log_chan(NULL, LOG_WARNING, "invalid whence %d, assuming SEEK_SET\n", whence);
		offset = sample_offset;
	}
	if (offset < 0) {
		ast_log_chan(NULL, LOG_WARNING, "negative offset %ld, resetting to 0\n", (long) offset);
		offset = 0;
	}
	if (whence == SEEK_FORCECUR && offset > max) { /* extend the file */
		...
		if (offset > max) {
			ast_log_chan(NULL, LOG_WARNING, "offset too large %ld, truncating to %ld\n", (long) offset, (long) max);
			offset = max;
		}
...
static int pcm_trunc(struct ast_filestream *fs)
{
	...
	if ((fd = fileno(fs->f)) < 0) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Unable to determine file descriptor for pcm filestream %p: %s\n", fs, strerror(errno));
		return -1;
	}
	if ((cur = ftello(fs->f)) < 0) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Unable to determine current position in pcm filestream %p: %s\n", fs, strerror(errno));
		...
		*/
		...
		if (stat_buf.st_size < fpos) {
			...
			if (fseek(fs->f, cur, SEEK_SET) < 0) {
				ast_log_chan(NULL,  LOG_WARNING, "Cannot seek in file: %s\n", strerror(errno) );
				return -1;
			}
			memset(buf, 0x55, 512);
			while (cur < fpos) {
				...
				if (fwrite(buf, 1, to_write, fs->f) != to_write) {
					ast_log_chan(NULL, LOG_ERROR, "Failed to write to file: %s\n", strerror(errno));
					return -1;
				}
				cur += to_write;
			}
...
static int check_header(struct ast_filestream *fs)
{
	...
	if (fread(header, 1, MIN_AU_HEADER_SIZE, f) != MIN_AU_HEADER_SIZE) {
		ast_log_chan(NULL, LOG_WARNING, "Read failed (header)\n");
		return -1;
	}
	magic = ltohl(header[AU_HDR_MAGIC_OFF]);
	if (magic != (uint32_t) AU_MAGIC) {
		ast_log_chan(NULL, LOG_WARNING, "Bad magic: 0x%x\n", magic);
	}
...
/*	data_size = ltohl(header[AU_HDR_DATA_SIZE_OFF]); */
	...
	if (encoding != AU_ENC_8BIT_ULAW) {
		ast_log_chan(NULL, LOG_WARNING, "Unexpected format: %u. Only 8bit ULAW allowed (%d)\n", encoding, AU_ENC_8BIT_ULAW);
		return -1;
	}
	sample_rate = ltohl(header[AU_HDR_SAMPLE_RATE_OFF]);
	if (sample_rate != DEFAULT_SAMPLE_RATE) {
		ast_log_chan(NULL, LOG_WARNING, "Sample rate can only be 8000 not %u\n", sample_rate);
		return -1;
	}
	channels = ltohl(header[AU_HDR_CHANNELS_OFF]);
	if (channels != 1) {
		ast_log_chan(NULL, LOG_WARNING, "Not in mono: channels=%u\n", channels);
		return -1;
	}
	/* Skip to data */
	fseek(f, 0, SEEK_END);
	...
	if (fseek(f, hdr_size, SEEK_SET) == -1 ) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to skip to data: %u\n", hdr_size);
		return -1;
	}
...
static int au_seek(struct ast_filestream *fs, off_t sample_offset, int whence)
{
	...
	if ((cur = ftello(fs->f)) < 0) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Unable to determine current position in au filestream %p: %s\n", fs, strerror(errno));
		return -1;
	}

	if (fseeko(fs->f, 0, SEEK_END) < 0) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Unable to seek to end of au filestream %p: %s\n", fs, strerror(errno));
		return -1;
	}

	if ((max = ftello(fs->f)) < 0) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Unable to determine max position in au filestream %p: %s\n", fs, strerror(errno));
		return -1;
	}
...
static int au_trunc(struct ast_filestream *fs)
{
	...
	if ((fd = fileno(fs->f)) < 0) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Unable to determine file descriptor for au filestream %p: %s\n", fs, strerror(errno));
		return -1;
	}
	if ((cur = ftello(fs->f)) < 0) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Unable to determine current position in au filestream %p: %s\n", fs, strerror(errno));
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "Raw/Sun uLaw/ALaw 8KHz (PCM,PCMA,AU), G.722 16Khz",
