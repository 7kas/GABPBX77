...
static int check_header_fmt(FILE *f, int hsize, int hz)
{
	...
	if (hsize < 16) {
		ast_log_chan(NULL, LOG_WARNING, "Unexpected header size %d\n", hsize);
		return -1;
	}
	if (fread(&format, 1, 2, f) != 2) {
		ast_log_chan(NULL, LOG_WARNING, "Read failed (format)\n");
		return -1;
	}
	if (ltohs(format) != 1) {
		ast_log_chan(NULL, LOG_WARNING, "Not a supported wav file format (%d). Only PCM encoded, 16 bit, mono, 8kHz/16kHz files are supported with a lowercase '.wav' extension.\n", ltohs(format));
		return -1;
	}
	...
	if (ltohs(chans) != 1) {
		ast_log_chan(NULL, LOG_WARNING, "Not in mono %d\n", ltohs(chans));
		return -1;
	}
	if (fread(&freq, 1, 4, f) != 4) {
		ast_log_chan(NULL, LOG_WARNING, "Read failed (freq)\n");
		return -1;
	}
	freq = ltohl(freq);
	if ((freq != 8000 && freq != 16000) || freq != hz) {
		ast_log_chan(NULL, LOG_WARNING, "Unexpected frequency mismatch %d (expecting %d)\n", freq, hz);
		return -1;
	}
	/* Ignore the byte frequency */
	if (fread(&bysec, 1, 4, f) != 4) {
		ast_log_chan(NULL, LOG_WARNING, "Read failed (BYTES_PER_SECOND)\n");
		return -1;
	}
	/* Check bytes per sample */
	if (fread(&bysam, 1, 2, f) != 2) {
		ast_log_chan(NULL, LOG_WARNING, "Read failed (BYTES_PER_SAMPLE)\n");
		return -1;
	}
	if (ltohs(bysam) != 2) {
		ast_log_chan(NULL, LOG_WARNING, "Can only handle 16bits per sample: %d\n", ltohs(bysam));
		return -1;
	}
	if (fread(&bisam, 1, 2, f) != 2) {
		ast_log_chan(NULL, LOG_WARNING, "Read failed (Bits Per Sample): %d\n", ltohs(bisam));
		return -1;
	}
	/* Skip any additional header */
	if (fseek(f,hsize-16,SEEK_CUR) == -1 ) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to skip remaining header bytes: %d\n", hsize-16 );
		return -1;
	}
	return 0;
}
...
static int check_header(FILE *f, int hz)
{
	...
	if (fread(&type, 1, 4, f) != 4) {
		ast_log_chan(NULL, LOG_WARNING, "Read failed (type)\n");
		return -1;
	}
	if (fread(&size, 1, 4, f) != 4) {
		ast_log_chan(NULL, LOG_WARNING, "Read failed (size)\n");
		return -1;
	}
#if __BYTE_ORDER == __BIG_ENDIAN
	size = ltohl(size);
	...
	if (fread(&formtype, 1, 4, f) != 4) {
		ast_log_chan(NULL, LOG_WARNING, "Read failed (formtype)\n");
		return -1;
	}
	if (memcmp(&type, "RIFF", 4)) {
		ast_log_chan(NULL, LOG_WARNING, "Does not begin with RIFF\n");
		return -1;
	}
	if (memcmp(&formtype, "WAVE", 4)) {
		ast_log_chan(NULL, LOG_WARNING, "Does not contain WAVE\n");
		return -1;
	}
	...
	for(;;)
	{
		...
		if (fread(&buf, 1, 4, f) != 4) {
			ast_log_chan(NULL, LOG_WARNING, "Read failed (block header format)\n");
			return -1;
		}
		/* Data has the actual length of data in it */
		if (fread(&data, 1, 4, f) != 4) {
			ast_log_chan(NULL, LOG_WARNING, "Read failed (block '%.4s' header length)\n", buf);
			return -1;
		}
#if __BYTE_ORDER == __BIG_ENDIAN
		data = ltohl(data);
		...
		if (fseek(f,data,SEEK_CUR) == -1 ) {
			ast_log_chan(NULL, LOG_WARNING, "Failed to skip '%.4s' block: %d\n", buf, data);
			return -1;
		}
	}
#if 0
...
static int update_header(FILE *f)
{
	...
	if (cur < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to find our position\n");
		return -1;
	}
	if (fseek(f, 4, SEEK_SET)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to set our position\n");
		return -1;
	}
	if (fwrite(&filelen, 1, 4, f) != 4) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to set write file size\n");
		return -1;
	}
	...
	if (fwrite(&datalen, 1, 4, f) != 4) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to set write datalen\n");
		return -1;
	}
	if (fseeko(f, cur, SEEK_SET)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to return to position\n");
		return -1;
	}
	return 0;
}
...
static int write_header(FILE *f, int writehz)
{
	...
	if (fwrite("RIFF", 1, 4, f) != 4) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to write header\n");
		return -1;
	}
...
static void wav_close(struct ast_filestream *s)
{
	...
	if (fs->bytes & 0x1) {
		if (fwrite(&zero, 1, 1, s->f) != 1) {
			ast_log_chan(NULL, LOG_WARNING, "fwrite() failed: %s\n", strerror(errno));
		}
	}
}
...
static int wav_write(struct ast_filestream *fs, struct ast_frame *f)
{
	...
	if (f->datalen > sizeof(tmp)) {
		ast_log_chan(NULL, LOG_WARNING, "Data length is too long\n");
		return -1;
	}
	tmpi = f->data.ptr;
...
#endif
		ast_log_chan(NULL, LOG_WARNING, "Bad write (%d): %s\n", res, strerror(errno));
		return -1;
	}
...
static int wav_seek(struct ast_filestream *fs, off_t sample_offset, int whence)
{
	...
	if ((cur = ftello(fs->f)) < 0) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Unable to determine current position in wav filestream %p: %s\n", fs, strerror(errno));
		return -1;
	}

	if (fseeko(fs->f, 0, SEEK_END) < 0) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Unable to seek to end of wav filestream %p: %s\n", fs, strerror(errno));
		return -1;
	}

	if ((max = ftello(fs->f)) < 0) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Unable to determine max position in wav filestream %p: %s\n", fs, strerror(errno));
		return -1;
	}
...
static int wav_trunc(struct ast_filestream *fs)
{
	...
	if ((fd = fileno(fs->f)) < 0) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Unable to determine file descriptor for wav filestream %p: %s\n", fs, strerror(errno));
		return -1;
	}
	if ((cur = ftello(fs->f)) < 0) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Unable to determine current position in wav filestream %p: %s\n", fs, strerror(errno));
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "Microsoft WAV/WAV16 format (8kHz/16kHz Signed Linear)",
