...
static int check_header(FILE *f)
{
	...
	if (fread(&fmt, 1, 4, f) != 4) {
		ast_log_chan(NULL, LOG_WARNING, "Read failed (fmt)\n");
		return -1;
	}
	if (memcmp(&fmt, "fmt ", 4)) {
		ast_log_chan(NULL, LOG_WARNING, "Does not say fmt\n");
		return -1;
	}
	...
	if (ltohs(format) != 49) {
		ast_log_chan(NULL, LOG_WARNING, "Not a GSM file %d\n", ltohs(format));
		return -1;
	}
	...
	if (ltohl(freq) != DEFAULT_SAMPLE_RATE) {
		ast_log_chan(NULL, LOG_WARNING, "Unexpected frequency %d\n", ltohl(freq));
		return -1;
	}
	/* Ignore the byte frequency */
	if (fread(&freq, 1, 4, f) != 4) {
		ast_log_chan(NULL, LOG_WARNING, "Read failed (X_1)\n");
		return -1;
	}
	/* Ignore the two weird fields */
	if (fread(&freq, 1, 4, f) != 4) {
		ast_log_chan(NULL, LOG_WARNING, "Read failed (X_2/X_3)\n");
		return -1;
	}
	/* Ignore the byte frequency */
	if (fread(&freq, 1, 4, f) != 4) {
		ast_log_chan(NULL, LOG_WARNING, "Read failed (Y_1)\n");
		return -1;
	}
	/* Check for the word fact */
	if (fread(&fact, 1, 4, f) != 4) {
		ast_log_chan(NULL, LOG_WARNING, "Read failed (fact)\n");
		return -1;
	}
	if (memcmp(&fact, "fact", 4)) {
		ast_log_chan(NULL, LOG_WARNING, "Does not say fact\n");
		return -1;
	}
	/* Ignore the "fact value" */
	if (fread(&fact, 1, 4, f) != 4) {
		ast_log_chan(NULL, LOG_WARNING, "Read failed (fact header)\n");
		return -1;
	}
	if (fread(&fact, 1, 4, f) != 4) {
		ast_log_chan(NULL, LOG_WARNING, "Read failed (fact value)\n");
		return -1;
	}
	/* Check for the word data */
	if (fread(&data, 1, 4, f) != 4) {
		ast_log_chan(NULL, LOG_WARNING, "Read failed (data)\n");
		return -1;
	}
	if (memcmp(&data, "data", 4)) {
		ast_log_chan(NULL, LOG_WARNING, "Does not say data\n");
		return -1;
	}
...
static int update_header(FILE *f)
{
	...
	if (fwrite(&filelen, 1, 4, f) != 4) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to write file size\n");
		return -1;
	}
	...
	if (fwrite(&samples, 1, 4, f) != 4) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to write samples\n");
		return -1;
	}
	...
	if (fwrite(&datalen, 1, 4, f) != 4) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to write datalen\n");
		return -1;
	}
...
static int wav_write(struct ast_filestream *s, struct ast_frame *f)
{
	...
	for (len = 0; len < f->datalen ; len += size) {
		...
		if (src && (res = fwrite(src, 1, MSGSM_FRAME_SIZE, s->f)) != MSGSM_FRAME_SIZE) {
			ast_log_chan(NULL, LOG_WARNING, "Bad write (%d/65): %s\n", res, strerror(errno));
			return -1;
		}
	}
	return 0;
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "Microsoft WAV format (Proprietary GSM)",
