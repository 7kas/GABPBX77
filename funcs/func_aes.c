	       ...
	       char *buf, size_t len)
{
	...
	if (ast_strlen_zero(args.data) || ast_strlen_zero(args.key)) {
		ast_log_chan(NULL, LOG_WARNING, "Syntax: %s(<key>,<data>) - missing argument!\n", cmd);
		return -1;
	}

	if (strlen(args.key) != AES_BLOCK_SIZE) {        /* key must be of 16 characters in length, 128 bits */
		ast_log_chan(NULL, LOG_WARNING, "Syntax: %s(<key>,<data>) - <key> parameter must be exactly 16 characters!\n", cmd);
		return -1;
	}
	...
	if (!tmp) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to allocate memory for data\n");
		return -1;
	}
	tmpP = tmp;
	...
	if (data_len >= len) {                        /* make sure to not go over buffer len */
		ast_log_chan(NULL, LOG_WARNING, "Syntax: %s(<keys>,<data>) - <data> exceeds buffer length.  Result may be truncated!\n", cmd);
		data_len = len - 1;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_DEFAULT, "AES dialplan functions",
