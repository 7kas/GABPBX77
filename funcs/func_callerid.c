		...
		<description>
			...
			<variablelist>
				<variable name="CALL_QUALIFIER">
					...
					<para>This variable is not automatically set by GABpbx.
					...
					that allows GABpbx to generate the Caller ID spill,
 ...
 *
 * \param chan GABpbx channel to read
 * \param cmd Not used
	...
	*buf = 0;
	...
	if (args.argc == 2) {
		...
		if (member.argc == 1 && !strcasecmp("all", member.subnames[0])) {
			...
			ast_log_chan(NULL, LOG_ERROR, "Unknown callerid data type '%s'.\n", data);
		}
	} else {
		enum ID_FIELD_STATUS status;
 ...
 *
 * \param chan GABpbx channel to update
 * \param cmd Not used
 * \param data Caller-id function datatype string
...
static int callerid_write(struct ast_channel *chan, const char *cmd, char *data, const char *value)
{
	...
	if (member.argc == 1 && !strcasecmp("rdnis", member.subnames[0])) {
		...
		if (('0' <= val[0]) && (val[0] <= '9')) {
			...
			ast_log_chan(NULL, LOG_ERROR, "Unknown callerid ani2 '%s', value unchanged\n", val);
		}
	} else if (!strcasecmp("ani", member.subnames[0])) {
	...
	*buf = 0;
	...
	if (member.argc == 1 && !strcasecmp("source", member.subnames[0])) {
		...
		case ID_FIELD_INVALID:
			break;
		default:
			ast_log_chan(NULL, LOG_ERROR, "Unknown connectedline data type '%s'.\n", data);
			break;
		}
	} else {
...
static int connectedline_write(struct ast_channel *chan, const char *cmd, char *data, const char *value)
{
	...
	if (member.argc == 1 && !strcasecmp("source", member.subnames[0])) {
		...
		if (source < 0) {
			ast_log_chan(NULL, LOG_ERROR, "Unknown connectedline source '%s', value unchanged\n", val);
		} else {
	...
	if (!strcasecmp("orig", member.subnames[0])) {
		if (member.argc == 2 && !strcasecmp("reason", member.subnames[1])) {
			...
			case ID_FIELD_INVALID:
				break;
			default:
				ast_log_chan(NULL, LOG_ERROR, "Unknown redirecting data type '%s'.\n", data);
				break;
			}
		}
...
static int redirecting_write(struct ast_channel *chan, const char *cmd, char *data, const char *value)
{
	...
	if (!strcasecmp("orig", member.subnames[0])) {
		if (member.argc == 2 && !strcasecmp("reason", member.subnames[1])) {
			 ...
			 * code to be unknown
			 */
				ast_log_chan(NULL, LOG_WARNING, "Unknown redirecting reason '%s', defaulting to unknown\n", val);
		...
		if (('0' <= val[0]) && (val[0] <= '9')) {
			...
			ast_log_chan(NULL, LOG_ERROR, "Unknown redirecting count '%s', value unchanged\n", val);
		}
	} else if (1 < member.argc && !strcasecmp("priv", member.subnames[0])) {
		if (!strcasecmp("orig", member.subnames[1])) {
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Party ID related dialplan functions (Caller-ID, Connected-line, Redirecting)");
