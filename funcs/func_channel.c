		...
		<description>
			...
			<variablelist>
				...
				<variable name="HANGUPCAUSE">
					<para>GABpbx cause of hangup (inbound/outbound)</para>
				</variable>
			</variablelist>
		</description>
	</function>
			     ...
			     char *data, char *buf, size_t len)
{
	...
	if (!strcasecmp(data, "audionativeformat")) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Unknown or unavailable item requested: '%s'\n", data);
		ret = -1;
	}
			      ...
			      char *data, const char *value)
{
	...
	if (!strcasecmp(data, "language"))
		...
		if (sscanf(value, "%d", &max_forwards) != 1) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to set max forwards to '%s'\n", value);
			ret = -1;
		} else {
			ast_channel_lock(chan);
...
static int func_channels_read(struct ast_channel *chan, const char *function, char *data, char *buf, size_t maxlen)
{
	...
	if (!ast_strlen_zero(data)) {
		if ((res = regcomp(&re, data, REG_EXTENDED | REG_ICASE | REG_NOSUB))) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Error compiling regular expression for %s(%s): %s\n", function, data, buf);
			return -1;
		}
	}
	...
	while ((c = ast_channel_iterator_next(iter))) {
		...
		if (ast_strlen_zero(data) || regexec(&re, ast_channel_name(c), 0, NULL, 0) == 0) {
			...
			if (buflen + namelen + (ast_strlen_zero(buf) ? 0 : 1) + 1 < maxlen) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Number of channels exceeds the available buffer space.  Output will be truncated!\n");
			}
		}
...
static int func_chan_exists_read(struct ast_channel *chan, const char *function, char *data, char *buf, size_t maxlen)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "%s: Channel name or unique ID required\n", function);
		return -1;
	}
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Channel information dialplan functions");
