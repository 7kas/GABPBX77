...
/*! \file
 *
 * \brief A function to retrieve variables from an GABpbx configuration file
 *
...
/*** DOCUMENTATION
	<function name="AST_CONFIG" language="en_US">
		...
		<description>
			<para>This function reads a variable from an GABpbx configuration file.</para>
		</description>
	</function>
	...
	char *buf, size_t len)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_ERROR, "AST_CONFIG() requires an argument\n");
		return -1;
	}
	...
	if (ast_strlen_zero(args.filename)) {
		ast_log_chan(NULL, LOG_ERROR, "AST_CONFIG() requires a filename\n");
		return -1;
	}

	if (ast_strlen_zero(args.category)) {
		ast_log_chan(NULL, LOG_ERROR, "AST_CONFIG() requires a category\n");
		return -1;
	}

	if (ast_strlen_zero(args.variable)) {
		ast_log_chan(NULL, LOG_ERROR, "AST_CONFIG() requires a variable\n");
		return -1;
	}

	if (!ast_strlen_zero(args.index)) {
		if (!sscanf(args.index, "%d", &index)) {
			ast_log_chan(NULL, LOG_ERROR, "AST_CONFIG() index must be an integer\n");
			return -1;
		}
	}
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "GABpbx configuration file variable access");
