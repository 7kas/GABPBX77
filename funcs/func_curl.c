...
/*** DOCUMENTATION
	<function name="CURL" language="en_US">
		...
		<description>
			...
			exten => s,1,Set(CURL(http://localhost:8088/static/astman.css)=/var/spool/gabpbx/tmp/astman.css))
			</example>
			<note>
	...
	if (chan) {
		...
		if (!(store = ast_channel_datastore_find(chan, &curl_info, NULL))) {
			...
			if (!(store = ast_datastore_alloc(&curl_info, NULL))) {
				ast_log_chan(NULL, LOG_ERROR, "Unable to allocate new datastore.  Cannot set any CURL options\n");
				ast_channel_unlock(chan);
				return -1;
			}

			if (!(list = ast_calloc(1, sizeof(*list)))) {
				ast_log_chan(NULL, LOG_ERROR, "Unable to allocate list head.  Cannot set any CURL options\n");
...
yuck:
		ast_log_chan(NULL, LOG_ERROR, "Unrecognized option: %s\n", name);
		return -1;
	}
 ...
 * GABpbx systems will be using an up-to-date cURL library. Therefore, we implement
...
static int acf_curl_helper(struct ast_channel *chan, struct curl_args *args)
{
	...
	if (!(curl = ast_threadstorage_get(&curl_instance, sizeof(*curl)))) {
		ast_log_chan(NULL, LOG_ERROR, "Cannot allocate curl structure\n");
		return -1;
	}

	if (url_is_vulnerable(args->url)) {
		ast_log_chan(NULL, LOG_ERROR, "URL '%s' is vulnerable to HTTP injection attacks. Aborting CURL() call.\n", args->url);
		return -1;
	}
	...
	if (curl_easy_perform(*curl) != 0) {
		ast_log_chan(NULL, LOG_WARNING, "%s ('%s')\n", curl_errbuf, args->url);
	}
	...
	for (i = 0; i < AST_VECTOR_SIZE(&hasfailurecode); ++i) {
		if (http_code == AST_VECTOR_GET(&hasfailurecode,i)){
			ast_log_chan(NULL, LOG_NOTICE, "%s%sCURL '%s' returned response code (%ld).\n",
...
static int acf_curl_exec(struct ast_channel *chan, const char *cmd, char *info, struct ast_str **buf, ssize_t len)
{
	...
	if (ast_strlen_zero(info)) {
		ast_log_chan(NULL, LOG_WARNING, "CURL requires an argument (URL)\n");
		return -1;
	}
...
static int acf_curl_write(struct ast_channel *chan, const char *cmd, char *name, const char *value)
{
	...
	if (ast_strlen_zero(args.file_path)) {
		ast_log_chan(NULL, LOG_WARNING, "CURL requires a file to write\n");
		return -1;
	}
	...
	if (!curl_params.cb_data.out_file) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to open file %s: %s (%d)\n",
			args.file_path,
			strerror(errno),
			errno);
		return -1;
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "Load external URL",
