	...
	if (ast_str_strlen(var_value)) {
		...
		while (curfieldptr != NULL && args.field != NULL) {
			...
			if (curfieldnum > start_field) {
				ast_log_chan(NULL, LOG_WARNING, "We're already past the field you wanted?\n");
			}
...
static int acf_sort_exec(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	case ERROR_NOARG:
		ast_log_chan(NULL, LOG_ERROR, "SORT() requires an argument\n");
		break;
	case ERROR_NOMEM:
	...
	case 0:
		...
		break;
	default:
		ast_log_chan(NULL, LOG_ERROR, "Unknown internal error\n");
	}
...
static int acf_cut_exec(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	case ERROR_NOARG:
		ast_log_chan(NULL, LOG_ERROR, "Syntax: CUT(<varname>,<char-delim>,<range-spec>) - missing argument!\n");
		break;
	case ERROR_NOMEM:
		ast_log_chan(NULL, LOG_ERROR, "Out of memory\n");
		break;
	case ERROR_USAGE:
		ast_log_chan(NULL, LOG_ERROR, "Usage: CUT(<varname>,<char-delim>,<range-spec>)\n");
		break;
	case 0:
		ret = 0;
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Cut out information from a string");
