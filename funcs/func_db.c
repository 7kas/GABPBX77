...
/*! \file
 *
 * \brief Functions for interaction with the GABpbx database
 *
...
/*** DOCUMENTATION
	<function name="DB" language="en_US">
		...
		<synopsis>
			Read from or write to the GABpbx database.
		</synopsis>
		<syntax argsep="/">
		...
		<description>
			<para>This function will read from or write a value to the GABpbx database.  On a
	...
	<function name="DB_EXISTS" language="en_US">
		...
		<synopsis>
			Check to see if a key exists in the GABpbx database.
		</synopsis>
		<syntax argsep="/">
			<parameter name="family" required="true" />
			<parameter name="key" required="true" />
		...
		<description>
			<para>This function will check to see if a key exists in the GABpbx
	...
	<function name="DB_KEYS" language="en_US">
		...
		<synopsis>
			Obtain a list of keys within the GABpbx database.
		</synopsis>
		<syntax>
		...
		<description>
			...
			at the prefix specified within the GABpbx database.  If no argument is
	...
	<function name="DB_KEYCOUNT" language="en_US">
		...
		<synopsis>
			Obtain the number of keys at a prefix within the GABpbx database.
		</synopsis>
		<syntax>
			<parameter name="prefix" />
		</syntax>
	...
	<function name="DB_DELETE" language="en_US">
		...
		<description>
			<para>This function will retrieve a value from the GABpbx database
			    ...
			    char *parse, char *buf, size_t len)
{
	...
	if (ast_strlen_zero(parse)) {
		ast_log_chan(NULL, LOG_WARNING, "DB requires an argument, DB(<family>/<key>)\n");
		return -1;
	}
			     ...
			     const char *value)
{
	...
	if (ast_strlen_zero(parse)) {
		ast_log_chan(NULL, LOG_WARNING, "DB requires an argument, DB(<family>/<key>)=<value>\n");
		return -1;
	}
	...
	* If this happens, allow it to go in, but warn the user of the issue and possible data corruption. */
	if (ast_strlen_zero(args.key) || args.key[0] == '/' || args.key[strlen(args.key) - 1] == '/' || strstr(args.key, "//")) {
		ast_log_chan(NULL, LOG_WARNING, "DB: key '%s' seems malformed\n", args.key);
	}
	if (ast_db_put(args.family, args.key, value)) {
		ast_log_chan(NULL, LOG_WARNING, "DB: Error writing value to database.\n");
	}
			      ...
			      char *parse, char *buf, size_t len)
{
	...
	if (ast_strlen_zero(parse)) {
		ast_log_chan(NULL, LOG_WARNING, "DB_EXISTS requires an argument, DB(<family>/<key>)\n");
		return -1;
	}
			      ...
			      char *parse, char *buf, size_t len)
{
	...
	if (ast_strlen_zero(parse)) {
		ast_log_chan(NULL, LOG_WARNING, "DB_DELETE requires an argument, DB_DELETE(<family>/<key>)\n");
		return -1;
	}
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Database (astdb) related dialplan functions");
