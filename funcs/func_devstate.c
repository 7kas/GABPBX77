 ...
 *
 * \note Props go out to Ahrimanes in \#gabpbx for requesting this at 4:30 AM
...
static int devstate_write(struct ast_channel *chan, const char *cmd, char *data, const char *value)
{
	...
	if (strncasecmp(data, "Custom:", len)) {
		ast_log_chan(NULL, LOG_WARNING, "The DEVICE_STATE function can only be used to set 'Custom:' device state!\n");
		return -1;
	}
	data += len;
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "DEVICE_STATE function called with no custom device name!\n");
		return -1;
	}
	...
	if (state_val == AST_DEVICE_UNKNOWN) {
		ast_log_chan(NULL, LOG_ERROR, "DEVICE_STATE function given invalid state value '%s'\n", value);
		return -1;
	}
...
static int hint_read(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "The HINT function requires an extension\n");
		return -1;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "Gets or sets a device state in the dialplan",
