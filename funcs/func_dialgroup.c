...
static int dialgroup_read(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	if (!grhead) {
		if (!ast_strlen_zero(cmd)) {
			ast_log_chan(NULL, LOG_WARNING, "No such dialgroup '%s'\n", data);
		}
		return -1;
	}
	...
	while ((entry = ao2_iterator_next(&i))) {
		...
		if (len - bufused > tmp + 2) {
			...
			if (!ast_strlen_zero(cmd)) {
				ast_log_chan(NULL, LOG_WARNING, "Dialgroup '%s' is too large.  Truncating list.\n", data);
			} else {
				res = 1;
				ao2_ref(entry, -1);
				break;
	...
	if (strcasecmp(args.op, "add") == 0) {
		for (j = 0; j < inter.argc; j++) {
			...
			if ((entry = ao2_alloc(sizeof(*entry), NULL))) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Unable to add '%s' to dialgroup '%s'\n", inter.faces[j], grhead->name);
			}
		}
		...
		for (j = 0; j < inter.argc; j++) {
			if ((entry = ao2_find(grhead->entries, inter.faces[j], OBJ_UNLINK))) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Interface '%s' not found in dialgroup '%s'\n", inter.faces[j], grhead->name);
			}
		}
	} else {
		ast_log_chan(NULL, LOG_ERROR, "Unrecognized operation: %s\n", args.op);
		needrefresh = 0;
	}
	ao2_ref(grhead, -1);
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Dialgroup dialplan function");
