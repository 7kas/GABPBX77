	...
	char *buf, size_t len)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_ERROR, "DIALPLAN_EXISTS() requires an argument\n");
		return -1;
	}
	...
	if (!ast_strlen_zero(args.priority)) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Invalid arguments provided to DIALPLAN_EXISTS\n");
		return -1;
	}
...
static int acf_isexten_exec(struct ast_channel *chan, const char *cmd, char *parse, char *buffer, size_t buflen)
{
	...
	if (ast_strlen_zero(args.extension)) {
		ast_log_chan(NULL, LOG_WARNING, "Syntax: VALID_EXTEN([<context>],<extension>[,<priority>]) - missing argument <extension>!\n");
		return -1;
	}
