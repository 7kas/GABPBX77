			 ...
			 char *buf, size_t len)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "%s", synopsis);
		return -1;
	}
...
static int enum_query_read(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "ENUMQUERY requires at least a number as an argument...\n");
		goto finish;
	}
	...
	if (!chan) {
		ast_log_chan(NULL, LOG_ERROR, "ENUMQUERY cannot be used without a channel!\n");
		goto finish;
	}
...
static int enum_result_read(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "ENUMRESULT requires two arguments (id and resultnum)\n");
		goto finish;
	}

	if (!chan) {
		ast_log_chan(NULL, LOG_ERROR, "ENUMRESULT can not be used without a channel!\n");
		goto finish;
	}
	...
	if (ast_strlen_zero(args.id)) {
		ast_log_chan(NULL, LOG_ERROR, "A result ID must be provided to ENUMRESULT\n");
		goto finish;
	}

	if (ast_strlen_zero(args.resultnum)) {
		ast_log_chan(NULL, LOG_ERROR, "A result number must be given to ENUMRESULT!\n");
		goto finish;
	}
	...
	ast_channel_unlock(chan);
	if (!datastore) {
		ast_log_chan(NULL, LOG_WARNING, "No ENUM results found for query id!\n");
		goto finish;
	}
	...
	if (sscanf(args.resultnum, "%30u", &num) != 1) {
		ast_log_chan(NULL, LOG_ERROR, "Invalid value '%s' for resultnum to ENUMRESULT!\n", args.resultnum);
		goto finish;
	}

	if (!num || num > erds->context->naptr_rrs_count) {
		ast_log_chan(NULL, LOG_WARNING, "Result number %u is not valid for ENUM query results for ID %s!\n", num, args.id);
		goto finish;
	}
			       ...
			       char *data, char *buf, size_t len)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "Syntax: TXTCIDNAME(number[,zone-suffix])\n");
		return -1;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_DEFAULT, "ENUM related dialplan functions",
