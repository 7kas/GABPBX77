...
/*** DOCUMENTATION
		...
		<description>
			...
			<variablelist>
				...
				<variable name="SYSTEMNAME">
					<para>value of the <literal>systemname</literal> option from <literal>gabpbx.conf</literal></para>
				</variable>
				<variable name="ASTCACHEDIR">
					<para>value of the <literal>astcachedir</literal> option from <literal>gabpbx.conf</literal></para>
				</variable>
				<variable name="ASTETCDIR">
					<para>value of the <literal>astetcdir</literal> option from <literal>gabpbx.conf</literal></para>
				</variable>
				<variable name="ASTMODDIR">
					<para>value of the <literal>astmoddir</literal> option from <literal>gabpbx.conf</literal></para>
				</variable>
				<variable name="ASTVARLIBDIR">
					<para>value of the <literal>astvarlib</literal> option from <literal>gabpbx.conf</literal></para>
				</variable>
				<variable name="ASTDBDIR">
					<para>value of the <literal>astdbdir</literal> option from <literal>gabpbx.conf</literal></para>
				</variable>
				<variable name="ASTKEYDIR">
					<para>value of the <literal>astkeydir</literal> option from <literal>gabpbx.conf</literal></para>
				</variable>
				<variable name="ASTDATADIR">
					<para>value of the <literal>astdatadir</literal> option from <literal>gabpbx.conf</literal></para>
				</variable>
				<variable name="ASTAGIDIR">
					<para>value of the <literal>astagidir</literal> option from <literal>gabpbx.conf</literal></para>
				</variable>
				<variable name="ASTSPOOLDIR">
					<para>value of the <literal>astspooldir</literal> option from <literal>gabpbx.conf</literal></para>
				</variable>
				<variable name="ASTRUNDIR">
					<para>value of the <literal>astrundir</literal> option from <literal>gabpbx.conf</literal></para>
				</variable>
				<variable name="ASTLOGDIR">
					<para>value of the <literal>astlogdir</literal> option from <literal>gabpbx.conf</literal></para>
				</variable>
				<variable name="ASTSBINDIR">
					<para>value of the <literal>astsbindir</literal> option from <literal>gabpbx.conf</literal></para>
				</variable>
				<variable name="ENTITYID">
					<para>Global Entity ID set automatically, or from <literal>gabpbx.conf</literal></para>
				</variable>
			</variablelist>
		</description>
	</function>
	<function name="STAT" language="en_US">
		...
		<description>
			<note>
				<para>If <literal>live_dangerously</literal> in <literal>gabpbx.conf</literal>
	...
	<function name="BASENAME" language="en_US">
		...
		<description>
			...
			same => n,Set(basename=${BASENAME(/etc/gabpbx/extensions.conf)})
	...
	<function name="DIRNAME" language="en_US">
		...
		<description>
			...
			<example title="Directory name">
			same => n,Set(dirname=${DIRNAME(/etc/gabpbx/extensions.conf)})
			same => n,NoOp(${dirname}) ; outputs /etc/gabpbx
			</example>
		</description>
		<see-also>
...
static int64_t count_lines(const char *filename, enum file_format newline_format)
{
	...
	if (!(ff = fopen(filename, "r"))) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to open '%s': %s\n", filename, strerror(errno));
		return -1;
	}
...
static int file_read(struct ast_channel *chan, const char *cmd, char *data, struct ast_str **buf, ssize_t len)
{
	...
	if (args.argc < 4 || !strchr(args.options, 'l')) {
		...
		if (fseeko(ff, 0, SEEK_END) < 0) {
			ast_log_chan(NULL, LOG_ERROR, "Cannot seek to end of '%s': %s\n", args.filename, strerror(errno));
			fclose(ff);
			return -1;
		}
		flength = ftello(ff);
		...
		if (offset < 0) {
			...
			if ((offset = ftello(ff)) < 0) {
				ast_log_chan(NULL, AST_LOG_ERROR, "Cannot determine offset position of '%s': %s\n", args.filename, strerror(errno));
				fclose(ff);
				return -1;
			}
		}
		...
		for (off_i = ftello(ff); off_i < flength && off_i < offset + length; off_i += sizeof(fbuf)) {
			...
			if (fread(fbuf, 1, sizeof(fbuf), ff) < sizeof(fbuf) && !feof(ff)) {
				ast_log_chan(NULL, LOG_ERROR, "Short read?!!\n");
				break;
			}
	...
	if (format == FF_UNKNOWN) {
		if ((format = file2format(args.filename)) == FF_UNKNOWN) {
			ast_log_chan(NULL, LOG_WARNING, "'%s' is not a line-based file\n", args.filename);
			return -1;
		}
	}
	...
	if (fseek(ff, 0, SEEK_END)) {
		ast_log_chan(NULL, LOG_ERROR, "Cannot seek to end of file '%s': %s\n", args.filename, strerror(errno));
		fclose(ff);
		return -1;
	}
	...
	if (offset < 0 || length < 0) {
		...
		for (i = (flength / sizeof(fbuf)) * sizeof(fbuf); i >= 0; i -= sizeof(fbuf)) {
			...
			if (fseeko(ff, i, SEEK_SET)) {
				ast_log_chan(NULL, LOG_ERROR, "Cannot seek to offset %" PRId64 ": %s\n", i, strerror(errno));
			}
	...
	if (offset_offset < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Offset '%s' refers to before the beginning of the file!\n", args.offset);
		fclose(ff);
		return -1;
	}
	...
	if (fseeko(ff, offset_offset, SEEK_SET)) {
		ast_log_chan(NULL, LOG_ERROR, "fseeko failed: %s\n", strerror(errno));
	}
	...
	if (args.argc < 4 || !strchr(args.options, 'l')) {
		...
		if (args.argc > 3 && strchr(args.options, 'a')) {
			...
			if (!(ff = fopen(args.filename, "a"))) {
				ast_log_chan(NULL, LOG_WARNING, "Cannot open file '%s' for appending: %s\n", args.filename, strerror(errno));
				return 0;
			}
			if (fwrite(value, 1, vlength, ff) < vlength) {
				ast_log_chan(NULL, LOG_ERROR, "Short write?!!\n");
			}
			fclose(ff);
			return 0;
		} else if (offset == 0 && length == LLONG_MAX) {
			if (!(ff = fopen(args.filename, "w"))) {
				ast_log_chan(NULL, LOG_WARNING, "Cannot open file '%s' for writing: %s\n", args.filename, strerror(errno));
				return 0;
			}
			if (fwrite(value, 1, vlength, ff) < vlength) {
				ast_log_chan(NULL, LOG_ERROR, "Short write?!!\n");
		...
		if (!(ff = fopen(args.filename, "r+"))) {
			ast_log_chan(NULL, LOG_WARNING, "Cannot open file '%s' for modification: %s\n", args.filename, strerror(errno));
			return 0;
		}
		fseeko(ff, 0, SEEK_END);
		flength = ftello(ff);
		...
		if (offset < 0) {
			if (fseeko(ff, offset, SEEK_END)) {
				ast_log_chan(NULL, LOG_ERROR, "Cannot seek to offset of '%s': %s\n", args.filename, strerror(errno));
				fclose(ff);
				return -1;
			}
			if ((offset = ftello(ff)) < 0) {
		...
		if (length < 0) {
			...
			if (length < 0) {
				ast_log_chan(NULL, LOG_ERROR, "Length '%s' exceeds the file length.  No data will be written.\n", args.length);
				fclose(ff);
				return -1;
			}
		}
		...
		if (length == vlength) {
			...
			if (truncate(args.filename, offset + vlength)) {
				ast_log_chan(NULL, LOG_ERROR, "Unable to truncate the file: %s\n", strerror(errno));
			}
			...
			if (fseeko(ff, offset + length, SEEK_SET)) {
				ast_log_chan(NULL, LOG_WARNING, "Unable to seek to %" PRId64 " + %" PRId64 " != %" PRId64 "?)\n", offset, length, ftello(ff));
			}
		...
		if (newline_format == FF_UNKNOWN && (newline_format = file2format(args.filename)) == FF_UNKNOWN) {
			ast_log_chan(NULL, LOG_ERROR, "File '%s' not in line format\n", args.filename);
			return -1;
		}
		...
		if (strchr(args.options, 'a')) {
			/* Append to file */
			if (!(ff = fopen(args.filename, "a"))) {
				ast_log_chan(NULL, LOG_ERROR, "Unable to open '%s' for appending: %s\n", args.filename, strerror(errno));
				return -1;
			}
			if (fwrite(value, 1, vlength, ff) < vlength) {
				ast_log_chan(NULL, LOG_ERROR, "Short write?!!\n");
			...
			off_t truncsize;
			if (!(ff = fopen(args.filename, "w"))) {
				ast_log_chan(NULL, LOG_ERROR, "Unable to open '%s' for writing: %s\n", args.filename, strerror(errno));
				return -1;
			}
			if (fwrite(value, 1, vlength, ff) < vlength) {
				ast_log_chan(NULL, LOG_ERROR, "Short write?!!\n");
			...
			if ((truncsize = ftello(ff)) < 0) {
				ast_log_chan(NULL, AST_LOG_ERROR, "Unable to determine truncate position of '%s': %s\n", args.filename, strerror(errno));
			}
			fclose(ff);
			if (truncsize >= 0 && truncate(args.filename, truncsize)) {
				ast_log_chan(NULL, LOG_ERROR, "Unable to truncate file '%s': %s\n", args.filename, strerror(errno));
				return -1;
			}
		} else {
			...
			if (offset < 0 && length < offset) {
				/* Nonsense! */
				ast_log_chan(NULL, LOG_ERROR, "Length cannot specify a position prior to the offset\n");
				return -1;
			}

			if (!(ff = fopen(args.filename, "r+"))) {
				ast_log_chan(NULL, LOG_ERROR, "Cannot open '%s' for modification: %s\n", args.filename, strerror(errno));
				return -1;
			}
			...
			if ((flength = ftello(ff)) < 0) {
				ast_log_chan(NULL, AST_LOG_ERROR, "Cannot determine end position of file '%s': %s\n", args.filename, strerror(errno));
				fclose(ff);
				return -1;
			}
			...
			if (offset < 0 || length < 0) {
				...
				for (i = (flength / sizeof(fbuf)) * sizeof(fbuf); i >= 0; i -= sizeof(fbuf)) {
					...
					if (fread(fbuf, 1, sizeof(fbuf), ff) < sizeof(fbuf) && !feof(ff)) {
						ast_log_chan(NULL, LOG_ERROR, "Short read: %s\n", strerror(errno));
						fclose(ff);
						return -1;
					}
			...
			if (length_offset - offset_offset == vlength + (strchr(args.options, 'd') ? 0 : strlen(format2term(newline_format)))) {
				...
				while ((cur = ftello(ff)) < flength) {
					if (cur < 0) {
						ast_log_chan(NULL, AST_LOG_ERROR, "Unable to determine last write position for '%s': %s\n", args.filename, strerror(errno));
						fclose(ff);
						return -1;
					}
				...
				if (truncate(args.filename, flength - (length_length - vlen))) {
					ast_log_chan(NULL, LOG_ERROR, "Truncation of file failed: %s\n", strerror(errno));
				}
			} else {
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Environment/filesystem dialplan functions");
