...
/*** DOCUMENTATION
	...
	<function name="EVAL_SUB" language="en_US">
		...
		<description>
			...
			in dialplan, which can then be wrapped using the GABpbx function interface using <literal>EVAL_SUB</literal>.</para>
			...
			are involved, and due to the way GABpbx parses dialplan, all functions in an application call are evaluated all the
...
static int eval_exten_read(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "The EVAL_EXTEN function requires an extension\n");
		return -1;
	}
...
static int eval_sub_read(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "The EVAL_SUB function requires an extension\n");
		*buf = '\0';
		return -1;
	}
	...
	if (gosub_res) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to execute Gosub(%s)\n", data);
		*buf = '\0';
		return -1;
	}
...
AST_MODULE_INFO_STANDARD_EXTENDED(GABPBX_GPL_KEY, "Extension evaluation function");
