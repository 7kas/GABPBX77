	...
	<function name="FRAME_DROP" language="en_US">
		...
		<syntax>
			<parameter name="direction" required="true">
				<para>List of frame types to be dropped for the specified direction. Direction can be <literal>TX</literal> or <literal>RX</literal>. The <literal>TX</literal> direction will prevent GABpbx from sending frames to a channel, and the <literal>RX</literal> direction will prevent GABpbx from receiving frames from a channel.</para>
...
AST_MODULE_INFO_STANDARD_EXTENDED(GABPBX_GPL_KEY, "Function to drop frames on a channel.");
