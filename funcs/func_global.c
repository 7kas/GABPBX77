...
static int shared_read(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "SHARED() requires an argument: SHARED(<var>[,<chan>])\n");
		return -1;
	}
	...
	if (!ast_strlen_zero(args.chan)) {
		...
		if (!(c_ref = ast_channel_get_by_name(args.chan)) && !(c_ref = ast_channel_get_by_name_prefix(prefix, strlen(prefix)))) {
			ast_log_chan(NULL, LOG_ERROR, "Channel '%s' not found!  Variable '%s' will be blank.\n", args.chan, args.var);
			return -1;
		}
		chan = c_ref;
	} else if (!chan) {
...
static int shared_write(struct ast_channel *chan, const char *cmd, char *data, const char *value)
{
	...
	if (!ast_strlen_zero(args.chan)) {
		...
		sprintf(prefix, "%s-", args.chan);
		if (!(c_ref = ast_channel_get_by_name(args.chan)) && !(c_ref = ast_channel_get_by_name_prefix(prefix, strlen(prefix)))) {
			ast_log_chan(NULL, LOG_ERROR, "Channel '%s' not found!  Variable '%s' not set to '%s'.\n", args.chan, args.var, value);
			return -1;
		}
		chan = c_ref;
	} else if (!chan) {
	...
	if (!(varstore = ast_channel_datastore_find(chan, &shared_variable_info, NULL))) {
		if (!(varstore = ast_datastore_alloc(&shared_variable_info, NULL))) {
			ast_log_chan(NULL, LOG_ERROR, "Unable to allocate new datastore.  Shared variable not set.\n");
			ast_channel_unlock(chan);
			if (c_ref) {
				c_ref = ast_channel_unref(c_ref);
			}
		...
		if (!(varshead = ast_calloc(1, sizeof(*varshead)))) {
			ast_log_chan(NULL, LOG_ERROR, "Unable to allocate variable structure.  Shared variable not set.\n");
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Variable dialplan functions");
