		...
		<syntax>
			...
			<parameter name="type" required="true">
				...
				<enumlist>
					...
					<enum name="ast"><para>Translated GABpbx cause code</para></enum>
				</enumlist>
			</parameter>
		</syntax>
		<description>
			<para>Gets technology-specific or translated GABpbx cause code information
...
static int hangupcause_read(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	*buf = 0;
	...
	if (args.argc != 2) {
		...
		ast_log_chan(NULL, LOG_WARNING, "The HANGUPCAUSE function must have 2 parameters, not %u\n", args.argc);
		return -1;
	}
	...
	if (!cause_code) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to find information for channel %s\n", args.channel);
		return -1;
	}
	...
	if (!strcmp(args.type, "ast")) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Information type not recognized (%s)\n", args.type);
		res = -1;
	}
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "HANGUPCAUSE related functions and applications");
