...
static int remove_hold_intercept(struct ast_channel *chan)
{
	...
	if (!datastore) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Cannot remove HOLD_INTERCEPT from %s: HOLD_INTERCEPT not currently enabled\n",
	...
	if (ast_framehook_detach(chan, data->framehook_id)) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Failed to remove HOLD_INTERCEPT framehook from channel %s\n",
		        ast_channel_name(chan));
		return -1;
	}

	if (ast_channel_datastore_remove(chan, datastore)) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Failed to remove HOLD_INTERCEPT datastore from channel %s\n",
		        ast_channel_name(chan));
		return -1;
	}
...
static int set_hold_intercept(struct ast_channel *chan)
{
	...
	datastore = ast_channel_datastore_find(chan, &hold_intercept_datastore, NULL);
	if (datastore) {
		ast_log_chan(NULL, AST_LOG_WARNING, "HOLD_INTERCEPT already set on '%s'\n",
		        ast_channel_name(chan));
		return 0;
	}
	...
	if (data->framehook_id < 0) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Failed to attach HOLD_INTERCEPT framehook to '%s'\n",
		        ast_channel_name(chan));
		ast_datastore_free(datastore);
		ast_free(data);
		return -1;
	...
	char *data, const char *value)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, AST_LOG_WARNING, "HOLD_INTERCEPT requires an argument\n");
		return -1;
	}
	...
	if (!strcasecmp(data, "set")) {
		...
		ast_log_chan(NULL, AST_LOG_WARNING, "HOLD_INTERCEPT: unknown option %s\n", data);
		res = -1;
	}
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Hold interception dialplan function");
