...
static int iconv_read(struct ast_channel *chan, const char *cmd, char *arguments, char *buf, size_t len)
{
	...
	if (ast_strlen_zero(arguments)) {
		ast_log_chan(NULL, LOG_WARNING, "Syntax: ICONV(<in-charset>,<out-charset>,<text>) - missing arguments!\n");
		return -1;
	}
	...
	if (args.argc < 3) {
		ast_log_chan(NULL, LOG_WARNING, "Syntax: ICONV(<in-charset>,<out-charset>,<text>) %u\n", args.argc);
		return -1;
	}
	...
	if (cd == (iconv_t) -1) {
		ast_log_chan(NULL, LOG_ERROR, "conversion from '%s' to '%s' not available. type 'iconv -l' in a shell to list the supported charsets.\n", args.in_charset, args.out_charset);
		return -1;
	}

	if (iconv(cd, (AST_ICONV_CAST) &args.text, &incount, &buf, &outcount) == (size_t) -1) {
		if (errno == E2BIG)
			ast_log_chan(NULL, LOG_WARNING, "Iconv: output buffer too small.\n");
		else if (errno == EILSEQ)
			ast_log_chan(NULL, LOG_WARNING,  "Iconv: illegal character.\n");
		else if (errno == EINVAL)
			ast_log_chan(NULL, LOG_WARNING,  "Iconv: incomplete character sequence.\n");
		else
			ast_log_chan(NULL, LOG_WARNING,  "Iconv: error %d: %s.\n", errno, strerror(errno));
	}
	*buf = '\0';
	iconv_close(cd);
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Charset conversions");
