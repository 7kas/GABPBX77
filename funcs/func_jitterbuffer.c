...
/*** DOCUMENTATION
	<function name="JITTERBUFFER" language="en_US">
		...
		<synopsis>
			Add a Jitterbuffer to the Read side of the channel. This dejitters the audio stream before it reaches the GABpbx core. This is a write only function.
		</synopsis>
		<syntax>
...
static int jb_helper(struct ast_channel *chan, const char *cmd, char *data, const char *value)
{
	...
	if (!ast_strlen_zero(value) && strcasecmp(value, "default")) {
		...
		if (res) {
			ast_log_chan(NULL, LOG_WARNING, "Invalid jitterbuffer parameters %s\n", value);
		}
	}
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Jitter buffer for read side of channel.");
