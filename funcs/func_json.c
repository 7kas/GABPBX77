...
static int parse_node(char **key, char *currentkey, char *nestchar, int count, struct ast_json *json, char *buf, size_t len, int *depth)
{
	...
	if (*depth > MAX_JSON_STACK) {
		ast_log_chan(NULL, LOG_WARNING, "Max JSON stack (%d) exceeded\n", MAX_JSON_STACK);
		return -1;
	}
	...
	switch(ast_json_typeof(jsonval)) {
		...
		case AST_JSON_OBJECT:
			...
			break;
		default:
			ast_log_chan(NULL, LOG_WARNING, "Got unsuported type %d\n", ast_json_typeof(jsonval));
			return -1;
	}
	return 0;
}
...
static int json_decode_read(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	if (ast_strlen_zero(args.key)) {
		ast_log_chan(NULL, LOG_WARNING, "%s requires a key\n", cmd);
		return -1;
	}
	...
	if (!ast_strlen_zero(args.nestchar)) {
		...
		if (seplen != 1) {
			ast_log_chan(NULL, LOG_WARNING, "Nesting separator '%s' has length %d and is invalid (must be a single character)\n", args.nestchar, seplen);
		} else {
	...
	if (!varsubst) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to allocate string\n");
		return -1;
	}
	...
	if (!json) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to parse as JSON: %s\n", ast_str_buffer(str));
		return -1;
	}
...
AST_MODULE_INFO_STANDARD_EXTENDED(GABPBX_GPL_KEY, "JSON decoding function");
