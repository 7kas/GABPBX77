...
static int get_lock(struct ast_channel *chan, char *lockname, int trylock)
{
	...
	if (!lock_store) {
		if (unloading) {
			ast_log_chan(NULL, LOG_ERROR, "%sLOCK has no datastore and func_lock is unloading, failing.\n",
					trylock ? "TRY" : "");
			return -1;
		}
		...
		if (!lock_store) {
			ast_log_chan(NULL, LOG_ERROR, "Unable to allocate new datastore.  No locks will be obtained.\n");
			return -1;
		}
	...
	if (!current) {
		...
		if ((res = ast_mutex_init(&current->mutex))) {
			ast_log_chan(NULL, LOG_ERROR, "Unable to initialize mutex: %s\n", strerror(res));
			ast_free(current);
			AST_LIST_UNLOCK(&locklist);
			return -1;
		}
		if ((res = ast_cond_init(&current->cond, NULL))) {
			ast_log_chan(NULL, LOG_ERROR, "Unable to initialize condition variable: %s\n", strerror(res));
...
static int unlock_read(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	if (!lock_store) {
		ast_log_chan(NULL, LOG_WARNING, "No datastore for dialplan locks.  Nothing was ever locked!\n");
...
static int unload_module(void)
{
	...
	while ((current = AST_LIST_REMOVE_HEAD(&locklist, entries))) {
		...
		while (current->owner || current->requesters) {
			if (!warned) {
				ast_log_chan(NULL, LOG_WARNING, "Waiting for %d requesters for %s lock %s.\n",
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Dialplan mutexes");
