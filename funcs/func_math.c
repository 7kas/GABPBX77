 ...
 * Updated by Naveen Albert <gabpbx@phreaknet.org>
 *
		...
		char *buf, size_t len)
{
	...
	if (ast_strlen_zero(parse)) {
		ast_log_chan(NULL, LOG_WARNING, "Syntax: MATH(<number1><op><number 2>[,<type_of_result>]) - missing argument!\n");
		return -1;
	}
	...
	if (mtype_of_result) {
		if (!strcasecmp(mtype_of_result, "float")
			...
			ast_log_chan(NULL, LOG_WARNING, "Unknown type of result requested '%s'.\n",
					mtype_of_result);
			return -1;
		}
	}
	...
	if (sscanf(mvalue1, "%30lf", &fnum1) != 1) {
		ast_log_chan(NULL, LOG_WARNING, "'%s' is not a valid number\n", mvalue1);
		return -1;
	}
                     ...
                     char *data, char *buf, size_t len)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "Syntax: %s(<data>) - missing argument!\n", cmd);
		return -1;
	}
	...
	if (!(var = pbx_builtin_getvar_helper(chan, data))) {
		ast_log_chan(NULL, LOG_NOTICE, "Failed to obtain variable %s, bailing out\n", data);
		ast_channel_unlock(chan);
		return -1;
	}

	if (ast_strlen_zero(var)) {
		ast_log_chan(NULL, LOG_NOTICE, "Variable %s doesn't exist - are you sure you wrote it correctly?\n", data);
		ast_channel_unlock(chan);
		return -1;
	}

	if (sscanf(var, "%30d%1c", &int_value, &endchar) == 0 || endchar != 0) {
		ast_log_chan(NULL, LOG_NOTICE, "The content of ${%s} is not a numeric value - bailing out!\n", data);
		ast_channel_unlock(chan);
		return -1;
	}
	...
	if (snprintf(returnvar, sizeof(returnvar), "%d", int_value) > 0) {
		...
		ast_log_chan(NULL, LOG_NOTICE, "Variable %s refused to be %sREMENTED, setting value to 0", data, cmd);
		ret = 0;
	}
			 ...
			 char *parse, char *buffer, size_t buflen)
{
	...
	if (ast_strlen_zero(args.num1) && ast_strlen_zero(args.num2)) {
		ast_log_chan(NULL, LOG_ERROR, "Missing argument for number(s).");
		return -1;
	}
			 ...
			 char *parse, char *buffer, size_t buflen)
{
	...
	if (ast_strlen_zero(args.num1) || sscanf(args.num1, "%30lf", &num1) != 1) {
		ast_log_chan(NULL, LOG_WARNING, "Bad or missing argument for number: %s", args.num1);
		return -1;
	}
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Mathematical dialplan function");
