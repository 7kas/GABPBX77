		...
		struct ast_frame *frame, enum ast_audiohook_direction direction)
{
	...
	if (ast_tvdiff_ms(now, state->last_hook) > state->interval * 1000) {
		if ((res = do_hook(chan, state))) {
			...
			ast_channel_unlock(chan);
			ast_log_chan(NULL, LOG_WARNING, "Failed to run hook on '%s'\n", name);
		}
		state->last_hook = now;
	}
...
static int hook_on(struct ast_channel *chan, const char *data, unsigned int hook_id)
{
	...
	if (ast_strlen_zero(args.interval) ||
		...
		ast_log_chan(NULL, LOG_WARNING, "Invalid hook interval: '%s'\n", S_OR(args.interval, ""));
		return -1;
	}

	if (ast_strlen_zero(args.context) || ast_strlen_zero(args.exten)) {
		ast_log_chan(NULL, LOG_WARNING, "A context and extension are required for PERIODIC_HOOK().\n");
		return -1;
	}
...
static int hook_off(struct ast_channel *chan, const char *hook_id)
{
	...
	if (!(datastore = ast_channel_datastore_find(chan, &hook_datastore, hook_id))) {
		ast_log_chan(NULL, LOG_WARNING, "Hook with ID '%s' not found on channel '%s'\n", hook_id,
...
static int hook_re_enable(struct ast_channel *chan, const char *uid)
{
	...
	if (!(datastore = ast_channel_datastore_find(chan, &hook_datastore, uid))) {
		ast_log_chan(NULL, LOG_WARNING, "Hook with ID '%s' not found on '%s'\n",
		...
		const char *value)
{
	...
	if (ast_false(value)) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Invalid value for PERIODIC_HOOK function: '%s'\n", value);
		res = -1;
	}
...
static int load_module(void)
{
	...
	if (!ast_context_find_or_create(NULL, NULL, context_name, AST_MODULE)) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to create %s dialplan context.\n", context_name);
		return AST_MODULE_LOAD_DECLINE;
	}

	/*
	 * Based on a handy recipe from the GABpbx Cookbook.
	 */
		...
		unsigned int interval, char *beep_id, size_t len)
{
	...
	if (hook_read(chan, NULL, args, beep_id, len)) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to enable periodic beep.\n");
		return -1;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS, "Periodic dialplan hooks.",
