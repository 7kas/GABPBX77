	...
	const char *cmd, char *data, struct ast_str **buf, ssize_t len)
{
	...
	if (ast_strlen_zero(parsed_data)) {
		ast_log_chan(NULL, AST_LOG_ERROR, "Cannot call %s without arguments\n", cmd);
		return -1;
	}
	...
	if (ast_strlen_zero(args.aor_name)) {
		ast_log_chan(NULL, AST_LOG_ERROR, "Cannot call %s without an AOR name to query\n", cmd);
		return -1;
	}

	if (ast_strlen_zero(args.field_name)) {
		ast_log_chan(NULL, AST_LOG_ERROR, "Cannot call %s with an empty field name to query\n", cmd);
		return -1;
	}

	pjsip_sorcery = ast_sip_get_sorcery();
	if (!pjsip_sorcery) {
		ast_log_chan(NULL, AST_LOG_ERROR, "Unable to retrieve PJSIP configuration: sorcery object is NULL\n");
		return -1;
	}

	aor_obj = ast_sorcery_retrieve_by_id(pjsip_sorcery, "aor", args.aor_name);
	if (!aor_obj) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Failed to retrieve information for AOR '%s'\n", args.aor_name);
		return -1;
	}
	...
	if (!strcmp(args.field_name, "contact")) {
		...
		if (!contacts) {
			ast_log_chan(NULL, LOG_WARNING, "Failed to retrieve contacts for AOR '%s'\n", args.aor_name);
			return -1;
		}
		...
		if (!change_set) {
			ast_log_chan(NULL, AST_LOG_WARNING, "Failed to retrieve information for AOR '%s': change set is NULL\n", args.aor_name);
			return -1;
		}
		...
		if (!it_change_set) {
			ast_log_chan(NULL, AST_LOG_WARNING, "Unknown property '%s' for PJSIP AOR\n", args.field_name);
			res = 1;
		}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_DEFAULT, "Get information about a PJSIP AOR",
