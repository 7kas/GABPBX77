	...
	const char *cmd, char *data, struct ast_str **buf, ssize_t len)
{
	...
	if (ast_strlen_zero(args.contact_name)) {
		ast_log_chan(NULL, AST_LOG_ERROR, "Cannot call %s without a contact name to query\n", cmd);
		return -1;
	}
	...
	if ((contact_name = strstr(args.contact_name, "@@"))) {
		...
		if (!aor_obj) {
			ast_log_chan(NULL, AST_LOG_WARNING, "Failed to retrieve information for contact '%s'\n", args.contact_name);
			return -1;
		}
	...
	if (!strcmp(args.field_name, "status")) {
		...
		if (!change_set) {
			ast_log_chan(NULL, AST_LOG_WARNING, "Failed to retrieve information for contact '%s': change set is NULL\n", args.contact_name);
			return -1;
		}
		...
		if (!it_change_set) {
			ast_log_chan(NULL, AST_LOG_WARNING, "Unknown property '%s' for PJSIP contact\n", args.field_name);
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_DEFAULT, "Get information about a PJSIP contact",
