	...
	const char *cmd, char *data, struct ast_str **buf, ssize_t len)
{
	...
	if (ast_strlen_zero(args.endpoint_name)) {
		ast_log_chan(NULL, AST_LOG_ERROR, "Cannot call %s without an endpoint name to query\n", cmd);
		return -1;
	}
	...
	if (!endpoint_obj) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Failed to retrieve information for endpoint '%s'\n", args.endpoint_name);
		return -1;
	}

	change_set = ast_sorcery_objectset_create(pjsip_sorcery, endpoint_obj);
	if (!change_set) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Failed to retrieve information for endpoint '%s': change set is NULL\n", args.endpoint_name);
		return -1;
	}
	...
	if (res) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Unknown property '%s' for PJSIP endpoint\n", args.field_name);
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_DEFAULT, "Get information about a PJSIP endpoint",
