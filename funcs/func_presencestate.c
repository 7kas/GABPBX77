			  ...
			  <optionlist>
			    <option name="e">
				  <para>On Write - Use this option when the subtype and message provided are Base64
					encoded. The values will be stored encoded within GABpbx, but all consumers of
...
static int presence_read(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "PRESENCE_STATE reading requires an argument \n");
		return -1;
	}
	...
	if (ast_strlen_zero(args.provider) || ast_strlen_zero(args.field)) {
		ast_log_chan(NULL, LOG_WARNING, "PRESENCE_STATE reading requires both presence provider and presence field arguments. \n");
		return -1;
	}

	state = ast_presence_state_nocache(args.provider, &subtype, &message);
	if (state == AST_PRESENCE_INVALID) {
		ast_log_chan(NULL, LOG_WARNING, "PRESENCE_STATE unknown \n");
		return -1;
	}
...
static int parse_data(char *data, enum ast_presence_state *state, char **subtype, char **message, char **options)
{
	...
	*state = ast_presence_state_val(state_str);
	...
	/* not a valid state */
	if (*state == AST_PRESENCE_INVALID) {
		ast_log_chan(NULL, LOG_WARNING, "Unknown presence state value %s\n", state_str);
		return -1;
	}
	...
	if (!ast_strlen_zero(*options) && !(strchr(*options, 'e'))) {
		ast_log_chan(NULL, LOG_NOTICE, "Invalid options  '%s'\n", *options);
		return -1;
	}
...
static int presence_write(struct ast_channel *chan, const char *cmd, char *data, const char *value)
{
	...
	if (strncasecmp(data, "CustomPresence:", len)) {
		ast_log_chan(NULL, LOG_WARNING, "The PRESENCE_STATE function can only set CustomPresence: presence providers.\n");
		return -1;
	}
	data += len;
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "PRESENCE_STATE function called with no custom device name!\n");
		return -1;
	}

	if (parse_data(args, &state, &subtype, &message, &options)) {
		ast_log_chan(NULL, LOG_WARNING, "Invalid arguments to PRESENCE_STATE\n");
		return -1;
	}
...
static char *handle_cli_presencestate_list(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	for (; db_entry; db_entry = db_entry->next) {
		...
		if (parse_data(state_info, &state, &subtype, &message, &options)) {
			ast_log_chan(NULL, LOG_WARNING, "Invalid CustomPresence entry %s encountered\n", db_entry->data);
			continue;
		}
...
AST_TEST_DEFINE(test_invalid_parse_data)
{
	...
	for (i = 0; i < ARRAY_LEN(tests); ++i) {
		...
		parse_result = parse_data(parse_string, &state, &subtype, &message, &options);
		if (parse_result == 0) {
			ast_log_chan(NULL, LOG_WARNING, "Invalid string parsing failed on %s\n", tests[i]);
			res = AST_TEST_FAIL;
			ast_free(parse_string);
			break;
		}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "Gets or sets a presence state in the dialplan",
