...
static int function_realtime_read(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "Syntax: REALTIME(family,fieldmatch[,matchvalue[,delim1[,delim2]]]) - missing argument!\n");
		return -1;
	}
	...
	if (resultslen > len) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to fetch. Realtime data is too large: need %zu, have %zu.\n", resultslen, len);
		if (chan) {
			ast_autoservice_stop(chan);
		}
		return -1;
...
static int function_realtime_write(struct ast_channel *chan, const char *cmd, char *data, const char *value)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "Syntax: %s(family,fieldmatch,matchvalue,updatecol) - missing argument!\n", cmd);
		return -1;
	}
	...
	if (res < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to update. Check the debug log for possible data repository related entries.\n");
	}
...
static int realtimefield_read(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "Syntax: %s(family,fieldmatch,matchvalue%s) - missing argument!\n", cmd, which == rtfield ? ",fieldname" : "");
		return -1;
	}
...
static int function_realtime_store(struct ast_channel *chan, const char *cmd, char *data, const char *value)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "Syntax: REALTIME_STORE(family,field1,field2,...,field30) - missing argument!\n");
		return -1;
	}
	...
	if (res < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to store. Check the debug log for possible data repository related entries.\n");
	} else {
...
static int function_realtime_readdestroy(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "Syntax: REALTIME_DESTROY(family,fieldmatch[,matchvalue[,delim1[,delim2]]]) - missing argument!\n");
		return -1;
	}
	...
	if (len > 0) {
		...
		if (resultslen > len) {
			 ...
			 * at it. */
			ast_log_chan(NULL, LOG_WARNING, "Failed to fetch/destroy. Realtime data is too large: need %zu, have %zu.\n", resultslen, len);
			if (chan) {
				ast_autoservice_stop(chan);
			}
			return -1;
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Read/Write/Store/Destroy values from a RealTime repository");
