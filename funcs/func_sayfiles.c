 ...
 *
 * Copyright (C) 2021, Naveen Albert <gabpbx@phreaknet.org>
 *
...
static int sayfile_exec(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "SAYFILES requires an argument\n");
		return 0;
	}
	...
	if (!strcmp(type, "alpha")) {
		...
		if (ast_str_to_int(value, &num)) {
			ast_log_chan(NULL, LOG_WARNING, "Invalid numeric argument: %s\n", value);
		} else {
		...
		ast_log_chan(NULL, LOG_WARNING, "Invalid say type specified: %s\n", type);
	}
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Say application files");
