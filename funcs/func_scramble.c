...
static int remove_scrambler(struct ast_channel *chan)
{
	...
	if (!datastore) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Cannot remove SCRAMBLE from %s: SCRAMBLE not currently enabled\n",
	...
	if (ast_audiohook_remove(chan, &data->audiohook)) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Failed to remove SCRAMBLE audiohook from channel %s\n", ast_channel_name(chan));
		return -1;
	}

	if (ast_channel_datastore_remove(chan, datastore)) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Failed to remove SCRAMBLE datastore from channel %s\n",
		        ast_channel_name(chan));
		return -1;
	}
...
static int scramble_write(struct ast_channel *chan, const char *cmd, char *data, const char *value)
{
	...
	if (!strcasecmp(args.direction, "tx")) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Direction must be either RX, TX, both, or remove\n");
		return -1;
	}
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Frequency inverting voice scrambler");
