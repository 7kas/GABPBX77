		                         ...
		                         char *buf, size_t len)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "Missing Argument!  Example:  Set(foo=${SHELL(echo \"bar\")})\n");
		return -1;
	}
	...
	if (len >= 1) {
		...
		if (ptr) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Failed to execute shell command '%s'\n", data);
			res = -1;
		}
	}
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Collects the output generated by a command executed by the system shell");
