	...
	const char *cmd, char *data, struct ast_str **buf, ssize_t len)
{
	...
	if (ast_strlen_zero(args.module_name)) {
		ast_log_chan(NULL, AST_LOG_ERROR, "Cannot call %s without a module name to query\n", cmd);
		return -1;
	}

	if (ast_strlen_zero(args.object_type)) {
		ast_log_chan(NULL, AST_LOG_ERROR, "Cannot call %s with an empty object type\n", cmd);
		return -1;
	}

	if (ast_strlen_zero(args.object_id)) {
		ast_log_chan(NULL, AST_LOG_ERROR, "Cannot call %s with an empty object name\n", cmd);
		return -1;
	}

	if (ast_strlen_zero(args.field_name)) {
		ast_log_chan(NULL, AST_LOG_ERROR, "Cannot call %s with an empty field name\n", cmd);
		return -1;
	}
		...
		if (strcmp(args.method, "concat") == 0) {
			...
			if (!ast_strlen_zero(args.method_arg)) {
				if (sscanf(args.method_arg, "%30d", &field_number) <= 0 || field_number <= 0 ) {
					ast_log_chan(NULL, AST_LOG_ERROR, "occurrence_number must be a positive integer\n");
					return -1;
				}
			}
		} else {
			ast_log_chan(NULL, AST_LOG_ERROR, "Retrieval method must be 'concat' or 'single'\n");
			return -1;
		}
	}
	...
	if (!sorcery) {
		ast_log_chan(NULL, AST_LOG_ERROR, "Failed to retrieve sorcery instance for module %s\n", args.module_name);
		return -1;
	}
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Get a field from a sorcery object");
