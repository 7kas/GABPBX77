...
static int speex_write(struct ast_channel *chan, const char *cmd, char *data, const char *value)
{
	...
	if (strcasecmp(data, "rx") && strcasecmp(data, "tx")) {
		ast_log_chan(NULL, LOG_ERROR, "Invalid argument provided to the %s function\n", cmd);
		return -1;
	}
	...
	if (!strcasecmp(cmd, "agc")) {
		...
		if ((*sdi)->agclevel > 32768.0) {
			ast_log_chan(NULL, LOG_WARNING, "AGC(%s)=%.01f is greater than 32768... setting to 32768 instead\n",
...
static int speex_read(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	if (!chan) {
		ast_log_chan(NULL, LOG_ERROR, "%s cannot be used without a channel!\n", cmd);
		return -1;
	}
	...
	if (!strcasecmp(data, "tx"))
		...
		ast_log_chan(NULL, LOG_ERROR, "%s(%s) must either \"tx\" or \"rx\"\n", cmd, data);
		return -1;
	}
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Noise reduction and Automatic Gain Control (AGC)");
