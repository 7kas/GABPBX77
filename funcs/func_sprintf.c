			...
			if (strchr("diouxXc", arg.format[i])) {
				...
				if (arg.var[argcount]) {
					if (sscanf(arg.var[argcount++], "%30d", &tmpi) != 1) {
						ast_log_chan(NULL, LOG_ERROR, "Argument '%s' is not an integer number for format '%s'\n", arg.var[argcount - 1], formatbuf);
						goto sprintf_fail;
					}
				} else {
					ast_log_chan(NULL, LOG_ERROR, "SPRINTF() has more format specifiers than arguments!\n");
					goto sprintf_fail;
				}
				...
				if (arg.var[argcount]) {
					if (sscanf(arg.var[argcount++], "%30lf", &tmpd) != 1) {
						ast_log_chan(NULL, LOG_ERROR, "Argument '%s' is not a floating point number for format '%s'\n", arg.var[argcount - 1], formatbuf);
						goto sprintf_fail;
					}
				} else {
					ast_log_chan(NULL, LOG_ERROR, "SPRINTF() has more format specifiers than arguments!\n");
				...
				*bufptr++ = arg.format[i];
				...
				ast_log_chan(NULL, LOG_ERROR, "Format type not supported: '%s' with argument '%s'\n", formatbuf, arg.var[argcount++]);
				goto sprintf_fail;
			}
			state = -1;
			break;
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "SPRINTF dialplan function");
