...
static struct srv_context *srv_datastore_setup(const char *service, struct ast_channel *chan)
{
	...
	if (ast_srv_lookup(&srds->context, service, &host, &port) < 0) {
		...
		ast_log_chan(NULL, LOG_NOTICE, "Error performing lookup of service '%s'\n", service);
		ast_free(srds);
		return NULL;
	}
	ast_autoservice_stop(chan);
...
static int srv_query_read(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	if (!chan) {
		ast_log_chan(NULL, LOG_WARNING, "%s cannot be used without a channel\n", cmd);
		return -1;
	}

	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "%s requires a service as an argument\n", cmd);
		return -1;
	}
...
static int srv_result_read(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "%s requires two arguments (id and resultnum)\n", cmd);
		return -1;
	}
	...
	if (ast_strlen_zero(args.field)) {
		ast_log_chan(NULL, LOG_ERROR, "A field must be provided when requesting SRV data\n");
		return -1;
	}

	if (sscanf(args.resultnum, "%30u", &num) != 1) {
		ast_log_chan(NULL, LOG_ERROR, "Invalid value '%s' for resultnum to %s\n", args.resultnum, cmd);
		return -1;
	}

	if (ast_srv_get_nth_record(srv_context, num, &host, &port, &priority, &weight)) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to get record number %u for %s\n", num, cmd);
		return -1;
	}
	...
	if (!strcasecmp(args.field, "host")) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Unrecognized SRV field '%s'\n", args.field);
		return -1;
	}
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "SRV related dialplan functions");
