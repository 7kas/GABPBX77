				...
				char *parse, char *buf, struct ast_str **sbuf, ssize_t len)
{
	...
	if (args.argc < 3) {
		ast_log_chan(NULL, LOG_ERROR, "Usage: FIELDNUM(<listname>,<delimiter>,<fieldvalue>)\n");
		res = -1;
	} else {
		varsubst = ast_alloca(strlen(args.varname) + 4);
		sprintf(varsubst, "${%s}", args.varname);
...
static int listfilter(struct ast_channel *chan, const char *cmd, char *parse, char *buf, struct ast_str **bufstr, ssize_t len)
{
	...
	if (args.argc < 3) {
		ast_log_chan(NULL, LOG_ERROR, "Usage: LISTFILTER(<listname>,<delimiter>,<fieldvalue>)\n");
		return -1;
	}
		  ...
		  size_t len)
{
	...
	if (!args.string) {
		ast_log_chan(NULL, LOG_ERROR, "Usage: FILTER(<allowed-chars>,<string>)\n");
		return -1;
	}

	if (args.allowed[0] == '"' && !ast_opt_dont_warn) {
		ast_log_chan(NULL, LOG_WARNING, "FILTER allowed characters includes the quote (\") character.  This may not be what you want.\n");
	}
	...
	for (; *(args.allowed);) {
		...
		if (*(args.allowed) == '-') {
			...
			if ((unsigned char) c2 < (unsigned char) c1 && !ast_opt_dont_warn) {
				ast_log_chan(NULL, LOG_WARNING, "Range wrapping in FILTER(%s,%s).  This may not be what you want.\n", parse, args.string);
			}
...
static int replace(struct ast_channel *chan, const char *cmd, char *data, struct ast_str **buf, ssize_t len)
{
	...
	if (args.argc < 2) {
		ast_log_chan(NULL, LOG_ERROR, "Usage: %s(<varname>,<search-chars>[,<replace-char>])\n", cmd);
		return -1;
	}
	...
	if (ast_strlen_zero(find) || ast_strlen_zero(args.varname)) {
		ast_log_chan(NULL, LOG_ERROR, "The characters to search for and the variable name must not be empty.\n");
		return -1;
	}
...
static int strreplace(struct ast_channel *chan, const char *cmd, char *data, struct ast_str **buf, ssize_t len)
{
	...
	if (ast_strlen_zero(args.find_string)) {
		ast_log_chan(NULL, LOG_ERROR, "No <find-string> specified\n");
		return -1;
	}
...
static int strbetween(struct ast_channel *chan, const char *cmd, char *data, struct ast_str **buf, ssize_t len)
{
	...
	if (!str) {
		ast_log_chan(NULL, LOG_ERROR, "Couldn't obtain string\n");
		return -1;
	}
	...
	if (args.argc != 2 || ast_strlen_zero(args.varname)) {
		ast_log_chan(NULL, LOG_ERROR, "Usage: %s(<varname>,<insert-string>)\n", cmd);
		return -1;
	}
		 ...
		 size_t len)
{
	...
	if (args.argc != 3) {
		ast_log_chan(NULL, LOG_ERROR, "Unexpected arguments: should have been in the form '\"<regex>\" <string>'\n");
		return -1;
	}
	...
	if ((errcode = regcomp(&regexbuf, args.reg, REG_EXTENDED | REG_NOSUB))) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Malformed input %s(%s): %s\n", cmd, parse, buf);
		return -1;
	}
...
static int hash_read(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	if (arg.argc == 2) {
		...
		if (!chan) {
			ast_log_chan(NULL, LOG_WARNING, "No channel and only 1 parameter was provided to %s function.\n", cmd);
			return -1;
		}
		...
		*buf = '\0';
		...
		for (i = 0; i < arg2.argc; i++) {
			 ...
			 */
			if (!varvalue) {
				ast_log_chan(NULL, LOG_WARNING, "No value found for '%s'\n", varname);
				*buf = '\0';
				return -1;
			}
...
static int quote(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	if (len < 3){ /* at least two for quotes and one for binary zero */
		ast_log_chan(NULL, LOG_ERROR, "Not enough buffer\n");
		return -1;
	}

	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "No argument specified!\n");
			...
			char *buf, size_t buflen)
{
	...
	if (ast_strftime(buf, buflen, args.format, &tm) <= 0)
		ast_log_chan(NULL, LOG_WARNING, "C function strftime() output nothing?!!\n");
			...
			char *buf, size_t buflen)
{
	...
	if (!data) {
		ast_log_chan(NULL, LOG_ERROR,
				"GABpbx function STRPTIME() requires an argument.\n");
		return -1;
	}
	...
	if (!ast_strptime(args.timestring, args.format, &tm)) {
		ast_log_chan(NULL, LOG_WARNING, "STRPTIME() found no time specified within the string\n");
	} else {
		struct timeval when;
...
{
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "EVAL requires an argument: EVAL(<string>)\n");
		return -1;
	}
...
#define beginning	(cmd[0] == 'S') /* SHIFT */
	...
	if (ast_strlen_zero(args.var)) {
		ast_log_chan(NULL, LOG_WARNING, "%s requires a variable name\n", cmd);
		return -1;
	}
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "String handling dialplan functions");
