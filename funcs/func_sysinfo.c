		                         ...
		                         char *buf, size_t len)
{
	...
	if (sysinfo(&sys_info)) {
		ast_log_chan(NULL, LOG_ERROR, "FAILED to retrieve system information\n");
		return -1;
	}
#endif
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "Syntax: ${SYSINFO(<parameter>)} - missing argument!)\n");
		return -1;
		...
		ast_log_chan(NULL, LOG_ERROR, "Unknown sysinfo parameter type '%s'.\n", data);
		return -1;
	}
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "System information related functions");
