...
/*** DOCUMENTATION
	<function name="TALK_DETECT" language="en_US">
		...
		<synopsis>
			Raises notifications when GABpbx detects silence or talking on a channel.
		</synopsis>
		<syntax>
		...
		<description>
			...
			potentially other GABpbx modules that listen for the internal
			notification.</para>
...
static int talk_detect_audiohook_cb(struct ast_audiohook *audiohook, struct ast_channel *chan, struct ast_frame *frame, enum ast_audiohook_direction direction)
{
	...
	if (update_talking) {
		...
		ast_verb_chan(NULL, 4, "%s is now %s\n", ast_channel_name(chan),
...
static int remove_talk_detect(struct ast_channel *chan)
{
	...
	if (!datastore) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Cannot remove TALK_DETECT from %s: TALK_DETECT not currently enabled\n",
		        ast_channel_name(chan));
		return -1;
	}
	td_params = datastore->data;
	...
	if (ast_audiohook_remove(chan, &td_params->audiohook)) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Failed to remove TALK_DETECT audiohook from channel %s\n",
		        ast_channel_name(chan));
		return -1;
	}

	if (ast_channel_datastore_remove(chan, datastore)) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Failed to remove TALK_DETECT datastore from channel %s\n",
		        ast_channel_name(chan));
		return -1;
	}
...
static int talk_detect_fn_write(struct ast_channel *chan, const char *function, char *data, const char *value)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, AST_LOG_WARNING, "TALK_DETECT requires an argument\n");
		return -1;
	}
	...
	if (!strcasecmp(data, "set")) {
		...
		if (!ast_strlen_zero(value)) {
			...
			if (!ast_strlen_zero(args.silence_threshold)) {
				if (sscanf(args.silence_threshold, "%30d", &dsp_silence_threshold) != 1) {
					ast_log_chan(NULL, AST_LOG_WARNING, "Failed to parse %s for dsp_silence_threshold\n",
					        args.silence_threshold);
					return -1;
				}

				if (dsp_silence_threshold < 1) {
					ast_log_chan(NULL, AST_LOG_WARNING, "Invalid value %d for dsp_silence_threshold\n",
			...
			if (!ast_strlen_zero(args.talking_threshold)) {
				if (sscanf(args.talking_threshold, "%30d", &dsp_talking_threshold) != 1) {
					ast_log_chan(NULL, AST_LOG_WARNING, "Failed to parse %s for dsp_talking_threshold\n",
					        args.talking_threshold);
					return -1;
				}

				if (dsp_talking_threshold < 1) {
					ast_log_chan(NULL, AST_LOG_WARNING, "Invalid value %d for dsp_talking_threshold\n",
		...
		ast_log_chan(NULL, AST_LOG_WARNING, "TALK_DETECT: unknown option %s\n", data);
		res = -1;
	}
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Talk detection dialplan function");
