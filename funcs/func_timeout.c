			...
			char *buf, size_t len)
{
	...
	if (!data) {
		ast_log_chan(NULL, LOG_ERROR, "Must specify type of timeout to get.\n");
		return -1;
	}
	...
	default:
		ast_log_chan(NULL, LOG_ERROR, "Unknown timeout type specified.\n");
		return -1;
	}
			 ...
			 const char *value)
{
	...
	if (!data) {
		ast_log_chan(NULL, LOG_ERROR, "Must specify type of timeout to set.\n");
		return -1;
	}
	...
	case 'A':
		...
		if (VERBOSITY_ATLEAST(3)) {
			if (!ast_tvzero(*ast_channel_whentohangup(chan))) {
				...
				ast_verb_chan(NULL, 3, "Channel will hangup at %s.\n", timestr);
			} else {
				ast_verb_chan(NULL, 3, "Channel hangup cancelled.\n");
			}
		}
		break;
	...
	case 'R':
		if (ast_channel_pbx(chan)) {
			...
			ast_verb_chan(NULL, 3, "Response timeout set to %.3f\n", ast_channel_pbx(chan)->rtimeoutms / 1000.0);
		}
		break;
	...
	case 'D':
		if (ast_channel_pbx(chan)) {
			...
			ast_verb_chan(NULL, 3, "Digit timeout set to %.3f\n", ast_channel_pbx(chan)->dtimeoutms / 1000.0);
		}
		break;
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Channel timeout dialplan functions");
