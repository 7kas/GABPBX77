...
static int volume_write(struct ast_channel *chan, const char *cmd, char *data, const char *value)
{
	...
	if (ast_strlen_zero(args.direction)) {
		ast_log_chan(NULL, LOG_ERROR, "Direction must be specified for VOLUME function\n");
		return -1;
	}
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Technology independent volume control");
