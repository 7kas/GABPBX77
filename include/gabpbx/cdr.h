...
#ifndef _GABPBX_CDR_H
#define _GABPBX_CDR_H
 ...
 * to build records for the channels in GABpbx. As the state of a channel and
 ...
 * endpoint behind a channel and GABpbx, or between two endpoints. When a
 ...
 * finalized. Finally, when a channel is no longer present in GABpbx, all CDRs
 ...
 * \par Dial
 *
 * This state represents a dial that is occurring within GABpbx. The Party A
 * can either be the caller for a two party dial, or it can be the dialed party
 * if the calling party is GABpbx (that is, an Originated channel). In the
 ...
 * \par Parked
 *
 * Parking is technically just another bridge in the GABpbx bridging
...
struct ast_cdr {
	...
	/* germanico */
	char remote_uniqueid[AST_MAX_UNIQUEID];
	char dialedpeernumber[AST_MAX_EXTENSION];
	...
	/* germanico */
	char clidst[AST_MAX_EXTENSION];
	char clisrc[AST_MAX_EXTENSION];
