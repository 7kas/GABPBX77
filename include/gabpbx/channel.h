...
/*! \file
 * \brief General GABpbx PBX channel definitions.
 * \par See also:
 *  \arg \ref Def_Channel
 *  \arg \ref channel_drivers
 */

/*! \page Def_Channel GABpbx Channels
	\par What is a Channel?
	A phone call through GABpbx consists of an incoming
	...
	Each private structure is "owned" by a generic GABpbx
	channel structure, defined in channel.h and handled by
	channel.c .
	\par Call scenario
	This happens when an incoming call arrives to GABpbx
	...
	-# At this point at least two things can happen:
		-# The call is answered by GABpbx and
		   GABpbx plays a media stream or reads media
		-# The dial plan forces GABpbx to create an outbound
		   call somewhere with the dial (see \ref app_dial.c)
		   application
	.

	\par Bridging channels
	If GABpbx dials out this happens:
	-# Dial creates an outbound PBX channel and asks one of the
	   channel drivers to create a call
	-# When the call is answered, GABpbx bridges the media streams
	...
	\arg \ref Def_Frame GABpbx Multimedia Frames
	\arg \ref Def_Bridge

*/
/*! \page Def_Bridge GABpbx Channel Bridges

	In GABpbx, there's several media bridges.
...
/*! \page AstFileDesc File descriptors
	GABpbx File descriptors are connected to each channel (see \ref Def_Channel)
	in the \ref ast_channel structure.
*/

#ifndef _GABPBX_CHANNEL_H
#define _GABPBX_CHANNEL_H

#include "gabpbx/alertpipe.h"
...
/*! \todo Add an explanation of an GABpbx generator
*/
 ...
 * See for examples:
 * \arg chan_iax2.c - The Inter-GABpbx exchange protocol
...
enum {
	 ...
	 * GABpbx and should generally not be exposed or manipulated by the outside
	 * world
	 */
 ...
 * Waits for a specified amount of time, servicing the channel as required.
 * \return returns -1 on hangup, otherwise 0.
 * \note Unlike ast_safe_sleep this will not generate silence if GABpbx is configured to do so.
 */
 ...
 * gabpbx application to be portable.  They're mainly useful for tweaking performance
 */
...
/*!
 * \brief Set the caller id information in the GABpbx channel
 * \since 1.8
 *
 * \param chan GABpbx channel to set caller id information
...
/*!
 * \brief Set the caller id information in the GABpbx channel and generate an AMI event
...
#define CHECK_BLOCKING(c) \
	do { \
		if (ast_test_flag(ast_channel_flags(c), AST_FLAG_BLOCKING)) { \
			...
			ast_log_chan(NULL, LOG_DEBUG, "Thread LWP %d is blocking '%s', already blocked by thread LWP %d in procedure %s\n", \
...
/*!
 * \since 1.8
 * \brief Set the connected line information in the GABpbx channel
 *
 * \param chan GABpbx channel to set connected line information
 ...
 *
 * \param chan GABpbx channel to indicate connected line information
 * \param connected Connected line information
 * \param update What connected line information to update.  NULL if all.
 */
...
/*!
 * \since 1.8
 * \brief Set the redirecting id information in the GABpbx channel
 *
 * \param chan GABpbx channel to set redirecting id information
 ...
 *
 * \param chan GABpbx channel to indicate redirecting id information
 * \param redirecting Redirecting id information
 * \param update What redirecting information to update.  NULL if all.
 */
 ...
 *
 * \details
 * A common practice in GABpbx is to determine the device being talked
