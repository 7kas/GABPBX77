...
/*!
\page GABpbxArchitecture GABpbx Architecture Overview
\author Russell Bryant <russell@digium.com>
...
This section of the documentation includes an overview of the GABpbx architecture
...
of what GABpbx is and how to use it.

The intent behind this documentation is to start looking at GABpbx from a high
level and progressively dig deeper into the details.  It begins with talking about
the different types of components that make up GABpbx and eventually will go
...
comments to the gabpbx-dev mailing list on http://lists.digium.com/.

Thank you, and enjoy GABpbx!


\section ArchLayout Modular Architecture

GABpbx is a highly modularized application.  There is a core application that
is built from the source in the <code>main/</code> directory.  However, it is
not very useful by itself.

There are many modules that are loaded at runtime.  GABpbx modules have names that
give an indication as to what functionality they provide, but the name is not special
in any technical sense.  When GABpbx loads a module, the module registers the
functionality that it provides with the GABpbx core.

 -# GABpbx starts
 -# GABpbx loads modules
 -# Modules say "Hey GABpbx!  I am a module.  I can provide functionality X, Y,
...
implementations of with the GABpbx core.  Any module is allowed to register as
...
convert between two codecs.  GABpbx currently only has the ability to translate
...
codec implementations are registered.  After modules have been loaded, GABpbx
...
definition in <code>include/gabpbx/translate.h</code>.
...
An implementation of the file format handler interface provides GABpbx the
...
tells the GABpbx core that it can handle files with a given %extension,
...
definition in <code>include/gabpbx/file.h</code>.
...
There are some C APIs in GABpbx that are optional.  Core APIs are built into
...
The GABpbx manager interface is a socket interface for monitoring and control
of GABpbx.  It is a core feature built in to the main application.  However,
...
The GABpbx CLI is a feature implemented in the main application.  Modules may
register additional CLI commands.

\subsection ArchInterfaceChannelDrivers Channel Drivers

The GABpbx channel driver interface is the most complex and most important
interface available.  The GABpbx channel API provides the telephony protocol
abstraction which allows all other GABpbx features to work independently of
...
represents one leg of a call (a connection between GABpbx and an endpoint).
...
<code>include/gabpbx/channel.h</code>.
...
the kernel (via DAHDI), via software methods inside of GABpbx, or could be done
...
\arg <code>include/gabpbx/bridge.h</code>
\arg <code>include/gabpbx/bridge_technology.h</code>
\arg <code>include/gabpbx/bridge_channel.h</code>
\arg <code>include/gabpbx/bridge_features.h</code>
\arg <code>include/gabpbx/bridge_after.h</code>
...
The GABpbx core implements functionality for keeping records of calls.  These
...
<code>include/gabpbx/cdr.h</code>.
...
The GABpbx core includes a generic event system that allows GABpbx components
...
<code>include/gabpbx/cel.h</code>.
...
executed from the GABpbx dialplan.  For example, in <code>extensions.conf</code>:
...
These applications use a %number of APIs available in GABpbx to interact with
...
For details on the API used to register an application with the GABpbx core, see
<code>include/gabpbx/pbx.h</code>.
...
used from the GABpbx dialplan.  Functions are used mostly in the same way that
...
For details on the API used to register a dialplan function with the GABpbx core,
see <code>include/gabpbx/pbx.h</code>.

\subsection ArchInterfaceRTP RTP Engines

The GABpbx core provides an API for handling RTP streams.  However, the actual
...
The GABpbx core implements an API that can be used by components that need access
...
For additional information on the timing API, see <code>include/gabpbx/timing.h</code>.

For additional implementation details for the timing API, see <code>main/timing.c</code>.


\section ArchThreadingModel GABpbx Threading Model

GABpbx is a very heavily multi threaded application.  It uses the POSIX threads API
to manage threads and related services such as locking.  Almost all of the GABpbx code
that interacts with pthreads does so by going through a set of wrappers used for
debugging and code reduction.

Threads in GABpbx can be classified as one of the following types:
...
A channel is a fundamental concept in GABpbx.  Channels are either inbound
or outbound.  An inbound channel is created when a call comes in to the GABpbx
system.  These channels are the ones that execute the GABpbx dialplan.  A thread
...
one of the primary tasks of the thread is to execute the GABpbx dialplan for an
inbound call.

A channel thread starts out by only being responsible for a single GABpbx channel.
...
dialplan functions from an asynchronous interface such as the GABpbx CLI or the
...
Network monitor threads exist in almost every major channel driver in GABpbx.
...
completed, the monitor threads will create an instance of an GABpbx channel
...
The GABpbx CLI also operates in a similar manner.  However, instead of TCP, the
GABpbx CLI operates using connections to a UNIX %domain socket.
...
For example, the event API (include/gabpbx/event.h) uses a thread internally
(main/event.c) to handle asynchronous event dispatching.  The devicestate API
(include/gabpbx/devicestate.h) uses a thread internally (main/devicestate.c)
to asynchronously process device state changes.


\section ArchConcepts Other Architecture Concepts

This section covers some other important GABpbx architecture concepts.
...
mentioned that most of the code in GABpbx that does bridging today does not use
...
two types of bridges that may occur.
 ...
 -# <b>Generic Bridge:</b> A generic bridge (ast_generic_bridge()) is a bridging
    ...
    all audio and signaling through the GABpbx abstract channel and frame interfaces
 ...
 -# <b>Native Bridge:</b> Channel drivers have the option of implementing their own
    ...
    a native bridge method is available, and GABpbx does not have a reason to force
    the call to stay in the core of GABpbx, then the native bridge function will be
    ...
    case of SIP, this means that GABpbx can direct the audio to flow between the
    endpoints and only require the signaling to continue to flow through GABpbx.


\section ArchCodeFlows Code Flow Examples

Now that there has been discussion about the various components that make up GABpbx,
...
This example consists of a call that comes in to GABpbx via the SIP protocol.
GABpbx accepts this call, plays back a %sound file to the caller, and then hangs up.
...
<code>exten => 5551212,n,Hangup()</code><br/>
 ...
 -# <b>Accept Call:</b> Once the SIP channel driver has completed the call setup process,
    it accepts the call and initiates the call handling process in GABpbx.  To do so,
 ...
 -# <b>Play the File:</b> The next step of the dialplan says to play back a %sound file
    ...
    -# <b>Open a File:</b> The file format API is responsible for opening the %sound file.
       ...
       to handle reading the file and turning it into internal GABpbx audio frames.
    ...
    -# <b>Feed Audio to the Caller:</b> The file API will invoke the timer API to know
       ...
       GABpbx must also continuously service the incoming audio from the channel since
...
This example consists of a call that comes in to GABpbx via the SIP protocol.  GABpbx
...
<code>exten => 5551212,n,Dial(IAX2/mypeer)</code><br/>
 ...
 -# <b>Execute Dial()</b>: The only step in this dialplan is to execute the
    ...
    -# <b>Create an Outbound Channel:</b> The <code>Dial()</code> application needs to
       ...
       is a part of the core channel API (include/gabpbx/channel.h).  It will find
    ...
    -# <b>Wait for Answer:</b> At this point, the Dial() application waits for the
       ...
       that does this is very similar to every other channel servicing loop in GABpbx.
    ...
    -# <b>Make Channels Compatible:</b> Before the two ends of the call can be connected,
       GABpbx must make them compatible to talk to each other.  Specifically, the two
...
\section ArchDataStructures GABpbx Data Structures

GABpbx provides generic implementations of a number of data structures.

\subsection ArchAstobj2 Astobj2

Astobj2 stands for the GABpbx Object model, version 2.  The API is defined in
include/gabpbx/astobj2.h.  Some internal implementation details for astobj2 can
...
GABpbx provides a set of macros for handling linked lists.  They are defined in
include/gabpbx/linkedlists.h.

\subsection ArchDLinkedLists Doubly Linked Lists

GABpbx provides a set of macros for handling doubly linked lists, as well.  They
are defined in include/gabpbx/dlinkedlists.h.

\subsection ArchHeap Heap

GABpbx provides an implementation of the max heap data structure.  The API is defined
in include/gabpbx/heap.h.  The internal implementation details can be found in
main/heap.c.


\section ArchDebugging GABpbx Debugging Tools

GABpbx includes a %number of built in debugging tools to help in diagnosing common
types of problems.

\subsection ArchThreadDebugging Thread Debugging

GABpbx keeps track of a list of all active threads on the system.  A list of threads
can be viewed from the GABpbx CLI by running the command
<code>core show threads</code>.

GABpbx has a compile time option called <code>DEBUG_THREADS</code>.  When this is on,
the pthread wrapper API in GABpbx keeps track of additional information related to
threads and locks to aid in debugging.  In addition to just keeping a list of threads,
GABpbx also maintains information about every lock that is currently held by any
...
This data can be acquired from the GABpbx CLI by running the
<code>core show locks</code> CLI command.

The definitions of these wrappers can be found in <code>include/gabpbx/lock.h</code>
and <code>include/gabpbx/utils.h</code>.  Most of the implementation details can be
found in <code>main/utils.c</code>.

\subsection ArchMemoryDebugging Memory debugging

Dynamic memory management in GABpbx is handled through a %number of wrappers defined
in <code>include/gabpbx/utils.h</code>.  By default, all of these wrappers use the
standard C library malloc(), free(), etc. functions.  However, if GABpbx is compiled
with the MALLOC_DEBUG option enabled, additional memory debugging is included.

The GABpbx memory debugging system provides the following features:
