...
\page AstFrameHookAPI GABpbx FrameHook API
...
\section FrameHookFunctionality How FrameHooks Work
	...
	channel API and the GABpbx core when going in the READ direction, and
...
\code
	...
	static struct ast_frame *event_cb(struct ast_channel *chan,
		...
		if (event == AST_FRAMEHOOK_EVENT_WRITE) {
			ast_log_chan(NULL, LOG_NOTICE, "YAY we received a frame in the write direction, Type: %d\n", frame->frametype)
			...
			ast_log_chan(NULL, LOG_NOTICE, "YAY we received a frame in the read direction: Type: %d\n", frame->frametype);
			ast_framehook_detach(chan, id); // the channel is guaranteed to be locked during this function call.
		}
