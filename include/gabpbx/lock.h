...
/*! \file
 * \brief GABpbx locking-related definitions:
...
/*! \page LockDef GABpbx thread locking models
 *
...
#ifndef _GABPBX_LOCK_H
#define _GABPBX_LOCK_H
...
/*!
 * \brief log info for the current lock with ast_log_chan(NULL, ).
 *
...
#define AO2_DEADLOCK_AVOIDANCE(obj) \
	do { \
		...
		if (__res < 0) { /* Could happen if the ao2 object does not have a mutex. */ \
			if (__res2) { \
				ast_log_chan(NULL, LOG_WARNING, "Could not unlock ao2 object '%s': %s and no lock info found!  I will NOT try to relock.\n", #obj, strerror(__res2)); \
			} else { \
				ao2_lock(obj); \
			} \
		} else { \
			if (__res2) { \
				ast_log_chan(NULL, LOG_WARNING, "Could not unlock ao2 object '%s': %s.  {{{Originally locked at %s line %d: (%s) '%s'}}}  I will NOT try to relock.\n", #obj, strerror(__res2), __filename, __lineno, __func, __mutex_name); \
			} else { \
...
#define CHANNEL_DEADLOCK_AVOIDANCE(chan) \
	do { \
		...
		if (__res < 0) { /* Shouldn't ever happen, but just in case... */ \
			if (__res2) { \
				ast_log_chan(NULL, LOG_WARNING, "Could not unlock channel '%s': %s and no lock info found!  I will NOT try to relock.\n", #chan, strerror(__res2)); \
			} else { \
				ast_channel_lock(chan); \
			} \
		} else { \
			if (__res2) { \
				ast_log_chan(NULL, LOG_WARNING, "Could not unlock channel '%s': %s.  {{{Originally locked at %s line %d: (%s) '%s'}}}  I will NOT try to relock.\n", #chan, strerror(__res2), __filename, __lineno, __func, __mutex_name); \
			} else { \
	...
	do { \
		...
		if (__res < 0) { /* Shouldn't ever happen, but just in case... */ \
			if (__res2 == 0) { \
				...
				ast_log_chan(NULL, LOG_WARNING, "Could not unlock mutex '%s': %s and no lock info found!  I will NOT try to relock.\n", #lock, strerror(__res2)); \
			} \
		} else { \
			if (__res2 == 0) { \
				__ast_pthread_mutex_lock(__filename, __lineno, __func, __mutex_name, lock); \
			} else { \
				ast_log_chan(NULL, LOG_WARNING, "Could not unlock mutex '%s': %s.  {{{Originally locked at %s line %d: (%s) '%s'}}}  I will NOT try to relock.\n", #lock, strerror(__res2), __filename, __lineno, __func, __mutex_name); \
			} \
		} \
	} while (0)
...
#define DEADLOCK_AVOIDANCE(lock) \
	do { \
		...
		if (!(__res = ast_mutex_unlock(lock))) { \
			...
			ast_log_chan(NULL, LOG_WARNING, "Failed to unlock mutex '%s' (%s).  I will NOT try to relock. {{{ THIS IS A BUG. }}}\n", #lock, strerror(__res)); \
		} \
	} while (0)
