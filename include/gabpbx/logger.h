...
#ifndef _GABPBX_LOGGER_H
#define _GABPBX_LOGGER_H

#include "gabpbx/options.h"	/* need option_debug */
...
void ast_log_chan(struct ast_channel *chan, int level, const char *file, int line, const char *function, const char *fmt, ...)
...
/*!
 * \brief Log a backtrace of the current thread's execution stack to the GABpbx log
 */
...
void __attribute__((format(printf, 6, 7))) __ast_verb_chan(struct ast_channel *chan, int level, const char *file, int line, const char *func, const char *fmt, ...);
...
#define ast_verb_chan(chan, level, ...) \
        do { \
                if (VERBOSITY_ATLEAST(level)) { \
                        __ast_verb_chan(chan, level, __FILE__, \
...
parameter in gabpbx.conf were added.  This allows a separate log channel to be created
...
1     --> app_voicemail_odbc.c:4483 store_file: dir: /var/spool/gabpbx/voicemail/default/1179/INBOX user: 1179 context: default msgnum: 8
2         app_voicemail_odbc.c:4502 store_file: Formats: wav49|gsm|wav Using format: 'WAV'
2         app_voicemail_odbc.c:4510 store_file: Base path: '/var/spool/gabpbx/voicemail/default/1179/INBOX/msg0008'
