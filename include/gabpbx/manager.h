...
#ifndef _GABPBX_MANAGER_H
#define _GABPBX_MANAGER_H
...
/*!
 \file
 \brief The AMI - GABpbx Manager Interface - is a TCP protocol created to
 manage GABpbx with third-party software.
...
#define DEFAULT_MANAGER_PORT 5038	/* Default port for GABpbx management via TCP */
#define DEFAULT_MANAGER_TLS_PORT 5039	/* Default port for GABpbx management via TCP */
...
#define EVENT_FLAG_TEST             (1 << 17) /* Test event used to signal the GABpbx Test Suite */
 ...
 * \brief Register a manager callback using XML documentation to describe the manager.
 *
 * \note For GABpbx core modules that are not independently
 * loadable.
 *
 * \warning If you use ast_manager_register_xml() instead when
 * you need to use this function, GABpbx will crash on load.
 */
...
/*! \brief External routines may send gabpbx manager events this way
...
/*! External routines may send gabpbx manager events this way
 * \param category Event category, matches manager authorization
 * \param event Event name
