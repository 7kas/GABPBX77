...
#ifndef _GABPBX_RES_FAX_H
#define _GABPBX_RES_FAX_H
...
struct ast_fax_session {
	...
	/*! name of the GABpbx channel using the fax session */
	char *channame;
	/*! unique ID of the GABpbx channel using the fax session */
	char *chan_uniqueid;
	/*! GABpbx channel using the fax session */
...
struct ast_fax_tech {
	...
	/*! sends an GABpbx frame to res_fax */
	struct ast_frame *(* const read)(struct ast_fax_session *);
	/*! writes an GABpbx frame to the fax session */
 ...
 *
 * The first four parameters can be represented with GABpbx's
