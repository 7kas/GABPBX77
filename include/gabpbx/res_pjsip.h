...
struct ast_sip_contact {
	...
	int via_port;
	/*! If true delete the contact on GABpbx restart/boot */
	int prune_on_boot;
...
/*!
 * \brief An entity with which GABpbx communicates
 */
struct ast_sip_endpoint {
	...
	AST_DECLARE_STRING_FIELDS(
		...
		AST_STRING_FIELD(user_agent_prefix);
	...
	/*! Pointer to the persistent GABpbx endpoint */
	...
	/*! The name of the geoloc profile to apply when GABpbx receives a call from this endpoint */
	AST_STRING_FIELD_EXTENDED(geoloc_incoming_call_profile);
	/*! The name of the geoloc profile to apply when GABpbx sends a call to this endpoint */
...
/*!
 * \brief Register a SIP service in GABpbx.
 *
 ...
 * callbacks, see the PJSIP documentation. GABpbx modules that call
 ...
 * This will likely occur when an GABpbx module is unloaded.
 *
 ...
 *
 * GABpbx provides a default authenticator, but it may be replaced by a
 ...
 * cannot be identified by one identifier, it may be identified by another.
 *
 * GABpbx provides two endpoint identifiers. One identifies endpoints based
 ...
 * \par GABpbx Threads
 *
 * GABpbx threads are those that originate from outside of SIP but within
 * GABpbx. The most common of these threads are PBX (channel) threads and
 ...
 * GABpbx data from outside of SIP, but any handling of SIP data should be
 * left to servants, \b especially if you wish to call into PJSIP for anything.
 * GABpbx threads are not registered with PJLIB, so attempting to call into
 ...
 * GABpbx automatically will push the request to a servant thread. When your
 ...
 * exist in order to do the work that GABpbx threads and PJSIP threads hand
...
/*!
 * \brief Replace domain and port of SIP URI to point to (external) signaling address of this GABpbx instance
 *
 * \param uri
 * \param tdata
 *
