...
/*!
 * \page AstRTPEngine GABpbx RTP Engine API
 *
 * The purpose of this API is to provide a way for multiple RTP stacks to be
 * used inside of GABpbx without any module that uses RTP knowing any
...
#ifndef _GABPBX_RTP_ENGINE_H
#define _GABPBX_RTP_ENGINE_H
...
/*!
 * Maximum size of an internal GABpbx channel unique ID.
 *
...
struct ast_rtp_payload_type {
	/*! If gabpbx_format is set, this is the internal
	 * gabpbx format represented by the payload */
	struct ast_format *format;
	/*! Is this an GABpbx value */
	int gabpbx_format;
...
struct ast_rtp_instance_stats {
	...
	/*! The GABpbx channel's unique ID that owns this instance */
...
/*! \brief Structure that represents the test functionality for res_rtp_gabpbx unit tests */
...
struct ast_rtp_engine {
	...
	/*! Callback for setting a payload.  If gabpbx  is to be used, gabpbx_format will be set, otherwise value in code is used. */
	void (*payload_set)(struct ast_rtp_instance *instance, int payload, int gabpbx_format, struct ast_format *format, int code);
	...
	void (*available_formats)(struct ast_rtp_instance *instance, struct ast_format_cap *to_endpoint, struct ast_format_cap *to_gabpbx, struct ast_format_cap *result);
 ...
 * \endcode
 *
 * This asks the RTP engine to read in RTP from the instance and return it as an GABpbx frame.
 *
 * \since 1.8
 */
 ...
 *
 * \param gabpbx_format True if the value in format is to be used.
 * \param format An gabpbx format
 ...
 *
 * \param codecs Codecs structure to look in
 * \param astformats A capabilities structure to put the GABpbx formats in.
 * \param nonastformats An integer to put the non-GABpbx formats in
 *
 * Example usage:
 *
 * \code
 ...
 * \endcode
 *
 * This retrieves all the formats known about in the codecs structure and puts the GABpbx ones in the integer
 * pointed to by astformats and the non-GABpbx ones in the integer pointed to by nonastformats.
 *
 * \since 1.8
 */
...
/*!
 * \brief Retrieve a rx mapped payload type based on whether it is an GABpbx format and the code
 *
 * \param codecs Codecs structure to look in
 ...
 * is an GABpbx format or non-format code.  If one is currently not
...
/*!
 * \brief Retrieve a rx mapped payload type based on whether it is an GABpbx format, the code and the sample rate.
 *
 * \param codecs Codecs structure to look in
 * \param gabpbx_format Non-zero if the given GABpbx format is present
 * \param format GABpbx format to look for
...
/*!
 * \brief Set a payload code for use with a specific GABpbx format
 *
 * \param codecs Codecs structure to manipulate
 * \param code The payload code
 * \param format GABpbx format
 *
...
/*!
 * \brief Set a payload code with sample rate for use with a specific GABpbx format
 *
 * \param codecs Codecs structure to manipulate
 * \param code The payload code
 * \param format GABpbx format
...
/*!
 * \brief Retrieve a tx mapped payload type based on whether it is an GABpbx format and the code
 * \since 14.0.0
 *
 * \param codecs Codecs structure to look in
 * \param gabpbx_format Non-zero if the given GABpbx format is present
 ...
 *
 * \param gabpbx_format Non-zero to look up using GABpbx format
 * \param format GABpbx format to look up
 ...
 * \param ast_format_capability GABpbx Formats we are looking up.
 * \param rtp_capability RTP codes that we are looking up
 * \param gabpbx_format Non-zero if the ast_format_capability structure is to be used, 0 if rtp_capability is to be used
 * \param options Additional options that may change the result
 *
 * \retval non-NULL success
 * \retval NULL failure
 ...
 *
 * \param chan Our own GABpbx channel
 * \param instance The first RTP instance
 * \param peer The peer GABpbx channel
 *
 * \retval 0 success
 * \retval -1 failure
 *
 ...
 * \param to_gabpbx Formats being sent/received towards GABpbx
 ...
 * \endcode
 *
 * This sees if it is possible to have ulaw communicated to the endpoint but signed linear received into GABpbx.
 *
 * \since 1.8
 */
...
/*!
 * \since 12
 * \brief Message type for an RTCP message sent from this GABpbx instance
 *
