...
#ifndef _GABPBX_STRINGS_H
#define _GABPBX_STRINGS_H
...
static force_inline int _ast_strlen_zero(const char *s, const char *file, const char *function, int line)
{
	...
	if (!strcmp(s, "(null)")) {
		ast_log_chan(NULL, __LOG_WARNING, file, line, function, "Possible programming error: \"(null)\" is not NULL!\n");
	}
	return 0;
}
 ...
 * ast_log_chan(NULL, LOG_INFO, "Format caps: %s\n", ast_format_cap_get_names(caps, &temp));
 * ast_free(temp);
 * \endcode
 *
 ...
 *
 * \code
 * ast_log_chan(NULL, LOG_INFO, "Format caps in: %s  Format caps out: %s\n",
 ...
 *
 * \details
 * Hash values used all over gabpbx are expected to be non-negative
