...
int ast_jb_put(struct ast_channel *chan, struct ast_frame *f)
{
	...
	if (!ast_test_flag(f, AST_FRFLAG_HAS_TIMING_INFO) || f->len < 2 || f->ts < 0) {
		ast_log_chan(NULL, LOG_WARNING, "%s received frame with invalid timing info: "
	...
	if (!frr) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to isolate frame for the jitterbuffer on channel '%s'\n", ast_channel_name(chan));
		return -1;
	}
...
static void jb_get_and_deliver(struct ast_channel *chan)
{
	...
	while (now >= jb->next) {
		...
		case AST_JB_IMPL_NOFRAME:
			...
			return;
		default:
			ast_log_chan(NULL, LOG_ERROR, "This should never happen!\n");
...
static int create_jb(struct ast_channel *chan, struct ast_frame *frr)
{
	...
	if (!jbobj) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to create jitterbuffer on channel '%s'\n", ast_channel_name(chan));
		return -1;
	}
	...
	if (res != AST_JB_IMPL_OK) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to put first frame in the jitterbuffer on channel '%s'\n", ast_channel_name(chan));
		/*
	...
	if (ast_test_flag(jbconf, AST_JB_LOG)) {
		...
		if (safe_fd < 0 || link(safe_logfile, logfile_pathname) || unlink(safe_logfile) || !(jb->logfile = fdopen(safe_fd, "w+b"))) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to create frame log file with pathname '%s': %s\n", logfile_pathname, strerror(errno));
			jb->logfile = NULL;
			if (safe_fd > -1) {
				close(safe_fd);
			}
	...
	ast_verb_chan(NULL, 3, "%s jitterbuffer created on channel %s\n", jbimpl->name, ast_channel_name(chan));
...
void ast_jb_destroy(struct ast_channel *chan)
{
	...
	if (ast_test_flag(jb, JB_CREATED)) {
		...
		ast_verb_chan(NULL, 3, "%s jitterbuffer destroyed on channel %s\n", jbimpl->name, ast_channel_name(chan));
	}
}
...
static struct timeval jitterbuffer_frame_get_ntp_timestamp(const struct jb_stream_sync *stream_sync, const struct ast_frame *frame)
{
	...
	/* Convert the GABpbx timestamp into an RTP timestamp, and then based on the difference we can
...
static struct ast_frame *hook_event_cb(struct ast_channel *chan, struct ast_frame *frame, enum ast_framehook_event event, void *data)
{
	...
	if (ast_channel_fdno(chan) == AST_JITTERBUFFER_FD && framedata->timer) {
		if (ast_timer_ack(framedata->timer, 1) < 0) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to acknowledge timer in jitter buffer\n");
			return frame;
		}
	}
...
static int jb_framedata_init(struct jb_framedata *framedata, struct ast_jb_conf *jb_conf)
{
	...
	if (!ast_strlen_zero(jb_conf->impl)) {
		if (!strcasecmp(jb_conf->impl, "fixed")) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Unknown Jitterbuffer type %s. Failed to create jitterbuffer.\n", jb_conf->impl);
			return -1;
		}
	}
