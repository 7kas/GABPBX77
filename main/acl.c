...
struct ast_acl_list *ast_duplicate_acl_list(struct ast_acl_list *original)
{
	...
	if (!(clone = ast_calloc(1, sizeof(*clone)))) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to allocate ast_acl_list struct while cloning an ACL\n");
		return NULL;
	}
	...
	AST_LIST_TRAVERSE(original, current_cursor, list) {
		if ((acl_new(&current_clone, current_cursor->name))) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to allocate ast_acl struct while cloning an ACL.\n");
		...
		if (current_cursor->acl && !current_clone->acl) {
			...
			ast_log_chan(NULL, LOG_ERROR, "Failed to duplicate HA list while cloning ACL.\n");
			ast_free_acl_list(clone);
			clone = NULL;
			break;
		}
	...
	while ((tmp = strsep(&list, ","))) {
		...
		AST_LIST_TRAVERSE(working_list, current, list) {
			if (!strcasecmp(current->name, tmp)) { /* ACL= */
				...
				ast_log_chan(NULL, LOG_ERROR, "Named ACL '%s' occurs multiple times in ACL definition. "
...
static void debug_ha_sense_appended(struct ast_ha *ha)
{
	...
	ast_log_chan(NULL, LOG_DEBUG, "%s/%s sense %u appended to ACL\n",
...
static struct ast_ha *append_ha_core(const char *sense, const char *stuff, struct ast_ha *path, int *error, int port_flags)
{
	...
	while ((tmp = strsep(&list, ","))) {
		...
		if (!ast_sockaddr_parse(&ha->addr, address, port_flags)) {
			ast_log_chan(NULL, LOG_WARNING, "Invalid IP address: %s\n", address);
			ast_free_ha(ha);
			if (error) {
				*error = 1;
			}
		...
		if (ast_sockaddr_ipv4_mapped(&ha->addr, &ha->addr)) {
			ast_log_chan(NULL, LOG_NOTICE, "IPv4-mapped ACL network address specified. "
		...
		if (!mask) {
			...
			if (!ast_sockaddr_parse(&ha->netmask, mask, PARSE_PORT_FORBID)) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid netmask: %s\n", mask);
				ast_free_ha(ha);
				if (error) {
					*error = 1;
				}
			...
			if (ast_sockaddr_ipv4_mapped(&ha->netmask, &ha->netmask)) {
				ast_log_chan(NULL, LOG_NOTICE, "IPv4-mapped ACL netmask specified. "
					"Converting to an IPv4 ACL netmask.\n");
			}
			mask_is_v4 = ast_sockaddr_is_ipv4(&ha->netmask);
			if (addr_is_v4 ^ mask_is_v4) {
				ast_log_chan(NULL, LOG_WARNING, "Address and mask are not using same address scheme.\n");
				ast_free_ha(ha);
				if (error) {
					*error = 1;
				}
			...
			ast_log_chan(NULL, LOG_WARNING, "Invalid CIDR netmask: %s\n", mask);
			ast_free_ha(ha);
			if (error) {
				*error = 1;
			}
		...
		if (ast_sockaddr_apply_netmask(&ha->addr, &ha->netmask, &ha->addr)) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Unable to apply netmask %s to address %s\n", failmask, failaddr);
			ast_free_ha(ha);
			if (error) {
				*error = 1;
			}
...
static enum ast_acl_sense ast_apply_acl_internal(struct ast_acl_list *acl_list, const struct ast_sockaddr *addr, const char *log_prefix)
{
	...
	AST_LIST_TRAVERSE(acl_list, acl, list) {
		if (acl->is_invalid) {
			...
			if (log_prefix) {
				ast_log_chan(NULL, LOG_WARNING, "%sRejecting '%s' due to use of an invalid ACL '%s'.\n",
		...
		if (acl->acl) {
			if (ast_apply_ha(acl->acl, addr) == AST_SENSE_DENY) {
				if (log_prefix) {
					ast_log_chan(NULL, LOG_NOTICE, "%sRejecting '%s' due to a failure to pass ACL '%s'\n",
							log_prefix, ast_sockaddr_stringify_addr(addr),
							ast_strlen_zero(acl->name) ? "(BASELINE)" : acl->name);
				}
				AST_LIST_UNLOCK(acl_list);
			...
			if (ast_sockaddr_is_ipv6(addr)) {
				if (ast_sockaddr_is_ipv4_mapped(addr)) {
					...
					if (!ast_sockaddr_ipv4_mapped(addr, &mapped_addr)) {
						ast_log_chan(NULL, LOG_ERROR, "%s provided to ast_sockaddr_ipv4_mapped could not be converted. That shouldn't be possible.\n",
								  ...
								  int family, struct ast_sockaddr *preference)
{
	...
	if (addrs_cnt > 0) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Unable to lookup '%s'\n", name);
		return -1;
	}
...
int ast_ouraddrfor(const struct ast_sockaddr *them, struct ast_sockaddr *us)
{
	...
	if (s < 0) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Cannot create socket to %s: %s\n",
	...
	if (ast_connect(s, them)) {
		sock_err = ast_strdupa(strerror(errno));
		ast_log_chan(NULL, LOG_WARNING, "Cannot connect to %s: %s\n",
			ast_sockaddr_stringify_addr(them), sock_err);
		close(s);
		return -1;
	}
	if (ast_getsockname(s, us)) {
		sock_err = ast_strdupa(strerror(errno));
		ast_log_chan(NULL, LOG_WARNING, "Cannot get socket name for connection to %s: %s\n",
			ast_sockaddr_stringify_addr(them), sock_err);
		close(s);
		return -1;
	}
...
int ast_find_ourip(struct ast_sockaddr *ourip, const struct ast_sockaddr *bindaddr, int family)
{
	...
	if (gethostname(ourhost, sizeof(ourhost) - 1)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to get hostname\n");
	} else {
