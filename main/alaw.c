...
void ast_alaw_init(void)
{
	...
	for (i = -32768; i < 32768; ++i) {
		...
		if (e1 != e2 || d1 != d3 || d2 != d3) {
			ast_log_chan(NULL, LOG_WARNING, "a-Law coding tables test failed on %d: e1=%u, e2=%u, d1=%d, d2=%d\n",
					i, (unsigned)e1, (unsigned)e2, (int)d1, (int)d2);
		}
	}
	ast_log_chan(NULL, LOG_NOTICE, "a-Law coding tables test complete.\n");
	...
	for (i = -32768; i < 32768; ++i) {
		...
		if (e1 != e2 || e2 != e3 || d1 != d2 || d2 != d3) {
			ast_log_chan(NULL, LOG_WARNING, "a-Law tandem transcoding test failed on %d: e1=%u, e2=%u, d1=%d, d2=%d, d3=%d\n",
					i, (unsigned)e1, (unsigned)e2, (int)d1, (int)d2, (int)d3);
		}
	}
	ast_log_chan(NULL, LOG_NOTICE, "a-Law tandem transcoding test complete.\n");
