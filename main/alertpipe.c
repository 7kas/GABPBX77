...
int ast_alertpipe_init(int alert_pipe[2])
{
	...
	ast_log_chan(NULL, LOG_WARNING, "Failed to create alert pipe with eventfd(), falling back to pipe(): %s\n",
		strerror(errno));
	...
	if (pipe2(alert_pipe, O_NONBLOCK)) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to create alert pipe: %s\n", strerror(errno));
		return -1;
	}
...
ast_alert_status_t ast_alertpipe_flush(int alert_pipe[2])
{
	...
	for (;;) {
		...
		if (bytes_read < 0) {
			...
			ast_log_chan(NULL, LOG_WARNING, "read() failed flushing alertpipe: %s\n",
				strerror(errno));
			return AST_ALERT_READ_FAIL;
		}
		if (!bytes_read) {
