		...
		const enum ast_aoc_request requests)
{
	...
	if (((unsigned int) charge_type > AST_AOC_CHARGE_UNIT) ||
		...
		ast_log_chan(NULL, LOG_WARNING, "Failed to create ast_aoc_decoded object, invalid input\n");
		return NULL;
	}

	if (!(decoded = ast_calloc(1, sizeof(struct ast_aoc_decoded)))) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to create ast_aoc_decoded object \n");
		return NULL;
	}
...
static int aoc_parse_ie(struct ast_aoc_decoded *decoded, unsigned char *data, unsigned int datalen)
{
	...
	while (datalen >= 2) {
		...
		len = data[1];
		if (len > datalen -2) {
			ast_log_chan(NULL, LOG_ERROR, "AOC information element length exceeds the total message size\n");
			return -1;
		}
		...
		case AOC_IE_CURRENCY:
			if (len == sizeof(struct aoc_ie_currency)) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Received invalid currency ie\n");
			}
			break;
		case AOC_IE_UNIT:
			if (len == sizeof(struct aoc_ie_unit)) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Received invalid unit ie\n");
			}
			break;
		case AOC_IE_BILLING:
			if (len == sizeof(struct aoc_ie_billing)) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Received invalid billing ie\n");
			}
			break;
		case AOC_IE_CHARGING_ASSOCIATION:
			if (len == sizeof(struct aoc_ie_charging_association)) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Received invalid charging association ie\n");
			}
			break;
		case AOC_IE_RATE:
			if (len == sizeof(struct aoc_ie_charging_rate)) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Received invalid charging rate ie\n");
			}
			break;
		case AOC_IE_TERMINATION_REQUEST:
			if (len == 0) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Received invalid termination request ie\n");
			}
			break;
		default:
			ast_log_chan(NULL, LOG_WARNING, "Unknown AOC Information Element, ignoring.\n");
		}
...
struct ast_aoc_decoded *ast_aoc_decode(struct ast_aoc_encoded *encoded, size_t size, struct ast_channel *chan)
{
	...
	if ((size - (sizeof(struct ast_aoc_encoded)) != ntohs(encoded->datalen))) {
		ast_log_chan(NULL, LOG_WARNING, "Corrupted aoc encoded object, can not decode\n");
		return NULL;
	}
...
static int aoc_append_ie(struct aoc_ie_data *ied, unsigned short ie_id, const void *data, unsigned short datalen)
{
	if (datalen > ((int)sizeof(ied->buf) - ied->pos)) {
		ast_log_chan(NULL, LOG_WARNING, "Failure to append AOC information element, out of space \n");
		return -1;
	}
...
struct ast_aoc_encoded *ast_aoc_encode(struct ast_aoc_decoded *decoded, size_t *out_size, struct ast_channel *chan)
{
	...
	*out_size = 0;
	...
	if (!(encoded = ast_calloc(1, size))) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to create ast_aoc_encoded object during decode routine. \n");
		return NULL;
	}
