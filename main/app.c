...
int ast_app_dtget(struct ast_channel *chan, const char *context, char *collect, size_t size, int maxlen, int timeout)
{
	...
	if ((ts = ast_get_indication_tone(ast_channel_zone(chan), "dial"))) {
		...
		ast_log_chan(chan, LOG_NOTICE, "Huh....? no dial for indications?\n");
	}
...
int ast_app_exec_sub(struct ast_channel *autoservice_chan, struct ast_channel *sub_chan, const char *sub_args, int ignore_hangup)
{
	...
	if (!funcs || !funcs->run_sub || !ast_module_running_ref(funcs->module)) {
		ast_log_chan(autoservice_chan, LOG_WARNING,
...
int __ast_vm_register(const struct ast_vm_functions *vm_table, struct ast_module *module)
{
	...
	if (!vm_table->module_name) {
		ast_log_chan(NULL, LOG_ERROR, "Voicemail provider missing required information.\n");
		return -1;
	}
	if (vm_table->module_version != VM_MODULE_VERSION) {
		ast_log_chan(NULL, LOG_ERROR, "Voicemail provider '%s' has incorrect version\n",
	...
	table = ao2_global_obj_ref(vm_provider);
	if (table) {
		ast_log_chan(NULL, LOG_WARNING, "Voicemail provider already registered by %s.\n",
			table->module_name);
...
void ast_vm_test_swap_table_in(const struct ast_vm_functions *vm_table)
{
	...
	if (provider_is_swapped) {
		ast_log_chan(NULL, LOG_ERROR, "Attempted to swap in test function table without swapping out old test table.\n");
		return;
	}
...
void ast_vm_test_swap_table_out(void)
{
	...
	if (!provider_is_swapped) {
		ast_log_chan(NULL, LOG_ERROR, "Attempted to swap out test function table, but none is currently installed.\n");
		return;
	}
...
int __ast_vm_greeter_register(const struct ast_vm_greeter_functions *vm_table, struct ast_module *module)
{
	...
	if (!vm_table->module_name) {
		ast_log_chan(NULL, LOG_ERROR, "Voicemail greeter provider missing required information.\n");
		return -1;
	}
	if (vm_table->module_version != VM_GREETER_MODULE_VERSION) {
		ast_log_chan(NULL, LOG_ERROR, "Voicemail greeter provider '%s' has incorrect version\n",
			vm_table->module_name);
		return -1;
	}
	...
	table = ao2_global_obj_ref(vm_greeter_provider);
	if (table) {
		ast_log_chan(NULL, LOG_WARNING, "Voicemail greeter provider already registered by %s.\n",
			table->module_name);
		return AST_MODULE_LOAD_DECLINE;
	}
...
{
	if (vm_warnings++ % 10 == 0) {
		ast_verb_chan(NULL, 3, "No voicemail provider registered.\n");
	}
}
...
static void vm_greeter_warn_no_provider(void)
{
	if (vm_greeter_warnings++ % 10 == 0) {
		ast_verb_chan(NULL, 3, "No voicemail greeter provider registered.\n");
	}
}
...
static int sf_stream(struct ast_channel *chan, struct ast_channel *chan2, const char *digits, int frequency, int is_external)
{
	...
	if (frequency >= 100000) {
		ast_log_chan(chan, LOG_WARNING, "Frequency too large: %d\n", frequency);
		return -1;
	}
...
#define SF_BUF_LEN  20
	...
	for (ptr = digits; *ptr; ptr++) {
		if (*ptr == 'w') {
			...
			ast_log_chan(chan, LOG_WARNING, "Illegal SF character '%c' in string. (0-9A-DwWfFhH allowed)\n", *ptr);
		}
	}
	...
	unsigned int durationkp, unsigned int durationst, int is_external)
{
	...
	for (ptr = digits; *ptr; ptr++) {
		if (*ptr == 'w') {
			...
			ast_log_chan(chan, LOG_WARNING, "Illegal MF character '%c' in string. (0-9*#ABCwWfFhH allowed)\n", *ptr);
		}
	}
...
static int dtmf_stream(struct ast_channel *chan, const char *digits, int between, unsigned int duration, int is_external)
{
	...
	for (ptr = digits; *ptr; ptr++) {
		if (*ptr == 'w') {
			...
			ast_log_chan(chan, LOG_WARNING, "Illegal DTMF character '%c' in string. (0-9*#aAbBcCdD allowed)\n", *ptr);
		}
	}
...
static void linear_release(struct ast_channel *chan, void *params)
{
	...
	if (ls->origwfmt && ast_set_write_format(chan, ls->origwfmt)) {
		ast_log_chan(chan, LOG_WARNING, "Unable to restore channel '%s' to format '%s'\n",
...
static int linear_generator(struct ast_channel *chan, void *data, int len, int samples)
{
	...
	if (len > sizeof(buf) - AST_FRIENDLY_OFFSET) {
		ast_log_chan(chan, LOG_WARNING, "Can't generate %d bytes of data!\n" , len);
...
static void *linear_alloc(struct ast_channel *chan, void *params)
{
	...
	if (ast_set_write_format(chan, ast_format_slin)) {
		ast_log_chan(chan, LOG_WARNING, "Unable to set '%s' to linear format (write)\n", ast_channel_name(chan));
		ao2_cleanup(ls->origwfmt);
		ast_free(ls);
		ls = params = NULL;
	}
...
int ast_linear_stream(struct ast_channel *chan, const char *filename, int fd, int allowoverride)
{
	...
	if (fd < 0) {
		...
		if (fd < 0) {
			ast_log_chan(chan, LOG_WARNING, "Unable to open file '%s': %s\n", tmpf, strerror(errno));
			return -1;
		}
	}
	...
	for (;;) {
		...
		if (!res) {
			if (pause_restart_point) {
				...
				ast_verb_chan(chan, 3, "ControlPlayback seek to offset %ld from end\n", offset);

				ast_seekstream(ast_channel_stream(chan), offset, SEEK_END);
				end = NULL;
				offset = 0;
			} else if (offset) {
				ast_verb_chan(chan, 3, "ControlPlayback seek to offset %ld\n", offset);
...
static enum control_tone_frame_response_result control_tone_frame_response(struct ast_channel *chan, struct ast_frame *fr, struct ast_tone_zone_sound *ts, const char *tone, int *paused)
{
	...
	case AST_CONTROL_STREAM_REVERSE:
		ast_log_chan(chan, LOG_NOTICE, "Media control operation 'reverse' not supported for media type 'tone'\n");
		return CONTROL_TONE_RESPONSE_NORMAL;
	case AST_CONTROL_STREAM_FORWARD:
		ast_log_chan(chan, LOG_NOTICE, "Media control operation 'forward' not supported for media type 'tone'\n");
		return CONTROL_TONE_RESPONSE_NORMAL;
	...
	*tone_indication = strsep(&tone_parser, ";");
	...
	if (!(strncmp(tone_parser, "tonezone=", 9))) {
		*tone_zone = tone_parser + 9;
		...
		ast_log_chan(NULL, LOG_ERROR, "Unexpected Tone URI component: %s\n", tone_parser);
		return -1;
	}
...
static struct ast_frame *make_silence(const struct ast_frame *orig)
{
	...
	do {
		if (ast_format_cmp(orig->subclass.format, ast_format_slin) == AST_FORMAT_CMP_NOT_EQUAL) {
			ast_log_chan(NULL, LOG_WARNING, "Attempting to silence non-slin frame\n");
			return NULL;
		}
	...
	ast_verb_chan(NULL, 4, "Silencing %zu samples\n", samples);
	...
	int skip_confirmation_sound, enum ast_record_if_exists if_exists)
{
	...
	if (!duration) {
		ast_log_chan(chan, LOG_WARNING, "Error play_and_record called without duration pointer\n");
		return -1;
	}
	...
	while ((fmt = strsep(&stringp, "|"))) {
		if (fmtcnt > AST_MAX_FORMATS - 1) {
			ast_log_chan(chan, LOG_WARNING, "Please increase AST_MAX_FORMATS in file.h\n");
			break;
		}
		/*
	...
	if (maxsilence > 0) {
		...
		if (!sildet) {
			ast_log_chan(chan, LOG_WARNING, "Unable to create silence detector :(\n");
			return -1;
		}
		ast_dsp_set_threshold(sildet, silencethreshold);
		res = set_read_to_slin(chan, &rfmt);
		if (res < 0) {
			ast_log_chan(chan, LOG_WARNING, "Unable to set to linear mode, giving up\n");
			ast_dsp_free(sildet);
			ao2_cleanup(rfmt);
			return -1;
		}
	...
	if (x == fmtcnt) {
		...
		for (;;) {
			if (!(res = ast_waitfor(chan, 2000))) {
				...
				if (!(res = ast_waitfor(chan, 2000))) {
					ast_log_chan(chan, LOG_WARNING, "No audio available on %s??\n", ast_channel_name(chan));
					res = -1;
				}
			}
			...
			if (f->frametype == AST_FRAME_VOICE) {
				...
				if (paused) {
					...
					if (muted) {
						...
						if (!silence) {
							ast_log_chan(chan, LOG_WARNING, "Error creating silence\n");
							break;
						}
						f = silence;
					}
				...
				if (maxsilence > 0) {
					...
					if (dspsilence > maxsilence) {
						...
						ast_verb_chan(chan, 3, "Recording automatically stopped after a silence of %d seconds\n", dspsilence/1000);
						res = 'S';
						outmsg = 2;
						break;
					}
				...
				/* Exit on any error */
				if (res) {
					ast_log_chan(chan, LOG_WARNING, "Error writing frame\n");
					break;
				}
				...
				/* stop recording with any digit */
					ast_verb_chan(chan, 3, "User ended message by pressing %c\n", f->subclass.integer);
					res = 't';
					outmsg = 2;
					break;
				}
				...
				if (strchr(canceldtmf, f->subclass.integer)) {
					ast_verb_chan(chan, 3, "User canceled message by pressing %c\n", f->subclass.integer);
					res = f->subclass.integer;
					outmsg = 0;
					break;
				}
				...
				if (f->subclass.integer == AST_CONTROL_RECORD_CANCEL) {
					ast_verb_chan(chan, 3, "Message canceled by control\n");
					outmsg = 0; /* cancels the recording */
					res = 0;
					break;
				} else if (f->subclass.integer == AST_CONTROL_RECORD_STOP) {
					ast_verb_chan(chan, 3, "Message ended by control\n");
					res = 0;
					break;
				} else if (f->subclass.integer == AST_CONTROL_RECORD_SUSPEND) {
					paused = !paused;
					ast_verb_chan(chan, 3, "Message %spaused by control\n",
						paused ? "" : "un");
					if (paused) {
					...
					muted = !muted;
					ast_verb_chan(chan, 3, "Message %smuted by control\n",
						muted ? "" : "un");
					 ...
					 */
					if (muted && !rfmt) {
						ast_verb_chan(chan, 3, "Setting read format to linear mode\n");
						res = set_read_to_slin(chan, &rfmt);
						if (res < 0) {
							ast_log_chan(chan, LOG_WARNING, "Unable to set to linear mode, giving up\n");
							break;
			...
			if (maxtime && !paused) {
				...
				if (maxtime < (end - start - paused_secs)) {
					ast_verb_chan(chan, 3, "Took too long, cutting it short...\n");
					res = 't';
					outmsg = 2;
					break;
				}
		...
		if (!f) {
			ast_verb_chan(chan, 3, "User hung up\n");
			res = -1;
			outmsg = 1;
		} else {
			ast_frfree(f);
		...
		ast_log_chan(chan, LOG_WARNING, "Error creating writestream '%s', format '%s'\n", recordfile, sfmt[x]);
	}
	...
	*duration = others[0] ? ast_tellstream(others[0]) / 8000 : 0;
	...
	if (rfmt && ast_set_read_format(chan, rfmt)) {
		ast_log_chan(chan, LOG_WARNING, "Unable to restore format %s to channel '%s'\n", ast_format_get_name(rfmt), ast_channel_name(chan));
	}
	ao2_cleanup(rfmt);
...
int ast_app_group_match_get_count(const char *groupmatch, const char *category)
{
	...
	if (ast_strlen_zero(groupmatch)) {
		ast_log_chan(NULL, LOG_NOTICE, "groupmatch empty\n");
		return 0;
	}

	/* if regex compilation fails, return zero matches */
	if (regcomp(&regexbuf_group, groupmatch, REG_EXTENDED | REG_NOSUB)) {
		ast_log_chan(NULL, LOG_ERROR, "Regex compile failed on: %s\n", groupmatch);
		return 0;
	}
...
static enum AST_LOCK_RESULT ast_lock_path_lockfile(const char *path)
{
	...
	if (fd < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to create lock file '%s': %s\n", path, strerror(errno));
	...
	if (res) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to lock path '%s': %s\n", path, strerror(errno));
...
static int ast_unlock_path_lockfile(const char *path)
{
	...
	if ((res = unlink(s))) {
		ast_log_chan(NULL, LOG_ERROR, "Could not unlock path '%s': %s\n", path, strerror(errno));
	} else {
...
static enum AST_LOCK_RESULT ast_lock_path_flock(const char *path)
{
	...
	if (lstat(fs, &st) == 0) {
		if ((st.st_mode & S_IFMT) == S_IFLNK) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to create lock file "
	...
	if ((fd = open(fs, O_WRONLY | O_CREAT, 0600)) < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to create lock file '%s': %s\n",
				fs, strerror(errno));
		return AST_LOCK_PATH_NOT_FOUND;
	}
	...
	if (lstat(fs, &st) != 0 && fstat(pl->fd, &ost) != 0 &&
		...
		ast_log_chan(NULL, LOG_WARNING, "Unable to create lock file '%s': "
...
int ast_record_review(struct ast_channel *chan, const char *playfile, const char *recordfile, int maxtime, const char *fmt, int *duration, const char *path)
{
	...
	/* barf if no pointer passed to store duration in */
	if (!duration) {
		ast_log_chan(chan, LOG_WARNING, "Error ast_record_review called without duration pointer\n");
		return -1;
	}
	...
	while ((cmd >= 0) && (cmd != 't')) {
		...
		case '2':
			/* Review */
			ast_verb_chan(chan, 3, "Reviewing the recording\n");
			cmd = ast_stream_and_wait(chan, recordfile, AST_DIGIT_ANY);
			break;
		case '3':
			message_exists = 0;
			/* Record */
			ast_verb_chan(chan, 3, "R%secording\n", recorded == 1 ? "e-r" : "");
			recorded = 1;
...
static int ivr_dispatch(struct ast_channel *chan, struct ast_ivr_option *option, char *exten, void *cbdata)
{
	...
	case AST_ACTION_BACKGROUND:
		...
		if (res < 0) {
			ast_log_chan(chan, LOG_NOTICE, "Unable to find file '%s'!\n", (char *)option->adata);
			res = 0;
		}
		return res;
	...
	case AST_ACTION_BACKLIST:
		...
		return res;
	default:
		ast_log_chan(chan, LOG_NOTICE, "Unknown dispatch function %u, ignoring!\n", option->action);
		return 0;
	}
	return -1;
}
...
static int ast_ivr_menu_run_internal(struct ast_channel *chan, struct ast_ivr_menu *menu, void *cbdata)
{
	...
	if (option_exists(menu, "s") < 0) {
		...
		if (option_exists(menu, "g") < 0) {
			ast_log_chan(chan, LOG_WARNING, "No 's' nor 'g' extension in menu '%s'!\n", menu->title);
			return -1;
		}
	}
	while (!res) {
...
char *ast_read_textfile(const char *filename)
{
	...
	if (stat(filename, &filesize) == -1) {
		ast_log_chan(NULL, LOG_WARNING, "Error can't stat %s\n", filename);
		return NULL;
	}
	...
	if ((fd = open(filename, O_RDONLY)) < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Cannot open file '%s' for reading: %s\n", filename, strerror(errno));
		return NULL;
	}
	...
	if ((output = ast_malloc(count))) {
		...
		if (res == count - 1) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Short read of %s (%d of %d): %s\n", filename, res, count - 1, strerror(errno));
			ast_free(output);
			output = NULL;
		}
	}
...
static int parse_options(const struct ast_app_option *options, void *_flags, char **args, char *optstr, int flaglen)
{
	...
	while (*s) {
		...
		if (*s == '(') {
			...
			if ((s = strchr(s, ')'))) {
				...
				*s++ = '\0';
				...
				ast_log_chan(NULL, LOG_WARNING, "Missing closing parenthesis for argument '%c' in string '%s'\n", curarg, arg);
				res = -1;
				break;
			}
		} else if (argloc) {
		...
		if (!options[curarg].flag) {
			ast_log_chan(NULL, LOG_WARNING, "Unrecognized option: '%c'\n", curarg);
		}
		if (flaglen == 32) {
		...
		case 'x':
			...
			if (strchr("0123456789ABCDEFabcdef", *(stream + 2)) && *(stream + 2) != '\0') {
				*consumed = 3;
				...
				ast_log_chan(NULL, LOG_ERROR, "Illegal character '%c' in hexadecimal string\n", *(stream + 2));
				return -1;
			}
...
int ast_safe_fork(int stop_reaper)
{
	...
	if (pid != 0) {
			...
			if (cur) {
				...
				if (shaun_of_the_dead_thread == AST_PTHREADT_NULL) {
					if (ast_pthread_create_background(&shaun_of_the_dead_thread, NULL, shaun_of_the_dead, NULL)) {
						ast_log_chan(NULL, LOG_ERROR, "Shaun of the Dead wants to kill zombies, but can't?!!\n");
		...
		if (pthread_sigmask(SIG_UNBLOCK, &signal_set, NULL)) {
			ast_log_chan(NULL, LOG_WARNING, "unable to unblock signals: %s\n", strerror(errno));
			_exit(1);
		}
