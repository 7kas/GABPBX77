/*
 * astobj2 - replacement containers for gabpbx data structures.
 *
...
int __ao2_lock(void *user_data, enum ao2_lock_req lock_how, const char *file, const char *func, int line, const char *var)
{
	...
	case AO2_ALLOC_OPT_LOCK_OBJ:
		...
		break;
	default:
		ast_log_chan(NULL, __LOG_ERROR, file, line, func, "Invalid lock option on ao2 object %p\n",
			user_data);
		return -1;
	}
...
enum ao2_lock_req __adjust_lock(void *user_data, enum ao2_lock_req lock_how, int keep_stronger)
{
	...
	case AO2_ALLOC_OPT_LOCK_OBJ:
		...
		break;
	default:
		ast_log_chan(NULL, LOG_ERROR, "Invalid lock option on ao2 object %p\n", user_data);
		/* Fall through */
	case AO2_ALLOC_OPT_LOCK_NOLOCK:
	case AO2_ALLOC_OPT_LOCK_MUTEX:
...
#define EXCESSIVE_REF_COUNT		100000
		...
		if (EXCESSIVE_REF_COUNT <= current_value && ret < EXCESSIVE_REF_COUNT) {
			...
			ast_log_chan(NULL, __LOG_ERROR, file, line, func, "%s\n", excessive_ref_buf);
	...
	if (current_value < 0) {
		ast_log_chan(NULL, __LOG_ERROR, file, line, func,
	...
	void *lockobj, const char *tag, const char *file, int line, const char *func)
{
	...
	case AO2_ALLOC_OPT_LOCK_OBJ:
		...
		if (!lockobj) {
			ast_log_chan(NULL, __LOG_ERROR, file, line, func, "AO2_ALLOC_OPT_LOCK_OBJ requires a non-NULL lockobj.\n");
			return NULL;
		}
	...
	default:
		...
		ast_log_chan(NULL, __LOG_DEBUG, file, line, func, "Invalid lock option requested\n");
		return NULL;
	}
	...
	const char *tag, const char *file, int line, const char *func)
{
	...
	if (data_size < sizeof(*weakproxy)) {
		ast_assert(0);
		ast_log_chan(NULL, LOG_ERROR, "Requested data_size smaller than minimum.\n");
		return NULL;
	}
...
int astobj2_init(void)
{
	...
	if (ast_opt_ref_debug) {
		...
		if (!ref_log) {
			ast_log_chan(NULL, LOG_ERROR, "Could not open ref debug log file: %s\n", ref_filename);
		}
	}
