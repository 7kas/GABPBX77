...
static void hash_ao2_node_destructor(void *v_doomed)
{
	...
	if (doomed->common.is_linked) {
		...
		if (!my_container->common.destroying
			...
			ast_log_chan(NULL, LOG_ERROR, "Container integrity failed before node deletion.\n");
		}
#endif	/* defined(AO2_DEBUG) */
...
static void hash_ao2_destroy(struct ao2_container_hash *self)
{
	...
	for (idx = self->n_buckets; idx--;) {
		if (!AST_DLLIST_EMPTY(&self->buckets[idx].list)) {
			ast_log_chan(NULL, LOG_ERROR, "Node ref leak.  Hash container still has nodes!\n");
			ast_assert(0);
			break;
		}
	}
...
static int hash_ao2_integrity(struct ao2_container_hash *self)
{
	...
	for (bucket = 0; bucket < self->n_buckets; ++bucket) {
		...
		if (!node) {
			ast_log_chan(NULL, LOG_ERROR, "Bucket %d list tail is NULL when it should not be!\n",
				bucket);
			return -1;
		}
		if (AST_DLLIST_NEXT(node, links)) {
			ast_log_chan(NULL, LOG_ERROR, "Bucket %d list tail node is not the last node!\n",
				bucket);
			return -1;
		}
		node = AST_DLLIST_FIRST(&self->buckets[bucket].list);
		if (!node) {
			ast_log_chan(NULL, LOG_ERROR, "Bucket %d list head is NULL when it should not be!\n",
				bucket);
			return -1;
		}
		if (AST_DLLIST_PREV(node, links)) {
			ast_log_chan(NULL, LOG_ERROR, "Bucket %d list head node is not the first node!\n",
				bucket);
			return -1;
		}
		for (; node; node = next) {
			...
			if (prev) {
				if (prev == node) {
					ast_log_chan(NULL, LOG_ERROR, "Bucket %d list node's prev pointer points to itself!\n",
						bucket);
					return -1;
				}
				if (node != AST_DLLIST_NEXT(prev, links)) {
					ast_log_chan(NULL, LOG_ERROR, "Bucket %d list node's prev node does not link back!\n",
						bucket);
					return -1;
				}
			} else if (node != AST_DLLIST_FIRST(&self->buckets[bucket].list)) {
				ast_log_chan(NULL, LOG_ERROR, "Bucket %d backward list chain is broken!\n",
					bucket);
				return -1;
			}
			...
			if (next) {
				if (next == node) {
					ast_log_chan(NULL, LOG_ERROR, "Bucket %d list node's next pointer points to itself!\n",
						bucket);
					return -1;
				}
				if (node != AST_DLLIST_PREV(next, links)) {
					ast_log_chan(NULL, LOG_ERROR, "Bucket %d list node's next node does not link back!\n",
						bucket);
					return -1;
				}
			} else if (node != AST_DLLIST_LAST(&self->buckets[bucket].list)) {
				ast_log_chan(NULL, LOG_ERROR, "Bucket %d forward list chain is broken!\n",
					bucket);
				return -1;
			}

			if (bucket != node->my_bucket) {
				ast_log_chan(NULL, LOG_ERROR, "Bucket %d node claims to be in bucket %d!\n",
			...
			if (bucket != bucket_exp) {
				ast_log_chan(NULL, LOG_ERROR, "Bucket %d node hashes to bucket %d!\n",
					bucket, bucket_exp);
				return -1;
			}
			...
			if (self->common.sort_fn) {
				if (obj_last
					...
					ast_log_chan(NULL, LOG_ERROR, "Bucket %d nodes out of sorted order!\n",
						bucket);
					return -1;
				}
		...
		if (count_obj != self->buckets[bucket].elements) {
			ast_log_chan(NULL, LOG_ERROR, "Bucket %d object count of %d does not match stat of %d!\n",
	...
	if (count_total_node != self->common.nodes) {
		ast_log_chan(NULL, LOG_ERROR, "Total node count of %d does not match stat of %d!\n",
