...
static void rb_ao2_node_destructor(void *v_doomed)
{
	...
	if (doomed->common.is_linked) {
		...
		if (!my_container->common.destroying
			&& ao2_container_check(doomed->common.my_container, OBJ_NOLOCK)) {
			ast_log_chan(NULL, LOG_ERROR, "Container integrity failed after node deletion.\n");
		}
#endif	/* defined(AO2_DEBUG) */
	}
...
static void rb_ao2_destroy(struct ao2_container_rbtree *self)
{
	...
	if (self->root) {
		ast_log_chan(NULL, LOG_ERROR, "Node ref leak.  Red-Black tree container still has nodes!\n");
		ast_assert(0);
	}
}
...
static int rb_ao2_integrity(struct ao2_container_rbtree *self)
{
	...
	if (self->root) {
		...
		if (self->root->parent) {
			if (self->root->parent == self->root) {
				ast_log_chan(NULL, LOG_ERROR, "Tree root parent pointer points to itself!\n");
			} else {
				ast_log_chan(NULL, LOG_ERROR, "Tree root is not a root node!\n");
			}
			return -1;
		}
		if (self->root->is_red) {
			/* Violation rbtree property 2. */
			ast_log_chan(NULL, LOG_ERROR, "Tree root is red!\n");
			res = -1;
		}
		node = self->root;
		do {
			if (node->left) {
				if (node->left == node) {
					ast_log_chan(NULL, LOG_ERROR, "Tree node's left pointer points to itself!\n");
					return -1;
				}
				if (node->left->parent != node) {
					ast_log_chan(NULL, LOG_ERROR, "Tree node's left child does not link back!\n");
					return -1;
				}
			}
			if (node->right) {
				if (node->right == node) {
					ast_log_chan(NULL, LOG_ERROR, "Tree node's right pointer points to itself!\n");
					return -1;
				}
				if (node->right->parent != node) {
					ast_log_chan(NULL, LOG_ERROR, "Tree node's right child does not link back!\n");
					return -1;
				}
			}
			...
			if (node->is_red) {
				...
				if (node->left && node->right) {
					...
					if (node->left->is_red) {
						...
						ast_log_chan(NULL, LOG_ERROR, "Tree node is red and its left child is red!\n");
						res = -1;
					}
					if (node->right->is_red) {
						/* Violation rbtree property 4. */
						ast_log_chan(NULL, LOG_ERROR, "Tree node is red and its right child is red!\n");
						res = -1;
					}
					 ...
					 */
					ast_log_chan(NULL, LOG_ERROR, "Tree node is red and it only has one child!\n");
					res = -1;
				}
			} else {
				/*
				...
				if (node->left && node->right) {
					...
					/* Violation rbtree property 5. */
					ast_log_chan(NULL, LOG_ERROR, "Tree node is black and its only child is black!\n");
					res = -1;
				}
			}
		...
		for (node = rb_node_most_left(self->root); node; node = rb_node_next(node)) {
			...
			if (obj_last) {
				if (self->common.sort_fn(obj_last, node->common.obj, OBJ_SEARCH_OBJECT) > 0) {
					ast_log_chan(NULL, LOG_ERROR, "Tree nodes are out of sorted order!\n");
					return -1;
				}
			}
	...
	if (count_obj != ao2_container_count(&self->common)) {
		ast_log_chan(NULL, LOG_ERROR, "Total object count does not match ao2_container_count()!\n");
		return -1;
	}
	...
	const char *tag, const char *file, int line, const char *func)
{
	...
	if (!sort_fn) {
		/* Sanity checks. */
		ast_log_chan(NULL, __LOG_ERROR, file, line, func, "Missing sort_fn()!\n");
		return NULL;
	}
