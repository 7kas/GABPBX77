...
#include "gabpbx/_private.h" /* prototype for ast_autoservice_init() */
...
static void *autoservice_run(void *ign)
{
	...
	while (!asexit) {
		...
		AST_LIST_TRAVERSE(&aslist, as, list) {
			if (!ast_check_hangup(as->chan)) {
				if (x < MAX_AUTOMONS) {
					...
					ast_log_chan(NULL, LOG_WARNING, "Exceeded maximum number of automatic monitoring events.  Fix autoservice.c\n");
				}
			}
		}
		...
		if (!x) {
			 ...
			 * problems when GABpbx runs at a different priority than other
...
int ast_autoservice_start(struct ast_channel *chan)
{
	...
	if (asthread == AST_PTHREADT_NULL) { /* need start the thread */
		if (ast_pthread_create_background(&asthread, NULL, autoservice_run, NULL)) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to create autoservice thread :(\n");
