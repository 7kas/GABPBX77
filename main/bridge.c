...
#include "gabpbx/bridge_internal.h"
#include "gabpbx/bridge_channel_internal.h"
#include "gabpbx/bridge_features.h"
#include "gabpbx/bridge_basic.h"
#include "gabpbx/bridge_technology.h"
#include "gabpbx/bridge_channel.h"
...
int __ast_bridge_technology_register(struct ast_bridge_technology *technology, struct ast_module *module)
{
	...
	if (ast_strlen_zero(technology->name)
		...
		|| !technology->write) {
		ast_log_chan(NULL, LOG_WARNING, "Bridge technology %s failed registration sanity check.\n",
			technology->name);
		return -1;
	}
	...
	AST_RWLIST_TRAVERSE(&bridge_technologies, current, entry) {
		if ((!strcasecmp(current->name, technology->name)) || (current == technology)) {
			ast_log_chan(NULL, LOG_WARNING, "A bridge technology of %s already claims to exist in our world.\n",
				technology->name);
	...
	ast_verb_chan(NULL, 5, "Registered bridge technology %s\n", technology->name);
...
int ast_bridge_technology_unregister(struct ast_bridge_technology *technology)
{
	...
	AST_RWLIST_TRAVERSE_SAFE_BEGIN(&bridge_technologies, current, entry) {
		if (current == technology) {
			...
			ast_verb_chan(NULL, 5, "Unregistered bridge technology %s\n", technology->name);
			break;
		}
	}
	AST_RWLIST_TRAVERSE_SAFE_END;
...
struct ast_bridge *bridge_register(struct ast_bridge *bridge)
{
	if (bridge) {
		...
		if (existing) {
			ast_log_chan(NULL, LOG_WARNING, "Bridge " BRIDGE_PRINTF_SPEC ": already registered\n",
				BRIDGE_PRINTF_VARS(bridge));
			ao2_ref(existing, -1);
		...
		if (!ao2_link(bridges, bridge)) {
			ast_log_chan(NULL, LOG_WARNING, "Bridge " BRIDGE_PRINTF_SPEC ": failed to link\n",
				BRIDGE_PRINTF_VARS(bridge));
			ast_bridge_destroy(bridge, 0);
			bridge = NULL;
		}
...
struct ast_bridge *bridge_alloc(size_t size, const struct ast_bridge_methods *v_table)
{
	...
	if (!v_table
		...
		ast_log_chan(NULL, LOG_ERROR, "Virtual method table for bridge class %s not complete.\n",
...
struct ast_bridge *bridge_base_init(struct ast_bridge *self, uint32_t capabilities, unsigned int flags, const char *creator, const char *name, const char *id)
{
...

	{
		...
		if (!(flags & AST_BRIDGE_FLAG_INVISIBLE)) {
			if (bridge_topics_init(self) != 0) {
				ast_log_chan(NULL, LOG_WARNING, "Bridge " BRIDGE_PRINTF_SPEC ": Could not initialize topics\n",
					BRIDGE_PRINTF_VARS(self));
				ao2_ref(self, -1);
				return NULL;
			}
	...
	if (!self->technology) {
		ast_log_chan(NULL, LOG_WARNING, "Bridge %s: Could not create class %s.  No technology to support it.\n",
	...
	if (self->technology->create && self->technology->create(self)) {
		ast_log_chan(NULL, LOG_WARNING, "Bridge %s: failed to setup bridge technology %s\n",
			self->uniqueid, self->technology->name);
		ao2_ref(self, -1);
		return NULL;
	}
	...
	if (self->technology->start && self->technology->start(self)) {
		ast_log_chan(NULL, LOG_WARNING, "Bridge %s: failed to start bridge technology %s\n",
			self->uniqueid, self->technology->name);
		ao2_ref(self, -1);
		return NULL;
	}
...
static int smart_bridge_operation(struct ast_bridge *bridge)
{
	...
	if (!new_technology) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Bridge %s has no technology available to support it.\n",
			bridge->uniqueid);
		return -1;
	}
	 ...
	 */
	ast_verb_chan(NULL, 4, "Bridge %s: switching from %s technology to %s\n",
...
/*
 * XXX GABPBX-22366 this needs more work.  The channels need
...
/*
 * XXX GABPBX-21271 make ast_bridge_join() require features to be allocated just like ast_bridge_impart() and not expect the struct back.
 *
...
/* XXX GABPBX-21271 features cannot be NULL when passed in. When it is changed to allocated we can do like ast_bridge_impart() and allocate one. */
	...
	struct bridge_channel_impart_cond *cond)
{
	...
	if (ast_channel_pbx(chan)) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Channel %s has a PBX thread and cannot be imparted into bridge %s\n",
			ast_channel_name(chan), bridge->uniqueid);
	...
	ast_channel_lock(chan);
	if (ast_test_flag(ast_channel_flags(chan), AST_FLAG_ZOMBIE)) {
		ast_log_chan(NULL, AST_LOG_NOTICE, "Channel %s is a zombie and cannot be imparted into bridge %s\n",
			ast_channel_name(chan), bridge->uniqueid);
		res = -1;
	} else {
		ast_channel_internal_bridge_channel_set(chan, bridge_channel);
...
int ast_bridge_depart(struct ast_channel *chan)
{
	...
	ast_channel_unlock(chan);
	if (!departable) {
		ast_log_chan(NULL, LOG_ERROR, "Channel %s cannot be departed.\n",
			ast_channel_name(chan));
		/*
	...
	struct ast_bridge_features *features, int play_tone, const char *xfersound)
{
	...
	if (chan_bridge) {
		...
		if (!yanked_chan) {
			ast_log_chan(NULL, LOG_WARNING, "Could not gain control of channel %s\n", ast_channel_name(chan));
			ast_bridge_features_destroy(features);
			return -1;
		}
	...
	if (play_tone && !ast_strlen_zero(xfersound)) {
		...
		if (!play_bridge_channel) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to play tone for channel %s. No longer in a bridge.\n",
	...
	struct ast_unreal_pvt *pvt)
{
	...
	if (other && other->state == BRIDGE_CHANNEL_STATE_WAIT) {
		...
		ast_verb_chan(NULL, 4, "Move-swap optimizing %s <-- %s.\n",
	...
	struct ast_unreal_pvt *pvt)
{
	...
	ast_verb_chan(NULL, 4, "Merge optimizing %s -- %s out.\n",
		ast_channel_name(chan_bridge_channel->chan),
		ast_channel_name(peer_bridge_channel->chan));
...
{
	struct ast_bridge_channel *bridge_channel;
/* XXX GABPBX-21271 the case of a dissolved bridge while channel is suspended is not handled. */
/* XXX GABPBX-21271 suspend/unsuspend needs to be rethought. The caller must block until it has successfully suspended the channel for temporary control. */
/* XXX GABPBX-21271 external suspend/unsuspend needs to be eliminated. The channel may be playing a file at the time and stealing it then is not good. */
	...
	enum ast_bridge_hook_remove_flags remove_flags)
{
	...
	ast_log_chan(NULL, LOG_ERROR, "Attempted to set limits without an AST_BRIDGE_BUILTIN_INTERVAL_LIMITS callback registered.\n");
	return -1;
}
...
/* XXX GABPBX-21271 make ast_bridge_features_init() static when make ast_bridge_join() requires features to be allocated. */
...
/* XXX GABPBX-21271 make ast_bridge_features_cleanup() static when make ast_bridge_join() requires features to be allocated. */
...
void ast_bridge_set_single_src_video_mode(struct ast_bridge *bridge, struct ast_channel *video_src_chan)
{
	...
	if (video_src_chan) {
		...
		ast_verb_chan(NULL, 5, "Video source in bridge '%s' (%s) is now '%s' (%s)\n",
...
#define BRIDGE_LOCK_ONE_OR_BOTH(b1, b2) \
	...
	if (bridge2) {
		...
		if (!locals[0] || !locals[1]) {
			ast_log_chan(NULL, LOG_ERROR, "Transfer failed probably due to an early hangup - "
		...
		transfer_channel_cb new_channel_cb, void *user_data)
{
	...
	if (!transfer_message) {
		 ...
		 * failure
		 */
		ast_log_chan(NULL, LOG_ERROR, "Unable to allocate memory for blind transfer publication from %s\n",
		...
		struct ast_channel *to_transfer_target)
{
	...
	if (!transfer_msg) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to create Stasis publication for attended transfer from %s\n",
	...
	if (to_transferee_bridge_channel) {
		...
		if (ast_bridge_channel_write_unhold(to_transferee_bridge_channel)) {
			ast_log_chan(NULL, LOG_ERROR, "Transferee channel disappeared during transfer!\n");
			res = AST_BRIDGE_TRANSFER_FAIL;
			goto end;
		}
	}
	...
	if (to_target_bridge_channel) {
		...
		if (ast_bridge_channel_write_unhold(to_target_bridge_channel)) {
			ast_log_chan(NULL, LOG_ERROR, "Target channel disappeared during transfer!\n");
			res = AST_BRIDGE_TRANSFER_FAIL;
			goto end;
		}
