...
static int build_dtmf_features(struct ast_flags *flags, const char *features)
{
	...
	for (feature = features; *feature; feature++) {
		if (!isupper(*feature)) {
			ast_log_chan(NULL, LOG_ERROR, "Features string '%s' rejected because it contains non-uppercase feature.\n", features);
			return -1;
		}
	...
	if (number_of_missing_features) {
		ast_log_chan(NULL, LOG_WARNING, "Features '%s' from features string '%s' can not be applied.\n", missing_features, features);
	}
...
int ast_bridge_features_ds_set_string(struct ast_channel *chan, const char *features)
{
	...
	if (ast_bridge_features_ds_set(chan, &flags)) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Failed to apply features datastore for '%s' to channel '%s'\n", features, ast_channel_name(chan));
		return -1;
	}
	ast_channel_unlock(chan);
	...
	struct ast_flags *flags, unsigned int feature_flag, const char *feature_name, enum ast_bridge_builtin_feature feature_bridge)
{
	...
	if (ast_test_flag(flags, feature_flag)
		...
		if (res) {
			ast_log_chan(NULL, LOG_ERROR, "Channel %s: Requested DTMF feature %s not available.\n",
	...
	struct ast_channel *transferer, const char *context)
{
	...
	if (!props) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to create props - channel %s, context %s\n",
	...
	if (ast_string_field_init(props, 64)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to initialize prop fields - channel %s, context %s\n",
			ast_channel_name(transferer), context);
		ao2_ref(props, -1);
		return NULL;
	}
	...
	if (!xfer_cfg) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to get transfer configuration from channel %s\n", ast_channel_name(props->transferer));
	...
	addr = strchr(tech, '/');
	if (!addr) {
		ast_log_chan(NULL, LOG_ERROR, "Transferer channel name does not follow typical channel naming format (tech/address)\n");
		ast_channel_unlock(props->transferer);
		ao2_ref(props, -1);
		return NULL;
	}
		...
		enum attended_transfer_stimulus stimulus)
{
	...
	if (!list) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to push event to attended transfer queue. Expect transfer to fail\n");
		return;
	}
		...
		struct ast_channel *transferee_channel, struct ast_channel *target_channel)
{
	...
	if (!transfer_msg) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to publish successful attended transfer from %s\n",
				ast_channel_name(props->transferer));
		return;
	}
		...
		struct ast_channel *transferee_channel, struct ast_channel *target_channel)
{
	...
	if (!transfer_msg) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to publish successful three-way transfer from %s\n",
				ast_channel_name(props->transferer));
		return;
	}
...
static void publish_transfer_fail(struct attended_transfer_properties *props)
{
	...
	if (!transfer_msg) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to publish failed transfer from %s\n",
				ast_channel_name(props->transferer));
		return;
	}
		...
		enum attended_transfer_stimulus stimulus)
{
	...
	default:
		ast_log_chan(NULL, LOG_WARNING, "Unexpected stimulus '%s' received in attended transfer state '%s'\n",
...
static int retransfer_enter(struct attended_transfer_properties *props)
{
	...
	if (!props->recall_target) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to request outbound channel for recall target\n");
		return -1;
	}

	if (attach_framehook(props, props->recall_target)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to attach framehook to recall target\n");
		ast_hangup(props->recall_target);
	...
	if (ast_call(props->recall_target, destination, 0)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to place outbound call to recall target\n");
		ast_hangup(props->recall_target);
		props->recall_target = NULL;
		return -1;
	}
	...
	if (ast_bridge_impart(props->transferee_bridge, props->recall_target, NULL, NULL,
		AST_BRIDGE_IMPART_CHAN_INDEPENDENT)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to place recall target into bridge\n");
		ast_hangup(props->recall_target);
		ast_channel_unref(props->recall_target);
		return -1;
	}
...
static void *attended_transfer_monitor_thread(void *data)
{
	...
	for (;;) {
		...
		if (state_properties[props->state].enter &&
			...
			ast_log_chan(NULL, LOG_ERROR, "State %s enter function returned an error for attended transfer %p\n",
					state_properties[props->state].state_name, props);
			break;
		}
...
static int grab_transfer(struct ast_channel *chan, char *exten, size_t exten_len, const char *context)
{
	...
	if (!ast_strlen_zero(extenoverride)) {
		...
		if (extenres) {
			...
			ast_channel_unlock(chan);
			ast_verb_chan(NULL, 3, "Transferring call to '%s@%s'", exten, context);
			return 0;
		}
		ast_log_chan(NULL, LOG_WARNING, "Override extension '%s' does not exist in context '%s'\n", extenoverride, context);
		/* since we didn't get a valid extension from the channel, fall back and grab it from the user as usual now */
	}

	xfer_cfg = ast_get_chan_features_xfer_config(chan);
	if (!xfer_cfg) {
		ast_log_chan(NULL, LOG_ERROR, "Channel %s: Unable to get transfer configuration\n",
	...
	do {
		...
		if (res < 0) {
			...
			if (ast_strlen_zero(exten)) {
				ast_verb_chan(NULL, 3, "Channel %s: Dialed no digits.\n", ast_channel_name(chan));
			} else {
				ast_verb_chan(NULL, 3, "Channel %s: Dialed '%s@%s' does not exist.\n",
...
static int feature_attended_transfer(struct ast_bridge_channel *bridge_channel, void *hook_pvt)
{
	...
	ast_verb_chan(NULL, 3, "Channel %s: Started DTMF attended transfer.\n",
		ast_channel_name(bridge_channel->chan));

	if (strcmp(bridge->v_table->name, "basic")) {
		ast_log_chan(NULL, LOG_ERROR, "Channel %s: Attended transfer attempted on unsupported bridge type '%s'.\n",
	...
	if (1 < bridge->inhibit_merge) {
		 ...
		 */
		ast_verb_chan(NULL, 3, "Channel %s: Bridge '%s' does not permit merging at this time.\n",
	...
	if (!props) {
		ast_log_chan(NULL, LOG_ERROR, "Channel %s: Unable to allocate control structure for performing attended transfer.\n",
			ast_channel_name(bridge_channel->chan));
		ast_bridge_merge_inhibit(bridge, -1);
		ao2_ref(bridge, -1);
		return 0;
	...
	if (add_transferer_role(props->transferer, attended_transfer)) {
		ast_log_chan(NULL, LOG_ERROR, "Channel %s: Unable to set transferrer bridge role.\n",
			ast_channel_name(bridge_channel->chan));
		attended_transfer_properties_shutdown(props);
		return 0;
	}
	...
	if (grab_transfer(bridge_channel->chan, exten, sizeof(exten), props->context)) {
		 ...
		 * extension.
		 */
		ast_verb_chan(NULL, 3, "Channel %s: Unable to acquire target extension for attended transfer.\n",
			ast_channel_name(bridge_channel->chan));
		ast_bridge_channel_write_unhold(bridge_channel);
		attended_transfer_properties_shutdown(props);
		return 0;
	...
	if (!props->transfer_target) {
		ast_log_chan(NULL, LOG_ERROR, "Channel %s: Unable to request outbound channel for attended transfer target.\n",
			ast_channel_name(bridge_channel->chan));
	...
	if (!props->target_bridge) {
		ast_log_chan(NULL, LOG_ERROR, "Channel %s: Unable to create bridge for attended transfer target.\n",
			ast_channel_name(bridge_channel->chan));
		stream_failsound(props->transferer);
		ast_bridge_channel_write_unhold(bridge_channel);
	...
	if (attach_framehook(props, props->transfer_target)) {
		ast_log_chan(NULL, LOG_ERROR, "Channel %s: Unable to attach framehook to transfer target.\n",
			ast_channel_name(bridge_channel->chan));
		stream_failsound(props->transferer);
		ast_bridge_channel_write_unhold(bridge_channel);
		ast_hangup(props->transfer_target);
	...
	if (ast_call(props->transfer_target, destination, 0)) {
		ast_log_chan(NULL, LOG_ERROR, "Channel %s: Unable to place outbound call to transfer target.\n",
			ast_channel_name(bridge_channel->chan));
		stream_failsound(props->transferer);
		ast_bridge_channel_write_unhold(bridge_channel);
		ast_hangup(props->transfer_target);
	...
	if (ast_bridge_impart(props->target_bridge, props->transfer_target, NULL, NULL,
		AST_BRIDGE_IMPART_CHAN_INDEPENDENT)) {
		ast_log_chan(NULL, LOG_ERROR, "Channel %s: Unable to place transfer target into bridge.\n",
			ast_channel_name(bridge_channel->chan));
		stream_failsound(props->transferer);
		ast_bridge_channel_write_unhold(bridge_channel);
		ast_hangup(props->transfer_target);
	...
	if (ast_pthread_create_detached(&thread, NULL, attended_transfer_monitor_thread, props)) {
		ast_log_chan(NULL, LOG_ERROR, "Channel %s: Unable to create monitoring thread for attended transfer.\n",
			ast_channel_name(bridge_channel->chan));
		stream_failsound(props->transferer);
		ast_bridge_channel_write_unhold(bridge_channel);
		attended_transfer_properties_shutdown(props);
...
static int feature_blind_transfer(struct ast_bridge_channel *bridge_channel, void *hook_pvt)
{
	...
	ast_verb_chan(NULL, 3, "Channel %s: Started DTMF blind transfer.\n",
		ast_channel_name(bridge_channel->chan));
...
static void on_personality_change_normal(struct ast_bridge *bridge)
{
	...
	AST_LIST_TRAVERSE(&bridge->channels, iter, entry) {
		if (add_normal_hooks(bridge, iter)) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to set up bridge hooks for channel %s. Features may not work properly\n",
		...
		enum bridge_basic_personality_type type)
{
	...
	case BRIDGE_BASIC_PERSONALITY_ATXFER:
		...
		break;
	default:
		ast_log_chan(NULL, LOG_WARNING, "Asked to initialize unexpected basic bridge personality type.\n");
		break;
	}
}
