...
void bridge_channel_settle_owed_events(struct ast_bridge *orig_bridge, struct ast_bridge_channel *bridge_channel)
{
	if (bridge_channel->owed.dtmf_digit) {
		...
		ast_log_chan(NULL, LOG_DTMF, "DTMF end '%c' simulated to bridge %s because %s left.  Duration %ld ms.\n",
...
int ast_bridge_channel_queue_frame(struct ast_bridge_channel *bridge_channel, struct ast_frame *fr)
{
	...
	if (DEBUG_ATLEAST(1)) {
		if (fr->frametype == AST_FRAME_TEXT) {
			ast_log_chan(NULL, LOG_DEBUG, "Queuing TEXT frame to '%s': %*.s\n", ast_channel_name(bridge_channel->chan),
				fr->datalen, (char *)fr->data.ptr);
		} else if (fr->frametype == AST_FRAME_TEXT_DATA) {
			struct ast_msg_data *msg = fr->data.ptr;
			ast_log_chan(NULL, LOG_DEBUG, "Queueing TEXT_DATA frame from '%s' to '%s:%s': %s\n",
	...
	if (ast_alertpipe_write(bridge_channel->alert_pipe)) {
		ast_log_chan(NULL, LOG_ERROR, "We couldn't write alert pipe for %p(%s)... something is VERY wrong\n",
			bridge_channel, ast_channel_name(bridge_channel->chan));
	}
	ast_bridge_channel_unlock(bridge_channel);
	return 0;
...
static void bridge_channel_park(struct ast_bridge_channel *bridge_channel, struct bridge_park *payload)
{
	if (!ast_parking_provider_registered()) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Unable to park %s: No parking provider loaded!\n",
			ast_channel_name(bridge_channel->chan));
		return;
	}
	...
	if (ast_parking_park_bridge_channel(bridge_channel, payload->parkee_uuid,
		...
		ast_log_chan(NULL, AST_LOG_WARNING, "Error occurred while parking %s\n",
			ast_channel_name(bridge_channel->chan));
	}
}
...
static unsigned int bridge_channel_feature_digit_timeout(struct ast_bridge_channel *bridge_channel)
{
	...
	if (!gen_cfg) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to retrieve features configuration.\n");
...
static void after_bridge_move_channel_fail(enum ast_bridge_after_cb_reason reason, void *data)
{
	...
	ast_log_chan(NULL, LOG_WARNING, "Unable to complete transfer: %s\n",
...
void bridge_channel_internal_pull(struct ast_bridge_channel *bridge_channel)
{
	...
	ast_verb_chan(NULL, 3, "Channel %s left '%s' %s-bridge <%s>\n",
		ast_channel_name(bridge_channel->chan),
...
int bridge_channel_internal_push_full(struct ast_bridge_channel *bridge_channel, int optimized)
{
	...
	ast_verb_chan(NULL, 3, "Channel %s %s%s%s '%s' %s-bridge <%s>\n",
		ast_channel_name(bridge_channel->chan),
...
static void bridge_channel_handle_write(struct ast_bridge_channel *bridge_channel)
{
	...
	if (AST_LIST_EMPTY(&bridge_channel->wr_queue)) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Weird.  No frame from bridge for %s to process?\n",
			ast_channel_name(bridge_channel->chan));
...
/* XXX GABPBX-21271 the external party use of suspended will go away as will these references because this is the bridge channel thread */
