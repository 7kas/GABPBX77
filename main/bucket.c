	...
	bucket_file_destroy_cb destroy_cb, struct ast_module *module)
{
	...
	ast_verb_chan(NULL, 5, "Registered bucket scheme '%s'\n", name);
...
static int bucket_copy(const char *infile, const char *outfile)
{
	...
	if ((ifd = open(infile, O_RDONLY)) < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to open %s in read-only mode, error: %s\n", infile, strerror(errno));
		return -1;
	}
	if ((ofd = open(outfile, O_WRONLY | O_TRUNC | O_CREAT, AST_FILE_MODE)) < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to open %s in write-only mode, error: %s\n", outfile, strerror(errno));
		close(ifd);
		return -1;
	}
...
int ast_bucket_init(void)
{
	...
	if (!schemes) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to create container for Bucket schemes\n");
		return -1;
	}

	if (__ast_sorcery_wizard_register(&bucket_wizard, NULL)) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to register sorcery wizard for 'bucket' intermediary\n");
		return -1;
	}

	if (__ast_sorcery_wizard_register(&bucket_file_wizard, NULL)) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to register sorcery wizard for 'file' intermediary\n");
		return -1;
	}

	if (!(bucket_sorcery = ast_sorcery_open())) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to create sorcery instance for Bucket support\n");
		return -1;
	}

	if (ast_sorcery_apply_default(bucket_sorcery, "bucket", "bucket", NULL) == AST_SORCERY_APPLY_FAIL) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to apply intermediary for 'bucket' object type in Bucket sorcery\n");
		return -1;
	}

	if (ast_sorcery_internal_object_register(bucket_sorcery, "bucket", bucket_alloc, NULL, NULL)) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to register 'bucket' object type in Bucket sorcery\n");
		return -1;
	}
	...
	if (ast_sorcery_apply_default(bucket_sorcery, "file", "bucket_file", NULL) == AST_SORCERY_APPLY_FAIL) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to apply intermediary for 'file' object type in Bucket sorcery\n");
		return -1;
	}

	if (ast_sorcery_internal_object_register(bucket_sorcery, "file", bucket_file_alloc, NULL, NULL)) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to register 'file' object type in Bucket sorcery\n");
		return -1;
	}
