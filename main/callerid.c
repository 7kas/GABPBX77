...
void callerid_get_dtmf(char *cidstring, char *number, int *flags)
{
	...
	if (cidstring[0] == 'B') {
		 ...
		 * for the best */
		ast_log_chan(NULL, LOG_WARNING, "Couldn't detect start-character. CID "
...
int callerid_feed_jp(struct callerid_state *cid, unsigned char *ubuf, int len, struct ast_format *codec)
{
	...
	while (mylen >= 160) {
		...
		if (mylen < 0) {
			ast_log_chan(NULL, LOG_ERROR, "No start bit found in fsk data.\n");
			return -1;
		}
		...
		if (res < 0) {
			ast_log_chan(NULL, LOG_NOTICE, "fsk_serial failed\n");
			return -1;
		}
		...
		if (res == 1) {
			...
			case 7:	/* NUMBER LENGTH */
				...
				if ((cid->len+2) >= sizeof(cid->rawdata)) {
					ast_log_chan(NULL, LOG_WARNING, "too long caller id string\n") ;
					return -1;
				}
				cid->rawdata[cid->pos++] = b;
				break;
			...
			case 11: /* CRC Checksum 2 */
				cid->sawflag = 12;
				if (cid->crc != 0) {
					ast_log_chan(NULL, LOG_WARNING, "crc checksum error\n") ;
					return -1;
				}
				...
				for (x = 0; x < cid->pos;) {
					...
					case 0x22: /* dialed number additional information */
						...
						default:   /* reserved */
							if (DEBUG_ATLEAST(2))
								ast_log_chan(NULL, LOG_NOTICE, "did info:#1=%X\n", (unsigned)cid->rawdata[x]);
							break ;
						}
						x++;
						/* numbering plan octed 4 */
				...
				break;
			default:
				ast_log_chan(NULL, LOG_ERROR, "invalid value in sawflag %d\n", cid->sawflag);
			}
		}
	}
	if (mylen) {
	...
	while (mylen >= 160) {
		...
		if (res == 1) {
			...
			case 4: /* Retrieve message */
				if (cid->pos >= 128) {
					ast_log_chan(NULL, LOG_WARNING, "Caller ID too long???\n");
					return -1;
				}
				cid->rawdata[cid->pos++] = b;
				cid->len--;
			...
			case 5: /* Check checksum */
				if ((b + cid->cksum) & 0xff) {
					ast_log_chan(NULL, LOG_NOTICE, "Caller*ID failed checksum\n");
					/* Try again */
					cid->sawflag = 0;
					break;
				}
				...
				if ((cid->type == 0x80) || (cid->type == 0x82)) {
					...
					for (x = 0; x < cid->pos;) {
						 ...
						 * GABpbx doesn't really have a corresponding thing for this,
						...
						case 4: /* Reason for Absence of Number. */
							res = cid->rawdata[x];
							if (res > 32) {
								ast_log_chan(NULL, LOG_NOTICE, "Truncating long caller ID number from %d bytes to 32\n", cid->rawdata[x]);
								res = 32;
							}
						...
						case 5: /* Reason for Redirection */
							res = cid->rawdata[x];
							if (res != 1) {
								ast_log_chan(NULL, LOG_WARNING, "Redirecting parameter length is %d?\n", res);
								break;
							}
							...
							case 0x5:
								...
								break;
							default:
								ast_log_chan(NULL, LOG_WARNING, "Redirecting reason is %02x?\n", *(cid->rawdata + x + 1));
								break;
							}
							break;
						...
						case 8: /* Reason for Absence of Name */
							res = cid->rawdata[x];
							if (res > 32) {
								ast_log_chan(NULL, LOG_NOTICE, "Truncating long caller ID name from %d bytes to 32\n", cid->rawdata[x]);
								res = 32;
							}
						...
						case 22: /* Something French */
							break;
						default:
							ast_log_chan(NULL, LOG_NOTICE, "Unknown IE %d\n", cid->rawdata[x - 1]);
						}
						res = cid->rawdata[x];
						if (0 > res){	/* Negative offset in the CID Spill */
							ast_log_chan(NULL, LOG_NOTICE, "IE %d has bad field length of %d at offset %d\n", cid->rawdata[x-1], cid->rawdata[x], x);
							/* Try again */
							cid->sawflag = 0;
							break;	/* Exit the loop */
						}
				...
				break;
			default:
				ast_log_chan(NULL, LOG_ERROR, "Dunno what to do with a digit in sawflag %d\n", cid->sawflag);
			}
		}
	}
	if (mylen) {
