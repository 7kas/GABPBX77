...
static enum ast_cc_agent_policies str_to_agent_policy(const char * const value)
{
	if (!strcasecmp(value, "never")) {
		...
		ast_log_chan(NULL, LOG_WARNING, "%s is an invalid value for cc_agent_policy. Switching to 'never'\n", value);
		return AST_CC_AGENT_NEVER;
	}
}
...
static enum ast_cc_monitor_policies str_to_monitor_policy(const char * const value)
{
	if (!strcasecmp(value, "never")) {
		...
		ast_log_chan(NULL, LOG_WARNING, "%s is an invalid value for cc_monitor_policy. Switching to 'never'\n", value);
		return AST_CC_MONITOR_NEVER;
	}
}
		...
		char *buf, size_t buf_len)
{
	...
	if (!strcasecmp(name, "cc_offer_timer")) {
		...
		ast_log_chan(NULL, LOG_WARNING, "%s is not a valid CC parameter. Ignoring.\n", name);
		return -1;
	}
...
void ast_set_cc_offer_timer(struct ast_cc_config_params *config, unsigned int value)
{
	...
	if (value == 0) {
		ast_log_chan(NULL, LOG_WARNING, "0 is an invalid value for cc_offer_timer. Retaining value as %u\n", config->cc_offer_timer);
		return;
	}
...
void ast_set_ccnr_available_timer(struct ast_cc_config_params *config, unsigned int value)
{
	/* 0 is an unreasonable value for any timer. Stick with the default */
	if (value == 0) {
		ast_log_chan(NULL, LOG_WARNING, "0 is an invalid value for ccnr_available_timer. Retaining value as %u\n", config->ccnr_available_timer);
		return;
	}
...
void ast_set_ccbs_available_timer(struct ast_cc_config_params *config, unsigned int value)
{
	/* 0 is an unreasonable value for any timer. Stick with the default */
	if (value == 0) {
		ast_log_chan(NULL, LOG_WARNING, "0 is an invalid value for ccbs_available_timer. Retaining value as %u\n", config->ccbs_available_timer);
		return;
	}
 ...
 * the documentation provided in include/gabpbx/ccss.h
 */
...
void ast_handle_cc_control_frame(struct ast_channel *inbound, struct ast_channel *outbound, void *frame_data)
{
	...
	if (!(cc_datastore = ast_channel_datastore_find(inbound, &dialed_cc_interfaces_info, NULL))) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to retrieve CC datastore while processing CC frame from '%s'. CC services will be unavailable.\n", device_name);
	...
	if (!(monitor = cc_device_monitor_init(device_name, dialstring, cc_data, core_instance->core_id))) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to create CC device interface for '%s'. CC services will be unavailable on this interface.\n", device_name);
		cc_unref(core_instance, "Returning early from ast_handle_cc_control_frame. Unref core_instance");
		call_destructor_with_no_monitor(cc_data->monitor_type, cc_data->private_data);
		return;
	}
...
{
	/* This should never happen... */
	ast_log_chan(NULL, LOG_WARNING, "Someone requested to change to CC_AVAILABLE? Ignoring.\n");
	return -1;
}
...
{
	if (!ast_cc_request_is_within_limits()) {
		ast_log_chan(NULL, LOG_WARNING, "Cannot request CC since there is no more room for requests\n");
...
static void build_cc_interfaces_chanvar(struct ast_cc_monitor *starting_point, struct ast_str **str)
{
	...
	if (length <= 1) {
		...
		ast_log_chan(NULL, LOG_ERROR, "CC_INTERFACES is empty. starting device_name:'%s'\n",
		...
		const char * const dialstring, enum ast_cc_service_type service, void *private_data)
{
	...
	if (ast_cc_monitor_count(device_name, monitor_type) >= ast_get_cc_max_monitors(cc_params)) {
		ast_log_chan(NULL, LOG_NOTICE, "Not queuing a CC frame for device %s since it already has its maximum monitors allocated\n", device_name);
		return -1;
	}
...
static int cccancel_exec(struct ast_channel *chan, const char *data)
{
	...
	if (strcmp(core_instance->agent->callbacks->type, "generic")) {
		ast_log_chan(NULL, LOG_WARNING, "CallCompletionCancel may only be used for calls with a generic agent\n");
...
static void initialize_cc_max_requests(void)
{
	...
	if (!cc_config || cc_config == CONFIG_STATUS_FILEINVALID) {
		ast_log_chan(NULL, LOG_WARNING, "Could not find valid ccss.conf file. Using cc_max_requests default\n");
	...
	if (!ast_strlen_zero(endptr)) {
		ast_log_chan(NULL, LOG_WARNING, "Invalid input given for cc_max_requests. Using default\n");
		global_cc_max_requests = GLOBAL_CC_MAX_REQUESTS_DEFAULT;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER, "Call Completion Supplementary Services",
