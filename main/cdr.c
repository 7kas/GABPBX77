		...
		<configFile name="cdr.conf">
			<configObject name="general">
				...
				<configOption name="initiatedseconds">
					...
					initiatedseconds to <literal>yes</literal>, you can force gabpbx to report any seconds
				...
				<configOption name="batch">
					...
					gabpbx server.</para>
					<warning><para>Use of batch mode may result in data loss after unsafe gabpbx termination,
				...
				<configOption name="scheduleronly">
					...
					<description><para>The CDR engine uses the internal gabpbx scheduler to determine when to post
				...
				<configOption name="safeshutdown">
					...
					<synopsis>Block shutdown of GABpbx until CDRs are submitted</synopsis>
					<description><para>When shutting down gabpbx, you can block until the CDRs are submitted.  If
					you don't, then data will likely be lost.  You can always check the size of
					the CDR batch buffer with the CLI <astcli>cdr status</astcli> command.  To enable blocking on
					submission of CDR data during gabpbx shutdown, set this to <literal>yes</literal>.</para>
					</description>
				</configOption>
			</configObject>
		</configFile>
...
struct cdr_object {
	...
	AST_DECLARE_STRING_FIELDS(
		...
		AST_STRING_FIELD(clisrc);
		AST_STRING_FIELD(clidst);
	);
...
static struct cdr_object *cdr_object_alloc(struct ast_channel_snapshot *chan, const struct timeval *event_time)
{
	...
	if (ast_strlen_zero(cdr->clisrc)) { 
		ast_string_field_set(cdr, clisrc, chan->caller->number);
	}
	if (ast_strlen_zero(cdr->clidst)) {
		ast_string_field_set(cdr, clidst, chan->dialplan->exten);
	}
...
static struct cdr_object *cdr_object_create_and_append(struct cdr_object *cdr, const struct timeval *event_time)
{
	...
	/* germanico - Heredar clisrc y clidst del CDR anterior */
	ast_string_field_set(new_cdr, clisrc, cdr_last->clisrc);
	ast_string_field_set(new_cdr, clidst, cdr_last->clidst);
...
static struct ast_cdr *cdr_object_create_public_records(struct cdr_object *cdr)
{
	...
	for (it_cdr = cdr; it_cdr; it_cdr = it_cdr->next) {
		...
		ast_copy_string(cdr_copy->clisrc, it_cdr->clisrc, sizeof(cdr_copy->clisrc));
		ast_copy_string(cdr_copy->clidst, it_cdr->clidst, sizeof(cdr_copy->clidst));
 ...
 * \param cdr The \ref cdr_object
 * \param hangupcause The GABpbx hangup cause code
 */
...
static void handle_dial_message(void *data, struct stasis_subscription *sub, struct stasis_message *message)
{
	...
	if (!cdr) {
		ast_log_chan(NULL, AST_LOG_WARNING, "No CDR for channel %s\n", caller ? caller->base->name : peer->base->name);
		ast_assert(0);
		return;
	}
...
static int cdr_generic_register(struct be_list *generic_list, const char *name, const char *desc, ast_cdrbe be)
{
	...
	if (!be) {
		ast_log_chan(NULL, LOG_WARNING, "CDR engine '%s' lacks backend\n", name);
	...
	AST_RWLIST_TRAVERSE(generic_list, cur, list) {
		if (!strcasecmp(name, cur->name)) {
			ast_log_chan(NULL, LOG_WARNING, "Already have a CDR backend called '%s'\n", name);
...
static int ast_cdr_generic_unregister(struct be_list *generic_list, const char *name)
{
	...
	if (!match->suspended && active_count != 0) {
		AST_RWLIST_UNLOCK(generic_list);
		ast_log_chan(NULL, AST_LOG_WARNING, "Unable to unregister CDR backend %s; %d CDRs are still active\n",
			name, active_count);
		return -1;
	}
	...
	ast_verb_chan(NULL, 5, "Unregistered '%s' CDR backend\n", name);
	ast_free(match);
...
void ast_cdr_format_var(struct ast_cdr *cdr, const char *name, char **ret, char *workspace, int workspacelen, int raw)
{
	...
	*ret = NULL;
	...
	/* germanico */
	} else if (!strcasecmp(name, "clisrc")) {
		ast_copy_string(workspace, cdr->clisrc, workspacelen);
        } else if (!strcasecmp(name, "clidst")) {
		ast_copy_string(workspace, cdr->clidst, workspacelen);
	...
	/* germanico
        } else if (!strcasecmp(name, "remote_uniqueid")) {
                ast_copy_string(workspace, cdr->remote_uniqueid, workspacelen); */
...
int ast_cdr_setvar(const char *channel_name, const char *name, const char *value)
{
	...
	for (x = 0; cdr_readonly_vars[x]; x++) {
		if (!strcasecmp(name, cdr_readonly_vars[x])) {
			ast_log_chan(NULL, LOG_ERROR, "Attempt to set the '%s' read-only variable!\n", name);
			return -1;
		}
	}
	...
	if (!it_cdrs) {
		ast_log_chan(NULL, AST_LOG_ERROR, "Unable to find CDR for channel %s\n", channel_name);
		return -1;
	}
...
static int cdr_object_format_property(struct cdr_object *cdr_obj, const char *name, char *value, size_t length)
{
	...
	/* germanico */
                ...
                ast_copy_string(value, cdr_obj->clisrc, length);
        } else if (!strcasecmp(name, "clidst")) {
                ast_copy_string(value, cdr_obj->clidst, length);
	...
	/* germanico
                ...
                ast_copy_string(value, party_a->base->remote_uniqueid, length); */
	...
	for (it_cdr = cdr; it_cdr; it_cdr = it_cdr->next) {
		...
		AST_LIST_TRAVERSE(&it_cdr->party_a.variables, variable, entries) {
			...
			if (ast_str_append(buf, 0, "level %d: %s%c%s%c", x, var, delim, S_OR(ast_var_value(variable), ""), sep) < 0) {
				ast_log_chan(NULL, LOG_ERROR, "Data Buffer Size Exceeded!\n");
				break;
			}
...
int ast_cdr_fork(const char *channel_name, struct ast_flags *options)
{
...

	{
		...
		/* germanico - Heredar clisrc y clidst del CDR anterior */
		ast_string_field_set(new_cdr, clisrc, cdr_obj->clisrc);
		ast_string_field_set(new_cdr, clidst, cdr_obj->clidst);
...
static void cdr_submit_batch(int do_shutdown)
{
	...
	if (!mod_cfg
		...
		if (ast_pthread_create_detached_background(&batch_post_thread, NULL, do_batch_backend_process, oldbatchitems)) {
			ast_log_chan(NULL, LOG_WARNING, "CDR processing thread could not detach, now trying in this thread\n");
			do_batch_backend_process(oldbatchitems);
		} else {
...
static int process_config(int reload)
{
	...
	if (aco_process_config(&cfg_info, reload) == ACO_PROCESS_ERROR) {
		...
		ast_log_chan(NULL, LOG_NOTICE, "Failed to process CDR configuration; using defaults\n");
...
static void cdr_enable_batch_mode(struct ast_cdr_config *config)
{
	...
	if (cdr_thread == AST_PTHREADT_NULL) {
		...
		if (ast_pthread_create_background(&cdr_thread, NULL, do_cdr, NULL) < 0) {
			ast_log_chan(NULL, LOG_ERROR, "Unable to start CDR thread.\n");
			return;
		}
	}
	...
	ast_log_chan(NULL, LOG_NOTICE, "CDR batch mode logging enabled, first of either size %u or time %u seconds.\n",
...
static int cdr_toggle_runtime_options(void)
{
	...
	if (mod_cfg
		...
		if (create_subscriptions()) {
			...
			ast_log_chan(NULL, AST_LOG_ERROR, "Failed to create Stasis subscriptions\n");
			ao2_cleanup(mod_cfg);
			return -1;
		}
		if (ast_test_flag(&mod_cfg->general->settings, CDR_BATCHMODE)) {
			...
			ast_log_chan(NULL, LOG_NOTICE, "CDR simple logging enabled.\n");
		}
	} else {
		destroy_subscriptions();
		ast_log_chan(NULL, LOG_NOTICE, "CDR logging disabled.\n");
	}
	ao2_cleanup(mod_cfg);
...
static int load_module(void)
{
	...
	if (mod_cfg) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Unable to obtain CDR configuration during module load?\n");
	}
	...
	if (!sched) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to create schedule context.\n");
		return AST_MODULE_LOAD_FAILURE;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER, "CDR Engine",
