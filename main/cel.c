...
static int cel_pre_apply_config(void)
{
	...
	ast_log_chan(NULL, LOG_ERROR, "Applications are listed to be tracked, but APP events are not tracked\n");
	return -1;
}
...
enum ast_cel_event_type ast_cel_str_to_event_type(const char *name)
{
	...
	ast_log_chan(NULL, LOG_ERROR, "Unknown event name '%s'\n", name);
...
static void check_retire_linkedid(struct ast_channel_snapshot *snapshot, const struct timeval *event_time)
{
	...
	if (!lid) {
		 ...
		 * of change to make after starting GABpbx.
		 */
		ast_log_chan(NULL, LOG_ERROR, "Something weird happened, couldn't find linkedid %s\n",
...
static int cel_linkedid_ref(const char *linkedid)
{
	...
	if (ast_strlen_zero(linkedid)) {
		ast_log_chan(NULL, LOG_ERROR, "The linkedid should never be empty\n");
		return -1;
	}
	if (!linkedids) {
		/* The CEL module is shutdown.  Abort. */
...
int ast_cel_fill_record(const struct ast_event *e, struct ast_cel_event_record *r)
{
	if (r->version != AST_CEL_EVENT_RECORD_VERSION) {
		ast_log_chan(NULL, LOG_ERROR, "Module ABI mismatch for ast_cel_event_record.  "
				"Please ensure all modules were compiled for "
				"this version of GABpbx.\n");
		return -1;
	}
	...
	struct stasis_message *message)
{
	...
	default:
		ast_log_chan(NULL, LOG_ERROR, "Unhandled %s event blob\n", ast_cel_get_type_name(event_type));
		break;
	}
}
...
static int create_routes(void)
{
	...
	if (ret) {
		ast_log_chan(NULL, AST_LOG_ERROR, "Failed to register for Stasis messages\n");
	}
...
static int load_module(void)
{
	...
	if (aco_process_config(&cel_cfg_info, 0)) {
		...
		if (!aco_set_defaults(&general_option, "general", cel_cfg->general)) {
			ast_log_chan(NULL, LOG_NOTICE, "Failed to process CEL configuration; using defaults\n");
...
static int reload_module(void)
{
	...
	ast_verb_chan(NULL, 3, "CEL logging %sabled.\n", is_enabled ? "en" : "dis");
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER, "CEL Engine",
