...
#include "gabpbx/_private.h"

#include <sys/time.h>
#include <signal.h>
#include <math.h>

#include "gabpbx/paths.h"	/* use ast_config_AST_SYSTEM_NAME */
...
#include "gabpbx/slinfactory.h"
#include "gabpbx/audiohook.h"
#include "gabpbx/framehook.h"
#include "gabpbx/timing.h"
#include "gabpbx/autochan.h"
#include "gabpbx/stringfields.h"
#include "gabpbx/global_datastores.h"
#include "gabpbx/channel_internal.h"
...
/*! \brief Register a new telephony channel in GABpbx */
int ast_channel_register(const struct ast_channel_tech *tech)
{
	...
	AST_RWLIST_TRAVERSE(&backends, chan, list) {
		if (!strcasecmp(tech->type, chan->tech->type)) {
			ast_log_chan(NULL, LOG_WARNING, "Already have a handler for type '%s'\n", tech->type);
			AST_RWLIST_UNLOCK(&backends);
			return -1;
		}
	}
	...
	ast_verb_chan(NULL, 5, "Registered channel type '%s' (%s)\n", chan->tech->type, chan->tech->description);
...
void ast_channel_unregister(const struct ast_channel_tech *tech)
{
	...
	AST_RWLIST_TRAVERSE_SAFE_BEGIN(&backends, chan, list) {
		if (chan->tech == tech) {
			...
			ast_free(chan);
			ast_verb_chan(NULL, 5, "Unregistered channel type '%s'\n", tech->type);
			break;
		}
	}
...
static int does_id_conflict(const char *uniqueid)
{
	...
	if (conflict) {
		ast_log_chan(NULL, LOG_ERROR, "Channel Unique ID '%s' already in use by channel %s(%p)\n",
		       ...
		       const char *function, const char *name_fmt, va_list ap)
{
	...
	if (ast_shutting_down()) {
		ast_log_chan(NULL, LOG_WARNING, "Channel allocation failed: Refusing due to active shutdown\n");
		return NULL;
	}
	...
	if (!(schedctx = ast_sched_context_create())) {
		ast_log_chan(NULL, LOG_WARNING, "Channel allocation failed: Unable to create schedule context\n");
...
static int __ast_queue_frame(struct ast_channel *chan, struct ast_frame *fin, int head, struct ast_frame *after)
{
	...
	if ((queued_frames + new_frames > 128 || queued_voice_frames + new_voice_frames > 96)) {
		...
		int count = 0;
		ast_log_chan(NULL, LOG_WARNING, "Exceptionally long %squeue length (%d voice / %d total) queuing to %s\n",
	...
	if (ast_channel_alert_writable(chan)) {
		...
		while (new_frames--) {
			if (ast_channel_alert_write(chan)) {
				ast_log_chan(NULL, LOG_WARNING, "Unable to write to alert pipe on %s (qlen = %u): %s!\n",
...
static int ast_channel_by_name_cb(void *obj, void *arg, void *data, int flags)
{
	...
	if (ast_strlen_zero(name)) {
		ast_log_chan(NULL, LOG_ERROR, "BUG! Must supply a channel name or partial name to match!\n");
		return CMP_STOP;
	}
...
static int ast_channel_by_exten_cb(void *obj, void *arg, void *data, int flags)
{
	...
	if (ast_strlen_zero(exten) || ast_strlen_zero(context)) {
		ast_log_chan(NULL, LOG_ERROR, "BUG! Must have a context and extension to match!\n");
		return CMP_STOP;
	}
...
static int ast_channel_by_uniqueid_cb(void *obj, void *arg, void *data, int flags)
{
	...
	if (ast_strlen_zero(uniqueid)) {
		ast_log_chan(NULL, LOG_ERROR, "BUG! Must supply a uniqueid or partial uniqueid to match!\n");
		return CMP_STOP;
	}
...
{
	ast_log_chan(NULL, LOG_ERROR, "BUG! Should never be called!\n");
	return CMP_STOP;
}
...
void ast_hangup(struct ast_channel *chan)
{
	...
	if (ast_test_flag(ast_channel_flags(chan), AST_FLAG_BLOCKING)) {
		ast_log_chan(NULL, LOG_WARNING, "Hard hangup called by thread LWP %d on %s, while blocked by thread LWP %d in procedure %s!  Expect a failure\n",
	...
	case AST_STATE_RING:
		...
		do {
			...
			for (;;) {
				...
				ms = ast_waitfor(chan, ms);
				if (ms < 0) {
					ast_log_chan(NULL, LOG_WARNING, "Error condition occurred when polling channel %s for a voice frame: %s\n", ast_channel_name(chan), strerror(errno));
					res = -1;
					break;
				}
				if (ms == 0) {
	...
	while ((ms = ast_remaining_ms(start, timeout_ms))) {
		...
		if (!rchan && outfd < 0 && ms) {
			if (errno == 0 || errno == EINTR)
				continue;
			ast_log_chan(NULL, LOG_WARNING, "Wait failed (%s)\n", strerror(errno));
			ast_channel_clear_flag(c, AST_FLAG_END_DTMF_ONLY);
			return -1;
		} else if (outfd > -1) {
			/* The FD we were watching has something waiting */
			ast_log_chan(NULL, LOG_WARNING, "The FD we were waiting for has something waiting. Waitfordigit returning numeric 1\n");
			ast_channel_clear_flag(c, AST_FLAG_END_DTMF_ONLY);
			return 1;
		} else if (rchan) {
			int res;
			...
			case AST_FRAME_CONTROL:
				...
				case -1:
					...
					break;
				default:
					ast_log_chan(NULL, LOG_WARNING, "Unexpected control subclass '%d'\n", f->subclass.integer);
					break;
				}
				break;
			case AST_FRAME_VOICE:
	...
	if (ast_channel_timingfd(chan) > -1 && ast_channel_fdno(chan) == AST_TIMING_FD) {
		...
		case AST_TIMING_EVENT_EXPIRED:
			if (ast_timer_ack(ast_channel_timer(chan), 1) < 0) {
				ast_log_chan(NULL, LOG_ERROR, "Failed to acknowledge timer in ast_read\n");
				goto done;
			}
	...
	if (!AST_LIST_EMPTY(ast_channel_readq(chan))) {
		...
		if (ast_test_flag(ast_channel_flags(chan), AST_FLAG_EXCEPTION)) {
			if (ast_channel_tech(chan)->exception)
				...
				ast_log_chan(NULL, LOG_WARNING, "Exception flag set on '%s', but no exception handler\n", ast_channel_name(chan));
				f = &ast_null_frame;
			}
			...
			ast_log_chan(NULL, LOG_WARNING, "No read routine on channel %s\n", ast_channel_name(chan));
	}
	...
	if (f) {
		...
		*/
		...
		case AST_FRAME_DTMF_END:
			...
			ast_log_chan(NULL, LOG_DTMF, "DTMF end '%c' received on %s, duration %ld ms\n", f->subclass.integer, ast_channel_name(chan), f->len);
			...
			if (ast_test_flag(ast_channel_flags(chan), AST_FLAG_DEFER_DTMF) || ast_test_flag(ast_channel_flags(chan), AST_FLAG_EMULATE_DTMF)) {
				...
				if (!ast_tvzero(*ast_channel_dtmf_tv(chan)) &&
					...
					ast_log_chan(NULL, LOG_DTMF, "DTMF begin emulation of '%c' with duration %u queued on %s\n", f->subclass.integer, ast_channel_emulate_dtmf_duration(chan), ast_channel_name(chan));
				...
				if (ast_test_flag(ast_channel_flags(chan), AST_FLAG_IN_DTMF)) {
					ast_log_chan(NULL, LOG_DTMF, "DTMF end accepted with begin '%c' on %s\n", f->subclass.integer, ast_channel_name(chan));
					...
					if (ast_tvdiff_ms(now, *ast_channel_dtmf_tv(chan)) < option_dtmfminduration) {
						f->len = ast_tvdiff_ms(now, *ast_channel_dtmf_tv(chan));
						ast_log_chan(NULL, LOG_DTMF, "DTMF end '%c' detected to have actual duration %ld on the wire, emulation will be triggered on %s\n", f->subclass.integer, f->len, ast_channel_name(chan));
					}
				} else if (!f->len) {
					ast_log_chan(NULL, LOG_DTMF, "DTMF end accepted without begin '%c' on %s\n", f->subclass.integer, ast_channel_name(chan));
					f->len = option_dtmfminduration;
				}
				if (f->len < option_dtmfminduration && !ast_test_flag(ast_channel_flags(chan), AST_FLAG_END_DTMF_ONLY)) {
					ast_log_chan(NULL, LOG_DTMF, "DTMF end '%c' has duration %ld but want minimum %u, emulating on %s\n", f->subclass.integer, f->len, option_dtmfminduration, ast_channel_name(chan));
					ast_set_flag(ast_channel_flags(chan), AST_FLAG_EMULATE_DTMF);
					ast_channel_dtmf_digit_to_emulate_set(chan, f->subclass.integer);
					...
					ast_log_chan(NULL, LOG_DTMF, "DTMF end passthrough '%c' on %s\n", f->subclass.integer, ast_channel_name(chan));
					if (f->len < option_dtmfminduration) {
						f->len = option_dtmfminduration;
					}
		...
		case AST_FRAME_DTMF_BEGIN:
			...
			ast_log_chan(NULL, LOG_DTMF, "DTMF begin '%c' received on %s\n", f->subclass.integer, ast_channel_name(chan));
			if (ast_test_flag(ast_channel_flags(chan), AST_FLAG_DEFER_DTMF | AST_FLAG_END_DTMF_ONLY | AST_FLAG_EMULATE_DTMF) ||
			    (!ast_tvzero(*ast_channel_dtmf_tv(chan)) &&
			      ast_tvdiff_ms(ast_tvnow(), *ast_channel_dtmf_tv(chan)) < AST_MIN_DTMF_GAP) ) {
				ast_log_chan(NULL, LOG_DTMF, "DTMF begin ignored '%c' on %s\n", f->subclass.integer, ast_channel_name(chan));
				ast_frfree(f);
				f = &ast_null_frame;
			} else {
				struct timeval now = ast_tvnow();
				...
				ast_channel_dtmf_tv_set(chan, &now);
				ast_log_chan(NULL, LOG_DTMF, "DTMF begin passthrough '%c' on %s\n", f->subclass.integer, ast_channel_name(chan));
			}
			break;
		case AST_FRAME_NULL:
			...
			if (ast_test_flag(ast_channel_flags(chan), AST_FLAG_EMULATE_DTMF)) {
				...
				if (!ast_channel_emulate_dtmf_duration(chan)) {
					...
					ast_clear_flag(ast_channel_flags(chan), AST_FLAG_EMULATE_DTMF);
					ast_channel_dtmf_digit_to_emulate_set(chan, 0);
					ast_log_chan(NULL, LOG_DTMF, "DTMF end emulation of '%c' queued on %s\n", f->subclass.integer, ast_channel_name(chan));
					if (ast_channel_audiohooks(chan)) {
						struct ast_frame *old_frame = f;
						f = ast_audiohook_write_list(chan, ast_channel_audiohooks(chan), AST_AUDIOHOOK_DIRECTION_READ, f);
						if (old_frame != f) {
...
void ast_channel_hangupcause_hash_set(struct ast_channel *chan, const struct ast_control_pvt_cause_code *cause_code, int datalen)
{
	...
	if (ast_channel_dialed_causes_add(chan, cause_code, datalen)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to store hangup cause for %s on %s\n", cause_code->chan_name, ast_channel_name(chan));
	}
...
static int indicate_data_internal(struct ast_channel *chan, int _condition, const void *data, size_t datalen)
{
	...
	if (res) {
		/* not handled */
		ast_log_chan(NULL, LOG_WARNING, "Unable to handle indication %u for '%s'\n", condition, ast_channel_name(chan));
	}
...
int ast_senddigit_mf_begin(struct ast_channel *chan, char digit)
{
	...
	if (digit >= '0' && digit <='9') {
		...
		/* not handled */
		ast_log_chan(NULL, LOG_WARNING, "Unable to generate MF tone '%c' for '%s'\n", digit, ast_channel_name(chan));
	}
	return 0;
}
...
int ast_prod(struct ast_channel *chan)
{
	...
	if (ast_channel_state(chan) != AST_STATE_UP) {
		...
		if (ast_write(chan, &a))
			ast_log_chan(NULL, LOG_WARNING, "Prodding channel '%s' failed\n", ast_channel_name(chan));
	}
	return 0;
}
...
static int set_format(struct ast_channel *chan, struct ast_format_cap *cap_set, const int direction, int interleaved_stereo)
{
	...
	if (ast_format_cap_empty(cap_native)) {
		ast_channel_unlock(chan);
		ast_log_chan(NULL, LOG_ERROR, "Unable to set format because channel %s supports no formats\n",
				ast_channel_name(chan));
		return -1;
	}
	...
	if (res < 0) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Unable to find a codec translation path: %s -> %s\n",
...
struct ast_channel *ast_call_forward(struct ast_channel *caller, struct ast_channel *orig, int *timeout, struct ast_format_cap *cap, struct outgoing_helper *oh, int *outstate)
{
	...
	if (!(new_chan = ast_request(type, cap, NULL, orig, data, &cause))) {
		ast_log_chan(NULL, LOG_NOTICE, "Unable to create channel for call forward to '%s/%s' (cause = %d)\n", type, data, cause);
	...
	if (res) {
		ast_log_chan(NULL, LOG_NOTICE, "Unable to call forward to channel %s/%s\n", type, (char *)data);
		ast_hangup(orig);
		ast_hangup(new_chan);
		return NULL;
	}
...
struct ast_channel *__ast_request_and_dial(const char *type, struct ast_format_cap *cap, const struct ast_assigned_ids *assignedids, const struct ast_channel *requestor, const char *addr, int timeout, int *outstate, const char *cid_num, const char *cid_name, struct outgoing_helper *oh)
{
	...
	if (!chan) {
		ast_log_chan(NULL, LOG_NOTICE, "Unable to request channel %s/%s\n", type, addr);
		handle_cause(cause, outstate);
		return NULL;
	}
	...
	if (ast_call(chan, addr, 0)) {	/* ast_call failed... */
		ast_log_chan(NULL, LOG_NOTICE, "Unable to call channel %s/%s\n", type, addr);
	} else {
		struct timeval start = ast_tvnow();
		...
		while (timeout && ast_channel_state(chan) != AST_STATE_UP) {
			...
			if (f->frametype == AST_FRAME_CONTROL) {
				...
				default:
					ast_log_chan(NULL, LOG_NOTICE, "Don't know what to do with control frame %d\n", f->subclass.integer);
				}
	...
	const struct ast_assigned_ids *assignedids, const struct ast_channel *requestor, const char *addr, int *cause)
{
	...
	*cause = AST_CAUSE_NOTDEFINED;
	...
	if (AST_RWLIST_RDLOCK(&backends)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to lock technology backend list\n");
		return NULL;
	}
	...
	if (!chan) {
		ast_log_chan(NULL, LOG_WARNING, "No channel type registered for '%s'\n", type);
		...
		if (!ast_format_cap_empty(tmp_cap)) {
			...
			*/
			...
			if (res < 0) {
				...
				ast_log_chan(NULL, LOG_WARNING, "No translator path exists for channel type %s (native %s) to %s\n", type,
	...
	if (set_security_requirements(requestor, c)) {
		ast_log_chan(NULL, LOG_WARNING, "Setting security requirements failed\n");
		ast_hangup(c);
		*cause = AST_CAUSE_BEARERCAPABILITY_NOTAVAIL;
		return NULL;
	}
...
static int ast_channel_make_compatible_helper(struct ast_channel *from, struct ast_channel *to)
{
	...
	if (no_path) {
		ast_log_chan(NULL, LOG_WARNING, "No path to translate from %s to %s\n",
	...
	if (ast_set_read_format(from, best_src_fmt)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to set read format on channel %s to %s\n",
			ast_channel_name(from), ast_format_get_name(best_src_fmt));
		return -1;
	}
	if (ast_set_write_format(to, best_src_fmt)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to set write format on channel %s to %s\n",
...
static void channel_do_masquerade(struct ast_channel *original, struct ast_channel *clonechan)
{
	...
	*ast_channel_hangup_handlers(clonechan) = exchange.handlers;
	...
	if (ast_channel_tech(original)->fixup && ast_channel_tech(original)->fixup(clonechan, original)) {
		ast_log_chan(NULL, LOG_WARNING, "Channel type '%s' could not fixup channel %s, strange things may happen. (clonechan)\n",
	...
	if (ast_channel_tech(clonechan)->fixup && ast_channel_tech(clonechan)->fixup(original, clonechan)) {
		ast_log_chan(NULL, LOG_WARNING, "Channel type '%s' could not fixup channel %s, strange things may happen. (original)\n",
...
int ast_channel_setoption(struct ast_channel *chan, int option, void *data, int datalen, int block)
{
	...
	if (block)
		ast_log_chan(NULL, LOG_ERROR, "XXX Blocking not implemented yet XXX\n");
...
static void *tonepair_alloc(struct ast_channel *chan, void *params)
{
	...
	if (ast_set_write_format(chan, ast_format_slin)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to set '%s' to signed linear format (write)\n", ast_channel_name(chan));
...
static int tonepair_generator(struct ast_channel *chan, void *data, int len, int samples)
{
	...
	if (len > sizeof(ts->data) / 2 - 1) {
		ast_log_chan(NULL, LOG_WARNING, "Can't generate that much data!\n");
		return -1;
	}
...
ast_group_t ast_get_group(const char *s)
{
	...
	while ((piece = strsep(&c, ","))) {
		if (sscanf(piece, "%30d-%30d", &start, &finish) == 2) {
			...
			ast_log_chan(NULL, LOG_ERROR, "Syntax error parsing group configuration '%s' at '%s'. Ignoring.\n", s, piece);
			continue;
		}
		for (x = start; x <= finish; x++) {
			if ((x > 63) || (x < 0)) {
				ast_log_chan(NULL, LOG_WARNING, "Ignoring invalid group %d (maximum group is 63)\n", x);
			} else
...
int ast_moh_start(struct ast_channel *chan, const char *mclass, const char *interpclass)
{
	...
	ast_verb_chan(NULL, 3, "Music class %s requested but no musiconhold loaded.\n", mclass ? mclass : (interpclass ? interpclass : "default"));
...
struct ast_silence_generator *ast_channel_start_silence_generator(struct ast_channel *chan)
{
	...
	if (ast_set_write_format(chan, ast_format_slin) < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Could not set write format to SLINEAR\n");
		ast_free(state);
		return NULL;
	}
...
void ast_channel_stop_silence_generator(struct ast_channel *chan, struct ast_silence_generator *state)
{
	...
	if (deactivate_silence_generator(chan)) {
		...
		if (ast_set_write_format(chan, state->old_write_format) < 0) {
			ast_log_chan(NULL, LOG_ERROR, "Could not return write format to its original state\n");
		}
	}
 ...
 * must be able to build gabpbx even without it (using a loadable 'say'
...
static int party_name_build_data(unsigned char *data, size_t datalen, const struct ast_party_name *name, const char *label, const struct ast_party_name_ies *ies)
{
	...
	if (name->str) {
		...
		if (datalen < pos + (sizeof(data[0]) * 2) + length) {
			ast_log_chan(NULL, LOG_WARNING, "No space left for %s name\n", label);
			return -1;
		}
		data[pos++] = ies->str;
		data[pos++] = length;
	...
	if (datalen < pos + (sizeof(data[0]) * 2) + 1) {
		ast_log_chan(NULL, LOG_WARNING, "No space left for %s name char set\n", label);
		return -1;
	}
	...
	if (datalen < pos + (sizeof(data[0]) * 2) + 1) {
		ast_log_chan(NULL, LOG_WARNING, "No space left for %s name presentation\n", label);
		return -1;
	}
	...
	if (datalen < pos + (sizeof(data[0]) * 2) + 1) {
		ast_log_chan(NULL, LOG_WARNING, "No space left for %s name valid\n", label);
		return -1;
	}
	data[pos++] = ies->valid;
	data[pos++] = 1;
...
static int party_number_build_data(unsigned char *data, size_t datalen, const struct ast_party_number *number, const char *label, const struct ast_party_number_ies *ies)
{
	...
	if (number->str) {
		...
		if (datalen < pos + (sizeof(data[0]) * 2) + length) {
			ast_log_chan(NULL, LOG_WARNING, "No space left for %s number\n", label);
			return -1;
		}
		data[pos++] = ies->str;
		data[pos++] = length;
	...
	if (datalen < pos + (sizeof(data[0]) * 2) + 1) {
		ast_log_chan(NULL, LOG_WARNING, "No space left for %s numbering plan\n", label);
		return -1;
	}
	data[pos++] = ies->plan;
	data[pos++] = 1;
	...
	if (datalen < pos + (sizeof(data[0]) * 2) + 1) {
		ast_log_chan(NULL, LOG_WARNING, "No space left for %s number presentation\n", label);
		return -1;
	}
	data[pos++] = ies->presentation;
	data[pos++] = 1;
	...
	if (datalen < pos + (sizeof(data[0]) * 2) + 1) {
		ast_log_chan(NULL, LOG_WARNING, "No space left for %s number valid\n", label);
		return -1;
	}
	data[pos++] = ies->valid;
	data[pos++] = 1;
...
static int party_subaddress_build_data(unsigned char *data, size_t datalen, const struct ast_party_subaddress *subaddress, const char *label, const struct ast_party_subaddress_ies *ies)
{
	...
	if (subaddress->str) {
		...
		if (datalen < pos + (sizeof(data[0]) * 2) + length) {
			ast_log_chan(NULL, LOG_WARNING, "No space left for %s subaddress\n", label);
			return -1;
		}
		data[pos++] = ies->str;
		data[pos++] = length;
	...
	if (datalen < pos + (sizeof(data[0]) * 2) + 1) {
		ast_log_chan(NULL, LOG_WARNING, "No space left for %s type of subaddress\n", label);
		return -1;
	}
	data[pos++] = ies->type;
	data[pos++] = 1;
	...
	if (datalen < pos + (sizeof(data[0]) * 2) + 1) {
		ast_log_chan(NULL, LOG_WARNING, "No space left for %s subaddress valid\n", label);
		return -1;
	}
	data[pos++] = ies->valid;
	data[pos++] = 1;
	...
	const struct ast_set_party_id *update)
{
	...
	if (id->tag) {
		...
		if (datalen < pos + (sizeof(data[0]) * 2) + length) {
			ast_log_chan(NULL, LOG_WARNING, "No space left for %s tag\n", label);
			return -1;
		}
		data[pos++] = ies->tag;
		data[pos++] = length;
	...
	if (ies->combined_presentation && (!update || update->number)) {
		...
		if (datalen < pos + (sizeof(data[0]) * 2) + 1) {
			ast_log_chan(NULL, LOG_WARNING, "No space left for %s combined presentation\n", label);
			return -1;
		}
...
int ast_connected_line_build_data(unsigned char *data, size_t datalen, const struct ast_party_connected_line *connected, const struct ast_set_party_connected_line *update)
{
	...
	if (datalen < pos + (sizeof(data[0]) * 2) + 1) {
		ast_log_chan(NULL, LOG_WARNING, "No space left for connected line frame version\n");
		return -1;
	}
	...
	if (datalen < pos + (sizeof(data[0]) * 2) + sizeof(value)) {
		ast_log_chan(NULL, LOG_WARNING, "No space left for connected line source\n");
		return -1;
	}
...
int ast_connected_line_parse_data(const unsigned char *data, size_t datalen, struct ast_party_connected_line *connected)
{
	...
	for (pos = 0; pos < datalen; pos += ie_len) {
		if (datalen < pos + sizeof(ie_id) + sizeof(ie_len)) {
			ast_log_chan(NULL, LOG_WARNING, "Invalid connected line update\n");
			return -1;
		}
		ie_id = data[pos++];
		ie_len = data[pos++];
...
/* Connected line party frame version */
		case AST_CONNECTED_LINE_VERSION:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid connected line frame version (%u)\n",
					(unsigned) ie_len);
				break;
			}
...
/* Connected line party id name */
		...
		case AST_CONNECTED_LINE_NAME_CHAR_SET:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid connected line name char set (%u)\n",
					(unsigned) ie_len);
				break;
			}
		...
		case AST_CONNECTED_LINE_NAME_PRESENTATION:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid connected line name presentation (%u)\n",
					(unsigned) ie_len);
				break;
			}
		...
		case AST_CONNECTED_LINE_NAME_VALID:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid connected line name valid (%u)\n",
					(unsigned) ie_len);
				break;
			}
...
/* Connected line party id number */
		...
		case AST_CONNECTED_LINE_NUMBER_PLAN:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid connected line numbering plan (%u)\n",
					(unsigned) ie_len);
				break;
			}
		...
		case AST_CONNECTED_LINE_NUMBER_PRESENTATION:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid connected line number presentation (%u)\n",
					(unsigned) ie_len);
				break;
			}
		...
		case AST_CONNECTED_LINE_NUMBER_VALID:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid connected line number valid (%u)\n",
					(unsigned) ie_len);
				break;
			}
...
/* Connected line party id subaddress */
		...
		case AST_CONNECTED_LINE_SUBADDRESS_TYPE:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid connected line type of subaddress (%u)\n",
					(unsigned) ie_len);
				break;
			}
		...
		case AST_CONNECTED_LINE_SUBADDRESS_VALID:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid connected line subaddress valid (%u)\n",
					(unsigned) ie_len);
				break;
			}
...
/* Connected line party id combined presentation */
		case AST_CONNECTED_LINE_ID_PRESENTATION:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid connected line combined presentation (%u)\n",
					(unsigned) ie_len);
				break;
			}
...
/* Private connected line party id name */
		...
		case AST_CONNECTED_LINE_PRIV_NAME_CHAR_SET:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid connected line private name char set (%u)\n",
					(unsigned) ie_len);
				break;
			}
		...
		case AST_CONNECTED_LINE_PRIV_NAME_PRESENTATION:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid connected line private name presentation (%u)\n",
					(unsigned) ie_len);
				break;
			}
		...
		case AST_CONNECTED_LINE_PRIV_NAME_VALID:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid connected line private name valid (%u)\n",
					(unsigned) ie_len);
				break;
			}
...
/* Private connected line party id number */
		...
		case AST_CONNECTED_LINE_PRIV_NUMBER_PLAN:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid connected line private numbering plan (%u)\n",
					(unsigned) ie_len);
				break;
			}
		...
		case AST_CONNECTED_LINE_PRIV_NUMBER_PRESENTATION:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid connected line private number presentation (%u)\n",
					(unsigned) ie_len);
				break;
			}
		...
		case AST_CONNECTED_LINE_PRIV_NUMBER_VALID:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid connected line private number valid (%u)\n",
					(unsigned) ie_len);
				break;
			}
...
/* Private connected line party id subaddress */
		...
		case AST_CONNECTED_LINE_PRIV_SUBADDRESS_TYPE:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid connected line private type of subaddress (%u)\n",
					(unsigned) ie_len);
				break;
			}
		...
		case AST_CONNECTED_LINE_PRIV_SUBADDRESS_VALID:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid connected line private subaddress valid (%u)\n",
					(unsigned) ie_len);
				break;
			}
...
/* Connected line party source */
		case AST_CONNECTED_LINE_SOURCE:
			if (ie_len != sizeof(value)) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid connected line source (%u)\n",
					(unsigned) ie_len);
				break;
			}
		...
		const struct ast_party_redirecting_reason_ies *ies)
{
	...
	if (datalen < pos + (sizeof(data[0]) * 2) + sizeof(value)) {
		ast_log_chan(NULL, LOG_WARNING, "No space left for %s code\n", label);
		return -1;
	}
	data[pos++] = ies->code;
	data[pos++] = sizeof(value);
	...
	if (reason->str) {
		...
		if (datalen < pos + (sizeof(data[0]) * 2) + length) {
			ast_log_chan(NULL, LOG_WARNING, "No space left for %s string\n", label);
			return -1;
		}
		data[pos++] = ies->str;
		data[pos++] = length;
...
int ast_redirecting_build_data(unsigned char *data, size_t datalen, const struct ast_party_redirecting *redirecting, const struct ast_set_party_redirecting *update)
{
	...
	if (datalen < pos + (sizeof(data[0]) * 2) + 1) {
		ast_log_chan(NULL, LOG_WARNING, "No space left for redirecting frame version\n");
		return -1;
	}
	...
	/* Redirecting count */
	if (datalen < pos + (sizeof(data[0]) * 2) + sizeof(value)) {
		ast_log_chan(NULL, LOG_WARNING, "No space left for redirecting count\n");
		return -1;
	}
...
int ast_redirecting_parse_data(const unsigned char *data, size_t datalen, struct ast_party_redirecting *redirecting)
{
	...
	for (pos = 0; pos < datalen; pos += ie_len) {
		if (datalen < pos + sizeof(ie_id) + sizeof(ie_len)) {
			ast_log_chan(NULL, LOG_WARNING, "Invalid redirecting update\n");
			return -1;
		}
		ie_id = data[pos++];
		ie_len = data[pos++];
...
/* Redirecting frame version */
		case AST_REDIRECTING_VERSION:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid redirecting frame version (%u)\n",
					(unsigned) ie_len);
				break;
			}
			frame_version = data[pos];
...
/* Redirecting-orig party id name */
		...
		case AST_REDIRECTING_ORIG_NAME_CHAR_SET:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid redirecting-orig name char set (%u)\n",
					(unsigned) ie_len);
				break;
			}
		...
		case AST_REDIRECTING_ORIG_NAME_PRESENTATION:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid redirecting-orig name presentation (%u)\n",
					(unsigned) ie_len);
				break;
			}
		...
		case AST_REDIRECTING_ORIG_NAME_VALID:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid redirecting-orig name valid (%u)\n",
					(unsigned) ie_len);
				break;
			}
...
/* Redirecting-orig party id number */
		...
		case AST_REDIRECTING_ORIG_NUMBER_PLAN:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid redirecting-orig numbering plan (%u)\n",
					(unsigned) ie_len);
				break;
			}
		...
		case AST_REDIRECTING_ORIG_NUMBER_PRESENTATION:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid redirecting-orig number presentation (%u)\n",
					(unsigned) ie_len);
				break;
			}
		...
		case AST_REDIRECTING_ORIG_NUMBER_VALID:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid redirecting-orig number valid (%u)\n",
					(unsigned) ie_len);
				break;
			}
...
/* Redirecting-orig party id subaddress */
		...
		case AST_REDIRECTING_ORIG_SUBADDRESS_TYPE:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid redirecting-orig type of subaddress (%u)\n",
					(unsigned) ie_len);
				break;
			}
		...
		case AST_REDIRECTING_ORIG_SUBADDRESS_VALID:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid redirecting-orig subaddress valid (%u)\n",
					(unsigned) ie_len);
				break;
			}
...
/* Redirecting-from party id name */
		...
		case AST_REDIRECTING_FROM_NAME_CHAR_SET:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid redirecting-from name char set (%u)\n",
					(unsigned) ie_len);
				break;
			}
		...
		case AST_REDIRECTING_FROM_NAME_PRESENTATION:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid redirecting-from name presentation (%u)\n",
					(unsigned) ie_len);
				break;
			}
		...
		case AST_REDIRECTING_FROM_NAME_VALID:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid redirecting-from name valid (%u)\n",
					(unsigned) ie_len);
				break;
			}
...
/* Redirecting-from party id number */
		...
		case AST_REDIRECTING_FROM_NUMBER_PLAN:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid redirecting-from numbering plan (%u)\n",
					(unsigned) ie_len);
				break;
			}
		...
		case AST_REDIRECTING_FROM_NUMBER_PRESENTATION:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid redirecting-from number presentation (%u)\n",
					(unsigned) ie_len);
				break;
			}
		...
		case AST_REDIRECTING_FROM_NUMBER_VALID:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid redirecting-from number valid (%u)\n",
					(unsigned) ie_len);
				break;
			}
...
/* Redirecting-from party id combined presentation */
		case AST_REDIRECTING_FROM_ID_PRESENTATION:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid redirecting-from combined presentation (%u)\n",
					(unsigned) ie_len);
				break;
			}
...
/* Redirecting-from party id subaddress */
		...
		case AST_REDIRECTING_FROM_SUBADDRESS_TYPE:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid redirecting-from type of subaddress (%u)\n",
					(unsigned) ie_len);
				break;
			}
		...
		case AST_REDIRECTING_FROM_SUBADDRESS_VALID:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid redirecting-from subaddress valid (%u)\n",
					(unsigned) ie_len);
				break;
			}
...
/* Redirecting-to party id name */
		...
		case AST_REDIRECTING_TO_NAME_CHAR_SET:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid redirecting-to name char set (%u)\n",
					(unsigned) ie_len);
				break;
			}
		...
		case AST_REDIRECTING_TO_NAME_PRESENTATION:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid redirecting-to name presentation (%u)\n",
					(unsigned) ie_len);
				break;
			}
		...
		case AST_REDIRECTING_TO_NAME_VALID:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid redirecting-to name valid (%u)\n",
					(unsigned) ie_len);
				break;
			}
...
/* Redirecting-to party id number */
		...
		case AST_REDIRECTING_TO_NUMBER_PLAN:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid redirecting-to numbering plan (%u)\n",
					(unsigned) ie_len);
				break;
			}
		...
		case AST_REDIRECTING_TO_NUMBER_PRESENTATION:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid redirecting-to number presentation (%u)\n",
					(unsigned) ie_len);
				break;
			}
		...
		case AST_REDIRECTING_TO_NUMBER_VALID:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid redirecting-to number valid (%u)\n",
					(unsigned) ie_len);
				break;
			}
...
/* Redirecting-to party id combined presentation */
		case AST_REDIRECTING_TO_ID_PRESENTATION:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid redirecting-to combined presentation (%u)\n",
					(unsigned) ie_len);
				break;
			}
...
/* Redirecting-to party id subaddress */
		...
		case AST_REDIRECTING_TO_SUBADDRESS_TYPE:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid redirecting-to type of subaddress (%u)\n",
					(unsigned) ie_len);
				break;
			}
		...
		case AST_REDIRECTING_TO_SUBADDRESS_VALID:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid redirecting-to subaddress valid (%u)\n",
					(unsigned) ie_len);
				break;
			}
...
/* Private redirecting-orig party id name */
		...
		case AST_REDIRECTING_PRIV_ORIG_NAME_CHAR_SET:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid private redirecting-orig name char set (%u)\n",
					(unsigned) ie_len);
				break;
			}
		...
		case AST_REDIRECTING_PRIV_ORIG_NAME_PRESENTATION:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid private redirecting-orig name presentation (%u)\n",
					(unsigned) ie_len);
				break;
			}
		...
		case AST_REDIRECTING_PRIV_ORIG_NAME_VALID:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid private redirecting-orig name valid (%u)\n",
					(unsigned) ie_len);
				break;
			}
...
/* Private redirecting-orig party id number */
		...
		case AST_REDIRECTING_PRIV_ORIG_NUMBER_PLAN:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid private redirecting-orig numbering plan (%u)\n",
					(unsigned) ie_len);
				break;
			}
		...
		case AST_REDIRECTING_PRIV_ORIG_NUMBER_PRESENTATION:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid private redirecting-orig number presentation (%u)\n",
					(unsigned) ie_len);
				break;
			}
		...
		case AST_REDIRECTING_PRIV_ORIG_NUMBER_VALID:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid private redirecting-orig number valid (%u)\n",
					(unsigned) ie_len);
				break;
			}
...
/* Private redirecting-orig party id subaddress */
		...
		case AST_REDIRECTING_PRIV_ORIG_SUBADDRESS_TYPE:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid private redirecting-orig type of subaddress (%u)\n",
					(unsigned) ie_len);
				break;
			}
		...
		case AST_REDIRECTING_PRIV_ORIG_SUBADDRESS_VALID:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid private redirecting-orig subaddress valid (%u)\n",
					(unsigned) ie_len);
				break;
			}
...
/* Private redirecting-from party id name */
		...
		case AST_REDIRECTING_PRIV_FROM_NAME_CHAR_SET:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid private redirecting-from name char set (%u)\n",
					(unsigned) ie_len);
				break;
			}
		...
		case AST_REDIRECTING_PRIV_FROM_NAME_PRESENTATION:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid private redirecting-from name presentation (%u)\n",
					(unsigned) ie_len);
				break;
			}
		...
		case AST_REDIRECTING_PRIV_FROM_NAME_VALID:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid private redirecting-from name valid (%u)\n",
					(unsigned) ie_len);
				break;
			}
...
/* Private redirecting-from party id number */
		...
		case AST_REDIRECTING_PRIV_FROM_NUMBER_PLAN:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid private redirecting-from numbering plan (%u)\n",
					(unsigned) ie_len);
				break;
			}
		...
		case AST_REDIRECTING_PRIV_FROM_NUMBER_PRESENTATION:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid private redirecting-from number presentation (%u)\n",
					(unsigned) ie_len);
				break;
			}
		...
		case AST_REDIRECTING_PRIV_FROM_NUMBER_VALID:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid private redirecting-from number valid (%u)\n",
					(unsigned) ie_len);
				break;
			}
...
/* Private redirecting-from party id subaddress */
		...
		case AST_REDIRECTING_PRIV_FROM_SUBADDRESS_TYPE:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid private redirecting-from type of subaddress (%u)\n",
					(unsigned) ie_len);
				break;
			}
		...
		case AST_REDIRECTING_PRIV_FROM_SUBADDRESS_VALID:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid private redirecting-from subaddress valid (%u)\n",
					(unsigned) ie_len);
				break;
			}
...
/* Private redirecting-to party id name */
		...
		case AST_REDIRECTING_PRIV_TO_NAME_CHAR_SET:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid private redirecting-to name char set (%u)\n",
					(unsigned) ie_len);
				break;
			}
		...
		case AST_REDIRECTING_PRIV_TO_NAME_PRESENTATION:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid private redirecting-to name presentation (%u)\n",
					(unsigned) ie_len);
				break;
			}
		...
		case AST_REDIRECTING_PRIV_TO_NAME_VALID:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid private redirecting-to name valid (%u)\n",
					(unsigned) ie_len);
				break;
			}
...
/* Private redirecting-to party id number */
		...
		case AST_REDIRECTING_PRIV_TO_NUMBER_PLAN:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid private redirecting-to numbering plan (%u)\n",
					(unsigned) ie_len);
				break;
			}
		...
		case AST_REDIRECTING_PRIV_TO_NUMBER_PRESENTATION:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid private redirecting-to number presentation (%u)\n",
					(unsigned) ie_len);
				break;
			}
		...
		case AST_REDIRECTING_PRIV_TO_NUMBER_VALID:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid private redirecting-to number valid (%u)\n",
					(unsigned) ie_len);
				break;
			}
...
/* Private redirecting-to party id subaddress */
		...
		case AST_REDIRECTING_PRIV_TO_SUBADDRESS_TYPE:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid private redirecting-to type of subaddress (%u)\n",
					(unsigned) ie_len);
				break;
			}
		...
		case AST_REDIRECTING_PRIV_TO_SUBADDRESS_VALID:
			if (ie_len != 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid private redirecting-to subaddress valid (%u)\n",
					(unsigned) ie_len);
				break;
			}
...
/* Redirecting reason code */
		case AST_REDIRECTING_REASON_CODE:
			if (ie_len != sizeof(value)) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid redirecting reason (%u)\n",
					(unsigned) ie_len);
				break;
			}
			memcpy(&value, data + pos, sizeof(value));
...
/* Redirecting orig-reason code */
		case AST_REDIRECTING_ORIG_REASON_CODE:
			if (ie_len != sizeof(value)) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid redirecting original reason (%u)\n",
					(unsigned) ie_len);
				break;
			}
			memcpy(&value, data + pos, sizeof(value));
...
/* Redirecting count */
		case AST_REDIRECTING_COUNT:
			if (ie_len != sizeof(value)) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid redirecting count (%u)\n",
					(unsigned) ie_len);
				break;
			}
			memcpy(&value, data + pos, sizeof(value));
...
static void channel_set_intercept_mode(int in_intercept_mode)
{
	...
	if (status) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to set dialplan intercept mode\n");
	}
}
...
int ast_channel_move(struct ast_channel *dest, struct ast_channel *source)
{
	...
	if (dest == source) {
		ast_log_chan(NULL, LOG_WARNING, "Can't move channel '%s' into itself!\n",
...
int ast_channel_suppress(struct ast_channel *chan, unsigned int direction, enum ast_frame_type frametype)
{
	...
	if (!(datastore_info = suppress_get_datastore_information(frametype))) {
		ast_log_chan(NULL, LOG_WARNING, "Attempted to suppress an unsupported frame type (%u).\n", frametype);
		return -1;
	}
	...
	if (!(suppress = ao2_alloc(sizeof(*suppress), NULL))) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to allocate data while attempting to suppress a stream.\n");
		return -1;
	}
	...
	if (framehook_id < 0) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Failed to attach framehook while attempting to suppress a stream.\n");
		ao2_ref(suppress, -1);
		return -1;
	}
	...
	if (!(datastore = ast_datastore_alloc(datastore_info, NULL))) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to allocate datastore while attempting to suppress a stream.\n");
...
int ast_channel_unsuppress(struct ast_channel *chan, unsigned int direction, enum ast_frame_type frametype)
{
	...
	if (!(datastore_info = suppress_get_datastore_information(frametype))) {
		ast_log_chan(NULL, LOG_WARNING, "Attempted to unsuppress an unsupported frame type (%u).\n", frametype);
		return -1;
	}
...
void ast_channel_end_dtmf(struct ast_channel *chan, char digit, struct timeval start, const char *why)
{
	...
	ast_log_chan(NULL, LOG_DTMF, "DTMF end '%c' simulated on %s due to %s, duration %ld ms\n",
