...
struct ast_channel {
	...
	struct ao2_container *dialed_causes;		/*!< Contains tech-specific and GABpbx cause data from dialed channels */
	...
	const struct ast_channel *requestor, const struct ast_channel_initializers *initializers, const char *file, int line, const char *function)
{
	...
	if (initializers) {
		if (initializers->version == 0) {
			ast_log_chan(NULL, LOG_ERROR, "Channel initializers must have a non-zero version.\n");
			return ast_channel_unref(tmp);
		} else if (initializers->version != AST_CHANNEL_INITIALIZERS_VERSION) {
			ast_log_chan(NULL, LOG_ERROR, "ABI mismatch for ast_channel_initializers. "
