 ...
 *	   if uid == -2 and gid == -2 is because rgabpbx client didn't send
...
static char *handle_load(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	case CLI_INIT:
		...
		e->usage =
			...
			"       Loads the specified module into GABpbx.\n";
		return NULL;
...
static char *handle_reload(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	for (x = e->args; x < a->argc; x++) {
		...
		case AST_MODULE_RELOAD_QUEUED:
			ast_cli(a->fd, "GABpbx cannot reload a module yet; request queued\n");
			break;
...
static char *handle_refresh(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	case CLI_INIT:
		...
		e->usage =
			...
			"       Unloads and loads the specified module into GABpbx.\n"
...
static char *handle_unload(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	case CLI_INIT:
		...
		e->usage =
			...
			"       Unloads the specified module from GABpbx. The -f\n"
...
static char * handle_showuptime(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	case CLI_INIT:
		...
		e->usage =
			...
			"       Shows GABpbx uptime information.\n"
...
static char *handle_modlist(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	case CLI_INIT:
		...
		e->usage =
			...
			"       Shows GABpbx modules currently in use, and usage statistics.\n";
		return NULL;
...
static char *handle_cli_check_permissions(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	if (a->argc == 4) {
		...
		if (!found) {
			ast_cli(a->fd, "You are not allowed to run any command on GABpbx\n");
		}
	} else {
...
static char *handle_cli_wait_fullybooted(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	case CLI_INIT:
		...
		e->usage =
			...
			"	Wait until GABpbx has fully booted.\n";
		return NULL;
	case CLI_GENERATE:
		return NULL;
	}
	...
	ast_cli(a->fd, "GABpbx has fully booted.\n");
 ...
 *
 * Users have reported that gabpbx will sometimes be killed because it can't allocate
...
static struct ast_cli_entry cli_cli[] = {
	...
	AST_CLI_DEFINE(handle_cli_wait_fullybooted, "Wait for GABpbx to be fully booted"),
...
static int set_full_cmd(struct ast_cli_entry *e)
{
	...
	if (!e->_full_cmd) {
		ast_log_chan(NULL, LOG_WARNING, "-- cannot allocate <%s>\n", buf);
		return -1;
	}
...
int ast_cli_perms_init(int reload)
{
	...
	if (ast_mutex_trylock(&permsconfiglock)) {
		ast_log_chan(NULL, LOG_NOTICE, "You must wait until last 'cli reload permissions' command finish\n");
		return 1;
	}
	...
	while ((cat = ast_category_browse(cfg, cat))) {
		...
		if (cat[0] == '@') {
			...
			gr = getgrnam(&cat[1]);
			if (!gr) {
				ast_log_chan(NULL, LOG_WARNING, "Unknown group '%s'\n", &cat[1]);
				continue;
			}
		} else {
			/* This is a user */
			pw = getpwnam(cat);
			if (!pw) {
				ast_log_chan(NULL, LOG_WARNING, "Unknown user '%s'\n", cat);
				continue;
			}
		}
		user_group = NULL;
		...
		for (v = ast_variable_browse(cfg, cat); v; v = v->next) {
			if (ast_strlen_zero(v->value)) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Empty permit/deny option in user '%s'\n", cat);
				continue;
			}
			if (!strcasecmp(v->name, "permit")) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Unknown '%s' option\n", v->name);
				continue;
			}
			if (perm) {
...
{
	if (e->inuse) {
		ast_log_chan(NULL, LOG_WARNING, "Can't remove command that is in use\n");
	} else {
...
int __ast_cli_register(struct ast_cli_entry *e, struct ast_module *module)
{
	...
	if (cli_is_registered(e)) {
		ast_log_chan(NULL, LOG_WARNING, "Command '%s' already registered (the same ast_cli_entry)\n",
	...
	*dst++ = NULL;
	...
	if (find_cli(e->cmda, 1)) {
		ast_log_chan(NULL, LOG_WARNING, "Command '%s' already registered (or something close enough)\n",
			S_OR(e->_full_cmd, e->command));
		goto done;
	}
	if (set_full_cmd(e)) {
		ast_log_chan(NULL, LOG_WARNING, "Error registering CLI Command '%s'\n",
			S_OR(e->_full_cmd, e->command));
		goto done;
	}
...
static char *parse_args(const char *s, int *argc, const char *argv[], int max, int *trailingwhitespace)
{
	...
	*trailingwhitespace = 0;
	...
	for (; *s ; s++) {
		if (x >= max - 1) {
			ast_log_chan(NULL, LOG_WARNING, "Too many arguments, truncating at %s\n", s);
			break;
		}
		if (*s == '"' && !escaped) {
			quoted = !quoted;
...
struct ast_vector_string *ast_cli_completion_vector(const char *text, const char *word)
{
	...
	if (ast_threadstorage_set_ptr(&completion_storage, vec)) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to initialize threadstorage for completion.\n");
		ast_free(vec);
