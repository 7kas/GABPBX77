...
void *aco_pending_config(struct aco_info *info)
{
	if (!(info && info->internal)) {
		ast_log_chan(NULL, LOG_ERROR, "This may not be called without an initialized aco_info!\n");
		return NULL;
	}
...
static int link_option_to_types(struct aco_info *info, struct aco_type **types, struct aco_option *opt)
{
	...
	while ((type = types[idx++])) {
		if (!type->internal) {
			ast_log_chan(NULL, LOG_ERROR, "Attempting to register option using uninitialized type\n");
			return -1;
		}
	...
	unsigned int no_doc, size_t argc, ...)
{
	...
	if (!opt->handler && !(opt->handler = ast_config_option_default_handler(opt->type))) {
		...
		ast_log_chan(NULL, LOG_ERROR, "No handler provided, and no default handler exists for type %u\n", opt->type);
		ao2_ref(opt, -1);
		return -1;
	};
...
static int find_option_cb(void *obj, void *arg, int flags)
{
	...
	ast_log_chan(NULL, LOG_ERROR, "Unknown match type. This should not be possible.\n");
	return CMP_STOP;
}
...
static struct aco_option *aco_option_find(struct aco_type *type, const char *name)
{
	...
	if (!type || !type->internal || !type->internal->opts) {
		ast_log_chan(NULL, LOG_NOTICE, "Attempting to use NULL or uninitialized config type\n");
		return NULL;
	}
	...
	for (x = 0, match = file->types[x]; match; match = file->types[++x]) {
		...
		if (!ast_strlen_zero(match->matchfield) && (!ast_strlen_zero(match->matchvalue) || match->matchfunc)) {
			if (!(val = ast_variable_retrieve(cfg, category, match->matchfield))) {
				ast_log_chan(NULL, LOG_ERROR, "Required match field '%s' not found\n", match->matchfield);
				return NULL;
			}
			if (match->matchfunc) {
...
static int process_category(struct ast_config *cfg, struct aco_info *info, struct aco_file *file, const char *cat, int preload) {
	...
	if (!(type = internal_aco_type_find(file, cfg, cat))) {
		ast_log_chan(NULL, LOG_ERROR, "Could not find config type for category '%s' in '%s'\n", cat, file->filename);
		return -1;
	}
	...
	if (!*field) {
		ast_log_chan(NULL, LOG_ERROR, "In %s: %s - No object to update!\n", file->filename, cat);
		return -1;
	}

	if (type->type == ACO_GLOBAL && *field) {
		if (aco_process_category_options(type, cfg, cat, *field)) {
			ast_log_chan(NULL, LOG_ERROR, "In %s: Processing options for %s failed\n", file->filename, cat);
			return -1;
		}
		...
		if (*field) {
			if (!(new_item = type->item_find(*field, cat))) {
				if (!(new_item = type->item_alloc(cat))) {
					ast_log_chan(NULL, LOG_ERROR, "In %s: Could not create item for %s\n", file->filename, cat);
					return -1;
				}
				if (aco_set_defaults(type, cat, new_item)) {
					ast_log_chan(NULL, LOG_ERROR, "In %s: Setting defaults for %s failed\n", file->filename, cat);
					return -1;
				}
				new = 1;
			}
		...
		if (type->item_pre_process && type->item_pre_process(new_item)) {
			ast_log_chan(NULL, LOG_ERROR, "In %s: Preprocess callback for %s failed\n", file->filename, cat);
			return -1;
		}
		...
		if (type->item_prelink && type->item_prelink(new_item)) {
			ast_log_chan(NULL, LOG_ERROR, "In %s: Pre-link callback for %s failed\n", file->filename, cat);
			return -1;
		}

		if (new && !ao2_link(*field, new_item)) {
			ast_log_chan(NULL, LOG_ERROR, "In %s: Linking config for %s failed\n", file->filename, cat);
			return -1;
		}
	}
	return 0;
...
{
	if (!info->internal) {
		ast_log_chan(NULL, LOG_ERROR, "Attempt to process %s with uninitialized aco_info\n", file->filename);
		return ACO_PROCESS_ERROR;
	}

	if (!(info->internal->pending = info->snapshot_alloc())) {
		ast_log_chan(NULL, LOG_ERROR, "In %s: Could not allocate temporary objects\n", file->filename);
		goto error;
	}
...
enum aco_process_status aco_process_config(struct aco_info *info, int reload)
{
	...
	if (!info->internal) {
		ast_log_chan(NULL, LOG_ERROR, "Attempting to process uninitialized aco_info\n");
		return ACO_PROCESS_ERROR;
	}

	if (!(info->files[0])) {
		ast_log_chan(NULL, LOG_ERROR, "No filename given, cannot proceed!\n");
		return ACO_PROCESS_ERROR;
	}
...
try_alias:
		...
		if (!cfg || cfg == CONFIG_STATUS_FILEMISSING) {
			...
			ast_log_chan(NULL, LOG_ERROR, "Contents of %s are invalid and cannot be parsed\n",
				file->filename);
			res = ACO_PROCESS_ERROR;
			break;
		}
...
int aco_process_var(struct aco_type *type, const char *cat, struct ast_variable *var, void *obj)
{
	...
	if (opt && opt->deprecated && !ast_strlen_zero(opt->aliased_to)) {
		...
		ast_log_chan(NULL, LOG_WARNING, "At line %d of %s option '%s' is deprecated. Use '%s' instead\n", var->lineno, var->file, var->name, alias);
		ao2_ref(opt, -1);
		opt = aco_option_find(type, alias);
	}

	if (!opt) {
		ast_log_chan(NULL, LOG_ERROR, "Could not find option suitable for category '%s' named '%s' at line %d of %s\n", cat, var->name, var->lineno, var->file);
		return -1;
	}

	if (!opt->handler) {
		/* It should be impossible for an option to not have a handler */
		ast_log_chan(NULL, LOG_ERROR, "BUG! Somehow a config option for %s/%s was created with no handler!\n", cat, var->name);
		return -1;
	}

#ifdef AST_DEVMODE
	if (opt->doc_unavailable) {
		ast_log_chan(NULL, LOG_ERROR, "Config option '%s' of type '%s' is not completely documented and can not be set\n", var->name, type->name);
		return -1;
	}
#endif

	if (opt->handler(opt, var, obj)) {
		ast_log_chan(NULL, LOG_ERROR, "Error parsing %s=%s at line %d of %s\n", var->name, var->value, var->lineno, var->file);
		return -1;
	}
...
/*	iter = ao2_iterator_init(type->internal->opts, AO2_ITERATOR_DONTLOCK); */
	...
	while ((opt = ao2_iterator_next(&iter))) {
		...
		if (opt->handler(opt, var, obj)) {
			ast_log_chan(NULL, LOG_ERROR, "Unable to set default for %s, %s=%s\n", category, var->name, var->value);
			ao2_ref(opt, -1);
			ao2_iterator_destroy(&iter);
			return -1;
		}
...
static int xmldoc_update_config_type(const char *module, const char *name, const char *category, const char *matchfield, const char *matchvalue, enum aco_category_op category_match)
{
	...
	if (!(results = ast_xmldoc_query("/docs/configInfo[@name='%s']/configFile/configObject[@name='%s']", module, name))) {
		ast_log_chan(NULL, LOG_WARNING, "Cannot update type '%s' in module '%s' because it has no existing documentation!"
	...
	if (!(type = ast_xml_xpath_get_first_result(results))) {
		ast_log_chan(NULL, LOG_WARNING, "Could not retrieve documentation for type '%s' in module '%s'\n", name, module);
		return XMLDOC_STRICT ? -1 : 0;
	}

	if (!(syntax = ast_xml_new_child(type, "syntax"))) {
		ast_log_chan(NULL, LOG_WARNING, "Could not create syntax node for type '%s' in module '%s'\n", name, module);
		return XMLDOC_STRICT ? -1 : 0;
	}

	if (!(matchinfo = ast_xml_new_child(syntax, "matchInfo"))) {
		ast_log_chan(NULL, LOG_WARNING, "Could not create matchInfo node for type '%s' in module '%s'\n", name, module);
		return XMLDOC_STRICT ? -1 : 0;
	}

	if (!(tmp = ast_xml_new_child(matchinfo, "category"))) {
		ast_log_chan(NULL, LOG_WARNING, "Could not create category node for type '%s' in module '%s'\n", name, module);
		return XMLDOC_STRICT ? -1 : 0;
	}
	...
	if (!ast_strlen_zero(matchfield)) {
		if (!(tmp = ast_xml_new_child(matchinfo, "field"))) {
			ast_log_chan(NULL, LOG_WARNING, "Could not add %s attribute for type '%s' in module '%s'\n", matchfield, name, module);
			return XMLDOC_STRICT ? -1 : 0;
		}
	...
	if (!config_info || !(config_type = find_xmldoc_type(config_info, name))) {
		ast_log_chan(NULL, LOG_WARNING, "Could not obtain XML documentation item for config type %s\n", name);
		return XMLDOC_STRICT ? -1 : 0;
	}

	if (ast_xmldoc_regenerate_doc_item(config_type)) {
		ast_log_chan(NULL, LOG_WARNING, "Could not update type '%s' with values from config type registration\n", name);
		return XMLDOC_STRICT ? -1 : 0;
	}
...
static int xmldoc_update_config_option(struct aco_type **types, const char *module, const char *name, const char *object_name, const char *default_value, unsigned int regex, enum aco_option_type type)
{
	...
	if (!config_info || !(config_option = find_xmldoc_option(config_info, types, name))) {
		ast_log_chan(NULL, LOG_ERROR, "XML Documentation for option '%s' in modules '%s' not found!\n", name, module);
		return XMLDOC_STRICT ? -1 : 0;
	}
	...
	if (!(results = ast_xmldoc_query("/docs/configInfo[@name='%s']/configFile/configObject[@name='%s']/configOption[@name='%s']", module, object_name, name))) {
		ast_log_chan(NULL, LOG_WARNING, "Could not find option '%s' with type '%s' in module '%s'\n", name, object_name, module);
		return XMLDOC_STRICT ? -1 : 0;
	}

	if (!(option = ast_xml_xpath_get_first_result(results))) {
		ast_log_chan(NULL, LOG_WARNING, "Could not obtain results for option '%s' with type '%s' in module '%s'\n", name, object_name, module);
		return XMLDOC_STRICT ? -1 : 0;
	}
	...
	if (ast_xmldoc_regenerate_doc_item(config_option)) {
		ast_log_chan(NULL, LOG_WARNING, "Could not update option '%s' with values from config option registration\n", name);
		return XMLDOC_STRICT ? -1 : 0;
	}
...
int aco_init(void)
{
	...
	if (!(xmldocs = ast_xmldoc_build_documentation("configInfo"))) {
		ast_log_chan(NULL, LOG_ERROR, "Couldn't build config documentation\n");
		return -1;
	}
	...
	if (opt->flags & PARSE_IN_RANGE) {
		...
		if (res) {
			if (opt->flags & PARSE_RANGE_DEFAULTS) {
				ast_log_chan(NULL, LOG_WARNING, "Failed to set %s=%s. Set to %d instead due to range limit (%d, %d)\n", var->name, var->value, *field, (int) opt->args[1], (int) opt->args[2]);
				res = 0;
			} else if (opt->flags & PARSE_DEFAULT) {
				ast_log_chan(NULL, LOG_WARNING, "Failed to set %s=%s, Set to default value %d instead.\n", var->name, var->value, *field);
				res = 0;
			}
		}
	} else if ((opt->flags & PARSE_DEFAULT) && ast_parse_arg(var->value, flags, field, (int) opt->args[1])) {
		ast_log_chan(NULL, LOG_WARNING, "Attempted to set %s=%s, but set it to %d instead due to default)\n", var->name, var->value, *field);
	} else {
...
static int uint_handler_fn(const struct aco_option *opt, struct ast_variable *var, void *obj) {
	...
	if (opt->flags & PARSE_IN_RANGE) {
		...
		if (res) {
			if (opt->flags & PARSE_RANGE_DEFAULTS) {
				ast_log_chan(NULL, LOG_WARNING, "Failed to set %s=%s. Set to %u instead due to range limit (%d, %d)\n", var->name, var->value, *field, (int) opt->args[1], (int) opt->args[2]);
				res = 0;
			} else if (opt->flags & PARSE_DEFAULT) {
				ast_log_chan(NULL, LOG_WARNING, "Failed to set %s=%s, Set to default value %u instead.\n", var->name, var->value, *field);
				res = 0;
			}
		}
	} else if ((opt->flags & PARSE_DEFAULT) && ast_parse_arg(var->value, flags, field, (unsigned int) opt->args[1])) {
		ast_log_chan(NULL, LOG_WARNING, "Attempted to set %s=%s, but set it to %u instead due to default)\n", var->name, var->value, *field);
	} else {
		res = ast_parse_arg(var->value, flags, field);
	}
