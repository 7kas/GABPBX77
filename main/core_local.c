...
static int local_call(struct ast_channel *ast, const char *dest, int timeout)
{
	...
	case LOCAL_CALL_ACTION_DIALPLAN:
		if (!ast_exists_extension(NULL, p->context, p->exten, 1, chan_cid)) {
			ast_log_chan(NULL, LOG_NOTICE, "No such extension/context %s@%s while calling Local channel\n",
				p->exten, p->context);
		} else {
		...
		*opts++ = '\0';
		...
		if (strchr(opts, 'j')) {
			if (ast_test_flag(&pvt->base, AST_UNREAL_NO_OPTIMIZATION)) {
				...
				ast_log_chan(NULL, LOG_ERROR, "You must use the 'n' option with the 'j' option to enable the jitter buffer\n");
			}
		}
		if (strchr(opts, 'm')) {
...
static struct ast_channel *local_request_with_stream_topology(const char *type, struct ast_stream_topology *topology, const struct ast_assigned_ids *assignedids, const struct ast_channel *requestor, const char *data, int *cause)
{
	...
	/* Allocate a new private structure and then GABpbx channels */
...
int ast_local_init(void)
{
	...
	if (ast_channel_register(&local_tech)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to register channel class 'Local'\n");
