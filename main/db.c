...
#include "gabpbx/paths.h"	/* use ast_config_AST_DB */
#include <sys/time.h>
#include <sys/types.h>
#include <sys/stat.h>
#include <unistd.h>
...
static int init_stmt(sqlite3_stmt **stmt, const char *sql, size_t len)
{
	...
	if (sqlite3_prepare(astdb, sql, len, stmt, NULL) != SQLITE_OK) {
		ast_log_chan(NULL, LOG_WARNING, "Couldn't prepare statement '%s': %s\n", sql, sqlite3_errmsg(astdb));
...
static int clean_stmt(sqlite3_stmt **stmt, const char *sql)
{
	if (sqlite3_finalize(*stmt) != SQLITE_OK) {
		ast_log_chan(NULL, LOG_WARNING, "Couldn't finalize statement '%s': %s\n", sql, sqlite3_errmsg(astdb));
		*stmt = NULL;
		return -1;
	}
	*stmt = NULL;
...
static int db_create_astdb(void)
{
	...
	if (sqlite3_step(create_astdb_stmt) != SQLITE_DONE) {
		ast_log_chan(NULL, LOG_WARNING, "Couldn't create astdb table: %s\n", sqlite3_errmsg(astdb));
		res = -1;
	}
...
static int db_open(void)
{
	...
	if (stat(dbname, &dont_care) && !stat(ast_config_AST_DB, &dont_care)) {
		if (convert_bdb_to_sqlite3()) {
			ast_log_chan(NULL, LOG_ERROR, "*** Database conversion failed!\n");
			ast_log_chan(NULL, LOG_ERROR, "*** GABpbx now uses SQLite3 for its internal\n");
			ast_log_chan(NULL, LOG_ERROR, "*** database. Conversion from the old astdb\n");
			ast_log_chan(NULL, LOG_ERROR, "*** failed. Most likely the astdb2sqlite3 utility\n");
			ast_log_chan(NULL, LOG_ERROR, "*** was not selected for build. To convert the\n");
			ast_log_chan(NULL, LOG_ERROR, "*** old astdb, please delete '%s'\n", dbname);
			ast_log_chan(NULL, LOG_ERROR, "*** and re-run 'make menuselect' and select astdb2sqlite3\n");
			ast_log_chan(NULL, LOG_ERROR, "*** in the Utilities section, then 'make && make install'.\n");
			ast_log_chan(NULL, LOG_ERROR, "*** It is also imperative that the user under which\n");
			ast_log_chan(NULL, LOG_ERROR, "*** GABpbx runs have write permission to the directory\n");
			ast_log_chan(NULL, LOG_ERROR, "*** where the database resides.\n");
			sleep(5);
		} else {
			ast_log_chan(NULL, LOG_NOTICE, "Database conversion succeeded!\n");
		}
	}

	ast_mutex_lock(&dblock);
	if (sqlite3_open(dbname, &astdb) != SQLITE_OK) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to open GABpbx database '%s': %s\n", dbname, sqlite3_errmsg(astdb));
		sqlite3_close(astdb);
		ast_mutex_unlock(&dblock);
		return -1;
	}
...
static int db_execute_sql(const char *sql, int (*callback)(void *, int, char **, char **), void *arg)
{
	...
	if (sqlite3_exec(astdb, sql, callback, arg, &errmsg) != SQLITE_OK) {
		ast_log_chan(NULL, LOG_WARNING, "Error executing SQL (%s): %s\n", sql, errmsg);
		sqlite3_free(errmsg);
		res = -1;
	}
...
int ast_db_put(const char *family, const char *key, const char *value)
{
	...
	if (fullkey_len < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to allocate memory for family/key '/%s/%s'\n",
			family, key);
		return -1;
	}
	...
	if (sqlite3_bind_text(put_stmt, 1, fullkey, fullkey_len, SQLITE_STATIC) != SQLITE_OK) {
		ast_log_chan(NULL, LOG_WARNING, "Couldn't bind key to stmt: %s\n", sqlite3_errmsg(astdb));
		res = -1;
	} else if (sqlite3_bind_text(put_stmt, 2, value, -1, SQLITE_STATIC) != SQLITE_OK) {
		ast_log_chan(NULL, LOG_WARNING, "Couldn't bind value to stmt: %s\n", sqlite3_errmsg(astdb));
		res = -1;
	} else if (sqlite3_step(put_stmt) != SQLITE_DONE) {
		ast_log_chan(NULL, LOG_WARNING, "Couldn't execute statement: %s\n", sqlite3_errmsg(astdb));
		res = -1;
	}
...
static int db_get_common(const char *family, const char *key, char **buffer, int bufferlen)
{
	...
	if (sqlite3_bind_text(get_stmt, 1, fullkey, fullkey_len, SQLITE_STATIC) != SQLITE_OK) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Couldn't get value\n");
		res = -1;
	} else {
...
int ast_db_exists(const char *family, const char *key)
{
	...
	if (res != SQLITE_OK) {
		ast_log_chan(NULL, LOG_WARNING, "Couldn't bind key to stmt: %d:%s\n", res, sqlite3_errmsg(astdb));
		res = 0;
...
int ast_db_del2(const char *family, const char *key)
{
	...
	if (ast_db_get(family, key, tmp, sizeof(tmp))) {
		ast_log_chan(NULL, LOG_WARNING, "AstDB key %s does not exist\n", fullkey);
		res = -1;
		...
		ast_log_chan(NULL, LOG_WARNING, "AstDB error (%s): %s\n", fullkey, sqlite3_errstr(mres));
		res = -1;
	}
	sqlite3_reset(del_stmt);
	db_sync();
	...
	int *prefix_len)
{
	...
	*prefix_len = 0;
	...
	if (!ast_strlen_zero(family)) {
		...
		if (*prefix_len < 0) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to allocate memory for family/keytree '/%s%s%s'\n",
...
int ast_db_deltree(const char *family, const char *keytree)
{
	...
	if (ast_strlen_zero(family) && !ast_strlen_zero(keytree)) {
		ast_log_chan(NULL, LOG_WARNING, "Key tree '%s' specified without family\n", keytree);
		return -1;
	}
	...
	if (prefix_len && (sqlite3_bind_text(stmt, 1, prefix, prefix_len, SQLITE_STATIC) != SQLITE_OK)) {
		ast_log_chan(NULL, LOG_WARNING, "Couldn't bind %s to stmt: %s\n", prefix, sqlite3_errmsg(astdb));
		res = -1;
	} else if (sqlite3_step(stmt) != SQLITE_DONE) {
		ast_log_chan(NULL, LOG_WARNING, "Couldn't execute stmt: %s\n", sqlite3_errmsg(astdb));
		res = -1;
	}
...
struct ast_db_entry *ast_db_gettree(const char *family, const char *keytree)
{
	...
	ast_mutex_lock(&dblock);
	if (prefix_len && (sqlite3_bind_text(stmt, 1, prefix, prefix_len, SQLITE_STATIC) != SQLITE_OK)) {
		ast_log_chan(NULL, LOG_WARNING, "Could not bind %s to stmt: %s\n", prefix, sqlite3_errmsg(astdb));
		sqlite3_reset(stmt);
		ast_mutex_unlock(&dblock);
		ast_free(prefix);
		return NULL;
...
static char *handle_cli_database_put(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	case CLI_INIT:
		...
		e->usage =
			...
			"       Adds or updates an entry in the GABpbx database for\n"
...
static char *handle_cli_database_get(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	case CLI_INIT:
		...
		e->usage =
			...
			"       Retrieves an entry in the GABpbx database for a given\n"
			"       family and key.\n";
		return NULL;
	case CLI_GENERATE:
		return NULL;
...
static char *handle_cli_database_del(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	case CLI_INIT:
		...
		e->usage =
			...
			"       Deletes an entry in the GABpbx database for a given\n"
			"       family and key.\n";
		return NULL;
	case CLI_GENERATE:
		return NULL;
...
static char *handle_cli_database_deltree(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	case CLI_INIT:
		...
		e->usage =
			...
			"       in the GABpbx database.  The two arguments may be\n"
...
static char *handle_cli_database_show(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	case CLI_INIT:
		...
		e->usage =
			...
			"       Shows GABpbx database contents, optionally restricted\n"
	...
	while (sqlite3_step(stmt) == SQLITE_ROW) {
		...
		if (!(key_s = (const char *) sqlite3_column_text(stmt, 0))) {
			ast_log_chan(NULL, LOG_WARNING, "Skipping invalid key!\n");
			continue;
		}
		if (!(value_s = (const char *) sqlite3_column_text(stmt, 1))) {
			ast_log_chan(NULL, LOG_WARNING, "Skipping invalid value!\n");
			continue;
		}
		++counter;
...
static char *handle_cli_database_showkey(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	case CLI_INIT:
		...
		e->usage =
			...
			"       Shows GABpbx database contents, restricted to a given key.\n";
		return NULL;
	case CLI_GENERATE:
		return NULL;
	}
