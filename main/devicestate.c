 ...
 *	functions within GABpbx. Currently we have providers
 ...
 *
 *	When a channel driver or gabpbx app changes state for
...
static const char * const devstatestring[][2] = {
	...
	{ /* 4 AST_DEVICE_INVALID */     "Invalid",     "INVALID"     }, /*!< Invalid - not known to GABpbx */
...
int ast_devstate_prov_add(const char *label, ast_devstate_prov_cb_type callback)
{
	...
	AST_RWLIST_TRAVERSE(&devstate_provs, devcb, list) {
		if (!strcasecmp(devcb->label, label)) {
			ast_log_chan(NULL, LOG_WARNING, "Device state provider '%s' already registered\n", label);
			ast_free(devprov);
...
int ast_device_state_engine_init(void)
{
	...
	if (ast_pthread_create_background(&change_thread, NULL, do_devstate_changes, NULL) < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to start device state change thread.\n");
		return -1;
	}
...
int devstate_init(void)
{
	...
	if (!devstate_message_sub) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to create subscription creating uncached device state aggregate events.\n");
		return -1;
	}
