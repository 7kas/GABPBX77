...
/*! \brief Dialing channel structure. Contains per-channel dialing options, gabpbx channel, and more! */
struct ast_dial_channel {
	...
	struct ast_channel *owner;		/*!< GABpbx channel */
...
static int begin_dial_prerun(struct ast_dial_channel *channel, struct ast_channel *chan, struct ast_format_cap *cap, const char *predial_string)
{
	...
	if (chan) {
		...
		if (max_forwards <= 0) {
			ast_log_chan(NULL, LOG_WARNING, "Cannot dial from channel '%s'. Max forwards exceeded\n",
	...
	if (fr->frametype == AST_FRAME_CONTROL) {
		...
		case AST_CONTROL_ANSWER:
			if (chan) {
				...
				ast_verb_chan(NULL, 3, "%s answered\n", ast_channel_name(channel->owner));
			}
			AST_LIST_LOCK(&dial->channels);
		...
		case AST_CONTROL_PROGRESS:
			...
			if (chan) {
				ast_verb_chan(NULL, 3, "%s is making progress, passing it to %s\n", ast_channel_name(channel->owner), ast_channel_name(chan));
		...
		case AST_CONTROL_VIDUPDATE:
			...
			ast_verb_chan(NULL, 3, "%s requested a video update, passing it to %s\n", ast_channel_name(channel->owner), ast_channel_name(chan));
		...
		case AST_CONTROL_SRCUPDATE:
			...
			ast_verb_chan(NULL, 3, "%s requested a source update, passing it to %s\n", ast_channel_name(channel->owner), ast_channel_name(chan));
		...
		case AST_CONTROL_CONNECTED_LINE:
			...
			ast_verb_chan(NULL, 3, "%s connected line has changed, passing it to %s\n", ast_channel_name(channel->owner), ast_channel_name(chan));
		...
		case AST_CONTROL_PROCEEDING:
			...
			if (chan) {
				ast_verb_chan(NULL, 3, "%s is proceeding, passing it to %s\n", ast_channel_name(channel->owner), ast_channel_name(chan));
		...
		case AST_CONTROL_HOLD:
			...
			ast_verb_chan(NULL, 3, "Call on %s placed on hold\n", ast_channel_name(chan));
		...
		case AST_CONTROL_UNHOLD:
			...
			ast_verb_chan(NULL, 3, "Call on %s left from hold\n", ast_channel_name(chan));
 ...
 * \note Returns the GABpbx channel that answered
 ...
 * \note Returns the GABpbx channel that answered and removes it from the dialing structure
 * \param dial Dialing structure
 */
