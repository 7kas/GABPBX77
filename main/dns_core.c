...
struct ast_dns_query *dns_query_alloc(const char *name, int rr_type, int rr_class, ast_dns_resolve_callback callback, void *data)
{
	...
	if (ast_strlen_zero(name)) {
		ast_log_chan(NULL, LOG_WARNING, "Could not perform asynchronous resolution, no name provided\n");
		return NULL;
	} else if (rr_type > 65536) {
		ast_log_chan(NULL, LOG_WARNING, "Could not perform asynchronous resolution of '%s', resource record type '%d' exceeds maximum\n",
			name, rr_type);
		return NULL;
	} else if (rr_type < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Could not perform asynchronous resolution of '%s', invalid resource record type '%d'\n",
			name, rr_type);
		return NULL;
	} else if (rr_class > 65536) {
		ast_log_chan(NULL, LOG_WARNING, "Could not perform asynchronous resolution of '%s', resource record class '%d' exceeds maximum\n",
			name, rr_class);
		return NULL;
	} else if (rr_class < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Could not perform asynchronous resolution of '%s', invalid resource class '%d'\n",
			name, rr_class);
		return NULL;
	} else if (!callback) {
		ast_log_chan(NULL, LOG_WARNING, "Could not perform asynchronous resolution of '%s', no callback provided\n",
			name);
		return NULL;
	}
	...
	if (!query->resolver) {
		ast_log_chan(NULL, LOG_ERROR, "Attempted to do a DNS query for '%s' of class '%d' and type '%d' but no resolver is available\n",
...
struct ast_dns_query_active *ast_dns_resolve_async(const char *name, int rr_type, int rr_class, ast_dns_resolve_callback callback, void *data)
{
	...
	if (active->query->resolver->resolve(active->query)) {
		ast_log_chan(NULL, LOG_ERROR, "Resolver '%s' returned an error when resolving '%s' of class '%d' and type '%d'\n",
...
int ast_dns_resolve(const char *name, int rr_type, int rr_class, struct ast_dns_result **result)
{
	...
	if (ast_strlen_zero(name)) {
		ast_log_chan(NULL, LOG_WARNING, "Could not perform synchronous resolution, no name provided\n");
		return -1;
	} else if (rr_type > 65536) {
		ast_log_chan(NULL, LOG_WARNING, "Could not perform synchronous resolution of '%s', resource record type '%d' exceeds maximum\n",
			name, rr_type);
		return -1;
	} else if (rr_type < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Could not perform synchronous resolution of '%s', invalid resource record type '%d'\n",
			name, rr_type);
		return -1;
	} else if (rr_class > 65536) {
		ast_log_chan(NULL, LOG_WARNING, "Could not perform synchronous resolution of '%s', resource record class '%d' exceeds maximum\n",
			name, rr_class);
		return -1;
	} else if (rr_class < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Could not perform synchronous resolution of '%s', invalid resource class '%d'\n",
			name, rr_class);
		return -1;
	} else if (!result) {
		ast_log_chan(NULL, LOG_WARNING, "Could not perform synchronous resolution of '%s', no result pointer provided for storing results\n",
			name);
		return -1;
	}
...
int ast_dns_resolve_ipv6_and_ipv4(struct ast_sockaddr *address, const char *host, const char *port)
{
	...
	if (!queries) {
		ast_log_chan(NULL, LOG_ERROR, "Couldn't allocate DNS query structure\n");
		return -1;
	}
	rc = ast_dns_query_set_add(queries, host, ns_t_aaaa, ns_c_in);
	if (rc != 0) {
		ast_log_chan(NULL, LOG_ERROR, "Couldn't add 'AAAA' DNS query for '%s'\n", host);
		return -1;
	}
	rc = ast_dns_query_set_add(queries, host, ns_t_a, ns_c_in);
	if (rc != 0) {
		ast_log_chan(NULL, LOG_ERROR, "Couldn't add 'A' DNS query for '%s'\n", host);
		return -1;
	}
	rc = ast_query_set_resolve(queries);
	if (rc != 0) {
		ast_log_chan(NULL, LOG_ERROR, "Query set resolve failure for '%s'\n", host);
		return -1;
	}
...
int ast_dns_resolver_register(struct ast_dns_resolver *resolver)
{
	...
	if (!resolver) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Registration of DNS resolver failed as it does not have a name\n");
		return -1;
	} else if (!resolver->resolve) {
		ast_log_chan(NULL, LOG_ERROR, "DNS resolver '%s' does not implement the resolve callback which is required\n",
			resolver->name);
		return -1;
	} else if (!resolver->cancel) {
		ast_log_chan(NULL, LOG_ERROR, "DNS resolver '%s' does not implement the cancel callback which is required\n",
			resolver->name);
		return -1;
	}
	...
	AST_LIST_TRAVERSE(&resolvers, iter, next) {
		if (!strcmp(iter->name, resolver->name)) {
			ast_log_chan(NULL, LOG_ERROR, "A DNS resolver with the name '%s' is already registered\n", resolver->name);
			AST_RWLIST_UNLOCK(&resolvers);
			return -1;
		}
	}
	...
	ast_verb_chan(NULL, 5, "Registered DNS resolver '%s' with priority '%d'\n", resolver->name, resolver->priority);
...
void ast_dns_resolver_unregister(struct ast_dns_resolver *resolver)
{
	...
	ast_verb_chan(NULL, 5, "Unregistered DNS resolver '%s'\n", resolver->name);
}
