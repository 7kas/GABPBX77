...
struct ast_dns_record *dns_naptr_alloc(struct ast_dns_query *query, const char *data, const size_t size)
{
	...
	if (ptr != end_of_record) {
		ast_log_chan(NULL, LOG_ERROR, "NAPTR record gave undersized string indications.\n");
		return NULL;
	}
	...
	if (flags_res == FLAGS_INVALID) {
		ast_log_chan(NULL, LOG_ERROR, "NAPTR Record contained invalid flags %.*s\n", flags_size, flags);
		return NULL;
	}

	if (services_invalid(services, services_size)) {
		ast_log_chan(NULL, LOG_ERROR, "NAPTR record contained invalid services %.*s\n", services_size, services);
		return NULL;
	}

	if (regexp_invalid(regexp, regexp_size)) {
		ast_log_chan(NULL, LOG_ERROR, "NAPTR record contained invalid regexp %.*s\n", regexp_size, regexp);
		return NULL;
	}
	...
	if (regexp_size && replacement_size > 1) {
		ast_log_chan(NULL, LOG_ERROR, "NAPTR record contained both a regexp and replacement\n");
		return NULL;
	}
