	...
	if (!ast_get_ip_or_srv_with_preference(&tmp, entry->name, entry->service, entry->result)) {
		...
		if (ast_sockaddr_cmp(&tmp, entry->result)) {
			...
			if (entry->update_func) {
				...
				ast_log_chan(NULL, LOG_NOTICE, "dnssrv: host '%s' changed from %s to %s\n",
...
static int refresh_list(const void *data)
{
	...
	if (ast_mutex_trylock(&refresh_lock)) {
		if (info->verbose) {
			ast_log_chan(NULL, LOG_WARNING, "DNS Manager refresh already in progress.\n");
		}
		return -1;
	}
...
static int do_reload(int loading)
{
	...
	for (v = ast_variable_browse(config, "general"); v; v = v->next) {
		if (!strcasecmp(v->name, "enable")) {
			...
			if (sscanf(v->value, "%30d", &interval) < 1) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Invalid refresh interval '%d' specified, using default\n", interval);
			} else {
	...
	if (enabled && refresh_interval) {
		ast_log_chan(NULL, LOG_NOTICE, "Managed DNS entries will be refreshed every %d seconds.\n", refresh_interval);
	}
	...
	if (enabled) {
		if (!was_enabled && (refresh_thread == AST_PTHREADT_NULL)) {
			if (ast_pthread_create_background(&refresh_thread, NULL, do_refresh, NULL) < 0) {
				ast_log_chan(NULL, LOG_ERROR, "Unable to start refresh thread.\n");
			}
		}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER, "DNS Manager",
