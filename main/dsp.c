...
static void mute_fragment(struct ast_dsp *dsp, fragment_t *fragment)
{
	if (dsp->mute_fragments >= ARRAY_LEN(dsp->mute_data)) {
		ast_log_chan(NULL, LOG_ERROR, "Too many fragments to mute. Ignoring\n");
		return;
	}
...
static void store_digit(digit_detect_state_t *s, char digit)
{
	...
	if (s->current_digits < MAX_DTMF_DIGITS) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Digit lost due to full buffer\n");
		s->lost_digits++;
	}
}
	...
	while (len) {
		...
		if (dsp->gsamps == dsp->gsamp_size) {
			...
			case PROG_MODE_UK:
				...
				break;
			default:
				ast_log_chan(NULL, LOG_WARNING, "Can't process in unknown prog mode '%u'\n", dsp->progmode);
			}
...
int ast_dsp_call_progress(struct ast_dsp *dsp, struct ast_frame *inf)
{
	if (inf->frametype != AST_FRAME_VOICE) {
		ast_log_chan(NULL, LOG_WARNING, "Can't check call progress of non-voice frames\n");
		return 0;
	}
	if (!ast_format_cache_is_slinear(inf->subclass.format)) {
		ast_log_chan(NULL, LOG_WARNING, "Can only check call progress in signed-linear frames, %s not supported\n",
...
static int ast_dsp_silence_noise_with_energy(struct ast_dsp *dsp, struct ast_frame *f, int *total, int *frames_energy, int noise)
{
	...
	if (f->frametype != AST_FRAME_VOICE) {
		ast_log_chan(NULL, LOG_WARNING, "Can't calculate silence on a non-voice frame\n");
		return 0;
	}
	...
	if (ast_format_cache_is_slinear(f->subclass.format)) {
		...
		if (ast_format_cmp(f->subclass.format, ast_format_ulaw) == AST_FORMAT_CMP_EQUAL) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Can only calculate silence on signed-linear, alaw or ulaw frames, %s not supported\n",
	...
	if (ast_format_cache_is_slinear(af->subclass.format)) {
		...
		if (dsp->display_inband_dtmf_warning) {
			...
			if ((dsp->features & DSP_FEATURE_DIGIT_DETECT) && (dsp->digitmode & DSP_DIGITMODE_DTMF)) {
				ast_log_chan(NULL, LOG_WARNING, "Inband DTMF is not supported on codec %s. Use RFC2833\n",
					ast_format_get_name(af->subclass.format));
			} else {
				ast_log_chan(NULL, LOG_WARNING, "Can only do DSP on signed-linear, alaw or ulaw frames (%s not supported)\n",
					ast_format_get_name(af->subclass.format));
			}
		}
	...
	if ((dsp->features & DSP_FEATURE_CALL_PROGRESS)) {
		...
		if (res) {
			...
			case AST_CONTROL_HANGUP:
				...
				break;
			default:
				ast_log_chan(NULL, LOG_WARNING, "Don't know how to represent call progress message %d\n", res);
			}
		}
	...
	for (v = ast_variable_browse(cfg, "default"); v; v = v->next) {
		if (!strcasecmp(v->name, "silencethreshold")) {
			if (sscanf(v->value, "%30d", &cfg_threshold) < 1) {
				ast_log_chan(NULL, LOG_WARNING, "Unable to convert '%s' to a numeric value.\n", v->value);
			} else if (cfg_threshold < 0) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid silence threshold '%d' specified, using default\n", cfg_threshold);
			} else {
			...
			if (sscanf(v->value, "%30f", &cfg_twist) < 1) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Invalid dtmf_normal_twist value '%.2f' specified, using default of %.2f\n", cfg_twist, dtmf_normal_twist);
			} else {
			...
			if (sscanf(v->value, "%30f", &cfg_twist) < 1) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Invalid dtmf_reverse_twist value '%.2f' specified, using default of %.2f\n", cfg_twist, dtmf_reverse_twist);
			} else {
			...
			if (sscanf(v->value, "%30f", &cfg_twist) < 1) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Invalid relax_dtmf_normal_twist value '%.2f' specified, using default of %.2f\n", cfg_twist, relax_dtmf_normal_twist);
			} else {
			...
			if (sscanf(v->value, "%30f", &cfg_twist) < 1) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Invalid relax_dtmf_reverse_twist value '%.2f' specified, using default of %.2f\n", cfg_twist, relax_dtmf_reverse_twist);
			} else {
			...
			if (sscanf(v->value, "%30d", &cfg_threshold) < 1) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Invalid dtmf_hits_to_begin value '%d' specified, using default of %d\n", cfg_threshold, dtmf_hits_to_begin);
			} else {
			...
			if (sscanf(v->value, "%30d", &cfg_threshold) < 1) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Invalid dtmf_misses_to_end value '%d' specified, using default of %d\n", cfg_threshold, dtmf_misses_to_end);
			} else {
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER, "DSP",
