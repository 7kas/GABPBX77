 ...
 * - See https://issues.gabpbx.org/view.php?id=6739
...
#include "gabpbx/enum.h"
#include "gabpbx/dns.h"
...
static int txt_callback(void *context, unsigned char *answer, int len, unsigned char *fullanswer)
{
	...
	if (i > len) {	/* illegal packet */
		ast_log_chan(NULL, LOG_WARNING, "txt_callback: malformed TXT record.\n");
		return 0;
	}

	if (i >= sizeof(c->txt)) {	/* too long? */
		ast_log_chan(NULL, LOG_WARNING, "txt_callback: TXT record too long.\n");
		i = sizeof(c->txt) - 1;
	}
...
static int blr_txt(const char *cc, const char *suffix)
{
	...
	ast_verb_chan(NULL, 4, "blr_txt()  cc='%s', suffix='%s', c_bl='%s'\n", cc, suffix, ienum_branchlabel);

	if (sizeof(domain) < (strlen(cc) * 2 + strlen(ienum_branchlabel) + strlen(suffix) + 2)) {
		ast_mutex_unlock(&enumlock);
		ast_log_chan(NULL, LOG_WARNING, "ERROR: string sizing in blr_txt.\n");
		return -1;
	}
	...
	ast_verb_chan(NULL, 4, "blr_txt() FQDN for TXT record: %s, cc was %s\n", domain, cc);
	...
	if (ret > 0) {
		...
		if ((ret >= 0) && (ret < 20)) {
			ast_verb_chan(NULL, 3, "blr_txt() BLR TXT record for %s is %d (apex: %s)\n", cc, ret, suffix);
			return ret;
		}
	}

	ast_verb_chan(NULL, 3, "blr_txt() BLR TXT record for %s not found (apex: %s)\n", cc, suffix);
...
static int ebl_callback(void *context, unsigned char *answer, int len, unsigned char *fullanswer)
{
	...
	if ((c->pos > 15) || len < 2) {	/* illegal packet */
		ast_log_chan(NULL, LOG_WARNING, "ebl_callback: malformed EBL record.\n");
		return 0;
	}
	...
	if ((i = dn_expand((unsigned char *)fullanswer, (unsigned char *)answer + len,
		...
		ast_log_chan(NULL, LOG_WARNING, "Failed to expand hostname\n");
		return 0;
	}
	c->apex[i] = 0;
	c->apex_len = i;
...
static int blr_ebl(const char *cc, const char *suffix, char *separator, int sep_len, char* apex, int apex_len)
{
	...
	ast_verb_chan(NULL, 4, "blr_ebl()  cc='%s', suffix='%s', c_bl='%s'\n", cc, suffix, ienum_branchlabel);

	if (sizeof(domain) < (strlen(cc) * 2 + strlen(ienum_branchlabel) + strlen(suffix) + 2)) {
		ast_mutex_unlock(&enumlock);
		ast_log_chan(NULL, LOG_WARNING, "ERROR: string sizing in blr_EBL.\n");
		return -1;
	}
	...
	ast_verb_chan(NULL, 4, "blr_ebl() FQDN for EBL record: %s, cc was %s\n", domain, cc);
...
static unsigned int parse_ie(char *data, unsigned int maxdatalen, unsigned char *src, unsigned int srclen)
{
	...
	if (len > srclen) {
		ast_log_chan(NULL, LOG_WARNING, "ENUM parsing failed: Wanted %u characters, got %u\n", len, srclen);
		return -1;
	}
...
static int parse_naptr(unsigned char *dst, int dstsize, char *tech, int techsize, unsigned char *answer, int len, unsigned char *naptrinput)
{
	...
	if (len < sizeof(struct naptr)) {
		ast_log_chan(NULL, LOG_WARNING, "NAPTR record length too short\n");
		return -1;
	}
	answer += sizeof(struct naptr);
	len -= sizeof(struct naptr);
	if ((res = parse_ie(flags, sizeof(flags) - 1, answer, len)) < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to get flags from NAPTR record\n");
		return -1;
	} else {
		answer += res;
		len -= res;
	...
	if ((res = parse_ie(services, sizeof(services) - 1, answer, len)) < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to get services from NAPTR record\n");
		return -1;
	} else {
		answer += res;
		len -= res;
	...
	if ((res = parse_ie(regexp, sizeof(regexp) - 1, answer, len)) < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to get regexp from NAPTR record\n");
		return -1;
	} else {
		answer += res;
		len -= res;
	...
	if (tolower(flags[0]) != 'u') {
		ast_log_chan(NULL, LOG_WARNING, "NAPTR Flag must be 'U' or 'u'.\n");
		return -1;
	}
	...
	if (regexp_len < 7) {
		ast_log_chan(NULL, LOG_WARNING, "Regex too short to be meaningful.\n");
		return -1;
	}
	...
	if ((delim2 == NULL)
		...
		ast_log_chan(NULL, LOG_WARNING, "Regex delimiter error (on \"%s\").\n", regexp);
		return -1;
 ...
 */
	...
	if (regcomp(&preg, pattern, re_flags)) {
		ast_log_chan(NULL, LOG_WARNING, "NAPTR Regex compilation error (regex = \"%s\").\n", regexp);
		return -1;
	}

	if (preg.re_nsub > ARRAY_LEN(pmatch)) {
		ast_log_chan(NULL, LOG_WARNING, "NAPTR Regex compilation error: too many subs.\n");
		regfree(&preg);
		return -1;
	}
	...
	if ((rc = regexec(&preg, (char *) naptrinput, max_bt, pmatch, 0))) {
		...
		ast_log_chan(NULL, LOG_WARNING, "NAPTR Regex match failed. Reason: %s\n", errbuff);
		regfree(&preg);
		return -1;
	}
	regfree(&preg);
	...
	while (*subst && (d_len > 0)) {
		if ((subst[0] == '\\') && isdigit(subst[1])) { /* is this character the beginning of a backreference */
			...
			if (matchindex >= ARRAY_LEN(pmatch)) {
				ast_log_chan(NULL, LOG_WARNING, "Error during regex substitution. Invalid pmatch index.\n");
				return -1;
			}
			...
			if (size > d_len) {
				ast_log_chan(NULL, LOG_WARNING, "Not enough space during NAPTR regex substitution.\n");
				return -1;
			}
			...
			if ((strlen((char *) naptrinput) >= pmatch[matchindex].rm_eo) && (pmatch[matchindex].rm_so <= pmatch[matchindex].rm_eo)) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Error during regex substitution. Invalid backreference index.\n");
				return -1;
			}
			...
			*d++ = *subst++;
			...
			ast_log_chan(NULL, LOG_WARNING, "Error during regex substitution.\n");
			return -1;
		}
	}
	*d = 0;
...
static int enum_callback(void *context, unsigned char *answer, int len, unsigned char *fullanswer)
{
	...
	if (res < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to parse naptr\n");
		return -1;
...
int ast_get_enum(struct ast_channel *chan, const char *number, char *dst, int dstlen, char *tech, int techlen, char* suffix, char* options, unsigned int record, struct enum_context **argcontext)
{
	...
	if (ast_strlen_zero(suffix)) {
		ast_log_chan(NULL, LOG_WARNING, "ast_get_enum need a suffix parameter now.\n");
		return -1;
	}
...
*/
	...
	/* Infrastructure ENUM rewrite */
		...
		if (sdl > strlen(number)) {	/* Number too short for this sdl? */
			ast_log_chan(NULL, LOG_WARNING, "I-ENUM: subdomain location %d behind number %s\n", sdl, number);
			ast_free(context);
			return 0;
		}
		...
		if ((sdl * 2 + strlen(middle) + 2) > sizeof(middle)) {
			ast_log_chan(NULL, LOG_WARNING, "ast_get_enum: not enough space for I-ENUM rewrite.\n");
			ast_free(context);
			return -1;
		}
	...
	if (strlen(left) * 2 + 2 > sizeof(domain)) {
		ast_log_chan(NULL, LOG_WARNING, "string to long in ast_get_enum\n");
		ast_free(context);
		return -1;
	}
	...
	if (cfg) {
		...
		if ((string = ast_variable_retrieve(cfg, "ienum", "ebl_alg"))) {
			...
			if (!strcasecmp(string, "txt"))
				...
				ast_log_chan(NULL, LOG_WARNING, "No valid parameter for ienum/ebl_alg.\n");
		}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER, "ENUM Support",
