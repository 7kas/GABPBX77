...
const char *ast_event_get_type_name(const struct ast_event *event)
{
	...
	if ((unsigned int)type >= ARRAY_LEN(event_names)) {
		ast_log_chan(NULL, LOG_ERROR, "Invalid event type - '%u'\n", type);
		return "";
	}
...
const char *ast_event_get_ie_type_name(enum ast_event_ie_type ie_type)
{
	if (ie_type <= 0 || ie_type >= ARRAY_LEN(ie_maps)) {
		ast_log_chan(NULL, LOG_ERROR, "Invalid IE type - '%d'\n", ie_type);
		return "";
	}
...
struct ast_event *ast_event_new(enum ast_event_type type, ...)
{
	...
	if (type >= AST_EVENT_TOTAL) {
		ast_log_chan(NULL, LOG_WARNING, "Someone tried to create an event of invalid "
			"type '%u'!\n", type);
		return NULL;
	}
		...
		ie_type = va_arg(ap, enum ast_event_ie_type))
	{
		...
		if (insert) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Unsupported PLTYPE(%d)\n", ie_value->ie_pltype);
		}
	}
	va_end(ap);
