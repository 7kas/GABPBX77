...
static int add_features_datastore(struct ast_channel *chan, const struct ast_flags *my_features, const struct ast_flags *peer_features)
{
	...
	if (!datastore) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to create channel features datastore.\n");
		return 0;
	}
	dialfeatures = ast_calloc(1, sizeof(*dialfeatures));
	if (!dialfeatures) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to allocate memory for feature flags.\n");
...
static void set_bridge_features_on_config(struct ast_bridge_config *config, const char *features)
{
	...
	for (feature = features; *feature; feature++) {
		...
		case 'x' :
			...
			break;
		default :
			ast_log_chan(NULL, LOG_WARNING, "Skipping unknown feature code '%c'\n", *feature);
			break;
		}
	}
}
		...
		struct ast_bridge_features *chan_features, struct ast_bridge_features *peer_features, int noanswer)
{
	...
	if (config->timelimit) {
		...
		if (ast_bridge_features_limits_construct(&call_duration_limits_chan)) {
			ast_log_chan(NULL, LOG_ERROR, "Could not construct caller duration limits. Bridge canceled.\n");

			return -1;
		}

		if (ast_bridge_features_limits_construct(&call_duration_limits_peer)) {
			ast_log_chan(NULL, LOG_ERROR, "Could not construct callee duration limits. Bridge canceled.\n");
		...
		if (abandon_call) {
			ast_log_chan(NULL, LOG_ERROR, "Could not set duration limits on one or more sides of the call. Bridge canceled.\n");
			return -1;
		}
	}
	...
	char *parse, struct timeval *calldurationlimit)
{
	...
	if (!config->timelimit) {
		ast_log_chan(NULL, LOG_WARNING, "Bridge does not accept L(%s)\n", limit_str);
	...
	if (!config->play_warning && !config->start_sound && !config->end_sound && config->timelimit) {
		...
		ast_verb_chan(NULL, 3, "Setting call duration limit to %.3lf seconds.\n",
		...
		ast_verb_chan(NULL, 4, "Limit Data for this call:\n");
		ast_verb_chan(NULL, 4, "timelimit      = %ld ms (%.3lf s)\n", config->timelimit, config->timelimit / 1000.0);
		ast_verb_chan(NULL, 4, "play_warning   = %ld ms (%.3lf s)\n", config->play_warning, config->play_warning / 1000.0);
		ast_verb_chan(NULL, 4, "play_to_caller = %s\n", play_to_caller ? "yes" : "no");
		ast_verb_chan(NULL, 4, "play_to_callee = %s\n", play_to_callee ? "yes" : "no");
		ast_verb_chan(NULL, 4, "warning_freq   = %ld ms (%.3lf s)\n", config->warning_freq, config->warning_freq / 1000.0);
		ast_verb_chan(NULL, 4, "start_sound    = %s\n", S_OR(config->start_sound, ""));
		ast_verb_chan(NULL, 4, "warning_sound  = %s\n", config->warning_sound);
		ast_verb_chan(NULL, 4, "end_sound      = %s\n", S_OR(config->end_sound, ""));
	}
	if (play_to_caller)
...
static int bridge_exec(struct ast_channel *chan, const char *data)
{
	...
	if (!current_dest_chan) {
		ast_verb_chan(NULL, 4, "Bridge failed because channel '%s' does not exist\n",
			args.dest_chan ?: "");
	...
	if (chan == current_dest_chan) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Unable to bridge channel %s with itself\n", ast_channel_name(chan));
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER, "Call Features",
