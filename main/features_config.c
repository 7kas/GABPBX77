		...
		<configFile name="features.conf">
			<configObject name="globals">
				...
				<configOption name="atxferdropcall" default="no">
					...
					<description>
						<para>When this option is set to <literal>no</literal>, then GABpbx will attempt to
						re-call the transferrer if the call to the transfer target fails. If the call to the
						transferrer fails, then GABpbx will wait <replaceable>atxferloopdelay</replaceable>
						...
						<para>When this option is set to <literal>yes</literal>, then GABpbx will not attempt
						to re-call the transferrer if the call to the transfer target fails. GABpbx will instead
	...
	while ((name = strsep(&group_names, "#"))) {
		...
		if (!group) {
			...
			if (item) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Unknown DYNAMIC_FEATURES item '%s' on channel %s.\n",
		...
		struct ast_variable *var, void *obj)
{
	...
	if (ast_strlen_zero(args.dtmf) ||
		...
		ast_log_chan(NULL, LOG_WARNING, "Invalid applicationmap syntax for '%s'. Missing required argument\n", var->name);
		return -1;
	}
	...
	if (paren) {
		...
		*paren++ = '\0';
		...
		if (ast_strlen_zero(args.app)) {
			ast_log_chan(NULL, LOG_WARNING, "Applicationmap item '%s' does not contain an application name.\n", var->name);
			return -1;
		}
	...
	if (!strcasecmp(args.activate_on, "self") || !strcasecmp(args.activate_on, "caller")) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Invalid 'activate_on' value %s for applicationmap item %s\n",
...
{
	if (!parking_warning) {
		ast_log_chan(NULL, LOG_WARNING, "Parkinglots are no longer configurable in features.conf; "
			"parking is now handled by res_parking.conf\n");
		parking_warning = 1;
	}
	ast_log_chan(NULL, LOG_WARNING, "The option '%s' is no longer configurable in features.conf.\n", var->name);
	return 0;
}
...
static int check_featuregroup(void *obj, void *arg, void *data, int flags)
{
	...
	if (*err) {
		ast_log_chan(NULL, LOG_WARNING, "Featuregroup %s refers to non-existent applicationmap item\n",
				group->name);
	}
	       ...
	       char *buf, size_t len)
{
	...
	if (res) {
		ast_log_chan(NULL, LOG_WARNING, "Invalid argument '%s' to FEATURE()\n", data);
	}
	       ...
	       char *buf, size_t len)
{
	...
	if (res) {
		ast_log_chan(NULL, LOG_WARNING, "Invalid argument '%s' to FEATUREMAP()\n", data);
	}
...
static int load_config(void)
{
	if (aco_info_init(&cfg_info)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to initialize configuration info for features\n");
		return -1;
	}
	...
	if (aco_process_config(&cfg_info, 0) == ACO_PROCESS_ERROR) {
		...
		if (aco_set_defaults(&global_option, "general", features_cfg->global) ||
			...
			ast_log_chan(NULL, LOG_ERROR, "Failed to load features.conf and failed to initialize defaults.\n");
			return -1;
		}

		ast_log_chan(NULL, LOG_NOTICE, "Could not load features config; using defaults\n");
