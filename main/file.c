...
#include "gabpbx/_private.h"	/* declare ast_file_init() */
 ...
 *
 * This is settable in gabpbx.conf.
 */
...
int __ast_format_def_register(const struct ast_format_def *f, struct ast_module *mod)
{
	...
	AST_RWLIST_TRAVERSE(&formats, tmp, list) {
		if (!strcasecmp(f->name, tmp->name)) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Tried to register '%s' format, already registered\n", f->name);
			return -1;
		}
	}
	...
	*tmp = *f;
	...
	AST_RWLIST_UNLOCK(&formats);
	ast_verb_chan(NULL, 5, "Registered file format %s, extension(s) %s\n", f->name, f->exts);
...
int ast_format_def_unregister(const char *name)
{
	...
	if (!res)
		ast_verb_chan(NULL, 5, "Unregistered format %s\n", name);
	else
		ast_log_chan(NULL, LOG_WARNING, "Tried to unregister format %s, already unregistered\n", name);
...
int ast_file_fdtemp(const char *path, char **filename, const char *template_name)
{
	...
	if (ast_asprintf(filename, "%s/%s", path, template_name) < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to set up temporary file path\n");
		return -1;
	}
	...
	if ((fd = mkstemp(*filename)) < 0) {
		ast_log_chan(NULL, LOG_NOTICE, "Failed to create temporary file\n");
		ast_free(*filename);
		return -1;
	}
...
int ast_stopstream(struct ast_channel *tmp)
{
	...
	if (ast_channel_stream(tmp)) {
		...
		if (ast_channel_oldwriteformat(tmp) && ast_set_write_format(tmp, ast_channel_oldwriteformat(tmp)))
			ast_log_chan(NULL, LOG_WARNING, "Unable to restore format back to %s\n", ast_format_get_name(ast_channel_oldwriteformat(tmp)));
	}
...
int ast_writestream(struct ast_filestream *fs, struct ast_frame *f)
{
	...
	if (f->frametype == AST_FRAME_VIDEO) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Tried to write non-voice frame\n");
		return -1;
	}
	if (ast_format_cmp(f->subclass.format, fs->fmt->format) != AST_FORMAT_CMP_NOT_EQUAL) {
		res =  fs->fmt->write(fs, f);
		if (res < 0)
			ast_log_chan(NULL, LOG_WARNING, "Natural write failed\n");
		else if (res > 0)
		...
		if (!fs->trans) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to translate to format %s, source format %s\n",
			...
			if ((trf = ast_translate(fs->trans, f, 0))) {
				...
				for (cur = trf; cur; cur = AST_LIST_NEXT(cur, frame_list)) {
					if ((res = fs->fmt->write(fs, trf))) {
						ast_log_chan(NULL, LOG_WARNING, "Translated frame write failed\n");
						break;
					}
				}
				ast_frfree(trf);
...
static int copy(const char *infile, const char *outfile)
{
	...
	if ((ifd = open(infile, O_RDONLY)) < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to open %s in read-only mode\n", infile);
		return -1;
	}
	if ((ofd = open(outfile, O_WRONLY | O_TRUNC | O_CREAT, AST_FILE_MODE)) < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to open %s in write-only mode\n", outfile);
		close(ifd);
		return -1;
	}
	while ( (len = read(ifd, buf, sizeof(buf)) ) ) {
		int res;
		if (len < 0) {
			ast_log_chan(NULL, LOG_WARNING, "Read failed on %s: %s\n", infile, strerror(errno));
			break;
		}
		/* XXX handle partial writes */
		res = write(ofd, buf, len);
		if (res != len) {
			ast_log_chan(NULL, LOG_WARNING, "Write failed on %s (%d of %d): %s\n", outfile, res, len, strerror(errno));
			len = -1; /* error marker */
			break;
		}
	}
...
static void filestream_close(struct ast_filestream *f)
{
	...
	case AST_MEDIA_TYPE_VIDEO:
		...
		break;
	default:
		ast_log_chan(NULL, AST_LOG_WARNING, "Unable to schedule deletion of filestream with unsupported type %s\n", f->fmt->name);
		break;
	}
}
...
static int fn_wrapper(struct ast_filestream *s, const char *comment, enum wrap_fn mode)
{
	...
	if (mode == WRAP_OPEN && (openfn = f->open) && openfn(s))
		ast_log_chan(NULL, LOG_WARNING, "Unable to open format %s\n", f->name);
	else if (mode == WRAP_REWRITE && f->rewrite && f->rewrite(s, comment))
		ast_log_chan(NULL, LOG_WARNING, "Unable to rewrite format %s\n", f->name);
	else {
	...
	AST_RWLIST_TRAVERSE(&formats, f, list) {
		...
		while ( (ext = strsep(&stringp, "|")) ) {
			...
			if (action == ACTION_OPEN) {
				...
				if (st.st_size == 0) {
					ast_log_chan(NULL, LOG_WARNING, "File %s detected to have zero size.\n", fn);
				}
			...
			case ACTION_DELETE:
				if ( (res = unlink(fn)) )
					ast_log_chan(NULL, LOG_WARNING, "unlink(%s) failed: %s\n", fn, strerror(errno));
				break;
			...
			case ACTION_COPY: {
				...
				if (!nfn)
					...
					if (res)
						ast_log_chan(NULL, LOG_WARNING, "%s(%s,%s) failed: %s\n",
			...
			default:
				ast_log_chan(NULL, LOG_WARNING, "Unknown helper %u\n", action);
			}
			ast_free(fn);
		}
	}
	...
	const char *filename, const char *preflang, int asis)
{
	...
	if (!fileexists_core(filename, NULL, preflang, buf, buflen, file_fmt_cap) ||
		...
		ast_log_chan(NULL, LOG_WARNING, "File %s does not exist in any format\n", filename);
	...
	const char *filename, const char *preflang)
{
	...
	for (i = 0; i < ast_format_cap_count(tmp_cap); ++i) {
		...
		ast_log_chan(NULL, LOG_WARNING, "File %s has video but couldn't be opened\n", filename);
		ao2_ref(format, -1);
	}
	ao2_ref(tmp_cap, -1);
	ao2_cleanup(nativeformats);
				...
				void *obj, int max_depth)
{
	...
	if (!(dir = opendir(path))) {
		ast_log_chan(NULL, LOG_ERROR, "Error opening directory - %s: %s\n",
			path, strerror(errno));
		return -1;
	}
 ...
 */
...
#endif
		{
			...
			if (stat(full_path, &statbuf)) {
				ast_log_chan(NULL, LOG_ERROR, "Error reading path stats - %s: %s\n",
	...
	if (!res && errno) {
		ast_log_chan(NULL, LOG_ERROR, "Error while reading directories - %s: %s\n",
			path, strerror(errno));
		res = -1;
	}
	...
	const char *preflang)
{
	...
	/* If file with the same name exists in /var/lib/gabpbx/sounds/custom directory, use that file.
	...
	if (!fs) {
		...
		if (!fs) {
			...
			ast_channel_lock(chan);
			ast_log_chan(NULL, LOG_WARNING, "Unable to open %s (format %s): %s\n",
...
struct ast_filestream *ast_readfile(const char *filename, const char *type, const char *comment, int flags, int check, mode_t mode)
{
	...
	AST_RWLIST_UNLOCK(&formats);
	if (!format_found)
		ast_log_chan(NULL, LOG_WARNING, "No such format '%s'\n", type);
...
struct ast_filestream *ast_writefile(const char *filename, const char *type, const char *comment, int flags, int check, mode_t mode)
{
	...
	AST_RWLIST_TRAVERSE(&formats, f, list) {
		...
		if (fd > -1) {
			...
			if (!bfile) {
				ast_log_chan(NULL, LOG_WARNING, "Whoa, fdopen failed: %s!\n", strerror(errno));
				close(fd);
				fd = -1;
			}
		}
		...
		if (fd > -1) {
			...
			if (!fs || rewrite_wrapper(fs, comment)) {
				ast_log_chan(NULL, LOG_WARNING, "Unable to rewrite %s\n", fn);
				close(fd);
				if (orig_fn) {
					unlink(fn);
					unlink(orig_fn);
	...
	ast_waitstream_fr_cb cb)
{
	...
	while (ast_channel_stream(c)) {
		...
		if (cmdfd < 0) {
			...
			if (res < 0) {
				ast_log_chan(NULL, LOG_WARNING, "Select failed (%s)\n", strerror(errno));
...
char *ast_format_str_reduce(char *fmts)
{
	...
	if (AST_RWLIST_RDLOCK(&formats)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to lock format list\n");
		return NULL;
	}
	...
	for (i = 0; i < x; i++) {
		...
		if (!fmts_ptr[i]) {
			ast_log_chan(NULL, LOG_WARNING, "ignoring unknown format '%s'\n", fmts_str[i]);
			continue;
		}
	...
	if (first) {
		ast_log_chan(NULL, LOG_WARNING, "no known formats found in format list (%s)\n", orig);
		return NULL;
	}
