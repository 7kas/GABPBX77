	...
	struct ast_variable *headers)
{
	 ...
	 * rather irritating feature creep on GABpbx.
	 *
	...
	struct ast_variable *headers)
{
	...
	ast_str_append(&out, 0,
		"<html><title>GABpbx HTTP Status</title>\r\n"
		"<body bgcolor=\"#ffffff\">\r\n"
		"<table bgcolor=\"#f1f1f1\" align=\"center\"><tr><td bgcolor=\"#e0e0ff\" colspan=\"2\" width=\"500\">\r\n"
		"<h2>&nbsp;&nbsp;GABpbx&trade; HTTP Status</h2></td></tr>\r\n");
	...
	ast_str_append(&out, 0, "</table><center><font size=\"-1\"><i>GABpbx and Digium are registered trademarks of Digium, Inc.</i></font></center></body></html>\r\n");
...
static struct ast_http_uri status_uri = {
	...
	.description = "GABpbx HTTP General Status",
	.uri = "httpstatus",
	.has_subtree = 0,
	.data = NULL,
	.key = __FILE__,
...
static struct ast_http_uri static_uri = {
	...
	.description = "GABpbx HTTP Static Delivery",
	.uri = "static",
	.has_subtree = 1,
	.data = NULL,
	.key= __FILE__,
...
static int http_body_read_contents(struct ast_tcptls_session_instance *ser, char *buf, int length, const char *what_getting)
{
	...
	if (total != length) {
		ast_log_chan(NULL, LOG_WARNING, "Wrong HTTP content read. Request %s (Wanted %d, Read %d)\n",
...
static int http_body_discard_contents(struct ast_tcptls_session_instance *ser, int length, const char *what_getting)
{
	...
	if (res < length) {
		ast_log_chan(NULL, LOG_WARNING, "Short HTTP request %s (Wanted %d but got %zd)\n",
			what_getting, length, res);
		return -1;
	}
	return 0;
...
static int http_body_get_chunk_length(struct ast_tcptls_session_instance *ser)
{
	...
	if (ast_iostream_gets(ser->stream, header_line, sizeof(header_line)) <= 0) {
		ast_log_chan(NULL, LOG_WARNING, "Short HTTP read of chunked header\n");
		return -1;
	}
	length = chunked_atoh(header_line, strlen(header_line));
	if (length < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Invalid HTTP chunk size\n");
		return -1;
	}
	return length;
}
...
static int http_body_check_chunk_sync(struct ast_tcptls_session_instance *ser)
{
	...
	if (res < sizeof(chunk_sync)) {
		ast_log_chan(NULL, LOG_WARNING, "Short HTTP chunk sync read (Wanted %zu)\n",
			sizeof(chunk_sync));
		return -1;
	}
	if (chunk_sync[0] != 0x0D || chunk_sync[1] != 0x0A) {
		ast_log_chan(NULL, LOG_WARNING, "HTTP chunk sync bytes wrong (0x%02hhX, 0x%02hhX)\n",
...
static int http_body_discard_chunk_trailer_headers(struct ast_tcptls_session_instance *ser)
{
	...
	for (;;) {
		if (ast_iostream_gets(ser->stream, header_line, sizeof(header_line)) <= 0) {
			ast_log_chan(NULL, LOG_WARNING, "Short HTTP read of chunked trailer header\n");
			return -1;
		}
	...
	struct ast_tcptls_session_instance *ser, struct ast_variable *headers)
{
	...
	if (0 < request->body_length) {
		...
		if (content_length > MAX_CONTENT_LENGTH) {
			ast_log_chan(NULL, LOG_WARNING, "Excessively long HTTP content. (%d > %d)\n",
...
static struct ast_http_auth *auth_create(const char *userid, const char *password)
{
	...
	if (!userid || !password) {
		ast_log_chan(NULL, LOG_ERROR, "Invalid userid/password\n");
		return NULL;
	}
...
struct ast_http_auth *ast_http_get_auth(struct ast_variable *headers)
{
	...
	for (v = headers; v; v = v->next) {
		...
		if (!password) {
			ast_log_chan(NULL, LOG_WARNING, "Invalid Authorization header\n");
			return NULL;
		}
...
int ast_http_response_status_line(const char *buf, const char *version, int code)
{
	...
	if (strncmp(buf, version, size) || buf[size] != ' ') {
		ast_log_chan(NULL, LOG_ERROR, "HTTP version not supported - "
			"expected %s\n", version);
		return -1;
	}
	...
	if (sscanf(buf, "%d", &status_code) != 1) {
		ast_log_chan(NULL, LOG_ERROR, "Could not read HTTP status code - "
			"%s\n", buf);
		return -1;
	}
			  ...
			  const char *value, const char *expected_value)
{
	...
	if (strcasecmp(value, expected_value)) {
		ast_log_chan(NULL, LOG_ERROR, "Invalid header value - expected %s "
			     ...
			     const char *value, const char *expected_value)
{
	...
	if (!strcasestr(expected_value, value)) {
		ast_log_chan(NULL, LOG_ERROR, "Header '%s' - could not locate '%s' "
...
static void *httpd_helper_thread(void *data)
{
	...
	if (ast_atomic_fetchadd_int(&session_count, +1) >= session_limit) {
		ast_log_chan(NULL, LOG_WARNING, "HTTP session count exceeded %d sessions.\n",
			session_limit);
		goto done;
	}
	...
	if (setsockopt(ast_iostream_get_fd(ser->stream), IPPROTO_TCP, TCP_NODELAY, (char *) &arg, sizeof(arg)) < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to set TCP_NODELAY on HTTP connection: %s\n", strerror(errno));
	}
...
static void add_redirect(const char *value)
{
	...
	if (!dest) {
		ast_log_chan(NULL, LOG_WARNING, "Invalid redirect '%s'\n", value);
		return;
	}
...
static void http_server_destroy(void *obj)
{
	...
	ast_verb_chan(NULL, 1, "Stopped http server '%s' listening at '%s'\n", server->name, server->address);
	...
	const struct ast_sockaddr *addr)
{
	...
	if (!server) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to allocate HTTP server '%s' at address '%s'\n",
			name, address);
		return NULL;
	}

	if (!(server->address = ast_strdup(address)) || !(server->name = ast_strdup(name))) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to complete setup for HTTP server '%s' at address '%s'\n",
			name, address);
		ao2_ref(server, -1);
		return NULL;
	}
...
static int http_server_start(struct ast_http_server *server)
{
	...
	if (server->args.accept_fd == -1) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to start HTTP server '%s' at address '%s'\n",
				server->name, server->address);
		return -1;
	}

	if (!ao2_link_flags(http_servers, server, OBJ_NOLOCK)) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to link HTTP server '%s' at address '%s'\n",
				server->name, server->address);
		return -1;
	}

	ast_verb_chan(NULL, 1, "Bound HTTP server '%s' to address %s\n", server->name, server->address);
	...
	uint32_t port)
{
	...
	if (!(num_addrs = ast_sockaddr_resolve(&addrs, host, 0, AST_AF_UNSPEC))) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to resolve host '%s'\n", host);
		return NULL;
	}
...
static int __ast_http_load(int reload)
{
	...
	snprintf(server_name, sizeof(server_name), "GABpbx/%s", ast_get_version());
	...
	for (; v; v = v->next) {
		...
		if (!strcasecmp(v->name, "servername")) {
			...
			if (ast_parse_arg(v->value, PARSE_UINT32 | PARSE_IN_RANGE | PARSE_DEFAULT,
				...
				ast_log_chan(NULL, LOG_WARNING, "Invalid port %s specified. Using default port %" PRId32 "\n",
			...
			if (ast_parse_arg(v->value, PARSE_INT32 | PARSE_DEFAULT | PARSE_IN_RANGE,
				...
				ast_log_chan(NULL, LOG_WARNING, "Invalid %s '%s' at line %d of http.conf\n",
			...
			ast_log_chan(NULL, LOG_WARNING, "Ignoring unknown option '%s' in http.conf\n", v->name);
		}
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER, "Built-in HTTP Server",
