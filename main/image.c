...
int ast_image_register(struct ast_imager *img)
{
	...
	ast_verb_chan(NULL, 5, "Registered format '%s' (%s)\n", img->name, img->desc);
	return 0;
}
...
void ast_image_unregister(struct ast_imager *img)
{
	...
	if (img)
		ast_verb_chan(NULL, 5, "Unregistered format '%s' (%s)\n", img->name, img->desc);
}
	...
	if (found) {
		...
		if (fd > -1) {
			if (!found->identify || found->identify(fd)) {
				...
				ast_log_chan(NULL, LOG_WARNING, "%s does not appear to be a %s file\n", buf, found->name);
			close(fd);
		} else
			ast_log_chan(NULL, LOG_WARNING, "Unable to open '%s': %s\n", buf, strerror(errno));
	} else
		ast_log_chan(NULL, LOG_WARNING, "Image file '%s' not found\n", filename);
