...
#include "gabpbx/_private.h" /* _init(), _reload() */
...
int ast_playtones_start(struct ast_channel *chan, int vol, const char *playlst, int interruptible)
{
	...
	while ((s = strsep(&stringp, separator)) && !ast_strlen_zero(s)) {
		...
		if (ast_tone_zone_part_parse(s, &tone_data)) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to parse tone part '%s'\n", s);
			continue;
		}
	...
	if (!d.nitems) {
		ast_log_chan(NULL, LOG_ERROR, "No valid tone parts\n");
		return -1;
	}
...
static int ast_set_indication_country(const char *country)
{
	...
	if (ast_strlen_zero(country)) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to get indication zone, country not set\n");
		return -1;
	}

	if (!(zone = ast_get_indication_zone(country))) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to get indication zone: %s\n", country);
		return -1;
	}

	ast_log_chan(NULL, LOG_NOTICE, "Setting default indication country to '%s'\n", country);
...
static int ast_register_indication_country(struct ast_tone_zone *zone)
{
	...
	ast_verb_chan(NULL, 5, "Registered indication country '%s'\n", zone->country);
...
static char *handle_cli_indication_add(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	if (!(tz = ast_get_indication_zone(a->argv[2]))) {
		...
		ast_log_chan(NULL, LOG_NOTICE, "Country '%s' does not exist, creating it.\n", a->argv[2]);
		...
		if (ast_register_indication_country(tz)) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to register new country\n");
			tz = ast_tone_zone_unref(tz);
			return CLI_FAILURE;
		}
	...
	if (ast_register_indication(tz, a->argv[3], a->argv[4])) {
		if (ast_strlen_zero(a->argv[3])) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to register indication %s\n", a->argv[2]);
		} else {
			ast_log_chan(NULL, LOG_WARNING, "Unable to register indication %s/%s\n", a->argv[2], a->argv[3]);
		}
		if (created_country) {
...
static char *handle_cli_indication_remove(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	if (a->argc == 3) {
		...
		if (ast_unregister_indication_country(a->argv[2])) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to unregister indication country %s\n", a->argv[2]);
			return CLI_FAILURE;
		}
	...
	if (!(tz = ast_get_indication_zone(a->argv[2]))) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to unregister indication %s/%s, country does not exists\n", a->argv[2], a->argv[3]);
		return CLI_FAILURE;
	}

	if (ast_unregister_indication(tz, a->argv[3])) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to unregister indication %s/%s\n", a->argv[2], a->argv[3]);
		res = CLI_FAILURE;
	}
...
static void store_tone_zone_ring_cadence(struct ast_tone_zone *zone, const char *val)
{
	...
	while ((ring = strsep(&c, ","))) {
		...
		if (!isdigit(ring[0]) || (value = atoi(ring)) == -1) {
			ast_log_chan(NULL, LOG_WARNING, "Invalid ringcadence given '%s'.\n", ring);
			continue;
		}
...
static int parse_tone_zone(struct ast_config *cfg, const char *country)
{
	...
	if (allocd) {
		if (!is_valid_tone_zone(zone)) {
			ast_log_chan(NULL, LOG_WARNING, "Indication country '%s' is invalid\n", country);
		} else if (ast_register_indication_country(zone)) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to register indication country '%s'.\n",
					country);
		}
	}
...
static int load_indications(int reload)
{
	...
	if (cfg == CONFIG_STATUS_FILEMISSING || cfg == CONFIG_STATUS_FILEINVALID) {
		ast_log_chan(NULL, LOG_WARNING, "Can't find indications config file %s.\n", config);
		return 0;
	...
	if (!ast_strlen_zero(country)) {
		ast_log_chan(NULL, LOG_NOTICE, "Default country for indication tones: %s\n", country);
		if (ast_set_indication_country(country)) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to set the default country (for indication tones)\n");
		}
	} else {
		ast_log_chan(NULL, LOG_WARNING, "Missing default country (for indication tones)\n");
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER, "Indication Tone Handling",
