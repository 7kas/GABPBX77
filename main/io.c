...
int ast_io_remove(struct io_context *ioc, int *_id)
{
	...
	if (!_id) {
		ast_log_chan(NULL, LOG_WARNING, "Asked to remove NULL?\n");
		return -1;
	}
	...
	ast_log_chan(NULL, LOG_NOTICE, "Unable to remove unknown id %p\n", _id);
...
void ast_io_dump(struct io_context *ioc)
{
	...
	ast_debug(1, "GABpbx IO Dump: %u entries, %u max entries\n", ioc->fdcnt, ioc->maxfdcnt);
	ast_debug(1, "================================================\n");
