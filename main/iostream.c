...
#include "gabpbx/iostream.h"          /* for DO_SSL */
...
#include "gabpbx/astobj2.h"           /* for ao2_alloc_options, ao2_alloc_... */
#include "gabpbx/logger.h"            /* for ast_debug, ast_log, LOG_ERROR */
#include "gabpbx/strings.h"           /* for gabpbx/threadstorage.h */
#include "gabpbx/threadstorage.h"     /* for ast_threadstorage_get, AST_TH... */
#include "gabpbx/time.h"              /* for ast_remaining_ms, ast_tvnow */
#include "gabpbx/utils.h"             /* for ast_wait_for_input, ast_wait_... */
	...
	if (stream->fd != -1) {
		...
		if (stream->ssl) {
			...
			if (res < 0) {
				...
				char err[256];
				ast_log_chan(NULL, LOG_ERROR, "SSL_shutdown() failed: %s, %s\n",
		 ...
		 * in the TCP stream.  See GABPBX-23548
		 */
		shutdown(stream->fd, SHUT_RDWR);
		if (close(stream->fd)) {
			ast_log_chan(NULL, LOG_ERROR, "close() failed: %s\n", strerror(errno));
		}
		stream->fd = -1;
	}
...
int ast_iostream_start_tls(struct ast_iostream **pstream, SSL_CTX *ssl_ctx, int client)
{
	...
	if (!stream->ssl) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to create new SSL connection\n");
		errno = ENOMEM;
		return -1;
	}
	...
	if (client && !ast_strlen_zero(stream->sni_hostname)) {
		if (!SSL_set_tlsext_host_name(stream->ssl, stream->sni_hostname)) {
			ast_log_chan(NULL, LOG_ERROR, "Unable to set SNI hostname '%s'\n",
				stream->sni_hostname);
			errno = EIO;
			return -1;
		}
	...
	if (res <= 0) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Problem setting up ssl connection: %s, %s\n",
			ERR_error_string(sslerr, err), ssl_error_to_string(sslerr, res));
		errno = EIO;
		return -1;
	}
	...
	return 0;
#else
	ast_log_chan(NULL, LOG_ERROR, "SSL not enabled in this build\n");
	errno = ENOTSUP;
	return -1;
#endif
}
