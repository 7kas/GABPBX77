...
struct ast_json *ast_json_vstringf(const char *format, va_list args)
{
	...
	if (format) {
		/* json_pack was not introduced until jansson-2.0 so GABpbx could never
...
int ast_json_init(void)
{
	...
	/* Setup to use GABpbx custom allocators */
	...
	version_check = json_pack("{s: o?, s: o*}",
		...
		"Bourne", NULL);
	if (!version_check) {
		ast_log_chan(NULL, LOG_ERROR, "There was a problem finding jansson 2.11 runtime libraries.\n"
			"Please rebuild GABpbx using ./configure --with-jansson-bundled.\n");
		return -1;
	}
