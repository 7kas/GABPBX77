...
#include "gabpbx/_private.h"   /* ast_ssl_init() */
...
#include "gabpbx/lock.h"       /* for ast_mutex_t, ast_mutex_init, ast_mut... */
#include "gabpbx/logger.h"     /* for ast_debug, ast_log, LOG_ERROR */
#include "gabpbx/utils.h"      /* for ast_calloc */
...
static void ssl_lock(int mode, int n, const char *file, int line)
{
	if (n < 0 || n >= ssl_num_locks) {
		ast_log_chan(NULL, LOG_ERROR, "OpenSSL is full of LIES!!! - "
				"ssl_num_locks '%d' - n '%d'\n",
				ssl_num_locks, n);
		return;
	}
...
/*!
 * \internal
 * \brief Common OpenSSL initialization for all of GABpbx.
 *
