...
#include "gabpbx/paths.h"	/* use ast_config_AST_MODULE_DIR */
#include <dirent.h>

#include "gabpbx/dlinkedlists.h"
...
/*** DOCUMENTATION
	<managerEvent language="en_US" name="Reload">
		<managerEventInstance class="EVENT_FLAG_SYSTEM">
			...
			<synopsis>Raised when a module has been reloaded in GABpbx.</synopsis>
			<syntax>
	...
	<managerEvent language="en_US" name="Load">
		<managerEventInstance class="EVENT_FLAG_SYSTEM">
			...
			<synopsis>Raised when a module has been loaded in GABpbx.</synopsis>
			<syntax>
				<parameter name="Module">
	...
	<managerEvent language="en_US" name="Unload">
		<managerEventInstance class="EVENT_FLAG_SYSTEM">
			...
			<synopsis>Raised when a module has been unloaded in GABpbx.</synopsis>
			<syntax>
				<parameter name="Module">
...
static void manual_mod_unreg(const char *resource)
{
	...
	/* When GABpbx shuts down the destructor is called automatically. */
...
static __attribute__((format(printf, 1, 2))) void module_load_error(const char *fmt, ...)
{
	...
	if (startup_error_builder) {
		...
		if (!copy || AST_VECTOR_APPEND(&startup_errors, copy)) {
			ast_log_chan(NULL, LOG_ERROR, "%s", ast_str_buffer(startup_error_builder));
			ast_free(copy);
		}
	} else {
...
void ast_module_register(const struct ast_module_info *info)
{
	 ...
	 * if an gabpbx module is dlopen'ed from outside the module loader.
	 */
...
static void logged_dlclose(const char *name, void *lib)
{
	...
	if (dlclose(lib)) {
		error = dlerror();
		ast_log_chan(NULL, AST_LOG_ERROR, "Failure in dlclose for module '%s': %s\n",
...
static void unload_dynamic_module(struct ast_module *mod)
{
	...
	if (is_module_loaded(name)) {
		ast_log_chan(NULL, LOG_WARNING, "Module '%s' could not be completely unloaded\n", name);
	}
#endif
}
	...
	do {
		...
		AST_DLLIST_TRAVERSE_BACKWARDS_SAFE_BEGIN(&module_list, mod, entry) {
			...
			if (mod->flags.running && !mod->flags.declined && mod->info->unload) {
				ast_verb_chan(NULL, 4, "Unloading %s\n", mod->resource);
				mod->info->unload();
			}
			module_destroy(mod);
			somethingchanged = 1;
...
static int graceful_unload_possible(struct ast_module *target, struct ast_vector_const_string *dependents)
{
	...
	AST_DLLIST_TRAVERSE(&module_list, mod, entry) {
		if (AST_VECTOR_GET_CMP(&mod->reffed_deps, target, AST_VECTOR_ELEM_DEFAULT_CMP)) {
			...
			if (!unloadable) {
				ast_log_chan(NULL, LOG_NOTICE, "Can't unload %s right now because %s is dependent on it\n", ast_module_name(target), ast_module_name(mod));
				return 0;
			}
			...
			if (AST_VECTOR_INSERT_AT(dependents, 0, name)) {
				ast_log_chan(NULL, LOG_ERROR, "Failed to add module '%s' to vector\n", ast_module_name(mod));
				return 0;
			}
			if (!--usecount) {
				break;
	...
	if (usecount) {
		ast_log_chan(NULL, LOG_NOTICE, "Module %s cannot be unloaded (would still have use count %d/%d after unloading dependents)\n", ast_module_name(target), usecount, target->usecount);
		return 0;
	}
	return 1;
}
...
static int auto_unload_resource(const char *resource_name, enum ast_module_unload_mode force, int recursive, struct ast_vector_const_string *dependents)
{
	...
	if (!(mod = find_resource(resource_name, 0))) {
		AST_DLLIST_UNLOCK(&module_list);
		ast_log_chan(NULL, LOG_WARNING, "Unload failed, '%s' could not be found\n", resource_name);
		return -1;
	}
	...
	if (!mod->flags.running || mod->flags.declined) {
		 ...
		 */
		ast_log_chan(NULL, LOG_NOTICE, "Unloading module '%s' that previously declined to load\n", resource_name);
		error = 0;
		res = 0;
	...
	if (!error && mod->usecount > 0 && recursive) {
		...
		if (graceful_unload_possible(mod, dependents)) {
			...
			for (i = AST_VECTOR_SIZE(dependents) - 1; i >= 0; i--) {
				...
				if (res) {
					ast_log_chan(NULL, LOG_WARNING, "Failed to unload %lu module%s automatically (%s could not be unloaded)\n", num_deps, ESS(num_deps), depname);
					...
					for (i++; i < num_deps; i++) {
						...
						if (res) {
							ast_log_chan(NULL, LOG_WARNING, "Could not load module '%s' again automatically\n", depname);
						}
					}
					break;
				}
	...
	if (!error && (mod->usecount > 0)) {
		if (force) {
			ast_log_chan(NULL, LOG_WARNING, "Warning:  Forcing removal of module '%s' with use count %d\n", resource_name, mod->usecount);
		} else {
			ast_log_chan(NULL, LOG_WARNING, "%s unload failed, '%s' has use count %d\n", recursive ? "Recursive soft" : "Soft", resource_name, mod->usecount);
			error = 1;
		}
	}
	...
	if (!error) {
		...
		res = mod->info->unload();
		if (res) {
			ast_log_chan(NULL, LOG_WARNING, "Firm unload failed for %s\n", resource_name);
			if (force <= AST_FORCE_FIRM) {
				error = 1;
			} else {
				ast_log_chan(NULL, LOG_WARNING, "** Dangerous **: Unloading resource anyway, at user request\n");
			}
		}
...
int ast_refresh_resource(const char *resource_name, enum ast_module_unload_mode force, int recursive)
{
	if (recursive) {
		...
		if (ast_load_resource(resource_name)) {
			ast_log_chan(NULL, LOG_WARNING, "Failed to load module '%s' again automatically\n", resource_name);
			AST_VECTOR_FREE(&dependents);
			return -1;
		}
		res = 0;
...
void ast_process_pending_reloads(void)
{
	...
	if (do_full_reload) {
		...
		ast_log_chan(NULL, LOG_NOTICE, "Executing deferred reload request.\n");
		ast_module_reload(NULL);
		return;
	}

	while ((item = AST_LIST_REMOVE_HEAD(&reload_queue, entry))) {
		ast_log_chan(NULL, LOG_NOTICE, "Executing deferred reload request for module '%s'.\n", item->module);
...
static void queue_reload_request(const char *module)
{
	...
	if (ast_strlen_zero(module)) {
		...
		if (!item) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to allocate reload queue item.\n");
			AST_LIST_UNLOCK(&reload_queue);
			return;
		}
...
static const char* loadresult2str(enum ast_module_load_result result)
{
	...
	ast_log_chan(NULL, LOG_WARNING, "Failed to find correct load result status. result %d\n", result);
...
enum ast_module_reload_result ast_module_reload(const char *name)
{
	...
	if (ast_mutex_trylock(&reloadlock)) {
		ast_verb_chan(NULL, 3, "The previous reload command didn't finish yet\n");
	...
	if (ast_opt_lock_confdir) {
		...
		for (try = 1, lockres = AST_LOCK_TIMEOUT; try < 6 && (lockres == AST_LOCK_TIMEOUT); try++) {
			...
			if (lockres == AST_LOCK_TIMEOUT) {
				ast_log_chan(NULL, LOG_WARNING, "Failed to grab lock on %s, try %d\n", ast_config_AST_CONFIG_DIR, try);
			}
		}
		if (lockres != AST_LOCK_SUCCESS) {
			ast_log_chan(NULL, AST_LOG_WARNING, "Cannot grab lock on %s\n", ast_config_AST_CONFIG_DIR);
	...
	AST_DLLIST_TRAVERSE(&module_list, cur, entry) {
		...
		ast_verb_chan(NULL, 3, "Reloading module '%s' (%s)\n", cur->resource, info->description);
...
static unsigned int inspect_module(const struct ast_module *mod)
{
	...
	if (!ast_strlen_zero(mod->info->buildopt_sum) &&
		...
		module_load_error("Module '%s' was not compiled with the same compile-time options as this version of GABpbx.\n", mod->resource);
...
static enum ast_module_load_result start_resource(struct ast_module *mod)
{
	...
	if (!ast_fully_booted) {
		ast_verb_chan(NULL, 4, "Loading %s.\n", mod->resource);
	}
	res = mod->info->load();
	...
	case AST_MODULE_LOAD_SUCCESS:
		if (!ast_fully_booted) {
			ast_verb_chan(NULL, 5, "%s => (%s)\n", mod->resource, term_color(tmp, mod->info->description, COLOR_BROWN, COLOR_BLACK, sizeof(tmp)));
		} else {
			ast_verb_chan(NULL, 4, "Loaded %s => (%s)\n", mod->resource, mod->info->description);
		}
	...
	struct module_vector *module_priorities, int required, int preload)
{
	...
	if ((mod = find_resource(resource_name, 0))) {
		if (mod->flags.running) {
			ast_log_chan(NULL, LOG_WARNING, "Module '%s' already loaded and running.\n", resource_name);
			return AST_MODULE_LOAD_DECLINE;
		}
	} else {
...
static int load_resource_list(struct load_order *load_order, int *mod_count)
{
	...
	if (AST_VECTOR_INIT(&module_priorities, 500)) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to initialize module loader.\n");
...
static int loader_config_init(struct load_order *load_order)
{
	...
	if (cfg == CONFIG_STATUS_FILEMISSING || cfg == CONFIG_STATUS_FILEINVALID) {
		ast_log_chan(NULL, LOG_WARNING, "'%s' invalid or missing.\n", AST_MODULE_CONFIG);
	...
	for (v = ast_variable_browse(cfg, "modules"); v; v = v->next) {
		...
		if (!strncasecmp(v->name, "preload", strlen("preload"))) {
			...
			if (!strcasecmp(v->name, "preload")) {
				...
				ast_log_chan(NULL, LOG_ERROR, "Unknown configuration option '%s'", v->name);
				goto done;
			}
	...
	if (ast_true(ast_variable_retrieve(cfg, "modules", "autoload"))) {
		...
		if (dir) {
			...
			ast_log_chan(NULL, LOG_ERROR, "Unable to open modules directory '%s'.\n", ast_config_AST_MODULE_DIR);
			goto done;
		}
	}
	...
	for (v = ast_variable_browse(cfg, "modules"); v; v = v->next) {
		...
		AST_LIST_TRAVERSE_SAFE_BEGIN(load_order, order, entry) {
			if (!resource_name_match(v->value, baselen, order->resource)) {
				if (order->builtin) {
					ast_log_chan(NULL, LOG_ERROR, "%s is a built-in module, you cannot specify 'noload'.\n", v->value);
					goto done;
				}

				if (order->required) {
					ast_log_chan(NULL, LOG_ERROR, "%s is configured with '%s' and 'noload', this is impossible.\n",
...
int load_modules(void)
{
	...
	ast_verb_chan(NULL, 1, "GABpbx Dynamic Loader Starting:\n");
	...
	if (load_count)
		ast_log_chan(NULL, LOG_NOTICE, "%u modules will be loaded.\n", load_count);

	res = load_resource_list(&load_order, &modulecount);
	if (res == -1) {
		ast_log_chan(NULL, LOG_WARNING, "Some non-required modules failed to load.\n");
		res = 0;
	}
	...
	AST_DLLIST_TRAVERSE(&module_list, cur, entry) {
		...
		if (cur->info->support_level == AST_MODULE_SUPPORT_DEPRECATED || !ast_strlen_zero(deprecated_in)
			...
			if (!ast_strlen_zero(deprecated_in)) {
				ast_str_append(&warning_msg, -1, " but %s deprecated in GABpbx version %s",
					 cur->info->support_level == AST_MODULE_SUPPORT_DEPRECATED ? "was" : "will be", deprecated_in);
				already_butted = 1;
			}

			if (!ast_strlen_zero(removed_in)) {
				ast_str_append(&warning_msg, -1, " %s will be removed in GABpbx version %s", already_butted ? "and" : "but", removed_in);
			} else {
		...
		if (cur->info->support_level == AST_MODULE_SUPPORT_DEPRECATED) {
			ast_log_chan(NULL, LOG_WARNING, "The deprecated module '%s' has been loaded and is running, it may be removed in a future version\n", cur->resource);
		}
#endif
	}
	...
	for (i = 0; i < AST_VECTOR_SIZE(&startup_errors); i++) {
		...
		ast_log_chan(NULL, LOG_ERROR, "%s", str);
		ast_free(str);
	}
