...
static char dateformat[256] = "%b %e %T";		/* Original GABpbx Format */
...
/*! \brief Logging channels used in the GABpbx logging system
 *
...
static struct logchannel *make_logchannel(const char *channel, const char *components, int lineno, int dynamic)
{
	...
	if (!strcasecmp(channel, "console")) {
		...
		*/
		...
		openlog("gabpbx", LOG_PID, chan->facility);
	} else {
		if (!(chan->fileptr = fopen(chan->filename, "a"))) {
			...
			fprintf(chan->fileptr, "[%s] GABpbx %s built by %s @ %s on a %s running %s on %s\n",
...
static int reload_logger(int rotate, const char *altconf)
{
	...
	if (logfiles.queue_log) {
		...
		ast_verb(1, "GABpbx Queue Logger restarted\n");
	} else {
		AST_RWLIST_UNLOCK(&logchannels);
		ast_verb_update();
	}
...
static char *handle_logger_add_channel(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	case CLI_INIT:
		...
		e->usage =
			...
			"       will exist until removed or until GABpbx is restarted.\n"
			...
			case LOGTYPE_FILE:
				{
					...
					if (res > 0) {
						...
						fprintf(stderr, "**** GABpbx Logging Error: ***********\n");
						if (errno == ENOMEM || errno == ENOSPC) {
							fprintf(stderr, "GABpbx logging error: Out of disk space, can't log to log file %s\n", chan->filename);
						} else {
...
int init_logger(void)
{
	...
	/* Re-initialize the logmsgs mutex.  The recursive mutex can be accessed prior
 	 * to GABpbx being forked into the background, which can cause the thread
...
void ast_log_chan(struct ast_channel *chan, int level,
                  ...
                  const char *fmt, ...)
{
		...
		const char *dialed = (ast_channel_dialed(chan) &&
					ast_channel_dialed(chan)->number.str &&
					ast_channel_dialed(chan)->number.str[0]) ?
					ast_channel_dialed(chan)->number.str :
					ast_channel_exten(chan);

                const char *context  = ast_channel_context(chan);
		ast_channel_unlock(chan);

                snprintf(prefix, sizeof(prefix),
                         "[%s->%s@%s] ",
                         caller, dialed, context);
...
log_without_prefix:
	...
	snprintf(newfmt, sizeof(newfmt), "%s%s", prefix, fmt ? fmt : "");

        va_list ap;
        va_start(ap, fmt);
        ast_log_ap(level, file, line, function, newfmt, ap);
        va_end(ap);
}
...
void __ast_verb_chan(struct ast_channel *chan, int level,
...
log_without_prefix:
        ...
        va_list ap;
        va_start(ap, fmt);
        __ast_verbose_ap(file, line, func, level, 0, newfmt, ap);
        va_end(ap);
}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER, "Logger",
