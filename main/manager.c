...
/*! \file
 *
 * \brief The GABpbx Management Interface - AMI
 *
 ...
 * - AMI-related API functions, used by internal gabpbx components
...
#include "gabpbx/paths.h"	/* use various ast_config_AST_* */
#include <ctype.h>
#include <sys/time.h>
#include <signal.h>
#include <sys/mman.h>
...
#include "gabpbx/tcptls.h"
#include "gabpbx/http.h"
 ...
 * \ref mansession is named this solely to keep the API the same in GABpbx. This structure really
				    ...
				    struct stasis_message *message)
{
	...
	if (!event_buffer) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Error while creating payload for event %s\n", type);
		ao2_cleanup(sessions);
		return;
	}
...
static char *handle_showmancmd(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	case CLI_INIT:
		...
		e->usage =
			...
			"	Shows the detailed description for a specific GABpbx manager interface command.\n";
		return NULL;
	case CLI_GENERATE:
		l = strlen(a->word);
		AST_RWLIST_RDLOCK(&actions);
...
#define HSMC_FORMAT "  %-*.*s  %-.*s\n"
	...
	case CLI_INIT:
		...
		e->usage =
			...
			"	Prints a listing of all the available GABpbx manager interface commands.\n";
		return NULL;
	case CLI_GENERATE:
		return NULL;
	}
...
static char *handle_kickmanconn(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	case CLI_INIT:
		...
		e->usage =
			...
			"	Kick an active GABpbx Manager Interface session\n";
		return NULL;
	case CLI_GENERATE:
		return NULL;
	}
...
static char *handle_showmaneventq(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	case CLI_INIT:
		...
		e->usage =
			...
			"	Prints a listing of all events pending in the GABpbx manger\n"
			"event queue.\n";
		return NULL;
	case CLI_GENERATE:
		return NULL;
...
static int authenticate(struct mansession *s, const struct message *m)
{
	...
	if (!(user = get_manager_by_name_locked(username))) {
		...
		ast_log_chan(NULL, LOG_NOTICE, "%s tried to authenticate with nonexistent user '%s'\n", ast_sockaddr_stringify_addr(&s->session->addr), username);
	} else if (user->acl && (ast_apply_acl(user->acl, &s->session->addr, "Manager User ACL: ") == AST_SENSE_DENY)) {
		report_failed_acl(s, username);
		ast_log_chan(NULL, LOG_NOTICE, "%s failed to pass IP ACL as '%s'\n", ast_sockaddr_stringify_addr(&s->session->addr), username);
	...
	if (error) {
		ast_log_chan(NULL, LOG_NOTICE, "%s failed to authenticate as '%s'\n", ast_sockaddr_stringify_addr(&s->session->addr), username);
		AST_RWLIST_UNLOCK(&users);
		return -1;
	}
...
void astman_live_dangerously(int new_live_dangerously)
{
	...
	{
		ast_log_chan(NULL, LOG_WARNING, "Manager Configuration load protection disabled.\n");
	}

	if (!new_live_dangerously && live_dangerously)
	{
		ast_log_chan(NULL, LOG_NOTICE, "Manager Configuration load protection enabled.\n");
	}
...
static enum error_type handle_updates(struct mansession *s, const struct message *m, struct ast_config *cfg, const char *dfn)
{
	...
	for (x = 0; x < 100000; x++) {	/* 100000 = the max number of allowed updates + 1 */
		...
		if (!strcasecmp(action, "newcat")) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Action-%06d: %s not handled\n", x, action);
...
static int action_login(struct mansession *s, const struct message *m)
{
	...
	if (manager_displayconnects(s->session)) {
		ast_verb_chan(NULL, 2, "%sManager '%s' logged on from %s\n", (s->session->managerid ? "HTTP " : ""), s->session->username, ast_sockaddr_stringify_addr(&s->session->addr));
	}
	...
	manager_hangup_cause_validator_t cause_validator)
{
	...
	if (cause_validator) {
		...
		if (causecode < 0 || causecode > 127 || *endptr != '\0') {
			ast_log_chan(NULL, LOG_NOTICE, "Invalid 'Cause: %s' in manager action Hangup\n", cause);
	...
	if (name_or_regex[0] != '/') {
		if (!(c = ast_channel_get_by_name(name_or_regex))) {
			ast_log_chan(NULL, LOG_NOTICE, "Request to hangup non-existent channel: %s\n",
				name_or_regex);
			astman_send_error(s, m, "No such channel");
			return 0;
		}

		ast_verb_chan(NULL, 3, "%sManager '%s' from %s, hanging up channel: %s\n",
	...
	if (varname[strlen(varname) - 1] == ')') {
		if (!c) {
			...
			if (c) {
				...
				ast_log_chan(NULL, LOG_ERROR, "Unable to allocate bogus channel for variable substitution.  Function results may be blank.\n");
		} else {
			ast_func_read(c, (char *) varname, workspace, sizeof(workspace));
		}
		varval = workspace;
...
static int coreshowchannelmap_add_to_map(struct ao2_container *c, const char *s)
{
	...
	str = ast_strdup(s);
	if (!str) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to append channel to channel map\n");
		return 1;
	}
...
static int manager_modulecheck(struct mansession *s, const struct message *m)
{
	 ...
	 * retrieve the 'GABPBX_FILE_VERSION' from that file, but we retain
...
static int process_message(struct mansession *s, const struct message *m)
{
	...
	if (ast_shutting_down()) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to process manager action '%s'. GABpbx is shutting down.\n", action);
		mansession_lock(s);
		astman_send_error(s, m, "GABpbx is shutting down");
		mansession_unlock(s);
		return 0;
	}
...
static int get_input(struct mansession *s, char *output)
{
	...
	if (s->session->inlen >= maxlen) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Discarding message from %s. Line too long: %.25s...\n", ast_sockaddr_stringify_addr(&s->session->addr), src);
	...
	while (res == 0) {
		...
		if (!s->session->authenticated) {
			if(time(&now) == -1) {
				ast_log_chan(NULL, LOG_ERROR, "error executing time(): %s\n", strerror(errno));
				return -1;
			}
	...
	if (res < 0) {
		...
		ast_log_chan(NULL, LOG_WARNING, "poll() returned error: %s\n", strerror(errno));
		return -1;
	}
		...
		if (res == 0) {
			...
			if (!s->session->authenticated) {
				...
				if (now - s->session->authstart > authtimeout) {
					if (displayconnects) {
						ast_verb_chan(NULL, 2, "Client from %s, failed to authenticate in %d seconds\n", ast_sockaddr_stringify_addr(&s->session->addr), authtimeout);
					}
					res = -1;
					break;
				}
...
static void *session_do(void *data)
{
	...
	if (setsockopt(ast_iostream_get_fd(ser->stream), IPPROTO_TCP, TCP_NODELAY, (char *) &arg, sizeof(arg)) < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to set TCP_NODELAY on manager connection: %s\n", strerror(errno));
	}
	...
	if(time(&session->authstart) == -1) {
		ast_log_chan(NULL, LOG_ERROR, "error executing time(): %s; disconnecting client\n", strerror(errno));
		ast_atomic_fetchadd_int(&unauth_sessions, -1);
		ao2_unlock(session);
	...
	astman_append(&s, "GABpbx Call Manager/%s\r\n", AMI_VERSION);	/* welcome prompt */
	for (;;) {
	...
	if (session->authenticated) {
		if (manager_displayconnects(session)) {
			ast_verb_chan(NULL, 2, "Manager '%s' %s from %s\n", session->username, session->kicked ? "kicked" : "logged off", ast_sockaddr_stringify_addr(&session->addr));
		}
	} else {
		ast_atomic_fetchadd_int(&unauth_sessions, -1);
		if (displayconnects) {
			ast_verb_chan(NULL, 2, "Connect attempt from '%s' unable to authenticate\n", ast_sockaddr_stringify_addr(&session->addr));
		}
	}
	...
	while ((session = ao2_iterator_next(&i)) && n_max > 0) {
		...
		if (session->sessiontimeout && (now > session->sessiontimeout) && !session->inuse) {
			if (session->authenticated
				...
				ast_verb_chan(NULL, 2, "HTTP Manager '%s' timed out from %s\n",
	...
	va_list ap)
{
	...
	ast_str_append(&buf, 0, "Root: %d\r\n"    , *ast_config_AST_SYSTEM_ROOT    );
	ast_str_append(&buf, 0, "PjsipUID: %d\r\n", *ast_config_AST_SYSTEM_PJSIPUID);
...
int ast_manager_unregister(const char *action)
{
	...
	if (cur) {
		...
		ast_verb_chan(NULL, 5, "Manager unregistered action %s\n", action);
	}
...
static int ast_manager_register_struct(struct manager_action *act)
{
	...
	AST_RWLIST_TRAVERSE(&actions, cur, list) {
		...
		if (ret == 0) {
			ast_log_chan(NULL, LOG_WARNING, "Manager: Action '%s' already registered\n", act->action);
			AST_RWLIST_UNLOCK(&actions);
			return -1;
		}
	...
	ast_verb_chan(NULL, 5, "Manager registered action %s\n", act->action);
...
static void close_mansession_file(struct mansession *s)
{
	if (s->stream) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Attempted to close file/file descriptor on mansession without a valid file or file descriptor.\n");
	}
}
...
static void process_output(struct mansession *s, struct ast_str **out, struct ast_variable *params, enum output_format format)
{
	...
	if (l > 0) {
		if (MAP_FAILED == (buf = mmap(NULL, l, PROT_READ | PROT_WRITE, MAP_PRIVATE, fd, 0))) {
			ast_log_chan(NULL, LOG_WARNING, "mmap failed.  Manager output was not processed\n");
		} else {
					     ...
					     struct ast_variable *headers)
{
	...
	if (!s.stream) {
		ast_log_chan(NULL, LOG_WARNING, "HTTP Manager, fdopen failed: %s!\n", strerror(errno));
	...
	if (process_message(&s, &m)) {
		if (session->authenticated) {
			if (manager_displayconnects(session)) {
				ast_verb_chan(NULL, 2, "HTTP Manager '%s' logged off from %s\n", session->username, ast_sockaddr_stringify_addr(&session->addr));
			}
		} else {
			if (displayconnects) {
				ast_verb_chan(NULL, 2, "HTTP Connect attempt from '%s' unable to authenticate\n", ast_sockaddr_stringify_addr(&session->addr));
			}
		}
...
#define TEST_STRING \
	...
	<input type=\"submit\">\n</form>\n"
		...
		ast_str_append(&out, 0, "<title>GABpbx&trade; Manager Interface</title>");
					     ...
					     struct ast_variable *headers)
{
	...
	if (sscanf(d.nonce, "%30lx", &nonce) != 1) {
		ast_log_chan(NULL, LOG_WARNING, "Received incorrect nonce in Digest <%s>\n", d.nonce);
		nonce = 0;
		goto out_401;
	}
	...
	if (!(session = find_session_by_nonce(d.username, nonce, &stale))) {
		...
		if (u_displayconnects) {
			ast_verb_chan(NULL, 2, "HTTP Manager '%s' logged in from %s\n", session->username, ast_sockaddr_stringify_addr(&session->addr));
		}
	...
	if (format == FORMAT_XML) {
		...
		"<html><head>\r\n"
		"<title>GABpbx&trade; Manager Interface</title>\r\n"
...
static int function_amiclient(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	if (ast_strlen_zero(data) ) {
		ast_log_chan(NULL, LOG_WARNING, "AMI_CLIENT() requires two arguments: AMI_CLIENT(<name>[,<arg>])\n");
		return -1;
	}
	...
	if (!(user = get_manager_by_name_locked(args.name))) {
		AST_RWLIST_UNLOCK(&users);
		ast_log_chan(NULL, LOG_ERROR, "There's no manager user called : \"%s\"\n", args.name);
		return -1;
	}
	AST_RWLIST_UNLOCK(&users);
	...
	if (!strcasecmp(args.param, "sessions")) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Invalid arguments provided to function AMI_CLIENT: %s\n", args.param);
		return -1;
...
static char *handle_manager_show_events(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	case CLI_INIT:
		...
		e->usage =
			"Usage: manager show events\n"
				"	Prints a listing of the available GABpbx manager interface events.\n";
		return NULL;
	case CLI_GENERATE:
		return NULL;
	}
	...
	if (!(it_events = ao2_callback(events, OBJ_MULTIPLE | OBJ_NOLOCK, NULL, NULL))) {
		ao2_unlock(events);
		ast_log_chan(NULL, AST_LOG_ERROR, "Unable to create iterator for events container\n");
		ast_free(buffer);
		ao2_ref(events, -1);
		return CLI_SUCCESS;
	}
	if (!(items = ast_calloc(sizeof(struct ast_xml_doc_item *), ao2_container_count(events)))) {
		ao2_unlock(events);
		ast_log_chan(NULL, AST_LOG_ERROR, "Unable to create temporary sorting array for events\n");
...
static int subscribe_all(void)
{
	if (manager_subscriptions_init()) {
		ast_log_chan(NULL, AST_LOG_ERROR, "Failed to initialize manager subscriptions\n");
		return -1;
	}
	if (manager_system_init()) {
		ast_log_chan(NULL, AST_LOG_ERROR, "Failed to initialize manager system handling\n");
		return -1;
	}
	if (manager_channels_init()) {
		ast_log_chan(NULL, AST_LOG_ERROR, "Failed to initialize manager channel handling\n");
		return -1;
	}
	if (manager_mwi_init()) {
		ast_log_chan(NULL, AST_LOG_ERROR, "Failed to initialize manager MWI handling\n");
		return -1;
	}
	...
	if (manager_endpoints_init()) {
		ast_log_chan(NULL, AST_LOG_ERROR, "Failed to initialize manager endpoints handling\n");
		return -1;
	}
...
static int __init_manager(int reload, int by_external_config)
{
	...
	if (cfg == CONFIG_STATUS_FILEUNCHANGED) {
		...
		ast_log_chan(NULL, LOG_NOTICE, "Unable to open AMI configuration manager.conf, or configuration is invalid.\n");
		return 0;
	}
	...
	for (var = ast_variable_browse(cfg, "general"); var; var = var->next) {
		...
		if (!strcasecmp(var->name, "enabled")) {
			...
			if (ast_parse_arg(val, PARSE_UINT32|PARSE_IN_RANGE, &bindport, 1024, 65535)) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid port number '%s'\n", val);
			}
			...
			if (ast_parse_arg(val, PARSE_ADDR|PARSE_PORT_IGNORE, NULL)) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid address '%s' specified, default '%s' will be used\n", val,
			...
			if (timeout < 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid authtimeout value '%s', using default value\n", var->value);
			} else {
				authtimeout = timeout;
			}
			...
			if (limit < 1) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid authlimit value '%s', using default value\n", var->value);
			} else {
				authlimit = limit;
			}
			...
			ast_log_chan(NULL, LOG_NOTICE, "Invalid keyword <%s> = <%s> in manager.conf [general]\n",
				var->name, val);
		}
	}
	...
	if (manager_enabled && !subscribed) {
		if (subscribe_all() != 0) {
			ast_log_chan(NULL, LOG_ERROR, "Manager subscription error\n");
			return -1;
		}
	}
		...
		while ((cat = ast_category_browse(ucfg, cat))) {
			...
			if ((!hasmanager && genhasmanager) || ast_true(hasmanager)) {
				...
				if (user_writetimeout) {
					...
					if (value < 100) {
						ast_log_chan(NULL, LOG_WARNING, "Invalid writetimeout value '%d' in users.conf\n", value);
					} else {
	...
	while ((cat = ast_category_browse(cfg, cat))) {
		...
		for (; var; var = var->next) {
			if (!strcasecmp(var->name, "secret")) {
				...
				if (acl_error) {
					ast_log_chan(NULL, LOG_ERROR, "Invalid ACL '%s' for manager user '%s' on line %d. Deleting user\n",
				...
				if (value < 100) {
					ast_log_chan(NULL, LOG_WARNING, "Invalid writetimeout value '%s' at line %d\n", var->value, var->lineno);
				} else {
					user->writetimeout = value;
				}
	...
	struct stasis_message *message)
{
	...
	ast_log_chan(NULL, LOG_NOTICE, "Reloading manager in response to ACL change event.\n");
	__init_manager(1, 1);
}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER, "GABpbx Manager Interface",
