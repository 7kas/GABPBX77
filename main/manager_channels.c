...
/*! \file
 *
 * \brief The GABpbx Management Interface - AMI (channel event handling)
 *
		...
		struct stasis_message *message)
{
	...
	if (!spyer) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Received ChanSpy Stop event with no spyer channel!\n");
		return;
	}
		...
		struct stasis_message *message)
{
	...
	spyer = ast_multi_channel_blob_get_channel(payload, "spyer_channel");
	if (!spyer) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Received ChanSpy Start event with no spyer channel!\n");
		return;
	}
	spyee = ast_multi_channel_blob_get_channel(payload, "spyee_channel");
	if (!spyee) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Received ChanSpy Start event with no spyee channel!\n");
		return;
	}
