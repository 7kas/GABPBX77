	...
	struct ast_variable *metadata)
{
	...
	if (stat(file_path, &st)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to obtain information for file %s for URI %s\n",
			file_path, uri);
		return -1;
	}
	...
	if (bucket_file) {
		...
		if (!bucket_file) {
			ast_log_chan(NULL, LOG_WARNING, "Failed to create file storage for %s and %s\n",
				uri, file_path);
			return -1;
		}
		created = 1;
	...
	if (created && ast_bucket_file_create(bucket_file)) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to create media for %s\n", uri);
		ao2_ref(bucket_file, -1);
		return -1;
	}
	media_cache_item_sync_to_astdb(bucket_file);
...
static int media_cache_item_populate_from_astdb(const char *uri, const char *hash)
{
	...
	if (ast_strlen_zero(bucket_file->path)) {
		ao2_ref(bucket_file, -1);
		ast_log_chan(NULL, LOG_WARNING, "Failed to restore media cache item for '%s' from AstDB: no 'path' specified\n",
			uri);
		return -1;
	}
