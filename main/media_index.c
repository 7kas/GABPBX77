	...
	const char *match_filename)
{
	...
	if (!f) {
		ast_log_chan(NULL, LOG_WARNING, "Could not open media description file '%s': %s\n", ast_str_buffer(description_file_path), strerror(errno));
		return -1;
	}
	...
	while (!feof(f)) {
		...
		if (!fgets(buf, sizeof(buf), f)) {
			if (ferror(f)) {
				ast_log_chan(NULL, LOG_ERROR, "Error reading from file %s: %s\n", ast_str_buffer(description_file_path), strerror(errno));
			}
			continue;
		}
		...
		if (strlen(buf) == sizeof(buf) - 1 && buf[sizeof(buf) - 1] != '\n') {
			ast_log_chan(NULL, LOG_WARNING, "Line too long, skipping. It begins with: %.32s...\n", buf);
...
static int read_dirs_cb(const char *dir_name, const char *filename, void *obj)
{
	 ...
	 * Example:
	 * From the filesystem:
	 * 	  index's base_dir = "/var/lib/gabpbx/sounds"
	 * 	  search_variant = "en"
	 * 	  search directory base = "/var/lib/gabpbx/sounds/en"
	 * 	  dirname_len = 27
	 *    current dir_name = "/var/lib/gabpbx/sounds/en/digits"
	 *    subdirs =                                     "/digits"
	 *    filename = "1.ulaw"
	 *
