		...
		<syntax argsep="/">
			...
			<enumlist>
				<enum name="to">
					...
					the recipient of the message that was received by GABpbx.</para>
					<para>
					</para>
				...
				<enum name="body">
					...
					message, this includes the body of the message that GABpbx
 ...
 * that the GABpbx core doesn't waste any time trying to generate
...
static int msg_q_cb(void *data)
{
	...
	if (res != 0) {
		ast_log_chan(NULL, LOG_WARNING, "No handler processed message from %s to %s\n",
		...
		char *data, char *buf, size_t len)
{
	...
	if (!(ds = ast_channel_datastore_find(chan, &msg_datastore, NULL))) {
		ast_channel_unlock(chan);
		ast_log_chan(NULL, LOG_ERROR, "No MESSAGE data found on the channel to read.\n");
		return -1;
	}
	...
	if (!strcasecmp(data, "to")) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Invalid argument to MESSAGE(): '%s'\n", data);
	}
		...
		char *data, const char *value)
{
	...
	if (!strcasecmp(data, "to")) {
		...
		if (!strcasecmp(value, "mark_all_outbound")) {
			...
			ast_log_chan(NULL, LOG_WARNING, "'%s' is not a valid value for custom_data\n", value);
		}
		...
		ast_log_chan(NULL, LOG_WARNING, "'%s' is not a valid write argument.\n", data);
	}
...
static int msg_send_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "An argument is required to MessageSend()\n");
	...
	if (ast_strlen_zero(args.destination)) {
		ast_log_chan(NULL, LOG_WARNING, "A 'to' URI is required for MessageSend()\n");
		pbx_builtin_setvar_helper(chan, "MESSAGE_SEND_STATUS", "INVALID_URI");
		return 0;
	}
	...
	if (!(ds = ast_channel_datastore_find(chan, &msg_datastore, NULL))) {
		ast_channel_unlock(chan);
		ast_log_chan(NULL, LOG_WARNING, "No message data found on channel to send.\n");
	...
	if (!msg_tech) {
		ast_log_chan(NULL, LOG_WARNING, "No message technology '%s' found.\n", tech_name);
...
int ast_msg_send(struct ast_msg *msg, const char *to, const char *from)
{
	...
	if (!msg_tech) {
		ast_log_chan(NULL, LOG_ERROR, "Unknown message tech: %s\n", tech_name);
		ast_rwlock_unlock(&msg_techs_lock);
		return -1;
	}
...
int ast_msg_tech_register(const struct ast_msg_tech *tech)
{
	...
	if (match) {
		ast_log_chan(NULL, LOG_ERROR, "Message technology already registered for '%s'\n",
		        tech->name);
		ast_rwlock_unlock(&msg_techs_lock);
		return -1;
	}
	...
	if (AST_VECTOR_APPEND(&msg_techs, tech)) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to register message technology for '%s'\n",
		        tech->name);
		ast_rwlock_unlock(&msg_techs_lock);
		return -1;
	}
	ast_verb_chan(NULL, 5, "Message technology '%s' registered.\n", tech->name);
...
int ast_msg_tech_unregister(const struct ast_msg_tech *tech)
{
	...
	if (match) {
		ast_log_chan(NULL, LOG_ERROR, "No '%s' message technology found.\n", tech->name);
		return -1;
	}

	ast_verb_chan(NULL, 5, "Message technology '%s' unregistered.\n", tech->name);
...
int ast_msg_handler_register(const struct ast_msg_handler *handler)
{
	...
	if (match) {
		ast_log_chan(NULL, LOG_ERROR, "Message handler already registered for '%s'\n",
		        handler->name);
		ast_rwlock_unlock(&msg_handlers_lock);
		return -1;
	}
	...
	if (AST_VECTOR_APPEND(&msg_handlers, handler)) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to register message handler for '%s'\n",
		        handler->name);
		ast_rwlock_unlock(&msg_handlers_lock);
		return -1;
	}
	ast_verb_chan(NULL, 5, "Message handler '%s' registered.\n", handler->name);
...
int ast_msg_handler_unregister(const struct ast_msg_handler *handler)
{
	...
	if (match) {
		ast_log_chan(NULL, LOG_ERROR, "No '%s' message handler found.\n", handler->name);
		return -1;
	}

	ast_verb_chan(NULL, 5, "Message handler '%s' unregistered.\n", handler->name);
	return 0;
}
...
/*!
 * \internal
 * \brief Clean up other resources on GABpbx shutdown
 *
 * \note This does not include the msg_q_tp object, which must be disposed
 * of prior to GABpbx checking for channel destruction in its shutdown
...
/*!
 * \internal
 * \brief Initialize stuff during GABpbx startup.
 *
 * Cleanup isn't a big deal in this function.  If we return non-zero,
 * GABpbx is going to exit.
 *
 * \retval 0 success
 * \retval non-zero failure
 */
