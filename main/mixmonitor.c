...
int ast_set_mixmonitor_methods(struct ast_mixmonitor_methods *method_table)
{
	...
	if (table_loaded) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Tried to set mixmonitor methods, but something else has already provided them.\n");
		return -1;
	}
...
int ast_clear_mixmonitor_methods(void)
{
	...
	if (!table_loaded) {
		ast_log_chan(NULL, LOG_ERROR, "Tried to clear mixmonitor methods, but none are currently loaded.\n");
		return -1;
	}
...
int ast_start_mixmonitor(struct ast_channel *chan, const char *filename, const char *options)
{
	...
	if (!mixmonitor_methods.start) {
		ast_log_chan(NULL, LOG_ERROR, "No loaded module currently provides MixMonitor starting functionality.\n");
		return -1;
	}
...
int ast_stop_mixmonitor(struct ast_channel *chan, const char *mixmon_id)
{
	...
	if (!mixmonitor_methods.stop) {
		ast_log_chan(NULL, LOG_ERROR, "No loaded module currently provides MixMonitor stopping functionality.\n");
		return -1;
	}
