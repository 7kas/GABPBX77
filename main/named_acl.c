...
static struct named_acl *named_acl_find_realtime(const char *name)
{
	...
	while ((item = ast_category_browse(cfg, item))) {
		...
		if (append_ha_error) {
			...
			ast_log_chan(NULL, LOG_ERROR, "Rejecting realtime ACL due to bad ACL definition '%s': %s - %s - %s\n", name, order, sense, rule);
	...
	if (!acl) {
		ast_log_chan(NULL, LOG_ERROR, "allocation error\n");
		ast_free_ha(built_ha);
		return NULL;
	}
...
struct ast_ha *ast_named_acl_find(const char *name, int *is_realtime, int *is_undefined)
{
	...
	if ((!cfg) || (!(cfg->named_acl_list))) {
		ast_log_chan(NULL, LOG_ERROR, "Attempted to find named ACL '%s', but the ACL configuration isn't available.\n", name);
		return NULL;
	}
	...
	if (!named_acl) {
		...
		if (ast_realtime_is_mapping_defined(ACL_FAMILY) && !ast_check_realtime(ACL_FAMILY)) {
			ast_log_chan(NULL, LOG_WARNING, "ACL '%s' does not exist. The ACL will be marked as undefined and will automatically fail if applied.\n"
	...
	if (!ha) {
		ast_log_chan(NULL, LOG_NOTICE, "ACL '%s' contains no rules. It is valid, but it will accept addresses unconditionally.\n", name);
	}
...
publish_failure:
	ast_log_chan(NULL, LOG_ERROR, "Failed to issue ACL change message for %s.\n",
...
static void cli_display_named_acl(int fd, const char *name)
{
	...
	if ((!cfg) || (!cfg->named_acl_list)) {
		ast_log_chan(NULL, LOG_ERROR, "Attempted to show named ACL '%s', but the acl configuration isn't available.\n", name);
		return;
	}
...
static int reload_module(void)
{
	...
	if (status == ACO_PROCESS_ERROR) {
		ast_log_chan(NULL, LOG_WARNING, "Could not reload ACL config\n");
		return 0;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER, "Named ACL system",
