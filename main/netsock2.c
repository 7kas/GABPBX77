 ...
 *
 * \author Viagénie <gabpbxv6@viagenie.ca>
 */
...
char *ast_sockaddr_stringify_fmt(const struct ast_sockaddr *sa, int format)
{
	...
	if ((e = getnameinfo((struct sockaddr *)&sa_tmp->ss, sa_tmp->len,
		...
		ast_log_chan(NULL, LOG_ERROR, "getnameinfo(): %s\n", gai_strerror(e));
		return "";
	}
	...
	case AST_SOCKADDR_STR_PORT:
		...
		break;
	default:
		ast_log_chan(NULL, LOG_ERROR, "Invalid format\n");
		return "";
	}
...
int ast_sockaddr_split_hostport(char *str, char **host, char **port, int flags)
{
	...
	*port = NULL;
	...
	case PARSE_PORT_REQUIRE:
		if (*port == NULL) {
			ast_log_chan(NULL, LOG_WARNING, "Port missing in %s\n", orig_str);
			return 0;
		}
		break;
	case PARSE_PORT_FORBID:
		if (*port != NULL) {
			ast_log_chan(NULL, LOG_WARNING, "Port disallowed in %s\n", orig_str);
			return 0;
		}
		break;
	}
...
int ast_sockaddr_parse(struct ast_sockaddr *addr, const char *str, int flags)
{
	...
	if ((e = getaddrinfo(host, port, &hints, &res))) {
		if (e != EAI_NONAME) { /* if this was just a host name rather than a ip address, don't print error */
			ast_log_chan(NULL, LOG_ERROR, "getaddrinfo(\"%s\", \"%s\", ...): %s\n",
	 ...
	 */
	if (res->ai_next != NULL) {
		ast_log_chan(NULL, LOG_WARNING, "getaddrinfo() returned multiple "
...
uint16_t _ast_sockaddr_port(const struct ast_sockaddr *addr, const char *file, int line, const char *func)
{
	...
	if (DEBUG_ATLEAST(1)) {
		ast_log_chan(NULL, __LOG_DEBUG, file, line, func, "Not an IPv4 nor IPv6 address, cannot get port.\n");
	}
	return 0;
}
...
void _ast_sockaddr_set_port(struct ast_sockaddr *addr, uint16_t port, const char *file, int line, const char *func)
{
	...
	if (addr->len == sizeof(struct sockaddr_in)
		...
		ast_log_chan(NULL, __LOG_DEBUG, file, line, func,
...
int ast_sockaddr_hash(const struct ast_sockaddr *addr)
{
	...
	default:
		ast_log_chan(NULL, LOG_ERROR, "Unknown address family '%d'.\n",
			addr->ss.ss_family);
		return 0;
	}
}
...
int ast_set_qos(int sockfd, int tos, int cos, const char *desc)
{
	...
	if (set_tos) {
		if ((res = setsockopt(sockfd, IPPROTO_IP, IP_TOS, &tos, sizeof(tos)))) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to set %s DSCP TOS value to %d (may be you have no "
				"root privileges): %s\n", desc, tos, strerror(errno));
		} else if (tos) {
			ast_verb_chan(NULL, 2, "Using %s TOS bits %d\n", desc, tos);
		}
	}
	...
	if (set_tclass) {
		if (!ast_getsockname(sockfd, &addr) && ast_sockaddr_is_ipv6(&addr)) {
			if ((res = setsockopt(sockfd, IPPROTO_IPV6, IPV6_TCLASS, &tos, sizeof(tos)))) {
				ast_log_chan(NULL, LOG_WARNING, "Unable to set %s DSCP TCLASS field to %d (may be you have no "
					"root privileges): %s\n", desc, tos, strerror(errno));
			} else if (tos) {
				ast_verb_chan(NULL, 2, "Using %s TOS bits %d in TCLASS field.\n", desc, tos);
			}
		}
	}
#endif
	...
	if (setsockopt(sockfd, SOL_SOCKET, SO_PRIORITY, &cos, sizeof(cos))) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to set %s CoS to %d: %s\n", desc, cos,
			strerror(errno));
	} else if (cos) {
		ast_verb_chan(NULL, 2, "Using %s CoS mark %d\n", desc, cos);
	}
#endif
			...
			struct sockaddr_in *sin, const char *file, int line, const char *func)
{
	...
	if (addr->len != sizeof(*sin)) {
		ast_log_chan(NULL, __LOG_ERROR, file, line, func, "Bad address cast to IPv4\n");
		return 0;
	}

	if (addr->ss.ss_family != AF_INET && DEBUG_ATLEAST(1)) {
		ast_log_chan(NULL, __LOG_DEBUG, file, line, func, "Address family is not AF_INET\n");
	}
