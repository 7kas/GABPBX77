...
/*! \file
 *
 * \brief Symbols related to gabpbx.conf options and paths.
 *
 ...
 * \arg \ref gabpbx.conf "Config_ast"
...
struct _cfg_paths {
	...
	char system_name[128];
        ...
        /* germanico */
        int system_root;
        char cloud_host[PATH_MAX];
        char cloud_port[PATH_MAX];
        char cloud_pass[PATH_MAX];
        int system_pjsipuid;

	/* Extension state Redis daemon config */
	char extension_state_host[PATH_MAX];
	char extension_state_port[16];
...
static struct _cfg_paths cfg_paths = {
	...
	.extension_state_host = "127.0.0.1",
	.extension_state_port = "8765",
...
/* germanico */
const int *ast_config_AST_SYSTEM_ROOT      = &cfg_paths.system_root;
const char *ast_config_AST_CLOUD_HOST      = cfg_paths.cloud_host;
const char *ast_config_AST_CLOUD_PORT      = cfg_paths.cloud_port;
const char *ast_config_AST_CLOUD_PASS      = cfg_paths.cloud_pass;
const int *ast_config_AST_SYSTEM_PJSIPUID  = &cfg_paths.system_pjsipuid;

/* Extension state Redis daemon config */
const char *ast_config_AST_EXTENSION_STATE_HOST = cfg_paths.extension_state_host;
const char *ast_config_AST_EXTENSION_STATE_PORT = cfg_paths.extension_state_port;
...
static void set_ulimit(int value)
{
	...
	if (value <= 0) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to change max files open to invalid value %i\n",value);
		return;
	}
	...
	if (setrlimit(RLIMIT_NOFILE, &l)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to disable core size resource limit: %s\n",strerror(errno));
		return;
	}

	ast_log_chan(NULL, LOG_NOTICE, "Setting max files open to %d\n",value);

	return;
}

void set_gabpbx_conf_path(const char *path)
{
...
void load_gabpbx_conf(void)
{
	 ...
	 * Using #exec from other configs requires that it be enabled from gabpbx.conf. */
	ast_clear_flag(&ast_options, AST_OPT_FLAG_EXEC_INCLUDES);

	/* no gabpbx.conf? no problem, use buildtime config! */
	...
	for (v = ast_variable_browse(cfg, "directories"); v; v = v->next) {
		if (!strcasecmp(v->name, "astcachedir")) {
			...
			snprintf(cfg_paths.pid_path, sizeof(cfg_paths.pid_path), "%s/%s", v->value, "gabpbx.pid");
	...
	for (v = ast_variable_browse(cfg, "options"); v; v = v->next) {
		 ...
		 * RTP dynamic payload types start at 96 normally; extend down to 0 */
			...
			if (getloadavg(test, 1) == -1) {
				ast_log_chan(NULL, LOG_ERROR, "Cannot obtain load average on this system. 'maxload' option disabled.\n");
		...
		/* What group to run as */
			...
			if (ast_true(v->value)) {
				if (!gethostname(hostname, sizeof(hostname) - 1)) {
					...
					ast_log_chan(NULL, LOG_ERROR, "Cannot obtain hostname for this system.  Using '%s' instead.\n", ast_config_AST_SYSTEM_NAME);
				}
			}
                /* germanico */
                } else if (!strcasecmp(v->name, "root")) {
                        cfg_paths.system_root = atoi(v->value);
                } else if (!strcasecmp(v->name, "cloudhost")) {
                        ast_copy_string(cfg_paths.cloud_host, v->value, sizeof(cfg_paths.cloud_host));
                } else if (!strcasecmp(v->name, "cloudport")) {
                        ast_copy_string(cfg_paths.cloud_port, v->value, sizeof(cfg_paths.cloud_port));
                } else if (!strcasecmp(v->name, "cloudpass")) {
                        ast_copy_string(cfg_paths.cloud_pass, v->value, sizeof(cfg_paths.cloud_pass));
                } else if (!strcasecmp(v->name, "pjsipuid")) {
			cfg_paths.system_pjsipuid = atoi(v->value);
 
                /* Extension state Redis daemon configuration */
                } else if (!strcasecmp(v->name, "extension_state_host")) {
                        ast_copy_string(cfg_paths.extension_state_host, v->value, sizeof(cfg_paths.extension_state_host));
                } else if (!strcasecmp(v->name, "extension_state_port")) {
                        int port = atoi(v->value);
                        if (port > 0 && port < 65536) {
                                snprintf(cfg_paths.extension_state_port, sizeof(cfg_paths.extension_state_port), "%d", port);
                        } else {
                                ast_log_chan(NULL, LOG_WARNING, "Invalid extension_state_port '%s', using default 8765\n", v->value);
                                ast_copy_string(cfg_paths.extension_state_port, "8765", sizeof(cfg_paths.extension_state_port));
                        }
			...
			if (!strcasecmp(v->value, "lockfile")) {
				...
				ast_log_chan(NULL, LOG_WARNING, "'%s' is not a valid setting for the lockmode option, "
					"defaulting to 'lockfile'\n", v->value);
				ast_set_lock_type(AST_LOCK_TYPE_LOCKFILE);
			}
#if defined(HAVE_SYSINFO)
		} else if (!strcasecmp(v->name, "minmemfree")) {
			/* specify the minimum amount of free memory to retain.  GABpbx should stop accepting new calls
			...
			if (!ast_str_to_eid(&tmp_eid, v->value)) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Invalid Entity ID '%s' provided\n", v->value);
			}
