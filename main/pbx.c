 ...
 * https://issues.gabpbx.org/view.php?id=17643
 */
...
int check_contexts(char *file, int line )
{
	...
	if (!contexts_table) {
		ast_log_chan(NULL, LOG_NOTICE,"Called from: %s:%d: No contexts_table!\n", file, line);
		usleep(500000);
	}
	...
	while( (c1 = ast_hashtab_next(t1))) {
		...
		if (!found) {
			ast_log_chan(NULL, LOG_NOTICE,"Called from: %s:%d: Could not find the %s context in the linked list\n", file, line, c1->name);
	...
	for(c2=contexts;c2;c2=c2->next) {
		...
		if (!c1) {
			ast_log_chan(NULL, LOG_NOTICE,"Called from: %s:%d: Could not find the %s context in the hashtab\n", file, line, c2->name);
			check_contexts_trouble();
		} else
			ast_unlock_contexts();
	}
	...
	for(c2=contexts;c2;c2=c2->next) {
		...
		if (c1) {
			...
			for(e1 = c1->root; e1; e1=e1->next)
			{
				...
				if (!e2) {
					if (e1->matchcid == AST_EXT_MATCHCID_ON) {
						ast_log_chan(NULL, LOG_NOTICE, "Called from: %s:%d: The %s context records "
			...
			if (!c2->root_table) {
				if (c2->root) {
					ast_log_chan(NULL, LOG_NOTICE,"Called from: %s:%d: No c2->root_table for context %s!\n", file, line, c2->name);
					usleep(500000);
				}
			} else {
				...
				while( (e2 = ast_hashtab_next(t1)) ) {
					...
					if (!found) {
						ast_log_chan(NULL, LOG_NOTICE,"Called from: %s:%d: The %s context records the exten %s but it is not in its root_table\n", file, line, c2->name, e2->exten);
						check_contexts_trouble();
					}
		...
		for(e1 = c2->root; e1; e1 = e1->next) {
			...
			for(e2=e1;e2;e2=e2->peer) {
				...
				if (e2 != e1 && e2->peer_table) {
					ast_log_chan(NULL, LOG_NOTICE,"Called from: %s:%d: The %s context, %s exten, %d priority has a peer_table entry, and shouldn't!\n", file, line, c2->name, e1->exten, e2->priority );
					check_contexts_trouble();
				}

				if (e2 != e1 && e2->peer_label_table) {
					ast_log_chan(NULL, LOG_NOTICE,"Called from: %s:%d: The %s context, %s exten, %d priority has a peer_label_table entry, and shouldn't!\n", file, line, c2->name, e1->exten, e2->priority );
					check_contexts_trouble();
				}

				if (e2 == e1 && !e2->peer_table){
					ast_log_chan(NULL, LOG_NOTICE,"Called from: %s:%d: The %s context, %s exten, %d priority doesn't have a peer_table!\n", file, line, c2->name, e1->exten, e2->priority );
					check_contexts_trouble();
				}

				if (e2 == e1 && !e2->peer_label_table) {
					ast_log_chan(NULL, LOG_NOTICE,"Called from: %s:%d: The %s context, %s exten, %d priority doesn't have a peer_label_table!\n", file, line, c2->name, e1->exten, e2->priority );
					check_contexts_trouble();
				}
				...
				if (!e3) {
					ast_log_chan(NULL, LOG_NOTICE,"Called from: %s:%d: The %s context, %s exten, %d priority is not reflected in the peer_table\n", file, line, c2->name, e1->exten, e2->priority );
					check_contexts_trouble();
				}
			}

			if (!e1->peer_table){
				ast_log_chan(NULL, LOG_NOTICE,"Called from: %s:%d: No e1->peer_table!\n", file, line);
				usleep(500000);
			}
			...
			while( (e2 = ast_hashtab_next(t1)) ) {
				...
				if (!found) {
					ast_log_chan(NULL, LOG_NOTICE,"Called from: %s:%d: The %s context, %s exten, %d priority is not reflected in the peer list\n", file, line, c2->name, e1->exten, e2->priority );
					check_contexts_trouble();
				}
			}
			ast_hashtab_end_traversal(t1);
...
static void update_scoreboard(struct scoreboard *board, int length, int spec, struct ast_exten *exten, char last, const char *callerid, int deleted, struct match_char *node)
{
	...
	ast_log_chan(NULL, LOG_NOTICE,"Scoreboarding (LONGER) %s, len=%d, score=%d\n", exten->exten, length, spec);
#endif
}
...
static struct ast_exten *get_canmatch_exten(struct match_char *node)
{
	...
	for (node2 = node; node2; node2 = node2->next_char) {
		if (node2->exten) {
#ifdef NEED_DEBUG_HERE
			ast_log_chan(NULL, LOG_NOTICE,"CanMatch_exten returns exten %s(%p)\n", node2->exten->exten, node2->exten);
#endif
			return node2->exten;
		}
	}
#ifdef NEED_DEBUG_HERE
	ast_log_chan(NULL, LOG_NOTICE,"CanMatch_exten returns NULL, match_char=%s\n", node->x);
#endif
	return 0;
}
...
#ifdef DEBUG_THIS
	if (tree)
		ast_log_chan(NULL, LOG_NOTICE,"new_find_extension called with %s on (sub)tree %s action=%s\n", str, tree->x, action2str(action));
	else
		ast_log_chan(NULL, LOG_NOTICE,"new_find_extension called with %s on (sub)tree NULL action=%s\n", str, action2str(action));
#endif
	...
	while (*src) {
		if (*src == '[' && pattern) {
			...
			*dst = '\0';
			...
			if (!length) {
				ast_log_chan(NULL, LOG_WARNING, "Empty character set in exten pattern '%s'. Ignoring.\n",
					extenbuf);
				node->buf[0] = '\0';
				continue;
			}
	...
	for (; pat_node[idx_cur].buf[0]; idx_cur = idx_next) {
		...
		if (already && (m2 = already_in_tree(m1, pat_node[idx_cur].buf, pattern))
			...
			if (!pat_node[idx_next].buf[0]) {
				...
				if (m2->exten) {
					ast_log_chan(NULL, LOG_WARNING, "Found duplicate exten. Had %s found %s\n",
...
static void create_match_char_tree(struct ast_context *con)
{
	...
	while ((e1 = ast_hashtab_next(t1))) {
		if (e1->exten) {
			...
			ast_log_chan(NULL, LOG_ERROR, "Attempt to create extension with no extension name.\n");
		}
	}
	ast_hashtab_end_traversal(t1);
}
...
#define BITS_PER	8	/* Number of bits per unit (byte). */
	...
	do {
		...
		if (!end) {
			ast_log_chan(NULL, LOG_WARNING, "Wrong usage of [] in the extension\n");
...
static int _extension_match_core(const char *pattern, const char *data, enum ext_match_t mode)
{
...
#ifdef NEED_DEBUG_HERE
	ast_log_chan(NULL, LOG_NOTICE,"match core: pat: '%s', dat: '%s', mode=%d\n", pattern, data, (int)mode);
#endif
	...
	if (pattern[0] != '_') { /* not a pattern, try exact or partial match */
...
#ifdef NEED_DEBUG_HERE
			ast_log_chan(NULL, LOG_NOTICE,"return (0) - pattern too short, cannot match\n");
#endif
			return 0;
		}
		...
		if (mode == E_MATCH) {
#ifdef NEED_DEBUG_HERE
			ast_log_chan(NULL, LOG_NOTICE,"return (!ext_cmp_exten(%s,%s) when mode== E_MATCH)\n", pattern, data);
#endif
			return !ext_cmp_exten(pattern, data); /* 1 on match, 0 on fail */
		}
		if (ld == 0 || !ext_cmp_exten_partial(pattern, data)) { /* partial or full match */
#ifdef NEED_DEBUG_HERE
			ast_log_chan(NULL, LOG_NOTICE,"return (mode(%d) == E_MATCHMORE ? lp(%d) > ld(%d) : 1)\n", mode, lp, ld);
#endif
			return (mode == E_MATCHMORE) ? lp > ld : 1; /* XXX should consider '!' and '/' ? */
		} else {
#ifdef NEED_DEBUG_HERE
			ast_log_chan(NULL, LOG_NOTICE,"return (0) when ld(%d) > 0 && pattern(%s) != data(%s)\n", ld, pattern, data);
#endif
			return 0;
		}
	}
	if (mode == E_MATCH && data[0] == '_') {
		 ...
		 */
#ifdef NEED_DEBUG_HERE
		ast_log_chan(NULL, LOG_NOTICE, "Comparing as patterns first. pattern:%s data:%s\n", pattern, data);
#endif
		if (!ext_cmp_pattern(pattern + 1, data + 1)) {
#ifdef NEED_DEBUG_HERE
			ast_log_chan(NULL, LOG_NOTICE,"return (1) - pattern matches pattern\n");
#endif
			return 1;
		}
	}
	...
	for (;;) {
		...
		case '[':	/* a range */
			...
			if (pattern >= end) {
#ifdef NEED_DEBUG_HERE
				ast_log_chan(NULL, LOG_NOTICE,"return (0) when pattern>=end\n");
#endif
				return 0;
			}
		...
		case 'N':
			if (*data < '2' || *data > '9') {
#ifdef NEED_DEBUG_HERE
				ast_log_chan(NULL, LOG_NOTICE,"return (0) N is not matched\n");
#endif
				return 0;
			}
			break;
		...
		case 'X':
			if (*data < '0' || *data > '9') {
#ifdef NEED_DEBUG_HERE
				ast_log_chan(NULL, LOG_NOTICE,"return (0) X is not matched\n");
#endif
				return 0;
			}
			break;
		...
		case 'Z':
			if (*data < '1' || *data > '9') {
#ifdef NEED_DEBUG_HERE
				ast_log_chan(NULL, LOG_NOTICE,"return (0) Z is not matched\n");
#endif
				return 0;
			}
			break;
...
#ifdef NEED_DEBUG_HERE
			ast_log_chan(NULL, LOG_NOTICE, "return (1) when '.' is matched\n");
#endif
			return 1;
		case '!':	/* Early match */
#ifdef NEED_DEBUG_HERE
			ast_log_chan(NULL, LOG_NOTICE, "return (2) when '!' is matched\n");
#endif
			return 2;
		default:
			if (*data != *pattern) {
#ifdef NEED_DEBUG_HERE
				ast_log_chan(NULL, LOG_NOTICE, "return (0) when *data(%c) != *pattern(%c)\n", *data, *pattern);
#endif
				return 0;
			}
			break;
...
#ifdef NEED_DEBUG_HERE
		ast_log_chan(NULL, LOG_NOTICE, "return (0) when data longer than pattern\n");
#endif
		return 0;
	}
...
#ifdef NEED_DEBUG_HERE
		ast_log_chan(NULL, LOG_NOTICE, "at end, return (%d) in 'exact match'\n", (mode==E_MATCHMORE) ? 0 : 1);
#endif
		return (mode == E_MATCHMORE) ? 0 : 1;	/* this is a failure for E_MATCHMORE */
	} else if (*pattern == '!')	{		/* early match */
#ifdef NEED_DEBUG_HERE
		ast_log_chan(NULL, LOG_NOTICE, "at end, return (2) when '!' is matched\n");
#endif
		return 2;
	} else {						/* partial match */
#ifdef NEED_DEBUG_HERE
		ast_log_chan(NULL, LOG_NOTICE, "at end, return (%d) which deps on E_MATCH\n", (mode == E_MATCH) ? 0 : 1);
#endif
...
int ast_extension_close(const char *pattern, const char *data, int needmore)
{
	if (needmore != E_MATCHMORE && needmore != E_CANMATCH)
		ast_log_chan(NULL, LOG_WARNING, "invalid argument %d\n", needmore);
	...
	const char *label, const char *callerid, enum ext_match_t action)
{
...
#ifdef NEED_DEBUG_HERE
	ast_log_chan(chan, LOG_NOTICE, "Looking for cont/ext/prio/label/action = %s/%s/%d/%s/%d\n", context, exten, priority, label, (int) action);
#endif
	...
	if (q->stacklen == 0) {
		...
		ast_log_chan(chan, LOG_WARNING, "Maximum PBX stack (%d) exceeded. Too many includes?\n", AST_PBX_MAX_STACK);
		return NULL;
	}
...
#ifdef NEED_DEBUG
	ast_log_chan(chan, LOG_NOTICE, "The Trie we are searching in:\n");
	log_match_char_tree(tmp->pattern_tree, "::  ");
#endif
	...
	do {
		if (!ast_strlen_zero(overrideswitch)) {
			...
			if (!asw) {
				ast_log_chan(chan, LOG_WARNING, "No such switch '%s'\n", name);
				break;
			}
			...
			if (eval && !(tmpdata = ast_str_thread_get(&switch_data, 512))) {
				ast_log_chan(chan, LOG_WARNING, "Can't evaluate overrideswitch?!\n");
				break;
			} else if (eval) {
	...
	if (extenpatternmatchnew) {
		...
		if (score.last_char == '!' && action == E_MATCHMORE) {
			 ...
			 */
#ifdef NEED_DEBUG_HERE
			ast_log_chan(chan, LOG_NOTICE,"Returning MATCHMORE NULL with exclamation point.\n");
#endif
			return NULL;
		}
...
#ifdef NEED_DEBUG_HERE
			ast_log_chan(chan, LOG_NOTICE,"Returning CANMATCH exten %s\n", score.canmatch_exten->exten);
#endif
		...
		if ((action == E_MATCHMORE || action == E_CANMATCH)  && eroot) {
			if (score.node) {
				...
				if (z) {
#ifdef NEED_DEBUG_HERE
					ast_log_chan(chan, LOG_NOTICE,"Returning CANMATCH/MATCHMORE next_match exten %s\n", z->exten);
#endif
				} else {
					if (score.canmatch_exten) {
#ifdef NEED_DEBUG_HERE
						ast_log_chan(chan, LOG_NOTICE,"Returning CANMATCH/MATCHMORE canmatchmatch exten %s(%p)\n", score.canmatch_exten->exten, score.canmatch_exten);
#endif
						return score.canmatch_exten;
					} else {
#ifdef NEED_DEBUG_HERE
						ast_log_chan(chan, LOG_NOTICE,"Returning CANMATCH/MATCHMORE next_match exten NULL\n");
#endif
					}
				}
				return z;
...
#ifdef NEED_DEBUG_HERE
			ast_log_chan(chan, LOG_NOTICE, "Returning CANMATCH/MATCHMORE NULL (no next_match)\n");
#endif
		...
		if (eroot) {
			...
			if (e) {	/* found a valid match */
				...
				q->foundcontext = context;
#ifdef NEED_DEBUG_HERE
				ast_log_chan(chan, LOG_NOTICE,"Returning complete match of exten %s\n", e->exten);
#endif
				return e;
			}
		}
	...
	for (idx = 0; idx < ast_context_switches_count(tmp); idx++) {
		...
		if (ast_get_switch_eval(sw)) {
			if (!(tmpdata = ast_str_thread_get(&switch_data, 512))) {
				ast_log_chan(chan, LOG_WARNING, "Can't evaluate switch?!\n");
				continue;
			}
...
#ifdef NEED_DEBUG_HERE
				ast_log_chan(chan, LOG_NOTICE,"Returning recursive match of %s\n", e->exten);
#endif
				return e;
			}
			if (q->swo)
  ...
  const char *label, const char *callerid, enum ext_match_t action, int *found, int combined_find_spawn)
{
	...
	if (e) {
		...
		if (matching_action) {
			...
			ast_unlock_contexts();
			if (!app) {
				ast_log_chan(c, LOG_WARNING, "No application '%s' for extension (%s, %s, %d)\n", e->app, context, exten, priority);
				return -1;
			}
			...
			if (VERBOSITY_ATLEAST(3)) {
				ast_verb_chan(c, 3, "Executing [%s@%s:%d] " COLORIZE_FMT "(\"" COLORIZE_FMT "\", \"" COLORIZE_FMT "\") %s\n",
		...
		if (matching_action) {
			...
			if (!q.swo->exec) {
				ast_log_chan(c, LOG_WARNING, "No execution engine for switch %s\n", q.swo->name);
				return -1;
			}
		...
		case STATUS_NO_CONTEXT:
			if (!matching_action && !combined_find_spawn)
				ast_log_chan(c, LOG_NOTICE, "Cannot find extension context '%s'\n", S_OR(context, ""));
			break;
		case STATUS_NO_EXTENSION:
			if (!matching_action && !combined_find_spawn)
				ast_log_chan(c, LOG_NOTICE, "Cannot find extension '%s' in context '%s'\n", exten, S_OR(context, ""));
			break;
		case STATUS_NO_PRIORITY:
			if (!matching_action && !combined_find_spawn)
				ast_log_chan(c, LOG_NOTICE, "No such priority %d in extension '%s' in context '%s'\n", priority, exten, S_OR(context, ""));
			break;
		case STATUS_NO_LABEL:
			if (context && !combined_find_spawn)
				ast_log_chan(c, LOG_NOTICE, "No such label '%s' in extension '%s' in context '%s'\n", S_OR(label, ""), exten, S_OR(context, ""));
			break;
		default:
...
/*!
 * \brief Send extension state update via UDP to Redis daemon
 */
static void send_state_to_redis_daemon(const char *context, const char *exten, 
                                     int state, const char *hint_app)
{
    struct sockaddr_in server_addr;
    int sockfd;
    char message[512];
    const char *daemon_host = ast_config_AST_EXTENSION_STATE_HOST;
    int daemon_port = atoi(ast_config_AST_EXTENSION_STATE_PORT);
    
    // Crear el socket UDP
    sockfd = socket(AF_INET, SOCK_DGRAM, 0);
    if (sockfd < 0) {
        ast_log(AST_LOG_WARNING, "Failed to create UDP socket for Redis daemon\n");
        return;
    }
    
    // Configurar la dirección del demonio
    memset(&server_addr, 0, sizeof(server_addr));
    server_addr.sin_family = AF_INET;
    server_addr.sin_port = htons(daemon_port);
    server_addr.sin_addr.s_addr = inet_addr(daemon_host);
    
    // Crear el mensaje JSON
    snprintf(message, sizeof(message),
             "{\"root\":\"%i\",\"context\":\"%s\",\"exten\":\"%s\",\"state\":%d,\"state_text\":\"%s\",\"hint\":\"%s\",\"timestamp\":%ld}",
             *ast_config_AST_SYSTEM_ROOT ? *ast_config_AST_SYSTEM_ROOT : 0,
             context ? context : "",
             exten ? exten : "",
             state,
    ...
    if (sendto(sockfd, message, strlen(message), 0, 
        ...
        ast_log(AST_LOG_WARNING, "Failed to send state update to Redis daemon: %s\n", 
                strerror(errno));
    } else {
        ast_debug(3, "Sent state update to Redis daemon: %s\n", message);
    }
...
static void device_state_notify_callbacks(struct ast_hint *hint, struct ast_str **hint_app)
{
	...
	/* Send message to redis by UDP */
	send_state_to_redis_daemon(context_name, exten_name, state, 
                              hint_app ? ast_str_buffer(*hint_app) : NULL);
...
static int ast_add_hint(struct ast_exten *e)
{
	...
	if (add_hintdevice(hint_new, ast_get_extension_app(e))) {
		ast_log_chan(NULL, LOG_WARNING, "Could not add devices for hint: %s@%s.\n",
...
void ast_pbx_h_exten_run(struct ast_channel *chan, const char *context)
{
	...
	if (found && spawn_error) {
		...
		ast_verb_chan(chan, 2, "Spawn extension (%s, %s, %d) exited non-zero on '%s'\n",
			ast_channel_context(chan), ast_channel_exten(chan),
			ast_channel_priority(chan), ast_channel_name(chan));
	}
		...
		struct ast_pbx_args *args)
{
	...
	if (ast_channel_pbx(c)) {
		ast_log_chan(c, LOG_WARNING, "%s already has PBX structure??\n", ast_channel_name(c));
		/* XXX and now what ? */
	...
	for (;;) {
		...
		if (found && res) {
			...
			if (strchr("0123456789ABCDEF*#", res)) {
				...
				ast_verb_chan(c, 2, "Spawn extension (%s, %s, %d) exited INCOMPLETE on '%s'\n", ast_channel_context(c), ast_channel_exten(c), ast_channel_priority(c), ast_channel_name(c));
				...
				ast_debug(1, "Spawn extension (%s,%s,%d) exited non-zero on '%s'\n", ast_channel_context(c), ast_channel_exten(c), ast_channel_priority(c), ast_channel_name(c));
				ast_verb_chan(c, 2, "Spawn extension (%s, %s, %d) exited non-zero on '%s'\n", ast_channel_context(c), ast_channel_exten(c), ast_channel_priority(c), ast_channel_name(c));
				...
				if ((res == AST_PBX_ERROR)
					...
					if (!strcmp(ast_channel_exten(c), "e")) {
						ast_verb_chan(c, 2, "Spawn extension (%s, %s, %d) exited ERROR while already on 'e' exten on '%s'\n", ast_channel_context(c), ast_channel_exten(c), ast_channel_priority(c), ast_channel_name(c));
						error = 1;
					} else {
		...
		if (invalid
			...
			if (ast_exists_extension(c, ast_channel_context(c), "i", 1,
				S_COR(ast_channel_caller(c)->id.number.valid, ast_channel_caller(c)->id.number.str, NULL))) {
				ast_verb_chan(c, 3, "Channel '%s' sent to invalid extension: context,exten,priority=%s,%s,%d\n",
					ast_channel_name(c), ast_channel_context(c), ast_channel_exten(c), ast_channel_priority(c));
				...
				ast_log_chan(c, LOG_WARNING, "Channel '%s' sent to invalid extension but no invalid handler: context,exten,priority=%s,%s,%d\n",
					ast_channel_name(c), ast_channel_context(c), ast_channel_exten(c), ast_channel_priority(c));
			...
			if (!waittime) {
				...
				if (!status)
					status = "UNKNOWN";
				ast_verb_chan(c, 3, "Auto fallthrough, channel '%s' status is '%s'\n", ast_channel_name(c), status);
			...
			if (!timeout
				...
				if (!timeout && !ast_strlen_zero(dst_exten)) {
					...
					if (ast_exists_extension(c, ast_channel_context(c), "i", 1,
						S_COR(ast_channel_caller(c)->id.number.valid, ast_channel_caller(c)->id.number.str, NULL))) {
						ast_verb_chan(c, 3, "Invalid extension '%s' in context '%s' on %s\n", dst_exten, ast_channel_context(c), ast_channel_name(c));
					...
					if (ast_exists_extension(c, ast_channel_context(c), "t", 1,
						S_COR(ast_channel_caller(c)->id.number.valid, ast_channel_caller(c)->id.number.str, NULL))) {
						ast_verb_chan(c, 3, "Timeout on %s\n", ast_channel_name(c));
						set_ext_pri(c, "t", 1);
					} else if (ast_exists_extension(c, ast_channel_context(c), "e", 1,
						S_COR(ast_channel_caller(c)->id.number.valid, ast_channel_caller(c)->id.number.str, NULL))) {
	...
	if (!found && !error) {
		ast_log_chan(c, LOG_WARNING, "Don't know what to do with '%s'\n", ast_channel_name(c));
	}
...
static int increase_call_count(const struct ast_channel *c)
{
	...
	if (ast_option_maxcalls) {
		if (countcalls >= ast_option_maxcalls) {
			ast_log_chan((struct ast_channel*) c, LOG_WARNING, "Maximum call limit of %d calls exceeded by '%s'!\n", ast_option_maxcalls, ast_channel_name(c));
			failed = -1;
		}
	}
	if (ast_option_maxload) {
		...
		if (curloadavg >= ast_option_maxload) {
			ast_log_chan((struct ast_channel*) c, LOG_WARNING, "Maximum loadavg limit of %f load exceeded by '%s' (currently %f)!\n", ast_option_maxload, ast_channel_name(c), curloadavg);
			failed = -1;
		}
	}
#if defined(HAVE_SYSINFO)
	if (option_minmemfree) {
		...
		if (!sysinfo(&sys_info)) {
			...
			if (curfreemem < option_minmemfree) {
				ast_log_chan((struct ast_channel*) c, LOG_WARNING, "Available system memory (~%" PRIu64 "MB) is below the configured low watermark (%ldMB)\n",
...
enum ast_pbx_result ast_pbx_start(struct ast_channel *c)
{
	...
	if (!c) {
		ast_log_chan(c, LOG_WARNING, "Asked to start thread on NULL channel?\n");
		return AST_PBX_FAILED;
	}

	if (!ast_test_flag(&ast_options, AST_OPT_FLAG_FULLY_BOOTED)) {
		ast_log_chan(c, LOG_WARNING, "PBX requires GABpbx to be fully booted\n");
		return AST_PBX_FAILED;
	}
	...
	if (ast_pthread_create_detached(&t, NULL, pbx_thread, c)) {
		ast_log_chan(c, LOG_WARNING, "Failed to create new channel thread\n");
		decrease_call_count();
		return AST_PBX_FAILED;
	}
...
int ast_context_remove_include2(struct ast_context *con, const char *include, const char *registrar)
{
	...
	for (idx = 0; idx < ast_context_includes_count(con); idx++) {
		...
		if (!strcmp(ast_get_include_name(i), include) &&
			...
			/* remove from list */
			ast_verb_chan(NULL, 3, "Removing inclusion of context '%s' in context '%s; registrar=%s'\n", include, ast_get_context_name(con), registrar);
...
int ast_context_remove_switch2(struct ast_context *con, const char *sw, const char *data, const char *registrar)
{
	...
	for (idx = 0; idx < ast_context_switches_count(con); idx++) {
		...
		if (!strcmp(ast_get_switch_name(i), sw) &&
			...
			ast_verb_chan(NULL, 3, "Removing switch '%s' from context '%s; registrar=%s'\n", sw, ast_get_context_name(con), registrar);
...
int ast_context_remove_extension_callerid2(struct ast_context *con, const char *extension, int priority, const char *callerid, int matchcallerid, const char *registrar, int already_locked)
{
...
#ifdef NEED_DEBUG
	ast_verb_chan(NULL, 3,"Removing %s/%s/%d%s%s from trees, registrar=%s\n", con->name, extension, priority, matchcallerid ? "/" : "", matchcallerid ? callerid : "", registrar);
#endif
#ifdef CONTEXT_DEBUG
	...
	if (exten) {
		if (priority == 0) {
			...
			if (!exten2)
				ast_log_chan(NULL, LOG_ERROR,"Trying to delete the exten %s from context %s, but could not remove from the root_table\n", extension, con->name);
			if (con->pattern_tree) {
				...
				if (x->exten) { /* this test for safety purposes */
					...
					ast_log_chan(NULL, LOG_WARNING,"Trying to delete an exten from a context, but the pattern tree node returned isn't a full extension\n");
				}
			}
		} else {
			...
			if (exten2) {
				if (exten2->label) { /* if this exten has a label, remove that, too */
					...
					if (!exten3) {
						ast_log_chan(NULL, LOG_ERROR, "Did not remove this priority label (%d/%s) "
				...
				if (!exten3) {
					ast_log_chan(NULL, LOG_ERROR, "Did not remove this priority (%d) from the "
				...
				if (ast_hashtab_size(exten->peer_table) == 0) {
					...
					if (!exten3) {
						ast_log_chan(NULL, LOG_ERROR, "Did not remove this exten (%s) from the "
		...
		ast_log_chan(NULL, LOG_WARNING,"Cannot find extension %s in root_table in context %s\n",
				extension, con->name);
	}
#ifdef NEED_DEBUG
	if (con->pattern_tree) {
		ast_log_chan(NULL, LOG_NOTICE,"match char tree after exten removal:\n");
...
static char *handle_show_hints(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	/* ... we have hints ... */
	ast_cli(a->fd, "\n    -= Registered GABpbx Dial Plan Hints =-\n");
...
static char *handle_eat_memory(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	if (!blocks) {
		ast_log_chan(NULL, LOG_ERROR, "Already out of mem?\n");
		return CLI_SUCCESS;
	}

	for (i = 0; sizes[i]; ++i) {
		int alloc_size = sizes[i];
		ast_log_chan(NULL, LOG_WARNING, "Allocating %d sized blocks (got %d blocks already)\n", alloc_size, blocks_pos);
		while (1) {
			void *block;
			if (blocks_pos >= blocks_max) {
				ast_log_chan(NULL, LOG_ERROR, "Memory buffer too small? Run me again :)\n");
				break;
			}
	...
	ast_log_chan(NULL, LOG_WARNING, "Allocated %lld bytes total!\n", allocated);
	return CLI_SUCCESS;
}
#endif
		...
		for (idx = 0; idx < ast_context_includes_count(c); idx++) {
			...
			if (exten) {
				...
				if (includecount >= AST_PBX_MAX_STACK) {
					ast_log_chan(NULL, LOG_WARNING, "Maximum include depth exceeded!\n");
				} else {
					int dupe = 0;
					int x;
					...
					if (!dupe) {
						...
						ast_log_chan(NULL, LOG_WARNING, "Avoiding circular include of %s within %s\n", ast_get_include_name(i), context);
					}
				}
			} else {
					...
					int includecount, const char *includes[])
{
	...
	if (ast_rdlock_contexts()) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Failed to lock contexts list for manager: listdialplan\n");
		return -1;
	}
...
struct ast_context *ast_context_find_or_create(struct ast_context **extcontexts, struct ast_hashtab *exttable, const char *name, const char *registrar)
{
	...
	if ((tmp = ast_calloc(1, length))) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Danger! We failed to allocate a context for %s!\n", name);
		if (!extcontexts) {
			ast_unlock_contexts();
		}
		return NULL;
...
static void context_table_create_autohints(struct ast_hashtab *table)
{
	...
	while ((con = ast_hashtab_next(iter))) {
		...
		ast_verb_chan(NULL, 3, "Enabled autohints support on context '%s'\n", con->name);
	}
	...
	if (context->root_table) {
		...
		while ((exten_item=ast_hashtab_next(exten_iter))) {
			...
			while ((prio_item=ast_hashtab_next(prio_iter))) {
				...
				if (!new) {
					ast_log_chan(NULL, LOG_ERROR,"Could not allocate a new context for %s in merge_and_delete! Danger!\n", context->name);
				...
				if (!res1 && new_exten_item && new_prio_item){
					ast_verb_chan(NULL, 3,"Dropping old dialplan item %s/%s/%d [%s(%s)] (registrar=%s) due to conflict with new dialplan\n",
...
void ast_merge_contexts_and_delete(struct ast_context **extcontexts, struct ast_hashtab *exttable, const char *registrar)
{
	 ...
	 * while the rest of gabpbx is now freely using the new stuff
	 * instead.
	 */
	...
	ft /= 1000000.0;
	ast_verb_chan(NULL, 5,"Time to scan old dialplan and merge leftovers back into the new: %8.6f sec\n", ft);

	ft = ast_tvdiff_us(endlocktime, writelocktime);
	ft /= 1000000.0;
	ast_verb_chan(NULL, 5,"Time to restore hints and swap in new dialplan: %8.6f sec\n", ft);

	ft = ast_tvdiff_us(enddeltime, endlocktime);
	ft /= 1000000.0;
	ast_verb_chan(NULL, 5,"Time to delete the old dialplan: %8.6f sec\n", ft);

	ft = ast_tvdiff_us(enddeltime, begintime);
	ft /= 1000000.0;
	ast_verb_chan(NULL, 5,"Total time merge_contexts_delete: %8.6f sec\n", ft);
	ast_verb_chan(NULL, 5, "%s successfully loaded %d contexts (enable debug for details).\n", registrar, ctx_count);
}
	...
	const char *data, int eval, const char *registrar)
{
	...
	ast_verb_chan(NULL, 3, "Including switch '%s/%s' in context '%s'\n",
...
int ast_async_goto(struct ast_channel *chan, const char *context, const char *exten, int priority)
{
	...
	if (!newchan) {
		ast_log_chan(chan, LOG_WARNING, "Unable to gain control of channel %s\n", ast_channel_name(chan));
		return -1;
	}
	ast_explicit_goto(newchan, context, exten, priority);
	if (ast_pbx_start(newchan)) {
		ast_hangup(newchan);
		ast_log_chan(chan, LOG_WARNING, "Unable to start PBX on %s\n", ast_channel_name(newchan));
		return -1;
	}
		...
		if (ep)	{		/* We're in the peer list, just insert ourselves */
			...
			if (x) { /* if the trie isn't formed yet, don't sweat this */
				if (x->exten) { /* this test for safety purposes */
					...
					ast_log_chan(NULL, LOG_ERROR,"Trying to delete an exten from a context, but the pattern tree node returned isn't an extension\n");
				}
			}
	...
	const char *registrar, const char *registrar_file, int registrar_line, int lock_context)
{
	...
	if (ast_strlen_zero(extension)) {
		ast_log_chan(NULL, LOG_ERROR,"You have to be kidding-- add exten '' to context %s? Figure out a name and call me back. Action ignored.\n",
				con->name);
	...
	if (DEBUG_ATLEAST(1)) {
		if (tmp->matchcid == AST_EXT_MATCHCID_ON) {
			ast_log_chan(NULL, LOG_DEBUG, "Added extension '%s' priority %d (CID match '%s') to %s (%p)\n",
				tmp->name, tmp->priority, tmp->cidmatch_display, con->name, con);
		} else {
			ast_log_chan(NULL, LOG_DEBUG, "Added extension '%s' priority %d to %s (%p)\n",
...
static void *pbx_outgoing_exec(void *data)
{
	...
	if (!ast_strlen_zero(outgoing->app)) {
		...
		if (app) {
			ast_verb_chan(chan, 4, "Launching %s(%s) on %s\n", outgoing->app, S_OR(outgoing->appdata, ""),
				ast_channel_name(chan));
			pbx_exec(chan, app, outgoing->appdata);
		} else {
			ast_log_chan(chan, LOG_WARNING, "No such application '%s'\n", outgoing->app);
		}
		...
		if (ast_pbx_run(chan)) {
			ast_log_chan(chan, LOG_ERROR, "Failed to start PBX on %s\n", ast_channel_name(chan));
			ast_hangup(chan);
		}
	}
...
static void pbx_outgoing_state_callback(struct ast_dial *dial)
{
	...
	ast_verb_chan(channel, 4, "Treating progress as answer on '%s' due to early media option\n",
	...
	const struct ast_assigned_ids *assignedids, const char *predial_callee)
{
	...
	if (synchronous == AST_OUTGOING_WAIT_COMPLETE) {
		...
		if (ast_pthread_create_detached(&thread, NULL, pbx_outgoing_exec, outgoing)) {
			ast_log_chan(dialed, LOG_WARNING, "Unable to spawn dialing thread for '%s/%s'\n", type, addr);
			ao2_ref(outgoing, -1);
			if (locked_channel) {
				if (!synchronous) {
					ast_channel_unlock(dialed);
	...
	if (res < 0 /* Call failed to get connected for some reason. */
		...
		if (failed) {
			...
			if (ast_pbx_run(failed)) {
				ast_log_chan(failed, LOG_ERROR, "Unable to run PBX on '%s'\n",
...
void __ast_context_destroy(struct ast_context *list, struct ast_hashtab *contexttab, struct ast_context *con, const char *registrar)
{
	...
	for (tmp = list; tmp; ) {
		...
		if (registrar) {
			...
			if (tmp->root_table) { /* it is entirely possible that the context is EMPTY */
				...
				while ((exten_item=ast_hashtab_next(exten_iter))) {
					...
					while ((prio_item=ast_hashtab_next(prio_iter))) {
						...
						ast_verb_chan(NULL, 5, "Remove %s/%s/%d, registrar=%s; con=%s(%p); con->root=%p\n",
			...
			ast_verb_chan(NULL, 3, "Deleting context %s registrar=%s\n", tmp->name, tmp->registrar);
			ast_debug(1, "delete ctx %s %s\n", tmp->name, tmp->registrar);
			ast_hashtab_remove_this_object(contexttab, tmp);
...
/*!
 * \internal
 * \brief Clean up resources on GABpbx shutdown.
 *
...
int load_pbx(void)
{
	...
	/* Initialize the PBX */
	ast_verb_chan(NULL, 1, "GABpbx PBX Core Initializing\n");

	ast_verb_chan(NULL, 5, "Registering builtin functions:\n");
...
int ast_context_verify_includes(struct ast_context *con)
{
	...
	if (includecount >= AST_PBX_MAX_STACK) {
		ast_log_chan(NULL, LOG_WARNING, "Context %s contains too many includes (%d). Maximum is %d.\n",
	...
	for (idx = 0; idx < includecount; idx++) {
		...
		res = -1;
		ast_log_chan(NULL, LOG_WARNING, "Context '%s' tries to include nonexistent context '%s'\n",
...
int pbx_parse_location(struct ast_channel *chan, char **contextp, char **extenp, char **prip, int *ipri, int *mode, char *rest)
{
	...
	if ((rest && sscanf(pri, "%30d%1s", ipri, rest) != 1) || sscanf(pri, "%30d", ipri) != 1) {
		*ipri = ast_findlabel_extension(chan, context ? context : ast_channel_context(chan),
			...
			S_COR(ast_channel_caller(chan)->id.number.valid, ast_channel_caller(chan)->id.number.str, NULL));
		if (*ipri < 1) {
			ast_log_chan(chan, LOG_WARNING, "Priority '%s' must be a number > 0, or valid label\n", pri);
			return -1;
		} else if (mode) {
			*mode = 0;
		}
...
static int pbx_parseable_goto(struct ast_channel *chan, const char *goto_string, int async)
{
	...
	if (ast_strlen_zero(goto_string)) {
		ast_log_chan(chan, LOG_WARNING, "Goto requires an argument ([[context,]extension,]priority)\n");
		return -1;
	}
