...
int ast_register_application2(const char *app, int (*execute)(struct ast_channel *, const char *), const char *synopsis, const char *description, void *mod)
{
	...
	if (cur) {
		ast_log_chan(NULL, LOG_WARNING, "Already have an application '%s'\n", app);
		AST_RWLIST_UNLOCK(&apps);
		return -1;
	}
	...
	ast_verb_chan(NULL, 5, "Registered application '" COLORIZE_FMT "'\n", COLORIZE(COLOR_BRCYAN, 0, tmp->name));
...
static char *handle_show_applications(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	if ((!like) && (!describing)) {
		ast_cli(a->fd, "    -= Registered GABpbx Applications =-\n");
	} else {
		ast_cli(a->fd, "    -= Matching GABpbx Applications =-\n");
	}
...
int ast_unregister_application(const char *app)
{
	...
	AST_RWLIST_TRAVERSE_SAFE_BEGIN(&apps, cur, list) {
		...
		if (!cmp) {
			...
			ast_verb_chan(NULL, 5, "Unregistered application '%s'\n", cur->name);
...
int ast_pbx_exec_application(struct ast_channel *chan, const char *app_name, const char *app_args)
{
	...
	if (!app) {
		ast_log_chan(chan, LOG_WARNING, "Could not find application (%s)\n", app_name);
	} else {
		...
		if (!ast_strlen_zero(app_args) && (substituted_args = ast_str_create(16))) {
			...
			if (!ast_strlen_zero(app_args)) {
				ast_log_chan(chan, LOG_WARNING, "Could not substitute application argument variables for %s\n", app_name);
			}
