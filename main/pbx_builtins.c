...
/*** DOCUMENTATION
	<application name="Answer" language="en_US">
		...
		<syntax>
			<parameter name="delay">
				<para>GABpbx will wait this number of milliseconds before returning to
		...
		<description>
			...
			<para>By default, GABpbx will wait for media for up to 500 ms, or
	...
	<application name="Goto" language="en_US">
		...
		<description>
			...
			<para>If the location that is put into the channel information is bogus, and gabpbx cannot
	...
	<application name="NoOp" language="en_US">
		...
		<syntax>
			<parameter name="text">
				<para>Any text provided can be viewed at the GABpbx CLI.</para>
			</parameter>
		</syntax>
		<description>
	...
	if (!ast_strlen_zero(data)) {
		...
		if (cause <= 0) {
			if (sscanf(data, "%30d", &cause) != 1 || cause <= 0) {
				ast_log_chan(chan, LOG_WARNING, "Invalid cause given to Hangup(): \"%s\"\n", data);
				cause = 0;
			}
		}
	} else {
...
static int pbx_builtin_gotoiftime(struct ast_channel *chan, const char *data)
{
	...
	if (!chan) {
		ast_log_chan(chan, LOG_WARNING, "GotoIfTime requires a channel on which to operate\n");
		return -1;
	}

	if (ast_strlen_zero(data)) {
		ast_log_chan(chan, LOG_WARNING, "GotoIfTime requires an argument:\n  <time range>,<days of week>,<days of month>,<months>[,<timezone>]?'labeliftrue':'labeliffalse'\n");
		return -1;
	}
	...
	if ((ctime = pbx_builtin_getvar_helper(chan, "TESTTIME")) && sscanf(ctime, "%ld", &timesecs) == 1) {
		...
		ast_log_chan(chan, LOG_WARNING, "Using current time to evaluate\n");
...
static int pbx_builtin_execiftime(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(chan, LOG_WARNING, "%s\n", usage);
		return -1;
	}
	...
	if (!ast_build_timing(&timing, s)) {
		ast_log_chan(chan, LOG_WARNING, "Invalid Time Spec: %s\nCorrect usage: %s\n", s, usage);
		ast_destroy_timing(&timing);
		return -1;
	}
		...
		*s++ = '\0';
		if ((e = strrchr(s, ')')))
			*e = '\0';
			...
			ast_log_chan(chan, LOG_WARNING, "Failed to find closing parenthesis\n");
	}
...
static int pbx_builtin_waitexten(struct ast_channel *chan, const char *data)
{
	...
	if (ast_test_flag(&flags, WAITEXTEN_MOH) && !opts[0] ) {
		ast_log_chan(chan, LOG_WARNING, "The 'm' option has been specified for WaitExten without a class.\n");
	...
	if (!res) {
		if (ast_check_hangup(chan)) {
			...
			ast_verb_chan(chan, 3, "Timeout on %s, continuing...\n", ast_channel_name(chan));
		} else if (ast_exists_extension(chan, ast_channel_context(chan), "t", 1,
			S_COR(ast_channel_caller(chan)->id.number.valid, ast_channel_caller(chan)->id.number.str, NULL))) {
			ast_verb_chan(chan, 3, "Timeout on %s, going to 't'\n", ast_channel_name(chan));
			...
			ast_log_chan(chan, LOG_WARNING, "Timeout but no rule 't' or 'e' in context '%s'\n",
...
static int pbx_builtin_background(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(chan, LOG_WARNING, "Background requires an argument (filename)\n");
		return -1;
	}
	...
	if (!res) {
		...
		while (!res && (front = ast_strsep(&back, '&', AST_STRSEP_STRIP | AST_STRSEP_TRIM))) {
			if ( (res = ast_streamfile(chan, front, args.lang)) ) {
				ast_log_chan(chan, LOG_WARNING, "ast_streamfile failed on %s for %s\n", ast_channel_name(chan), (char*)data);
				res = 0;
				mres = 1;
				break;
			}
...
static int pbx_builtin_gotoif(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(chan, LOG_WARNING, "Ignoring, since there is no variable to check\n");
		return 0;
	}
...
static int pbx_builtin_saynumber(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(chan, LOG_WARNING, "SayNumber requires an argument (number)\n");
		return -1;
	}
	...
	if (ast_str_to_int(tmp, &number_val)) {
		ast_log_chan(chan, LOG_WARNING, "argument '%s' to SayNumber could not be parsed as a number.\n", tmp);
		return 0;
	}
	...
	if (options) {
		if ( strcasecmp(options, "f") && strcasecmp(options, "m") &&
			...
			ast_log_chan(chan, LOG_WARNING, "SayNumber gender option is either 'f', 'm', 'c' or 'n'\n");
			return -1;
		}
	}
	...
	if (res < 0 && !ast_check_hangup_locked(chan)) {
		ast_log_chan(chan, LOG_WARNING, "We were unable to say the number %s, is it too large?\n", tmp);
	}
...
static int pbx_builtin_sayordinal(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(chan, LOG_WARNING, "SayOrdinal requires an argument (number)\n");
		return -1;
	}
	ast_copy_string(tmp, data, sizeof(tmp));
	strsep(&number, ",");
	...
	if (ast_str_to_int(tmp, &number_val)) {
		ast_log_chan(chan, LOG_WARNING, "argument '%s' to SayOrdinal could not be parsed as a number.\n", tmp);
		return 0;
	}
	...
	if (options) {
		if ( strcasecmp(options, "f") && strcasecmp(options, "m") &&
			strcasecmp(options, "c") && strcasecmp(options, "n") ) {
			ast_log_chan(chan, LOG_WARNING, "SayOrdinal gender option is either 'f', 'm', 'c' or 'n'\n");
			return -1;
		}
	}
...
static int pbx_builtin_saycharacters_case(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(chan, LOG_WARNING, "SayAlphaCase requires two arguments (options, characters)\n");
		return 0;
	}
	...
	if (!args.options || strlen(args.options) != 1) {
		ast_log_chan(chan, LOG_WARNING, "SayAlphaCase options are mutually exclusive and required\n");
		return 0;
	}
	...
	case 'u':
		...
		break;
	default:
		ast_log_chan(chan, LOG_WARNING, "Invalid option: '%s'\n", args.options);
		return 0;
	}
...
int load_pbx_builtins(void)
{
	...
	for (x = 0; x < ARRAY_LEN(builtins); x++) {
		if (ast_register_application2(builtins[x].name, builtins[x].execute, NULL, NULL, NULL)) {
			ast_log_chan(NULL, LOG_ERROR, "Unable to register builtin application '%s'\n", builtins[x].name);
			return -1;
		}
	}
