...
#endif
		ast_verb_chan(NULL, 5, "Unregistered custom function %s\n", cur->name);
	}
	AST_RWLIST_UNLOCK(&acf_root);
...
int __ast_custom_function_register(struct ast_custom_function *acf, struct ast_module *mod)
{
	...
	if (cur) {
		ast_log_chan(NULL, LOG_ERROR, "Function %s already registered.\n", acf->name);
		AST_RWLIST_UNLOCK(&acf_root);
		return -1;
	}
	...
	ast_verb_chan(NULL, 5, "Registered custom function '" COLORIZE_FMT "'\n", COLORIZE(COLOR_BRCYAN, 0, acf->name));
...
static char *func_args(char *function)
{
	...
	if (!args) {
		ast_log_chan(NULL, LOG_WARNING, "Function '%s' doesn't contain parentheses.  Assuming null argument.\n", function);
	} else {
		char *p;
		*args++ = '\0';
		if ((p = strrchr(args, ')'))) {
			*p = '\0';
			...
			ast_log_chan(NULL, LOG_WARNING, "Can't find trailing parenthesis for function '%s(%s'?\n", function, args);
		}
	}
	return args;
}
...
void pbx_live_dangerously(int new_live_dangerously)
{
	if (new_live_dangerously && !live_dangerously) {
		ast_log_chan(NULL, LOG_WARNING, "Privilege escalation protection disabled!\n"
			"See https://docs.gabpbx.org/Configuration/Dialplan/Privilege-Escalations-with-Dialplan-Functions/ for more details.\n");
	}

	if (!new_live_dangerously && live_dangerously) {
		ast_log_chan(NULL, LOG_NOTICE, "Privilege escalation protection enabled.\n");
	}
...
int ast_thread_inhibit_escalations(void)
{
	...
	if (thread_inhibit_escalations == NULL) {
		ast_log_chan(NULL, LOG_ERROR, "Error inhibiting privilege escalations for current thread\n");
		return -1;
	}
...
int ast_thread_inhibit_escalations_swap(int inhibit)
{
	...
	thread_inhibit_escalations = ast_threadstorage_get(
		&thread_inhibit_escalations_tl, sizeof(*thread_inhibit_escalations));
	if (thread_inhibit_escalations == NULL) {
		ast_log_chan(NULL, LOG_ERROR, "Error swapping privilege escalations inhibit for current thread\n");
		return -1;
	}
...
static int thread_inhibits_escalations(void)
{
	...
	thread_inhibit_escalations = ast_threadstorage_get(
		&thread_inhibit_escalations_tl, sizeof(*thread_inhibit_escalations));
	if (thread_inhibit_escalations == NULL) {
		ast_log_chan(NULL, LOG_ERROR, "Error checking thread's ability to run dangerous functions\n");
...
int ast_func_read(struct ast_channel *chan, const char *function, char *workspace, size_t len)
{
	...
	if (acfptr == NULL) {
		ast_log_chan(chan, LOG_ERROR, "Function %s not registered\n", copy);
	} else if (!acfptr->read && !acfptr->read2) {
		ast_log_chan(chan, LOG_ERROR, "Function %s cannot be read\n", copy);
	} else if (!is_read_allowed(acfptr)) {
		ast_log_chan(chan, LOG_ERROR, "Dangerous function %s read blocked\n", copy);
	} else if (acfptr->read) {
		if (acfptr->mod) {
...
int ast_func_write(struct ast_channel *chan, const char *function, const char *value)
{
	...
	if (acfptr == NULL) {
		...
		ast_log_chan(chan, LOG_ERROR, "Function %s cannot be written to\n", copy);
	} else if (!is_write_allowed(acfptr)) {
		ast_log_chan(chan, LOG_ERROR, "Dangerous function %s write blocked\n", copy);
	} else {
		int res;
		struct ast_module_user *u = NULL;
