...
/*** DOCUMENTATION
	<application name="Set" language="en_US">
		...
		<description>
			...
			<note><para>If (and only if), in <filename>/etc/gabpbx/gabpbx.conf</filename>, you have
	...
	size_t *used, int use_both)
{
	...
	while (!ast_strlen_zero(whereweare)) {
		...
		if (nextvar) {
			...
			len = vare - vars;
			if (brackets) {
				ast_log_chan(c, LOG_WARNING, "Error in extension logic (missing '}')\n");
			} else {
			...
			if (isfunction) {
				...
				if (!c || (c && res < 0 && use_both)) {
					...
					if (bogus) {
						...
						ast_log_chan(c, LOG_ERROR, "Unable to allocate bogus channel for function value substitution.\n");
						res = -1;
					}
...
void pbx_substitute_variables_helper_full_location(struct ast_channel *c, struct varshead *headp, const char *cp1, char *cp2, int count, size_t *used, const char *context, const char *exten, int pri)
{
	...
	*cp2 = 0; /* just in case nothing ends up there */
	...
	if ((*recurse_depth)++ >= MAX_VARIABLE_SUB_RECURSE_DEPTH) {
		ast_log_chan(c, LOG_ERROR, "Exceeded maximum variable substitution recursion depth (%d) - possible infinite recursion in dialplan?\n", MAX_VARIABLE_SUB_RECURSE_DEPTH);
		(*recurse_depth)--;
		return;
	}
		...
		if ((var = ast_var_name(variables)) && (val = ast_var_value(variables))
		   /* && !ast_strlen_zero(var) && !ast_strlen_zero(val) */
			...
			if (ast_str_append(buf, 0, "%s=%s\n", var, val) < 0) {
				ast_log_chan(chan, LOG_ERROR, "Data Buffer Size Exceeded!\n");
				break;
			} else
				total++;
		} else
...
void pbx_builtin_pushvar_helper(struct ast_channel *chan, const char *name, const char *value)
{
	...
	if (name[strlen(name)-1] == ')') {
		...
		ast_log_chan(chan, LOG_WARNING, "Cannot push a value onto a function\n");
	...
	if (value && (newvariable = ast_var_assign(name, value))) {
		if (headp == &globals)
			ast_verb_chan(chan, 2, "Setting global variable '%s' to '%s'\n", name, value);
...
int pbx_builtin_setvar(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(chan, LOG_WARNING, "Set requires one variable name/value pair.\n");
		return 0;
	}
	...
	value = mydata;
	if (!value) {
		ast_log_chan(chan, LOG_WARNING, "Set requires an '=' to be a valid assignment.\n");
		return 0;
	}

	if (strchr(name, ' ')) {
		ast_log_chan(chan, LOG_WARNING, "Please avoid unnecessary spaces on variables as it may lead to unexpected results ('%s' set to '%s').\n", name, mydata);
	}
...
int pbx_builtin_setvar_multiple(struct ast_channel *chan, const char *vdata)
{
	...
	if (ast_strlen_zero(vdata)) {
		ast_log_chan(chan, LOG_WARNING, "MSet requires at least one variable name/value pair.\n");
		return 0;
	}
	...
	for (x = 0; x < args.argc; x++) {
		...
		if (pair.argc == 2) {
			...
			ast_log_chan(chan, LOG_WARNING, "MSet: ignoring entry '%s' with no '='\n", pair.name);
		} else {
			ast_log_chan(chan, LOG_WARNING, "MSet: ignoring entry '%s' with no '=' (in %s@%s:%d\n", pair.name, ast_channel_exten(chan), ast_channel_context(chan), ast_channel_priority(chan));
		}
	}
