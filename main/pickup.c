...
int ast_pickup_call(struct ast_channel *chan)
{
	...
	if (!pickup_cfg) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to retrieve pickup configuration. Unable to play pickup sounds\n");
	}
	...
	if (target) {
		ast_log_chan(NULL, LOG_NOTICE, "Pickup %s attempt by %s\n", ast_channel_name(target), ast_channel_name(chan));
		...
		if (!res) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Pickup %s failed by %s\n", ast_channel_name(target), ast_channel_name(chan));
		}
		target = ast_channel_unref(target);
	}
...
int ast_do_pickup(struct ast_channel *chan, struct ast_channel *target)
{
	...
	if (ast_answer(chan)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to answer '%s'\n", chan_name);
		goto pickup_failed;
	}

	if (ast_queue_control(chan, AST_CONTROL_ANSWER)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to queue answer on '%s'\n", chan_name);
		goto pickup_failed;
	}
	...
	if (ast_channel_move(target, chan)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to complete call pickup of '%s' with '%s'\n",
