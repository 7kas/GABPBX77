		...
		if (next_state == AST_PRESENCE_INVALID) {
			...
			if ((address = strchr(label, '/'))) {
				...
				*address++ = '\0';
				...
				if (!provider) {
					ast_log_chan(NULL, LOG_WARNING, "No provider found for label: %s\n", label);
				}
			} else {
				ast_log_chan(NULL, LOG_WARNING, "No label found for presence state provider: %s\n", label);
			}
		}
...
AST_TEST_DEFINE(test_presence_chan)
{
	...
	if (ast_channel_register(&presence_test_tech)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to register channel type '%s'\n", presence_test_tech.type);
	...
	if (state != presence_test_alice_state) {
		ast_log_chan(NULL, LOG_WARNING, "Presence state of '%s' returned '%s' instead of the expected value '%s'\n",
