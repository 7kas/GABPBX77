...
struct ast_refer {
	...
	/* Whether to refer to GABpbx itself, if refer_to is an GABpbx endpoint. */
	int to_self;
...
int ast_refer_send(struct ast_refer *refer)
{
	...
	if (!refer_tech) {
		ast_log_chan(NULL, LOG_ERROR, "Unknown refer tech: %s\n", tech_name);
...
int ast_refer_tech_register(const struct ast_refer_tech *tech)
{
	...
	if (match) {
		ast_log_chan(NULL, LOG_ERROR, "Refer technology already registered for '%s'\n",
		        tech->name);
		ast_rwlock_unlock(&refer_techs_lock);
		return -1;
	}
	...
	if (AST_VECTOR_APPEND(&refer_techs, tech)) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to register refer technology for '%s'\n",
		        tech->name);
		ast_rwlock_unlock(&refer_techs_lock);
		return -1;
	}
	ast_verb_chan(NULL, 5, "Refer technology '%s' registered.\n", tech->name);
...
int ast_refer_tech_unregister(const struct ast_refer_tech *tech)
{
	...
	if (match) {
		ast_log_chan(NULL, LOG_ERROR, "No '%s' refer technology found.\n", tech->name);
		return -1;
	}

	ast_verb_chan(NULL, 5, "Refer technology '%s' unregistered.\n", tech->name);
