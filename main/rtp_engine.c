...
#include "gabpbx/_private.h"          /* for ast_rtp_engine_init prototype */
#include "gabpbx/astobj2.h"           /* for ao2_cleanup, ao2_ref, etc */
#include "gabpbx/channel.h"           /* for ast_channel_name, etc */
#include "gabpbx/codec.h"             /* for ast_codec_media_type2str, etc */
#include "gabpbx/format.h"            /* for ast_format_cmp, etc */
#include "gabpbx/format_cache.h"      /* for ast_format_adpcm, etc */
#include "gabpbx/format_cap.h"        /* for ast_format_cap_alloc, etc */
#include "gabpbx/json.h"              /* for ast_json_ref, etc */
#include "gabpbx/linkedlists.h"       /* for ast_rtp_engine::<anonymous>, etc */
#include "gabpbx/lock.h"              /* for ast_rwlock_unlock, etc */
#include "gabpbx/logger.h"            /* for ast_log, ast_debug, etc */
#include "gabpbx/manager.h"
#include "gabpbx/module.h"            /* for ast_module_unref, etc */
#include "gabpbx/netsock2.h"          /* for ast_sockaddr_copy, etc */
#include "gabpbx/options.h"           /* for ast_option_rtpptdynamic */
#include "gabpbx/pbx.h"               /* for pbx_builtin_setvar_helper */
#include "gabpbx/res_srtp.h"          /* for ast_srtp_res */
#include "gabpbx/rtp_engine.h"        /* for ast_rtp_codecs, etc */
#include "gabpbx/stasis.h"            /* for stasis_message_data, etc */
#include "gabpbx/stasis_channels.h"   /* for ast_channel_stage_snapshot, etc */
#include "gabpbx/strings.h"           /* for ast_str_append, etc */
#include "gabpbx/time.h"              /* for ast_tvdiff_ms, ast_tvnow */
#include "gabpbx/translate.h"         /* for ast_translate_available_formats */
#include "gabpbx/utils.h"             /* for ast_free, ast_strdup, etc */
#include "gabpbx/vector.h"            /* for AST_VECTOR_GET, etc */
...
static struct ast_rtp_mime_type {
	/*! \brief A mapping object between the GABpbx codec and this RTP payload */
...
/*!
 * \brief Mapping between GABpbx codecs and rtp payload types
 *
	...
	int payload, int rtp_code, int primary_mapping, unsigned int sample_rate)
{
	...
	type->gabpbx_format = type->format != NULL;
...
int ast_rtp_engine_register2(struct ast_rtp_engine *engine, struct ast_module *module)
{
	...
	if (ast_strlen_zero(engine->name) || !engine->new || !engine->destroy || !engine->write || !engine->read) {
		ast_log_chan(NULL, LOG_WARNING, "RTP Engine '%s' failed sanity check so it was not registered.\n", !ast_strlen_zero(engine->name) ? engine->name : "Unknown");
		return -1;
	}
	...
	AST_RWLIST_TRAVERSE(&engines, current_engine, entry) {
		if (!strcmp(current_engine->name, engine->name)) {
			ast_log_chan(NULL, LOG_WARNING, "An RTP engine with the name '%s' has already been registered.\n", engine->name);
	...
	ast_verb_chan(NULL, 5, "Registered RTP engine '%s'\n", engine->name);
...
int ast_rtp_engine_unregister(struct ast_rtp_engine *engine)
{
	...
	if ((current_engine = AST_RWLIST_REMOVE(&engines, engine, entry))) {
		ast_verb_chan(NULL, 5, "Unregistered RTP engine '%s'\n", engine->name);
	}
...
int ast_rtp_glue_register2(struct ast_rtp_glue *glue, struct ast_module *module)
{
	...
	AST_RWLIST_TRAVERSE(&glues, current_glue, entry) {
		if (!strcasecmp(current_glue->type, glue->type)) {
			ast_log_chan(NULL, LOG_WARNING, "RTP glue with the name '%s' has already been registered.\n", glue->type);
	...
	ast_verb_chan(NULL, 5, "Registered RTP glue '%s'\n", glue->type);
...
int ast_rtp_glue_unregister(struct ast_rtp_glue *glue)
{
	...
	if ((current_glue = AST_RWLIST_REMOVE(&glues, glue, entry))) {
		ast_verb_chan(NULL, 5, "Unregistered RTP glue '%s'\n", glue->type);
	}
		...
		void *data)
{
	...
	if (!engine) {
		ast_log_chan(NULL, LOG_ERROR, "No RTP engine was found. Do you have one loaded?\n");
		AST_RWLIST_UNLOCK(&engines);
		return NULL;
	}
...
static void payload_mapping_rx_clear_primary(struct ast_rtp_codecs *codecs, struct ast_rtp_payload_type *to_match)
{
	...
	for (idx = 0; idx < AST_VECTOR_SIZE(&codecs->payload_mapping_rx); ++idx) {
		...
		if (current->gabpbx_format && to_match->gabpbx_format) {
			if (ast_format_cmp(current->format, to_match->format) == AST_FORMAT_CMP_NOT_EQUAL) {
				continue;
			}
		} else if (!current->gabpbx_format && !to_match->gabpbx_format) {
...
static void rtp_codecs_payloads_copy_rx(struct ast_rtp_codecs *src, struct ast_rtp_codecs *dest, struct ast_rtp_instance *instance)
{
	...
	for (idx = 0; idx < AST_VECTOR_SIZE(&src->payload_mapping_rx); ++idx) {
		...
		if (instance && instance->engine && instance->engine->payload_set) {
			ao2_lock(instance);
			instance->engine->payload_set(instance, idx, type->gabpbx_format, type->format, type->rtp_code);
			ao2_unlock(instance);
		}
	}
}
...
void ast_rtp_codecs_payloads_set_m_type(struct ast_rtp_codecs *codecs, struct ast_rtp_instance *instance, int payload)
{
	...
	if (!payload_mapping_tx_is_present(codecs, new_type)) {
		...
		if (AST_VECTOR_REPLACE(&codecs->payload_mapping_tx, payload, new_type)) {
			...
			ao2_lock(instance);
			instance->engine->payload_set(instance, payload, new_type->gabpbx_format, new_type->format, new_type->rtp_code);
			ao2_unlock(instance);
		}
	} else {
		ao2_ref(new_type, -1);
	...
	for (idx = 0; idx < mime_types_len; ++idx) {
		...
		if (!payload_mapping_tx_is_present(codecs, new_type)) {
			...
			if (AST_VECTOR_REPLACE(&codecs->payload_mapping_tx, pt, new_type)) {
				...
				ao2_lock(instance);
				instance->engine->payload_set(instance, pt, new_type->gabpbx_format, new_type->format, new_type->rtp_code);
				ao2_unlock(instance);
			}
		} else {
			ao2_ref(new_type, -1);
 ...
 * \since 14.0.0
 *
 * \param gabpbx_format Non-zero if the given GABpbx format is present
 * \param format GABpbx format to look for
 * \param code The non-GABpbx format code to look for
 *
 ...
 */
static int find_static_payload_type(int gabpbx_format, const struct ast_format *format, int code)
{
	int idx;
	int payload = -1;
	...
	if (!gabpbx_format) {
		for (idx = 0; idx < AST_RTP_MAX_PT; ++idx) {
			if (static_RTP_PT[idx]
				&& !static_RTP_PT[idx]->gabpbx_format
...
static int find_unused_payload(const struct ast_rtp_codecs *codecs)
{
	 ...
	 * in GABpbx because when Compact Headers are activated, no rtpmap is
 ...
 */
static int rtp_codecs_assign_payload_code_rx(struct ast_rtp_codecs *codecs, int gabpbx_format, struct ast_format *format, int code, int explicit, unsigned int sample_rate)
{
	int payload = code, i;
	struct ast_rtp_payload_type *new_type;
	static struct ast_rtp_payload_type *ignore[AST_RTP_MAX_PT] = {0};
	...
	if (!explicit) {
		payload = find_static_payload_type(gabpbx_format, format, code);

		if (payload < 0 && (!gabpbx_format || !ast_option_rtpusedynamic)) {
			return payload;
		}
	}
	...
	if (payload > -1 && (payload < AST_RTP_PT_FIRST_DYNAMIC
		...
		*/
		 ...
		 */
		ast_log_chan(NULL, LOG_WARNING, "No dynamic RTP payload type values available "
...
int ast_rtp_codecs_payload_code(struct ast_rtp_codecs *codecs, int gabpbx_format, struct ast_format *format, int code)
{
	return ast_rtp_codecs_payload_code_sample_rate(codecs, gabpbx_format, format, code, 0);
}

int ast_rtp_codecs_payload_code_sample_rate(struct ast_rtp_codecs *codecs, int gabpbx_format, struct ast_format *format, int code, unsigned int sample_rate)
{
	struct ast_rtp_payload_type *type;
	int idx;
	int payload = -1;
	...
	if (payload < 0) {
		payload = rtp_codecs_assign_payload_code_rx(codecs, gabpbx_format, format,
			code, 0, sample_rate);
	}
	ast_rwlock_unlock(&static_RTP_PT_lock);
...
int ast_rtp_codecs_payload_code_tx_sample_rate(struct ast_rtp_codecs *codecs, int gabpbx_format, const struct ast_format *format, int code, unsigned int sample_rate)
{
	struct ast_rtp_payload_type *type;
	int idx;
	int payload = -1;
...
int ast_rtp_codecs_payload_code_tx(struct ast_rtp_codecs *codecs, int gabpbx_format, const struct ast_format *format, int code)
{
	return ast_rtp_codecs_payload_code_tx_sample_rate(codecs, gabpbx_format, format, code, 0);
}
...
const char *ast_rtp_lookup_mime_subtype2(const int gabpbx_format,
	const struct ast_format *format, int code, enum ast_rtp_options options)
{
	...
	for (i = 0; i < mime_types_len; i++) {
		if (ast_rtp_mime_types[i].payload_type.gabpbx_format && gabpbx_format && format &&
		...
		} else if (!ast_rtp_mime_types[i].payload_type.gabpbx_format && !gabpbx_format &&
...
unsigned int ast_rtp_lookup_sample_rate2(int gabpbx_format,
...
char *ast_rtp_lookup_mime_multiple2(struct ast_str *buf, struct ast_format_cap *ast_format_capability, int rtp_capability, const int gabpbx_format, enum ast_rtp_options options)
{
	int found = 0;
	const char *name;
	if (!buf) {
	...
	if (gabpbx_format) {
		...
		for (x = 0; x < ast_format_cap_count(ast_format_capability); x++) {
			...
			name = ast_rtp_lookup_mime_subtype2(gabpbx_format, tmp_fmt, 0, options);
			ao2_ref(tmp_fmt, -1);
		...
		for (x = 1; x <= AST_RTP_MAX; x <<= 1) {
			if (rtp_capability & x) {
				name = ast_rtp_lookup_mime_subtype2(gabpbx_format, NULL, x, options);
				ast_str_append(&buf, 0, "%s|", name);
				found = 1;
			}
		}
...
void ast_rtp_instance_early_bridge_make_compatible(struct ast_channel *c_dst, struct ast_channel *c_src)
{
	...
	if (glue_dst->update_peer(c_dst, instance_src, vinstance_src, tinstance_src, cap_src, 0)) {
		ast_log_chan(NULL, LOG_WARNING, "Channel '%s' failed to setup early bridge to '%s'\n",
...
int ast_rtp_instance_early_bridge(struct ast_channel *c0, struct ast_channel *c1)
{
	...
	if (!(glue0 = ast_rtp_instance_get_glue(ast_channel_tech(c0)->type)) || !(glue1 = ast_rtp_instance_get_glue(ast_channel_tech(c1)->type))) {
		ast_log_chan(NULL, LOG_WARNING, "Can't find native functions for channel '%s'\n", glue0 ? ast_channel_name(c1) : ast_channel_name(c0));
		goto done;
	}
...
int ast_rtp_instance_make_compatible(struct ast_channel *chan, struct ast_rtp_instance *instance, struct ast_channel *peer)
{
	...
	if (!peer_instance) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to get_rtp_info for peer type %s\n", glue->type);
		ast_channel_unlock(peer);
		return -1;
	}
	if (peer_instance->engine != instance->engine) {
		ast_log_chan(NULL, LOG_ERROR, "Peer engine mismatch for type %s\n", glue->type);
		ast_channel_unlock(peer);
...
void ast_rtp_instance_available_formats(struct ast_rtp_instance *instance, struct ast_format_cap *to_endpoint, struct ast_format_cap *to_gabpbx, struct ast_format_cap *result)
{
	if (instance->engine->available_formats) {
		ao2_lock(instance);
		instance->engine->available_formats(instance, to_endpoint, to_gabpbx, result);
		ao2_unlock(instance);
	...
	ast_translate_available_formats(to_endpoint, to_gabpbx, result);
}
...
int ast_rtp_dtls_cfg_parse(struct ast_rtp_dtls_cfg *dtls_cfg, const char *name, const char *value)
{
	if (!strcasecmp(name, "dtlsenable")) {
		...
		if (!ast_strlen_zero(value) && !ast_file_is_readable(value)) {
			ast_log_chan(NULL, LOG_ERROR, "%s file %s does not exist or is not readable\n", name, value);
			return -1;
		}
...
int ast_rtp_dtls_cfg_validate(struct ast_rtp_dtls_cfg *dtls_cfg)
{
	if (dtls_cfg->ephemeral_cert) {
		if (!ast_strlen_zero(dtls_cfg->certfile)) {
			ast_log_chan(NULL, LOG_ERROR, "You cannot request automatically generated certificates"
			...
			ast_log_chan(NULL, LOG_NOTICE, "dtls_pvt_file, dtls_cafile, and dtls_ca_path are"
...
static void set_next_mime_type(struct ast_format *format, int rtp_code, const char *type, const char *subtype, unsigned int sample_rate)
{
	...
	if (format) {
		ast_rtp_mime_types[x].payload_type.gabpbx_format = 1;
...
static void add_static_payload(int payload, struct ast_format *format, int rtp_code)
{
	...
	/*
	 * Either the given payload is truly a static type, or GABpbx is
...
size_t ast_rtp_instance_get_recv_buffer_max(struct ast_rtp_instance *instance)
{
	...
	if (!test) {
		ast_log_chan(NULL, LOG_ERROR, "There is no test engine set up!\n");
		return 0;
	}
...
struct ast_json *ast_rtp_convert_stats_json(const struct ast_rtp_instance_stats *stats)
{
	...
	if (ret) {
		ast_log_chan(NULL, LOG_WARNING, "Could not create rtp statistics info. channel: %s\n", stats->channel_uniqueid);
