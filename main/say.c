 ...
 *						by Naveen Albert <gabpbx@phreaknet.org>
 */
...
static struct ast_str* ast_get_money_en_dollars_str(const char *str, const char *lang)
{
	...
	if (amt >= 100) {
		...
		if (!fnrecurse) {
			ast_log_chan(NULL, LOG_WARNING, "Couldn't get string for dollars\n");
		} else {
	...
	if (cents > 0) {
		...
		if (!fnrecurse) {
			ast_log_chan(NULL, LOG_ERROR, "Couldn't get string for cents\n");
		} else {
			fnr = ast_str_buffer(fnrecurse);
...
struct ast_str* ast_get_money_str(const char *str, const char *lang)
{
	...
	ast_log_chan(NULL, LOG_WARNING, "Language %s not currently supported, defaulting to US Dollars\n", lang);
	/* Default to english */
	return ast_get_money_en_dollars_str(str, lang);
}
		...
		if (num < 0) {
			...
			if (num < 1000){
				...
				if (num < 1000000) { /* 1,000,000 */
					...
					if (!fnrecurse) {
						ast_log_chan(NULL, LOG_ERROR, "Couldn't get string for num\n");
					} else {
						fnr = ast_str_buffer(fnrecurse);
					...
					if (num < 1000000000) {	/* 1,000,000,000 */
						...
						if (num < INT_MAX) {
							...
							ast_log_chan(NULL, LOG_WARNING, "Number '%d' is too big for me\n", num);
							res = -1;
						}
					}
				}
...
struct ast_str* ast_get_number_str(int num, const char *lang)
{
	...
	ast_log_chan(NULL, LOG_WARNING, "Language %s not currently supported, defaulting to English\n", lang);
	/* Default to english */
	return get_number_str_en(num, lang);
}
	...
	while (!res && (num || playh)) {
		if (num < 0) {
			...
			if (num > 0) {
				...
				ast_log_chan(NULL, LOG_ERROR, "Unsupported ordinal number: %d\n", num);
			}
			num = 0;
		} else if (num < 100) {
			int base = (num / 10) * 10;
...
/*! \page Def_syntaxlang GABpbx Language Syntaxes supported
    \note Not really language codes.
	For these language codes, GABpbx will change the syntax when
...
static int wait_file(struct ast_channel *chan, const char *ints, const char *file, const char *lang)
{
	...
	if ((res = ast_streamfile(chan, file, lang))) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to play message %s\n", file);
	}
	if (!res) {
		res = ast_waitstream(chan, ints);
	}
...
static int ast_say_number_full_he(struct ast_channel *chan, int num, const char *ints, const char *language, const char *options, int audiofd, int ctrlfd)
{
	...
	ast_verb_chan(NULL, 3, "ast_say_digits_full: started. num: %d, options=\"%s\"\n", num, options);
	...
	ast_verb_chan(NULL, 3, "ast_say_digits_full: num: %d, state=%d, options=\"%s\", mf=%d\n", num, state, options, mf);
	...
	while (!res && (num || (state > 0))) {
		 ...
		 */
		ast_verb_chan(NULL, 3, "ast_say_digits_full: num: %d, state=%d, options=\"%s\", mf=%d, tmpnum=%d\n", num, state, options, mf, tmpnum);
...
static int ast_say_number_full_pt(struct ast_channel *chan, int num, const char *ints, const char *language, const char *options, int audiofd, int ctrlfd)
{
	...
	while (!res && num ) {
		if (num < 0) {
			...
			/* number is too big */
			ast_log_chan(NULL, LOG_WARNING, "Number '%d' is too big to say.", num);
			res = -1;
		}
		if (!res) {
			if (!ast_streamfile(chan, fn, language)) {
...
static int ast_say_enumeration_full_he(struct ast_channel *chan, int num, const char *ints, const char *language, const char *options, int audiofd, int ctrlfd)
{
	...
	ast_verb_chan(NULL, 3, "ast_say_digits_full: num: %d, options=\"%s\", mf=%d\n", num, options, mf);
...
int ast_say_date_with_format_en(struct ast_channel *chan, time_t t, const char *ints, const char *lang, const char *format, const char *tzone)
{
	...
	for (offset=0 ; format[offset] != '\0' ; offset++) {
		...
		switch (format[offset]) {
			...
			default:
				/* Unknown character */
				ast_log_chan(NULL, LOG_WARNING, "Unknown character in datetime format %s: %c at pos %d\n", format, format[offset], offset);
		}
		/* Jump out on DTMF */
		if (res) {
			break;
...
int ast_say_date_with_format_ja(struct ast_channel *chan, time_t time, const char *ints, const char *lang, const char *format, const char *timezone)
{
	...
	for (offset = 0; format[offset] != '\0'; offset++) {
		ast_log_chan(NULL, LOG_DEBUG, "Parsing %c (offset %d) in %s\n", format[offset], offset, format);
		switch (format[offset]) {
			/* NOTE:  if you add more options here, please try to be consistent with strftime(3) */
		case '\'':
			/* Literal name of a sound file */
