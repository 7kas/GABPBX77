...
int ast_sched_start_thread(struct ast_sched_context *con)
{
	...
	if (con->sched_thread) {
		ast_log_chan(NULL, LOG_ERROR, "Thread already started on this scheduler context\n");
		return -1;
	}
...
int ast_sched_del_nonrunning(struct ast_sched_context *con, int id)
{
	...
	if (s) {
		if (!ast_heap_remove(con->sched_heap, s)) {
			ast_log_chan(NULL, LOG_WARNING,"sched entry %d not in the sched heap?\n", s->sched_id->id);
		}
		sched_release(con, s);
...
void ast_sched_dump(struct ast_sched_context *con)
{
	...
	when = ast_tvnow();
#ifdef SCHED_MAX_CACHE
	ast_log_chan(NULL, LOG_DEBUG, "GABpbx Schedule Dump (%zu in Q, %u Total, %u Cache, %u high-water)\n",
		ast_heap_size(con->sched_heap), con->eventcnt - 1, con->schedccnt, con->highwater);
#else
	ast_log_chan(NULL, LOG_DEBUG, "GABpbx Schedule Dump (%zu in Q, %u Total, %u high-water)\n",
		ast_heap_size(con->sched_heap), con->eventcnt - 1, con->highwater);
#endif

	ast_log_chan(NULL, LOG_DEBUG, "=============================================================\n");
	ast_log_chan(NULL, LOG_DEBUG, "|ID    Callback          Data              Time  (sec:ms)   |\n");
	ast_log_chan(NULL, LOG_DEBUG, "+-----+-----------------+-----------------+-----------------+\n");
	ast_mutex_lock(&con->lock);
	heap_size = ast_heap_size(con->sched_heap);
	for (x = 1; x <= heap_size; x++) {
		...
		ast_log_chan(NULL, LOG_DEBUG, "|%.4d | %-15p | %-15p | %.6ld : %.6ld |\n",
			q->sched_id->id,
			q->callback,
			q->data,
			(long)delta.tv_sec,
