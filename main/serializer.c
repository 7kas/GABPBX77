...
int ast_serializer_pool_destroy(struct ast_serializer_pool *pool)
{
	...
	if (pool->shutdown_group) {
		...
		if (remaining) {
			...
			ast_log_chan(NULL, LOG_WARNING, "'%s' serializer pool destruction timeout. "
	...
	unsigned int size, struct ast_threadpool *threadpool, int timeout)
{
	...
	for (idx = 0; idx < size; ++idx) {
		...
		if (!tps) {
			...
			ast_log_chan(NULL, LOG_ERROR, "Pool create: unable to create named serializer '%s'\n",
					tps_name);
			return NULL;
		}
		...
		if (AST_VECTOR_APPEND(&pool->serializers, tps)) {
			ast_serializer_pool_destroy(pool);
			ast_log_chan(NULL, LOG_ERROR, "Pool create: unable to append named serializer '%s'\n",
					tps_name);
			return NULL;
		}
	}
...
int ast_serializer_pool_set_alerts(struct ast_serializer_pool *pool, long high, long low)
{
	...
	tps_queue_high = high;
	if (tps_queue_high <= 0) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Invalid '%s-*' taskprocessor high water alert "
	...
	if (tps_queue_low < -1 || tps_queue_high < tps_queue_low) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Invalid '%s-*' taskprocessor low water clear alert "
	...
	for (idx = 0; idx < AST_VECTOR_SIZE(&pool->serializers); ++idx) {
		...
		if (ast_taskprocessor_alert_set_levels(cur, tps_queue_low, tps_queue_high)) {
			ast_log_chan(NULL, AST_LOG_WARNING, "Failed to set alert levels for serializer '%s'.\n",
