...
{
	if (!api_provider) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to send custom SIP INFO. No API provider registered\n");
		return -1;
	}
...
{
	if (api_provider) {
		ast_log_chan(NULL, LOG_WARNING, "SIP provider %s has already registered. Not registering provider %s\n",
				api_provider->name, provider->name);
		return -1;
	}

	if (provider->version != AST_SIP_API_VERSION) {
		ast_log_chan(NULL, LOG_WARNING, "SIP API provider version mismatch: Current version is %d but provider "
