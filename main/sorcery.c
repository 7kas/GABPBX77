...
int __ast_sorcery_wizard_register(const struct ast_sorcery_wizard *interface, struct ast_module *module)
{
	...
	if ((wizard = ao2_find(wizards, interface->name, OBJ_KEY | OBJ_NOLOCK))) {
		ast_log_chan(NULL, LOG_WARNING, "Attempted to register sorcery wizard '%s' twice\n",
			interface->name);
		goto done;
	}
	...
	ast_verb_chan(NULL, 5, "Sorcery registered wizard '%s'\n", interface->name);
...
int ast_sorcery_wizard_unregister(const struct ast_sorcery_wizard *interface)
{
	...
	if (wizard) {
		...
		ao2_ref(wizard, -1);
		ast_verb_chan(NULL, 5, "Sorcery unregistered wizard '%s'\n", interface->name);
		return 0;
	} else {
		return -1;
	}
...
struct ast_sorcery *__ast_sorcery_open(const char *module_name, const char *file, int line, const char *func)
{
	...
	if (__ast_sorcery_apply_config(sorcery, module_name, module_name) == AST_SORCERY_APPLY_FAIL) {
		ast_log_chan(NULL, LOG_ERROR, "Error attempting to apply configuration %s to sorcery.\n", module_name);
		goto failure_cleanup;
	}
...
#undef WIZARD_COMPARE
	...
	if (internal_wizard->callbacks.open && !(object_wizard->data = internal_wizard->callbacks.open(wizard_args))) {
		ast_log_chan(NULL, LOG_WARNING, "Wizard '%s' failed to open mapping for object type '%s' with data: %s\n",
...
static int sorcery_object_load(void *obj, void *arg, int flags)
{
	...
	if (details->reload && !sorcery_reloadable(details->sorcery, details->type)) {
		ast_log_chan(NULL, LOG_NOTICE, "Type '%s' is not reloadable, maintaining previous values\n",
			details->type);
		return 0;
	}
