...
static struct ao2_container *get_languages(void)
{
	...
	if (srcdir == NULL) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to open %s\n", ast_str_buffer(media_dir));
		return NULL;
	}
	...
	while((dent = readdir(srcdir)) != NULL) {
		...
		if (stat(ast_str_buffer(variant_dir), &st) < 0) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to stat %s\n", ast_str_buffer(variant_dir));
			continue;
		}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER, "Sounds Index",
