...
#include "gabpbx/stasis_internal.h"
			...
			<configObject name="declined_message_types">
				...
				<configOption name="decline">
					...
					<description>
						...
						message type that GABpbx is forbidden from creating and can be
 ...
 *      the GABpbx process (ast_channel_topic_all(), etc.) but most of these
 ...
 * https://docs.gabpbx.org/Development/Roadmap/GABpbx-12-Projects/GABpbx-12-API-Improvements/Stasis-Message-Bus/Using-the-Stasis-Message-Bus/Stasis-Subscriber-Shutdown-Problem/.
 *
...
static int link_topic_proxy(struct stasis_topic *topic, const char *name, const char *detail)
{
	...
	if (topic_tmp) {
		ast_log_chan(NULL, LOG_ERROR, "The same topic is already exist. name: %s\n", name);
		ao2_ref(topic_tmp, -1);
	...
	int synchronous)
{
	...
	if (!synchronous) {
		if (ast_taskprocessor_push_local(sub->mailbox, dispatch_exec_async, message)) {
			/* Push failed; ugh. */
			ast_log_chan(NULL, LOG_ERROR, "Dropping async dispatch\n");
			ao2_cleanup(message);
			return 0;
		}
	} else {
		...
		if (ast_taskprocessor_push_local(sub->mailbox, dispatch_exec_sync, &std)) {
			/* Push failed; ugh. */
			ast_log_chan(NULL, LOG_ERROR, "Dropping sync dispatch\n");
			ao2_cleanup(message);
...
void stasis_log_bad_type_access(const char *name)
{
	...
	if (!stasis_message_type_declined(name)) {
		ast_log_chan(NULL, LOG_ERROR, "Use of %s() before init/after destruction\n", name);
	}
#endif
}
...
int stasis_init(void)
{
	...
	if (aco_process_config(&cfg_info, 0) == ACO_PROCESS_ERROR) {
		...
		if (aco_set_defaults(&threadpool_option, "threadpool", default_cfg->threadpool_options)) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to initialize defaults on Stasis configuration object\n");
		...
		if (aco_set_defaults(&declined_option, "declined_message_types", default_cfg->declined_message_types)) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to load stasis.conf and failed to initialize defaults.\n");
			ao2_ref(default_cfg, -1);

			return -1;
		}

		ast_log_chan(NULL, LOG_NOTICE, "Could not load Stasis configuration; using defaults\n");
		ao2_global_obj_replace_unref(globals, default_cfg);
		cfg = default_cfg;
	} else {
		cfg = ao2_global_obj_ref(globals);
		if (!cfg) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to obtain Stasis configuration object\n");
	...
	ao2_ref(cfg, -1);
	if (!threadpool) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to create 'stasis-core' threadpool\n");
