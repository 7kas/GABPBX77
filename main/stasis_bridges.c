		...
		<managerEventInstance class="EVENT_FLAG_CALL">
			...
			<syntax>
				...
				<parameter name="IsExternal">
					<para>Indicates if the transfer was performed outside of GABpbx. For instance,
...
int bridge_topics_init(struct ast_bridge *bridge)
{
	...
	if (ast_strlen_zero(bridge->uniqueid)) {
		ast_log_chan(NULL, LOG_ERROR, "Bridge id initialization required\n");
		return -1;
	}
...
void bridge_topics_destroy(struct ast_bridge *bridge)
{
	...
	if (!bridge->topic) {
		ast_log_chan(NULL, LOG_WARNING, "Bridge " BRIDGE_PRINTF_SPEC " topic is NULL\n",
			BRIDGE_PRINTF_VARS(bridge));
		return;
	}
