...
struct stasis_caching_topic *stasis_caching_unsubscribe(struct stasis_caching_topic *caching_topic)
{
	...
	if (stasis_subscription_is_subscribed(caching_topic->sub)) {
		...
		ast_log_chan(NULL, LOG_ERROR, "stasis_caching_topic unsubscribed multiple times\n");
	}
...
struct stasis_caching_topic *stasis_caching_topic_create(struct stasis_topic *original_topic, struct stasis_cache *cache)
{
	...
	if (!cache->registered) {
		if (ao2_container_register(new_name, cache->entries, print_cache_entry)) {
			ast_log_chan(NULL, LOG_ERROR, "Stasis cache container '%p' for '%s' did not register\n",
