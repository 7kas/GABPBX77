...
struct ast_channel_snapshot *ast_channel_snapshot_create(struct ast_channel *chan)
{
	 ...
	 * GABPBX-28164) they are each regenerated every time a channel snapshot is created.
	 */
...
void ast_channel_publish_varset(struct ast_channel *chan, const char *name, const char *value)
{
	...
	if (result == AST_UTF8_REPLACE_VALID) {
		...
		ast_log_chan(NULL, LOG_WARNING, "%s: The contents of variable '%s' had invalid UTF-8 sequences which were replaced",
	...
	blob = ast_json_pack("{s: s, s: s}",
			     ...
			     "value", new_value);
	if (!blob) {
		ast_log_chan(NULL, LOG_ERROR, "Error creating message\n");
		return;
	}
