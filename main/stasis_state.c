...
error_return:
	ast_log_chan(NULL, LOG_ERROR, "Unable to allocate state '%s' in manager '%s'\n",
	...
	struct stasis_state_manager *manager, const char *id)
{
	...
	if (!sub) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to create subscriber to %s/%s\n",
	...
	struct stasis_state_manager *manager, const char *id)
{
	...
	if (!pub) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to create publisher to %s/%s\n",
			stasis_topic_name(manager->all_topic), id);
		return NULL;
	}
