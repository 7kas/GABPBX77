...
static void *inotify_daemon(void *data)
{
	...
	if (inotify_fd < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Cannot initialize file notification service: %s (%d)\n", strerror(errno), errno);
	...
	for (;/*ever*/;) {
		...
		if ((res = read(inotify_fd, iev, real_sizeof_iev)) < sizeof(*iev) && res > 0) {
			...
			ast_log_chan(NULL, LOG_ERROR, "Inotify read less than a full event (%zd < %zu)?!!\n", res, sizeof(*iev));
			break;
		} else if (res < 0) {
			...
			ast_log_chan(NULL, LOG_ERROR, "Inotify failed: %s\n", strerror(errno));
			break;
		}
		AST_LIST_LOCK(&zonelist);
...
#endif
				{
					ast_log_chan(NULL, LOG_NOTICE, "Removing cached TZ entry '%s' because underlying file changed.\n", name);
				}
				AST_LIST_REMOVE_CURRENT(list);
				sstate_free(cur);
				continue;
		...
		return -1;
	{
		...
		*/
		...
		if ((fid = open(name, OPEN_MODE)) == -1)
			return -1;
		if (ast_fully_booted) {
			/* If we don't wait until GABpbx is fully booted, it's possible
