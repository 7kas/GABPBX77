...
static void *default_tps_processing_function(void *data)
{
	...
	while (!pvt->dead) {
		...
		if (res != 0 && errno != EINTR) {
			ast_log_chan(NULL, LOG_ERROR, "ast_sem_wait(): %s\n",
				strerror(errno));
			/* Just give up */
			break;
		}
...
static void default_task_pushed(struct ast_taskprocessor_listener *listener, int was_empty)
{
	...
	if (ast_sem_post(&pvt->sem) != 0) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to notify of enqueued task: %s\n",
			strerror(errno));
	}
}
...
static void default_listener_shutdown(struct ast_taskprocessor_listener *listener)
{
	...
	if (pthread_equal(pthread_self(), pvt->poll_thread)) {
		...
		if (res != 0) {
			ast_log_chan(NULL, LOG_ERROR, "pthread_detach(): %s\n", strerror(errno));
		}
	} else {
		res = pthread_join(pvt->poll_thread, NULL);
		if (res != 0) {
			ast_log_chan(NULL, LOG_ERROR, "pthread_join(): %s\n", strerror(errno));
		}
	}
...
static void tps_shutdown(void)
{
	...
	if (objcount > 0) {
		...
		if (!sorted_tps || ao2_container_dup(sorted_tps, tps_singletons, 0)) {
			ast_log_chan(NULL, LOG_ERROR, "unable to get sorted list of taskprocessors");
		}
		else {
			iter = ao2_iterator_init(sorted_tps, AO2_ITERATOR_UNLINK);
			while ((tps = ao2_iterator_next(&iter))) {
				ast_log_chan(NULL, LOG_ERROR, "taskprocessor '%s'\n", tps->name);
			}
		}
...
int ast_tps_init(void)
{
	...
	if (!tps_singletons) {
		ast_log_chan(NULL, LOG_ERROR, "taskprocessor container failed to initialize!\n");
		return -1;
	}

	if (AST_VECTOR_RW_INIT(&overloaded_subsystems, 10)) {
		ao2_ref(tps_singletons, -1);
		ast_log_chan(NULL, LOG_ERROR, "taskprocessor subsystems vector failed to initialize!\n");
		return -1;
	}
...
static struct tps_task *tps_task_alloc(int (*task_exe)(void *datap), void *datap)
{
	struct tps_task *t;
	if (!task_exe) {
		ast_log_chan(NULL, LOG_ERROR, "task_exe is NULL!\n");
		return NULL;
	}

	t = ast_calloc(1, sizeof(*t));
	if (!t) {
		ast_log_chan(NULL, LOG_ERROR, "failed to allocate task!\n");
		return NULL;
	}
...
static void tps_alert_add(struct ast_taskprocessor *tps, int delta)
{
	...
	if (DEBUG_ATLEAST(3)
		...
		&& !old != !tps_alert_count) {
		ast_log_chan(NULL, LOG_DEBUG, "Taskprocessor '%s' %s the high water alert.\n",
...
{
	if (!tps) {
		ast_log_chan(NULL, LOG_ERROR, "no taskprocessor specified!\n");
		return NULL;
	}
	return tps->name;
}
...
static void *default_listener_pvt_alloc(void)
{
	...
	if (ast_sem_init(&pvt->sem, 0, 0) != 0) {
		ast_log_chan(NULL, LOG_ERROR, "ast_sem_init(): %s\n", strerror(errno));
		ast_free(pvt);
		return NULL;
	}
	return pvt;
...
static struct ast_taskprocessor *__allocate_taskprocessor(const char *name, struct ast_taskprocessor_listener *listener)
{
	...
	if (!p) {
		ast_log_chan(NULL, LOG_WARNING, "failed to create taskprocessor '%s'\n", name);
		return NULL;
	}
	...
	if (!(ao2_link_flags(tps_singletons, p, OBJ_NOLOCK))) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to add taskprocessor '%s' to container\n", p->name);
		listener->tps = NULL;
		ao2_ref(p, -2);
		return NULL;
	}
...
static struct ast_taskprocessor *__start_taskprocessor(struct ast_taskprocessor *p)
{
	if (p && p->listener->callbacks->start(p->listener)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to start taskprocessor listener for taskprocessor %s\n",
			p->name);
		ast_taskprocessor_unreference(p);
...
struct ast_taskprocessor *ast_taskprocessor_get(const char *name, enum ast_tps_options create)
{
	...
	if (ast_strlen_zero(name)) {
		ast_log_chan(NULL, LOG_ERROR, "requesting a nameless taskprocessor!!!\n");
		return NULL;
	}
...
static int taskprocessor_push(struct ast_taskprocessor *tps, struct tps_task *t)
{
	...
	if (!tps) {
		ast_log_chan(NULL, LOG_ERROR, "tps is NULL!\n");
		return -1;
	}

	if (!t) {
		ast_log_chan(NULL, LOG_ERROR, "t is NULL!\n");
		return -1;
	}
	...
	if (tps->tps_queue_high <= tps->tps_queue_size) {
		if (!tps->high_water_alert) {
			ast_log_chan(NULL, LOG_WARNING, "The '%s' task processor queue reached %ld scheduled tasks%s.\n",
