...
#include "gabpbx/tcptls.h"            /* for ast_tls_config, ast_tcptls_se... */
#include "gabpbx/iostream.h"          /* for DO_SSL, ast_iostream_close, a... */
...
#include "gabpbx/app.h"               /* for ast_read_textfile */
#include "gabpbx/astobj2.h"           /* for ao2_ref, ao2_t_ref, ao2_alloc */
#include "gabpbx/compat.h"            /* for strcasecmp */
#include "gabpbx/config.h"            /* for ast_parse_arg, ast_parse_flag... */
#include "gabpbx/io.h"                /* for ast_sd_get_fd */
#include "gabpbx/lock.h"              /* for AST_PTHREADT_NULL */
#include "gabpbx/logger.h"            /* for ast_log, LOG_ERROR, ast_debug */
#include "gabpbx/netsock2.h"          /* for ast_sockaddr_copy, ast_sockad... */
#include "gabpbx/pbx.h"               /* for ast_thread_inhibit_escalations */
#include "gabpbx/utils.h"             /* for ast_true, ast_free, ast_wait_... */
...
static int check_tcptls_cert_name(ASN1_STRING *cert_str, const char *hostname, const char *desc)
{
	...
	if (strlen((char *) str) != ret) {
		ast_log_chan(NULL, LOG_WARNING, "Invalid certificate %s length (contains NULL bytes?)\n", desc);
...
static void *handle_tcptls_connection(void *data)
{
	...
	if (ast_thread_inhibit_escalations()) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to inhibit privilege escalations; killing connection from peer '%s'\n",
	...
	if (ast_thread_user_interface_set(1)) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to set user interface status; killing connection from peer '%s'\n",
			ast_sockaddr_stringify(&tcptls_session->remote_address));
		ast_tcptls_close_session_file(tcptls_session);
		ao2_ref(tcptls_session, -1);
		return NULL;
	...
	if (tcptls_session->parent->tls_cfg) {
		...
		if (ast_iostream_start_tls(&tcptls_session->stream, tcptls_session->parent->tls_cfg->ssl_ctx, tcptls_session->client) < 0) {
			...
			if (ssl) {
				ast_log_chan(NULL, LOG_ERROR, "Unable to set up ssl connection with peer '%s'\n",
					ast_sockaddr_stringify(&tcptls_session->remote_address));
			}
			ast_tcptls_close_session_file(tcptls_session);
			ao2_ref(tcptls_session, -1);
		...
		if ((tcptls_session->client && !ast_test_flag(&tcptls_session->parent->tls_cfg->flags, AST_SSL_DONT_VERIFY_SERVER))
			...
			if (!peer) {
				ast_log_chan(NULL, LOG_ERROR, "No SSL certificate to verify from peer '%s'\n",
					ast_sockaddr_stringify(&tcptls_session->remote_address));
				ast_tcptls_close_session_file(tcptls_session);
				ao2_ref(tcptls_session, -1);
				return NULL;
			...
			if (res != X509_V_OK) {
				ast_log_chan(NULL, LOG_ERROR, "Certificate from peer '%s' did not verify: %s\n",
					ast_sockaddr_stringify(&tcptls_session->remote_address),
			...
			if (!ast_test_flag(&tcptls_session->parent->tls_cfg->flags, AST_SSL_IGNORE_COMMON_NAME)) {
				...
				if (!found) {
					ast_log_chan(NULL, LOG_ERROR, "Certificate common name from peer '%s' did not match (%s)\n",
...
#else
		ast_log_chan(NULL, LOG_ERROR, "TLS client failed: GABpbx is compiled without OpenSSL support. Install OpenSSL development headers and rebuild GABpbx after running ./configure\n");
		ast_tcptls_close_session_file(tcptls_session);
		ao2_ref(tcptls_session, -1);
		return NULL;
#endif /* DO_SSL */
...
void *ast_tcptls_server_root(void *data)
{
	...
	for (;;) {
		...
		if (fd < 0) {
			if (errno != EAGAIN
				...
				&& errno != EINTR
				&& errno != ECONNABORTED) {
				ast_log_chan(NULL, LOG_ERROR, "TCP/TLS accept failed: %s\n", strerror(errno));
				if (errno != EMFILE) {
					break;
				}
			}
		...
		if (ast_pthread_create_detached_background(&launched, NULL, handle_tcptls_connection, tcptls_session)) {
			ast_log_chan(NULL, LOG_ERROR, "TCP/TLS unable to launch helper thread for peer '%s': %s\n",
				ast_sockaddr_stringify(&tcptls_session->remote_address),
				strerror(errno));
			ao2_ref(tcptls_session, -1);
		}
	...
	ast_log_chan(NULL, LOG_ERROR, "TCP/TLS listener thread ended abnormally\n");

	/* Close the listener socket so GABpbx doesn't appear dead. */
	fd = desc->accept_fd;
	desc->accept_fd = -1;
	if (0 <= fd) {
		close(fd);
...
static void __ssl_setup_certs(struct ast_tls_config *cfg, const size_t cert_file_len, const char *key_type_extension, const char *key_type)
{
	...
	if (access(cert_file, F_OK) == 0) {
		if (SSL_CTX_use_certificate_chain_file(cfg->ssl_ctx, cert_file) == 0) {
			ast_log_chan(NULL, LOG_WARNING, "TLS/SSL error loading public %s key (certificate) from <%s>.\n", key_type, cert_file);
			write_openssl_error_to_log();
		} else if (SSL_CTX_use_PrivateKey_file(cfg->ssl_ctx, cert_file, SSL_FILETYPE_PEM) == 0) {
			ast_log_chan(NULL, LOG_WARNING, "TLS/SSL error loading private %s key from <%s>.\n", key_type, cert_file);
			write_openssl_error_to_log();
		} else if (SSL_CTX_check_private_key(cfg->ssl_ctx) == 0) {
			ast_log_chan(NULL, LOG_WARNING, "TLS/SSL error matching private %s key and certificate in <%s>.\n", key_type, cert_file);
			write_openssl_error_to_log();
		}
	}
}
...
static int __ssl_setup(struct ast_tls_config *cfg, int client)
{
#ifndef DO_SSL
	if (cfg->enabled) {
		ast_log_chan(NULL, LOG_ERROR, "TLS server failed: GABpbx is compiled without OpenSSL support. Install OpenSSL development headers and rebuild GABpbx after running ./configure\n");
		cfg->enabled = 0;
	}
	return 0;
#else
	...
	if (client) {
		...
		if (ast_test_flag(&cfg->flags, AST_SSL_SSLV2_CLIENT)) {
			ast_log_chan(NULL, LOG_WARNING, "Usage of SSLv2 is discouraged due to known vulnerabilities. Please use 'tlsv1' or leave the TLS method unspecified!\n");
			cfg->ssl_ctx = SSL_CTX_new(SSLv2_client_method());
		} else
#endif
#if !defined(OPENSSL_NO_SSL3_METHOD) && !(defined(OPENSSL_API_COMPAT) && (OPENSSL_API_COMPAT >= 0x10100000L))
		if (ast_test_flag(&cfg->flags, AST_SSL_SSLV3_CLIENT)) {
			ast_log_chan(NULL, LOG_WARNING, "Usage of SSLv3 is discouraged due to known vulnerabilities. Please use 'tlsv1' or leave the TLS method unspecified!\n");
...
#else
	ast_log_chan(NULL, LOG_WARNING, "Your version of OpenSSL leaves you potentially vulnerable "
	...
	if (!ast_strlen_zero(cfg->certfile)) {
		...
		if (SSL_CTX_use_certificate_chain_file(cfg->ssl_ctx, cfg->certfile) == 0) {
			if (!client) {
				...
				ast_log_chan(NULL, LOG_ERROR, "TLS/SSL error loading cert file. <%s>\n", cfg->certfile);
				write_openssl_error_to_log();
				cfg->enabled = 0;
				SSL_CTX_free(cfg->ssl_ctx);
				cfg->ssl_ctx = NULL;
		...
		if ((SSL_CTX_use_PrivateKey_file(cfg->ssl_ctx, tmpprivate, SSL_FILETYPE_PEM) == 0) || (SSL_CTX_check_private_key(cfg->ssl_ctx) == 0 )) {
			if (!client) {
				...
				ast_log_chan(NULL, LOG_ERROR, "TLS/SSL error loading private key file. <%s>\n", tmpprivate);
				write_openssl_error_to_log();
				cfg->enabled = 0;
				SSL_CTX_free(cfg->ssl_ctx);
				cfg->ssl_ctx = NULL;
		...
		if (!client) {
			...
			/* expects a file name which contains _rsa. like gabpbx_rsa.pem
	...
	if (!ast_strlen_zero(cfg->cipher)) {
		if (SSL_CTX_set_cipher_list(cfg->ssl_ctx, cfg->cipher) == 0 ) {
			if (!client) {
				ast_log_chan(NULL, LOG_ERROR, "TLS/SSL cipher error <%s>\n", cfg->cipher);
				write_openssl_error_to_log();
				cfg->enabled = 0;
				SSL_CTX_free(cfg->ssl_ctx);
				cfg->ssl_ctx = NULL;
	...
	if (!ast_strlen_zero(cfg->cafile) || !ast_strlen_zero(cfg->capath)) {
		if (SSL_CTX_load_verify_locations(cfg->ssl_ctx, S_OR(cfg->cafile, NULL), S_OR(cfg->capath,NULL)) == 0) {
			ast_log_chan(NULL, LOG_ERROR, "TLS/SSL CA file(%s)/path(%s) error\n", cfg->cafile, cfg->capath);
			write_openssl_error_to_log();
		}
	}
		...
		if (bio != NULL) {
			...
			if (dh != NULL) {
				if (SSL_CTX_set_tmp_dh(cfg->ssl_ctx, dh)) {
					...
					ast_verb_chan(NULL, 2, "TLS/SSL DH initialized, PFS cipher-suites enabled\n");
				}
				DH_free(dh);
			}
			BIO_free(bio);
	...
	if (SSL_CTX_ctrl(cfg->ssl_ctx, SSL_CTRL_SET_ECDH_AUTO, 1, NULL)) {
		ast_verb_chan(NULL, 2, "TLS/SSL ECDH initialized (automatic), faster PFS ciphers enabled\n");
		...
		if (ecdh != NULL) {
			if (SSL_CTX_set_tmp_ecdh(cfg->ssl_ctx, ecdh)) {
				ast_verb_chan(NULL, 2, "TLS/SSL ECDH initialized (secp256r1), faster PFS cipher-suites enabled\n");
			}
			EC_KEY_free(ecdh);
		}
#endif
	...
	ast_verb_chan(NULL, 2, "TLS/SSL certificate ok\n");	/* We should log which one that is ok. This message doesn't really make sense in production use */
	return 1;
#endif
}
	...
	struct ast_tcptls_session_instance *tcptls_session, int timeout)
{
	...
	if (socket_connect(desc->accept_fd, &desc->remote_address, timeout)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to connect %s to %s: %s\n", desc->name,
...
struct ast_tcptls_session_instance *ast_tcptls_client_create(struct ast_tcptls_session_args *desc)
{
	...
	if (desc->accept_fd < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to allocate socket for %s: %s\n",
	...
	if (!ast_sockaddr_isnull(&desc->local_address) &&
		...
		if (ast_bind(desc->accept_fd, &desc->local_address)) {
			ast_log_chan(NULL, LOG_ERROR, "Unable to bind %s to %s: %s\n",
				desc->name,
	...
	if (!ast_strlen_zero(desc->hostname)) {
		if (ast_iostream_set_sni_hostname(tcptls_session->stream, desc->hostname) != 0) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to set SNI hostname '%s' on connection '%s'\n",
...
void ast_tcptls_server_start(struct ast_tcptls_session_args *desc)
{
	...
	if (listen(desc->accept_fd, 10)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to listen for %s!\n", desc->name);
		goto error;
	}

systemd_socket_activation:
	if (ast_pthread_create_background(&desc->master, NULL, desc->accept_fn, desc)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to launch thread for %s on %s: %s\n",
			desc->name,
			ast_sockaddr_stringify(&desc->local_address),
			strerror(errno));
		goto error;
...
int ast_tls_read_conf(struct ast_tls_config *tls_cfg, struct ast_tcptls_session_args *tls_desc, const char *varname, const char *value)
{
	if (!strcasecmp(varname, "tlsenable") || !strcasecmp(varname, "sslenable")) {
		...
		if (ast_parse_arg(value, PARSE_ADDR, &tls_desc->local_address))
			ast_log_chan(NULL, LOG_ERROR, "Invalid %s '%s'\n", varname, value);
