...
#include "gabpbx/stasis.h"
#include "gabpbx/json.h"
#include "gabpbx/app.h"		/* for ast_replace_sigchld(), etc. */
...
int ast_test_register(ast_test_cb_t *cb)
{
	...
	if (!cb) {
		ast_log_chan(NULL, LOG_ERROR, "Attempted to register test without all required information\n");
		registration_errors++;
		return -1;
	}
...
int ast_test_capture_command(struct ast_test_capture *capture, const char *file, char *const argv[], const char *data, unsigned datalen)
{
	...
	if (data != NULL && datalen > 0) {
		if (pipe(fd0) == -1) {
			ast_log_chan(NULL, LOG_ERROR, "Couldn't open stdin pipe: %s\n", strerror(errno));
			goto cleanup;
		}
		fcntl(fd0[1], F_SETFL, fcntl(fd0[1], F_GETFL, 0) | O_NONBLOCK);
	} else {
		if ((fd0[0] = open("/dev/null", O_RDONLY)) == -1) {
			ast_log_chan(NULL, LOG_ERROR, "Couldn't open /dev/null: %s\n", strerror(errno));
			goto cleanup;
		}
	}

	if (pipe(fd1) == -1) {
		ast_log_chan(NULL, LOG_ERROR, "Couldn't open stdout pipe: %s\n", strerror(errno));
		goto cleanup;
	}

	if (pipe(fd2) == -1) {
		ast_log_chan(NULL, LOG_ERROR, "Couldn't open stderr pipe: %s\n", strerror(errno));
		goto cleanup;
	}
	...
	if ((pid = fork()) == -1) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to fork(): %s\n", strerror(errno));
		goto cleanup;
		...
		execvp(file, argv);
		ast_log_chan(NULL, LOG_ERROR, "Failed to execv(): %s\n", strerror(errno));
		exit(1);
		...
		if ((cmd = fmemopen(buf, sizeof(buf), "w")) == NULL) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to open memory buffer: %s\n", strerror(errno));
			kill(pid, SIGKILL);
			goto cleanup;
		}
		...
		ast_log_chan(NULL, LOG_TRACE, "run: %.*s\n", (int)sizeof(buf), buf);

		if ((out = open_memstream(&capture->outbuf, &capture->outlen)) == NULL) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to open output buffer: %s\n", strerror(errno));
			kill(pid, SIGKILL);
			goto cleanup;
		}

		if ((err = open_memstream(&capture->errbuf, &capture->errlen)) == NULL) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to open error buffer: %s\n", strerror(errno));
			kill(pid, SIGKILL);
			goto cleanup;
		}
		...
		while (1) {
			...
			if (n == pid && WIFEXITED(wstatus)) {
				...
				ast_log_chan(NULL, LOG_TRACE, "run: pid %d exits %d\n", capture->pid, capture->exitcode);
...
static int test_generate_results(const char *name, const char *category, const char *xml_path, const char *txt_path)
{
	...
	if (!ast_strlen_zero(xml_path)) {
		if (!(f_xml = fopen(xml_path, "w"))) {
			ast_log_chan(NULL, LOG_WARNING, "Could not open file %s for xml test results\n", xml_path);
			res = -1;
			goto done;
		}
	}
	if (!ast_strlen_zero(txt_path)) {
		if (!(f_txt = fopen(txt_path, "w"))) {
			ast_log_chan(NULL, LOG_WARNING, "Could not open file %s for text output of test results\n", txt_path);
			res = -1;
			goto done;
		}
	}
	...
	if (f_txt) {
		fprintf(f_txt, "GABpbx Version:         %s\n", ast_get_version());
		fprintf(f_txt, "GABpbx Version Number:  %s\n", ast_get_version_num());
...
static struct ast_test *test_alloc(ast_test_cb_t *cb)
{
	...
	if (!test) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to allocate test, registration failed.\n");
		return NULL;
	}
	...
	if (ast_strlen_zero(test->info.name)) {
		ast_log_chan(NULL, LOG_ERROR, "Test has no name, test registration refused.\n");
		return test_free(test);
	}

	if (ast_strlen_zero(test->info.category)) {
		ast_log_chan(NULL, LOG_ERROR, "Test %s has no category, test registration refused.\n",
			test->info.name);
		return test_free(test);
	}

	if (test->info.category[0] != '/' || test->info.category[strlen(test->info.category) - 1] != '/') {
		ast_log_chan(NULL, LOG_WARNING, "Test category '%s' for test '%s' is missing a leading or trailing slash.\n",
			test->info.category, test->info.name);
		/*
	...
	if (ast_strlen_zero(test->info.summary)) {
		ast_log_chan(NULL, LOG_ERROR, "Test %s%s has no summary, test registration refused.\n",
			test->info.category, test->info.name);
		return test_free(test);
	}
	if (test->info.summary[strlen(test->info.summary) - 1] == '\n') {
		ast_log_chan(NULL, LOG_WARNING, "Test %s%s summary has a trailing newline.\n",
			test->info.category, test->info.name);
		/*
		 * Flag an error anyways so test_registrations fails but allow the
		 * test to be registered.
	...
	if (ast_strlen_zero(test->info.description)) {
		ast_log_chan(NULL, LOG_ERROR, "Test %s%s has no description, test registration refused.\n",
			test->info.category, test->info.name);
		return test_free(test);
	}
	if (test->info.description[strlen(test->info.description) - 1] == '\n') {
		ast_log_chan(NULL, LOG_WARNING, "Test %s%s description has a trailing newline.\n",
			test->info.category, test->info.name);
		/*
		 * Flag an error anyways so test_registrations fails but allow the
		 * test to be registered.
	...
	if (!(test->status_str = ast_str_create(128))) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to allocate status_str for %s%s, test registration failed.\n",
			test->info.category, test->info.name);
		return test_free(test);
	}
...
static char *test_cli_generate_results(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	case CLI_HANDLER:
		...
		if (a->argc == 5) {
			...
			ast_str_set(&buf, 0, "%s/gabpbx_test_results-%ld.%s", ast_config_AST_LOG_DIR, (long) time.tv_sec, type);
