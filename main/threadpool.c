...
struct worker_thread {
	/*! A unique (within a run of GABpbx) ID for the thread. Used for hashing and searching */
	int id;
...
static int activate_thread(void *obj, void *arg, int flags)
{
	...
	if (!ao2_link(pool->active_threads, worker)) {
		 ...
		 */
		ast_log_chan(NULL, LOG_WARNING, "Failed to activate thread %d. Remaining idle\n",
				worker->id);
		return 0;
	}
	...
	for (i = 0; i < delta; ++i) {
		...
		if (ao2_link(pool->idle_threads, worker)) {
			if (worker_thread_start(worker)) {
				ast_log_chan(NULL, LOG_ERROR, "Unable to start worker thread %d. Destroying.\n", worker->id);
				ao2_unlink(pool->active_threads, worker);
			}
		} else {
			ast_log_chan(NULL, LOG_WARNING, "Failed to activate worker thread %d. Destroying.\n", worker->id);
		}
		ao2_ref(worker, -1);
	}
}
...
static int zombify_threads(void *obj, void *arg, void *data, int flags)
{
	...
	if ((*num_to_zombify)-- > 0) {
		if (!ao2_link(pool->zombie_threads, worker)) {
			ast_log_chan(NULL, LOG_WARNING, "Failed to zombify active thread %d. Thread will remain active\n", worker->id);
			return 0;
		}
		...
		const struct ast_threadpool_options *options)
{
	...
	if (options->version != AST_THREADPOOL_OPTIONS_VERSION) {
		ast_log_chan(NULL, LOG_WARNING, "Incompatible version of threadpool options in use.\n");
		return NULL;
	}
