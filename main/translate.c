...
static struct ast_trans_pvt *newpvt(struct ast_translator *t, struct ast_format *explicit_dst)
{
	...
	if (!pvt->f.subclass.format) {
		...
		if (!pvt->f.subclass.format) {
			...
			if (!codec) {
				ast_log_chan(NULL, LOG_ERROR, "Unable to get destination codec\n");
				destroy(pvt);
				return NULL;
			}
		...
		if (!pvt->f.subclass.format) {
			ast_log_chan(NULL, LOG_ERROR, "Unable to create format\n");
			destroy(pvt);
			return NULL;
		}
	}
...
static int framein(struct ast_trans_pvt *pvt, struct ast_frame *f)
{
	...
	if (f->samples == 0) {
		...
		if (!f->src || strcasecmp(f->src, "ast_prod")) {
			ast_log_chan(NULL, LOG_WARNING, "no samples for %s\n", pvt->t->name);
		}
	}
...
struct ast_trans_pvt *ast_translator_build_path(struct ast_format *dst, struct ast_format *src)
{
	...
	if (src_index < 0 || dst_index < 0) {
		ast_log_chan(NULL, LOG_WARNING, "No translator path: (%s codec is not valid)\n", src_index < 0 ? "starting" : "ending");
		return NULL;
	}
	...
	while (src_index != dst_index) {
		...
		if (!t) {
			ast_log_chan(NULL, LOG_WARNING, "No translator path from %s to %s\n",
		...
		if (!(cur = newpvt(t, explicit_dst))) {
			ast_log_chan(NULL, LOG_WARNING, "Failed to build translator step from %s to %s\n",
				ast_format_get_name(src), ast_format_get_name(dst));
			ast_translator_free_path(head);
			AST_RWLIST_UNLOCK(&translators);
			return NULL;
...
static void generate_computational_cost(struct ast_translator *t, int seconds)
{
	...
	pvt = newpvt(t, NULL);
	if (!pvt) {
		ast_log_chan(NULL, LOG_WARNING, "Translator '%s' appears to be broken and will probably fail.\n", t->name);
		t->comp_cost = 999999;
		return;
	}
	...
	while (num_samples < seconds * out_rate) {
		...
		if (!f) {
			ast_log_chan(NULL, LOG_WARNING, "Translator '%s' failed to produce a sample frame.\n", t->name);
			destroy(pvt);
			t->comp_cost = 999999;
			return;
		}
...
int __ast_register_translator(struct ast_translator *t, struct ast_module *mod)
{
	...
	if (!src_codec) {
		ast_assert(0);
		ast_log_chan(NULL, LOG_WARNING, "Failed to register translator: unknown source codec %s\n", t->src_codec.name);
		return -1;
	}

	dst_codec = ast_codec_get(t->dst_codec.name, t->dst_codec.type, t->dst_codec.sample_rate);
	if (!dst_codec) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to register translator: unknown destination codec %s\n", t->dst_codec.name);
		return -1;
	}

	if (add_codec2index(src_codec) || add_codec2index(dst_codec)) {
		if (matrix_resize(0)) {
			ast_log_chan(NULL, LOG_WARNING, "Translator matrix can not represent any more translators.  Out of resources.\n");
			return -1;
		}
	...
	if (!mod) {
		ast_log_chan(NULL, LOG_WARNING, "Missing module pointer, you need to supply one\n");
		return -1;
	}

	if (!t->buf_size) {
		ast_log_chan(NULL, LOG_WARNING, "empty buf size, you need to supply one\n");
		return -1;
	}
	if (!t->table_cost && !(t->table_cost = generate_table_cost(src_codec, dst_codec))) {
		ast_log_chan(NULL, LOG_WARNING, "Table cost could not be generated for %s, "
	...
	if (t->src_fmt_index < 0 || t->dst_fmt_index < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Invalid translator path: (%s codec is not valid)\n", t->src_fmt_index < 0 ? "starting" : "ending");
		return -1;
	}
	if (t->src_fmt_index >= cur_max_index) {
		ast_log_chan(NULL, LOG_WARNING, "Source codec %s is larger than cur_max_index\n", t->src_codec.name);
		return -1;
	}

	if (t->dst_fmt_index >= cur_max_index) {
		ast_log_chan(NULL, LOG_WARNING, "Destination codec %s is larger than cur_max_index\n", t->dst_codec.name);
		return -1;
	}
	...
	ast_verb_chan(NULL, 5, "Registered translator '%s' from codec %s to %s, table cost, %d, computational cost %d\n",
...
int ast_unregister_translator(struct ast_translator *t)
{
	...
	AST_RWLIST_TRAVERSE_SAFE_BEGIN(&translators, u, list) {
		if (u == t) {
			...
			ast_verb_chan(NULL, 5, "Unregistered translator '%s' from codec %s to %s\n",
				term_color(tmp, t->name, COLOR_MAGENTA, COLOR_BLACK, sizeof(tmp)),
	...
	struct ast_format **src_fmt_out)
{
	...
	if (ast_format_cap_empty(dst_cap) || ast_format_cap_empty(src_cap)) {
		ast_log_chan(NULL, LOG_ERROR, "Cannot determine best translation path since one capability supports no formats\n");
		return -1;
	}
