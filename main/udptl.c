 ...
 *
 * GABpbx supports T.38 fax passthrough, origination and termination. It does
 * not support gateway operation.
 *
 * UDPTL is handled very much like RTP. It can be reinvited to go directly between
 * the endpoints, without involving GABpbx in the media stream.
 *
 * \b References:
 * - udptl.c
 * - app_fax.c
			    ...
			    unsigned int *len, const uint8_t *data, unsigned int num_octets)
{
	...
	for (octet_idx = 0; ; num_octets -= enclen, octet_idx += enclen) {
		...
		if (enclen + *len > buflen) {
			ast_log_chan(NULL, LOG_ERROR, "UDPTL (%s): Buffer overflow detected (%u + %u > %u)\n",
...
struct ast_frame *ast_udptl_read(struct ast_udptl *udptl)
{
	...
	if (res < 0) {
		if (errno != EAGAIN)
			ast_log_chan(NULL, LOG_WARNING, "UDPTL (%s): read error: %s\n",
	...
	if (udptl_debug_test_addr(&addr)) {
		...
		ast_verb_chan(NULL, 1, "UDPTL (%s): packet from %s (seq %d, len %d)\n",
...
static void calculate_local_max_datagram(struct ast_udptl *udptl)
{
	...
	if (udptl->local_max_ifp == -1) {
		ast_log_chan(NULL, LOG_WARNING, "UDPTL (%s): Cannot calculate local_max_datagram before local_max_ifp has been set.\n",
			LOG_TAG(udptl));
...
static void calculate_far_max_ifp(struct ast_udptl *udptl)
{
	...
	if (udptl->far_max_datagram == -1) {
		ast_log_chan(NULL, LOG_WARNING, "UDPTL (%s): Cannot calculate far_max_ifp before far_max_datagram has been set.\n",
			LOG_TAG(udptl));
		udptl->far_max_ifp = -1;
		return;
	}
...
struct ast_udptl *ast_udptl_new_with_bindaddr(struct ast_sched_context *sched, struct io_context *io, int callbackmode, struct ast_sockaddr *addr)
{
	...
	if (!cfg || !cfg->general) {
		ast_log_chan(NULL, LOG_ERROR, "Could not access global udptl options!\n");
		return NULL;
	}
	...
	if ((udptl->fd = ast_socket_nonblock(ast_sockaddr_is_ipv6(addr) ?
		...
		ast_free(udptl);
		ast_log_chan(NULL, LOG_WARNING, "Unable to allocate socket: %s\n", strerror(errno));
		return NULL;
	}
	...
	for (;;) {
		...
		if (errno != EADDRINUSE && errno != EACCES) {
			ast_log_chan(NULL, LOG_WARNING, "Unexpected bind error: %s\n", strerror(errno));
			close(udptl->fd);
			ast_free(udptl);
			return NULL;
		}
		...
		if (x > cfg->general->end)
			x = cfg->general->start;
		if (x == startplace) {
			ast_log_chan(NULL, LOG_WARNING, "No UDPTL ports remaining\n");
			close(udptl->fd);
			ast_free(udptl);
			return NULL;
		}
...
int ast_udptl_write(struct ast_udptl *s, struct ast_frame *f)
{
	...
	if ((f->frametype != AST_FRAME_MODEM) ||
		...
		ast_log_chan(NULL, LOG_WARNING, "UDPTL (%s): UDPTL can only send T.38 data.\n",
			LOG_TAG(s));
		return -1;
	}
	...
	if ((signed int) len > 0 && !ast_sockaddr_isnull(&s->them)) {
		if (ast_sendto(s->fd, buf, len, 0, &s->them) < 0) {
			ast_log_chan(NULL, LOG_NOTICE, "UDPTL (%s): Transmission error to %s: %s\n",
				LOG_TAG(s), ast_sockaddr_stringify(&s->them), strerror(errno));
		}
		if (udptl_debug_test_addr(&s->them)) {
			ast_verb_chan(NULL, 1, "UDPTL (%s): packet to %s (seq %u, len %u)\n",
...
static int removed_options_handler(const struct aco_option *opt, struct ast_variable *var, void *obj)
{
	if (!strcasecmp(var->name, "t38faxudpec")) {
		ast_log_chan(NULL, LOG_WARNING, "t38faxudpec in udptl.conf is no longer supported.\n");
	} else if (!strcasecmp(var->name, "t38faxmaxdatagram")) {
		ast_log_chan(NULL, LOG_WARNING, "t38faxmaxdatagram in udptl.conf is no longer supported; value is now supplied by T.38 applications.\n");
	}
	return 0;
}
	...
	if (aco_process_config(&cfg_info, reload) == ACO_PROCESS_ERROR) {
		if (!reload) {
			...
			if (aco_set_defaults(&general_option, "general", udptl_cfg->general)) {
				ast_log_chan(NULL, LOG_ERROR, "Failed to load udptl.conf and failed to initialize defaults.\n");
				return;
			}

			ast_log_chan(NULL, LOG_NOTICE, "Could not load udptl config; using defaults\n");
...
static int udptl_pre_apply_config(void) {
...
#ifndef SO_NO_CHECK
	if (cfg->general->nochecksums) {
		ast_log_chan(NULL, LOG_WARNING, "Disabling UDPTL checksums is not supported on this operating system!\n");
	...
	if (cfg->general->use_even_ports && (cfg->general->start & 1)) {
		++cfg->general->start;
		ast_log_chan(NULL, LOG_NOTICE, "Odd numbered udptlstart specified but use_even_ports enabled. udptlstart is now %u\n", cfg->general->start);
	}
	if (cfg->general->start > cfg->general->end) {
		ast_log_chan(NULL, LOG_WARNING, "Unreasonable values for UDPTL start/end ports; defaulting to %s-%s.\n", __stringify(DEFAULT_UDPTLSTART), __stringify(DEFAULT_UDPTLEND));
		cfg->general->start = DEFAULT_UDPTLSTART;
		cfg->general->end = DEFAULT_UDPTLEND;
	}
	if (cfg->general->use_even_ports && (cfg->general->end & 1)) {
		--cfg->general->end;
		ast_log_chan(NULL, LOG_NOTICE, "Odd numbered udptlend specified but use_even_ports enabled. udptlend is now %u\n", cfg->general->end);
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER, "UDPTL",
