...
#define SET_VALUE(param, field, size) \
	...
	if (!res) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to create URI object\n");
		return NULL;
	}
...
struct ast_uri *ast_uri_parse(const char *uri)
{
	...
	if (uriParseUriA(&state, uri) != URI_SUCCESS) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to parse URI %s\n", uri);
					     ...
					     const char *port, const char *secure_port)
{
	...
	if (!strncmp(uri, scheme, len)) {
		...
		if (!with_scheme) {
			ast_log_chan(NULL, LOG_ERROR, "Unable to allocate uri '%s' with "
				"scheme '%s'", uri, scheme);
			return NULL;
		}
