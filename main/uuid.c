...
static void generate_uuid(struct ast_uuid *uuid)
{
	 ...
	 *     coded locations that libuuid assumes for the daemon, GABpbx must be
...
struct ast_uuid *ast_str_to_uuid(char *str)
{
	...
	if (res) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to convert string %s into a UUID\n", str);
		ast_free(uuid);
		return NULL;
	}
	return uuid;
...
void ast_uuid_init(void)
{
	...
	if (dev_urandom_fd < 0) {
		ast_log_chan(NULL, LOG_WARNING, "It appears your system does not have /dev/urandom on it. This\n"
