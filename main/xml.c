...
struct ast_xml_doc *ast_xml_open(char *filename)
{
	...
	ast_log_chan(NULL, LOG_NOTICE, "XSLT support not found. XML documentation may be incomplete.\n");
#endif /* HAVE_LIBXSLT */
...
struct ast_xml_xpath_results *ast_xml_query(struct ast_xml_doc *doc, const char *xpath_str)
{
	...
	if (!(context = xmlXPathNewContext((xmlDoc *) doc))) {
		ast_log_chan(NULL, LOG_ERROR, "Could not create XPath context!\n");
		return NULL;
	}
	result = xmlXPathEvalExpression((xmlChar *) xpath_str, context);
	xmlXPathFreeContext(context);
	if (!result) {
		ast_log_chan(NULL, LOG_WARNING, "Error for query: %s\n", xpath_str);
		return NULL;
	}
	...
	struct ast_xml_namespace_def_vector *namespaces)
{
	...
	for (i = 0; i < AST_VECTOR_SIZE(namespaces); i++) {
		...
		if (xmlXPathRegisterNs(context, (xmlChar *)ns.prefix,
			(xmlChar *)ns.href) != 0) {
			xmlXPathFreeContext(context);
			ast_log_chan(NULL, LOG_ERROR, "Could not register namespace %s:%s\n",
				ns.prefix, ns.href);
			return NULL;
		}
	}
