...
static char *xmldoc_string_wrap(const char *text, int columns)
{
	...
	/* sanity check */
	if (!text || columns <= 0) {
		ast_log_chan(NULL, LOG_WARNING, "Passing wrong arguments while trying to wrap the text\n");
		return NULL;
	}
...
static void xmldoc_string_cleanup(const char *text, struct ast_str **output, int lastspaces, int maintain_newlines)
{
	...
	*output = ast_str_create(textlen);
	if (!(*output)) {
		ast_log_chan(NULL, LOG_ERROR, "Problem allocating output buffer\n");
		return;
	}
...
#define MP(__a) ((multiple ? __a : ""))
	...
	if (ast_strlen_zero(rootname) || ast_strlen_zero(childname)) {
		ast_log_chan(NULL, LOG_WARNING, "Tried to look in XML tree with faulty rootname or childname while creating a syntax.\n");
		return NULL;
	}
	...
	for (; node; node = GOTONEXT(reverse, node)) {
		...
		if (xmldoc_has_inside(node, "argument")) {
			...
			if (!paramnameattr) {
				ast_log_chan(NULL, LOG_WARNING, "Malformed XML %s: no %s name\n", rootname, childname);
				if (syntax) {
...
/*!
 * \internal
 * \brief Get the content of a field (synopsis, description, etc) from an gabpbx document tree node
 *
...
/*!
 * \internal
 * \brief Get the content of a field (synopsis, description, etc) from an gabpbx document tree
 *
...
static char *xmldoc_build_field(const char *type, const char *name, const char *module, const char *var, int raw)
{
	...
	if (ast_strlen_zero(type) || ast_strlen_zero(name)) {
		ast_log_chan(NULL, LOG_ERROR, "Tried to look in XML tree with faulty values.\n");
		return NULL;
	}
	...
	if (!node) {
		AST_RWLIST_UNLOCK(&xmldoc_tree);
		ast_log_chan(NULL, LOG_WARNING, "Couldn't find %s %s in XML documentation"
...
static struct ast_xml_doc_item *ast_xml_doc_item_alloc(const char *name, const char *type)
{
	...
	if (!item) {
		ast_log_chan(NULL, AST_LOG_ERROR, "Failed to allocate memory for ast_xml_doc_item instance\n");
		return NULL;
	}
	...
	if (   !(item->synopsis = ast_str_create(128))
		...
		ast_log_chan(NULL, AST_LOG_ERROR, "Failed to allocate strings for ast_xml_doc_item instance\n");
		goto ast_xml_doc_item_failure;
	}

	if (ast_string_field_init(item, 64)) {
		ast_log_chan(NULL, AST_LOG_ERROR, "Failed to initialize string field for ast_xml_doc_item instance\n");
		goto ast_xml_doc_item_failure;
	}
...
static void build_config_docs(struct ast_xml_node *cur, struct ast_xml_doc_item_list *root)
{
	...
	for (iter = ast_xml_node_get_children(cur); iter; iter = ast_xml_node_get_next(iter)) {
		...
		if (!(item = xmldoc_build_documentation_item(iter, iter_name, ast_xml_node_get_name(iter)))) {
			ast_log_chan(NULL, LOG_ERROR, "Could not build documentation for '%s:%s'\n", ast_xml_node_get_name(iter), iter_name);
...
struct ao2_container *ast_xmldoc_build_documentation(const char *type)
{
	...
	if (!docs) {
		ast_log_chan(NULL, AST_LOG_ERROR, "Failed to create container for xml document item instances\n");
		return NULL;
	}
...
static char *handle_dump_docs(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	dumpdoc = ast_xml_new();
	if (!dumpdoc) {
		ast_log_chan(NULL, LOG_ERROR, "Could not create new XML document\n");
		return CLI_FAILURE;
	}
	...
	if (!dumproot) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Could not create new XML root node\n");
		return CLI_FAILURE;
	}
	...
	AST_LIST_TRAVERSE(&xmldoc_tree, doctree, entry) {
		...
		if (!kids_copy) {
			ast_xml_close(dumpdoc);
			ast_log_chan(NULL, LOG_ERROR, "Could not create copy of XML node list\n");
			return CLI_FAILURE;
		}
	...
	if (!(f = fopen(a->argv[2], "w"))) {
		ast_xml_close(dumpdoc);
		ast_log_chan(NULL, LOG_ERROR, "Could not open file '%s': %s\n", a->argv[2], strerror(errno));
		return CLI_FAILURE;
	}
...
static int xmldoc_load_documentation(int first_time)
{
	...
	if ((cfg = ast_config_load2("gabpbx.conf", "" /* core can't reload */, cnfflags)) && cfg != CONFIG_STATUS_FILEINVALID) {
	...
	if (first_time) {
		...
		/* register function to be run when gabpbx finish. */
	...
	if (globret == GLOB_NOSPACE) {
		ast_log_chan(NULL, LOG_WARNING, "XML load failure, glob expansion of pattern '%s' failed: Not enough memory\n", xmlpattern);
		ast_free(xmlpattern);
		return 1;
	} else if (globret  == GLOB_ABORTED) {
		ast_log_chan(NULL, LOG_WARNING, "XML load failure, glob expansion of pattern '%s' failed: Read error\n", xmlpattern);
		ast_free(xmlpattern);
		return 1;
	}
	ast_free(xmlpattern);
	...
	for (i = 0; i < globbuf.gl_pathc; i++) {
		...
		if (!tmpdoc) {
			ast_log_chan(NULL, LOG_ERROR, "Could not open XML documentation at '%s'\n", globbuf.gl_pathv[i]);
			continue;
		}
		/* Get doc root node and check if it starts with '<docs>' */
		root_node = ast_xml_get_root(tmpdoc);
		if (!root_node) {
			ast_log_chan(NULL, LOG_ERROR, "Error getting documentation root node\n");
			ast_xml_close(tmpdoc);
			continue;
		}
		/* Check root node name for malformed xmls. */
		if (strcmp(ast_xml_node_get_name(root_node), "docs")) {
			ast_log_chan(NULL, LOG_ERROR, "Documentation file is not well formed!\n");
			ast_xml_close(tmpdoc);
			continue;
		}
		doc_tree = ast_calloc(1, sizeof(*doc_tree));
		if (!doc_tree) {
			ast_log_chan(NULL, LOG_ERROR, "Unable to allocate documentation_tree structure!\n");
			ast_xml_close(tmpdoc);
			continue;
		}
		doc_tree->doc = tmpdoc;
