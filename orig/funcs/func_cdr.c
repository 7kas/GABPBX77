...
static struct timeval cdr_retrieve_time(struct ast_channel *chan, const char *time_name)
{
	...
	if (sscanf(tempbuf, "%ld.%ld", &tv_sec, &tv_usec) == 2) {
		...
		ast_log_chan(NULL, AST_LOG_WARNING, "Failed to fully extract '%s' from CDR\n", time_name);
	}
...
static void cdr_read_callback(void *data, struct stasis_subscription *sub, struct stasis_message *message)
{
	...
	if (ast_strlen_zero(payload->arguments)) {
		ast_log_chan(NULL, AST_LOG_WARNING, "%s requires a variable (%s(variable[,option]))\n)",
	...
	if (ast_test_flag(&flags, OPT_FLOAT)
		...
		if (!strcasecmp("start", args.variable)
			...
			if (sscanf(tempbuf, "%ld.%ld", &tv_sec, &tv_usec) != 2) {
				ast_log_chan(NULL, AST_LOG_WARNING, "Unable to parse %s (%s) from the CDR for channel %s\n",
...
static void cdr_prop_write_callback(void *data, struct stasis_subscription *sub, struct stasis_message *message)
{
	...
	if (ast_strlen_zero(payload->arguments)) {
		ast_log_chan(NULL, AST_LOG_WARNING, "%s requires a variable (%s(variable)=value)\n)",
			payload->cmd, payload->cmd);
		return;
	}
	if (ast_strlen_zero(payload->value)) {
		ast_log_chan(NULL, AST_LOG_WARNING, "%s requires a value (%s(variable)=value)\n)",
			payload->cmd, payload->cmd);
		return;
	}
	parse = ast_strdupa(payload->arguments);
	...
	if (!strcasecmp("party_a", args.variable)) {
		...
		ast_log_chan(NULL, AST_LOG_WARNING, "Unknown option %s used with %s\n", args.variable, payload->cmd);
		return;
	}
		    ...
		    char *buf, size_t len)
{
	...
	if (!cdr_read_message_type()) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Failed to manipulate CDR for channel %s: message type not available\n",
	...
	if (!message) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Failed to manipulate CDR for channel %s: unable to create message\n",
			ast_channel_name(chan));
		return -1;
	}
	...
	if (ast_strlen_zero(ast_channel_name(chan))) {
		...
		if (!router) {
			ast_log_chan(NULL, AST_LOG_WARNING, "Failed to manipulate CDR for channel %s: no message router\n",
				ast_channel_name(chan));
			return -1;
		}
	...
	const char *value)
{
	...
	if (ast_strlen_zero(arguments)) {
		ast_log_chan(NULL, LOG_WARNING, "%s requires a variable (%s(variable)=value)\n)",
			cmd, cmd);
		return -1;
	}
	if (!value) {
		ast_log_chan(NULL, LOG_WARNING, "%s requires a value (%s(variable)=value)\n)",
			cmd, cmd);
		return -1;
	}
	...
	if (!strcasecmp(args.variable, "accountcode")) {
		ast_log_chan(NULL, LOG_WARNING, "Using the %s function to set 'accountcode' is deprecated. Please use the CHANNEL function instead.\n",
			cmd);
	...
	if (!strcasecmp(args.variable, "amaflags")) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Using the %s function to set 'amaflags' is deprecated. Please use the CHANNEL function instead.\n",
			cmd);
		if (isdigit(*value)) {
	...
	if (!strcasecmp(args.variable, "peeraccount")) {
		ast_log_chan(NULL, LOG_WARNING, "The 'peeraccount' setting is not supported. Please set the 'accountcode' on the appropriate channel using the CHANNEL function.\n");
		return 0;
	}

	/* The remaining CDR variables are handled by CDR processing code */
	if (!cdr_write_message_type()) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to manipulate CDR for channel %s: message type not available\n",
			ast_channel_name(chan));
		return -1;
	}
	...
	ao2_ref(payload, -1);
	if (!message) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to manipulate CDR for channel %s: unable to create message\n",
			ast_channel_name(chan));
		return -1;
	}
	router = ast_cdr_message_router();
	if (!router) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to manipulate CDR for channel %s: no message router\n",
			ast_channel_name(chan));
		ao2_ref(message, -1);
		return -1;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_DEFAULT, "Call Detail Record (CDR) dialplan functions",
