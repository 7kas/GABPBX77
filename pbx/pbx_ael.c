...
static int pbx_load_module(void)
{
	...
	if (access(rfilename,R_OK) != 0) {
		ast_log_chan(NULL, LOG_NOTICE, "File %s not found; AEL declining load\n", rfilename);
	...
	if (errs == 0 && sem_err == 0) {
		...
		if (ast_compile_ael2(&local_contexts, local_table, parse_tree)) {
			ast_log_chan(NULL, LOG_ERROR, "AEL compile failed! Aborting.\n");
		...
		ast_log_chan(NULL, LOG_ERROR, "Sorry, but %d syntax errors and %d semantic errors were detected. It doesn't make sense to compile.\n", errs, sem_err);
		destroy_pval(parse_tree); /* free up the memory */
		return AST_MODULE_LOAD_DECLINE;
	}
	destroy_pval(parse_tree); /* free up the memory */
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_DEFAULT, "GABpbx Extension Language Compiler",
