...
#include "gabpbx/paths.h"	/* ast_config_AST_CONFIG_DIR */
#include "gabpbx/pbx.h"
#include "gabpbx/config.h"
#include "gabpbx/module.h"
#include "gabpbx/logger.h"
#include "gabpbx/cli.h"
#include "gabpbx/channel.h"	/* AST_MAX_EXTENSION */
...
static char *complete_dialplan_remove_include(struct ast_cli_args *a)
{
	...
	if (a->pos == 3) {		/* "dialplan remove include _X_" */
		if (ast_wrlock_contexts()) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to lock context list\n");
			return NULL;
		}
		...
		if (!dupline) {
			ast_log_chan(NULL, LOG_ERROR, "Out of free memory\n");
			return NULL;
		}
		strsep(&dupline, " ");

		if (ast_rdlock_contexts()) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to lock contexts list\n");
			ast_free(context);
			return NULL;
		}
		...
		ast_unlock_contexts();
		if (!res)
			ast_log_chan(NULL, LOG_WARNING, "%s not included anywhere\n", context);
		ast_free(context);
		return res;
...
static char *complete_dialplan_add_include(struct ast_cli_args *a)
{
	...
	if (a->pos == 3) {		/* 'dialplan add include _X_' (context) ... */
		...
		if (!strlen(context) || strcmp(into, "into")) {
			ast_log_chan(NULL, LOG_ERROR, "bad context %s or missing into %s\n",
				context, into);
			goto error3;
		}
...
static char *handle_cli_dialplan_save(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	case CLI_INIT:
		...
		e->usage =
			...
			"\n"
			"Example: dialplan save                 (/etc/gabpbx/extensions.conf)\n"
...
static char *complete_dialplan_remove_context(struct ast_cli_args *a)
{
	...
	/* try to lock contexts list ... */
	if (ast_rdlock_contexts()) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to lock contexts list\n");
		return NULL;
	}
...
static char *complete_dialplan_add_ignorepat(struct ast_cli_args *a)
{
	if (a->pos == 4)
		...
		if (!dupline) {
			ast_log_chan(NULL, LOG_ERROR, "Malloc failure\n");
			return NULL;
		}
...
static char *complete_dialplan_remove_ignorepat(struct ast_cli_args *a)
{
	...
	if (a->pos == 3) {
		...
		if (!dupline) {
			ast_log_chan(NULL, LOG_WARNING, "Out of free memory\n");
			return NULL;
		}
...
process_extension:
				...
				if (!strcmp(pri,"hint")) {
					...
					ast_log_chan(NULL, LOG_WARNING, "Invalid priority '%s' at line %d of %s\n",
						pri, v->lineno, vfile);
					ast_free(tc);
					continue;
				}
				...
				if (ast_context_add_include2(con, realvalue, registrar)) {
					switch (errno) {
					case ENOMEM:
						ast_log_chan(NULL, LOG_WARNING, "Out of memory for context addition\n");
						break;

					case EBUSY:
						ast_log_chan(NULL, LOG_WARNING, "Failed to lock context(s) list, please try again later\n");
						break;
...
static void startup_event_cb(void *data, struct stasis_subscription *sub, struct stasis_message *message)
{
	...
	ast_log_chan(NULL, LOG_WARNING, "users.conf is deprecated and will be removed in a future version of GABpbx\n");
...
static void pbx_load_users(void)
{
	...
	/*! \todo Remove users.conf support in GABpbx 23 */
	...
	for (cat = ast_category_browse(cfg, NULL); cat ; cat = ast_category_browse(cfg, cat)) {
		...
		if (!ast_strlen_zero(iface)) {
			...
			if (!con) {
				ast_log_chan(NULL, LOG_ERROR, "Can't find/create user context '%s'\n", userscontext);
				return;
			}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_DEFAULT, "Text Extension Configuration",
