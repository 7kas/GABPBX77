...
static int dundi_answer_entity(struct dundi_transaction *trans, struct dundi_ies *ies, char *ccontext)
{
	...
	if (st) {
		...
		if (ast_pthread_create_detached(&lookupthread, NULL, dundi_query_thread, st)) {
			...
			trans->thread = 0;
			ast_log_chan(NULL, LOG_WARNING, "Unable to create thread!\n");
			ast_free(st);
...
static int dundi_prop_precache(struct dundi_transaction *trans, struct dundi_ies *ies, char *ccontext)
{
	...
	for (x=0;x<ies->anscount;x++) {
		if (trans->parent->respcount < trans->parent->maxcount) {
			...
			ast_log_chan(NULL, LOG_NOTICE, "Dropping excessive answers in precache for %s@%s\n",
...
static int dundi_encrypt(struct dundi_transaction *trans, struct dundi_packet *pack)
{
	...
	if ((ied.pos + bytes) >= sizeof(ied.buf)) {
		ast_log_chan(NULL, LOG_NOTICE, "Final packet too large!\n");
		return -1;
	}
...
static int check_key(struct dundi_peer *peer, unsigned char *newkey, unsigned char *newsig, uint32_t keycrc32)
{
	...
	if (!key) {
		ast_log_chan(NULL, LOG_NOTICE, "Unable to find key '%s' to decode shared key from '%s'\n",
			peer->outkey, ast_eid_to_str(eid_str, sizeof(eid_str), &peer->eid));
		return -1;
	}
	...
	if (!skey) {
		ast_log_chan(NULL, LOG_NOTICE, "Unable to find key '%s' to verify shared key from '%s'\n",
			peer->inkey, ast_eid_to_str(eid_str, sizeof(eid_str), &peer->eid));
		return -1;
	}
	...
	if (res != 16) {
		if (res >= 0)
			ast_log_chan(NULL, LOG_NOTICE, "Weird, key decoded to the wrong size (%d)\n", res);
		return 0;
	}
...
static int handle_command_response(struct dundi_transaction *trans, struct dundi_hdr *hdr, int datalen, int encrypted)
{
	...
	if (datalen) {
		...
		if (dundi_parse_ies(&ies, bufcpy, datalen) < 0) {
			ast_log_chan(NULL, LOG_WARNING, "Failed to parse DUNDI information elements!\n");
			goto return_cleanup;
		}
	}
	switch(cmd) {
		...
		if (!peer || !peer->dynamic) {
			...
			if (hasauth) {
				...
				if (ast_sockaddr_cmp(&peer->addr, &trans->addr)) {
					ast_verb_chan(NULL, 3, "Registered DUNDi peer '%s' at '%s'\n",
						ast_eid_to_str(eid_str, sizeof(eid_str), &peer->eid),
				...
				if (trans->parent && trans->parent->dr) {
					...
					for (x=0;x<ies.anscount;x++) {
						if (trans->parent->respcount < trans->parent->maxcount) {
							...
							ast_log_chan(NULL, LOG_NOTICE, "Dropping excessive answers to request for %s@%s\n",
								trans->parent->number, trans->parent->dcontext);
					}
	...
	case DUNDI_COMMAND_REGRESPONSE:
		...
		if (ies.cause < 1) {
			...
			if (!hasauth) {
				ast_log_chan(NULL, LOG_NOTICE, "Reponse to register not authorized!\n");
				if (!final) {
	...
	AST_LIST_TRAVERSE(&trans->packets, pack, list) {
		if ((pack->h->oseqno + 1) % 255 == iseqno) {
			...
			if (!AST_LIST_EMPTY(&trans->lasttrans)) {
				ast_log_chan(NULL, LOG_WARNING, "Whoa, there was still a last trans?\n");
...
static int dundi_xmit(struct dundi_packet *pack)
{
	...
	if (res < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to transmit to '%s': %s\n",
	...
	if (ast_test_flag(trans, FLAG_ISREG | FLAG_ISQUAL | FLAG_STOREHIST)) {
		AST_LIST_TRAVERSE(&peers, peer, list) {
			...
			if (peer->qualtrans == trans) {
				if (fromtimeout) {
					if (peer->lastms > -1)
						ast_log_chan(NULL, LOG_NOTICE, "Peer '%s' has become UNREACHABLE!\n", ast_eid_to_str(eid_str, sizeof(eid_str), &peer->eid));
					peer->lastms = -1;
				} else {
					...
					if (ms < peer->maxms) {
						if ((peer->lastms >= peer->maxms) || (peer->lastms < 0))
							ast_log_chan(NULL, LOG_NOTICE, "Peer '%s' has become REACHABLE!\n", ast_eid_to_str(eid_str, sizeof(eid_str), &peer->eid));
					} else if (peer->lastms < peer->maxms) {
						ast_log_chan(NULL, LOG_NOTICE, "Peer '%s' has become TOO LAGGED (%d ms)\n", ast_eid_to_str(eid_str, sizeof(eid_str), &peer->eid), ms);
					}
					peer->lastms = ms;
				}
				peer->qualtrans = NULL;
...
static int dundi_rexmit(const void *data)
{
	...
	if (pack->retrans < 1) {
		...
		if (!ast_test_flag(pack->parent, FLAG_ISQUAL)) {
			ast_log_chan(NULL, LOG_NOTICE, "Max retries exceeded to host '%s' msg %d on call %d\n",
...
static int dundi_send(struct dundi_transaction *trans, int cmdresp, int flags, int final, struct dundi_ie_data *ied)
{
	...
	if (pack) {
		...
		if (!res)
			res = dundi_xmit(pack);
		if (res)
			ast_log_chan(NULL, LOG_NOTICE, "Failed to send packet to '%s'\n", ast_eid_to_str(eid_str, sizeof(eid_str), &trans->them_eid));
...
static int do_autokill(const void *data)
{
	...
	char eid_str[20];
	ast_log_chan(NULL, LOG_NOTICE, "Transaction to '%s' took too long to ACK, destroying\n",
		ast_eid_to_str(eid_str, sizeof(eid_str), &trans->them_eid));
	trans->autokillid = -1;
...
static int dundi_discover(struct dundi_transaction *trans)
{
	...
	int x;
	if (!trans->parent) {
		ast_log_chan(NULL, LOG_WARNING, "Tried to discover a transaction with no parent?!?\n");
		return -1;
	}
	memset(&ied, 0, sizeof(ied));
...
static int dundi_query(struct dundi_transaction *trans)
{
	...
	int x;
	if (!trans->parent) {
		ast_log_chan(NULL, LOG_WARNING, "Tried to query a transaction with no parent?!?\n");
		return -1;
	}
	memset(&ied, 0, sizeof(ied));
	dundi_ie_append_short(&ied, DUNDI_IE_VERSION, DUNDI_DEFAULT_VERSION);
...
static int precache_transactions(struct dundi_request *dr, struct dundi_mapping *maps, int mapcount, int *expiration, int *foundanswers)
{
	...
	AST_LIST_TRAVERSE(&dr->trans, trans, parentlist) {
		if (trans->thread)
			ast_log_chan(NULL, LOG_WARNING, "This shouldn't happen, really...\n");
		trans->thread = 1;
	}
	AST_LIST_UNLOCK(&peers);
...
static int dundi_lookup_internal(struct dundi_result *result, int maxret, struct ast_channel *chan, const char *dcontext, const char *number, int ttl, int blockempty, struct dundi_hint_metadata *hmd, int *expiration, int cbypass, int modeselect, dundi_eid *skip, dundi_eid *avoid[], int direct[])
{
	...
	memset(&dr, 0, sizeof(dr));
	if (pipe(dr.pfds)) {
		ast_log_chan(NULL, LOG_WARNING, "pipe failed: %s\n" , strerror(errno));
		return -1;
	}
	dr.dr = result;
	dr.hmd = hmd;
...
static void dundi_precache_full(void)
{
	...
	AST_LIST_TRAVERSE(&mappings, cur, list) {
		ast_log_chan(NULL, LOG_NOTICE, "Should precache context '%s'\n", cur->dcontext);
		ast_rdlock_contexts();
		con = NULL;
...
static int dundi_precache_internal(const char *context, const char *number, int ttl, dundi_eid *avoids[])
{
	...
	if (foundanswers) {
		if (dr.expiration > 0)
			...
			ast_log_chan(NULL, LOG_NOTICE, "Weird, expiration = %d, but need to precache for %s@%s?!\n", dr.expiration, dr.number, dr.dcontext);
	}
	start = ast_tvnow();
...
static int dundifunc_read(struct ast_channel *chan, const char *cmd, char *num, char *buf, size_t len)
{
	...
	if (ast_strlen_zero(num)) {
		ast_log_chan(NULL, LOG_WARNING, "DUNDILOOKUP requires an argument (number)\n");
		return -1;
	}
...
static int dundi_query_read(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "DUNDIQUERY requires an argument (number)\n");
		return -1;
	}

	if (!chan) {
		ast_log_chan(NULL, LOG_ERROR, "DUNDIQUERY can not be used without a channel!\n");
		return -1;
	}
...
static int dundi_result_read(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "DUNDIRESULT requires an argument (id and resultnum)\n");
		goto finish;
	}

	if (!chan) {
		ast_log_chan(NULL, LOG_ERROR, "DUNDRESULT can not be used without a channel!\n");
		goto finish;
	}
	...
	if (ast_strlen_zero(args.id)) {
		ast_log_chan(NULL, LOG_ERROR, "A result ID must be provided to DUNDIRESULT\n");
		goto finish;
	}

	if (ast_strlen_zero(args.resultnum)) {
		ast_log_chan(NULL, LOG_ERROR, "A result number must be given to DUNDIRESULT!\n");
		goto finish;
	}
	...
	if (!datastore) {
		ast_log_chan(NULL, LOG_WARNING, "No DUNDi results found for query ID '%s'\n", args.id);
		goto finish;
	}
	...
	if (sscanf(args.resultnum, "%30u", &num) != 1) {
		ast_log_chan(NULL, LOG_ERROR, "Invalid value '%s' for resultnum to DUNDIRESULT!\n",
			args.resultnum);
		goto finish;
	}
	...
	if (num && drds->num_results > 0 && num <= drds->num_results) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Result number %u is not valid for DUNDi query results for ID %s!\n", num, args.id);
...
static void build_mapping(const char *name, const char *value)
{
	...
	if ((x == 1) && ast_strlen_zero(fields[0])) {
		...
		if ((sscanf(fields[1], "%30d", &map->_weight) == 1) && (map->_weight >= 0) && (map->_weight <= MAX_WEIGHT)) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Invalid weight '%s' specified, deleting entry '%s/%s'\n", fields[1], map->dcontext, map->lcontext);
		}
		for (y = 4;y < x; y++) {
			if (!strcasecmp(fields[y], "nounsolicited"))
				...
				ast_log_chan(NULL, LOG_WARNING, "Don't know anything about option '%s'\n", fields[y]);
		}
	} else
		ast_log_chan(NULL, LOG_WARNING, "Expected at least %d arguments in map, but got only %d\n", 4, x);
}
...
static int do_register(const void *data)
{
	...
	if (peer->regtrans) {
		...
		ast_log_chan(NULL, LOG_NOTICE, "Unable to create new transaction for registering to '%s'!\n", ast_eid_to_str(eid_str, sizeof(eid_str), &peer->eid));
...
static void build_peer(dundi_eid *eid, struct ast_variable *v, int *globalpcmode)
{
		...
		if (!strcasecmp(v->name, "inkey")) {
			...
			if (!strcasecmp(v->value, "dynamic")) {
				...
				if (ast_sockaddr_resolve(&addrs, v->value, PARSE_PORT_FORBID, AF_UNSPEC) > 0) {
					...
					ast_log_chan(NULL, LOG_WARNING, "Unable to find host '%s' at line %d\n", v->value, v->lineno);
					peer->dead = 1;
				}
			}
			...
			if (!ast_str_to_eid(&testeid, v->value))
				...
				ast_log_chan(NULL, LOG_WARNING, "'%s' is not a valid DUNDi Entity Identifier at line %d\n", v->value, v->lineno);
			...
			if (!strcasecmp(v->value, "primary"))
				...
				ast_log_chan(NULL, LOG_WARNING, "'%s' is not a valid order, should be primary, secondary, tertiary or quartiary at line %d\n", v->value, v->lineno);
			}
			...
			if (!strcasecmp(v->value, "no")) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Qualification of peer '%s' should be 'yes', 'no', or a number of milliseconds at line %d of dundi.conf\n",
			...
			if (!strcasecmp(v->value, "inbound"))
				...
				ast_log_chan(NULL, LOG_WARNING, "Unknown model '%s', should be 'none', 'outbound', 'inbound', or 'symmetric' at line %d\n",
					v->value, v->lineno);
			}
			...
			if (!strcasecmp(v->value, "inbound"))
				...
				ast_log_chan(NULL, LOG_WARNING, "Unknown pcmodel '%s', should be 'none', 'outbound', 'inbound', or 'symmetric' at line %d\n",
					v->value, v->lineno);
			}
		}
	}
	...
	if (!peer->model && !peer->pcmodel) {
		ast_log_chan(NULL, LOG_WARNING, "Peer '%s' lacks a model or pcmodel, discarding!\n",
			ast_eid_to_str(eid_str, sizeof(eid_str), &peer->eid));
		peer->dead = 1;
	} else if ((peer->model & DUNDI_MODEL_INBOUND) && (peer->pcmodel & DUNDI_MODEL_OUTBOUND)) {
		ast_log_chan(NULL, LOG_WARNING, "Peer '%s' may not be both inbound/symmetric model and outbound/symmetric precache model, discarding!\n",
			ast_eid_to_str(eid_str, sizeof(eid_str), &peer->eid));
		peer->dead = 1;
	} else if ((peer->model & DUNDI_MODEL_OUTBOUND) && (peer->pcmodel & DUNDI_MODEL_INBOUND)) {
		ast_log_chan(NULL, LOG_WARNING, "Peer '%s' may not be both outbound/symmetric model and inbound/symmetric precache model, discarding!\n",
			ast_eid_to_str(eid_str, sizeof(eid_str), &peer->eid));
		peer->dead = 1;
	} else if (!AST_LIST_EMPTY(&peer->include) && !(peer->model & DUNDI_MODEL_OUTBOUND) && !(peer->pcmodel & DUNDI_MODEL_INBOUND)) {
		ast_log_chan(NULL, LOG_WARNING, "Peer '%s' is supposed to be included in outbound searches but isn't an outbound peer or inbound precache!\n",
			ast_eid_to_str(eid_str, sizeof(eid_str), &peer->eid));
	} else if (!AST_LIST_EMPTY(&peer->permit) && !(peer->model & DUNDI_MODEL_INBOUND) && !(peer->pcmodel & DUNDI_MODEL_OUTBOUND)) {
		ast_log_chan(NULL, LOG_WARNING, "Peer '%s' is supposed to have permission for some inbound searches but isn't an inbound peer or outbound precache!\n",
			ast_eid_to_str(eid_str, sizeof(eid_str), &peer->eid));
	} else {
	...
	if (x < res) {
		...
		if (!strcasecmp(results[x].tech, "SIP") || !strcasecmp(results[x].tech, "PJSIP")) {
			...
			if (strcasecmp(results[x].tech, "SIP")) {
				ast_log_chan(NULL, LOG_WARNING, "%s cannot be specified by DUNDi peers (peer should use SIP for DUNDi lookups instead)\n", results[x].tech);
			}
		...
		if (outgoing_sip_tech == DUNDI_PROTO_PJSIP) {
			...
			if (ast_strlen_zero(pjsip_outgoing_endpoint)) {
				ast_log_chan(NULL, LOG_WARNING, "PJSIP calls require an endpoint to be specified explicitly (use the pjsip_outgoing_endpoint option in dundi.conf)\n");
				return -1;
			}
			/* Take IP/number and turn it into sip:number@IP */
			if (ast_strlen_zero(ip)) {
				ast_log_chan(NULL, LOG_WARNING, "PJSIP destination is empty?\n");
				return -1;
			}
			number = strsep(&ip, "/");
...
static void set_host_ipaddr(struct ast_sockaddr *sin)
{
	...
	if (gethostname(hn, sizeof(hn) - 1) < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to get host name!\n");
		return;
	}
...
static int set_config(char *config_file, struct ast_sockaddr *sin, int reload, struct ast_sockaddr *sin2)
{
	...
	if (ast_eid_is_empty(&ast_eid_default)) {
		ast_log_chan(NULL, LOG_WARNING, "Entity ID is not set.\n");
	}
		...
		if (!strcasecmp(v->name, "port")){
			...
			if (get_ipaddress(bind_addr, sizeof(bind_addr), v->value, AF_UNSPEC) == 0) {
				if (!ast_sockaddr_parse(sin, bind_addr, 0)) {
					ast_log_chan(NULL, LOG_WARNING, "Invalid host/IP '%s'\n", v->value);
				}
			}
			...
			if ((sscanf(v->value, "%30d", &x) == 1) && (x > 0) && (x < DUNDI_DEFAULT_TTL)) {
				...
				ast_log_chan(NULL, LOG_WARNING, "'%s' is not a valid TTL at line %d, must be number from 1 to %d\n",
			...
			if (!ast_str_to_eid(&testeid, v->value))
				...
				ast_log_chan(NULL, LOG_WARNING, "Invalid global endpoint identifier '%s' at line %d\n", v->value, v->lineno);
			...
			if (outgoing_tech != DUNDI_PROTO_SIP && outgoing_tech != DUNDI_PROTO_PJSIP) {
				ast_log_chan(NULL, LOG_WARNING, "outgoing_sip_tech must be SIP or PJSIP\n");
			} else {
			...
			if ((sscanf(v->value, "%30d", &x) == 1)) {
				...
				ast_log_chan(NULL, LOG_WARNING, "'%s' is not a valid cache time at line %d. Using default value '%d'.\n",
	...
	if (last_port == 0) {
		...
		ast_log_chan(NULL, LOG_WARNING, "change to port ignored until next gabpbx re-start\n");
	}
	...
	while(cat) {
		if (strcasecmp(cat, "general") && strcasecmp(cat, "mappings")) {
			...
			if (!ast_str_to_eid(&testeid, cat))
				...
				ast_log_chan(NULL, LOG_NOTICE, "Ignoring invalid EID entry '%s'\n", cat);
		}
...
static int load_module(void)
{
	...
	if (!ast_sockaddr_isnull(&sin2)) {
		if ((ast_sockaddr_is_ipv4(&sin) == ast_sockaddr_is_ipv4(&sin2)) || (ast_sockaddr_is_ipv6(&sin) == ast_sockaddr_is_ipv6(&sin2))) {
			ast_log_chan(NULL, LOG_ERROR, "bindaddr & bindaddr2 should be different IP protocols.\n");
			goto declined;
		}
		...
		if (ast_sockaddr_is_ipv4(&sin)) {
			if (ast_bind(netsocket, &sin)) {
				ast_log_chan(NULL, LOG_ERROR, "Unable to bind to %s : %s\n",
	...
	if (ast_register_switch(&dundi_switch))
		ast_log_chan(NULL, LOG_ERROR, "Unable to register DUNDi switch\n");
	...
	ast_verb_chan(NULL, 2, "DUNDi Ready and Listening on %s\n", ast_sockaddr_stringify(&sin));
	if (!ast_sockaddr_isnull(&sin2))
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_DEFAULT, "Distributed Universal Number Discovery (DUNDi)",
