...
static int lua_pbx_exec(lua_State *L)
{
	...
	ast_verb_chan(NULL, 3, "Executing [%s@%s:%d] %s(\"%s\", \"%s\")\n",
...
/*!
 * \brief Create the 'application' metatable, used to execute gabpbx
...
static int lua_error_function(lua_State *L)
{
	...
	/* pass number arguments right through back to gabpbx*/
	if (lua_isnumber(L, -1)) {
		return 1;
	}
...
static lua_State *lua_get_state(struct ast_channel *chan)
{
	...
	if (!chan) {
		L = luaL_newstate();
		if (!L) {
			ast_log_chan(NULL, LOG_ERROR, "Error allocating lua_State, no memory\n");
			return NULL;
		}

		if (lua_load_extensions(L, NULL)) {
			const char *error = lua_tostring(L, -1);
			ast_log_chan(NULL, LOG_ERROR, "Error loading extensions.lua: %s\n", error);
			lua_close(L);
			return NULL;
		}
		return L;
		...
		if (!datastore) {
			...
			if (!datastore) {
				ast_log_chan(NULL, LOG_ERROR, "Error allocation channel datastore for lua_State\n");
				return NULL;
			}
			...
			if (lua_load_extensions(L, chan)) {
				const char *error = lua_tostring(L, -1);
				ast_log_chan(NULL, LOG_ERROR, "Error loading extensions.lua for %s: %s\n", ast_channel_name(chan), error);
...
static int exists(struct ast_channel *chan, const char *context, const char *exten, int priority, const char *callerid, const char *data)
{
	...
	if (!u) {
		ast_log_chan(NULL, LOG_ERROR, "Error adjusting use count, probably could not allocate memory\n");
		return 0;
	}
...
static int exec(struct ast_channel *chan, const char *context, const char *exten, int priority, const char *callerid, const char *data)
{
	...
	if (!lua_find_extension(L, context, exten, priority, &exists, 1)) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Could not find extension %s in context %s\n", exten, context);
		if (!chan) lua_close(L);
		ast_module_user_remove(u);
		return -1;
	}
	...
	if (res) {
		if (res == LUA_ERRRUN) {
			...
			if (lua_isnumber(L, -1)) {
				...
				const char *error = lua_tostring(L, -1);
				ast_log_chan(NULL, LOG_ERROR, "Error executing lua extension: %s\n", error);
			}
		} else if (res == LUA_ERRERR) {
			res = -1;
			ast_log_chan(NULL, LOG_ERROR, "Error in the lua error handler (this is probably a bug in pbx_lua)\n");
...
static int lua_find_extension(lua_State *L, const char *context, const char *exten, int priority, ast_switch_f *func, int push_func)
{
	...
	lua_getglobal(L, "extensions");
	if (lua_isnil(L, -1)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to find 'extensions' table in extensions.lua\n");
		lua_pop(L, 1);
		return 0;
	}
...
static int load_module(void)
{
	...
	if (ast_register_switch(&lua_switch)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to register Lua PBX switch\n");
		return AST_MODULE_LOAD_FAILURE;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS, "Lua PBX Switch",
