	...
	if (var) {
		...
		if (!ast_strlen_zero(app)) {
			...
			if (a) {
				...
				ast_log_chan(NULL, LOG_NOTICE, "No such application '%s' for extension '%s' in context '%s'\n", app, exten, context);
		} else {
			ast_log_chan(NULL, LOG_WARNING, "No application specified for realtime extension '%s' in context '%s'\n", exten, context);
		}
	}
	return res;
}
...
AST_MODULE_INFO_STANDARD_EXTENDED(GABPBX_GPL_KEY, "Realtime Switch");
