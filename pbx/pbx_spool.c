...
static void parse_line(char *line, unsigned int lineno, struct outgoing *o)
{
	...
	c = strchr(line, ':');
	if (!c) {
		ast_log_chan(NULL, LOG_NOTICE, "Syntax error at line %d of %s\n", lineno, o->fn);
		return;
	}
	*c = '\0';
	...
	if (!strcasecmp(line, "channel")) {
		...
		if ((c2 = strchr(c, '/'))) {
			*c2 = '\0';
			...
			ast_log_chan(NULL, LOG_NOTICE, "Channel should be in form Tech/Dest at line %d of %s\n", lineno, o->fn);
		}
		...
		if (sscanf(c, "%30d", &o->maxretries) != 1) {
			ast_log_chan(NULL, LOG_WARNING, "Invalid max retries at line %d of %s\n", lineno, o->fn);
			o->maxretries = 0;
		}
		...
		if ((sscanf(c, "%30d", &o->priority) != 1) || (o->priority < 1)) {
			ast_log_chan(NULL, LOG_WARNING, "Invalid priority at line %d of %s\n", lineno, o->fn);
			o->priority = 1;
		}
	} else if (!strcasecmp(line, "retrytime")) {
		if ((sscanf(c, "%30d", &o->retrytime) != 1) || (o->retrytime < 1)) {
			ast_log_chan(NULL, LOG_WARNING, "Invalid retrytime at line %d of %s\n", lineno, o->fn);
			o->retrytime = 300;
		}
	} else if (!strcasecmp(line, "waittime")) {
		if ((sscanf(c, "%30d", &o->waittime) != 1) || (o->waittime < 1)) {
			ast_log_chan(NULL, LOG_WARNING, "Invalid waittime at line %d of %s\n", lineno, o->fn);
			o->waittime = 45;
		}
		...
		if (sscanf(c, "%30ld", &o->callingpid) != 1) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to retrieve calling PID!\n");
			o->callingpid = 0;
		}
		...
		if (c2) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Malformed \"%s\" argument.  Should be \"%s: variable=value\"\n", line, line);
		}
		...
		ast_log_chan(NULL, LOG_WARNING, "Unknown keyword '%s' at line %d of %s\n", line, lineno, o->fn);
	}
}
...
static int apply_outgoing(struct outgoing *o, FILE *f)
{
	...
	while (fgets(buf, sizeof(buf), f)) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Skipping extremely long line at line %d of %s\n", lineno, o->fn);
	...
	if (ast_strlen_zero(o->tech)
		...
		ast_log_chan(NULL, LOG_WARNING, "At least one of app or extension must be specified, "
...
static void safe_append(struct outgoing *o, time_t now, char *s)
{
	...
	if (utime(o->fn, &tbuf)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to set utime on %s: %s\n", o->fn, strerror(errno));
	}
}
...
static int remove_from_queue(struct outgoing *o, const char *status)
{
	...
	if (ast_mkdir(qdonedir, 0777)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to create queue directory %s -- outgoing spool archiving disabled\n", qdonedir);
		unlink(o->fn);
		return -1;
	}
...
static void *attempt_thread(void *data)
{
	...
	if (!ast_strlen_zero(o->app)) {
		ast_verb_chan(NULL, 3, "Attempting call on %s/%s for application %s(%s) (Retry %d)\n", o->tech, o->dest, o->app, o->data, o->retries);
		...
		ast_verb_chan(NULL, 3, "Attempting call on %s/%s for %s@%s:%d (Retry %d)\n", o->tech, o->dest, o->exten, o->context,o->priority, o->retries);
	...
	if (res) {
		ast_log_chan(NULL, LOG_NOTICE, "Call failed to go through, reason (%d) %s\n", reason, ast_channel_reason2str(reason));
		if (o->retries >= o->maxretries + 1) {
			/* Max retries exceeded */
			ast_log_chan(NULL, LOG_NOTICE, "Queued call to %s/%s expired without completion after %d attempt%s\n", o->tech, o->dest, o->retries - 1, ((o->retries - 1) != 1) ? "s" : "");
		...
		ast_log_chan(NULL, LOG_NOTICE, "Call completed to %s/%s\n", o->tech, o->dest);
...
static void launch_service(struct outgoing *o)
{
	...
	if ((ret = ast_pthread_create_detached(&t, NULL, attempt_thread, o))) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to create thread :( (returned error: %d)\n", ret);
		free_outgoing(o);
	}
}
...
static int scan_service(const char *fn, time_t now)
{
...
#endif
		{
			ast_log_chan(NULL, LOG_WARNING, "Unable to open %s: '%s'(%d), deleting\n",
	...
	fclose(f);
	if (res) {
		ast_log_chan(NULL, LOG_WARNING, "Invalid file contents in %s, deleting\n", o->fn);
		remove_from_queue(o, "Failed");
		free_outgoing(o);
		return -1;
	}
...
static void queue_file(const char *filename, time_t when)
{
	...
	if (when == 0) {
		if (stat(filename, &st)) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to stat %s: %s\n", filename, strerror(errno));
			return;
		}
...
static void *scan_thread(void *unused)
{
	...
	if (inotify_fd < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to initialize "
#ifdef HAVE_INOTIFY
			"inotify(7)"
#else
			"kqueue(2)"
	...
	if (!(dir = opendir(qdir))) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to open directory %s: %s\n", qdir, strerror(errno));
		return NULL;
	}
	...
	if (kevent(inotify_fd, &kev, 1, &event, 1, &nowait) < 0 && errno != 0) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to watch directory %s: %s\n", qdir, strerror(errno));
	}
#endif
	now = time(NULL);
			...
			if ((res = poll(&pfd, 1, waittime)) > 0 && (stage = 1) &&
				...
				for (iev = (void *) buf; res >= sizeof(*iev); iev = (struct inotify_event *) (((char *) iev) + len)) {
					...
					if (iev->mask & IN_CREATE) {
						...
						ast_log_chan(NULL, LOG_ERROR, "Unexpected event %d for file '%s'\n", (int) iev->mask, iev->name);
					}
...
static void *scan_thread(void *unused)
{
	...
	for (;;) {
		...
		if (stat(qdir, &st)) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to stat %s\n", qdir);
			continue;
		}
		...
		if (!(dir = opendir(qdir))) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to open directory %s: %s\n", qdir, strerror(errno));
			continue;
		}
		...
		while ((de = readdir(dir))) {
			...
			if (st.st_mtime <= now) {
				...
				if (res > 0) {
					...
					ast_log_chan(NULL, LOG_WARNING, "Failed to scan service '%s'\n", fn);
				} else if (!next) {
...
static int load_module(void)
{
	...
	if (ast_mkdir(qdir, 0777)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to create queue directory %s -- outgoing spool disabled\n", qdir);
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Outgoing Spool Support");
