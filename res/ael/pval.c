...
/*! \file
 *
 * \brief Compile symbolic GABpbx Extension Logic into GABpbx extensions, version 2.
 *
 */
...
#include "gabpbx/pval.h"
#include "gabpbx/ael_structs.h"
#ifdef AAL_ARGCHECK
#include "gabpbx/argdesc.h"
#endif
...
void ael2_print(char *fname, pval *tree)
{
	...
	if ( !fin ) {
		ast_log_chan(NULL, LOG_ERROR, "Couldn't open %s for writing.\n", fname);
		return;
	}
...
static void check_macro_returns(pval *macro)
{
	...
	if (!macro->u3.macro_statements)
	{
		...
		ast_log_chan(NULL, LOG_WARNING, "Warning: file %s, line %d-%d: The macro %s is empty! I will insert a return.\n",
	...
	for (i=macro->u3.macro_statements; i; i=i->next) {
		...
		if (i->next == NULL) {
			if (i->type != PV_RETURN) {
				pval *z = calloc(1, sizeof(struct pval));
				ast_log_chan(NULL, LOG_WARNING, "Warning: file %s, line %d-%d: The macro %s does not end with a return; I will insert one.\n",
						macro->filename, macro->startline, macro->endline, macro->u1.str);
...
static int extension_matches(pval *here, const char *exten, const char *pattern)
{
	...
	if (pattern[0] == '_') {
		...
		if ( strlen(pattern)*5 >= 2000 ) /* safety valve */ {
			ast_log_chan(NULL, LOG_ERROR,"Error: The pattern %s is way too big. Pattern matching cancelled.\n",
					pattern);
			return 0;
		}
		...
		*r++ = '?';
		for (p=pattern+1; *p; p++) {
			...
			case '[':
				...
				*r++ = ']';
				if ( *p != ']') {
					ast_log_chan(NULL, LOG_WARNING, "Warning: file %s, line %d-%d: The extension pattern '%s' is missing a closing bracket \n",
		...
		*r++ = *p++; /* put in the closing null */
		...
		if ( err1 ) {
			...
			regfree(&preg);
			ast_log_chan(NULL, LOG_WARNING, "Regcomp of %s failed, error code %d\n",
					reg1, err1);
			return 0;
		}
		...
		if ( err1 ) {
			/* ast_log_chan(NULL, LOG_NOTICE,"*****************************[%d]Extension %s did not match %s(%s)\n",
			   err1,exten, pattern, reg1); */
			return 0; /* no match */
		} else {
			/* ast_log_chan(NULL, LOG_NOTICE,"*****************************Extension %s matched %s\n",
			   exten, pattern); */
			return 1;
		}
	}
...
static void check_expr2_input(pval *expr, char *str)
{
	...
	if ( !strncmp(str+spaces,"$[",2) ) {
		ast_log_chan(NULL, LOG_WARNING, "Warning: file %s, line %d-%d: The expression '%s' is redundantly wrapped in '$[ ]'. \n",
...
static void check_includes(pval *includes)
{
	...
	for (p4=includes->u1.list; p4; p4=p4->next) {
		...
		if (!that_other_context && strcmp(incl_context, "parkedcalls") != 0) {
			ast_log_chan(NULL, LOG_WARNING, "Warning: file %s, line %d-%d: The included context '%s' cannot be found.\n\
...
static void check_timerange(pval *p)
{
	...
	e = strchr(times, '-');
	if (!e) {
		ast_log_chan(NULL, LOG_WARNING, "Warning: file %s, line %d-%d: The time range format (%s) requires a '-' surrounded by two 24-hour times of day!\n",
	...
	*e = '\0';
	...
	while (*e && !isdigit(*e))
		e++;
	if (!*e) {
		ast_log_chan(NULL, LOG_WARNING, "Warning: file %s, line %d-%d: The time range format (%s) is missing the end time!\n",
				p->filename, p->startline, p->endline, p->u1.str);
		warns++;
	}
	if (sscanf(times, "%2d:%2d", &s1, &s2) != 2) {
		ast_log_chan(NULL, LOG_WARNING, "Warning: file %s, line %d-%d: The start time (%s) isn't quite right!\n",
				p->filename, p->startline, p->endline, times);
		warns++;
	}
	if (sscanf(e, "%2d:%2d", &e1, &e2) != 2) {
		ast_log_chan(NULL, LOG_WARNING, "Warning: file %s, line %d-%d: The end time (%s) isn't quite right!\n",
				p->filename, p->startline, p->endline, times);
		warns++;
	}
	...
	s1 = s1 * 30 + s2/2;
	if ((s1 < 0) || (s1 >= 24*30)) {
		ast_log_chan(NULL, LOG_WARNING, "Warning: file %s, line %d-%d: The start time (%s) is out of range!\n",
				p->filename, p->startline, p->endline, times);
		warns++;
	}
	e1 = e1 * 30 + e2/2;
	if ((e1 < 0) || (e1 >= 24*30)) {
		ast_log_chan(NULL, LOG_WARNING, "Warning: file %s, line %d-%d: The end time (%s) is out of range!\n",
...
static void check_dow(pval *DOW)
{
	...
	if (s >= 7) {
		ast_log_chan(NULL, LOG_WARNING, "Warning: file %s, line %d-%d: The day (%s) must be one of 'sun', 'mon', 'tue', 'wed', 'thu', 'fri', or 'sat'!\n",
	...
	if (c) {
		...
		if (e >= 7) {
			ast_log_chan(NULL, LOG_WARNING, "Warning: file %s, line %d-%d: The end day (%s) must be one of 'sun', 'mon', 'tue', 'wed', 'thu', 'fri', or 'sat'!\n",
...
static void check_day(pval *DAY)
{
	...
	if (sscanf(day, "%2d", &s) != 1) {
		ast_log_chan(NULL, LOG_WARNING, "Warning: file %s, line %d-%d: The start day of month (%s) must be a number!\n",
				DAY->filename, DAY->startline, DAY->endline, day);
		warns++;
	}
	else if ((s < 1) || (s > 31)) {
		ast_log_chan(NULL, LOG_WARNING, "Warning: file %s, line %d-%d: The start day of month (%s) must be a number in the range [1-31]!\n",
				DAY->filename, DAY->startline, DAY->endline, day);
		warns++;
	}
	s--;
	if (c) {
		if (sscanf(c, "%2d", &e) != 1) {
			ast_log_chan(NULL, LOG_WARNING, "Warning: file %s, line %d-%d: The end day of month (%s) must be a number!\n",
					DAY->filename, DAY->startline, DAY->endline, c);
			warns++;
		}
		else if ((e < 1) || (e > 31)) {
			ast_log_chan(NULL, LOG_WARNING, "Warning: file %s, line %d-%d: The end day of month (%s) must be a number in the range [1-31]!\n",
					DAY->filename, DAY->startline, DAY->endline, day);
			warns++;
		}
		e--;
...
static void check_month(pval *MON)
{
	...
	if (s >= 12) {
		ast_log_chan(NULL, LOG_WARNING, "Warning: file %s, line %d-%d: The start month (%s) must be a one of: 'jan', 'feb', ..., 'dec'!\n",
	...
	if (c) {
		...
		if (e >= 12) {
			ast_log_chan(NULL, LOG_WARNING, "Warning: file %s, line %d-%d: The end month (%s) must be a one of: 'jan', 'feb', ..., 'dec'!\n",
...
static int check_break(pval *item)
{
	...
	ast_log_chan(NULL, LOG_ERROR,"Error: file %s, line %d-%d: 'break' not in switch, for, or while statement!\n",
...
static int check_continue(pval *item)
{
	...
	ast_log_chan(NULL, LOG_ERROR,"Error: file %s, line %d-%d: 'continue' not in 'for' or 'while' statement!\n",
			item->filename, item->startline, item->endline);
	errs++;
...
static void check_label(pval *item)
{
	...
	if( !alright )
	{
		ast_log_chan(NULL, LOG_ERROR,"Error: file %s, line %d-%d: Label %s is not within an extension or macro!\n",
	...
	if( x && x != item )
	{
		ast_log_chan(NULL, LOG_ERROR,"Error: file %s, line %d-%d: Duplicate label %s! Previously defined at file %s, line %d.\n",
...
static void check_goto(pval *item)
{
	...
	if ( !(item->u1.list)->next && !(item->u1.list)->u1.str ) {
		ast_log_chan(NULL, LOG_ERROR,"Error: file %s, line %d-%d: goto:  empty label reference found!\n",
				item->filename, item->startline, item->endline);
		errs++;
	}
	...
	if (!item->u1.list->next && !strstr(item->u1.list->u1.str,"${")) {
		...
		if (!x) {
			ast_log_chan(NULL, LOG_ERROR,"Error: file %s, line %d-%d: goto:  no label %s exists in the current extension!\n",
	...
	if (item->u1.list->next && !item->u1.list->next->next) {
		...
		if (!strstr((item->u1.list)->u1.str,"${")
			...
			if (!x) {
				ast_log_chan(NULL, LOG_ERROR,"Error: file %s, line %d-%d: goto:  no label '%s,%s' exists in the current context, or any of its inclusions!\n",
		...
		if (!strstr((item->u1.list)->u1.str,"${")
			...
			if (!x) {
				...
				if (that_context) {
					...
					if (!found) {
						ast_log_chan(NULL, LOG_ERROR,"Error: file %s, line %d-%d: goto:  no label %s|%s exists in the context %s or its inclusions!\n",
						...
						if( mac ) {    /* yes! */
							...
							if( mac != targ )
							{
								ast_log_chan(NULL, LOG_WARNING, "Warning: file %s, line %d-%d: It's bad form to have a goto in a macro to a target outside the macro!\n",
										item->filename, item->startline, item->endline);
								warns++;
							}
						}
					...
					if (pfiq.status != STATUS_SUCCESS) {
						ast_log_chan(NULL, LOG_WARNING,"Warning: file %s, line %d-%d: goto:  Couldn't find goto target %s|%s|%s, not even in extensions.conf!\n",
								item->filename, item->startline, item->endline, first->u1.str, second->u1.str, third->u1.str);
						warns++;
					}
#else
					ast_log_chan(NULL, LOG_WARNING,"Warning: file %s, line %d-%d: goto:  Couldn't find goto target %s|%s|%s in the AEL code!\n",
							item->filename, item->startline, item->endline, first->u1.str, second->u1.str, third->u1.str);
					warns++;
#endif
				}
...
static void find_pval_goto_item(pval *item, int lev)
{
	...
	if (lev>100) {
		ast_log_chan(NULL, LOG_ERROR,"find_pval_goto in infinite loop! item_type: %u\n\n", item->type);
		return;
	}
	...
	case ARGD_OPTIONSET:
		...
		for (ac=app->opts; ac; ac=ac->next) {
			if (strlen(ac->name)==1  ||  strchr(ac->name,'(')) {
				...
				if (p && *p == 'j') {
					ast_log_chan(NULL, LOG_ERROR, "Error: file %s, line %d-%d: The j option in the %s application call is not appropriate for AEL!\n",
				...
				if (p) {
					*p = '+';
					if (ac->name[1] == '(') {
						if (*(p+1) != '(') {
							ast_log_chan(NULL, LOG_WARNING, "Warning: file %s, line %d-%d: The %c option in the %s application call should have an (argument), but doesn't!\n",
		...
		for (q=opcop; *q; q++) {
			if ( *q != '+' && *q != '(' && *q != ')') {
				ast_log_chan(NULL, LOG_WARNING, "Warning: file %s, line %d-%d: The %c option in the %s application call is not available as an option!\n",
...
int check_app_args(pval* appcall, pval *arglist, struct argapp *app)
{
	...
	for (pa = arglist; pa; pa=pa->next) {
		if (!ad) {
			ast_log_chan(NULL, LOG_WARNING, "Warning: file %s, line %d-%d: Extra argument %s not in application call to %s !\n",
			...
			do {
				...
				if (!z) {
					...
					if (ad->type == ARGD_REQUIRED) {
						ast_log_chan(NULL, LOG_WARNING, "Warning: file %s, line %d-%d: Required argument %s not in application call to %s !\n",
...
void check_switch_expr(pval *item, struct argapp *apps)
{
	...
	if (v && v->vals) {
		...
		for (c=v->vals; c; c=c->next) {
			...
			if (!f1) {
				ast_log_chan(NULL, LOG_WARNING,"Warning: file %s, line %d-%d: switch with expression(%s) does not handle the case of %s !\n",
		...
		if (!f1) {
			ast_log_chan(NULL, LOG_WARNING,"Warning: file %s, line %d-%d: Couldn't find an application call in this extension that sets the  expression (%s) value!\n",
					item->filename, item->startline, item->endline, item->u1.str);
			warns++;
		}
	}
	...
	ast_log_chan(NULL, LOG_WARNING,"Warning: file %s, line %d-%d: A default case was automatically added to the switch.\n",
		...
		if (i->type == PV_CONTEXT || i->type == PV_MACRO) {
			for (j=i->next; j; j=j->next) {
				if ( j->type == PV_CONTEXT || j->type == PV_MACRO ) {
					...
					{
						ast_log_chan(NULL, LOG_WARNING,"Warning: file %s, line %d-%d: The context name (%s) is also declared in file %s, line %d-%d! (and neither is marked 'extend')\n",
...
static void check_abstract_reference(pval *abstract_context)
{
	...
	ast_log_chan(NULL, LOG_WARNING,"Warning: file %s, line %d-%d: Couldn't find a reference to this abstract context (%s) in any other context!\n",
...
void check_pval_item(pval *item, struct argapp *apps, int in_globals)
{
		...
		if (!macro_def) {
			...
			if (pfiq.status != STATUS_SUCCESS) {
				...
				if (pfiq2.status == STATUS_SUCCESS) {
					ast_log_chan(NULL, LOG_WARNING, "Warning: file %s, line %d-%d: macro call to non-existent %s! (macro-%s was found in the extensions.conf stuff, but we are using gosubs!)\n",
							item->filename, item->startline, item->endline, item->u1.str, item->u1.str);
					warns++;
				} else {
					ast_log_chan(NULL, LOG_WARNING, "Warning: file %s, line %d-%d: macro call to non-existent %s! (Not even in the extensions.conf stuff!)\n",
							item->filename, item->startline, item->endline, item->u1.str);
					warns++;
				}
			}
#else
			ast_log_chan(NULL, LOG_WARNING, "Warning: file %s, line %d-%d: macro call to %s cannot be found in the AEL code!\n",
					item->filename, item->startline, item->endline, item->u1.str);
			warns++;
			...
			if (pfiq.status != STATUS_SUCCESS) {
				ast_log_chan(NULL, LOG_WARNING, "Warning: file %s, line %d-%d: macro call to %s was not found in the AEL, nor the extensions.conf !\n",
						item->filename, item->startline, item->endline, item->u1.str);
				warns++;
			}
			...
			ast_log_chan(NULL, LOG_ERROR,"Error: file %s, line %d-%d: macro call to %s references a context, not a macro!\n",
					item->filename, item->startline, item->endline, item->u1.str);
			errs++;
		} else {
			...
			if (hereargs != thereargs ) {
				ast_log_chan(NULL, LOG_ERROR, "Error: file %s, line %d-%d: The macro call to %s has %d arguments, but the macro definition has %d arguments\n",
	...
	case PV_APPLICATION_CALL:
		...
		*/
		...
		if (app_def && app_def->type == PV_MACRO) {
			ast_log_chan(NULL, LOG_ERROR,"Error: file %s, line %d-%d: application call to %s references an existing macro, but had no & preceding it!\n",
					item->filename, item->startline, item->endline, item->u1.str);
			errs++;
		}
		if (strcasecmp(item->u1.str,"GotoIf") == 0
			...
			|| strcasecmp(item->u1.str,"execIf") == 0 ) {
			ast_log_chan(NULL, LOG_WARNING,"Warning: file %s, line %d-%d: application call to %s affects flow of control, and needs to be re-written using AEL if, while, goto, etc. keywords instead!\n",
					item->filename, item->startline, item->endline, item->u1.str);
			warns++;
		}
		if (strcasecmp(item->u1.str,"macroexit") == 0) {
				ast_log_chan(NULL, LOG_WARNING, "Warning: file %s, line %d-%d: I am converting the MacroExit call here to a return statement.\n",
						item->filename, item->startline, item->endline);
				item->type = PV_RETURN;
				free(item->u1.str);
				item->u1.str = 0;
		...
		if (!found) {
			ast_log_chan(NULL, LOG_WARNING,"Warning: file %s, line %d-%d: application call to %s not listed in applist database!\n",
					item->filename, item->startline, item->endline, item->u1.str);
			warns++;
		} else
		...
		*/
		...
		if( !in_globals ) { /* don't check stuff inside the globals context; no wrapping in $[ ] there... */
			...
			if ( strpbrk(item->u2.val,"~!-+<>=*/&^") && !strstr(item->u2.val,"${") ) {
				ast_log_chan(NULL, LOG_WARNING,"Warning: file %s, line %d-%d: expression %s has operators, but no variables. Interesting...\n",
	...
	case PV_LABEL:
		...
		*/
		if ( strspn(item->u1.str, "0123456789") == strlen(item->u1.str) ) {
			ast_log_chan(NULL, LOG_WARNING,"Warning: file %s, line %d-%d: label '%s' is numeric, this is bad practice!\n",
					item->filename, item->startline, item->endline, item->u1.str);
			warns++;
		}
	...
	case PV_RANDOM:
		...
		*/
		...
		ast_expr_clear_extra_error_info();
		if ( strpbrk(item->u1.str,"~!-+<>=*/&^") && !strstr(item->u1.str,"${") ) {
			ast_log_chan(NULL, LOG_WARNING,"Warning: file %s, line %d-%d: random expression '%s' has operators, but no variables. Interesting...\n",
					item->filename, item->startline, item->endline, item->u1.str);
			warns++;
		}
		check_expr2_input(item,item->u1.str);
...
/* =============================================================================================== */
/* "CODE" GENERATOR -- Convert the AEL representation to gabpbx extension language */
/* =============================================================================================== */
...
void add_extensions(struct ael_extension *exten)
{
	...
	if (!exten) {
		ast_log_chan(NULL, LOG_WARNING, "This file is Empty!\n" );
		return;
	}
	do {
		...
		if (exten->hints) {
			if (ast_add_extension2(exten->context, 0 /*no replace*/, realext, PRIORITY_HINT, NULL, exten->cidmatch,
				...
				ast_log_chan(NULL, LOG_WARNING, "Unable to add step at priority 'hint' of extension '%s'\n",
						exten->name);
			}
		}
		...
		for (pr=exten->plist; pr; pr=pr->next) {
			...
			*/
			...
			if (ast_add_extension2(exten->context, 0 /*no replace*/, realext, pr->priority_num, (label?label:NULL), exten->cidmatch,
				...
				ast_log_chan(NULL, LOG_WARNING, "Unable to add step at priority '%d' of extension '%s'\n", pr->priority_num,
						exten->name);
			}
			last = pr;
		}
...
int ast_compile_ael2(struct ast_context **local_contexts, struct ast_hashtab *local_table, struct pval *root)
{
	...
	add_extensions(exten_list);   /* actually makes calls to create priorities in ast_contexts -- feeds dialplan to gabpbx */
...
{
	if (item == NULL) {
		ast_log_chan(NULL, LOG_WARNING, "null item\n");
		return;
	}
...
int  pvalCheckType( pval *p, char *funcname, pvaltype type )
{
	if (p->type != type)
	{
		ast_log_chan(NULL, LOG_ERROR, "Func: %s the pval passed is not appropriate for this function!\n", funcname);
		return 0;
	}
	return 1;
}
...
pval *pvalCreateNode( pvaltype type )
{
	pval *p = calloc(1,sizeof(pval)); /* why, oh why, don't I use ast_calloc? Way, way, way too messy if I do! */
					  /* remember, this can be used externally or internally to gabpbx */
	if (p) {
		p->type = type;
	}
	return p;
...
void pvalGlobalsAddStatement( pval *p, pval *statement )
{
	if (p->type != PV_GLOBALS) {
		ast_log_chan(NULL, LOG_ERROR, "pvalGlobalsAddStatement called where first arg is not a Globals!\n");
	} else {
		if (!p->u1.statements) {
...
void pvalTopLevAddObject( pval *p, pval *contextOrObj )
{
	if (p) {
		...
		ast_log_chan(NULL, LOG_ERROR, "First arg to pvalTopLevel is NULL!\n");
	}
}
