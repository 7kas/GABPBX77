	...
	struct ast_websocket *ws_session, int (*validator)(struct ast_json *))
{
	...
	if (ast_websocket_set_timeout(ws_session, config->general->write_timeout)) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to set write timeout %d on ARI web socket\n",
	...
	struct ast_ari_websocket_session *session)
{
	...
	while (!message) {
		...
		if (res <= 0) {
			ast_log_chan(NULL, LOG_WARNING, "WebSocket poll error: %s\n",
				strerror(errno));
			return NULL;
		}
		...
		if (res != 0) {
			ast_log_chan(NULL, LOG_WARNING, "WebSocket read error: %s\n",
				strerror(errno));
			return NULL;
		}
	...
	struct ast_json *message)
{
	...
	if (!session->validator(message)) {
		ast_log_chan(NULL, LOG_ERROR, "Outgoing message failed validation\n");
	...
	if (str == NULL) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to encode JSON object\n");
		return -1;
	}

	if (ast_websocket_write_string(session->ws_session, str)) {
		ast_log_chan(NULL, LOG_NOTICE, "Problem occurred during websocket write to %s, websocket closed\n",
