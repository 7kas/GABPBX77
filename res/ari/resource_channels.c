...
#include "gabpbx/stasis_app_snoop.h"
 ...
 *
 * Since GABpbx 14, it has been possible for down channels, as well as unanswered
	...
	struct ast_ari_response *response)
{
	...
	if (!ast_strlen_zero(args->label)) {
		...
		if (sscanf(args->label, "%30d", &ipri) != 1) {
			...
			if (ipri == -1) {
				ast_log_chan(NULL, AST_LOG_ERROR, "Requested label: %s can not be found in context: %s\n", args->label, context);
		...
		if (ipri == 0) {
			ast_log_chan(NULL, AST_LOG_ERROR, "Invalid priority label '%s' specified for extension %s in context: %s\n",
...
static void *ari_originate_dial(void *data)
{
	...
	if (!ast_strlen_zero(origination->appdata)) {
		...
		if (app) {
			ast_verb_chan(NULL, 4, "Launching Stasis(%s) on %s\n", origination->appdata,
				ast_channel_name(ast_dial_answered(dial)));
			pbx_exec(ast_dial_answered(dial), app, origination->appdata);
		} else {
			ast_log_chan(NULL, LOG_WARNING, "No such application 'Stasis'\n");
		}
	} else {
		...
		if (ast_pbx_run(answered)) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to start PBX on %s\n", ast_channel_name(answered));
		} else {
...
static int json_to_ast_variables(struct ast_ari_response *response, struct ast_json *json_variables, struct ast_variable **variables)
{
	...
	ast_log_chan(NULL, AST_LOG_ERROR, "Unable to convert 'variables' in JSON body to channel variables\n");
...
static void *ari_channel_thread(void *data)
{
	...
	if (!stasis_app) {
		ast_log_chan(NULL, LOG_ERROR, "Stasis dialplan application is not registered");
