...
static int apply_option_timeout (int *var, char *timeout_arg)
{
	if (ast_strlen_zero(timeout_arg)) {
		ast_log_chan(NULL, LOG_ERROR, "No duration value provided for the timeout ('t') option.\n");
		return -1;
	}

	if (sscanf(timeout_arg, "%d", var) != 1 || *var < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Duration value provided for timeout ('t') option must be 0 or greater.\n");
		return -1;
	}
...
static int setup_park_common_datastore(struct ast_channel *parkee, const char *parker_uuid, const char *comeback_override, int randomize, int time_limit, int silence_announce)
{
	...
	if (!ast_strlen_zero(parker_dial_string)) {
		...
		ast_verb_chan(NULL, 4, "Setting Parker dial string to %s from %s value\n",
			parker_dial_string,
		...
		int use_ringing, int randomize, int time_limit, int silence_announcements)
{
	...
	if (!lot) {
		ast_log_chan(NULL, LOG_ERROR, "Could not find parking lot: '%s'\n", lot_name);
		return NULL;
	}
...
static int parked_call_app_exec(struct ast_channel *chan, const char *data)
{
	...
	lot = parking_lot_find_by_name(lot_name);
	if (!lot) {
		ast_log_chan(NULL, LOG_ERROR, "Could not find the requested parking lot\n");
	...
	if (!ast_strlen_zero(args.parking_space)) {
		if (sscanf(args.parking_space, "%d", &target_space) != 1 || target_space < 0) {
			ast_stream_and_wait(chan, "pbx-invalidpark", "");
			ast_log_chan(NULL, LOG_ERROR, "value '%s' for parking_space argument is invalid. Must be an integer greater than 0.\n", args.parking_space);
			return -1;
		}
	}
	 ...
	 * behavior consistent with GABpbx versions prior to 12.
	 */
	return -1;
}
...
static void announce_to_dial(char *dial_string, char *announce_string, int parkingspace, struct ast_channel_snapshot *parkee_snapshot)
{
	...
	ast_verb_chan(NULL, 3, "Dial Tech,String: (%s,%s)\n", dial_tech, dial_string);

	if (!cap_slin) {
		ast_log_chan(NULL, LOG_WARNING, "PARK: Failed to announce park.\n");
	...
	if (!dchan) {
		ast_log_chan(NULL, LOG_WARNING, "PARK: Unable to allocate announce channel.\n");
		goto announce_cleanup;
	}

	ast_verb_chan(NULL, 4, "Announce Template: %s\n", announce_string);

	for (cur_announce = strsep(&announce_string, ":"); cur_announce; cur_announce = strsep(&announce_string, ":")) {
		ast_verb_chan(NULL, 4, "Announce:%s\n", cur_announce);
		if (!strcmp(cur_announce, "PARKED")) {
			...
			if (!dres) {
				...
				ast_log_chan(NULL, LOG_WARNING, "ast_streamfile of %s failed on %s\n", cur_announce, ast_channel_name(dchan));
			}
		}
	}
...
static int park_and_announce_app_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_ERROR, "ParkAndAnnounce has required arguments. No arguments were provided.\n");
		return -1;
	}
	...
	if (ast_strlen_zero(args.announce_template)) {
		...
		ast_log_chan(NULL, LOG_ERROR, "ParkAndAnnounce requires the announce_template argument.\n");
		return -1;
	}

	if (ast_strlen_zero(args.dial)) {
		/* improperly configured arguments */
		ast_log_chan(NULL, LOG_ERROR, "ParkAndAnnounce requires the dial argument.\n");
		return -1;
	}

	if (!strchr(args.dial, '/')) {
		ast_log_chan(NULL, LOG_ERROR, "ParkAndAnnounce dial string '%s' is improperly formed.\n", args.dial);
		return -1;
	}
