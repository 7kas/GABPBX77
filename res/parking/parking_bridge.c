...
static struct parked_user *generate_parked_user(struct parking_lot *lot, struct ast_channel *chan, const char *parker_channel_name, const char *parker_dial_string, int use_random_space, int time_limit)
{
	...
	if (lot->mode == PARKINGLOT_DISABLED) {
		ast_log_chan(NULL, LOG_NOTICE, "Tried to park in a parking lot that is no longer able to be parked to.\n");
		return NULL;
	}
	...
	if (use_random_space) {
		...
		if (!ast_strlen_zero(parkingexten)) {
			if (sscanf(parkingexten, "%30d", &preferred_space) != 1 || preferred_space <= 0) {
				ast_log_chan(NULL, LOG_WARNING, "PARKINGEXTEN='%s' is not a valid parking space.\n", parkingexten);
	...
	if (parking_space == -1) {
		ast_log_chan(NULL, LOG_NOTICE, "Failed to get parking space in lot '%s'. All full.\n", lot->name);
		ao2_ref(new_parked_user, -1);
		ao2_unlock(lot);
		return NULL;
	}
...
static int bridge_parking_push(struct ast_bridge_parking *self, struct ast_bridge_channel *bridge_channel, struct ast_bridge_channel *swap)
{
	...
	if (swap) {
		...
		if (parking_channel_set_roles(bridge_channel->chan, self->lot, use_ringing)) {
			ast_log_chan(NULL, LOG_WARNING, "Failed to apply holding bridge roles to %s while joining the parking lot.\n",
	...
	ast_verb_chan(NULL, 3, "Parking '" COLORIZE_FMT "' in '" COLORIZE_FMT "' at space %d\n",
