...
static int parking_park_bridge_channel(struct ast_bridge_channel *bridge_channel, const char *uuid_parkee, const char *uuid_parker, const char *app_data)
{
	...
	if (!parker) {
		ast_log_chan(NULL, LOG_NOTICE, "Channel with uuid %s left before we could start parking the call. Parking canceled.\n", uuid_parker);
	...
	if (ast_bridge_move(parking_bridge, original_bridge, bridge_channel->chan, NULL, 1)) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to move %s into the parking bridge.\n",
...
static int parking_park_call(struct ast_bridge_channel *parker, char *exten, size_t length)
{
	...
	if (!lot) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Cannot Park %s: lot %s unknown\n",
...
static int parking_duration_callback(struct ast_bridge_channel *bridge_channel, void *hook_pvt)
{
	...
	if (ast_wrlock_contexts()) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to lock the contexts list. Can't add the park-dial extension.\n");
		return -1;
	}

	if (!(park_dial_context = ast_context_find_or_create(NULL, NULL, PARK_DIAL_CONTEXT, BASE_REGISTRAR))) {
		ast_log_chan(NULL, LOG_ERROR, "Parking dial context '%s' does not exist and unable to create\n", PARK_DIAL_CONTEXT);
	...
	if (ast_wrlock_context(park_dial_context)) {
		ast_log_chan(NULL, LOG_ERROR, "failed to obtain write lock on context '%s'\n", PARK_DIAL_CONTEXT);
		if (ast_unlock_contexts()) {
			ast_assert(0);
		}
		goto abandon_extension_creation;
	...
	if (!duplicate_returnexten) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to create parking redial parker extension %s@%s - Dial(%s)\n",
...
void parking_set_duration(struct ast_bridge_features *features, struct parked_user *user)
{
	...
	if (ast_bridge_interval_hook(features, 0, time_limit,
		...
		ast_log_chan(NULL, LOG_ERROR, "Failed to apply duration limit to the parked call.\n");
		ao2_ref(user, -1);
	}
}
...
static int func_get_parkingslot_channel(struct ast_channel *chan, const char *function, char *data, char *buf, size_t len)
{
	...
	if (args.argc < 2) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Usage: %s(<parking_space>,<parking_lot>)\n",
			function);
		return -1;
	}
