...
/* GABpbx ADSI button definitions */
...
static int adsi_careful_send(struct ast_channel *chan, unsigned char *buf, int len, int *remain)
{
	...
	if (remain && *remain) {
		...
		if (ast_write(chan, &outf)) {
			ast_log_chan(NULL, LOG_WARNING, "Failed to carefully write frame\n");
			return -1;
		}
	...
	while (len) {
		...
		if (ast_format_cmp(inf->subclass.format, ast_format_ulaw) != AST_FORMAT_CMP_EQUAL) {
			ast_log_chan(NULL, LOG_WARNING, "Channel not in ulaw?\n");
			ast_frfree(inf);
			return -1;
		}
...
static int __adsi_transmit_messages(struct ast_channel *chan, unsigned char **msg, int *msglen, int *msgtype)
{
	...
	if (ast_channel_adsicpe(chan) == AST_ADSI_UNAVAILABLE) {
		...
		ast_log_chan(NULL, LOG_WARNING, "ADSI is not supported for %s\n", ast_channel_name(chan));
		errno = ENOSYS;
		return -1;
	}
	...
	while (retries < maxretries) {
		if (!(ast_channel_adsicpe(chan) & ADSI_FLAG_DATAMODE)) {
			...
			if (adsi_careful_send(chan, buf, 680, NULL)) {
				ast_log_chan(NULL, LOG_WARNING, "Unable to send CAS\n");
			}
			...
			for (;;) {
				if (((res = ast_waitfor(chan, waittime)) < 1)) {
					...
					ast_verb_chan(NULL, 4, "No ADSI CPE detected (%d)\n", res);
				...
				if (f->frametype == AST_FRAME_DTMF) {
					if (f->subclass.integer == 'A') {
						...
						if (f->subclass.integer == 'D') {
							...
							ast_log_chan(NULL, LOG_WARNING, "Unknown ADSI response '%c'\n", f->subclass.integer);
						}
						if (!ast_channel_adsicpe(chan)) {
							ast_channel_adsicpe_set(chan, AST_ADSI_UNAVAILABLE);
						}
			...
			ast_verb_chan(NULL, 4, "ADSI Compatible CPE Detected\n");
		} else {
		...
		while ((x < 6) && msg[x]) {
			if ((res = adsi_generate(buf + pos, msgtype[x], msg[x], msglen[x], x+1 - start, (x == 5) || !msg[x+1], ast_format_ulaw)) < 0) {
				ast_log_chan(NULL, LOG_WARNING, "Failed to generate ADSI message %d on channel %s\n", x + 1, ast_channel_name(chan));
				return -1;
			}
		...
		if (ack[0] == 'D') {
			...
			retries++;
			ast_log_chan(NULL, LOG_WARNING, "Unexpected response to ack: %s (retry %d)\n", ack, retries);
		}
	}
	if (retries >= maxretries) {
		ast_log_chan(NULL, LOG_WARNING, "Maximum ADSI Retries (%d) exceeded\n", maxretries);
		errno = ETIMEDOUT;
		return -1;
	}
	return 0;
...
static int adsi_transmit_message_full(struct ast_channel *chan, unsigned char *msg, int msglen, int msgtype, int dowait)
{
	...
	if (msglen > 253) {
		ast_log_chan(NULL, LOG_WARNING, "Can't send ADSI message of %d bytes, too large\n", msglen);
		return -1;
	}
	...
	if (ast_set_write_format(chan, ast_format_ulaw)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to set write format to ULAW\n");
		return -1;
	}

	if (ast_set_read_format(chan, ast_format_ulaw)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to set read format to ULAW\n");
		if (writeformat) {
			if (ast_set_write_format(chan, writeformat)) {
				ast_log_chan(NULL, LOG_WARNING, "Unable to restore write format to %s\n", ast_format_get_name(writeformat));
			}
		}
		return -1;
	}
...
static int adsi_get_cpeid(struct ast_channel *chan, unsigned char *cpeid, int voice)
{
	...
	if (res != 4) {
		ast_log_chan(NULL, LOG_WARNING, "Got %d bytes back of encoded DTMF, expecting 4\n", res);
		res = 0;
	} else {
		res = 1;
	}
...
static int adsi_get_cpeinfo(struct ast_channel *chan, int *width, int *height, int *buttons, int voice)
{
	...
	if (strlen((char *) buf) != 2) {
		ast_log_chan(NULL, LOG_WARNING, "Got %d bytes of width, expecting 2\n", res);
		res = 0;
	} else {
		res = 1;
	}
	...
	if (res) {
		...
		if (strlen((char *) buf) != 2) {
			ast_log_chan(NULL, LOG_WARNING, "Got %d bytes of height, expecting 2\n", res);
			res = 0;
		} else {
			res = 1;
		}
	...
	if (res) {
		...
		if (strlen((char *) buf) != 1) {
			ast_log_chan(NULL, LOG_WARNING, "Got %d bytes of buttons, expecting 1\n", res);
			res = 0;
		} else {
			res = 1;
		}
...
static int adsi_load_session(struct ast_channel *chan, unsigned char *app, int ver, int data)
{
	...
	if (app) {
		...
		if (!strcmp(resp, "B")) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Unexpected CPE response to script query: %s\n", resp);
		}
	} else
		return 1;
	return 0;
...
static void init_state(void)
{
	...
	ast_copy_string(intro[1], "GABpbx", sizeof(intro[1]));
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "ADSI Resource",
