...
/*** DOCUMENTATION
	<configInfo name="res_aeap" language="en_US">
		<synopsis>GABpbx External Application Protocol (AEAP) module for GABpbx</synopsis>
		<configFile name="aeap.conf">
			<configObject name="client">
				...
				<configOption name="codecs">
					...
					<description><para>
					If this is specified, GABpbx will use this for codec related negotiations
					with the external application. Otherwise, GABpbx will default to using the
...
/* GABpbx External Application Protocol sorcery object */
...
static int client_config_apply(const struct ast_sorcery *sorcery, void *obj)
{
	...
	if (ast_strlen_zero(cfg->url)) {
		ast_log_chan(NULL, LOG_ERROR, "AEAP - URL must be present for '%s'\n", ast_sorcery_object_get_id(cfg));
		return -1;
	}

	if (!ast_begins_with(cfg->url, "ws")) {
		ast_log_chan(NULL, LOG_ERROR, "AEAP - URL must be ws or wss for '%s'\n", ast_sorcery_object_get_id(cfg));
		return -1;
	}
	...
	int connect, int timeout)
{
	...
	if (!url && !protocol) {
		ast_log_chan(NULL, LOG_ERROR, "AEAP: unable to get configuration for '%s'\n", id);
		return NULL;
	}
...
struct ast_variable *ast_aeap_custom_fields_get(const char *id)
{
	...
	cfg = client_config_get(id);
	if (!cfg) {
		ast_log_chan(NULL, LOG_WARNING, "AEAP: no client configuration '%s' to get fields\n", id);
		return NULL;
	}
...
static int load_module(void)
{
	...
	{
		ast_log_chan(NULL, LOG_ERROR, "AEAP - failed to open sorcery\n");
		return AST_MODULE_LOAD_DECLINE;
	}
	...
	if (ast_sorcery_object_register(aeap_sorcery, "client", client_config_alloc,
		...
		ast_log_chan(NULL, LOG_ERROR, "AEAP - failed to register client sorcery object\n");
		return AST_MODULE_LOAD_DECLINE;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER,
	"GABpbx External Application Protocol Module for GABpbx",
