...
static struct ast_aeap_message *message_create(const struct ast_aeap_message_type *type)
{
	...
	if (!msg) {
		ast_log_chan(NULL, LOG_ERROR, "AEAP message %s: unable to create\n", type->type_name);
		return NULL;
	}
	...
	const void *params)
{
	...
	if (type->construct1(msg, params)) {
		ast_log_chan(NULL, LOG_ERROR, "AEAP message %s: unable to construct1\n", type->type_name);
		ao2_ref(msg, -1);
		return NULL;
	}
	...
	const char *msg_type, const char *name, const char *id, const void *params)
{
	...
	if (type->construct2(msg, msg_type, name, id, params)) {
		ast_log_chan(NULL, LOG_ERROR, "AEAP message %s: unable to construct2\n", type->type_name);
		ao2_ref(msg, -1);
		return NULL;
	}
...
const char *ast_aeap_message_id_generate(struct ast_aeap_message *message)
{
	...
	if (strlen(uuid_str) != (AST_UUID_STR_LEN - 1)) {
		ast_log_chan(NULL, LOG_ERROR, "AEAP message %s failed to generate UUID for message '%s'",
