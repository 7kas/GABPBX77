	...
	const char *name, const char *id, const void *params)
{
	...
	if (!msg_data) {
		ast_log_chan(NULL, LOG_ERROR, "AEAP message json: failed to create data for '%s: %s'", msg_type, name);
		return -1;
	}

	if (params && ast_json_object_update(msg_data, (struct ast_json *)params)) {
		ast_log_chan(NULL, LOG_ERROR, "AEAP message json: failed to update data for '%s: %s'", msg_type, name);
