...
/*! \file
 *
 * \brief AGI - the GABpbx Gateway Interface
 *
...
#include "gabpbx/paths.h"	/* use many ast_config_AST_*_DIR */
...
/*** DOCUMENTATION
	...
	<agi name="database del" language="en_US">
		...
		<description>
			<para>Deletes an entry in the GABpbx database for a given
	...
	<agi name="database deltree" language="en_US">
		...
		<description>
			...
			within a <replaceable>family</replaceable> in the GABpbx database.</para>
	...
	<agi name="database get" language="en_US">
		...
		<description>
			<para>Retrieves an entry in the GABpbx database for a given <replaceable>family</replaceable>
	...
	<agi name="database put" language="en_US">
		...
		<description>
			<para>Adds or updates an entry in the GABpbx database for a given
	...
	<agi name="record file" language="en_US">
		...
		<syntax>
			...
			<parameter name="beep">
				<para>Causes GABpbx to play a beep as recording begins. This argument
		...
		<description>
			...
			file. <replaceable>beep</replaceable> can take any value, and causes GABpbx
	...
	<agi name="verbose" language="en_US">
		...
		<synopsis>
			Logs a message to the gabpbx verbose log.
		</synopsis>
		<syntax>
	...
	<application name="AGI" language="en_US">
		...
		<description>
			<para>Executes an GABpbx Gateway Interface compliant
			program on a channel. AGI allows GABpbx to launch external programs written
			...
			<enumlist>
				<enum name="AGI">
					...
					<filename>gabpbx.conf</filename> will be used.
					</para>
				</enum>
				<enum name="FastAGI">
					<para>Connect GABpbx to a FastAGI server using a TCP connection.
			...
			<example title="Start the AGI script my-cool-script.sh located in the astagidir directory, specified in gabpbx.conf">
		...
		<see-also>
			...
			<ref type="filename">gabpbx.conf</ref>
		</see-also>
	</application>
	...
	<application name="DeadAGI" language="en_US">
		...
		<description>
			<warning>
				...
				of GABpbx. Use the replacement application <literal>AGI</literal> instead
...
static int get_agi_cmd(struct ast_channel *chan, struct agi_cmd **cmd)
{
	...
	if (!store) {
		ast_log_chan(NULL, LOG_ERROR, "Huh? Async AGI datastore disappeared on Channel %s!\n",
...
static int add_agi_cmd(struct ast_channel *chan, const char *cmd_buff, const char *cmd_id)
{
	...
	store = ast_channel_datastore_find(chan, &agi_commands_datastore_info, NULL);
	if (!store) {
		ast_log_chan(NULL, LOG_WARNING, "Channel %s is not setup for Async AGI.\n", ast_channel_name(chan));
		return -1;
	}
	agi_commands = store->data;
...
static int add_to_agi(struct ast_channel *chan)
{
	...
	if (!agi_cmds_list) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to allocate Async AGI commands list.\n");
...
#define AMI_BUF_SIZE 2048
	...
	if (efd) {
		ast_log_chan(NULL, LOG_WARNING, "Async AGI does not support Enhanced AGI yet\n");
		return AGI_RESULT_FAILURE;
	}

	/* add AsyncAGI datastore to the channel */
	if (add_to_agi(chan)) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to start Async AGI on channel %s\n", ast_channel_name(chan));
		return AGI_RESULT_FAILURE;
	}
	...
	res = pipe(fds);
	if (res) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to create Async AGI pipe\n");
		/*
	...
	if (res <= 0) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to read from Async AGI pipe on channel %s: %s\n",
...
static int handle_connection(const char *agiurl, const struct ast_sockaddr addr, const int netsockfd)
{
	...
	while ((res = ast_poll(pfds, 1, MAX_AGI_CONNECT)) != 1) {
		if (errno != EINTR) {
			if (!res) {
				ast_log_chan(NULL, LOG_WARNING, "FastAGI connection to '%s' timed out after MAX_AGI_CONNECT (%d) milliseconds.\n",
					agiurl, MAX_AGI_CONNECT);
			} else {
				ast_log_chan(NULL, LOG_WARNING, "Connect to '%s' failed: %s\n", agiurl, strerror(errno));
			}
	...
	if (getsockopt(pfds[0].fd, SOL_SOCKET, SO_ERROR, &conresult, &reslen) < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Connection to %s failed with error: %s\n",
			ast_sockaddr_stringify(&addr), strerror(errno));
		return 1;
	}

	if (conresult) {
		ast_log_chan(NULL, LOG_WARNING, "Connecting to '%s' failed for url '%s': %s\n",
...
static enum agi_result launch_netscript(char *agiurl, char *argv[], int *fds)
{
	...
	if (!(num_addrs = ast_sockaddr_resolve(&addrs, host, 0, AST_AF_UNSPEC))) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to locate host '%s'\n", host);
		return AGI_RESULT_FAILURE;
	}
	...
	for (i = 0; i < num_addrs; i++) {
		...
		if ((s = ast_socket_nonblock(addrs[i].ss.ss_family, SOCK_STREAM, IPPROTO_TCP)) < 0) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to create socket: %s\n", strerror(errno));
			continue;
		}
		...
		if (ast_connect(s, &addrs[i]) && errno == EINPROGRESS) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Connection to %s failed with unexpected error: %s\n",
	...
	if (i == num_addrs) {
		ast_log_chan(NULL, LOG_WARNING, "Couldn't connect to any host.  FastAGI failed.\n");
		return AGI_RESULT_FAILURE;
	}
...
static enum agi_result launch_ha_netscript(char *agiurl, char *argv[], int *fds)
{
	...
	if (strlen(agiurl) < 7) { /* Remove hagi:// */
		ast_log_chan(NULL, LOG_WARNING, "An error occurred parsing the AGI URI: %s", agiurl);
		return AGI_RESULT_FAILURE;
	}
	...
	if (strchr(host, ':')) {
		ast_log_chan(NULL, LOG_WARNING, "Specifying a port number disables SRV lookups: %s\n", agiurl);
	...
	while (!(srv_ret = ast_srv_lookup(&context, service, &srvhost, &srvport))) {
		...
		if (result == AGI_RESULT_FAILURE || result == AGI_RESULT_NOTFOUND) {
			ast_log_chan(NULL, LOG_WARNING, "AGI request failed for host '%s' (%s:%d)\n", host, srvhost, srvport);
		} else {
	 ...
	 */
	if (srv_ret < 0) {
		ast_log_chan(NULL, LOG_WARNING, "SRV lookup failed for %s\n", agiurl);
	}
...
static enum agi_result launch_script(struct ast_channel *chan, char *script, int argc, char *argv[], int *fds, int *efd, int *opid, int *safe_fork_called)
{
	...
	*safe_fork_called = 0;
	...
	if (stat(script, &st)) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to execute '%s': File does not exist.\n", script);
		return AGI_RESULT_NOTFOUND;
	}

	if (pipe(toast)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to create toast pipe: %s\n",strerror(errno));
		return AGI_RESULT_FAILURE;
	}
	if (pipe(fromast)) {
		ast_log_chan(NULL, LOG_WARNING, "unable to create fromast pipe: %s\n", strerror(errno));
		close(toast[0]);
		close(toast[1]);
		return AGI_RESULT_FAILURE;
	}
	if (efd) {
		if (pipe(audio)) {
			ast_log_chan(NULL, LOG_WARNING, "unable to create audio pipe: %s\n", strerror(errno));
			close(fromast[0]);
			close(fromast[1]);
			close(toast[0]);
			close(toast[1]);
		...
		if (res < 0) {
			ast_log_chan(NULL, LOG_WARNING, "unable to set audio pipe parameters: %s\n", strerror(errno));
			close(fromast[0]);
			close(fromast[1]);
			close(toast[0]);
			close(toast[1]);
	...
	*safe_fork_called = 1;
	...
	if ((pid = ast_safe_fork(1)) < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to fork(): %s\n", strerror(errno));
		return AGI_RESULT_FAILURE;
	}
	if (!pid) {
	...
	ast_verb_chan(NULL, 3, "Launched AGI Script %s\n", script);
	fds[0] = toast[0];
	fds[1] = fromast[1];
	if (efd)
		*efd = audio[1];
...
static int handle_streamfile(struct ast_channel *chan, AGI *agi, int argc, const char * const argv[])
{
	...
	ast_verb_chan(NULL, 3, "<%s> Playing '%s.%s' (escape_digits=%s) (sample_offset %ld) (language '%s')\n",
...
static int handle_getoption(struct ast_channel *chan, AGI *agi, int argc, const char * const argv[])
{
	...
	if (!(fs = ast_openstream(chan, argv[2], ast_channel_language(chan)))) {
		ast_agi_send(agi->fd, chan, "200 result=-1 endpos=%ld\n", sample_offset);
		ast_log_chan(NULL, LOG_WARNING, "Unable to open %s\n", argv[2]);
		return RESULT_FAILURE;
	}
	...
	ast_verb_chan(NULL, 3, "Playing '%s' (escape_digits=%s) (timeout %d)\n", argv[2], edigits, timeout);
...
static int handle_exec(struct ast_channel *chan, AGI *agi, int argc, const char * const argv[])
{
	...
	ast_verb_chan(NULL, 3, "AGI Script Executing Application: (%s) Options: (%s)\n", argv[1], argc >= 3 ? argv[2] : "");
	...
	if ((app_to_exec = pbx_findapp(argv[1]))) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Could not find application (%s)\n", argv[1]);
		res = -2;
	}
	ast_agi_send(agi->fd, chan, "200 result=%d\n", res);
...
int AST_OPTIONAL_API_NAME(ast_agi_register)(struct ast_module *mod, agi_command *cmd)
{
	...
	if (!find_command(cmd->cmda, 1)) {
		*((enum ast_doc_src *) &cmd->docsrc) = AST_STATIC_DOC;
		...
		AST_RWLIST_UNLOCK(&agi_commands);
		ast_verb_chan(NULL, 5, "AGI Command '%s' registered\n",fullcmd);
		return 1;
	} else {
		ast_log_chan(NULL, LOG_WARNING, "Command already registered!\n");
		return 0;
	}
}
...
int AST_OPTIONAL_API_NAME(ast_agi_unregister)(agi_command *cmd)
{
	...
	AST_RWLIST_UNLOCK(&agi_commands);
	if (unregistered) {
		ast_verb_chan(NULL, 5, "AGI Command '%s' unregistered\n",fullcmd);
	}
	return unregistered;
}
...
normal:
			if (whitespace) {
				if (x >= MAX_ARGS -1) {
					ast_log_chan(NULL, LOG_WARNING, "Too many arguments, truncating\n");
					break;
				}
...
static enum agi_result run_agi(struct ast_channel *chan, char *request, AGI *agi, int pid, int *status, int dead, int argc, char *argv[])
{
	...
	if (!(readf = fdopen(agi->ctrl, "r"))) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to fdopen file descriptor\n");
	...
	for (;;) {
		...
		if (c) {
			...
			if (!buf[0]) {
				/* Program terminated */
				ast_verb_chan(NULL, 3, "<%s>AGI Script %s completed, returning %d\n", ast_channel_name(chan), request, returnstatus);
				if (pid > 0)
			...
			if (--retry <= 0) {
				ast_log_chan(NULL, LOG_WARNING, "No channel, no fd?\n");
				returnstatus = AGI_RESULT_FAILURE;
				break;
			}
		}
	...
	if (send_sighup) {
		if (pid > -1) {
			if (kill(pid, SIGHUP)) {
				ast_log_chan(NULL, LOG_WARNING, "unable to send SIGHUP to AGI process %d: %s\n", pid, strerror(errno));
...
static int agi_exec_full(struct ast_channel *chan, const char *data, int enhanced, int dead)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "AGI requires an argument (script)\n");
		return -1;
	}
	if (dead)
...
static int eagi_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_check_hangup(chan)) {
		ast_log_chan(NULL, LOG_ERROR, "EAGI cannot be run on a dead/hungup channel, please use AGI.\n");
		return 0;
	}
	...
	if (requested_format_name) {
		...
		if (requested_format) {
			ast_verb_chan(NULL, 3, "<%s> Setting EAGI audio pipe format to %s\n",
					 ast_channel_name(chan), ast_format_get_name(requested_format));
		} else {
			ast_log_chan(NULL, LOG_ERROR, "Could not find requested format: %s\n", requested_format_name);
		}
	}

	readformat = ao2_bump(ast_channel_readformat(chan));
	if (ast_set_read_format(chan, requested_format ?: ast_format_slin)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to set channel '%s' to linear mode\n", ast_channel_name(chan));
	...
	if (!res) {
		if (ast_set_read_format(chan, readformat)) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to restore channel '%s' to format %s\n", ast_channel_name(chan),
...
{
	ast_log_chan(NULL, LOG_WARNING, "DeadAGI has been deprecated, please use AGI in all cases!\n");
...
AST_TEST_DEFINE(test_agi_null_docs)
{
	...
	case TEST_INIT:
		...
		info->description = "Test whether an AGI command with no documentation will crash GABpbx";
...
#ifndef HAVE_NULLSAFE_PRINTF
	/* Test for condition without actually crashing GABpbx */
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER, "GABpbx Gateway Interface (AGI)",
