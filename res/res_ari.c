 ...
 * <a href="https://github.com/gabpbx/gabpbx_rest_libraries">languages</a>,
			...
			<configObject name="general">
				...
				<configOption name="enabled">
					...
					<description>
						...
						<note>
							<para>ARI uses GABpbx's HTTP server, which must also be enabled in <filename>http.conf</filename>.</para>
						</note>
					</description>
					<see-also>
						<ref type="filename">http.conf</ref>
						<ref type="link">https://docs.gabpbx.org/Configuration/Core-Configuration/GABpbx-Builtin-mini-HTTP-Server/</ref>
					</see-also>
				</configOption>
				...
				<configOption name="auth_realm">
					...
					<synopsis>Realm to use for authentication. Defaults to GABpbx REST Interface.</synopsis>
				</configOption>
			   ...
			   struct ast_ari_response *response)
{
	...
	if (!origin_allowed(origin)) {
		ast_log_chan(NULL, LOG_NOTICE, "Origin header '%s' does not match an allowed origin.\n", origin);
		return;
	}
	...
	struct ast_json *body, struct ast_ari_response *response)
{
	...
	if (response->message == NULL && response->response_code == 0) {
		...
		ast_log_chan(NULL, LOG_ERROR, "ARI %s %s not implemented\n",
			  ...
			  struct ast_ari_response *response)
{
	...
	if (absolute_api_dirname == NULL) {
		ast_log_chan(NULL, LOG_ERROR, "Error determining real directory for rest-api\n");
		ast_ari_response_error(
	...
	if (obj == NULL) {
		ast_log_chan(NULL, LOG_ERROR, "Error parsing resource file: %s:%d(%d) %s\n",
...
static struct ast_ari_conf_user *authenticate_api_key(const char *api_key)
{
	...
	if (!password) {
		ast_log_chan(NULL, LOG_WARNING, "Invalid api_key\n");
		return NULL;
	}
				...
				struct ast_variable *headers)
{
	...
	if (response.response_code > 0) {
		...
		ast_http_request_close_on_completion(ser);
		ast_ari_response_error(&response, 503, "Service Unavailable", "GABpbx not booted");
...
static struct ast_http_uri http_uri = {
	...
	.description = "GABpbx RESTful API",
	.uri = "ari",
	.has_subtree = 1,
	.data = NULL,
	.key = __FILE__,
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER, "GABpbx RESTful Interface",
