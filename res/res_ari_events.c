	...
	struct ast_variable *get_params, struct ast_variable *headers, const char *session_id)
{
	...
	if (!response) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to create response.\n");
		goto fin;
	}
...
fin: __attribute__((unused))
	if (!response) {
		...
		if (response->message) {
			...
			ast_log_chan(NULL, LOG_ERROR, "Missing response message\n");
		}
...
#endif
	if (!session) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to create ARI session\n");
		goto fin;
	}
	...
	struct ast_variable *headers, struct ast_json *body, struct ast_ari_response *response)
{
	...
	default:
		if (200 <= code && code <= 299) {
			...
			ast_log_chan(NULL, LOG_ERROR, "Invalid error response %d for /events/user/{eventName}\n", code);
			is_valid = 0;
		}
	}

	if (!is_valid) {
		ast_log_chan(NULL, LOG_ERROR, "Response validation failed for /events/user/{eventName}\n");
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_DEFAULT, "RESTful API module - WebSocket resource",
