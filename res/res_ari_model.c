...
static int check_type(struct ast_json *json, enum ast_json_type expected)
{
	...
	if (!json) {
		ast_log_chan(NULL, LOG_ERROR, "Expected type %s, was NULL\n",
	...
	if (expected != actual) {
		ast_log_chan(NULL, LOG_ERROR, "Expected type %s, was %s\n",
...
static int check_range(intmax_t minval, intmax_t maxval, struct ast_json *json)
{
	...
	if (v < minval || maxval < v) {
		ast_log_chan(NULL, LOG_ERROR, "Value out of range. Expected %jd <= %jd <= %jd\n", minval, v, maxval);
		return 0;
	}
	return 1;
}
...
int ast_ari_validate_boolean(struct ast_json *json)
{
	...
	case AST_JSON_FALSE:
		return 1;
	default:
		ast_log_chan(NULL, LOG_ERROR, "Expected type boolean, was %s\n",
...
int ast_ari_validate_date(struct ast_json *json)
{
	...
	if (regexec(&date_regex, str, 0, NULL, 0) != 0) {
		ast_log_chan(NULL, LOG_ERROR, "Date field is malformed: '%s'\n", str);
		return 0;
	}
	return 1;
}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER | AST_MODFLAG_GLOBAL_SYMBOLS, "ARI Model validators",
