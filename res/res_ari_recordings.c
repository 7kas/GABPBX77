	...
	struct ast_variable *headers, struct ast_json *body, struct ast_ari_response *response)
{
	...
	default:
		if (200 <= code && code <= 299) {
			...
			ast_log_chan(NULL, LOG_ERROR, "Invalid error response %d for /recordings/stored\n", code);
			is_valid = 0;
		}
	}

	if (!is_valid) {
		ast_log_chan(NULL, LOG_ERROR, "Response validation failed for /recordings/stored\n");
	...
	struct ast_variable *headers, struct ast_json *body, struct ast_ari_response *response)
{
	...
	default:
		if (200 <= code && code <= 299) {
			...
			ast_log_chan(NULL, LOG_ERROR, "Invalid error response %d for /recordings/stored/{recordingName}\n", code);
			is_valid = 0;
		}
	}

	if (!is_valid) {
		ast_log_chan(NULL, LOG_ERROR, "Response validation failed for /recordings/stored/{recordingName}\n");
		ast_ari_response_error(response, 500,
			"Internal Server Error", "Response validation failed");
	}
#endif /* AST_DEVMODE */
	...
	struct ast_variable *headers, struct ast_json *body, struct ast_ari_response *response)
{
	...
	default:
		if (200 <= code && code <= 299) {
			...
			ast_log_chan(NULL, LOG_ERROR, "Invalid error response %d for /recordings/stored/{recordingName}/file\n", code);
			is_valid = 0;
		}
	}

	if (!is_valid) {
		ast_log_chan(NULL, LOG_ERROR, "Response validation failed for /recordings/stored/{recordingName}/file\n");
		ast_ari_response_error(response, 500,
			"Internal Server Error", "Response validation failed");
	}
#endif /* AST_DEVMODE */
	...
	struct ast_variable *headers, struct ast_json *body, struct ast_ari_response *response)
{
	...
	default:
		if (200 <= code && code <= 299) {
			...
			ast_log_chan(NULL, LOG_ERROR, "Invalid error response %d for /recordings/stored/{recordingName}/copy\n", code);
			is_valid = 0;
		}
	}

	if (!is_valid) {
		ast_log_chan(NULL, LOG_ERROR, "Response validation failed for /recordings/stored/{recordingName}/copy\n");
		ast_ari_response_error(response, 500,
			"Internal Server Error", "Response validation failed");
	}
#endif /* AST_DEVMODE */
	...
	struct ast_variable *headers, struct ast_json *body, struct ast_ari_response *response)
{
	...
	default:
		if (200 <= code && code <= 299) {
			...
			ast_log_chan(NULL, LOG_ERROR, "Invalid error response %d for /recordings/live/{recordingName}\n", code);
			is_valid = 0;
		}
	}

	if (!is_valid) {
		ast_log_chan(NULL, LOG_ERROR, "Response validation failed for /recordings/live/{recordingName}\n");
		ast_ari_response_error(response, 500,
			"Internal Server Error", "Response validation failed");
	}
#endif /* AST_DEVMODE */
	...
	struct ast_variable *headers, struct ast_json *body, struct ast_ari_response *response)
{
	...
	default:
		if (200 <= code && code <= 299) {
			...
			ast_log_chan(NULL, LOG_ERROR, "Invalid error response %d for /recordings/live/{recordingName}/stop\n", code);
			is_valid = 0;
		}
	}

	if (!is_valid) {
		ast_log_chan(NULL, LOG_ERROR, "Response validation failed for /recordings/live/{recordingName}/stop\n");
		ast_ari_response_error(response, 500,
			"Internal Server Error", "Response validation failed");
	}
#endif /* AST_DEVMODE */
	...
	struct ast_variable *headers, struct ast_json *body, struct ast_ari_response *response)
{
	...
	default:
		if (200 <= code && code <= 299) {
			...
			ast_log_chan(NULL, LOG_ERROR, "Invalid error response %d for /recordings/live/{recordingName}/pause\n", code);
			is_valid = 0;
		}
	}

	if (!is_valid) {
		ast_log_chan(NULL, LOG_ERROR, "Response validation failed for /recordings/live/{recordingName}/pause\n");
		ast_ari_response_error(response, 500,
			"Internal Server Error", "Response validation failed");
	}
#endif /* AST_DEVMODE */
	...
	struct ast_variable *headers, struct ast_json *body, struct ast_ari_response *response)
{
	...
	default:
		if (200 <= code && code <= 299) {
			...
			ast_log_chan(NULL, LOG_ERROR, "Invalid error response %d for /recordings/live/{recordingName}/mute\n", code);
			is_valid = 0;
		}
	}

	if (!is_valid) {
		ast_log_chan(NULL, LOG_ERROR, "Response validation failed for /recordings/live/{recordingName}/mute\n");
		ast_ari_response_error(response, 500,
			"Internal Server Error", "Response validation failed");
	}
#endif /* AST_DEVMODE */
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_DEFAULT, "RESTful API module - Recording resources",
