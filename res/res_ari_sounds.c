	...
	struct ast_variable *headers, struct ast_json *body, struct ast_ari_response *response)
{
	...
	default:
		if (200 <= code && code <= 299) {
			...
			ast_log_chan(NULL, LOG_ERROR, "Invalid error response %d for /sounds\n", code);
			is_valid = 0;
		}
	}

	if (!is_valid) {
		ast_log_chan(NULL, LOG_ERROR, "Response validation failed for /sounds\n");
	...
	struct ast_variable *headers, struct ast_json *body, struct ast_ari_response *response)
{
	...
	default:
		if (200 <= code && code <= 299) {
			...
			ast_log_chan(NULL, LOG_ERROR, "Invalid error response %d for /sounds/{soundId}\n", code);
			is_valid = 0;
		}
	}

	if (!is_valid) {
		ast_log_chan(NULL, LOG_ERROR, "Response validation failed for /sounds/{soundId}\n");
		ast_ari_response_error(response, 500,
			"Internal Server Error", "Response validation failed");
	}
#endif /* AST_DEVMODE */
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_DEFAULT, "RESTful API module - Sound resources",
