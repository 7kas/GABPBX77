...
/*! \file
 *
 * \brief AudioSocket support for GABpbx
 *
...
#include "gabpbx/res_audiosocket.h"
#include "gabpbx/channel.h"
#include "gabpbx/module.h"
#include "gabpbx/uuid.h"
#include "gabpbx/format_cache.h"

#define	MODULE_DESCRIPTION	"AudioSocket support functions for GABpbx"
	...
	const struct ast_sockaddr addr, const int netsockfd)
{
	...
	while ((res = ast_poll(pfds, 1, MAX_CONNECT_TIMEOUT_MSEC)) != 1) {
		if (errno != EINTR) {
			if (!res) {
				ast_log_chan(NULL, LOG_WARNING, "AudioSocket connection to '%s' timed"
...
const int ast_audiosocket_connect(const char *server, struct ast_channel *chan)
{
	...
	if (chan && ast_autoservice_start(chan) < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to start autoservice for channel "
			"%s\n", ast_channel_name(chan));
		goto end;
	}

	if (ast_strlen_zero(server)) {
		ast_log_chan(NULL, LOG_ERROR, "No AudioSocket server provided\n");
		goto end;
	}

	if (!(num_addrs = ast_sockaddr_resolve(&addrs, server, PARSE_PORT_REQUIRE,
		AST_AF_UNSPEC))) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to resolve AudioSocket service using %s - "
	...
	for (i = 0; i < num_addrs; i++) {
		...
		if (!ast_sockaddr_port(&addrs[i])) {
			 ...
			 */
			ast_log_chan(NULL, LOG_ERROR, "No port provided for %s\n",
		...
		if (ast_connect(s, &addrs[i]) && errno == EINPROGRESS) {
			...
			ast_log_chan(NULL, LOG_ERROR, "Connection to %s failed with unexpected error: %s\n",
	...
	if (chan && ast_autoservice_stop(chan) < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to stop autoservice for channel %s\n",
		ast_channel_name(chan));
		close(s);
		return -1;
	}
	...
	if (i == num_addrs) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to connect to AudioSocket service\n");
		close(s);
		return -1;
	}
...
const int ast_audiosocket_init(const int svc, const char *id)
{
	...
	if (ast_strlen_zero(id)) {
		ast_log_chan(NULL, LOG_ERROR, "No UUID for AudioSocket\n");
		return -1;
	}

	if (uuid_parse(id, uu)) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to parse UUID '%s'\n", id);
		return -1;
	}
	...
	if (write(svc, buf, 3 + 16) != 3 + 16) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to write data to AudioSocket\n");
		ret = -1;
	}
...
struct ast_frame *ast_audiosocket_receive_frame(const int svc)
{
	...
	if (n != 1) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to read type header from AudioSocket\n");
		return NULL;
	}
	if (kind == 0x00) {
	...
	if (kind != 0x10) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Received non-audio AudioSocket message\n");
		not_audio = 1;
	}

	n = read(svc, &len_high, 1);
	if (n != 1) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to read data length from AudioSocket\n");
		return NULL;
	}
	len += len_high * 256;
	...
	data = ast_malloc(len);
	if (!data) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to allocate for data from AudioSocket\n");
		return NULL;
	}
	...
	while (i < len) {
		...
		if (n < 0) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to read data from AudioSocket\n");
			ret = n;
			break;
		}
		if (n == 0) {
			ast_log_chan(NULL, LOG_ERROR, "Insufficient data read from AudioSocket\n");
			ret = -1;
			break;
		}
		i += n;
...
{
	ast_verb_chan(NULL, 5, "Loading AudioSocket Support module\n");
	return AST_MODULE_LOAD_SUCCESS;
}

static int unload_module(void)
{
	ast_verb_chan(NULL, 5, "Unloading AudioSocket Support module\n");
	return AST_MODULE_LOAD_SUCCESS;
}

AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER, "AudioSocket support",
