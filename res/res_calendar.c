...
static struct ast_calendar *build_calendar(struct ast_config *cfg, const char *cat, const struct ast_calendar_tech *tech)
{
	...
	if (!cal) {
		...
		if (!(cal = ao2_alloc(sizeof(*cal), calendar_destructor))) {
			ast_log_chan(NULL, LOG_ERROR, "Could not allocate calendar structure. Stopping.\n");
			return NULL;
		}
		...
		if (!cal->events) {
			ast_log_chan(NULL, LOG_ERROR, "Could not allocate events container for %s\n", cat);
			cal = unref_calendar(cal);
			return NULL;
		}

		if (ast_string_field_init(cal, 32)) {
			ast_log_chan(NULL, LOG_ERROR, "Couldn't create string fields for %s\n", cat);
			cal = unref_calendar(cal);
			return NULL;
		}
	} else {
		...
		if (!strcasecmp(v->name, "autoreminder")) {
			...
			if ((name = (value = ast_strdup(v->value)))) {
				...
				if (value) {
					...
					ast_log_chan(NULL, LOG_WARNING, "Malformed argument. Should be '%s: variable=value'\n", v->name);
				}
				ast_free(name);
			}
		}
...
static int load_tech_calendars(struct ast_calendar_tech *tech)
{
	...
	if (!calendar_config) {
		ast_log_chan(NULL, LOG_WARNING, "Calendar support disabled, not loading %s calendar module\n", tech->type);
		return -1;
	}
...
int ast_calendar_register(struct ast_calendar_tech *tech)
{
	...
	AST_LIST_TRAVERSE(&techs, iter, list) {
		if(!strcasecmp(tech->type, iter->type)) {
			ast_log_chan(NULL, LOG_WARNING, "Already have a handler for calendar type '%s'\n", tech->type);
	...
	ast_verb_chan(NULL, 2, "Registered calendar type '%s' (%s)\n", tech->type, tech->description);
...
void ast_calendar_unregister(struct ast_calendar_tech *tech)
{
	...
	AST_LIST_TRAVERSE_SAFE_BEGIN(&techs, iter, list) {
		...
		ast_verb_chan(NULL, 2, "Unregistered calendar type '%s'\n", tech->type);
		break;
	}
...
static void *do_notify(void *data)
{
	...
	if ((dest = strchr(tech, '/'))) {
		*dest = '\0';
		...
		ast_log_chan(NULL, LOG_WARNING, "Channel should be in form Tech/Dest (was '%s')\n", tech);
		goto notify_cleanup;
	}

	if (!(dial = ast_dial_create())) {
		ast_log_chan(NULL, LOG_ERROR, "Could not create dial structure\n");
		goto notify_cleanup;
	}

	if (ast_dial_append(dial, tech, dest, NULL) < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Could not append channel\n");
		goto notify_cleanup;
	}
	...
	if (!(chan = ast_channel_alloc(1, AST_STATE_DOWN, 0, 0, 0, 0, 0, NULL, NULL, 0, "Calendar/%s-%s", event->owner->name, buf))) {
		ast_log_chan(NULL, LOG_ERROR, "Could not allocate notification channel\n");
		goto notify_cleanup;
	}
	...
	if (!caps) {
		ast_log_chan(NULL, LOG_ERROR, "Could not allocate capabilities, notification not being sent!\n");
		goto notify_cleanup;
	}
	...
	if (!(datastore = ast_datastore_alloc(&event_notification_datastore, NULL))) {
		ast_log_chan(NULL, LOG_ERROR, "Could not allocate datastore, notification not being sent!\n");
		goto notify_cleanup;
	}
	...
	ast_verb_chan(NULL, 3, "Dialing %s for notification on calendar %s\n", event->owner->notify_channel, event->owner->name);
	res = ast_dial_run(dial, chan, 0);

	if (res != AST_DIAL_RESULT_ANSWERED) {
		ast_verb_chan(NULL, 3, "Notification call for %s was not completed\n", event->owner->name);
	} else {
...
static int calendar_event_notify(const void *data)
{
	...
	if (!(event && event->owner)) {
		ast_log_chan(NULL, LOG_ERROR, "Extremely low-cal...in fact cal is NULL!\n");
		return res;
	}
	...
	if (ast_pthread_create_background(&notify_thread, NULL, do_notify, event) < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Could not create notification thread\n");
		return res;
	}
...
static int calendar_devstate_change(const void *data)
{
	...
	if (!event) {
		ast_log_chan(NULL, LOG_WARNING, "Event was NULL!\n");
		return 0;
	}
...
static int schedule_calendar_event(struct ast_calendar *cal, struct ast_calendar_event *old_event, struct ast_calendar_event *cmp_event)
{
	...
	if (!cmp_event || old_event->end != event->end) {
		...
		if (devstate_sched_end <= 0) { /* if we let this slip by, GABpbx will assert */
			ast_log_chan(NULL, LOG_WARNING, "Whoops! Event end notification scheduled in the past: %ld ms\n", (long) devstate_sched_end);
		} else {
			ast_mutex_lock(&refreshlock);
...
static int load_config(int reload)
{
	...
	if (!(tmpcfg = ast_config_load2("calendar.conf", "calendar", config_flags)) ||
		...
		ast_log_chan(NULL, LOG_ERROR, "Unable to load config calendar.conf\n");
		return -1;
	}
...
static int calendar_busy_exec(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "CALENDAR_BUSY requires an argument: CALENDAR_BUSY(<calendar_name>)\n");
		return -1;
	}
	...
	if (!cal) {
		ast_log_chan(NULL, LOG_WARNING, "Could not find calendar '%s'\n", data);
		return -1;
	}
...
static int add_event_to_list(struct eventlist *events, struct ast_calendar_event *event, time_t start, time_t end)
{
	...
	if (!(entry = ast_calloc(1, sizeof(*entry)))) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to allocate memory for event list\n");
		return -1;
	}
...
static int calendar_query_exec(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	if (!chan) {
		ast_log_chan(NULL, LOG_WARNING, "%s requires a channel to store the data on\n", cmd);
		return -1;
	}
	...
	if (ast_strlen_zero(args.calendar)) {
		ast_log_chan(NULL, LOG_WARNING, "%s requires a calendar argument\n", cmd);
		return -1;
	}

	if (!(cal = find_calendar(args.calendar))) {
		ast_log_chan(NULL, LOG_WARNING, "Unknown calendar '%s'\n", args.calendar);
		return -1;
	}
	...
	if (!(eventlist_datastore = ast_datastore_alloc(&eventlist_datastore_info, buf))) {
		ast_log_chan(NULL, LOG_ERROR, "Could not allocate datastore!\n");
		cal = unref_calendar(cal);
		ao2_ref(events, -1);
		return -1;
	}
...
static void calendar_join_attendees(struct ast_calendar_event *event, char *buf, size_t len)
{
	...
	if (!(tmp = ast_str_create(32))) {
		ast_log_chan(NULL, LOG_ERROR, "Could not allocate memory for attendees!\n");
		return;
	}
...
static int calendar_query_result_exec(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	if (!chan) {
		ast_log_chan(NULL, LOG_WARNING, "%s requires a channel\n", cmd);
		return -1;
	}
	...
	if (ast_strlen_zero(args.id) || ast_strlen_zero(args.field)) {
		ast_log_chan(NULL, LOG_WARNING, "%s requires an id and a field", cmd);
		return -1;
	}

	ast_channel_lock(chan);
	if (!(datastore = ast_channel_datastore_find(chan, &eventlist_datastore_info, args.id))) {
		ast_log_chan(NULL, LOG_WARNING, "There is no event notification datastore with id '%s' on '%s'!\n", args.id, ast_channel_name(chan));
		ast_channel_unlock(chan);
		return -1;
	}
	ast_channel_unlock(chan);
	...
	if (!(events = datastore->data)) {
		ast_log_chan(NULL, LOG_WARNING, "The datastore contains no data!\n");
		return -1;
	}
	...
	AST_LIST_TRAVERSE(events, entry, list) {
		...
		if (!strcasecmp(args.field, "summary")) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Unknown field '%s'\n", args.field);
		}
		break;
	}
...
static int calendar_write_exec(struct ast_channel *chan, const char *cmd, char *data, const char *value)
{
	...
	if (!(val_dup = ast_strdup(value))) {
		ast_log_chan(NULL, LOG_ERROR, "Could not allocate memory for values\n");
		goto write_cleanup;
	}
	...
	if (!(cal = find_calendar(fields.field[0]))) {
		ast_log_chan(NULL, LOG_WARNING, "Couldn't find calendar '%s'\n", fields.field[0]);
		goto write_cleanup;
	}

	if (!(cal->tech->write_event)) {
		ast_log_chan(NULL, LOG_WARNING, "Calendar '%s' has no write function!\n", cal->name);
		goto write_cleanup;
	}
	...
	if (ast_strlen_zero(fields.field[0])) {
		ast_log_chan(NULL, LOG_WARNING, "CALENDAR_WRITE requires a calendar name!\n");
		goto write_cleanup;
	}

	if (fields.argc - 1 != values.argc) {
		ast_log_chan(NULL, LOG_WARNING, "CALENDAR_WRITE should have the same number of fields (%u) and values (%u)!\n", fields.argc - 1, values.argc);
		goto write_cleanup;
	}
	...
	for (i = 1, j = 0; i < fields.argc; i++, j++) {
		if (!strcasecmp(fields.field[i], "summary")) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Unknown calendar event field '%s'\n", fields.field[i]);
		}
	}
	...
	if((ret = cal->tech->write_event(event))) {
		ast_log_chan(NULL, LOG_WARNING, "Writing event to calendar '%s' failed!\n", cal->name);
	}
...
static int calendar_event_read(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "%s requires an argument\n", cmd);
		return -1;
	}

	ast_channel_lock(chan);
	if (!(datastore = ast_channel_datastore_find(chan, &event_notification_datastore, NULL))) {
		ast_log_chan(NULL, LOG_WARNING, "There is no event notification datastore on '%s'!\n", ast_channel_name(chan));
		ast_channel_unlock(chan);
		return -1;
	}
	ast_channel_unlock(chan);
...
static int reload(void)
{
	...
	AST_LIST_TRAVERSE(&techs, iter, list) {
		if (load_tech_calendars(iter)) {
			ast_log_chan(NULL, LOG_WARNING, "Failed to reload %s calendars, module disabled\n", iter->type);
		}
	}
	AST_LIST_UNLOCK(&techs);
...
static int load_module(void)
{
	...
	if (!calendars) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to allocate calendars container!\n");
	...
	if (!(sched = ast_sched_context_create())) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to create sched context\n");
		ast_config_destroy(calendar_config);
		calendar_config = NULL;
		return AST_MODULE_LOAD_DECLINE;
	}
	...
	if (ast_pthread_create_background(&refresh_thread, NULL, do_refresh, NULL) < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to start refresh thread--notifications disabled!\n");
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER, "GABpbx Calendar integration",
