...
static int auth_credentials(void *userdata, const char *realm, int attempts, char *username, char *secret)
{
	...
	if (attempts > 1) {
		ast_log_chan(NULL, LOG_WARNING, "Invalid username or password for CalDAV calendar '%s'\n", pvt->owner->name);
		return -1;
	}
...
static struct ast_str *caldav_request(struct caldav_pvt *pvt, const char *method, struct ast_str *req_body, struct ast_str *subdir, const char *content_type)
{
	...
	if (!pvt) {
		ast_log_chan(NULL, LOG_ERROR, "There is no private!\n");
		return NULL;
	}

	if (!(response = ast_str_create(512))) {
		ast_log_chan(NULL, LOG_ERROR, "Could not allocate memory for response.\n");
		return NULL;
	}
	...
	if (ret != NE_OK) {
		ast_log_chan(NULL, LOG_WARNING, "Unknown response to CalDAV calendar %s, request %s to %s: %s\n", pvt->owner->name, method, buf, ne_get_error(pvt->session));
		ast_free(response);
		return NULL;
	}
...
static int caldav_write_event(struct ast_calendar_event *event)
{
	...
	if (!event) {
		ast_log_chan(NULL, LOG_WARNING, "No event passed!\n");
		return -1;
	}

	if (!(event->start && event->end)) {
		ast_log_chan(NULL, LOG_WARNING, "The event must contain a start and an end\n");
		return -1;
	}
	if (!(body = ast_str_create(512)) ||
		!(subdir = ast_str_create(32))) {
		ast_log_chan(NULL, LOG_ERROR, "Could not allocate memory for request!\n");
		goto write_cleanup;
	}
...
static struct ast_str *caldav_get_events_between(struct caldav_pvt *pvt, time_t start_time, time_t end_time)
{
	...
	if (!(body = ast_str_create(512))) {
		ast_log_chan(NULL, LOG_ERROR, "Could not allocate memory for body of request!\n");
		return NULL;
	}
...
static void caldav_add_event(icalcomponent *comp, struct icaltime_span *span, void *data)
{
	...
	if (!(pvt && pvt->owner)) {
		ast_log_chan(NULL, LOG_ERROR, "Require a private structure with an owner\n");
		return;
	}

	if (!(event = ast_calendar_event_alloc(pvt->owner))) {
		ast_log_chan(NULL, LOG_ERROR, "Could not allocate an event!\n");
		return;
	}
	...
	if ((prop = icalcomponent_get_first_property(comp, ICAL_UID_PROPERTY))) {
		...
		ast_log_chan(NULL, LOG_WARNING, "No UID found, but one is required. Generating, but updates may not be accurate\n");
		if (!ast_strlen_zero(event->summary)) {
	...
	if (!(prop = icalcomponent_get_first_property(valarm, ICAL_TRIGGER_PROPERTY))) {
		ast_log_chan(NULL, LOG_WARNING, "VALARM has no TRIGGER, skipping!\n");
		ao2_link(pvt->events, event);
		event = ast_calendar_unref_event(event);
		return;
	}
	...
	if (icaltriggertype_is_null_trigger(trigger)) {
		ast_log_chan(NULL, LOG_WARNING, "Bad TRIGGER for VALARM, skipping!\n");
		ao2_link(pvt->events, event);
		event = ast_calendar_unref_event(event);
		return;
	}
	...
	xmlParserSeverities severity, xmlTextReaderLocatorPtr locator)
{
	...
	case XML_PARSER_SEVERITY_WARNING:
		ast_log_chan(NULL, LOG_WARNING, "While parsing CalDAV response at line %d: %s\n",
	...
	default:
		ast_log_chan(NULL, LOG_ERROR, "While parsing CalDAV response at line %d: %s\n",
			xmlTextReaderLocatorLineNumber(locator),
			msg);
		break;
	}
...
static void *caldav_load_calendar(void *void_data)
{
	...
	if (!(cal && (cfg = ast_calendar_config_acquire()))) {
		ast_log_chan(NULL, LOG_ERROR, "You must enable calendar support for res_caldav to load\n");
		return NULL;
	}

	if (ao2_trylock(cal)) {
		if (cal->unloading) {
			ast_log_chan(NULL, LOG_WARNING, "Unloading module, load_calendar cancelled.\n");
		} else {
			ast_log_chan(NULL, LOG_WARNING, "Could not lock calendar, aborting!\n");
		}
	...
	if (!(pvt = ao2_alloc(sizeof(*pvt), caldav_destructor))) {
		ast_log_chan(NULL, LOG_ERROR, "Could not allocate caldav_pvt structure for calendar: %s\n", cal->name);
		ast_calendar_config_release();
		return NULL;
	}
	...
	if (!(pvt->events = ast_calendar_event_container_alloc())) {
		ast_log_chan(NULL, LOG_ERROR, "Could not allocate space for fetching events for calendar: %s\n", cal->name);
	...
	if (ast_string_field_init(pvt, 32)) {
		ast_log_chan(NULL, LOG_ERROR, "Couldn't allocate string field space for calendar: %s\n", cal->name);
		pvt = unref_caldav(pvt);
		ao2_unlock(cal);
		ast_calendar_config_release();
		return NULL;
	...
	if (ast_strlen_zero(pvt->url)) {
		ast_log_chan(NULL, LOG_WARNING, "No URL was specified for CalDAV calendar '%s' - skipping.\n", cal->name);
		pvt = unref_caldav(pvt);
		ao2_unlock(cal);
		return NULL;
	}
	...
	if (ne_uri_parse(pvt->url, &pvt->uri) || pvt->uri.host == NULL || pvt->uri.path == NULL) {
		ast_log_chan(NULL, LOG_WARNING, "Could not parse url '%s' for CalDAV calendar '%s' - skipping.\n", pvt->url, cal->name);
		pvt = unref_caldav(pvt);
		ao2_unlock(cal);
		return NULL;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "GABpbx CalDAV Calendar Integration",
