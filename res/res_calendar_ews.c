...
static int auth_credentials(void *userdata, const char *realm, int attempts, char *username, char *secret)
{
	...
	if (attempts > 1) {
		ast_log_chan(NULL, LOG_WARNING, "Invalid username or password for Exchange Web Service calendar '%s'\n", pvt->owner->name);
		return -1;
	}
...
static int ssl_verify(void *userdata, int failures, const ne_ssl_certificate *cert)
{
	...
	if (failures & NE_SSL_UNTRUSTED) {
		ast_log_chan(NULL, LOG_WARNING, "Untrusted SSL certificate for calendar %s!\n", pvt->owner->name);
		return 0;
	}
...
static int startelm(void *userdata, int parent, const char *nspace, const char *name, const char **atts)
{
	...
	if (!strcmp(name, "Envelope") ||
		...
		if (!ctx->cdata) {
			ast_log_chan(NULL, LOG_ERROR, "Could not allocate CDATA!\n");
			return NE_XML_ABORT;
		}
...
static int cdata(void *userdata, int state, const char *cdata, size_t len)
{
	...
	if (!ctx->event) {
		ast_log_chan(NULL, LOG_ERROR, "Parsing event data, but event object does not exist!\n");
		return 1;
	}

	if (!ctx->cdata) {
		ast_log_chan(NULL, LOG_ERROR, "String for storing CDATA is unitialized!\n");
		return 1;
	}
...
static int endelm(void *userdata, int state, const char *nspace, const char *name)
{
	...
	if (!strcmp(name, "Subject")) {
		...
		if (ctx->event) {
			...
			ast_log_chan(NULL, LOG_ERROR, "Event data ended in XML, but event object does not exist!\n");
			return 1;
		}
...
static int send_ews_request_and_parse(struct ast_str *request, struct xml_context *ctx)
{
	...
	if (!ast_str_strlen(request)) {
		ast_log_chan(NULL, LOG_ERROR, "No request to send!\n");
		return -1;
	}
	...
	if (ret != NE_OK) { /* Error handling */
		ast_log_chan(NULL, LOG_WARNING, "Unable to communicate with Exchange Web Service at '%s': %s\n", ctx->pvt->url, ne_get_error(ctx->pvt->session));
...
static void *ewscal_load_calendar(void *void_data)
{
	...
	if (!(cal && (cfg = ast_calendar_config_acquire()))) {
		ast_log_chan(NULL, LOG_ERROR, "You must enable calendar support for res_ewscal to load\n");
		return NULL;
	}
	...
	if (!(pvt = ao2_alloc(sizeof(*pvt), ewscal_destructor))) {
		ast_log_chan(NULL, LOG_ERROR, "Could not allocate ewscal_pvt structure for calendar: %s\n", cal->name);
		ast_calendar_config_release();
		return NULL;
	}
	...
	if (ast_strlen_zero(pvt->url)) {
		ast_log_chan(NULL, LOG_WARNING, "No URL was specified for Exchange Web Service calendar '%s' - skipping.\n", cal->name);
		pvt = unref_ewscal(pvt);
		ao2_unlock(cal);
		return NULL;
	}
	...
	if (ne_uri_parse(pvt->url, &pvt->uri) || pvt->uri.host == NULL || pvt->uri.path == NULL) {
		ast_log_chan(NULL, LOG_WARNING, "Could not parse url '%s' for Exchange Web Service calendar '%s' - skipping.\n", pvt->url, cal->name);
		pvt = unref_ewscal(pvt);
		ao2_unlock(cal);
		return NULL;
	}
	...
	ne_set_useragent(pvt->session, "GABpbx");
...
static int load_module(void)
{
	...
	if (ne_version_match(0, 29) && ne_version_match(0, 30)) {
		ast_log_chan(NULL, LOG_ERROR, "Exchange Web Service calendar module require neon >= 0.29.1, but %s is installed.\n", ne_version_string());
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "GABpbx MS Exchange Web Service Calendar Integration",
