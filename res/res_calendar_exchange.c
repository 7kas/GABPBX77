	...
	case IKS_CLOSE:
		if (!strcasecmp(state->tag, "response")) {
			...
			if (ast_strlen_zero(event->uid)) {
				ast_log_chan(NULL, LOG_ERROR, "This event has no UID, something has gone wrong\n");
...
static int auth_credentials(void *userdata, const char *realm, int attempts, char *username, char *secret)
{
	...
	if (attempts > 1) {
		ast_log_chan(NULL, LOG_WARNING, "Invalid username or password for Exchange calendar '%s'\n", pvt->owner->name);
		return -1;
	}
...
static int exchangecal_write_event(struct ast_calendar_event *event)
{
	...
	if (!(uid = ast_str_create(AST_UUID_STR_LEN)) ||
		...
		ast_log_chan(NULL, LOG_ERROR, "Unable to allocate memory for request values\n");
		goto write_cleanup;
	}
	...
	if (!is_valid_uuid(uid)) {
		ast_log_chan(NULL, LOG_WARNING, "An invalid uid was provided, you may leave this field blank to have one generated for you\n");
		goto write_cleanup;
	}
...
static void *exchangecal_load_calendar(void *void_data)
{
	...
	if (!(cal && (cfg = ast_calendar_config_acquire()))) {
		ast_log_chan(NULL, LOG_ERROR, "You must enable calendar support for res_exchangecal to load\n");
		return NULL;
	}
	...
	if (!(pvt = ao2_alloc(sizeof(*pvt), exchangecal_destructor))) {
		ast_log_chan(NULL, LOG_ERROR, "Could not allocate exchangecal_pvt structure for calendar: %s\n", cal->name);
		ast_calendar_config_release();
		return NULL;
	}
	...
	if (ast_strlen_zero(pvt->url)) {
		ast_log_chan(NULL, LOG_WARNING, "No URL was specified for Exchange calendar '%s' - skipping.\n", cal->name);
		pvt = unref_exchangecal(pvt);
		ao2_unlock(cal);
		return NULL;
	}
	...
	if (ne_uri_parse(pvt->url, &pvt->uri) || pvt->uri.host == NULL || pvt->uri.path == NULL) {
		ast_log_chan(NULL, LOG_WARNING, "Could not parse url '%s' for Exchange calendar '%s' - skipping.\n", pvt->url, cal->name);
		pvt = unref_exchangecal(pvt);
		ao2_unlock(cal);
		return NULL;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "GABpbx MS Exchange Calendar Integration",
