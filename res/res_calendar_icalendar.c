...
static int auth_credentials(void *userdata, const char *realm, int attempts, char *username, char *secret)
{
	...
	if (attempts > 1) {
		ast_log_chan(NULL, LOG_WARNING, "Invalid username or password for iCalendar '%s'\n", pvt->owner->name);
		return -1;
	}
...
static icalcomponent *fetch_icalendar(struct icalendar_pvt *pvt)
{
	...
	if (ret != NE_OK || !ast_str_strlen(response)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to retrieve iCalendar '%s' from '%s': %s\n", pvt->owner->name, pvt->url, ne_get_error(pvt->session));
		ast_free(response);
		return NULL;
	}
	...
	if (!ast_strlen_zero(ast_str_buffer(response))) {
		...
		if (!comp) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Failed to parse iCalendar data: %s\n", icalerror_perror());
		}
	}
	ast_free(response);
 ...
 static void icalendar_update_events(struct icalendar_pvt *pvt)
{
	...
	if (!pvt) {
		ast_log_chan(NULL, LOG_ERROR, "iCalendar is NULL\n");
		return;
	}

	if (!pvt->owner) {
		ast_log_chan(NULL, LOG_ERROR, "iCalendar is an orphan!\n");
		return;
	}

	if (!pvt->data) {
		ast_log_chan(NULL, LOG_ERROR, "The iCalendar has not been parsed!\n");
		return;
	}
...
static void *ical_load_calendar(void *void_data)
{
	...
	if (!(cal && (cfg = ast_calendar_config_acquire()))) {
		ast_log_chan(NULL, LOG_ERROR, "You must enable calendar support for res_icalendar to load\n");
		return NULL;
	}
	if (ao2_trylock(cal)) {
		if (cal->unloading) {
	...
	if (!(pvt = ao2_alloc(sizeof(*pvt), icalendar_destructor))) {
		ast_log_chan(NULL, LOG_ERROR, "Could not allocate icalendar_pvt structure for calendar: %s\n", cal->name);
		ast_calendar_config_release();
		return NULL;
	}
	...
	if (ast_strlen_zero(pvt->url)) {
		ast_log_chan(NULL, LOG_WARNING, "No URL was specified for iCalendar '%s' - skipping.\n", cal->name);
		pvt = unref_icalendar(pvt);
		ao2_unlock(cal);
		return NULL;
	}
	...
	if (ne_uri_parse(pvt->url, &pvt->uri) || pvt->uri.host == NULL || pvt->uri.path == NULL) {
		ast_log_chan(NULL, LOG_WARNING, "Could not parse url '%s' for iCalendar '%s' - skipping.\n", pvt->url, cal->name);
		pvt = unref_icalendar(pvt);
		ao2_unlock(cal);
		return NULL;
	}
	...
	if (!(pvt->data = fetch_icalendar(pvt))) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to parse iCalendar '%s'\n", cal->name);
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "GABpbx iCalendar .ics file integration",
