...
static void load_config(int reload)
{
	...
	if (!(cfg = ast_config_load(config_file, config_flags)) || cfg == CONFIG_STATUS_FILEINVALID) {
		ast_log_chan(NULL, LOG_ERROR, "res_clialiases configuration file '%s' not found\n", config_file);
		return;
	...
	for (v = ast_variable_browse(cfg, "general"); v; v = v->next) {
		if (strcmp(v->name, "template")) {
			ast_log_chan(NULL, LOG_WARNING, "%s is not a correct option in [%s]\n", v->name, "general");
			continue;
		}
		...
		for (v1 = ast_variable_browse(cfg, v->value); v1; v1 = v1->next) {
			...
			if (existing) {
				ast_log_chan(NULL, LOG_WARNING, "Alias '%s' could not be unregistered and has been retained\n",
					existing->alias);
				ao2_ref(existing, -1);
				continue;
			}
			...
			ast_verb_chan(NULL, 5, "Aliased CLI command '%s' to '%s'\n", v1->name, v1->value);
			ao2_ref(alias, -1);
		}
	}
...
static int unload_module(void)
{
	...
	if (ao2_container_count(cli_aliases)) {
		ast_log_chan(NULL, LOG_ERROR, "Could not unregister all CLI aliases\n");
		return -1;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_DEFAULT, "CLI Aliases",
