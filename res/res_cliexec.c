...
static char *handle_exec(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	if (!caps) {
		ast_log_chan(NULL, LOG_WARNING, "Could not allocate an empty format capabilities structure\n");
		return CLI_FAILURE;
	}

	if (ast_format_cap_append(caps, ast_format_ulaw, 0)) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to append a ulaw format to capabilities for channel nativeformats\n");
		return CLI_FAILURE;
	}

	if (ast_format_cap_append(caps, ast_format_alaw, 0)) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to append an alaw format to capabilities for channel nativeformats\n");
		return CLI_FAILURE;
	}

	if (ast_format_cap_append(caps, ast_format_h264, 0)) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to append an h264 format to capabilities for channel nativeformats\n");
		return CLI_FAILURE;
	}
	...
	if (!app) {
		...
		if (substituted_args) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Could not substitute application argument variables for %s\n", app_name);
			ast_cli(a->fd, "Executing: %s(%s)\n", app_name, app_args);
...
AST_MODULE_INFO_STANDARD_EXTENDED(GABPBX_GPL_KEY, "Simple dialplan execution from the CLI");
