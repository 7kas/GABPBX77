...
static struct ast_variable *realtime_curl(const char *url, const char *unused, const struct ast_variable *fields)
{
	...
	if (!ast_custom_function_find("CURL")) {
		ast_log_chan(NULL, LOG_ERROR, "func_curl.so must be loaded in order to use res_config_curl.so!!\n");
		return NULL;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "Realtime Curl configuration",
