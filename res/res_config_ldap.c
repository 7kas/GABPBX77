...
#define RES_CONFIG_LDAP_DEFAULT_BASEDN "gabpbx"
			...
			if (static_table_config == table_config) {
				if (DEBUG_ATLEAST(3)) {
					...
					if (tmpdebug && tmpdebug2) {
						ast_log_chan(NULL, LOG_DEBUG, "Added to vars - %s = %s\n", tmpdebug->value, tmpdebug2->value);
					}
				}
...
{
	if (!table_config) {
		ast_log_chan(NULL, LOG_ERROR, "No table config\n");
		return NULL;
	} else {
		...
		do {
			...
			if (result != LDAP_SUCCESS && is_ldap_connect_error(result)) {
				ast_log_chan(NULL, LOG_WARNING, "Failed to query directory. Try %d/3\n", tries + 1);
				tries++;
				if (tries < 3) {
		...
		if (result != LDAP_SUCCESS) {
			ast_log_chan(NULL, LOG_WARNING, "Failed to query directory. Error: %s.\n", ldap_err2string(result));
			ast_debug(2, "dn=%s\n", dn);
			...
			if ((num_entry = ldap_count_entries(ldapConn, ldap_result_msg)) > 0) {
				...
				if (num_entry > 1) {
					ast_log_chan(NULL, LOG_NOTICE, "More than one entry for dn=%s. Take only 1st one\n", dn);
				}
			} else {
	...
	const char *basedn, const char *table_name, const struct ast_variable *fields)
{
	...
	if (!table_name) {
		ast_log_chan(NULL, LOG_ERROR, "No table_name specified.\n");
		ast_free(clean_basedn);
		return NULL;
	}

	if (!field) {
		ast_log_chan(NULL, LOG_ERROR, "Realtime retrieval requires at least 1 parameter"
	...
	if (!table_config) {
		ast_log_chan(NULL, LOG_WARNING, "No table named '%s'.\n", table_name);
		ast_mutex_unlock(&ldap_lock);
		ast_free(clean_basedn);
		return NULL;
	}
	...
	if (result != LDAP_SUCCESS) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to query directory. Error: %s.\n", ldap_err2string(result));
		ast_log_chan(NULL, LOG_WARNING, "Query: %s\n", ast_str_buffer(filter));
	} else {
 ...
 * this is an area of gabpbx that could do with a lot of modification
      ...
      const char *table_name, const struct ast_variable *fields)
{
	...
	if (!fields) {
	    ast_log_chan(NULL, LOG_WARNING, "realtime retrieval requires at least 1 parameter and 1 value to search on.\n");
	    return NULL;
	}
	...
	if (vars) {
		cfg = ast_config_new();
		if (!cfg) {
			ast_log_chan(NULL, LOG_ERROR, "Unable to create a config!\n");
		} else {
			struct ast_variable **p = vars;
...
/*! \brief See GABpbx Realtime Documentation
 *
	...
	const char *file, struct ast_config *cfg, struct ast_flags config_flags, const char *sugg_incl, const char *who_asked)
{
	...
	if (ast_strlen_zero(file) || !strcasecmp(file, RES_CONFIG_LDAP_CONF)) {
		ast_log_chan(NULL, LOG_ERROR, "Missing configuration file: %s. Can't configure myself.\n", RES_CONFIG_LDAP_CONF);
		return NULL;
	}
	...
	if (!vars) {
		ast_log_chan(NULL, LOG_WARNING, "Could not find config '%s' in directory.\n", file);
		return NULL;
	}
	...
	for (vars_count = 0, p = vars; *p; p++) {
		...
		if (!category) {
			ast_log_chan(NULL, LOG_ERROR, "No category name in entry '%s'  for file '%s'.\n",
					(dn ? dn->value : "?"), file);
		} else if (!cat_metric) {
			ast_log_chan(NULL, LOG_ERROR, "No category metric in entry '%s'(category: %s) for file '%s'.\n",
					(dn ? dn->value : "?"), category->value, file);
		} else if (!var_metric) {
			ast_log_chan(NULL, LOG_ERROR, "No variable metric in entry '%s'(category: %s) for file '%s'.\n",
					(dn ? dn->value : "?"), category->value, file);
		} else if (!var_name) {
			ast_log_chan(NULL, LOG_ERROR, "No variable name in entry '%s' (category: %s metric: %s) for file '%s'.\n",
					(dn ? dn->value : "?"), category->value,
					cat_metric->value, file);
		} else if (!var_val) {
			ast_log_chan(NULL, LOG_ERROR, "No variable value in entry '%s' (category: %s metric: %s variable: %s) for file '%s'.\n",
					(dn ? dn->value : "?"), category->value,
...
static LDAPMod *ldap_mod_create(const char *attribute, const char *new_value)
{
	...
	if (!(mod && type)) {
		ast_log_chan(NULL, LOG_ERROR, "Memory allocation failure creating LDAP modification\n");
		ldap_memfree(type);
		ldap_memfree(mod);
		return NULL;
	}
...
static LDAPMod **massage_mods_for_entry(LDAPMessage *entry, LDAPMod **mods)
{
	...
	if (!copies) {
		ast_log_chan(NULL, LOG_ERROR, "Memory allocation failure massaging LDAP modification\n");
		return NULL;
	}
...
static int update2_ldap(const char *basedn, const char *table_name, const struct ast_variable *lookup_fields, const struct ast_variable *update_fields)
{
	...
	if (!update_count) {
		ast_log_chan(NULL, LOG_WARNING, "Need at least one parameter to modify.\n");
		return res;
	}
	...
	table_config = table_config_for_table_name(table_name);
	if (!table_config) {
		ast_log_chan(NULL, LOG_ERROR, "No table named '%s'.\n", table_name);
		ast_mutex_unlock(&ldap_lock);
		return res;
	}
	...
	if (!modifications) {
		ast_log_chan(NULL, LOG_ERROR, "Memory allocation failure\n");
		goto early_bailout;
	}
	...
	if (DEBUG_ATLEAST(3)) {
		...
		for (i = 0; modifications[i]; i++) {
			if (modifications[i]->mod_op != LDAP_MOD_DELETE) {
				ast_log_chan(NULL, LOG_DEBUG, "%s => %s\n", modifications[i]->mod_type,
					modifications[i]->mod_values[0]);
			} else {
				ast_log_chan(NULL, LOG_DEBUG, "deleting %s\n", modifications[i]->mod_type);
			}
		}
	}
	...
	for (ldap_entry = ldap_first_entry(ldapConn, ldap_result_msg);
		...
		if ((error = ldap_modify_ext_s(ldapConn, dn, working, NULL, NULL)) != LDAP_SUCCESS)  {
			ast_log_chan(NULL, LOG_ERROR, "Couldn't modify dn:%s because %s", dn, ldap_err2string(error));
		}
...
{
	if (parse_config() < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Cannot load LDAP RealTime driver.\n");
		return 0;
	}
	...
	if (!ldap_reconnect())  {
		ast_log_chan(NULL, LOG_WARNING, "Couldn't establish connection to LDAP directory. Check debug.\n");
	}

	ast_config_engine_register(&ldap_engine);
	ast_verb_chan(NULL, 1, "LDAP RealTime driver loaded.\n");
...
static int unload_module(void)
{
	...
	ast_verb_chan(NULL, 1, "LDAP RealTime driver unloaded.\n");
...
static int reload(void)
{
	...
	if (parse_config() < 0) {
		ast_log_chan(NULL, LOG_NOTICE, "Cannot reload LDAP RealTime driver.\n");
		ast_mutex_unlock(&ldap_lock);
		return 0;
	}

	if (!ldap_reconnect())  {
		ast_log_chan(NULL, LOG_WARNING, "Couldn't establish connection to your directory server. Check debug.\n");
	}

	ast_verb_chan(NULL, 2, "LDAP RealTime driver reloaded.\n");
...
static int parse_config(void)
{
	...
	if (config == CONFIG_STATUS_FILEMISSING || config == CONFIG_STATUS_FILEINVALID) {
		ast_log_chan(NULL, LOG_ERROR, "Cannot load configuration file: %s\n", RES_CONFIG_LDAP_CONF);
		return -1;
	}

	if (!(s = ast_variable_retrieve(config, "_general", "user"))) {
		ast_log_chan(NULL, LOG_NOTICE, "No directory user found, anonymous binding as default.\n");
		user[0] = '\0';
	} else {
	...
	if (!ast_strlen_zero(user)) {
		if (!(s = ast_variable_retrieve(config, "_general", "pass"))) {
			ast_log_chan(NULL, LOG_WARNING, "No directory password found, using 'gabpbx' as default.\n");
			ast_copy_string(pass, "gabpbx", sizeof(pass));
		} else {
	...
	if ((s = ast_variable_retrieve(config, "_general", "url"))) {
		...
		if (!(s = ast_variable_retrieve(config, "_general", "port")) || sscanf(s, "%5d", &port) != 1 || port > 65535) {
			ast_log_chan(NULL, LOG_NOTICE, "No directory port found, using 389 as default.\n");
			port = 389;
		}

		snprintf(url, sizeof(url), "ldap://%s:%d", host, port);
	} else {
		ast_log_chan(NULL, LOG_ERROR, "No directory URL or host found.\n");
		ast_config_destroy(config);
		return -1;
	}

	if (!(s = ast_variable_retrieve(config, "_general", "basedn"))) {
		ast_log_chan(NULL, LOG_ERROR, "No LDAP base dn found, using '%s' as default.\n", RES_CONFIG_LDAP_DEFAULT_BASEDN);
	...
	if (!(s = ast_variable_retrieve(config, "_general", "version")) && !(s = ast_variable_retrieve(config, "_general", "protocol"))) {
		ast_log_chan(NULL, LOG_NOTICE, "No explicit LDAP version found, using 3 as default.\n");
	} else if (sscanf(s, "%30d", &version) != 1 || version < 1 || version > 6) {
		ast_log_chan(NULL, LOG_WARNING, "Invalid LDAP version '%s', using 3 as default.\n", s);
		version = 3;
	}
...
static int ldap_reconnect(void)
{
	...
	if (ast_strlen_zero(url)) {
		ast_log_chan(NULL, LOG_ERROR, "Not enough parameters to connect to ldap directory\n");
		return 0;
	}

	if (LDAP_SUCCESS != ldap_initialize(&ldapConn, url)) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to init ldap connection to '%s'. Check debug for more info.\n", url);
		return 0;
	}

	if (LDAP_OPT_SUCCESS != ldap_set_option(ldapConn, LDAP_OPT_PROTOCOL_VERSION, &version)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to set LDAP protocol version to %d, falling back to default.\n", version);
	}
	...
	if (bind_result == LDAP_SUCCESS) {
		...
		ast_log_chan(NULL, LOG_WARNING, "bind failed: %s\n", ldap_err2string(bind_result));
		ldap_unbind_ext_s(ldapConn, NULL, NULL);
		ldapConn = NULL;
		return 0;
	}
 ...
 *
 */
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "LDAP realtime interface",
