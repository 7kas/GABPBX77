...
static void publish_cluster_discovery_to_stasis_full(struct corosync_node *node, int joined)
{
	...
	ast_log_chan(NULL, AST_LOG_NOTICE, "Node %u (%s) at %s %s the cluster\n",
		node->id,
		eid,
		addr,
		joined ? "joined" : "left");
...
static void publish_mwi_to_stasis(struct ast_event *event)
{
	...
	if (ast_publish_mwi_state_full(mailbox, context, (int)new_msgs,
		...
		ast_log_chan(NULL, LOG_WARNING, "Failed to publish MWI message for %s@%s from %s\n",
			mailbox, context, eid);
	}
}
...
static void publish_device_state_to_stasis(struct ast_event *event)
{
	...
	if (ast_publish_device_state_full(device, state, cachable, event_eid)) {
		char eid[18];
		ast_eid_to_str(eid, sizeof(eid), event_eid);
		ast_log_chan(NULL, LOG_WARNING, "Failed to publish device state message for %s from %s\n",
			device, eid);
	}
}
		...
		uint32_t nodeid, uint32_t pid, void *msg, size_t msg_len)
{
	...
	if (event_type == AST_EVENT_PING) {
		...
		ast_log_chan(NULL, LOG_NOTICE, "Got event PING from server with EID: '%s'\n", buf);
	}
...
static void publish_event_to_corosync(struct ast_event *event)
{
	...
	if (corosync_node_joined && !ast_rwlock_tryrdlock(&init_cpg_lock)) {
		...
		if ((cs_err = cpg_mcast_joined(cpg_handle, CPG_TYPE_FIFO, &iov, 1)) != CS_OK) {
			ast_log_chan(NULL, LOG_WARNING, "CPG mcast failed (%u) for event %s (%u)\n",
		...
		ast_log_chan(NULL, LOG_WARNING, "CPG mcast not executed for event %s (%u): initializing CPG.\n",
				ast_event_get_type_name(event), ast_event_get_type(event));
	}
}
...
static void publish_to_corosync(struct stasis_message *message)
{
	...
	if (ast_event_get_type(event) == AST_EVENT_PING) {
		...
		eid = (struct ast_eid *)ast_event_get_ie_raw(event, AST_EVENT_IE_EID);
		ast_eid_to_str(buf, sizeof(buf), (struct ast_eid *) eid);
		ast_log_chan(NULL, LOG_NOTICE, "Sending event PING from this server with EID: '%s'\n", buf);
	}
		...
		const struct cpg_address *joined_list, size_t joined_list_entries)
{
	...
	for (i = 0; i < left_list_entries; i++) {
		...
		for (j = 0; j < ARRAY_LEN(event_types); j++) {
			...
			ast_log_chan(NULL, LOG_NOTICE, "Clearing %i events of type %s of node %i from stasis cache.\n", messages_count, event_types[j].name, node->id);
			ao2_callback(messages, OBJ_NODATA, clear_node_cache, event_types[j].topic_fn());
			ast_log_chan(NULL, LOG_NOTICE, "Cleared events of type %s from stasis cache.\n", event_types[j].name);
	...
	for (i = 0; i < ARRAY_LEN(event_types); i++) {
		...
		messages_count = ao2_container_count(messages);
		ast_log_chan(NULL, LOG_NOTICE, "Sending %i events of type %s to corosync.\n", messages_count, event_types[i].name);
		ao2_callback(messages, OBJ_NODATA, dump_cache_cb, NULL);
		ast_log_chan(NULL, LOG_NOTICE, "Sent events of type %s to corosync.\n", event_types[i].name);
...
static void send_cluster_notify(void)
{
	...
	if (!ast_rwlock_tryrdlock(&init_cpg_lock)) {
		...
		if ((cs_err = corosync_cfg_local_get(cfg_handle, &node_id)) != CS_OK) {
			ast_log_chan(NULL, LOG_WARNING, "Failed to extract Corosync node ID for this node. Not informing cluster of existance.\n");
			return;
		}

		if (((cs_err = corosync_cfg_get_node_addrs(cfg_handle, node_id, 1, &num_addrs, &corosync_addr)) != CS_OK) || (num_addrs < 1)) {
			ast_log_chan(NULL, LOG_WARNING, "Failed to get local Corosync address. Not informing cluster of existance.\n");
			return;
		}
	...
	if ((res = getnameinfo(sa, sa_len, buf, sizeof(buf), NULL, 0, NI_NUMERICHOST))) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to determine name of local Corosync address: %s (%d). Not informing cluster of existance.\n",
...
static void *dispatch_thread_handler(void *data)
{
	...
	if (!ast_rwlock_tryrdlock(&init_cpg_lock)) {
		...
		if ((cs_err = cpg_fd_get(cpg_handle, &pfd[0].fd)) != CS_OK) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to get CPG fd.  This module is now broken.\n");
			ast_rwlock_unlock(&init_cpg_lock);
		...
		if ((cs_err = corosync_cfg_fd_get(cfg_handle, &pfd[1].fd)) != CS_OK) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to get CFG fd.  This module is now broken.\n");
			ast_rwlock_unlock(&init_cpg_lock);
			ast_debug(5, "dispatch_thread_handler unlock\n");
			return NULL;
		}
		...
		ast_log_chan(NULL, LOG_ERROR, "Failed to get fd: initializing CPG.  This module is now broken.\n");
		return NULL;
	}
	send_cluster_notify();
	while (!dispatch_thread.stop) {
		...
		if (res == -1 && errno != EINTR && errno != EAGAIN) {
			ast_log_chan(NULL, LOG_ERROR, "poll() error: %s (%d)\n", strerror(errno), errno);
			...
			if (!ast_rwlock_tryrdlock(&init_cpg_lock)) {
				...
				if ((cs_err = cpg_local_get(cpg_handle, &local_nodeid)) == CS_OK) {
					...
					ast_copy_string(name.value, "gabpbx", sizeof(name.value));
					...
					if ((cs_err = cpg_membership_get(cpg_handle, &name, address, &entries)) == CS_OK) {
						...
						if (!found) {
							ast_log_chan(NULL, LOG_WARNING, "Failed to check CPG node membership\n");
							corosync_node_joined = 0;
							cs_err = CS_ERR_BAD_HANDLE;
						}
					} else {
						ast_log_chan(NULL, LOG_WARNING, "Failed to get CPG node membership: %u\n", cs_err);
						corosync_node_joined = 0;
						cs_err = CS_ERR_BAD_HANDLE;
					}
				} else {
					ast_log_chan(NULL, LOG_WARNING, "Failed to get CPG local node id: %u\n", cs_err);
					corosync_node_joined = 0;
					cs_err = CS_ERR_BAD_HANDLE;
				}
				ast_rwlock_unlock(&init_cpg_lock);
				...
				ast_log_chan(NULL, LOG_WARNING, "Failed to check CPG node membership: initializing CPG.\n");
				corosync_node_joined = 0;
				cs_err = CS_ERR_BAD_HANDLE;
			}
		} else {
			if (!ast_rwlock_tryrdlock(&init_cpg_lock)) {
				...
				if (pfd[0].revents & POLLIN) {
					if ((cs_err = cpg_dispatch(cpg_handle, CS_DISPATCH_ALL)) != CS_OK) {
						ast_log_chan(NULL, LOG_WARNING, "Failed CPG dispatch: %u\n", cs_err);
					}
				}

				if (pfd[1].revents & POLLIN) {
					if ((cs_err = corosync_cfg_dispatch(cfg_handle, CS_DISPATCH_ALL)) != CS_OK) {
						ast_log_chan(NULL, LOG_WARNING, "Failed CFG dispatch: %u\n", cs_err);
					}
				}
				ast_rwlock_unlock(&init_cpg_lock);
				ast_debug(5, "dispatch_thread_handler unlock\n");
			} else {
				ast_log_chan(NULL, LOG_WARNING, "Failed to dispatch: initializing CPG.\n");
			}
		}
		if (cs_err == CS_ERR_LIBRARY || cs_err == CS_ERR_BAD_HANDLE) {

			/* If corosync gets restarted out from under GABpbx, try to recover. */

			ast_log_chan(NULL, LOG_NOTICE, "Attempting to recover from corosync failure.\n");
			...
			if (!ast_rwlock_trywrlock(&init_cpg_lock)) {
				...
				if (cpg_handle && (cs_err = cpg_finalize(cpg_handle)) != CS_OK) {
					ast_log_chan(NULL, LOG_ERROR, "Failed to finalize cpg (%d)\n", (int) cs_err);
				}

				if (cfg_handle && (cs_err = corosync_cfg_finalize(cfg_handle)) != CS_OK) {
					ast_log_chan(NULL, LOG_ERROR, "Failed to finalize cfg (%d)\n", (int) cs_err);
				}

				if ((cs_err = corosync_cfg_initialize(&cfg_handle, &cfg_callbacks)) != CS_OK) {
					ast_log_chan(NULL, LOG_ERROR, "Failed to initialize cfg (%d)\n", (int) cs_err);
					ast_rwlock_unlock(&init_cpg_lock);
					ast_debug(5, "dispatch_thread_handler unlock\n");
					sleep(5);
					continue;
				...
				if ((cs_err = cpg_initialize(&cpg_handle, &cpg_callbacks) != CS_OK)) {
					ast_log_chan(NULL, LOG_ERROR, "Failed to initialize cpg (%d)\n", (int) cs_err);
					ast_rwlock_unlock(&init_cpg_lock);
					ast_debug(5, "dispatch_thread_handler unlock\n");
					sleep(5);
					continue;
				...
				if ((cs_err = cpg_fd_get(cpg_handle, &pfd[0].fd)) != CS_OK) {
					ast_log_chan(NULL, LOG_ERROR, "Failed to get CPG fd.\n");
					ast_rwlock_unlock(&init_cpg_lock);
					ast_debug(5, "dispatch_thread_handler unlock\n");
					sleep(5);
					continue;
				...
				if ((cs_err = corosync_cfg_fd_get(cfg_handle, &pfd[1].fd)) != CS_OK) {
					ast_log_chan(NULL, LOG_ERROR, "Failed to get CFG fd.\n");
					ast_rwlock_unlock(&init_cpg_lock);
					ast_debug(5, "dispatch_thread_handler unlock\n");
					sleep(5);
					continue;
				...
				if ((cs_err = cpg_join(cpg_handle, &name)) != CS_OK) {
					ast_log_chan(NULL, LOG_ERROR, "Failed to join cpg (%d)\n", (int) cs_err);
					ast_rwlock_unlock(&init_cpg_lock);
					ast_debug(5, "dispatch_thread_handler unlock\n");
					sleep(5);
					continue;
				...
				ast_rwlock_unlock(&init_cpg_lock);
				ast_debug(5, "dispatch_thread_handler unlock\n");
				ast_log_chan(NULL, LOG_NOTICE, "Corosync recovery complete.\n");
				send_cluster_notify();
			} else {
				ast_log_chan(NULL, LOG_NOTICE, "Failed to recover from corosync failure: initializing CPG.\n");
			}
		}
	}
	...
	if (!ast_rwlock_tryrdlock(&init_cpg_lock)) {
		...
		for (i = 1, cs_err = cpg_iteration_next(cpg_iter, &cpg_desc);
			...
			*/
			...
			if (cs_err != CS_OK) {
				ast_log_chan(NULL, LOG_WARNING, "Failed to get node addresses\n");
				continue;
			}
...
static int load_general_config(struct ast_config *cfg)
{
	...
	for (v = ast_variable_browse(cfg, "general"); v && !res; v = v->next) {
		if (!strcasecmp(v->name, "publish_event")) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Unknown option '%s'\n", v->name);
		}
	}
...
static int load_config(unsigned int reload)
{
	...
	while ((cat = ast_category_browse(cfg, cat))) {
		if (!strcasecmp(cat, "general")) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Unknown configuration section '%s'\n", cat);
		}
	}
...
static void cleanup_module(void)
{
	...
	if (stasis_router) {
		...
		for (i = 0; i < ARRAY_LEN(event_types); i++) {
			...
			if (subscribe && event_types[i].cache_fn && event_types[i].message_type_fn) {
				...
				messages_count = ao2_container_count(messages);
				ast_log_chan(NULL, LOG_NOTICE, "Clearing %i events of type %s of other nodes from stasis cache.\n", messages_count, event_types[i].name);
	...
	if (dispatch_thread.id != AST_PTHREADT_NULL) {
		...
		if (ast_carefulwrite(dispatch_thread.alert_pipe[1], &meepmeep, 1,
					5000) == -1) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to write to pipe: %s (%d)\n",
					strerror(errno), errno);
		}
...
static int load_module(void)
{
	...
	if (!corosync_aggregate_topic) {
		ast_log_chan(NULL, AST_LOG_ERROR, "Failed to create stasis topic for corosync\n");
		goto failed;
	}

	stasis_router = stasis_message_router_create(corosync_aggregate_topic);
	if (!stasis_router) {
		ast_log_chan(NULL, AST_LOG_ERROR, "Failed to create message router for corosync topic\n");
		goto failed;
	}
	...
	if (STASIS_MESSAGE_TYPE_INIT(corosync_ping_message_type) != 0) {
		ast_log_chan(NULL, AST_LOG_ERROR, "Failed to initialize corosync ping message type\n");
		goto failed;
	}
	...
	if (!ast_rwlock_trywrlock(&init_cpg_lock)) {
		...
		if ((cs_err = cpg_join(cpg_handle, &name)) != CS_OK) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to join: (%d)\n", (int) cs_err);
			ast_rwlock_unlock(&init_cpg_lock);
			ast_debug(5, "load_module unlock\n");
			goto failed;
		}
		...
		if (pipe(dispatch_thread.alert_pipe) == -1) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to create alert pipe: %s (%d)\n",
					strerror(errno), errno);
			ast_rwlock_unlock(&init_cpg_lock);
			ast_debug(5, "load_module unlock\n");
			goto failed;
		...
		if (corosync_pthread_create_background(&dispatch_thread.id, NULL,
			...
			ast_log_chan(NULL, LOG_ERROR, "Error starting CPG dispatch thread.\n");
			goto failed;
		}
...
AST_MODULE_INFO_STANDARD_EXTENDED(GABPBX_GPL_KEY, "Corosync");
