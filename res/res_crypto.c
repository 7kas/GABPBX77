...
#include "gabpbx/cli.h"           /* for ast_cli, ast_cli_args, ast_cli_entry */
#include "gabpbx/compat.h"        /* for strcasecmp */
#include "gabpbx/io.h"            /* for ast_hide_password, ast_restore_tty */
#include "gabpbx/linkedlists.h"   /* for AST_RWLIST_TRAVERSE, AST_RWLIST_U... */
#include "gabpbx/logger.h"        /* for ast_log, LOG_WARNING, LOG_NOTICE */
#include "gabpbx/md5.h"           /* for MD5Final, MD5Init, MD5Update, MD5... */
#include "gabpbx/module.h"        /* for ast_module_flags::AST_MODFLAG_GLO... */
#include "gabpbx/options.h"       /* for ast_opt_init_keys */
#include "gabpbx/paths.h"         /* for ast_config_AST_KEY_DIR */
#include "gabpbx/utils.h"         /* for ast_copy_string, ast_base64decode */
#include "gabpbx/file.h"          /* for ast_file_read_dirs */

#define AST_API_MODULE
#include "gabpbx/crypto.h"        /* for AST_KEY_PUBLIC, AST_KEY_PRIVATE */

/*
 * GABpbx uses RSA keys with SHA-1 message digests for its
...
static struct ast_key *try_load_key(const char *dir, const char *fname, int ifd, int ofd, int *not2)
{
	...
	/* Open file */
	if (!(f = fopen(ffname, "r"))) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to open key file %s: %s\n", ffname, strerror(errno));
		return NULL;
	}

	n = fstat(fileno(f), &st);
	if (n != 0) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to stat key file: %s: %s\n", ffname, strerror(errno));
		fclose(f);
		return NULL;
	}

	if (!S_ISREG(st.st_mode)) {
		ast_log_chan(NULL, LOG_ERROR, "Key file is not a regular file: %s\n", ffname);
		fclose(f);
		return NULL;
	}
...
#define FILE_MODE_BITS (S_ISUID|S_ISGID|S_ISVTX|S_IRWXU|S_IRWXG|S_IRWXO)
	...
	if (ktype == AST_KEY_PRIVATE &&
		...
		ast_log_chan(NULL, LOG_ERROR, "Private key file has bad permissions: %s: %#4o\n", ffname, st.st_mode & FILE_MODE_BITS);
		fclose(f);
		return NULL;
	}
	...
	if (key->pkey) {
		if (EVP_PKEY_size(key->pkey) == (AST_CRYPTO_RSA_KEY_BITS / 8)) {
			/* Key loaded okay */
			key->ktype &= ~KEY_NEEDS_PASSCODE;
			ast_verb_chan(NULL, 3, "Loaded %s key '%s'\n", key->ktype == AST_KEY_PUBLIC ? "PUBLIC" : "PRIVATE", key->name);
			ast_debug(1, "Key '%s' loaded OK\n", key->name);
			key->delme = 0;
		} else {
			ast_log_chan(NULL, LOG_NOTICE, "Key '%s' is not expected size.\n", key->name);
		}
	} else if (key->infd != -2) {
		ast_log_chan(NULL, LOG_WARNING, "Key load %s '%s' failed\n", key->ktype == AST_KEY_PUBLIC ? "PUBLIC" : "PRIVATE", key->name);
		if (ofd > -1) {
		...
		ast_log_chan(NULL, LOG_NOTICE, "Key '%s' needs passcode.\n", key->name);
		key->ktype |= KEY_NEEDS_PASSCODE;
		if (!notice) {
			if (!ast_opt_init_keys) {
				ast_log_chan(NULL, LOG_NOTICE, "Add the '-i' flag to the gabpbx command line if you want to automatically initialize passcodes at launch.\n");
			}
			notice++;
		}
		/* Keep it anyway */
...
int AST_OPTIONAL_API_NAME(ast_sign_bin)(struct ast_key *key, const char *msg, int msglen, unsigned char *dsig)
{
	...
	if (key->ktype != AST_KEY_PRIVATE) {
		ast_log_chan(NULL, LOG_WARNING, "Cannot sign with a public key\n");
		return -1;
	}

	if (siglen < EVP_PKEY_size(key->pkey)) {
		ast_log_chan(NULL, LOG_WARNING, "Signature buffer too small\n");
		return -1;
	}
	...
	if ((res = evp_pkey_sign(key->pkey, digest, sizeof(digest), dsig, &siglen, RSA_PKCS1_PADDING)) <= 0) {
		ast_log_chan(NULL, LOG_WARNING, "RSA Signature (key %s) failed %d\n", key->name, res);
		return -1;
	}

	if (siglen != EVP_PKEY_size(key->pkey)) {
		ast_log_chan(NULL, LOG_WARNING, "Unexpected signature length %u, expecting %d\n", siglen, EVP_PKEY_size(key->pkey));
		return -1;
	}
...
int AST_OPTIONAL_API_NAME(ast_decrypt_bin)(unsigned char *dst, const unsigned char *src, int srclen, struct ast_key *key)
{
	...
	if (key->ktype != AST_KEY_PRIVATE) {
		ast_log_chan(NULL, LOG_WARNING, "Cannot decrypt with a public key\n");
		return -1;
	}
	...
	if (srclen % blocksize) {
		ast_log_chan(NULL, LOG_NOTICE, "Tried to decrypt something not a multiple of %u bytes\n", blocksize);
		return -1;
	}
...
static int evp_pkey_encrypt(EVP_PKEY *pkey, const unsigned char *in, unsigned inlen, unsigned char *out, unsigned *outlen, unsigned padding)
{
	...
	if (padding != RSA_PKCS1_OAEP_PADDING) {
		ast_log_chan(NULL, LOG_WARNING, "Only OAEP padding is supported for now\n");
		return -1;
	}
...
int AST_OPTIONAL_API_NAME(ast_encrypt_bin)(unsigned char *dst, const unsigned char *src, int srclen, struct ast_key *key)
{
	...
	if (key->ktype != AST_KEY_PUBLIC) {
		ast_log_chan(NULL, LOG_WARNING, "Cannot encrypt with a private key\n");
		return -1;
	}
	...
	while (srclen) {
		...
		if ((res = evp_pkey_encrypt(key->pkey, src, bytes, dst, &dstlen, RSA_PKCS1_OAEP_PADDING)) != blocksize) {
			ast_log_chan(NULL, LOG_NOTICE, "How odd, encrypted size is %d\n", res);
			return -1;
		}
		src += bytes;
		srclen -= bytes;
...
int AST_OPTIONAL_API_NAME(ast_check_signature_bin)(struct ast_key *key, const char *msg, int msglen, const unsigned char *dsig)
{
	...
	if (key->ktype != AST_KEY_PUBLIC) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Cannot check message signature with a private key\n");
		return -1;
	}
...
int AST_OPTIONAL_API_NAME(ast_check_signature)(struct ast_key *key, const char *msg, const char *sig)
{
	...
	if ((res = ast_base64decode(dsig, sig, sizeof(dsig))) != sizeof(dsig)) {
		ast_log_chan(NULL, LOG_WARNING, "Signature improper length (expect %d, got %d)\n", (int)sizeof(dsig), (int)res);
		return -1;
	}
...
int AST_OPTIONAL_API_NAME(ast_aes_encrypt)(const unsigned char *in, unsigned char *out, const ast_aes_encrypt_key *key)
{
	...
	if ((res = evp_cipher_aes_encrypt(in, out, AST_CRYPTO_AES_BLOCKSIZE / 8, key)) <= 0) {
		ast_log_chan(NULL, LOG_ERROR, "AES encryption failed\n");
	}
	return res;
}
...
int AST_OPTIONAL_API_NAME(ast_aes_decrypt)(const unsigned char *in, unsigned char *out, const ast_aes_decrypt_key *key)
{
	...
	if ((res = evp_cipher_aes_decrypt(in, out, AST_CRYPTO_AES_BLOCKSIZE / 8, key)) <= 0) {
		ast_log_chan(NULL, LOG_ERROR, "AES decryption failed\n");
	}
	return res;
}
...
static void crypto_load(int ifd, int ofd)
{
	...
	if (ast_file_read_dirs(ast_config_AST_KEY_DIR, crypto_load_cb, &on_file, 1) == -1) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to open key directory '%s'\n", ast_config_AST_KEY_DIR);
	}

	if (on_file.note) {
		ast_log_chan(NULL, LOG_NOTICE, "Please run the command 'keys init' to enter the passcodes for the keys\n");
	}
...
#define FORMAT "%-18s %-8s %-16s %-33s\n"
	...
	case CLI_INIT:
		...
		e->usage =
			"Usage: keys show\n"
			"       Displays information about RSA keys known by GABpbx\n";
		return NULL;
	case CLI_GENERATE:
		return NULL;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER, "Cryptographic Digital Signatures",
