 ...
 *
 * T.38-gateway integration into gabpbx app_fax and rework
 * 2008-2011, Gregory Hinton Nietsky <gregory@distrotech.co.za>
 * dns Telecom http://www.dnstelecom.co.za
 *
 * Modified to make T.38-gateway compatible with GABpbx 1.6.2
		...
		<managerEventInstance class="EVENT_FLAG_REPORTING">
			...
			<syntax>
				...
				<parameter name="TransmitAttempts" required="true">
					<para>Total FAX sessions for which GABpbx is/was the transmitter</para>
				</parameter>
				<parameter name="ReceiveAttempts" required="true">
					<para>Total FAX sessions for which GABpbx is/was the recipient</para>
				</parameter>
...
static void debug_check_frame_for_silence(struct ast_fax_session *s, unsigned int c2s, struct ast_frame *frame)
{
	...
	if (history->silence != wassil) {
		...
		if ((last_consec_frames != 0)) {
			ast_verb_chan(NULL, 0, "Channel '%s' fax session '%u', [ %.3ld.%.6ld ], %s sent %u frames (%u ms) of %s.\n",
...
static struct ast_fax_session_details *find_details(struct ast_channel *chan)
{
	...
	if (!(details = datastore->data)) {
		ast_log_chan(NULL, LOG_WARNING, "Huh?  channel '%s' has a FAX datastore without data!\n", ast_channel_name(chan));
		ast_channel_unlock(chan);
		return NULL;
	}
	ao2_ref(details, 1);
...
static struct ast_fax_session_details *find_or_create_details(struct ast_channel *chan)
{
	...
	if (!(details = session_details_new())) {
		ast_log_chan(NULL, LOG_WARNING, "channel '%s' can't get a FAX details structure for the datastore!\n", ast_channel_name(chan));
		return NULL;
	}
	if (!(datastore = ast_datastore_alloc(&fax_datastore, NULL))) {
		ao2_ref(details, -1);
		ast_log_chan(NULL, LOG_WARNING, "channel '%s' can't get a datastore!\n", ast_channel_name(chan));
		return NULL;
	}
	...
	for (j = 0; j < i; j++) {
		if (!strcasecmp(m[j], "v17")) {
			...
			*bits |= AST_FAX_MODEM_V34;
			...
			ast_log_chan(NULL, LOG_WARNING, "ignoring invalid modem setting: '%s', valid options {v17 | v27 | v29 | v34}\n", m[j]);
		}
	}
	return 0;
}
...
int ast_fax_tech_register(struct ast_fax_tech *tech)
{
	...
	ast_verb_chan(NULL, 3, "Registered handler for '%s' (%s)\n", fax->tech->type, fax->tech->description);
...
void ast_fax_tech_unregister(struct ast_fax_tech *tech)
{
	...
	ast_verb_chan(NULL, 3, "Unregistering FAX module type '%s'\n", tech->type);
	...
	AST_RWLIST_TRAVERSE_SAFE_BEGIN(&faxmodules, fax, list) {
		...
		ast_free(fax);
		ast_verb_chan(NULL, 4, "Unregistered FAX module type '%s'\n", tech->type);
		break;
	}
...
const char *ast_fax_state_to_str(enum ast_fax_state state)
{
	...
	case AST_FAX_STATE_INACTIVE:
		return "Inactive";
	default:
		ast_log_chan(NULL, LOG_WARNING, "unhandled FAX state: %u\n", state);
		return "Unknown";
	}
}
...
void ast_fax_log(int level, const char *file, const int line, const char *function, const char *msg)
{
	if (fax_logger_level != -1) {
		...
		ast_log_chan(NULL, level, file, line, function, "%s", msg);
	}
}
...
static unsigned int fax_rate_str_to_int(const char *ratestr)
{
	...
	if (sscanf(ratestr, "%d", &rate) != 1) {
		ast_log_chan(NULL, LOG_ERROR, "failed to sscanf '%s' to rate\n", ratestr);
		return 0;
	}
	switch (rate) {
	case 2400:
	...
	case 33600:
		return rate;
	default:
		ast_log_chan(NULL, LOG_WARNING, "ignoring invalid rate '%s'.  Valid options are {2400 | 4800 | 7200 | 9600 | 12000 | 14400 | 28800 | 33600}\n", ratestr);
		return 0;
	}
}
...
static struct ast_fax_session *fax_session_reserve(struct ast_fax_session_details *details, struct ast_fax_tech_token **token)
{
	...
	if (!faxmod) {
		char caps[128] = "";
		ast_log_chan(NULL, LOG_ERROR, "Could not locate a FAX technology module with capabilities (%s)\n", ast_fax_caps_to_str(details->caps, caps, sizeof(caps)));
		ao2_ref(s, -1);
		return NULL;
	}
...
static struct ast_fax_session *fax_session_new(struct ast_fax_session_details *details, struct ast_channel *chan, struct ast_fax_session *reserved, struct ast_fax_tech_token *token)
{
	...
	if (!(s->tech_pvt = s->tech->new_session(s, token))) {
		ast_log_chan(NULL, LOG_ERROR, "FAX session failed to initialize.\n");
		ao2_ref(s, -1);
		return NULL;
	}
	/* link the session to the session container */
	if (!(ao2_link(faxregistry.container, s))) {
		ast_log_chan(NULL, LOG_ERROR, "failed to add FAX session '%u' to container.\n", s->id);
		ao2_ref(s, -1);
		return NULL;
	}
...
#define GENERIC_FAX_EXEC_ERROR(fax, chan, errorstr, reason)	\
	do {	\
		ast_log_chan(NULL, LOG_ERROR, "channel '%s' FAX session '%u' failure, reason: '%s' (%s)\n", ast_channel_name(chan), fax->id, reason, errorstr); \
...
static int set_fax_t38_caps(struct ast_channel *chan, struct ast_fax_session_details *details)
{
	...
	case T38_STATE_NEGOTIATING: {
		...
		if (ast_indicate_data(chan, AST_CONTROL_T38_PARAMETERS, &parameters, sizeof(parameters)) != AST_T38_REQUEST_PARMS) {
			ast_log_chan(NULL, LOG_ERROR, "channel '%s' is in an unsupported T.38 negotiation state, cannot continue.\n", ast_channel_name(chan));
			return -1;
		}
		details->caps |= AST_FAX_TECH_T38;
		break;
	...
	while ((ms = ast_remaining_ms(start, timeout_ms))) {
		...
		if ((frame->frametype == AST_FRAME_CONTROL) &&
			...
			case AST_T38_REFUSED:
				ast_log_chan(NULL, LOG_WARNING, "channel '%s' refused to disable T.38\n", ast_channel_name(chan));
				ast_frfree(frame);
				return -1;
			default:
				ast_log_chan(NULL, LOG_ERROR, "channel '%s' failed to disable T.38\n", ast_channel_name(chan));
				ast_frfree(frame);
				return -1;
			}
			ast_frfree(frame);
...
static int generic_fax_exec(struct ast_channel *chan, struct ast_fax_session_details *details, struct ast_fax_session *reserved, struct ast_fax_tech_token *token)
{
	...
	if (!(fax = fax_session_new(details, chan, reserved, token))) {
		ast_log_chan(NULL, LOG_ERROR, "Can't create a FAX session, FAX attempt failed.\n");
	...
	if (details->caps & AST_FAX_TECH_AUDIO) {
		...
		if (ast_set_write_format(chan, ast_format_slin) < 0) {
			ast_log_chan(NULL, LOG_ERROR, "channel '%s' failed to set write format to signed linear'.\n", ast_channel_name(chan));
		...
		if (ast_set_read_format(chan, ast_format_slin) < 0) {
			ast_log_chan(NULL, LOG_ERROR, "channel '%s' failed to set read format to signed linear.\n", ast_channel_name(chan));
			ao2_unlink(faxregistry.container, fax);
			ao2_ref(fax, -1);
			return -1;
		}
		...
		if (!(fax->smoother = ast_smoother_new(320))) {
			ast_log_chan(NULL, LOG_WARNING, "Channel '%s' FAX session '%u' failed to obtain a smoother.\n", ast_channel_name(chan), fax->id);
		}
	} else {
		...
		if (ready_chan) {
			...
			if ((frame->frametype == AST_FRAME_CONTROL) &&
				...
				if (t38negotiated && !was_t38) {
					...
					ast_verb_chan(NULL, 3, "Channel '%s' switched to T.38 FAX session '%u'.\n", ast_channel_name(chan), fax->id);
				}
			...
			if (ms && (ofd < 0)) {
				if ((errno == 0) || (errno == EINTR)) {
					...
					ast_log_chan(NULL, LOG_WARNING, "something bad happened while channel '%s' was polling.\n", ast_channel_name(chan));
...
static int receivefax_t38_init(struct ast_channel *chan, struct ast_fax_session_details *details)
{
	...
	if (ast_channel_get_t38_state(chan) != T38_STATE_NEGOTIATING) {
		...
		if (ast_playtones_start(chan, 1024, "!2100/3000", 1)) {
			ast_log_chan(NULL, LOG_ERROR, "error generating CED tone on %s\n", ast_channel_name(chan));
			return -1;
		}
		...
		while ((ms = ast_remaining_ms(start, timeout_ms))) {
			...
			if (ms < 0) {
				ast_log_chan(NULL, LOG_ERROR, "error while generating CED tone on %s\n", ast_channel_name(chan));
			...
			if (!(frame = ast_read(chan))) {
				ast_log_chan(NULL, LOG_ERROR, "error reading frame while generating CED tone on %s\n", ast_channel_name(chan));
				ast_playtones_stop(chan);
				return -1;
			}
	...
	while ((ms = ast_remaining_ms(start, timeout_ms))) {
		...
		ms = ast_waitfor(chan, ms);
		if (ms < 0) {
			ast_log_chan(NULL, LOG_WARNING, "error on '%s' while waiting for T.38 negotiation.\n", ast_channel_name(chan));
			return -1;
		}
		if (ms == 0) { /* all done, nothing happened */
			ast_log_chan(NULL, LOG_WARNING, "channel '%s' timed-out during the T.38 negotiation.\n", ast_channel_name(chan));
			details->caps &= ~AST_FAX_TECH_T38;
			break;
		}
		...
		if ((frame->frametype == AST_FRAME_CONTROL) &&
			...
			case AST_T38_NEGOTIATED:
				...
				break;
			case AST_T38_REFUSED:
				ast_log_chan(NULL, LOG_WARNING, "channel '%s' refused to negotiate T.38\n", ast_channel_name(chan));
				details->caps &= ~AST_FAX_TECH_T38;
				break_loop = 1;
				break;
			default:
				ast_log_chan(NULL, LOG_ERROR, "channel '%s' failed to negotiate T.38\n", ast_channel_name(chan));
				details->caps &= ~AST_FAX_TECH_T38;
				break_loop = 1;
				break;
			}
	...
	if (details->option.allow_audio != AST_FAX_OPTFLAG_TRUE) {
		ast_log_chan(NULL, LOG_WARNING, "Audio FAX not allowed on channel '%s' and T.38 negotiation failed; aborting.\n", ast_channel_name(chan));
		return -1;
	}
...
static int receivefax_exec(struct ast_channel *chan, const char *data)
{
	...
	if (!(details = find_or_create_details(chan))) {
		...
		ast_log_chan(NULL, LOG_ERROR, "System cannot provide memory for session requirements.\n");
		return -1;
	}
	...
	if (details->gateway_id > 0) {
		...
		set_channel_variables(chan, details);
		ast_log_chan(NULL, LOG_ERROR, "executing ReceiveFAX on a channel with a T.38 Gateway is not supported\n");
		return -1;
	}
	...
	if (details->maxrate < details->minrate) {
		...
		set_channel_variables(chan, details);
		ast_log_chan(NULL, LOG_ERROR, "maxrate %u is less than minrate %u\n", details->maxrate, details->minrate);
		return -1;
	}

	if (check_modem_rate(details->modems, details->minrate)) {
		ast_fax_modem_to_str(details->modems, modems, sizeof(modems));
		ast_log_chan(NULL, LOG_ERROR, "'modems' setting '%s' is incompatible with 'minrate' setting %u\n", modems, details->minrate);
		ast_string_field_set(details, error, "INVALID_ARGUMENTS");
	...
	if (check_modem_rate(details->modems, details->maxrate)) {
		ast_fax_modem_to_str(details->modems, modems, sizeof(modems));
		ast_log_chan(NULL, LOG_ERROR, "'modems' setting '%s' is incompatible with 'maxrate' setting %u\n", modems, details->maxrate);
		ast_string_field_set(details, error, "INVALID_ARGUMENTS");
	...
	if (ast_strlen_zero(data)) {
		...
		set_channel_variables(chan, details);
		ast_log_chan(NULL, LOG_WARNING, "%s requires an argument (filename[,options])\n", app_receivefax);
		return -1;
	}
	...
	if (ast_test_flag(&opts, OPT_CALLERMODE) || ast_test_flag(&opts, OPT_CALLEDMODE)) {
		...
		ast_string_field_set(details, resultstr, "invalid arguments");
		set_channel_variables(chan, details);
		ast_log_chan(NULL, LOG_WARNING, "%s does not support polling\n", app_receivefax);
		return -1;
	}
	...
	ast_verb_chan(NULL, 3, "Channel '%s' receiving FAX '%s'\n", ast_channel_name(chan), args.filename);
	...
	if (!(s = fax_session_reserve(details, &token))) {
		...
		set_channel_variables(chan, details);
		ast_log_chan(NULL, LOG_ERROR, "Unable to reserve FAX session.\n");
		return -1;
	}
	...
	if (ast_channel_state(chan) != AST_STATE_UP) {
		if (ast_answer(chan)) {
			...
			set_channel_variables(chan, details);
			ast_log_chan(NULL, LOG_WARNING, "Channel '%s' failed answer attempt.\n", ast_channel_name(chan));
			fax_session_release(s, token);
			return -1;
		}
	}
	...
	if (!ast_test_flag(&opts, OPT_FORCE_AUDIO) && (details->caps & AST_FAX_TECH_T38)) {
		if (receivefax_t38_init(chan, details)) {
			...
			set_channel_variables(chan, details);
			fax_session_release(s, token);
			ast_log_chan(NULL, LOG_ERROR, "error initializing channel '%s' in T.38 mode\n", ast_channel_name(chan));
			return -1;
		}
	}
	...
	if (report_receive_fax_status(chan, args.filename)) {
		ast_log_chan(NULL, AST_LOG_ERROR, "Error publishing ReceiveFAX status message\n");
	}
...
static int sendfax_t38_init(struct ast_channel *chan, struct ast_fax_session_details *details)
{
	...
	if (ast_channel_get_t38_state(chan) != T38_STATE_NEGOTIATING) {
		if (ast_playtones_start(chan, 1024, "!1100/500,!0/3000,!1100/500,!0/3000,!1100/500,!0/3000", 1)) {
			ast_log_chan(NULL, LOG_ERROR, "error generating CNG tone on %s\n", ast_channel_name(chan));
			return -1;
		}
	}
	...
	while ((ms = ast_remaining_ms(start, timeout_ms))) {
		...
		if (ms < 0) {
			ast_log_chan(NULL, LOG_ERROR, "error while generating CNG tone on %s\n", ast_channel_name(chan));
			ast_playtones_stop(chan);
			return -1;
		}
		...
		if (!(frame = ast_read(chan))) {
			ast_log_chan(NULL, LOG_ERROR, "error reading frame while generating CNG tone on %s\n", ast_channel_name(chan));
			ast_playtones_stop(chan);
			return -1;
		}
	...
	if (details->option.request_t38 == AST_FAX_OPTFLAG_TRUE) {
		...
		if (details->option.allow_audio == AST_FAX_OPTFLAG_TRUE) {
			if (ast_playtones_start(chan, 1024, "!1100/500,!0/3000", 1)) {
				ast_log_chan(NULL, LOG_ERROR, "error generating second CNG tone on %s\n", ast_channel_name(chan));
				return -1;
			}
			...
			while ((ms = ast_remaining_ms(start, timeout_ms))) {
				...
				ms = ast_waitfor(chan, ms);
				if (ms < 0) {
					ast_log_chan(NULL, LOG_ERROR, "error while generating second CNG tone on %s\n", ast_channel_name(chan));
					ast_playtones_stop(chan);
					return -1;
				}
				if (ms == 0) { /* all done, nothing happened */
				...
				if (!(frame = ast_read(chan))) {
					ast_log_chan(NULL, LOG_ERROR, "error reading frame while generating second CNG tone on %s\n", ast_channel_name(chan));
					ast_playtones_stop(chan);
					return -1;
				}
...
static int sendfax_exec(struct ast_channel *chan, const char *data)
{
	...
	if (details->gateway_id > 0) {
		...
		set_channel_variables(chan, details);
		ast_log_chan(NULL, LOG_ERROR, "executing SendFAX on a channel with a T.38 Gateway is not supported\n");
		return -1;
	}
	...
	if (ast_strlen_zero(data)) {
		...
		ast_string_field_set(details, resultstr, "invalid arguments");
		set_channel_variables(chan, details);
		ast_log_chan(NULL, LOG_WARNING, "%s requires an argument (filename[&filename[&filename]][,options])\n", app_sendfax);
		return -1;
	}
	parse = ast_strdupa(data);
	AST_STANDARD_APP_ARGS(args, parse);
	...
	while ((c = strsep(&filenames, "&"))) {
		if (access(c, (F_OK | R_OK)) < 0) {
			...
			set_channel_variables(chan, details);
			ast_log_chan(NULL, LOG_ERROR, "access failure.  Verify '%s' exists and check permissions.\n", args.filenames);
			return -1;
		}
	...
	ast_verb_chan(NULL, 3, "Channel '%s' sending FAX:\n", ast_channel_name(chan));
	AST_LIST_TRAVERSE(&details->documents, doc, next) {
		ast_verb_chan(NULL, 3, "   %s\n", doc->filename);
	}
	...
	if (!(filenames = generate_filenames_string(details, "FileName: ", "\r\n"))) {
		ast_log_chan(NULL, LOG_ERROR, "Error generating SendFAX manager event\n");
		return (!channel_alive) ? -1 : 0;
	}

	/* send out the AMI completion event */
	if (report_send_fax_status(chan, details)) {
		ast_log_chan(NULL, AST_LOG_ERROR, "Error publishing SendFAX status message\n");
	}
...
static struct fax_gateway *fax_gateway_new(struct ast_channel *chan, struct ast_fax_session_details *details)
{
	...
	if (!(gateway->chan_v21_session = fax_v21_session_new(chan))) {
		ast_log_chan(NULL, LOG_ERROR, "Can't create V21 session on chan %s for T.38 gateway session\n", ast_channel_name(chan));
		ao2_ref(gateway, -1);
		return NULL;
	}
	...
	if (details->gateway_timeout && !(gateway->s = fax_session_reserve(details, &gateway->token))) {
		details->caps &= ~AST_FAX_TECH_GATEWAY;
		ast_log_chan(NULL, LOG_ERROR, "Can't reserve a FAX session, gateway attempt failed.\n");
		ao2_ref(gateway, -1);
		return NULL;
	}
...
static int fax_gateway_start(struct fax_gateway *gateway, struct ast_fax_session_details *details, struct ast_channel *chan)
{
	...
	if (!(s = fax_session_new(details, chan, gateway->s, gateway->token))) {
		...
		set_channel_variables(chan, details);
		report_fax_status(chan, details, "No Available Resource");
		ast_log_chan(NULL, LOG_ERROR, "Can't create a FAX session, gateway attempt failed.\n");
		return -1;
	}
...
static struct ast_frame *fax_gateway_request_t38(struct fax_gateway *gateway, struct ast_channel *chan)
{
	...
	if (!details) {
		ast_log_chan(NULL, LOG_ERROR, "no FAX session details found on chan %s for T.38 gateway session, odd\n", ast_channel_name(chan));
	...
	if (!(fp = ast_frisolate(&control_frame))) {
		ast_log_chan(NULL, LOG_ERROR, "error generating T.38 request control frame on chan %s for T.38 gateway session\n", ast_channel_name(chan));
		return NULL;
	}
...
static struct ast_frame *fax_gateway_detect_v21(struct fax_gateway *gateway, struct ast_channel *chan, struct ast_channel *peer, struct ast_channel *active, struct ast_frame *f)
{
	...
	if (gateway->detected_v21) {
		 ...
		 * Find out if this has been overridden. If so, instead of waiting have GABpbx
		...
		if (details) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Detect v21 - no session details for channel '%s'\n",
					ast_channel_name(chan));
		}
	...
	if (control_params->request_response == AST_T38_REQUEST_NEGOTIATE) {
		...
		if (state_other == T38_STATE_UNKNOWN) {
			...
			if (fax_gateway_start(gateway, details, chan)) {
				ast_log_chan(NULL, LOG_ERROR, "error starting T.38 gateway for T.38 channel %s and G.711 channel %s\n", ast_channel_name(active), ast_channel_name(other));
			...
			ast_log_chan(NULL, LOG_WARNING, "%s is attempting to negotiate T.38 while %s is in an unsupported state\n", ast_channel_name(active), ast_channel_name(other));
			ao2_ref(details, -1);
			return f;
		}
	} else if (gateway->t38_state == T38_STATE_NEGOTIATING
...
static struct ast_frame *fax_gateway_framehook(struct ast_channel *chan, struct ast_frame *f, enum ast_framehook_event event, void *data)
{
	...
	case AST_FRAMEHOOK_EVENT_READ:
		...
		break;
	default:
		ast_log_chan(NULL, LOG_WARNING, "unhandled framehook event %u\n", event);
		return f;
	}
	...
	if (gateway->t38_state == T38_STATE_NEGOTIATED) {
		...
		if (!gateway->s || !gateway->s->tech_pvt) {
			ast_log_chan(NULL, LOG_ERROR, "no FAX session on chan %s for T.38 gateway session, odd", ast_channel_name(chan));
			return f;
		}
	...
	if (result) {
		...
		case 't':
			...
			if (ast_exists_extension(chan, target_context, "fax", 1,
				...
				ast_verb_chan(NULL, 2, "Redirecting '%s' to fax extension due to %s detection\n",
				...
				ast_log_chan(NULL, LOG_NOTICE, "FAX %s detected but no fax extension in context (%s)\n",
...
static char *cli_fax_show_version(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	case CLI_INIT:
		...
		e->usage =
			...
			"       Show versions of FAX For GABpbx components.\n";
		return NULL;
	case CLI_GENERATE:
		return NULL;
	}
	...
	ast_cli(a->fd, "FAX For GABpbx Components:\n");
...
static char *cli_fax_show_settings(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	ast_cli(a->fd, "FAX For GABpbx Settings:\n");
...
static char *cli_fax_show_session(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	if (sscanf(a->argv[3], "%u", &tmp.id) != 1) {
		ast_log_chan(NULL, LOG_ERROR, "invalid session id: '%s'\n", a->argv[3]);
	...
	struct ast_fax_session *session, const char *id_text)
{
	...
	if (!filenames) {
		ast_log_chan(NULL, LOG_ERROR, "Error generating Files string");
		ao2_unlock(session);
		return -1;
	}
...
static struct ast_cli_entry fax_cli[] = {
	AST_CLI_DEFINE(cli_fax_show_version, "Show versions of FAX For GABpbx components"),
...
static int set_t38timeout(const char *value, unsigned int *t38timeout)
{
	...
	if (sscanf(value, "%u", &timeout) != 1) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to get timeout from '%s'\n", value);
		return -1;
	} else if (timeout) {
		*t38timeout = timeout;
	} else {
		ast_log_chan(NULL, LOG_ERROR, "T.38 negotiation timeout must be non-zero\n");
		return -1;
	}
...
static int set_config(int reload)
{
	...
	if (!(cfg = ast_config_load2(config, "res_fax", config_flags))) {
		ast_log_chan(NULL, LOG_NOTICE, "Configuration file '%s' not found, %s options.\n",
				config, reload ? "not changing" : "using default");
		return 0;
	}

	if (cfg == CONFIG_STATUS_FILEINVALID) {
		ast_log_chan(NULL, LOG_NOTICE, "Configuration file '%s' is invalid, %s options.\n",
				config, reload ? "not changing" : "using default");
		return 0;
	}
...
static int acf_faxopt_read(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	if (!details) {
		ast_log_chan(NULL, LOG_ERROR, "channel '%s' can't read FAXOPT(%s) because it has never been written.\n", ast_channel_name(chan), data);
		return -1;
	}
	if (!strcasecmp(data, "ecm")) {
		...
		if (AST_LIST_EMPTY(&details->documents)) {
			...
			ast_log_chan(NULL, LOG_ERROR, "channel '%s' can't read FAXOPT(%s), there was an error generating the filenames list.\n", ast_channel_name(chan), data);
			res = -1;
		}
		...
		ast_log_chan(NULL, LOG_WARNING, "channel '%s' can't read FAXOPT(%s) because it is unhandled!\n", ast_channel_name(chan), data);
		res = -1;
	}
	ao2_ref(details, -1);
...
static int acf_faxopt_write(struct ast_channel *chan, const char *cmd, char *data, const char *value)
{
	...
	if (!(details = find_or_create_details(chan))) {
		ast_log_chan(NULL, LOG_WARNING, "channel '%s' can't set FAXOPT(%s) to '%s' because it failed to create a datastore.\n", ast_channel_name(chan), data, value);
		return -1;
	}
	...
	if (!strcasecmp(data, "ecm")) {
		...
		if (ast_true(val)) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Unsupported value '%s' passed to FAXOPT(ecm).\n", value);
		}
	} else if (!strcasecmp(data, "t38gateway") || !strcasecmp(data, "gateway") ||
		   !strcasecmp(data, "t38_gateway") || !strcasecmp(data, "faxgateway")) {
		const char *val = ast_skip_blanks(value);
		...
		if (ast_true(val)) {
			if (details->gateway_id < 0) {
				...
				if (timeout) {
					...
					if (sscanf(timeout, "%30u", &gwtimeout) == 1) {
						...
						ast_log_chan(NULL, LOG_WARNING, "Unsupported timeout '%s' passed to FAXOPT(%s).\n", timeout, data);
					}
				}

				details->gateway_id = fax_gateway_attach(chan, details);
				if (details->gateway_id < 0) {
					ast_log_chan(NULL, LOG_ERROR, "Error attaching T.38 gateway to channel %s.\n", ast_channel_name(chan));
					res = -1;
				} else {
					ast_debug(1, "Attached T.38 gateway to channel %s.\n", ast_channel_name(chan));
				}
			} else {
				ast_log_chan(NULL, LOG_WARNING, "Attempt to attach a T.38 gateway on channel (%s) with gateway already running.\n", ast_channel_name(chan));
			}
		} else if (ast_false(val)) {
			ast_channel_lock(chan);
			ast_framehook_detach(chan, details->gateway_id);
			...
			ast_log_chan(NULL, LOG_WARNING, "Unsupported value '%s' passed to FAXOPT(%s).\n", value, data);
		}
	} else if (!strcasecmp(data, "faxdetect")) {
		const char *val = ast_skip_blanks(value);
		char *timeout = strchr(val, ',');
		...
		if (ast_true(val) || !strcasecmp(val, "t38") || !strcasecmp(val, "cng")) {
			if (details->faxdetect_id < 0) {
				...
				if (faxdetect < 0) {
					ast_log_chan(NULL, LOG_ERROR, "Error attaching FAX detect to channel %s.\n", ast_channel_name(chan));
					res = -1;
				} else {
					ast_debug(1, "Attached FAX detect to channel %s.\n", ast_channel_name(chan));
				}
			} else {
				ast_log_chan(NULL, LOG_WARNING, "Attempt to attach a FAX detect on channel (%s) with FAX detect already running.\n", ast_channel_name(chan));
			}
		} else if (ast_false(val)) {
			ast_channel_lock(chan);
			ast_framehook_detach(chan, details->faxdetect_id);
		...
		ast_log_chan(NULL, LOG_WARNING, "channel '%s' set FAXOPT(%s) to '%s' is unhandled!\n", ast_channel_name(chan), data, value);
		res = -1;
	}
...
static int unload_module(void)
{
	...
	if (ast_custom_function_unregister(&acf_faxopt) < 0) {
		ast_log_chan(NULL, LOG_WARNING, "failed to unregister function '%s'\n", acf_faxopt.name);
	}

	if (ast_unregister_application(app_sendfax) < 0) {
		ast_log_chan(NULL, LOG_WARNING, "failed to unregister '%s'\n", app_sendfax);
	}
...
static int load_module(void)
{
	...
	if (set_config(0) < 0) {
		ast_log_chan(NULL, LOG_ERROR, "failed to load configuration file '%s'\n", config);
		ao2_ref(faxregistry.container, -1);
		return AST_MODULE_LOAD_DECLINE;
	}
	...
	if (ast_register_application_xml(app_sendfax, sendfax_exec) < 0) {
		ast_log_chan(NULL, LOG_WARNING, "failed to register '%s'.\n", app_sendfax);
		ao2_ref(faxregistry.container, -1);
		return AST_MODULE_LOAD_DECLINE;
	}
	...
	if (ast_manager_register_xml("FAXSessions", EVENT_FLAG_CALL, manager_fax_sessions)) {
		ast_log_chan(NULL, LOG_WARNING, "failed to register 'FAXSessions' AMI command.\n");
	...
	if (ast_manager_register_xml("FAXSession", EVENT_FLAG_CALL, manager_fax_session)) {
		ast_log_chan(NULL, LOG_WARNING, "failed to register 'FAXSession' AMI command.\n");
		ast_manager_unregister("FAXSession");
		ast_unregister_application(app_receivefax);
		ast_unregister_application(app_sendfax);
		ao2_ref(faxregistry.container, -1);
	...
	if (ast_manager_register_xml("FAXStats", EVENT_FLAG_REPORTING, manager_fax_stats)) {
		ast_log_chan(NULL, LOG_WARNING, "failed to register 'FAXStats' AMI command.\n");
		ast_manager_unregister("FAXSession");
		ast_manager_unregister("FAXSessions");
		ast_unregister_application(app_receivefax);
		ast_unregister_application(app_sendfax);
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER, "Generic FAX Applications",
