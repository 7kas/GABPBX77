...
static int t38_tx_packet_handler(t38_core_state_t *t38_core_state, void *data, const uint8_t *buf, int len, int count)
{
	...
	/* TODO: GABpbx does not provide means of resending the same packet multiple
...
static int update_stats(struct spandsp_pvt *p, int completion_code)
{
	...
	default:
		...
		ast_log_chan(NULL, LOG_WARNING, "unknown FAX session result '%d' (%s)\n", completion_code, t30_completion_code_to_str(completion_code));
		return -1;
	}
	return 0;
}
...
/*! \brief Send spandsp log messages to gabpbx.
...
#endif
{
	if (level == SPAN_LOG_ERROR) {
		ast_log_chan(NULL, LOG_ERROR, "%s", msg);
	} else if (level == SPAN_LOG_WARNING) {
		ast_log_chan(NULL, LOG_WARNING, "%s", msg);
	} else {
...
#else
		ast_log_chan(NULL, LOG_WARNING, "v34 not supported in this version of spandsp\n");
#endif
	}
...
static void *spandsp_fax_new(struct ast_fax_session *s, struct ast_fax_tech_token *token)
{
	...
	if ((!(p = ast_calloc(1, sizeof(*p))))) {
		ast_log_chan(NULL, LOG_ERROR, "Cannot initialize the spandsp private FAX technology structure.\n");
		goto e_return;
	}

	if (s->details->caps & AST_FAX_TECH_V21_DETECT) {
		if (spandsp_v21_new(p)) {
			ast_log_chan(NULL, LOG_ERROR, "Cannot initialize the spandsp private v21 technology structure.\n");
			goto e_return;
		}
	...
	if (s->details->caps & AST_FAX_TECH_RECEIVE) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Are we sending or receiving? The FAX requirements (capabilities: 0x%X) were not properly set.\n", s->details->caps);
		goto e_free;
	}

	if (!(p->timer = ast_timer_open())) {
		ast_log_chan(NULL, LOG_ERROR, "Channel '%s' FAX session '%u' failed to create timing source.\n", s->channame, s->id);
		goto e_free;
	}
...
static struct ast_frame *spandsp_fax_read(struct ast_fax_session *s)
{
	...
	if (ast_timer_ack(p->timer, 1) < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to acknowledge timer for FAX session '%u'\n", s->id);
		return NULL;
	}
...
static int spandsp_v21_detect(struct ast_fax_session *s, const struct ast_frame *f)
{
	...
	/* frame in other formats cannot be passed to spandsp, it could cause segfault */
		...
		ast_log_chan(NULL, LOG_WARNING, "Frame format %s not supported, v.21 detection skipped\n", ast_format_get_name(f->subclass.format));
		return -1;
	}
...
static int spandsp_fax_write(struct ast_fax_session *s, const struct ast_frame *f)
{
	...
	if (s->state == AST_FAX_STATE_COMPLETE) {
		ast_log_chan(NULL, LOG_WARNING, "FAX session '%u' is in the '%s' state.\n", s->id, ast_fax_state_to_str(s->state));
		return -1;
	}
...
static int spandsp_fax_gateway_start(struct ast_fax_session *s)
{
	 ...
	 * (GABpbx changes media ports thus we send a few packets to reinitialize
...
static int spandsp_fax_start(struct ast_fax_session *s)
{
	...
	if (ast_timer_set_rate(p->timer, SPANDSP_FAX_TIMER_RATE)) {
		ast_log_chan(NULL, LOG_ERROR, "FAX session '%u' error setting rate on timing source.\n", s->id);
		return -1;
	}
...
static int load_module(void)
{
	...
	if (ast_fax_tech_register(&spandsp_fax_tech) < 0) {
		ast_log_chan(NULL, LOG_ERROR, "failed to register FAX technology\n");
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_DEFAULT, "Spandsp G.711 and T.38 FAX Technologies",
