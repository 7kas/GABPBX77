...
#include "gabpbx/utils.h"             /* for MIN */
...
static struct ast_format *celt_set(const struct ast_format *format, const char *name, const char *value)
{
	...
	if (sscanf(value, "%30u", &val) != 1) {
		ast_log_chan(NULL, LOG_WARNING, "Unknown value '%s' for attribute type '%s'\n",
			value, name);
		ao2_ref(cloned, -1);
		return NULL;
	}
	...
	if (!strcasecmp(name, "sample_rate")) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Unknown attribute type '%s'\n", name);
		ao2_ref(cloned, -1);
		return NULL;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "CELT Format Attribute Module",
