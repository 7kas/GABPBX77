...
#include "gabpbx/utils.h"             /* for ast_free */
...
static void ilbc_generate_sdp_fmtp(const struct ast_format *format, unsigned int payload, struct ast_str **str)
{
	...
	/* When the VoIP/SIP client Zoiper calls GABpbx and its
	 ...
	 * make that client happy, GABpbx sends mode always.
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "iLBC Format Attribute Module",
