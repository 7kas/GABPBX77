...
static struct ast_format *siren7_parse_sdp_fmtp(const struct ast_format *format, const char *attributes)
{
	...
	if (sscanf(attribs, "bitrate=%30u", &val) == 1) {
		if (val != 32000) {
			ast_log_chan(NULL, LOG_WARNING, "Got Siren7 offer at %u bps, but only 32000 bps supported; ignoring.\n", val);
			return NULL;
		}
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "Siren7 Format Attribute Module",
