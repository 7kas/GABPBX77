	...
	enum ast_geoloc_format format, struct ast_variable *location_info)
{
	...
	case AST_GEOLOC_FORMAT_LAST:
		ast_log_chan(NULL, LOG_ERROR, "Location '%s' must have a format\n", id);
		return -1;
	case AST_GEOLOC_FORMAT_CIVIC_ADDRESS:
		...
		if (result != AST_GEOLOC_VALIDATE_SUCCESS) {
			ast_log_chan(NULL, LOG_ERROR, "Location '%s' has invalid item '%s' in the location\n",
				id, failed);
			return result;
		}
		break;
	case AST_GEOLOC_FORMAT_GML:
		...
		if (result != AST_GEOLOC_VALIDATE_SUCCESS) {
			ast_log_chan(NULL, LOG_ERROR, "%s for item '%s' in location '%s'\n",
	...
	case AST_GEOLOC_FORMAT_URI:
		...
		if (!uri) {
			...
			ast_log_chan(NULL, LOG_ERROR, "Geolocation location '%s' format is set to '%s' but no 'URI' was found in location parameter '%s'\n",
...
static int validate_location_source(const char *id, const char *location_source)
{
	if (!ast_strlen_zero(location_source)) {
		...
		if (rc == 1) {
			ast_log_chan(NULL, LOG_ERROR, "Geolocation location '%s' location_source '%s' must be a FQDN."
...
static int geoloc_location_apply_handler(const struct ast_sorcery *sorcery, void *obj)
{
	...
	if (!location->location_info) {
		ast_log_chan(NULL, LOG_ERROR, "Location '%s' is missing required element 'location_info'",
			location_id);
		return -1;
	}
...
static int geoloc_profile_apply_handler(const struct ast_sorcery *sorcery, void *obj)
{
	...
	if (!ast_strlen_zero(profile->location_reference)) {
		if (profile->location_info ||
			...
			ast_log_chan(NULL, LOG_ERROR, "Profile '%s' can't have location_reference and location_info or format at the same time",
				id);
			return -1;
		}
		return 0;
	...
	if (!ast_strlen_zero(profile->location_reference)) {
		...
		if (!location) {
			ast_log_chan(NULL, LOG_ERROR, "Profile '%s' has a location_reference '%s' that doesn't exist",
...
int geoloc_config_load(void)
{
	...
	if (!(geoloc_sorcery = ast_sorcery_open())) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to open geolocation sorcery\n");
	...
	if (result != AST_SORCERY_APPLY_SUCCESS) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to apply defaults for geoloc location object with sorcery\n");
		return AST_MODULE_LOAD_DECLINE;
	}

	rc = ast_sorcery_object_register(geoloc_sorcery, "location", geoloc_location_alloc, NULL, geoloc_location_apply_handler);
	if (rc != 0) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to register geoloc location object with sorcery\n");
		return AST_MODULE_LOAD_DECLINE;
	}
	...
	if (result == AST_SORCERY_APPLY_FAIL) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to add memory wizard mapping to geoloc profile object\n");
		return AST_MODULE_LOAD_DECLINE;
	}
	...
	if (rc != 0) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to register geoloc profile object with sorcery\n");
		return AST_MODULE_LOAD_DECLINE;
	}
	...
	if (rc != 0) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to load default geoloc profiles\n");
		return AST_MODULE_LOAD_DECLINE;
	}
