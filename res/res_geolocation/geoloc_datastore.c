...
struct ast_datastore *ast_geoloc_datastore_create(const char *id)
{
	...
	if (ast_strlen_zero(id)) {
		ast_log_chan(NULL, LOG_ERROR, "A geoloc datastore can't be allocated with a NULL or empty id\n");
		return NULL;
	}

	ds = ast_datastore_alloc(&geoloc_datastore_info, NULL);
	if (!ds) {
		ast_log_chan(NULL, LOG_ERROR, "Geoloc datastore '%s' couldn't be allocated\n", id);
		return NULL;
	}
	...
	if (!eds) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Private structure for geoloc datastore '%s' couldn't be allocated\n", id);
		return NULL;
	}
	ds->data = eds;
	...
	if (rc != 0) {
		ast_datastore_free(ds);
		ast_log_chan(NULL, LOG_ERROR, "Vector for geoloc datastore '%s' couldn't be initialized\n", id);
		return NULL;
	}
	...
	struct ast_geoloc_eprofile *eprofile)
{
	...
	if (rc != 0) {
		ao2_ref(eprofile, -1);
		ast_log_chan(NULL, LOG_ERROR, "Couldn't add eprofile '%s' to geoloc datastore '%s'\n", eprofile->id, eds->id);
		return -1;
	}
	...
	struct ast_geoloc_eprofile *eprofile, int index)
{
	...
	if (rc != 0) {
		ao2_ref(eprofile, -1);
		ast_log_chan(NULL, LOG_ERROR, "Couldn't add eprofile '%s' to geoloc datastore '%s' in position '%d'\n",
...
struct ast_datastore *ast_geoloc_datastore_create_from_profile_name(const char *profile_name)
{
	...
	if (!profile) {
		ast_log_chan(NULL, LOG_ERROR, "A profile with the name '%s' was not found\n", profile_name);
		return NULL;
	}

	ds = ast_geoloc_datastore_create(profile_name);
	if (!ds) {
		ast_log_chan(NULL, LOG_ERROR, "A datastore couldn't be allocated for profile '%s'\n", profile_name);
		ao2_ref(profile, -1);
		return NULL;
	}
	...
	if (!eprofile) {
		ast_datastore_free(ds);
		ast_log_chan(NULL, LOG_ERROR, "An effective profile with the name '%s' couldn't be allocated\n", profile_name);
		return NULL;
	}
