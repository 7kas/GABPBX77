...
({ \
	if (ast_test_flag(&opts, OPT_GEOLOC_RESOLVE)) { \
		...
		if (!resolved) { \
			ast_log_chan(NULL, LOG_ERROR, "%s: Unable to resolve " #_param "\n", chan_name); \
	...
	const char *cmd, char *data, struct ast_str **buf, ssize_t len)
{
	...
	if (ast_strlen_zero(parsed_data)) {
		ast_log_chan(NULL, LOG_ERROR, "%s: Cannot call without arguments\n", chan_name);
	...
	if (ast_strlen_zero(args.field)) {
		ast_log_chan(NULL, LOG_ERROR, "%s: Cannot call without a field to query\n", chan_name);
		pbx_builtin_setvar_helper(chan, "GEOLOCPROFILESTATUS", "-1");
		return 0;
	}
	...
	if (!ast_strlen_zero(args.options)) {
		if (ast_app_parse_options(action_options, &opts, NULL, args.options)) {
			ast_log_chan(NULL, LOG_ERROR, "%s: Invalid options: %s\n", chan_name, args.options);
			pbx_builtin_setvar_helper(chan, "GEOLOCPROFILESTATUS", "-1");
			return 0;
		}
	}
	...
	if (!ds) {
		ast_log_chan(NULL, LOG_NOTICE, "%s: There is no geoloc profile on this channel\n", chan_name);
	...
	if (!eprofile) {
		ast_log_chan(NULL, LOG_ERROR, "%s: Unable to duplicate eprofile\n", chan_name);
		pbx_builtin_setvar_helper(chan, "GEOLOCPROFILESTATUS", "-2");
		return 0;
	}
	...
	if (ast_strings_equal(args.field, "inheritable")) {
		...
		ast_log_chan(NULL, LOG_ERROR, "%s: Field '%s' is not valid\n", chan_name, args.field);
		pbx_builtin_setvar_helper(chan, "GEOLOCPROFILESTATUS", "-3");
	}
...
({ \
	...
	if (v == AST_GEOLOC_INVALID_VALUE) { \
		ast_log_chan(NULL, LOG_ERROR, "%s: %s '%s' is invalid\n", _chan_name, #_field, value); \
		pbx_builtin_setvar_helper(chan, "GEOLOCPROFILESTATUS", "-3"); \
		return 0; \
	} \
	_ep->_field = v; \
...
({ \
	...
	if (!_list) { \
		ast_log_chan(NULL, LOG_ERROR, "%s: %s '%s' is malformed or contains invalid values", _chan_name, #_field, _value); \
		pbx_builtin_setvar_helper(chan, "GEOLOCPROFILESTATUS", "-3"); \
		return 0; \
	} \
...
if (ast_test_flag(&opts, OPT_GEOLOC_RESOLVE)) { \
	struct ast_variable *resolved = geoloc_eprofile_resolve_varlist( \
		eprofile->_param, eprofile->location_variables, chan); \
	if (!resolved) { \
		ast_log_chan(NULL, LOG_ERROR, "%s: Unable to resolve " #_param " %p %p\n", chan_name, eprofile->_param, eprofile->location_variables); \
		pbx_builtin_setvar_helper(chan, "GEOLOCPROFILESTATUS", "-3"); \
		return 0; \
	} \
	 ...
	 const char *value)
{
	...
	if (ast_strlen_zero(args.field)) {
		ast_log_chan(NULL, LOG_ERROR, "%s: Cannot call without a field to set\n", chan_name);
		pbx_builtin_setvar_helper(chan, "GEOLOCPROFILESTATUS", "-1");
		return 0;
	}
	...
	if (!ds) {
		...
		if (!ds) {
			ast_log_chan(NULL, LOG_WARNING, "%s: Unable to create geolocation datastore\n", chan_name);
			pbx_builtin_setvar_helper(chan, "GEOLOCPROFILESTATUS", "-2");
			return 0;
		}
	...
	if (!eprofile) {
		...
		if (!eprofile) {
			ast_log_chan(NULL, LOG_ERROR, "%s: Could not allocate eprofile\n", chan_name);
			pbx_builtin_setvar_helper(chan, "GEOLOCPROFILESTATUS", "-2");
			return 0;
		}
		rc = ast_geoloc_datastore_add_eprofile(ds, eprofile);
		if (rc <= 0) {
			ast_log_chan(NULL, LOG_ERROR, "%s: Could not add eprofile to datastore\n", chan_name);
			pbx_builtin_setvar_helper(chan, "GEOLOCPROFILESTATUS", "-2");
			return 0;
		}
	}
	...
	if (ast_strings_equal(args.field, "inheritable")) {
		...
		ao2_cleanup(loc);
		if (!loc) {
			ast_log_chan(NULL, LOG_ERROR, "%s: Location reference '%s' doesn't exist\n", chan_name, value);
			pbx_builtin_setvar_helper(chan, "GEOLOCPROFILESTATUS", "-3");
			return 0;
		}
