...
int ast_geoloc_eprofile_refresh_location(struct ast_geoloc_eprofile *eprofile)
{
	...
	if (!ast_strlen_zero(eprofile->location_reference)) {
		...
		if (!loc) {
			ast_log_chan(NULL, LOG_ERROR, "Profile '%s' referenced location '%s' does not exist!", eprofile->id,
...
static int set_loc_src(struct ast_geoloc_eprofile *eprofile, const char *uri, const char *ref_str)
{
	...
	if (!ast_strlen_zero(loc_src)) {
		if (ast_begins_with(loc_src, "loc-src=")) {
			...
			if (rc == 1) {
				ast_log_chan(NULL, LOG_WARNING, "%s: URI '%s' has an invalid 'loc-src' parameter."
	...
	struct ast_channel *chan, struct ast_str **buf, const char *ref_str)
{
	...
	if (eprofile->format != AST_GEOLOC_FORMAT_URI) {
		ast_log_chan(NULL, LOG_ERROR, "%s: '%s' is not a URI profile.  It's '%s'\n",
	...
	if (ast_strlen_zero(result)) {
		ast_log_chan(NULL, LOG_ERROR, "%s: '%s' is a URI profile but had no, or an empty, 'URI' entry in location_info\n",
			ref_str, eprofile->id);
		return NULL;
	}
	...
	enum ast_geoloc_format format, const char *ref_str)
{
	...
	if (locinfo_list == NULL) {
		ast_log_chan(NULL, LOG_WARNING, "%s: There were no elements in the location info\n", ref_str);
...
int geoloc_eprofile_load(void)
{
	...
	if (!pidf_to_eprofile_xslt) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to read pidf_to_eprofile_xslt from memory\n");
	...
	if (!eprofile_to_pidf_xslt) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to read eprofile_to_pidf_xslt from memory\n");
