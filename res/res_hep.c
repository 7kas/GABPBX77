		...
		<configFile name="hep.conf">
			<configObject name="general">
				...
				<description><para>
					...
					to configure GABpbx as a Homer capture agent.
					</para>
				</description>
				...
				<configOption name="uuid_type" default="call-id">
					...
					<description>
						<enumlist>
							...
							<enum name="channel"><para>Use the GABpbx channel name</para></enum>
						</enumlist>
					</description>
				</configOption>
...
/*! GABpbx vendor ID. Used for custom data to send to a capture node */
#define GABPBX_VENDOR_ID 0x0004
...
static struct hepv3_runtime_data *hepv3_data_alloc(struct hepv3_global_config *config)
{
	...
	if (ast_sockaddr_resolve_first_af(&data->remote_addr, config->capture_address, PARSE_PORT_REQUIRE, AST_AF_UNSPEC)) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Failed to create address from %s\n", config->capture_address);
		ao2_ref(data, -1);
		return NULL;
	...
	if (data->sockfd < 0) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Failed to create socket for address %s: %s\n",
...
static int hep_queue_cb(void *data)
{
	...
	if (ast_sockaddr_is_ipv4(&capture_info->src_addr) != ast_sockaddr_is_ipv4(&capture_info->dst_addr)) {
		ast_log_chan(NULL, AST_LOG_NOTICE, "Unable to send packet: Address Family mismatch between source/destination\n");
		return -1;
	}
	...
	if (res < 0) {
		ast_log_chan(NULL, AST_LOG_ERROR, "Error [%d] while sending packet to HEPv3 server: %s\n",
			errno, strerror(errno));
	} else if (res != sock_buffer_len) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Failed to send complete packet to HEPv3 server: %d of %u sent\n",
			res, sock_buffer_len);
		res = -1;
	}
...
static int hepv3_config_pre_apply(void)
{
	...
	if (ast_strlen_zero(config->general->capture_address)) {
		ast_log_chan(NULL, AST_LOG_ERROR, "Missing required configuration option 'capture_address'\n");
		return -1;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER, "HEPv3 API",
