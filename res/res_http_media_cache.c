...
static int http_media_cache_config_pre_apply(void)
{
	...
	if (!ast_strlen_zero(cfg->general->curl_protocols)) {
		ast_log_chan(NULL, AST_LOG_ERROR, "'protocols' not supported by linked CURL library. Please recompile against newer CURL.\n");
		return -1;
	}

	if (!ast_strlen_zero(cfg->general->curl_redir_protocols)) {
		ast_log_chan(NULL, AST_LOG_ERROR, "'redirect_protocols' not supported by linked CURL library. Please recompile against newer CURL.\n");
		return -1;
	}
#endif
...
static size_t curl_header_callback(char *buffer, size_t size, size_t nitems, void *data)
{
	...
	if (realsize > MAX_HEADER_LENGTH) {
		ast_log_chan(NULL, LOG_WARNING, "cURL header length of '%zu' is too large: max %d\n",
...
static char *file_extension_from_url_path(struct ast_bucket_file *bucket_file, char *buffer, size_t capacity)
{
	...
	if (!uri) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to parse URI: %s\n",
...
static CURL *get_curl_instance(struct curl_bucket_file_data *cb_data)
{
	...
	if (!ast_strlen_zero(cfg->general->curl_protocols)) {
		...
		if (rc != CURLE_OK) {
			ast_log_chan(NULL, AST_LOG_ERROR, "Setting protocols to '%s' failed: %d\n", cfg->general->curl_protocols, rc);
	...
	if (!ast_strlen_zero(cfg->general->curl_redir_protocols)) {
		...
		if (rc != CURLE_OK) {
			ast_log_chan(NULL, AST_LOG_ERROR, "Setting redirect_protocols to '%s' failed: %d\n", cfg->general->curl_redir_protocols, rc);
			curl_easy_cleanup(curl);
			return NULL;
		}
#endif
	...
	if (rc != CURLE_OK) {
		ast_log_chan(NULL, AST_LOG_ERROR, "Setting dns_cache_timeout to '%d' failed: %d\n", cfg->general->curl_dns_cache_timeout, rc);
		curl_easy_cleanup(curl);
		return NULL;
	}
...
static int bucket_file_run_curl(struct ast_bucket_file *bucket_file)
{
	...
	if (!cb_data.out_file) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to open file '%s' for writing: %s (%d)\n",
	...
	if (http_code / 100 == 2) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Failed to retrieve URL '%s': server returned %ld\n",
	...
	void *data, const char *type, const char *id)
{
	...
	if (strcmp(type, "file")) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to create storage: invalid bucket type '%s'\n", type);
		return NULL;
	}

	if (ast_strlen_zero(id)) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to create storage: no URI\n");
		return NULL;
	}

	bucket_file = ast_bucket_file_alloc(id);
	if (!bucket_file) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to create storage for '%s'\n", id);
		return NULL;
	}

	if (ast_bucket_file_temporary_create(bucket_file)) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to create temporary storage for '%s'\n", id);
...
static int load_module(void)
{
	...
	if (aco_process_config(&cfg_info, 0) == ACO_PROCESS_ERROR) {
		...
		ast_log_chan(NULL, LOG_NOTICE, "Could not load res_http_media_cache config; using defaults\n");
		cfg = conf_alloc();
		if (!cfg) {
			aco_info_destroy(&cfg_info);
			return AST_MODULE_LOAD_DECLINE;
		...
		if (aco_set_defaults(&general_option, "general", cfg->general)) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to initialize res_http_media_cache defaults.\n");
			ao2_ref(cfg, -1);
			aco_info_destroy(&cfg_info);
			return AST_MODULE_LOAD_DECLINE;
		}
	...
	if (ast_bucket_scheme_register("http", &http_bucket_wizard, &http_bucket_file_wizard,
			NULL, NULL)) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to register Bucket HTTP wizard scheme implementation\n");
		return AST_MODULE_LOAD_DECLINE;
	}

	if (ast_bucket_scheme_register("https", &https_bucket_wizard, &https_bucket_file_wizard,
			NULL, NULL)) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to register Bucket HTTPS wizard scheme implementation\n");
		return AST_MODULE_LOAD_DECLINE;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_DEFAULT, "HTTP Media Cache Backend",
