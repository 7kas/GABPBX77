...
/*!
 * \file
 * \brief HTTP POST upload support for GABpbx HTTP server
 *
...
static void post_raw(GMimePart *part, const char *post_dir, const char *fn)
{
	...
	if ((fd = open(filename, O_CREAT | O_WRONLY | O_TRUNC, 0666)) == -1) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to open %s for writing file from a POST!\n", filename);
...
#endif
{
	...
	if (GMIME_IS_MESSAGE_PART(part)) {
		ast_log_chan(NULL, LOG_WARNING, "Got unexpected GMIME_IS_MESSAGE_PART\n");
		return;
	} else if (GMIME_IS_MESSAGE_PARTIAL(part)) {
		ast_log_chan(NULL, LOG_WARNING, "Got unexpected GMIME_IS_MESSAGE_PARTIAL\n");
		return;
	} else if (GMIME_IS_MULTIPART(part)) {
#ifndef AST_GMIME_VER_24
		GList *l;

		ast_log_chan(NULL, LOG_WARNING, "Got unexpected GMIME_IS_MULTIPART, trying to process subparts\n");
...
#else
		ast_log_chan(NULL, LOG_WARNING, "Got unexpected MIME subpart.\n");
#endif
		...
		ast_log_chan(NULL, LOG_ERROR, "Encountered unknown MIME part. This should never happen!\n");
	}
}
...
static int http_post_callback(struct ast_tcptls_session_instance *ser, const struct ast_http_uri *urih, const char *uri, enum ast_http_method method, struct ast_variable *get_vars, struct ast_variable *headers)
{
	...
	if (!(f = tmpfile())) {
		ast_log_chan(NULL, LOG_ERROR, "Could not create temp file.\n");
	...
	for (var = headers; var; var = var->next) {
		...
		if (!strcasecmp(var->name, "Content-Length")) {
			if ((sscanf(var->value, "%30u", &content_len)) != 1) {
				ast_log_chan(NULL, LOG_ERROR, "Invalid Content-Length in POST request!\n");
				fclose(f);
				ast_http_request_close_on_completion(ser);
	...
	if (fseek(f, SEEK_SET, 0)) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to seek temp file back to beginning.\n");
		fclose(f);
	...
	if (!message) {
		ast_log_chan(NULL, LOG_ERROR, "Error parsing MIME data\n");
	...
	if (!process_message(message, ast_str_buffer(post_dir))) {
		ast_log_chan(NULL, LOG_ERROR, "Invalid MIME data, found no parts!\n");
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_DEFAULT, "HTTP POST support",
