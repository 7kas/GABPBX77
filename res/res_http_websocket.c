...
/*! \file
 *
 * \brief WebSocket support for the GABpbx internal HTTP server
 *
...
#include "gabpbx/uri.h"
#include "gabpbx/uuid.h"

#define AST_API_MODULE
#include "gabpbx/http_websocket.h"
...
static void session_destroy_fn(void *obj)
{
	...
	if (session->stream) {
		...
		if (session->stream) {
			...
			session->stream = NULL;
			ast_verb_chan(NULL, 2, "WebSocket connection %s '%s' closed\n", session->client ? "to" : "from",
...
int AST_OPTIONAL_API_NAME(ast_websocket_server_add_protocol2)(struct ast_websocket_server *server, struct ast_websocket_protocol *protocol)
{
	...
	if (protocol->version != AST_WEBSOCKET_PROTOCOL_VERSION) {
		ast_log_chan(NULL, LOG_WARNING, "WebSocket could not register sub-protocol '%s': "
	...
	ast_verb_chan(NULL, 5, "WebSocket registered sub-protocol '%s'\n", protocol->name);
	ao2_ref(protocol, -1);
...
int AST_OPTIONAL_API_NAME(ast_websocket_server_remove_protocol)(struct ast_websocket_server *server, const char *name, ast_websocket_callback callback)
{
	...
	ast_verb_chan(NULL, 5, "WebSocket unregistered sub-protocol '%s'\n", name);
...
int AST_OPTIONAL_API_NAME(ast_websocket_close)(struct ast_websocket *session, uint16_t reason)
{
	...
	if (res != fsize) {
		ast_iostream_close(session->stream);
		session->stream = NULL;
		ast_verb_chan(NULL, 2, "WebSocket connection %s '%s' forcefully closed due to fatal write error\n",
...
static inline int ws_safe_read(struct ast_websocket *session, char *buf, size_t len, enum ast_websocket_opcode *opcode)
{
	...
	for (;;) {
		...
		if (rlen != xlen) {
			if (rlen == 0) {
				ast_log_chan(NULL, LOG_WARNING, "Web socket closed abruptly\n");
			...
			if (rlen < 0 && errno != EAGAIN) {
				ast_log_chan(NULL, LOG_ERROR, "Error reading from web socket: %s\n", strerror(errno));
				*opcode = AST_WEBSOCKET_OPCODE_CLOSE;
				session->closing = 1;
				ao2_unlock(session);
				return -1;
			...
			if (!--sanity) {
				ast_log_chan(NULL, LOG_WARNING, "Websocket seems unresponsive, disconnecting ...\n");
				*opcode = AST_WEBSOCKET_OPCODE_CLOSE;
				session->closing = 1;
				ao2_unlock(session);
				return -1;
		...
		if (ast_iostream_wait_for_input(session->stream, 1000) < 0) {
			ast_log_chan(NULL, LOG_ERROR, "ast_iostream_wait_for_input returned err: %s\n", strerror(errno));
			*opcode = AST_WEBSOCKET_OPCODE_CLOSE;
			session->closing = 1;
			ao2_unlock(session);
			return -1;
	...
	if (*opcode == AST_WEBSOCKET_OPCODE_TEXT || *opcode == AST_WEBSOCKET_OPCODE_BINARY || *opcode == AST_WEBSOCKET_OPCODE_CONTINUATION ||
		...
		*payload = &session->buf[frame_size]; /* payload will start here, at the end of the options, if any */
		...
		if (frame_size > MAXIMUM_FRAME_SIZE) {
			ast_log_chan(NULL, LOG_WARNING, "Cannot fit huge websocket frame of %zu bytes\n", frame_size);
			/* The frame won't fit :-( */
		...
		if (*payload_len) {
			if (!(new_payload = ast_realloc(session->payload, (session->payload_len + *payload_len)))) {
				ast_log_chan(NULL, LOG_WARNING, "Failed allocation: %p, %zu, %"PRIu64"\n",
		...
		ast_log_chan(NULL, LOG_WARNING, "WebSocket unknown opcode %u\n", *opcode);
...
int AST_OPTIONAL_API_NAME(ast_websocket_uri_cb)(struct ast_tcptls_session_instance *ser, const struct ast_http_uri *urih, const char *uri, enum ast_http_method method, struct ast_variable *get_vars, struct ast_variable *headers)
{
	...
	if (!upgrade || strcasecmp(upgrade, "websocket")) {
		ast_log_chan(NULL, LOG_WARNING, "WebSocket connection from '%s' could not be accepted - did not request WebSocket\n",
		...
		if (!protocol_handler) {
			...
			ast_log_chan(NULL, LOG_WARNING, "WebSocket connection from '%s' could not be accepted - no protocols requested\n",
				ast_sockaddr_stringify(&ser->remote_address));
		...
		ast_log_chan(NULL, LOG_WARNING, "WebSocket connection from '%s' could not be accepted - unsupported version '00/76' chosen\n",
			ast_sockaddr_stringify(&ser->remote_address));
		websocket_bad_request(ser);
		return 0;
	}
	...
	if (!protocol_handler) {
		ast_log_chan(NULL, LOG_WARNING, "WebSocket connection from '%s' could not be accepted - no protocols out of '%s' supported\n",
	...
	if (version == 7 || version == 8 || version == 13) {
		...
		if (!(session = ao2_alloc(sizeof(*session) + AST_UUID_STR_LEN + 1, session_destroy_fn))) {
			ast_log_chan(NULL, LOG_WARNING, "WebSocket connection from '%s' could not be accepted\n",
				ast_sockaddr_stringify(&ser->remote_address));
			websocket_bad_request(ser);
			ao2_ref(protocol_handler, -1);
			return 0;
		...
		if (!ast_uuid_generate_str(session->session_id, sizeof(session->session_id))) {
			ast_log_chan(NULL, LOG_WARNING, "WebSocket connection from '%s' could not be accepted - failed to generate a session id\n",
				ast_sockaddr_stringify(&ser->remote_address));
		...
		ast_log_chan(NULL, LOG_WARNING, "WebSocket connection from '%s' could not be accepted - unsupported version '%d' chosen\n",
	...
	if (setsockopt(ast_iostream_get_fd(ser->stream), SOL_SOCKET, SO_KEEPALIVE, &flags, sizeof(flags))) {
		ast_log_chan(NULL, LOG_WARNING, "WebSocket connection from '%s' could not be accepted - failed to enable keepalive\n",
			ast_sockaddr_stringify(&ser->remote_address));
		websocket_bad_request(ser);
		ao2_ref(session, -1);
		ao2_ref(protocol_handler, -1);
	...
	if (ast_getsockname(ast_iostream_get_fd(ser->stream), &session->local_address)) {
		ast_log_chan(NULL, LOG_WARNING, "WebSocket connection from '%s' could not be accepted - failed to get local address\n",
			ast_sockaddr_stringify(&ser->remote_address));
		websocket_bad_request(ser);
		ao2_ref(session, -1);
		ao2_ref(protocol_handler, -1);
	...
	ast_verb_chan(NULL, 2, "WebSocket connection from '%s' for protocol '%s' accepted using version '%d'\n", ast_sockaddr_stringify(&ser->remote_address), protocol ? : "", version);
...
static struct ast_http_uri websocketuri = {
	...
	.description = "GABpbx HTTP WebSocket",
	.uri = "ws",
	.has_subtree = 0,
	.data = NULL,
	.key = __FILE__,
...
static void websocket_echo_callback(struct ast_websocket *session, struct ast_variable *parameters, struct ast_variable *headers)
{
	...
	while ((res = ast_websocket_wait_for_input(session, -1)) > 0) {
		...
		if (ast_websocket_read(session, &payload, &payload_len, &opcode, &fragmented)) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Read failure during WebSocket echo loop\n");
			break;
		}
	...
	enum ast_websocket_result *result)
{
	...
	if (!ast_sockaddr_resolve(&addr, host, 0, 0)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to resolve address %s\n",
			host);
		ao2_ref(args, -1);
...
static char *websocket_client_create_key(void)
{
	...
	if (!encoded) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to allocate client websocket key\n");
		return NULL;
	}
	...
	struct ast_websocket_client_options *options, enum ast_websocket_result *result)
{
	...
	if (!ws) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to allocate websocket\n");
		*result = WS_ALLOCATE_ERROR;
		return NULL;
	}
	...
	if (!(ws->client = ao2_alloc(
		...
		ast_log_chan(NULL, LOG_ERROR, "Unable to allocate websocket client\n");
		*result = WS_ALLOCATE_ERROR;
		return NULL;
	}
	...
	struct websocket_client *client, int response_code)
{
	...
	case 101:
		return 0;
	case 400:
		ast_log_chan(NULL, LOG_ERROR, "Received response 400 - Bad Request "
			"- from %s\n", client->host);
		return WS_BAD_REQUEST;
	case 404:
		ast_log_chan(NULL, LOG_ERROR, "Received response 404 - Request URL not "
			"found - from %s\n", client->host);
		return WS_URL_NOT_FOUND;
	}

	ast_log_chan(NULL, LOG_ERROR, "Invalid HTTP response code %d from %s\n",
	...
	struct websocket_client *client)
{
	...
	while (ast_iostream_gets(client->ser->stream, buf, sizeof(buf)) <= 0) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Unable to retrieve HTTP status line.");
	...
	while (1) {
		...
		if (!has_upgrade &&
			...
			ast_log_chan(NULL, LOG_ERROR, "Extensions received, but not "
				"supported by client\n");
	...
	struct websocket_client *client)
{
	...
	if (ast_iostream_printf(client->ser->stream,
			...
			client->key,
			protocols) < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to send handshake.\n");
	...
	(struct ast_websocket *ws, char **buf)
{
	...
	while (fragmented) {
		if (ast_websocket_read(ws, &payload, &payload_len,
				       &opcode, &fragmented)) {
			ast_log_chan(NULL, LOG_ERROR, "Client WebSocket string read - "
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER, "HTTP WebSocket Support",
