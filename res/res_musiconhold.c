...
static struct mohclass *_mohclass_unref(struct mohclass *class, const char *tag, const char *file, int line, const char *funcname)
{
	...
	if (dup) {
		if (__ao2_ref(dup, -1, tag, file, line, funcname) == 2) {
			ast_log_chan(NULL, LOG_WARNING, "Attempt to unref mohclass %p (%s) when only 1 ref remained, and class is still in a container! (at %s:%d (%s))\n",
...
static void moh_post_start(struct ast_channel *chan, const char *moh_class_name)
{
	...
	ast_verb_chan(NULL, 3, "Started music on hold, class '%s', on channel '%s'\n",
...
static void moh_post_stop(struct ast_channel *chan)
{
	...
	ast_verb_chan(NULL, 3, "Stopped music on hold on %s\n", ast_channel_name(chan));
...
static int ast_moh_files_next(struct ast_channel *chan)
{
	...
	if (!file_count) {
		ast_log_chan(NULL, LOG_WARNING, "No files available for class '%s'\n", state->class->name);
		ao2_ref(files, -1);
		return -1;
	}
	...
	for (tries = 0; tries < file_count; ++tries) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Unable to open file '%s': %s\n", AST_VECTOR_GET(files, state->pos), strerror(errno));
		state->pos++;
		state->pos %= file_count;
	}
...
static int moh_files_generator(struct ast_channel *chan, void *data, int len, int samples)
{
	...
	while (sample_queue > 0) {
		...
		ast_frfree(f);
		if (res < 0) {
			ast_log_chan(NULL, LOG_WARNING, "Failed to write frame to '%s': %s\n", ast_channel_name(chan), strerror(errno));
			return -1;
		}
	}
	return res;
...
static void *moh_files_alloc(struct ast_channel *chan, void *params)
{
	...
	if (!state && (state = ast_calloc(1, sizeof(*state)))) {
		...
		if (state->class) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Uh Oh. Restarting MOH with an active class\n");
		}
	}
...
static int spawn_mp3(struct mohclass *class)
{
	...
	if (!strcasecmp(class->dir, "nodir")) {
		...
		if (!dir && strncasecmp(class->dir, "http://", 7)) {
			ast_log_chan(NULL, LOG_WARNING, "%s is not a valid directory\n", class->dir);
			return -1;
		}
	}
	...
	if (pipe(fds)) {
		ast_log_chan(NULL, LOG_WARNING, "Pipe failed\n");
		return -1;
	}
	if (!files) {
		ast_log_chan(NULL, LOG_WARNING, "Found no files in '%s'\n", class->dir);
		close(fds[0]);
		close(fds[1]);
		return -1;
	}
	...
	if (!class->pid) {
		...
		if (strncasecmp(class->dir, "http://", 7) && strcasecmp(class->dir, "nodir") && chdir(class->dir) < 0) {
			ast_log_chan(NULL, LOG_WARNING, "chdir() failed: %s\n", strerror(errno));
			_exit(1);
		}
		setpgid(0, getpid());
		if (ast_test_flag(class, MOH_CUSTOM)) {
...
static void killpid(int pid, size_t delay, enum kill_methods kill_method)
{
	if (killer(pid, SIGHUP, kill_method) < 0) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Unable to send a SIGHUP to MOH process '%d'?!!: %s\n", pid, strerror(errno));
	} else {
	...
	if (killer(pid, SIGTERM, kill_method) < 0) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Unable to terminate MOH process '%d'?!!: %s\n", pid, strerror(errno));
	} else {
	...
	if (killer(pid, SIGKILL, kill_method) < 0) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Unable to kill MOH process '%d'?!!: %s\n", pid, strerror(errno));
	} else {
	...
	for(;/* ever */;) {
		...
		if (class->srcfd < 0) {
			if ((class->srcfd = spawn_mp3(class)) < 0) {
				ast_log_chan(NULL, LOG_WARNING, "Unable to spawn mp3player\n");
				/* Try again later */
				sleep(500);
				continue;
			}
		...
		if (class->timer) {
			...
			if (ast_poll(&pfd, 1, -1) > 0) {
				if (ast_timer_ack(class->timer, 1) < 0) {
					ast_log_chan(NULL, LOG_ERROR, "Failed to acknowledge timer for mp3player\n");
					return NULL;
				}
				/* 25 samples per second => 40ms framerate => 320 samples */
				res = 320; /* 320/40 = 8 samples/ms */
			} else {
				ast_log_chan(NULL, LOG_WARNING, "poll() failed: %s\n", strerror(errno));
				res = 0;
			}
			pthread_testcancel();
		} else {
			...
			if (delta < MOH_MS_INTERVAL) {	/* too early */
				...
				ast_log_chan(NULL, LOG_NOTICE, "Request to schedule in the past?!?!\n");
				deadline = tv_tmp;
			}
...
static int play_moh_exec(struct ast_channel *chan, const char *data)
{
	...
	if (!ast_strlen_zero(args.duration)) {
		if (sscanf(args.duration, "%30d", &timeout) == 1) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Invalid MusicOnHold duration '%s'. Will wait indefinitely.\n", args.duration);
		}
	}

	class = S_OR(args.class, NULL);
	if (ast_moh_start(chan, class, NULL)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to start music on hold class '%s' on channel %s\n", class, ast_channel_name(chan));
		return 0;
	}
...
static struct mohclass *_get_mohbyname(const char *name, int warn, int flags, const char *file, int lineno, const char *funcname)
{
	...
	if (!moh && warn) {
		ast_log_chan(NULL, LOG_WARNING, "Music on Hold class '%s' not found in memory. Verify your configuration.\n", name);
	}
...
static struct mohdata *mohalloc(struct mohclass *cl)
{
	...
	if (ast_pipe_nonblock(moh->pipe)) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to create pipe: %s\n", strerror(errno));
		ast_free(moh);
		return NULL;
	}
...
static void *moh_alloc(struct ast_channel *chan, void *params)
{
	...
	if ((res = mohalloc(class))) {
		...
		if (ast_set_write_format(chan, class->format)) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to set channel '%s' to format '%s'\n", ast_channel_name(chan),
...
static int moh_generate(struct ast_channel *chan, void *data, int len, int samples)
{
	...
	if (len > sizeof(buf) - AST_FRIENDLY_OFFSET) {
		ast_log_chan(NULL, LOG_WARNING, "Only doing %d of %d requested bytes on %s\n", (int)sizeof(buf), len, ast_channel_name(chan));
...
static void moh_parse_options(struct ast_variable *var, struct mohclass *mohclass)
{
	...
	for (; var; var = var->next) {
		if (!strcasecmp(var->name, "name")) {
			...
			if (ast_begins_with(var->value, "/") || strstr(var->value, "://")) {
				...
				if (ast_begins_with(dup, "/")) {
					...
					if (last_pos_dot && last_pos_dot > last_pos_slash) {
						ast_log_chan(NULL, LOG_WARNING, "The playlist entry '%s' may include an extension, which could prevent it from playing.\n",
							dup);
					}
				}
				...
				ast_log_chan(NULL, LOG_ERROR, "Playlist entries must be a URL or an absolute path, '%s' provided.\n", var->value);
			}
			...
			if (!deprecation_warning) {
				ast_log_chan(NULL, LOG_WARNING, "Music on hold 'random' setting is deprecated in 14.  Please use 'sort=random' instead.\n");
			...
			if (!mohclass->format) {
				ast_log_chan(NULL, LOG_WARNING, "Unknown format '%s' -- defaulting to SLIN\n", var->value);
			...
			if (sscanf(var->value, "%zu", &mohclass->kill_delay) == 1) {
				...
				ast_log_chan(NULL, LOG_WARNING, "kill_escalation_delay '%s' is invalid.  Setting to 100ms\n", var->value);
			...
			if (!strcasecmp(var->value, "process")) {
				...
				ast_log_chan(NULL, LOG_WARNING, "kill_method '%s' is invalid.  Setting to 'process_group'\n", var->value);
				mohclass->kill_method = KILL_METHOD_PROCESS_GROUP;
			}
	...
	if (playlist_entries) {
		...
		if (strcasecmp(mohclass->mode, "playlist")) {
			ast_log_chan(NULL, LOG_NOTICE, "Ignoring playlist entries because we are in '%s' mode.\n",
				mohclass->mode);
...
static int init_files_class(struct mohclass *class)
{
	...
	if (!res) {
		ast_verb_chan(NULL, 3, "Files not found in %s for moh class:%s\n",
...
static int init_app_class(struct mohclass *class)
{
	...
	if (!(class->timer = ast_timer_open())) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to create timer: %s\n", strerror(errno));
		return -1;
	}
	if (class->timer && ast_timer_set_rate(class->timer, 25)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to set 40ms frame rate: %s\n", strerror(errno));
		ast_timer_close(class->timer);
		class->timer = NULL;
	}

	if (ast_pthread_create_background(&class->thread, NULL, monmp3thread, class)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to create moh thread...\n");
		if (class->timer) {
			ast_timer_close(class->timer);
			class->timer = NULL;
		}
...
static int _moh_register(struct mohclass *moh, int reload, int unref, const char *file, int line, const char *funcname)
{
	...
	if (mohclass && !moh_diff(mohclass, moh)) {
		ast_log_chan(NULL, LOG_WARNING, "Music on Hold class '%s' already exists\n", moh->name);
	...
	if (!strcasecmp(moh->mode, "files")) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Don't know how to do a mode '%s' music on hold\n", moh->mode);
		if (unref) {
...
static void local_ast_moh_cleanup(struct ast_channel *chan)
{
	...
	if (state) {
		...
		if (state->class) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Uh Oh. Cleaning up MOH with an active class\n");
		}
		ao2_cleanup(state->origwfmt);
		ao2_cleanup(state->mohwfmt);
		ast_free(state);
...
static int local_ast_moh_start(struct ast_channel *chan, const char *mclass, const char *interpclass)
{
		...
		if ((mohclass = moh_class_malloc())) {
			...
			if (ast_strlen_zero(mohclass->dir)) {
				if (!strcasecmp(mohclass->mode, "custom") || !strcasecmp(mohclass->mode, "playlist")) {
					...
					ast_log_chan(NULL, LOG_WARNING, "A directory must be specified for class '%s'!\n", mohclass->name);
					mohclass = mohclass_unref(mohclass, "unreffing potential mohclass (no directory specified)");
					return -1;
				}
			}
			if (ast_strlen_zero(mohclass->mode)) {
				ast_log_chan(NULL, LOG_WARNING, "A mode must be specified for class '%s'!\n", mohclass->name);
			...
			if (ast_strlen_zero(mohclass->args) && !strcasecmp(mohclass->mode, "custom")) {
				ast_log_chan(NULL, LOG_WARNING, "An application must be specified for class '%s'!\n", mohclass->name);
			...
			if (ast_test_flag(global_flags, MOH_CACHERTCLASSES)) {
				...
				if (state && state->class) {
					...
					ast_log_chan(NULL, LOG_NOTICE, "This channel already has a MOH class attached (%s)!\n", state->class->name);
				}
				...
				if (!strcasecmp(mohclass->mode, "files")) {
					...
					if (strchr(mohclass->args, 'r')) {
						static int deprecation_warning = 0;
						if (!deprecation_warning) {
							ast_log_chan(NULL, LOG_WARNING, "Music on hold 'application=r' setting is deprecated in 14.  Please use 'sort=random' instead.\n");
							deprecation_warning = 1;
						}
						ast_set_flag(mohclass, MOH_RANDOMIZE);
					}
					...
					if (state && state->class) {
						...
						if (ast_pthread_create_background(&mohclass->thread, NULL, monmp3thread, mohclass)) {
							ast_log_chan(NULL, LOG_WARNING, "Unable to create moh...\n");
							if (mohclass->timer) {
								ast_timer_close(mohclass->timer);
								mohclass->timer = NULL;
							}
	...
	if (mohclass->answeredonly && (ast_channel_state(chan) != AST_STATE_UP)) {
		ast_verb_chan(NULL, 3, "The channel '%s' is not answered yet. Ignore the moh request.\n", ast_channel_name(chan));
		return -1;
	}
		...
		if (!strcasecmp(cat, "general")) {
			for (var = ast_variable_browse(cfg, cat); var; var = var->next) {
				if (!strcasecmp(var->name, "cachertclasses")) {
					...
					ast_log_chan(NULL, LOG_WARNING, "Unknown option '%s' in [general] section of musiconhold.conf\n", var->name);
				}
			}
			continue;
		}
...
{
	ast_verb_chan(NULL, 2, "Destroying musiconhold processes\n");
	if (mohclasses) {
...
static int load_module(void)
{
	...
	if (!load_moh_classes(0) && ast_check_realtime("musiconhold") == 0) { 	/* No music classes configured, so skip it */
		ast_log_chan(NULL, LOG_WARNING, "No music on hold classes configured, "
...
static int unload_module(void)
{
	...
	if (res < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to unload res_musiconhold due to active MOH channels\n");
		return res;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "Music On Hold Resource",
