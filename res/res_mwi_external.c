...
#include "gabpbx/res_mwi_external.h"
...
static int mwi_sorcery_init(void)
{
	...
	if (!mwi_sorcery) {
		ast_log_chan(NULL, LOG_ERROR, "MWI external: Sorcery failed to open.\n");
		return -1;
	}
	...
	if (ast_sorcery_apply_default(mwi_sorcery, MWI_MAILBOX_TYPE, "astdb",
		...
		ast_log_chan(NULL, LOG_ERROR, "MWI external: Sorcery could not setup wizards.\n");
		return -1;
	}
	...
	if (res) {
		ast_log_chan(NULL, LOG_ERROR, "MWI external: Sorcery could not register object type '%s'.\n",
			MWI_MAILBOX_TYPE);
		return -1;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER, "Core external MWI resource",
