...
static struct odbc_txn_frame *create_transaction(struct ast_channel *chan, const char *name, const char *dsn)
{
	...
	if ((txn_store = ast_channel_datastore_find(chan, &txn_info, NULL))) {
		...
		if (!(txn_store = ast_datastore_alloc(&txn_info, NULL))) {
			ast_log_chan(NULL, LOG_ERROR, "Unable to allocate a new datastore.  Cannot create a new transaction.\n");
			ast_channel_unlock(chan);
			return NULL;
		}

		if (!(oldlist = ast_calloc(1, sizeof(*oldlist)))) {
			ast_log_chan(NULL, LOG_ERROR, "Unable to allocate datastore list head.  Cannot create a new transaction.\n");
...
static int acf_transaction_write(struct ast_channel *chan, const char *cmd, char *s, const char *value)
{
	...
	if (strcasecmp(args.property, "transaction") == 0) {
		...
		if (ast_true(value)) {
			...
			ast_log_chan(NULL, LOG_ERROR, "Invalid value for forcecommit: '%s'\n", S_OR(value, ""));
		...
		if (isolation == 0) {
			pbx_builtin_setvar_helper(chan, "ODBC_RESULT", "INVALID_VALUE");
			ast_log_chan(NULL, LOG_ERROR, "Invalid isolation specification: '%s'\n", S_OR(value, ""));
		...
		ast_log_chan(NULL, LOG_ERROR, "Unknown property: '%s'\n", args.property);
		return -1;
	}
}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER, "ODBC transaction resource",
