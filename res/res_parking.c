...
static int option_handler_parkpos(const struct aco_option *opt, struct ast_variable *var, void *obj)
{
	...
	if (sscanf(var->value, "%30d-%30d", &low, &high) != 2) {
		ast_log_chan(NULL, LOG_WARNING, "Format for parking positions is a-b, where a and b are numbers\n");
	} else if (high < low || low <= 0 || high <= 0) {
		ast_log_chan(NULL, LOG_WARNING, "Format for parking positions is a-b, where a <= b\n");
	} else {
...
static int option_handler_findslot(const struct aco_option *opt, struct ast_variable *var, void *obj)
{
	...
	if (!strcmp(var->value, "first")) {
		...
		ast_log_chan(NULL, LOG_WARNING, "value '%s' is not valid for findslot option.\n", var->value);
		return -1;
	}
...
static int extension_is_compatible(struct parking_lot_cfg *lot_cfg, const char *app_type, struct ast_exten *extension)
{
	...
	if (strcmp(extension_registrar, BASE_REGISTRAR)) {
		ast_log_chan(NULL, LOG_ERROR, "Parking lot '%s' -- Needs an extension '%s@%s', but that extension is already owned by %s.\n",
		        lot_cfg->name, extension_name, extension_context, extension_registrar);
		return 0;
	}

	if (strcmp(extension_application, app_type)) {
		ast_log_chan(NULL, LOG_ERROR, "Parking lot '%s' -- Needs an extension '%s@%s' with a non-exclusive %s application, "
...
int parking_lot_cfg_create_extensions(struct parking_lot_cfg *lot_cfg)
{
	...
	if (!(lot_context = ast_context_find_or_create(NULL, NULL, lot_cfg->parking_con, parkext_registrar_pointer))) {
		ast_log_chan(NULL, LOG_ERROR, "Parking lot '%s' -- Needs a context '%s' which does not exist and GABpbx was unable to create\n",
	...
	if ((existing_exten = pbx_find_extension(NULL, NULL, &find_info, lot_cfg->parking_con, lot_cfg->parkext, 1, NULL, NULL, E_MATCH))) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Parking lot '%s' -- Failed to add %s extension '%s@%s' to the PBX.\n",
	...
	for (parkingspace = lot_cfg->parking_start; parkingspace <= lot_cfg->parking_stop; parkingspace++) {
		...
		if (lot_cfg->parkaddhints) {
			...
			if ((existing_exten = pbx_find_extension(NULL, NULL, &find_info, lot_cfg->parking_con, space, PRIORITY_HINT, NULL, NULL, E_MATCH))) {
				ast_log_chan(NULL, LOG_ERROR, "Parking lot '%s' -- Needs to add a hint '%s' at '%s@%s' but one already exists owned by %s\n",
			...
			if (parking_add_extension(lot_context, 0, space, PRIORITY_HINT, hint_device, "", parkedcall_registrar_pointer)) {
				ast_log_chan(NULL, LOG_ERROR, "Parking lot '%s' -- Failed to add hint '%s@%s' to the PBX.\n",
...
struct parking_lot *parking_lot_build_or_update(struct parking_lot_cfg *lot_cfg, int dynamic)
{
	...
	if (!lot) {
		...
		if (dynamic) {
			ast_log_chan(NULL, LOG_ERROR, "Tried to create dynamic parking lot with name '%s' but a lot with that name already exists.\n", lot_cfg->name);
			ao2_cleanup(lot);
			return NULL;
		}
	}
...
static struct parking_lot *create_dynamic_lot_full(const char *name, struct ast_channel *chan, int forced)
{
	...
	if (!template_lot) {
		ast_log_chan(NULL, LOG_ERROR, "Lot %s does not exist. Can not use it as a dynamic parking lot template.\n",
			template_name);
		return NULL;
	}
	...
	if (!cfg) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to allocate dynamic parking lot configuration.\n");
		return NULL;
	}
	...
	if (parking_lot_cfg_create_extensions(cfg)) {
		ast_log_chan(NULL, LOG_ERROR, "Extensions for dynamic parking lot '%s' could not be registered. Dynamic lot creation failed.\n", name);
		return NULL;
	}
	...
	if ((lot = parking_lot_find_by_name(name))) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Started creating dynamic parking lot '%s', but a parking lot with that name already exists.\n", name);
		ao2_ref(lot, -1);
		return NULL;
	}
	...
	if (!lot) {
		ast_log_chan(NULL, LOG_NOTICE, "Failed to build dynamic parking lot '%s'\n", name);
	}
...
static int verify_default_parking_lot(void)
{
	...
	if (!lot_cfg) {
		...
		ast_log_chan(NULL, AST_LOG_NOTICE, "Adding %s profile to res_parking\n", DEFAULT_PARKING_LOT);
...
static int configure_parking_extensions(void)
{
	...
	if (res) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Extension registration failed. Previously configured lot extensions were removed and can not be safely restored.\n");
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "Call Parking Resource",
