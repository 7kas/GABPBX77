...
/*! \file
 *
 * \brief Phone provisioning application for the gabpbx internal http server
 *
...
static int lookup_iface(const char *iface, struct in_addr *address)
{
	...
	if (mysock < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to create socket: %s\n", strerror(errno));
		return -1;
	}
	...
	if (res < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to get IP of %s: %s\n", iface, strerror(errno));
...
static struct extension *build_extension(const char *name, struct varshead *vars)
{
	...
	if (!exten->headp) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to clone variables for extension '%s'\n", name);
	...
	if (AST_LIST_EMPTY(&user->extensions)) {
		...
		AST_LIST_TRAVERSE_SAFE_BEGIN(&user->extensions, exten_iter, entry) {
			if (exten->index < exten_iter->index) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Duplicate linenumber=%d for %s\n", exten->index, user->macaddress);
				return -1;
...
static int phoneprov_callback(struct ast_tcptls_session_instance *ser, const struct ast_http_uri *urih, const char *uri, enum ast_http_method method, struct ast_variable *get_vars, struct ast_variable *headers)
{
	...
	if (!route->user) { /* Static file */
		...
		lseek(fd, 0, SEEK_SET);
		if (len < 0) {
			ast_log_chan(NULL, LOG_WARNING, "Could not load file: %s (%d)\n", path, len);
			close(fd);
			goto out500;
		}
		...
		if (!server) {
			...
			if ((res = getsockname(ast_iostream_get_fd(ser->stream), &name.sa, &namelen))) {
				ast_log_chan(NULL, LOG_WARNING, "Could not get server IP, breakage likely.\n");
			} else {
		...
		if (!(result = ast_str_create(512))) {
			ast_log_chan(NULL, LOG_ERROR, "Could not create result string!\n");
			if (tmp) {
				ast_free(tmp);
			}
...
static int pp_each_extension_helper(struct ast_channel *chan, const char *cmd, char *data, char *buf, struct ast_str **bufstr, int len)
{
	...
	if (ast_strlen_zero(args.mac) || ast_strlen_zero(args.template)) {
		ast_log_chan(NULL, LOG_WARNING, "PP_EACH_EXTENSION requires both a macaddress and template filename.\n");
		return 0;
	}

	if (!(user = find_user(args.mac))) {
		ast_log_chan(NULL, LOG_WARNING, "Could not find user with mac = '%s'\n", args.mac);
		return 0;
	}
...
static struct ast_http_uri phoneprovuri = {
	...
	.description = "GABpbx HTTP Phone Provisioning Tool",
	.uri = "phoneprov",
	.has_subtree = 1,
	.data = NULL,
	.key = __FILE__,
...
static struct varshead *get_defaults(void)
{
	...
	if (!defaults) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to create default var list.\n");
		return NULL;
	}

	if (!(phoneprov_cfg = ast_config_load("phoneprov.conf", config_flags))
		|| phoneprov_cfg == CONFIG_STATUS_FILEINVALID) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to load config phoneprov.conf\n");
	...
	if (value) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Unable to find a valid server address or name.\n");
	}
	...
	if (!value) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to load default profile.\n");
	...
	if (!(cfg = ast_config_load("users.conf", config_flags)) || cfg == CONFIG_STATUS_FILEINVALID) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to load users.conf\n");
		ast_var_list_destroy(defaults);
		return NULL;
	}
...
static int load_users(void)
{
	...
	if (!defaults) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to load default variables.\n");
		return -1;
	}

	if (!(cfg = ast_config_load("users.conf", config_flags))
		|| cfg == CONFIG_STATUS_FILEINVALID) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to load users.conf\n");
		ast_var_list_destroy(defaults);
		return -1;
	}
	...
	while ((cat = ast_category_browse(cfg, cat))) {
		...
		if (strcasecmp(cat, "general") && strcasecmp(cat, "authentication")) {
			...
			if (!ast_var_find(variables, variable_lookup[AST_PHONEPROV_STD_MAC])) {
				ast_log_chan(NULL, LOG_WARNING, "autoprov set for %s, but no mac address - skipping.\n", cat);
				ast_var_list_destroy(variables);
				continue;
			}
...
static int load_common(void)
{
	...
	if (!ao2_container_count(profiles)) {
		ast_log_chan(NULL, LOG_ERROR, "There are no provisioning profiles in phoneprov.conf.\n");
		return -1;
	}
...
static int load_module(void)
{
	...
	if (!profiles) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to allocate profiles container.\n");
	...
	if (!http_routes) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to allocate routes container.\n");
		goto error;
	}

	if (load_common()) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to load provisioning profiles.\n");
		goto error;
	}
	...
	if (!users) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to allocate users container.\n");
		goto error;
	}
	...
	if (!providers) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to allocate providers container.\n");
		goto error;
	}

	/* Register ourselves as the provider for users.conf */
	if (ast_phoneprov_provider_register(SIPUSERS_PROVIDER_NAME, load_users)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable register users config provider.  Others may succeed.\n");
	}
...
static int reload(void)
{
	...
	/* Reload the profiles */
	if (load_common()) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to reload provisioning profiles.\n");
		unload_module();
		return AST_MODULE_LOAD_DECLINE;
	}
	...
	for(; (provider = ao2_iterator_next(&i)); ao2_ref(provider, -1)) {
		if (provider->load_users()) {
			ast_log_chan(NULL, LOG_ERROR, "Unable to load provider '%s' users. Reload aborted.\n", provider->provider_name);
			continue;
		}
	}
	ao2_iterator_destroy(&i);
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER, "HTTP Phone Provisioning",
	...
	ast_phoneprov_load_users_cb load_users)
{
	...
	if (ast_strlen_zero(provider_name)) {
		ast_log_chan(NULL, LOG_ERROR, "Provider name can't be empty.\n");
		return -1;
	}

	if (!providers) {
		ast_log_chan(NULL, LOG_WARNING, "Provider '%s' cannot be registered: res_phoneprov not loaded.\n", provider_name);
		return -1;
	}

	provider = find_provider(provider_name);
	if (provider) {
		ast_log_chan(NULL, LOG_ERROR, "There is already a provider registered named '%s'.\n", provider_name);
		ao2_ref(provider, -1);
		return -1;
	}
	...
	if (!provider) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to allocate sufficient memory for provider '%s'.\n", provider_name);
		return -1;
	}

	if (ast_string_field_init(provider, 32)) {
		ao2_ref(provider, -1);
		ast_log_chan(NULL, LOG_ERROR, "Unable to allocate sufficient memory for provider '%s' stringfields.\n", provider_name);
		return -1;
	}
	...
	if (provider->load_users()) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to load provider '%s' users. Register aborted.\n", provider_name);
...
int ast_phoneprov_add_extension(char *provider_name, struct varshead *vars)
{
	...
	if (!vars) {
		ast_log_chan(NULL, LOG_ERROR, "Variable list can't be empty.\n");
		return -1;
	}

	username = ast_var_find(vars, variable_lookup[AST_PHONEPROV_STD_USERNAME]);
	if (!username) {
		ast_log_chan(NULL, LOG_ERROR, "Extension name can't be empty.\n");
		return -1;
	}

	mac = ast_var_find(vars, variable_lookup[AST_PHONEPROV_STD_MAC]);
	if (!mac) {
		ast_log_chan(NULL, LOG_ERROR, "MAC Address can't be empty.\n");
		return -1;
	}

	provider = find_provider(provider_name);
	if (!provider) {
		ast_log_chan(NULL, LOG_ERROR, "Provider '%s' wasn't found in the registry.\n", provider_name);
		return -1;
	}
	...
	if (!profile_name) {
		ast_log_chan(NULL, LOG_ERROR, "No profile could be found for user '%s' - skipping.\n", username);
		return -1;
	}
	if (!(profile = find_profile(profile_name))) {
		ast_log_chan(NULL, LOG_ERROR, "Could not look up profile '%s' - skipping.\n", profile_name);
		return -1;
	}
	...
	if (!(user = find_user(mac))) {
		...
		if (!(user = build_user(mac, profile, provider_name))) {
			ast_log_chan(NULL, LOG_ERROR, "Could not create user for '%s' - skipping\n", mac);
			return -1;
		}

		if (!(exten = build_extension(username, vars))) {
			ast_log_chan(NULL, LOG_ERROR, "Could not create extension for '%s' - skipping\n", user->macaddress);
			return -1;
		}

		if (add_user_extension(user, exten)) {
			ast_log_chan(NULL, LOG_WARNING, "Could not add extension '%s' to user '%s'\n", exten->name, user->macaddress);
			exten = delete_extension(exten);
			return -1;
		}

		if (build_user_routes(user)) {
			ast_log_chan(NULL, LOG_WARNING, "Could not create http routes for '%s' - skipping\n", user->macaddress);
			return -1;
		}
		ao2_link(users, user);
		...
		if (strcmp(provider_name, user->provider_name)) {
			ast_log_chan(NULL, LOG_ERROR, "MAC address '%s' was already added by provider '%s' - skipping\n", user->macaddress, user->provider_name);
			return -1;
		}
