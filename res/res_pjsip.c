...
static int register_service(void *data)
{
	...
	if (!ast_pjsip_endpoint) {
		ast_log_chan(NULL, LOG_ERROR, "There is no PJSIP endpoint. Unable to register services\n");
		return -1;
	}
	if (pjsip_endpt_register_module(ast_pjsip_endpoint, *module) != PJ_SUCCESS) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to register module %.*s\n", (int) pj_strlen(&(*module)->name), pj_strbuf(&(*module)->name));
		return -1;
	}
...
int ast_sip_register_authenticator(struct ast_sip_authenticator *auth)
{
	if (registered_authenticator) {
		ast_log_chan(NULL, LOG_WARNING, "Authenticator %p is already registered. Cannot register a new one\n", registered_authenticator);
		return -1;
	}
...
void ast_sip_unregister_authenticator(struct ast_sip_authenticator *auth)
{
	if (registered_authenticator != auth) {
		ast_log_chan(NULL, LOG_WARNING, "Trying to unregister authenticator %p but authenticator %p registered\n",
...
int ast_sip_requires_authentication(struct ast_sip_endpoint *endpoint, pjsip_rx_data *rdata)
{
	...
	if (!registered_authenticator) {
		ast_log_chan(NULL, LOG_WARNING, "No SIP authenticator registered. Assuming authentication is not required\n");
		return 0;
	}
...
{
	if (!registered_authenticator) {
		ast_log_chan(NULL, LOG_WARNING, "No SIP authenticator registered. Assuming authentication is successful\n");
...
int ast_sip_register_outbound_authenticator(struct ast_sip_outbound_authenticator *auth)
{
	if (registered_outbound_authenticator) {
		ast_log_chan(NULL, LOG_WARNING, "Outbound authenticator %p is already registered. Cannot register a new one\n", registered_outbound_authenticator);
		return -1;
	}
...
void ast_sip_unregister_outbound_authenticator(struct ast_sip_outbound_authenticator *auth)
{
	if (registered_outbound_authenticator != auth) {
		ast_log_chan(NULL, LOG_WARNING, "Trying to unregister outbound authenticator %p but outbound authenticator %p registered\n",
...
{
	if (!registered_outbound_authenticator) {
		ast_log_chan(NULL, LOG_WARNING, "No SIP outbound authenticator registered. Cannot respond to authentication challenge\n");
		return -1;
	}
						 ...
						 const char *name)
{
	...
	if (!id_list_item) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to add endpoint identifier. Out of memory.\n");
		return -1;
	}
...
int ast_sip_set_tpselector_from_transport(const struct ast_sip_transport *transport, pjsip_tpselector *selector)
{
	...
	if (!transport_state) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to retrieve PJSIP transport state for '%s'\n",
...
#else
		ast_log_chan(NULL, LOG_WARNING, "Connection reuse could not be disabled on transport '%s' as support is not available\n",
			ast_sorcery_object_get_id(transport));
#endif
	} else {
		res = -1;
...
int ast_sip_set_tpselector_from_transport_name(const char *transport_name, pjsip_tpselector *selector)
{
	...
	if (!transport) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to retrieve PJSIP transport '%s'\n",
			transport_name);
		return -1;
	}
	...
	const char *uri, const char *request_user)
{
	...
	if (!ast_strlen_zero(outbound_proxy)) {
		...
		if (!(route = pjsip_parse_hdr(dlg->pool, &ROUTE_HNAME, tmp.ptr, tmp.slen, NULL))) {
			ast_log_chan(NULL, LOG_ERROR, "Could not create dialog to endpoint '%s' as outbound proxy URI '%s' is not valid\n",
	...
	pjsip_rx_data *rdata, pj_status_t *status, create_dlg_uac create_fun)
{
	...
	*status = create_fun(pjsip_ua_instance(), rdata, &contact, &dlg);
	if (*status != PJ_SUCCESS) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Could not create dialog with endpoint %s. %s\n",
	...
	char *transport_type, const char *local_name, int local_port, const char *contact)
{
	...
	if (!ast_strlen_zero(contact)) {
		...
		if (contact_hdr) {
			...
			if (!contact_hdr->uri) {
				ast_log_chan(NULL, LOG_WARNING, "Unable to parse contact URI from '%s'.\n", contact);
				return -1;
			}
		}
	}
...
static int create_in_dialog_request(const pjsip_method *method, struct pjsip_dialog *dlg, pjsip_tx_data **tdata)
{
	if (pjsip_dlg_create_request(dlg, method, -1, tdata) != PJ_SUCCESS) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to create in-dialog request.\n");
		return -1;
	}
		...
		const char *uri, struct ast_sip_contact *provided_contact, pjsip_tx_data **tdata)
{
	...
	if (ast_strlen_zero(uri)) {
		if (!endpoint && (!contact || ast_strlen_zero(contact->uri))) {
			ast_log_chan(NULL, LOG_ERROR, "An endpoint and/or uri must be specified\n");
			return -1;
		}
		...
		if (!contact || ast_strlen_zero(contact->uri)) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to retrieve contact for endpoint %s\n",
	...
	if (!pool) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to create PJLIB memory pool\n");
		return -1;
	}
	...
	if (!sip_uri || (!PJSIP_URI_SCHEME_IS_SIP(sip_uri) && !PJSIP_URI_SCHEME_IS_SIPS(sip_uri))) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to create outbound %.*s request to endpoint %s as URI '%s' is not valid\n",
	...
	if (sip_dialog_create_from(pool, &from, fromuser,
		...
		ast_log_chan(NULL, LOG_ERROR, "Unable to create From header for %.*s request to endpoint %s\n",
				(int) pj_strlen(&method->name), pj_strbuf(&method->name),
				endpoint ? ast_sorcery_object_get_id(endpoint) : "<none>");
		pjsip_endpt_release_pool(ast_sip_get_pjsip_endpoint(), pool);
		ast_sip_tpselector_unref(&selector);
	...
	if (pjsip_endpt_create_request(ast_sip_get_pjsip_endpoint(), method, &remote_uri,
		...
		ast_log_chan(NULL, LOG_ERROR, "Unable to create outbound %.*s request to endpoint %s\n",
				(int) pj_strlen(&method->name), pj_strbuf(&method->name),
				endpoint ? ast_sorcery_object_get_id(endpoint) : "<none>");
		pjsip_endpt_release_pool(ast_sip_get_pjsip_endpoint(), pool);
		ast_sip_tpselector_unref(&selector);
	...
	if (endpoint && !ast_strlen_zero(endpoint->outbound_proxy) &&
		...
		ast_log_chan(NULL, LOG_ERROR, "Unable to apply outbound proxy on request %.*s to endpoint %s as outbound proxy URI '%s' is not valid\n",
		...
		struct ast_sip_contact *contact, pjsip_tx_data **tdata)
{
	...
	if (!pmethod) {
		ast_log_chan(NULL, LOG_WARNING, "Unknown method '%s'. Cannot send request\n", method);
		return -1;
	}
...
static int send_in_dialog_request(pjsip_tx_data *tdata, struct pjsip_dialog *dlg)
{
	if (pjsip_dlg_send_request(dlg, tdata, -1, NULL) != PJ_SUCCESS) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to send in-dialog request.\n");
		return -1;
	}
	return 0;
}
	...
	pjsip_tx_data *tdata, pj_int32_t timeout, void *token, pjsip_endpt_send_callback cb)
{
	...
	if (ret_val != PJ_SUCCESS) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Error %d '%s' sending %.*s request to endpoint %s\n",
...
static void send_request_cb(void *token, pjsip_event *e)
{
	...
	if (e->type == PJSIP_EVENT_TSX_STATE) {
		...
		case PJSIP_EVENT_RX_MSG:
			...
			break;
		default:
			ast_log_chan(NULL, LOG_ERROR, "Unexpected PJSIP event %u\n", e->body.tsx_state.type);
			break;
		}
	}
...
static void sip_thread_start(void)
{
	...
	if (!servant_id) {
		ast_log_chan(NULL, LOG_ERROR, "Could not set SIP servant ID in thread-local storage.\n");
		return;
	}
	*servant_id = SIP_SERVANT_ID;
	...
	if (pj_thread_register("GABpbx Thread", *desc, &thread) != PJ_SUCCESS) {
		ast_log_chan(NULL, LOG_ERROR, "Couldn't register thread with PJLIB.\n");
	}
}
...
static void set_id_from_hdr(pjsip_fromto_hdr *hdr, struct ast_party_id *id)
{
	...
	if (result != AST_UTF8_REPLACE_VALID) {
		ast_log_chan(NULL, LOG_WARNING, "CallerID Name '" PJSTR_PRINTF_SPEC
	...
	const char *user, char **uri)
{
	...
	if (!contact) {
		ast_log_chan(NULL, LOG_WARNING, "Dest: '%s'. Couldn't find contact for endpoint '%s'\n",
			to, endpoint_name);
		return -1;
	}
	...
	if (!pool) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to allocate ParseUri endpoint pool.\n");
		return -1;
	}
	...
	if (!name_addr || (!PJSIP_URI_SCHEME_IS_SIP(name_addr->uri) && !PJSIP_URI_SCHEME_IS_SIPS(name_addr->uri))) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to parse URI '%s'\n", contact->uri);
		err = -1;
		goto out;
	}
...
static struct ast_sip_endpoint *handle_single_token(const char *to, char *destination, int get_default_outbound, char **uri) {
	...
	contact = ast_sip_location_retrieve_contact_from_aor_list(endpoint->aors);
	if (!contact) {
		ast_log_chan(NULL, LOG_WARNING, "Dest: '%s'. Found endpoint '%s' but didn't find an aor/contact for it\n",
			to, endpoint_name);
		ao2_cleanup(endpoint);
		goto failure;
	}
	...
	char *slash, char *atsign, char *scheme)
{
	...
	if (!endpoint) {
		ast_log_chan(NULL, LOG_WARNING, "Dest: '%s'. Didn't find endpoint with name '%s'\n",
			to, endpoint_name);
		goto failure;
	}
	...
	if (!contact) {
		ast_log_chan(NULL, LOG_WARNING, "Dest: '%s'. Found endpoint '%s' but didn't find contact for aor '%s'\n",
			to, endpoint_name, ast_sorcery_object_get_id(aor));
		ao2_cleanup(aor);
		goto failure;
	}
	...
	char *slash, char *atsign, char *scheme, int get_default_outbound)
{
	...
	*atsign = '\0';
	...
	contact = ast_sip_location_retrieve_contact_from_aor_list(endpoint->aors);
	if (!contact) {
		ast_log_chan(NULL, LOG_WARNING, "Dest: '%s'. Found endpoint '%s' but didn't find contact\n",
			to, endpoint_name);
		goto failure;
	}
...
#define DEBUG_BUF_SIZE 256
	...
	if (!parsed_name_addr || (!PJSIP_URI_SCHEME_IS_SIP(parsed_name_addr->uri)
		...
		ast_log_chan(NULL, LOG_WARNING, "To address '%s' is not a valid SIP/SIPS URI\n", to);
		return -1;
	}
	...
	if (!tdata_name_addr || (!PJSIP_URI_SCHEME_IS_SIP(tdata_name_addr->uri)
		...
		ast_log_chan(NULL, LOG_WARNING, "tdata To address '%s' is not a valid SIP/SIPS URI\n", to);
		return -1;
	}
...
int ast_sip_update_from(pjsip_tx_data *tdata, char *from)
{
	...
	if (parsed_name_addr) {
		...
		if (!PJSIP_URI_SCHEME_IS_SIP(parsed_name_addr->uri)
				&& !PJSIP_URI_SCHEME_IS_SIPS(parsed_name_addr->uri)) {
			ast_log_chan(NULL, LOG_WARNING, "From address '%s' is not a valid SIP/SIPS URI\n", from);
			return -1;
		}
...
/*! \brief Convert SIP hangup causes to GABpbx hangup causes */
...
static int load_pjsip(void)
{
	...
	if (pjsip_endpt_create(&caching_pool.factory, "SIP", &ast_pjsip_endpoint) != PJ_SUCCESS) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to create PJSIP endpoint structure. Aborting load\n");
		goto error;
	}
	...
	if (!memory_pool) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to create memory pool for SIP. Aborting load\n");
		goto error;
	}
	...
	if (!pj_gethostip(pj_AF_INET(), &host_ip_ipv4)) {
		...
		ast_verb_chan(NULL, 3, "Local IPv4 address determined to be: %s\n", host_ip_ipv4_string);
	}
	...
	if (!pj_gethostip(pj_AF_INET6(), &host_ip_ipv6)) {
		...
		ast_verb_chan(NULL, 3, "Local IPv6 address determined to be: %s\n", host_ip_ipv6_string);
	}
	...
	if (status != PJ_SUCCESS) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to start SIP monitor thread. Aborting load\n");
		goto error;
	}
...
static int load_module(void)
{
	...
	if (pj_register_strerror(PJMEDIA_ERRNO_START, PJ_ERRNO_SPACE_SIZE, pjmedia_strerror)
		!= PJ_SUCCESS) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to register pjmedia error codes.  Codes will not be decoded.\n");
	}
	...
	if (ast_sip_initialize_system()) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to initialize SIP 'system' configuration section. Aborting load\n");
		goto error;
	}
	...
	if (!sip_serializer_pool) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to create SIP serializer pool. Aborting load\n");
		goto error;
	}

	if (ast_sip_initialize_scheduler()) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to start scheduler. Aborting load\n");
		goto error;
	}
	...
	if (ast_sip_initialize_transport_events()) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to initialize SIP transport monitor. Aborting load\n");
		goto error;
	}
	...
	if (ast_res_pjsip_preinit_options_handling()) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to pre-initialize OPTIONS handling. Aborting load\n");
		goto error;
	}

	if (ast_res_pjsip_initialize_configuration()) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to initialize SIP configuration. Aborting load\n");
		goto error;
	}
	...
	if (ast_sip_initialize_transport_management()) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to initialize SIP transport management. Aborting load\n");
		goto error;
	}

	if (ast_sip_initialize_distributor()) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to register distributor module. Aborting load\n");
		goto error;
	}

	if (ast_sip_register_service(&supplement_module)) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to initialize supplement hooks. Aborting load\n");
		goto error;
	}

	if (ast_res_pjsip_init_options_handling(0)) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to initialize OPTIONS handling. Aborting load\n");
		goto error;
	}
	...
	if (ast_res_pjsip_init_message_filter()) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to initialize message IP updating. Aborting load\n");
		goto error;
	}
...
static int reload_module(void)
{
	...
	if (ast_sip_push_task_wait_servant(NULL, reload_configuration_task, NULL)) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to reload PJSIP\n");
		return -1;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER, "Basic SIP resource",
