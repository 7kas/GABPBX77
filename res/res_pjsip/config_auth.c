...
#else
		ast_log_chan(NULL, LOG_WARNING, "OAuth support is not available in the version of PJSIP in use\n");
		return -1;
#endif
	} else {
		ast_log_chan(NULL, LOG_WARNING, "Unknown authentication storage type '%s' specified for %s\n",
				var->value, var->name);
		return -1;
	}
	return 0;
	...
	const char *value)
{
	...
	while ((val.ptr = ast_strip(strsep(&iana_names, ",")))) {
		...
		if (!algo) {
			ast_log_chan(NULL, LOG_WARNING, "%s: Unknown %s digest algorithm '%s' specified\n",
				id, agent_type, val.ptr);
			res = -1;
			continue;
		}
		if (!ast_sip_auth_is_algorithm_supported(algo->algorithm_type)) {
			ast_log_chan(NULL, LOG_WARNING, "%s: %s digest algorithm '%s' is not supported by the version of OpenSSL in use\n",
				id, agent_type, val.ptr);
			res = -1;
			continue;
		}
...
static int password_digest_handler(const struct aco_option *opt, struct ast_variable *var, void *obj)
{
	...
	while ((unparsed_digest = ast_strsep(&value, ',', AST_STRSEP_TRIM))) {
		...
		if (!algo) {
			ast_log_chan(NULL, LOG_WARNING, "%s: Unknown password_digest algorithm '%s' specified\n",
				auth_name, iana_name);
			return -1;
		}
		if (!ast_sip_auth_is_algorithm_supported(algo->algorithm_type)) {
			ast_log_chan(NULL, LOG_WARNING, "%s: password_digest algorithm '%s' is not supported by the version of OpenSSL in use\n",
				auth_name, iana_name);
			return -1;
		}
		if (strlen(digest) != algo->digest_str_length) {
			ast_log_chan(NULL, LOG_WARNING, "%s: password_digest algorithm '%s' length (%d) must be %d\n",
	...
	const pjsip_auth_algorithm_type algorithm_type, const char *which_supported)
{
	...
	if (!pw_digest && ast_strlen_zero(auth->auth_pass)) {
		ast_log_chan(NULL, LOG_ERROR, "%s: No plain text or digest password found for algorithm "
...
static int auth_apply(const struct ast_sorcery *sorcery, void *obj)
{
	...
	if (ast_strlen_zero(auth->auth_user)) {
		ast_log_chan(NULL, LOG_ERROR, "%s: No authentication username\n", id);
		return -1;
	}
	...
	if (auth->type == AST_SIP_AUTH_TYPE_GOOGLE_OAUTH) {
		if (ast_strlen_zero(auth->refresh_token)
			...
			ast_log_chan(NULL, LOG_ERROR, "%s: 'google_oauth' authentication specified but refresh_token,"
