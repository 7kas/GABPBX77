...
#define DEFAULT_USERAGENT_PREFIX "GABpbx PBX"
...
#define DEFAULT_FROM_USER "gabpbx"
...
static int global_apply(const struct ast_sorcery *sorcery, void *obj)
{
	...
	if (ast_strlen_zero(cfg->default_from_user)) {
		ast_log_chan(NULL, LOG_ERROR,
			"Global option 'default_from_user' can't be empty.  Set it to a valid value or remove the entry to accept 'gabpbx' as the default\n");
		return -1;
	}
	...
	if (res) {
		ast_log_chan(NULL, LOG_WARNING, "global: Invalid values in default_auth_algorithms_uas. "
	...
	AST_VECTOR_FREE(&algorithms);
	if (res) {
		ast_log_chan(NULL, LOG_WARNING, "global: Invalid values in default_auth_algorithms_uac. "
	...
	struct ast_variable *var, void *obj)
{
	...
	if (!strcasecmp(var->value, "none")) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Unknown overload trigger '%s' specified for %s\n",
				var->value, var->name);
		return -1;
	}
	return 0;
