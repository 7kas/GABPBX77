...
static int system_apply(const struct ast_sorcery *sorcery, void *obj)
{
	...
	if (system->timert1 < TIMER_T1_MIN) {
		ast_log_chan(NULL, LOG_WARNING, "Timer T1 setting is too low. Setting to %d\n", TIMER_T1_MIN);
	...
	if (system->timerb < min_timerb) {
		ast_log_chan(NULL, LOG_WARNING, "Timer B setting is too low. Setting to %d\n", min_timerb);
...
int ast_sip_initialize_system(void)
{
	...
	if (!system_sorcery) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to open SIP system sorcery\n");
		return -1;
	}
	...
	if (ast_sorcery_object_register_no_reload(system_sorcery, "system", system_alloc, NULL, system_apply)) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to register with sorcery (is res_sorcery_config loaded?)\n");
	...
	if (!system) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to allocate default system config.\n");
		ast_sorcery_unref(system_sorcery);
		return -1;
	}

	if (system_apply(system_sorcery, system)) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to apply default system config.\n");
		ast_sorcery_unref(system_sorcery);
		return -1;
	}
...
static int system_create_resolver_and_set_nameservers(void *data)
{
	...
	if (!discovered_nameservers) {
		ast_log_chan(NULL, LOG_ERROR, "Could not retrieve local system nameservers, resorting to system resolution\n");
		return 0;
	}

	if (!ao2_container_count(discovered_nameservers)) {
		ast_log_chan(NULL, LOG_ERROR, "There are no local system nameservers configured, resorting to system resolution\n");
	...
	if (!(resolver = pjsip_endpt_get_resolver(ast_sip_get_pjsip_endpoint()))) {
		...
		if (status != PJ_SUCCESS) {
			ast_log_chan(NULL, LOG_ERROR, "Could not create DNS resolver(%d), resorting to system resolution\n", status);
			ao2_ref(discovered_nameservers, -1);
			return 0;
		}
	}
	...
	if (status != PJ_SUCCESS) {
		ast_log_chan(NULL, LOG_ERROR, "Could not set nameservers on DNS resolver in PJSIP(%d), resorting to system resolution\n",
			status);
		return 0;
	}
	...
	if (!pjsip_endpt_get_resolver(ast_sip_get_pjsip_endpoint())) {
		...
		if (status != PJ_SUCCESS) {
			ast_log_chan(NULL, LOG_ERROR, "Could not set DNS resolver in PJSIP(%d), resorting to system resolution\n", status);
			return 0;
		}
	}
