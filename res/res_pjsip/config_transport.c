...
#define BIND_DELAY_US 100000
	...
	if (!temp_state) {
		ast_log_chan(NULL, LOG_ERROR, "Transport '%s' failed to allocate memory\n", transport_id);
		return -1;
	}

	if (transport->async_operations != 1) {
		ast_log_chan(NULL, LOG_WARNING, "The async_operations setting on transport '%s' has been set to '%d'. The setting can no longer be set and is always 1.\n",
	...
	if (!transport->flow && (!perm_state || transport->allow_reload)) {
		if (temp_state->state->host.addr.sa_family != PJ_AF_INET && temp_state->state->host.addr.sa_family != PJ_AF_INET6) {
			ast_log_chan(NULL, LOG_ERROR, "Transport '%s' could not be started as binding not specified\n", transport_id);
			return -1;
		}
	...
	if (!ast_strlen_zero(transport->external_signaling_address)) {
		if (temp_state->state->host.addr.sa_family == pj_AF_INET()) {
			...
			ast_log_chan(NULL, LOG_ERROR, "Unknown address family for transport '%s', could not get external signaling address\n",
					transport_id);
			return -1;
		}
		...
		if (ast_dnsmgr_lookup(transport->external_signaling_address, &temp_state->state->external_signaling_address, &temp_state->state->external_signaling_address_refresher, NULL) < 0) {
			ast_log_chan(NULL, LOG_ERROR, "Could not create dnsmgr for external signaling address on '%s'\n", transport_id);
			return -1;
		}
	}
	...
	if (!ast_strlen_zero(transport->external_media_address)) {
		if (temp_state->state->host.addr.sa_family == pj_AF_INET()) {
			...
			ast_log_chan(NULL, LOG_ERROR, "Unknown address family for transport '%s', could not get external media address\n",
					transport_id);
			return -1;
		}
		...
		if (ast_dnsmgr_lookup(transport->external_media_address, &temp_state->state->external_media_address, &temp_state->state->external_media_address_refresher, NULL) < 0) {
			ast_log_chan(NULL, LOG_ERROR, "Could not create dnsmgr for external media address on '%s'\n", transport_id);
			return -1;
		}
	}
	...
	if (transport->flow) {
...
#ifdef HAVE_PJSIP_TLS_TRANSPORT_RESTART
		ast_log_chan(NULL, LOG_NOTICE, "Transport '%s' is not fully reloadable, not reloading: protocol, bind, TLS (everything but certificate and private key if filename is unchanged), TCP, ToS, or CoS options.\n", transport_id);
		...
		if (transport->type == AST_TRANSPORT_TLS) {
			if (strcmp(perm_state->transport->cert_file, temp_state->transport->cert_file) ||
				...
				ast_log_chan(NULL, LOG_ERROR, "Unable to restart TLS transport '%s' as certificate or private key filename has changed\n",
					transport_id);
				...
				if (pjsip_tls_transport_restart(perm_state->state->factory, &perm_state->state->host, NULL) != PJ_SUCCESS) {
					ast_log_chan(NULL, LOG_ERROR, "Failed to restart TLS transport '%s'\n", transport_id);
				} else {
...
#else
		ast_log_chan(NULL, LOG_NOTICE, "Transport '%s' is not fully reloadable, not reloading: protocol, bind, TLS, TCP, ToS, or CoS options.\n", transport_id);
#endif
		...
		if (transport->tcp_keepalive_enable) {
			...
			ast_log_chan(NULL, LOG_DEBUG, "TCP Keepalive enabled for transport '%s'. Idle Time: %d, Interval: %d, Count: %d\n",
			...
			sockopt_count++;
#else
			ast_log_chan(NULL, LOG_WARNING, "TCP keepalive settings for '%s' not set due to PJSIP built without support for setting all options. Consider using bundled PJSIP.\n",
		...
		if (transport->async_operations > 1 && ast_compare_versions(pj_get_version(), "2.5.0") < 0) {
			ast_log_chan(NULL, LOG_ERROR, "Transport: %s: When protocol=tls and pjproject version < 2.5.0, async_operations can't be > 1\n",
					ast_sorcery_object_get_id(obj));
			return -1;
		}
...
#else
		ast_log_chan(NULL, LOG_ERROR, "Transport: %s: PJSIP has not been compiled with TLS transport support, ensure OpenSSL development packages are installed\n",
			ast_sorcery_object_get_id(obj));
		return -1;
#endif
	} else if ((transport->type == AST_TRANSPORT_WS) || (transport->type == AST_TRANSPORT_WSS)) {
		if (transport->cos || transport->tos) {
			ast_log_chan(NULL, LOG_WARNING, "TOS and COS values ignored for websocket transport\n");
			...
			ast_log_chan(NULL, LOG_WARNING, "TLS certificate values ignored for websocket transport as they are configured in http.conf\n");
		}
		res = PJ_SUCCESS;
	}
	...
	if (res != PJ_SUCCESS) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Transport '%s' could not be started: %s\n", ast_sorcery_object_get_id(obj), msg);
		return -1;
	}
...
static int transport_tls_file_handler(const struct aco_option *opt, struct ast_variable *var, void *obj)
{
	...
	if (!ast_file_is_readable(var->value)) {
		ast_log_chan(NULL, LOG_ERROR, "Transport: %s: %s %s is either missing or not readable\n",
	...
	if (!strcasecmp(var->name, "ca_list_file")) {
...
#else
		ast_log_chan(NULL, LOG_WARNING, "GABpbx has been built against a version of pjproject that does not "
		...
		if (stat(var->value, &state->cert_file_stat)) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to stat certificate file '%s' for transport '%s' due to '%s'\n",
		...
		if (stat(var->value, &state->privkey_file_stat)) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to stat private key file '%s' for transport '%s' due to '%s'\n",
				var->value, ast_sorcery_object_get_id(obj), strerror(errno));
			return -1;
		}
		ast_sorcery_object_set_has_dynamic_contents(transport);
...
static int transport_cipher_add(struct ast_sip_transport_state *state, const char *name)
{
	...
	if (pj_ssl_cipher_is_supported(cipher)) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Cipher '%s' is unsupported\n", name);
		return -1;
	}
}
#endif
...
static int transport_tls_cipher_handler(const struct aco_option *opt, struct ast_variable *var, void *obj)
{
	...
	while ((name = ast_strip(strsep(&parse, ",")))) {
		...
		if (ARRAY_LEN(state->ciphers) <= state->tls.ciphers_num) {
			ast_log_chan(NULL, LOG_ERROR, "Too many ciphers specified (maximum allowed is %d)\n", SIP_TLS_MAX_CIPHERS);
			res = -1;
			break;
		}
...
static int transport_tos_handler(const struct aco_option *opt, struct ast_variable *var, void *obj)
{
	...
	if (ast_str2tos(var->value, &value)) {
		ast_log_chan(NULL, LOG_ERROR, "Error configuring transport '%s' - Could not "
...
int ast_sip_initialize_sorcery_transport(void)
{
	...
	if (!transport_states) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to allocate transport states container\n");
		return -1;
	}
