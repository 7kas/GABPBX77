	...
	struct ast_sip_aor **aor, struct ast_sip_contact **contact)
{
	...
	if (ast_strlen_zero(aor_list) || !(rest = ast_strdupa(aor_list))) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to determine contacts from empty aor list\n");
		return;
	}
...
static int prune_boot_contacts_cb(void *obj, void *arg, int flags)
{
	...
	if (contact->prune_on_boot
		...
		ast_verb_chan(NULL, 3, "Removed contact '%s' from AOR '%s' due to system boot\n",
...
static int permanent_uri_handler(const struct aco_option *opt, struct ast_variable *var, void *obj)
{
	...
	while ((contact_uri = ast_strip(strsep(&contacts, ",")))) {
		...
		if (ast_sip_validate_uri_length(contact_uri)) {
			ast_log_chan(NULL, LOG_ERROR, "Contact uri or hostname length exceeds pjproject limit or is not a sip(s) uri: %s\n", contact_uri);
			return -1;
		}
...
static int contact_apply_handler(const struct ast_sorcery *sorcery, void *object)
{
	...
	if (ast_strlen_zero(contact->uri)) {
		ast_log_chan(NULL, LOG_ERROR, "A URI on dynamic contact '%s' is empty\n",
...
int ast_sip_initialize_sorcery_location(void)
{
	...
	if (!contact_formatter) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to allocate memory for contact_formatter\n");
		return -1;
	}
	...
	if (!aor_formatter) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to allocate memory for aor_formatter\n");
		return -1;
	}
