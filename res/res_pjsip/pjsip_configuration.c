...
#include "gabpbx/res_stir_shaken.h"
...
static int ident_handler(const struct aco_option *opt, struct ast_variable *var, void *obj)
{
	...
	while ((val = ast_strip(strsep(&idents, ",")))) {
		...
		if (method == -1) {
			ast_log_chan(NULL, LOG_ERROR, "Unrecognized identification method %s specified for endpoint %s\n",
	...
	*buf = ast_malloc(buf_size);
	...
	for (idx = 0; idx < methods; ++idx) {
		...
		if (buf_size <= buf_used) {
			...
			*(*buf + (buf_size - 1)) = '\0';
			ast_log_chan(NULL, LOG_WARNING, "Truncated identify_by string: %s\n", *buf);
			break;
		}
	}
...
static int redirect_method_handler(const struct aco_option *opt, struct ast_variable *var, void *obj)
{
	...
	if (!strcasecmp(var->value, "user")) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Unrecognized redirect method %s specified for endpoint %s\n",
...
static int direct_media_method_handler(const struct aco_option *opt, struct ast_variable *var, void *obj)
{
	...
	if (!strcasecmp(var->value, "invite") || !strcasecmp(var->value, "reinvite")) {
		...
		ast_log_chan(NULL, LOG_NOTICE, "Unrecognized option value %s for %s on endpoint %s\n",
...
static int stir_shaken_handler(const struct aco_option *opt, struct ast_variable *var, void *obj)
{
	...
	ast_log_chan(NULL, LOG_WARNING, "Endpoint %s: Option 'stir_shaken' is no longer supported.  Use 'stir_shaken_profile' instead.\n",
	...
	struct ast_variable *var, void *obj)
{
	...
	if (ast_str2tos(var->value, &value)) {
		ast_log_chan(NULL, LOG_ERROR, "Error configuring endpoint '%s' - Could not "
			"interpret '%s' value '%s'\n",
	...
	struct ast_variable *var, void *obj)
{
	...
	for (val = var->value; *val; val++) {
		if (!isalpha(*val) && !isdigit(*val) && !strchr(valid_uri_marks, *val)) {
			ast_log_chan(NULL, LOG_ERROR, "Error configuring endpoint '%s' - '%s' field "
	...
	struct ast_variable *var, void *obj)
{
	...
	if (res < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Endpoint '%s': %s for option '%s'\n",
	...
	if (strcmp(var->name, "codec_prefs_incoming_offer") == 0) {
		if (prefs.operation == CODEC_NEGOTIATION_OPERATION_UNION) {
			ast_log_chan(NULL, LOG_ERROR, "Endpoint '%s': Codec preference '%s' has invalid value '%s' for option: '%s'",
				ast_sorcery_object_get_id(endpoint),
		...
		ast_log_chan(NULL, LOG_ERROR, "Endpoint '%s': Unsupported option '%s'\n",
			ast_sorcery_object_get_id(endpoint),
			var->name);
		return -1;
	}
...
int ast_sip_persistent_endpoint_add_to_regcontext(const char *regcontext)
{
	...
	if (!ast_context_find_or_create(NULL, NULL, regcontext, "PJSIP")) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to create regcontext '%s'\n", regcontext);
		return -1;
	}
...
int ast_sip_persistent_endpoint_update_state(const char *endpoint_name, enum ast_endpoint_state state)
{
	...
	if (state == AST_ENDPOINT_ONLINE) {
		...
		ast_verb_chan(NULL, 2, "Endpoint %s is now Reachable\n", ast_endpoint_get_resource(persistent->endpoint));
	} else {
		...
		ast_verb_chan(NULL, 2, "Endpoint %s is now Unreachable\n", ast_endpoint_get_resource(persistent->endpoint));
	}
...
static int sip_endpoint_apply_handler(const struct ast_sorcery *sorcery, void *obj)
{
	...
	if (endpoint->extensions.timer.min_se < 90) {
		ast_log_chan(NULL, LOG_ERROR, "Session timer minimum expires time must be 90 or greater on endpoint '%s'\n",
			ast_sorcery_object_get_id(endpoint));
		return -1;
	} else if (endpoint->extensions.timer.sess_expires < endpoint->extensions.timer.min_se) {
		ast_log_chan(NULL, LOG_ERROR, "Session timer expires must be greater than minimum session expires time on endpoint '%s'\n",
			ast_sorcery_object_get_id(endpoint));
		return -1;
	}
	...
	if (endpoint->preferred_codec_only) {
		if (endpoint->media.incoming_call_offer_pref.flags != (AST_SIP_CALL_CODEC_PREF_LOCAL | AST_SIP_CALL_CODEC_PREF_INTERSECT | AST_SIP_CALL_CODEC_PREF_ALL)) {
			ast_log_chan(NULL, LOG_ERROR, "Setting both preferred_codec_only and incoming_call_offer_pref is not supported on endpoint '%s'\n",
				ast_sorcery_object_get_id(endpoint));
			return -1;
		}
	...
	if (endpoint->media.webrtc) {
		...
		if (endpoint->media.rtp.dtls_cfg.rekey) {
			ast_log_chan(NULL, LOG_WARNING, "DTLS renegotiation is not supported with WebRTC. Disabling dtls_rekey.\n");
	...
	if (!ast_strlen_zero(endpoint->geoloc_incoming_call_profile) ||
		...
		if (!ast_geoloc_is_loaded()) {
			ast_log_chan(NULL, LOG_ERROR, "A geoloc incoming and/or outgoing call_profile was specified on endpoint '%s'"
		...
		if (!ast_strlen_zero(endpoint->geoloc_incoming_call_profile)) {
			...
			if (!profile) {
				ast_log_chan(NULL, LOG_ERROR, "geoloc_incoming_call_profile '%s' on endpoint '%s' doesn't exist\n",
		...
		if (!ast_strlen_zero(endpoint->geoloc_outgoing_call_profile)) {
			...
			if (!profile) {
				ast_log_chan(NULL, LOG_ERROR, "geoloc_outgoing_call_profile '%s' on endpoint '%s' doesn't exist\n",
...
int ast_res_pjsip_initialize_configuration(void)
{
	...
	if (ast_sip_initialize_sorcery_auth()) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to register SIP authentication support\n");
	...
	if (ast_sorcery_object_register(sip_sorcery, "endpoint", ast_sip_endpoint_alloc, NULL, sip_endpoint_apply_handler)) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to register SIP endpoint object with sorcery\n");
		ast_sorcery_unref(sip_sorcery);
		sip_sorcery = NULL;
		return -1;
	}
	...
	if (ast_sorcery_internal_object_register(sip_sorcery, "nat_hook", sip_nat_hook_alloc, NULL, NULL)) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to register nat_hook\n");
	}
	...
	ast_sorcery_object_field_register(sip_sorcery, "endpoint", "sdp_session", "GABpbx", OPT_STRINGFIELD_T, 0, STRFLDSET(struct ast_sip_endpoint, media.sdpsession));
	...
	ast_sorcery_object_field_register(sip_sorcery, "endpoint", "rtp_engine", "gabpbx", OPT_STRINGFIELD_T, 0, STRFLDSET(struct ast_sip_endpoint, media.rtp.engine));
	...
	ast_sorcery_object_field_register(sip_sorcery, "endpoint", "user_agent_prefix", "", OPT_STRINGFIELD_T, 0, STRFLDSET(struct ast_sip_endpoint, user_agent_prefix));

	if (ast_sip_initialize_sorcery_transport()) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to register SIP transport support with sorcery\n");
		ast_sorcery_unref(sip_sorcery);
		sip_sorcery = NULL;
		return -1;
	}
	...
	if (ast_sip_initialize_sorcery_location()) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to register SIP location support with sorcery\n");
		ast_sorcery_unref(sip_sorcery);
		sip_sorcery = NULL;
		return -1;
	}
	...
	if (ast_sip_initialize_sorcery_domain_alias()) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to register SIP domain aliases support with sorcery\n");
		ast_sorcery_unref(sip_sorcery);
		sip_sorcery = NULL;
		return -1;
	}
	...
	if (ast_sip_initialize_sorcery_global()) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to register SIP Global support\n");
		ast_sorcery_unref(sip_sorcery);
		sip_sorcery = NULL;
		return -1;
	}
	...
	if (!endpoint_formatter) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to allocate memory for endpoint_formatter\n");
		ast_sorcery_unref(sip_sorcery);
		sip_sorcery = NULL;
		return -1;
	}
...
int ast_sip_retrieve_auths(const struct ast_sip_auth_vector *auths, struct ast_sip_auth **out)
{
	...
	for (i = 0; i < AST_VECTOR_SIZE(auths); ++i) {
		...
		if (!out[i]) {
			ast_log_chan(NULL, LOG_NOTICE, "Couldn't find auth '%s'. Cannot authenticate\n", name);
			return -1;
		}
	}
	...
	struct ast_sip_auth_objects_vector *auth_objects)
{
	...
	for (i = 0; i < AST_VECTOR_SIZE(auth_ids); ++i) {
		...
		if (!auth_object) {
			ast_log_chan(NULL, LOG_WARNING, "Auth object '%s' could not be found\n", name);
		} else {
