...
static int create_artificial_auth(void)
{
	...
	fake_auth = alloc_artificial_auth(default_realm, default_algos_uac,
			default_algos_uas);
	if (!fake_auth) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to create artificial auth\n");
		return -1;
	}
...
static void log_failed_request(pjsip_rx_data *rdata, char *msg, unsigned int count, unsigned int period)
{
	...
	if (count) {
		ast_log_chan(NULL, LOG_NOTICE, "Request '%s' from '%s' failed for '%s' (callid: %s) - %s"
...
int ast_sip_initialize_distributor(void)
{
	...
	if (!unid_formatter) {
		ast_sip_destroy_distributor();
		ast_log_chan(NULL, LOG_ERROR, "Unable to allocate memory for unid_formatter\n");
		return -1;
	}
