...
static pj_status_t send_options_response(pjsip_rx_data *rdata, int code)
{
	...
	if (status != PJ_SUCCESS) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to create response (%d)\n", status);
		return status;
	}
	...
	if (status != PJ_SUCCESS) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to send response (%d)\n", status);
	}
...
static int sip_options_contact_status_notify_task(void *obj)
{
	...
	if (cs_new->last_status != cs_new->status) {
		...
		ast_verb_chan(NULL, 3, "Contact %s/%s is now %s.  RTT: %.3f msec\n",
			cs_new->aor,
			cs_new->uri,
...
static void qualify_contact_cb(void *token, pjsip_event *e)
{
	...
	if (ast_sip_push_task(contact_callback_data->aor_options->serializer,
		...
		ast_log_chan(NULL, LOG_WARNING, "Unable to queue contact status update for '%s' on AOR '%s', state will be incorrect\n",
...
static int sip_options_qualify_contact(void *obj, void *arg, int flags)
{
	...
	if (ast_sip_create_request("OPTIONS", NULL, endpoint, NULL, contact, &tdata)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to create request to qualify contact %s on AOR %s\n",
	...
	if (!ast_strlen_zero(contact->outbound_proxy) &&
		...
		ast_log_chan(NULL, LOG_ERROR, "Unable to apply outbound proxy on request to qualify contact %s\n",
			contact->uri);
	...
	if (!contact_status) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to retrieve contact status information for contact %s on AOR %s\n",
			contact->uri, aor_options->name);
		pjsip_tx_data_dec_ref(tdata);
		return 0;
	}
	...
	if (!contact_callback_data) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to create object to contain callback data for contact %s on AOR %s\n",
			contact->uri, aor_options->name);
		pjsip_tx_data_dec_ref(tdata);
		return 0;
	}
	...
	if (ast_sip_send_out_of_dialog_request(tdata, endpoint,
		...
		qualify_contact_cb)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to send request to qualify contact %s on AOR %s\n",
			contact->uri, aor_options->name);
		ao2_ref(contact_callback_data, -1);
	}
	...
	struct ast_sip_contact *contact)
{
	...
	ast_verb_chan(NULL, 2, "Contact %s/%s has been deleted\n", contact->aor, contact->uri);
	...
	enum ast_sip_contact_status_type status)
{
	...
	if (cs_new->status != cs_new->last_status) {
		ast_verb_chan(NULL, 3, "Contact %s/%s is now %s.\n",
	...
	struct ast_sip_aor *aor, int is_new)
{
	...
	if (!existing_contacts) {
		ast_log_chan(NULL, LOG_WARNING, "Synchronization of AOR '%s' failed for qualify, retaining existing state\n",
			aor_options->name);
		return;
	}
	...
	if (aor_options->qualify_frequency != aor->qualify_frequency
		...
		if (aor_options->qualify_frequency
			...
			if (!aor_options->sched_task) {
				ast_log_chan(NULL, LOG_ERROR, "Unable to schedule qualify for contacts of AOR '%s'\n",
					aor_options->name);
			}
		}
	}
...
void ast_res_pjsip_cleanup_options_handling(void)
{
	...
	if (remaining) {
		ast_log_chan(NULL, LOG_WARNING, "Cleanup incomplete. Could not stop %d AORs.\n",
			remaining);
	}
