...
#include "gabpbx/dns_query_set.h"
...
static int sip_resolve_invoke_user_callback(void *data)
{
	...
	if (DEBUG_ATLEAST(2)) {
		...
		for (idx = 0; idx < resolve->addresses.count; ++idx) {
			...
			ast_log_chan(NULL, LOG_DEBUG, "[%p] Address '%d' is %s with transport '%s'\n",
				resolve, idx, addr,
...
static void sip_check_transport(pj_pool_t *pool, pjsip_transport_type_e transport, const char *name)
{
	...
	if (transport == PJSIP_TRANSPORT_UDP) {
		...
		ast_verb_chan(NULL, 2, "'%s' is an unsupported SIP transport\n", name);
		return;
	}

	if (pjsip_tpmgr_find_local_addr2(pjsip_endpt_get_tpmgr(ast_sip_get_pjsip_endpoint()),
		pool, &prm) == PJ_SUCCESS) {
		ast_verb_chan(NULL, 2, "'%s' is an available SIP transport\n", name);
		sip_available_transports[resolver_transport] = 1;
	} else {
		ast_verb_chan(NULL, 2, "'%s' is not an available SIP transport, disabling resolver support for it\n",
			name);
	}
}
...
void ast_sip_initialize_resolver(void)
{
	...
	ast_log_chan(NULL, LOG_NOTICE, "The version of PJSIP in use does not support external resolvers, using PJSIP provided resolver\n");
}
