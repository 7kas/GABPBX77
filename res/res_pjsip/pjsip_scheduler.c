...
static int run_task(void *data)
{
	...
	if (schtd->flags & AST_SIP_SCHED_TASK_TRACK) {
		ast_log_chan(NULL, LOG_DEBUG, "Sched %p: Running %s\n", schtd, schtd->name);
	}
	ao2_lock(schtd);
	...
	if (schtd->current_scheduler_id < 0) {
		schtd->interval = 0;
		ao2_unlock(schtd);
		ast_log_chan(NULL, LOG_ERROR, "Sched %p: Failed to reschedule task %s\n", schtd, schtd->name);
		ao2_unlink(tasks, schtd);
		return -1;
	}
	...
	ao2_unlock(schtd);
	if (schtd->flags & AST_SIP_SCHED_TASK_TRACK) {
		ast_log_chan(NULL, LOG_DEBUG, "Sched %p: Rescheduled %s for %d ms\n", schtd, schtd->name,
			delay);
	}
...
static int push_to_serializer(const void *data)
{
	...
	if (schtd->flags & AST_SIP_SCHED_TASK_TRACK) {
		ast_log_chan(NULL, LOG_DEBUG, "Sched %p: Ready to run %s\n", schtd, schtd->name);
	}
...
int ast_sip_sched_task_cancel(struct ast_sip_sched_task *schtd)
{
	...
	if (schtd->flags & AST_SIP_SCHED_TASK_TRACK) {
		ast_log_chan(NULL, LOG_DEBUG, "Sched %p: Canceling %s\n", schtd, schtd->name);
	}
...
static void schtd_dtor(void *data)
{
	...
	if (schtd->flags & AST_SIP_SCHED_TASK_TRACK) {
		ast_log_chan(NULL, LOG_DEBUG, "Sched %p: Destructor %s\n", schtd, schtd->name);
	}
...
#define ID_LEN 13 /* task_deadbeef */
	...
	if (schtd->flags & AST_SIP_SCHED_TASK_TRACK) {
		ast_log_chan(NULL, LOG_DEBUG, "Sched %p: Scheduling %s for %d ms\n", schtd, schtd->name,
			interval);
	}
...
int ast_sip_initialize_scheduler(void)
{
	...
	if (!scheduler_context) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to create scheduler. Aborting load\n");
		return -1;
	}
	...
	if (!tasks) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to allocate task container. Aborting load\n");
		ast_sched_context_destroy(scheduler_context);
		return -1;
	}
