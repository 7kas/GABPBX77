	...
	pj_uint32_t verify_status)
{
	...
	if (pj_ssl_cert_get_verify_status_strings(verify_status, status, &count) != PJ_SUCCESS) {
		ast_log_chan(NULL, LOG_ERROR, "Error retrieving certificate verification result(s)\n");
		return;
	}

	AST_SIP_MAKE_REMOTE_IPADDR_PORT_STR(transport, transport_remote_ipaddr_port);
	for (i = 0; i < count; ++i) {
		ast_log_chan(NULL, log_level, _A_, "Transport '%s' to remote '%.*s' - %s - %s\n", transport->factory->info,
	...
	const pjsip_tls_state_info *state_info)
{
	...
	if (!state) {
		 ...
		 */
		ast_log_chan(NULL, LOG_ERROR, "Transport state not found for '%s'\n", transport->factory->info);
		return 0;
	}
