...
static void *keepalive_transport_thread(void *data)
{
	...
	if (pj_thread_register("GABpbx Keepalive Thread", desc, &thread) != PJ_SUCCESS) {
		ast_log_chan(NULL, LOG_ERROR, "Could not register keepalive thread with PJLIB, keepalives will not occur.\n");
		return NULL;
	}
...
static int idle_sched_init_pj_thread(void)
{
	if (!pj_thread_is_registered()) {
		...
		if (!desc) {
			ast_log_chan(NULL, LOG_ERROR, "Could not get thread desc from thread-local storage.\n");
			return -1;
		}
...
static int idle_sched_cb(const void *data)
{
	...
	if (monitored) {
		if (!monitored->sip_received) {
			ast_log_chan(NULL, LOG_NOTICE, "Shutting down transport '%s' since no request was received in %d seconds\n",
...
static void keepalive_global_loaded(const char *object_type)
{
	...
	if (new_interval) {
		...
		ast_log_chan(NULL, LOG_NOTICE, "Keepalive support can not be disabled once activated.\n");
		return;
	} else {
	...
	if (ast_pthread_create(&keepalive_thread, NULL, keepalive_transport_thread, NULL)) {
		ast_log_chan(NULL, LOG_ERROR, "Could not create thread for sending keepalive messages.\n");
...
int ast_sip_initialize_transport_management(void)
{
	...
	if (!transports) {
		ast_log_chan(NULL, LOG_ERROR, "Could not create container for transports to perform keepalive on.\n");
	...
	if (!sched) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to create keepalive scheduler context.\n");
		ao2_global_obj_release(monitored_transports);
		return AST_MODULE_LOAD_DECLINE;
	}

	if (ast_sched_start_thread(sched)) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to start keepalive scheduler thread\n");
