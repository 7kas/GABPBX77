...
static int apply_acl(pjsip_rx_data *rdata, struct ast_acl_list *acl)
{
	...
	if (ast_apply_acl(acl, &addr, "SIP ACL: ") != AST_SENSE_ALLOW) {
		ast_log_chan(NULL, LOG_WARNING, "Incoming SIP message from %s did not pass ACL test\n", ast_sockaddr_stringify(&addr));
		return 1;
	}
	return 0;
}
...
static pj_bool_t acl_on_rx_msg(pjsip_rx_data *rdata)
{
	...
	if (!acls) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to retrieve ACL sorcery data\n");
		return PJ_FALSE;
	}
...
static int acl_handler(const struct aco_option *opt, struct ast_variable *var, void *obj)
{
	...
	if (!strncmp(var->name, "contact_", 8)) {
		...
		if (error) {
			ast_log_chan(NULL, LOG_ERROR, "Bad contact ACL '%s' at line '%d' of pjsip.conf\n",
					var->value, var->lineno);
		}
	} else {
		ast_append_acl(var->name, var->value, &sip_acl->acl, &error, &ignore);
		if (error) {
			ast_log_chan(NULL, LOG_ERROR, "Bad ACL '%s' at line '%d' of pjsip.conf\n",
					var->value, var->lineno);
		}
	}

	if (error) {
		ast_log_chan(NULL, LOG_ERROR, "There is an error in ACL configuration. Blocking ALL SIP traffic.\n");
...
static int load_module(void)
{
	...
	if (ast_sorcery_object_register(ast_sip_get_sorcery(), SIP_SORCERY_ACL_TYPE,
		...
		ast_log_chan(NULL, LOG_ERROR, "Failed to register SIP %s object with sorcery\n",
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "PJSIP ACL Resource",
