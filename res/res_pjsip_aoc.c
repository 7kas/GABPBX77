...
static int aoc_send_as_xml(void * data)
{
	...
	if (!pool) {
		ast_log_chan(NULL, LOG_ERROR, "Could not create a memory pool for AOC XML\n");
		return 1;
	}
	...
	if (ast_aoc_get_msg_type(adata->decoded) == AST_AOC_D ||
		...
		if (size >= xml_max_size) {
			ast_log_chan(NULL, LOG_ERROR, "aoc+xml body text too large\n");
			return 1;
		}
		xml[size] = 0;
		...
		if (ast_aoc_get_msg_type(adata->decoded) == AST_AOC_D) {
			...
			if (ast_sip_create_request("INFO", adata->session->inv_session->dlg,
				...
				ast_log_chan(NULL, LOG_ERROR, "Could not create AOC INFO request\n");
				return 1;
			}
			if (ast_sip_add_body(tdata, &body)) {
				ast_log_chan(NULL, LOG_ERROR, "Could not add body to AOC INFO request\n");
			...
			if (!datastore) {
				...
				if (!datastore) {
					ast_log_chan(NULL, LOG_ERROR, "Unable to create datastore for AOC-D.\n");
					return 1;
				}
				datastore->data = NULL;
			...
			if (!datastore) {
				...
				if (!datastore) {
					ast_log_chan(NULL, LOG_ERROR, "Unable to create datastore for AOC-E.\n");
					return 1;
				}
				datastore->data = NULL;
				if (ast_sip_session_add_datastore(adata->session, datastore)) {
		...
		if (adata->channel_state == AST_STATE_UP ||
			...
			if (!datastore) {
				...
				if (!datastore) {
					ast_log_chan(NULL, LOG_ERROR, "Unable to create datastore for AOC-S.\n");
					return 1;
				}
				if (ast_sip_session_add_datastore(adata->session, datastore)) {
					ast_log_chan(NULL, LOG_ERROR, "Unable to create datastore for AOC-S.\n");
		...
		enum ast_framehook_event event, void *data)
{
	...
	if (!adata) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to allocate AOC data\n");
		return f;
	}

	adata->decoded = ast_aoc_decode((struct ast_aoc_encoded *) f->data.ptr, f->datalen, ast);
	if (!adata->decoded) {
		ast_log_chan(NULL, LOG_ERROR, "Error decoding indicated AOC data\n");
		ao2_ref(adata, -1);
		return f;
	}
	...
	if (ast_sip_push_task(adata->session->serializer, aoc_send_as_xml, adata)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to send AOC XML for channel %s\n", ast_channel_name(ast));
		ao2_ref(adata, -1);
	}
...
static void aoc_attach_framehook(struct ast_sip_session *session)
{
	...
	if (framehook_id < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Could not attach AOC Frame hook, AOC will be unavailable on '%s'\n",
	...
	if (tdata->msg->body && pjsip_media_type_cmp(&tdata->msg->body->content_type,
		...
		if (tdata_sdp_info->sdp) {
			...
			rc = pjsip_create_multipart_sdp_body(tdata->pool, tdata_sdp_info->sdp,
					&multipart_body);
			if (rc != PJ_SUCCESS) {
				ast_log_chan(NULL, LOG_ERROR, "Unable to create sdp multipart body\n");
				return;
			}
		} else {
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "PJSIP AOC Support",
