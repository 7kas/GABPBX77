...
/*** DOCUMENTATION
	<configInfo name="res_pjsip_config_wizard" language="en_US">
		...
		<description><para>
			...
			<para><literal>https://docs.gabpbx.org/Configuration/Channel-Drivers/SIP/Configuring-res_pjsip/PJSIP-Configuration-Wizard/</literal></para>
		</description>
	...
	const char *id, const char *type, struct ast_variable *vars)
{
	...
	if (!obj) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to allocate an object of type '%s' with id '%s'.\n", type, id);
		return NULL;
	}

	if (ast_sorcery_objectset_apply(sorcery, obj, vars)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to apply object type '%s' with id '%s'.  Check preceeding errors.\n", type, id);
		ao2_ref(obj, -1);
		return NULL;
	}
...
static int variable_list_append(struct ast_variable **existing, const char *name, const char *value)
{
	...
	if (!new) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to allocate memory for new variable '%s'.\n", name);
		return -1;
	}
...
static int add_hints(const char *context, const char *exten, const char *application, const char *id)
{
	...
	if (ast_wrlock_contexts()) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to lock the contexts list.\n");
		return -1;
	}

	if (!(hint_context = ast_context_find_or_create(NULL, NULL, context, BASE_REGISTRAR))) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to find or create hint context '%s'\n", context);
	...
	if (ast_wrlock_context(hint_context)) {
		ast_unlock_contexts();
		ast_log_chan(NULL, LOG_ERROR, "failed to obtain write lock on context\n");
		return -1;
	}
	ast_unlock_contexts();

	if (add_extension(hint_context, exten, PRIORITY_HINT, hint_device)) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to add hint '%s@%s' to the PBX.\n",
		        exten, context);
	}
	...
	struct ast_category *wiz, struct string_vector *remote_hosts_vector)
{
	...
	if (!host_count) {
		ast_log_chan(NULL, LOG_ERROR, "Wizard '%s' must have 'remote_hosts' if it sends registrations.\n", id);
		return -1;
	}
	...
	if (is_variable_true(wizvars, "sends_auth")) {
		if (!(username = ast_variable_find_last_in_list(wizvars, "outbound_auth/username"))) {
			ast_log_chan(NULL, LOG_ERROR, "Wizard '%s' must have 'outbound_auth/username' if it sends"
				" authentication.\n", id);
			return -1;
		}
	} else {
	...
	const struct ast_sorcery *sorcery, const char *object_type, int reloaded)
{
	...
	if (!otw) {
		ast_log_chan(NULL, LOG_ERROR, "There was no wizard for object type '%s'\n", object_type);
		return;
	}
	...
	while ((category = ast_category_browse_filtered(cfg, NULL, category, "type=^wizard$"))) {
		...
		if (!last_cat || changes) {
			...
			if (wizard_apply_handler(sorcery, otw, category)) {
				ast_log_chan(NULL, LOG_ERROR, "Unable to create objects for wizard '%s'\n", id);
			}
		}
	}
	...
	while ((category = ast_category_browse_filtered(otw->last_config, NULL, category, "type=^wizard$"))) {
		...
		if (!existing) {
			ast_log_chan(NULL, LOG_ERROR, "Unable to allocate temporary container.\n");
			break;
		}

		search = ast_variable_new("@pjsip_wizard", id, "");
		if (!search) {
			ast_log_chan(NULL, LOG_ERROR, "Unable to allocate memory for vaiable '@pjsip_wizard'.\n");
			break;
		}
	...
	struct ast_sorcery *sorcery, const char *object_type)
{
	...
	if (is_one_of(object_type, object_types)) {
		if (ast_sorcery_object_type_apply_wizard(sorcery, object_type,
			...
			ast_log_chan(NULL, LOG_ERROR, "Unable to apply sangoma wizard to object type '%s'\n", object_type);
			return;
		}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER, "PJSIP Config Wizard",
