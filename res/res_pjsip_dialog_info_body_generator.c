...
static void dialog_info_to_string(void *body, struct ast_str **str)
{
	...
	if (size <= AST_PJSIP_XML_PROLOG_LEN) {
		ast_log_chan(NULL, LOG_WARNING, "dialog-info+xml body text too large\n");
		return;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "PJSIP Extension State Dialog Info+XML Provider",
