			...
			<configObject name="identify">
				...
				<configOption name="match">
					...
					<description>
						...
						enabled and a source port is not provided, GABpbx will perform
						...
						port is specified when the hostname is configured, GABpbx will
...
static struct ast_sip_endpoint *common_identify(ao2_callback_fn *identify_match_cb, void *arg)
{
	...
	if (endpoint) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Identify '%s' points to endpoint '%s' but endpoint could not be found\n",
			ast_sorcery_object_get_id(match), match->endpoint_name);
	}
...
static int ip_identify_match_handler(const struct aco_option *opt, struct ast_variable *var, void *obj)
{
	...
	while ((current_string = ast_strip(strsep(&input_string, ",")))) {
		...
		if (mask || ast_sockaddr_parse(&address, current_string, 0)) {
			...
			if (!identify->matches || error) {
				ast_log_chan(NULL, LOG_ERROR, "Failed to add address '%s' to ip endpoint identifier '%s'\n",
		...
		if (!identify->hosts) {
			...
			if (!identify->hosts) {
				ast_log_chan(NULL, LOG_ERROR, "Failed to create container to store hosts on ip endpoint identifier '%s'\n",
		...
		if (error) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to store host '%s' for resolution on ip endpoint identifier '%s'\n",
				current_string, ast_sorcery_object_get_id(obj));
			return -1;
		}
	}
...
static int ip_identify_apply(const struct ast_sorcery *sorcery, void *obj)
{
	...
	if (ast_strlen_zero(identify->endpoint_name)) {
		ast_log_chan(NULL, LOG_ERROR, "Identify '%s' missing required endpoint name.\n",
			ast_sorcery_object_get_id(identify));
		return -1;
	}
	if (ast_strlen_zero(identify->match_header) /* No header to match */
		...
		ast_log_chan(NULL, LOG_ERROR, "Identify '%s' is not configured to match anything.\n",
			ast_sorcery_object_get_id(identify));
		return -1;
	}
	...
	if (!ast_strlen_zero(identify->match_header)) {
		...
		if (!c_value) {
			ast_log_chan(NULL, LOG_ERROR, "Identify '%s' missing ':' separator in match_header '%s'.\n",
		...
		*c_value = '\0';
		...
		if (ast_strlen_zero(c_header)) {
			ast_log_chan(NULL, LOG_ERROR, "Identify '%s' has no SIP header to match in match_header '%s'.\n",
				ast_sorcery_object_get_id(identify), identify->match_header);
			return -1;
		}
		...
		if (2 < len && c_value[0] == '/' && c_value[len - 1] == '/') {
			...
			if (regcomp(&identify->regex_header_buf, c_value, REG_EXTENDED | REG_NOSUB)) {
				ast_log_chan(NULL, LOG_ERROR, "Identify '%s' failed to compile match_request_uri regex '%s'.\n",
	...
	if (!ast_strlen_zero(identify->match_request_uri)) {
		...
		if (2 < len && c_string[0] == '/' && c_string[len - 1] == '/') {
			...
			if (regcomp(&identify->regex_request_uri_buf, c_string, REG_EXTENDED | REG_NOSUB)) {
				ast_log_chan(NULL, LOG_ERROR, "Identify '%s' failed to compile match_header regex '%s'.\n",
	...
	while ((current_string = ao2_iterator_next(&i))) {
		...
		if (results == 0) {
			ast_log_chan(NULL, LOG_WARNING, "Identify '%s' provided address '%s' did not resolve to any address\n",
				ast_sorcery_object_get_id(identify), current_string);
		} else if (results == -1) {
			ast_log_chan(NULL, LOG_ERROR, "Identify '%s' failed when adding resolution results of '%s'\n",
				ast_sorcery_object_get_id(identify), current_string);
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "PJSIP IP endpoint identifier",
