...
#include "gabpbx/res_pjsip_body_generator_types.h"
	...
	struct ast_state_cb_info *info)
{
	...
	if (!task_data) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to create notify task data\n");
		return NULL;
	}
	...
	if ((info->exten_state == AST_EXTENSION_DEACTIVATED) ||
		...
		ast_verb_chan(NULL, 2, "Watcher for hint %s %s\n", exten, info->exten_state
		...
		const char *resource)
{
	...
	if (!ast_exists_extension(NULL, context, resource, PRIORITY_HINT, NULL)) {
		ast_log_chan(NULL, LOG_NOTICE, "Endpoint '%s' state subscription failed: "
		...
		const char *resource, char *display_name, int display_name_size)
{
	...
	if (!ast_get_hint(NULL, 0, display_name, display_name_size, NULL, context, resource)) {
		ast_log_chan(NULL, LOG_NOTICE, "Endpoint '%s': "
			"Extension '%s' does not exist in context '%s' or has no associated hint\n",
			ast_sorcery_object_get_id(endpoint), resource, context);
		return -1;
	}
...
static int subscription_established(struct ast_sip_subscription *sip_sub)
{
	...
	if ((exten_state_sub->id = ast_extension_state_add_destroy_extended(
		...
		ast_log_chan(NULL, LOG_WARNING, "Unable to subscribe endpoint '%s' to extension '%s@%s'\n",
	...
	if (add_datastore(exten_state_sub)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to add to subscription datastore.\n");
		ao2_cleanup(exten_state_sub);
		return -1;
	}
...
static int exten_state_publisher_cb(void *data)
{
	...
	pool = pjsip_endpt_create_pool(ast_sip_get_pjsip_endpoint(), "pub_state_body",
		1024, 1024);
	if (!pool) {
		ast_log_chan(NULL, LOG_WARNING, "Exten state publishing unable to create memory pool\n");
		exten_state_pub_data_destroy(pub_data);
		ast_free(body_text);
		return 0;
	}
	...
	for (idx = 0; idx < AST_VECTOR_SIZE(&pub_data->pubs); ++idx) {
		...
		if (ast_strlen_zero(uri)) {
			ast_log_chan(NULL, LOG_WARNING, "PUBLISH client '%s' has no from_uri or server_uri defined.\n",
				publisher->name);
			continue;
		}
		...
		if (ast_strlen_zero(uri)) {
			ast_log_chan(NULL, LOG_WARNING, "PUBLISH client '%s' has no to_uri or server_uri defined.\n",
				publisher->name);
			continue;
		}
...
static int publisher_start(struct ast_sip_outbound_publish *configuration, struct ast_sip_outbound_publish_client *client)
{
	...
	if (ast_strlen_zero(body_full)) {
		ast_log_chan(NULL, LOG_ERROR, "Outbound extension state publisher '%s': Body not set\n",
			name);
		return -1;
	}
	...
	if (ast_strlen_zero(body_type) || ast_strlen_zero(body_subtype)) {
		ast_log_chan(NULL, LOG_ERROR, "Outbound extension state publisher '%s': Body '%s' missing type or subtype\n",
			name, body_full);
		return -1;
	}

	if (!ast_sip_pubsub_is_body_generator_registered(body_type, body_subtype)) {
		ast_log_chan(NULL, LOG_ERROR, "Outbound extension state publisher '%s': '%s' body generator not registered\n",
			name, body_full);
		return -1;
	}
	...
	if (!ast_strlen_zero(context)) {
		if (build_regex(&publisher->context_regex, context)) {
			ast_log_chan(NULL, LOG_ERROR, "Outbound extension state publisher '%s': Could not build context filter '%s'\n",
				name, context);
			ao2_ref(publisher, -1);
			return -1;
		}
	...
	if (!ast_strlen_zero(exten)) {
		if (build_regex(&publisher->exten_regex, exten)) {
			ast_log_chan(NULL, LOG_ERROR, "Outbound extension state publisher '%s': Could not build exten filter '%s'\n",
				name, exten);
			ao2_ref(publisher, -1);
			return -1;
		}
	...
	if (!publisher->datastores) {
		ast_log_chan(NULL, LOG_ERROR, "Outbound extension state publisher '%s': Could not create datastores container\n",
			name);
		ao2_ref(publisher, -1);
		return -1;
	}
...
static int unload_module(void)
{
	 ...
	 * gabpbx is shutting down.  If a pjsip_evsub_unregister_pkg
...
static int load_module(void)
{
	...
	if (!publishers) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to create container to store extension state publishers\n");
	...
	if (ast_sip_register_subscription_handler(&presence_handler)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to register subscription handler %s\n",
	...
	if (ast_sip_register_event_publisher_handler(&presence_publisher)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to register presence publisher %s\n",
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "PJSIP Extension State Notifications",
