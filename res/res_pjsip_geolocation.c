	...
	char **pidf_body, unsigned int *pidf_len)
{
	...
	if (!rdata->msg_info.msg->body) {
		ast_log_chan(NULL, LOG_WARNING, "%s: There's no message body in which to search for '%s'.  Skipping\n",
	...
	if (ast_sip_are_media_types_equal(&rdata->msg_info.ctype->media,
		...
		*pidf_len = rdata->msg_info.msg->body->len;
		...
		if (!mp) {
			ast_log_chan(NULL, LOG_WARNING, "%s: A Geolocation header was found with URI '%s'"
...
static int handle_incoming_request(struct ast_sip_session *session, struct pjsip_rx_data *rdata)
{
	...
	if (config_profile->precedence != AST_GEOLOC_PRECED_DISCARD_CONFIG) {
		...
		if (!config_eprofile) {
			ast_log_chan(NULL, LOG_WARNING, "%s: Unable to create config_eprofile from "
	...
	if (geoloc_hdr && config_profile->precedence != AST_GEOLOC_PRECED_DISCARD_INCOMING) {
		...
		while (geoloc_hdr_value && !incoming_eprofile) {
			...
			if (ast_strlen_zero(geoloc_uri) || geoloc_uri[0] != '<' || strchr(geoloc_uri, '>') == NULL) {
				ast_log_chan(NULL, LOG_WARNING, "%s: Geolocation header has no or bad URI '%s'.  Skipping\n", session_name,
			...
			if (!ast_begins_with(geoloc_uri, "<cid:")) {
				...
				if (!incoming_eprofile) {
					ast_log_chan(NULL, LOG_WARNING, "%s: Unable to create effective profile for URI '%s'.  Skipping\n",
						session_name, geoloc_uri);
					continue;
				}
			} else {
				...
				if (!incoming_doc) {
					ast_log_chan(NULL, LOG_WARNING, "%s: Unable to parse pidf document for URI '%s'\n",
						session_name, geoloc_uri);
					continue;
				}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER, "res_pjsip_geolocation Module for GABpbx",
