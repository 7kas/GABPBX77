...
static int incoming_request(struct ast_sip_session *session, pjsip_rx_data * rdata)
{
	...
	if (!datastore) {
		if (!(datastore =
			...
			ast_log_chan(NULL, AST_LOG_ERROR, "Unable to create datastore for header functions.\n");
			return 0;
		}
...
static int update_header(void *obj)
{
	...
	if (!datastore || !datastore->data) {
		ast_log_chan(NULL, AST_LOG_ERROR, "No headers had been previously added to this session.\n");
		return -1;
	}
	...
	if (!hdr) {
		ast_log_chan(NULL, AST_LOG_ERROR, "There was no header named %s.\n", data->header_name);
		return -1;
	}
...
static int func_read_headers(struct ast_channel *chan, const char *function, char *data, char *buf, size_t len)
{
	...
	if (!chan || strncmp(ast_channel_name(chan), "PJSIP/", 6)) {
		ast_log_chan(NULL, LOG_ERROR, "This function requires a PJSIP channel.\n");
		return -1;
	}
...
static int func_response_read_headers(struct ast_channel *chan, const char *function, char *data, char *buf, size_t len)
{
	...
	if (ast_strlen_zero(args.header_pattern)) {
		ast_log_chan(NULL, AST_LOG_ERROR, "This function requires a pattern.\n");
		return -1;
	}
...
static int func_read_header(struct ast_channel *chan, const char *function, char *data, char *buf, size_t len)
{
	...
	if (ast_strlen_zero(args.action)) {
		ast_log_chan(NULL, AST_LOG_ERROR, "This function requires an action.\n");
		return -1;
	}
	if (ast_strlen_zero(args.header_name)) {
		ast_log_chan(NULL, AST_LOG_ERROR, "This function requires a header name.\n");
		return -1;
	}
...
static int read_param(void *obj)
{
	...
	if (param_len >= data->len) {
		ast_log_chan(NULL, LOG_ERROR, "Buffer is too small for parameter value (%zu > %zu)\n", param_len, data->len);
		return -1;
	}
...
static int add_param(void *obj)
{
	...
	if (!PJSIP_URI_SCHEME_IS_SIP(dlg_info_uri) && !PJSIP_URI_SCHEME_IS_SIPS(dlg_info_uri)) {
		ast_log_chan(NULL, LOG_WARNING, "Non SIP/SIPS URI\n");
		return -1;
	}
...
static int func_read_param(struct ast_channel *chan, const char *function, char *data, char *buf, size_t len)
{
	...
	if (ast_strlen_zero(args.param_type)) {
		ast_log_chan(NULL, AST_LOG_ERROR, "This function requires a parameter type.\n");
		return -1;
	}
	if (ast_strlen_zero(args.param_name)) {
		ast_log_chan(NULL, AST_LOG_ERROR, "This function requires a parameter name.\n");
		return -1;
	}
	...
	if (ast_strlen_zero(args.header_name) || strcasecmp(args.header_name, "From")) {
		ast_log_chan(NULL, LOG_WARNING, "Only the From header is currently supported\n");
		return -1;
	}
	...
	if (!strcasecmp(args.param_type, "header")) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Parameter type '%s' is invalid: must be 'header' or 'uri'\n", args.param_type);
		return -1;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "PJSIP Header Functions",
