...
static int evaluate_equal(struct operator *op, enum aco_option_type type, void *op_left, struct expression_token *op_right)
{
	...
	case OPT_UINT_T:
	{
		...
		if (sscanf(op_right->field, "%30d", &right) != 1) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to extract field '%s': not an integer\n", op_right->field);
			return -1;
		}
	...
	case OPT_DOUBLE_T:
	{
		...
		if (sscanf(op_right->field, "%lf", &right) != 1) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to extract field '%s': not a double\n", op_right->field);
			return -1;
		}
	...
	/* Used for timeval */
	{
		...
		if ((right.tv_sec = ast_string_to_time_t(op_right->field)) == -1) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to extract field '%s': not a timestamp\n", op_right->field);
			return -1;
		}
	...
	/* In our case, we operate only on pj_sockaddr_t */
	{
		...
		if (pj_sockaddr_parse(pj_AF_UNSPEC(), 0, &str_right, &right) != PJ_SUCCESS) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to convert field '%s': not an IPv4 or IPv6 address\n", op_right->field);
			return -1;
		}
	...
	default:
		ast_log_chan(NULL, LOG_WARNING, "Cannot evaluate field '%s': invalid type for operator '%s'\n",
...
static int evaluate_not(struct operator *op, enum aco_option_type type, void *operand)
{
	...
	case OPT_UINT_T:
		return !(*(int *)operand);
	default:
		ast_log_chan(NULL, LOG_WARNING, "Cannot evaluate: invalid operand type for operator '%s'\n", op->symbol);
	}
...
static int evaluate_like(struct operator *op, enum aco_option_type type, void *op_left, struct expression_token *op_right)
{
	...
	/* In our case, we operate on pj_str_t */
	{
		...
		if (regcomp(&regexbuf, op_right->field, REG_EXTENDED | REG_NOSUB)) {
			ast_log_chan(NULL, LOG_WARNING, "Failed to compile '%s' into a regular expression\n", op_right->field);
			return -1;
		}
...
#define APPEND_TO_OUTPUT(output, token) do { \
	...
	for (i = AST_VECTOR_SIZE(&operators) - 1; i >= 0; i--) {
		...
		if (top == &left_paren) {
			ast_log_chan(NULL, LOG_WARNING, "Unbalanced '(' parentheses in expression!\n");
			continue;
		}
...
static int evaluate_history_entry(struct pjsip_history_entry *entry, struct expression_token *queue)
{
	...
	for (it_queue = queue; it_queue; it_queue = it_queue->next) {
		...
		if (AST_VECTOR_SIZE(&stack) < it_queue->op->operands) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to evaluate expression operator '%s': not enough operands\n",
				it_queue->op->symbol);
			goto error;
		}
		...
		if (it_queue->op->operands == 1) {
			...
			if (op_one->token_type != TOKEN_TYPE_RESULT) {
				ast_log_chan(NULL, LOG_WARNING, "Unable to evaluate '%s': operand is not the result of an operation\n",
					it_queue->op->symbol);
				goto error;
			}
			...
			if (op_two->token_type == TOKEN_TYPE_FIELD) {
				...
				if (!field) {
					ast_log_chan(NULL, LOG_WARNING, "Unknown or unrecognized field: %s\n", op_two->field);
					goto error;
				}
				...
				ast_log_chan(NULL, LOG_WARNING, "Attempting to evaluate an operator: %s\n", op_two->op->symbol);
				goto error;
			}
			...
			ast_log_chan(NULL, LOG_WARNING, "Operator '%s' has an invalid number of operands\n", it_queue->op->symbol);
			ast_assert(0);
			goto error;
		}
	...
	if (AST_VECTOR_SIZE(&stack) != 1) {
		ast_log_chan(NULL, LOG_WARNING, "Expression was unbalanced: %zu results remained after evaluation\n",
	...
	if (final->token_type != TOKEN_TYPE_RESULT) {
		ast_log_chan(NULL, LOG_WARNING, "Expression did not create a usable result\n");
		goto error;
	}
	result = final->result;
	ast_free(final);
...
static void display_single_entry(struct ast_cli_args *a, struct pjsip_history_entry *entry)
{
	...
	if (pjsip_msg_print(entry->msg, buf, PJSIP_MAX_PKT_LEN) == -1) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to print SIP message %d: packet too large!\n", entry->number);
		ast_free(buf);
		return;
	}
...
static int load_module(void)
{
	...
	if (log_level < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to register history log level\n");
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "PJSIP History",
