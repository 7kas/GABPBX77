	...
	for (iter = ast_msg_var_iterator_init(msg);
		...
		if (!strcasecmp(name, "Max-Forwards")) {
			...
			if (sscanf(value, "%30d", &max_forwards) != 1 || --max_forwards == 0) {
				...
				ast_log_chan(NULL, LOG_NOTICE, "MESSAGE(Max-Forwards) reached zero.  MESSAGE not sent.\n");
				return -1;
			}
...
static char *sip_to_pjsip(char *buf, int size, int capacity)
{
	...
	if (count + size >= capacity) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to handle MESSAGE- incoming uri "
 ...
 *
 * \param rdata The SIP request
 * \param msg The gabpbx message structure to fill in.
 */
...
static void update_content_type(pjsip_tx_data *tdata, struct ast_msg *msg, struct ast_sip_body *body)
{
	...
	if (content_type) {
		...
		if (!parsed) {
			ast_log_chan(NULL, LOG_WARNING, "Failed to parse '%s' as a content type. Using text/plain\n",
				content_type);
			return;
		}
...
static int msg_send(void *data)
{
	...
	if (ast_sip_create_request("MESSAGE", NULL, endpoint, uri, NULL, &tdata)) {
		ast_log_chan(NULL, LOG_WARNING, "PJSIP MESSAGE - Could not create request\n");
		return -1;
	}
	...
	if (ast_sip_add_body(tdata, &body)) {
		...
		ast_log_chan(NULL, LOG_ERROR, "PJSIP MESSAGE - Could not add body to request\n");
		return -1;
	}
	...
	if (ast_sip_send_request(tdata, NULL, endpoint, NULL, NULL)) {
		ast_log_chan(NULL, LOG_ERROR, "PJSIP MESSAGE - Could not send request\n");
		return -1;
	}
...
static int sip_msg_send(const struct ast_msg *msg, const char *destination, const char *from)
{
	...
	if (ast_strlen_zero(destination)) {
		ast_log_chan(NULL, LOG_ERROR, "SIP MESSAGE - a 'To' URI  must be specified\n");
		return -1;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "PJSIP Messaging Support",
