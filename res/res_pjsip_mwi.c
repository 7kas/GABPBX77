...
static int send_unsolicited_mwi_notify_to_contact(void *obj, void *arg, int flags)
{
	...
	if (ast_sip_create_request("NOTIFY", NULL, endpoint, NULL, contact, &tdata)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to create unsolicited NOTIFY request to endpoint %s URI %s\n", sub->id, contact->uri);
		return 0;
	}
	...
	if (ast_sip_pubsub_generate_body_content(body.type, body.subtype, &body_data, &body_text)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to generate SIP MWI NOTIFY body.\n");
		ast_free(body_text);
		pjsip_tx_data_dec_ref(tdata);
		return 0;
	}
		...
		struct ast_sip_message_accumulator *counter)
{
	...
	if (!endpoint) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to send unsolicited MWI to %s because endpoint does not exist\n",
				sub->id);
		return;
	}
	if (ast_strlen_zero(endpoint->aors)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to send unsolicited MWI to %s because the endpoint has no"
				" configured AORs\n", sub->id);
		return;
	}
	...
	while ((aor_name = ast_strip(strsep(&endpoint_aors, ",")))) {
		...
		if (!aor) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to locate AOR %s for unsolicited MWI\n", aor_name);
			continue;
		}
	...
	struct ast_sip_endpoint *endpoint, struct ast_sip_subscription *sip_sub)
{
	...
	if (add_mwi_datastore(sub)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to add datastore for MWI subscription to %s\n",
			sub->id);
		ao2_ref(sub, -1);
		return NULL;
	}
	...
	struct ast_sip_endpoint *endpoint, struct ast_sip_subscription *sip_sub, const char *name)
{
	...
	if (!aor) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to locate aor %s. MWI subscription failed.\n", name);
		return NULL;
	}
...
static int unload_module(void)
{
	...
	if (ast_shutdown_final()) {
		...
		if (ast_serializer_pool_destroy(mwi_serializer_pool)) {
			ast_log_chan(NULL, LOG_WARNING, "Unload incomplete. Try again later\n");
			return -1;
		}
...
static int load_module(void)
{
	...
	if (!mwi_serializer_pool) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Failed to create MWI serializer pool. The default SIP pool will be used for MWI\n");
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "PJSIP MWI resource",
