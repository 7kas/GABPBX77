...
{
	if (not_allowed(name)) {
		ast_log_chan(NULL, LOG_WARNING, "Cannot specify %s header, "
			"ignoring\n", name);
		return;
	}
	...
	if (!strcasecmp(name, "Content-type")) {
		...
		if (!multiple_headers_allowed(name)) {
			...
			if (pjsip_msg_find_hdr_by_name(tdata->msg, &hdr_name, NULL)) {
				ast_log_chan(NULL, LOG_ERROR, "Only one '%s' header can be added to a NOTIFY, "
						"ignoring \"%s: %s\"\n", name, name, value);
				return;
			}
		}
...
static void build_ami_notify(pjsip_tx_data *tdata, void *info)
{
	...
	for (i = vars; i; i = i->next) {
		if (!strcasecmp(i->name, "Content-Length")) {
			ast_log_chan(NULL, LOG_NOTICE, "It is not necessary to specify Content-Length, ignoring.\n");
			continue;
		}
...
static int notify_contact(void *obj, void *arg, int flags)
{
	...
	if (ast_sip_create_request("NOTIFY", NULL, data->endpoint,
				   NULL, contact, &tdata)) {
		ast_log_chan(NULL, LOG_WARNING, "SIP NOTIFY - Unable to create request for "
			"contact %s\n",	contact->uri);
		return -1;
	}
	...
	if (ast_sip_send_request(tdata, NULL, data->endpoint, NULL, NULL)) {
		ast_log_chan(NULL, LOG_ERROR, "SIP NOTIFY - Unable to send request for "
			"contact %s\n",	contact->uri);
		return -1;
	}
...
static int notify_endpoint(void *obj)
{
	...
	if (ast_strlen_zero(data->endpoint->aors)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to NOTIFY - "
...
static int notify_uri(void *obj)
{
	...
	if (!endpoint) {
		ast_log_chan(NULL, LOG_WARNING, "No default outbound endpoint set, can not send "
			"NOTIFY requests to arbitrary URIs.\n");
		return -1;
	}

	if (ast_strlen_zero(data->uri)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to NOTIFY - URI is blank.\n");
		return -1;
	}
	...
	task_channel_data_create data_create)
{
	...
	if (strcmp(ast_channel_tech(ch)->type, "PJSIP")) {
		ast_log_chan(NULL, LOG_WARNING, "Channel was a non-PJSIP channel: %s\n", channel_name);
...
static int app_notify(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "PJSIPNotify requires arguments (to, &header=...)\n");
		return -1;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "CLI/AMI PJSIP NOTIFY Support",
