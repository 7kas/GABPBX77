 ...
 *
 * This module handles authentication when GABpbx is the UAC.
 *
 */
	...
	pjsip_rx_data *challenge, pjsip_tx_data *old_request, pjsip_tx_data **new_request)
{
	...
	if (status != PJ_SUCCESS) {
		...
		ast_log_chan(NULL, LOG_ERROR, "%s:%s: Failed to initialize client authentication session: %s\n",
			id, src_name, pj_err);
		res = -1;
		goto cleanup;
	}
	...
	default:
		pj_strerror(status, pj_err, PJ_ERR_MSG_SIZE);
		ast_log_chan(NULL, LOG_WARNING, "%s:%s: Failed to set authentication credentials: %s\n",
			id, src_name, pj_err);
		res = -1;
		goto cleanup;
	}
	...
	case PJSIP_EFAILEDCREDENTIAL:
		pj_strerror(status, pj_err, PJ_ERR_MSG_SIZE);
		ast_log_chan(NULL, LOG_WARNING, "%s:%s: Authentication credentials not accepted by server. %s\n",
			id, src_name, pj_err);
		break;
	default:
		pj_strerror(status, pj_err, PJ_ERR_MSG_SIZE);
		ast_log_chan(NULL, LOG_WARNING, "%s:%s: Unable to create request with auth: %s\n",
			id, src_name, pj_err);
		break;
	}
	res = -1;
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "PJSIP authentication resource",
