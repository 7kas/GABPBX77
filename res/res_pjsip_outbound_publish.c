			...
			<configObject name="outbound-publish">
				...
				<configOption name="server_uri">
					...
					<description><para>
						...
						This URI is used as the request URI of the outbound PUBLISH request from GABpbx.
					</para></description>
				</configOption>
...
static void schedule_publish_refresh(struct sip_outbound_publisher *publisher, int expiration)
{
	...
	if (pjsip_endpt_schedule_timer(ast_sip_get_pjsip_endpoint(), &publisher->timer, &delay) != PJ_SUCCESS) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to pass timed publish refresh to scheduler\n");
		ao2_ref(publisher, -1);
	}
	ao2_ref(publish, -1);
}
	...
	while ((state = ao2_iterator_next(&i))) {
		...
		if (!state->client->started) {
			...
			if (!handler) {
				...
				ast_log_chan(NULL, LOG_ERROR, "Failed to start outbound publish with event '%s' for client '%s'\n",
					publish->event, ast_sorcery_object_get_id(publish));
			} else {
...
int ast_sip_register_event_publisher_handler(struct ast_sip_event_publisher_handler *handler)
{
	...
	if (!handler->start_publishing || !handler->stop_publishing) {
		ast_log_chan(NULL, LOG_ERROR, "Handler does not implement required callbacks. Cannot register\n");
		return -1;
	} else if (ast_strlen_zero(handler->event_name)) {
		ast_log_chan(NULL, LOG_ERROR, "No event package specified for event publisher handler. Cannot register\n");
		return -1;
	}

	existing = find_publisher_handler_for_event_name(handler->event_name);
	if (existing) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to register event publisher handler for event %s. "
	...
	pj_str_t *server_uri, pj_str_t *to_uri, pj_str_t *from_uri)
{
	...
	if (sip_outbound_publisher_set_uri(pool, publish->server_uri, publisher->user, server_uri)) {
		ast_log_chan(NULL, LOG_ERROR, "Invalid server URI '%s' specified on outbound publish '%s'\n",
	...
	if (ast_strlen_zero(publish->to_uri)) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Invalid to URI '%s' specified on outbound publish '%s'\n",
	...
	if (ast_strlen_zero(publish->from_uri)) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Invalid from URI '%s' specified on outbound publish '%s'\n",
...
static int sip_outbound_publisher_init(void *data)
{
	...
	if (!pool) {
		ast_log_chan(NULL, LOG_ERROR, "Could not create pool for URI validation on outbound publish '%s'\n",
	...
	if (pjsip_publishc_init(publisher->client, &event, &server_uri, &from_uri, &to_uri,
		...
		ast_log_chan(NULL, LOG_ERROR, "Failed to initialize publishing client on outbound publish '%s'\n",
			ast_sorcery_object_get_id(publish));
		pjsip_endpt_release_pool(ast_sip_get_pjsip_endpoint(), pool);
		pjsip_publishc_destroy(publisher->client);
		return -1;
	...
	struct ast_sip_outbound_publish_client *client, const char *user)
{
	...
	if (ast_sip_push_task_wait_servant(NULL, sip_outbound_publisher_init, publisher)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to create publisher for outbound publish '%s'\n",
			ast_sorcery_object_get_id(client->publish));
		ao2_ref(publisher, -1);
		return NULL;
	}
...
static int explicit_publish_destroy(void *data)
{
	 ...
	 * to destroy it. Also, the ref for the GABpbx publishing client that
...
static int cancel_and_unpublish(void *obj, void *arg, int flags)
{
	...
	if (ast_sip_push_task(publisher->serializer, cancel_refresh_timer_task, ao2_bump(publisher))) {
		ast_log_chan(NULL, LOG_WARNING, "Could not stop refresh timer on outbound publish '%s'\n",
			ast_sorcery_object_get_id(client->publish));
		ao2_ref(publisher, -1);
	}
	...
	if (!publisher->sending) {
		if (ast_sip_push_task(publisher->serializer, send_unpublish_task, ao2_bump(publisher))) {
			ast_log_chan(NULL, LOG_WARNING, "Could not send unpublish message on outbound publish '%s'\n",
				ast_sorcery_object_get_id(client->publish));
			ao2_ref(publisher, -1);
		}
	}
...
#define DESTROY_CLIENT() do {			   \
	...
	if (param->code == 401 || param->code == 407) {
		...
		if (publisher->auth_attempts == publish->max_auth_attempts) {
			DESTROY_CLIENT();
			ast_log_chan(NULL, LOG_ERROR, "Reached maximum number of PUBLISH authentication attempts on outbound publish '%s'\n",
				ast_sorcery_object_get_id(publish));
	...
	if (param->code == 412) {
		...
		if (sip_outbound_publisher_init(publisher)) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to create a new outbound publish client for '%s' on 412 response\n",
				ast_sorcery_object_get_id(publish));
			goto end;
		}
		...
		if (!expires || !expires->ivalue) {
			DESTROY_CLIENT();
			ast_log_chan(NULL, LOG_ERROR, "Received 423 response on outbound publish '%s' without a Min-Expires header\n",
				ast_sorcery_object_get_id(publish));
			goto end;
		}
		...
		publisher->sending = NULL;
		if (!param->rdata) {
			ast_log_chan(NULL, LOG_NOTICE, "No response received for outbound publish '%s'\n",
				ast_sorcery_object_get_id(publish));
		}
	}
...
static int validate_publish_config(struct ast_sip_outbound_publish *publish)
{
	if (ast_strlen_zero(publish->server_uri)) {
		ast_log_chan(NULL, LOG_ERROR, "No server URI specified on outbound publish '%s'\n",
			ast_sorcery_object_get_id(publish));
		return -1;
	} else if (ast_sip_validate_uri_length(publish->server_uri)) {
		ast_log_chan(NULL, LOG_ERROR, "Server URI or hostname length exceeds pjproject limit or is not a sip(s) uri: '%s' on outbound publish '%s'\n",
			publish->server_uri,
			ast_sorcery_object_get_id(publish));
		return -1;
	} else if (ast_strlen_zero(publish->event)) {
		ast_log_chan(NULL, LOG_ERROR, "No event type specified for outbound publish '%s'\n",
			ast_sorcery_object_get_id(publish));
		return -1;
	} else if (!ast_strlen_zero(publish->from_uri)
		&& ast_sip_validate_uri_length(publish->from_uri)) {
		ast_log_chan(NULL, LOG_ERROR, "From URI or hostname length exceeds pjproject limit or is not a sip(s) uri: '%s' on outbound publish '%s'\n",
			publish->from_uri,
			ast_sorcery_object_get_id(publish));
		return -1;
	} else if (!ast_strlen_zero(publish->to_uri)
		&& ast_sip_validate_uri_length(publish->to_uri)) {
		ast_log_chan(NULL, LOG_ERROR, "To URI or hostname length exceeds pjproject limit or is not a sip(s) uri: '%s' on outbound publish '%s'\n",
			publish->to_uri,
			ast_sorcery_object_get_id(publish));
		return -1;
	}
				  ...
				  struct ast_sip_outbound_publish_state *current_state)
{
	...
	if (ast_sip_push_task_wait_servant(NULL, sip_outbound_publisher_reinit_all,
		 ...
		 */
		current_state->client->publish = publish;
		ast_log_chan(NULL, LOG_ERROR, "Unable to reinitialize client(s) for outbound publish '%s'\n",
...
#define ADD_TO_NEW_STATES(__obj) \
	...
	if (!new_states) {
		...
		if (!new_states) {
			ast_log_chan(NULL, LOG_ERROR, "Unable to allocate new states container\n");
			return -1;
		}
	}
	...
	if (!new_state) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to create state for outbound publish '%s'\n",
...
static int unload_module(void)
{
	...
	if (remaining) {
		ast_log_chan(NULL, LOG_WARNING, "Unload incomplete.  Could not stop %d outbound publications.  Try again later.\n",
			remaining);
		return -1;
	}
...
static int load_module(void)
{
	...
	if (ast_sorcery_object_register(ast_sip_get_sorcery(), "outbound-publish", sip_outbound_publish_alloc, NULL,
		...
		ast_log_chan(NULL, LOG_ERROR, "Unable to register 'outbound-publish' type with sorcery\n");
		unload_module();
		return AST_MODULE_LOAD_DECLINE;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER, "PJSIP Outbound Publish Support",
