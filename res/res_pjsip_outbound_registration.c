			...
			<configObject name="registration">
				...
				<configOption name="server_uri">
					...
					<description><para>
						...
						is used as the request URI of the outbound REGISTER request from GABpbx.</para>
	...
	pjsip_tx_data *tdata)
{
	...
	*callback_invoked = 0;
	...
	if (!ast_strlen_zero(client_state->user_agent)) {
		...
		if (!user_agent_hdr) {
			ast_log_chan(NULL, LOG_ERROR, "Could not add custom User-Agent to outbound registration %s, sending REGISTER request with non-custom header\n", client_state->registration_name);
		} else {
...
static int handle_client_registration(void *data)
{
	...
	if (set_outbound_initial_authentication_credentials(client_state->client, &client_state->outbound_auths)) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to set initial authentication credentials\n");
	}
	...
	if (DEBUG_ATLEAST(1)) {
		...
		ast_log_chan(NULL, LOG_DEBUG, "Outbound REGISTER attempt %u to '%.*s' with client '%.*s'\n",
	...
	if (!add_configured_supported_headers(client_state, tdata)) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to set supported headers\n");
		return -1;
	}
...
static void sip_outbound_registration_timer_cb(pj_timer_heap_t *timer_heap, struct pj_timer_entry *entry)
{
	...
	if (ast_sip_push_task(client_state->serializer, handle_client_registration, client_state)) {
		ast_log_chan(NULL, LOG_WARNING, "Scheduled outbound registration could not be executed.\n");
		ao2_ref(client_state, -1);
	}
}
...
static void schedule_registration(struct sip_outbound_registration_client_state *client_state, unsigned int seconds)
{
	...
	if (pjsip_endpt_schedule_timer(ast_sip_get_pjsip_endpoint(), &client_state->timer, &delay) != PJ_SUCCESS) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to schedule registration to server '%.*s' from client '%.*s'\n",
				(int) info.server_uri.slen, info.server_uri.ptr,
				(int) info.client_uri.slen, info.client_uri.ptr);
		ao2_ref(client_state, -1);
	}
			   ...
			   const char *server_uri, const char *client_uri)
{
	...
	if (response->rdata) {
		ast_log_chan(NULL, LOG_WARNING, "Temporal response '%d' received from '%s' on "
			"registration attempt to '%s', retrying in '%u'\n",
			response->code, server_uri, client_uri, interval);
	} else {
		ast_log_chan(NULL, LOG_WARNING, "No response received from '%s' on "
			"registration attempt to '%s', retrying in '%u'\n",
...
static void registration_transport_monitor_setup(const char *transport_key, const char *registration_name)
{
	...
	if((monitor_res = ast_sip_transport_monitor_register_replace_key(transport_key, registration_transport_shutdown_cb,
		...
		ast_log_chan(NULL, LOG_NOTICE, "Failed to register transport monitor for regisration %s: %d\n", registration_name, monitor_res);
	}
	ao2_ref(monitor, -1);
}
...
static int handle_registration_response(void *data)
{
	...
	if (response->code == 408 || response->code == 503) {
		...
		if (response->code == 494) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Failed to create authenticated REGISTER request to server '%s' from client '%s'\n",
					server_uri, client_uri);
		}
	...
	if (PJSIP_IS_STATUS_IN_CLASS(response->code, 200)) {
		...
		if (response->client_state->retries == response->client_state->max_retries) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Maximum retries reached when attempting outbound registration to '%s' with client '%s', stopping registration attempt\n",
				server_uri, client_uri);
		} else {
		...
		if (response->code == 403
			...
			ast_log_chan(NULL, LOG_WARNING, "403 Forbidden fatal response received from '%s' on registration attempt to '%s', retrying in '%u' seconds\n",
			...
			ast_log_chan(NULL, LOG_WARNING, "'%d' fatal response received from '%s' on registration attempt to '%s', retrying in '%u' seconds\n",
				response->code, server_uri, client_uri, response->client_state->fatal_retry_interval);
		} else {
			/* Finally if there's no hope of registering give up */
			update_client_state_status(response->client_state, SIP_REGISTRATION_REJECTED_PERMANENT);
			if (response->rdata) {
				ast_log_chan(NULL, LOG_WARNING, "Fatal response '%d' received from '%s' on registration attempt to '%s', stopping outbound registration\n",
					response->code, server_uri, client_uri);
			} else {
				ast_log_chan(NULL, LOG_WARNING, "Fatal registration attempt to '%s', stopping outbound registration\n", client_uri);
			}
		}
	}
...
static void sip_outbound_registration_response_cb(struct pjsip_regc_cbparam *param)
{
	...
	*callback_invoked = 1;
	...
	if (ast_sip_push_task(client_state->serializer, handle_registration_response, response)) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to pass incoming registration response to threadpool\n");
...
static void sip_outbound_registration_state_destroy(void *obj)
{
	...
	if (!state->client_state) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Failed to pass outbound registration client destruction to threadpool\n");
		ao2_ref(state->client_state, -1);
	}
}
...
static const char *fetch_google_access_token(const struct ast_sip_auth *auth)
{
	...
	if (ast_func_write(NULL, "CURLOPT(conntimeout)", "10")) {
		ast_log_chan(NULL, LOG_ERROR, "CURL is unavailable. This is required for Google OAuth 2.0 authentication. Please ensure it is loaded.\n");
		return NULL;
	}
	...
	ast_free(cmd);
	if (res) {
		ast_log_chan(NULL, LOG_ERROR, "Could not retrieve Google OAuth 2.0 authentication\n");
		return NULL;
	}
	...
	if (!json) {
		ast_log_chan(NULL, LOG_ERROR, "Could not parse Google OAuth 2.0 authentication: %d(%d) %s: '%s'\n",
	...
	if (!token) {
		ast_log_chan(NULL, LOG_ERROR, "Could not find Google OAuth 2.0 access_token in: '%s'\n",
			buf);
	}
		...
		const struct ast_sip_auth_vector *auth_vector)
{
	...
	for (idx = 0; idx < auth_size; ++idx) {
		...
		case AST_SIP_AUTH_TYPE_GOOGLE_OAUTH:
			...
			if (!access_token) {
				ast_log_chan(NULL, LOG_WARNING, "Obtaining Google OAuth access token failed\n");
...
static int sip_outbound_registration_regc_alloc(void *data)
{
	...
	if (!pool) {
		ast_log_chan(NULL, LOG_ERROR, "Could not create pool for URI validation on outbound registration '%s'\n",
			ast_sorcery_object_get_id(registration));
		return -1;
	}
	...
	if (!uri) {
		ast_log_chan(NULL, LOG_ERROR, "Invalid server URI '%s' specified on outbound registration '%s'\n",
	...
	uri = pjsip_parse_uri(pool, tmp.ptr, tmp.slen, 0);
	if (!uri) {
		ast_log_chan(NULL, LOG_ERROR, "Invalid client URI '%s' specified on outbound registration '%s'\n",
...
static int sip_outbound_registration_apply(const struct ast_sorcery *sorcery, void *obj)
{
	...
	if (ast_strlen_zero(applied->server_uri)) {
		ast_log_chan(NULL, LOG_ERROR, "No server URI specified on outbound registration '%s'\n",
			ast_sorcery_object_get_id(applied));
		return -1;
	} else if (ast_sip_validate_uri_length(applied->server_uri)) {
			ast_log_chan(NULL, LOG_ERROR, "Server URI or hostname length exceeds pjproject limit or is not a sip(s) uri: '%s'\n",
				ast_sorcery_object_get_id(applied));
			return -1;
	} else if (ast_strlen_zero(applied->client_uri)) {
		ast_log_chan(NULL, LOG_ERROR, "No client URI specified on outbound registration '%s'\n",
			ast_sorcery_object_get_id(applied));
		return -1;
	} else if (ast_sip_validate_uri_length(applied->client_uri)) {
			ast_log_chan(NULL, LOG_ERROR, "Client URI or hostname length exceeds pjproject limit or is not a sip(s) uri: '%s'\n",
				ast_sorcery_object_get_id(applied));
			return -1;
	} else if (applied->line && ast_strlen_zero(applied->endpoint)) {
		ast_log_chan(NULL, LOG_ERROR, "Line support has been enabled on outbound registration '%s' without providing an endpoint\n",
			ast_sorcery_object_get_id(applied));
		return -1;
	} else if (!ast_strlen_zero(applied->endpoint) && !applied->line) {
		ast_log_chan(NULL, LOG_ERROR, "An endpoint has been specified on outbound registration '%s' without enabling line support\n",
			ast_sorcery_object_get_id(applied));
		return -1;
	}
	...
	if (ast_sip_push_task(new_state->client_state->serializer,
		...
		ast_log_chan(NULL, LOG_ERROR, "Failed to perform outbound registration on '%s'\n",
...
static int unload_module(void)
{
	...
	if (remaining) {
		 ...
		 */
		ast_log_chan(NULL, LOG_WARNING, "Unload incomplete.  Could not stop %d outbound registrations.  Try again later.\n",
			remaining);
		return -1;
	}
...
static int load_module(void)
{
	...
	if (!new_states) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to allocate registration states container\n");
		unload_module();
		return AST_MODULE_LOAD_DECLINE;
	}
	...
	if (ast_sorcery_instance_observer_add(ast_sip_get_sorcery(),
		...
		ast_log_chan(NULL, LOG_ERROR, "Unable to register observers.\n");
		unload_module();
		return AST_MODULE_LOAD_DECLINE;
	}
	...
	if (!cli_formatter) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to allocate memory for cli formatter\n");
		unload_module();
		return AST_MODULE_LOAD_DECLINE;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "PJSIP Outbound Registration Support",
