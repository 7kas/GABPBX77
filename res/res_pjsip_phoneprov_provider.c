...
static void *phoneprov_alloc(const char *name)
{
	...
	if (!pp || !(pp->vars = ast_var_list_create())) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to allocate memory for phoneprov structure %s\n",
			name);
		ao2_cleanup(pp);
		return NULL;
	}
...
static int assign_and_insert(const char *name, const char *value, struct varshead *vars)
{
	...
	if (!var) {
		ast_log_chan(NULL, LOG_ERROR, "Could not allocate variable memory for variable.\n");
		return -1;
	}
	...
	char *port_string)
{
	...
	*port_string = '\0';
	...
	endpoint = ast_sorcery_retrieve_by_id(sorcery, "endpoint",
		endpoint_name);
	if (!endpoint) {
		ast_log_chan(NULL, LOG_ERROR, "phoneprov %s contained invalid endpoint %s.\n", id,
			endpoint_name);
		return -1;
	}
	...
	transport = ast_sorcery_retrieve_by_id(sorcery, "transport",
		endpoint->transport);
	if (!transport) {
		ast_log_chan(NULL, LOG_ERROR, "Endpoint %s contained invalid transport %s.\n", endpoint_name,
			endpoint->transport);
		return -1;
	}
	...
	if (!auth) {
		ast_log_chan(NULL, LOG_ERROR, "phoneprov %s contained invalid auth %s.\n", id, auth_name);
		return -1;
	}
...
static void users_apply_handler(struct phoneprov *pp)
{
	...
	if (!ast_var_find(pp->vars,
		...
		ast_log_chan(NULL, LOG_ERROR, "phoneprov %s must contain a MAC entry.\n", id);
		return;
	}

	if (!ast_var_find(pp->vars,
		ast_phoneprov_std_variable_lookup(AST_PHONEPROV_STD_PROFILE))) {
		ast_log_chan(NULL, LOG_ERROR, "phoneprov %s must contain a PROFILE entry.\n", id);
		return;
	}
	...
	if (!ast_var_find(pp->vars,
		ast_phoneprov_std_variable_lookup(AST_PHONEPROV_STD_PROFILE))) {
		ast_log_chan(NULL, LOG_ERROR, "phoneprov %s didn't contain a PROFILE entry.\n", id);
	}
...
static int load_module(void)
{
	...
	if (ast_phoneprov_provider_register(AST_MODULE, load_users)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to register pjsip phoneprov provider.\n");
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "PJSIP Phoneprov Provider",
