...
static int pidf_generate_body_content(void *body, void *data)
{
	...
	if (!pjpidf_pres_add_note(state_data->pool, pres, pj_cstr(&note, pidfnote))) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to add note to PIDF presence\n");
		return -1;
	}
...
static void pidf_to_string(void *body, struct ast_str **str)
{
	...
	if (size <= AST_PJSIP_XML_PROLOG_LEN) {
		ast_log_chan(NULL, LOG_WARNING, "PIDF body text too large\n");
		return;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "PJSIP Extension State PIDF Provider",
