...
static int pidf_supplement_body(void *body, void *data)
{
	...
	if (!(node = ast_sip_presence_xml_create_node(
		...
		ast_log_chan(NULL, LOG_WARNING, "Unable to create PIDF tuple\n");
		return -1;
	}
	...
	if (!(node = ast_sip_presence_xml_create_node(
		...
		ast_log_chan(NULL, LOG_WARNING, "Unable to create PIDF tuple status\n");
		return -1;
	}

	if (!(node = ast_sip_presence_xml_create_node(
		      state_data->pool, node, "digium_presence"))) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to create digium presence\n");
		return -1;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "PJSIP PIDF Sangoma presence supplement",
