...
#include "gabpbx/res_pjsip_presence_xml.h"
...
/*** DOCUMENTATION
	...
	<managerEvent language="en_US" name="InboundSubscriptionDetail">
		<managerEventInstance class="EVENT_FLAG_COMMAND">
			...
			<synopsis>
				Provides details about an inbound subscription - one in which GABpbx
				handles SUBSCRIBE requests and periodically sends NOTIFYs to its
				subscribers.
			</synopsis>
			<syntax>
				<parameter name="Role">
					<para>GABpbx's role for this subscription. This will always be
				...
				<parameter name="SubscriptionType">
					<para>GABpbx currently supports the following subscription types, but
	...
	<managerEvent language="en_US" name="OutboundSubscriptionDetail">
		<managerEventInstance class="EVENT_FLAG_COMMAND">
			...
			<synopsis>
				Provides details about an outbound subscription - one in which GABpbx
		...
		<configFile name="pjsip.conf">
			<configObject name="subscription_persistence">
				...
				<configOption name="prune_on_boot">
					...
					and therefore the subscription must be deleted after an gabpbx restart.
					</synopsis>
				</configOption>
			...
			<configObject name="resource_list">
				...
				<description>
					...
					<note>
						<para>Current limitations limit the size of SIP NOTIFY requests that GABpbx sends
				...
				<configOption name="event">
					...
					<description><para>
						The SIP event package describes the types of resources that GABpbx reports
						the state of.
					</para>
						<enumlist>
				...
				<configOption name="list_item">
					...
					<description>
						<para>In general GABpbx looks up list items in the following way:</para>
				...
				<configOption name="full_state" default="no">
					...
					<description>
						<para>If this option is enabled, and a resource changes state, then GABpbx will construct
						a notification that contains the state of all resources in the list. If the option is
						disabled, GABpbx will construct a notification that only contains the states of
						resources that have changed.</para>
						<note>
							<para>Even with this option disabled, there are certain situations where GABpbx is forced
							to send a notification with the states of all resources in the list. When a subscriber
							renews or terminates its subscription to the list, GABpbx MUST send a full state
							notification.</para>
						</note>
					</description>
				</configOption>
				<configOption name="notification_batch_interval" default="0">
					...
					<synopsis>Time GABpbx should wait, in milliseconds, before sending notifications.</synopsis>
					<description>
						<para>When a resource's state changes, it may be desired to wait a certain amount before GABpbx
						sends a notification to subscribers. This allows for other state changes to accumulate, so that
						GABpbx can communicate multiple state changes in a single notification instead of rapidly sending
...
struct resource_list {
	...
	/*! Indicates if GABpbx sends full or partial state on notifications. */
	unsigned int full_state;
	/*! Time, in milliseconds GABpbx waits before sending a batched notification.*/
...
enum sip_subscription_tree_state {
	...
	/*! A terminate has been requested by GABpbx, the client, or pjproject */
...
struct sip_subscription_tree {
	...
	/*! On gabpbx restart, this is the task data used
...
enum sip_persistence_update_type {
	...
	/*! Subscription recreated by gabpbx on startup */
	...
	pjsip_rx_data *rdata, enum sip_persistence_update_type type)
{
	...
	if (rdata) {
		...
		if (contact_hdr) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Contact not updated due to missing contact header\n");
		}
...
static struct ast_sip_subscription_handler *subscription_get_handler_from_rdata(pjsip_rx_data *rdata, const char *endpoint)
{
	...
	if (!event_header) {
		ast_log_chan(NULL, LOG_WARNING, "Incoming SUBSCRIBE request from %s with no Event header\n",
	...
	if (!handler) {
		ast_log_chan(NULL, LOG_WARNING, "No registered subscribe handler for event %s from %s\n", event,
			endpoint ? endpoint : "Unknown");
	}
...
static struct resource_list *retrieve_resource_list(const char *resource, const char *event)
{
	...
	if (strcmp(list->event, event)) {
		ast_log_chan(NULL, LOG_WARNING, "Found resource list %s, but its event type (%s) does not match SUBSCRIBE's (%s)\n",
		...
		const char *resource, const char *display_name, struct sip_subscription_tree *tree)
{
	...
	if (!msg) {
		ast_log_chan(NULL, LOG_ERROR, "No dialog message saved for SIP subscription. Cannot allocate subscription for resource %s\n", resource);
		return NULL;
	}
		...
		pj_status_t *dlg_status, struct subscription_persistence *persistence)
{
	...
	if (!dlg) {
		if (*dlg_status != PJ_EEXISTS) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to create dialog for SIP subscription\n");
		}
		ao2_ref(sub_tree, -1);
		return NULL;
	}
...
static int sub_persistence_recreate(void *obj)
{
	...
	if (!handler || !handler->notifier) {
		ast_log_chan(NULL, LOG_WARNING, "Failed recreating '%s' subscription: Could not get subscription handler.\n",
	...
	if (!generator) {
		ast_log_chan(NULL, LOG_WARNING, "Failed recreating '%s' subscription: Body generator not available.\n",
			persistence->endpoint);
		ast_sorcery_delete(ast_sip_get_sorcery(), persistence);
		return 0;
	}
	...
	endpoint = ast_sorcery_retrieve_by_id(ast_sip_get_sorcery(), "endpoint",
		persistence->endpoint);
	if (!endpoint) {
		ast_log_chan(NULL, LOG_WARNING, "Failed recreating '%s' subscription: The endpoint was not found\n",
			persistence->endpoint);
		ast_sorcery_delete(ast_sip_get_sorcery(), persistence);
		return 0;
	}
	...
	if (!expires_header) {
		...
		if (!expires_header) {
			ast_log_chan(NULL, LOG_WARNING, "Failed recreating '%s' subscription: Could not update expires header.\n",
				persistence->endpoint);
			ast_sorcery_delete(ast_sip_get_sorcery(), persistence);
			ao2_ref(endpoint, -1);
			return 0;
	...
	if (PJSIP_IS_STATUS_IN_CLASS(resp, 200)) {
		...
		if (!sub_tree) {
			if (dlg_status != PJ_EEXISTS) {
				ast_log_chan(NULL, LOG_WARNING, "Failed recreating '%s' subscription: Could not create subscription tree.\n",
					persistence->endpoint);
				ast_sorcery_delete(ast_sip_get_sorcery(), persistence);
			}
		} else {
...
static int subscription_persistence_recreate(void *obj, void *arg, int flags)
{
	...
	if (ast_sip_create_rdata_with_contact(&rdata, persistence->packet, persistence->src_name,
		...
		ast_log_chan(NULL, LOG_WARNING, "Failed recreating '%s' subscription: The message could not be parsed\n",
			persistence->endpoint);
		ast_sorcery_delete(ast_sip_get_sorcery(), persistence);
		return 0;
	}
	...
	if (rdata.msg_info.msg->type != PJSIP_REQUEST_MSG) {
		ast_log_chan(NULL, LOG_NOTICE, "Failed recreating '%s' subscription: Stored a SIP response instead of a request.\n",
			persistence->endpoint);
		ast_sorcery_delete(ast_sip_get_sorcery(), persistence);
		return 0;
	}
	...
	if (!serializer) {
		ast_log_chan(NULL, LOG_WARNING, "Failed recreating '%s' subscription: Could not get distributor serializer.\n",
			persistence->endpoint);
		ast_sorcery_delete(ast_sip_get_sorcery(), persistence);
		return 0;
	}
	...
	if (ast_sip_push_task_wait_serializer(serializer, sub_persistence_recreate,
		&recreate_data)) {
		ast_log_chan(NULL, LOG_WARNING, "Failed recreating '%s' subscription: Could not continue under distributor serializer.\n",
			persistence->endpoint);
		ast_sorcery_delete(ast_sip_get_sorcery(), persistence);
	}
...
static int subscription_persistence_load(void *data)
{
	...
	if (!pool) {
		ast_log_chan(NULL, LOG_WARNING, "Could not create a memory pool for recreating SIP subscriptions\n");
		return 0;
	}
		...
		struct ast_sip_endpoint *endpoint, const char *resource)
{
	...
	if (!contact || ast_strlen_zero(contact->uri)) {
		ast_log_chan(NULL, LOG_WARNING, "No contacts configured for endpoint %s. Unable to create SIP subsription\n",
				ast_sorcery_object_get_id(endpoint));
		ao2_ref(sub_tree, -1);
		ao2_cleanup(contact);
		return NULL;
...
static int sip_subscription_send_request(struct sip_subscription_tree *sub_tree, pjsip_tx_data *tdata)
{
	...
	if (allocate_tdata_buffer(tdata)) {
		ast_log_chan(NULL, LOG_ERROR, "SIP request %s is too large to send.\n", tdata->info);
...
int ast_sip_register_publish_handler(struct ast_sip_publish_handler *handler)
{
	if (ast_strlen_zero(handler->event_name)) {
		ast_log_chan(NULL, LOG_ERROR, "No event package specified for publish handler. Cannot register\n");
		return -1;
	}
	...
	if (!handler->publications) {
		ast_log_chan(NULL, LOG_ERROR, "Could not allocate publications container for event '%s'\n",
			handler->event_name);
		return -1;
	}
...
int ast_sip_register_subscription_handler(struct ast_sip_subscription_handler *handler)
{
	...
	if (ast_strlen_zero(handler->event_name)) {
		ast_log_chan(NULL, LOG_ERROR, "No event package specified for subscription handler. Cannot register\n");
		return -1;
	}
	...
	for (i = 0; i < num_accept; ++i) {
		...
		if (generator) {
			...
			if (strcmp(generator->body_type, body_type)) {
				ast_log_chan(NULL, LOG_WARNING, "Body generator '%s/%s'(%p) does not accept the type of data this event generates\n",
...
static int initial_notify_task(void * obj)
{
	...
	if (ind->expires != PJSIP_EXPIRES_NOT_SPECIFIED) {
		...
		if (!ind->sub_tree->expiration_task) {
			ast_log_chan(NULL, LOG_ERROR, "Unable to create expiration timer of %d seconds for %s\n",
				ind->expires, name);
		}
	}
...
static pj_bool_t pubsub_on_rx_subscribe_request(pjsip_rx_data *rdata)
{
	...
	if (!endpoint->subscription.allow) {
		ast_log_chan(NULL, LOG_WARNING, "Subscriptions not permitted for endpoint %s.\n", ast_sorcery_object_get_id(endpoint));
	...
	if (!ast_sip_is_uri_sip_sips(request_uri)) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Request URI '%s' is not a sip: or sips: URI.\n", uri_str);
	...
	if (expires_header) {
		...
		if (expires_header->ivalue < endpoint->subscription.minexpiry) {
			ast_log_chan(NULL, LOG_WARNING, "Subscription expiration %d is too brief for endpoint %s. Minimum is %u\n",
...
static pj_bool_t pubsub_on_rx_publish_request(pjsip_rx_data *rdata)
{
	...
	event_header = pjsip_msg_find_hdr_by_name(rdata->msg_info.msg, &str_event_name, rdata->msg_info.msg->hdr.next);
	if (!event_header) {
		ast_log_chan(NULL, LOG_WARNING, "Incoming PUBLISH request from %s with no Event header\n",
			ast_sorcery_object_get_id(endpoint));
		pjsip_endpt_respond_stateless(ast_sip_get_pjsip_endpoint(), rdata, 489, NULL, NULL, NULL);
		return PJ_TRUE;
	}
	...
	if (!handler) {
		ast_log_chan(NULL, LOG_WARNING, "No registered publish handler for event %s from %s\n", event,
			ast_sorcery_object_get_id(endpoint));
		pjsip_endpt_respond_stateless(ast_sip_get_pjsip_endpoint(), rdata, 489, NULL, NULL, NULL);
		return PJ_TRUE;
	}
...
int ast_sip_pubsub_register_body_generator(struct ast_sip_pubsub_body_generator *generator)
{
	...
	if (existing) {
		AST_RWLIST_UNLOCK(&body_generators);
		ast_log_chan(NULL, LOG_WARNING, "A body generator for %s/%s is already registered.\n",
		...
		struct ast_sip_body_data *data, struct ast_str **str)
{
	...
	if (!generator) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to find a body generator for %s/%s\n",
				type, subtype);
		return -1;
	}

	if (strcmp(data->body_type, generator->body_type)) {
		ast_log_chan(NULL, LOG_WARNING, "%s/%s body generator does not accept the type of data provided\n",
			type, subtype);
		return -1;
	}
	...
	if (!body) {
		ast_log_chan(NULL, LOG_WARNING, "%s/%s body generator could not to allocate a body\n",
			type, subtype);
		return -1;
	}
...
static pj_bool_t pubsub_on_rx_mwi_notify_request(pjsip_rx_data *rdata)
{
	...
	*atsign = '\0';
	...
	if (ast_publish_mwi_state(mailbox, context,
		summary.voice_messages_new, summary.voice_messages_old)) {
		ast_log_chan(NULL, LOG_ERROR, "Incoming MWI: Endpoint: '%s' Could not publish MWI to stasis.  "
		...
		int *p_st_code, pj_str_t **p_st_text, pjsip_hdr *res_hdr, pjsip_msg_body **p_body)
{
				...
				if (new_root) {
					if (cmp_subscription_childrens(old_root, new_root)) {
						...
						if (ast_sip_push_task(sub_tree->serializer, destroy_subscriptions_task, old_root)) {
							ast_log_chan(NULL, LOG_ERROR, "Failed to push task to destroy old subscriptions for RLS '%s->%s'.\n",
								ast_sorcery_object_get_id(endpoint), old_root->resource);
						}
					} else {
	...
	if (ast_sip_push_task(sub_tree->serializer, serialized_pubsub_on_refresh_timeout, ao2_bump(sub_tree))) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Failed to push task to send NOTIFY.\n");
		...
		struct ast_variable *var, void *obj)
{
	...
	while ((item = ast_strip(strsep(&items, ",")))) {
		...
		if (item_in_vector(list, item)) {
			ast_log_chan(NULL, LOG_WARNING, "Ignoring duplicated list item '%s'\n", item);
			continue;
		}
...
static int resource_list_apply_handler(const struct ast_sorcery *sorcery, void *obj)
{
	...
	if (ast_strlen_zero(list->event)) {
		ast_log_chan(NULL, LOG_WARNING, "Resource list '%s' has no event set\n",
				ast_sorcery_object_get_id(list));
		return -1;
	}

	if (AST_VECTOR_SIZE(&list->items) == 0) {
		ast_log_chan(NULL, LOG_WARNING, "Resource list '%s' has no list items\n",
				ast_sorcery_object_get_id(list));
		return -1;
	}
...
static int load_module(void)
{
	...
	if (!(sched = ast_sched_context_create())) {
		ast_log_chan(NULL, LOG_ERROR, "Could not create scheduler for publication expiration\n");
		return AST_MODULE_LOAD_DECLINE;
	}

	if (ast_sched_start_thread(sched)) {
		ast_log_chan(NULL, LOG_ERROR, "Could not start scheduler thread for publication expiration\n");
	...
	if (ast_sorcery_object_register(sorcery, "subscription_persistence", subscription_persistence_alloc,
		NULL, NULL)) {
		ast_log_chan(NULL, LOG_ERROR, "Could not register subscription persistence object support\n");
		ast_sched_context_destroy(sched);
		return AST_MODULE_LOAD_DECLINE;
	}
	...
	if (ast_sip_register_service(&pubsub_module)) {
		ast_log_chan(NULL, LOG_ERROR, "Could not register pubsub service\n");
		ast_sched_context_destroy(sched);
		return AST_MODULE_LOAD_DECLINE;
	}

	if (pjsip_evsub_init_module(ast_sip_get_pjsip_endpoint()) != PJ_SUCCESS) {
		ast_log_chan(NULL, LOG_ERROR, "Could not initialize pjsip evsub module.\n");
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER, "PJSIP event resource",
