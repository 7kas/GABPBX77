	...
	enum ast_transfer_type transfer_type)
{
	...
	if (refer->progress && !refer->attended && !refer->progress->refer_blind_progress) {
		...
		if (!refer->progress->transferee) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Could not copy channel name '%s' during transfer - assuming success\n",
				ast_channel_name(chan));
		...
		if (refer->progress->framehook < 0) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Could not attach REFER transfer progress monitoring hook to channel '%s' - assuming success\n",
				ast_channel_name(chan));
		...
		if (!refer->progress->bridge_sub) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Could not create bridge stasis subscription for monitoring progress on transfer of channel '%s' - assuming success\n",
					ast_channel_name(chan));
	...
	if (!ast_strlen_zero(get_xfrdata)) {
		...
		if (pbxvar) {
			for (; hdr != end; hdr = hdr->next) {
				if (!pj_strnicmp2(&hdr->name, prefix, strlen(prefix))) {
					...
					if (len < 0) {
						...
						ast_log_chan(NULL, LOG_WARNING, "Could not store header '%s' from transfer on channel '%s' - too long text\n", hdr_name, ast_channel_name(chan));
						continue;
					}
					buf[len] = '\0';
			...
			ast_log_chan(NULL, LOG_ERROR, "Channel '%s' failed to allocate buffer for TRANSFER_DATA variable\n", ast_channel_name(chan));
		}
	}
...
static int refer_send(void *data)
{
	...
	if (!tmp_str) {
		ast_log_chan(NULL, LOG_WARNING, "PJSIP REFER - Refer to not a valid resource identifier or SIP URI\n");
		return -1;
	}
	if (!(refer = ao2_alloc(sizeof(struct refer_out_of_dialog), refer_out_of_dialog_destroy))) {
		ast_log_chan(NULL, LOG_ERROR, "PJSIP REFER - Could not allocate resources.\n");
		return -1;
	}
	...
	if (!refer->dlg) {
		ast_log_chan(NULL, LOG_WARNING, "PJSIP REFER - Could not create dialog\n");
		ao2_cleanup(refer);
		return -1;
	}
	...
	if (pjsip_xfer_create_uac(refer->dlg, &xfer_cb, &sub) != PJ_SUCCESS) {
		ast_log_chan(NULL, LOG_WARNING, "PJSIP REFER - Could not create uac\n");
		ao2_cleanup(refer);
		return -1;
	}
	...
	if (pjsip_xfer_initiate(sub, pj_cstr(&tmp, refer_to_str), &tdata) != PJ_SUCCESS) {
		ast_log_chan(NULL, LOG_WARNING, "PJSIP REFER - Could not create request\n");
		goto failure;
	}
...
static int sip_refer_send(const struct ast_refer *refer)
{
	...
	if (ast_strlen_zero(ast_refer_get_to(refer))) {
		ast_log_chan(NULL, LOG_ERROR, "SIP REFER - a 'To' URI  must be specified\n");
		return -1;
	}
	...
	pjsip_param *replaces_param, struct refer_progress *progress)
{
	...
	if (replaces_param) {
		...
		if (!(replaces = pjsip_parse_hdr(rdata->tp_info.pool, &str_replaces, replaces_content.ptr,
			...
			ast_log_chan(NULL, LOG_ERROR, "Received REFER request on channel '%s' from endpoint '%s' with invalid Replaces header, rejecting\n",
	...
	if (ast_sip_session_defer_termination(session)) {
		ast_log_chan(NULL, LOG_ERROR, "Channel '%s' from endpoint '%s' attempted ari-only transfer but could not defer termination, rejecting\n",
	...
	pjsip_param *replaces_param, struct refer_progress *progress)
{
	...
	if ((dlg = pjsip_ua_find_dialog(&replaces->call_id, &replaces->to_tag, &replaces->from_tag, PJ_TRUE))) {
		...
		if (!(attended = refer_attended_alloc(session, other_session, progress))) {
			ast_log_chan(NULL, LOG_ERROR, "Received REFER request on channel '%s' from endpoint '%s' for local dialog but could not allocate structure to complete, rejecting\n",
				ast_channel_name(session->channel), ast_sorcery_object_get_id(session->endpoint));
			return 500;
		}

		if (ast_sip_session_defer_termination(session)) {
			ast_log_chan(NULL, LOG_ERROR, "Received REFER request on channel '%s' from endpoint '%s' for local dialog but could not defer termination, rejecting\n",
				ast_channel_name(session->channel), ast_sorcery_object_get_id(session->endpoint));
		...
		if (!ast_exists_extension(NULL, context, "external_replaces", 1, NULL)) {
			ast_log_chan(NULL, LOG_ERROR, "Received REFER for remote session on channel '%s' from endpoint '%s' but 'external_replaces' extension not found in context %s\n",
		...
		if (ast_sip_session_defer_termination(session)) {
			ast_log_chan(NULL, LOG_ERROR, "Received REFER for remote session on channel '%s' from endpoint '%s' but could not defer termination, rejecting\n",
				ast_channel_name(session->channel),
				ast_sorcery_object_get_id(session->endpoint));
			return 500;
		}
	...
	struct refer_progress *progress)
{
	...
	if (!ast_exists_extension(NULL, context, exten, 1, NULL)) {
		ast_log_chan(NULL, LOG_ERROR, "Channel '%s' from endpoint '%s' attempted blind transfer to '%s@%s' but target does not exist\n",
	...
	if (ast_sip_session_defer_termination(session)) {
		ast_log_chan(NULL, LOG_ERROR, "Channel '%s' from endpoint '%s' attempted blind transfer but could not defer termination, rejecting\n",
			ast_channel_name(session->channel),
			ast_sorcery_object_get_id(session->endpoint));
		return 500;
	}
...
static int refer_incoming_refer_request(struct ast_sip_session *session, struct pjsip_rx_data *rdata)
{
	...
	if (!session->endpoint->allowtransfer) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Endpoint %s transfer attempt blocked due to configuration\n",
				ast_sorcery_object_get_id(session->endpoint));
		return 0;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "PJSIP Blind and Attended Transfer Support",
