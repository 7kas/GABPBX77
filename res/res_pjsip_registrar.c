...
#define BASE_REGISTRAR "res_pjsip_registrar"
...
static int registrar_add_contact(void *obj, void *arg, int flags)
{
	...
	if (parsed && (PJSIP_URI_SCHEME_IS_SIP(parsed) || PJSIP_URI_SCHEME_IS_SIPS(parsed))) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Skipping invalid Contact URI \"%.*s\" for AOR %s\n",
	...
	struct ast_sip_contact *contact, const char *aor_name)
{
	...
	if (aor_size) {
		if (VERBOSITY_ATLEAST(3)) {
			...
			ast_verb_chan(NULL, 3, "Removed contact '%s' from AOR '%s' due to %s\n",
	...
	struct aor_core_response *response)
{
	...
	/* germanico */
	char user_agent_first_five[6] = { 0 };
	...
	if (registrar_validate_contacts(rdata, details.pool, contacts, aor, permanent, &added, &updated, &deleted)) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Failed to validate contacts in REGISTER request from '%s'\n",
	...
	if (registrar_validate_path(rdata, aor, &path_str)) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Invalid modifications made to REGISTER request from '%s' by intervening proxy\n",
				ast_sorcery_object_get_id(endpoint));
		response->code = 420;
		pjsip_endpt_release_pool(ast_sip_get_pjsip_endpoint(), details.pool);
		return;
	...
	if (contact_count > aor->max_contacts) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Registration attempt from endpoint '%s' (%s:%d) to AOR '%s' will exceed max contacts of %u\n",
	...
	if (user_agent_hdr) {
        ...
        /* germanico filter by user agent */
	if (!ast_strlen_zero(endpoint->user_agent_prefix))
	{
	if (!ast_strlen_zero(user_agent))
	{
                        strncpy(user_agent_first_five, user_agent, 5);
                        if (strstr(endpoint->user_agent_prefix, user_agent_first_five) == NULL)
                        {
                                ast_log_chan(NULL, LOG_WARNING, "User-Agent prefix '%s' not found in allowed substrings '%s' for endpoint '%s'.\n", user_agent_first_five, endpoint->user_agent_prefix, ast_sorcery_object_get_id(endpoint));
                                response->code = 500;
                                pjsip_endpt_release_pool(ast_sip_get_pjsip_endpoint(), details.pool);
                                return;
                        }
                ...
                {
                        ast_log_chan(NULL, LOG_WARNING, "Unknown User-Agent, but User Agent Prefix not empty '%s'.\n", ast_sorcery_object_get_id(endpoint));
                        response->code = 500;
                        pjsip_endpt_release_pool(ast_sip_get_pjsip_endpoint(), details.pool);
                        return;
                }
	...
	for (; (contact_hdr = (pjsip_contact_hdr *) pjsip_msg_find_hdr(rdata->msg_info.msg, PJSIP_H_CONTACT, contact_hdr->next)); pj_pool_reset(details.pool)) {
		...
		if (!contact) {
			...
			if (!expiration) {
				ast_verb_chan(NULL, 3, "Attempted to remove non-existent contact '%s' from AOR '%s' by request\n",
			...
			if (!contact) {
				ast_log_chan(NULL, LOG_ERROR, "Unable to bind contact '%s' to AOR '%s'\n",
					contact_uri, aor_name);
				continue;
			}
			...
			ast_verb_chan(NULL, 3, "Added contact '%s' to AOR '%s' with expiration of %d seconds\n",
			...
			if (!contact_update) {
				ast_log_chan(NULL, LOG_ERROR, "Failed to update contact '%s' expiration time to %d seconds.\n",
	...
	const char *aor_name)
{
	...
	struct ast_context *hint_ctx;
	char hint_device[AST_MAX_EXTENSION + 7];
	...
	/* germanico dynamic hint */
	const char *hint_context_name = S_OR(endpoint->subscription.context, endpoint->context);
	const char *callerid_num = S_OR(endpoint->id.self.number.str, aor_name);

	/* Verificar/crear el contexto antes de añadir el hint */
	struct ast_context *hint_context = ast_context_find_or_create(NULL, NULL, hint_context_name, "res_pjsip_registrar");
	if (hint_context) {
		snprintf(hintpjsip, sizeof(hintpjsip), "PJSIP/%s", aor_name);
    
		if (ast_add_extension(hint_context_name, 0, callerid_num, PRIORITY_HINT, NULL, NULL, 
		hintpjsip, NULL, NULL, "realtime_peer") < 0) {
			ast_log(LOG_WARNING, "Failed to add dynamic hint for %s @ %s in context %s\n", 
				hintpjsip, callerid_num, hint_context_name);
		} else {
			ast_debug(3, "Added dynamic hint for %s @ %s in context %s\n", 
				hintpjsip, callerid_num, hint_context_name);
		}
	} else {
		ast_log(LOG_ERROR, "Failed to find or create context '%s' for dynamic hint\n", hint_context_name);
	}
...
static struct ast_sip_aor *find_registrar_aor(struct pjsip_rx_data *rdata, struct ast_sip_endpoint *endpoint)
{
	...
	if (ast_strlen_zero(aor_name) || !(aor = ast_sip_location_retrieve_aor(aor_name))) {
		...
		ast_log_chan(NULL, LOG_WARNING, "AOR '%s' not found for endpoint '%s' (%s:%d)\n",
...
static pj_bool_t registrar_on_rx_request(struct pjsip_rx_data *rdata)
{
	...
	if (ast_strlen_zero(endpoint->aors)) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Endpoint '%s' (%s:%d) has no configured AORs\n", ast_sorcery_object_get_id(endpoint),
			rdata->pkt_info.src_name, rdata->pkt_info.src_port);
		return PJ_TRUE;
	}
	...
	if (!PJSIP_URI_SCHEME_IS_SIP(rdata->msg_info.to->uri) && !PJSIP_URI_SCHEME_IS_SIPS(rdata->msg_info.to->uri)) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Endpoint '%s' (%s:%d) attempted to register to an AOR with a non-SIP URI\n", ast_sorcery_object_get_id(endpoint),
			rdata->pkt_info.src_name, rdata->pkt_info.src_port);
		return PJ_TRUE;
	}
	...
	if (!aor->max_contacts) {
		...
		ast_log_chan(NULL, LOG_WARNING, "AOR '%s' has no configured max_contacts. Endpoint '%s' (%s:%d) unable to register\n",
			aor_name, ast_sorcery_object_get_id(endpoint),
			rdata->pkt_info.src_name, rdata->pkt_info.src_port);
	} else {
	...
	if (check_interval) {
		if (check_thread == AST_PTHREADT_NULL) {
			if (ast_pthread_create_background(&check_thread, NULL, check_expiration_thread, NULL)) {
				ast_log_chan(NULL, LOG_ERROR, "Could not create thread for checking contact expiration.\n");
				return;
			}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "PJSIP Registrar Support",
