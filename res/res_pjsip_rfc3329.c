...
static void rfc3329_incoming_response(struct ast_sip_session *session, struct pjsip_rx_data *rdata)
{
	...
	if (!datastore
		...
		ast_log_chan(NULL, AST_LOG_WARNING, "Could not store session data. Still attempting requests, but they might be missing necessary headers.\n");
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "PJSIP RFC3329 Support (partial)",
