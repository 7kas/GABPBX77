...
#include "gabpbx/linkedlists.h"       /* for AST_LIST_NEXT */
...
#include "gabpbx/res_pjsip_session_caps.h"
...
static int rtp_check_timeout(const void *data)
{
	 ...
	 * outside GABpbx return for later check.
	 */
	...
	ast_log_chan(NULL, LOG_NOTICE, "Disconnecting channel '%s' for lack of %s RTP activity in %d seconds\n",
	...
	const pjmedia_sdp_session *sdp)
{
	...
	if (!(session_media->rtp = ast_rtp_instance_new(session->endpoint->media.rtp.engine, sched, media_address, NULL))) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to create RTP instance using RTP engine '%s'\n", session->endpoint->media.rtp.engine);
		return -1;
	}
...
static int apply_cap_to_bundled(struct ast_sip_session_media *session_media,
	...
	struct ast_stream *gabpbx_stream, struct ast_format_cap *joint)
{
	if (!joint) {
		return -1;
	}

	ast_stream_set_formats(gabpbx_stream, joint);
	...
	const struct pjmedia_sdp_media *stream)
{
	...
	if (!remote) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to allocate %s incoming remote capabilities\n",
...
static int set_caps(struct ast_sip_session *session,
	...
	struct ast_sip_session_media *session_media_transport,
	const struct pjmedia_sdp_media *stream,
	int is_offer, struct ast_stream *gabpbx_stream)
{
	...
	if (!(caps = ast_format_cap_alloc(AST_FORMAT_CAP_FLAG_DEFAULT)) ||
		...
		ast_log_chan(NULL, LOG_ERROR, "Failed to allocate %s capabilities\n",
			ast_codec_media_type2str(session_media->type));
	...
	if (!ast_format_cap_count(joint)) {
		...
		ast_rtp_codecs_payloads_destroy(&codecs);
		ast_log_chan(NULL, LOG_NOTICE, "No joint capabilities for '%s' media stream between our configuration(%s) and incoming SDP(%s)\n",
			ast_codec_media_type2str(session_media->type),
	...
	apply_cap_to_bundled(session_media, session_media_transport, gabpbx_stream, joint);

	if (session->channel && ast_sip_session_is_pending_stream_default(session, gabpbx_stream)) {
...
static pjmedia_sdp_attr* generate_rtpmap_attr(struct ast_sip_session *session, pjmedia_sdp_media *media, pj_pool_t *pool,
					      int rtp_code, int gabpbx_format, struct ast_format *format, int code)
{
	...
	rtpmap.clock_rate = ast_rtp_lookup_sample_rate2(gabpbx_format, format, code);
	pj_strdup2(pool, &rtpmap.enc_name, ast_rtp_lookup_mime_subtype2(gabpbx_format, format, code, options));
...
static pjmedia_sdp_attr* generate_rtpmap_attr2(struct ast_sip_session *session, pjmedia_sdp_media *media, pj_pool_t *pool,
					      int rtp_code, int gabpbx_format, struct ast_format *format, int code, int sample_rate)
{
#ifndef HAVE_PJSIP_ENDPOINT_COMPACT_FORM
	extern pj_bool_t pjsip_use_compact_form;
#else
	...
	struct ast_sip_session_media *session_media)
{
	...
	if (dtls->set_configuration(session_media->rtp, &session->endpoint->media.rtp.dtls_cfg)) {
		ast_log_chan(NULL, LOG_ERROR, "Attempted to set an invalid DTLS-SRTP configuration on RTP instance '%p'\n",
			session_media->rtp);
		return -1;
	}
	...
	pjmedia_sdp_attr *attr)
{
	...
	if (!pj_strcmp2(&attr->name, "setup")) {
		if (!pj_stricmp2(value, "active")) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Unsupported setup attribute value '%*s'\n", (int)value->slen, value->ptr);
		}
		...
		if (!pj_stricmp2(value, "new")) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Unsupported connection attribute value '%*s'\n", (int)value->slen, value->ptr);
		}
		...
		ast_copy_pj_str(fingerprint_text, value, sizeof(fingerprint_text));
			...
			if (!strcasecmp(hash, "sha-1")) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Unsupported fingerprint hash type '%s'\n",
				hash);
			}
		}
	}
...
static int negotiate_incoming_sdp_stream(struct ast_sip_session *session,
	...
	int index, struct ast_stream *gabpbx_stream)
{
	...
	set_session_media_remotely_held(session_media, session, stream, gabpbx_stream, addrs);

	joint = set_incoming_call_offer_cap(session, session_media, stream);
	res = apply_cap_to_bundled(session_media, session_media_transport, gabpbx_stream, joint);
	ao2_cleanup(joint);
	if (res != 0) {
				      ...
				      struct pjmedia_sdp_session *sdp, const struct pjmedia_sdp_session *remote, struct ast_stream *stream)
{
	...
	if (session_media_transport == session_media || !session_media->bundled) {
		...
		if (ast_strlen_zero(hostip)) {
			ast_log_chan(NULL, LOG_ERROR, "No local host IP available for stream %s\n",
				ast_codec_media_type2str(session_media->type));
	...
	if (AST_VECTOR_INIT(&sample_rates, 1)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to add dtmf formats to SDP!\n");
	...
	for (index = 0; index < ast_format_cap_count(caps); ++index) {
		...
		if (!ast_rtp_lookup_sample_rate2(1, format, 0)) {
			ast_log_chan(NULL, LOG_WARNING, "Format '%s' can not be added to SDP, consider disallowing it on endpoint '%s'\n",
		...
		if (session_media_transport != session_media) {
			if ((rtp_code = ast_rtp_codecs_payload_code(ast_rtp_instance_get_codecs(session_media_transport->rtp), 1, format, 0)) == -1) {
				ast_log_chan(NULL, LOG_WARNING,"Unable to get rtp codec payload code for %s\n", ast_format_get_name(format));
				ao2_ref(format, -1);
				continue;
			}
...
static int apply_negotiated_sdp_stream(struct ast_sip_session *session,
	...
	const struct pjmedia_sdp_session *remote, int index, struct ast_stream *gabpbx_stream)
{
	RAII_VAR(struct ast_sockaddr *, addrs, NULL, ast_free);
	...
	SCOPE_ENTER(1, "%s Stream: %s\n", ast_sip_session_get_name(session),
		ast_str_tmp(128, ast_stream_to_str(gabpbx_stream, &STR_TMP)));
	...
	if (set_caps(session, session_media, session_media_transport, remote_stream, 0, gabpbx_stream)) {
	...
	ast_rtp_instance_set_stream_num(session_media->rtp, ast_stream_get_position(gabpbx_stream));
	...
	set_session_media_remotely_held(session_media, session, remote_stream, gabpbx_stream, addrs);
...
/*! \brief Unloads the sdp RTP/AVP module from GABpbx */
...
static int load_module(void)
{
	...
	if (ast_sip_session_register_sdp_handler(&audio_sdp_handler, STR_AUDIO)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to register SDP handler for %s stream type\n", STR_AUDIO);
		goto end;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "PJSIP SDP RTP/AVP stream handler",
