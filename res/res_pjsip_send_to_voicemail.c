...
static int handle_incoming_request(struct ast_sip_session *session, struct pjsip_rx_data *rdata)
{
	...
	if (!other_party) {
		...
		ast_log_chan(NULL, LOG_WARNING, "%s (%s) attempted to transfer to voicemail, "
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "PJSIP REFER Send to Voicemail Support",
