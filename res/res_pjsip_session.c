	...
	if (handler_list) {
		...
		AST_LIST_TRAVERSE(&handler_list->list, iter, next) {
			if (!strcmp(iter->id, handler->id)) {
				ast_log_chan(NULL, LOG_WARNING, "Handler '%s' already registered for stream type '%s'.\n", handler->id, stream_type);
				return -1;
			}
		}
...
int ast_sip_session_is_pending_stream_default(const struct ast_sip_session *session, const struct ast_stream *stream)
{
	...
	if (!session->pending_media_state->topology) {
		ast_log_chan(NULL, LOG_WARNING, "Pending topology was NULL for channel '%s'\n",
...
static void set_remote_mslabel_and_stream_group(struct ast_sip_session *session,
	...
	const pjmedia_sdp_session *sdp,
	const struct pjmedia_sdp_media *stream,
	struct ast_stream *gabpbx_stream)
{
	int index;
	...
	for (index = 0; index < AST_VECTOR_SIZE(&session->pending_media_state->sessions); ++index) {
		...
		ast_stream_set_group(gabpbx_stream, index);
		break;
	}
}
...
static int handle_negotiated_sdp_session_media(struct ast_sip_session_media *session_media,
		...
		const pjmedia_sdp_session *remote, int index, struct ast_stream *gabpbx_stream)
{
	/* See if there are registered handlers for this media stream type */
	 ...
	 */
	if (!ast_sip_session_is_pending_stream_default(session, gabpbx_stream) || strcmp(media, "audio")) {
		/* Determine the state of the stream based on our local SDP */
		if (pjmedia_sdp_media_find_attr2(local_stream, "sendonly", NULL)) {
			ast_stream_set_state(gabpbx_stream, AST_STREAM_STATE_SENDONLY);
		} else if (pjmedia_sdp_media_find_attr2(local_stream, "recvonly", NULL)) {
			ast_stream_set_state(gabpbx_stream, AST_STREAM_STATE_RECVONLY);
		} else if (pjmedia_sdp_media_find_attr2(local_stream, "inactive", NULL)) {
			ast_stream_set_state(gabpbx_stream, AST_STREAM_STATE_INACTIVE);
		} else {
			ast_stream_set_state(gabpbx_stream, AST_STREAM_STATE_SENDRECV);
		}
	} else {
		ast_stream_set_state(gabpbx_stream, AST_STREAM_STATE_SENDRECV);
	}
	...
	set_remote_mslabel_and_stream_group(session, session_media, remote, remote->media[index], gabpbx_stream);
	...
	if (handler) {
		ast_debug(4, "%s: Applying negotiated SDP media stream '%s' using %s SDP handler\n",
			...
			handler->id);
		res = handler->apply_negotiated_sdp_stream(session, session_media, local, remote, index, gabpbx_stream);
		if (res >= 0) {
...
static int handle_negotiated_sdp(struct ast_sip_session *session, const pjmedia_sdp_session *local, const pjmedia_sdp_session *remote)
{
	...
	if (!session->pending_media_state->topology) {
		if (session->active_media_state->topology) {
			...
			if (!active_media_state_clone) {
				ast_log_chan(NULL, LOG_WARNING, "%s: Unable to clone active media state\n",
					ast_sip_session_get_name(session));
				return -1;
			}
			...
			ast_log_chan(NULL, LOG_WARNING, "%s: No pending or active media state\n",
				ast_sip_session_get_name(session));
			return -1;
		}
	}
	 ...
	 * up session media instances (and GABpbx streams) that correspond to
	...
	if (ast_stream_topology_get_count(session->pending_media_state->topology) != local->media_count
		...
		ast_log_chan(NULL, LOG_WARNING, "%s: Local SDP contains %d media streams while we expected it to contain %u\n",
			ast_sip_session_get_name(session),
		...
		ast_sip_session_sdp_creation_cb on_sdp_creation)
{
	...
	if (!inv_session->neg || (pjmedia_sdp_neg_get_state(inv_session->neg) != PJMEDIA_SDP_NEG_STATE_REMOTE_OFFER &&
		...
		ast_log_chan(NULL, LOG_WARNING, "Requested to regenerate local SDP answer for channel '%s' but negotiation in state '%s'\n",
	...
	if (!new_answer) {
		ast_log_chan(NULL, LOG_WARNING, "Could not create a new local SDP answer for channel '%s'\n",
	...
	struct ast_sip_contact *contact, pjsip_inv_session *inv_session, pjsip_rx_data *rdata)
{
	...
	if (pjsip_inv_add_ref(inv_session) != PJ_SUCCESS) {
		ast_log_chan(NULL, LOG_ERROR, "Can't increase the session reference counter\n");
		return NULL;
	}
#endif
	...
	struct ast_stream_topology *req_topology)
{
	...
	if (ast_strlen_zero(uri)) {
		ast_log_chan(NULL, LOG_ERROR, "Endpoint '%s': No URI available.  Is endpoint registered?\n",
			ast_sorcery_object_get_id(endpoint));
...
static enum sip_get_destination_result get_destination(struct ast_sip_session *session, pjsip_rx_data *rdata)
{
	...
	if (!pickup_cfg) {
		ast_log_chan(NULL, LOG_ERROR, "%s: Unable to retrieve pickup configuration options. Unable to detect call pickup extension\n",
			ast_sip_session_get_name(session));
		pickupexten = "";
	} else {
...
static pj_bool_t session_on_rx_request(pjsip_rx_data *rdata)
{
	...
	case PJSIP_INVITE_METHOD:
		if (dlg) {
			ast_log_chan(NULL, LOG_WARNING, "on_rx_request called for INVITE in mid-dialog?\n");
			break;
		}
		handled = PJ_TRUE;
...
static void __print_debug_details(const char *function, pjsip_inv_session *inv, pjsip_transaction *tsx, pjsip_event *e)
{
	...
	ast_log_chan(NULL, LOG_DEBUG, "Function %s called on event %s\n",
		function, pjsip_event_str(e->type));
	if (!inv) {
		ast_log_chan(NULL, LOG_DEBUG, "Transaction %p does not belong to an inv_session?\n", tsx);
		ast_log_chan(NULL, LOG_DEBUG, "The transaction state is %s\n",
	...
	if (!session) {
		ast_log_chan(NULL, LOG_DEBUG, "inv_session %p has no ast session\n", inv);
	} else {
		ast_log_chan(NULL, LOG_DEBUG, "The state change pertains to the endpoint '%s(%s)'\n",
			ast_sorcery_object_get_id(session->endpoint),
			session->channel ? ast_channel_name(session->channel) : "");
	}
	if (inv->invite_tsx) {
		ast_log_chan(NULL, LOG_DEBUG, "The inv session still has an invite_tsx (%p)\n",
			inv->invite_tsx);
	} else {
		ast_log_chan(NULL, LOG_DEBUG, "The inv session does NOT have an invite_tsx\n");
	}
	if (tsx) {
		ast_log_chan(NULL, LOG_DEBUG, "The %s %.*s transaction involved in this state change is %p\n",
			pjsip_role_name(tsx->role),
			(int) pj_strlen(&tsx->method.name), pj_strbuf(&tsx->method.name),
			tsx);
		ast_log_chan(NULL, LOG_DEBUG, "The current transaction state is %s\n",
			pjsip_tsx_state_str(tsx->state));
		ast_log_chan(NULL, LOG_DEBUG, "The transaction state change event is %s\n",
			pjsip_event_str(e->body.tsx_state.type));
	} else {
		ast_log_chan(NULL, LOG_DEBUG, "There is no transaction involved in this state change\n");
	}
	ast_log_chan(NULL, LOG_DEBUG, "The current inv state is %s\n", pjsip_inv_state_name(inv->state));
}
...
static void session_inv_on_create_offer(pjsip_inv_session *inv, pjmedia_sdp_session **p_offer)
{
	...
	/* Some devices send a re-INVITE offer with empty SDP. GABpbx by default return
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER, "PJSIP Session resource",
