	...
	const struct ast_format_cap *joint)
{
	...
	ast_log_chan(NULL, level, file, line, function, "'%s' Caps for %s %s call with pref '%s' - remote: %s local: %s joint: %s\n",
	...
	struct ast_flags codec_pref)
{
	...
	if (!joint || !local_filtered || !remote_filtered) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to allocate %s call offer capabilities\n",
				ast_codec_media_type2str(media_type));
		ao2_cleanup(joint);
