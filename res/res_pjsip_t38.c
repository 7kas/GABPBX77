	...
	struct t38_state *state, enum ast_sip_session_t38state new_state)
{
	...
	case T38_PEER_REINVITE:
		...
		if (pjsip_endpt_schedule_timer(ast_sip_get_pjsip_endpoint(), &state->timer, &delay) != PJ_SUCCESS) {
			ast_log_chan(NULL, LOG_WARNING, "Scheduling of automatic T.38 rejection for channel '%s' failed\n",
...
static int t38_reinvite_response_cb(struct ast_sip_session *session, pjsip_rx_data *rdata)
{
	...
	if (!session->channel || !state) {
		ast_log_chan(NULL, LOG_WARNING, "Received %d response to T.38 re-invite on '%s' but state unavailable\n",
			status.code,
			session->channel ? ast_channel_name(session->channel) : "unknown channel");
		return 0;
	}
	...
	if (status.code / 100 == 2) {
		...
		if (!session_media) {
			ast_log_chan(NULL, LOG_WARNING, "Received %d response to T.38 re-invite on '%s' but no active session media\n",
...
static int t38_interpret_parameters(void *obj)
{
	...
	case AST_T38_REQUEST_NEGOTIATE:         /* Request T38 */
		...
		if (!parameters->max_ifp) {
			...
			if (!session_media) {
				ast_log_chan(NULL, LOG_ERROR, "Failed to negotiate parameters for reinvite on channel '%s' (No pending session media).\n",
			...
			if (!session_media) {
				ast_log_chan(NULL, LOG_ERROR, "Failed to negotiate parameters on channel '%s' (No default session media).\n",
					data->session->channel ? ast_channel_name(data->session->channel) : "unknown channel");
				break;
			}
			ast_udptl_set_local_max_ifp(session_media->udptl, state->our_parms.max_ifp);
	...
	case AST_T38_REQUEST_PARMS: {		/* Application wants remote's parameters re-sent */
		...
		if (data->session->t38state == T38_PEER_REINVITE) {
			session_media = data->session->pending_media_state->default_session[AST_MEDIA_TYPE_IMAGE];
			if (!session_media) {
				ast_log_chan(NULL, LOG_ERROR, "Failed to request parameters for reinvite on channel '%s' (No pending session media).\n",
					data->session->channel ? ast_channel_name(data->session->channel) : "unknown channel");
				break;
			}
			parameters.max_ifp = ast_udptl_get_far_max_ifp(session_media->udptl);
...
static void t38_attach_framehook(struct ast_sip_session *session)
{
	...
	if (framehook_id < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Could not attach T.38 Frame hook, T.38 will be unavailable on '%s'\n",
			ast_channel_name(session->channel));
		ast_channel_unlock(session->channel);
		return;
	}
	...
	if (!datastore) {
		ast_log_chan(NULL, LOG_ERROR, "Could not alloc T.38 Frame hook datastore, T.38 will be unavailable on '%s'\n",
			ast_channel_name(session->channel));
...
/*! \brief Unloads the SIP T.38 module from GABpbx */
...
static int load_module(void)
{
	...
	if (ast_sip_session_register_sdp_handler(&image_sdp_handler, "image")) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to register SDP handler for image stream type\n");
		goto end;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "PJSIP T.38 UDPTL Support",
