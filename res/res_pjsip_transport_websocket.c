...
static int transport_create(void *data)
{
	...
	if (!newtransport) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to allocate WebSocket transport.\n");
		goto on_error;
	}
	...
	if (!(pool = pjsip_endpt_create_pool(endpt, "ws", 512, 512))) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to allocate WebSocket endpoint pool.\n");
		goto on_error;
	}
	 ...
	 * pjproject or GABpbx.  Other places are reconstituting subscriptions
	...
	if (!newtransport->rdata.tp_info.pool) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to allocate WebSocket rdata.\n");
...
static void websocket_cb(struct ast_websocket *session, struct ast_variable *parameters, struct ast_variable *headers)
{
	...
	if (ast_sip_push_task_wait_serializer(serializer, transport_create, &create_data)) {
		ast_log_chan(NULL, LOG_ERROR, "Could not create WebSocket transport.\n");
...
static pj_bool_t websocket_on_rx_msg(pjsip_rx_data *rdata)
{
	...
	if (contact
		...
		if (DEBUG_ATLEAST(4)) {
			...
			ast_log_chan(NULL, LOG_DEBUG, "%s re-writing Contact URI from %s%.*s%s:%d%s%.*s to %s;transport=%s\n",
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "PJSIP WebSocket Transport Support",
