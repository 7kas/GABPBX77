		...
		<configFile name="prometheus.conf">
			<configObject name="general">
				...
				<description>
					...
					to configure GABpbx to serve up statistics for a Prometheus server.
					</para>
					<note>
						<para>You must enable GABpbx's HTTP server in <filename>http.conf</filename>
				...
				<configOption name="core_metrics_enabled" default="yes">
					...
					<description>
						<para>
						Core metrics show various properties of the GABpbx system, including
						...
						what version of GABpbx they're running.
						</para>
						<enumlist>
							<enum name="no" />
							<enum name="yes" />
				...
				<configOption name="auth_username">
					...
					<description>
						...
						<warning>
							...
							GABpbx system being made easily scrapable by the
				...
				<configOption name="auth_realm" default="GABpbx Prometheus Metrics">
					<since>
						<version>17.0.0</version>
					</since>
...
#include "gabpbx/res_prometheus.h"
...
#define CORE_PROPERTIES_HELP "GABpbx instance properties. The value of this will always be 1."

#define CORE_UPTIME_HELP "GABpbx instance uptime in seconds."

#define CORE_LAST_RELOAD_HELP "Time since last GABpbx reload in seconds."
...
static struct prometheus_metric core_metrics[] = {
	PROMETHEUS_METRIC_STATIC_INITIALIZATION(
		PROMETHEUS_METRIC_COUNTER,
		"gabpbx_core_properties",
		CORE_PROPERTIES_HELP,
		NULL),
	PROMETHEUS_METRIC_STATIC_INITIALIZATION(
		PROMETHEUS_METRIC_COUNTER,
		"gabpbx_core_uptime_seconds",
		CORE_UPTIME_HELP,
		get_core_uptime_cb),
	PROMETHEUS_METRIC_STATIC_INITIALIZATION(
		PROMETHEUS_METRIC_COUNTER,
		"gabpbx_core_last_reload_seconds",
...
int prometheus_metric_register(struct prometheus_metric *metric)
{
	...
	for (i = 0; i < AST_VECTOR_SIZE(&metrics); i++) {
		...
		if (prometheus_metric_cmp(existing, metric)) {
			ast_log_chan(NULL, AST_LOG_NOTICE,
	...
	if (AST_VECTOR_APPEND(&metrics, metric)) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Failed to grow vector to make room for Prometheus metric: %s\n",
			metric->name);
		return -1;
	}
...
static int prometheus_config_pre_apply(void)
{
	...
	if (!ast_strlen_zero(config->general->auth_username)
		...
		ast_log_chan(NULL, AST_LOG_ERROR, "'auth_username' set without a corresponding 'auth_password'\n");
		return -1;
	}
...
static int reload_module(void) {
	...
	for (i = 0; i < AST_VECTOR_SIZE(&providers); i++) {
		...
		if (provider->reload_cb(general_config)) {
			ast_log_chan(NULL, AST_LOG_WARNING, "Failed to reload metrics provider %s\n", provider->name);
	...
	if (ast_http_uri_link(&prometheus_uri)) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Failed to re-register Prometheus Metrics URI during reload\n");
		return -1;
	}
...
static int load_module(void)
{
	...
	aco_option_register(&cfg_info, "auth_realm", ACO_EXACT, global_options, "GABpbx Prometheus Metrics", OPT_STRINGFIELD_T, 0, STRFLDSET(struct prometheus_general_config, auth_realm));
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER, "GABpbx Prometheus Module",
	...
	/* This module explicitly calls into res_pjsip if GABpbx is built with PJSIP support, so they are required. */
	.requires = "res_pjsip",
