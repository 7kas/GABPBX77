...
static int unbound_resolver_resolve(struct ast_dns_query *query)
{
	...
	if (!data) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to allocate resolver data for resolution of '%s'\n",
			ast_dns_query_get_name(query));
		return -1;
	}
	...
	if (res) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to perform async DNS resolution of '%s'\n",
			ast_dns_query_get_name(query));
		ao2_ref(query, -1);
	}
...
static int unbound_config_preapply(struct unbound_config *cfg)
{
	...
	cfg->global->state = ao2_alloc_options(sizeof(*cfg->global->state), unbound_config_state_destructor,
		AO2_ALLOC_OPT_LOCK_NOLOCK);
	if (!cfg->global->state) {
		ast_log_chan(NULL, LOG_ERROR, "Could not allocate unbound resolver state structure\n");
		return -1;
	}

	cfg->global->state->resolver = unbound_resolver_alloc();
	if (!cfg->global->state->resolver) {
		ast_log_chan(NULL, LOG_ERROR, "Could not create an unbound resolver\n");
		return -1;
	}
	...
	if (res) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to set hosts file to '%s' in unbound resolver: %s\n",
	...
	if (cfg->global->nameservers) {
		...
		while (!res && (nameserver = ao2_iterator_next(&it_nameservers))) {
			...
			if (res) {
				ast_log_chan(NULL, LOG_ERROR, "Failed to add nameserver '%s' to unbound resolver: %s\n",
	...
	if (res) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to set resolv.conf file to '%s' in unbound resolver: %s\n",
	...
	if (!ast_strlen_zero(cfg->global->ta_file)) {
		...
		if (res) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to set trusted anchor file to '%s' in unbound resolver: %s\n",
	...
	if (unbound_resolver_start(cfg->global->state->resolver)) {
		ast_log_chan(NULL, LOG_ERROR, "Could not start unbound resolver thread\n");
		return -1;
	}
...
static int unbound_config_apply_default(void)
{
	...
	if (!cfg) {
		ast_log_chan(NULL, LOG_ERROR, "Could not create default configuration for unbound resolver\n");
		return -1;
	}
	...
	ast_verb_chan(NULL, 1, "Starting unbound resolver using default configuration\n");
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "Unbound DNS Resolver Support",
