...
#include "gabpbx/dns_recurring.h"
...
#include "gabpbx/conversions.h"
#include "gabpbx/options.h"
#include "gabpbx/logger_category.h"
...
static void pj_thread_register_check(void)
{
	...
	if (!desc) {
		ast_log_chan(NULL, LOG_ERROR, "Could not get thread desc from thread-local storage. Expect awful things to occur\n");
		return;
	}
	pj_bzero(*desc, sizeof(*desc));

	if (pj_thread_register("GABpbx Thread", *desc, &thread) != PJ_SUCCESS) {
		ast_log_chan(NULL, LOG_ERROR, "Coudln't register thread with PJLIB.\n");
	}
	return;
}
...
static void ast_rtp_ice_start(struct ast_rtp_instance *instance)
{
	...
	if (ice_reset_session(instance)) {
		ast_log_chan(NULL, LOG_NOTICE, "(%p) ICE failed to create replacement session\n", instance);
		return;
	}
	...
	if (cand_cnt < ao2_container_count(rtp->ice_active_remote_candidates)) {
		ast_log_chan(NULL, LOG_WARNING, "(%p) ICE lost %d candidates. Consider increasing PJ_ICE_MAX_CAND in PJSIP\n",
			instance, ao2_container_count(rtp->ice_active_remote_candidates) - cand_cnt);
	}

	if (!has_rtp) {
		ast_log_chan(NULL, LOG_WARNING, "(%p) ICE no RTP candidates; skipping checklist\n", instance);
	}
	...
	if (!has_rtcp && rtp->ice_num_components > 1) {
		ast_log_chan(NULL, LOG_WARNING, "(%p) ICE no RTCP candidates; skipping checklist\n", instance);
	}
	...
	if (rtp->ice && has_rtp && (has_rtcp || rtp->ice_num_components == 1)) {
		...
		ast_log_chan(NULL, LOG_WARNING, "(%p) ICE failed to create session check list: %s\n", instance, reason);
	}
...
static void ast_rtp_on_turn_rx_rtp_data(pj_turn_sock *turn_sock, void *pkt, unsigned pkt_len, const pj_sockaddr_t *peer_addr, unsigned addr_len)
{
	...
	if (ice) {
		...
		if (status != PJ_SUCCESS) {
			...
			ast_log_chan(NULL, LOG_WARNING, "(%p) ICE PJ Rx error status code: %d '%s'.\n",
...
static void ast_rtp_on_turn_rx_rtcp_data(pj_turn_sock *turn_sock, void *pkt, unsigned pkt_len, const pj_sockaddr_t *peer_addr, unsigned addr_len)
{
	...
	if (ice) {
		...
		if (status != PJ_SUCCESS) {
			...
			pj_strerror(status, buf, sizeof(buf));
			ast_log_chan(NULL, LOG_WARNING, "PJ ICE Rx error status code: %d '%s'.\n",
				(int)status, buf);
			return;
		}
		...
		enum ast_transport transport, const char *server, unsigned int port, const char *username, const char *password)
{
	...
	ao2_cleanup(ice);
	if (status != PJ_SUCCESS) {
		ast_log_chan(NULL, LOG_WARNING, "(%p) Could not create a TURN client socket\n", instance);
		ao2_lock(instance);
		return;
	}
	...
	enum ast_rtp_dtls_setup setup, struct ast_rtp_instance *instance)
{
	...
	if (!(dtls->ssl = SSL_new(ssl_ctx))) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to allocate memory for SSL\n");
		goto error;
	}

	if (!(dtls->read_bio = BIO_new(BIO_s_mem()))) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to allocate memory for inbound SSL traffic\n");
		goto error;
	}
	...
	if (!(dtls->write_bio = BIO_new(dtls_bio_methods))) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to allocate memory for outbound SSL traffic\n");
		goto error;
	}
	...
	if (!ast_strlen_zero(dtls_cfg->pvtfile)) {
		...
		if (bio) {
			if (apply_dh_params(rtp->ssl_ctx, bio)) {
				...
				ast_verb_chan(NULL, 2, "DTLS DH initialized, PFS enabled\n");
			}
			BIO_free(bio);
		}
	}
	...
	if (ecdh) {
		if (SSL_CTX_set_tmp_ecdh(rtp->ssl_ctx, ecdh)) {
			...
			if (SSL_CTX_ctrl(rtp->ssl_ctx, SSL_CTRL_SET_ECDH_AUTO, 1, NULL)) {
				ast_verb_chan(NULL, 2, "DTLS ECDH initialized (automatic), faster PFS enabled\n");
			} else {
				ast_verb_chan(NULL, 2, "DTLS ECDH initialized (secp256r1), faster PFS enabled\n");
			}
		}
		EC_KEY_free(ecdh);
	}
										...
										struct dtls_cert_info *cert_info)
{
	...
	if (create_ephemeral_ec_keypair(&cert_info->private_key)) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to create ephemeral ECDSA keypair\n");
		goto error;
	}

	if (create_ephemeral_certificate(cert_info->private_key, &cert_info->certificate)) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to create ephemeral X509 certificate\n");
		goto error;
	}
...
static int create_certificate_ephemeral(struct ast_rtp_instance *instance,
										...
										struct dtls_cert_info *cert_info)
{
	ast_log_chan(NULL, LOG_ERROR, "Your version of OpenSSL does not support ECDSA keys\n");
	return -1;
}
										...
										struct dtls_cert_info *cert_info)
{
	...
	if (!fp) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to read private key from file '%s': %s\n", private_key_file, strerror(errno));
		goto error;
	}

	if (!PEM_read_PrivateKey(fp, &private_key, NULL, NULL)) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to read private key from PEM file '%s'\n", private_key_file);
		fclose(fp);
		goto error;
	}

	if (fclose(fp)) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to close private key file '%s': %s\n", private_key_file, strerror(errno));
		goto error;
	}

	certbio = BIO_new(BIO_s_file());
	if (!certbio) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to allocate memory for certificate fingerprinting on RTP instance '%p'\n",
				instance);
		goto error;
	}
	...
	if (!BIO_read_filename(certbio, dtls_cfg->certfile)
	   || !(cert = PEM_read_bio_X509(certbio, NULL, 0, NULL))) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to read certificate from file '%s'\n", dtls_cfg->certfile);
		goto error;
	}
...
static int ast_rtp_dtls_set_configuration(struct ast_rtp_instance *instance, const struct ast_rtp_dtls_cfg *dtls_cfg)
{
	...
	if (!ast_rtp_engine_srtp_is_registered()) {
		ast_log_chan(NULL, LOG_ERROR, "SRTP support module is not loaded or available. Try loading res_srtp.so.\n");
		return -1;
	}
	...
	if (dtls_cfg->suite == AST_AES_CM_128_HMAC_SHA1_80) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Unsupported suite specified for DTLS-SRTP on RTP instance '%p'\n", instance);
		return -1;
	}
	...
	if (!load_dtls_certificate(instance, dtls_cfg, &cert_info)) {
		...
		if (!SSL_CTX_use_certificate(rtp->ssl_ctx, cert_info.certificate)) {
			ast_log_chan(NULL, LOG_ERROR, "Specified certificate for RTP instance '%p' could not be used\n",
					instance);
			return -1;
		}
		...
		if (!SSL_CTX_use_PrivateKey(rtp->ssl_ctx, cert_info.private_key)
			...
			ast_log_chan(NULL, LOG_ERROR, "Specified private key for RTP instance '%p' could not be used\n",
					instance);
			return -1;
		}
		...
		if (rtp->local_hash == AST_RTP_DTLS_HASH_SHA1) {
			...
			ast_log_chan(NULL, LOG_ERROR, "Unsupported fingerprint hash type on RTP instance '%p'\n",
				instance);
			return -1;
		}

		if (!X509_digest(cert_info.certificate, type, fingerprint, &size) || !size) {
			ast_log_chan(NULL, LOG_ERROR, "Could not produce fingerprint from certificate for RTP instance '%p'\n",
					instance);
			return -1;
		}
	...
	if (!ast_strlen_zero(dtls_cfg->cipher)) {
		if (!SSL_CTX_set_cipher_list(rtp->ssl_ctx, dtls_cfg->cipher)) {
			ast_log_chan(NULL, LOG_ERROR, "Invalid cipher specified in cipher list '%s' for RTP instance '%p'\n",
	...
	if (!ast_strlen_zero(dtls_cfg->cafile) || !ast_strlen_zero(dtls_cfg->capath)) {
		if (!SSL_CTX_load_verify_locations(rtp->ssl_ctx, S_OR(dtls_cfg->cafile, NULL), S_OR(dtls_cfg->capath, NULL))) {
			ast_log_chan(NULL, LOG_ERROR, "Invalid certificate authority file '%s' or path '%s' specified for RTP instance '%p'\n",
...
static void ast_rtp_ice_start_media(pj_ice_sess *ice, pj_status_t status)
{
	 ...
	 * happens then GABpbx won't complete DTLS initialization. RTP packets are still
	...
	ast_verb_chan(NULL, 4, "%p -- Strict RTP learning after ICE completion\n", rtp);
...
static void dtls_srtp_start_timeout_timer(struct ast_rtp_instance *instance, struct ast_rtp *rtp, int rtcp)
{
	...
	if ((dtls->timeout_timer = ast_sched_add_variable(rtp->sched, timeout, cb, instance, 1)) < 0) {
		ao2_ref(instance, -1);
		ast_log_chan(NULL, LOG_WARNING, "Scheduling '%s' DTLS retransmission for RTP instance [%p] failed.\n",
...
static int dtls_srtp_add_local_ssrc(struct ast_rtp *rtp, struct ast_rtp_instance *instance, int rtcp, unsigned int ssrc, int set_remote_policy)
{
	...
	if (!SSL_export_keying_material(dtls->ssl, material, SRTP_MASTER_LEN * 2, "EXTRACTOR-dtls_srtp", 19, NULL, 0, 0)) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to extract SRTP keying material from DTLS-SRTP negotiation on RTP instance '%p'\n",
			instance);
		return -1;
	}
	...
	if (res_srtp_policy->set_master_key(local_policy, local_key, SRTP_MASTER_KEY_LEN, local_salt, SRTP_MASTER_SALT_LEN) < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Could not set key/salt information on local policy of '%p' when setting up DTLS-SRTP\n", rtp);
		goto error;
	}

	if (res_srtp_policy->set_suite(local_policy, rtp->suite)) {
		ast_log_chan(NULL, LOG_WARNING, "Could not set suite to '%u' on local policy of '%p' when setting up DTLS-SRTP\n", rtp->suite, rtp);
		goto error;
	}
	...
	if (set_remote_policy) {
		...
		if (res_srtp_policy->set_master_key(remote_policy, remote_key, SRTP_MASTER_KEY_LEN, remote_salt, SRTP_MASTER_SALT_LEN) < 0) {
			ast_log_chan(NULL, LOG_WARNING, "Could not set key/salt information on remote policy of '%p' when setting up DTLS-SRTP\n", rtp);
			goto error;
		}

		if (res_srtp_policy->set_suite(remote_policy, rtp->suite)) {
			ast_log_chan(NULL, LOG_WARNING, "Could not set suite to '%u' on remote policy of '%p' when setting up DTLS-SRTP\n", rtp->suite, rtp);
			goto error;
		}
	...
	if (ast_rtp_instance_add_srtp_policy(instance, remote_policy, local_policy, rtcp)) {
		ast_log_chan(NULL, LOG_WARNING, "Could not set policies when setting up DTLS-SRTP on '%p'\n", rtp);
		goto error;
	}
...
static int dtls_srtp_setup(struct ast_rtp *rtp, struct ast_rtp_instance *instance, int rtcp)
{
	...
	if (rtp->dtls_verify & AST_RTP_DTLS_VERIFY_FINGERPRINT) {
		...
		if (!(certificate = SSL_get_peer_certificate(dtls->ssl))) {
			ast_log_chan(NULL, LOG_WARNING, "No certificate was provided by the peer on RTP instance '%p'\n", instance);
			return -1;
		}
		...
		if (rtp->remote_fingerprint[0]) {
			...
			if (rtp->remote_hash == AST_RTP_DTLS_HASH_SHA1) {
				...
				ast_log_chan(NULL, LOG_WARNING, "Unsupported fingerprint hash type on RTP instance '%p'\n", instance);
				return -1;
			}
			...
			if (!X509_digest(certificate, type, fingerprint, &size) ||
				...
				ast_log_chan(NULL, LOG_WARNING, "Fingerprint provided by remote party does not match that of peer certificate on RTP instance '%p'\n",
					instance);
				return -1;
			}
		}
	...
	if (dtls_srtp_add_local_ssrc(rtp, instance, rtcp, ast_rtp_instance_get_ssrc(instance), 1)) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to add local source '%p'\n", rtp);
		return -1;
	}
...
static int __rtp_recvfrom(struct ast_rtp_instance *instance, void *buf, size_t size, int flags, struct ast_sockaddr *sa, int rtcp)
{
	...
	if ((*in >= 20) && (*in <= 63)) {
		...
		if (!dtls->ssl) {
			ast_log_chan(NULL, LOG_ERROR, "Received SSL traffic on RTP instance '%p' without an SSL session\n",
				instance);
			return -1;
		}
		...
		if (rtp->ice) {
			 ...
			 */
			if (!rtp->ice_media_started) {
				ast_log_chan(NULL, LOG_WARNING, "%s: DTLS packet from %s dropped. ICE not completed yet.\n",
			...
			if (!pass_src_check) {
				ast_log_chan(NULL, LOG_WARNING, "%s: DTLS packet from %s dropped. Source not in ICE active candidate list.\n",
					ast_rtp_instance_get_channel_id(instance),
					ast_sockaddr_stringify(sa));
				return 0;
			}
		...
		if ((len < 0) && (SSL_get_error(dtls->ssl, len) == SSL_ERROR_SSL)) {
			...
			ast_log_chan(NULL, LOG_ERROR, "DTLS failure occurred on RTP instance '%p' due to reason '%s', terminating\n",
...
static int create_new_socket(const char *type, int af)
{
	...
	if (sock < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Unable to allocate %s socket: %s\n", type, strerror(errno));
		return sock;
	}
				      ...
				      int transport)
{
	...
	if (getifaddrs(&ifa) < 0) {
		...
		ast_log_chan(NULL, LOG_ERROR, "(%p) ICE Error obtaining list of local addresses: %s\n",
...
static int rtp_allocate_transport(struct ast_rtp_instance *instance, struct ast_rtp *rtp)
{
	...
	if ((rtp->s =
		...
		ast_log_chan(NULL, LOG_WARNING, "Failed to create a new socket for RTP instance '%p'\n", instance);
		return -1;
	}
	...
	for (i = 0; i <= maxloops; i++) {
		...
		if (x == startplace || (errno != EADDRINUSE && errno != EACCES)) {
			ast_log_chan(NULL, LOG_ERROR, "Oh dear... we couldn't allocate a port for RTP instance '%p'\n", instance);
			close(rtp->s);
			rtp->s = -1;
			return -1;
		}
	...
	if (icesupport) {
		...
		if (ice_create(instance, &rtp->bind_address, x, 0)) {
			ast_log_chan(NULL, LOG_NOTICE, "(%p) ICE failed to create session\n", instance);
		} else {
			rtp->ice_port = x;
...
static int ast_rtp_dtmf_begin(struct ast_rtp_instance *instance, char digit)
{
	...
	if ((digit <= '9') && (digit >= '0')) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Don't know how to represent '%c'\n", digit);
		return -1;
	}
	...
	for (i = 0; i < 2; i++) {
		...
		if (res < 0) {
			ast_log_chan(NULL, LOG_ERROR, "RTP Transmission error to %s: %s\n",
...
static int ast_rtcp_write(const void *data)
{
	...
	if (res < 0) {
		ast_log_chan(NULL, LOG_ERROR, "RTCP %s transmission error to %s, rtcp halted %s\n",
				sr ? "SR" : "RR",
		...
		if (res < 0) {
			...
			if (rtp->rtcp && rtp->rtcp->schedid < 0) {
				...
				if (rtp->rtcp->schedid < 0) {
					ao2_ref(instance, -1);
					ast_log_chan(NULL, LOG_WARNING, "scheduling RTCP transmission failed.\n");
				}
			}
		}
...
static void rtp_write_rtcp_fir(struct ast_rtp_instance *instance, struct ast_rtp *rtp, struct ast_sockaddr *remote_address)
{
	...
	if (res < 0) {
		ast_log_chan(NULL, LOG_ERROR, "RTCP FIR transmission error: %s\n", strerror(errno));
	} else {
...
static void rtp_write_rtcp_psfb(struct ast_rtp_instance *instance, struct ast_rtp *rtp, struct ast_frame *frame, struct ast_sockaddr *remote_address)
{
	...
	if (res < 0) {
		ast_log_chan(NULL, LOG_ERROR, "RTCP PSFB transmission error: %s\n", strerror(errno));
	} else {
		ast_rtcp_calculate_sr_rr_statistics(instance, rtcp_report, rtp->bundled ? *remote_address : rtp->rtcp->them, ice, sr);
	}
...
static int ast_rtp_write(struct ast_rtp_instance *instance, struct ast_frame *frame)
{
	...
	if (frame->frametype != AST_FRAME_VOICE && frame->frametype != AST_FRAME_VIDEO && frame->frametype != AST_FRAME_TEXT) {
		ast_log_chan(NULL, LOG_WARNING, "RTP can only send voice, video, and text\n");
		return -1;
	}
	...
	if (codec < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Don't know how to send format %s packets with RTP\n",
	...
	if (!rtp->smoother && ast_format_can_be_smoothed(format)) {
		...
		if (framing_ms) {
			...
			if (!rtp->smoother) {
				ast_log_chan(NULL, LOG_WARNING, "Unable to create smoother: format %s ms: %u len: %u\n",
...
static struct ast_frame *process_cn_rfc3389(struct ast_rtp_instance *instance, unsigned char *data, int len, unsigned int seqno, unsigned int timestamp, int payloadtype, int mark)
{
	...
	if (!ast_test_flag(rtp, FLAG_3389_WARNING)) {
		...
		ast_log_chan(NULL, LOG_NOTICE, "Comfort noise support incomplete in GABpbx (RFC 3389). Please turn off on client if possible. Client address: %s\n",
			ast_sockaddr_stringify(&remote_address));
...
static struct ast_frame *ast_rtcp_read(struct ast_rtp_instance *instance)
{
	...
	if ((res = rtcp_recvfrom(instance, read_area, read_area_size,
		...
		if (errno != EAGAIN) {
			ast_log_chan(NULL, LOG_WARNING, "RTCP Read error: %s.  Hanging up.\n",
	...
	struct ast_rtp_instance *instance1, unsigned int *rtpheader, int len, int hdrlen)
{
	...
	bridged_payload = ast_rtp_codecs_payload_code_tx_sample_rate(ast_rtp_instance_get_codecs(instance1),
		payload_type->gabpbx_format, payload_type->format, payload_type->rtp_code, payload_type->sample_rate);
...
static int rtp_transport_wide_cc_feedback_produce(const void *data)
{
	...
	res = rtcp_sendto(instance, (unsigned int *)rtcpheader, packet_len, 0, &remote_address, &ice);
	if (res < 0) {
		ast_log_chan(NULL, LOG_ERROR, "RTCP transport-cc feedback error to %s due to %s\n",
	...
	unsigned char *data, int len)
{
	...
	if (transport_rtp->transport_wide_cc.schedid < 0 && transport_rtp->rtcp) {
		...
		if (transport_rtp->transport_wide_cc.schedid < 0) {
			ao2_ref(transport, -1);
			ast_log_chan(NULL, LOG_WARNING, "Scheduling RTCP transport-cc feedback transmission failed on RTP instance '%p'\n",
				transport);
		}
	}
}
	...
	unsigned int bundled)
{
	...
	if (ext) {
		...
		if (profile == 0xbede) {
			...
			if (profile == 0x505a) {
				ast_log_chan(NULL, LOG_DEBUG, "Found Zfone extension in RTP stream - zrtp - not supported.\n");
			} else {
				/* SDP negotiated RTP extensions can not currently be output in logging */
				ast_log_chan(NULL, LOG_DEBUG, "Found unknown RTP Extensions %x\n", profile);
			}
		}
	...
	if (res < hdrlen) {
		ast_log_chan(NULL, LOG_WARNING, "RTP Read too short (%d, expecting %d\n", res, hdrlen);
	...
	if (!payload->gabpbx_format) {
		...
		if (payload->rtp_code == AST_RTP_DTMF) {
			...
			ast_log_chan(NULL, LOG_NOTICE, "Unknown RTP codec %d received from '%s'\n",
				payloadtype,
				ast_sockaddr_stringify(remote_address));
		}
	...
	case AST_MEDIA_TYPE_IMAGE:
		/* Fall through */
	default:
		ast_log_chan(NULL, LOG_WARNING, "Unknown or unsupported media type: %s\n",
...
static struct ast_frame *ast_rtp_read(struct ast_rtp_instance *instance, int rtcp)
{
	...
	if ((res = rtp_recvfrom(instance, read_area, read_area_size, 0,
		...
		ast_assert(errno != EBADF);
		if (errno != EAGAIN) {
			ast_log_chan(NULL, LOG_WARNING, "RTP Read error: %s.  Hanging up.\n",
				(errno) ? strerror(errno) : "Unspecified");
			return NULL;
		}
		return &ast_null_frame;
	...
	if (res < hdrlen) {
		...
		for (i = 0; i < res; ++i) {
			if (read_area[i] != '\0') {
				ast_log_chan(NULL, LOG_WARNING, "RTP Read too short\n");
				return &ast_null_frame;
			}
		}
		return &ast_null_frame;
	...
	case STRICT_RTP_LEARN:
		...
		if (!ast_sockaddr_isnull(&rtp->strict_rtp_address)
			...
			ast_verb_chan(NULL, 4, "%p -- Strict RTP learning complete - Locking on source address %s\n",
			...
			if (!ast_sockaddr_cmp(&target_address, &addr)) {
				...
				ast_verb_chan(NULL, 4, "%p -- Strict RTP switching to RTP target address %s as source\n",
			...
			if (!ast_sockaddr_cmp(&rtp->rtp_source_learn.proposed_address, &addr)) {
				if (rtp->rtp_source_learn.stream_type == AST_MEDIA_TYPE_UNKNOWN) {
					...
					ast_verb_chan(NULL, 4, "%p -- Strict RTP qualifying stream type: %s\n",
						rtp, ast_codec_media_type2str(rtp->rtp_source_learn.stream_type));
				}
				if (!rtp_learning_rtp_seq_update(&rtp->rtp_source_learn, seqno)) {
					/* Accept the new RTP stream */
					ast_verb_chan(NULL, 4, "%p -- Strict RTP switching source address to %s\n",
						rtp, ast_sockaddr_stringify(&addr));
					ast_sockaddr_copy(&rtp->strict_rtp_address, &addr);
					rtp_learning_seq_init(&rtp->rtp_source_learn, seqno);
					break;
...
static void ast_rtp_remote_address_set(struct ast_rtp_instance *instance, struct ast_sockaddr *addr)
{
	...
	if (strictrtp && rtp->strict_rtp_state != STRICT_RTP_OPEN
		 ...
		 */
		ast_verb_chan(NULL, 4, "%p -- Strict RTP learning after remote address set to: %s\n",
			rtp, ast_sockaddr_stringify(addr));
		rtp_learning_start(rtp);
	}
}
...
static int ast_rtp_sendcng(struct ast_rtp_instance *instance, int level)
{
	...
	if (res < 0) {
		ast_log_chan(NULL, LOG_ERROR, "RTP Comfort Noise Transmission error to %s: %s\n", ast_sockaddr_stringify(&remote_address), strerror(errno));
		return res;
	}
...
static void stunaddr_resolve_callback(const struct ast_dns_query *query)
{
	...
	if (!store_stunaddr_resolved(query)) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to resolve stunaddr '%s'. Cancelling recurring resolution.\n", stunaddr_name);
		return;
	}
	...
	if (!lowest_ttl) {
		ast_log_chan(NULL, LOG_WARNING, "Resolution for stunaddr '%s' returned TTL = 0. Recurring resolution was cancelled.\n", ast_dns_query_get_name(query));
	}
}
...
static void clean_stunaddr(void) {
	if (stunaddr_resolver) {
		if (ast_dns_resolve_recurring_cancel(stunaddr_resolver)) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to cancel recurring DNS resolution of previous stunaddr.\n");
		}
...
static int rtp_reload(int reload, int by_external_config)
{
	...
	if ((s = ast_variable_retrieve(cfg, "general", "dtmftimeout"))) {
		...
		if ((dtmftimeout < 0) || (dtmftimeout > 64000)) {
			ast_log_chan(NULL, LOG_WARNING, "DTMF timeout of '%d' outside range, using default of '%d' instead\n",
				dtmftimeout, DEFAULT_DTMF_TIMEOUT);
			dtmftimeout = DEFAULT_DTMF_TIMEOUT;
		};
	}
	...
	if ((s = ast_variable_retrieve(cfg, "general", "probation"))) {
		if ((sscanf(s, "%d", &learning_min_sequential) != 1) || learning_min_sequential <= 1) {
			ast_log_chan(NULL, LOG_WARNING, "Value for 'probation' could not be read, using default of '%d' instead\n",
	...
	if ((s = ast_variable_retrieve(cfg, "general", "stunaddr"))) {
		...
		if (!ast_parse_arg(hostport, PARSE_ADDR, &stunaddr_parsed)) {
			...
			if (!stunaddr_resolver) {
				ast_log_chan(NULL, LOG_ERROR, "Failed to setup recurring DNS resolution of stunaddr '%s'",
					host);
			}
		} else {
			ast_log_chan(NULL, LOG_ERROR, "Failed to parse stunaddr '%s'", hostport);
		}
	}
	if ((s = ast_variable_retrieve(cfg, "general", "turnaddr"))) {
		...
		if (ast_parse_arg(s, PARSE_INADDR, &addr)) {
			ast_log_chan(NULL, LOG_WARNING, "Invalid TURN server address: %s\n", s);
		} else {
	...
	for (var = ast_variable_browse(cfg, "ice_host_candidates"); var; var = var->next) {
		...
		if (ast_parse_arg(var->name, PARSE_ADDR | PARSE_PORT_IGNORE, &local_addr)) {
			ast_log_chan(NULL, LOG_WARNING, "Invalid local ICE host address: %s\n", var->name);
			continue;
		}
		...
		if (ast_parse_arg(var->value, PARSE_ADDR | PARSE_PORT_IGNORE, &advertised_addr)) {
			ast_log_chan(NULL, LOG_WARNING, "Invalid advertised ICE host address: %s\n", var->value);
			continue;
		}

		if (!(candidate = ast_calloc(1, sizeof(*candidate)))) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to allocate ICE host candidate mapping.\n");
			break;
		}
	...
	if ((s = ast_variable_retrieve(cfg, "general", "dtls_mtu"))) {
		if ((sscanf(s, "%d", &dtls_mtu) != 1) || dtls_mtu < 256) {
			ast_log_chan(NULL, LOG_WARNING, "Value for 'dtls_mtu' could not be read, using default of '%d' instead\n",
				DEFAULT_DTLS_MTU);
			dtls_mtu = DEFAULT_DTLS_MTU;
		}
	}
	...
	if (rtpstart & 1) {
		rtpstart++;
		ast_log_chan(NULL, LOG_WARNING, "Odd start value for RTP port in rtp.conf, rounding up to %d\n", rtpstart);
	}

	if (rtpstart >= rtpend) {
		ast_log_chan(NULL, LOG_WARNING, "Unreasonable values for RTP start/end port in rtp.conf\n");
		rtpstart = DEFAULT_RTP_START;
		rtpend = DEFAULT_RTP_END;
	}
	ast_verb_chan(NULL, 2, "RTP Allocating from port range %d -> %d\n", rtpstart, rtpend);
	return 0;
}
