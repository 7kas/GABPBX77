	...
	const char *options)
{
	...
	if (ast_app_parse_options(multicast_rtp_options, &mcast_options->opts,
		...
		ast_log_chan(NULL, LOG_WARNING, "Error parsing multicast RTP options\n");
...
static int set_type(struct multicast_rtp *multicast, const char *type)
{
	if (!strcasecmp(type, "basic")) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Unrecognized multicast type '%s' specified.\n", type);
		return -1;
	}
...
static void set_ttl(int sock, const char *ttl_str)
{
	...
	if (sscanf(ttl_str, "%30d", &ttl) < 1) {
		ast_log_chan(NULL, LOG_WARNING, "Invalid multicast ttl option '%s'\n", ttl_str);
		return;
	}

	if (setsockopt(sock, IPPROTO_IP, IP_MULTICAST_TTL, &ttl, sizeof(ttl)) < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Could not set multicast ttl to '%s': %s\n",
...
static void set_loop(int sock, const char *loop_str)
{
	...
	if (sscanf(loop_str, "%30hhu", &loop) < 1) {
		ast_log_chan(NULL, LOG_WARNING, "Invalid multicast loop option '%s'\n", loop_str);
		return;
	}

	if (setsockopt(sock, IPPROTO_IP, IP_MULTICAST_LOOP, &loop, sizeof(loop)) < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Could not set multicast loop to '%s': %s\n",
...
static void set_if(int sock, const char *if_str)
{
	...
	if (!inet_aton(if_str, &iface)) {
		ast_log_chan(NULL, LOG_WARNING, "Cannot parse if option '%s'\n", if_str);
	}

	if (setsockopt(sock, IPPROTO_IP, IP_MULTICAST_IF, &iface, sizeof(iface)) < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Could not set multicast if to '%s': %s\n",
...
static int multicast_send_control_packet(struct ast_rtp_instance *instance, struct multicast_rtp *multicast, int command)
{
	...
	if (ast_sockaddr_is_ipv6(&remote_address)) {
		ast_log_chan(NULL, LOG_WARNING, "Cannot send control packet for IPv6 "
			"remote address.\n");
		return -1;
	}
...
static int rtp_raw_write(struct ast_rtp_instance *instance, struct ast_frame *frame, int codec)
{
	...
	if (ast_sendto(multicast->socket, (void *) rtpheader, frame->datalen + hdrlen, 0, &remote_address) < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Multicast RTP Transmission error to %s: %s\n",
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER, "Multicast RTP Engine",
