 ...
 * http://lists.digium.com/pipermail/gabpbx-dev/2003-June/000884.html
 *
...
/*** DOCUMENTATION
	...
	<function name="SMDI_MSG_RETRIEVE" language="en_US">
		...
		<description>
			...
			the global SMDI message queue, and can not be accessed by any other GABpbx
...
/*! \brief A mapping between an SMDI mailbox ID and an GABpbx mailbox */
struct mailbox_mapping {
	...
	AST_DECLARE_STRING_FIELDS(
		...
		/*! The name of the mailbox on the GABpbx side */
...
static int smdi_toggle_mwi(struct ast_smdi_interface *iface, const char *mailbox, int on)
{
	...
	if (!(file = fopen(iface->name, "w"))) {
		ast_log_chan(NULL, LOG_ERROR, "Error opening SMDI interface %s (%s) for writing\n", iface->name, strerror(errno));
		return 1;
	}
...
static void purge_old_messages(struct ast_smdi_interface *iface, enum smdi_message_type type)
{
	...
	while (msg) {
		...
		if (elapsed > iface->msg_expiry) {
			...
			ao2_ref(msg, -1);
			ast_log_chan(NULL, LOG_NOTICE, "Purged expired message from %s SMDI %s message queue.  "
	...
	while ((c = fgetc(iface->file))) {
		...
		if (c == 'D') { /* MD message */
			...
			for (i = 0; i < sizeof(md_msg->mesg_desk_num) - 1; i++) {
				c = fgetc(iface->file);
				if (c == EOF) {
					ast_log_chan(NULL, LOG_ERROR, "Unexpected EOF while reading MD message\n");
					ao2_ref(md_msg, -1);
					return NULL;
				}
			...
			for (i = 0; i < sizeof(md_msg->mesg_desk_term) - 1; i++) {
				c = fgetc(iface->file);
				if (c == EOF) {
					ast_log_chan(NULL, LOG_ERROR, "Unexpected EOF while reading SMDI message\n");
					ao2_ref(md_msg, -1);
					return NULL;
				}
			...
			for (i = 0; i < sizeof(mwi_msg->cause) - 1; i++) {
				c = fgetc(iface->file);
				if (c == EOF) {
					ast_log_chan(NULL, LOG_ERROR, "Unexpected EOF while reading MWI message\n");
					ao2_ref(mwi_msg, -1);
					return NULL;
				}
			...
			ast_log_chan(NULL, LOG_ERROR, "Unknown SMDI message type received on %s (M%c).\n", iface->name, c);
			start = 0;
		}
	}

	ast_log_chan(NULL, LOG_ERROR, "Error reading from SMDI interface %s, stopping listener thread\n", iface->name);
	return NULL;
}
...
static int smdi_load(int reload)
{
	...
	if (!(conf = ast_config_load(config_file, config_flags)) || conf == CONFIG_STATUS_FILEINVALID) {
		if (reload)
			ast_log_chan(NULL, LOG_NOTICE, "Unable to reload config %s: SMDI untouched\n", config_file);
		else
			ast_log_chan(NULL, LOG_NOTICE, "Unable to load config %s: SMDI disabled\n", config_file);
		return 1;
	...
	for (v = ast_variable_browse(conf, "interfaces"); v; v = v->next) {
		...
		if (!strcasecmp(v->name, "baudrate")) {
			if (!strcasecmp(v->value, "9600"))
				...
				ast_log_chan(NULL, LOG_NOTICE, "Invalid baud rate '%s' specified in %s (line %d), using default\n", v->value, config_file, v->lineno);
				baud_rate = B9600;
			}
		} else if (!strcasecmp(v->name, "msdstrip")) {
			if (!sscanf(v->value, "%30d", &msdstrip)) {
				ast_log_chan(NULL, LOG_NOTICE, "Invalid msdstrip value in %s (line %d), using default\n", config_file, v->lineno);
				msdstrip = 0;
			...
			if (!sscanf(v->value, "%30ld", &msg_expiry)) {
				ast_log_chan(NULL, LOG_NOTICE, "Invalid msgexpirytime value in %s (line %d), using default\n", config_file, v->lineno);
			...
			if (!strcasecmp(v->value, "even"))
				...
				ast_log_chan(NULL, LOG_NOTICE, "Invalid parity bit setting in %s (line %d), using default\n", config_file, v->lineno);
				paritybit = PARENB;
			}
			...
			if (!strcasecmp(v->value, "7"))
				...
				ast_log_chan(NULL, LOG_NOTICE, "Invalid character size setting in %s (line %d), using default\n", config_file, v->lineno);
				charsize = CS7;
			}
			...
			if (reload && old_ifaces) {
				...
				if ((iface = ao2_find(old_ifaces, v->value, OBJ_SEARCH_KEY))) {
					ast_log_chan(NULL, LOG_NOTICE, "SMDI interface %s already running, not restarting\n", iface->name);
			...
			if (!(iface->file = fopen(iface->name, "r"))) {
				ast_log_chan(NULL, LOG_ERROR, "Error opening SMDI interface %s (%s)\n", iface->name, strerror(errno));
				continue;
			}
			...
			if (tcgetattr(iface->fd, &iface->mode)) {
				ast_log_chan(NULL, LOG_ERROR, "Error getting attributes of %s (%s)\n", iface->name, strerror(errno));
				continue;
			}

			/* set the desired speed */
			if (cfsetispeed(&iface->mode, baud_rate) || cfsetospeed(&iface->mode, baud_rate)) {
				ast_log_chan(NULL, LOG_ERROR, "Error setting baud rate on %s (%s)\n", iface->name, strerror(errno));
				continue;
			}
			...
			if (tcsetattr(iface->fd, TCSAFLUSH, &iface->mode)) {
				ast_log_chan(NULL, LOG_ERROR, "Error setting attributes on %s (%s)\n", iface->name, strerror(errno));
				continue;
			}
			 ...
			 */
			ast_verb_chan(NULL, 3, "Starting SMDI monitor thread for %s\n", iface->name);
			if (ast_pthread_create_background(&iface->thread, NULL, smdi_read, iface)) {
				ast_log_chan(NULL, LOG_ERROR, "Error starting SMDI monitor thread for %s\n", iface->name);
				continue;
			}
			...
			ast_log_chan(NULL, LOG_NOTICE, "Ignoring unknown option %s in %s\n", v->name, config_file);
		}
	}
	...
	for (v = ast_variable_browse(conf, "mailboxes"); v; v = v->next) {
		if (!strcasecmp(v->name, "smdiport")) {
			...
			if (!(mailbox_iface = ao2_find(new_ifaces, v->value, OBJ_SEARCH_KEY))) {
				ast_log_chan(NULL, LOG_NOTICE, "SMDI interface %s not found\n", v->value);
				continue;
			}
		} else if (!strcasecmp(v->name, "pollinginterval")) {
			if (sscanf(v->value, "%30u", &mwi_monitor.polling_interval) != 1) {
				ast_log_chan(NULL, LOG_ERROR, "Invalid value for pollinginterval: %s\n", v->value);
				mwi_monitor.polling_interval = DEFAULT_POLLING_INTERVAL;
			}
		} else {
			if (!mailbox_iface) {
				ast_log_chan(NULL, LOG_ERROR, "Mailbox mapping ignored, no valid SMDI interface specified in mailboxes section\n");
				continue;
			}
	...
	if (!AST_LIST_EMPTY(&mwi_monitor.mailbox_mappings) && mwi_monitor.thread == AST_PTHREADT_NULL
		...
		ast_log_chan(NULL, LOG_ERROR, "Failed to start MWI monitoring thread.  This module will not operate.\n");
		return -1;
	}
...
static int smdi_msg_retrieve_read(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_ERROR, "SMDI_MSG_RETRIEVE requires an argument\n");
		goto return_error;
	}

	if (!chan) {
		ast_log_chan(NULL, LOG_ERROR, "SMDI_MSG_RETRIEVE must be used with a channel\n");
		goto return_error;
	}
	...
	if (ast_strlen_zero(args.port) || ast_strlen_zero(args.search_key)) {
		ast_log_chan(NULL, LOG_ERROR, "Invalid arguments provided to SMDI_MSG_RETRIEVE\n");
		goto return_error;
	}

	if (!(iface = ast_smdi_interface_find(args.port))) {
		ast_log_chan(NULL, LOG_ERROR, "SMDI port '%s' not found\n", args.port);
		goto return_error;
	}
	...
	if (!ast_strlen_zero(args.timeout)) {
		if (sscanf(args.timeout, "%30u", &timeout) != 1) {
			ast_log_chan(NULL, LOG_ERROR, "'%s' is not a valid timeout\n", args.timeout);
			timeout = SMDI_RETRIEVE_TIMEOUT_DEFAULT;
		}
	}

	if (!(md_msg = smdi_message_wait(iface, timeout, SMDI_MD, args.search_key, options))) {
		ast_log_chan(NULL, LOG_WARNING, "No SMDI message retrieved for search key '%s' after "
...
static int smdi_msg_read(struct ast_channel *chan, const char *cmd, char *data, char *buf, size_t len)
{
	...
	if (!chan) {
		ast_log_chan(NULL, LOG_ERROR, "SMDI_MSG can not be called without a channel\n");
		goto return_error;
	}

	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "SMDI_MSG requires an argument\n");
		goto return_error;
	}
	...
	if (ast_strlen_zero(args.id)) {
		ast_log_chan(NULL, LOG_WARNING, "ID must be supplied to SMDI_MSG\n");
		goto return_error;
	}
	...
	if (!datastore) {
		ast_log_chan(NULL, LOG_WARNING, "No SMDI message found for message ID '%s'\n", args.id);
		goto return_error;
	}
	...
	if (!strcasecmp(args.component, "number")) {
		...
		ast_log_chan(NULL, LOG_ERROR, "'%s' is not a valid message component for SMDI_MSG\n",
			args.component);
		goto return_error;
	}
...
static int load_module(void)
{
	...
	if (res < 0) {
		...
		ast_log_chan(NULL, LOG_NOTICE, "No SMDI interfaces are available to listen on, not starting SMDI listener.\n");
	}
...
static int reload(void)
{
	...
	if (res < 0) {
		...
		ast_log_chan(NULL, LOG_WARNING, "No SMDI interfaces were specified to listen on, not starting SDMI listener.\n");
	}
	return 0;
}

AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER, "Simplified Message Desk Interface (SMDI) Resource",
