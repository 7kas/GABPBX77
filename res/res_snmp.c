...
#define	MODULE_DESCRIPTION	"SNMP [Sub]Agent for GABpbx"
...
static int load_config(void)
{
	...
	if (cfg == CONFIG_STATUS_FILEMISSING || cfg == CONFIG_STATUS_FILEINVALID) {
		ast_log_chan(NULL, LOG_WARNING, "Could not load res_snmp.conf\n");
		return 0;
	}
	...
	while (cat) {
		...
		if (strcasecmp(cat, "general") == 0) {
			while (var) {
				if (strcasecmp(var->name, "subagent") == 0) {
					if (ast_true(var->value))
						...
						ast_log_chan(NULL, LOG_ERROR, "Value '%s' does not evaluate to true or false.\n", var->value);
					...
					ast_log_chan(NULL, LOG_ERROR, "Unrecognized variable '%s' in category '%s'\n", var->name, cat);
					ast_config_destroy(cfg);
					return 1;
				}
				var = var->next;
			...
			ast_log_chan(NULL, LOG_ERROR, "Unrecognized category '%s'\n", cat);
			ast_config_destroy(cfg);
			return 1;
		}
...
static int load_module(void)
{
	...
	ast_verb_chan(NULL, 1, "Loading [Sub]Agent Module\n");
...
{
	ast_verb_chan(NULL, 1, "Unloading [Sub]Agent Module\n");

	res_snmp_dont_stop = 0;
	return ((thread != AST_PTHREADT_NULL) ? pthread_join(thread, NULL) : 0);
}

AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_DEFAULT, "SNMP [Sub]Agent for GABpbx",
