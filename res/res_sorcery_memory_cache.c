...
static void set_passthru_update(uint32_t value)
{
	...
	passthru_update_thread_id = ast_threadstorage_get(&passthru_update_id_storage,
		sizeof(*passthru_update_thread_id));
	if (!passthru_update_thread_id) {
		ast_log_chan(NULL, LOG_ERROR, "Could not set passthru update ID for sorcery memory cache thread\n");
		return;
	}
...
static int sorcery_memory_cache_create(const struct ast_sorcery *sorcery, void *data, void *object)
{
	...
	if (cache->maximum_objects && ao2_container_count(cache->objects) >= cache->maximum_objects) {
		if (remove_oldest_from_cache(cache)) {
			ast_log_chan(NULL, LOG_ERROR, "Unable to make room in cache for sorcery object '%s'.\n",
	...
	if (add_to_cache(cache, cached)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to add object '%s' to the cache\n",
			ast_sorcery_object_get_id(object));
		ao2_unlock(cache->objects);
		ao2_ref(cached, -1);
		return -1;
...
static int stale_cache_update(const void *data)
{
	...
	if (task_data->cache->maximum_objects && ao2_container_count(backend_objects) >= task_data->cache->maximum_objects) {
		ast_log_chan(NULL, LOG_ERROR, "The backend contains %d objects while the sorcery memory cache '%s' is explicitly configured to only allow %d\n",
	...
	if (ao2_container_count(task_data->cache->objects) != ao2_container_count(backend_objects)) {
		ast_log_chan(NULL, LOG_WARNING, "The backend contains %d objects while only %d could be added to sorcery memory cache '%s'\n",
	...
	struct sorcery_memory_cached_object *cached)
{
	...
	if (cached->stale_update_sched_id == -1) {
		...
		if (cached->stale_update_sched_id < 0) {
			ao2_cleanup(task_data);
			ast_log_chan(NULL, LOG_ERROR, "Unable to update stale cached object type '%s', ID '%s'.\n",
				ast_sorcery_object_get_type(cached->object), ast_sorcery_object_get_id(cached->object));
		}
	}
	ao2_unlock(cached);
...
static void *sorcery_memory_cache_open(const char *data)
{
	...
	while (!ast_strlen_zero(options) && (option = strsep(&options, ","))) {
		...
		if (!strcasecmp(name, "name")) {
			if (ast_strlen_zero(value)) {
				ast_log_chan(NULL, LOG_ERROR, "A name must be specified for the memory cache\n");
				return NULL;
			}
			ast_free(cache->name);
			...
			if (configuration_parse_unsigned_integer(value, &cache->maximum_objects) != 1) {
				ast_log_chan(NULL, LOG_ERROR, "Unsupported maximum objects value of '%s' used for memory cache\n",
					value);
				return NULL;
			}
			...
			if (configuration_parse_unsigned_integer(value, &cache->object_lifetime_maximum) != 1) {
				ast_log_chan(NULL, LOG_ERROR, "Unsupported object maximum lifetime value of '%s' used for memory cache\n",
					value);
				return NULL;
			}
		} else if (!strcasecmp(name, "object_lifetime_stale")) {
			if (configuration_parse_unsigned_integer(value, &cache->object_lifetime_stale) != 1) {
				ast_log_chan(NULL, LOG_ERROR, "Unsupported object stale lifetime value of '%s' used for memory cache\n",
					value);
				return NULL;
			}
			...
			ast_log_chan(NULL, LOG_ERROR, "Unsupported option '%s' used for memory cache\n", name);
			return NULL;
		}
	}
	...
	if (!cache->objects) {
		ast_log_chan(NULL, LOG_ERROR, "Could not create a container to hold cached objects for memory cache\n");
		return NULL;
	}
	...
	if (!cache->object_heap) {
		ast_log_chan(NULL, LOG_ERROR, "Could not create heap to hold cached objects\n");
		return NULL;
	}
...
static int load_module(void)
{
	...
	if (!caches) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to create container for configured caches\n");
		unload_module();
	...
	if (!sched) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to create scheduler for cache management\n");
		unload_module();
		return AST_MODULE_LOAD_DECLINE;
	}

	if (ast_sched_start_thread(sched)) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to create scheduler thread for cache management\n");
		unload_module();
		return AST_MODULE_LOAD_DECLINE;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER, "Sorcery Memory Cache Object Wizard",
