		...
		const struct ast_sorcery *sorcery, const char *type)
{
	...
	if (!object_type) {
		ast_log_chan(NULL, LOG_WARNING, "Unknown sorcery object type %s. Expect errors\n", type);
...
static void sorcery_realtime_retrieve_multiple(const struct ast_sorcery *sorcery, void *data, const char *type, struct ao2_container *objects, const struct ast_variable *fields)
{
	...
	if (!fields) {
		...
		if (config->fetch == UNQUALIFIED_FETCH_ERROR) {
			ast_log_chan(NULL, LOG_ERROR, "Unqualified fetch prevented on %s\n", config->family);
			return;
		}
		if (config->fetch == UNQUALIFIED_FETCH_WARN) {
			ast_log_chan(NULL, LOG_WARNING, "Unqualified fetch requested on %s\n", config->family);
		}
	...
	while ((option = strsep(&tmp, ","))) {
		...
		if (!strcasecmp(name, "allow_unqualified_fetch")) {
			if (ast_strlen_zero(value) || !strcasecmp(value, "yes")) {
				...
				ast_log_chan(NULL, LOG_ERROR, "Unrecognized value in %s:%s: '%s'\n", family, name, value);
				return NULL;
			}
		} else {
			ast_log_chan(NULL, LOG_ERROR, "Unrecognized option in %s: '%s'\n", family, name);
			return NULL;
		}
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER, "Sorcery Realtime Object Wizard",
