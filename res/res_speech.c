...
int ast_speech_register(struct ast_speech_engine *engine)
{
	...
	if (!engine->create || !engine->write || !engine->destroy) {
		ast_log_chan(NULL, LOG_WARNING, "Speech recognition engine '%s' did not meet minimum API requirements.\n", engine->name);
		return -1;
	}

	/* If an engine is already loaded with this name, error out */
	if (ast_speech_find_engine(engine->name)) {
		ast_log_chan(NULL, LOG_WARNING, "Speech recognition engine '%s' already exists.\n", engine->name);
		return -1;
	}

	ast_verb_chan(NULL, 5, "Registered speech recognition engine '%s'\n", engine->name);
	...
	if (!default_engine) {
		...
		ast_verb_chan(NULL, 5, "Made '%s' the default speech recognition engine\n", engine->name);
	}
	AST_RWLIST_UNLOCK(&engines);
...
struct ast_speech_engine *ast_speech_unregister2(const char *engine_name)
{
	...
	AST_RWLIST_TRAVERSE_SAFE_BEGIN(&engines, engine, list) {
		if (!strcasecmp(engine->name, engine_name)) {
			...
			ast_verb_chan(NULL, 5, "Unregistered speech recognition engine '%s'\n", engine_name);
			/* All went well */
			break;
		}
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER, "Generic Speech Recognition API",
