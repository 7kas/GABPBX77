...
/*! \file
 *
 * \brief GABpbx External Application Speech Engine
 *
 */
...
#include "gabpbx/res_aeap.h"
#include "gabpbx/res_aeap_message.h"
...
#define log_error(obj, fmt, ...) \
	ast_log_chan(NULL, LOG_ERROR, "AEAP speech (%p): " fmt "\n", obj, ##__VA_ARGS__)
 ...
 *
 * Create, then sends a request to an GABpbx external application, and then blocks
 ...
 *
 * \param aeap Pointer to an GABpbx external application object
...
static void ast_aeap_speech_on_error(struct ast_aeap *aeap)
{
	struct ast_speech *speech = ast_aeap_user_data_object_by_id(aeap, "speech");
	if (!speech) {
		ast_log_chan(NULL, LOG_ERROR, "aeap generated error with no associated speech object");
		return;
	}
...
static struct ast_speech_engine *speech_engine_alloc(const char *name)
{
	...
	if (!engine) {
		ast_log_chan(NULL, LOG_ERROR, "AEAP speech: unable create engine '%s'\n", name);
		return NULL;
	}
...
static void speech_engine_alloc_and_register(const char *name, const struct ast_format_cap *formats)
{
	...
	if (formats && ast_format_cap_append_from_cap(engine->formats,
		...
		ast_log_chan(NULL, LOG_WARNING, "AEAP speech: Unable to add engine '%s' formats\n", name);
		ao2_ref(engine, -1);
		return;
	}

	if (ast_speech_register(engine)) {
		ast_log_chan(NULL, LOG_WARNING, "AEAP speech: Unable to register engine '%s'\n", name);
		ao2_ref(engine, -1);
	}
}
...
static void speech_engine_alloc_and_register2(const char *name, const char *codec_names)
{
	...
	if (codec_names && ast_format_cap_update_by_allow_disallow(engine->formats, codec_names, 1)) {
		ast_log_chan(NULL, LOG_WARNING, "AEAP speech: Unable to add engine '%s' codecs\n", name);
		ao2_ref(engine, -1);
		return;
	}
...
static int load_engine(void *obj, void *arg, int flags)
{
	...
	if (!formats) {
		...
		if (!formats) {
			ast_log_chan(NULL, LOG_ERROR, "AEAP speech: unable to allocate default engine format for '%s'\n", id);
			return 0;
		}
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "GABpbx External Application Speech Engine",
