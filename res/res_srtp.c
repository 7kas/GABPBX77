...
/* See https://docs.gabpbx.org/Deployment/Secure-Calling/ */

#include "gabpbx.h"                   /* for NULL, size_t, memcpy, etc */
...
#include "gabpbx/astobj2.h"           /* for ao2_t_ref, etc */
#include "gabpbx/frame.h"             /* for AST_FRIENDLY_OFFSET */
#include "gabpbx/logger.h"            /* for ast_log, ast_debug, etc */
#include "gabpbx/module.h"            /* for ast_module_info, etc */
#include "gabpbx/sdp_srtp.h"
#include "gabpbx/res_srtp.h"          /* for ast_srtp_cb, ast_srtp_suite, etc */
#include "gabpbx/rtp_engine.h"        /* for ast_rtp_engine_register_srtp, etc */
#include "gabpbx/utils.h"             /* for ast_free, ast_calloc */
...
static struct ast_srtp *res_srtp_new(void)
{
	...
	if (!(srtp = ast_calloc(1, sizeof(*srtp)))) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to allocate memory for srtp\n");
		return NULL;
	}
...
static struct ast_srtp_policy *ast_srtp_policy_alloc()
{
	...
	if (!(tmp = ao2_t_alloc(sizeof(*tmp), policy_destructor, "Allocating policy"))) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to allocate memory for srtp_policy\n");
	}
...
static int policy_set_suite(crypto_policy_t *p, enum ast_srtp_suite suite)
{
	...
	default:
		ast_log_chan(NULL, LOG_ERROR, "Invalid crypto suite: %u\n", suite);
		return -1;
	}
}
...
tryagain:
	...
	if (!srtp->session) {
		ast_log_chan(NULL, LOG_ERROR, "SRTP unprotect %s - missing session\n", rtcp ? "rtcp" : "rtp");
		errno = EINVAL;
		return -1;
	}
	...
	if (retry == 0  && res == err_status_replay_old) {
		ast_log_chan(NULL, AST_LOG_NOTICE, "SRTP unprotect failed with %s, retrying\n", srtp_errstr(res));
		...
		if (srtp->session) {
			...
			if (policy) {
				...
				ast_log_chan(NULL, LOG_ERROR, "SRTP session could not be re-created after unprotect failure: %s\n", srtp_errstr(res_srtp_create));
	...
	if (res != err_status_ok && res != err_status_replay_fail ) {
		 ...
		 */
		if (rtcp) {
			ast_verb_chan(NULL, 2, "SRTCP unprotect failed on SSRC %u because of %s\n",
				ast_rtp_instance_get_ssrc(srtp->rtp), srtp_errstr(res));
		} else {
			if ((srtp->warned >= 10) && !((srtp->warned - 10) % 150)) {
				ast_verb_chan(NULL, 2, "SRTP unprotect failed on SSRC %u because of %s %d\n",
...
static int ast_srtp_protect(struct ast_srtp *srtp, void **buf, int *len, int rtcp)
{
	...
	if (!srtp->session) {
		ast_log_chan(NULL, LOG_ERROR, "SRTP protect %s - missing session\n", rtcp ? "rtcp" : "rtp");
		errno = EINVAL;
		return -1;
	}
	...
	if ((res = rtcp ? srtp_protect_rtcp(srtp->session, localbuf, len) : srtp_protect(srtp->session, localbuf, len)) != err_status_ok && res != err_status_replay_fail) {
		ast_log_chan(NULL, LOG_WARNING, "SRTP protect: %s\n", srtp_errstr(res));
		return -1;
	}
...
static int ast_srtp_create(struct ast_srtp **srtp, struct ast_rtp_instance *rtp, struct ast_srtp_policy *policy)
{
	...
	if (status != err_status_ok) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Failed to create srtp session on rtp instance (%p) - %s\n",
...
static int ast_srtp_replace(struct ast_srtp **srtp, struct ast_rtp_instance *rtp, struct ast_srtp_policy *policy)
{
	...
	if (res) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to replace srtp (%p) on rtp instance (%p) "
				"- keeping old\n", *srtp, rtp);
	}
...
static int ast_srtp_add_stream(struct ast_srtp *srtp, struct ast_srtp_policy *policy)
{
	...
	if ((match = find_policy(srtp, &policy->sp, OBJ_POINTER))) {
		if (policy->sp.ssrc.type != ssrc_specific) {
			ast_log_chan(NULL, AST_LOG_WARNING, "Cannot replace an existing wildcard policy\n");
			ao2_t_ref(match, -1, "Unreffing already existing policy");
			return -1;
		} else {
			if (srtp_remove_stream(srtp->session, match->sp.ssrc.value) != err_status_ok) {
				ast_log_chan(NULL, AST_LOG_WARNING, "Failed to remove SRTP stream for SSRC %u\n", match->sp.ssrc.value);
			}
	...
	if (srtp_add_stream(srtp->session, &policy->sp) != err_status_ok) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Failed to add SRTP stream for %s %u\n",
			policy->sp.ssrc.type == ssrc_specific ? "SSRC" : "type",
			policy->sp.ssrc.type == ssrc_specific ? policy->sp.ssrc.value : policy->sp.ssrc.type);
		return -1;
	}
...
static int ast_srtp_change_source(struct ast_srtp *srtp, unsigned int from_ssrc, unsigned int to_ssrc)
{
	...
	if ((match = find_policy(srtp, &sp, OBJ_POINTER | OBJ_UNLINK))) {
		...
		if (ast_srtp_add_stream(srtp, match)) {
			ast_log_chan(NULL, LOG_WARNING, "Couldn't add stream\n");
...
static struct ast_sdp_crypto *crypto_init_keys(struct ast_sdp_crypto *p, const int key_len)
{
	...
	if (p->key_len != key_len) {
		ast_log_chan(NULL, LOG_ERROR, "base64 encode/decode bad len %d != %d\n", p->key_len, key_len);
		return NULL;
	}

	if (memcmp(remote_key, p->local_key, p->key_len)) {
		ast_log_chan(NULL, LOG_ERROR, "base64 encode/decode bad key\n");
		return NULL;
	}
...
static int res_sdp_crypto_build_offer(struct ast_sdp_crypto *p, int taglen)
{
	...
	if (res == -1 || !p->a_crypto) {
		ast_log_chan(NULL, LOG_ERROR, "Could not allocate memory for crypto line\n");
		return -1;
	}
...
static int set_crypto_policy(struct ast_srtp_policy *policy, int suite_val, const unsigned char *master_key, int key_len, unsigned long ssrc, int inbound)
{
	...
	if (policy_res.set_suite(policy, suite_val)) {
		ast_log_chan(NULL, LOG_WARNING, "Could not set remote SRTP suite\n");
		return -1;
	}
...
static int crypto_activate(struct ast_sdp_crypto *p, int suite_val, unsigned char *remote_key, int key_len, struct ast_rtp_instance *rtp)
{
	...
	if (ast_rtp_instance_add_srtp_policy(rtp, remote_policy, local_policy, 0)) {
		ast_log_chan(NULL, LOG_WARNING, "Could not set SRTP policies\n");
		goto err;
	}
...
static int res_sdp_crypto_parse_offer(struct ast_rtp_instance *rtp, struct ast_sdp_srtp *srtp, const char *attr)
{
	...
	if (!tag || !suite) {
		ast_log_chan(NULL, LOG_WARNING, "Unrecognized crypto attribute a=%s\n", attr);
		return -1;
	}

	/* RFC4568 9.1 - tag is 1-9 digits */
	if (sscanf(tag, "%30d", &tag_from_sdp) != 1 || tag_from_sdp < 0 || tag_from_sdp > 999999999) {
		ast_log_chan(NULL, LOG_WARNING, "Unacceptable a=crypto tag: %s\n", tag);
		return -1;
	}

	if (!ast_strlen_zero(session_params)) {
		ast_log_chan(NULL, LOG_WARNING, "Unsupported crypto parameters: %s\n", session_params);
		return -1;
	}

	/* On egress, GABpbx sent several crypto lines in the SIP/SDP offer
	...
	while ((key_param = strsep(&key_params, ";"))) {
		...
		if (mki && *mki != '1') {
			ast_log_chan(NULL, LOG_NOTICE, "Crypto MKI handling is not supported: ignoring attribute %s\n", attr);
			continue;
		}
		...
		if (lifetime) {
			if (!strncmp(lifetime, "2^", 2)) {
				...
				if (sscanf(lifetime_val, "%30u", &n_lifetime) != 1) {
					ast_log_chan(NULL, LOG_NOTICE, "Failed to parse lifetime value in crypto attribute: %s\n", attr);
					continue;
				}

				if (n_lifetime > 48) {
					/* Yeah... that's a bit big. */
					ast_log_chan(NULL, LOG_NOTICE, "Crypto lifetime exponent of '%u' is a bit large; using 48\n", n_lifetime);
					n_lifetime = 48;
				}
			...
			if (sdes_lifetime < 1048576) {
				ast_log_chan(NULL, LOG_NOTICE, "Rejecting crypto attribute '%s': lifetime '%f' too short\n", attr, sdes_lifetime);
				continue;
			}
		}
	...
	if (!found) {
		ast_log_chan(NULL, LOG_NOTICE, "SRTP crypto offer not acceptable: '%s'\n", attr);
		return -1;
	}

	key_len_from_sdp = ast_base64decode(remote_key, key_salt, sizeof(remote_key));
	if (key_len_from_sdp != key_len_expected) {
		ast_log_chan(NULL, LOG_WARNING, "SRTP descriptions key length is '%d', not '%d'\n",
...
static const char *res_sdp_srtp_get_attr(struct ast_sdp_srtp *srtp, int dtls_enabled, int default_taglen_32)
{
	...
	if (!srtp->crypto) {
		if (AST_LIST_NEXT(srtp, sdp_srtp_list)) {
			...
			ast_log_chan(NULL, LOG_ERROR, "SRTP SDP list was not empty\n");
		} else {
			 ...
			 * exclude a suite, change this array and recompile GABpbx.
	...
	ast_log_chan(NULL, LOG_WARNING, "No SRTP key management enabled\n");
	return NULL;
}
...
static int res_srtp_init(void)
{
	...
	if (srtp_init() != err_status_ok) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Failed to initialize libsrtp\n");
		return -1;
	}
	...
	if (ast_rtp_engine_register_srtp(&srtp_res, &policy_res)) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Failed to register SRTP with rtp engine\n");
		res_srtp_shutdown();
		return -1;
	}

	if (ast_sdp_crypto_register(&res_sdp_crypto_api)) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Failed to register SDP SRTP crypto API\n");
		res_srtp_shutdown();
		return -1;
	}

#ifdef HAVE_SRTP_GET_VERSION
	ast_verb_chan(NULL, 2, "%s initialized\n", srtp_get_version_string());
#else
	ast_verb_chan(NULL, 2, "libsrtp initialized\n");
#endif
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER, "Secure RTP (SRTP)",
