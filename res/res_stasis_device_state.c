...
#include "gabpbx/stasis_app_device_state.h"
			      ...
			      const char *name, enum ast_device_state state)
{
	...
	if (!json) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to create device state json object\n");
		return;
	}
	...
	const char *name, const char *value)
{
	...
	if (strncasecmp(name, DEVICE_STATE_SCHEME_STASIS, size)) {
		ast_log_chan(NULL, LOG_ERROR, "Update can only be used to set "
	...
	name += size;
	if (ast_strlen_zero(name)) {
		ast_log_chan(NULL, LOG_ERROR, "Update requires custom device name!\n");
		return STASIS_DEVICE_STATE_MISSING;
	}

	if (!value || (state = ast_devstate_val(value)) == AST_DEVICE_UNKNOWN) {
		ast_log_chan(NULL, LOG_ERROR, "Unknown device state "
			"value '%s'\n", value);
...
enum stasis_device_state_result stasis_app_device_state_delete(const char *name)
{
	...
	if (strncasecmp(name, DEVICE_STATE_SCHEME_STASIS, size)) {
		ast_log_chan(NULL, LOG_ERROR, "Can only delete '%s' device states!\n",
			DEVICE_STATE_SCHEME_STASIS);
		return STASIS_DEVICE_STATE_NOT_CONTROLLED;
	}
	...
	name += size;
	if (ast_strlen_zero(name)) {
		ast_log_chan(NULL, LOG_ERROR, "Delete requires a device name!\n");
		return STASIS_DEVICE_STATE_MISSING;
	}
...
static int subscribe_device_state(struct stasis_app *app, void *obj)
{
	...
	if (!sub->sub) {
		ao2_unlock(device_state_subscriptions);
		ast_log_chan(NULL, LOG_ERROR, "Unable to subscribe to device %s\n",
			sub->device_name);
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS, "Stasis application device state support",
