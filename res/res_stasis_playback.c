...
#include "gabpbx/stasis_app_playback.h"
#include "gabpbx/stasis_app_recording.h"
	...
	const char *uniqueid)
{
	...
	if (playback->state == STASIS_PLAYBACK_STATE_CANCELED) {
		ast_log_chan(NULL, LOG_NOTICE, "%s: Playback canceled for %s\n",
	...
	long playedms, int res, int hangup, const char *uniqueid)
{
	...
	if (res == 0) {
		...
		if (playback->state == STASIS_PLAYBACK_STATE_STOPPED) {
			ast_log_chan(NULL, LOG_NOTICE, "%s: Playback stopped for %s\n",
				uniqueid, playback->media);
		} else {
			ast_log_chan(NULL, LOG_WARNING, "%s: Playback failed for %s\n",
				uniqueid, playback->media);
			if (playback->media_index == AST_VECTOR_SIZE(&playback->medias) - 1 || hangup ) {
	...
	struct ast_channel *chan)
{
	...
	for (index = 0; index < AST_VECTOR_SIZE(&playback->medias); index++) {
		...
		if (ast_begins_with(playback->media, SOUND_URI_SCHEME)) {
			...
			if (!recording) {
				ast_log_chan(NULL, LOG_ERROR, "Attempted to play recording '%s' on channel '%s' but recording does not exist",
			...
			if (sscanf(playback->media + strlen(NUMBER_URI_SCHEME), "%30d", &number) != 1) {
				ast_log_chan(NULL, LOG_ERROR, "Attempted to play number '%s' on channel '%s' but number is invalid",
			...
			/* Play URL */
			ast_log_chan(NULL, LOG_ERROR, "Attempted to play URI '%s' on channel '%s' but scheme is unsupported\n",
		...
		if (hangup) {
			ast_log_chan(NULL, LOG_DEBUG, "Channel: %s already hangup, stop playback\n", ast_channel_name(chan));
			break;
		}
	...
	const char *playback_id)
{
	...
	if (!playback) {
		ast_log_chan(NULL, LOG_ERROR, "Couldn't find playback %s\n",
			playback_id);
		return;
	}
	...
	int skipms, long offsetms, const char *id)
{
	...
	for (i = 0; i < media_count; i++) {
		...
		if (ast_strlen_zero(media[i])) {
			ast_log_chan(NULL, LOG_ERROR, "Attempted to play media on channel '%s' but no media URI was provided.\n",
	...
	enum stasis_app_playback_media_operation operation)
{
	...
	if (operation >= STASIS_PLAYBACK_MEDIA_OP_MAX) {
		ast_log_chan(NULL, LOG_ERROR, "Invalid playback operation %u\n", operation);
		return -1;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS, "Stasis application playback support",
