	...
	struct ast_channel *chan, void *data)
{
	...
	if (stasis_app_get_bridge(control)) {
		ast_log_chan(NULL, LOG_ERROR, "Cannot record channel while in bridge\n");
	...
	struct stasis_app_recording_options *options)
{
	...
	if ((recording->options->if_exists == AST_RECORD_IF_EXISTS_FAIL) &&
		...
		ast_log_chan(NULL, LOG_WARNING, "Recording file '%s' already exists and ifExists option is failure.\n",
			recording->absolute_name);
		errno = EEXIST;
		ao2_ref(recording, -1);
		return NULL;
	...
	enum stasis_app_recording_media_operation operation)
{
	...
	if ((unsigned int)recording->state >= STASIS_APP_RECORDING_STATE_MAX) {
		ast_log_chan(NULL, LOG_WARNING, "Invalid recording state %u\n",
			recording->state);
		return -1;
	}

	if ((unsigned int)operation >= STASIS_APP_RECORDING_OPER_MAX) {
		ast_log_chan(NULL, LOG_WARNING, "Invalid recording operation %u\n",
			operation);
		return -1;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER, "Stasis application recording support",
