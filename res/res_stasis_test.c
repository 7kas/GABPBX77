	...
	int num_messages, int timeout_millis)
{
	...
	while (sink->num_messages < num_messages) {
		...
		if (r != 0) {
			ast_log_chan(NULL, LOG_ERROR, "Unexpected condition error: %s\n",
				strerror(r));
			break;
		}
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER, "Stasis test utilities",
