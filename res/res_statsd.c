...
/*** DOCUMENTATION
	<configInfo name="res_statsd" language="en_US">
		...
		<description>
			...
			allows GABpbx and its modules to send statistics to a StatsD
...
static int load_module(void)
{
	...
	if (aco_process_config(&cfg_info, 0) == ACO_PROCESS_ERROR) {
		...
		ast_log_chan(NULL, LOG_NOTICE, "Could not load statsd config; using defaults\n");
		cfg = conf_alloc();
		if (!cfg) {
			aco_info_destroy(&cfg_info);
			return AST_MODULE_LOAD_DECLINE;
		...
		if (aco_set_defaults(&global_option, "general", cfg->global)) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to initialize statsd defaults.\n");
			ao2_ref(cfg, -1);
			aco_info_destroy(&cfg_info);
			return AST_MODULE_LOAD_DECLINE;
		}
 ...
 */
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER, "StatsD client support",
