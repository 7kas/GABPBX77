	...
	struct ast_stir_shaken_vs_ctx *ctx)
{
	...
	if (!ctx->chan) {
		ast_log_chan(NULL, LOG_ERROR, "Channel is required to add STIR/SHAKEN verification\n");
		return -1;
	}
	...
	if (!ctx->identity_hdr) {
		ast_log_chan(NULL, LOG_ERROR, "No identity to add STIR/SHAKEN verification to channel "
			"%s\n", chan_name);
		return -1;
	}

	if (!ctx->attestation) {
		ast_log_chan(NULL, LOG_ERROR, "Attestation cannot be NULL to add STIR/SHAKEN verification to "
			"channel %s\n", chan_name);
		return -1;
	}
	...
	if (!stir_datastore) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to allocate space for STIR/SHAKEN datastore for "
			"channel %s\n", chan_name);
		return -1;
	}
	...
	if (!stir_datastore->identity) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to allocate space for STIR/SHAKEN datastore "
	...
	if (!chan_datastore) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to allocate space for datastore for channel "
			"%s\n", chan_name);
		stir_datastore_free(stir_datastore);
		return -1;
	}
	...
	char *data, char *buf, size_t len)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "%s requires at least one argument\n", function);
		return -1;
	}

	if (!chan) {
		ast_log_chan(NULL, LOG_ERROR, "No channel for %s function\n", function);
		return -1;
	}
	...
	if (ast_strlen_zero(first)) {
		ast_log_chan(NULL, LOG_ERROR, "An argument must be passed to %s\n", function);
		return -1;
	}
	...
	if (!strcasecmp(first, "count")) {
		...
		if (!ast_strlen_zero(second)) {
			ast_log_chan(NULL, LOG_ERROR, "%s only takes 1 paramater for 'count'\n", function);
			return -1;
		}
	 ...
	 */
	if (ast_strlen_zero(second)) {
		ast_log_chan(NULL, LOG_ERROR, "Retrieving a value using %s requires two paramaters (index, value) "
			"- only index was given\n", function);
		return -1;
	}

	if (ast_str_to_uint(first, &target_index)) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to convert index %s to integer for function %s\n",
			first, function);
		return -1;
	}
	...
	if (current_index != target_index || !chan_datastore) {
		ast_log_chan(NULL, LOG_WARNING, "No STIR/SHAKEN results for index '%s'\n", first);
		return -1;
	}
	...
	if (!strcasecmp(second, "identity")) {
		...
		ast_log_chan(NULL, LOG_ERROR, "No such value '%s' for %s\n", second, function);
		return -1;
	}
...
static int check_for_old_config(void)
{
	...
	" in the source tree at https://github.com/gabpbx/gabpbx/raw/master/configs/samples/stir_shaken.conf.sample"
	" or visit https://docs.gabpbx.org/Deployment/STIR-SHAKEN for more information.";
	...
	if (cfg == CONFIG_STATUS_FILEMISSING) {
		...
		cfg = NULL;
		ast_log_chan(NULL, LOG_ERROR, "The stir_shaken.conf file is invalid\n");
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER, "STIR/SHAKEN Module for GABpbx",
