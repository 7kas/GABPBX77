...
static char *attestation_show(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	if (!as_is_config_loaded()) {
		ast_log_chan(NULL, LOG_WARNING,"Stir/Shaken attestation service disabled.  Either there were errors in the 'attestation' object in stir_shaken.conf or it was missing altogether.\n");
		return CLI_FAILURE;
	}
