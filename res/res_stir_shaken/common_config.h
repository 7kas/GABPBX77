...
{ \
	...
	if (cfg->__substruct __lc_param == __base_enum ## _ ## __unknown) { \
		ast_log_chan(NULL, LOG_WARNING, "Unknown value '%s' specified for %s\n", \
			var->value, var->name); \
		return -1; \
	} \
	return 0; \
...
{ \
	...
	if (rc != 0) { \
		ast_log_chan(NULL, LOG_ERROR, "%s: Unable to copy field %s from %s to %s\n", \
...
/*!
 * \brief Output configuration settings to the GABpbx CLI
 *
 * \param obj A sorcery object containing configuration data
 * \param arg GABpbx CLI argument object
