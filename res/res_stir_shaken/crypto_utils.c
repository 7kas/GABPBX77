	...
	const char *fmt, ...)
{
	...
	if (length) {
		ast_log_chan(NULL, level, file, line, function, "%s\n", buffer);
	}
	...
	const char *long_name)
{
	...
	if (ast_strlen_zero(oid) || ast_strlen_zero(short_name) ||
		...
		ast_log_chan(NULL, LOG_ERROR, "One or more of oid, short_name or long_name are NULL or empty\n");
		return -1;
	}

	nid = OBJ_sn2nid(short_name);
	if (nid != NID_undef) {
		ast_log_chan(NULL, LOG_NOTICE, "NID %d, object %s already registered\n", nid, short_name);
		return nid;
	}
	...
	ast_log_chan(NULL, LOG_NOTICE, "Registered object %s as NID %d\n", short_name, nid);
	...
	int nid, const char *short_name)
{
	...
	if (nid <= 0) {
		nid = OBJ_sn2nid(short_name);
		if (nid == NID_undef) {
			ast_log_chan(NULL, LOG_ERROR, "Extension object for %s not found\n", short_name);
			return NULL;
		}
	} else {
		const char *tmp = OBJ_nid2sn(nid);
		if (!tmp) {
			ast_log_chan(NULL, LOG_ERROR, "Extension object for NID %d not found\n", nid);
			return NULL;
		}
	}
	...
	if (ex_idx < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Extension index not found in certificate\n");
		return NULL;
	}
	ex = X509_get_ext(cert, ex_idx);
	if (!ex) {
		ast_log_chan(NULL, LOG_ERROR, "Extension not found in certificate\n");
		return NULL;
	}
...
EVP_PKEY *crypto_load_privkey_from_file(const char *filename)
{
	...
	if (ast_strlen_zero(filename)) {
		ast_log_chan(NULL, LOG_ERROR, "filename was null or empty\n");
		return NULL;
	}

	fp = fopen(filename, "r");
	if (!fp) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to open %s: %s\n", filename, strerror(errno));
		return NULL;
	}
...
X509 *crypto_load_cert_from_memory(const char *buffer, size_t size)
{
	...
	if (ast_strlen_zero(buffer) || size <= 0) {
		ast_log_chan(NULL, LOG_ERROR, "buffer was null or empty\n");
		return NULL;
	}
...
static int dump_mem_bio(BIO *bio, unsigned char **buffer)
{
	...
	*buffer = ast_malloc(raw_key_len);
	if (!*buffer) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to allocate memory for raw public key\n");
		return -1;
	}
...
struct crypto_cert_store *crypto_create_cert_store(void)
{
	...
	if (!store) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to create crypto_cert_store\n");
		return NULL;
	}
...
static int crypto_load_store_from_cert_file(X509_STORE *store, const char *file)
{
	...
	if (ast_strlen_zero(file)) {
		ast_log_chan(NULL, LOG_ERROR, "file was null or empty\n");
		return -1;
	}
	...
	const char *path)
{
	if (ast_strlen_zero(file) && ast_strlen_zero(path)) {
		ast_log_chan(NULL, LOG_ERROR, "Both file and path can't be NULL\n");
		return -1;
	}

	if (!store || !store->certs) {
		ast_log_chan(NULL, LOG_ERROR, "store or store->certs is NULL\n");
		return -1;
	}
	...
	const char *path)
{
	...
	if (!store || !store->untrusted || !store->untrusted_stack) {
		ast_log_chan(NULL, LOG_ERROR, "store wasn't initialized properly\n");
		return -1;
	}
...
int crypto_show_cli_store(struct crypto_cert_store *store, int fd)
{
	...
	for (i = 0; i < count ; i++) {
		...
		if (X509_OBJECT_get_type(o) == X509_LU_X509) {
			...
			ast_log_chan(NULL, LOG_ERROR, "CRLs are not allowed in the CA cert store\n");
		}
	}
...
int crypto_is_cert_time_valid(X509*cert, time_t reftime)
{
	...
	if (!notbefore || !notafter) {
		ast_log_chan(NULL, LOG_ERROR, "Either notbefore or notafter were not present in the cert\n");
		return 0;
	}
