...
static int verification_apply(const struct ast_sorcery *sorcery, void *obj)
{
	...
	if (!cfg->vcfg_common.acl) {
		...
		if (error) {
			...
			ast_log_chan(NULL, LOG_ERROR, "%s: Unable to create default acl rule for '%s: %s'\n",
				id, "permit", "0.0.0.0/0");
			return -1;
		}
...
static char *cli_verification_show(struct ast_cli_entry *e, int cmd, struct ast_cli_args *a)
{
	...
	if (!vs_is_config_loaded()) {
		ast_log_chan(NULL, LOG_WARNING,"Stir/Shaken verification service disabled.  Either there were errors in the 'verification' object in stir_shaken.conf or it was missing altogether.\n");
		return CLI_FAILURE;
	}
