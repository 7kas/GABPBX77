...
static int stun_monitor_request(const void *blarg)
{
	...
	if (args.stun_sock < 0) {
		...
		if (ast_get_ip(&stun_addr, args.server_hostname)) {
			/* Lookup failed. */
			ast_log_chan(NULL, LOG_WARNING, "Unable to lookup STUN server '%s'\n",
		...
		if (args.stun_sock < 0) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to create STUN socket: %s\n", strerror(errno));
			goto monitor_request_cleanup;
		}
		if (ast_connect(args.stun_sock, &stun_addr)) {
			ast_log_chan(NULL, LOG_WARNING, "STUN Failed to connect to %s: %s\n",
	...
	if (res) {
		...
		if (!args.stun_poll_failed_gripe) {
			...
			ast_log_chan(NULL, LOG_WARNING, "STUN poll %s. Re-evaluating STUN server address.\n",
		...
		if (memcmp(&no_addr, &answer, sizeof(no_addr))
			...
			ast_log_chan(NULL, LOG_NOTICE, "Old external address/port %s:%d now seen as %s:%d.\n",
...
publish_failure:
				if (msg) {
					...
					ast_log_chan(NULL, LOG_ERROR, "Failed to issue network change message.\n");
				}
			} else {
...
static void stun_stop_monitor(void)
{
	...
	if (sched) {
		...
		sched = NULL;
		ast_log_chan(NULL, LOG_NOTICE, "STUN monitor stopped\n");
	}
}
...
static int stun_start_monitor(void)
{
	...
	if (!(sched = ast_sched_context_create())) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to create stun monitor scheduler context\n");
		return -1;
	}
	...
	if (ast_sched_add_variable(sched, (args.refresh * 1000), stun_monitor_request, NULL, 1) < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to schedule STUN network monitor \n");
		ast_sched_context_destroy(sched);
		sched = NULL;
		return -1;
	}

	ast_log_chan(NULL, LOG_NOTICE, "STUN monitor started\n");
	...
	for (v = ast_variable_browse(cfg, "general"); v; v = v->next) {
		if (!strcasecmp(v->name, "stunaddr")) {
			if (setup_stunaddr(v->value, !startup)) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid STUN server address: %s at line %d\n",
					v->value, v->lineno);
			}
		} else if (!strcasecmp(v->name, "stunrefresh")) {
			if ((sscanf(v->value, "%30u", &args.refresh) != 1) || !args.refresh) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid stunrefresh value '%s', must be an integer > 0 at line %d\n", v->value, v->lineno);
				args.refresh = DEFAULT_MONITOR_REFRESH;
			}
		} else {
			ast_log_chan(NULL, LOG_WARNING, "Invalid config option %s at line %d\n",
				v->value, v->lineno);
		}
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER, "STUN Network Monitor",
