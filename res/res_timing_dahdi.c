...
static void *dahdi_timer_open(void)
{
	...
	if ((timer->fd = open("/dev/dahdi/timer", O_RDWR)) < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to create dahdi timer: %s\n", strerror(errno));
		ast_free(timer);
		return NULL;
	}
...
static int dahdi_timer_set_rate(void *data, unsigned int rate)
{
	...
	if (ioctl(timer->fd, DAHDI_TIMERCONFIG, &samples)) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to configure DAHDI timing fd for %d sample timer ticks\n",
			samples);
		return -1;
	}
...
#define SEE_TIMING "For more information on GABpbx timing modules, including ways to potentially fix this problem, please see https://docs.gabpbx.org/Configuration/Core-Configuration/Timing-Interfaces/\n"
...
static int dahdi_test_timer(void)
{
	...
	if (ioctl(fd, DAHDI_TIMERCONFIG, &x)) {
		ast_log_chan(NULL, LOG_ERROR, "You have DAHDI built and drivers loaded, but the DAHDI timer test failed to set DAHDI_TIMERCONFIG to %d.\n" SEE_TIMING, x);
		close(fd);
		return -1;
	}

	if ((x = ast_wait_for_input(fd, 300)) < 0) {
		ast_log_chan(NULL, LOG_ERROR, "You have DAHDI built and drivers loaded, but the DAHDI timer could not be polled during the DAHDI timer test.\n" SEE_TIMING);
		close(fd);
		return -1;
	}
	...
	if (!x) {
		const char dahdi_timer_error[] = {
			"GABpbx has detected a problem with your DAHDI configuration and will shutdown for your protection.  You have options:"
			"\n\t1. You only have to compile DAHDI support into GABpbx if you need it.  One option is to recompile without DAHDI support."
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "DAHDI Timing Interface",
