...
#include "gabpbx/poll-compat.h"       /* for ast_poll() */
...
static void *kqueue_timer_open(void)
{
	...
	if (!(timer = ao2_alloc(sizeof(*timer), timer_destroy))) {
		ast_log_chan(NULL, LOG_ERROR, "Alloc failed for kqueue_timer structure\n");
		return NULL;
	}

	if ((timer->handle = kqueue()) < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to create kqueue fd: %s\n",
			strerror(errno));
		ao2_ref(timer, -1);
		return NULL;
	}
	...
	if (kqueue_timer_init_continuous_event(timer) != 0) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to create continuous event: %s\n",
			strerror(errno));
		ao2_ref(timer, -1);
		return NULL;
	}
...
static int kqueue_timer_set_rate(void *data, unsigned int rate)
{
	...
	if (retval == -1) {
		ast_log_chan(NULL, LOG_ERROR, "[%d]: Error queing timer: %s\n",
...
static int kqueue_timer_ack(void *data, unsigned int quantity)
{
	...
	if (retval == -1) {
		ast_log_chan(NULL, LOG_ERROR, "[%d]: Error sampling kqueue: %s\n",
			timer->handle, strerror(errno));
		ao2_unlock(timer);
		return -1;
	}
	...
	for (i = 0; i < retval; i++) {
		...
		case EVFILT_TIMER:
			if (kev[i].data > quantity) {
				ast_log_chan(NULL, LOG_ERROR, "[%d]: Missed %ju\n",
					timer->handle,
		...
		case CONTINUOUS_EVFILT_TYPE:
			...
			break;
		default:
			ast_log_chan(NULL, LOG_ERROR, "[%d]: Spurious kevent type %d.\n",
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "KQueue Timing Interface",
