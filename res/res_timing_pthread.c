...
static int pthread_timer_set_rate(void *data, unsigned int rate)
{
	...
	if (rate > MAX_RATE) {
		ast_log_chan(NULL, LOG_ERROR, "res_timing_pthread only supports timers at a "
				"max rate of %d / sec\n", MAX_RATE);
		errno = EINVAL;
		return -1;
	}
...
static void signal_pipe(struct pthread_timer *timer)
{
	...
	if (-1 == res) {
		ast_log_chan(NULL, LOG_ERROR, "Error writing to timing pipe: %s\n",
				strerror(errno));
	} else {
...
static void unsignal_pipe(struct pthread_timer *timer)
{
	...
	if (-1 == res) {
		ast_log_chan(NULL, LOG_ERROR, "Error reading from pipe: %s\n",
				strerror(errno));
	} else {
...
static int init_timing_thread(void)
{
	...
	if (ast_pthread_create_background(&timing_thread.thread, NULL, do_timing, NULL)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to start timing thread.\n");
		return -1;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "pthread Timing Interface",
