...
static void *timerfd_timer_open(void)
{
	...
	if (!(timer = ao2_alloc(sizeof(*timer), timer_destroy))) {
		ast_log_chan(NULL, LOG_ERROR, "Could not allocate memory for timerfd_timer structure\n");
		return NULL;
	}
	if ((timer->fd = timerfd_create(CLOCK_MONOTONIC, 0)) < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to create timerfd timer: %s\n", strerror(errno));
		ao2_ref(timer, -1);
		return NULL;
	}
...
static int timerfd_timer_ack(void *data, unsigned int quantity)
{
	...
	do {
		...
		if (timerfd_gettime(timer->fd, &timer_status)) {
			ast_log_chan(NULL, LOG_ERROR, "Call to timerfd_gettime() using handle %d error: %s\n", timer->fd, strerror(errno));
			expirations = 0;
			res = -1;
			break;
		}
		...
		if (read_result == -1) {
			if (errno == EINTR || errno == EAGAIN) {
				...
				ast_log_chan(NULL, LOG_ERROR, "Read error: %s\n", strerror(errno));
				res = -1;
				break;
			}
		}
...
static int load_module(void)
{
	...
	if ((fd = timerfd_create(CLOCK_MONOTONIC, 0)) < 0) {
		ast_log_chan(NULL, LOG_ERROR, "timerfd_create() not supported by the kernel.  Not loading.\n");
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_LOAD_ORDER, "Timerfd Timing Interface",
