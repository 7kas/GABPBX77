 ...
 *
 * Naveen Albert <gabpbx@phreaknet.org>
 *
	...
	if (frame->frametype == AST_FRAME_DTMF) {
		...
		if (result == 'q') {
			...
			if (now >= di->hitsrequired) {
				...
				if (!message) {
					ast_log_chan(NULL, LOG_ERROR, "Unable to publish tone detected event for ARI on channel '%s'", ast_channel_name(chan));
					return 1;
				} else {
...
static int remove_detect(struct ast_channel *chan)
{
	...
	if (!datastore) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Cannot remove TONE_DETECT from %s: TONE_DETECT not currently enabled\n",
		        ast_channel_name(chan));
		return -1;
	}
	data = datastore->data;
	...
	if (ast_audiohook_remove(chan, &data->audiohook)) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Failed to remove TONE_DETECT audiohook from channel %s\n", ast_channel_name(chan));
		return -1;
	}

	if (ast_channel_datastore_remove(chan, datastore)) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Failed to remove TONE_DETECT datastore from channel %s\n",
		        ast_channel_name(chan));
		return -1;
	}
...
static int freq_parser(char *freqs, int *freq1, int *freq2) {
	...
	if (ast_strlen_zero(freqs)) {
		ast_log_chan(NULL, LOG_ERROR, "No frequency specified\n");
		return -1;
	}
	...
	if (!ast_strlen_zero(f3)) {
		ast_log_chan(NULL, LOG_WARNING, "Only up to 2 frequencies may be specified: %s\n", freqs);
		return -1;
	}
	if (ast_str_to_int(f1, freq1)) {
		ast_log_chan(NULL, LOG_WARNING, "Frequency must be an integer: %s\n", f1);
		return -1;
	}
	if (*freq1 < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Sorry, no negative frequencies: %d\n", *freq1);
		return -1;
	}
	if (!ast_strlen_zero(f2)) {
		ast_log_chan(NULL, LOG_WARNING, "Sorry, currently only 1 frequency is supported\n");
		return -1;
		...
		if (*freq2 < 1) {
			ast_log_chan(NULL, LOG_WARNING, "Sorry, positive frequencies only: %d\n", *freq2);
			return -1;
		}
	}
	return 0;
...
static char* goto_parser(struct ast_channel *chan, char *loc) {
	...
	if (!dest) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to parse goto: %s,%s,%s\n", context, exten, pri);
		return NULL;
	}
...
static int detect_read(struct ast_channel *chan, const char *cmd, char *data, char *buffer, size_t buflen)
{
	...
	if (strchr(data, 't')) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Invalid direction: %s\n", data);
	}
...
static int detect_write(struct ast_channel *chan, const char *cmd, char *data, const char *value)
{
	...
	if (!ast_strlen_zero(args.duration) && (ast_str_to_int(args.duration, &duration) || duration < 1)) {
		ast_log_chan(NULL, LOG_WARNING, "Invalid duration: %s\n", args.duration);
		return -1;
	}
	if (ast_test_flag(&flags, OPT_HITS_REQ) && !ast_strlen_zero(opt_args[OPT_ARG_HITS_REQ])) {
		if ((ast_str_to_int(opt_args[OPT_ARG_HITS_REQ], &hitsrequired) || hitsrequired < 1)) {
			ast_log_chan(NULL, LOG_WARNING, "Invalid number hits required: %s\n", opt_args[OPT_ARG_HITS_REQ]);
			return -1;
		}
	}
	if (ast_test_flag(&flags, OPT_DECIBEL) && !ast_strlen_zero(opt_args[OPT_ARG_DECIBEL])) {
		if ((ast_str_to_int(opt_args[OPT_ARG_DECIBEL], &db) || db < 1)) {
			ast_log_chan(NULL, LOG_WARNING, "Invalid decibel level: %s\n", opt_args[OPT_ARG_DECIBEL]);
			return -1;
		}
	}
...
static int wait_exec(struct ast_channel *chan, const char *data)
{
	...
	if (!ast_strlen_zero(args.timeout) && (sscanf(args.timeout, "%30lf", &timeoutf) != 1 || timeoutf < 0)) {
		ast_log_chan(NULL, LOG_WARNING, "Invalid timeout: %s\n", args.timeout);
		pbx_builtin_setvar_helper(chan, "WAITFORTONESTATUS", "ERROR");
		return -1;
	}
	...
	if (!ast_strlen_zero(args.times) && (ast_str_to_int(args.times, &times) || times < 1)) {
		ast_log_chan(NULL, LOG_WARNING, "Invalid number of times: %s\n", args.times);
		pbx_builtin_setvar_helper(chan, "WAITFORTONESTATUS", "ERROR");
		return -1;
	}
...
static int scan_exec(struct ast_channel *chan, const char *data)
{
	...
	if (!ast_strlen_zero(args.threshold) && (ast_str_to_int(args.threshold, &threshold) || threshold < 1)) {
		ast_log_chan(NULL, LOG_WARNING, "Invalid threshold: %s\n", args.threshold);
		pbx_builtin_setvar_helper(chan, "TONESCANSTATUS", "ERROR");
		return -1;
	}
	timeout = 1000 * timeoutf;
	...
	if (!ast_strlen_zero(args.zone)) {
		if (ast_dsp_set_call_progress_zone(dsp, args.zone)) {
			ast_log_chan(NULL, LOG_WARNING, "Invalid call progress zone: %s\n", args.zone);
			pbx_builtin_setvar_helper(chan, "TONESCANSTATUS", "ERROR");
			ast_dsp_free(dsp);
			return -1;
		}
...
AST_MODULE_INFO_STANDARD_EXTENDED(GABPBX_GPL_KEY, "Tone detection module");
