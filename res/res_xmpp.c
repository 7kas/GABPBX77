 ...
 *
 * A reference module for interfacting GABpbx directly as a client or component with
...
#include "gabpbx/config_options.h"
...
/*** DOCUMENTATION
	<application name="JabberSend" language="en_US" module="res_xmpp">
		...
		<description>
			...
			<replaceable>gabpbx</replaceable>, configured in xmpp.conf.</para>
			<example title="Send 'Hello world' to Bob">
			same => n,JabberSend(gabpbx,bob@domain.com,Hello world)
			</example>
		</description>
		<see-also>
	...
	<function name="JABBER_RECEIVE" language="en_US" module="res_xmpp">
		...
		<description>
			...
			the <replaceable>gabpbx</replaceable> XMPP account configured in xmpp.conf.</para>
			<example title="Receive a message">
			same => n,Set(msg=${JABBER_RECEIVE(gabpbx,bob@domain.com)})
			</example>
		</description>
		<see-also>
			<ref type="function" module="res_xmpp">JABBER_STATUS</ref>
	...
	<application name="JabberSendGroup" language="en_US" module="res_xmpp">
		...
		<syntax>
			<parameter name="Jabber" required="true">
				<para>Client or transport GABpbx uses to connect to Jabber.</para>
			</parameter>
			...
			<parameter name="Nickname" required="false">
				<para>The nickname GABpbx uses in the chat room.</para>
			</parameter>
		</syntax>
		<description>
	...
	<application name="JabberJoin" language="en_US" module="res_xmpp">
		...
		<syntax>
			...
			<parameter name="Nickname" required="false">
				<para>The nickname GABpbx will use in the chat room.</para>
		...
		<description>
			<para>Allows GABpbx to join a chat room.</para>
		</description>
	</application>
	<application name="JabberLeave" language="en_US" module="res_xmpp">
		...
		<description>
			<para>Allows GABpbx to leave a chat room.</para>
		</description>
	</application>
		...
		<configFile name="xmpp.conf">
			...
			<configObject name="client">
				...
				<configOption name="timeout">
					...
					than this value will be deleted by GABpbx. This option applies to incoming messages only
...
static struct ast_xmpp_client *xmpp_client_alloc(const char *name)
{
	...
	if (!client->buddies) {
		ast_log_chan(NULL, LOG_ERROR, "Could not initialize buddy container for '%s'\n", name);
		ao2_ref(client, -1);
		return NULL;
	}

	if (ast_string_field_init(client, 512)) {
		ast_log_chan(NULL, LOG_ERROR, "Could not initialize stringfields for '%s'\n", name);
		ao2_ref(client, -1);
		return NULL;
	}

	if (!(client->stack = iks_stack_new(8192, 8192))) {
		ast_log_chan(NULL, LOG_ERROR, "Could not create an Iksemel stack for '%s'\n", name);
		ao2_ref(client, -1);
		return NULL;
	}
...
static int xmpp_config_prelink(void *newitem)
{
	...
	if (ast_strlen_zero(clientcfg->user)) {
		ast_log_chan(NULL, LOG_ERROR, "No user specified on client '%s'\n", clientcfg->name);
		return -1;
	} else if (ast_strlen_zero(clientcfg->password) && ast_strlen_zero(clientcfg->refresh_token)) {
		ast_log_chan(NULL, LOG_ERROR, "No password or refresh_token specified on client '%s'\n", clientcfg->name);
		return -1;
	} else if (ast_strlen_zero(clientcfg->server)) {
		ast_log_chan(NULL, LOG_ERROR, "No server specified on client '%s'\n", clientcfg->name);
		return -1;
		...
		ast_log_chan(NULL, LOG_ERROR, "No oauth_clientid or oauth_secret specified, so client '%s' can't be used\n", clientcfg->name);
		return -1;
	}
...
static void xmpp_pubsub_create_affiliations(struct ast_xmpp_client *client, const char *node)
{
	...
	if (!modify_affiliates) {
		ast_log_chan(NULL, LOG_ERROR, "Could not create IQ for creating affiliations on client '%s'\n", client->name);
		return;
	}
	...
	const char *oldmsgs, const char *newmsgs)
{
	...
	iks_insert_attrib(mailbox_node, "xmlns", "http://gabpbx.org");
					     ...
					     const char *device_state, unsigned int cachable)
{
	...
	iks_insert_attrib(state, "xmlns", "http://gabpbx.org");
...
static void xmpp_pubsub_unsubscribe(struct ast_xmpp_client *client, const char *node)
{
	...
	if (!request) {
		ast_log_chan(NULL, LOG_ERROR, "Could not create IQ when creating pubsub unsubscription on client '%s'\n", client->name);
		return;
	}
...
static void xmpp_pubsub_subscribe(struct ast_xmpp_client *client, const char *node)
{
	...
	if (!cfg || !cfg->global || !request) {
		ast_log_chan(NULL, LOG_ERROR, "Could not create IQ when creating pubsub subscription on client '%s'\n", client->name);
		return;
	}
...
static int xmpp_pubsub_handle_event(void *data, ikspak *pak)
{
	...
	if (!item) {
		ast_log_chan(NULL, LOG_ERROR, "Could not parse incoming PubSub event\n");
...
static int xmpp_pubsub_handle_error(void *data, ikspak *pak)
{
	...
	if (!cfg || !cfg->global) {
		ast_log_chan(NULL, LOG_ERROR, "No global configuration available\n");
		return IKS_FILTER_EAT;
	}
	...
	if (error_num > 399 && error_num < 500 && error_num != 404) {
		...
		ast_log_chan(NULL, LOG_ERROR, "PubSub Server error, %s\n", error);
		return IKS_FILTER_EAT;
	}
	...
	if (!strcasecmp(iks_name(orig_request), "publish")) {
		...
		if ((request = xmpp_pubsub_iq_create(client, "set"))) {
			...
			ast_log_chan(NULL, LOG_ERROR, "PubSub publish could not create IQ\n");
		}
...
static int acf_jabberstatus_read(struct ast_channel *chan, const char *name, char *data, char *buf, size_t buflen)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_ERROR, "Usage: JABBER_STATUS(<sender>,<jid>[/<resource>])\n");
		return 0;
	}
	AST_STANDARD_APP_ARGS(args, data);

	if (args.argc != 2) {
		ast_log_chan(NULL, LOG_ERROR, "JABBER_STATUS requires 2 arguments: sender and jid.\n");
		return -1;
	}

	AST_NONSTANDARD_APP_ARGS(jid, args.jid, '/');
	if (jid.argc < 1 || jid.argc > 2) {
		ast_log_chan(NULL, LOG_WARNING, "Wrong JID %s, exiting\n", args.jid);
		return -1;
	}

	if (!cfg || !cfg->clients || !(clientcfg = xmpp_config_find(cfg->clients, args.sender))) {
		ast_log_chan(NULL, LOG_WARNING, "Could not find sender connection: '%s'\n", args.sender);
		return -1;
	}
...
static int xmpp_join_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_ERROR, "%s requires arguments (sender,jid[,nickname])\n", app_ajijoin);
		return -1;
	}
	s = ast_strdupa(data);
	...
	if (strchr(args.jid, '/')) {
		ast_log_chan(NULL, LOG_ERROR, "Invalid room name : resource must not be appended\n");
		return -1;
	}

	if (!cfg || !cfg->clients || !(clientcfg = xmpp_config_find(cfg->clients, args.sender))) {
		ast_log_chan(NULL, LOG_ERROR, "Could not find sender connection: '%s'\n", args.sender);
		return -1;
	}

	if (ast_strlen_zero(args.nick)) {
		if (ast_test_flag(&clientcfg->flags, XMPP_COMPONENT)) {
			snprintf(nick, sizeof(nick), "gabpbx");
		} else {
	...
	if (!ast_strlen_zero(args.jid) && strchr(args.jid, '@')) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Problem with specified jid of '%s'\n", args.jid);
	}
...
static int xmpp_leave_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(args.jid) || !strchr(args.jid, '@')) {
		ast_log_chan(NULL, LOG_ERROR, "No jabber ID specified\n");
		return -1;
	}
...
static int xmpp_send_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "%s requires arguments (account,jid,message)\n", app_ajisend);
		return -1;
	}
	s = ast_strdupa(data);
...
static int xmpp_sendgroup_exec(struct ast_channel *chan, const char *data)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_ERROR, "%s requires arguments (sender,groupchatid,message[,nickname])\n", app_ajisendgroup);
		return -1;
	}
	s = ast_strdupa(data);
...
static int acf_jabberreceive_read(struct ast_channel *chan, const char *name, char *data, char *buf, size_t buflen)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "%s requires arguments (account,jid[,timeout])\n", name);
		return -1;
	}
	...
	if (jid.argc < 1 || jid.argc > 2 || strlen(args.jid) > XMPP_MAX_JIDLEN) {
		ast_log_chan(NULL, LOG_WARNING, "Invalid JID : %s\n", parse);
		return -1;
	}
	...
	if (ast_strlen_zero(args.timeout)) {
		...
		if (timeout <= 0) {
			ast_log_chan(NULL, LOG_WARNING, "Invalid timeout specified: '%s'\n", args.timeout);
			return -1;
		}
	}
	...
	if (!cfg || !cfg->clients || !(clientcfg = xmpp_config_find(cfg->clients, args.account))) {
		ast_log_chan(NULL, LOG_WARNING, "Could not find client %s, exiting\n", args.account);
		return -1;
	}
	...
	if (chan && ast_autoservice_start(chan) < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Cannot start autoservice for channel %s\n", ast_channel_name(chan));
		return -1;
	}
		...
		AST_LIST_TRAVERSE_SAFE_BEGIN(&clientcfg->client->messages, message, list) {
			if (jid.argc == 1) {
				...
				if (!resource || strlen(resource) == 0) {
					ast_log_chan(NULL, LOG_WARNING, "Remote JID has no resource : %s\n", message->from);
					if (strncasecmp(jid.screenname, message->from, jidlen)) {
						continue;
					}
				} else {
	...
	if (chan && ast_autoservice_stop(chan) < 0) {
		ast_log_chan(NULL, LOG_WARNING, "Cannot stop autoservice for channel %s\n", ast_channel_name(chan));
	}

	/* return if we timed out */
	if (!found) {
		ast_log_chan(NULL, LOG_NOTICE, "Timed out : no message received from %s\n", args.jid);
		return -1;
	}
 ...
 * \param client GABpbx's XMPP client
...
static int xmpp_send_cb(const struct ast_msg *msg, const char *to, const char *from)
{
	...
	if (ast_strlen_zero(sender)) {
		ast_log_chan(NULL, LOG_ERROR, "MESSAGE(from) of '%s' invalid for XMPP\n", from);
		return -1;
	}

	if (!cfg || !cfg->clients || !(clientcfg = xmpp_config_find(cfg->clients, sender))) {
		ast_log_chan(NULL, LOG_WARNING, "Could not finder account to send from as '%s'\n", sender);
		return -1;
	}
	...
	if ((res = ast_xmpp_client_send_message(clientcfg->client, dest, ast_msg_get_body(msg))) != IKS_OK) {
		ast_log_chan(NULL, LOG_WARNING, "Failed to send XMPP message (%d).\n", res);
	}
...
static int xmpp_client_unsubscribe_user(struct ast_xmpp_client *client, const char *user)
{
	...
	if (!(iq = iks_new("iq")) || !(query = iks_new("query")) || !(item = iks_new("item"))) {
		ast_log_chan(NULL, LOG_WARNING, "Could not allocate memory for roster removal of '%s' from client '%s'\n",
			user, client->name);
		goto done;
	}
	...
	if (ast_xmpp_client_send(client, iq)) {
		ast_log_chan(NULL, LOG_WARNING, "Could not send roster removal request of '%s' from client '%s'\n",
			user, client->name);
	}
...
static int xmpp_client_subscribe_user(void *obj, void *arg, int flags)
{
	...
	if (ast_xmpp_client_send(client, iks_make_s10n(IKS_TYPE_SUBSCRIBE, buddy->id,
						       "Greetings! I am the GABpbx Open Source PBX and I want to subscribe to your presence\n"))) {
		ast_log_chan(NULL, LOG_WARNING, "Could not send subscription for '%s' on client '%s'\n",
		...
		if (!(buddy = ao2_find(client->buddies, iks_find_attrib(item, "jid"), OBJ_KEY))) {
			if (ast_test_flag(&clientcfg->flags, XMPP_AUTOPRUNE)) {
				...
				if (xmpp_client_unsubscribe_user(client, iks_find_attrib(item, "jid"))) {
					ast_log_chan(NULL, LOG_ERROR, "Could not unsubscribe user '%s' on client '%s'\n",
			...
			if (!(buddy = xmpp_client_create_buddy(client->buddies, iks_find_attrib(item, "jid")))) {
				ast_log_chan(NULL, LOG_ERROR, "Could not allocate buddy '%s' on client '%s'\n", iks_find_attrib(item, "jid"),
					client->name);
				continue;
			}
		}
...
static void xmpp_client_set_presence(struct ast_xmpp_client *client, const char *to, const char *from, int level, const char *desc)
{
	...
	if (!cfg || !cfg->clients || !(clientcfg = xmpp_config_find(cfg->clients, client->name)) ||
		...
		ast_log_chan(NULL, LOG_ERROR, "Unable to allocate stanzas for setting presence status for client '%s'\n", client->name);
		goto done;
	}
	...
	iks_insert_attrib(cnode, "node", "http://www.gabpbx.org/xmpp/client/caps");
	iks_insert_attrib(cnode, "ver", "gabpbx-xmpp");
...
static int xmpp_client_service_discovery_get_hook(void *data, ikspak *pak)
{
	...
	if (!(iq = iks_new("iq")) || !(query = iks_new("query")) || !(ident = iks_new("identity")) || !(disco = iks_new("feature")) ||
		...
		ast_log_chan(NULL, LOG_ERROR, "Could not allocate memory for responding to service discovery request from '%s' on client '%s'\n",
	...
	iks_insert_attrib(ident, "name", "gabpbx");
...
static int xmpp_connect_hook(void *data, ikspak *pak)
{
	...
	if (!(roster = iks_make_iq(IKS_TYPE_GET, IKS_NS_ROSTER))) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to allocate memory for roster request for client '%s'\n", client->name);
		return -1;
	}
...
#ifndef HAVE_OPENSSL
	ast_log_chan(NULL, LOG_ERROR, "TLS connection for client '%s' cannot be established. OpenSSL is not available.\n", client->name);
	return -1;
#else
	if (iks_send_raw(client->parser, "<starttls xmlns='urn:ietf:params:xml:ns:xmpp-tls'/>") == IKS_NET_TLSFAIL) {
		ast_log_chan(NULL, LOG_ERROR, "TLS connection for client '%s' cannot be started.\n", client->name);
		return -1;
	}
...
static int xmpp_client_requested_tls(struct ast_xmpp_client *client, struct ast_xmpp_client_config *cfg, int type, iks *node)
{
...
#ifndef HAVE_OPENSSL
	ast_log_chan(NULL, LOG_ERROR, "Somehow we managed to try to start TLS negotiation on client '%s' without OpenSSL support, disconnecting\n", client->name);
	return -1;
#else
	...
	if (xmpp_send_stream_header(client, cfg, client->jid->server) != IKS_OK) {
		ast_log_chan(NULL, LOG_ERROR, "TLS connection for client '%s' could not be established, failed to send stream header after negotiation\n",
			client->name);
		return -1;
	}
...
failure:
	...
	ast_log_chan(NULL, LOG_ERROR, "TLS connection for client '%s' cannot be established. "
...
static int xmpp_client_authenticate_digest(struct ast_xmpp_client *client, struct ast_xmpp_client_config *cfg, int type, iks *node)
{
	...
	if (!(iq = iks_new("iq")) || !(query = iks_insert(iq, "query"))) {
		ast_log_chan(NULL, LOG_ERROR, "Stanzas could not be allocated for authentication on client '%s'\n", client->name);
		iks_delete(iq);
		return -1;
	}
...
static int xmpp_client_authenticate_sasl(struct ast_xmpp_client *client, struct ast_xmpp_client_config *cfg, int type, iks *node)
{
	...
	if ((features & IKS_STREAM_SASL_MD5) && !xmpp_is_secure(client)) {
		if (iks_start_sasl(client->parser, IKS_SASL_DIGEST_MD5, (char*)client->jid->user, (char*)cfg->password) != IKS_OK) {
			ast_log_chan(NULL, LOG_ERROR, "Tried to authenticate client '%s' using SASL DIGEST-MD5 but could not\n", client->name);
			return -1;
		}
	...
	if (!(features & IKS_STREAM_SASL_PLAIN)) {
		ast_log_chan(NULL, LOG_ERROR, "Tried to authenticate client '%s' using SASL PLAIN but server does not support it\n", client->name);
		return -1;
	}

	if (!(auth = iks_new("auth"))) {
		ast_log_chan(NULL, LOG_ERROR, "Could not allocate memory for SASL PLAIN authentication for client '%s'\n", client->name);
		return -1;
	}
...
static int xmpp_client_authenticating(struct ast_xmpp_client *client, struct ast_xmpp_client_config *cfg, int type, iks *node)
{
	...
	if (features & IKS_STREAM_BIND) {
		...
		if (!(auth = iks_make_resource_bind(client->jid))) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to allocate memory for stream bind on client '%s'\n", client->name);
			return -1;
		}
	...
	if (features & IKS_STREAM_SESSION) {
		...
		if (!(auth = iks_make_session())) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to allocate memory for stream session on client '%s'\n", client->name);
			return -1;
		}
...
static int xmpp_component_authenticate(struct ast_xmpp_client *client, struct ast_xmpp_client_config *cfg, int type, iks *node)
{
	...
	if (xmpp_client_send_raw_message(client, message) != IKS_OK) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to send handshake for component '%s'\n", client->name);
		return -1;
	}
...
static int xmpp_component_service_discovery_get_hook(void *data, ikspak *pak)
{
	...
	if (!cfg || !cfg->clients || !(clientcfg = xmpp_config_find(cfg->clients, client->name)) ||
		...
		ast_log_chan(NULL, LOG_ERROR, "Failed to allocate stanzas for service discovery get response to '%s' on component '%s'\n",
	...
	if (!(node = iks_find_attrib(pak->query, "node"))) {
		...
		iks_insert_attrib(identity, "name", "GABpbx The Open Source PBX");
	...
	if (ast_xmpp_client_send(client, iq)) {
		ast_log_chan(NULL, LOG_WARNING, "Could not send response to service discovery request on component '%s'\n",
			client->name);
	}
...
static int xmpp_component_register_get_hook(void *data, ikspak *pak)
{
	...
	if (!cfg || !cfg->clients || !(clientcfg = xmpp_config_find(cfg->clients, client->name)) ||
		...
		ast_log_chan(NULL, LOG_ERROR, "Failed to allocate stanzas for register get response to '%s' on component '%s'\n",
			pak->from->partial, client->name);
		goto done;
	}
	...
	if (!(buddy = ao2_find(client->buddies, pak->from->partial, OBJ_KEY))) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Received register attempt from '%s' but buddy is not configured on component '%s'\n",
			pak->from->partial, client->name);
	} else if (!(node = iks_find_attrib(pak->query, "node"))) {
		iks_insert_cdata(instructions, "Welcome to GABpbx - the Open Source PBX.\n", 0);
		iks_insert_node(query, instructions);
		ao2_ref(buddy, -1);
	} else {
		ast_log_chan(NULL, LOG_WARNING, "Received register get to component '%s' using unsupported node '%s' from '%s'\n",
	...
	if (ast_xmpp_client_send(client, iq)) {
		ast_log_chan(NULL, LOG_WARNING, "Could not send response to '%s' for received register get on component '%s'\n",
			pak->from->partial, client->name);
	}
...
static int xmpp_component_register_set_hook(void *data, ikspak *pak)
{
	...
	if (!(iq = iks_new("iq")) || !(presence = iks_new("presence")) || !(x = iks_new("x"))) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to allocate stanzas for register set response to '%s' on component '%s'\n",
			pak->from->partial, client->name);
		goto done;
	}
	...
	if (ast_xmpp_client_send(client, iq)) {
		ast_log_chan(NULL, LOG_WARNING, "Could not send response to '%s' for received register set on component '%s'\n",
			pak->from->partial, client->name);
		goto done;
	}
	...
	if (ast_xmpp_client_send(client, presence)) {
		ast_log_chan(NULL, LOG_WARNING, "Could not send subscription to '%s' on component '%s'\n",
			pak->from->partial, client->name);
	}
...
static int xmpp_component_service_discovery_items_hook(void *data, ikspak *pak)
{
	...
	if (!cfg || !cfg->clients || !(clientcfg = xmpp_config_find(cfg->clients, client->name)) ||
		...
		ast_log_chan(NULL, LOG_ERROR, "Failed to allocate stanzas for service discovery items response to '%s' on component '%s'\n",
			pak->from->partial, client->name);
		goto done;
	}
	...
	if (!(node = iks_find_attrib(pak->query, "node"))) {
		...
		iks_insert_attrib(item, "name", "GABpbx Commands");
		iks_insert_attrib(item, "jid", clientcfg->user);
		...
		ast_log_chan(NULL, LOG_WARNING, "Received service discovery items request to component '%s' using unsupported node '%s' from '%s'\n",
			client->name, node, pak->from->partial);
		goto done;
	}

	if (ast_xmpp_client_send(client, iq)) {
		ast_log_chan(NULL, LOG_WARNING, "Could not send response to service discovery items request from '%s' on component '%s'\n",
			pak->from->partial, client->name);
	}
...
static int xmpp_component_authenticating(struct ast_xmpp_client *client, struct ast_xmpp_client_config *cfg, int type, iks *node)
{
	...
	if (strcmp(iks_name(node), "handshake")) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to authenticate component '%s'\n", client->name);
		return -1;
	}
...
static int xmpp_pak_presence(struct ast_xmpp_client *client, struct ast_xmpp_client_config *cfg, iks *node, ikspak *pak)
{
	...
	if (!(buddy = ao2_find(client->buddies, pak->from->partial, OBJ_KEY))) {
		...
		if (strcmp(client->jid->partial, pak->from->partial)) {
			ast_log_chan(NULL, LOG_WARNING, "Received presence information about '%s' despite not having them in roster on client '%s'\n",
				pak->from->partial, client->name);
		}
		return 0;
	}
	...
	if (!(resource = ao2_callback(buddy->resources, OBJ_NOLOCK, xmpp_resource_cmp, pak->from->resource))) {
		...
		if (status != STATUS_DISAPPEAR) {
			if (!(resource = ao2_alloc(sizeof(*resource), xmpp_resource_destructor))) {
				ast_log_chan(NULL, LOG_ERROR, "Could not allocate resource object for resource '%s' of buddy '%s' on client '%s'\n",
	...
	if (resource && (status != STATUS_DISAPPEAR)) {
		...
		if ((node && strcmp(resource->caps.node, node)) || (ver && strcmp(resource->caps.version, ver))) {
			...
			if (xmpp_client_send_disco_info_request(client, pak->from->full, client->jid->full)) {
				ast_log_chan(NULL, LOG_WARNING, "Could not send discovery information request to resource '%s' of buddy '%s' on client '%s', capabilities may be incomplete\n", resource->resource, buddy->id, client->name);
			}
		}
...
static int xmpp_pak_s10n(struct ast_xmpp_client *client, struct ast_xmpp_client_config *cfg,iks *node, ikspak *pak)
{
	...
	case IKS_TYPE_SUBSCRIBE:
		if (ast_test_flag(&cfg->flags, XMPP_AUTOREGISTER)) {
			...
			if ((presence = iks_new("presence")) && (status = iks_new("status"))) {
				...
				iks_insert_cdata(status, "GABpbx has approved your subscription", 0);
				iks_insert_node(presence, status);

				if (ast_xmpp_client_send(client, presence)) {
					ast_log_chan(NULL, LOG_ERROR, "Could not send subscription acceptance to '%s' from client '%s'\n",
						pak->from->partial, client->name);
				}
			} else {
				ast_log_chan(NULL, LOG_ERROR, "Could not allocate presence stanzas for accepting subscription from '%s' to client '%s'\n",
					pak->from->partial, client->name);
			}
	...
	case IKS_TYPE_SUBSCRIBED:
		...
		if (!buddy) {
			ast_log_chan(NULL, LOG_WARNING, "Could not find or create buddy '%s' on client '%s'\n",
				pak->from->partial, client->name);
		} else {
			ao2_ref(buddy, -1);
		}
...
static int xmpp_action_hook(void *data, int type, iks *node)
{
	...
	if (!node) {
		ast_log_chan(NULL, LOG_ERROR, "xmpp_action_hook was called without a packet\n");
		return IKS_HOOK;
	}
...
static int xmpp_client_reconnect(struct ast_xmpp_client *client)
{
	...
	if (!client->filter && !(client->filter = iks_filter_new())) {
		ast_log_chan(NULL, LOG_ERROR, "Could not create IKS filter for client connection '%s'\n", client->name);
		return -1;
	}
	...
	if (res == IKS_NET_NOCONN) {
		ast_log_chan(NULL, LOG_ERROR, "No XMPP connection available when trying to connect client '%s'\n", client->name);
		return -1;
	} else if (res == IKS_NET_NODNS) {
		ast_log_chan(NULL, LOG_ERROR, "No DNS available for XMPP connection when trying to connect client '%s'\n", client->name);
		return -1;
	}
...
static int xmpp_client_receive(struct ast_xmpp_client *client, unsigned int timeout)
{
	...
	while (1) {
		...
		switch (ret) {
		case IKS_NOMEM:
			ast_log_chan(NULL, LOG_WARNING, "Parsing failure: Out of memory.\n");
			break;
		case IKS_BADXML:
			ast_log_chan(NULL, LOG_WARNING, "Parsing failure: Invalid XML.\n");
			break;
		case IKS_HOOK:
			ast_log_chan(NULL, LOG_WARNING, "Parsing failure: Hook returned an error.\n");
			break;
		}
		if (ret != IKS_OK) {
			return ret;
...
static void *xmpp_client_thread(void *data)
{
	...
	do {
		...
		if (res == IKS_HOOK) {
			...
			ast_log_chan(NULL, LOG_ERROR, "[%s] TLS failure\n", client->name);
			...
			if (res == IKS_OK) {
				...
				ast_log_chan(NULL, LOG_WARNING, "[%s] Network timeout\n", client->name);
			}
		} else if (res == IKS_NET_RWERR) {
			ast_log_chan(NULL, LOG_WARNING, "[%s] Socket read error\n", client->name);
			ast_xmpp_client_disconnect(client);
			sleep_with_backoff(&sleep_time);
		} else if (res == IKS_NET_NOSOCK) {
			ast_log_chan(NULL, LOG_WARNING, "[%s] No socket\n", client->name);
		} else if (res == IKS_NET_NOCONN) {
			ast_log_chan(NULL, LOG_WARNING, "[%s] No connection\n", client->name);
		} else if (res == IKS_NET_NODNS) {
			ast_log_chan(NULL, LOG_WARNING, "[%s] No DNS\n", client->name);
		} else if (res == IKS_NET_NOTSUPP) {
			ast_log_chan(NULL, LOG_WARNING, "[%s] Not supported\n", client->name);
		} else if (res == IKS_NET_DROPPED) {
			ast_log_chan(NULL, LOG_WARNING, "[%s] Dropped?\n", client->name);
...
static int fetch_access_token(struct ast_xmpp_client_config *cfg)
{
	...
	if (ast_func_read(NULL, cmd, cBuf, sizeof(cBuf) - 1)) {
		ast_log_chan(NULL, LOG_ERROR, "CURL is unavailable. This is required for OAuth 2.0 authentication of XMPP client '%s'. Please ensure it is loaded.\n",
			cfg->name);
		return -1;
	}
	...
	ast_log_chan(NULL, LOG_ERROR, "An error occurred while performing OAuth 2.0 authentication for client '%s': %s\n", cfg->name, cBuf);
...
static int xmpp_client_config_post_apply(void *obj, void *arg, int flags)
{
	...
	if (cfg->client->reconnect) {
		...
		if (!(cfg->client->parser = iks_stream_new(ast_test_flag(&cfg->flags, XMPP_COMPONENT) ? "jabber:component:accept" : "jabber:client", cfg->client,
							   xmpp_action_hook))) {
			ast_log_chan(NULL, LOG_ERROR, "Iksemel stream could not be created for client '%s' - client not active\n", cfg->name);
			return -1;
		}
		...
		if (!strchr(cfg->user, '/') && !ast_test_flag(&cfg->flags, XMPP_COMPONENT)) {
			char resource[strlen(cfg->user) + strlen("/gabpbx-xmpp") + 1];

			snprintf(resource, sizeof(resource), "%s/gabpbx-xmpp", cfg->user);
		...
		if (!cfg->client->jid || (ast_strlen_zero(cfg->client->jid->user) && !ast_test_flag(&cfg->flags, XMPP_COMPONENT))) {
			ast_log_chan(NULL, LOG_ERROR, "Jabber identity '%s' could not be created for client '%s' - client not active\n", cfg->user, cfg->name);
			return -1;
		}
...
static void xmpp_pubsub_request_nodes(struct ast_xmpp_client *client, const char *collection)
{
	...
	if (!request) {
		ast_log_chan(NULL, LOG_ERROR, "Could not request pubsub nodes on client '%s' - IQ could not be created\n", client->name);
		return;
	}
...
static int xmpp_pubsub_delete_node_list(void *data, ikspak* pak)
{
	...
	if (iks_has_children(pak->query)) {
		item = iks_first_tag(pak->query);
		ast_log_chan(NULL, LOG_WARNING, "Connection: %s  Node name: %s\n", client->jid->partial,
			iks_find_attrib(item, "node"));
		while ((item = iks_next_tag(item))) {
...
static int load_module(void)
{
	...
	if (ast_eid_is_empty(&ast_eid_default)) {
		ast_log_chan(NULL, LOG_WARNING, "Entity ID is not set. The distributing device state or MWI will not work.\n");
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_GLOBAL_SYMBOLS | AST_MODFLAG_LOAD_ORDER, "GABpbx XMPP Interface",
