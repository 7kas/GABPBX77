...
/*! \file
 *
 * \brief SNMP Agent / SubAgent support for GABpbx
 *
...
/*
 * There is some collision collision between netsmp and gabpbx names,
...
#include "gabpbx/paths.h"	/* need ast_config_AST_SOCKET */
...
/* Collision between Net-SNMP and GABpbx */
...
/* Forward declaration */
static void init_gabpbx_mib(void);

/*
 * Anchor for all the GABpbx MIB values
 */
static oid gabpbx_oid[] = { 1, 3, 6, 1, 4, 1, 22736, 1 };

/*
 * MIB values -- these correspond to values in the GABpbx MIB,
...
{
	ast_verb_chan(NULL, 2, "Starting %sAgent\n", res_snmp_agentx_subagent ? "Sub" : "");
	...
	snmp_shutdown("gabpbx");

	ast_verb_chan(NULL, 2, "Terminating %sAgent\n", res_snmp_agentx_subagent ? "Sub" : "");
...
static int term_gabpbx_mib(int majorID, int minorID, void *serverarg, void *clientarg)
{
	unregister_sysORTable(gabpbx_oid, OID_LENGTH(gabpbx_oid));
	return 0;
}

static void init_gabpbx_mib(void)
{
	static struct variable4 gabpbx_vars[] = {
	...
	register_sysORTable(gabpbx_oid, OID_LENGTH(gabpbx_oid),
			"GABPBX-MIB implementation for GABpbx.");

	REGISTER_MIB("res_snmp", gabpbx_vars, variable4, gabpbx_oid);

	snmp_register_callback(SNMP_CALLBACK_LIBRARY,
			   SNMP_CALLBACK_SHUTDOWN,
			   term_gabpbx_mib, NULL);
}
