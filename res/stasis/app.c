...
static void app_dtor(void *obj)
{
	...
	ast_verb_chan(NULL, 1, "Destroying Stasis app %s\n", app->name);
...
struct stasis_app *app_create(const char *name, stasis_app_cb handler, void *data, enum stasis_app_subscription_model subscription_model)
{
	...
	ast_verb_chan(NULL, 1, "Creating Stasis app '%s'\n", name);
	...
	if (!ast_context_find(context_name)) {
		if (!ast_context_find_or_create(NULL, NULL, context_name, "res_stasis")) {
			ast_log_chan(NULL, LOG_WARNING, "Could not create context '%s' for Stasis application '%s'\n", context_name, name);
		} else {
		...
		ast_log_chan(NULL, LOG_WARNING, "Not creating context '%s' for Stasis application '%s' because it already exists\n",
			context_name, name);
	}
...
void app_send(struct stasis_app *app, struct ast_json *message)
{
	...
	if (ast_json_object_set(message, "gabpbx_id", ast_json_string_create(
			ast_eid_to_str(eid, sizeof(eid), &ast_eid_default)))) {
		ast_log_chan(NULL, AST_LOG_WARNING, "Failed to append EID to outgoing event %s\n",
...
void app_deactivate(struct stasis_app *app)
{
	...
	ast_verb_chan(NULL, 1, "Deactivating Stasis app '%s'\n", app->name);
	app->handler = NULL;
	ao2_cleanup(app->data);
	app->data = NULL;
...
void app_update(struct stasis_app *app, stasis_app_cb handler, void *data)
{
	...
	if (app->handler && app->data) {
		...
		ast_verb_chan(NULL, 1, "Replacing Stasis app '%s'\n", app->name);
		...
		ast_verb_chan(NULL, 1, "Activating Stasis app '%s'\n", app->name);
	}
...
struct ast_json *app_to_json(const struct stasis_app *app)
{
	...
	while ((forwards = ao2_iterator_next(&i))) {
		...
		if (append_res != 0) {
			ast_log_chan(NULL, LOG_ERROR, "Error building response\n");
...
static int unsubscribe(struct stasis_app *app, const char *kind, const char *id, int terminate)
{
	...
	if (!id) {
		if (!strcmp(kind, "bridge")) {
			...
			ast_log_chan(NULL, LOG_WARNING, "Unknown subscription kind '%s'\n", kind);
			return -1;
		}
	}
	...
	struct ast_json *filter, const char *filter_type)
{
	...
	if (filter && ast_json_typeof(filter) != AST_JSON_ARRAY) {
		ast_log_chan(NULL, LOG_ERROR, "Invalid json type event filter - app: %s, filter: %s\n",
	...
	if (filter) {
		...
		for (i = 0; i < ast_json_array_size(filter) &&
			...
			if (ast_strlen_zero(ast_json_object_string_get(obj, "type"))) {
				ast_log_chan(NULL, LOG_ERROR, "Filter event must have a type - app: %s, "
						"filter: %s\n",	app->name, filter_type);
				return -1;
			}
		}
