...
struct stasis_app_control *control_create(struct ast_channel *channel, struct stasis_app *app)
{
	...
	if (res != 0) {
		ast_log_chan(NULL, LOG_ERROR, "Error initializing ast_cond_t: %s\n",
			strerror(errno));
		ao2_ref(control, -1);
		return NULL;
	}
	...
	struct ast_channel *chan, void *data)
{
	...
	if (!control->next_app) {
		ast_log_chan(NULL, LOG_ERROR, "Allocation failed for next app\n");
		return -1;
	}
	...
	if (move_data->app_args) {
		...
		while ((token = strtok_r(move_data->app_args, ",", &move_data->app_args))) {
			...
			if (!(arg = ast_strdup(token))) {
				ast_log_chan(NULL, LOG_ERROR, "Allocation failed for next app arg\n");
				control_move_cleanup(control);
				return -1;
			}
			...
			if (res) {
				ast_log_chan(NULL, LOG_ERROR, "Failed to append arg to next app args\n");
				ast_free(arg);
				control_move_cleanup(control);
				return -1;
			}
	...
	struct ast_channel *chan, void *data)
{
	...
	if (!res) {
		ast_log_chan(NULL, LOG_NOTICE, "Unsupported transfer requested on channel '%s'\n",
...
int control_swap_channel_in_bridge(struct stasis_app_control *control, struct ast_bridge *bridge, struct ast_channel *chan, struct ast_channel *swap)
{
	...
	if (res != 0) {
		ast_log_chan(NULL, LOG_ERROR, "Error setting after-bridge callback\n");
		return -1;
	}
	...
	if (res != 0) {
		 ...
		 */
		ast_log_chan(NULL, LOG_ERROR, "Error adding channel to bridge\n");
		ao2_lock(control);
	...
	struct ast_channel *chan, void *data)
{
	...
	if (bridge != stasis_app_get_bridge(control)) {
		ast_log_chan(NULL, LOG_WARNING, "%s: Not in bridge %s; not removing\n",
			stasis_app_control_get_channel_id(control),
			bridge->uniqueid);
		return -1;
	}
...
void control_wait(struct stasis_app_control *control)
{
	...
	while (ao2_container_count(control->command_queue) == 0) {
		...
		if (res < 0) {
			ast_log_chan(NULL, LOG_ERROR, "Error waiting on command queue\n");
			break;
		}
	}
	ao2_unlock(control->command_queue);
