...
if [ -z $GABPBX_VERSION ]; then
	GABPBX_VERSION=$(./build_tools/make_version .)
fi
...
genhtml --prefix ${PWD} --ignore-errors source ${LCOV_DIR}/filtered.info \
	--legend --title "GABpbx ${GABPBX_VERSION}" --output-directory=${COVERAGE_DIR} > /dev/null
