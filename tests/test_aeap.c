...
static int handle_msg(struct ast_aeap *aeap, struct ast_aeap_message *message, void *data)
{
	...
	*passed = !strcmp(ast_aeap_message_id(message), AEAP_MESSAGE_ID) &&
	...
	if (!*passed) {
		ast_log_chan(NULL, LOG_ERROR, "Name '%s' did not equal '%s' for message '%s'",
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_DEFAULT, "GABpbx External Application Protocol Object Tests",
