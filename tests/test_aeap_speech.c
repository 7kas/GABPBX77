...
static int speech_test_server_handle_request(struct ast_websocket *ws, const void *buf, uint64_t size)
{
	...
	if (!req) {
		ast_log_chan(NULL, LOG_ERROR, "speech test handle request: unable to load json\n");
		return -1;
	}

	name = ast_json_object_string_get(req, "request");
	if (!name) {
		ast_log_chan(NULL, LOG_ERROR, "speech test handle request: no name\n");
		ast_json_unref(req);
		return -1;
	}
	...
	if (!resp) {
		ast_log_chan(NULL, LOG_ERROR, "speech test handle request: unable to create response '%s'\n", name);
		ast_json_unref(req);
		return -1;
	}
	...
	if (!strcmp(name, "setup")) {
		...
		ast_log_chan(NULL, LOG_ERROR, "speech test handle request: unsupported request '%s'\n", name);
		return -1;
	}

	if (res) {
		ast_log_chan(NULL, LOG_ERROR, "speech test handle request: unable to build response '%s'\n", name);
		ast_json_unref(resp);
		ast_json_unref(req);
		return -1;
	}
	...
	if (!resp_buf) {
		ast_log_chan(NULL, LOG_ERROR, "speech test handle request: unable to dump response '%s'\n", name);
		ast_json_unref(req);
		return -1;
	}
	...
	if (res) {
		ast_log_chan(NULL, LOG_ERROR, "speech test handle request: unable to write response '%s'\n", name);
	}
	...
	struct ast_variable *headers)
{
	...
	while ((res = ast_websocket_wait_for_input(ws, -1)) > 0) {
		...
		if (ast_websocket_read(ws, &payload, &payload_len, &opcode, &fragmented)) {
			ast_log_chan(NULL, LOG_ERROR, "speech test: Read failure in server loop\n");
			break;
		}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_DEFAULT, "GABpbx External Application Protocol Speech test(s)",
