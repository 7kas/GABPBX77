...
AST_TEST_DEFINE(get_docs)
{
	...
	headers = ast_variable_new("Host", "stasis.gabpbx.org", __FILE__);
	...
	ast_test_validate(test, 0 == strcmp("http://stasis.gabpbx.org/ari", basePath));
...
AST_TEST_DEFINE(get_docs_hackerz)
{
	...
	response = response_alloc();
	ast_ari_get_docs("../../../../sbin/gabpbx", "", headers, response);
	ast_test_validate(test, 404 == response->response_code);
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_DEFAULT, "ARI testing",
