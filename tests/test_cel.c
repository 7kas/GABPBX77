...
/*! \brief A placeholder for GABpbx's 'real' CEL configuration */
...
AST_TEST_DEFINE(test_cel_unanswered_inbound_call)
{
	...
	case TEST_INIT:
		...
		info->description =
			...
			"inbound to GABpbx, executes some dialplan, but\n"
			"is never answered.";
		return AST_TEST_NOT_RUN;
	case TEST_EXECUTE:
		break;
...
AST_TEST_DEFINE(test_cel_unanswered_outbound_call)
{
	...
	case TEST_INIT:
		...
		info->description =
			"Test CEL records for a call that is\n"
			"outbound to GABpbx but is never answered.";
		return AST_TEST_NOT_RUN;
	case TEST_EXECUTE:
		break;
	}
...
AST_TEST_DEFINE(test_cel_dial_pickup)
{
	...
	case TEST_INIT:
		...
		info->description =
			"Test CEL records for a call that is\n"
			"inbound to GABpbx, executes some dialplan, and\n"
			"is picked up.";
		return AST_TEST_NOT_RUN;
	case TEST_EXECUTE:
		break;
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "CEL unit tests");
