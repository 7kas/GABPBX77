...
static int test_config_validity(struct ast_config *cfg)
{
	...
	for (i = 0; i < ARRAY_LEN(categories); ++i) {
		...
		if (strcmp(cat_iter, categories[i].category)) {
			ast_log_chan(NULL, LOG_ERROR, "Category name mismatch, %s does not match %s\n", cat_iter, categories[i].category);
			return -1;
		}
		for (j = 0; j < ARRAY_LEN(categories[i].vars); ++j) {
			...
			if (strcmp(var->name, categories[i].vars[j].name)) {
				ast_log_chan(NULL, LOG_ERROR, "Variable name mismatch, %s does not match %s\n", var->name, categories[i].vars[j].name);
				return -1;
			}
			if (strcmp(var->value, categories[i].vars[j].val)) {
				ast_log_chan(NULL, LOG_ERROR, "Variable value mismatch, %s does not match %s\n", var->value, categories[i].vars[j].val);
				return -1;
			}
		}
	}
...
AST_TEST_DEFINE(config_hook)
{
	 ...
	 */
	hook_run = 0;
	cfg = ast_config_load("gabpbx.conf", config_flags);
	ast_config_destroy(cfg);
	if (hook_run) {
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Config test module");
