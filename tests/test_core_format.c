...
static int load_module(void)
{
	...
	if (ast_codec_register(&test_core_format_codec)) {
		ast_log_chan(NULL, AST_LOG_ERROR, "Failed to register test_core_format_codec\n");
		return AST_MODULE_LOAD_DECLINE;
	}

	if (ast_format_interface_register("test_core_format_codec", &test_core_format_attr)) {
		ast_log_chan(NULL, AST_LOG_ERROR, "Failed to register format interface for test_core_format_codec\n");
		return AST_MODULE_LOAD_DECLINE;
	}
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Core format API test module");
