...
AST_TEST_DEFINE(resolver_set_result)
{
	...
	for (i = 0; i < ARRAY_LEN(results); ++i) {
		if (ast_dns_resolver_set_result(&some_query, results[i].secure, results[i].bogus,
				results[i].rcode, "gabpbx.org", DNS_ANSWER, DNS_ANSWER_SIZE)) {
...
AST_TEST_DEFINE(resolver_set_result_off_nominal)
{
	...
	if (!ast_dns_resolver_set_result(&some_query, 1, 1, NOERROR, "gabpbx.org",
...
AST_TEST_DEFINE(resolver_add_record)
{
	...
	if (ast_dns_resolver_set_result(&some_query, 0, 0, NOERROR, "gabpbx.org",
				DNS_ANSWER, DNS_ANSWER_SIZE)) {
...
static void *resolution_thread(void *dns_query)
{
	...
	ast_dns_resolver_set_result(query, 0, 0, NOERROR, "gabpbx.org", DNS_ANSWER, DNS_ANSWER_SIZE);
...
AST_TEST_DEFINE(resolver_resolve_sync)
{
	...
	if (ast_dns_resolve("gabpbx.org", T_A, C_IN, &result)) {
...
AST_TEST_DEFINE(resolver_resolve_sync_off_nominal)
{
	...
	struct dns_resolve_data {
		...
		{ "gabpbx.org", -1,        C_IN,      &result },
		{ "gabpbx.org", 65536 + 1, C_IN,      &result },
		{ "gabpbx.org", T_A,       -1,        &result },
		{ "gabpbx.org", T_A,       65536 + 1, &result },
		{ "gabpbx.org", T_A,       C_IN,      NULL },
	};
...
AST_TEST_DEFINE(resolver_resolve_async)
{
	...
	active = ast_dns_resolve_async("gabpbx.org", T_A, C_IN, async_callback, async_data);
	if (!active) {
...
AST_TEST_DEFINE(resolver_resolve_async_off_nominal)
{
	...
	struct dns_resolve_data {
		...
		{ "gabpbx.org", -1,        C_IN,      stub_callback },
		{ "gabpbx.org", 65536 + 1, C_IN,      stub_callback },
		{ "gabpbx.org", T_A,       -1,        stub_callback },
		{ "gabpbx.org", T_A,       65536 + 1, stub_callback },
		{ "gabpbx.org", T_A,       C_IN,      NULL },
	};
	...
	active = ast_dns_resolve_async("gabpbx.org", T_A, C_IN, stub_callback, NULL);
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "DNS API Tests");
