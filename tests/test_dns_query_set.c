...
static enum ast_test_result_state query_set_test(struct ast_test *test, int resolve, int cancel)
{
	...
	for (idx = 0; idx < total; ++idx) {
		if (ast_dns_query_set_add(query_set, "gabpbx.org", T_A, C_IN)) {
	...
	for (idx = 0; idx < ast_dns_query_set_num_queries(query_set); ++idx) {
		...
		if (strcmp(ast_dns_query_get_name(query), "gabpbx.org")) {
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "DNS query set tests");
