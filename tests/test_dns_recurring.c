...
AST_TEST_DEFINE(recurring_query)
{
	...
	recurring_query = ast_dns_resolve_recurring("gabpbx.org", T_A, C_IN, async_callback, rdata);
	if (!recurring_query) {
...
AST_TEST_DEFINE(recurring_query_off_nominal)
{
	...
	recurring = ast_dns_resolve_recurring("gabpbx.org", T_A, C_IN, stub_callback, NULL);
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Recurring DNS query tests");
