...
static int http_callback(struct ast_tcptls_session_instance *ser, const struct ast_http_uri *urih, const char *uri, enum ast_http_method method, struct ast_variable *get_params, struct ast_variable *headers)
{
	...
	if (send_file) {
		...
		if (fd == -1) {
			ast_log_chan(NULL, LOG_ERROR, "Unable to open temp file for testing: %s (%d)", strerror(errno), errno);
			goto error;
		}

		memset(buf, 1, sizeof(buf));
		if (write(fd, buf, sizeof(buf)) != sizeof(buf)) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to write expected number of bytes to pipe\n");
			close(fd);
			goto error;
		}
		close(fd);
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_DEFAULT, "HTTP Media Cache Backend Tests",
