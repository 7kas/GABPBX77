...
static int num_to_mailbox(char *mailbox, size_t size, size_t num)
{
	if (snprintf(mailbox, 10, MAILBOX_PREFIX "%zu", num) == -1) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to convert mailbox to string\n");
		return -1;
	}
...
static int mailbox_to_num(const char *mailbox, size_t *num)
{
	...
	if (!p) {
		ast_log_chan(NULL, LOG_ERROR, "Prefix separator '~' not found in '%s'\n", mailbox);
		return -1;
	}

	if (ast_str_to_umax(++p, &tmp)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to convert mailbox '%s' to numeric\n", mailbox);
		return -1;
	}
	*num = (size_t) tmp;
...
static int validate_data(struct ast_mwi_state *mwi_state)
{
	...
	if (mwi_state->urgent_msgs != val || mwi_state->new_msgs != val ||
		...
		ast_log_chan(NULL, LOG_ERROR, "Unexpected MWI state data for '%s', %d != %zu\n",
...
static int subscriptions_destroy(struct subscriptions *subs)
{
	...
	if (running_total != sum_total) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to destroy all MWI subscriptions: running=%zu, sum=%zu\n",
...
static int subscriptions_create(struct subscriptions *subs)
{
	...
	for (i = 0; i < MAILBOX_COUNT; ++i) {
		...
		if (!sub) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to create a MWI subscriber for mailbox '%s'\n", mailbox);
			break;
		}

		if (AST_VECTOR_APPEND(subs, sub)) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to add to MWI sub to vector for mailbox '%s'\n", mailbox);
			ao2_ref(sub, -1);
			break;
		}
	...
	if (i != MAILBOX_COUNT || running_total != sum_total) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to create all MWI subscriptions: running=%zu, sum=%zu\n",
				running_total, sum_total);
...
static int publishers_create(struct publishers *pubs)
{
	...
	for (i = 0; i < MAILBOX_COUNT; ++i) {
		...
		if (!pub) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to create an MWI publisher for mailbox '%s'\n", mailbox);
			break;
		}

		if (AST_VECTOR_APPEND(pubs, pub)) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to add to an MWI publisher to vector for mailbox '%s'\n", mailbox);
			ao2_ref(pub, -1);
			break;
		}
	}
	...
	if (i != MAILBOX_COUNT) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to create all MWI publishers: count=%zu\n", i);
...
static int explicit_publish_cb(struct ast_mwi_state *mwi_state, void *data)
{
	...
	if (!pub) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to locate MWI publisher for mailbox '%s'\n", mwi_state->uniqueid);
		return CMP_STOP;
	}
...
static int publish(on_mwi_state cb, void *user_data)
{
	...
	if (running_total != sum_total) {
		ast_log_chan(NULL, LOG_ERROR, "Failed MWI state callback (1): running=%zu, sum=%zu\n",
				running_total, sum_total);
		return -1;
	}
	...
	if (running_total != sum_total) {
		ast_log_chan(NULL, LOG_ERROR, "Failed MWI state callback (2): running=%zu, sum=%zu\n",
				running_total, sum_total);
		return -1;
	}
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "MWI testing");
