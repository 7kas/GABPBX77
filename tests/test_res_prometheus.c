...
AST_TEST_DEFINE(config_general_core_metrics)
{
	...
	ast_test_validate(test, strstr(ast_str_buffer(buffer), "gabpbx_core_properties") != NULL);

	ast_test_status_update(test, " -> Checking for uptime\n");
	ast_test_validate(test, strstr(ast_str_buffer(buffer), "gabpbx_core_uptime_seconds") != NULL);

	ast_test_status_update(test, " -> Checking for last reload\n");
	ast_test_validate(test, strstr(ast_str_buffer(buffer), "gabpbx_core_last_reload_seconds") != NULL);

	ast_test_status_update(test, " -> Checking for scrape time\n");
	ast_test_validate(test, strstr(ast_str_buffer(buffer), "gabpbx_core_scrape_time_ms") != NULL);
...
AST_TEST_DEFINE(bridge_to_string)
{
	...
	ast_test_validate(test, strstr(ast_str_buffer(response), "gabpbx_bridges_channels_count{") != NULL);
	ast_test_validate(test, match_count(ast_str_buffer(response), "# HELP gabpbx_bridges_channels_count Number of channels in the bridge.") == 1);
	ast_free(response);
	return AST_TEST_PASS;
}
...
static int process_config(int reload)
{
	...
	if (!config || config == CONFIG_STATUS_FILEINVALID) {
		ast_log_chan(NULL, AST_LOG_NOTICE, "HTTP config file is invalid; declining load");
		return -1;
	...
	if (!enabled || ast_false(enabled)) {
		...
		ast_log_chan(NULL, AST_LOG_NOTICE, "HTTP server is disabled; declining load");
		return -1;
	}
	...
	if (!bindaddr) {
		ast_config_destroy(config);
		ast_log_chan(NULL, AST_LOG_NOTICE, "HTTP config file fails to specify 'bindaddr'; declining load");
		return -1;
	}
...
AST_MODULE_INFO(GABPBX_GPL_KEY, AST_MODFLAG_DEFAULT, "Prometheus Core Unit Tests",
	.load = load_module,
