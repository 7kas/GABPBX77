	...
	enum test_type type)
{
	...
	*instance1 = ast_rtp_instance_new("gabpbx", test_sched, &addr1, "instance1");
	*instance2 = ast_rtp_instance_new("gabpbx", test_sched, &addr2, "instance2");
 ...
 * Its purpose is only to aid in code development in res_rtp_gabpbx.
 */
...
AST_TEST_DEFINE(nack_no_packet_loss)
{
	...
	if ((test_init_rtp_instances(&instance1, &instance2, test_sched, TEST_TYPE_NACK)) < 0) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to initialize test!\n");
...
AST_TEST_DEFINE(fir_nominal)
{
	...
	/*
	 * This will not directly write a frame out, but cause GABpbx to see it as a FIR
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "RTP/RTCP test module");
