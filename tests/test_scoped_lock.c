...
AST_TEST_DEFINE(lock_test)
{
	...
	indicator = 0;
	{
		...
		if (indicator != 1) {
			ast_log_chan(NULL, LOG_ERROR, "The lock was not acquired via RAII");
			res = AST_TEST_FAIL;
		}
	}
	if (indicator != 0) {
		ast_log_chan(NULL, LOG_ERROR, "The lock was not released when the variable went out of scope");
		res = AST_TEST_FAIL;
	}
...
AST_TEST_DEFINE(cleanup_order)
{
...

	{
		...
		if (!object->reffed || !object->locked) {
			ast_log_chan(NULL, LOG_ERROR, "Test failed due to out of order initializations");
			res = AST_TEST_FAIL;
		}
	}

	if (object->reffed || object->locked) {
		ast_log_chan(NULL, LOG_ERROR, "Test failed due to out of order cleanups\n");
		res = AST_TEST_FAIL;
	}
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "SCOPED_LOCK test module");
