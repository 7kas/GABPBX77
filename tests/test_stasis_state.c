...
static int validate_data(const char *id, struct foo_data *foo)
{
	...
	if (ast_str_to_umax(id, &tmp)) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to convert the state's id '%s' to numeric\n", id);
		return -1;
	}
	num = (size_t) tmp;
	...
	if (!foo) {
		...
		ast_log_chan(NULL, LOG_ERROR, "Expected state data for '%s'\n", id);
		return -1;
	}

	if (expect_null) {
		ast_log_chan(NULL, LOG_ERROR, "Expected NULL state data for '%s'\n", id);
		return -1;
	}

	if (foo->bar != num) {
		ast_log_chan(NULL, LOG_ERROR, "Unexpected state data for '%s'\n", id);
		return -1;
	}
...
static int subscriptions_destroy(struct stasis_state_manager *manager, struct subscriptions *subs)
{
	...
	if (running_total != sum_total) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to destroy all subscriptions: running=%zu, sum=%zu\n",
	...
	struct subscriptions *subs)
{
	...
	for (i = 0; i < TOPIC_COUNT; ++i) {
		...
		if (snprintf(id, 10, "%zu", i) == -1) {
			ast_log_chan(NULL, LOG_ERROR, "Unable to convert subscriber id to string\n");
			break;
		}

		sub = stasis_state_subscribe_pool(manager, id, foo_type_cb, NULL);
		if (!sub) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to create a state subscriber for id '%s'\n", id);
			ao2_ref(sub, -1);
			break;
		}

		if (AST_VECTOR_APPEND(subs, sub)) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to add to foo_sub to vector for id '%s'\n", id);
			ao2_ref(sub, -1);
			break;
		}
	...
	if (i != TOPIC_COUNT || running_total != sum_total) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to create all subscriptions: running=%zu, sum=%zu\n",
				running_total, sum_total);
...
static int publishers_destroy(struct stasis_state_manager *manager, struct publishers *pubs)
{
	...
	for (i = 0; i < TOPIC_COUNT; ++i) {
		...
		if (snprintf(id, 10, "%zu", i) == -1) {
			ast_log_chan(NULL, LOG_ERROR, "Unable to convert publisher id to string\n");
			return -1;
		}
	...
	struct publishers *pubs)
{
	...
	for (i = 0; i < TOPIC_COUNT; ++i) {
		...
		if (!pub) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to create a state publisher for id '%s'\n", id);
			break;
		}

		if (AST_VECTOR_APPEND(pubs, pub)) {
			ast_log_chan(NULL, LOG_ERROR, "Failed to add to publisher to vector for id '%s'\n", id);
			ao2_ref(pub, -1);
			break;
		}
	}
	...
	if (i != TOPIC_COUNT) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to create all publishers: count=%zu\n", i);
...
static struct stasis_message *create_foo_type_message(const char *id)
{
	...
	if (!foo) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to allocate foo data for '%s'\n", id);
		return NULL;
	}
	...
	if (!msg) {
		ast_log_chan(NULL, LOG_ERROR, "Failed to create stasis message for '%s'\n", id);
	}
...
static int explicit_publish_cb(const char *id, struct stasis_message *msg, void *user_data)
{
	...
	if (!pub) {
		ast_log_chan(NULL, LOG_ERROR, "Unable to locate publisher for id '%s'\n", id);
		return CMP_STOP;
	}
	...
	void *user_data)
{
	...
	if (running_total != sum_total) {
		ast_log_chan(NULL, LOG_ERROR, "Failed manager_callback (1): running=%zu, sum=%zu\n",
				running_total, sum_total);
		return -1;
	}
	...
	if (running_total != sum_total) {
		ast_log_chan(NULL, LOG_ERROR, "Failed manager_callback (2): running=%zu, sum=%zu\n",
				running_total, sum_total);
		return -1;
	}
...
cleanup:
	...
	if (ao2_ref(manager, 0) != 1) {
		ast_log_chan(NULL, LOG_ERROR, "Memory leak - Too many references held on manager\n");
		return AST_TEST_FAIL;
	}
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Stasis state testing");
