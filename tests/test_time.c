...
AST_TEST_DEFINE(test_timezone_watch)
{
	...
	for (type = 0; type <
		...
		for (i = 0; i < ARRAY_LEN(zones); i++) {
			...
			if ((system_res = ast_safe_system(syscmd))) {
				ast_log_chan(NULL, LOG_WARNING, "system(%s) returned non-zero: %d\n", syscmd, system_res);
			}
	...
	if (ast_safe_system(syscmd)) {
		ast_log_chan(NULL, LOG_WARNING, "system(%s) returned non-zero.\n", syscmd);
	}
...
AST_MODULE_INFO_STANDARD(GABPBX_GPL_KEY, "Time Tests");
