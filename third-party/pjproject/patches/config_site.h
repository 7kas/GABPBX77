/*
 * GABpbx config_site.h
 */

#include <sys/select.h>

/*
 * Since both pjproject and gabpbx source files will include config_site.h,
 * we need to make sure that only pjproject source files include gabpbx_malloc_debug.h.
 */
#if !defined(_GABPBX_ASTMM_H)
#include "gabpbx_malloc_debug.h"
#endif

/*
 * Defining PJMEDIA_HAS_SRTP to 0 does NOT disable GABpbx's ability to use srtp.
 * It only disables the pjmedia srtp transport which GABpbx doesn't use.
 * The reason for the disable is that while GABpbx works fine with older libsrtp
