 ...
 * locked up (in fact, what has happened is that GABpbx runs at
 ...
 * GABpbx.  Should that happen, that monitoring thread may take
 * immediate action to slow down GABpbx to regular priority,
 * thus allowing an administrator to login to the system and
 * restart GABpbx or perhaps take another course of action
 ...
 * answer is, in GABpbx, nothing.  Process scope threads are the
 * alternative, but they aren't supported in Linux.
 */

static const char explanation[] =
"This file is created when GABpbx is run with a realtime priority (-p).  It\n"
"must continue to exist, and the astcanary process must be allowed to continue\n"
"running, or else the GABpbx process will, within a short period of time,\n"
"slow itself down to regular priority.\n\n"
"The technical explanation for this file is to provide an assurance to GABpbx\n"
"that there are no threads that have gone into runaway mode, thus hogging the\n"
"CPU, and making the GABpbx machine seem to be unresponsive.  When that\n"
"happens, the astcanary process will be unable to update the timestamp on this\n"
"file, and GABpbx will notice within 120 seconds and react.  Slowing the\n"
"GABpbx process down to regular priority will permit an administrator to\n"
...
int main(int argc, char *argv[])
{
	...
	for (parent = atoi(argv[2]); parent == getppid() ;) {
		/* Update the modification times (checked from GABpbx) */
