...
static int db_open_sqlite3(const char *dbname)
{
	if (sqlite3_open(dbname, &sql3db) != SQLITE_OK) {
		fprintf(stderr, "Unable to open GABpbx database '%s': %s\n", dbname, sqlite3_errmsg(sql3db));
...
int main(int argc, char *argv[])
{
	...
	if (argc != 2) {
		...
		fprintf(stderr, "and will produce a file 'astdb' in the current directory\n"
				...
				"the new astdb to its location: often /var/lib/gabpbx/astdb\n");
		exit(-1);
	}
	...
	printf("astdb file to the original's location. Often /var/lib/gabpbx/astdb.\n");
