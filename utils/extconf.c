 ...
 * for operations outside of gabpbx. A huge, awful hack.
 *
 */

/*!
 * \li \ref extconf.c uses the configuration file \ref extconfig.conf and \ref extensions.conf and \ref gabpbx.conf
...
#include "gabpbx/paths.h"	/* we use AST_CONFIG_DIR */
...
static void ast_log_chan(NULL, int level, const char *file, int line, const char *function, const char *fmt, ...) __attribute__((format(printf, 5, 6)));
...
#include "gabpbx/inline_api.h"
#include "gabpbx/endian.h"
#include "gabpbx/ast_expr.h"
#include "gabpbx/extconf.h"
...
/* lock.h */
#define _GABPBX_LOCK_H /* A small indication that this is horribly wrong. */
...
/* GABpbx REQUIRES recursive (not error checking) mutexes
...
#define log_mutex_error(canlog, ...)  do { if (canlog) ast_log_chan(NULL, LOG_ERROR, __VA_ARGS__); else fprintf(stderr, __VA_ARGS__); } while (0)
...
int ast_safe_system(const char *s)
{
	...
	if (pid == 0) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Fork failed: %s\n", strerror(errno));
		res = -1;
	}
...
static struct ast_config_include *ast_include_new(struct ast_config *conf, const char *from_file, const char *included_file, int is_exec, const char *exec_file, int from_lineno, char *real_included_file_name, int real_included_file_name_size)
{
	...
	if (inc)
	{
		...
		ast_log_chan(NULL, LOG_WARNING,"'%s', line %d:  Same File included more than once! This data will be saved in %s if saved back to disk.\n", from_file, from_lineno, real_included_file_name);
	} else
...
enum ast_option_flags {
	...
	/*! Execute an gabpbx CLI command upon startup */
...
/*! These are the options that set by default when GABpbx starts */
...
static void ast_log_chan(NULL, int level, const char *file, int line, const char *function, const char *fmt, ...)
{
	va_list vars;
	va_start(vars,fmt);
...
static struct timeval tvfix(struct timeval a)
{
	if (a.tv_usec >= ONE_MILLION) {
		ast_log_chan(NULL, LOG_WARNING, "warning too large timestamp %ld.%ld\n",
			(long)a.tv_sec, (long int) a.tv_usec);
		a.tv_sec += a.tv_usec / ONE_MILLION;
		a.tv_usec %= ONE_MILLION;
	} else if (a.tv_usec < 0) {
		ast_log_chan(NULL, LOG_WARNING, "warning negative timestamp %ld.%ld\n",
			(long)a.tv_sec, (long int) a.tv_usec);
		a.tv_usec = 0;
	}
	return a;
...
static int ast_func_write(struct ast_channel *chan, const char *function, const char *value)
{
	...
	/* ast_log_chan(NULL, LOG_ERROR, "Function %s not registered\n", function); we are not interested in the details here */
...
static int pbx_builtin_setvar(struct ast_channel *chan, const void *data)
{
	...
	if (ast_strlen_zero(data)) {
		ast_log_chan(NULL, LOG_WARNING, "Set requires at least one variable name/value pair.\n");
		return 0;
	}
	...
	for (x = 0; x < argc; x++) {
		...
		if ((value = strchr(name, '='))) {
			*value++ = '\0';
			...
			ast_log_chan(NULL, LOG_WARNING, "Ignoring entry '%s' with no = (and not last 'options' entry)\n", name);
	}
...
static unsigned get_range(char *src, int max, char *const names[], const char *msg)
{
	...
	while ((part = strsep(&src, "&"))) {
		...
		if ((start = lookup_name(part, names, max)) < 0) {
			ast_log_chan(NULL, LOG_WARNING, "Invalid %s '%s', skipping element\n", msg, part);
			continue;
		}
		if (endpart) { /* find end of range */
			if ((end = lookup_name(endpart, names, max)) < 0) {
				ast_log_chan(NULL, LOG_WARNING, "Invalid end %s '%s', skipping element\n", msg, endpart);
				continue;
			}
		} else {
			end = start;
	...
	while ((part = strsep(&times, "&"))) {
		if (!(endpart = strchr(part, '-'))) {
			if (sscanf(part, "%2d:%2d", &st_h, &st_m) != 2 || st_h < 0 || st_h > 23 || st_m < 0 || st_m > 59) {
				ast_log_chan(NULL, LOG_WARNING, "%s isn't a valid time.\n", part);
				continue;
			}
		...
		*endpart++ = '\0';
		...
		if (!*endpart) {
			ast_log_chan(NULL, LOG_WARNING, "Invalid time range starting with '%s-'.\n", part);
			continue;
		}
		if (sscanf(part, "%2d:%2d", &st_h, &st_m) != 2 || st_h < 0 || st_h > 23 || st_m < 0 || st_m > 59) {
			ast_log_chan(NULL, LOG_WARNING, "'%s' isn't a valid start time.\n", part);
			continue;
		}
		if (sscanf(endpart, "%2d:%2d", &endh, &endm) != 2 || endh < 0 || endh > 23 || endm < 0 || endm > 59) {
			ast_log_chan(NULL, LOG_WARNING, "'%s' isn't a valid end time.\n", endpart);
			continue;
		}
...
static struct ast_config_engine *find_engine(const char *family, char *database, int dbsiz, char *table, int tabsiz)
{
	...
	if (map && !ret)
		ast_log_chan(NULL, LOG_WARNING, "Realtime mapping for '%s' found to engine '%s', but the engine is not available\n", map->name, map->driver);
...
static struct ast_config *ast_config_internal_load(const char *filename, struct ast_config *cfg, int withcomments, const char *suggested_incl_file)
{
	...
	if (cfg->include_level == cfg->max_include_level) {
		ast_log_chan(NULL, LOG_WARNING, "Maximum Include level (%d) exceeded\n", cfg->max_include_level);
		return NULL;
	}

	cfg->include_level++;
	/*  silence is golden!
		ast_log_chan(NULL, LOG_WARNING, "internal loading file %s level=%d\n", filename, cfg->include_level);
	*/

	if (strcmp(filename, extconfig_conf) && strcmp(filename, "gabpbx.conf") && config_engine_list) {
	...
	/* silence is golden
	   ast_log_chan(NULL, LOG_WARNING, "finished internal loading file %s level=%d\n", filename, cfg->include_level);
	*/
...
static int process_text_line(struct ast_config *cfg, struct ast_category **cat, char *buf, int lineno, const char *configfile, int withcomments, const char *suggested_include_file)
{
	...
	if (cur[0] == '[') {
		...
		c = strchr(cur, ']');
		if (!c) {
			ast_log_chan(NULL, LOG_WARNING, "parse error: no closing ']', line %d of %s\n", lineno, configfile);
			return -1;
		}
		*c++ = '\0';
		cur++;
				...
				if (!strcasecmp(cur, "!")) {
					...
					*cat = category_get(cfg, catname, 1);
					if (!*cat) {
						...
						ast_log_chan(NULL, LOG_WARNING, "Category addition requested, but category '%s' does not exist, line %d of %s\n", catname, lineno, configfile);
						return -1;
					}
					if (newcat) {
					...
					if (!base) {
						ast_log_chan(NULL, LOG_WARNING, "Inheritance requested, but category '%s' does not exist, line %d of %s\n", cur, lineno, configfile);
						return -1;
					}
		...
		if (do_exec && !ast_opt_exec_includes) {
			ast_log_chan(NULL, LOG_WARNING, "Cannot perform #exec unless execincludes option is enabled in gabpbx.conf (options section)!\n");
			do_exec = 0;
		}
		if (do_include || do_exec) {
			if (c) {
				...
				if (do_exec) {
					...
					if (snprintf(cmd, sizeof(cmd), "%s > %s 2>&1", cur, exec_file) >= sizeof(cmd)) {
						ast_log_chan(NULL, LOG_ERROR, "Failed to construct command string to execute %s.\n", cur);
					...
					exec_file[0] = '\0';
				/* A #include */
				/* ast_log_chan(NULL, LOG_WARNING, "Reading in included file %s withcomments=%d\n", cur, withcomments); */
				...
				if(!do_include)
					return 0;
				/* ast_log_chan(NULL, LOG_WARNING, "Done reading in included file %s withcomments=%d\n", cur, withcomments); */

			} else {
				ast_log_chan(NULL, LOG_WARNING, "Directive '#%s' needs an argument (%s) at line %d of %s\n",
			...
			ast_log_chan(NULL, LOG_WARNING, "Unknown directive '%s' at line %d of %s\n", cur, lineno, configfile);
	} else {
		...
		if (c) {
			*c = 0;
			...
			ast_log_chan(NULL, LOG_WARNING, "EXTENSIONS.CONF: No '=' (equal sign) in line %d of %s\n", lineno, configfile);
		}
	}
	return 0;
}
...
static struct ast_config *config_text_file_load(const char *database, const char *table, const char *filename, struct ast_config *cfg, int withcomments, const char *suggested_include_file)
{
	...
	do {
		...
		if (!S_ISREG(statbuf.st_mode)) {
			ast_log_chan(NULL, LOG_WARNING, "'%s' is not a regular file, ignoring\n", fn);
			continue;
		}
		if (option_verbose > 1) {
		...
		if (!(f = fopen(fn, "r"))) {
			if (option_debug)
				ast_log_chan(NULL, LOG_DEBUG, "No file to parse: %s\n", fn);
			if (option_verbose > 1)
		...
		count++;
		if (option_debug)
			ast_log_chan(NULL, LOG_DEBUG, "Parsing %s\n", fn);
		if (option_verbose > 1)
			ast_verbose("Found\n");
		while(!feof(f)) {
			lineno++;
					...
					if ((comment_p > new_buf) && (*(comment_p-1) == '\\')) {
						...
						if (comment < MAX_NESTED_COMMENTS) {
							*comment_p = '\0';
							...
							ast_log_chan(NULL, LOG_ERROR, "Maximum nest limit of %d reached.\n", MAX_NESTED_COMMENTS);
						}
	...
	if (comment) {
		ast_log_chan(NULL, LOG_WARNING,"Unterminated comment detected beginning on line %d\n", nest[comment]);
	}
...
int localized_config_text_file_save(const char *configfile, const struct ast_config *cfg, const char *generator)
{
	...
	if ((f = fopen(fn, "w"))) {
		...
		if (option_debug)
			ast_log_chan(NULL, LOG_DEBUG, "Unable to open for writing: %s\n", fn);
		if (option_verbose > 1)
...
static int ast_add_hint(struct ast_exten *e)
{
	...
	AST_RWLIST_TRAVERSE(&hints, hint, list) {
		if (hint->exten == e) {
			if (option_debug > 1)
				ast_log_chan(NULL, LOG_DEBUG, "HINTS: Not re-adding existing hint %s: %s\n", ast_get_extension_name(e), ast_get_extension_app(e));
			return -1;
		}
	}

	if (option_debug > 1)
		ast_log_chan(NULL, LOG_DEBUG, "HINTS: Adding hint %s: %s\n", ast_get_extension_name(e), ast_get_extension_app(e));
	...
	struct ast_exten *el, struct ast_exten *e, int replace)
{
	...
	if (e->priority == tmp->priority) {
		...
		if (!replace) {
			ast_log_chan(NULL, LOG_WARNING, "Unable to register extension '%s', priority %d in '%s', already in use\n", tmp->exten, tmp->priority, con->name);
			tmp->datad(tmp->data);
			free(tmp);
			return -1;
		}
											...
											enum ext_match_t action)
{
	...
	if (q->stacklen == 0) {
		...
		ast_log_chan(NULL, LOG_WARNING, "Maximum PBX stack exceeded\n");
		return NULL;
	}
	...
	AST_LIST_TRAVERSE(&tmp->alts, sw, list) {
		...
		if (!asw) {
			ast_log_chan(NULL, LOG_WARNING, "No such switch '%s'\n", sw->name);
			continue;
		}
...
static struct ast_context *__ast_context_create(struct ast_context **extcontexts, const char *name, const char *registrar, int existsokay)
{
	...
	for (tmp = *loc_contexts; tmp; tmp = tmp->next) {
		if (!strcasecmp(tmp->name, name)) {
			if (!existsokay) {
				ast_log_chan(NULL, LOG_WARNING, "Tried to register context '%s', already in use\n", name);
				tmp = NULL;
			}
			if (!extcontexts)
				ast_unlock_contexts();
	...
	if ((tmp = ast_calloc(1, length))) {
		...
		*loc_contexts = tmp;
		if (option_debug)
			ast_log_chan(NULL, LOG_DEBUG, "Registered context '%s'\n", tmp->name);
		if (option_verbose > 2)
	...
	const char *registrar)
{
	...
	*/
	...
	if (option_debug) {
		if (tmp->matchcid) {
			ast_log_chan(NULL, LOG_DEBUG, "Added extension '%s' priority %d (CID match '%s') to %s\n",
				tmp->exten, tmp->priority, tmp->cidmatch, con->name);
		} else {
			ast_log_chan(NULL, LOG_DEBUG, "Added extension '%s' priority %d to %s\n",
				tmp->exten, tmp->priority, con->name);
		}
	}
	if (option_verbose > 2) {
								...
								const char *label, const char *callerid, enum ext_match_t action)
{
	...
	if (e) {
		...
		if (matching_action)
			...
			if (!q.swo->exec) {
				ast_log_chan(NULL, LOG_WARNING, "No execution engine for switch %s\n", q.swo->name);
				res = -1;
			}
		...
		case STATUS_NO_CONTEXT:
			if (!matching_action)
				ast_log_chan(NULL, LOG_NOTICE, "Cannot find extension context '%s'\n", context);
			break;
		case STATUS_NO_EXTENSION:
			if (!matching_action)
				ast_log_chan(NULL, LOG_NOTICE, "Cannot find extension '%s' in context '%s'\n", exten, context);
			break;
		case STATUS_NO_PRIORITY:
			if (!matching_action)
				ast_log_chan(NULL, LOG_NOTICE, "No such priority %d in extension '%s' in context '%s'\n", priority, exten, context);
			break;
		case STATUS_NO_LABEL:
			if (context)
				ast_log_chan(NULL, LOG_NOTICE, "No such label '%s' in extension '%s' in context '%s'\n", label, exten, context);
			break;
		default:
			if (option_debug)
				ast_log_chan(NULL, LOG_DEBUG, "Shouldn't happen!\n");
		}
...
/*! \brief  Support for GABpbx built-in variables in the dialplan
\note	See also
	...
	*cp2 = 0; /* just in case there's nothing to do */
	...
	while (!ast_strlen_zero(whereweare) && count) {
		...
		if (nextvar) {
			...
			if (brackets)
				ast_log_chan(NULL, LOG_NOTICE, "Error in extension logic (missing '}' in '%s')\n", cp1);
			len = vare - vars - 1;
			...
			if (isfunction) {
				...
				if (option_debug)
					ast_log_chan(NULL, LOG_DEBUG, "Function result is '%s'\n", cp4 ? cp4 : "(null)");
			} else {
			...
			if (brackets)
				ast_log_chan(NULL, LOG_NOTICE, "Error in extension logic (missing ']')\n");
			len = vare - vars - 1;
			...
			if (length) {
				if (option_debug)
					ast_log_chan(NULL, LOG_DEBUG, "Expression result is '%s'\n", cp2);
				count -= length;
				cp2 += length;
				*cp2 = 0;
			}
		...
		for (v = ast_variable_browse(cfg, cxt); v; v = v->next) {
			if (!strcasecmp(v->name, "exten")) {
				...
				if (tc) {
						...
						*label++ = '\0';
						...
						if (end)
							*end = '\0';
							...
							ast_log_chan(NULL, LOG_WARNING, "Label missing trailing ')' at line %d\n", v->lineno);
					}
					plus = strchr(pri, '+');
					if (plus)
						*plus++ = '\0';
					if (!strcmp(pri,"hint"))
						...
						if (lastpri > -2)
							...
							ast_log_chan(NULL, LOG_WARNING, "Can't use 'next' priority on the first entry!\n");
					} else if (!strcmp(pri, "same") || !strcmp(pri, "s")) {
						if (lastpri > -2)
							ipri = lastpri;
						else
							ast_log_chan(NULL, LOG_WARNING, "Can't use 'same' priority on the first entry!\n");
					} else if (sscanf(pri, "%30d", &ipri) != 1 &&
					    (ipri = ast_findlabel_extension2(NULL, con, realext, pri, cidmatch)) < 1) {
						ast_log_chan(NULL, LOG_WARNING, "Invalid priority/label '%s' at line %d\n", pri, v->lineno);
						ipri = 0;
					}
					appl = S_OR(stringp, "");
					...
					if (firstc && (!firstp || firstc < firstp)) {
						...
						if ((end = strrchr(data, ')'))) {
							*end = '\0';
							...
							ast_log_chan(NULL, LOG_WARNING, "No closing parenthesis found? '%s(%s'\n", appl, data);
						}
					...
					if (ipri) {
						...
						if (!ast_opt_dont_warn && !strcmp(realext, "_."))
							ast_log_chan(NULL, LOG_WARNING, "The use of '_.' for an extension is strongly discouraged and can have unexpected behavior.  Please use '_X.' instead at line %d\n", v->lineno);
						if (ast_add_extension2(con, 0, realext, ipri, label, cidmatch, appl, strdup(data), ast_free_ptr, global_registrar)) {
							ast_log_chan(NULL, LOG_WARNING, "Unable to register extension at line %d\n", v->lineno);
						}
					}
					free(tc);
				}
				...
				if (ast_context_add_include2(con, realvalue, global_registrar))
					ast_log_chan(NULL, LOG_WARNING, "Unable to include context '%s' in context '%s'\n", v->value, cxt);
			} else if (!strcasecmp(v->name, "ignorepat")) {
				memset(realvalue, 0, sizeof(realvalue));
				pbx_substitute_variables_helper(NULL, v->value, realvalue, sizeof(realvalue) - 1);
				if (ast_context_add_ignorepat2(con, realvalue, global_registrar))
					ast_log_chan(NULL, LOG_WARNING, "Unable to include ignorepat '%s' in context '%s'\n", v->value, cxt);
				...
				if (ast_context_add_switch2(con, appl, data, !strcasecmp(v->name, "eswitch"), global_registrar))
					ast_log_chan(NULL, LOG_WARNING, "Unable to include switch '%s' in context '%s'\n", v->value, cxt);
			} else {
				ast_log_chan(NULL, LOG_WARNING, "==!!== Unknown directive: %s at line %d -- IGNORING!!!\n", v->name, v->lineno);
			}
		}
	}
	ast_config_destroy(cfg);
...
static void __ast_context_destroy(struct ast_context *con, const char *registrar)
{
	...
	for (tmp = contexts; tmp; ) {
		...
		for (; tmp; tmpl = tmp, tmp = tmp->next) {
			if (option_debug)
				ast_log_chan(NULL, LOG_DEBUG, "check ctx %s %s\n", tmp->name, tmp->registrar);
		...
		if (option_debug)
			ast_log_chan(NULL, LOG_DEBUG, "delete ctx %s %s\n", tmp->name, tmp->registrar);
		next = tmp->next;
		if (tmpl)
			tmpl->next = next;
		else
...
static void ast_merge_contexts_and_delete(struct ast_context **extcontexts, const char *registrar)
{
	...
	*/
	...
	if (registrar) {
		...
		if (option_debug)
			ast_log_chan(NULL, LOG_DEBUG, "must remove any reg %s\n", registrar);
		...
		while (tmp) {
			ast_log_chan(NULL, LOG_WARNING, "must remove %s  reg %s\n", tmp->name, tmp->registrar);
	...
	if (lasttmp) {
		...
		*extcontexts = NULL;
		...
		ast_log_chan(NULL, LOG_WARNING, "Requested contexts didn't get merged\n");
	...
	while ( (inc = ast_walk_context_includes(con, inc)) )
		if (!ast_context_find(inc->rname)) {
			...
			if (strcasecmp(inc->rname,"parkedcalls")!=0)
				ast_log_chan(NULL, LOG_WARNING, "Context '%s' tries to include the nonexistent context '%s'\n",
